...
export class VehicleSocketDataProxy extends BaseSceneSocketDataProxy {
  ...
  protected proxyStart() {
    ServerDataManager.listen(
      S2CProtocol.U_C_VEHICLE_CAMPAIGN_LIST_RSP,
      this,
      this.__onDataHandler,
    ); //初始化玩家列表
    ServerDataManager.listen(
      S2CProtocol.U_C_VEHICLE_JOIN,
      this,
      this.__onDataHandler,
    ); //玩家新加入副本
    ServerDataManager.listen(
      S2CProtocol.U_C_VEHICLE_EXIT,
      this,
      this.__onDataHandler,
    ); //玩家离开副本
    ServerDataManager.listen(
      S2CProtocol.U_C_VEHICLE_MOVE,
      this,
      this.__onDataHandler,
    ); //同步玩家移动
    ServerDataManager.listen(
      S2CProtocol.U_C_VEHICLE_SYNC_BUFF,
      this,
      this.__onDataHandler,
    ); //同步buff
    ServerDataManager.listen(
      S2CProtocol.U_C_VEHICLE_SYNC_SKILL,
      this,
      this.__onDataHandler,
    ); //同步技能信息
    ServerDataManager.listen(
      S2CProtocol.U_C_VEHICLE_ATTACK_RSP,
      this,
      this.__onDataHandler,
    ); //同步玩家攻击信息
    ServerDataManager.listen(
    ...
    ServerDataManager.listen(
      S2CProtocol.U_C_VEHICLE_NPC_NODE,
      this,
      this.__onDataHandler,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_VEHICLE_DETAILE,
      this,
      this.__onDataHandler,
    );
    ServerDataManager.listen(
    ...
    ServerDataManager.listen(
      S2CProtocol.U_C_VEHICLE_END,
      this,
      this.__onDataHandler,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_VEHICLE_BOSS_ATTACK_TIP,
      this,
      this.__onDataHandler,
    );
  }
