...
import { EmOuterCityWarCastlePeriodType, EmOuterCityWarHeroType, EmOuterCityWarPlayerState } from "../../../constant/OuterCityWarDefine";
import { OuterCityManager } from "../../../manager/OuterCityManager";
import { OuterCityEvent, OuterCityWarEvent } from "../../../constant/event/NotificationEvent";
...
export class OuterCityWarManager {
    ...
    private addEvent() {
        /** 接收处理外城所有城堡的下发信息 */
        ServerDataManager.listen(S2CProtocol.U_C_OUTCITYWAR_NODE_STATE, this, this.onCastleInfo);
        /** 只接收处理当前所在城堡的下发信息 */
        ServerDataManager.listen(S2CProtocol.U_C_OUTCITYWAR_BUILD_STATE, this, this.onBuildInfo);
        ServerDataManager.listen(S2CProtocol.U_C_OUT_CITY_WAR_GUILD_INFO_UPDATE, this, this.onGuildInfoUpdate);
        ServerDataManager.listen(S2CProtocol.U_C_OUT_CITY_WAR_GUILD_NOTICE, this, this.onGuildNoticeUpdate);
        ServerDataManager.listen(S2CProtocol.U_C_OUT_CITY_WAR_SCORE_INFO, this, this.onNpcScore);
        ServerDataManager.listen(S2CProtocol.U_C_OUTCITYWAR_PLAYER, this, this.onPlayerInfo);

        //
    }

    private removeEvent() {
        ServerDataManager.cancel(S2CProtocol.U_C_OUTCITYWAR_NODE_STATE, this, this.onCastleInfo);
        ServerDataManager.cancel(S2CProtocol.U_C_OUTCITYWAR_BUILD_STATE, this, this.onBuildInfo);
        ServerDataManager.cancel(S2CProtocol.U_C_OUT_CITY_WAR_GUILD_INFO_UPDATE, this, this.onGuildInfoUpdate);
        ServerDataManager.cancel(S2CProtocol.U_C_OUT_CITY_WAR_GUILD_NOTICE, this, this.onGuildNoticeUpdate);
        ServerDataManager.cancel(S2CProtocol.U_C_OUT_CITY_WAR_SCORE_INFO, this, this.onNpcScore);
        ServerDataManager.cancel(S2CProtocol.U_C_OUTCITYWAR_PLAYER, this, this.onPlayerInfo);
    }
    ...
    private onCastleInfo(pkg: PackageIn) {
        ...
        Logger.outcityWar("返回: 城堡节点：" + lastState + "->" + curState + "-" + castleMsg.leftTime, castleMsg)

        if (castleMsg.hasOwnProperty("state")) {
            let state1 = this._model.castleState == EmOuterCityWarCastlePeriodType.Fighting && castleMsg.state == EmOuterCityWarCastlePeriodType.Peace
            let state2 = this._model.castleState == EmOuterCityWarCastlePeriodType.Fighting && castleMsg.state == EmOuterCityWarCastlePeriodType.Protected
    ...
    private onPlayerInfo(pkg: PackageIn) {
        ...
        Logger.outcityWar("返回: 城堡节点玩家  当前进度:" + this.recvPlayerInfoCnt + "/" + allNum, msg)
        for (let index = 0; index < msg.playerStatusMsg.length; index++) {
            ...
            // 自己的信息
            if (playInfo.userId == this._model.playerInfo.userId && playInfo.heroType == EmOuterCityWarHeroType.Hero) {
                ...
                NotificationManager.Instance.dispatchEvent(OuterCityWarEvent.ACTION_POINT)
            }
        }

        if (this.recvPlayerInfoCnt == allNum) {
            Logger.outcityWar("返回: 城堡节点玩家 接收完成", this._model.playerListMap)
            this.recvPlayerInfoCnt = 0;
            this.calculateBuildInfo()
            NotificationManager.Instance.dispatchEvent(OuterCityWarEvent.ALL_BUILD_INFO)
        }
    }
    ...
    public calculateBuildInfo() {
        ...
        for (const buildInfo of this._model.buildingInfoMap.values()) {
            ...
            for (const pInfo of this._model.playerListMap.values()) {
                if (!this._model.checkInCurCastleJoinBattleList(pInfo.userId, pInfo.heroType)) {
                    continue
                }
                ...
                if (buildInfo.isPetBuild) {
                    if (pInfo.defenseSitePet == buildInfo.sonType && pInfo.orderIdPet > 0) {
         ...
         */
        for (let index = 0; index < this._model.buildingSontypeList.length; index++) {
            ...
            for (let i = 0; i < OuterCityWarModel.BuildSiteCount; i++) {
                ...
                let state = buildInfo.isPetBuild ? sitePlayerInfo.statePet : sitePlayerInfo.state
        ...
        for (let index = 0; index < this._model.buildingSontypeList.length; index++) {
            ...
            ocuppyStr += "\n" + buildInfo.buildName + ": " + buildInfo.preOneCompleteOccupyByAttacker + "-" + buildInfo.preOneCompleteOccupyByDefencer + ";"
        }
    ...
    private onGuildNoticeUpdate(pkg: PackageIn) {
        ...
        if (msg.hasOwnProperty("noticeMsg")) {
            this._model.curCastleNoticeStr = msg.noticeMsg;
            NotificationManager.Instance.dispatchEvent(OuterCityWarEvent.GUILD_NOTICE)
        }
    }
    ...
    private onNpcScore(pkg: PackageIn) {
        ...
        NotificationManager.Instance.dispatchEvent(OuterCityWarEvent.DEFENCE_NPC_SCORE)
    }
    ...
    private onBuildInfo(pkg: PackageIn) {
        ...
        if (buildInfo) {
            ...
            NotificationManager.Instance.dispatchEvent(OuterCityWarEvent.BUILD_INFO, buildInfo)
        }
    }
    ...
    sendOccupyBuildSite(sonType: number, orderId: number) {
        ...
        Logger.outcityWar("请求: 占领 城堡-建筑-驻防点", castleNodeId, sonType, orderId)
    ...
    sendGiveUpBuildSite(sonType: number, orderId: number) {
        ...
        Logger.outcityWar("请求: 放弃 城堡-建筑-驻防点", castleNodeId, sonType, orderId)
        SocketManager.Instance.send(C2SProtocol.C_OUT_CITY_WAR_GIVE_UP_CASTLE, msg);
    }
    ...
    sendJoinWar(userId: number, attend: BooleanType) {
        ...
        SocketManager.Instance.send(C2SProtocol.C_OUT_CITY_WAR_ARRANGE_GARRISON, msg);
    }
    ...
    sendBuyActionPoint() {
        ...
        SocketManager.Instance.send(C2SProtocol.C_OUT_CITY_WAR_BUY_ACTION_POINT, msg);
    }
    ...
    sendEditNotice(str: string) {
        ...
        SocketManager.Instance.send(C2SProtocol.C_OUT_CITY_WAR_EDIT_GUILD_NOTICE, msg);
    }
    ...
    */
    ...
    sendSelfPlayInfo() {
        ...
        let msg: NodeStateReqMsg = new NodeStateReqMsg();
        msg.nodeId = castleNodeId;
        SocketManager.Instance.send(C2SProtocol.C_OUT_CITY_WAR_ONE_PLAYER_STATUS, msg);
    }

    static createPlayInfoWithMsg(msg: PlayerStatusMsg, playerInfo: OuterCityWarPlayerInfo = null): OuterCityWarPlayerInfo {
        if (!playerInfo) {
        ...
        if (msg.hasOwnProperty("buffMsg")) {
            ...
            for (let index = 0; index < msg.buffMsg.length; index++) {
                ...
                playerInfo.buffInfoList[index] = new OuterCityWarBuildBuffInfo(buffMsg.buildId, buffMsg.buffId, buffMsg.buffType, buffMsg.buffCondition)
            }
        }
        return playerInfo;
    }

    static syncCastleInfoWithMsg(msg: CastleMsg, castleInfo: BaseCastle): BaseCastle {
