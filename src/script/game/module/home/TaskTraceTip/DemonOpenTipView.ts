...
export class DemonOpenTipView extends TaskTraceTipWnd {
  ...
  initView() {
    ...
    let text = LangManager.Instance.GetTranslation(
      "tasktracetip.view.DemonOpenTipView.content",
    );
    this.setContentText(text);
    this.setContentIcon(
      FUIHelper.getItemURL("Base", "asset.taskTraceTips.DemonIcon"),
    );
    this.setBtnTitle(
      LangManager.Instance.GetTranslation(
        "tasktracetip.view.DemonOpenTipView.text",
      ),
    );
  }
  ...
  protected __btnHandler(e: Event) {
    ...
    if (canJoin == 0) {
      ...
      MessageTipManager.Instance.show(
        ConsortiaManager.Instance.model.demonInfo.getLimitTip(canJoin),
      );
