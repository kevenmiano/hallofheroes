...
import { NotificationEvent, WorldBossEvent } from "../constant/event/NotificationEvent";
...
export default class WorldBossManager {
    ...
    public setup() {
        ...
        ServerDataManager.listen(S2CProtocol.U_C_CAMPAIGN_STATE, this, this.__updateWorldsBossStateHandler);
        ServerDataManager.listen(S2CProtocol.U_C_WARFIELD_STATE, this, this.__updateWarFightStateHandler);
        ServerDataManager.listen(S2CProtocol.C_WORLDBOSSWOUND_REWARD, this, this.__updateRewardStateHandler);
        ServerDataManager.listen(S2CProtocol.U_C_OPRATE_REPLACEMENT_STATE, this, this.__buyStuntmanResultHandler);
        ServerDataManager.listen(S2CProtocol.U_C_REPLACEMENT_STATE, this, this.__initStuntmanInfoHanlder);
        this.reqWorldBossStates();
    }

    private __buyStuntmanResultHandler(pkg: PackageIn) {
        var msg: OprateReplacemntMsg = pkg.readBody(OprateReplacemntMsg) as OprateReplacemntMsg;
        ...
        if (msg.op == 1) { //购买
            if (msg.result == 1) {
                ...
                MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("worldboss.BuyAvatarFrame.command05"));
            }
        } else if (msg.op == 2) { //取消
    ...
    private __updateWorldsBossStateHandler(pkg: PackageIn) {
        var msg: WorldBossStateMsg = pkg.readBody(WorldBossStateMsg) as WorldBossStateMsg;
        ...
        for (const key in dic) {
            if (Object.prototype.hasOwnProperty.call(dic, key)) {
                ...
                if (bossTemp.state == 0 && WorldBossHelper.checkWorldBoss(bossTemp.CampaignId)) newBossOpen = true;
            }
        }
    ...
    private __updateWarFightStateHandler(pkg: PackageIn) {
        ...
        for (const key in pvpWarFightDic) {
            if (Object.prototype.hasOwnProperty.call(pvpWarFightDic, key)) {
                ...
                if (bossTemp.state == 0 && WorldBossHelper.checkPvp(bossTemp.CampaignId)) newBossOpen = true;
            }
        }
        DataCommonManager.playerInfo.isOpenPvpWar = newBossOpen;
        NotificationManager.Instance.dispatchEvent(WorldBossEvent.UPDATE_WARFIGHT_STATE_LIST);
    }

    private __updateRewardStateHandler(pkg: PackageIn) {
        var msg: WorldBossWoundRewardListMsg = pkg.readBody(WorldBossWoundRewardListMsg) as WorldBossWoundRewardListMsg;
        if (CampaignManager.Instance.worldBossModel) {
            var initFlag: boolean = false;
            if (!CampaignManager.Instance.worldBossModel.rewards) {
                CampaignManager.Instance.worldBossModel.rewards = new Array();
                initFlag = true;
            }
            var i: number = 0;
            var reward = null
            if (initFlag) {
                ...
                NotificationManager.Instance.dispatchEvent(WorldBossEvent.REWARDS_INIT, null);
            } else {
                for (i = 0; i < msg.rewardinfo.length; i++) {
                    reward = msg.rewardinfo[i];
