...
export class SRolePropertyCom extends FUI_SRolePropertyCom {
  ...
  private __bagItemUpdateHandler(infos: GoodsInfo[]) {
    for (let info of infos)
      if (
        info.objectId == this._data.id &&
        info.templateInfo.MasterType == GoodsType.EQUIP
      ) {
        this.updateView();
      } else if (
        info.userId == this._data.userId &&
        info.templateInfo.SonType == GoodsSonType.RESIST_GEM
      ) {
        this.updateView();
      }
  }
  ...
  private updateView() {
    ...
    this.txt_point_4
      ...
      .flushVars();
    this.txt_point_value5.text =
      this._data.attackProrerty.totalIntensity.toString();
    this.txt_point_value6.text =
      this._data.attackProrerty.totalTenacity.toString();
    let value1: any = 0;
    let value2: any = 0;
    if (this._data.attackProrerty.totalIntensity > 0) {
      value1 = (
        (100 * this._data.attackProrerty.totalIntensity) /
        (this._data.attackProrerty.totalIntensity + 8000)
      ).toFixed(2);
    }
    if (this._data.attackProrerty.totalTenacity > 0) {
      value2 = (
        (100 * this._data.attackProrerty.totalTenacity) /
        (this._data.attackProrerty.totalTenacity + 8000)
      ).toFixed(2);
    }
    let point5Tips: string = LangManager.Instance.GetTranslation(
      "SRolePropertyCom.tips1",
      value1,
    );
    let point6Tips: string = LangManager.Instance.GetTranslation(
      "SRolePropertyCom.tips2",
      value2,
    );
    FUIHelper.setTipData(this.txt_point_5, EmWindow.CommonTips, point5Tips);
    FUIHelper.setTipData(this.txt_point_6, EmWindow.CommonTips, point6Tips);
    this.txt_property_0
    ...
    this.txt_property_3
      ...
      .flushVars();
    this.txt_property_value4.text =
      this._data.attackProrerty.totalForceHit.toString();
    this.txt_property_value5.text =
      this._data.attackProrerty.totalParry.toString();
    this.txt_property_6
    ...
    if (this._data.attackProrerty.totalForceHit > 0) {
      value4 = (
        this._data.attackProrerty.totalForceHit /
        ArmyManager.Instance.thane.grades /
        1.5
      ).toFixed(2);
    }
    value4 =
      value4 <= ArmyManager.Instance.thane.grades * 2 + 10
        ? value4
        : ArmyManager.Instance.thane.grades * 2 + 10;
    value4 = value4 <= 150 ? value4 : 150;
    let propertyTips4: string = LangManager.Instance.GetTranslation(
      "SRolePropertyCom.tips3",
      value4,
    );
    ...
    if (this._data.attackProrerty.totalParry > 0) {
      value5 = (
        this._data.attackProrerty.totalParry /
        ArmyManager.Instance.thane.grades /
        1.5
      ).toFixed(2);
    }
    value5 =
      value5 <= ArmyManager.Instance.thane.grades * 2 + 10
        ? value5
        : ArmyManager.Instance.thane.grades * 2 + 10;
    value5 = value5 <= 150 ? value5 : 150;
    let propertyTips5: string = LangManager.Instance.GetTranslation(
      "SRolePropertyCom.tips4",
      value5,
    );
