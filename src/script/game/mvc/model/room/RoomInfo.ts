...
export class RoomInfo extends SimpleRoomInfo {
    ...
    public get mapTemplate(): t_s_campaignData {
        let temp: t_s_campaignData = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_campaign, this.campaignId);
        if (!temp) {
            // 由于多人副本多个关卡, 每进入一个关卡发的是 map中的Id, 再通过Id找CampaignId
            let Id = this.campaignId;
            let tempMap: t_s_mapData = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_map, Id);
            if (tempMap) {
                temp = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_campaign, tempMap.CampaignId);
            }
            if (temp) {
        ...
        for (const key in this._armyList) {
            ...
            if (campaignArmy && campaignArmy.baseHero.userId == army.userId){
                if(this.isCross){
                    ...
                    delete this._armyList[army.baseHero.serviceName + "_" + army.baseHero.userId];
                }
            }
        }
        this._armyList[serverName + "_" + army.baseHero.userId] = army;
        this.dispatchEvent(RoomEvent.ADD_PLAYER_ROOM, army);
        NotificationManager.Instance.dispatchEvent(NotificationEvent.ROOM_INFO_UPDATE);
     ...
     * 
     */
    public removePlayerByUserId(userId: number, serverName: string = ""): CampaignArmy {
        Logger.info("[RoomInfo]removePlayerByUserId 需要删除", userId, serverName, this._armyList)
    ...
    public getPlayerByUserId(userId: number, serverName: string = ""): CampaignArmy {
        if (!this.isCross) serverName = "";
        return this._armyList[serverName + "_" + userId] as CampaignArmy;
    }

    public getPlayerByArmyId(armyId: number, serverName: string = ""): CampaignArmy {
    ...
    public get allPlayerReader(): boolean {
        for (const key in this._armyList) {
            if (this._armyList.hasOwnProperty(key)) {
                ...
                if (info.roomState == RoomState.STATE_UNUSE && this.houseOwnerId != info.baseHero.userId) return false;
            }
        }
        return true;
    }
