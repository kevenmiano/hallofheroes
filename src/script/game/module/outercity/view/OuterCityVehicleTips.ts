...
export default class OuterCityVehicleTips extends BaseWindow {
    ...
    private initData() {
        ...
        this.descTxt3.text = LangManager.Instance.GetTranslation("OuterCityVehicleItem.titleDescTxt1");
        this.descTxt4.text = LangManager.Instance.GetTranslation("OuterCityVehicleItem.titleDescTxt2");
        this.descTxt5.text = LangManager.Instance.GetTranslation("OuterCityVehicleInfoWnd.descTxt3");
        if (this._info) {
    ...
    private initView() {
        if (this._info) {
            if (this._info.info.occupyLeagueName == "") {//占领的公会信息无
                this.occupyConsortiaNameTxt.text = LangManager.Instance.GetTranslation("maze.MazeFrame.Order");
                this.occupyConsortiaNameTxt.color = ColorConstant.LIGHT_TEXT_COLOR;
            } else {
                this.occupyConsortiaNameTxt.text = "<" + this._info.info.occupyLeagueName + ">";
    ...
    private initEvent() {
        ...
        this.goodsList.itemRenderer = Laya.Handler.create(this, this.__renderListItem, null, false);
    ...
    private lookInfoHandler() {
        if (this._info.info.state == 2) {
            MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("OuterCityVehicleTips.lookInfoHandler.tips"));
            return;
        }
        this._mapView.motionTo(new Laya.Point(parseInt((this._info.movePosX * 20).toString()) - StageReferance.stageWidth / 2, parseInt((this._info.movePosY * 20).toString()) - StageReferance.stageHeight / 2));
    ...
    private btnGotoHandler() {
        ...
        if (this._info.leftTime <= 0) {//点击的是静态物资车
            if (self) {//玩家自己处于物资车状态
                if (self.wildInfo.templateId == this._info.templateId) {//点击的是自己的物资车
            ...
            } else {//玩家是处于空闲状态，移动到固定的物资车点
                if (OuterCityManager.Instance.model.checkTeamInFight(this._info)
                    || OuterCityManager.Instance.model.checkCanAttackVehicle(this._info)) {//可攻击
                    NotificationManager.Instance.dispatchEvent(OuterCityEvent.OUTERCITY_LOCK_VEHICLE_FIGHT, this._info);
                } else if (OuterCityManager.Instance.model.checkAllInFighting(this._info)) {
