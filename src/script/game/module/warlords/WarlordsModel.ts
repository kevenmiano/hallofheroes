...
export default class WarlordsModel extends FrameDataBase {
  ...
  public initTimerInfo(timerInfoMsg: TimerInfoMsg) {
    let matchDate: Date;
    matchDate = DateFormatter.parse(
      timerInfoMsg.readyOpen,
      "YYYY-MM-DD hh:mm:ss",
    );
    this._matchDateDic.add(1, matchDate);
    matchDate = DateFormatter.parse(
      timerInfoMsg.readyStop,
      "YYYY-MM-DD hh:mm:ss",
    );
    this._matchDateDic.add(2, matchDate);
    matchDate = DateFormatter.parse(
      timerInfoMsg.finalOpen,
      "YYYY-MM-DD hh:mm:ss",
    );
    this._matchDateDic.add(3, matchDate);
    matchDate = DateFormatter.parse(
      timerInfoMsg.finalStop,
      "YYYY-MM-DD hh:mm:ss",
    );
  ...
  public static getDisplayRank(rank: number): number {
    ...
    if (rank <= WarlordsModel.DIVIDING_RANK) return rank;
    else return rank - WarlordsModel.DIVIDING_RANK;
  }
  ...
  public getWarlordsPlayerInfoByUserIdAndServerName(
    userId: number,
    serverName: string,
  ): WarlordsPlayerInfo {
    for (const key in this.accessFinalList) {
      if (Object.prototype.hasOwnProperty.call(this.accessFinalList, key)) {
        ...
        if (
          ...
          playerInfo.serverName == serverName
        ) {
          return playerInfo;
        }
      }
  ...
  public addListData(type: number, key: number, obj: object) {
    switch (type) {
      case WarlordsModel.BETTING:
        if (key > 0) {
          if (this._bettingDic[key]) {
            //如果原来存在, 覆盖前要先将其竞猜名次betRank设为0
  ...
  public updateBettingByType(type: number) {
    ...
    switch (type) {
      ...
      case 1:
        ...
        for (let k in this._curBettingDic) {
          //copy, 不能直接将_curBettingDic赋值给_bettingDic, 这样就指向同一个对象了
  ...
  public getBetResultList(): Array<number> {
    ...
    for (let i: number = 1; i <= 3; i++) {
      if (
        this._bettingDic[i] &&
        this._jobTop3Dic[i] &&
        this._bettingDic[i].userKey == this._jobTop3Dic[i].userKey &&
        !StringHelper.isNullOrEmpty(this._bettingDic[i].userKey)
      ) {
        list[i] = 1;
      } else {
        list[i] = 0;
