...
export class SkillItemListController {
  ...
  private init() {
    ...
    for (let i: number = 1; i <= 7; i++) {
      if (arr.hasOwnProperty("fastkey_" + i)) {
        info = ConfigMgr.Instance.getTemplateByID(
  ...
  addEvent() {
    ...
    ServerDataManager.listen(
      S2CProtocol.U_B_USE_ITEM_RESULT,
      this,
      this.__useBattleItemServerBack,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_SEAL_ORDER,
      this,
      this.__onRecvSealOrder,
    );

    if (this._heroRoleInfo) {
      this._heroRoleInfo.addEventListener(
        RoleEvent.SP,
        this.onHeroSpChanged,
        this,
      );
      this._heroRoleInfo.addEventListener(
        RoleEvent.AWAKEN,
  ...
  offEvent() {
    ...
    if (this._heroRoleInfo) {
      this._heroRoleInfo.removeEventListener(
        RoleEvent.SP,
        this.onHeroSpChanged,
        this,
      );
  ...
  private onSkillCdHandler(data: any) {
    ...
    Logger.battle(
      "[SkillItemListController]onSkillCdHandler",
      skillId,
      waitTime,
    );
  ...
  private __skillEnable(b: boolean) {
    ...
    if (
      ...
      BattleManager.Instance.battleModel.isControledByBoss()
    ) {
      //泰拉神庙副本里面boss控制了玩家, 持续时间内技能不可用
      this.view.setEnableForce(false);
      return;
    }
    let isOver: boolean = BattleManager.Instance.battleModel.isOver;
    let restricted: boolean =
      BattleManager.Instance.battleModel.selfHero.restricted;
    let selfView = BattleManager.Instance.getSelfRoleView();
    //如果这时还是宠物形态 但是觉醒值是0 要设为false 因为这时要等释放解除变身技能
    let isWaitChangeBody: boolean =
      this._heroRoleInfo.isPetState && this._heroRoleInfo.awaken == 0;
  ...
  private onHeroSpChanged(data: any) {
    ...
    if (data.buffCause) {
      let skillId: number =
        BattleManager.Instance.battleModel.currentReadySkillId;
      if (skillId != -1) {
        let skillTemp: t_s_skilltemplateData =
          this.getSkilltemplateDataById(skillId);
        if (
          skillTemp &&
          skillTemp.Cost > BattleManager.Instance.battleModel.selfHero.sp
        ) {
  ...
  private __skillUse(data: any) {
    ...
    if (data instanceof t_s_skilltemplateData) {
      ...
      Logger.battle(
        "[SkillItemListController]__skillUse使用普通技能 : " +
          currentskillId +
          ", type:" +
          skillInfo.MasterType,
      );
      SocketSendManager.Instance.gameCommand(
        battleId,
        livingID,
        currentskillId,
      );
      ...
      Logger.battle(
        "[SkillItemListController]__skillUse使用符文技能 : " + item.skillTempId,
      );
      SocketSendManager.Instance.sendUseItemInBattle(
        battleId,
        livingID,
        item.skillTempId,
      );
      ...
      BattleManager.Instance.battleModel.currentReadySkillId =
        currentProp.skillTemp.TemplateId;
      this.addTrailPropUsedFlag(currentProp);
      SocketSendManager.Instance.sendUseBattleTrialProp(
        battleId,
        currentProp.id,
        livingID,
      );
  ...
  private addTrailPropUsedFlag(item: TrailPropInfo) {
    ...
    if (heroView && heroView.info && heroView.info.isLiving) {
      heroView.addSkillFlagByIconPath(
        IconFactory.getTecIconByIcon(item.skillTemp.Icons),
      );
    }
  }
  /**
  ...
  private addPropUsedFlag(item: BattlePropItem) {
    ...
    if (heroView && heroView.info && heroView.info.isLiving) {
      heroView.addSkillFlagByIconPath(
        IconFactory.getTecIconByIcon(item.getSkillTemplate().Icons),
      );
    }
  }
  /**
  ...
  private __useBattleItemServerBack(pkg: PackageIn) {
    ...
    if (msg.result == 0) {
      ...
      if (roleId == self.livingId) {
        ...
        if (heroView) {
          if (heroView.info && heroView.info.isLiving) {
            ...
            let skillTemp: t_s_skilltemplateData =
              TempleteManager.Instance.getSkillTemplateInfoById(msg.tempId);
            if (skillTemp)
              heroView.addSkillFlagByIconPath(
                IconFactory.getTecIconByIcon(skillTemp.Icons),
              );
          }
        }
      }
      ...
      Logger.battle(
        "[SkillItemListController]__useBattleItemServerBack使用技能失败 : " +
          msg.tempId,
      );
      this.view.setEnable(true);
    }
  ...
  private __skillUseServerBack(pkg: PackageIn) {
    ...
    if (
      result == 1 &&
      roleId == BattleManager.Instance.battleModel.selfHero.livingId
    ) {
      let str2: string = LangManager.Instance.GetTranslation(
        "battle.view.ui.SkillItemListController.str2",
      );
    }
    let heroView: BaseRoleView;
      ...
      if (isSelf) {
        ...
        if (heroView && heroView.info && heroView.info.isLiving) {
          ...
          NotificationManager.Instance.sendNotification(
            ...
            [skillInfo, qteTime, castTime, aims],
          );
        }
      } else {
        heroView = mapRoleDic[roleId];
        if (heroView && heroView.info && heroView.info.isLiving) {
          skillInfo = ConfigMgr.Instance.getTemplateByID(
            ConfigType.t_s_skilltemplate,
            skillId,
          );
          //添加续气效果, 并在头上显示技能图标
    ...
    for (const key in allSkill) {
      if (Object.prototype.hasOwnProperty.call(allSkill, key)) {
        ...
        if (element instanceof t_s_skilltemplateData) {
          if (
            element.TemplateId == skillId ||
            (BattleManager.Instance.battleModel.isAutoFight &&
              element.Parameter3 == skillId)
          ) {
            ////技能id匹配 技能QTE id 匹配
  ...
  private __cleanSkillSelect(event: Event) {
    let heroView: BaseRoleView =
      BattleManager.Instance.battleModel.selfHero.view;
    if (heroView) {
