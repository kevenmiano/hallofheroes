...
export class OuterCityManager {
    ...
    public steup($contro: OuterCityScene, $model: OuterCityModel): void {
        ...
        this._model = $model;
        this._model.mineUpperLimitArr = ConfigInfoManager.Instance.getOuterCityMineUpperLimit();

        ServerDataManager.listen(S2CProtocol.U_C_PLAY_GETMOIVE, this, this.__playGetMovieHandler);
        ServerDataManager.listen(S2CProtocol.U_C_MAPBOSS_DATAS, this, this.onGetBossInfo);
        ServerDataManager.listen(S2CProtocol.U_C_MAPBOSS_FOG, this, this.onMapBossFog);
        ServerDataManager.listen(S2CProtocol.U_C_MAPBOSS_BOX, this, this.onMapBossBox);
        ServerDataManager.listen(S2CProtocol.U_C_OUT_CITY_VEHICLE_INFO, this, this.onMapVehicleInfo);
        if (this.coolDownUint == 0) {
    ...
    private onGetBossInfo(pkg: PackageIn): void {
        ...
        if (this.model.bossInfo.leftFogTime > 0) {
            NotificationManager.Instance.dispatchEvent(OuterCityEvent.OUTER_CITY_REMOVE_FOG, null);
        }
        ...
        this.model.bossInfo.bosslist = ArrayUtils.sortOn(this.model.bossInfo.bosslist, "grade", ArrayConstant.DESCENDING);
        NotificationManager.Instance.dispatchEvent(OuterCityEvent.OUTER_CITY_GET_BOSSINFO, this.model);
    }
    ...
    private onMapVehicleInfo(pkg: PackageIn) {
        let msg: VehicleAllInfoMsg = pkg.readBody(VehicleAllInfoMsg) as VehicleAllInfoMsg;
        if (msg) {
            let wildLand: WildLand;
            let pInfo: PhysicInfo;
        ...
        NotificationManager.Instance.dispatchEvent(OuterCityEvent.OUTER_CITY_VEHICLE_UPDATE);
    }

    private getPlayerArr(info: VehicleInfoMsg, type: number): Array<VehiclePlayerInfo> {
    ...
    private coolDownFog(): void {
        ...
        if (this.model.bossInfo.leftFogTime == 0) {
            NotificationManager.Instance.dispatchEvent(OuterCityEvent.OUTER_CITY_ADD_FOG, null);
        }
    ...
    private onMapBossBox(pkg: PackageIn): void {
        ...
        NotificationManager.Instance.dispatchEvent(OuterCityEvent.OUTER_CITY_UPDATE_BOX, null);
    }
    /**
     * 外城战斗获取动画
     *
    ...
    private __playGetMovieHandler(pkg: PackageIn): void {
        SocketSceneBufferManager.Instance.addPkgToBuffer(pkg, SceneType.OUTER_CITY_SCENE, this.playGetMovie.bind(this));
    }

    private playGetMovie(pkg: PackageIn): void {
        let msg: FightOverMoiveMsg = pkg.readBody(FightOverMoiveMsg) as FightOverMoiveMsg;
        GameBaseQueueManager.Instance.addAction(new BattleGetMoviesAction(this._controler.selfArmy, msg));
        msg = null;
    }
    ...
    public dispose(): void {
        ...
        ServerDataManager.cancel(S2CProtocol.U_C_MAPBOSS_DATAS, this, this.onGetBossInfo);
        ServerDataManager.cancel(S2CProtocol.U_C_MAPBOSS_FOG, this, this.onMapBossFog);
        ServerDataManager.cancel(S2CProtocol.U_C_MAPBOSS_BOX, this, this.onMapBossBox);
        ServerDataManager.cancel(S2CProtocol.U_C_OUT_CITY_VEHICLE_INFO, this, this.onMapVehicleInfo);
    }
    ...
    public vehicleOperation(type: number, templateId: number) {
        ...
        SocketManager.Instance.send(C2SProtocol.C_OUT_CITY_VEHICLE_CHANGE_PLAYER, msg);
    }
