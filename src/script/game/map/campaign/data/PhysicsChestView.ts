...
export class PhysicsChestView extends MapPhysicsBase {
    private _monsterDropBoxName: FilterFrameText = new FilterFrameText(140, 20, undefined, 14);
    private _goods: any[];
    ...
    protected updateView() {
        ...
        if (state == NodeState.HIDE) {
            ...
            NotificationManager.Instance.dispatchEvent(NotificationEvent.OPEN_CHEST_BOX, null);
            UIManager.Instance.ShowWind(EmWindow.BattleFallGoodsWnd, { callBack: this.disposeCall, nodeInfo: this.info, arrayData: this._goods, signId: (<ChestInfo>this.info.info).signId })
            this._isOpen = true;
            this.visible = false;
        }
    ...
    public mouseClickHandler(evt: Laya.Event): boolean {
        ...
        this._time = time;
        var armyView: any = CampaignManager.Instance.controller.getArmyView(CampaignManager.Instance.mapModel.selfMemberData);
        var leng: number = new Laya.Point(armyView.x, armyView.y).distance(new Laya.Point(this.x, this.y).x, new Laya.Point(this.x, this.y).y);
        if (leng > 100) {
