...
export default class HomeBugleNoticeWnd extends FUI_HomeBugleNoticeWnd {
    ...
    private moveIn() {
        ...
        if (this._currentData.channel == ChatChannel.SYS_ALERT) {
            ...
            var useBigHorn: string = LangManager.Instance.GetTranslation("chatII.view.ChatTopBugleView.useBigHorn");
            var sendInfo: string = "[" + this._currentData.senderName + "] " + useBigHorn;
            let elementText: string = "";
            let elements = this._currentData.getAllElements();
            for (let index = 0; index < elements.length; index++) {
                let element = elements[index];
            ...
            var useCrossHorn: string = LangManager.Instance.GetTranslation("chatII.view.ChatTopBugleView.useCrossHorn");
            var crossInfo: string = "[color='#00d8ff'][" + this._currentData.serverName + "-" + this._currentData.senderName + "][/color>] " + useCrossHorn;
            let elementText: string = "";
            let elements = this._currentData.getAllElements();
            for (let index = 0; index < elements.length; index++) {
                let element = elements[index];
            ...
            crossInfo = "[" + this._currentData.serverName + "-" + this._currentData.senderName + "] " + useCrossHorn;
                    ...
                    switch (this._currentData.bigBugleType) {
                        ...
                        case 2:
                            ...
                            if (this._currentData.receiverName == this.thane.nickName) {
                                ...
                                var roseTempId: number = (this._currentData.bigBugleType == 1 ? 2170003 : 2170004);
                                DelayActionsUtils.Instance.addAction(new AlertTipAction([roseTempId, this._currentData.senderName,this._currentData.uid], this.showPresentBackCall.bind(this)));
                            }
                            this.showFireworkCall([90, this._dwellTime / 1000, this._currentData.bigBugleType])
                            break;
                        case 3:
                            this.c1.setSelectedIndex(1);
                            if (this._currentData.receiverName == this.thane.nickName) {
                                //彩带
                                DelayActionsUtils.Instance.addAction(new AlertTipAction([2170005, this._currentData.senderName], this.showPresentBackCall.bind(this)));
                            }
                            this.showFireworkCall([90, this._dwellTime / 1000, this._currentData.bigBugleType])
                        ...
                        case 4:
                            this.c1.setSelectedIndex(1);
                            if (this._currentData.receiverName == this.thane.nickName) {//星星
                                DelayActionsUtils.Instance.addAction(new AlertTipAction([2170006, this._currentData.senderName], this.showPresentBackCall.bind(this)));
                            }
                            this.showFireworkCall([90, this._dwellTime / 1000, this._currentData.bigBugleType])
                        ...
                        default:
                            if (this._currentData.receiverName == this.thane.nickName) {
                                var roseTempId: number = (this._currentData.bigBugleType == 1 ? 2170003 : 2170004);
                                DelayActionsUtils.Instance.addAction(new AlertTipAction([roseTempId, this._currentData.senderName], this.showPresentBackCall.bind(this)));
                            }
                            break;
                    }
                }
    ...
    private addEvent() {
        ...
        NotificationManager.Instance.addEventListener(ChatEvent.UPDATE_BIGBUGLE_VIEW, this.__addDataHandler, this);
    }
