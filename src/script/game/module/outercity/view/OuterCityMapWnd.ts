...
export default class OuterCityMapWnd extends BaseWindow {
  ...
  public OnInitWind() {
    super.OnInitWind();
    let aInfo: BaseArmy = this.outerCityModel.getWorldArmyByUserId(
      this.playerModel.playerInfo.userId,
    );
    if (aInfo) {
  ...
  private initCastlePosition() {
    ...
    for (let key in this.outerCityModel.allCastles) {
      ...
      for (let k in dic) {
        baseCastle = dic[k];
        let smallMapCityItem: SmallMapCityItem = FUIHelper.createFUIInstance(
        ...
        smallMapCityItem.x =
          parseInt(
            (baseCastle.posX * Tiles.WIDTH * this.getScaleX()).toString(),
          ) - parseInt((smallMapCityItem.width / 2).toString());
        smallMapCityItem.y =
          parseInt(
            (baseCastle.posY * Tiles.HEIGHT * this.getScaleY()).toString(),
          ) - parseInt((smallMapCityItem.height / 2).toString());
  ...
  private initTreasurePosition() {
    ...
    for (let i: number = 0; i < len; i++) {
      ...
      let smallMapTreasureItem: SmallMapTreasureItem =
        FUIHelper.createFUIInstance(
          EmPackName.OuterCity,
          "SmallMapTreasureItem",
        ) as SmallMapTreasureItem;
      smallMapTreasureItem.info = treasureInfo;
      smallMapTreasureItem.x =
        parseInt(
          (treasureInfo.posX * Tiles.WIDTH * this.getScaleX()).toString(),
        ) - parseInt((smallMapTreasureItem.width / 2).toString());
      smallMapTreasureItem.y =
        parseInt(
          (treasureInfo.posY * Tiles.HEIGHT * this.getScaleY()).toString(),
        ) - parseInt((smallMapTreasureItem.height / 2).toString());
    ...
    for (let key in this.outerCityModel.allWildLand) {
      ...
      for (let k in dic) {
        ...
        if (wildLand && wildLand.info.types == PosType.OUTERCITY_MINE) {
          let smallMapMineItem: SmallMapMineItem = FUIHelper.createFUIInstance(
            EmPackName.OuterCity,
            "SmallMapMineItem",
          ) as SmallMapMineItem;
          ...
          smallMapMineItem.x =
            parseInt(
              (wildLand.posX * Tiles.WIDTH * this.getScaleX()).toString(),
            ) - parseInt((smallMapMineItem.width / 2).toString());
          smallMapMineItem.y =
            parseInt(
              (wildLand.posY * Tiles.HEIGHT * this.getScaleY()).toString(),
            ) - parseInt((smallMapMineItem.height / 2).toString());
  ...
  private initBossPosition() {
    ...
    for (let i: number = 0; i < bossArr.length; i++) {
      ...
      if (
        wildLand.info.types == PosType.OUTERCITY_BOSS_NPC &&
        wildLand.bossStatus != 0
      ) {
        let smallMapBossItem: SmallMapBossItem = FUIHelper.createFUIInstance(
          EmPackName.OuterCity,
          "SmallMapBossItem",
        ) as SmallMapBossItem;
        ...
        smallMapBossItem.x =
          parseInt(
            (wildLand.posX * Tiles.WIDTH * this.getScaleX()).toString(),
          ) - parseInt((smallMapBossItem.width / 2).toString());
        smallMapBossItem.y =
          parseInt(
            (wildLand.posY * Tiles.HEIGHT * this.getScaleY()).toString(),
          ) - parseInt((smallMapBossItem.height / 2).toString());
        this._bossContainer.addChild(smallMapBossItem.displayObject);
        this._bossDic.set(
          wildLand.posX + "_" + wildLand.posY,
          smallMapBossItem,
        );
      }
    }
  ...
  private initVehiclePosition() {
    ...
    for (let key in this.outerCityModel.allVehicleNode) {
      ...
      if (
        wildLand &&
        wildLand.info.types == PosType.OUTERCITY_VEHICLE &&
        wildLand.status != 2
      ) {
        let smallMapVehicleItem: SmallMapVehicleItem =
          FUIHelper.createFUIInstance(
            EmPackName.OuterCity,
            "SmallMapVehicleItem",
          ) as SmallMapVehicleItem;
        ...
        if (selfVehicle && selfVehicle.wildInfo.templateId == parseInt(key)) {
          ...
          this._selfPoint.x =
            parseInt(
              (wildLand.movePosX * Tiles.WIDTH * this.getScaleX()).toString(),
            ) - parseInt((smallMapVehicleItem.width / 2).toString());
          this._selfPoint.y =
            parseInt(
              (wildLand.movePosY * Tiles.HEIGHT * this.getScaleY()).toString(),
            ) - parseInt((smallMapVehicleItem.height / 2).toString());
        }
        smallMapVehicleItem.x =
          parseInt(
            (wildLand.movePosX * Tiles.WIDTH * this.getScaleX()).toString(),
          ) - parseInt((smallMapVehicleItem.width / 2).toString());
        smallMapVehicleItem.y =
          parseInt(
            (wildLand.movePosY * Tiles.HEIGHT * this.getScaleY()).toString(),
          ) - parseInt((smallMapVehicleItem.height / 2).toString());
  ...
  private updateVehiclePosition() {
    for (let key in this._vehicleDic) {
      ...
      if (wildLand.status != 2) {
        ...
        smallMapVehicleItem.x =
          parseInt(
            (wildLand.info.posX * Tiles.WIDTH * this.getScaleX()).toString(),
          ) - parseInt((smallMapVehicleItem.width / 2).toString());
        smallMapVehicleItem.y =
          parseInt(
            (wildLand.info.posY * Tiles.HEIGHT * this.getScaleY()).toString(),
          ) - parseInt((smallMapVehicleItem.height / 2).toString());
      } else {
        //移除
  ...
  private initPlayerPosition() {
    ...
    for (const key in this.outerCityModel.allArmyDict) {
      if (this.outerCityModel.allArmyDict.hasOwnProperty(key)) {
        ...
        if (
          army.userId == this.playerModel.playerInfo.userId &&
          army.armyView
        ) {
          smallMapPlayerItem = FUIHelper.createFUIInstance(
            EmPackName.OuterCity,
            "SmallMapPlayerItem",
          ) as SmallMapPlayerItem;
          smallMapPlayerItem.headId = army.baseHero.headId;
          smallMapPlayerItem.info = army;
          this._selfPoint = new Laya.Point();
          smallMapPlayerItem.x =
            parseInt((army.armyView.x * this.getScaleX()).toString()) -
            parseInt((smallMapPlayerItem.width / 2).toString());
          smallMapPlayerItem.y =
            parseInt((army.armyView.y * this.getScaleY()).toString()) -
            parseInt((smallMapPlayerItem.height / 2).toString());
  ...
  private initEvent() {
    NotificationManager.Instance.addEventListener(
      ...
      this.closeWndHandler,
      this,
    );
    NotificationManager.Instance.addEventListener(
      OuterCityEvent.UPDATE_SECOND_NODE_DATA,
      this.updateNodeInfoHandler,
      this,
    );
    if (this._selfArmyView && this._selfArmyView.info)
      this._selfArmyView.info.addEventListener(
        AiEvents.NEXT_POINT,
        this.__walkNextHandler,
        this,
      );
    this.outerCityModel.addEventListener(
  ...
  private removeEvent() {
    ...
    if (this._selfArmyView && this._selfArmyView.info)
      this._selfArmyView.info.removeEventListener(
        AiEvents.NEXT_POINT,
        this.__walkNextHandler,
        this,
      );
    this.outerCityModel.removeEventListener(
      OuterCityEvent.LAY_MAP_ARMY,
      this.initPlayerPosition,
      this,
    ); //地图上显示马
  ...
  private initLocation() {
    if (!this._selfPoint) {
      let selfVehicleView: OutercityVehicleArmyView =
        OuterCityManager.Instance.model.getSelfVehicle();
      if (selfVehicleView) {
  ...
  private updateMemberPosition(msg: PosMoveMsg) {
    let smallMapPlayerItem: SmallMapPlayerItem = this._playerDic.get(
      msg.armyUserId,
    ); //好友位置更新
    if (smallMapPlayerItem) {
      while (msg.routes.length > 0) {
        ...
        smallMapPlayerItem.x =
          parseInt((node.x * this.getScaleX()).toString()) -
          parseInt((smallMapPlayerItem.width / 2).toString());
        smallMapPlayerItem.y =
          parseInt((node.y * this.getScaleY()).toString()) -
          parseInt((smallMapPlayerItem.height / 2).toString());
      }
    } else {
      let teamModel = FreedomTeamManager.Instance.model;
      if (
        teamModel &&
        teamModel.getMemberByUserId(msg.armyUserId) &&
  ...
  private __walkNextHandler() {
    let smallMapPlayerItem: SmallMapPlayerItem = this._playerDic.get(
      ArmyManager.Instance.army.userId,
    );
    let army: BaseArmy = this.outerCityModel.getWorldArmyByUserId(
      ArmyManager.Instance.army.userId,
    );
    smallMapPlayerItem.x =
      parseInt((army.armyView.x * this.getScaleX()).toString()) -
