...
export default class MarketBuyWnd extends BaseWindow {
  ...
  private initTranslate() {
    ...
    this.sellCountLab.text = LangManager.Instance.GetTranslation(
      "Market.sell.sellCount",
      0,
    );
    this.buyCountLab.text = LangManager.Instance.GetTranslation(
      "Market.sell.buyCount",
      0,
    );
    // this.pushCountLab.text = LangManager.Instance.GetTranslation("Market.sell.pushCount", 0, 0);

    this.sellPriceTxt.text = LangManager.Instance.GetTranslation(
      "Market.buy.sellPrice",
    );
    this.sellCountTxt.text = LangManager.Instance.GetTranslation(
      "Market.buy.sellCount",
    );

    this.buyPriceTxt.text = LangManager.Instance.GetTranslation(
      "Market.buy.buyPrice",
    );
    this.buyCountTxt.text = LangManager.Instance.GetTranslation(
      "Market.buy.buyCount",
    );

    this.myBuyPriceTxt.text = LangManager.Instance.GetTranslation(
      "Market.buy.minePrice",
    );
    this.myBuyCountTxt.text = LangManager.Instance.GetTranslation(
      "Market.buy.mineCount",
    );
    this.mySumPriceTxt.text = LangManager.Instance.GetTranslation(
      "Market.buy.sumPrice",
    );

    this.pushBtn.title =
      LangManager.Instance.GetTranslation("Market.buy.pushTxt");
  }

  private addEvent() {
    this.marketBuyList.setVirtual();
    this.marketBuyList.itemRenderer = Laya.Handler.create(
      this,
      this.onMarketBuyRender,
      null,
      false,
    );

    this.marketSellList.setVirtual();
    this.marketSellList.itemRenderer = Laya.Handler.create(
      this,
      this.onMarketSellRender,
      null,
      false,
    );
  ...
  private initPuchargeView() {
    ...
    this.sellListData = msg.sellPriceList.sort((a, b) => {
      return a.top - b.top;
    });

    this.buyListData = msg.purchasePriceList.sort((a, b) => {
      return a.top - b.top;
    });
    ...
    this.goodsNameLab.color = GoodsSonType.getColorByProfile(
      this._cacheGoodsInfo.templateInfo.Profile,
    );

    this.goodsItem.info = this._cacheGoodsInfo;
    this._cacheLimit = MarketManager.Instance.getLimitBuyTemplateId(
      this._cacheGoodsInfo.templateId,
    );

    let defaultPoint = this.buyListData[0]
      ? this.buyListData[0].point
      : this.sellListData[0]
        ? this.sellListData[0].point
        : this._cacheLimit.MinPrice;

    let priceStep = Math.floor(this._cacheLimit.MinPrice * 0.2);

    this.priceStepper.show(
      0,
      defaultPoint,
      this._cacheLimit.MinPrice,
      this._cacheLimit.MaxPrice,
      this._cacheLimit.MaxPrice,
      priceStep,
      Laya.Handler.create(this, this.onPriceChange, null, false),
    );
  ...
  private onPriceChange() {
    this.buyPriceLab.text =
      this.priceStepper.value * this.buyStepper.value + "";
    this.updatePushInfo();
  }
  ...
  private openAlert(p: number, c: number, goodsName: string) {
    ...
    //最小出售订单
    let myMinOrder = MarketManager.Instance.getSellOrderMinByTemplateId(
      this._cacheGoodsInfo.templateId,
    );
    //您正以更低价格出售相同物品
    if (myMinOrder && p >= myMinOrder.point) {
      MessageTipManager.Instance.show(
        LangManager.Instance.GetTranslation("Market.sell.errorSame"),
      );
      return;
    }

    let content: string = LangManager.Instance.GetTranslation(
      "market.buy.pushConfirm",
      p,
      c,
      goodsName,
      p * c,
