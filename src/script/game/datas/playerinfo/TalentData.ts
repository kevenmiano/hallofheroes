...
export class TalentData extends GameEventDispatcher {
    ...
    public initAllSkill(job: number) {
        ...
        this.allTaletTemp = TempleteManager.Instance.getAllTalentTemplateInfoByJob(this._job * 10);
    ...
    public existSkill(info: t_s_skilltemplateData): number {
        ...
        for (var id of talentList) {
            ...
            var temp: t_s_skilltemplateData = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_skilltemplate, id.toString());
    ...
    public checkUpgrade():boolean{
        var gradeInfo: t_s_upgradetemplateData = TempleteManager.Instance.getTemplateByTypeAndLevel(this.talentGrade + 1, 21);
        if (gradeInfo) {
            if (ResourceManager.Instance.gold.count >= gradeInfo.TemplateId && ArmyManager.Instance.thane.gp >= gradeInfo.Data && ArmyManager.Instance.thane.grades >= 50){
                return true;
            }else{
                return false;
            }  
