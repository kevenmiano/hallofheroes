...
export default class PassRewardCom extends FUI_PassRewardCom {
    ...
    onConstruct() {
        ...
        let cfgVal = TempleteManager.Instance.getConfigInfoByConfigName('passcheck_max_grade').ConfigValue
        if(cfgVal){
            this._maxGrade = parseInt(cfgVal);
        }
        this.list.setVirtual();
        this.list.itemRenderer = Laya.Handler.create(this, this.onRenderList0, null, false);
        this.list.on(fgui.Events.SCROLL, this, this.onScroll);
        this.listData = TempleteManager.Instance.getPassCheckCfeByJob(this.ctrlData.passRewardInfo.passIndex);
    ...
    onAdvance(){
        if(PassportView.isLogin){
            ...
            NotificationManager.Instance.dispatchEvent(PassCheckEvent.RECEIVE_PASS_REWARD,[false]);
        }
    ...
    scrollEnd() {
        ...
        if (this._curIndex != index) {
            ...
            if(this.grade >= this._maxGrade){//ii.	当玩家达到战令最大等级后, 预览栏固定只显示溢出等级奖励, 左侧常规等级奖励滑动不再影响预览栏
                this.showNext();
            }else{
                ...
                if (nextData) {
                    ...
                    this.next.img_claim0.visible = this.next.img_claim1.visible = this.next.img_claim1.visible = false;
                    this.next.img_red0.visible = this.next.img_red1.visible = this.next.img_red2.visible = false;
                    this.next.img_lock0.visible = this.next.img_lock1.visible = this.next.img_lock2.visible = false;
                }
    ...
    private showNext(){
        let maxCfg = TempleteManager.Instance.getPassCheckItemByGrade(-1,this.ctrlData.passRewardInfo.passIndex);
        if(!maxCfg){
            maxCfg = TempleteManager.Instance.getPassCheckItemByGrade(-1,0);
        }
        this.next.setData(maxCfg);
        //只要返回给你的这个奖励等级小于你当前的等级的时候  溢出奖励, 已领取等级
        if(this.ctrlData.passRewardInfo.rewardGrade < this.grade || this.grade==0){
            if(this.grade>this._maxGrade){
        ...
        this.next.txt_level.text = ">"+LangManager.Instance.GetTranslation('public.level2', this._maxGrade);
        ...
        if(this.ctrlData.passRewardInfo.rewardGrade == 0){
            ...
            this.next.txt_count.text = 'X'+ (this.grade-this.ctrlData.passRewardInfo.rewardGrade);;
        }
    ...
    dispose(): void {
        ...
        this.list && this.list.off(fgui.Events.SCROLL, this, this.onScroll);
        super.dispose();
    }
}