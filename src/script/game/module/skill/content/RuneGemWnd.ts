...
export default class RuneGemWnd extends BaseWindow {
    ...
    public OnShowWind(): void {
        super.OnShowWind();
        this.runehole_spot_use = TempleteManager.Instance.getConfigInfoByConfigName("runehole_spot_use").ConfigValue;
    ...
    helpBtnClick() {
        ...
        let content = LangManager.Instance.GetTranslation("runeGem.help0", this.runehole_spot_use, this.maxPoint);
    ...
    public OnHideWind(): void {
        ...
        if (this.params && this.params.openFromRuneBag) {
            FrameCtrlManager.Instance.open(EmWindow.RuneHoldEquipWnd, this.params.data);
        } else {
            this.closeGem && NotificationManager.Instance.dispatchEvent(RuneEvent.CLOSE_GEM);
        }
        super.OnHideWind();
    }
