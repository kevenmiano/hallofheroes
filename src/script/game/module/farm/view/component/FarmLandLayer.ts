...
export class FarmLandLayer extends Laya.Sprite {
  ...
  private initView() {
    ...
    for (i = 0; i < FarmModel.LAND_NUM; i++) {
      var item: FarmLandItem = this.comLand.getChild(
        "item_" + Utils.numFormat(i, 2),
      ) as FarmLandItem;
      item.pos = i;
  ...
  private __onClickHandler(evt: Laya.Event) {
    if (
      evt.target.name == "FarmLand_ClickItem" ||
      (evt.target.name == "FarmLand_BtnBag" && FarmManager.Instance.showingBag)
    ) {
    } else {
  ...
  private __lightLand(show: boolean) {
    ...
    for (let i = 0; i < FarmModel.LAND_NUM; i++) {
      ...
      if (item && item.imgSelected) item.imgSelected.visible = false;

      if (!show) continue;

      if (!item.isOpen || !item.info) {
        //未开启或者无土地信息
        continue;
      }

      if (item.info.hasCrop) {
        //有作物
        continue;
      }
