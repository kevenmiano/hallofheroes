...
export class OuterCityCastleTips extends BaseWindow {
    ...
    if (this._info instanceof WildLand) {
      if (this._info.info.types == PosType.OUTERCITY_VEHICLE) {
        let wildLand: WildLand =
          OuterCityManager.Instance.model.allVehicleNode.get(
  ...
  private gotoBtnHandler() {
    ...
    if (this._info.info.types == PosType.OUTERCITY_VEHICLE) {
      ...
      if (StringHelper.isNullOrEmpty(wildInfo.info.occupyLeagueName)) {
        //点击的是静态物资车
        if (selfVehicleView) {
          //玩家自己处于物资车状态
          if (selfVehicleView.wildInfo.templateId == wildInfo.templateId) {
            //点击的是自己的物资车
            FrameCtrlManager.Instance.open(
          ...
          //玩家是处于空闲状态，
          if (
            OuterCityManager.Instance.model.checkTeamInFight(wildInfo) ||
            OuterCityManager.Instance.model.checkCanAttackVehicle(wildInfo)
          ) {
            //可攻击
            ...
            OuterCityManager.Instance.model.checkAllInFighting(wildInfo)
          ) {
            MessageTipManager.Instance.show(
              LangManager.Instance.GetTranslation(
