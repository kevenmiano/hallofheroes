...
export class CampaignMapDragMediator implements IMediator {
    ...
    private __dragingCallBack(): void {
        ...
        if (this._endPoint.distance(this._startPoint.x, this._startPoint.y) > this.checkDistance) {
    ...
    private dragingSceneImp(): void {
        (this._target as CampaignMapView).event(CampaignMapEvent.MOVE_SCENET_END, this._target);
    }
    ...
    private sendPath() {
        ...
        if (army) {
            MapSocketOuterManager.sendCampaignArmyMove(army.id, this._walkPath, army.mapId, PlayerVisualFollow.Type2);
        }
    }
