...
export class FarmModel extends GameEventDispatcher {
    ...
    constructor() {
        ...
        let cfgTemp = null;
        let cfgTempValue = 0;
        cfgTemp = TempleteManager.Instance.getConfigInfoByConfigName("FarmCrop_Operate_GP")
        if(cfgTemp) {
            cfgTempValue = Number(cfgTemp.ConfigValue);
        }
        this._expWeed = this._expWorm = this._expRevive = cfgTempValue;

        cfgTemp = TempleteManager.Instance.getConfigInfoByConfigName("Farm_TodayFromFriend_GP");
        if(cfgTemp) {
            cfgTempValue = Number(cfgTemp.ConfigValue);
        }
        this._maxDayGpFromFriend = cfgTempValue;

        cfgTemp = TempleteManager.Instance.getConfigInfoByConfigName("Farm_TodayStolen_Count");
        if(cfgTemp) {
            cfgTempValue = Number(cfgTemp.ConfigValue);
        }
        this.maxDayStealCount = cfgTempValue;

        cfgTemp = TempleteManager.Instance.getConfigInfoByConfigName("Farm_Accelerate_Point");
        if(cfgTemp) {
            cfgTempValue = Number(cfgTemp.ConfigValue);
        }
    ...
    private commitSelected() {
        this.dispatchEvent(FarmEvent.SELECTED_CHANGE, this._curSelectedFarmInfo, this._curSelectedUserInfo);
    }
    ...
    public expByOper(oper: number): number {
        ...
        let exp: number = 0;
        let upGrade: t_s_upgradetemplateData = TempleteManager.Instance.getTemplateByTypeAndLevel(this.myFarm.grade + 1, UpgradeType.UPGRADE_TYPE_FARM);
        ...
        switch (oper) {
            ...
            case FarmOperateType.GIVE_POWER:
                let fInfo: ThaneInfo = FriendManager.getInstance().getFriendById(this.curSelectedFarmInfo.userId);
                if (fInfo) {
        ...
        if (this.curSelectedFarmInfo.userId != this.myFarm.userId && (this.myFarm.dayGpFromFriend + exp > this._maxDayGpFromFriend)) {
