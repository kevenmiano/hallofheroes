...
import { RewardEvent, DictionaryEvent, PawnEvent } from '../../constant/event/NotificationEvent';
...
export default class BaseOfferReward extends GameEventDispatcher {
	...
	public getProgress(): any[] {
		...
		for (let i: number = 0; i < len; i++) {
			let str: string;
			let conditionTemp: t_s_rewardcondictionData = this.rewardTemp.conditionList[i];
	...
	private __pawnsChangeHandler(e: ArmyPawn) {
		let pawn: ArmyPawn = e;
		let nowPawnsNum: number = ArmyManager.Instance.getTotalPawnsNumberBySonType(pawn.templateInfo.SonType);
	...
	private checkCount(conditionTemp: t_s_rewardcondictionData, index: number): string {
		if (this.rewardInfo) {
			...
			return " (" + this.rewardInfo["condition_" + (index + 1)] + "/" + conditionTemp.Para2 + ") ";
		}
		return "";
	}

	private checkPawnsCount(conditionTemp: t_s_rewardcondictionData): string {
		let pawnsNum: number = ArmyManager.Instance.getTotalPawnsNumberBySonType(conditionTemp.Para1);
	...
	private checkGoodsCount(conditionTemp: t_s_rewardcondictionData): string {
		let count: number = GoodsManager.Instance.getGoodsNumByTempId(conditionTemp.Para1);
