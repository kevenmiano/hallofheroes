...
export class BatterModel
{
    ...
    public set isStart(value:boolean)
    {
        ...
        if(this._isStart ==  false)
            NotificationManager.Instance.sendNotification(BattleNotic.SET_BATTERUI_VISIBLE,false);
    }
    ...
    public set batterCount(value:number)
    {
        ...
        if(this._lock == false)
        {
            NotificationManager.Instance.sendNotification(BattleNotic.SET_BATTER_COUNT,this._batterCount);
        }
        if(!this._isSendVisible&&this._batterCount>2)
        {
            NotificationManager.Instance.sendNotification(BattleNotic.SET_BATTERUI_VISIBLE,true);
    ...
    public set damageTotal(value:number)
    {
        ...
        if(this._lock == false)
        {
            NotificationManager.Instance.sendNotification(BattleNotic.SET_TOTAL_DAMAGE,this._damageTotal);
        }
    }
