...
export class HookManager extends GameEventDispatcher {
    ...
    public setup() {
        ...
        this._hookInfolist = [];
        let config = TempleteManager.Instance.getConfigInfoByConfigName("temple_open");
        let info: HookInfo = null;
        if (config) {//"12:00-14:00,20|15:00-16:00,20|19:00-21:00,20"
    ...
    private initEvent() {
        ServerDataManager.listen(S2CProtocol.U_C_TEMPLE_INFO, this, this.__hookUpdateHandler);
    }

    private offEvent() {
        clearInterval(this._timeInterval); this._timeInterval = 0;
    }
    ...
    public enterFrame() {
        ...
        for (let index = 0; index < this._hookInfolist.length; index++) {
            ...
            if ((systemTime.getHours() == startDate.getHours() && systemTime.getMinutes() == startDate.getMinutes()) ||
                (systemTime.getHours() == endDate.getHours() && systemTime.getMinutes() == endDate.getMinutes())) {
                //请求体力状态
