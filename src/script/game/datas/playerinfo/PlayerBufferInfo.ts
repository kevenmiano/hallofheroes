...
export class PlayerBufferInfo extends GameEventDispatcher {
  ...
  public set leftTime(value: number) {
    ...
    if (this._leftTime > 0) this._time.start();
    else this._time.stop();
  }
  ...
  public get name(): string {
    if (this.bufferType == PlayerBufferType.EXP_RATE) {
      return LangManager.Instance.GetTranslation(
        "yishi.datas.playerinfo.PlayerBufferInfo.name",
        this.rateCount / 100,
      );
    } else if (this.bufferType == PlayerBufferType.LORDS_BUFFER) {
      ...
      return LangManager.Instance.GetTranslation(
        "yishi.datas.playerinfo.PlayerBufferInfo.BasePropertyName" +
          this.masterType,
      );
    }
  ...
  public get description(): string {
    if (this.bufferType == PlayerBufferType.EXP_RATE) {
      return LangManager.Instance.GetTranslation(
        "yishi.datas.playerinfo.PlayerBufferInfo.description",
        this.rateCount,
      );
    } else if (
      this.bufferType == PlayerBufferType.LORDS_BUFFER ||
      this.bufferType == PlayerBufferType.BASE_PROPERTY_BUFFER
    ) {
      return this._description;
    } else if (this.bufferType == PlayerBufferType.KINGCONTRACT) {
      return this.KingTemplate.template.DescriptionLang;
    } else if (this.bufferType == PlayerBufferType.OTHER_BUFFER) {
      var des: string = (this.template as t_s_effecttemplateData)
        .DescriptionLang;
  ...
  public getGoodsTemplate(): t_s_itemtemplateData {
    //GoodsTemplateInfo
    return ConfigMgr.Instance.getTemplateByID(
      ConfigType.t_s_itemtemplate,
      this.goodsTemplateId.toString(),
    );
  }
