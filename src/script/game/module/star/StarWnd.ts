...
export default class StarWnd extends BaseWindow {
    ...
    OnShowWind() {
        ...
        this.gold.addEventListener(ResourceEvent.RESOURCE_UPDATE, this.resourcesUpdateHandler, this);
        this.initPlanet()
        this.initStarList()
    ...
    public __addDisplayStar(data: any) {
        ...
        let addStarTip: string = LangManager.Instance.GetTranslation("star.view.DisplayStarView.addStarTip01", sInfo.template.TemplateNameLang, sInfo.grade);
    ...
    public __delDisplayStar(data: any) {
        ...
        if (this.starModel.delWay == StarModel.PICK_UP) {
            let rate: number = this.moveDstPos.distance(displayStar.x, displayStar.y) / this.moveDstPos.distance(0, 0);
            time = 1.5 * rate
            timeAdd = 1
            TweenMax.to(displayStar, time, { x: this.moveDstPos.x, y: this.moveDstPos.y, onComplete: this.__moveComplete.bind(this), onCompleteParams: [displayStar, sInfo.pos] });
        } else if (this.starModel.delWay == StarModel.SELL) {
            time = 1
            TweenMax.to(displayStar, time, { scaleX: 1.5, scaleY: 1.5, alpha: 0, onComplete: this.__moveComplete.bind(this), onCompleteParams: [displayStar, sInfo.pos] });
        } else {
    ...
    public __starComposeHandler(sInfo: StarInfo) {
        ...
        if (sInfo && sInfo.bagType == StarBagType.TEMP && this._displayStarDic[sInfo.pos]) {
    ...
    public __randomPosHandler() {
        ...
        for (let index: number = 0; index < siteListLen; index++) {
            ...
            if (siteList[index] == 1) {
                maxOpenIndex = index
                if (index >= 1) UIFilter.normal(this.arrowList.getChildAt(index - 1).asCom)
            } else {
                if (index >= 1) UIFilter.gray(this.arrowList.getChildAt(index - 1).asCom)
            }
        }
    ...
    public __starPointUpdateHandler() {
        this.pointText.text = LangManager.Instance.GetTranslation("public.colon", LangManager.Instance.GetTranslation("answer.view.rank.score")) + String(DataCommonManager.playerInfo.starPoint);
    }
    ...
    private oKsaleBtnClick() {
        if (this.oKsaleBtnClickAlert()) {
            FrameCtrlManager.Instance.open(EmWindow.StarOperate, this.oKsaleBtnClickAlertBack.bind(this))
        } else {
    ...
    private oKstarBtnClick() {
        ...
        //     return;
        // }
        if (this.starManager.tempStarList.getList().length >= StarModel.MAX_DISPLAY_STAR) {
            MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("star.view.DisplayStarView.command01"));
            return;
        }
    ...
    private exchangeBtnClick() {
        if (this.canClose)
            FrameCtrlManager.Instance.open(EmWindow.ShopCommWnd, { shopType: ShopModel.STAR_SHOP, returnToWin: EmWindow.Star }, null, EmWindow.Star)
    }

    private starBagBtnClick() {
        if (this.canClose)
            FrameCtrlManager.Instance.open(EmWindow.StarBag, { returnToWin: EmWindow.Star }, null, EmWindow.Star)
    }

    private helpBtnClick() {
        let title: string = LangManager.Instance.GetTranslation("star.view.StarHelpFrame.title");
        let content: string = LangManager.Instance.GetTranslation("star.view.StarHelpFrame.helpContent");
    ...
    private __onPlanetClickItem(item: PlanetItem) {
        ...
        if (this.starManager.tempStarList.getList().length >= StarModel.MAX_DISPLAY_STAR) {
            MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("star.view.MakeStarView.command03"));
            return;
        }
    ...
    private autoStarBtnClick() {
        let vipPrevilige = VIPManager.Instance.model.isOpenPrivilege(VipPrivilegeType.AUTO_STAR);
        if (!vipPrevilige) {
            let minGrade: number = VIPManager.Instance.model.getMinGradeHasPrivilege(VipPrivilegeType.AUTO_STAR);
            MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("star.view.autoStar.command", minGrade.toString()));
        } else {
            ...
            this.starManager.sendBtnAutoStar(costValue, sellMaxQuality, composeMaxQuality);
        }
    }
    ...
    dispose() {
        ...
        this.gold.removeEventListener(ResourceEvent.RESOURCE_UPDATE, this.resourcesUpdateHandler, this);
        super.dispose()
