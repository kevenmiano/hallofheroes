...
export default class CarnivalAwardPointPageItem extends FUI_CarnivalAwardPointPageItem {
    ...
    private addEvent() {
        ...
        this.goodsList2.itemRenderer = Laya.Handler.create(this, this.renderListItem2, null, false);
    }
    ...
    private clickHandler(e: MouseEvent) {
        if (this._temp) CarnivalManager.Instance.opRequest(CarnivalManager.OP_SCORE_REWARD, this._temp.Id);
    }
    ...
    public set info(value) {
        this._temp = value;
        if (this._temp) {
            this.txtTitle.text = LangManager.Instance.GetTranslation("carnival.awardpointItem.title", this._temp.Target);
            this.refreshView();
            this._itemList = [];
