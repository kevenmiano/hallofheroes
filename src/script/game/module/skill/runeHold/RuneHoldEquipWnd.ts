...
export default class RuneHoldEquipWnd extends BaseWindow {
  ...
  public OnInitWind() {
    ...
    this.txt_energy0.text = LangManager.Instance.GetTranslation(
      "armyII.viewII.allocate.number",
    );
    if (this.params && this.params.frameData) {
      [this.holeInfo, this.spos] = this.params.frameData;
    } else {
      let runeHoldCom = (NewbieUtils.getFrame(EmWindow.Skill) as SkillWnd)
        .runeHoldCom;
      if (runeHoldCom) {
    ...
    this.runeItems = [
      this.equipRuneList.rr0,
      this.equipRuneList.rr1,
      this.equipRuneList.rr2,
      this.equipRuneList.rr3,
      this.equipRuneList.rr4,
    ];
    this.runeHoldProfileFilter = (
      this.controler.data as SkillWndData
    ).runeHoldProfileFilter;
    this.updateGridInfo(
    ...
    if (str) {
      ...
      UIManager.Instance.ShowWind(EmWindow.TodayNotAlert, {
        ...
        checkTxt: LangManager.Instance.GetTranslation(
          "active.expback.frame.selectbtntxt",
        ),
        backFunction: this.confirmFunc.bind(this),
      });
    }
  }
  ...
  private initBagRune() {
    this.equipBagList.itemRenderer = Laya.Handler.create(
      this,
      this.onItemRenderer,
      null,
      false,
    );
    this.equipBagList.on(
      fairygui.Events.CLICK_ITEM,
      this,
      this.onSelectBagRune,
    );
  ...
  private onItemRenderer(index: number, item: EquipHoldBagItem) {
    ...
    item.info = info;
    let state = info
      ? Enum_BagGridState.Item
      : this.openGridNum > index
        ? Enum_BagGridState.Empty
        : Enum_BagGridState.Lock;
    item.state = state;
    ...
    item.hadPropertyFlag =
      info.bagType == BagType.RUNE_EQUIP ||
      !!this.equipTypes[info.templateInfo.Property1];

    if (this.selectedIndexs[index]) {
      item.selectState = ItemSelectState.Selected;
      return;
    }
    item.selectState = this.c1.selectedIndex
      ? ItemSelectState.Selectable
      : ItemSelectState.Default;
  }
  ...
  private setRuneItemValue(value: GoodsInfo | number, item: FUI_EquipHoldItem) {
    ...
    if (value instanceof GoodsInfo) {
      ...
      item.runeName.color = GoodsSonType.getColorByProfile(
        ...
        if (pos > 0) {
          ...
          if (preInfo == 0) {
            MessageTipManager.Instance.show(
              LangManager.Instance.GetTranslation("runeGem.preHole"),
            );
            return;
          }
        }
  ...
  private onSelectBagRune(item: EquipHoldBagItem) {
    ...
    //加个时间间隔, 避免快速点击。
    let curTime =
      PlayerManager.Instance.currentPlayerModel.sysCurtime.getTime();
    ...
    this.controler.reqRuneGemEquip(
      this.holeInfo.holeId,
      this.spos + 1,
      info.templateId,
      info.pos,
    );
  ...
  private onDestoryClick() {
    ...
    UIManager.Instance.ShowWind(EmWindow.FilterRuneWnd, {
      filterType: 2,
      isResolve: true,
    });
  ...
  private onGetMoreClick() {
    UIManager.Instance.ShowWind(EmWindow.RuneGemWnd, {
      openFromRuneBag: true,
      data: [this.holeInfo, this.spos],
    });
    this.hide();
  }
  ...
  private onResolve() {
    ...
    if (isAlterTips) {
      ...
      let prompt: string = LangManager.Instance.GetTranslation("public.prompt");
      let content: string =
        LangManager.Instance.GetTranslation("runeGem.resolve");
      SimpleAlertHelper.Instance.Show(
        SimpleAlertHelper.SIMPLE_ALERT,
        null,
        prompt,
        ...
        confirm,
        cancel,
        this.resolveAlertCallback.bind(this),
      );
      return;
    } else {
      this.doResolve();
  ...
  private doResolve() {
    if (this.selectedIndexs.length <= 0) {
      MessageTipManager.Instance.show(
        LangManager.Instance.GetTranslation("petEuip.resolveTip2"),
      );
      return;
    }
    let length = this.selectedIndexs.length;
    ...
    if (item.info.bagType == BagType.RUNE_EQUIP) {
      MessageTipManager.Instance.show(
        LangManager.Instance.GetTranslation(
          "runeHoldEquipWnd.resolve.equipedTip",
        ),
      );
      return;
    }
   ...
   */
  private onPutoff() {
    this.control.reqRuneGemUnload(
      this.holeInfo.holeId,
      this.equipRuneList.RadioGroup.selectedIndex + 1,
    );
    this.putOffBtn.visible = false;
  }
  ...
  private onFilterSelected() {
    ...
    let profileFilter = (this.controler.data as SkillWndData)
      .runeProfileFilter2;
