...
export default class LoginInitDataAccept {
  ...
  private addEvent() {
    ...
    ServerDataManager.listen(
      S2CProtocol.U_C_LOAD_CASTLE,
      this,
      this.__loadCastleHandler,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_LOAD_BUILD,
      this,
      this.__loadBuildHandler,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_LOAD_BUILD_ORDER,
      this,
      this.__loadBuildOrderHandler,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_LOAD_FRIEND_LIST,
      this,
      this.__loadFriendListHandler,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_LOAD_WATER,
      this,
      this.__loadFarmHandler,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_SEND_SYSPROPERTY,
      this,
      this.__setSystemTime,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_PLAYER_AASREFRESH,
      this,
      this.__refreshIndygleTime,
    );
    ServerDataManager.listen(
    ...
    ServerDataManager.listen(
      S2CProtocol.U_C_SEND_VERSION,
      this,
      this.__versionHandler,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_ISTAKE_CHALLENGEREWARD,
      this,
      this.__challengeTakeHandler,
    );
    ServerDataManager.listen(
      S2CProtocol.U_CH_SNS_REQ,
      this,
      this.__snsReqHandler,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_ISTAKE_CROSS_SCORE_REWARD,
      this,
      this.__pvpTakeHandler,
    );
    ServerDataManager.listen(
  ...
  private nextStep(str: string) {
    Logger.info(
      str + " cost time-----------------------:" + new Date().getTime(),
    );
    this._current++;
    Logger.info(
      str + "==================================== cur : " + this._current,
    );
  ...
  public static loginOverRequest() {
    //登陆后再请求的数据
  ...
  private __challengeTakeHandler(pkg: PackageIn) {
    ...
    PlayerManager.Instance.currentPlayerModel.playerInfo.isChallReward =
      msg.isTake;
    PlayerManager.Instance.currentPlayerModel.playerInfo.challRewardLastRank =
      msg.param1;
  }
  ...
  private __snsReqHandler(pkg: PackageIn) {
    ...
    this.nextStep(
      "__snsReqHandler " +
        ...
        JSON.stringify(msg) +
        "---" +
        PlayerManager.Instance.currentPlayerModel.playerInfo.showVersion.toString(),
    );
  }
  ...
  private __pvpTakeHandler(pkg: PackageIn) {
    ...
    PlayerManager.Instance.currentPlayerModel.playerInfo.canAcceptCrossScoreAward =
      msg.isTake;
  ...
  private __versionHandler(pkg: PackageIn) {
    ...
    PlayerManager.Instance.currentPlayerModel.playerInfo.showVersion =
      msg.openVersion;
    PlayerManager.Instance.currentPlayerModel.playerInfo.showStory =
      msg.openStory;
    PlayerManager.Instance.currentPlayerModel.playerInfo.showStoryModule =
      msg.module;
    this.nextStep(
      "__versionHandler: " +
        " msg :" +
  ...
  private __setSystemTime(pkg: PackageIn) {
    ...
    PlayerManager.Instance.currentPlayerModel.sysCurtime = DateFormatter.parse(
      msg.sysCurtime,
      "YYYY-MM-DD hh:mm:ss",
    );
    PlayerManager.Instance.currentPlayerModel.sysStartTime =
      DateFormatter.parse(msg.sysStarTime, "YYYY-MM-DD hh:mm:ss");
    this.nextStep(
      "__setSystemTime: " +
        " msg :" +
        JSON.stringify(msg) +
        " --- " +
        PlayerManager.Instance.currentPlayerModel.sysCurtime.toString(),
    );
  ...
  private __refreshIndygleTime(pkg: PackageIn) {
    ...
    this.nextStep(
      "__refreshIndygleTime: " +
        " msg :" +
        JSON.stringify(msg) +
        " --- " +
        PlayerManager.Instance.currentPlayerModel.indulgeTime.toString(),
    );
  }
  ...
  private __colosseumTimeHandler(pkg: PackageIn) {
    ...
    this.nextStep(
      "__colosseumTimeHandler: " + " msg :" + msg + info.remainTime.toString(),
    );
    // 确保冷却队列已经实例化
    // QueueBar.Instance;
  }
  ...
  private __loadFriendListHandler(pkg: PackageIn) {
    let msg: FriendRelationList = pkg.readBody(
      FriendRelationList,
    ) as FriendRelationList;

    let relationList = [];
    let GroupList = [];
    for (const key in msg.relationList) {
      if (msg.relationList.hasOwnProperty(key)) {
        let fInfo: RelationPlayerMsg = msg.relationList[
          key
    ...
    this.nextStep(
      "::" +
        msg.maxPage +
        "__loadFriendListHandler : " +
        " msg :" +
        msg +
        LoginInitDataAccept.total +
        " ",
    );
  }
  ...
  private __loadFarmHandler(pkg: PackageIn) {
    ...
    if (msg.farm && msg.farm.treeInfo) {
      let fInfo: FarmInfo = FarmManager.Instance.model.getFarmInfo(
        msg.farm.userId,
      );
      if (fInfo) {
    ...
    msg.friendFarmState.forEach((fsMsg) => {
      let fsInfo: FriendFarmStateInfo =
        FarmManager.Instance.model.getFarmStateInfo(fsMsg.userId);
      fsInfo.canSteal = fsMsg.isReap;
      if (this.thane.userId == fsMsg.userId) {
        Logger.xjy("更新自己神树状态");
      }
      Logger.xjy(
        "[LoginInitDataAccept]__getFriendFarmStateHandler",
        this.thane.userId,
        fsMsg.userId,
        fsMsg.isChargeEnergy,
      );
  ...
  private readFarmInfoMsg(info: FarmInfo, msg: any): FarmInfo {
    ...
    if (info.treeInfo) this.readTreeInfoMsg(info.treeInfo, msg.treeInfo);
    else {
  ...
  private readLandInfoMsg(
    ...
    info.plantTime = DateFormatter.parse(
      msg.plantingTime,
      "YYYY-MM-DD hh:mm:ss",
    );
    info.matureTime = DateFormatter.parse(
      msg.matureTime,
      "YYYY-MM-DD hh:mm:ss",
    );
    info.originMatureTime = DateFormatter.parse(
      msg.originMatureTime,
      "YYYY-MM-DD hh:mm:ss",
    );
    Logger.xjy(
      "[LoginInitDataAccept]readLandOperMsg",
      "pos=" + info.pos,
      "plantTime=" + info.plantTime,
      "matureTime=" + info.matureTime,
      "originMatureTime=" + info.originMatureTime,
      "sysCurTimeBySecond=" +
  ...
  private readTreeInfoMsg(info, msg: TreeInfoMsg): any {
    ...
    info.nextPickTime = DateFormatter.parse(
      msg.nextPickTime,
      "YYYY-MM-DD hh:mm:ss",
    );
    info.lastWaterTime = DateFormatter.parse(
      msg.lastWaterTime,
      "YYYY-MM-DD hh:mm:ss",
    );
  ...
  private __loadBuildOrderHandler(pkg: PackageIn) {
    ...
    msg.buildOrder.forEach((bInfo) => {
      ...
      oInfo.beginTime = DateFormatter.parse(
        bInfo.beginTime,
        "YYYY-MM-DD hh:mm:ss",
      );
