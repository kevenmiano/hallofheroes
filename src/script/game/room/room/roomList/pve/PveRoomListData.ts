...
export default class PveRoomListData extends FrameDataBase {
    ...
    private formatList(arr: t_s_campaignData[], type: number = RoomCampaignType.Multy) {
        ...
        arr.forEach(item => {
            ...
            obj["CampaignNameLang"] = item.CampaignNameLang + " (" + LangManager.Instance.GetTranslation("public.level2", str) + ")";
            if (type != RoomCampaignType.Multy) {
                obj["CampaignNameLang"] += " " + LangManager.Instance.GetTranslation("public.Activity");
            }
