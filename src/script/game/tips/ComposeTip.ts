    ...
    private updateView() {
        if (this._info) {
            ...
            if (player.composeList.indexOf(this._info.templateInfo.Property1) >= 0) {
                this.txt_study.text = LangManager.Instance.GetTranslation("yishi.view.tips.goods.ComposeTip.vStudy");
            }
            else {
                this.txt_study.text = "";
            }
            ...
            let needStr: string = "";
            let composeTemp: t_s_composeData = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_compose, this._info.templateInfo.Property1);
            if (composeTemp) {
                ...
                for (let i: number = 1; i < 5; i++) {
                    if (composeTemp["Material" + i] > 0) {
                        materialTemp = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_itemtemplate, composeTemp["Material" + i]);
                        needStr += (materialTemp.TemplateNameLang + "*" + composeTemp["Count" + i]);
                    }
                }
            }
            this.txt_need.text = LangManager.Instance.GetTranslation("yishi.view.tips.goods.ComposeTip.need") + needStr;
            ...
            if (BagHelper.isOpenConsortiaStorageWnd()) {//在公会仓库中
