...
export class NPCPatrolMediator implements IMediator, IEnterFrame {
  ...
  public register(target: object) {
    ...
    if (
      this._npc &&
      (<SpaceNode>this._npc.nodeInfo).info.types == SpaceNodeType.MOVEMENT
    ) {
      this._npc.aiInfo.moveState = AiStateType.NPC_RANDOM_MOVE_STATE;
      this.__moveStateHandler(null);
      this._npc.aiInfo.addEventListener(
        AiEvents.MOVE_STATE,
        this.__moveStateHandler.bind(this),
      );
  ...
  public unregister(target: object) {
    this._npc.aiInfo.removeEventListener(
      AiEvents.MOVE_STATE,
      this.__moveStateHandler.bind(this),
    );
