...
export class ConsortiaUpgradeWnd extends BaseWindow {
    ...
    private initEvent() {
        ...
        this._data.addEventListener(ConsortiaEvent.UPDA_COOLDOWN_TIME, this.__onCooldownTimeUpdata, this);
    }
    ...
    private initList() {
        for (let index = 0; index < this.list.numItems; index++) {
            ...
            switch (index) {
                case 0:
                    info = new ConsortiaUpgradeInfo(index, ConsortiaUpgradeType.CONSORTIA, this.consortiaInfo.levels)
                    break;
                case 1:
                    info = new ConsortiaUpgradeInfo(index, ConsortiaUpgradeType.SAFE_DEPOSIT_BOX, this.consortiaInfo.storeLevel)
                    break;
                case 2:
                    info = new ConsortiaUpgradeInfo(index, ConsortiaUpgradeType.CONSORTIA_ALTAR, this.consortiaInfo.altarLevel)
                    break;
                case 3:
                    info = new ConsortiaUpgradeInfo(index, ConsortiaUpgradeType.LING_SHI, this.consortiaInfo.schoolLevel)
                    break;
                case 4:
                    info = new ConsortiaUpgradeInfo(index, ConsortiaUpgradeType.CONSORTIA_SHOP, this.consortiaInfo.shopLevel)
                    break;
                default:
                    break;
            }
        ...
        this.itemCurLevel.getChild("txtName").text = LangManager.Instance.GetTranslation("Consortia.Upgrade.Introductions")
        this.itemNextLevel.getChild("txtName").text = LangManager.Instance.GetTranslation("Consortia.Upgrade.NextLevel")
        this.itemUpgradeRequirements.getChild("txtName").text = LangManager.Instance.GetTranslation("Consortia.Upgrade.Demand")
        this.itemUpgradeConsume.getChild("txtName").text = LangManager.Instance.GetTranslation("Consortia.Upgrade.Consume")
        this.itemUpgradeCool.getChild("txtName").text = LangManager.Instance.GetTranslation("Consortia.Upgrade.CoolTime")
    ...
    private btnQuickCoolClick() {
        let cfgValue = 1;
        let cfgItem = TempleteManager.Instance.getConfigInfoByConfigName("ConsortiaQuic_Price");
        if (cfgItem) {
            cfgValue = Number(cfgItem.ConfigValue);
        }
        let point: number = cfgValue * Math.ceil(this._data.timeLeft / 600);
        let content: string = LangManager.Instance.GetTranslation("consortia.view.myConsortia.ConsortiaFrameTopView.quickBtnAlertTipData", point);
        SimpleAlertHelper.Instance.Show(SimpleAlertHelper.USEBINDPOINT_ALERT, { point: point ,checkDefault:true}, null, content, null, null, (b: boolean, check: boolean, data: any) => {
            if (b) this._contorller.sendQuickTime(check);
        });
    }

    private btnUpgradeClick(evt: Event) {
        let tInfo: t_s_consortialevelData = TempleteManager.Instance.getConsortiaTempleteByTypeAndLevel(this._currentBtnInfo.type, this._currentBtnInfo.grade + 1);
        if (this.consortiaInfo.offer < tInfo.NeedOffer) {
            var content = LangManager.Instance.GetTranslation("consortia.helper.ConsortiaUpgradeHelper.content");
        ...
        this._contorller.consortiaUpgrade(this._currentBtnInfo.type);
    }
    ...
    private refreshCoolTime() {
        ...
        if (timeLeft > 0) {
            this.txtCountDown.text = LangManager.Instance.GetTranslation("Consortia.Upgrade.UpgradeCoolDownTime") + DateFormatter.getConsortiaCountDate(timeLeft);
    ...
    private refreshGrade() {
        for (let index = 0; index < this.list.numItems; index++) {
            ...
            element.text = LangManager.Instance.GetTranslation("public.level2", vData.grade);
        }
    }
    ...
    private refreshView() {
        switch (this._currentBtnInfo.type) {
            case ConsortiaUpgradeType.CONSORTIA:
                this.txtUpgradeTitle.text = LangManager.Instance.GetTranslation("Consortia.Upgrade.GradeTitle");
                break;
            case ConsortiaUpgradeType.SAFE_DEPOSIT_BOX:
                this.txtUpgradeTitle.text = LangManager.Instance.GetTranslation("Consortia.Upgrade.BagTitle");
                break;
            case ConsortiaUpgradeType.CONSORTIA_ALTAR:
                    this.txtUpgradeTitle.text = LangManager.Instance.GetTranslation("Consortia.Upgrade.AltarTitle");
                    break;
            case ConsortiaUpgradeType.LING_SHI:
                    this.txtUpgradeTitle.text = LangManager.Instance.GetTranslation("Consortia.Upgrade.SkillTitle");
                    break;
            case ConsortiaUpgradeType.CONSORTIA_SHOP:
                this.txtUpgradeTitle.text = LangManager.Instance.GetTranslation("Consortia.Upgrade.ShopTitle");
                break;
        }
        //当前等级
        this.itemCurLevel.text = this._currentBtnInfo.templete ? this._currentBtnInfo.templete.DescriptionLang : "";
        
        if (this.checkMaxLevel()) return;

        //下一等级
        let tInfo = TempleteManager.Instance.getConsortiaTempleteByTypeAndLevel(this._currentBtnInfo.type, this._currentBtnInfo.grade + 1);
        this.itemNextLevel.text = tInfo.DescriptionLang;

        // 前置要求
        this._satisfy = true;
        let preTemp: t_s_consortialevelData = TempleteManager.Instance.getConsortiaTempleteById(tInfo.PreTemplateId);
        if (preTemp) {
            let c = this.itemUpgradeRequirements.getController("showColor")
            if (this.consortiaInfo.getLevelByUpgradeType(preTemp.Types) < preTemp.Levels) {
                c.selectedIndex = 1;
                this._satisfy = false;
            } else {
                c.selectedIndex = 0;
            }
            this.itemUpgradeRequirements.title = LangManager.Instance.GetTranslation("consortia.view.myConsortia.chairmanPath.ConsortiaUpgradeFrame.needed", preTemp.LevelNameLang, preTemp.Levels);
        ...
        //消耗财富
        this.itemUpgradeConsume.getController("showColor").selectedIndex = this.consortiaInfo.offer < tInfo.NeedOffer ? 1 : 0;
        ...
        //需要时间
        this.itemUpgradeCool.text = DateFormatter.getConsortiaCountDate(tInfo.CodeTime)
    ...
    private removeEvent() {
        ...
        this._data.removeEventListener(ConsortiaEvent.UPDA_COOLDOWN_TIME, this.__onCooldownTimeUpdata, this);
    }
