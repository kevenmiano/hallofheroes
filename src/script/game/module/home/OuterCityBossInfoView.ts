...
export class OuterCityBossInfoView extends FUI_OuterCityBossInfoView {
    ...
    constructor() {
        super();
        if(TaskManage.Instance.IsTaskFinish(TaskManage.SETARMY_TASK) && SceneManager.Instance.currentType != SceneType.OUTER_CITY_SCENE){
            this._outerCityModel = new OuterCityModel()
            OuterCityManager.Instance.steup(new OuterCityScene(), this._outerCityModel);
    ...
    private initData() {
       ...
       (this.location.getChild("posTxt") as fgui.GTextField).color = ColorConstant.GOLD_COLOR;
        this.bossInfoTxt.getChild("posTxt").text = LangManager.Instance.GetTranslation("OuterCityBossInfoView.bossInfoTxt");
    }

    private addEvent() {
        NotificationManager.Instance.addEventListener(OuterCityEvent.OUTER_CITY_GET_BOSSINFO, this.onGetBossInfo, this);
        NotificationManager.Instance.addEventListener(OuterCityEvent.OUTER_CITY_UPDATE_BOSS_TIME, this.onUpdateBossTime, this);
        NotificationManager.Instance.addEventListener(OuterCityEvent.UPDATE_TREASURE_INFO,this.updateTreasureInfo,this);
        this.btn_help.onClick(this, this.onHelpClick);
        this.bossTreasureList.itemRenderer = Laya.Handler.create(this, this.renderTreasureList, null, false);
        ...
        BuildingManager.Instance.addEventListener(BuildingEvent.U_SECURITY_INFO_UPDATE,this.updateResourceTxt,this);
    }

    private removeEvent() {
        NotificationManager.Instance.removeEventListener(OuterCityEvent.OUTER_CITY_GET_BOSSINFO, this.onGetBossInfo, this);
        NotificationManager.Instance.removeEventListener(OuterCityEvent.OUTER_CITY_UPDATE_BOSS_TIME, this.onUpdateBossTime, this);
        ...
        NotificationManager.Instance.removeEventListener(OuterCityEvent.UPDATE_TREASURE_INFO,this.updateTreasureInfo,this);
        this.location.offClick(this,this.resourceLocationTxtHandler);
        this.bossInfoTxt.offClick(this,this.openBossInfo);
        BuildingManager.Instance.removeEventListener(BuildingEvent.U_SECURITY_INFO_UPDATE,this.updateResourceTxt,this);
    }
    ...
    private updateView(): void {
        ...
        for (i = 0; i < 3; i++) {
            ...
            this["_enemyCountTxt" + i].text = LangManager.Instance.GetTranslation("fish.FishFrame.countText", count, this._model.bossInfo["canKillCount" + (i + 1)]);
            if(count == this._model.bossInfo["canKillCount" + (i + 1)]){
    ...
    private updateResourceTxt(){
        if(this._model){
            ...
            this.location.getChild("posTxt").text = this._count + "/" + this._model.getPlayerCanOccupyMineMaxCount(ArmyManager.Instance.thane.grades);
        }
    }

    private onHelpClick() {
        let title: string = LangManager.Instance.GetTranslation("public.help");
        let content: string = LangManager.Instance.GetTranslation("map.outercity.view.helpFrame.helpContent.boss");
        UIManager.Instance.ShowWind(EmWindow.Help, { title: title, content: content });
    }

    private onHelp2Click(){
        let title: string = LangManager.Instance.GetTranslation("public.help");
        let content: string = LangManager.Instance.GetTranslation("map.outercity.view.helpFrame.helpContent");
        UIManager.Instance.ShowWind(EmWindow.Help, { title: title, content: content });
    }
