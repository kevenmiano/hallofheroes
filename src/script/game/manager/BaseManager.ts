...
export class BaseManager extends GameEventDispatcher {
  ...
  public setup() {
    ServerDataManager.listen(
      S2CProtocol.U_C_RELOADDIRTY,
      this,
      this.__reloadAgentDirtyHandler,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_WORLD_PROSPERITY,
      this,
      this.__getWorldProsperityHandler,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_WORLDBOSS_REPORT,
      this,
      this.__worldBossReportHandler,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_DATARESET,
      this,
      this.__resetDataHandler,
    );
    ServerDataManager.listen(
    ...
    SocketManager.Instance.addEventListener(
      Laya.Event.CLOSE,
      this.__socketCloseHandler,
      this,
    );
    ServerDataManager.listen(
      S2CProtocol.U_G_LOGIN_OTHER,
      this,
      this.__loginOtherHandler,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_LOCK_SCREEN,
      this,
      this.__lockSceneHandler,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_CAMPAIGN_ROOM_CREATE,
      this,
      this.__createRoomHandler,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_CAMPAIGN_ROOM_INVITE,
      this,
      this.__roomInviteHandler,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_CAMPAIGN_CREATE,
      this,
      this.__createCampaignHandler,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_GAME_PLOT,
      this,
      this.__gamePlotHandler,
    );
    ServerDataManager.listen(S2CProtocol.U_C_MSG, this, this.__msgHandler);
    ServerDataManager.listen(
      S2CProtocol.U_C_CHANNEL_ALERT,
      this,
      this.__serverBroadcastHandler,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_ARMY_CHANGESHARP,
      this,
      this.__armyChangeSharpHandler,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_ARMY_SUPPROT,
      this,
      this.__addPawnHandler,
    );
    ServerDataManager.listen(
      S2CProtocol.U_B_FIGHT_CANCELED,
      this,
      BaseManager.onFightCanceled,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_PLAYER_LOGINSTATEE,
      this,
      this.__loginStateHandler,
    );

    ServerDataManager.listen(
      S2CProtocol.U_C_CAMPAIGN_CARDS,
      this,
      this.__campaignCardsHandler,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_CAMPAIGN_TAKE_CARD,
      this,
      this.__campaingTakeCardHandler,
    );

    NotificationManager.Instance.addEventListener(
      MsgEventType.SESSION_OVER,
      this.__sessionOverHandler,
      this,
    );
    NotificationManager.Instance.addEventListener(
      S2CProtocol.U_C_GAME_PLOT,
      this.__gamePlotActionHandler,
      this,
    );
    NotificationManager.Instance.addEventListener(
      S2CProtocol.U_C_MSG,
      this.__showMsgHandler,
      this,
    );
    ...
    ServerDataManager.listen(
      S2CProtocol.U_C_WORLDBOSS_LOAD,
      this,
      this.__worldBossLoadHandler,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_SECURITY_CODE,
      this,
      this.__securityCodeHandler,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_SWITCH,
      this,
      this.__switchesUpdateHandler,
    );

    ServerDataManager.listen(
      S2CProtocol.U_C_TOWER_REPORT,
      this,
      this.__towerReportHandler,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_GUILDWAR_STATE,
      this,
      this.__guildWarStateHandler,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_MATCH_STATE,
      this,
      this.__requestHandler,
    );
    ServerDataManager.listen(
      S2CProtocol.U_WARFIELD_FORCE_QUIT,
      this,
      this.__forceQuitBattleCampaign,
    );
    ServerDataManager.listen(
      S2CProtocol.U_CHECK_WARFIELD_COUNT_RS,
      this,
      this.__warFightRemainNumber,
    );
    ServerDataManager.listen(S2CProtocol.U_CCC, this, this.__cccHandler);
    ServerDataManager.listen(
      S2CProtocol.U_C_VEHICLE_STATE,
      this,
      this.__vehicleStartHandler,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_LORDS_OPENSTATE,
      this,
      this.__warlordsOpenHandler,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_PLAYER_SPACE_CREATE,
      this,
      this.__createSpaceHandler,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_PLAYER_RETURN_TO_SPACE,
      this,
      this.__returnSpaceHandler,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_VIPMOUNT_STATE,
      this,
      this.__vipMountStateHandler,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_MINE_ACTIVE_STATE,
      this,
      this.__mineralHandler,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_PET_RANK_REWARD,
      this,
      this.__petRankRewardHandler,
    );
    ServerDataManager.listen(
    ...
    ServerDataManager.listen(
      S2CProtocol.U_C_VIPCUSTOM_NOTIFY,
      this,
      this.__vipCustomNotifyHandler,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_PVP_TIMER,
      this,
      this.__pvpTimerHandler,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_SEVEN_SIGN_INFO,
      this,
      this.sevenLoginDayInfo,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_GROWTH_FUND_INFO,
      this,
      this.growthFundInfo,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_RECOVER,
      this,
      this.onRecoverHandler,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_CROSSMULTI_MATCHSTATE,
      this,
      this.__crossMutiStateHandler,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_PETISLAND_BOSS_START,
      this,
      this.__petBossSwitchHandler,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_TREASURE_MINE_INFO,
      this,
      this.__treasureInfoHandler,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_UIPLAY_LIST,
      this,
      this.__uiPlayListInfo,
    );
  }

  private __petBossSwitchHandler(pkg: PackageIn) {
    let msg: PetIslandDefendStateMsg = pkg.readBody(
      PetIslandDefendStateMsg,
    ) as PetIslandDefendStateMsg;
    ...
    if (!petBossModel.isOpen) {
      ...
      ArmyManager.Instance.thane.grades >= OpenGrades.PET &&
      BaseManager.petBossMapId != petBossModel.mapId
    ) {
      DelayActionsUtils.Instance.addAction(
        new AlertTipAction({ message: "" }, this.__petBossOpenHandler),
      );
    }
  ...
  private __petBossOpenHandler(result: string): void {
    ...
    data.title = LangManager.Instance.GetTranslation(
      "yishi.manager.BaseManager.tip.title",
    );
    data.content = LangManager.Instance.GetTranslation(
      "yishi.manager.BaseManager.PetBossIsOpen",
    );
    TaskTraceTipManager.Instance.cleanByType(TipMessageData.PET_BOSS);
  ...
  private onRecoverHandler(pkg: PackageIn) {
    let infosMsg: RecoverInfosMsg = pkg.readBody(
      RecoverInfosMsg,
    ) as RecoverInfosMsg;
  ...
  private growthFundInfo(pkg: PackageIn) {
    let msg: GrowthFundDataInfoRsp = pkg.readBody(
      GrowthFundDataInfoRsp,
    ) as GrowthFundDataInfoRsp;
    if (!this.ctrlData) return;
    let basePackageArr: Array<GrowthFundItemInfo> =
      this.ctrlData.growthFundInfoArr;
    ...
    PlayerManager.Instance.currentPlayerModel.playerInfo.hasBuyGrowthFund =
      msg.isPay == 1 ? false : true;
    PlayerManager.Instance.currentPlayerModel.playerInfo.todayHasClickGrowthFund =
      msg.isOpen == 1 ? false : true;
    for (let i = 0; i < len; i++) {
    ...
    //数组排序（可领取>不能领取>已经领取）
    basePackageArr = ArrayUtils.sortOn(
      basePackageArr,
      ["sortOrder", "grade"],
  ...
  private sevenLoginDayInfo(pkg: PackageIn) {
    let msg: SevenSignInfoRsp = pkg.readBody(
      SevenSignInfoRsp,
    ) as SevenSignInfoRsp;
    if (msg) {
      ...
      for (let i = 0; i < msg.rewardInfo.length; i++) {
        ...
        if (rewardInfo) {
          ...
          reward.status = this.ctrlData.getSevenLoginRewardStatus(
            reward.day,
            msg.rewardSite,
            msg.days,
          );
      ...
      rewardInfoArr = ArrayUtils.sortOn(
        rewardInfoArr,
        ["day"],
  ...
  private getSevenLoginEndTime(startTime: number) {
    ...
    this.ctrlData.sevenLoginEndTime =
  ...
  private __vipCustomNotifyHandler(pkg: PackageIn) {
    ...
    if (msg.param1 == 1) {
      let content: string = LangManager.Instance.GetTranslation(
        "yishi.view.tips.VIPCustomTipView.QQTips",
        msg.param4,
        msg.param5,
      );
  ...
  private __pvpTimerHandler(pkg: PackageIn) {
    let msg: TimerInfoListMsg = pkg.readBody(
      TimerInfoListMsg,
    ) as TimerInfoListMsg;
    ...
    if (b) {
          ...
          if (KingTowerManager.Instance.kingTowerInfo.isKingTowerOverMaxCount) {
            MessageTipManager.Instance.show(
              LangManager.Instance.GetTranslation(
                "kingtower.chat.invite.countNotEnough",
              ),
            );
            return true;
          }
      ...
      if (this.isInCampaign) {
        ...
        let checkTxt: string = LangManager.Instance.GetTranslation(
          "BaseManager.SimpleAlertHelper.tipText",
        );
        if (
          roomInfo &&
          roomInfo.mapTemplate &&
          roomInfo.mapTemplate.isKingTower
        ) {
          ...
          !this.isActivity &&
          PlayerManager.Instance.currentPlayerModel.playerInfo.multiCopyCount >=
            1
        ) {
          // 房主已进副本  默认勾选 且 显示勾选框
          SimpleAlertHelper.Instance.Show(
          ...
          roomInfo &&
          roomInfo.mapTemplate &&
          roomInfo.mapTemplate.isTaila
        ) {
          // 周副本(泰拉神庙)默认勾选 且 看情况显示勾选框,  也属于活动isActivity = true, 所以提前判断
          let type =
            this.playerInfo.tailaCount > 0
              ? SimpleAlertHelper.CHATINVIE_ALERT
              : SimpleAlertHelper.SIMPLE_ALERT;
          SimpleAlertHelper.Instance.Show(
            type,
            {
              ...
              roomInfo: roomInfo,
              checkDefault: this.playerInfo.tailaCount > 0,
            },
            prompt,
            content,
            confirm,
          ...
          this.isActivity &&
          PlayerManager.Instance.currentPlayerModel.playerInfo.trialCount >= 1
        ) {
          SimpleAlertHelper.Instance.Show(
            SimpleAlertHelper.CHATINVIE_ALERT,
            { checkRickText: checkTxt, roomInfo: roomInfo, checkDefault: true },
  ...
  private __mulityCampaignRequestHandler(pkg: PackageIn) {
    ...
    if (roomInfo.roomType == 1) {
      ...
      if (ArmyManager.Instance.thane.grades < OpenGrades.CHALLENGE) {
        MessageTipManager.Instance.show(
          LangManager.Instance.GetTranslation(
            "pvp.view.PvPMultiView.command01",
          ),
        );
        return;
      }
    ...
    if (roomInfo.campaignId == 0) {
      ...
      if (templateInfo.isKingTower) {
        ...
        mapName =
          templateInfo.CampaignNameLang + difficultyGrade + "_" + activity;
      } else {
        /**非王者之塔 */
        /**活动副本 */
    ...
    if (roomInfo.campaignId != 0) {
      content = LangManager.Instance.GetTranslation(
        "chat.view.ChatView.EnterRoomTips",
        mapName,
        roomInfo.id,
      );
    } else {
      /**竞技场 */
      content = LangManager.Instance.GetTranslation(
        "chat.view.ChatView.EnterArenaTips",
        roomInfo.id,
      );
    }
  ...
  private __vipMountStateHandler(pkg: PackageIn) {
    ...
    if (msg.param7 == true) {
      ...
      tip.content = LangManager.Instance.GetTranslation(
        "tasktracetip.view.VipMountActivateTipView.Content",
      );
      ...
      tip.content = LangManager.Instance.GetTranslation(
        "tasktracetip.view.VipMountLoseTipView.Content",
      );
      TaskTraceTipManager.Instance.showView(tip);
    }
  }
  ...
  private __createSpaceHandler(pkg: PackageIn) {
    ...
    SocketDataProxyManager.Instance.addCreateSceneQueue(
      pkg,
    ...
    SpaceManager.Instance.model.mapNodesData =
      SpaceTemplateManager.Instance.nodeDic.get(self.campaignId);
    SpaceManager.Instance.readMember(self);
    if (SwitchPageHelp.isEnterSpaceNow()) {
      Logger.base("[BaseManager]切换到天空之城场景");
      SceneManager.Instance.setScene(
        SceneType.SPACE_SCENE,
        { isShowLoading: false },
        true,
      );
  ...
  private __forceQuitBattleCampaign(pkg: PackageIn) {
    DelayActionsUtils.Instance.addAction(
      new AlertTipAction(null, this.quitBattleCampaignReport.bind(this)),
    );
  }
  private quitBattleCampaignReport(obj: object) {
    let str: string = LangManager.Instance.GetTranslation(
      "BaseManager.forceQuitBattleCampaign",
    );
    let prompt: string = LangManager.Instance.GetTranslation("public.prompt");
    SimpleAlertHelper.Instance.Show(
      SimpleAlertHelper.SIMPLE_ALERT,
  ...
  private __switchesUpdateHandler(pkg: PackageIn) {
    let msg: SwitchInfoListMsg = pkg.readBody(
      SwitchInfoListMsg,
    ) as SwitchInfoListMsg;
    ...
    for (let i = 0; i < msg.switches.length; i++) {
      ...
      switch (swmsg.switchType) {
        ...
        case 16: //战场
          ...
          ConfigManager.info.PVP_CAMPAIGN_BEGIN_DATE = DateFormatter.parse(
            swmsg.beginDate,
            "YYYY-MM-DD hh:mm:ss",
          );
          ConfigManager.info.PVP_CAMPAIGN_END_DATE = DateFormatter.parse(
            swmsg.endDate,
            "YYYY-MM-DD hh:mm:ss",
          );
          break;
        ...
        case 2010: //综合入口页签
          ...
          NotificationManager.Instance.dispatchEvent(
            SwitchEvent.COMPREHENSIVE_ENTRANCE,
          ); //综合入口页签
          break;
        case 2011: //综合入口-官网按钮
          ...
          NotificationManager.Instance.dispatchEvent(
            SwitchEvent.COMPREHENSIVE_WEBSITE,
          ); //综合入口-官网按钮
          break;
        case 2012: //综合入口-公众号
          ConfigManager.info.COMPREHENSIVE_OFFICIAL_ACCOUNTS = swmsg.isOpen;
          NotificationManager.Instance.dispatchEvent(
            SwitchEvent.COMPREHENSIVE_OFFICIAL_ACCOUNTS,
          ); //综合入口-公众号
          break;
        case 2013: //综合入口-游戏推送
          ...
          NotificationManager.Instance.dispatchEvent(
            SwitchEvent.COMPREHENSIVE_PUSH,
          ); //综合入口-客服弹窗
          break;
        case 2014: //综合入口-客服弹窗
          ConfigManager.info.COMPREHENSIVE_CUSTOMER = swmsg.isOpen;
          NotificationManager.Instance.dispatchEvent(
            SwitchEvent.COMPREHENSIVE_CUSTOMER,
          ); //综合入口-客服弹窗
          break;
        case 2015: //个人中心-隐私相关
        ...
        case 2018: //禁止发言
          ...
          ConfigManager.info.CHAT_FORBID_BEGIN_DATE = DateFormatter.parse(
            swmsg.beginDate,
            "YYYY-MM-DD hh:mm:ss",
          );
          ConfigManager.info.CHAT_FORBID_END_DATE = DateFormatter.parse(
            swmsg.endDate,
            "YYYY-MM-DD hh:mm:ss",
          );
          break;
  ...
  private __onGetResultCompleted(info: any, content: object) {
  ...
  private __worldBossLoadHandler(pkg: PackageIn) {
    let msg: WorldBossInfoLoadMsg = pkg.readBody(
      WorldBossInfoLoadMsg,
    ) as WorldBossInfoLoadMsg;
    let id: number =
    ...
    if (b && this.thane.grades >= 20)
      new PreLoadWorldBoss(msg.campaignId, msg.leftTime);
  }

  private __worldBossReportHandler(pkg: PackageIn) {
    let msg: WorldBossReportMsg = pkg.readBody(
      WorldBossReportMsg,
    ) as WorldBossReportMsg;
    if (msg.type == 1) {
      //世界boss结算
      let str: string = "";
      if (msg.totalWound == 0) {
        if (msg.order == 0) {
          str = LangManager.Instance.GetTranslation(
            "yishi.manager.BaseManager.str11",
            msg.gold,
          );
        } else {
          str = LangManager.Instance.GetTranslation(
            "yishi.manager.BaseManager.str12",
            msg.order,
            msg.gold,
          );
        }
        ...
        if (msg.order == 0) {
          str = LangManager.Instance.GetTranslation(
            "yishi.manager.BaseManager.str21",
            msg.totalWound,
            msg.gold,
          );
        } else {
          str = LangManager.Instance.GetTranslation(
            "yishi.manager.BaseManager.str22",
            msg.totalWound,
            msg.order,
            msg.gold,
          );
      ...
      DelayActionsUtils.Instance.addAction(
        new AlertTipAction({ message: str }, this.worldBossReport.bind(this)),
      );
    } else if (msg.type == 2) {
      //试练之塔结算
      let str3: string = LangManager.Instance.GetTranslation(
        "yishi.manager.BaseManager.str23",
        msg.order,
        msg.gold,
        msg.strengy,
      );
      DelayActionsUtils.Instance.addAction(
        new AlertTipAction(
          { message: str3 },
          this.worldBossReport,
          SceneType.PVE_ROOM_SCENE,
          0,
          this,
        ),
      );
  ...
  private worldBossReport(str: string, flag: boolean = true) {
    ...
    if (
      ...
      PlayerManager.Instance.scoreRatingCondition ==
        StoreRatingsType.BOSS_TERMINATOR_APPELL
    ) {
  ...
  private __resetDataHandler(pkg: PackageIn) {
    ...
    PlayerManager.Instance.currentPlayerModel.playerInfo.attackCount =
      msg.attackCount;
    PlayerManager.Instance.currentPlayerModel.playerInfo.seminaryCount =
      msg.blessingCount;
    PlayerManager.Instance.currentPlayerModel.playerInfo.seminaryEffect =
      msg.blessingBuff;
    let buildInfo: BuildInfo =
      BuildingManager.Instance.model.buildingListByID[-11];
    if (buildInfo) buildInfo.property1 = msg.transEnergy;
    let info: BuildingOrderInfo =
      BuildingManager.Instance.model.getOrderById(10000);
    if (info) {
    ...
    PlayerManager.Instance.currentPlayerModel.playerInfo.isGetKingBuffer =
      msg.king_Buff;
    PlayerManager.Instance.currentPlayerModel.playerInfo.diamondIndex =
      msg.UpRuneByPointTimes;
  ...
  private __addPawnHandler(pkg: PackageIn) {
    ...
    DelayActionsUtils.Instance.addAction(
      new AlertTipAction({}, this.__addPawnDelayHandler.bind(this)),
    );
  }
  private __addPawnDelayHandler(result: string) {
    ...
    data.title = LangManager.Instance.GetTranslation(
      "yishi.manager.BaseManager.data.title",
    );
    data.content = LangManager.Instance.GetTranslation(
      "yishi.manager.BaseManager.data.content",
    );
  ...
  private __armyChangeSharpHandler(pkg: PackageIn) {
    let cArmy: ArmyChangeSharpMsg = pkg.readBody(
      ArmyChangeSharpMsg,
    ) as ArmyChangeSharpMsg;
    ...
    if (RoomManager.Instance.roomInfo) {
      let rArmy: CampaignArmy = RoomManager.Instance.roomInfo.getPlayerByArmyId(
        cArmy.armyId,
        serverName,
      );
      if (rArmy) {
    ...
    if (CampaignManager.Instance.mapModel) {
      let bArmy: CampaignArmy =
        CampaignManager.Instance.mapModel.getBaseArmyByArmyId(
          cArmy.armyId,
          cArmy["serverName"],
        );
      if (bArmy) {
    ...
    if (SpaceManager.Instance.model) {
      let sArmy: SpaceArmy = SpaceManager.Instance.model.getBaseArmyByUserId(
        cArmy.armyId,
      );
      if (sArmy) {
  ...
  private _roomInvite(msg: RoomInviteMsg, quick: boolean = false) {
    ...
    if (msg.tempId) {
      // 获取副本对象
      campaign = ConfigMgr.Instance.getTemplateByID(
    ...
    let nickName: string = msg.nickName
      ? "[color=#00ff00]" + msg.nickName + "[/color]"
      : "";
    ...
    // 普通副本邀请
      ...
      let str: string = StringHelper.isNullOrEmpty(msg.name)
        ? nameMap
        : msg.name;
      str += " " + lvstr;
  ...
  private __createCampaignHandler(pkg: PackageIn) {
    ...
    if (!BattleManager.loginToBattleFlag) {
      let paraII: object;
      if (type == CreateCampaignType.CREATE_TYPE_TRANS)
        paraII = { isShowLoading: false };
      SceneManager.Instance.setScene(
        SceneType.CAMPAIGN_MAP_SCENE,
        paraII,
        true,
  ...
  private __lockSceneHandler(pkg: PackageIn) {
    //0x008D
    let msg: LockScreenRspMsg = pkg.readBody(
      LockScreenRspMsg,
    ) as LockScreenRspMsg;
  ...
  private lockScene(pkg: PackageIn) {
    ...
    NotificationManager.Instance.dispatchEvent(NotificationEvent.LOCK_SCENE, {
      lockTime: time,
      lockScene: scene,
    });
  }
  ...
  private __battleInfoResultHandler(pkg: PackageIn) {
    ...
    SocketSceneBufferManager.Instance.addPkgToBuffer(
      pkg,
      rtnScene,
      this.showAddBloodFrameBack.bind(this),
    );
  }
  private showAddBloodFrameBack(pkg: PackageIn) {
    let battleMsg: BattleReportMsg = pkg.readBody(
      BattleReportMsg,
    ) as BattleReportMsg;
    let rtnSceneList: any[] = [2, 8, 11, 19]; //这些场景不弹提示
    if (rtnSceneList.indexOf(battleMsg.battleScene) >= 0) return;
    DelayActionsUtils.Instance.addAction(
      new AlertTipAction(battleMsg.battleResult, this.showAlertTip.bind(this)),
    );
  }
    ...
    switch (msg.param1) {
      ...
      case WarlordsModel.PROCESS_BET:
        ...
        if (!PlayerManager.Instance.currentPlayerModel.hasShowPrelimReportWnd) {
          DelayActionsUtils.Instance.addAction(
            new AlertTipAction(null, this.showPrelimReportCall.bind(this)),
          );
  ...
  public showAlertTip(result: object) {
    ...
    if (
      this.thane.blood <= 0 &&
      this.thane.grades >= 10 &&
      this.thane.hp < this.thane.attackProrerty.totalLive
    ) {
      // if (TipsBar.Instance.messageBtnList.hasType(TipMessageData.BUY_BLOOD)) return;
      tip = new TipMessageData();
      tip.type = TipMessageData.BUY_BLOOD;
      tip.title = LangManager.Instance.GetTranslation(
        "yishi.manager.BaseManager.tip.title02",
      );
      TaskTraceTipManager.Instance.showView(tip);
    }
  ...
  private createOneDelayAction(result: string) {
    if (result == "") return;
    DelayActionsUtils.Instance.addAction(
      new AlertTipAction({ message: result }, this.showAllFallGoods.bind(this)),
    );
  }
  ...
  public get Message(): string {
    ...
    msg =
      ...
      CoreTransactionManager.getInstance().getMessage() +
      "\n";
    msg =
      msg +
      "GameBaseQueueManager : " +
      GameBaseQueueManager.Instance.getMessage() +
      "\n";
    msg =
      msg +
      "SocketSceneBufferManager : " +
      SocketSceneBufferManager.Instance.getMessage() +
      "\n";
    return msg;
  }
  ...
  public loadBackResource() {
    if (this._timeId2 == 0) {
      this._timeId2 = setTimeout(
        this.loadBackResource.bind(this),
        1000 * 60 * 30,
      );
      return;
    }
  ...
  private __treasureInfoHandler(pkg: PackageIn): void {
    ...
    let countDownDate: Date = Utils.formatTimeZone(
      Number(endData.getTime()),
      zoneOffset,
    ); //时区同步
    ...
    for (let i: number = 0; i < msg.minerals.length; i++) {
      let mineral: TreasureMineLandInfo = msg.minerals[
        i
      ] as TreasureMineLandInfo;
      currentMineralsList.push(this.readMineralInfo(mineral));
    }
    currentMineralsList = ArrayUtils.sortOn(
      currentMineralsList,
      ["type"],
      ArrayConstant.NUMERIC,
    );
    ...
    for (let i: number = 0; i < msg.consortiaMinerals.length; i++) {
      let mineral: TreasureMineLandInfo = msg.consortiaMinerals[
        i
      ] as TreasureMineLandInfo;
      conosrtMineralsList.push(this.readMineralInfo(mineral));
    }
    conosrtMineralsList = ArrayUtils.sortOn(
      conosrtMineralsList,
      ["type"],
      ArrayConstant.NUMERIC,
    );
  ...
  private __uiPlayListInfo(pkg: PackageIn) {
    let msg: UserUiPlayListMsg = pkg.readBody(
      UserUiPlayListMsg,
    ) as UserUiPlayListMsg;
    if (msg) {
