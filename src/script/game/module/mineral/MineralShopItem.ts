...
export default class MineralShopItem extends FUI_MineralShopItem {
    ...
    private removeEvent() {
        ...
        if (this.Btn_Exchange) this.Btn_Exchange.offClick(this, this.__clickHandler);
    }
    ...
    private __blurHandler() {
        ...
        let max: number = Math.floor(this.playerInfo.mineral / this._shopData.MineScore);
    ...
    protected __clickHandler(event: MouseEvent) {
        if (this.playerInfo.mineral < this._shopData.MineScore) {
            MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("view.subshop.MineralShopItem.notEnough"));
            return;
        }
        ...
        ShopManager.Instance.sendShoping(this._shopData.Id, Number(this.txt_num.text), this.playerInfo.userId, false);
        this.txt_num.text = "1";
    }

    private setEnable() {
        if (this._shopData.NeedMinGrade > this.thane.grades) {
            this.txt_Level.text = LangManager.Instance.GetTranslation("view.subshop.MineralShopItem.minLevel", this._shopData.NeedMinGrade);
    ...
    public set info(value: ShopGoodsInfo) {
        ...
        this.txt_Name.text = "[color='" + goodsInfo.templateInfo.profileColor + " ']" + goodsInfo.templateInfo.TemplateNameLang + "[/color]";

        this.count.text = this._shopData.MineScore + "";
        this.txt_num.text = "1";
        this.setEnable();
        let goods: t_s_itemtemplateData = TempleteManager.Instance.getGoodsTemplatesByTempleteId(this._shopData.ItemId);
        if (goods.SonType == 218) {//坐骑
