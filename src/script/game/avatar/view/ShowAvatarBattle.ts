...
export class ShowAvatarBattle extends Laya.Sprite implements Disposeable {
  ...
  constructor(
    type: HeroMovieClipRefType,
    showName: boolean = true,
    showWeakBreatheAction: boolean = true,
  ) {
    super();
    this._type = type;
  ...
  protected initView() {
    this._txtName = new FilterFrameText(
      10,
      20,
      undefined,
      186,
  ...
  protected refresh() {
    ...
    } else if (this._data instanceof GameLoadNeedData) {
      // 士兵、Boss、怪物
  ...
  resetView() {
    this.showName(false);
    this._movie && ObjectUtils.disposeObject(this._movie);
    this._movie = null;
  }
