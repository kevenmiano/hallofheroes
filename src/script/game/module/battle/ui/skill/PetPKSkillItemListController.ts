...
export default class PetPKSkillItemListController {
  ...
  private getSkills(petTemlateId: number): Array<any> {
    ...
    if (petTemlateId > 0) {
      ...
      for (let i: number = 0; i < 6; i++) {
        let skillTemplateData: t_s_skilltemplateData =
          petData.getChangeSkillTemplate(
            Number(fastKey[i]),
  ...
  private addEvent() {
    ...
    NotificationManager.Instance.addEventListener(
      NotificationEvent.UPDATE_SELECTED_PET,
      this.__updateSelectePet,
      this,
    );
    NotificationManager.Instance.addEventListener(
      BattleNotic.SPECIAL_SKILL_ENABLE,
      this.__specialSkillEnable,
      this,
    );
    NotificationManager.Instance.addEventListener(
      NotificationEvent.UPDATE_SELECTED_PET_STATUS,
      this.updateStatus,
      this,
    );
    NotificationManager.Instance.addEventListener(
  ...
  private updateStatus(petTemplateId: number) {
    ...
    let hero1: HeroRoleInfo =
      this.battleModel.getBaseRoleInfoByPetTemplateId(petTemplateId); //死亡的英灵
    let item: any;
  ...
  private updateBuffer() {
    this.view &&
      this.view.checkIsLockByPetInfo(this.battleModel.currentSelectedPet);
  }
  ...
  private __useBattleItemServerBack(pkg: PackageIn) {
    ...
    if (msg.result == 0) {
      ...
      if (flag) {
        ...
        let prop: BattlePropItem = this.battleModel.lookHero.getPropByID(
          msg.tempId,
        );
        if (prop) {
