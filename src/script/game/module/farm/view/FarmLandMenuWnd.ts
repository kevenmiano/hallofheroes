...
export class FarmLandMenuWnd extends BaseWindow {
  ...
  private btnAccelerateClick() {
    if (this._info) {
      if (this._info.hasAccelerate) {
        //检测是否已加速
        MessageTipManager.Instance.show(
          LangManager.Instance.GetTranslation(
            "farm.view.FarmOperMenu.hasAccelerateTip",
          ),
        );
        this.dispose();
        return;
      ...
      var tip: string = LangManager.Instance.GetTranslation(
        "farm.view.FarmOperMenu.accelerateTip",
        FarmManager.Instance.model.acceleratePay,
      );
      SimpleAlertHelper.Instance.Show(
        SimpleAlertHelper.SIMPLE_ALERT,
        { oper: FarmOperateType.ACCELERATE, land: this._info },
        null,
        tip,
        null,
        null,
  ...
  public set info(value: FarmLandInfo) {
    ...
    if (value) {
      ...
      this.btnAccelerate.title = LangManager.Instance.GetTranslation(
        value.hasAccelerate
          ? "yishi.view.tips.CropTip.accelerated"
          : "farm.view.FarmOperMenu.accelerate",
      );
      let pvalue = Math.floor(
        (value.growTime / value.totalMatureTimeUnAccelerate) * 100,
      );
      this.prog.value = isNaN(pvalue) || !pvalue ? 0 : pvalue;
      this.txtTimeValue.text = DateFormatter.getStopDateString(
        value.remainMatureTime,
      );
      ...
      if (gTemp) {
        ...
        if (parseInt((gTemp.Property1 / 60).toString()) <= 0)
          _tempStr = LangManager.Instance.GetTranslation(
            "public.needMinutes",
            gTemp.Property1 % 60,
          );
        ...
        else _tempStr = gTemp.Property3.toString();
