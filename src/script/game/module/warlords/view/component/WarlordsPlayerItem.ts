...
export default class WarlordsPlayerItem extends FUI_WarlordsPlayerItem {
    ...
    public set index(value: number) {
        ...
        var appellInfo: t_s_appellData = TempleteManager.Instance.getAppellTemplateByID(appellId);
        if (appellInfo) {
            this._honerView = new AppellView(appellInfo.ImgWidth, appellInfo.ImgHeight, appellId);
    ...
    public set info(vInfo: ThaneInfo) {
        ...
        if (this._info) {
            ...
            if (this._honerView) this._heroFigureCon.displayObject.addChild(this._honerView);
        } else {
            this.c1.selectedIndex = this._index - 1;
            this._heroFigure.data = null;
            this.nameTxt.text = LangManager.Instance.GetTranslation("Warlords.WarlordsPlayerItem.nameTxt");
