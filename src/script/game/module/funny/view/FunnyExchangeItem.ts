...
export default class FunnyExchangeItem extends FUI_FunnyExchangeItem {
    ...
    private initEvent() {
        ...
        this.exchangeList1.itemRenderer = Laya.Handler.create(this, this.renderGoodsListItem1, null, false);
        this.exchangeList2.itemRenderer = Laya.Handler.create(this, this.renderGoodsListItem2, null, false);
        this.exchangeList1.itemProvider = Laya.Handler.create(this, this.getListItemResource, null, false);
        this.exchangeList2.itemProvider = Laya.Handler.create(this, this.getListItemResource, null, false);
    }
    ...
    private onCheckHandler() {
        ...
        if (!warns) {
            let tipsText: string = "";
            if (checkState) {
                tipsText = LangManager.Instance.GetTranslation("funny.datas.FunnyBagData.alert1");
            } else {
                tipsText = LangManager.Instance.GetTranslation("funny.datas.FunnyBagData.alert2");
            }
            UIManager.Instance.ShowWind(EmWindow.TodayNotAlert, { state: 2, content: tipsText, backFunction: this.handlerAlertCB.bind(this), closeFunction: this.handlerAlertClose.bind(this) });
        } else {
    ...
    private isFashion(reward: FunnyRewardData): boolean {
        ...
        ginfo.strengthenGrade = reward.strengthenGrade <= 0 ? 1 : reward.strengthenGrade;
    ...
    renderGoodsListItem1(index: number, item: BaseItem) {
        ...
        if (infoData.templateId == ItemID.DIAMOND_PROP) {//钻石
            ownCount = this.playerModel.playerInfo.point;
            countVlaue = ownCount.toString();
        } else if (infoData.templateId == ItemID.GOLD_PROP) {//黄金
            ownCount = ResourceManager.Instance.gold.count;
            countVlaue = FormularySets.toStringSelf(ResourceManager.Instance.gold.count, HomeWnd.STEP).toString();
        } else {
            ownCount = FunnyHelper.getBagCount(infoData.templateId);
            countVlaue = ownCount.toString();
        }
        item.text = "[color=" + this.getLackColor(ownCount >= count) + "]" + countVlaue + "[/color]/" + count;
        if (item.isConsume)
        ...
        if (this._info) {
            ...
            if (this._info.conditionList[0].id == FunnyConditionType.EXCHANGE) {
                ...
                if (this._info.conditionList[0].bak2 > 0) {
                    this.txt_Count.text = this._info.getCount + "/" + this._info.conditionList[0].bak2.toString();
