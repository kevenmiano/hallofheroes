...
export default class MineralCarView extends NpcAvatarView {
  ...
  public avatarBaseViewType: eAvatarBaseViewType =
    eAvatarBaseViewType.MineralCar;
  ...
  attackFun(): boolean {
    ...
    if (this.inMineralMapAndNotInFight) {
      ...
      var attackPoint: Laya.Point = NpcAttackHelper.getAttackPoint(
        this.nodeInfo as CampaignNode,
  ...
  private get inMineralMapAndNotInFight(): boolean {
    return (
      ...
      ArmyState.checkCampaignAttack(this._army.state)
    );
  }
  ...
  protected __isDieHandler(evt) {
    ...
    if (
      CampaignArmyState.checkDied(this._army.isDie) ||
      !ArmyState.checkCampaignAttack(this._army.state)
    ) {
  ...
  private __updateCarInfo(event: CampaignEvent) {
    ...
    if (this._carInfo) {
      ...
      this._mineralTxt.text = LangManager.Instance.GetTranslation(
        "map.campaign.view.physics.MineralCarView.minerals",
        this._carInfo.minerals,
      );
  ...
  private get needUpdate(): boolean {
    var newInfo: MineralCarInfo = this.mineralModel.carInfos[
      this._owerId
    ] as MineralCarInfo;
    ...
    if (
      (this._carInfo.minerals == 0 && newInfo.minerals > 0) || //无矿--> 有矿
      (this._carInfo.minerals < 200 && newInfo.minerals >= 200)
    ) {
      //未满--> 满矿
  ...
  refreshAvatarView() {
    ...
    if (this._carInfo) {
      ...
      url = PathManager.getAvatarResourcePath(
        String(this._carInfo.quality + 1),
        -1,
        1,
        AvatarPosition.BODY,
        -1,
        AvatarResourceType.MINERAL_CAR,
      );
  ...
  private get isSelfConsortia(): boolean {
    if (this._army && this._army.baseHero) {
      if (StringHelper.isNullOrEmpty(this._army.baseHero.consortiaName))
        return false;
      ...
      if (this.mapModel.isCross) {
        ...
        return (
          this._army.baseHero.consortiaName == this.playerInfo.consortiaName &&
          this._army.baseHero.consortiaID == consortiaID
        );
      } else {
        return (
          this._army.baseHero.consortiaName == this.playerInfo.consortiaName
