...
export default class DialogWnd extends BaseWindow {
    ...
    private swapHeadImg(curInfo: DialogMessageInfo) {
        ...
        ResMgr.Instance.loadRes(url, () => {
            ...
            if (res && tempSp && !tempSp.isDisposed) {
                tempSp.url = url;
                Laya.Tween.to(tempSp, { alpha: 1, colorTransform: { tint: 0x000000, tintAmount: 0.01 } }, 0.5);
                if (curInfo.roleId == 21) {//特殊图片特殊处理
    ...
    public showMessage(info: DialogMessageInfo) {
        ...
        if (this._isAuto) {
            this._autoTimeId = setTimeout(this.defAutoClickToNextHandler.bind(this), this.delayTime * 1000);
        }
    }
    ...
    private addSentenceTxt() {
        ...
        if (PlayerManager.Instance.currentPlayerModel.playerInfo.nickName == PlayerManager.Instance.currentPlayerModel.userInfo.userId + "$") {
            this._sentence = this._sentence.replace(reg, LangManager.Instance.GetTranslation("public.nickName"));
        }
        else {
            this._sentence = this._sentence.replace(reg, PlayerManager.Instance.currentPlayerModel.playerInfo.nickName);
        }
        ...
        if (list) {
            ...
            if (this._sentence == storyXmlList[0].txts.item[1]["txt_" + langKey]) {
                ...
                this._guardImge = FUIHelper.createFUIInstance(EmPackName.Dialog, "asset.novice.GuardImage");
                if (CampaignManager.Instance.controller) {
                    var myArmy = CampaignManager.Instance.controller.getArmyView(CampaignManager.Instance.mapModel.selfMemberData);
        ...
        this._speakHandler = new DialogWordHandler(this.typerRichtext, "[color=#ffc68f][size=22]" + this._name + "[/size][/color]<br><br><br><br>" + "", this._sentence, this.speakEndHandler.bind(this));
    ...
    public triggerAutoClick() {
        ...
        Laya.timer.loop(DialogModel.QuickAutoTime, this, this.autoClickToNextHandler)
