...
export default class OutyardBlessWnd extends BaseWindow {
    ...
    private addEvent() {
        ...
        NotificationManager.Instance.addEventListener(NotificationEvent.OUTYARD_FULL_INFO, this.selfInfoHandler, this);
        NotificationManager.Instance.addEventListener(NotificationEvent.OUTYARD_SELF_INFO, this.selfInfoHandler, this);
        NotificationManager.Instance.addEventListener(NotificationEvent.OUTYARD_UPDATE_INFO, this.selfInfoHandler, this);
    }

    private removeEvent() {
        this.attackBtn.offClick(this, this.onAttackHandler);
        this.defenceBtn.offClick(this, this.onDefenceHandler);
        NotificationManager.Instance.removeEventListener(NotificationEvent.OUTYARD_FULL_INFO, this.selfInfoHandler, this);
        NotificationManager.Instance.removeEventListener(NotificationEvent.OUTYARD_SELF_INFO, this.selfInfoHandler, this);
        NotificationManager.Instance.removeEventListener(NotificationEvent.OUTYARD_UPDATE_INFO, this.selfInfoHandler, this);
    }
    ...
    private selfInfoHandler() {
        ...
        this.attackCostTxt.text = LangManager.Instance.GetTranslation("outyard.OutyardBlessFrame.attackNoteTxt.text1", this._config1[0],this._config1[1]);
        this.defenceCostTxt.text = LangManager.Instance.GetTranslation("outyard.OutyardBlessFrame.attackNoteTxt.text2", this._config2[0],this._config2[1]);
    }
    ...
    private checkBlessing(type: number): boolean {
        ...
        if (type == 1 && this._config1[2] <= myGuild.attackBuffLevel) {
            MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("outyard.StackHead.Buff.AttackMax"));
            return false;
        }
        if (type == 2 && this._config2[2] <= myGuild.defenceBuffLevel) {
            MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("outyard.StackHead.Buff.DefenceMax"));
            return false;
        }
        if (stateMsg.state == 2) {//备战中
            MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("outyard.OutyardBlessFrame.cannotBlessingHandler"));
            return false;
        }
        if (stateMsg.state == 3) {//开战中
            if ((type == 1 && this._config1[0] > selfMsg.actionPoint) || (type == 2 && this._config2[0] > selfMsg.actionPoint)) {
                MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("outyard.OutyardBlessFrame.executionNotHandler"));
                return false;
            }
        }
        if (stateMsg.state == 4) {
            MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("outyard.OutyardBlessFrame.balanceHandler"));
            return false;
        }
        if (stateMsg.state == 5) {
            MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("outyard.OutyardBlessFrame.overHandler"));
            return false;
        }
        return true;
    }
