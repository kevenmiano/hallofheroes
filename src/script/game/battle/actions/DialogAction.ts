...
export class DialogAction extends MapBaseAction {
    ...
    private updateImp() {
        var dialogCon: DialogControl = new DialogControl(this.actionOver.bind(this));
    ...
    private readPackage(): DialogModel {
        ...
        for (var i: number = 0; i < msg.info.length; i++) {
            ...
            if ((msg.info[i] as PlotInfoMsg).roleName == "%s") {
                (msg.info[i] as PlotInfoMsg).roleName = PlayerManager.Instance.currentPlayerModel.playerInfo.nickName
                if (dInfo.roleId == 0) {
                    dInfo.roleId = PlayerManager.Instance.currentPlayerModel.playerInfo.pics;
                }
            }
            if ((msg.info[i] as PlotInfoMsg).roleName == PlayerManager.Instance.currentPlayerModel.userInfo.userId + "$") {
