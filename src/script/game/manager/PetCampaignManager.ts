...
export class PetCampaignManager extends GameEventDispatcher {
  ...
  public addEvent() {
    ServerDataManager.listen(
      S2CProtocol.U_C_UIPLAY_INFO,
      this,
      this._playInfoUpdate,
    );
  }
  ...
  private _playInfoUpdate(pkg: PackageIn) {
    let msg: UserUiPlayInfoMsg = pkg.readBody(
      UserUiPlayInfoMsg,
    ...
    if (msg) {
      ...
      let len: number =
        PetCampaignManager.Instance.model.userUiPlayListMsg.uiPlayInfoList
          .length;
      for (let i: number = 0; i < len; i++) {
        let item: UserUiPlayInfoMsg = PetCampaignManager.Instance.model
          .userUiPlayListMsg.uiPlayInfoList[i] as UserUiPlayInfoMsg;
      ...
      if (!flag && !msg.itemAdd) {
        PetCampaignManager.Instance.model.userUiPlayListMsg.uiPlayInfoList.push(
          msg,
        );
      }
