...
export class OuterCityBorderLayer extends Sprite implements Disposeable {
  ...
  private init(): void {
    this._verticalBorder = fgui.UIPackage.createObject(
      EmWindow.OuterCity,
      "BorderMaskVAsset",
    ).asImage;
    this._horizontalBorder = fgui.UIPackage.createObject(
      EmWindow.OuterCity,
      "BorderMaskHAsset",
    ).asImage;

    StageReferance.addEventListener(
      Laya.Event.RESIZE,
  ...
  public update(off: number = 0): void {
    ...
    if (mapView.y > -150) {
      ...
      this._horizontalBorder.y =
        mapTemp.Height - this._horizontalBorder.height + off;
      this._horizontalBorder.x = -mapView.x - StageReferance.stageWidth;
      this._horizontalBorder.scaleY = 1;
      this.addChild(this._horizontalBorder.displayObject);
    } else {
  ...
  public dispose(): void {
    StageReferance.removeEventListener(
      Laya.Event.RESIZE,
      this.__stageResizeHandler,
      this,
    );
