...
export class RemotePetListView extends FUI_RemotePetListView {
    ...
    public init(selectPetView: RemotePetHeadSelectView, selectFriendView: RemotePetFriendView) {
    ...
    private initView() {
        this.headItemsView = [this.h1 as RemotePetHeadItemView, this.h2 as RemotePetHeadItemView, this.h3 as RemotePetHeadItemView];
        this.skillItemsView = [this.s1 as RemotePetSkillsView, this.s2 as RemotePetSkillsView, this.s3 as RemotePetSkillsView];

        for (let i = 0; i < this.headItemsView.length; i++) {
            //7 为好友英灵
            this.headItemsView[i].pos = i == 3 ? 7 : i + 1;
            this.headItemsView[i].onClick(this, this.onHeadClick, [this.headItemsView[i]]);
    ...
    private onHeadClick(head: RemotePetHeadItemView) {
        ...
        this._selectPetView.show(this.playerInfo.petList, head.petData, this.selectPet.bind(this))
    ...
    private onDeleteClick(index: number) {
        //好友英灵 无法更改
        if (index == 3) {
            MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("remotepet.RemotePetFriend.notmove"));
            return;
        }
    ...
    private addEvent() {
        ...
        this.model.addEventListener(RemotePetEvent.COMMIT, this.petChangeHandler, this);
    }

    private removeEvent() {
        this.model.removeEventListener(RemotePetEvent.PET_CHANGE, this.petChangeHandler, this);
        this.model.removeEventListener(RemotePetEvent.COMMIT, this.petChangeHandler, this);
    }

    private updateSelectFlag(selectItem: RemotePetHeadItemView = this._curSelectItem) {
