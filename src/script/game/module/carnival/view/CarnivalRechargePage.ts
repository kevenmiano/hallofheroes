 ...
 * 嘉年华---充值有礼
 */
export default class CarnivalRechargePage
  extends FUI_CarnivalRechargePage
  implements CarnivalBasePage
{
  ...
  private initTranslate() {
    this.tenBtn.title = LangManager.Instance.GetTranslation(
      "Carnival.CarnivalRecharge.tenBtn",
    );
    this.oneBtn.title = LangManager.Instance.GetTranslation(
      "Carnival.CarnivalRecharge.oneBtn",
    );
  }

  private initView() {
    this.Img_CarnivalRecharge_Bg.url = this.model.getThemeFolderImgPath(
      CARNIVAL_FOLDER.RECHARGE,
      "Img_CarnivalRecharge_Bg",
    );
    this.Img_Txt_Carnival_Recharge.url = this.model.getThemeFolderImgPath(
      CARNIVAL_FOLDER.RECHARGE,
      "Img_Txt_Carnival_Recharge" + "_" + getDefaultLangageKey(),
    );
    this.Img_CarnivalRecharge_Frame.url = this.model.getThemeFolderImgPath(
      CARNIVAL_FOLDER.RECHARGE,
      "Img_CarnivalRecharge_Frame",
    );
    this.addOutItems();
    this.addInItems();
  }
  ...
  private addOutItems() {
    let outDataList = TempleteManager.Instance.getCarnivalLuckDrawTempList(
      1,
      1,
    );
    ...
    this._outLightCc = new LightTurnController(
      this._outDic,
      5,
      2000,
      35,
  ...
  private addInItems() {
    ...
    this._inLightCc = new LightTurnController(
      this._inDic,
      5,
      1500,
      35,
  ...
  private showRewardTipInfo() {
    ...
    while ((temp = this._tipPools.pop())) {
      if (temp) {
        ...
        let gTemp = TempleteManager.Instance.getGoodsTemplatesByTempleteId(
          temp.Item,
        );
        if (gTemp) {
          ...
          let rewardStr = LangManager.Instance.GetTranslation(
            "carnival.recharge.get",
            gTemp.TemplateNameLang,
            temp.ItemNum,
          );
  ...
  private clickHandler(v, e, c: number[]) {
    ...
    CarnivalManager.Instance.opRequest(
      CarnivalManager.OP_CHARGE_LOTTERY,
      0,
      count,
    );
  }
  ...
  protected updateInfoHandler() {
    ...
    for (let rewardId of restIds) {
      let temp =
        TempleteManager.Instance.getCarnivalLuckDrawTempInfo(+rewardId);
  ...
  private refreshView() {
    ...
    this.carnival_recharge.txt_title.text = LangManager.Instance.GetTranslation(
      "carnival.allRecharge",
    );
    this.carnival_recharge.txt_value.text =
      " " + this.model.totalCharge.toString();
