...
export class PetChallengeRankRewardItem extends FUI_PetChallengeRankRewardItem {
  ...
  public set info(rankIdx: number) {
    ...
    if (rankIdx) {
      let appelTmp = this.control.data.rewardCate[
        PetChallengeRewardType.APPELL.toString() +
          PetChallengeTimeRewardType.WeekReward.toString() +
          rankIdx.toString()
      ] as PetChallengeRewardTemplate;
      this.txtAppellValue.text = appelTmp.rewardName;
      let buffTmp = this.control.data.rewardCate[
        PetChallengeRewardType.BUFF.toString() +
          PetChallengeTimeRewardType.WeekReward.toString() +
          rankIdx.toString()
      ] as PetChallengeRewardTemplate;
      this.txtAttrValue.text = buffTmp.rewardName;
      this.imgRank.url = fgui.UIPackage.getItemURL(
        EmPackName.Base,
        RankData.RankRes[rankIdx - 1],
      );
    } else {
    }
  }
