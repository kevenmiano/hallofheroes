...
export class SocketGameReader {
    ...
    public static readBufferII(msg: AttackOrderMsg): BufferDamageData[] {
        ...
        for (let i: number = 0; i < size; i++) {
            ...
            if (Logger.openBattleBuffCon(buffer)) {
                Logger.info("XXX[socketGameReader]readBufferII bufferName=" + buffer.getBufferName() + ", processType=" + bufferMsg.seeType + ", templateId= " + bufferMsg.templateId + ", currentTurn = " + bufferMsg.curTurn + ", countWay = " + bufferMsg.curRound);
            }
        }
        return buffers;
    }
    ...
    public static readLookHeroRoleInfo(heroMsg: HeroMsg, petMsg: HeroMsg): HeroRoleInfo {
        let bloodA: number;
        let bloodB: number;
    ...
    public static readHeroRoleInfo(heroMsg: HeroMsg, petMsg: HeroMsg): HeroRoleInfo {
        let bloodA: number;
        let bloodB: number;
        let role: HeroRoleInfo = new HeroRoleInfo();
        role.petLivingId = heroMsg.livingId2;
    ...
    public static readReinforceBuffer(heroMsg: HeroMsg, role) {
        ...
        if (heroMsg.buffers) {
            ...
            for (let i: number = 0; i < length; i++) {
                ...
                Logger.yyz(heroMsg.nickName + "拥有buffer:::::::" + bufferMsg.templateId);
                buffer = new BufferDamageData();
    ...
    private static readTrialProps(heroMsg: HeroMsg): any[] {
        let arr: any[] = [];
        Logger.yyz("英雄: : : " + heroMsg.userId + "收到试练buff长度: : : " + heroMsg.trials.length);
    ...
    public static readSimpleAndSnsInfo(pInfo: FriendItemCellInfo, msg: RelationPlayerMsg) {
        ...
        pInfo.logOutTime = DateFormatter.parse(msg.player.logOutTime, "YYYY-MM-DD hh:mm:ss");
