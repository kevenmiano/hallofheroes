...
import { OuterCityEvent, SpaceEvent } from "../../constant/event/NotificationEvent";
...
export default class SmallMapBar extends BaseFguiCom {
    ...
    private addEvent() {
        ...
        CustomerServiceManager.Instance.addEventListener(CustomerServiceManager.NEW_REPLY, this.__newServiceReplyHandler, this);
    }
    ...
    private removeEvent() {
        ...
        CustomerServiceManager.Instance.removeEventListener(CustomerServiceManager.NEW_REPLY, this.__newServiceReplyHandler, this);
    }
    ...
    private __castleInfo() {
        ...
        for (let index = 0; index < nodeIdList.length; index++) {
            ...
            // }
            if (castleInfo.state == EmOuterCityWarCastlePeriodType.DeclaringWar || castleInfo.state == EmOuterCityWarCastlePeriodType.Fighting) {
    ...
    private coordinates(): void {
        let posX: number = Math.floor((StageReferance.stageWidth / 2 - this._BigMap_X * UIConstant.SMALL_MAP_SCALE) / 50);
        let posY: number = Math.floor((StageReferance.stageHeight / 2 - this._BigMap_Y * UIConstant.SMALL_MAP_SCALE) / 50);
        posX = (posX < 0 ? 0 : posX);
        posY = (posY < 0 ? 0 : posY);
    ...
    private enterHome() {
        if (ArmyManager.Instance.thane.grades < OpenGrades.ENTER_SPACE) {
            MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("smallMapBar.enterHome.tips"))
            return;
        }
        if(ArmyManager.Instance.army.onVehicle){
            MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("OuterCityCastleTips.gotoBtnTips"));
        ...
        if (WorldBossHelper.checkPetLand(CampaignManager.Instance.mapId)) {
            let content = LangManager.Instance.GetTranslation("MainToolBar.extPetIsland");
    ...
    public switchSmallMapState(state: number) {
        ...
        switch (this._curState) {
            case SmallMapBar.SPACE_SMALL_MAP_STATE:
                ...
                this.title1.text = this.title2.text = SpaceManager.Instance.model.mapTempInfo.MapNameLang;
                break;
            case SmallMapBar.CASTLE_SMALL_MAP_STATE:
                this.state.selectedIndex = 1;
                this.title1.text = this.title2.text = LangManager.Instance.GetTranslation("mainBar.SmallMapBar.castleTitle");
                break;
            case SmallMapBar.CAMPAIGN_SMALL_MAP_STATE:
            case SmallMapBar.CAMPAIGN_SMALL_MAP_STATE2:
                this.state.selectedIndex = 2;
                this.title1.text = this.title2.text = CampaignManager.Instance.mapModel.mapTempInfo.MapNameLang;
                break;
            case SmallMapBar.OUTERCITY_SMALL_MAP_STATE:
                this.state.selectedIndex = 3;
                this.title1.text = this.title2.text = LangManager.Instance.GetTranslation("mainBar.SmallMapBar.outercityTitle");
                break;
            default:
                break;
        }
