...
export class GoldenTreeRecordWnd extends BaseWindow {
  ...
  private initEvent() {
    this.list_rewards.itemRenderer = Laya.Handler.create(
      this,
      this.onListRender1,
      null,
      false,
    );
    this.list_luckShow.itemRenderer = Laya.Handler.create(
      this,
      this.onListRender2,
      null,
      false,
    );
    this.bottleModel.addEventListener(
      BottleEvent.UPDATE_REWARDS_INFO,
      this.updateRewardsInfoHandler,
      this,
    );
    this.bottleModel.addEventListener(
      BottleEvent.UPDATE_LUCK_SHOW_INFO,
      this.updateLuckShowInfoHandler,
      this,
    );
  }
  ...
  private updateRewardsInfoHandler() {
    if (this.bottleModel.bottleRewardsInfo) {
      ...
      for (
        let i: number = 0;
        i < this.bottleModel.bottleRewardsInfo.length;
        i++
      ) {
  ...
  private updateLuckShowInfoHandler() {
    if (this.bottleModel.bottleLuckShowInfo) {
      ...
      for (
        let i: number = 0;
        i < this.bottleModel.bottleLuckShowInfo.length;
        i++
      ) {
        let chatData: ChatData = new ChatData();
        chatData.channel = ChatChannel.BOTTLE;
  ...
  private removeEvent() {
    ...
    this.bottleModel.removeEventListener(
      BottleEvent.UPDATE_REWARDS_INFO,
      this.updateRewardsInfoHandler,
      this,
    );
