...
export default class PveCampaignData extends FrameDataBase {
    ...
    public getTreeData(): any[] {
        ...
        let cpDataList = this.selectedLand.chapterDic.getList() as CampaignChapterInfo[]
        ...
        for (let index = 0; index < cpDataList.length; index++) {
            ...
            if (cpData.canOpen(cpData.chapterId)) {
                firstData.push(new SelectCampaignItemData(eSelectCampaignItemType.FirstItem, cpName, cpData.chapterId, cpEnabled,grayFilterFlag1))
            }
            ...
            for (let i = 0; i < areaDataList.length; i++) {
                ...
                if (areaEnabled) {
                    ...
                    for (let index = 0; index < mainTaskList.length; index++) {
                        ...
                        for (let i = 0; i < taskTemp.conditionList.length; i++) {
                            const conTemp: t_s_rewardcondictionData = taskTemp.conditionList[i];
                ...
                temp.push(new SelectCampaignItemData(eSelectCampaignItemType.SecondItem, { icon: areaData.getAreaImg(true), text: areaData.getItemTitle(), bTask: bTask }, areaData.areaId, areaEnabled,grayFilterFlag2))
            }
            secondData.push(new SelectCampaignItemData(eSelectCampaignItemType.SecondItem, temp))

            let rTitle = LangManager.Instance.GetTranslation("selectcampaign.view.chapterReward") + cpData.getMaxOverAreaCount() + "/" + areaDataList.length
            let rbComplete = cpData.isAllOpenArea()
            rewardTipData.push(new SelectCampaignItemData(eSelectCampaignItemType.RewardTipItem, { text: rTitle, bComplete: rbComplete }, cpData.chapterId))
        }
    ...
    public getRecentCanSelectChapter() {
        ...
        for (let j: number = 0; j < len; j++) {
            ...
            if (this.taskCampaignTem && this.taskCampaignTem.DungeonId == chapter.chapterId) {
        ...
        this.recentChapter = (cpDataList[cpDataList.length - 1] as CampaignChapterInfo);
    }

    public getChapterOpenLen(): number {
        let openLen = 0
        let chapterList = this.selectedLand.chapterDic.getList() as CampaignChapterInfo[];
