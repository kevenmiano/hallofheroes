...
export default class SevenGoalsTaskItem extends FUI_SevenGoalsTaskItem {
    ...
    private initEvent() {
        ...
        this.goodsList.itemRenderer = Laya.Handler.create(this, this.renderGoodsListItem, null, false);
    }
    ...
    renderGoodsListItem(index: number, item: BaseItem) {
        ...
        if (this.sevenData.Day < this.sevenGoalsModel.sevenCurrentDay)//选中的是之前的天数
        {
            if (this.sevenGoalsModel.tabValue == 0)//选中的tab是成长任务
            {
    ...
    private refreshView() {
        if (this._info) {
            this.sevenData = TempleteManager.Instance.getSevenTaskByTaskId(this._info.taskId);
            if (this.sevenData) {
                this.pointValueTxt.text = this.sevenData.Num.toString();
                this.DescTxt.text = this.sevenData.TitleLang;
                if (this.sevenGoalsModel.sevenNotNeedProgressTypeArr.indexOf(this.sevenData.Type) != -1) {
                    this.countTxt.text = "";
                }
                else {
                    this.countTxt.text = "(" + this._info.finishNum + "/" + this.sevenData.Para1 + ")";
                }
    ...
    private gotoBtnHandler() {
        switch (this.sevenData.Type) {
            ...
            case 3:
                this.exitMainFrame();
                if (this.sevenData.Para2 == 3) {//仓库
                    UIManager.Instance.ShowWind(EmWindow.DepotWnd, BuildingManager.Instance.model.getBuildingInfoBySonType(BuildingType.WAREHOUSE));
                } else if (this.sevenData.Para2 == 1) {//精炼炉
                    UIManager.Instance.ShowWind(EmWindow.CrystalWnd, BuildingManager.Instance.model.getBuildingInfoBySonType(BuildingType.CRYSTALFURNACE));
                }
                break;
            case 4:
            case 39:
            ...
            case 36:
                if (ArmyManager.Instance.thane.grades < OpenGrades.STAR) {
                    MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("armyII.SkillFrame.AlertMsg", OpenGrades.STAR));
                    return;
                }
                this.exitMainFrame();
            ...
            case 20:
                if (ArmyManager.Instance.thane.grades < OpenGrades.MOUNT) {
                    MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("armyII.SkillFrame.AlertMsg", OpenGrades.MOUNT));
                    return;
                }
                this.exitMainFrame();
            ...
            case 18:
                if (ArmyManager.Instance.thane.grades < OpenGrades.TALENT) {
                    MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("armyII.SkillFrame.AlertMsg", OpenGrades.TALENT));
                    return;
                }
                this.exitMainFrame();
                FrameCtrlManager.Instance.open(EmWindow.Skill);
            ...
            case 81:
                if (ArmyManager.Instance.thane.grades < OpenGrades.PET) {
                    MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("armyII.SkillFrame.AlertMsg", OpenGrades.PET));
                    return;
                }
                this.exitMainFrame();
            ...
            case 38:
                if (ArmyManager.Instance.thane.grades < OpenGrades.MAZE) {
                    MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("armyII.SkillFrame.AlertMsg", OpenGrades.MAZE));
                    return true;
                }
                this.exitMainFrame();
            ...
            case 57:
                this.exitMainFrame();
                UIManager.Instance.ShowWind(EmWindow.SeminaryWnd, BuildingManager.Instance.model.getBuildingInfoBySonType(BuildingType.SEMINARY));
                break;
            case 53:
            case 60:
                if (ArmyManager.Instance.thane.grades < OpenGrades.WORLD_BOSS) {
                    MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("armyII.SkillFrame.AlertMsg", OpenGrades.WORLD_BOSS));
                    return true;
                }
            ...
            case 66:
                if (ArmyManager.Instance.thane.grades < OpenGrades.RVR) {
                    MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("armyII.SkillFrame.AlertMsg", OpenGrades.RVR));
                    return true;
                }
                if (MopupManager.Instance.model.isMopup) {
                    let str: string = LangManager.Instance.GetTranslation("mopup.MopupManager.mopupTipData01");
            ...
            case 72:
                if (ArmyManager.Instance.thane.grades < OpenGrades.HOOK) {
                    MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("armyII.SkillFrame.AlertMsg", OpenGrades.HOOK));
                    return true;
                }
                this.exitMainFrame();
            ...
            case 76:
                if (ArmyManager.Instance.thane.grades < OpenGrades.MYSTERIOUS) {
                    MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("armyII.SkillFrame.AlertMsg", OpenGrades.MYSTERIOUS));
                    return true;
                }
                this.exitMainFrame();
            ...
            case 83:
                this.exitMainFrame();
                UIManager.Instance.ShowWind(EmWindow.PoliticsFrameWnd, BuildingManager.Instance.model.getBuildingInfoBySonType(BuildingType.OFFICEAFFAIRS));
                break;
        }
    }
