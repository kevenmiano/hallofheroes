...
export class FarmFriendInfoView extends BaseFguiCom {
	...
	private removeFarmListener(target: FarmInfo) {
		if (target) {
			target.removeEventListener(FarmEvent.FARM_GP_CHANGE, this.__gpChanged, this);
			target.removeEventListener(FarmEvent.FARM_GRADE_CHANGE, this.__gradeChanged, this);
		}
	}
	...
	private addFarmListener(target: FarmInfo) {
		if (target) {
			...
			target.addEventListener(FarmEvent.FARM_GRADE_CHANGE, this.__gradeChanged, this);
		}
	}

	private __gradeChanged(event: FarmEvent) {
		if (this._data) {
			this.levelTxt.text = LangManager.Instance.GetTranslation("public.level2", this._data.grade);
	...
	private __gpChanged(event: FarmEvent) {
		if (this._data && !this.comPart.isDisposed) {
			...
			var upGrade: t_s_upgradetemplateData = this.getNextGradeTemp(this._data.grade + 1, UpgradeType.UPGRADE_TYPE_FARM);
			if (upGrade) {
				...
				percentage = 1;
				str = LangManager.Instance.GetTranslation("yishi.view.tips.goods.StarTip.gp01");
			}
	...
	public set data(info: FarmInfo) {
		...
		if (this._data) {
			if (this._data.userId == this.thane.userId) {
				...
				var fInfo: ThaneInfo = IMManager.Instance.getFriendInfo(this._data.userId);
				if (fInfo) {
            ...
            if (this._data.frameId > 0) {
                ...
                if (itemData) {
                    ...
                    this.imgIcon.headEffect = (Number(itemData.Property1) == 1) ? itemData.Avata : "";
                }
            }else{
			...
			this.levelTxt.text = LangManager.Instance.GetTranslation("public.level2", 1);
		}
