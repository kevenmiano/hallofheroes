...
export class WildSoulCollection extends GameEventDispatcher {
  ...
  /**按**/
  private sortMount(
    info1: t_s_mounttemplateData,
    info2: t_s_mounttemplateData,
  ): number {
  ...
  private specialMountListHasTemplate(id: number): boolean {
    ...
    for (
      let i: number = 0;
      i < WildSoulCollection._magicMountTemplateList.length;
      i++
    ) {
      let temp: t_s_mounttemplateData =
        WildSoulCollection._magicMountTemplateList[i];
  ...
  public get totalPage(): number {
    return Math.max(
      Math.ceil(this.mountTemplates.length / WildSoulCollection.PAGE_SIZE),
      1,
    );
  }
  ...
  public get pageData() {
    var startIndex: number =
      (this._currentIndex - 1) * WildSoulCollection.PAGE_SIZE;
    ...
    for (let index = 0; index < count; index++) {
      ...
      isActive =
        element.Activation == 1
          ? this.isLightTemplate(element.TemplateId)
          : true;
      if (isActive) {
  ...
  public canActive(v: t_s_mounttemplateData) {
    ...
    let goodTemplate: t_s_itemtemplateData =
      TempleteManager.Instance.getGoodsTemplatesByTempleteId(v.NeedItemId);
    if (goodTemplate) {
      //去背包中找解锁的物品
      let find: Array<GoodsInfo> =
        GoodsManager.Instance.getBagGoodsByTemplateId(goodTemplate.TemplateId);
  ...
  public canLianHua(vData: t_s_mounttemplateData) {
    ...
    let flag1: boolean =
      GoodsManager.Instance.getGoodsNumByTempId(vData.StarItem) > 0
        ? true
        : false;
    var info: WildSoulInfo = MountsManager.Instance.avatarList.getWildSoulInfo(
      vData.TemplateId,
    );
    if (info) {
