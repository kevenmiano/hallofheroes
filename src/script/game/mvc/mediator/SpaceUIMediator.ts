...
export class SpaceUIMediator implements IMediator, IEnterFrame {
  ...
  private check(element: SpaceNpcView): boolean {
    if (
      (element.nodeInfo.info.id == SpaceNodeType.ID_TREASURE_MAP &&
        !ComponentSetting.TREASURE_MAP) ||
      (element.nodeInfo.info.id == SpaceNodeType.ID_SINGLE_PASS &&
        !ComponentSetting.SINGLE_PASS) ||
      (element.nodeInfo.info.id == SpaceNodeType.PET_CAMPAIGN &&
        !ComponentSetting.PET_CAMPAIGN) ||
      (element.nodeInfo.info.id == SpaceNodeType.ID_PET_CHALLENGE &&
        !ComponentSetting.PET_CHALLENGE) ||
      (element.nodeInfo.info.id == SpaceNodeType.PET_REMOTE &&
        !ComponentSetting.PET_REMOTE)
    ) {
      return false;
    }
    return true;
          ...
          if (btn) {
            ...
            if (!btn.parent) {
              ...
              btn.y =
                ...
                btn.height -
                10 +
                element.getPosYById(element.nodeInfo.info.id);
              this._uiSprite.addChild(btn);
            }
          } else {
            let path: string;
            path =
              "res/game/common/map.space.NPCBtn" +
              element.nodeInfo.info.id +
              ".png";
