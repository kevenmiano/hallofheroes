...
export class ShowWeaponAvatar extends ShowAvatarLoader
{
    ...
    private initEvent()
    {
        NotificationManager.Instance.addEventListener(NotificationEvent.WEAPON_CHANGE, this.__infoUpdateHandler, this);
    }

    private removeEvent()
    {
        NotificationManager.Instance.removeEventListener(NotificationEvent.WEAPON_CHANGE, this.__infoUpdateHandler, this);
    }
    ...
    private onFlowComplete()
    {
        TweenMax.to(this._content, 0.4, {
            dropShadowFilter:{angle:45, blurX:6, blurY:6, strength:0.1, quality:1, distance:0, color:0xffffff},
            glowFilter:{color:0x67d118, alpha:1, blurX:15, blurY:15, strength:0.1, quality:1},
            colorMatrixFilter:{contrast:1, brightness:1, saturation:1, hue:0},
            onComplete:this.stop, delay:0.3
        });
    }
