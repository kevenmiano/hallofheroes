...
export default class PetWnd extends BaseWindow {
  ...
  public OnInitWind() {
    ...
    this._tabSubMap.set(mainIdxAttr, [
      this["tabAdvance"],
      this["tabIntensify"],
    ]);

    let mainIndexAtt2 = Math.floor(PetModel.TabIndex.Pet_Potency / 10);
    this._tabSubMap.set(mainIndexAtt2, [
      this["subTabPotency"],
      this["subTabArtifact"],
    ]);
    ...
    this._uiAttrAdvance = new UIAttrAdvance(
      pagenode[PetModel.TabIndex.Pet_AttrAdvance],
    );
    this._uiAttrIntensify = new UIAttrIntensify(
      pagenode[PetModel.TabIndex.Pet_AttrIntensify],
    );
    ...
    this._uiFormation = new UIFormation(
      pagenode[PetModel.TabIndex.Pet_Formation],
    );
    ...
    if (ComponentSetting.PET_REFING) {
      this._uiRefining = new UIRefining(
        pagenode[PetModel.TabIndex.Pet_Refining],
      );
    ...
    this._petView.y = -30;
    this.comFigure.displayObject.setChildIndex(
      this._petView,
      this.comFigure.displayObject.numChildren - 4,
    );
  ...
  private addEvent() {
    this.playerInfo.addEventListener(
      PetEvent.PET_ADD,
      this.__addPetHandler,
      this,
    );
    this.playerInfo.addEventListener(
      PetEvent.PET_REMOVE,
      this.__removePetHandler,
      this,
    );
    this.playerInfo.addEventListener(
      PetEvent.PET_UPDATE,
    ...
    this.playerInfo.addEventListener(
      PetEvent.PET_MAXCOUNT_CHANGE,
      this.__addPetHandler,
      this,
    );
    if (this.farmInfo) {
      this.farmInfo.addEventListener(
        FarmEvent.PET_LAND_OPEN,
        this.__updatePracticePetHandler,
        this,
      );
      this.farmInfo.addEventListener(
        FarmEvent.PET_LAND_UPDATE,
        this.__updatePracticePetHandler,
        this,
      );
    }
  ...
  private removeEvent() {
    ...
    if (this.farmInfo) {
      this.farmInfo.removeEventListener(
        FarmEvent.PET_LAND_OPEN,
        this.__updatePracticePetHandler,
        this,
      );
  ...
  private delayRefreshPetList(
    time: number = 200,
    selectDefItem: boolean = true,
  ) {
  ...
  private __refreshPetList(selectDefItem: boolean = true) {
    ...
    if (selectDefItem) {
      this.selectDefPetItem(
        this.ctrl.selectedPet ? this.ctrl.selectedPet.petId : undefined,
      );
    }
  }
  ...
  private __removePetHandler(value: PetData) {
    ...
    var index =
      PlayerManager.Instance.currentPlayerModel.playerInfo.petList.indexOf(
        this.ctrl.selectedPet,
      );
    if (index < 0) {
  ...
  private resetViewByTabIndex(tabIndex: number, lastTabIndex: boolean = false) {
    switch (tabIndex) {
      ...
      case PetModel.TabIndex.Pet_AttrIntensify:
        this._uiAttrCommon.resetView();
        if (this._uiAttrIntensify)
          this._uiAttrIntensify.resetView(lastTabIndex);
        break;
      case PetModel.TabIndex.Pet_Refining:
  ...
  private refreshPetFigureInfo() {
    if (!this._data) {
      ...
      this.comFigure.txtName.color = PetData.getQualityColor(
  ...
  private checkEditSkill(): boolean {
    if (this.uiSkill && this.uiSkill.isSetting) {
      ...
      SimpleAlertHelper.Instance.Show(
        ...
        null,
        null,
        this.__checkEditSkill.bind(this),
      );
      return true;
    }
    return false;
