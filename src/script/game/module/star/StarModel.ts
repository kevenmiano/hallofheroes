...
export default class StarModel extends FrameDataBase {
	...
	public getStarPow(): number {
		...
		let equipStarList: SimpleDictionary = StarManager.Instance.getStarListByBagType(StarBagType.THANE);
	...
	public getNameByIndex(indeX: number): string {
		if (!this._randomStarName) {
			this._randomStarName = [
				LangManager.Instance.GetTranslation('star.view.MakeStarView.greenCrystal.name'),
				LangManager.Instance.GetTranslation('star.view.MakeStarView.whiteCrystal.name'),
				LangManager.Instance.GetTranslation('star.view.MakeStarView.yellowCrystal.name'),
				LangManager.Instance.GetTranslation('star.view.MakeStarView.redCrystal.name'),
				LangManager.Instance.GetTranslation('star.view.MakeStarView.blueCrystal.name')
			];
		}
	...
	public checkAction(action: number, value: number = 0): boolean {
		...
		if (tempStarNum <= 0) {
			MessageTipManager.Instance.show(LangManager.Instance.GetTranslation('star.view.DisplayStarView.command02'));
			return false;
		}
		...
		switch (action) {
			case StarModel.PICK_UP:
				...
				if (playerStarNum + this.currentStarNum > PlayerModel.ORIGINAL_OPEN_BAG_COUNT + DataCommonManager.playerInfo.starBagCount) {
					if (this.currentStarNum > 0) {
						MessageTipManager.Instance.show(LangManager.Instance.GetTranslation('star.view.DisplayStarView.command03'));
					}
				}
				...
				MessageTipManager.Instance.show(LangManager.Instance.GetTranslation('star.view.DisplayStarView.command04'));
				return false;
			case StarModel.SELL:
				for (let index = 0; index < tempStarList.getList().length; index++) {
					const sInfo: StarInfo = tempStarList.getList()[index];
					if (sInfo.template.Profile <= value && sInfo.grade == 1 && sInfo.gp == 0) {
						return true;
					}
				}
				MessageTipManager.Instance.show(LangManager.Instance.GetTranslation('star.view.DisplayStarView.command05'));
				return false;
		}
		return false;
	}
	...
	public openEquipGridNum() {
		let extraOpenGrid = Math.floor((DataCommonManager.thane.grades - OpenGrades.STAR) / 5)
