...
import { NotificationEvent, NativeEvent } from "../../../constant/event/NotificationEvent";
...
export default class TaskTraceProgressTipWnd {
    ...
    private __timerHandler() {
        ...
        this.txtCountdown.text = LangManager.Instance.GetTranslation('openBox.tip.countdown', timeStr);
        if (time == 0) {
            this.Hide();
        }
    }
    ...
    */
    async Show(data = null) {
        ...
        if (this.wnd) {
            LayerMgr.Instance.addToLayer(this.wnd.displayObject, EmLayer.GAME_TOP_LAYER, -2);
    ...
    public Hide() {
        if (this.wnd) {
            ...
            LayerMgr.Instance.removeByLayer(this.wnd.displayObject, EmLayer.GAME_TOP_LAYER);
            this.dispose();
        }
        this._data = null;
        if (TaskTraceTipManager.Instance.showTipList.length > 0) {
    ...
    protected initEvent() {
        this.operationBtn && !this.operationBtn.isDisposed && this.operationBtn.onClick(this, this.__btnHandler);
        this.closeBtn && !this.closeBtn.isDisposed && this.closeBtn.onClick(this, this.onCloseWnd);
        NotificationManager.Instance.addEventListener(UIBarEvent.SHOW_TASK_TRACE, this.__show, this);
        NotificationManager.Instance.addEventListener(UIBarEvent.HIDE_TASK_TRACE, this.__hide, this);
        NotificationManager.Instance.addEventListener(NotificationEvent.SWITCH_SCENE, this._onSceneViewAdded, this);
        NotificationManager.Instance.addEventListener(NativeEvent.AFTER_STATUS_BAR_CHANGE, this.__afterStatusBarChange, this);
    }

    protected removeEvent() {
        this.operationBtn && !this.operationBtn.isDisposed && this.operationBtn.offClick(this, this.__btnHandler);
        this.closeBtn && !this.closeBtn.isDisposed && this.closeBtn.offClick(this, this.onCloseWnd);
        NotificationManager.Instance.removeEventListener(UIBarEvent.SHOW_TASK_TRACE, this.__show, this);
        NotificationManager.Instance.removeEventListener(UIBarEvent.HIDE_TASK_TRACE, this.__hide, this);
        NotificationManager.Instance.removeEventListener(NotificationEvent.SWITCH_SCENE, this._onSceneViewAdded, this);
        NotificationManager.Instance.removeEventListener(NativeEvent.AFTER_STATUS_BAR_CHANGE, this.__afterStatusBarChange, this);
    }
