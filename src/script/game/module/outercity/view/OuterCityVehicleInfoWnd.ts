...
export default class OuterCityVehicleInfoWnd extends BaseWindow {
  ...
  private initView() {
    if (this._wildLand.info.occupyLeagueName == "") {
      ...
      this.consortiaNameTxt.text =
        "<" + this._wildLand.info.occupyLeagueName + ">";
      if (
        this._outercityModel.checkIsSameConsortiaByName(
          this._wildLand.info.occupyLeagueName,
        )
      ) {
        //同工会的
    ...
    if (this._wildLand.leftTime > 0) {
      ...
      this.leftTimeTxt.text = DateFormatter.getConsortiaCountDate(
        this._count,
        false,
      );
  ...
  private __renderGoodsListItem(
    index: number,
    item: OuterCityVehicleRewardItem,
  ) {
  ...
  private onHelp() {
    let title = LangManager.Instance.GetTranslation(
      "OuterCityVehicleInfoWnd.helpTitle",
    );
    let content = LangManager.Instance.GetTranslation(
      "OuterCityVehicleInfoWnd.helpContent",
    );
  ...
  private updateView() {
    if (this._wildLand) {
      ...
      this._wildLand =
        OuterCityManager.Instance.model.allVehicleNode.get(templateId);
      this.initView();
    }
  }
