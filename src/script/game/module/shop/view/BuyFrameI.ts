...
export class BuyFrameI extends BaseWindow {
    ...
    private initEvent() {
        ...
        this.checkbox_useBind.on(fgui.Events.STATE_CHANGED, this, this.__useBindPointHandler);
    ...
    public data(value: any, count: number = 1, parm: Object = null) {
        ...
        if (this._data instanceof ShopGoodsInfo) {
            ...
            let good: t_s_itemtemplateData = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_itemtemplate, itemid)
        ...
        if (this._data) {
            ...
            if (this._data instanceof ShopGoodsInfo) {
                ...
                if (this.c1.selectedIndex == 0) {
                    if (sonType == 218 && good.Property2 < 0) {
                        if (MountsManager.Instance.avatarList.isLightTemplate(good.Property1)) {
                    ...
                    if (sonType == 212 && good.TemplateId != 2120044)  //不是神兽之魂的坐骑碎片卡
                    {
                        //优化标记 这个数据也不小
                        ...
                        let good2: t_s_itemtemplateData = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_itemtemplate, composeGoodsId)
                        if (good2) {
                            if (MountsManager.Instance.avatarList.isLightTemplate(good2.Property1) && good2.Property2 < 0) {
                                MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("shop.view.frame.BuyFrameI.mopupexitst"));
                                return;
                            }
                        }
                    }
            ...
            if (Number(this.txt_price.text) > this.PayMoney) {
                if (this._data.PayType == ShopGoodsInfo.PAY_BY_POINT) {
                    ...
                    MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("star.view.MakeStarView.command02"));
                    ...
                    MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("shop.view.frame.BuyFrameI.SociatyContribLack"));
                } else if (this._data.PayType == ShopGoodsInfo.PAY_BY_GLORY) {
                    MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("ui.resourcebar.view.quickbuy.QuickBuyFrame.gloryTip"));
                } else if (this._data.PayType == ShopGoodsInfo.PAY_BY_BACKPLAYER) {
                    MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("shop.view.frame.BuyFrameI.NotEnoughBackPlayer"));
                } else {
                    MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("shop.view.frame.BuyFrameI.MoneyLack"));
                }
                // this.OnBtnClose();
            ...
            if (this.c1.selectedIndex == 0) {
                if (this._data.PayType == ShopGoodsInfo.PAY_BY_MAZE) {
                    let info: ShopGoodsInfo = TempleteManager.Instance.getMazeShopTempInfoByItemId(this._data.ItemId);
                    let goods: t_s_itemtemplateData = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_itemtemplate, this._data.ItemId)
    ...
    private alert() {
        ...
        SimpleAlertHelper.Instance.Show(SimpleAlertHelper.SIMPLE_ALERT, null, LangManager.Instance.GetTranslation("BuyFrameI.tip01"),
            LangManager.Instance.GetTranslation("BuyFrameI.tip02"),
            LangManager.Instance.GetTranslation("public.confirm"),
            LangManager.Instance.GetTranslation("public.cancel1"), result);
    }

    private buy(useBind: boolean = true) {
        let temp: t_s_itemtemplateData = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_itemtemplate, this._data.ItemId)
        if (this.isFashionGoods(temp)) {
            ShopManager.Instance.sendShoping((<ShopGoodsInfo>this._data).Id, this.stepper.value, this.playerInfo.userId, false, C2SProtocol.C_FASHION_SHOP_BUY, useBind);
        } else {
            ShopManager.Instance.sendShoping((<ShopGoodsInfo>this._data).Id, this.stepper.value, this.playerInfo.userId, false, 0x1422, useBind);
        }
        ...
        if (cc.selectedIndex == 0) {
            ...
            if (this._data instanceof MainShopInfo) {
                ...
                if (this._data.PayType == 2) {
                    ...
                    templateId = ShopManager.Instance.model.getTemplateId(this._data.PayType);
                    this.tipItem.setInfo(templateId);
                }
            }
            this.showPrompt = 0;
            ShopManager.Instance.isHandsel = 0;
            this.frame.text = LangManager.Instance.GetTranslation("campaign.TrailShop.BuyBtnTxt");
            this.checkbox_useBind.visible = this.checkbox_useBind.selected = this._isUseBindPoint;
        }
            ...
            this.frame.text = LangManager.Instance.GetTranslation("yishi.view.frame.BuyFrameI.Presentation");
        }
        this.updatePrompt();
    }
    ...
    protected refreshView() {
        ...
        if (goodsInfo.templateId == CommonConstant.RUNE_GEM_ENERGY) {
            this.txt_count.text = PlayerManager.Instance.currentPlayerModel.playerInfo.runePowerPoint.toString();
        } else {
        ...
        if (shopGoodsInfo.PayType == ShopGoodsInfo.PAY_BY_POINT || shopGoodsInfo.PayType == ShopGoodsInfo.PAY_BY_GIFT) {
        ...
        if ((this._data.canBuyNum != -1 && this._data.PayType == ShopGoodsInfo.PAY_BY_POINT) ||
            !FrameCtrlManager.Instance.isOpen(EmWindow.ShopWnd) ||
            // this.shopController.frame == null ||
            (this._data.PayType != ShopGoodsInfo.PAY_BY_GIFT && this._data.PayType != ShopGoodsInfo.PAY_BY_POINT)) {
        ...
        let leftNum = 0;
        let maxCount = 999;
        if (Number(shopGoodsInfo.OneDayCount) != -1 && shopGoodsInfo.OneDayCount != undefined) {//日限购
        ...
        } else if (Number(shopGoodsInfo.WeeklyLimit) != -1 && shopGoodsInfo.WeeklyLimit != undefined) {//周限购
        ...
        if(shopGoodsInfo.ItemId == CommonConstant.PET_SOUL_STONE){
            this.stepper.show(0, this._count, 1, leftNum, this.currentCanCount, 1, this._handler);
        }else{
            this.stepper.show(0, this._count, this._count, leftNum, this.currentCanCount, 1, this._handler);
        }
    ...
    public get PayMoney(): number {
        if (this._data.PayType == ShopGoodsInfo.PAY_BY_POINT || this._data instanceof MainShopInfo) {
    ...
    private refreshLimitView() {
        ...
        if (this._data.canTotalCount > 0) {
            ...
            this.txt_canTotal.text = LangManager.Instance.GetTranslation("shop.view.GoodsItem.limited03"); //限量出售
            ...
            this.txt_canTotal.text = LangManager.Instance.GetTranslation("shop.view.GoodsItem.limited"); //售完
            this.txt_canTotalValue.text = "";
            totalFlag = true;
        }
        ...
        if (this._data.type == ShopGoodsInfo.PAY_BY_GIFT) {
            ...
            if (num > 0) {
                ...
                this.txt_canOne.text = LangManager.Instance.GetTranslation("shop.view.GoodsItem.limited06", num); //每4小时限购{n}个
                oneFlag = true;
            } else {
                this.txt_canOne.color = "#CC0000";
                this.txt_canOne.text = LangManager.Instance.GetTranslation("shop.view.GoodsItem.limited05");//购买量达到上限
                oneFlag = true;
            }
        } else {
            if (this._data.canOneCount >= 0) {
                ...
                textValue = LangManager.Instance.GetTranslation("shop.view.GoodsItem.limited04", showNum);  //每人限购 + "";
                if (showNum <= 0) {
                    this.txt_canOne.color = "#CC0000";
                    textValue = LangManager.Instance.GetTranslation("shop.view.GoodsItem.limited05");//购买量达到上限
                }
                oneFlag = true;
            ...
            this.checkbox_useBind.visible = this._isUseBindPoint && this.c1.selectedIndex == 0;
        }
        this.updatePrompt();
    }
    ...
    private removeEvent() {
        ...
        this.checkbox_useBind.off(fgui.Events.STATE_CHANGED, this, this.__useBindPointHandler);
