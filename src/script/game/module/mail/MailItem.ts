...
export default class MailItem extends FUI_MailItem {
  ...
  public remainTime(): number {
    ...
    if (this._data.isRead) {
      remain =
        this._data.ValidityDate * 24 -
        (this.nowDate - this._data.readDate.getTime()) / (60 * 60 * 1000);
    } else {
      remain =
        this._data.ValidityDate * 24 -
        (this.nowDate - this._data.sendDate.getTime()) / (60 * 60 * 1000);
    }
    if (remain && remain > 0) {
      return remain;
    } else {
  ...
  private setIcon() {
    switch (this._data.MailType) {
      case EmailType.BATTLE_REPORT:
        this.mailIcon.url = String(
          this._data.isRead ? this.unFoldIcon : this.foldIcon,
        );
