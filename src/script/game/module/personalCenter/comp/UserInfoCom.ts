...
export default class UserInfoCom extends FUI_UserInfoCom {
    ...
    private addEvent() {
        ...
        PlayerManager.Instance.currentPlayerModel.playerInfo.addEventListener(PlayerEvent.NICKNAME_UPDATE, this.updateName, this);
        NotificationManager.Instance.addEventListener(NotificationEvent.UPDATE_HEADFRAME_INFO, this.updateHead, this);
    ...
    private onChangeRole() {
        ...
        SimpleAlertHelper.Instance.Show(SimpleAlertHelper.SIMPLE_ALERT, null, LangManager.Instance.GetTranslation("public.prompt"),
            ...
            LangManager.Instance.GetTranslation("public.cancel1"), cb);
    }
    ...
    private initUserInfo() {
        ...
        this.txt_consortia.text = playerInfo.consortiaName ? playerInfo.consortiaName : LangManager.Instance.GetTranslation("armyII.viewII.attribute.PlayerAttributeView.Not");
        ...
        let cfg: t_s_honorequipData = TempleteManager.Instance.geHonorCfgByType(1, stage);
        if (cfg) {
        ...
        let value = info.appellInfo ? info.appellInfo.TitleLang : LangManager.Instance.GetTranslation("armyII.viewII.attribute.PlayerAttributeView.Not");
        this.txt_title.setVar("text", value).flushVars();
        if (info.appellInfo) {
            this._honerView = new AppellView(info.appellInfo.ImgWidth, info.appellInfo.ImgHeight, info.appellId);
    ...
    private updateHeadImg(isupdate: boolean = true) {
        this.updateHead();
        if (isupdate) {
            MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("armyII.viewII.information.InformationView.saveSucceed"));
        }
    ...
    private updateName(e: PlayerEvent) {
        this.txt_name.text = PlayerManager.Instance.currentPlayerModel.playerInfo.nickName;
    }
    ...
    private onBtnNameClick() {
        SimpleAlertHelper.Instance.Show(SimpleAlertHelper.SIMPLE_ALERT, null, LangManager.Instance.GetTranslation("public.prompt"),
            ...
            LangManager.Instance.GetTranslation("public.confirm"),
            LangManager.Instance.GetTranslation("public.cancel1"), this.checkHasCard.bind(this));
    }
    ...
    private checkHasCard(b: boolean): boolean {
        ...
        if (goodsArr.length > 0) {
            ...
            //需要知道改名卡的模板ID
            var data: ShopGoodsInfo = TempleteManager.Instance.getShopTempInfoByItemId(CommonConstant.RENAME_CARD_TEMPID);
            let obj = {
                info: data,
                count: 1,
    ...
    public removeEvent() {
        ...
        PlayerManager.Instance.currentPlayerModel.playerInfo.removeEventListener(PlayerEvent.NICKNAME_UPDATE, this.updateName, this);
        NotificationManager.Instance.removeEventListener(NotificationEvent.UPDATE_HEADFRAME_INFO, this.updateHead, this);
