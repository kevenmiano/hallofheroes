...
export class RuneTip extends BaseTips {
  ...
  private updateView() {
    if (this._info) {
      ...
      this.isRuneGem =
        this._info.templateInfo.SonType >= 400 &&
        this._info.templateInfo.SonType < 500;
      ...
      if (this.isRuneGem) {
        ...
        this.txt_needJob.text =
          LangManager.Instance.GetTranslation("ConsortiaInfoWnd.n7Txt.title") +
          str;
      } else {
        str =
  ...
  private onBtnUseClick() {
    ...
    if (this.isRuneGem) {
      if (BagHelper.OPEN_RUNE_BAG_TYPE == 1) {
        ...
        BagHelper.OPEN_RUNE_BAG_TYPE == 0 ||
  ...
  private onBtnBatchUseClick() {
    if (BagHelper.OPEN_RUNE_BAG_TYPE == 1) {
      ...
      if (
        this.btn_batchUse.title ==
        LangManager.Instance.GetTranslation("armyII.viewII.allocate.upgrade")
      ) {
        NotificationManager.Instance.sendNotification(
