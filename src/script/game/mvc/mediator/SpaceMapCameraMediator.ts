...
export class SpaceMapCameraMediator
  extends Laya.Sprite
  implements IMediator, IEnterFrame
{
  ...
  private __lookAtRoleHandler(data: SpaceArmy) {
    ...
    targetPosX =
      Math.abs(targetPosX - curPosX) < this._xLimen + this._bevelSpeed
        ? curPosX
        : targetPosX;
    targetPosY =
      Math.abs(targetPosY - curPosY) < this._yLimen + this._bevelSpeed
        ? curPosY
        : targetPosY;
    this._updatePoint.x = targetPosX;
    this._updatePoint.y = targetPosY;
    (<SpaceSceneMapView>this._target).setPosition(
      this._updatePoint.x,
      this._updatePoint.y,
    );
  }
  ...
  private moveXY(para: boolean = false) {
    ...
    let targetPosY: number =
      this._role.y - (stageHeight >> 1) - this._yLimenOff;
    let curPosX: number = this._target.x;
    let curPosY: number = this._target.y;
