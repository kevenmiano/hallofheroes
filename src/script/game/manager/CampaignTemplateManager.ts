...
export class CampaignTemplateManager implements IManager {
    // 999策划约定屏蔽的副本
        ...
        for (const key in campaignTemplateDic) {
            if (Object.prototype.hasOwnProperty.call(campaignTemplateDic, key)) {
                ...
                if (temp.DungeonId != CampaignTemplateManager.ShieldDoundleID) {
                    chapterInfo = landInfo.getChapterById(temp.DungeonId) as CampaignChapterInfo;
    ...
    private addAreaToChapter(temp: t_s_campaignData, chapterInfo: CampaignChapterInfo, landInfo: CampaignLandInfo) {
        if (!chapterInfo) {
    ...
    private addMapTempToArea(temp: t_s_campaignData, areaInfo: CampaignAreaInfo, chapterInfo: CampaignChapterInfo) {
        if (!areaInfo) {
        ...
        if (!areaInfo.getMapById(temp.CampaignId))
            areaInfo.addMapTemplate(temp);
    }
    ...
    public getMutiCampaignList(): t_s_campaignData[] {
        ...
        for (let key in land.chapterDic) {
            if (land.chapterDic.hasOwnProperty(key) && !key.startsWith("__")) {
                // 0新手副本、3活动副本 
                if (key == CampaignTemplateManager.ShieldDoundleID.toString() || key == "0" || key == "3") continue;
