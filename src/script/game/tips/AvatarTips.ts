...
export default class AvatarTips extends BaseTips {
    ...
    private initData() {
        ...
        this.itemList.itemRenderer = Laya.Handler.create(this, this.renderItem, null, false);
    }
    ...
    setTipsData() {
        ...
        for (let i = 0; i < this.rewardData.length; i++) {
            let info: t_s_itemtemplateData = TempleteManager.Instance.getGoodsTemplatesByTempleteId(this.rewardData[i].templateId);
            if (!info) {
                return;
            }
            ...
            if (isAppell) {
                ...
                let appellInfo: t_s_appellData = TempleteManager.Instance.getAppellTemplateByID(appellId);
                if (appellInfo) {
                    this._honerView = new AppellView(appellInfo.ImgWidth, appellInfo.ImgHeight, appellInfo.TemplateId);
