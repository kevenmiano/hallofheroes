...
export class PetAvatarView extends HeroAvatarView {
  ...
  protected createNickName() {
    AvatarInfoUILayerHandler.handle_CON_CREATE(
      ...
      { fontSize: 16, shadow: true, strokeWidth: 0 },
    );
  }
  ...
  public showAvatar(
    ...
    let bodyAvatar: string = PathManager.getAvatarResourcePath(
      body,
      2,
      1,
      AvatarPosition.PET,
    ...
    if (this._bodyUrl != bodyAvatar) {
      ...
      (this._avatar as HeroAvatar).showAvatar(
        Boolean(bodyAvatar),
        AvatarPosition.PET,
      );
  ...
  public addTail() {
    ...
    if (!this._tailMc) {
      this._tailMc = FUIHelper.createFUIInstance(
        EmPackName.BaseCommon,
        PetAvatarView.TAIL_RES,
      ) as fgui.GMovieClip;
  ...
  private doFollow() {
    ...
    if (!this.stage) {
      ...
      this.distance(this, this.followTarget) > PetAvatarView.FOLLOW_DISTANCE
    ) {
      let vec2d = new Laya.Point(
  ...
  protected playMovie() {
    ...
    if (SceneManager.Instance.currentType == SceneType.SPACE_SCENE) {
      this.noShadow = false;
      if (
        (<SpaceArmyView>this.followTarget).isFlying &&
        !SpaceManager.Instance.model.getWalkable(curPos.x, curPos.y)
      ) {
        this.noShadow = true;
      }
      ...
      b = CampaignManager.Instance.mapModel.walkableValueIsPara(
        4,
        curPos.x,
        curPos.y,
      );
