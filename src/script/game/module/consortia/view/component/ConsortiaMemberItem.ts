...
export class ConsortiaMemberItem extends FUI_ConsortiaMemberItem {
  ...
  set info(value: ThaneInfo) {
    ...
    if (this._info && this._info.templateInfo) {
      ...
      this.positionTxt.text = ConsortiaDutyLevel.getDutyByButyLevel(
        this._info.dutyId,
      );
  ...
  private __onClickHandler(evt: Event) {
    ...
    let selfThane = this._contorller.getSimplePlayerInfoById(
      this.playerInfo.userId,
    );
    let itemThane = this._info;
    let pt = this.btnMore.displayObject.localToGlobal(
      new Laya.Point(0, 0),
      false,
    );
    let contributeStr =
      this._info.consortiaOffer.toString() +
      " / " +
      this._info.consortiaTotalOffer.toString();
    let jianSeStr =
      this._info.consortiaJianse.toString() +
      " / " +
      this._info.consortiaTotalJianse.toString();
    ConsortiaPlayerMenu.Show(
      selfThane,
      itemThane,
      pt,
      contributeStr,
    ...
    if (this._info) {
      if (this._info.id == ArmyManager.Instance.thane.id) {
        return LangManager.Instance.GetTranslation(
          "consortia.view.myConsortia.ConsortiaMemberItem.StateTip",
        );
      }
      if (this._info.right == UserType.ROBOT) {
        let RightTip: string = LangManager.Instance.GetTranslation(
          "consortia.view.myConsortia.ConsortiaMemberItem.RightTip",
        );
        return RightTip;
      }
      if (this._info.isOnline) {
        let StateTip: string = LangManager.Instance.GetTranslation(
          "consortia.view.myConsortia.ConsortiaMemberItem.StateTip",
        );
        return StateTip;
      }
      ...
      if ((this._info.logOutTime as Date).getTime() == 946656000000) {
        let logOutTimeTip: string = LangManager.Instance.GetTranslation(
      ...
      let num: number =
        date.getTime() - (this._info.logOutTime as Date).getTime();
      let hour: number = Math.floor(num / 1000 / 3600);
      let day: number = Math.floor(hour / 24);
      if (hour <= 0) {
        let inHourTip: string = LangManager.Instance.GetTranslation(
          "consortia.view.myConsortia.ConsortiaMemberItem.inHourTip",
        );
        return inHourTip;
      } else if (hour < 24) {
        let hourTip: string = LangManager.Instance.GetTranslation(
          "consortia.view.myConsortia.ConsortiaMemberItem.hourTip",
          hour,
        );
        return hourTip;
      } else if (day < 30) {
        let dayTip: string = LangManager.Instance.GetTranslation(
          "consortia.view.myConsortia.ConsortiaMemberItem.dayTip",
          day,
        );
        return dayTip;
      } else {
        let longTip: string = LangManager.Instance.GetTranslation(
          "consortia.view.myConsortia.ConsortiaMemberItem.longTip",
        );
        return longTip;
      }
    }
