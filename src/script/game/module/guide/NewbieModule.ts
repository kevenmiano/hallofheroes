...
export default class NewbieModule {
  ...
  public static get Instance(): NewbieModule {
    return this._instance
      ? this._instance
      : (this._instance = new NewbieModule());
  }
  ...
  public checkCommonCons(): boolean {
    if (SceneManager.Instance.lock || MopupManager.Instance.model.isMopup)
      return false;
    return true;
  }
    ...
    if (this.finishNodeStr.indexOf(nodeId.toString()) == -1) {
      ...
      if (nodeId > 0) {
        ...
        SDKManager.Instance.getChannel().postGameEvent(
          ...
          JSON.stringify({
            eventToken: GameEventString.complete_the_first_battle,
          }),
        );
      }
      if (this.recordLocal) {
