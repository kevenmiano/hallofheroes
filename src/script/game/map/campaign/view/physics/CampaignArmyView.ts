...
import { ArmyEvent, CampaignMapEvent, JoyStickEvent, NotificationEvent, ObjectsEvent, PetEvent, PhysicsEvent } from "../../../../constant/event/NotificationEvent";
...
export class CampaignArmyView extends HeroAvatarView {
    public avatarBaseViewType: eAvatarBaseViewType = eAvatarBaseViewType.CampaignArmy;
    ...
    public get isSelfConsortia(): boolean {
        if (this._data && this._data.baseHero) {
            if (StringHelper.isNullOrEmpty(this._data.baseHero.consortiaName)) return false;
            ...
            if (this.mapModel && this.mapModel.isCross) {
                ...
                return this._data.baseHero.consortiaName == this.playerInfo.consortiaName && this._data.baseHero.consortiaID == consortiaID;
            } else {
    ...
    public set data(value: CampaignArmy) {
        ...
        if (WorldBossHelper.checkPvp(this._data.mapId) || WorldBossHelper.checkMineral(this._data.mapId)) {
            arr.push(TransportMediator);
        }
        this._mediatorKey = MediatorMananger.Instance.registerMediatorList(arr, this, CampaignArmyView.NAME);
    }
    ...
    protected removeEvent() {
        ...
        if (this.thane) {
            ...
            this.thane.removeEventListener(PlayerEvent.PLAYER_INFO_UPDATE, this.__updateInfoHandler, this);
            this.thane.removeEventListener(PlayerEvent.APPELL_CHANGE, this.__updateInfoHandler, this);
        }
        NotificationManager.Instance.removeEventListener(NotificationEvent.QUIT_CAMPAIGN, this.__lockKeyHandler, this);
        ...
        if (this._data) {
            ...
            this._data.removeEventListener(CampaignMapEvent.UPDATE_CAMPAIGN_ARMY, this.__updateCampaignArmyHandler, this);
            this._data.removeEventListener(CampaignMapEvent.ONLINE_STATE, this.__onlineStateHandler, this);
            this._data.baseHero.removeEventListener(PlayerEvent.PLAYER_AVATA_CHANGE, this.__heroPropertyHandler, this);
            ...
            this._data.removeEventListener(PhysicsEvent.UPDATE_CUR_HP, this.__updateCurHpHandler, this);
            this._data.removeEventListener(CampaignMapEvent.HANGUP_STATE, this.__hangupStateHandler, this);
            this._data.removeEventListener(CampaignMapEvent.HANGUP_ADD, this.__hangupAddHandler, this);
            this._data.removeEventListener(CampaignMapEvent.BUFFER_TEMP_ID, this.__updateBufferHandler, this);
            this._data.removeEventListener(CampaignMapEvent.UPDATE_TEAM_ID, this.__updateTeamHandler, this);
            this._data.petInfo.removeEventListener(ArmyEvent.PETINFO_CHANGED, this.__updatePetAvatarHandler, this);
            this._data.removeEventListener(ArmyEvent.UPDATE_STATE, this.__refresArmyStateHandler, this);
        }

        NotificationManager.Instance.off(JoyStickEvent.JoystickUp, this.onJoystickUp, this);
        NotificationManager.Instance.off(JoyStickEvent.JoystickMoving, this.onJoystickMoving, this);
    }

    protected addEvent() {
        super.addEvent();
        this._data.addEventListener(CampaignMapEvent.UPDATE_CAMPAIGN_ARMY, this.__updateCampaignArmyHandler, this);
        ...
        this._data.addEventListener(ArmyEvent.UPDATE_STATE, this.__refresArmyStateHandler, this);
        if (this.isSelf) {
            this.thane.addEventListener(PlayerEvent.PLAYER_AVATA_CHANGE, this.__heroPropertyHandler, this);
            this.thane.addEventListener(PlayerEvent.PLAYER_INFO_UPDATE, this.__updateInfoHandler, this);
            this.thane.addEventListener(PlayerEvent.APPELL_CHANGE, this.__updateInfoHandler, this);
            NotificationManager.Instance.addEventListener(NotificationEvent.QUIT_CAMPAIGN, this.__lockKeyHandler, this);
            PlayerManager.Instance.addEventListener(PetEvent.PET_LEVEL_UP, this.__petLevelUpHandler, this);
            NotificationManager.Instance.on(JoyStickEvent.JoystickUp, this.onJoystickUp, this);
            NotificationManager.Instance.on(JoyStickEvent.JoystickMoving, this.onJoystickMoving, this);
        } else {
        ...
        this._data.addEventListener(PhysicsEvent.UPDATE_CUR_HP, this.__updateCurHpHandler, this);
        this._data.addEventListener(CampaignMapEvent.IS_DIE, this.__isDieHandler, this);
        this._data.addEventListener(CampaignMapEvent.HANGUP_STATE, this.__hangupStateHandler, this);
        this._data.addEventListener(CampaignMapEvent.HANGUP_ADD, this.__hangupAddHandler, this);
        this._data.addEventListener(CampaignMapEvent.BUFFER_TEMP_ID, this.__updateBufferHandler, this);
        this._data.addEventListener(CampaignMapEvent.UPDATE_TEAM_ID, this.__updateTeamHandler, this);
    ...
    protected setConsortiaName(name: string, nameColor: number, noSymbol: boolean = false) {
    ...
    public get isHoner(): boolean {
        ...
        if (this.isSelf) {
            ...
            return this._data.baseHero.honer >= this._data.baseHero.firstHonerTemp.Data;
        }
    }
    ...
    protected showAvatar(value: CampaignArmy) {
        ...
        if (RoomManager.Instance.roomInfo)
            roomPlayer = RoomManager.Instance.roomInfo.getPlayerByUserId(value.baseHero.userId, value.baseHero.serviceName);
        ...
        let bodyAvatar: string = thaneInfo.bodyEquipAvata == "" ? JobType.getDefaultBodyEquipNameByJob(job) : thaneInfo.bodyEquipAvata;
        let armyAvatar: string = thaneInfo.armsEquipAvata == "" ? JobType.getDefaultArmysEquipNameByJob(job) : thaneInfo.armsEquipAvata;
        //測試
        ...
        if (changeShapeId > 0) {
            ...
            armyAvatar = "";
            if (!this.avatarView) {
                this.avatarView = new HeroAvatar(bodyAvatar, AvatarResourceType.NPC, sex, delay, job);
            }
            ...
            args = this.createResourceLoadInfo(url, AvatarStaticData.BASE_NPC_STAND, AvatarStaticData.BASE_NPC_WALK, AvatarPosition.BODY);
        ...
        if (!this.avatarView) {
            this.avatarView = new HeroAvatar(bodyAvatar, AvatarResourceType.PLAYER_ARMY, sex, delay)
        }
        this.avatarView.currentMountId = mountTemplate ? mountTemplate.TemplateId : -1;
        this.objName = value.baseHero.nickName;
        this.uuid = value.id + "_" + value.baseHero.serviceName;
        ...
        Logger.xjy("[CampaignArmyView]changeShapeId=", changeShapeId, ", showFashionAvatar=", showFashionAvatar, ", mountTemplate=", value.mountTemplate)
        Logger.xjy("[CampaignArmyView]showAvatar wingAvatar=" + wingAvatar + ", cloakAvatar=" + cloakAvatar + ", hairDownAvatar=" + hairDownAvatar + ", hairUpAvatar=" + hairUpAvatar + ", armyAvatar=" + armyAvatar + ", bodyAvatar=" + bodyAvatar + ", mountAvatar=" + mountAvatar)
    ...
    private showPet(petInfo: ArmyPetInfo) {
        ...
        if (this.petAvatarView) {
            ...
            if (this.petAvatarView.followType == 1) {
                if (this._petAvatarView.parent != CampaignManager.Instance.mapView.walkLayer) {
                    CampaignManager.Instance.mapView.walkLayer.addChild(this._petAvatarView);
                    this.initPetPos();
                }
            } else {
            ...
            this.petAvatarView.showAvatar(petName, petTemQuaity, petAvatar, petInfo.petTemplateId, this.uuid);
        }
    }
    ...
    protected __heroPropertyHandler(evt: PlayerEvent) {
        ...
        let roomPlayer: CampaignArmy = RoomManager.Instance.roomInfo.getPlayerByUserId(this._data.baseHero.userId, this._data.baseHero.serviceName);
        if (!roomPlayer) {
            return;
        }
        if (this.isSelf) {
    ...
    private __updateBufferHandler(evt: CampaignMapEvent) {
        ...
        if (this._data.bufferTempId > 0) {
            this._buffEffect = new SimpleAvatarView(178, 200, PathManager.warFightBuff);
            this.addChildAt(this._buffEffect, 0);
            this._buffEffect.x = - 91;
            this._buffEffect.y = - 146;
        } else {
            if (this._buffEffect) {
                if (this._buffEffect.parent) this._buffEffect.parent.removeChild(this._buffEffect);
    ...
    private __hangupStateHandler(evt: CampaignMapEvent) {
        if (this._data.hangupState > 0) {
            if (!this._hangupView) this._hangupView = new SimpleAvatarView(80, 180, PathManager.getHookStatePath(this._data.hangupState));
    ...
    private __chatHandler(chatData: ChatData) {
        let isOpen: boolean = ChatManager.Instance.getSwitchState(CampaignManager.Instance.mapModel.mapTempInfo.MessageBoxType)
    ...
    protected newMove() {
        if (this.info.moveOverState == 1) {//移进视野
            CampaignManager.Instance.mapModel.moveToVisible(this.data);
        } else if (this.info.moveOverState == 2) {//移出视野
    ...
    private setAvaterStepFrame() {
        if (this._avatar) {
            ...
            if (this._avatar.state == Avatar.WALK && this.isSelf && (this._avatar.frameX == 0 || this._avatar.frameX == 2 || this._avatar.frameX == 4)) {
    ...
    protected playMovie() {
        ...
        if (pos.x != this._currentPos.x || pos.y != this._currentPos.y) {
            ...
            if (this._data.isDie == 0) {
                let b: boolean = this.mapModel.walkableValueIsPara(4, this.x / Tiles.WIDTH, this.y / Tiles.HEIGHT);
                this._avatar.alpha = (b ? .5 : 1);
                if (this._petAvatarView) {
                    this._petAvatarView.alpha = (b ? .5 : 1);
                }
            }
            this._data.curPosX = this._currentPos.x;
            this._data.curPosY = this._currentPos.y;
    ...
    public changeSpeedTo(precent: number = .5) {
    ...
    public dispose() {
        ...
        this.clearFireView();
        ObjectUtils.disposeViolentMc(this._bloodMc); this._bloodMc = null;
