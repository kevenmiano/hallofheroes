...
export default class CarnivalModel {
  ...
  /**获取主题子资源 */
  public getThemeFolderImgPath(
    type: CARNIVAL_FOLDER,
    name: string,
    fileType: string = ".png",
  ): string {
  ...
  public hasScoreRewardInfoAward(): boolean {
    let _tempList = TempleteManager.Instance.getCarnivalByType(
      CarnivalModel.TYPE_AWARD,
    );
  ...
  public hasDayTaskReward(): boolean {
    ...
    for (let index = 0; index < count; index++) {
      ...
      let _tempInfo: t_s_carnivaldailychallengeData =
        TempleteManager.Instance.getCarnivalDailyChallengeTempInfo(
          value.taskId,
        );
      if (_tempInfo) {
  ...
  public hasFreeDiscount(): boolean {
    ...
    let limitInfo = TempleteManager.Instance.getConfigInfoByConfigName(
      "carnival_buy_gift_limit",
    );
    if (limitInfo != null) {
    ...
    for (var i: number = 0; i < len; i++) {
      ...
      if (_tempInfo) {
        ...
        if (leftNum < 0) leftNum = 0;
        //刷新次数
        if (_tempInfo.Target == 0 && _tempInfo.Price == 0 && leftNum > 0) {
          //免费购买
          hasRewardState = true;
          break;
        }
  ...
  public hasOnlineReward(): boolean {
    ...
    let tempList = TempleteManager.Instance.getCarnivalByType(
    ...
    let onLineTimes: number =
      (new Date().getTime() - this.openFrameTime) / 1000 / 60;
