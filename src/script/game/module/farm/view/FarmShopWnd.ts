...
export default class FarmShopWnd extends BaseWindow {
    ...
    private addEvent() {
        ...
        this._model.addEventListener(ShopEvent.GOODS_LIST_UPDATE, this.refreshGoods, this);
        this.btn_buy.onClick(this, this.buyHandler);
        GoodsManager.Instance.addEventListener(BagEvent.UPDATE_BAG, this.updateHandler, this);
    }

    private removeEvent() {
        // this.tab.off(fgui.Events.STATE_CHANGED, this, this.onTabChanged);
        this._model.removeEventListener(ShopEvent.GOODS_LIST_UPDATE, this.refreshGoods, this);
        this.btn_buy.offClick(this, this.buyHandler);
        GoodsManager.Instance.removeEventListener(BagEvent.UPDATE_BAG, this.updateHandler, this);
    }
    ...
    private showBuyAlert() {
        ...
        let str: string = ShopCommWnd.getCurrencyNameByType(ShopGoodsInfo.FARM_SHOP);
        let content: string = LangManager.Instance.GetTranslation("shopcommwnd.buy.alert", totalCost, str, this.stepper.value, this.txt_name.text);
        SimpleAlertHelper.Instance.Show(SimpleAlertHelper.SIMPLE_ALERT, null, prompt, content, confirm, cancel, this.result.bind(this));
    }
    ...
    private buy(useBind: boolean = true) {
        ...
        if (this.isFashionGoods(temp)) {
            ShopManager.Instance.sendShoping(this._selectedItemData.Id, this.stepper.value, DataCommonManager.playerInfo.userId, false, C2SProtocol.C_FASHION_SHOP_BUY, useBind);
        } else {
            ShopManager.Instance.sendShoping(this._selectedItemData.Id, this.stepper.value, DataCommonManager.playerInfo.userId, false, C2SProtocol.U_C_SHOP_BUY, useBind);
        }
    }

    private renderListItem(index: number, item: FarmShopItem) {
        item.info = this._model.farmShopList[index] ? this._model.farmShopList[index] : null;
    }
        ...
        if (this._selectedItemData) {
            ...
            if (goodsTempInfo && goodsTempInfo.MasterType != GoodsType.EQUIP && goodsTempInfo.MasterType != GoodsType.HONER) {
                ...
                if (parseInt((goodsTempInfo.Property1 / 60).toString()) <= 0)
                    this.timeTxt.text = LangManager.Instance.GetTranslation("public.needMinutes", goodsTempInfo.Property1 % 60);
                else if (goodsTempInfo.Property1 % 60 == 0)
                    this.timeTxt.text = parseInt((goodsTempInfo.Property1 / 60).toString()) + LangManager.Instance.GetTranslation("public.time.hour");
                else
                    this.timeTxt.text = LangManager.Instance.GetTranslation("public.needHM", parseFloat((goodsTempInfo.Property1 / 60).toString()), goodsTempInfo.Property1 % 60);
                var outputTemp: t_s_itemtemplateData = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_itemtemplate, goodsTempInfo.Property2);
                if (outputTemp)
                    this.profitTxt.text = goodsTempInfo.Property3 + " " + outputTemp.TemplateNameLang;
                else
                    this.profitTxt.text = goodsTempInfo.Property3.toString();
            }
            this.txt_name.color = GoodsSonType.getColorByProfile(profile);
            this.updateMaxCanBuyCount();
            this.txt_hasNum.text = GoodsManager.Instance.getGoodsNumByTempId(this._selectedItemData.ItemId) + "";
            ...
            this.txt_price.text = (this.stepper.value * this._selectedItemData.price).toString();
        }
    }

    private updateMaxCanBuyCount() {
        this.MaxCanBuyCount = Math.floor(ResourceManager.Instance.gold.count / this._selectedItemData.price);
        if (this.MaxCanBuyCount <= 0) this.MaxCanBuyCount = 0;
        this.stepper.resetStepper();
        this.stepper.updateLimit(this.MaxCanBuyCount > 0 ? this.MaxCanBuyCount : 1, this.MaxCanBuyCount > 0 ? this.MaxCanBuyCount : 1);
