...
export default class UISkill extends BaseFguiCom {
  ...
  private initView() {
    ...
    this.skillLibList.on(
      fairygui.Events.CLICK_ITEM,
      this,
      this.onSkillLibListClickItem,
    );
    this.fastSkillList.on(
      fairygui.Events.CLICK_ITEM,
      this,
      this.onFastSkillListClickItem,
    );
    this.skillLibList.itemRenderer = Laya.Handler.create(
      this,
      this.onRenderListLibItem,
      null,
      false,
    );
  ...
  private addEvent() {
    if (this._data) {
      this._data.addEventListener(
        PetEvent.PET_CHANGE_SKILL_CHANGE,
        this.mPetChangeSkillChange,
        this,
      );
    }
  }
  ...
  private initSkillDataTemp() {
    if (this._data) {
      this._skillDataTempArr =
        this.shopModel.petSkill[this._data.template.PetType];
  ...
  private updateFastSkill() {
    ...
    for (let i: number = 0; i < PetData.CHANGE_SKILL_NUM; i++) {
      let item: PetFastSkillItem = this.fastSkillList.getChildAt(
        i,
      ) as PetFastSkillItem;

      const temp = this._data.getChangeSkillTemplate(
        Number(fastKey[i]),
  ...
  private updateEditMode() {
    for (let i = 0; i < this.skillLibList.numChildren; i++) {
      const skillItem: PetSkillItem = this.skillLibList.getChildAt(
        i,
      ) as PetSkillItem;
    ...
    for (let i = 0; i < this.fastSkillList.numChildren; i++) {
      const skillItem: PetFastSkillItem = this.fastSkillList.getChildAt(
        i,
      ) as PetFastSkillItem;
      if (skillItem && !skillItem.isDisposed) {
        skillItem.draggable = this.isSetting;
  ...
  private emptyFastSkillItem(info: t_s_skilltemplateData) {
    for (let index = 0; index < this.fastSkillList.numChildren; index++) {
      const item: PetFastSkillItem = this.fastSkillList.getChildAt(
        index,
      ) as PetFastSkillItem;
      if (item.info == info) {
        item.info = null;
  ...
  private getSkillLibItemBySonType(sonType: number) {
    for (let index = 0; index < this.skillLibList.numChildren; index++) {
      const item: PetSkillItem = this.skillLibList.getChildAt(
        index,
      ) as PetSkillItem;
  ...
  private onFastSkillDragStart(evt: Laya.Event) {
    let item: PetFastSkillItem = <PetFastSkillItem>(
  ...
  private updateEquipedFlag() {
    for (let index = 0; index < this.skillLibList.numChildren; index++) {
      ...
      if (item) {
        item.equiped = PetData.inFastSkillKeyTemp(
          item.info && item.info.SonType,
        );
      }
    }
  }

  private mPetChangeSkillChange(
    petData: PetData,
    skillInfo: t_s_skilltemplateData,
  ) {
  ...
  private fastEquipSkill(info: t_s_skilltemplateData, srcPoint?: Laya.Point) {
    ...
    if (srcPoint) {
      ...
      let dstPoint: Laya.Point = fastItem.parent.localToGlobal(
        fastItem.x,
        fastItem.y,
      );
      Laya.Tween.to(
  ...
  private activeDoubleSkill() {
    let cfgValue = 200;
    let cfgItem =
      TempleteManager.Instance.getConfigInfoByConfigName("pet_skillpoint");
    if (cfgItem) {
    ...
    SimpleAlertHelper.Instance.Show(
      ...
      null,
      null,
      this.__activeDoubleSkill.bind(this),
    );
  }
  ...
  private onSwitchSkillBtnClick(targetBtn) {
    ...
    if (PlayerManager.Instance.currentPlayerModel.playerInfo.isSkillEditOpen) {
      ...
      SimpleAlertHelper.Instance.Show(
        ...
        null,
        null,
        this.__onSwitchSkillBtnClick.bind(this),
      );
      return;
    }
  ...
  public btnSetClick() {
    ...
    if (
      ...
      this.btnSet.title = LangManager.Instance.GetTranslation(
