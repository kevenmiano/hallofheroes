...
export class WelfareManager extends GameEventDispatcher {
  ...
  public static get Instance(): WelfareManager {
    if (!WelfareManager._instance)
      WelfareManager._instance = new WelfareManager();
  ...
  private initEvent() {
    ServerDataManager.listen(
      S2CProtocol.U_C_GRADE_PACKET,
      this,
      this.levelGiftInfo,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_PASS_REWARD_INFO,
      this,
      this.onRecvPassRewardInfo,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_PASS_TASK_FRESH_INFO,
      this,
      this.onRecvPassTaskRefresh,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_PASS_TASK_INFO,
      this,
      this.onRecvPassTaskInfo,
    );
    // 请求 福利（绑定手机、邮箱状态）返回都是这个接口
    ServerDataManager.listen(
      S2CProtocol.U_C_MAIL_CHECK,
      this,
      this.onMailCheckInfoHandler,
    );
    PlayerManager.Instance.currentPlayerModel.playerInfo.addEventListener(
      PlayerEvent.IS_BIND_VERTIFY_PROMPTED,
  ...
  onRecvPassTaskInfo(pkg: PackageIn) {
    let msg: PassTaskAllInfoMsg = pkg.readBody(
      PassTaskAllInfoMsg,
    ) as PassTaskAllInfoMsg;
    if (!msg) return;
  ...
  private levelGiftInfo(pkg: PackageIn) {
    ...
    if (msg) {
      ...
      for (let i: number = 0; i < msg.allPacket.length; i++) {
        ...
        let levelGiftItemInfo: LevelGiftItemInfo =
          this.readLevelPacketData(item);
  ...
  private getState1(item1: GradePacketMsg, item2: GradePacketRsp): number {
    ...
    if ((item2.receiveSite >> item1.id) & 0x01) {
      ...
      this.ctrlData.currentGetPackageId =
        this.ctrlData.currentGetPackageId > item1.id
          ? this.ctrlData.currentGetPackageId
          : item1.id;
    } else {
  ...
  public isOpenBind(type: number = 1) {
    ...
    let configTemp =
      TempleteManager.Instance.getConfigInfoByConfigName("bind_open_level");
    if (configTemp) {
  ...
  public isReachOpenBindCon(type: number = 1) {
    ...
    if (configTemp) {
      let countsValue: Array<string> = configTemp.ConfigValue.split(",");
      return (
        ArmyManager.Instance.thane.grades >= parseInt(countsValue[type - 1])
      );
    }
    return false;
  }
  ...
  public getBindPhoneRedFlag() {
    return (
      !this.bindPhoneTabFirstOpen || WelfareManager.Instance.bindPhoneState == 2
    );
  }

  public getBindMailRedFlag() {
    return (
      !this.bindMailTabFirstOpen || WelfareManager.Instance.bindMailState == 2
    );
  }
  //////////////////////////////////////////////////////////////////////////////////////
    ...
    switch (taskType) {
      ...
      case 50: //神秘商店刷新{TaskNum}次商品
        if (ArmyManager.Instance.thane.grades < OpenGrades.MYSTERIOUS) {
          ...
          let str =
            LangManager.Instance.GetTranslation(
              "pveroomlist.view.PVERoomItem.command01",
      ...
      case 20: //消耗体力{TaskNum}点
        DayGuideManager.Instance.switchScene(
          DayGuideCatecory.PASS_SINGLE_CAMPAIGN,
        );
        break;
      case 9:
      ...
      case 12:
        DayGuideManager.Instance.switchScene(
          DayGuideCatecory.CONSUME_SMALL_BUGLE,
        );
        break;
      case 13:
      ...
      case 19:
        DayGuideManager.Instance.switchScene(
          DayGuideCatecory.PASS_MUL_CAMPAIGN,
        );
        break;
        ...
        if (petBossModel.isOpen) {
          if (ArmyManager.Instance.thane.grades < OpenGrades.PET) {
            ...
            SwitchPageHelp.walkToCrossMapTarget(
              petBossModel.mapId + "," + petBossModel.nodeId,
            );
          }
        } else {
          MessageTipManager.Instance.show(
      ...
      case 39: //完成{TaskNum}次英灵战役挑战
        if (
          ...
          CampaignManager.Instance.mapModel.mapId > 0
        ) {
          //单人挑战功能在副本中无法点击跳转, 修改为跳转至单人挑战界面, 但无法选择角色攻击
