...
export default class IconAlertHelperWnd extends BaseWindow {
    ...
    OnShowWind() {
        ...
        if (this.param) {
            ...
            if (this.content)
                this.popAlertFrame(this.content[0], this.content[1], this.content[2], this.content[3]);
        }
    }
    ...
    private removeEvent() {
        ...
        this.n1.getChild("closeBtn").off(Laya.Event.CLICK, this, this.cloaseHandler);
    }
    ...
    private popAlertFrame(title: string, msg: string, goodsTempID: number, goodsCount: number, enableHTML: boolean = false) {
        ...
        if (goodsTempID) {
            ...
            var temp: t_s_itemtemplateData = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_itemtemplate, gInfo.templateId.toString());
            if (temp) {
            ...
            ObjectUtils.disposeObject(this.GoodsCountTxt); this.GoodsCountTxt = null;
            ObjectUtils.disposeObject(this.item); this.item = null;
        }
        this.updatePos();
    }
    ...
    private updatePos() {
        if (this.AlertTxt.width <= this._frameMinW - 70) {
            ...
            this.Btn_cancel.x = (this.n1.width - this._frameMinW) / 4 + this._buttonGap;
            this.Btn_confirm.x = (this._frameMinW - 70) / 4 - 191 - this._buttonGap - 12;
        }
        this.AlertTxt.y = 64;
        if (this.item) {
            ...
            this.Btn_confirm.y = this.Btn_cancel.y = this.AlertTxt.y + this.AlertTxt.height + 20;
        }
        this.n1.height = this.Btn_confirm.y + 90;
    }

    private backCall(fun: Function, b: boolean) {
        var vCanFiveParams: string = LangManager.Instance.GetTranslation("yishi.utils.IconAlertHelper.CanFiveParams");
        if (fun == null) return;
        switch ((this._data.length - 1)) {
            ...
            case 4:
                ...
                break;
            case 5:
                fun(b, this._data[1], this._data[2], this._data[3], this._data[4], this._data[5]);
                break;
            default:
