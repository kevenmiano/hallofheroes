...
export default class ForgeWnd extends BaseWindow {
    ...
    public OnInitWind() {
        ...
        this._tabMainList = [this["tabQiangHua"], this["tabXiangQian"], this["tabHeCheng"], this["tabXiLian"], this["tabFenJie"], this["tabZhuanHuan"], this["tabShenZhu"]]
        let mainIdxHC = Math.floor(ForgeData.TabIndex.HC / 10)
        this._tabSubMap.set(mainIdxHC, [this["tabZB"], this["tabZBSJ"], this["tabZBJJ"], this["tabSJ"], this["tabBS"], this["tabDJ"]])
        this["tabSJ"].visible = ComponentSetting.FORGE_SJ;
        this["tabShenZhu"].visible = ComponentSetting.SHEN_ZHOU;
        this["tabZB"].getController("cFontSize").selectedIndex = isOversea() ? 1 : 0;
    ...
    public refreshBagList(data: any[]) {
        ...
        this.bagItemList.numItems = Math.ceil(len / ForgeData.BagPageSize) * ForgeData.BagPageSize
        // this.onScrollEnd()
    }
    ...
    private __bagItemDeleteHandler(info: GoodsInfo) {
        var item: StoreBagCell = this._itemDic[info.pos + "_" + info.objectId + "_" + info.bagType];
    ...
    private __bagItemUpdateHandler(info: GoodsInfo) {
        if (info.templateInfo.TransformId > 0 && this.curTabIndex != ForgeData.TabIndex.ZH) {
            // 卸下A的宝石D（情况4） 
        ...
        for (const key in this._itemDic) {
            if (Object.prototype.hasOwnProperty.call(this._itemDic, key)) {
                ...
                if (item && item.info && equiped) {
                    switch (index) {
                        case ForgeData.TabIndex.QH:
                            normal = item.info.templateInfo.SonType == GoodsSonType.SONTYPE_INTENSIFY
                            break;
                        case ForgeData.TabIndex.XQ:
                            normal = (item.info.templateInfo.SonType == GoodsSonType.SONTYPE_MOUNT) || (item.info.templateInfo.SonType == GoodsSonType.SONTYPE_MOUNT_PORP)
                            break;
                        case ForgeData.TabIndex.XL:
                            normal = (item.info.templateInfo.SonType == GoodsSonType.SONTYPE_REFRESH) || (item.info.templateInfo.TemplateId == ItemID.REFRESH_LOCK_PROP)
                            break;
                        // case ForgeData.TabIndex.FJ:
                ...
                if (!key.startsWith("__")) {
                    item.item.getChild("title")['color'] = !normal ? '#332F28' : '#FFECC6';
                }
            }
        }
    }
    ...
    private interruptCallback(changeToTabIndex: number): boolean {
        this._tabbar.soundRes = changeToTabIndex == ForgeData.TabIndex.XQ ? SoundIds.BAG_EQUIP_SOUND : SoundIds.CONFIRM_SOUND
        ...
        switch (changeToTabIndex) {
            ...
            case ForgeData.TabIndex.HC_SJ:
                if (this._openTab3) {
                    ...
                    MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("store.view.StoreFrame.command01", OpenGrades.INSERT));
                    return true
                }
            case ForgeData.TabIndex.XL:
            // case ForgeData.TabIndex.FJ:
            case ForgeData.TabIndex.ZH:
                if (this._openTab6) {
                    ...
                    MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("store.view.StoreFrame.command01", OpenGrades.REFRESH));
                    return true
                }
            case ForgeData.TabIndex.SZ:
                ...
                if (this._openTab7) {
                    ...
                    MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("store.view.StoreFrame.command01", OpenGrades.MAX_GRADE));
                    return true
                }
                break;
            default:
    ...
    public getWeaponItem(): StoreBagCell {
        for (let index = 0; index < this.bagItemList.numItems; index++) {
            ...
            if (bagCell && bagCell.info && bagCell.info.templateInfo.SonType == GoodsSonType.SONTYPE_WEAPON)
                return bagCell;
        }
        return null;
    }
    ...
    private get curBagListHasEquip() {
        for (let index = 0; index < this.bagItemList.numItems; index++) {
            const bagCell = this.bagItemList.getChildAt(index) as StoreBagCell;
            if (bagCell && bagCell.info && bagCell.info.templateInfo.MasterType == GoodsType.EQUIP) {
                return true
            }
        }
        return false
    ...
    private btnQuickAddClick() {
        if (!this.curBagListHasEquip) {
            MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("store.view.StoreFrame.notHaveFenjieProp"));
            return
        }
        ...
        for (let index = 0; index < this.bagItemList.numItems; index++) {
            ...
            if (item) {
                ...
                if (info && item.selectState == ItemSelectState.Selected) {
                    ...
                    PlayerManager.Instance.moveBagToBag(info.bagType, info.objectId, info.pos, BagType.Hide, 0, endPos, 1);
                }
            }
        }
