...
export class ChaseNPCMediator implements IMediator, IEnterFrame {
  ...
  public register(target: object) {
    ...
    if (this._selfArmy) {
      this._selfArmyView = CampaignManager.Instance.controller.getArmyView(
        this._selfArmy,
      );
    }
    NotificationManager.Instance.addEventListener(
      NotificationEvent.CHASE_NPC,
      this.__chaseNPCHandler,
      this,
    );
  }
  ...
  public unregister(target: object) {
    ...
    ObjectUtils.disposeObject(this._selectFlag);
  ...
  private unLockWarFight() {
    ...
    if (this._map)
      this._map.off(
        ...
        this,
        this.__onClickHandler.bind(this),
      );
  }
  ...
  public enterFrame() {
    ...
    if (
      !(
        this._selfArmyView &&
        this._npcView &&
        NodeState.displayState(this._npcInfo.info.state)
      )
    ) {
      this.unLockWarFight();
      return;
    ...
    if (leng < 3200) {
      ...
      CampaignArmyViewHelper.selfArmyToEnd(
        this._selfArmyView.aiInfo,
        this._selfArmyView,
      );
      EnterFrameManager.Instance.unRegisteEnterFrame(this);
    } else if (this._count % 25 == 0) {
      //跟随目标
      Logger.warn("[ChaseNPCMediator]NPC寻路");
      this.searchPath(
        this._selfArmyView.x,
        this._selfArmyView.y,
        this._targetPoint.x,
        this._targetPoint.y,
      );
    }
  ...
  private searchPath(
    ...
    var arr: any[] = SearchPathHelper.searchPath(
      new Laya.Point(startX, startY),
