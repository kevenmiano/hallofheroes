...
export class ShowPetAvatar extends Laya.Sprite {
	...
	public static figureResUnLoadMap: Map<string, boolean> = new Map<string, boolean>();
	...
	private __onResComplete(res: any) {
		...
		if (!AnimationManager.Instance.getCache(pre_url)) {
			let sucess = AnimationManager.Instance.createAnimation(pre_url, "", undefined, "", AnimationManager.BattleEffectFormatLen)
		...
		if (res.offset) {
			...
			if (offset.posDownX)
				offsetX = offset.posDownX;
			if (offset.posDownY)
				offsetY = offset.posDownY;
			if (offset.posUpY)
				this.nameHeight = offset.posDownY - offset.posUpY;
		}
