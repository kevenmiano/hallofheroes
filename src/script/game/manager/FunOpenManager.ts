...
export default class FunOpenManager extends GameEventDispatcher {
  ...
  public refreshFunOpen(aniType: number = 0) {
    ...
    // 新手本不显示
    if (
      !WorldBossHelper.checkIsNoviceMap(
    ...
    NewbieBaseActionMediator.setMainToolBarBtn(
      this.funOpenCallBack,
      null,
      showArgs,
      addShow,
      aniType,
  ...
  public checkNewFunOpen(grade: number): boolean {
    ...
    let storys: t_s_systemopentips = ConfigMgr.Instance.getSync(
