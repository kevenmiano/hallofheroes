...
export class MapUtils {
  ...
  public static getCurrentMinScreen(rect: Laya.Rectangle): string[] {
    ...
    let p: Laya.Point = new Laya.Point(
      rect.x - StageReferance.stageWidth / 2 - 100,
      rect.y - StageReferance.stageWidth / 2 - 100,
    );
    ...
    arr.push(
      MapUtils.getFileName(p.x + 1000, p.y + 1000, unitWidth, unitHeight),
    ); //中间
    if (tempX < Laya.stage.width) {
      arr.push(MapUtils.getFileName(p.x + 2000, p.y, unitWidth, unitHeight)); //中间
      arr.push(
        MapUtils.getFileName(p.x + 2000, p.y + 1000, unitWidth, unitHeight),
      ); //中间
    }
    tempY = 1000 - (p.y % 1000);
    if (tempY < 150) {
      ...
      arr.push(
        MapUtils.getFileName(p.x + 1000, p.y + 2000, unitWidth, unitHeight),
      ); //中间
    }
    return arr;
  }
