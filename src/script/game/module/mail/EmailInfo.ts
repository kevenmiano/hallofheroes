...
export default class EmailInfo {
    ...
    public getFirstGoods(): number {
        for (var i: number = 1; i <= 5; i++) {
            if (!this["IsAnnex" + i] && this["Annex" + i] != 0) return this["Annex" + i + "Name"];
        }
        return 0;
    }
    ...
    public get leftTime(): string {
        ...
        if (remain > 0) {
            ...
            time = LangManager.Instance.GetTranslation("emailII.view.EmailItemView.leftTime", time)
        } else {
            time = LangManager.Instance.GetTranslation("emailII.data.EmailInfo.leftTime.timeOut");
        }
        return time;
    }
    ...
    public remainTime(): number {
        var remain: number = 0;
        if (this.isRead) {
            remain = this.ValidityDate * 24 - (this.nowDate - this.readDate.getTime()) / (60 * 60 * 1000);
        } else {
            remain = this.ValidityDate * 24 - (this.nowDate - this.sendDate.getTime()) / (60 * 60 * 1000);
        }
