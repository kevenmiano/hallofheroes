...
export default class ArtifactCell extends FUI_ArtifactCell {
  ...
  public set info(goodsInfo: GoodsInfo) {
    ...
    if (this._goodsInfo) {
      ...
      this.isIdentify.selectedIndex = GoodsCheck.hasIdentify(this._goodsInfo)
        ? 1
        : 0;
      this.c1.selectedIndex = 2;
      let res =
        CommonConstant.QUALITY_RES[this._goodsInfo.templateInfo.Profile - 1];
      this.goodsIcon.icon = IconFactory.getGoodsIconByTID(
        this._goodsInfo.templateInfo.TemplateId,
      );
