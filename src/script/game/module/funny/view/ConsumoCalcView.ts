...
*/
export default class ConsumoCalcView extends FUI_ConsumoCalcView implements FunnyContent{
    ...
    initView() {
        if (!this._infoData || this._infoData.type == FunnyType.REDEEMING_TYPE || this._infoData.type == FunnyType.ACTIVITY_CODE_TYPE) return;
        ...
        _itemList = ArrayUtils.sortOn(_itemList, ["awardOrder", "order"], [ArrayConstant.NUMERIC, ArrayConstant.NUMERIC]);
    ...
    initEvent() {
        this.list1.itemRenderer = Laya.Handler.create(this, this.renderBoxList, null, false);
    }
     ...
     * */
    private setTimeText() {
        this.timeTxt.text = LangManager.Instance.GetTranslation("funny.FunnyRightView.active.timeText", DateFormatter.getMonthDayString(this._infoData.startTime), DateFormatter.getMonthDayString(this._infoData.endTime));
        this.setRemainTime();
    }
    ...
    public setRemainTime() {
        if (!this._infoData || this._infoData.type == FunnyType.REDEEMING_TYPE) return;
        var remainTime: number = this._infoData.endTime / 1000 - this.playerModel.sysCurTimeBySecond;
        if (remainTime >= 60) {
            this.timeTxt.text = LangManager.Instance.GetTranslation("funny.FunnyRightView.active.remainTime", DateFormatter.getFullTimeString(remainTime));
        } else if (remainTime > 0) {
            this.timeTxt.text = LangManager.Instance.GetTranslation("funny.FunnyRightView.active.remainTime", DateFormatter.getFullDateString(remainTime));
        } else {
            this.timeTxt.text = LangManager.Instance.GetTranslation("feedback.FeedBackItem.outDate");
        }

        if (this._infoData.startTime > this.playerModel.nowDate) {
            if (this._infoData.type == FunnyType.UPGRAD_LEVEL) {
                this.timeTxt.text = LangManager.Instance.GetTranslation("public.unopen");
            } else {
                this.timeTxt.text = LangManager.Instance.GetTranslation("public.unopen") + "  " + LangManager.Instance.GetTranslation("funny.FunnyRightView.active.timeText", DateFormatter.getMonthDayString(this._infoData.startTime), DateFormatter.getMonthDayString(this._infoData.endTime));
            }
        }
    }
