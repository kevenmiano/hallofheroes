...
export class PhysicsCastleView extends MapPhysicsAttackingBase {
  ...
  constructor() {
    ...
    this.addEvent();
    this._defenceView = fgui.UIPackage.createObject(
      EmWindow.OuterCity,
      "CastleDefenceView",
      CastleDefenceView,
    ) as CastleDefenceView;
  ...
  public get resourcesPath(): string {
    return PathManager.solveMapPhysicsBySonType(
      this.castleInfo.tempInfo.SonType,
    );
  }
  ...
  public get isSelfCastle(): boolean {
    if (this.castleInfo) {
      return (
        this.castleInfo.templateId == OuterCityModel.SELT_CALSTE_TEMPLATEID
      );
    }
    return false;
  }
