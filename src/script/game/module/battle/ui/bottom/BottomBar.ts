...
export class BottomBar {
  ...
  private addEvent() {
    if (this._selfInfo) {
      ...
      this._selfInfo.addEventListener(
        RoleEvent.MORPH,
        this.__morphHandler,
        this,
      );
  ...
  private removeEvent() {
    if (this._selfInfo) {
      this._selfInfo.removeEventListener(
        RoleEvent.SP,
  ...
  public get leftAwakenFigure(): SelfAwakenView {
    //新手用
  ...
  public dispose() {
    ...
    if (this._leftAwakenFigure) {
      this._leftAwakenFigure.dispose();
      this._leftAwakenFigure = null;
    }
    if (this._skillList) {
