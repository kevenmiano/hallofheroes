...
export default class ColosseumCtrl extends FrameCtrlBase {
    ...
    protected addDataListener() {
        ...
        ServerDataManager.listen(S2CProtocol.U_C_CHALLENGE_INFO, this, this.onChallengeInfo);
        ServerDataManager.listen(S2CProtocol.U_C_TAKECHALLENGE_RESULT, this, this.onTakeChallengeResult);
    }
    protected delDataListener() {
        super.delDataListener()
        ServerDataManager.cancel(S2CProtocol.U_C_CHALLENGE_INFO, this, this.onChallengeInfo);
        ServerDataManager.cancel(S2CProtocol.U_C_TAKECHALLENGE_RESULT, this, this.onTakeChallengeResult);
    }
    ...
    private __colosseumTimeHandler(pkg: PackageIn) {
        var order: BuildingOrderInfo = BuildingManager.Instance.model.colosseumOrderList[0];
        let msg: ChallengeCoolTimeMsg = pkg.readBody(ChallengeCoolTimeMsg) as ChallengeCoolTimeMsg;
    ...
    private onChallengeInfo(pkg: PackageIn) {
        ...
        for (let i = 0; i < len; i++) {
            ...
            if (thane.challengeRank == 0)
                this.data.heroList.unshift(aInfo);
            else
                this.data.heroList.push(aInfo);
        }
        ...
        for (let i = 0; i < len; i++) {
            ...
            if (aInfo) {
                ...
                let temp: t_s_itemtemplateData = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_itemtemplate, avatarInfo.clothTempId);
                if (temp) {
                    thane.bodyEquipAvata = temp.Avata;
                }
                temp = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_itemtemplate, avatarInfo.armTempId);
                if (temp) {
                    thane.armsEquipAvata = temp.Avata;
                }
                temp = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_itemtemplate, avatarInfo.fashionWingTempId);
                if (temp) thane.wingAvata = temp.Avata;
                temp = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_itemtemplate, avatarInfo.fashionArmTempId);
                if (temp) thane.armsFashionAvata = temp.Avata;
                temp = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_itemtemplate, avatarInfo.fashionClothTempId);
                if (temp) thane.bodyFashionAvata = temp.Avata;
                temp = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_itemtemplate, avatarInfo.fashionHeadTempId);
        ...
        for (let i = 0; i < len; i++) {
            ...
            eventData.logDate = DateFormatter.parse(log.logDate, "YYYY-MM-DD hh:mm:ss");
            ...
            if (this.data.eventsList.length > 10)
                this.data.eventsList.splice(9, 1);
        }
