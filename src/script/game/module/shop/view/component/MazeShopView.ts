...
export class MazeShopView extends FUI_MazeShopView {
    ...
    showBuyAlert() {
        ...
        let content: string = LangManager.Instance.GetTranslation("mazeshop.buy.alert", totalCost, this.stepper.value, this.SelectedGoodsTxt.text);
    ...
    private updateLimit() {
        ...
        if (GoodsCheck.isEquip(temp)) {//如果为装备
            ...
            if (weekLimitCount > 0) {//有周限购优先显示周限购
                ...
                leftTxt = ((weekLimitCount - weekHasBuyCount) > 0 ? "[color=#FFECC6]" : "[color=#FF0000]") + (weekLimitCount - weekHasBuyCount) + "[/color]";
                ...
                leftTxt = ((dayLimitCount - dayHasBuyCount) > 0 ? "[color=#FFECC6]" : "[color=#FF0000]") + (dayLimitCount - dayHasBuyCount) + "[/color]";
    ...
    private getName(): string {
        ...
        if (!goodsTempInfo) {
            ...
            let noFindGoods: string = LangManager.Instance.GetTranslation("consortia.view.myConsortia.building.ConsortiaShopGoodsItem.noFindGoods");
            return noFindGoods;
        }
        ...
        this.SelectedGoodsTxt.color = FilterFrameText.Colors[eFilterFrameText.ItemQuality][goodsInfo.templateInfo.Profile - 1];
    ...
    private removeEvent() {
        ...
        this._model && this._model.removeEventListener(ShopEvent.GOODS_SELECT_UPDATE, this._refreshRight, this);
        this._model && this._model.removeEventListener(ShopEvent.PAGE_UPDATE, this.__refreshPage, this);
