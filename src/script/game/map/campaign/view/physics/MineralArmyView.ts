...
import { AvatarInfoUIEvent, CampaignEvent, CampaignMapEvent, NotificationEvent } from "../../../../constant/event/NotificationEvent";
...
import { AvatarInfoUILayer, AvatarInfoUILayerHandler } from '../../../view/layer/AvatarInfoUILayer';
 ...
 * 
 */
export class MineralArmyView extends CampaignArmyView implements IBaseMouseEvent {
    ...
    public mouseClickHandler(evt: Laya.Event): boolean {
        ...
        if (this.inMineralMapAndNotInFight) {
            ...
            if (!this.isSelf && !this.isSelfConsortia && !this.isSelfTeam) {
                NotificationManager.Instance.dispatchEvent(NotificationEvent.LOCK_PVP_WARFIGHT, this.data);
            }
            evt.stopPropagation();
            return false;
        }
    ...
    public mouseOutHandler(evt: Laya.Event): boolean {
        if (!this.isSelf) {
            if (this._avatar && (<HeroAvatar>this._avatar).getCurrentPixels() >= 0) return true;
        }
        if (this._avatar) {
    ...
    private get inMineralMapAndNotInFight(): boolean {
        return (WorldBossHelper.checkMineral(this.mapModel.mapId) && this._data.state != ArmyState.STATE_FIGHT);
    }
    ...
    public mouseMoveHandler(evt: Laya.Event): boolean {
        ...
        if (this._avatar) {
            if ((<HeroAvatar>this._avatar).getCurrentPixels() >= 0) {
                ...
                return this.inMineralMapAndNotInFight && !this.isSelfConsortia && !this.isSelfTeam;
            } else {
                UIFilter.normal(this._avatar);
                if (evt) {
                    this.throughDispatch(evt);
        ...
        for (const key in arr) {
            if (Object.prototype.hasOwnProperty.call(arr, key)) {
                ...
                if (NodeState.displayState(element.info.state)) {
                    if (element.nodeView && element.nodeView != this && element.nodeView.stage) {
    ...
    private get minerals(): string {
        var result: number = 0;
        result = this.mineralModel.carInfos[this.data.baseHero.userId] ? this.mineralModel.carInfos[this.data.baseHero.userId].minerals : 0;
        return LangManager.Instance.GetTranslation("map.campaign.view.physics.MineralCarView.minerals", result);
    }
    ...
    protected setMineralText() {
        ...
        if (this._mineralTxt && !this._mineralTxt.parent) this.addChild(this._mineralTxt);
    }
    ...
    public dispose() {
        if (this._mineralTxt) this._mineralTxt.dispose(); this._mineralTxt = null;
        super.dispose();
    }
}