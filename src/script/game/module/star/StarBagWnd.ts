...
export default class StarBagWnd extends BaseWindow {
  ...
  private onClickRightItem(item: StarItemBase) {
    ...
    if (this.isProcessSell || this.isProcessCompose) {
      ...
      if (item.selectState == StarSelectState.Selectable) {
        ...
        item.itemDrag.info.pos != this._firstSelectPos
      ) {
        item.selectState = StarSelectState.Selected;
      }
    }
    ...
    for (let i: number = 0; i < PlayerModel.EQUIP_STAR_BAG_COUNT; i++) {
      ...
      if (i < openGrid) {
        ...
        if (!ComponentSetting.STAR_BAG && i > 4) {
          ...
          let openLevel =
            OpenGrades.STAR +
            (i + 1 - StarModel.BASE_OPEN_EQUIP_GRID) * StarModel.ADD_GRID_LEVEL;
          item.txtOpenDesc.text = LangManager.Instance.GetTranslation(
            "armyII.viewII.equip.EquipView.runes",
            openLevel,
          );
        }
      }
  ...
  private addEvent() {
    NotificationManager.Instance.addEventListener(
      StarEvent.UPDATE_STAR,
      this.__updateHandler,
      this,
    );
    NotificationManager.Instance.addEventListener(
      StarEvent.DELETE_STAR,
      this.__deleteHandler,
      this,
    );
    NotificationManager.Instance.addEventListener(
      StarEvent.UPDATE_STAR_POWER,
      this.__updateStarPowHandler,
      this,
    );
    NotificationManager.Instance.addEventListener(
      StarEvent.STAR_COMPOSE,
      this.__starCompose,
      this,
    );
    DataCommonManager.playerInfo.addEventListener(
      PlayerEvent.STARBAG_CAPICITY_INCRESS,
      this.__increaseBagHandler,
      this,
    );
    NotificationManager.Instance.addEventListener(
      StarEvent.START_SELL_SELECT_STATUS,
      this.__selecteHander,
      this,
    );
    NotificationManager.Instance.addEventListener(
      StarEvent.STAR_EXCHANGE,
      this.exchange,
      this,
    );
    this.on(Laya.Event.CLICK, this, this.onClickHandler);
    NotificationManager.Instance.addEventListener(
      StarEvent.STAR_NEW_COMPOSE,
      this.__starNewCompose,
      this,
    );
    NotificationManager.Instance.addEventListener(
      StarEvent.STAR_LEFT_EXCHANGE,
      this.leftExchange,
      this,
    );
    NotificationManager.Instance.addEventListener(
      StarEvent.EXIT_STAR_EXCHANGE,
      this.resetLeftExchange,
      this,
    );
    NotificationManager.Instance.addEventListener(
      StarEvent.EXIT_STAR_LEFT_EXCHANGE,
      this.resetRightExchange,
      this,
    );
  }
  ...
  private leftExchange(data: any) {
    ...
    for (let i: number = 0; i < len; i++) {
      let starItemBase: StarItemBase = this.rightStarList.getChildAt(
        i,
      ) as StarItemBase;
    ...
    this.btnOneKeyEquip.enabled =
      this.btnFinishing.enabled =
      this.btnQuickSell.enabled =
        false;
      ...
      if (item instanceof StarItem) {
        if (item.info && item.info.template) {
          if (item.info.template.TemplateId == 4) {
            MessageTipManager.Instance.show(
              LangManager.Instance.GetTranslation("starTips.clickTips"),
            );
            return;
          }
  ...
  private __updateStarPowHandler() {
    if (this.isShowing && !this.destroyed) {
      this.comStarPower.getChild("title").asLabel.text = String(
        this.starManager.starModel.getStarPow(),
      );
  ...
  private __increaseBagHandler() {
    for (
      let i: number = PlayerModel.ORIGINAL_OPEN_BAG_COUNT;
      i <
      PlayerModel.ORIGINAL_OPEN_BAG_COUNT +
        DataCommonManager.playerInfo.starBagCount;
      i++
    ...
    MessageTipManager.Instance.show(
      LangManager.Instance.GetTranslation(
        "armyII.viewII.horoscope.PlayerStarListView.command01",
      ),
    );
  }
  ...
  private exchange(data: any) {
    ...
    for (let i: number = 0; i < len; i++) {
      let starItemBase: StarItemBase = this.leftStarList.getChildAt(
        i,
      ) as StarItemBase;
      if (starItemBase && starItemBase.opened) {
        starItemBase.flag = true;
  ...
  private btnOneKeyEquipClick() {
    ...
    let equipIdlePosList: number[] =
      this.starManager.getStarEquipBagIdlePosList();
    let equipIdleNum: number = equipIdlePosList.length;
    if (equipIdleNum == 0) {
      MessageTipManager.Instance.show(
        LangManager.Instance.GetTranslation(
          "cell.view.starbag.StarBagCell.command05",
        ),
      );
      return;
    }
    ...
    playerDic.getList().forEach((info: StarInfo, pos: number) => {
      if (
        !equipMasterList[info.template.MasterType] &&
        info.template.Profile != 6
      ) {
        let existInfo = needEquipListDic.get(info.template.MasterType);
        let bMorePower =
          existInfo &&
          StarHelper.getStarTotalExp(info) >
            StarHelper.getStarTotalExp(existInfo);
    ...
    if (needEquipList.length == 0) {
      MessageTipManager.Instance.show(
        LangManager.Instance.GetTranslation("StarBagWnd.NotFindSuitableEquip"),
      );
      return;
    }
    needEquipList.sort((a, b) => {
      return StarHelper.getStarTotalExp(b) - StarHelper.getStarTotalExp(a);
    });

    // 发送移动协议
    let len =
      needEquipList.length > equipIdleNum ? equipIdleNum : needEquipList.length;
    for (let index = 0; index < len; index++) {
      const info = needEquipList[index];
      StarManager.Instance.starMove(
        info.pos,
        StarBagType.PLAYER,
        equipIdlePosList[index],
        StarBagType.THANE,
      );
    }
  }
  ...
  private btnQuickComposeConfirmClick() {
    if (!this.eatItem || !this.eatItem.info || !this.eatItem.info.template)
      return;
    ...
    if (totalGP <= 0) return;
    let tempName: string = StarHelper.getStarHtmlName(
      this.eatItem.info.template,
    );
    let content = LangManager.Instance.GetTranslation(
      "StarBagWnd.ComposeStarTip",
      tempName,
      totalGP,
    );
    ...
    if (tempLevel > sEatInfo.grade) {
      content += LangManager.Instance.GetTranslation(
        "cell.view.starbag.StarBagCell.Level.up",
        tempLevel,
      );
    }
    SimpleAlertHelper.Instance.Show(
      ...
      (b: boolean) => {
        if (b && beEatenStarArr.length > 0) {
          ...
          NotificationManager.Instance.dispatchEvent(
            StarEvent.STAR_COMPOSE_COMPLETE,
          );
          this.__starCompose({ compose: false });
        }
      },
