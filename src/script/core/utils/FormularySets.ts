...
export class FormularySets {
   ...
   *
   */
  public static calcPawnCountByCaptainAndLevel(
    captain: number,
    level: number,
  ): number {
   ...
   *
   */
  public static calcHPByLevelAndPhysique(
    level: number,
    physique: number,
  ): number {
  ...
  public static calcCritByAgility(aglity: number, level: number): number {
    return Math.floor(
      (FormularySets.calcCritValueByAgility(aglity) / level) * 0.005,
    );
  }
  /**
   * 通过护甲计算暴击值
   ...
   *
   */
  public static calcPhysicalAttackByStrength(
    strength: number,
    agility: number,
  ): number {
   ...
   *
   */
  public static calcPhysicalDefenceByStrengthAndPhysique(
    strength: number,
    physique: number,
  ): number {
   ...
   *
   */
  public static calcMagicAttackByAbility(
    ability: number,
    agility: number,
  ): number {
  ...
  public static toStringSelf(value: number, step: number): string {
    ...
    if (isOversea()) {
      if (value > step) {
        ...
        str =
          value + LangManager.Instance.GetTranslation("public.tenThousands");
      } else str = Math.floor(value).toString();
      return str;
    } else {
      if (value > step) {
