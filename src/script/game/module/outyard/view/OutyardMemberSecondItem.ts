...
export default class OutyardMemberSecondItem extends FUI_OutyardMemberSecondItem {
    ...
    private changeBtnHandler() {
        FrameCtrlManager.Instance.open(EmWindow.OutyardChangeWnd, { selectSenior: this._index, defaultSelectInfo: this._info });
    }
    ...
    public set info(value: OutyardUserInfo) {
        if (value) {
            ...
            this.addPrecentTxt2.text = (100 - OutyardManager.Instance.defenceDebuffArr[0] * this._info.defenceDebuffLevel).toString() +"%";
            ...
            var config: Array<any> = ConfigInfoManager.Instance.getStackHeadSeniorGeneralBuff();
            ...
            if(stateMsg.state == OutyardModel.FIGHTING){
                if (this._info.defenceAlive || StringHelper.isNullOrEmpty(this._info.userUid)) {//如果有人是活着的或者位置没有人
