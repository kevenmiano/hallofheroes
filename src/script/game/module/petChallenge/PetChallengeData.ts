...
import { PetChallengeEvent, PetChallengeRewardType, PetChallengeTimeRewardType } from "../../constant/PetDefine";
...
export default class PetChallengeData extends FrameDataBase {
    ...
    public commitInfoChange() {
        NotificationManager.Instance.dispatchEvent(PetChallengeEvent.CHALLENGE_INFO_CHAGNE);
    }

    public commitTimeChange() {
        NotificationManager.Instance.dispatchEvent(PetChallengeEvent.CHALLENGE_TIME_CHANGE);
    }

    public commitEventChange() {
        NotificationManager.Instance.dispatchEvent(PetChallengeEvent.CHALLENGE_EVENT_CHANGE);
    }
    ...
    private initRewardData(): void {
        ...
        let valueArr: any[] = LangManager.Instance.GetTranslation("PetChallengeRewardTemplate.allAddValue").split(",");
        let appellArr: any[] = LangManager.Instance.GetTranslation("PetChallengeRewardTemplate.allAppellValue").split(",");
        ...
        for (let i: number = 1; i <= 3; i++) {
            ...
            let key = temp.RewardType.toString() + temp.Type.toString() + temp.rankIdx;
            ...
            temp.Type = PetChallengeTimeRewardType.WeekReward;//周奖励
            temp.rankIdx = i;
            var appell: t_s_appellData = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_appell, appellArr[i]);
            if (appell) {
        ...
        let dropArr: t_s_dropconditionData[] = TempleteManager.Instance.getDropConditionByType(38)
        ...
        if (!this._weekGoodRewards) {
            ...
            for (const key in dic) {
                if (Object.prototype.hasOwnProperty.call(dic, key)) {
                    ...
                    if (temp.Type == PetChallengeTimeRewardType.WeekReward && temp.RewardType == PetChallengeRewardType.GOODS) {
                        this._weekGoodRewards.push(temp);
                    }
                }
            }
            ArrayUtils.sortOn(this._weekGoodRewards, ["RewardType", "TemplateId"], [ArrayConstant.NUMERIC | ArrayConstant.DESCENDING, ArrayConstant.NUMERIC])
        }
     ...
     * 失败返加负
     */
    public static getResultScore(selfScore: number, defencerScore: number, win: boolean) {
        let reScore = 0;
        let dValue = selfScore - defencerScore;
        if (win) {
            let attackAdd = Number(TempleteManager.Instance.getConfigInfoByConfigName("Pet_arena_reward_win").ConfigValue);
            ...
            let attackReduce = Number(TempleteManager.Instance.getConfigInfoByConfigName("Pet_arena_reward_lose").ConfigValue);
