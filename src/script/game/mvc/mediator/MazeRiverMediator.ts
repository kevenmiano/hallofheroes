...
export default class MazeRiverMediator implements IMediator {
    ...
    register(target: any) {
        ...
        if (this._selfArmy) {
            this._selfArmy.addEventListener(CampaignMapEvent.IS_DIE, this.__isDieHandler, this);
            this.__isDieHandler(null);
        }
    }

    unregister(target: any) {
        if (this._selfArmy) this._selfArmy.removeEventListener(CampaignMapEvent.IS_DIE, this.__isDieHandler, this);
    }
