...
export default class LuckyExchangeItem extends FUI_LuckyExchangeItem {
    ...
    protected onConstruct() {
        ...
        this.iconList.itemRenderer = Laya.Handler.create(this, this.renderIconList, null, false);
    }
    ...
    private refreshView() {
        ...
        let infoData: LuckExchangeTempMsg = FunnyManager.Instance.getLuckExchangeShowData(showID);
        let value:number = 0;
        if (this._type == 0) {
        ...
        let list: Array<LuckExchangeItemTempMsg> = LuckyExchangeManager.Instance.getGoodsByDropType(infoData,value);
        this._rareInfoDataList = this.getDataList(list);
        this.iconList.numItems = this._rareInfoDataList.length;

        if (this._type == 0) {//一定获得以下物品
            this.titleTxt.text = LangManager.Instance.GetTranslation("LuckyExchangeItem.titleTxt2");
        }
        else if (this._type == 1) {//有几率获得以下物品
            this.titleTxt.text = LangManager.Instance.GetTranslation("LuckyExchangeItem.titleTxt3");
        }
        else if (this._type == 2) {
            if (LuckyExchangeManager.Instance.needSpecialTxt) {//并随机获得以下一个物品
                this.titleTxt.text = LangManager.Instance.GetTranslation("LuckyExchangeItem.titleTxt1");
            }
            else {//随机获得以下一个物品
                this.titleTxt.text = LangManager.Instance.GetTranslation("LuckyExchangeItem.titleTxt4");
            }
        }
    }
