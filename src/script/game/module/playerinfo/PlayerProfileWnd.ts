...
export class PlayerProfileWnd extends BaseWindow {
    ...
    public OnInitWind() {
        ...
        this.txt_consortia.text = thane.consortiaName ? thane.consortiaName : LangManager.Instance.GetTranslation("armyII.viewII.attribute.PlayerAttributeView.Not");
        ...
        let appellData: t_s_appellData = TempleteManager.Instance.getAppellInfoTemplateByID(thane.appellId) as t_s_appellData;
        if (appellData) {
            this._honerView = new AppellView(appellData.ImgWidth, appellData.ImgHeight, thane.appellId);
            ...
            this.txt_title.text = LangManager.Instance.GetTranslation("armyII.viewII.attribute.PlayerAttributeView.Not");
            if (this._honerView) this._honerView.visible = false;
            this.txt_title.visible = true;
        }

        this.txt_title.text = thane.appellInfo ? thane.appellInfo.TitleLang : LangManager.Instance.GetTranslation("armyII.viewII.attribute.PlayerAttributeView.Not");
        if (thane) {
        ...
        if (thane.frameId > 0) {
            let itemData: t_s_itemtemplateData = TempleteManager.Instance.getGoodsTemplatesByTempleteId(thane.frameId);
            if (itemData) {
