...
export class PetSaveWnd extends BaseWindow {
  ...
  public OnInitWind(): void {
    ...
    this.txtFrameTitle.text =
      LangManager.Instance.GetTranslation("PetSave.txt0");
  ...
  private onRenderPetItem(index: number, item: PetSaveItem) {
    ...
    if (petData) {
      ...
      if (this.petList.numItems == 1) {
        ...
        item.selectState = petData.isSave
          ? ItemSelectState.Selected
  ...
  okBtnClick() {
    ...
    if (!isAllSave) {
      ...
      MessageTipManager.Instance.show(
        LangManager.Instance.GetTranslation("PetSave.txt6"),
      );
    } else {
      this.isFlash = false;
