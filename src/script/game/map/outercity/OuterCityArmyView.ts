...
import { ArmyEvent, OuterCityEvent, PetEvent, PhysicsEvent } from "../../constant/event/NotificationEvent";
 ...
 * @ver 1.0
 */
export class OuterCityArmyView extends HeroAvatarView implements IBaseMouseEvent //ITipedDisplay
{
    ...
    public avatarBaseViewType: eAvatarBaseViewType = eAvatarBaseViewType.OuterCityArmy;
    ...
    public static NAME: string = "map.outercity.view.mapphysics.OuterCityArmyView";
    ...
    private get isSelfConsortia(): boolean {
        if (this._armyInfo && this._armyInfo.baseHero) {
            ...
            return this._armyInfo.baseHero.consortiaName == this.playerInfo.consortiaName;
        }
        return false;
    }
    ...
    private get honerStyle(): string {
        return "map.campaign.HonerText" + this._armyInfo.baseHero.appellInfo.Quality;
    }

    private get honerName(): string {
        return this._armyInfo.baseHero.appellInfo && this._armyInfo.baseHero.appellInfo.TitleLang;
    }
    ...
    protected walkOver(): void {
        ...
        this.standImp();
        if (this._armyInfo.userId == this.selfUserId) {
            NotificationManager.Instance.dispatchEvent(OuterCityEvent.MOVE_OVER, new Point(this.x, this.y));
        }
    }
    ...
    protected playMovie(): void {
        ...
        this._count++;
        if (this._count > 10) {
            let b: boolean = OutercityNpcActivationMediator.checkOutScene(this, this.sizeInfo, this.parent as Sprite);
            this.isPlaying = !b;
            this._count = 0;
        }
    ...
    public set armyInfo(value: UserArmy) {
        ...
        value.addEventListener(ArmyEvent.UPDATE_STATE, this.__updateArmyStateHandler, this);
        if (value.userId == this.playerInfo.userId) {
            ...
            value.baseHero.addEventListener(PlayerEvent.PLAYER_AVATA_CHANGE, this.__heroPropertyHandler, this);
        }
        value.baseHero.addEventListener(PlayerEvent.PLAYER_INFO_UPDATE, this.__updateInfoHandler, this);
        value.petInfo.addEventListener(ArmyEvent.PETINFO_CHANGED, this.__updatePetAvatarHandler, this);
        this._mediatorKey = MediatorMananger.Instance.registerMediatorList(arr, this, OuterCityArmyView.NAME);
    ...
    private showPet(petInfo: ArmyPetInfo): void {
        ...
        if (this.petAvatarView) {
            ...
            if (this.petAvatarView.followType == 1) {
                if (this._petAvatarView.parent != OuterCityManager.Instance.mapView.worldWalkLayer) {
    ...
    private showAvatar(value: UserArmy): void {
        ...
        this.avatarView.scaleX = this.avatarView.scaleY = .85;
    ...
    public set avatarView(value: Avatar) {
        if (this._avatar) {
            this._avatar.off(Avatar.SIZETYPE_CHANGE, this, this.__onSizeTypeChangeHandler);
        }
    ...
    public dispose(): void {
        ...
        if (this._armyInfo) {
            this._armyInfo.baseHero.removeEventListener(PlayerEvent.PLAYER_AVATA_CHANGE, this.__heroPropertyHandler, this);
            this._armyInfo.baseHero.removeEventListener(PlayerEvent.PLAYER_INFO_UPDATE, this.__updateInfoHandler, this);
            this._armyInfo.removeEventListener(PhysicsEvent.CHAT_DATA, this.__chatHandler, this);
            this._armyInfo.removeEventListener(ArmyEvent.UPDATE_STATE, this.__updateArmyStateHandler, this);
        ...
        this._armyInfo.petInfo.removeEventListener(ArmyEvent.PETINFO_CHANGED, this.__updatePetAvatarHandler, this);
        if (this._attacking) {
    ...
    public mouseClickHandler(evt: Laya.Event): boolean {
        ...
        if ((this.parent as WorldWalkLayer).checkClickPlayerNum(evt.stageX, evt.stageY)) return true;


        if ((<HeroAvatar>this._avatar).getCurrentPixels() < 50) {
            if (CursorManagerII.Instance.currentState != CursorManagerII.PVP_ATTACT_CURSOR) {
                OuterCityManager.Instance.mapView.worldWalkLayer.mouseClickHandler(null);
            }
            return false;
        }
        if (this._armyInfo.baseHero.userId != PlayerManager.Instance.currentPlayerModel.playerInfo.userId) {
            if (CursorManagerII.Instance.currentState == CursorManagerII.PVP_ATTACT_CURSOR) {
    ...
    public armyBack(): void {
        let str: string = LangManager.Instance.GetTranslation("map.OuterCityArmyView.command01")
        MessageTipManager.Instance.show(str);
    }


    private __chatHandler(chatData: ChatData): void {
        let isOpen: boolean = ChatManager.Instance.getSwitchState(OuterCityManager.Instance.model.mapTempInfo.MessageBoxType)
    ...
    public get isTeammate() {
        if (!this.isSelf) {
            if (FreedomTeamManager.Instance.inMyTeam(this._armyInfo && this._armyInfo.userId)) {
                return true;
            }
        }
        return false;
