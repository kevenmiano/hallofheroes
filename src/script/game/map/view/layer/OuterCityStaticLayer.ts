...
export class OuterCityStaticLayer extends SimpleMapLayer {
  ...
  private __resizeHandler(evt) {
    ...
    if (
      StageReferance.stageWidth <= MapData.stageWidth &&
      StageReferance.stageHeight <= MapData.stageHeight
    ) {
      ...
      if (
        oModel &&
        MapData.mapId == oModel.mapId &&
        MapData.mapBitmap &&
        this.checkPointEquit(oModel.targetPoint, MapData.movePos)
      ) {
        this._isOldMap = true;
        ...
        this._floorBitmap.size(
          StageReferance.stageWidth + OuterCityStaticLayer.drawOff * 2,
          StageReferance.stageHeight + OuterCityStaticLayer.drawOff * 2,
        );
      }
    } else {
      // this._floorBitmap.graphics.drawTexture(null, 0, 0, StageReferance.stageWidth + OuterCityStaticLayer.drawOff * 2, StageReferance.stageHeight + OuterCityStaticLayer.drawOff * 2, null, 1, "#FF0000");
  ...
  public upDateNextRend() {
    ...
    this._firstTimeId = 0;
    if (this._isFirst) {
      this._firstTimeId = setInterval(
        this.upDateNextRend.bind(this),
        500,
  ...
  private draw() {
    ...
    let rect: Laya.Rectangle = new Laya.Rectangle(
      0,
      0,
      this._floorBitmap.width,
      this._floorBitmap.height,
    );
    ...
    for (
      let i = 0, len = range.length;
      i < len;
      i++ //地图的层次 从102 至...
    ) {
      const index = range[i];
      for (const key in dic) {
        //piece 为 1000X1000的区域
          ...
          if (arr) {
            for (const key in arr) {
              if (arr.hasOwnProperty(key)) {
                ...
                let bit: Laya.Texture =
                  MapElmsLibrary.Instance.getElementByPath(
                    ComponentSetting.RES_ANIMATION + info.url,
                  );
                if (bit) {
                  point.x =
                    start.x +
                    (info.x % 1000) +
                    (this.parent as Laya.Sprite).x +
                    OuterCityStaticLayer.drawOff;
                  point.y =
                    start.y +
                    (info.y % 1000) +
                    (this.parent as Laya.Sprite).y +
                    OuterCityStaticLayer.drawOff;
                  rect.width = bit.width;
                  rect.height = bit.height;
                  buffer.graphics.drawTexture(
                    bit,
                    point.x,
                    point.y,
                    rect.width,
    ...
    let bufferTexture = buffer.drawToTexture(
      StageReferance.stageWidth + OuterCityStaticLayer.drawOff * 2,
      StageReferance.stageHeight + OuterCityStaticLayer.drawOff * 2,
      0,
      0,
    ...
    this.drawBackground();
    this._floorBitmap.x =
      -(this.parent as Laya.Sprite).x - OuterCityStaticLayer.drawOff;
    this._floorBitmap.y =
      -(this.parent as Laya.Sprite).y - OuterCityStaticLayer.drawOff;
  }
