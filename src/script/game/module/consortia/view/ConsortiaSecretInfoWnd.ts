...
export default class ConsortiaSecretInfoWnd extends BaseWindow {
  ...
  private initData() {
    ...
    let configTemp: ConfigInfosTempInfo =
      TempleteManager.Instance.getConfigInfoByConfigName("Fam_Buffer_Price");
  ...
  private initEvent() {
    ...
    this._model.addEventListener(
      ...
      this.chairmanChangeHandler,
      this,
    );
    this._model.secretInfo.addEventListener(
      ConsortiaEvent.SECRET_UPDATE,
      this.secretUpdateHandler,
      this,
    );
    this._model.secretInfo.addEventListener(
      ConsortiaEvent.PICK_TIME_UPDATE,
      this.pickTimeUpdateHandler,
      this,
    );
  ...
  private removeEvent() {
    ...
    this._model.secretInfo.removeEventListener(
      ConsortiaEvent.SECRET_UPDATE,
      this.secretUpdateHandler,
      this,
    );
  ...
  private onHelpClick() {
    ...
    let content: string = LangManager.Instance.GetTranslation(
      "map.campaign.view.ui.secretland.ConsortiaSecretInfoView.helpContent",
    );
  ...
  private onGetTreeClick() {
    ...
    var content: string = LangManager.Instance.GetTranslation(
      "map.campaign.view.ui.secretland.ConsortiaSecretInfoView.callTree",
    );
  ...
  private showGetBuffAlert(): boolean {
    var preDate: Date = new Date(
      SharedManager.Instance.secretGetBuffTipCheckDate,
    );
    ...
    if (outdate) {
      var content: string = LangManager.Instance.GetTranslation(
        "map.campaign.view.ui.secretland.ConsortiaSecretInfoView.getBuff",
        this._secretBuffPrice,
      );
      UIManager.Instance.ShowWind(EmWindow.TodayNotAlert, {
        content: content,
        backFunction: this.startGetBuffAlertBack.bind(this),
        closeFunction: null,
        state: 2,
      });
    }
  ...
  onCallMonsterClick() {
    ...
    var prompt: string = LangManager.Instance.GetTranslation("public.prompt");
    var content: string = LangManager.Instance.GetTranslation(
      "map.campaign.view.ui.secretland.ConsortiaSecretInfoView.callMonster",
    );
    SimpleAlertHelper.Instance.Show(
      SimpleAlertHelper.SIMPLE_ALERT,
      null,
      ...
      confirm,
      cancel,
      this.callMonsterCall.bind(this),
    );
  }
  ...
  private refreshBtns() {
    this.callTreeBtn.visible =
      this.getBufferBtn.visible =
      this.callMonsterBtn.visible =
        false;
    this._btnCount = 0;
    if (!this._info) return;
    if (
      this._info.treeState == ConsortiaSecretInfo.Call_STATE &&
      this.isChairman(ConsortiaDutyInfo.CALL_TREE)
    ) {
  ...
  private refreshInfoView() {
    ...
    this.treeLevelTxt
      .setVar("level", this._info.treeGrade.toString())
      .flushVars();
    var upGrade: t_s_upgradetemplateData =
      TempleteManager.Instance.getTemplateByTypeAndLevel(
        this._info.treeGrade + 1,
        UpgradeType.UPGRADE_TYPE_CONSORTIA_TREE,
      );
    if (upGrade) {
      this.expValueTxt.text = this._info.treeGp + " / " + upGrade.Data;
      this.precentImg.fillAmount = this._info.treeGp / upGrade.Data;
    } else {
      this.expValueTxt.text = LangManager.Instance.GetTranslation(
        "map.campaign.view.ui.secretland.ConsortiaTreeExpView.maxLevel",
      );
  ...
  public refreshTimeView() {
    if (this._info && this._info.remainGainTime > 0) {
      ...
      this.timeTxt1.text = DateFormatter.getSevenDateString(
  ...
  private updateOffMonsterHandler() {
    this.timeTxt2.text = DateFormatter.getSevenDateString(
