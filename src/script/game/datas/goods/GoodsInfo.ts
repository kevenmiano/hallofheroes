...
export class GoodsInfo {
  ...
  public get mouldRankShow(): number {
    if (
      this._mouldRank > ForgeData.MOULD_MAX_RANK &&
      this._mouldRank <= ForgeData.MOULD_MAX_RANK_SENIOR
    ) {
      //8-16
      return this._mouldRank - ForgeData.MOULD_MAX_RANK;
    } else if (this._mouldRank > ForgeData.MOULD_MAX_RANK_SENIOR) {
      //16-24
  ...
  public get isSeniorEquip(): boolean {
    if (
      this.templateInfo &&
      this.templateInfo.MasterType == GoodsType.EQUIP &&
      !GoodsCheck.isFashion(this.templateInfo)
    ) {
  ...
  public get isSeniorEquipMaterial(): boolean {
    if (
      this.templateInfo &&
      this.templateInfo.StrengthenMax > 0 &&
      this.templateInfo.Profile == 5 &&
      this.templateInfo.NeedGrades == 80
    ) {
      return true;
    }
    return false;
  ...
  public get isWearyItem() {
    return (
      this.templateInfo &&
      this.templateInfo.Property1 == 5 &&
      this.templateInfo.Property2 > 0
    );
  }
  /**
   *二级密码锁 true:已上锁
  ...
  public get totalAttribute(): number {
    return (
      this.templateInfo.MagicAttack +
      this.templateInfo.Attack +
      this.templateInfo.MagicDefence +
      this.templateInfo.Defence +
      this.templateInfo.Conat +
  ...
  public getBasePropertyScore(): number {
    ...
    let count: number =
      ...
      Number(temp.Live / 5) +
      temp.Conat +
      (temp.Power + temp.Agility + temp.Intellect) * 4 +
      (temp.Captain + temp.Physique) * 2;
  ...
  public getEquipAdditionScore(): number {
    ...
    count =
      ...
      (this.getAddition(temp.Power) +
        this.getAddition(temp.Agility) +
        this.getAddition(temp.Intellect)) *
        4 +
      (this.getAddition(temp.Captain) + this.getAddition(temp.Physique)) * 2;

    count +=
      this.getMouldAddition(temp.Attack) +
      ...
      (this.getMouldAddition(temp.Power) +
        this.getMouldAddition(temp.Agility) +
        this.getMouldAddition(temp.Intellect)) *
        4 +
      (this.getMouldAddition(temp.Captain) +
        this.getMouldAddition(temp.Physique)) *
        2;
  ...
  private getAddition(preValue: number): number {
    ...
    return (
      Number(preValue * this.strengthenGrade * 0.1) + this.strengthenGrade * 5
    );
  }
  ...
  private getById(id: number): number {
    ...
    let count: number =
      skillPropertyInfo.Attack +
      skillPropertyInfo.Defence +
      skillPropertyInfo.MagicAttack +
      skillPropertyInfo.MagicDefence +
      skillPropertyInfo.ForceHit +
      Number(skillPropertyInfo.Live / 5) +
      skillPropertyInfo.Conat +
      (skillPropertyInfo.Power +
        skillPropertyInfo.Agility +
        skillPropertyInfo.Intellect) *
        4 +
