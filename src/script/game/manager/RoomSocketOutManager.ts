...
export class RoomSocketOutManager {
  ...
  public static sendChangeRoomOwner(targetId: number, serverName: string = "") {
    ...
    SocketManager.Instance.send(
      C2SProtocol.U_C_CAMPAIGN_ROOM_MASTER_CHANGE,
      msg,
    );
  }
   ...
   * @param targetUserId
   */
  public static crossSendKickPlayerAlert(
    targetUserId: number,
    nameKey: string = "",
  ) {
    var content: string = LangManager.Instance.GetTranslation(
      "room.manager.RoomSocketOutManager.content",
    );
    SimpleAlertHelper.Instance.Show(
      null,
      [targetUserId, nameKey],
      null,
      content,
      null,
      null,
      RoomSocketOutManager.crossKickPlayerAlertCallBack,
    );
  }

  private static crossKickPlayerAlertCallBack(
    b: boolean,
    flag: boolean,
    data: any,
  ) {
  ...
  public static sendKickPlayerAlert(targetUserId: number) {
    ...
    SimpleAlertHelper.Instance.Show(
      ...
      null,
      null,
      RoomSocketOutManager.kickPlayerAlertCallBack,
    );
  }
  private static kickPlayerAlertCallBack(
    b: boolean,
    flag: boolean,
    targetUserId: number,
  ) {
  ...
  public static sendRequestTeamPosInSpace(
    isGet: boolean = true,
    list: Array<any> = [],
  ) {
