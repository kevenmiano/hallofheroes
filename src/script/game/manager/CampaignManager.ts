...
export class CampaignManager {
    ...
    private addEvent() {
        ServerDataManager.listen(S2CProtocol.U_C_CAMPAIGN_ENTER, this, this.__campaignStartHandler);
    }

    private removeEvent() {
        ServerDataManager.cancel(S2CProtocol.U_C_CAMPAIGN_ENTER, this, this.__campaignStartHandler);
    }
    ...
    private readMember(mInfo: RoomPlayerMsg) {//读取房间成员
        ...
        if (userId == selfBaseHero.userId && serverName == selfBaseHero.serviceName) {
    ...
    public dispose() {
        ...
        this.removeEvent();
        this.mapId = 0;
        NotificationManager.Instance.dispatchEvent(CampaignEvent.CAMPAIGN_OVER, null);
        CampaignManager.CampaignOverState = false;
    ...
    public needHidePlayerScene(mapId: number): boolean {
        return WorldBossHelper.checkWorldBoss(mapId) || WorldBossHelper.checkPetLand(mapId);
    }
}