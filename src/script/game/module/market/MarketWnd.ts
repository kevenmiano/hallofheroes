...
export default class MarketWnd extends BaseWindow {
  ...
  private initTranslate() {
    ...
    this.sellBtn.title =
      LangManager.Instance.GetTranslation("Market.main.sell");
  }

  private addEvent() {
    this.sellBtn.onClick(this, this.onSellTap);
    this.marketTabs.itemRenderer = Laya.Handler.create(
      this,
      this.onTableRender,
      null,
      false,
    ...
    MarketManager.Instance.addEventListener(
      MarketManager.OnMarketPurcharseList,
  ...
  private OnMineOrderList(marketOrders: IMarketOrderMsg[]) {
    ...
    this.updatePush(
      marketOrders.length,
      MarketManager.Instance.marketPuchareMax,
    );
  }
  ...
  private OnMarketOrderOption(msg: MarketOrderRespMsg) {
    ...
    if (msg.result && msg.op == 1 && msg.type == 2) {
      MessageTipManager.Instance.show(
        LangManager.Instance.GetTranslation("market.buy.pushSuccess"),
      );
      return;
    }
    ...
    if (!msg.result && msg.op == 1 && msg.type == 2) {
      MessageTipManager.Instance.show(
        LangManager.Instance.GetTranslation("Market.buy.failed"),
      );
      return;
    }
    ...
    if (!msg.result && msg.op == 1 && msg.type == 1) {
      MessageTipManager.Instance.show(
        LangManager.Instance.GetTranslation("market.sell.pushFailed"),
      );
      return;
    }
  }
  ...
  private updatePush(c: number, t: number) {
    this.pushLab.text = LangManager.Instance.GetTranslation(
      "Market.sell.pushCount",
      c,
      t,
    );
  }
