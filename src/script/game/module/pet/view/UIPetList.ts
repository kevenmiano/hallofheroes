...
export default class UIPetList {
  ...
  private onRenderListItem(index: number, item: PetItem) {
    ...
    if (!itemData) {
      item.info = null;
      let playerPetMaxCount: number =
        PlayerManager.Instance.currentPlayerModel.playerInfo.petMaxCount;
  ...
  private activePetBag(pos: number) {
    ...
    let temp: ConfigInfosTempInfo =
      TempleteManager.Instance.getConfigInfoByConfigName("add_pet_count");
    if (temp) {
    ...
    for (let i: number = 0; i < count; i++) {
      let cost: number = Math.min(
        Number(arr[0]) + activedCount * Number(arr[1]),
        Number(arr[2]),
      );
      totalCost += cost;
      activedCount++;
    ...
    SimpleAlertHelper.Instance.Show(
      ...
      null,
      null,
      (result: boolean, flag: boolean, data: any = 0) => {
        if (result) {
          let hasMoney: number;
          if (flag) {
