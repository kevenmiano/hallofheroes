...
export class FashionManager extends GameEventDispatcher {
  ...
  private __receivedBookHandler(pkg: PackageIn) {
    ...
    for (let j: number = 0; j < idLeng; j++) {
      ...
      if (
        this.fashionModel.bookList[msg.fashionIds[j]] != null ||
        goodsTemplateInfo.Property3 != 1
      ) {
  ...
  public get fashionIdentityProgress(): number {
    ...
    if (this.fashionModel.identityedList.length > 0) {
      //如果鉴定过则不闪
      return 10000;
    }
    // 0 背包 1 时装 2 时装图鉴
  ...
  getOptType(info: GoodsInfo): string {
    if (this.fashionModel.selectedPanel == FashionModel.FASHION_COMPOSE_PANEL) {
      ...
      } else if (info.bagType == BagType.Hide) {
        //从合成区取出
  ...
  getBatchType(info: GoodsInfo): string {
    if (this.fashionModel.selectedPanel == FashionModel.FASHION_COMPOSE_PANEL) {
      //合成
      if (info.bagType == BagType.Player && info.count > 1) {
        //批量放进合成区
        if (FashionManager.Instance.canMoveToHideBag(info, true)) {
  ...
  putInOut(info: GoodsInfo, count = 1) {
    ...
    if (this.fashionModel.scene == FashionModel.SHOP_SCENE) {
      //商城时装界面
      if (
        info.moveType == FashionModel.TRY_ON &&
        this.fashionModel.isFashion(info)
      ) {
      ...
      if (
        ...
        if (info.bagType == BagType.Player) {
          ...
          if (FashionManager.Instance.canMoveToHideBag(info, true)) {
            if (
              this.fashionModel.isFashion(info) &&
              FashionManager.Instance.getFashionLevelByInfo(info) == 9
            ) {
              MessageTipManager.Instance.show(
                LangManager.Instance.GetTranslation(
      ...
      if (
        ...
        if (
          ...
          info.bagType == BagType.HeroEquipment
        ) {
          //放进转换区
