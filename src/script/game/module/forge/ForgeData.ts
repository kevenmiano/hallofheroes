...
export default class ForgeData extends FrameDataBase {
    ...
    // 2180022        冰牙卡
    // 2180028        魔兽·影灵卡
    public static ComposeOne = [2180003, 2180004, 2180005, 2180013, 2180022, 2180028]
    ...
    public refreshBagData(index: number = 0) {
        this.bagData = [];
        let arr: any[] = [];
        let arr1: any[] = GoodsManager.Instance.getHeroEquipListById(this.thane.id).getList();
        ...
        for (let i: number = 0; i < arr.length; i++) {
            ...
            if (!GoodsManager.Instance.isGneralBagGoods(gInfo) && !GoodsManager.Instance.isHeroGoods(this.uArmy.baseHero ? this.uArmy.baseHero.id : 0, gInfo))
                continue;
            if (this.filterGoods(gInfo))
                this.bagData.push(gInfo);
        }
    }
    ...
    public set currentComposeCount(value: number) {
        ...
        if ((this._selectedCompose && value == 0) && (this._selectedCompose.canMakeCount > 0 || (this._selectedCompose.template)))
            value = 1;
    ...
    public getSuitName(itemType: number, groupId: number): string {
        if (groupId == eForgeComposeEquipGroup.HERALDRY_RELIC) {
            return LangManager.Instance.GetTranslation("store.view.compose.ComposeCellButton.suitGroupS1");
        }
        switch (itemType) {
            case eForgeComposeTargetItemType.SUIT_MENU_CELL:
                return LangManager.Instance.GetTranslation("store.view.compose.ComposeCellButton.suitGroup", groupId);
            case eForgeComposeTargetItemType.EPIC_MENU_CELL:
                return LangManager.Instance.GetTranslation("store.view.compose.ComposeCellButton.suitEpicGroup", groupId);
            default:
                return "";
        }
    }
    ...
    public getTreeDataByType(type: number) {
        ...
        var playerJob = PlayerManager.Instance.currentPlayerModel.playerInfo.job % 3;
        if (playerJob == 0) playerJob = 3;
        let isReverse:boolean=false;//是否倒序
        if(type == ForgeData.COMPOSE_TYPE_EQUIP || type ==ForgeData.COMPOSE_TYPE_UPGRADE_EQUIP || type ==ForgeData.COMPOSE_TYPE_ADVANCE_EQUIP){
            isReverse = true;
        }
        ...
        if(isReverse){
            for (let index = slist.length-1; index >=0; index--) {
                ...
                if (cGoodInfo.template.NewMaterial.toString().substr(0, 3) == CommonConstant.MOUNT_CARD) {
                ...
                let temp = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_itemtemplate, cGoodInfo.template.NewMaterial);
                ...
                cGoodInfo.cellType = type == ForgeData.COMPOSE_TYPE_ADVANCE_EQUIP ? eForgeComposeTargetItemType.EPIC_CELL : eForgeComposeTargetItemType.SUIT_CELL;
                let groupId = cGoodInfo.template.NeedMinLevel;
                if (temp.SonType == GoodsSonType.SONTYPE_HERALDRY || temp.SonType == GoodsSonType.SONTYPE_RELIC) {
                ...
                if (!suitInfo) {
                    ...
                    suitInfo.cellType = type == ForgeData.COMPOSE_TYPE_ADVANCE_EQUIP ? eForgeComposeTargetItemType.EPIC_MENU_CELL : eForgeComposeTargetItemType.SUIT_MENU_CELL;;
        ...
        Logger.info("[ForgeData]getTreeDataByType", type, this.composeSuitDic, this.composeListDic)
        if(isReverse && (type == ForgeData.COMPOSE_TYPE_EQUIP || type ==ForgeData.COMPOSE_TYPE_ADVANCE_EQUIP) && openHERALDRY){
            //纹章与圣物在最上，套装以等级倒序排列
    ...
    private calcGoodNumber() {
        ...
        let sDic: SimpleDictionary = GoodsManager.Instance.getHeroEquipBagAndPlayerBagList();
        sDic.forEach((item: GoodsInfo) => {
            if (this.goodsNumList[item.templateId]) {
                this.goodsNumList[item.templateId] = Number(this.goodsNumList[item.templateId]) + item.count;
            }
            else {
    ...
    private getNumByTempId(tempId: number): number {
        if (this.goodsNumList[tempId])
            return Number(this.goodsNumList[tempId]);
        else
            return 0;
    }
    ...
    public initComposeInfo() {
        ...
        //优化标记-- 优化上面逻辑
        let composeListId = PlayerManager.Instance.currentPlayerModel.playerInfo.composeList;
    ...
    private sortComposeList(tem1: ComposeGoodsInfo, tem2: ComposeGoodsInfo): number {
    ...
    public getComposeInfoByTemId(type: number, goodInfo: GoodsInfo): ComposeGoodsInfo {
        ...
        if (list) {
            for (let index = 0; index < list.length; index++) {
                ...
                if (info.template.Material1 == goodInfo.templateId)
                    return info;
            }
        }
        return null;
    ...
    private checkOwn(item: t_s_composeData): boolean {
        ...
        return (playerInfo.composeList && playerInfo.composeList.indexOf(item.Id) >= 0)
    }
    ...
    public getCurrentUpgradeEquipGoodsInfo(): GoodsInfo {
        ...
        if (this._selectedCompose) {
            ...
            goodsInfo = GoodsManager.Instance.getGoodsByObjectIdAndGoodID(this.thane.id, goodTempId);
            if (goodsInfo) {
                return goodsInfo;
            }
            else {
    ...
    private filterGoods(gInfo: GoodsInfo): boolean {
        ...
        if (this.curTabIndex == ForgeData.TabIndex.QH) {
            if (gInfo.templateInfo.SonType == GoodsSonType.SONTYPE_INTENSIFY)
                return true;
            else if (gInfo.templateInfo.StrengthenMax > 0)
                return true;
        }
        else if (this.curTabIndex == ForgeData.TabIndex.XQ) {
            if (gInfo.templateInfo.SonType == GoodsSonType.ARTIFACT)//镶嵌页签，屏蔽sontype为120的物品
                return false;
                ...
                return true;
            else if (gInfo.templateInfo.MasterType == GoodsType.EQUIP && gInfo.templateInfo.SonType != GoodsSonType.SONTYPE_WING)
                return true;
        }
            ...
            if (gInfo.templateInfo.SonType == GoodsSonType.SONTYPE_REFRESH)
                return true;
            else if (gInfo.templateInfo.MasterType == GoodsType.EQUIP && gInfo.templateInfo.Refresh != 0)
                return true;
        ...
        // } 
            ...
            if (gInfo.templateInfo.TransformId > 0)
                return true;
        } else if (this.curTabIndex == ForgeData.TabIndex.SZ) {
            if (gInfo.templateId == ForgeData.MOULD_NEED_MATERIAL1)
                return true;
            else if (gInfo.templateId == ForgeData.MOULD_NEED_MATERIAL2)
                return true;
            ...
            //     return true;
            else if (gInfo.templateInfo.StrengthenMax > 0 && gInfo.templateInfo.Profile >= GoodsProfile.Level5 && gInfo.templateInfo.NeedGrades >= 80)
                return true;
        }
        return false;
    }
    ...
    private sortHeroEquip(item1: GoodsInfo, item2: GoodsInfo): number {
        if (!item1.isBinds && item2.isBinds) {
            ...
            if (item1.templateInfo.Profile > item2.templateInfo.Profile) {
                ...
                else if (item1.templateInfo.NeedGrades < item2.templateInfo.NeedGrades) {
                    return 1;
                }
                else {
    ...
    private sortByTpye(item1: GoodsInfo, item2: GoodsInfo): number {
        ...
        else if ((item1.isBinds == true && item2.isBinds == true) || (item1.isBinds == false && item2.isBinds == false)) {
            if (item1.templateInfo.MasterType > item2.templateInfo.MasterType) {
                return -1;
            }
            else if (item1.templateInfo.MasterType == item2.templateInfo.MasterType) {
                if (item1.templateInfo.NeedGrades > item2.templateInfo.NeedGrades) {
                    return -1;
                }
                else if (item1.templateInfo.NeedGrades == item2.templateInfo.NeedGrades) {
                    ...
                    else if (item1.templateInfo.TemplateId == item2.templateInfo.TemplateId) {
                        if (item1.templateInfo.Profile > item2.templateInfo.Profile) {
                            return -1;
                        }
                        else if (item1.templateInfo.Profile < item2.templateInfo.Profile) {
    ...
    public static isFashion(info: GoodsInfo): boolean {
        ...
        if (info.templateInfo.SonType == GoodsSonType.FASHION_HEADDRESS) return true;//时装帽子
    ...
    public static getSuccessRate(gInfo1: GoodsInfo, gInfo2: GoodsInfo): number {
        let rate: number;
        if (gInfo1) {
            rate = ForgeData.SUCCESS_RATE - gInfo1.strengthenGrade * ForgeData.RATE_STEP;
            rate = rate > 100 ? 100 : rate < 10 ? 10 : rate;
        }
        if (gInfo2)
            rate += gInfo2.templateInfo.Property1;
        rate = rate > 100 ? 100 : rate
        return rate;
    }

    public static checkCanIntensify(neededGold: number, gInfo1: GoodsInfo): boolean {
        let level1: number = gInfo1.strengthenGrade;
        let gold: number = ResourceManager.Instance.gold.count;
        let str: string = "";
        if (neededGold > gold) {
            str = LangManager.Instance.GetTranslation("store.utils.StoreHelper.command01", neededGold - gold);
            MessageTipManager.Instance.show(str);
            return false;
        }
        if (level1 >= gInfo1.templateInfo.StrengthenMax) {
            str = LangManager.Instance.GetTranslation("store.utils.StoreHelper.command02");
            MessageTipManager.Instance.show(str);
            return false;
        }
        return true;
    ...
    public get upgradeEquipOpen() {
        let upgradelist = this.getComposeListByType(ForgeData.COMPOSE_TYPE_UPGRADE_EQUIP)
    ...
    public get advanceEquipOpen() { 
        let advancelist = this.getComposeListByType(ForgeData.COMPOSE_TYPE_ADVANCE_EQUIP)
