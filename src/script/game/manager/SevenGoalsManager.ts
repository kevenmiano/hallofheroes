...
export default class SevenGoalsManager extends GameEventDispatcher {
  ...
  private initEvent() {
    ServerDataManager.listen(
      S2CProtocol.U_C_SEVEN_TARGET_INFO,
      this,
      this.sevenGoalsInfo,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_SEVEN_TARGET_TASK_RECEIVE,
      this,
      this.sevenGoalsTaskRewardInfo,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_SEVEN_TARGET_GIFT_RECEIVE,
      this,
      this.sevenGoalsBagRewardInfo,
    );
  }
    ...
    if (msg) {
      ...
      if (msg.sevenTask.length > 1) {
        ...
        for (let i = 0; i < msg.sevenTask.length; i++) {
          ...
          for (
            let j: number = 0;
            j < this._sevenGoalsModel.sevenTask.length;
            j++
          ) {
        ...
        this._sevenGoalsModel.sevenTask = ArrayUtils.sortOn(
          this._sevenGoalsModel.sevenTask,
          ["status"],
          [ArrayConstant.NUMERIC],
        );
      }
  ...
  private readSevenTaskInfo(msg: SevenTaskMsg): SevenTaskInfo {
    ...
    if (msg) {
      ...
      sevenTaskInfo.day = this._sevenGoalsModel.getDayValue(
        sevenTaskInfo.taskId,
      );
    }
