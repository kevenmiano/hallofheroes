...
export default class CarnivalWnd extends BaseWindow {
  ...
  public OnInitWind(): void {
    ...
    this.Img_Txt_Carnival.url = this.carnivalModel.getThemeImgPath(
      "Img_Txt_Carnival" + "_" + getDefaultLangageKey(),
    );
    this.Img_Carnival_Bg.url = this.carnivalModel.getThemeImgPath(
      "Img_Carnival_Bg",
      ".jpg",
    );
    ...
    this.addEvent();
    var tabStr: string =
      LangManager.Instance.GetTranslation("carnival.tab.names");
  ...
  private initActTime() {
    ...
    if (!isRewardTime) {
      ...
      this.activityTime.text = LangManager.Instance.GetTranslation(
        "carnival.active.reeardtime",
        endStr,
      );
      return;
    }
    ...
    var endStr: string = "";
    var eC =
      TempleteManager.Instance.getConfigInfoByConfigName("CarnivalStopDate");
    if (eC) {
      endStr = eC.ConfigValue;
    }

    this.activityTime.text = LangManager.Instance.GetTranslation(
      "carnival.active.time",
      endStr,
    );
  }
  ...
  renderTabListItem(index: number, item: CarnivalTabBtn) {
    ...
    if (index == 0) {
      ...
      this.gameItem = item;
      if (
        this.carnivalModel.hasGameCounts() &&
      ...
      if (
        this.carnivalModel.hasFreeDiscount() &&
        CarnivalManager.Instance.isRewardTime
      ) {
        item.redPointState.selectedIndex = 1;
      } else {
      ...
      if (
        this.carnivalModel.hasOnlineReward() &&
        CarnivalManager.Instance.isRewardTime
      ) {
        item.redPointState.selectedIndex = 1;
      } else {
  ...
  private addEvent() {
    ...
    CarnivalManager.Instance.addEventListener(
      CarnivalManager.EVENT_TASK_UPDATE,
      this.updateTaskInfoHandler,
      this,
    );
  ...
  private onBuy() {
    FrameCtrlManager.Instance.open(
      EmWindow.ShopWnd,
      { returnToWin: EmWindow.Carnival, page: 7 },
      null,
      EmWindow.Carnival,
    );
  }
  ...
  btn_helpClick() {
    ...
    let content: string =
      LangManager.Instance.GetTranslation("carnival.helpTip");
