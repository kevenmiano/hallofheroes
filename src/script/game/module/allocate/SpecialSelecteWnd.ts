    ...
    if (!flag) {
      MessageTipManager.Instance.show(
        LangManager.Instance.GetTranslation(
          "SpecialSelecteWnd.selected.property",
        ),
      );
      return;
    }
    ...
    for (let i: number = 0; i < this.tempArr.length; i++) {
      ...
      if (
        !TempleteManager.Instance.getPawnSpecialTemplateByID(
          parseInt(this.tempArr[i].specialAbility),
        )
      )
        continue;
      let specialSelectedItem: SpecialSelectedItem = <SpecialSelectedItem>(
        this.contentlist.addItemFromPool()
      );
