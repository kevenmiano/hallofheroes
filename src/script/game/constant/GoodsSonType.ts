...
export default class GoodsSonType {
    ...
    */
    ...
    public static EQUIP_SONTYPE_LIST: Array<number> = [101, 102, 103, 104, 105, 106, 107, 108, 109];

    public static getSonTypeName(type: number): string {
        switch (type) {
            case GoodsSonType.SONTYPE_WEAPON:
                return LangManager.Instance.GetTranslation("bag.datas.GoodsSonType.WEAPON");
            case GoodsSonType.SONTYPE_ASSISTANT:
                return LangManager.Instance.GetTranslation("bag.datas.GoodsSonType.ASSISTANT");
            case GoodsSonType.SONTYPE_HEADDRESS:
                return LangManager.Instance.GetTranslation("bag.datas.GoodsSonType.HEADDRESS");
            case GoodsSonType.SONTYPE_CLOTHES:
                return LangManager.Instance.GetTranslation("bag.datas.GoodsSonType.CLOTHES");
            case GoodsSonType.SONTYPE_MANTEAU:
                return LangManager.Instance.GetTranslation("bag.datas.GoodsSonType.MANTEAU");
            case GoodsSonType.SONTYPE_NECKLACE:
                return LangManager.Instance.GetTranslation("bag.datas.GoodsSonType.NECKLACE");
            case GoodsSonType.SONTYPE_RING:
                return LangManager.Instance.GetTranslation("bag.datas.GoodsSonType.RING");
            case GoodsSonType.SONTYPE_TRINKET:
                return LangManager.Instance.GetTranslation("bag.datas.GoodsSonType.TRINKET");
            case GoodsSonType.SONTYPE_HONER:
                return LangManager.Instance.GetTranslation("bag.datas.GoodsSonType.HONOR");
            case GoodsSonType.SONTYPE_RUNNES:
                return LangManager.Instance.GetTranslation("bag.datas.GoodsSonType.RUNNES");
            case GoodsSonType.SONTYPE_WING:
                return LangManager.Instance.GetTranslation("bag.datas.GoodsSonType.WING");
            case GoodsSonType.FASHION_CLOTHES:
                return LangManager.Instance.GetTranslation("bag.datas.GoodsSonType.FashionClothes");
            case GoodsSonType.FASHION_HEADDRESS:
                return LangManager.Instance.GetTranslation("bag.datas.GoodsSonType.FahsionHat");
            case GoodsSonType.FASHION_WEAPON:
                return LangManager.Instance.GetTranslation("bag.datas.GoodsSonType.FashionWeapon");
            case GoodsSonType.SONTYPE_HERALDRY:
                return LangManager.Instance.GetTranslation("bag.datas.GoodsSonType.heraldry");
            case GoodsSonType.SONTYPE_RELIC:
                return LangManager.Instance.GetTranslation("bag.datas.GoodsSonType.relic");
        }
    ...
    public static isGradeFix(hero: ThaneInfo, info: t_s_itemtemplateData): boolean {
        if (hero && info && (info.NeedGrades == 0 || hero.grades >= info.NeedGrades))
            return true;
        return false;
    }
    ...
    public static isPreciousGoods(info: GoodsInfo): boolean {
        ...
        if (info.templateInfo.MasterType == GoodsType.PET_CARD && info.templateInfo.Profile >= 4) return true;
        if (info.templateInfo.MasterType == GoodsType.PROP && info.templateInfo.Profile >= 4) return true;
        if (info.templateInfo.MasterType == GoodsType.HONER && info.templateInfo.Profile >= 4) return true;
        if (info.templateInfo.MasterType == GoodsType.EQUIP && info.templateInfo.SuiteId != 0) return true;
