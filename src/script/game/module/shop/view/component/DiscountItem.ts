...
export default class DiscountItem extends FUI_DiscountItem {
    ...
    setItemData(data: GoodsInfo, cost: number = 0) {
        ...
        this.txt_name.color = GoodsSonType.getColorByProfile(data.templateInfo.Profile);
        this.txt_des.text = data.templateInfo.DescriptionLang;
        if (data.validDate == 0) {
            this.txt_validDate.text = LangManager.Instance.GetTranslation("shop.discount.validDate0");
        } else {
            let timeStr: string;
            ...
            this.txt_validDate.text = LangManager.Instance.GetTranslation("yishi.view.tips.goods.EquipTip.time.text") + ":" + timeStr;
        }
