...
export default class SinglepassResultWnd extends BaseWindow {
  ...
  private refreshView() {
    ...
    this.ImgDiamage1.url = this.getImgDiamageUrl(
      this._battleReportMsg.goldBonus,
    );
    ...
    if (this._battleReportMsg.petGpBonus < 0)
      this._battleReportMsg.petGpBonus = 0;
    this.txtPetExp.text = "+" + this._battleReportMsg.petGpBonus.toString();
    this.imgGrade.url = this.getImgDiamageUrl(
      this._battleReportMsg.battleScene,
    );
    this._goodsList = [];
    ...
    if (playerInfo.enterWarPet) {
      this.imgPetHead.url = IconFactory.getPetHeadSmallIcon(
        playerInfo.enterWarPet.templateId,
      );
