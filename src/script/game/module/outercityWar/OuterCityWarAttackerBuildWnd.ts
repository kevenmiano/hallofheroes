...
export default class OuterCityWarAttackerBuildWnd extends BaseWindow {
    ...
    private initView() {
        ...
        this.txtDesc.text = LangManager.Instance.GetTranslation("outerCityWar.attackerBuildWndDesc")
        ...
        this.txtCapaity.text = LangManager.Instance.GetTranslation("OuterCityWarAccacker.capaity")
    ...
    private refreshView() {
        let b = this.fightModel.isCastleFighting && this.fightModel.checkJoin(this.fightModel.selfUserId) && this.guildId != this.fightModel.selfGuildId
        this.btnAttack.visible = b;

        // 公会参战未被击退/公会参战人数
        let str = LangManager.Instance.GetTranslation("public.diagonalSign", this.buildInfo.attackGuildCurCnt, this.buildInfo.attackGuildTotalCnt)
    ...
    private refreshBuildBuffView() {
        let buildSkillTemp = this.fightModel.getBuildBuffTemplateByPlayerCnt(this.buildInfo.attackGuildCurCnt)
        if (buildSkillTemp) {
            let buildBuffDesc = LangManager.Instance.GetTranslation("outerCityWar.buildBuff00");

            this.gSkill.visible = true;
            this.skillItem.getChild("txtTitle").text = LangManager.Instance.GetTranslation("public.level3", buildSkillTemp.ActiveSkillLevel);
            ...
            if (skillTemp) {
                ...
                Logger.outcityWar("配置技能模板不存在", buildSkillTemp.ActiveSkillTemplate)
            }
        } else {
    ...
    private btnAttackClick() {
        if (this.fightModel.checkGuildOut(this.buildInfo.attackGuildId)) {
            MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("outerCityWar.guildRepulse", this.buildInfo.attackGuildName))
            return
        }
        if (this.fightModel.checkPlayerOut(this.fightModel.playerInfo.userId)) {
            MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("outerCityWar.playerRepulse"))
            return
        }
        OuterCityWarManager.Instance.sendAttackAttackerBuild(this.buildInfo.attackGuildId)
    }
    ...
    private get playerInfoList(): OuterCityWarPlayerInfo[] {
        ...
        temp = ArrayUtils.sortOn(temp, ["heroCapaity"], [ArrayConstant.NUMERIC | ArrayConstant.DESCENDING])
        return temp
    }
