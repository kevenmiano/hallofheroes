...
import { CampaignMapEvent, SceneViewEvent } from "../../../constant/event/NotificationEvent";
...
export class SpaceSceneMapView extends Laya.Sprite {
    ...
    protected registerList() {
        ...
        this._mediatorKey = MediatorMananger.Instance.registerMediatorList(arr, this, SpaceSceneMapView.NAME);
    }

    protected initView() {
        this._bgLayer = new BgLayer(this, this._model.mapTempInfo, this._model.floorData, true);
        ...
        AvatarInfoUILayerHandler.setHandlerView(this._avatarInfoUILayer, AvatarInfoUIName.SpaceSceneMapView);
    ...
    private __addToStageHandler() {
        ...
        this.registerList();
        this.lookAtTarget();
        NotificationManager.Instance.dispatchEvent(SceneViewEvent.ADD_TO_STAGE, SceneType.SPACE_SCENE);
    ...
    private lookAtTarget(isAdd: boolean = false) {
        ...
        if (isAdd) {
            this.moveCenterPoint(new Laya.Point(lookAtRole.x, lookAtRole.y), .1, false);
        }
        this.event(CampaignMapEvent.LOOK_AT_ROLE, lookAtRole);
    }

    private moveCenterPoint(p: Laya.Point, dealy: number = 0.6, isTween: boolean = true) {
        ...
        if (isTween) {
            TweenLite.to(this, dealy, { x: p.x, y: p.y, onUpdate: this.moveEndCheck });
        }
        else {
    ...
    public dispose() {
        ...
        if (this._warlordsLayer) ObjectUtils.disposeObject(this._warlordsLayer); this._warlordsLayer = null;
        if (this._shadowUILayer) ObjectUtils.disposeObject(this._shadowUILayer); this._shadowUILayer = null;
        if (this._npcLayer) ObjectUtils.disposeObject(this._npcLayer); this._npcLayer = null;
