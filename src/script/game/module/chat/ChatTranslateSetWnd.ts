...
import t_s_chattranslateset, { translateParam, } from "../../config/t_s_chattranslateset";
...
export default class ChatTranslateSetWnd extends BaseWindow {
    ...
    public OnInitWind() {
        super.OnInitWind();
        this.setCenter();
        let cfgData = ConfigMgr.Instance.getSync(ConfigType.t_s_chattranslateset) as t_s_chattranslateset;
        if (cfgData) {
            this.translatesLanguage = cfgData.mDataList;
        } else {
            this.translatesLanguage = []
        }
        this.title.text = LangManager.Instance.GetTranslation("ChatWnd.TranslateSetWnd.Title")
        let langKey = PlayerManager.Instance.currentPlayerModel.playerInfo.chatTranslateKey;
        if (langKey == "") {
            langKey = "en";
        }
    ...
    private onClickItem(item: fgui.GButton) {
        let clickIndex = this.itemList.childIndexToItemIndex(this.itemList.getChildIndex(item));
    ...
    public OnHideWind() {
        ...
        IMManager.Instance.sendTranslateSetting(this.translatesLanguage[this.curSelect].key);
    }
