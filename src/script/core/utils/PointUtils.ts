...
export class PointUtils {
  ...
  public static scaleTransform(
    p: Laya.Point,
    scaleX: number,
    scaleY: number,
  ): Laya.Point {
    return new Laya.Point(
      parseInt((p.x / scaleX).toString()),
      parseInt((p.y / scaleY).toString()),
    );
  }
  public static scaleTransformII(
    p: Laya.Point,
    scaleX: number,
    scaleY: number,
  ): Laya.Point {
  ...
  public static strToPoint(value: string, delim: string): Laya.Point {
    ...
    if (arr.length == 2) {
      ...
      var errorTip: string = LangManager.Instance.GetTranslation(
        "map.internals.utils.PointUtils.errorTip",
      );
