...
export class CoreTransactionManager implements IEnterFrame {
  ...
  constructor() {
    ...
    this._vUnregisterProtocol = LangManager.Instance.GetTranslation(
      "yishi.manager.CoreTransactionManager.vUnregisterProtocol",
    );
  }
  ...
  public enterFrame() {
    ...
    for (let j: number = 0; j < this._executable.length; j++) {
      ...
      if (handler) {
        ...
        throw new Error(
          "QueueManager" + this._vUnregisterProtocol + ": " + pkg.code,
        );
      }
    }
  ...
  public handlerPackage(pkg: PackageIn) {
    let handler: ISocketTransaction = this._handlerList[
      pkg.code
    ] as ISocketTransaction;
    if (handler) {
      handler.configure(pkg);
      handler.handlePackage();
    }
  ...
  private registerAllHandler() {
    //每次进出战斗都会重新实例化以下对象
    let createGameTransaction = Laya.ClassUtils.getClass(
      "CreateGameTransaction",
    );
    let campaignOverTransaction = Laya.ClassUtils.getClass(
