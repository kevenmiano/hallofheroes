...
export default class ColosseumRewardsItem extends FUI_ColosseumRewardsItem {
    ...
    getDes(str: string) {
        ...
        let arrs = str.split(',');
        if (arrs.length == 2) {
            ret = LangManager.Instance.GetTranslation('RoomList.pvp.rewards.item.txt1', arrs[0], arrs[1]);
        } else if (arrs.length == 1) {
            ret = LangManager.Instance.GetTranslation('RoomList.pvp.rewards.item.txt2', arrs[0]);
        }
        return ret;
    }
