...
export class PetSelectWnd extends BaseWindow {
	...
	/**初始化视图 */
	private initView() {
		this.txtFrameTitle.text = LangManager.Instance.GetTranslation("pet.exchange.select");
	...
	/**初始化事件 */
	private initEvent() {
		this["list"].itemRenderer = Laya.Handler.create(this, this.onRenderPetItem, null, false);
		this["list"].itemProvider = Laya.Handler.create(this, this.onRenderPetItemResource, null, false);
	}
	...
	private refreshPetList(): void {
		...
		this._petCanExchangeList = [];
		this._petCannotExchangeList = [];
		this._petList = PlayerManager.Instance.currentPlayerModel.playerInfo.petList as PetData[];
		for (let index = 0; index < this._petList.length; index++) {
			...
			if (petInfo.template.Property2 != sourcePetStage) {
				petInfo.ismind = true;
				petInfo.remind = LangManager.Instance.GetTranslation("pet.exchange.levelCondition.lack");
				this._petCannotExchangeList.push(petInfo);
			} else {
				this._petCanExchangeList.push(petInfo);
			}
		}
		this._petCanExchangeList = ArrayUtils.sortOn(this._petCanExchangeList, ["fightPower"], [ArrayConstant.NUMERIC | ArrayConstant.DESCENDING]);
		this._petCannotExchangeList = ArrayUtils.sortOn(this._petCannotExchangeList, ["fightPower"], [ArrayConstant.NUMERIC | ArrayConstant.DESCENDING]);

		if (this._petCanExchangeList.length > 0) {
			let canExchangeTitle = new PetData();
			canExchangeTitle.title = LangManager.Instance.GetTranslation("pet.exchange.meet.condition");
			this._petExchangeList.push(canExchangeTitle);
			this._petExchangeList = this._petExchangeList.concat(this._petCanExchangeList);
		}

		if (this._petCannotExchangeList.length > 0) {
			let cannotExchangeTitle = new PetData();
			cannotExchangeTitle.title = LangManager.Instance.GetTranslation("pet.exchange.notMeet.condition");
			this._petExchangeList.push(cannotExchangeTitle);
			this._petExchangeList = this._petExchangeList.concat(this._petCannotExchangeList);
		}
	...
	/**渲染列表数据项 */
	private onRenderPetItem(index: number, item: PetSelectItem | PetSelectItemTitle): void {
		if (item instanceof PetSelectItemTitle) {
			...
			if (this._targetPet && this._targetPet.petId === item.info.petId) item.getController("petSelect").selectedIndex = 1;
