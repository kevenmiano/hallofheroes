...
export default class WarlordsBetWnd extends BaseWindow {
    ...
    private confirm: string = LangManager.Instance.GetTranslation("public.confirm");
    ...
    private initView() {
        this.betInfoGroup.visible = this.betTimeGroup.visible = this.hasCostMoneyGroup.visible = false;
        this.peopleSeleectGroup.visible = this.rewardInfoGroup.visible = this.successFailGroup.visible = false;
        ...
        switch (this.warlordsModel.process) {
            ...
            case WarlordsModel.PROCESS_PRELIM:
                ...
                this.firstBtn.enabled = this.secondBtn.enabled = this.thirdBtn.enabled = false;
                this.betTimeTxt.text = this.warlordsModel.getMatchDateString(2, LangManager.Instance.GetTranslation("public.dateFormat"));
                break;
            case WarlordsModel.PROCESS_FINAL:
                ...
                this.hasCostMoneyGroup.visible = this.successFailGroup.visible = this.betTimeGroup.visible = true;
                this.betTimeGroup.y = 450;
                this.hasCostMoneyGroup.y = 512;
                this.firstBtn.enabled = this.secondBtn.enabled = this.thirdBtn.enabled = false;
                this.betTimeTxt.text = LangManager.Instance.GetTranslation("warlords.WarlordsBetFrame.str02");
                break;
            case WarlordsModel.PROCESS_BET:
                this._cstyle = WarlordsModel.PROCESS_BET;
                this.successFailGroup.visible = this.betInfoGroup.visible = this.peopleSeleectGroup.visible = true;
                break;
            case WarlordsModel.PROCESS_OVER:
                this._cstyle = WarlordsModel.PROCESS_OVER;
                this.rewardInfoGroup.visible = true;
                this.successPic.visible = this.notSelectedPic.visible = this.failPic.visible = false;
                break;
        }
    }
    ...
    private initEvent() {
        ...
        this.warlordsModel.addEventListener(WarlordsEvent.CUR_BETTING_CHANGE, this.warlordsBetSelectedHandler, this);
    ...
    private removeEvent() {
        ...
        this.warlordsModel.removeEventListener(WarlordsEvent.CUR_BETTING_CHANGE, this.warlordsBetSelectedHandler, this);
    ...
    private helpBtnHandler() {
        ...
        let content: string = LangManager.Instance.GetTranslation("warlords.WarlordsBetFrame.helpContent");
    ...
    refreshView() {
        ...
        this._tempInfo = this.warlordsModel.getListData(WarlordsModel.BETTING, 1) as WarlordsPlayerInfo;
        this.firstTxt.text = this._tempInfo ? this._tempInfo.nickname : "";
        this._tempInfo = this.warlordsModel.getListData(WarlordsModel.BETTING, 2) as WarlordsPlayerInfo;
        this.secondTxt.text = this._tempInfo ? this._tempInfo.nickname : "";
        this._tempInfo = this.warlordsModel.getListData(WarlordsModel.BETTING, 3) as WarlordsPlayerInfo;
        ...
        if (this._cstyle == WarlordsModel.PROCESS_OVER) {
            ...
            for (let i: number = 1; i <= 3; i++) {
                this._tempInfo = this.warlordsModel.getListData(WarlordsModel.JOB_TOP3, i) as WarlordsPlayerInfo;
            ...
            this.betNum = (this.warlordsModel.betNum > 0 || this._cstyle == WarlordsModel.PROCESS_FINAL) ? this.warlordsModel.betNum : WarlordsModel.MIN_BET_NUM;
            if (this.warlordsModel.hasBet) {
                this.firstBtn.enabled = this.secondBtn.enabled = this.thirdBtn.enabled = false;
            }
        }
    ...
    warlordsBetSelectedHandler() {
        this._tempInfo = this.warlordsModel.getListData(WarlordsModel.CUR_BETTING, 1) as WarlordsPlayerInfo;
        if (this._tempInfo) {
            this.firstTxt.text = this._tempInfo.nickname;
            this.firstBtn.title = LangManager.Instance.GetTranslation("WarlordsBetWnd.selectedBtnTxt1");
        }
        else {
            this.firstTxt.text = "";
            this.firstBtn.title = LangManager.Instance.GetTranslation("WarlordsBetWnd.selectedBtnTxt2");
        }
        this._tempInfo = this.warlordsModel.getListData(WarlordsModel.CUR_BETTING, 2) as WarlordsPlayerInfo;
        if (this._tempInfo) {
            this.secondTxt.text = this._tempInfo.nickname;
            this.secondBtn.title = LangManager.Instance.GetTranslation("WarlordsBetWnd.selectedBtnTxt1");
        }
        else {
            this.secondTxt.text = "";
            this.secondBtn.title = LangManager.Instance.GetTranslation("WarlordsBetWnd.selectedBtnTxt2");
        }
        this._tempInfo = this.warlordsModel.getListData(WarlordsModel.CUR_BETTING, 3) as WarlordsPlayerInfo;
        if (this._tempInfo) {
            this.thirdTxt.text = String(this._tempInfo.nickname);
            this.thirdBtn.title = LangManager.Instance.GetTranslation("WarlordsBetWnd.selectedBtnTxt1");//更改
        }
        else {
            this.thirdTxt.text = "";
            this.thirdBtn.title = LangManager.Instance.GetTranslation("WarlordsBetWnd.selectedBtnTxt2");//选择
        }
    }
    ...
    onCommitBetClick() {
        if (this.warlordsModel.hasBet) {
            MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("warlords.WarlordsBetFrame.str10"));
            return;
        }
        if (this._betNum < WarlordsModel.MIN_BET_NUM || this._betNum > WarlordsModel.MAX_BET_NUM) {
            MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("warlords.WarlordsBetFrame.betNumTip", WarlordsModel.MIN_BET_NUM, WarlordsModel.MAX_BET_NUM));
            return;
        }
        this._tempInfo = this.warlordsModel.getListData(WarlordsModel.CUR_BETTING, 1) as WarlordsPlayerInfo;
        ...
        if (this.secondTxt.text == "" || this.firstTxt.text == "" || this.thirdTxt.text == "") {
            MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("warlords.WarlordsBetFrame.str07"));
            return;
        }
        var needBuyNum: number = this.getNeedBuyNum();
        if (needBuyNum > 0) {
            var tip: string = LangManager.Instance.GetTranslation("warlords.WarlordsBetFrame.gloryBuyTip", needBuyNum, needBuyNum);
            SimpleAlertHelper.Instance.Show(SimpleAlertHelper.SIMPLE_ALERT, [needBuyNum, first, second, third], this.prompt, tip, this.confirm, this.cancel, this.gloryBuyCall.bind(this));
            return;
        }
    ...
    private gloryBuyCall(b: boolean, flag: boolean, data: any[]) {
        if (b) {
            ...
            if (PlayerManager.Instance.currentPlayerModel.playerInfo.point < needBuyNum) {
    ...
    private commitBet(first: string, second: string, third: string) {
        var tip: string = LangManager.Instance.GetTranslation("warlords.WarlordsBetFrame.str08");
        SimpleAlertHelper.Instance.Show(SimpleAlertHelper.SIMPLE_ALERT, [first, second, third], this.prompt, tip, this.confirm, this.cancel, this.commitCall.bind(this));
    }
