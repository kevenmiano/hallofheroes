...
export class ConsortiaTreeExpView extends Laya.Sprite {
  ...
  private initView() {
    this._view = FUIHelper.createFUIInstance(
  ...
  private initEvent() {
    if (this._consortiaSecretInfo)
      this._consortiaSecretInfo.addEventListener(
    ...
    this.consortiaModel.secretInfo.addEventListener(
      ConsortiaEvent.TREE_STATE_UPDATE,
      this.consortiaTreeStateUpdateHandler,
      this,
    );
  }
  ...
  private removeEvent() {
    if (this._consortiaSecretInfo)
      this._consortiaSecretInfo.removeEventListener(
        ConsortiaEvent.SECRET_UPDATE,
        this.secretUpdateHandler,
        this,
      );
    ...
    this.consortiaModel.secretInfo.removeEventListener(
      ConsortiaEvent.TREE_STATE_UPDATE,
      this.consortiaTreeStateUpdateHandler,
      this,
    );
  ...
  private refreshView() {
    ...
    this.progShow.getChild("title").text =
      this._consortiaSecretInfo.givePowerCount +
      " / " +
      (this.consortiaModel.consortiaInfo.levels * 20 + 10);
  }
  ...
  private refreshTreeViewByState() {
    if (
      ConsortiaManager.Instance.model.secretInfo.treeState ==
      ConsortiaSecretInfo.GIVE_POWER_STATE
    ) {
