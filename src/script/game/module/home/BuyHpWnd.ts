...
export default class BuyHpWnd extends BaseWindow {
  ...
  public OnInitWind() {
    this.n8.getChild("title").text = LangManager.Instance.GetTranslation(
      "mainBar.view.quickbuy.QuickBuyFrame.title",
    );
    this.setCenter();
    this.itemListInfo.itemRenderer = Laya.Handler.create(
      this,
      this.renderListItem,
      null,
      false,
  ...
  private updateProgress() {
    if (ArmyManager.Instance.army) {
      this.myhp.getChild("progress").text =
        ArmyManager.Instance.army.baseHero.hp +
        " / " +
        ArmyManager.Instance.army.baseHero.attackProrerty.totalLive;
      ...
      this.hpProgress.getChild("progress").text =
        ArmyManager.Instance.army.baseHero.blood +
        " / " +
        ArmyManager.Instance.army.baseHero.attackProrerty.totalLive;
  ...
  private get dataList(): Array<ShopGoodsInfo> {
    let arr: Array<ShopGoodsInfo> = [];
    //优化标记 这个是否可以直接用元数组
    for (const key in ConfigMgr.Instance.bloodShopTemplateDic) {
      if (
        Object.prototype.hasOwnProperty.call(
          ConfigMgr.Instance.bloodShopTemplateDic,
          key,
        )
      ) {
