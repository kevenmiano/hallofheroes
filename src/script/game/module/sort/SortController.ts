...
export default class SortController extends FrameCtrlBase {
  ...
  public loadData(path: string) {
    // let self = this;
    var path: string =
      PathManager.info.TEMPLATE_PATH + path + "?v=" + new Date().getTime();
  ...
  public pageBtnHandler(isPrePage: boolean) {
    //翻页
    if (isPrePage && this._model.currentPage > 1) this._model.currentPage--;
  ...
  private resolveDayAddXML(value, cross: boolean) {
    if (value && value.DayAdds) {
      ...
      this._model.createDate = DateFormatter.parse(
        xml.info.createDate,
        "YYYY-MM-DD hh:mm:ss",
      );
      this._model.createDate.setHours(3, 0, 0, 0);
      var dayAdd = xml.DayAdd;
      var items: Array<any> = cross
        ? this._model.dayAddList2
        : this._model.dayAddList;
        ...
        for (const key in dayAdd) {
          if (Object.prototype.hasOwnProperty.call(dayAdd, key)) {
            ...
            if (item) {
              ...
              if (
                sortData.userId == this.playerInfo.userId &&
                item.site == this.playerInfo.site
              ) {
  ...
  private resolveWeekAddXML(value, cross: boolean) {
    if (value && value.WeekAdds) {
      ...
      var items: Array<any> = cross
        ? this._model.weekAddList2
        : this._model.weekAddList;
  ...
  private resolveTotalAddXML(value, cross: boolean) {
    if (value && value.TotalAdds) {
      ...
      var items: Array<any> = cross
        ? this._model.accumulationAddList2
        : this._model.accumulationAddList;
  ...
  private resolvePlayerFightXML(value, cross: boolean) {
    if (value && value.PlayerFights) {
      ...
      var items: Array<any> = cross
        ? this._model.playerFightList2
        : this._model.playerFightList;
  ...
  private resolveConstainOrderXML(value, cross: boolean) {
    if (value && value.ConstainOrder) {
      ...
      var items = cross
        ? this._model.consortiaLevelList2
        : this._model.consortiaLevelList;
      if (list instanceof Array) {
        for (const key in list) {
          if (Object.prototype.hasOwnProperty.call(list, key)) {
            const item = list[key];
  ...
  private resolveConstainFightXML(value, cross: boolean) {
    if (value && value.ConstainFight) {
      ...
      var items = cross
        ? this._model.consortiaFightList2
        : this._model.consortiaFightList;
      if (list instanceof Array) {
        for (const key in list) {
          if (Object.prototype.hasOwnProperty.call(list, key)) {
            var item = list[key];
  ...
  private resolvePlayerGesteXML(value, cross: boolean) {
    if (value && value.PlayerGeste) {
      ...
      var items = cross
        ? this._model.playerHonourList2
        : this._model.playerHonourList;
      if (list instanceof Array) {
        for (const key in list) {
          if (Object.prototype.hasOwnProperty.call(list, key)) {
            var item = list[key];
  ...
  private resolveCharmDayAddXML(value, cross: boolean) {
    if (value && value.CharmDayAdds) {
      ...
      var items = cross
        ? this._model.charmsDayAddList2
        : this._model.charmsDayAddList;
      if (list instanceof Array) {
        for (const key in list) {
          if (Object.prototype.hasOwnProperty.call(list, key)) {
            var item = list[key];
  ...
  private resolveCharmWeekAddXML(value, cross: boolean) {
    if (value && value.CharmWeekAdds) {
      ...
      var items = cross
        ? this._model.charmsWeekAddList2
        : this._model.charmsWeekAddList;
      if (list instanceof Array) {
        for (const key in list) {
          if (Object.prototype.hasOwnProperty.call(list, key)) {
            var item = list[key];
            if (item) {
              ...
              if (
                ...
                this.playerInfo.playerOrdeInfo.charmsWeekOrder =
                  Number(key) + 1;
              }
              items.push(sortData);
            }
  ...
  private resolveCharmTotalAddXML(value, cross: boolean) {
    if (value && value.CharmTotalAdds) {
      ...
      var items = cross
        ? this._model.charmsTotalAddList2
        : this._model.charmsTotalAddList;
      if (list instanceof Array) {
        for (const key in list) {
          if (Object.prototype.hasOwnProperty.call(list, key)) {
            var item = list[key];
  ...
  private resolveSoulScoreTotalAddXML(value, cross: boolean) {
    if (value && value.SoulScoreTotalAdds) {
      ...
      var items = cross
        ? this._model.soulScoreTotalAddList2
        : this._model.soulScoreTotalAddList;
      if (list instanceof Array) {
        for (const key in list) {
          if (Object.prototype.hasOwnProperty.call(list, key)) {
            var item = list[key];
      ...
      if (list instanceof Array) {
        for (const key in list) {
          if (Object.prototype.hasOwnProperty.call(list, key)) {
            ...
            if (!warlordsList) {
              warlordsList = [];
              this._model.warlordsDic.add(
                ThaneInfoHelper.getJob(job),
                warlordsList,
              );
            }
      ...
      if (list instanceof Array) {
        for (const key in list) {
          if (Object.prototype.hasOwnProperty.call(list, key)) {
            ...
            sortData.fightCapacity = Number(item.FightCapacity)
              ? Number(item.FightCapacity)
              : 0;
            sortData.consortiaName = item.ConsortiaName
              ? String(item.ConsortiaName)
              : String(item.consortiaName);
            sortData.serverName = LangManager.Instance.GetTranslation(
              "sort.serverName",
              String(item.serverId),
            );
            sortData.mainSite = item.mainSite;
            sortData.job = Number(item.Job)
              ? Number(item.Job)
              : Number(item.job);
            ...
            sortData.army.baseHero.serviceName =
            ...
            sortData.army.baseHero.IsVipAndNoExpirt =
              item.IsVip == "true" ? true : false;
  ...
  public static read(item: any): SortData {
    ...
    sortInfo.fightCapacity = Number(item.fightCapaity)
      ? Number(item.fightCapaity)
      : 0;
    sortInfo.consortiaName = item.ConsortiaName
      ? String(item.ConsortiaName)
      : String(item.consortiaName);
    ...
    sortInfo.serverName = LangManager.Instance.GetTranslation(
      "sort.serverName",
      String(item.serverId),
    );
    ...
    sortInfo.army.baseHero.IsVipAndNoExpirt =
      item.IsVip == "true" ? true : false;
    ...
    sortInfo.army.baseHero.consortiaName = item.ConsortiaName
      ? String(item.ConsortiaName)
      : String(item.consortiaName);
    ...
    sortInfo.army.baseHero.playerOrdeInfo.fightCapacityOrder = Number(
      item.gpOrde,
    );
    sortInfo.army.baseHero.playerOrdeInfo.honourOrder = Number(
      item.CurrentOrder,
    );
    ...
    sortInfo.army.baseHero.attackProrerty.totalPhyDefence =
      item.totalPhyDefence;
    sortInfo.army.baseHero.attackProrerty.totalMagicAttack =
      item.totalMagicAttack;
    sortInfo.army.baseHero.attackProrerty.totalMagicDefence =
      item.totalMagicDefence;
    ...
    sortInfo.army.baseHero.baseProperty.fashionIntellect =
      item.fashionIntellect;
    ...
    sortInfo.army.baseHero.charms = Number(item.totalCharm);
    sortInfo.army.baseHero.headId = Number(item.headId)
      ? Number(item.headId)
      : 0;
    sortInfo.army.baseHero.frameId = Number(item.frameId)
      ? Number(item.frameId)
      : 0;
  ...
  private cross() {
    switch (this._model.currentSelected) {
      case SortModel.SELF_LEVEL:
        ...
        else this._model.currentShowList = this._model.accumulationAddList2;
        break;
      ...
      case SortModel.SELF_CHARMS:
        ...
        else this._model.currentShowList = this._model.charmsTotalAddList2;
        break;
  ...
  private notCross() {
    switch (this._model.currentSelected) {
      case SortModel.SELF_LEVEL:
        ...
        else this._model.currentShowList = this._model.accumulationAddList;
        break;
      case SortModel.SELF_POW:
      ...
      case SortModel.SELF_CHARMS:
        ...
        else this._model.currentShowList = this._model.charmsTotalAddList;
        break;
      case SortModel.SELF_SOULSCORE:
  ...
  public get isHideTabCell(): boolean {
    if (
      this._model.currentSelected == SortModel.SELF_LEVEL &&
      this._model.addType == SortModel.ACCUMULAT_ADD
    ) {
      return true;
    }
    return false;
