...
export class CampaignNpcPhysics extends CampaignNpcView {
    ...
    private attackFun(): boolean {
        ...
        let armyView: any = CampaignManager.Instance.controller.getArmyView(CampaignManager.Instance.mapModel.selfMemberData);
        ...
        if (WorldBossHelper.checkPvp(CampaignManager.Instance.mapId)) {
            ...
            let walkable = (CampaignManager.Instance.controller.getWalkable(globalPt.x / Tiles.WIDTH, globalPt.y / Tiles.HEIGHT))
            Logger.xjy("[CampaignNpcPhysics]attackFun点击位置", this.mouseX, this.mouseY, globalPt.x, globalPt.y, walkable)
            if (walkable) {
                attackPoint = globalPt
            } else {
                // 战场采矿找随机范围内点站立
                attackPoint = NpcAttackHelper.getAttackPointImp(<CampaignNode>this.info, new Laya.Point(this.x, this.y), 50);
            }
        } else {
            attackPoint = NpcAttackHelper.getAttackPoint(<CampaignNode>this.info, new Laya.Point(armyView.x, armyView.y), new Laya.Point(this.x, this.y));
        }
        Logger.xjy("[CampaignNpcPhysics]attackFun", (this.info as CampaignNode).nodeId, "玩家位置", armyView.x, armyView.y, "节点位置", this.x, this.y, "攻击位置", attackPoint.x, attackPoint.y);
    ...
    public mouseMoveHandler(evt: Laya.Event): boolean {
        ...
        if (WorldBossHelper.checkGvg(mapId)) {
            ...
            if (this.info.info.types == PosType.COPY_START ||
