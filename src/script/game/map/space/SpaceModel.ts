...
export class SpaceModel extends FloorMapInfo {
  ...
  public set currentFloorData(arr: any[]) {
    ...
    this.dispatchEvent(OuterCityEvent.CURRENT_CONFIG_CHANGE, {
      movies: this.currentSceneMovies,
      tops: this.currentSceneTops,
    });
  }
  ...
  public set glowTarget(value: Laya.Sprite) {
    ...
    if (
      this._glowTarget &&
      this._glowTarget.hasOwnProperty("setNormalFilter")
    ) {
  ...
  public get isOnObstacle(): boolean {
    var armyView: any = SpaceManager.Instance.controller.getArmyView(
      this.selfArmy,
    );
    if (!this.mapTielsData) {
      return false;
    ...
    if (
      this.getPointValue(armyView.x / Tiles.WIDTH, armyView.y / Tiles.HEIGHT)
    ) {
      return false;
    }
    var point: Laya.Point = this.find(
      2,
      parseInt((armyView.x / Tiles.WIDTH).toString()),
      parseInt((armyView.y / Tiles.HEIGHT).toString()),
    );
    if (point) {
      return false;
    }
    ...
    for (let i: number = 0; i < len; i++) {
      ...
      switch (spaceNode.info.types) {
        ...
        default:
          ...
          dis = MathHelper.distance(
            this._checkTempPoint.x,
            this._checkTempPoint.y,
