...
import { EmOuterCityWarBuildSortType, EmOuterCityWarCastlePeriodType, EmOuterCityWarPlayerState } from "../../../constant/OuterCityWarDefine";
...
export class OuterCityWarBuildInfo {
    ...
    set sonType(v: number) {
        if (this._sonType != v) {
            this._sonType = v
            this.fightBuildTemplate = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_castlebattlebuilding, this._sonType) as t_s_castlebattlebuildingData;
        }
    }
    get sonType(): number {
    ...
    set tempId(v: number) {
        if (this._tempId != v) {
            this._tempId = v
            this.buildTemplate = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_buildingtemplate, this._tempId) as t_s_buildingtemplateData;
        }
    }
    get tempId(): number {
        return this._tempId
    ...
    get attackGuildCurCnt(): number {
        return this.fightModel.getGuildPlayerCnt(this.attackGuildId, false, BooleanType.TRUE, [EmOuterCityWarPlayerState.FREE, EmOuterCityWarPlayerState.DEFANCE])
    }
    /** 进攻营地：公会参战的数量 */
    get attackGuildTotalCnt(): number {
        return this.fightModel.getGuildPlayerCnt(this.attackGuildId, false, BooleanType.TRUE)
    }
    ...
    public checkPreOneCompleteOccupy() {
        ...
        if (list1.length > 0) {
            ...
            for (let index = 0; index < list1.length; index++) {
                ...
                if (buildInfo) {
                    let isCompleteOccupy = buildInfo.completeOccupyCamp == CampType.Attack;
        ...
        if (list2.length > 0) {
            ...
            for (let index = 0; index < list2.length; index++) {
                ...
                let buildInfo = this.fightModel.buildingInfoMap.get(sontype)
                if (buildInfo) {
                    let isCompleteOccupy = buildInfo.completeOccupyCamp == CampType.Defence;
                    b = b || isCompleteOccupy;
                }
            }
        ...
        NotificationManager.Instance.dispatchEvent(OuterCityWarEvent.BUILD_PRE_COMPLETE_OCCUPY_STATE, this)
    ...
    private initPreBuildList() {
        ...
        if (cfg.AttackPreBattleBuilding.length > 0) {
            ...
            if (nameArr.length > 0) {
                this.strAttackerAttackBuildTip = nameArr.join(LangManager.Instance.GetTranslation("public.or"))
            }
        }
        ...
        if (cfg.DefendPreBattleBuilding.length > 0) {
            ...
            if (nameArr.length > 0) {
                this.strDefencerAttackBuildTip = nameArr.join(LangManager.Instance.GetTranslation("public.or"))
            }
        }
    }
    ...
    private initOccupyScore() {
        this.completeOccupyScore = Number(this.fightBuildTemplate.CompleteOwnExtraPoint)
        for (let index = 0; index < OuterCityWarModel.BuildSiteCount; index++) {
    ...
    public get buildName() {
        if (this.isAttackSite) {
            if (this.attackGuildId) {
                ...
                if (this.fightModel.castleState == EmOuterCityWarCastlePeriodType.Fighting) {
    ...
    public getAttackGuildName() {
        ...
        let str = ""
        if (this.isRepulsed) {
            str = LangManager.Instance.GetTranslation("public.playerInfo.repulsedState")
        } else {
            str = LangManager.Instance.GetTranslation("public.diagonalSign", this.attackGuildCurCnt, this.attackGuildTotalCnt)
        }
        return LangManager.Instance.GetTranslation("public.parentheses3", this.attackGuildName, str)
    }
