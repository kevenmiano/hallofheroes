...
export default class RankCtrl extends FrameCtrlBase {
    ...
    switch (this.rankIndex) {
      ...
      case RankIndex.RankItemR5_001:
        ResMgr.Instance.loadRes(
          PathManager.crossOrderPath,
          this.__RankItemR5_001.bind(this),
          null,
          Laya.Loader.BUFFER,
        );
        break;
  ...
  private __RankItemR5_001(res: any) {
    ...
    if (res) {
      ...
      if (
        dataObj &&
        dataObj.CrossScoreTotalAdds &&
        dataObj.CrossScoreTotalAdds.CrossScoreTotalAdd
      ) {
      ...
      for (let i: number = 0; i < rankArray.length; i++) {
        let itemData: any = rankArray[i];
        if (
          itemData.nikcName == this.playerInfo.nickName &&
      ...
      if (
        dataObj &&
        dataObj.CrossScoreTotalAdds &&
        dataObj.CrossScoreTotalAdds.info
      ) {
        this.data.createDate = DateFormatter.parse(
          dataObj.CrossScoreTotalAdds.info.createDate,
          "YYYY-MM-DD hh:mm:ss",
        );
        this.data.createDate.setHours(3, 0, 0, 0);
        this.data.selfInfo.time = this.data.createDate
          ? DateFormatter.format(this.data.createDate, "MM-DD hh:mm")
          : "";
      }

      if (selfRank == -1) {
        this.data.selfInfo.rank = LangManager.Instance.GetTranslation(
