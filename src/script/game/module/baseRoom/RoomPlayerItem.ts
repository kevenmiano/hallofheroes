...
export default class RoomPlayerItem extends FUI_RoomPlayerItem {
  ...
  protected onConstruct() {
    ...
    this._heroFigure = new ShowAvatar(
      false,
      this.__onCompleteHandler.bind(this),
      true,
    );
    this._heroFigureCon.displayObject.addChild(this._heroFigure);
    this._heroFigure.pos(-35, 160);

    this._petNameShadowTxt = new FilterFrameText(
      240,
      20,
      undefined,
      16,
    ...
    this._petNameTxt = new FilterFrameText(
      240,
      20,
      undefined,
      16,
  ...
  private setPetName(petNameString: string = "") {
    ...
    var petName: string = this._info.baseHero.petName
      ? this._info.baseHero.petName
      : "";
    if (petNameString != "") {
    ...
    let offY = 0;
    if (
      this._info.baseHero.petTemplate.PetAvatar.indexOf("pet_rainbow") != -1
    ) {
      offX += 10;
      offY -= 10;
    }
    ...
    this._petNameTxt.pos(
      this._heroFigure.petAvatar.x - 40 + offX,
      this._heroFigure.petAvatar.y + 60 + offY,
    );
    this._petNameShadowTxt.pos(
      this._heroFigure.petAvatar.x - 40 + offX + 1,
      this._heroFigure.petAvatar.y + 60 + offY + 1,
    );

    if (this._info.baseHero.petQuaity > 0) {
      this._petNameTxt.color = PetData.getQualityColor(
        this._info.baseHero.petQuaity - 1,
      );
    }
  ...
  private removeArmyEvent() {
    if (this._info) {
      ...
      this._info.removeEventListener(
        ...
        this.__updatePlayerInfoHandler,
        this,
      );
      this._info.baseHero.removeEventListener(
        PlayerEvent.PLAYER_AVATA_CHANGE,
        this.__heroInfoChange,
        this,
      );
      this._info.baseHero.removeEventListener(
  ...
  private addArmyEvent() {
    if (this._info) {
      ...
      this._info.baseHero.addEventListener(
        PlayerEvent.PLAYER_AVATA_CHANGE,
        this.__heroInfoChange,
        this,
      );
  ...
  private __chatHandler(chatData: ChatData) {
    ...
    if (!this._chatPopView) {
      this._chatPopView = FUIHelper.createFUIInstance(
        EmPackName.Base,
        "ChatBubbleTip",
      );
    }
    if (this.name == "item1") {
      this._chatPopView.updateContent(
        chatData.htmlText,
        chatData.encodemsg,
        this.__chatBackCall.bind(this),
        0,
        chatData.channel,
      );
  ...
  private __heroInfoChange() {
    Logger.xjy(
      "[RoomPlayerItem]__heroInfoChange英雄信息改变",
      this._info.baseHero,
    );
  ...
  private setNickName() {
    if (this._info.userId == ArmyManager.Instance.army.userId) {
      //自己
      this.txtRank.color =
        FilterFrameText.Colors[eFilterFrameText.AvatarName][2];
    } else if (
      this._info.baseHero.consortiaID != 0 &&
      this._info.baseHero.consortiaID == this.playerInfo.consortiaID
    ) {
      //同工会
      this.txtRank.color =
        FilterFrameText.Colors[eFilterFrameText.AvatarName][1];
    } else {
      this.txtRank.color =
        FilterFrameText.Colors[eFilterFrameText.AvatarName][0];
    }
    if (this._type == RoomPlayerItemType.PvpChallenge) {
      ...
      this.txtRank.text =
        LangManager.Instance.GetTranslation("public.level2", level) +
        " " +
        this._info.baseHero.nickName;
    }
  ...
  private setConsortiaName() {
    if (this._info.userId == ArmyManager.Instance.army.userId) {
      this.txtGuildName.color =
        FilterFrameText.Colors[eFilterFrameText.AvatarName][2];
    } else if (
      this._info.baseHero.consortiaName &&
      this._info.baseHero.consortiaName == this.playerInfo.consortiaName
    ) {
      this.txtGuildName.color =
        FilterFrameText.Colors[eFilterFrameText.AvatarName][1];
    } else {
    ...
    this.txtGuildName.text = this._info.baseHero.consortiaName
      ? "<" + this._info.baseHero.consortiaName + ">"
      : "";
  ...
  public updateRoomPlayerState(info: CampaignArmy) {
    ...
    if (info) {
      ...
      if (info.roomState == RoomPlayerState.PLAYER_STATE_WAITE) {
        ...
        info.roomState == RoomPlayerState.PLAYER_STATE_READY ||
        info.roomState == RoomPlayerState.PLAYER_STATE_HOST
      ) {
        res = "Btn_Sim_TickG";
      }
      // }
  ...
  private btnOwnerOpClick(evt: Laya.Event) {
    ...
    let pos = this.displayObject.localToGlobal(
      new Laya.Point(this.width / 2, this.height / 2),
      true,
      Laya.stage,
    );
    evt.stopPropagation();
    ...
    if (this._currentHero) {
      ...
      if (!BuildingManager.Instance.model.checkHasIdleColosseumOrder()) {
        str = LangManager.Instance.GetTranslation(
          "colosseum.view.ColosseumListView.command02",
        );
      ...
      if (this.ctrl.data.remainFreeCount <= 0) {
        if (this.ctrl.data.remainBuyCount > 0) {
          ...
          str = LangManager.Instance.GetTranslation(
            "colosseum.view.ColosseumPlayerItem.buyCountTip",
            payNum,
          );
          SimpleAlertHelper.Instance.Show(
            ...
            null,
            null,
            this.__btnChallengeClick.bind(this),
          );
        } else {
          str = LangManager.Instance.GetTranslation(
            "colosseum.view.ColosseumListView.command01",
          );
          MessageTipManager.Instance.show(str);
        }
        return;
  ...
  private updateRank() {
    ...
    this.txtPower.text = LangManager.Instance.GetTranslation(
      "RoomList.pvp.colosseum.player.txt2",
      this._currentHero.fightingCapacity,
    );
    this.txtScore.text = LangManager.Instance.GetTranslation(
      "RoomList.pvp.colosseum.player.txt1",
      this._currentHero.challengeScore,
    );
  }
  ...
  private get isOwnerItem(): boolean {
    return (
      this._info &&
      this.roomInfo &&
      this._info.baseHero.userId == this.roomInfo.houseOwnerId
    );
  }
