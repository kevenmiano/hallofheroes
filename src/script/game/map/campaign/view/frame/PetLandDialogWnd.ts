...
export default class PetLandDialogWnd extends BaseWindow {
  ...
  protected refresh() {
    if (this._nodeInfo) {
      this.npcName.text = LangManager.Instance.GetTranslation(
        "public.colon",
        this._nodeInfo.info.names,
      );
      this._itemList = [];
      this._itemList1 = [];
    ...
    if (this._nodeInfo) {
      if (targetItem == this.leaveItem) {
        ...
        if (
          ...
          targetItem.title ==
          LangManager.Instance.GetTranslation(
            "PetLandDialogWnd.targetItem.title",
          )
        ) {
  ...
  protected initOptions(options: Array<string>) {
    ...
    if (
      RingTaskManager.Instance.getRingTaskType() == RewardConditionType.TALKTASK
    ) {
      if (
        ...
        var ringTaskItem: CampaignDialogOptionItem =
          new CampaignDialogOptionItem(
            this._mapId,
            this._nodeInfo.nodeId,
            str,
  ...
  private initOptionsPetLand(options: Array<string>) {
    ...
    if (this._nodeInfo.param4 == "1") {
      //要有921任务才能显示
      var flag: boolean = TaskManage.Instance.cate.hasTaskAndNotCompleted(
        TaskManage.PET_SYSTEM_OPEN_TASK02,
      );
      if (!flag) {
        return;
    ...
    for (var i: number = 0; i < options.length; i++) {
      if (
        options[i] ==
        LangManager.Instance.GetTranslation("petLandDialogWnd.optionTitle")
      ) {
        optionItem = new CampaignDialogOptionItem(
          this._mapId,
          this._nodeInfo.nodeId,
          options[i],
          i,
  ...
  private __timeTickHandler(evt) {
    ...
    var nodePos: Laya.Point = new Laya.Point(
      this._nodeInfo.nodeView.x,
      this._nodeInfo.nodeView.y,
    );
  ...
  private changeDialogContext() {
    ...
    var completeItem: CampaignDialogOptionItem = new CampaignDialogOptionItem(
      this._mapId,
      this._nodeInfo.nodeId,
      "完成",
      0,
