...
export class FogView extends Laya.Sprite {
  ...
  constructor(mapW: number, mapH: number, maskdata: Laya.Texture) {
    ...
    if (maskdata) {
      ...
      this._model.addEventListener(
        CampaignMapEvent.SYS_FOG_DATA,
        this.__sysFogDataHandler,
        this,
      );
      this._model.addEventListener(
        CampaignMapEvent.LOCAL_UPDATE_FOG,
        this.__updateFogDataHandler,
        this,
      );
      this._model.addEventListener(
        CampaignMapEvent.SYN_FOG_DATA,
        this.__synFogHandler,
        this,
      );
  ...
  private draw() {
    ...
    var startY: number = parseInt(
      (my - this._maskBitmapData.height).toString(),
    );
    var endX: number = parseInt(
      (mx + StageReferance.stageWidth + FogView.offe).toString(),
    );
    var endY: number = parseInt(
      (my + StageReferance.stageHeight + FogView.offe).toString(),
    );
    let count: number = 0;
    ...
    for (let element of values) {
      ...
      this._bitmapContainer.graphics.drawTexture(
  ...
  private initMask(w: number, h: number) {
    var count: number = 0;
    for (
      var i: number = -FogView.unitWidth;
      i < w + FogView.unitWidth;
      i += FogView.unitWidth
    ) {
      count = 0;
      for (
        var j: number = -FogView.unitHeight;
        j < h + FogView.unitHeight;
        j += FogView.unitHeight
      ) {
        count++;
  ...
  public static sloveFogPoint(posX: number, posY: number): Laya.Point {
    var p: Laya.Point;
    p = new Laya.Point(
      parseInt((posX + FogView.unitWidth / 2).toString()),
      parseInt((posY + FogView.unitHeight / 2).toString()),
    );
  ...
  private __synFogHandler(data: any) {
    ...
    var obj: any = data;
    this.updatePos(
      obj.y * FogView.unitWidth,
      obj.x * FogView.unitHeight,
      obj.type,
    );
  }
  ...
  private updatePosImp(p: Laya.Point, vy: number, type: number) {
    ...
    if (type >= FogGridType.OPEN_TWO) {
      ...
      this.removeByKey(new Laya.Point(p.x + FogView.unitWidth * 2, p.y));
      this.removeByKey(
        new Laya.Point(p.x + FogView.unitWidth, p.y - FogView.unitHeight * 2),
      );
    ...
    if (type >= FogGridType.OPEN_THREE) {
      ...
      this.removeByKey(new Laya.Point(p.x + FogView.unitWidth * 3, p.y)); //3
      this.removeByKey(
        new Laya.Point(p.x + FogView.unitWidth, p.y - FogView.unitHeight * 3),
      ); //3
      ...
      this.removeByKey(
        new Laya.Point(
          p.x + 2 * FogView.unitWidth,
          p.y - FogView.unitHeight * 2,
        ),
      ); //3
      this.removeByKey(
        new Laya.Point(
    ...
    if (type >= FogGridType.OPEN_FOUR) {
      ...
      this.removeByKey(new Laya.Point(p.x, p.y + FogView.unitHeight * 4)); //4
      this.removeByKey(
        new Laya.Point(p.x - FogView.unitWidth, p.y - FogView.unitHeight * 4),
      ); //4
      this.removeByKey(
        new Laya.Point(p.x - FogView.unitWidth, p.y + FogView.unitHeight * 4),
      ); //4
    ...
    if (vy == 0) {
      ...
      this.removeByKey(
        new Laya.Point(p.x - FogView.unitWidth, p.y - FogView.unitHeight),
      );
      this.removeByKey(new Laya.Point(p.x, p.y + FogView.unitHeight));
      this.removeByKey(
        new Laya.Point(p.x - FogView.unitWidth, p.y + FogView.unitHeight),
      ...
      if (type >= FogGridType.OPEN_TWO) {
        this.removeByKey(
          new Laya.Point(p.x - FogView.unitWidth * 2, p.y - FogView.unitHeight),
        );
        this.removeByKey(
          new Laya.Point(p.x + FogView.unitWidth, p.y - FogView.unitHeight),
        );
      ...
      if (type >= FogGridType.OPEN_THREE) {
        this.removeByKey(
          new Laya.Point(
            p.x - 2 * FogView.unitWidth,
            p.y - FogView.unitHeight * 3,
          ),
        ); //3
        this.removeByKey(
          new Laya.Point(p.x + 2 * FogView.unitWidth, p.y - FogView.unitHeight),
        ); //3
        this.removeByKey(
          new Laya.Point(p.x - 3 * FogView.unitWidth, p.y - FogView.unitHeight),
        ); //3
        this.removeByKey(
          new Laya.Point(p.x + 2 * FogView.unitWidth, p.y + FogView.unitHeight),
        ); //3
      ...
      if (type >= FogGridType.OPEN_FOUR) {
        this.removeByKey(
          new Laya.Point(p.x - 4 * FogView.unitWidth, p.y - FogView.unitHeight),
        ); //4
        this.removeByKey(
          new Laya.Point(p.x - 4 * FogView.unitWidth, p.y + FogView.unitHeight),
        ); //4
