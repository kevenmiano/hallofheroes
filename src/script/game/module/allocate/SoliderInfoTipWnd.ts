...
export default class SoliderInfoTipWnd extends BaseWindow {
    ...
    public OnInitWind() {
        ...
        this.BtnLevelUp.title = LangManager.Instance.GetTranslation("armyII.viewII.allocate.upgrade");
        this.BtnLevelUp2.title = LangManager.Instance.GetTranslation("armyII.viewII.allocate.upgrade");
        if (this.param) {
    ...
    private __soliderClickHandler() {
        ...
        let totalCanStaffingPawn = ArmyManager.Instance.army.baseHero.attackProrerty.totalConatArmy;
        ...
        Logger.info("[SoliderInfoTipWnd]处理方式", type, lackCount, ap.templateInfo.PawnNameLang)
        if (lackCount > 0) {
            let content = LangManager.Instance.GetTranslation("Allocate.askAutoRecruitPawn");
            SimpleAlertHelper.Instance.Show(SimpleAlertHelper.SIMPLE_ALERT, null, null, content, null, null, (b: boolean) => {
                if (!b) {
                    switch (type) {
                        ...
                        case 2:
                            ...
                            break;
                        case 3:
                            this.allocateControler.staffingPawnForReplace(ap, castleOwnPawnCount, dstId, dstCount);
                            break;
                    }
                    return;
                }
                ...
                //剩余的黄金还能招募的士兵数量
                let canRecruitNum = Math.floor(ResourceManager.Instance.gold.count / this.recruitOneGold);
                ...
                if (canMax <= 0) {//能招募的数量为0
                    if (canRecruitNum <= 0) {//剩余的黄金还能招募的士兵数量为0
                        if (ownPawns <= 0) {//该兵种数量为0
                            MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("Allocate.GoldNotEnough"));
                        } else {
                            this.allocateControler.showStopRecruitTip(100);
                            Logger.info("[SoliderInfoTipWnd]没钱招募了且城堡有兵, 直接上阵已有的兵数量", type, lackCount, ap.templateInfo.PawnNameLang)
                            ...
                            switch (type) {
                                ...
                                case 2:
                                    ...
                                    break;
                                case 3:
                                    this.allocateControler.staffingPawnForReplace(ap, actualStaffingCount, dstId, dstCount);
                                    break;
                            }
                        }
                        return
                    } else if (leftPeopleCanBydstCount <= 0) {//人口数量为0
                        if (ownPawns <= 0) {//该兵种数量为0
                            ...
                            Logger.info("[SoliderInfoTipWnd]没人口招募了且城堡有兵, 直接上阵已有的兵数量", type, lackCount, ap.templateInfo.PawnNameLang)
                            actualStaffingCount = ownPawns;
                            switch (type) {
                                case 1:
                                    this.allocateControler.staffingNewPawn(ap, actualStaffingCount);
                ...
                this.allocateControler.sendRecruitPawn(type, ap, recruitNum, actualStaffingCount, dstId, dstCount,true);
    ...
    private get recruitOneGold(): number {
        let arr: Array<t_s_buildingtemplateData> = BuildingManager.Instance.getMaxEffectBuildingTemplate(this.ap.templateInfo);
        if (!arr) return 0;
        ...
        let goldValue: number = playerModel.playerEffect.getRecruitPawnResourceAddition(this.ap.templateInfo.GoldConsume * (1 - buildTemp.Property4 / 100));
    ...
    private initData() {
        if (this.type == 1)//部队界面
        {
            if (this.ap && this.ap.templateInfo && this.ap.ownPawns > 0) {
                this.PawnNameTxt.text = this.ap.templateInfo.PawnNameLang + " " + LangManager.Instance.GetTranslation("buildings.casern.view.RecruitPawnCell.command06", this.ap.templateInfo.Level);
                this.DescTxt.text = this.ap.templateInfo.DescriptionLang;
                this.SpecialTxt.text = LangManager.Instance.GetTranslation("gameguide.GameGuideModel.guideInfoTitle20");
                let specialTemp: t_s_specialtemplateData = TempleteManager.Instance.getPawnSpecialTemplateByID(parseInt(this.ap.specialAbility));
                if (specialTemp) {
                    this.SpecialDescTxt.text = LangManager.Instance.GetTranslation("public.level.name", specialTemp.TemplateNameLang, specialTemp.Grades);
                }
                else {
                    this.SpecialDescTxt.text = LangManager.Instance.GetTranslation("PawnLevelUpWnd.SpecialAbilityNameTxt.openTxt");
                }
                ...
                for (let key in this.attributeArray) {
                    ...
                    if (this.ap[attribute] > 0) {
                        ...
                        (<fgui.GTextField>currentItem.getChild("AttributeValueTxt")).text = this.ap[attribute].toString();
                    }
                }
