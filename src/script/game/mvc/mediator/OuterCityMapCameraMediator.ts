 ...
 *
 */
export class OuterCityMapCameraMediator
  extends Laya.Sprite
  implements IMediator, IEnterFrame
{
  ...
  private setMapSize() {
    let w: number = this.outercityModel
      ? this.outercityModel.mapTempInfo.Width
      : 30000;
    let h: number = this.outercityModel
      ? this.outercityModel.mapTempInfo.Height
      : 30000;
  ...
  enterFrame() {
    if (
      this.outercityModel &&
      this.outercityModel.isWalkIng &&
      !OuterCityMapCameraMediator.isLockCamera
    ) {
      ...
      if (targetPosX + stageWidth > this.mapWidth)
        targetPosX = this.mapWidth - stageWidth;
      if (targetPosY + stageHeight > this.mapHeight)
        targetPosY = this.mapHeight - stageHeight;
      ...
      targetPosX =
        Math.abs(targetPosX - curPosX) < this._xLimen ? curPosX : targetPosX;
      targetPosY =
        Math.abs(targetPosY - curPosY) < this._yLimen ? curPosY : targetPosY;
      let offX: number = targetPosX - curPosX;
      let offY: number = targetPosY - curPosY;
      if (Math.abs(offX) < this._xLimen && Math.abs(offY) < this._yLimen)
        return;
