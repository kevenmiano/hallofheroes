...
export default class Num {
  ...
  constructor(value?: number, magnitude: number = 0) {
    if (value != undefined) this.init(value, magnitude);
  }
  ...
  setNum(num: Num | number): Num {
    if (typeof num === "number") return this.setSmall(num);
  ...
  add(num: Num | number, ret?: Num): Num {
    let n: Num;
    if (typeof num === "number") n = tmp.setSmall(num);
    else n = num;
    ret = ret || new Num(0);
    ...
    if (diff > 0) {
      ...
      else ret.setBig(this);
    } else {
      if (diff > -10) ret.init(n.value + this.value / 10 ** -diff, n.magnitude);
      else ret.setBig(n);
    }
    return ret;
  }
  ...
  sub(num: Num | number, ret?: Num): Num {
    ...
    if (diff > 0) {
      ...
      if (diff > -10) ret.init(this.value / 10 ** -diff - n.value, n.magnitude);
      else ret.setBig(n).ineg();
    }
    return ret;
  }
  ...
  mul(num: Num | number, ret?: Num): Num {
    ret = ret || new Num();
    if (typeof num === "number") ret.init(this.value * num, this.magnitude);
    else ret.init(this.value * num.value, this.magnitude + num.magnitude);
    return ret;
  }
  ...
  compare(num: Num | number): number {
    ...
    if (this.value > 0) {
      if (n.value > 0) {
        let diff = this.magnitude - n.magnitude;
        if (diff === 0) return this.value - n.value;
        return diff;
      }
      return 1;
    } else if (this.value < 0) {
      if (n.value < 0) {
        ...
        if (diff === 0) return n.value - this.value;
        return diff;
      }
      return -1;
    } else {
  ...
  static fromString(text: string): Num {
    if (text) {
      ...
      if (offset === 0) {
        Logger.error("没有字面值");
        return null;
      } else if (offset === len) return new Num(parseInt(text));
      else {
