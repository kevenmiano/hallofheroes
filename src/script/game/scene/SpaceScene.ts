...
export default class SpaceScene
  extends BaseSceneView
  implements IBaseSceneView
{
  ...
  public preLoadingStart(data: Object = null): Promise<void> {
    ...
    return new Promise(async (resolve) => {
      ...
      SpaceManager.Instance.model.mapNodesData =
        SpaceTemplateManager.Instance.nodeDic.get(SpaceManager.SpaceMapId);
      ...
      if (
        this._model &&
        MapData.mapId == this._model.mapTempInfo.Id &&
        MapData.mapBitmap &&
  ...
  public enter(preScene: BaseSceneView, data: Object = null): Promise<void> {
    return new Promise(async (resolve) => {
      ...
      this._flyFindPath = new FindPath8(
        w,
        h,
        this._model.mapTielsData,
        false,
    ...
    return new Promise(async (resolve) => {
      ...
      if (this._preSceneData) {
        if (this._preSceneData.isOpenColosseum) {
          ...
          FrameCtrlManager.Instance.open(EmWindow.RoomList, {
            roomSceneType: RoomSceneType.PVP,
          });
        ...
        // }
          ...
          this._preSceneData.hasOwnProperty("isOpenSinglePassResultView") &&
          this._preSceneData.isOpenSinglePassResultView
        ) {
          ...
          FrameCtrlManager.Instance.open(EmWindow.PetCampaignWnd, {
            playId: PetCampaignManager.Instance.model.userUiPlayInfoMsg.playId,
          });
      ...
      if (
        SpaceManager.Instance.model.isOnObstacle &&
        !this._model.isFlying(this._model.selfArmy.mountTemplateId)
      ) {
  ...
  public moveArmyByPos(
    ...
    if (
      nodeInfo &&
      nodeInfo.info.types != SpaceNodeType.TREASURE_MAP &&
      nodeInfo.info.types != SpaceNodeType.MOVEMENT &&
      nodeInfo.param1 != SpaceNodeType.PARAM_TRANSFER
    ) {
    ...
    if (
      start.x < 0 ||
      start.x > this._model.mapTempInfo.Width ||
      start.y < 0 ||
      this._model.mapTempInfo.Height < start.y
    ) {
      return false;
    }
  ...
  private initRectScan() {
    ...
    this._rectScanUtils.outHeight =
      this._model.mapTempInfo.Height / Tiles.HEIGHT;
