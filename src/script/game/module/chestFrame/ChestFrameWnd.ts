...
export default class ChestFrameWnd extends BaseWindow {
  ...
  OnShowWind() {
    ...
    this.cardList.itemRenderer = Laya.Handler.create(
      this,
      this.renderListItem,
      null,
      false,
    ...
    this._autoTurnTimer.addEventListener(
      TimerEvent.TIMER,
      this.__timerHandler,
      this,
    );
    ...
    this["txt_team_value"].text =
      "+" + this._reportMsg.teamJoin.toFixed(0) + "%";
    this["txt_prop_value"].text =
      "+" + this._reportMsg.propJoin.toFixed(0) + "%";

    let grade = this.thane.grades;
    if (this._single) {
      this.showAgain.selectedIndex =
        grade >= GlobalConfig.ChestFrame.AgainLimitLevel ? 1 : 0;
      this.btnLeave.title = LangManager.Instance.GetTranslation(
        "ChestFrameWnd.Btn.Leave",
      ); //单人副本场景,展示离开战役
    } else {
      this.btnLeave.title =
        LangManager.Instance.GetTranslation("public.confirm1"); //英雄之门场景,展示确定
  ...
  private leaveCampaign() {
    ...
    if (curScene == SceneType.CAMPAIGN_MAP_SCENE && !this._bRestart) {
      ...
      this._curBattleType == BattleType.BATTLE_CHALLENGE
    ) {
      if (!this._bRestart) {
  ...
  private btnAgainClick() {
    let content: string = LangManager.Instance.GetTranslation(
      "map.internals.battle.ChestFrame.againContent",
    );
    SimpleAlertHelper.Instance.Show(
      ...
      null,
      null,
      this.__btnAgainClick.bind(this),
    );
  ...
  private onClickItem(item: ChestItem) {
    ...
    CampaignSocketOutManager.Instance.sendCampaignCard(
      item.isPay,
      item.index,
  ...
  public __updateCardHandler(msg: CampaignTakeCardsMsg) {
    ...
    if (msg.playerId == this.thane.userId) {
      this._autoTurnTimer && this._autoTurnTimer.stop();
      this._turnExitTimer = setTimeout(
        this.leaveCampaign.bind(this),
        ChestFrameData.TurnExitTime * 1000,
      );
    }
  ...
  private __timerHandler() {
    let time: number =
      ChestFrameData.CampaignDownTime + 1 - this._autoTurnTimer.currentCount;
    if (time <= 0) {
      time = 0;
      this.autoTurnCard();
