...
export default class ActivityTimeWnd extends BaseWindow {
  ...
  private addListenerEvent() {
    this.ayList.itemRenderer = Laya.Handler.create(
      this,
      this.onItemRender,
      null,
      false,
  ...
  private initData() {
    this.listData = [
      // 世界boss（开启），默认下一场，可展开
      {
        title: LangManager.Instance.GetTranslation("funpreview.WorldBoss"),
        openCondition: "",
        openServerTimes: "",
        openLocalTimes: "",
        curServerTime: "",
        curLocalTime: "",
        grade: OpenGrades.WORLD_BOSS,
      },

      // 多人竞技（开启 / 结束），默认下一场，可展开
      {
        grade: OpenGrades.CHALLENGE,
        title: LangManager.Instance.GetTranslation("funpreview.PvpBattle"),
        openCondition: "",
        openServerTimes: "",
        openLocalTimes: "",
        curServerTime: "",
      ...
      // 战场（开启 / 结束），默认下一场，可展开
      {
        grade: OpenGrades.RVR,
        title: LangManager.Instance.GetTranslation("funpreview.RvrBattle"),
        openCondition: "",
        openServerTimes: "",
        openLocalTimes: "",
        curServerTime: "",
...

      {
        title: LangManager.Instance.GetTranslation(
          "funpreview.ConsortiaBattle",
        ),
        grade: OpenGrades.CONSORTIA,
        openCondition: "",
        openServerTimes: "",
        openLocalTimes: "",
        curServerTime: "",
      ...
      {
        grade: OpenGrades.PET,
        title: LangManager.Instance.GetTranslation("funpreview.PetBoss"),
        openCondition: "",
        openServerTimes: "",
        openLocalTimes: "",
        curServerTime: "",
      ...
      // 紫晶矿场（开启 / 结束）
      {
        grade: OpenGrades.STAR,
        title: LangManager.Instance.GetTranslation("funpreview.Mine"),
        openCondition: "",
        openServerTimes: "",
        openLocalTimes: "",
        curServerTime: "",
  ...
  private onItemRender(index: number, item: any) {
  ...
  private setWorldBossState(data: ActivityTimeData) {
    ...
    data.openCondition = LangManager.Instance.GetTranslation(
      "public.level3",
      data.grade,
    );
  }
  ...
  private setPvpBattle(data: ActivityTimeData) {
    ...
    for (let timess of timeList) {
      ...
      openTimes2.push(
        this.formatLocalTime(startTime) + "-" + this.formatLocalTime(stopTime),
      );
      ...
      if (sh > currentMinutes && !curOpenTime) {
        ...
        curOpenTime2 =
          this.formatLocalTime(startTime) +
          "-" +
          this.formatLocalTime(stopTime);
        continue;
      }
      //已结束
    ...
    if (!curOpenTime) {
      ...
      curOpenTime2 =
        this.formatLocalTime(sp[0]) + "-" + this.formatLocalTime(sp[1]);
    }
    data.openServerTimes = openTimes.join("\n");
  ...
  private setGuildWarState(data: ActivityTimeData) {
    ...
    for (let t of times) {
      ...
      let tt = t.split(",");
      openTimes2.push(
        this.formatLocalTime(tt[0]) + "-" + this.formatLocalTime(tt[1]),
      );
      let ttt = tt[0].split(":");
      let ttt2 = tt[1].split(":");
      let sminutes = +ttt[0] * 60 + +ttt[1];
    ...
    data.curLocalTime =
      this.formatLocalTime(cur[0]) + "-" + this.formatLocalTime(cur[1]);
