...
export default class MemToolWnd extends BaseWindow {
    ...
    private addEvent() {
        ...
        this.memList.itemRenderer = Laya.Handler.create(this, this.renderListItem, null, false);
        this.btnP.onClick(this, this.onClosePreview);
        MemToolModel.instance.addEventListener('ResfreshMemTool', this.onRefresh, this);
    ...
    onPreview(tex) {
        if (tex) {
            ...
            //@ts-ignore
            this.imgPreview.onExternalLoadSuccess(tex); 
        }
    }
