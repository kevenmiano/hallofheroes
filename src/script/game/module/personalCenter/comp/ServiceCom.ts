...
export default class ServiceCom extends FUI_ServiceCom {
    ...
    onConstruct() {
        super.onConstruct();
        this.txt_warn.text = LangManager.Instance.GetTranslation("SuggestWnd.uploadSuccessTxt");
        this.btn_upload.title = LangManager.Instance.GetTranslation("ServiceCom.uploadScreenTxt");
        this.btn_submit.title = LangManager.Instance.GetTranslation("public.submit");
        this.getChild("n3").text = LangManager.Instance.GetTranslation("ServiceCom.questionTypeTxt");
        this.getChild("n28").text = LangManager.Instance.GetTranslation("SuggestWnd.occurTimeTxt");
        this.getChild("n35").text = LangManager.Instance.GetTranslation("ServiceCom.titleDescTxt");
        this.getChild("n36").text = LangManager.Instance.GetTranslation("ServiceCom.questionTitleTxt");
        this.getChild("n39").text = LangManager.Instance.GetTranslation("ServiceCom.payOrderTxt");
        this.getChild("n42").text = LangManager.Instance.GetTranslation("ServiceCom.payTypeTxt");
        this.getChild("n48").text = LangManager.Instance.GetTranslation("ServiceCom.payValueTxt");
        this.getChild("n49").text = LangManager.Instance.GetTranslation("ServiceCom.payDescTxt");
    ...
    init() {
        ...
        this.beginYear = PlayerManager.Instance.currentPlayerModel.sysCurtime.getFullYear();
        ...
        for (let i = 1; i <= 12; i++) {
            this.monthArr.push(i + LangManager.Instance.GetTranslation("public.month"));
        }
    ...
    private addEvent(): void {
        ...
        this.combox0.on(fairygui.Events.STATE_CHANGED, this, this.onSelectItem, [0]);
        this.combox1.on(fairygui.Events.STATE_CHANGED, this, this.onSelectItem, [1]);
        this.combox2.on(fairygui.Events.STATE_CHANGED, this, this.onSelectItem, [2]);
        this.combox3.on(fairygui.Events.STATE_CHANGED, this, this.onSelectItem, [3]);
    ...
    private onSelectItem(index: number, combo: fgui.GComboBox) {
        if (index == 0) {
            ...
            this.txt_desc.text = '';
            this.txt_num.text = LangManager.Instance.GetTranslation("customerservice.CustomerServiceBaseView.content01", this.MAX_CHARS);
            return;
        }
        this.curYear = this.yearArr[this.combox1.selectedIndex];
        this.curMonth = this.monthArr[this.combox2.selectedIndex];
        if (index == 2) {
            let maxDays = this.getMonthDayCount(this.beginYear + this.combox1.selectedIndex, 1 + this.combox2.selectedIndex);
                ...
                file.onchange = function (e: any): void {
                    if (file.files.length > 0) {
                        if (1024 * 1024 < file.files[0].size) {
                            MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("ServiceCom.onchangeTips"));
                        } else {
    ...
    private onSubmit() {
        ...
        if (this.submitDelay()) {
            if (this.combox0.selectedIndex == 0) {
                CustomerServiceManager.Instance.model.setRechargeData(this.txt_input1.text, this.txt_input2.text, this.txt_input3.text);
            }
            date = new Date(y, m, d);
            CustomerServiceManager.Instance.model.setData(this.txt_title.text, this.txt_desc.text, date, this.combox0.selectedIndex);
