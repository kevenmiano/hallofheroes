...
export class WorldMapCtrl extends FrameCtrlBase {
  ...
  public sendMap(mapId: number): boolean {
    //传送
    this._toMapId = mapId;
    if (!this.checkScene()) {
      return false;
    ...
    let max: number = this.playerEffect.getTransferPowerLimitAddition(
      build.templeteInfo.Property2,
    );
    ...
    let prompt: string = LangManager.Instance.GetTranslation(
      "map.internals.view.displaye.PhysicsMenu.PORTAL",
    );
    SimpleAlertHelper.Instance.Show(
      SimpleAlertHelper.USEBINDPOINT_ALERT,
      { point: recharge2, checkDefault: true },
      prompt,
      str,
      confirm,
      cancel,
      this.transferBack.bind(this),
    );
    return true;
  }
  ...
  public lookMap(mapId: number): boolean {
    //查看
    if (!this.checkScene()) {
      return false;
    }
  ...
  private checkScene(): boolean {
    if (SceneManager.Instance.currentType == SceneType.CAMPAIGN_MAP_SCENE) {
      let str: string =
        LangManager.Instance.GetTranslation("worldmap.command01");
