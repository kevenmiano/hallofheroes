...
export default class ChatFormat {
    ...
    public static createExtraChatCellByContent(srr: Array<string>, chatdata: ChatData): Array<ChatCellData> {
        let i: number = 0;
        let index: number = 0;
        let str: string = "";
        ...
        for (i = 0; i < srr.length; i++) {
            ...
            if (index > 0) {
                cellData = ChatFormat.createGeneralCellData(msg.substr(0, index), chatdata.channel)
                list.push(cellData);
            }
            msg = msg.substr(index + str.length, msg.length);
            if (chatdata.channel == ChatChannel.SYSTEM || chatdata.channel == ChatChannel.INFO || chatdata.channel == ChatChannel.SYS_ALERT || chatdata.channel == ChatChannel.NOTICE) {
                list = list.concat(ChatFormat.createSysCellDataByContent(str, chatdata.userType, chatdata.vipGrade, chatdata.channel, chatdata.senderName, chatdata.receiverName));
            } else if (chatdata.type == 3) {
                xml = XmlMgr.Instance.decode(str);
                cellData = ChatFormat.createGeneralCellData("[", chatdata.channel);
                list.push(cellData);
                cellData = ChatFormat.createPlayerCellData(chatdata.channel, chatdata.userType, Number(xml.a.id), xml.a.serverName, chatdata.vipGrade, Number(xml.a.consortiaId), String(xml.a.name));
                list.push(cellData);
                cellData = ChatFormat.createGeneralCellData("]", chatdata.channel);
                list.push(cellData);
            }
    ...
    public static createSysCellDataByContent(content: string, userType: number, vipGrade: number, channel: number, senderName: string, receiverName: string): Array<ChatCellData> {
        ...
        if (type == 1) {
            ...
            list.push(cellData);
            cellData = ChatFormat.createPlayerCellData(channel, userType, Number(xml.a.id), xml.a.serverName, vipGrade, Number(xml.a.consortiaId), String(xml.a.name));
            list.push(cellData);
            cellData = ChatFormat.createGeneralCellData("]", channel);
            list.push(cellData);
            ...
            cellData = ChatFormat.createRoseBackCellData(content, senderName, receiverName);
            list.push(cellData);
        } else if (type == 12) {
    ...
    public static createExpressionByIndex(eindex: string, channel?: number): ChatCellData {
        let cellData: any = new ChatCellData();
        ...
        //     eindex = "03";
        // }
        cellData.text = " <img src='res/game/face/face" + eindex + ".png' width='30' height='30'></img> ";
        return cellData;
    }

    private static createImageCellDataBySonType(sonType: number, channel: number): ChatCellData {
        let valueTxt: string = "";
    ...
    public static createChannelCellLinkData(msg: string): ChatCellData {
        ...
        cellData.text = LangManager.Instance.GetTranslation("appell.AppellFrame.AppellLink.LinkText");
        cellData.data = msg;
        return cellData;
    }

    public static createVipLinkCellData(channel: number, content: string): ChatCellData {
        let cellData: any = new ChatCellData();
    ...
    public static createStarCellData(content: string): ChatCellData {
        let xml: any = XmlMgr.Instance.decode(content);
        let startemp: t_s_startemplateData = TempleteManager.Instance.getStarTemplateById(Number(xml.a.id));
        ...
        //物品名称颜色区分
        let goodsName: string = "[url='event:'|type:" + cellData.cellType + dataTxt + "]" + "[color=" + GoodsHelp.getGoodColorString(startemp.Profile) + "]" + '[' + xml.a.name + "]" + "[/color]" + "[/url]";
    ...
    public static createCardCellData(content: string): ChatCellData {
        let xml: any = XmlMgr.Instance.decode(content);
        let cardtemp: t_s_powcardtemplateData = TempleteManager.Instance.getPowerCardTemplateByID(Number(xml.a.id));
    ...
    public static createGoodsCellData(content: string): ChatCellData {
        ...
        let tempinfo: t_s_itemtemplateData = TempleteManager.Instance.getGoodsTemplatesByTempleteId(Number(xml.a.tempid));
        ...
        let cellData: ChatCellData = new ChatCellData();
        if (tempinfo.MasterType == GoodsType.PROP || tempinfo.MasterType == GoodsType.PET_CARD) {
            cellData.cellType = ChatCellType.PROP;
        } else if (tempinfo.MasterType == GoodsType.EQUIP || tempinfo.MasterType == GoodsType.HONER) {
        ...
        //物品名称颜色区分
        let goodsName: string = "&nbsp;[url='event:'|cellType:" + cellData.cellType + dataTxt + "]" + "[color=" + GoodsHelp.getGoodColorString(tempinfo.Profile) + "]" + '[' + xml.a.name + "]" + "[/color]" + "[/url]&nbsp;";
        cellData.text = goodsName;
        return cellData;
    }

    public static createGeneralCellData(content: string, channel: number): ChatCellData {
        let cellData: ChatCellData = new ChatCellData();
    ...
    public static createAppellCellData(channel: number, appellId: number): ChatCellData {
        let cellData: ChatCellData = new ChatCellData();
        cellData.cellType = ChatCellType.APPELL;
        cellData.appellId = appellId;
        cellData.faceLink = "chatII.Appell" + AppellManager.Instance.model.getPerfixStyle(appellId);
        let appellInfo: t_s_appellData = TempleteManager.Instance.getAppellInfoTemplateByID(appellId);
        if (appellInfo)
            cellData.text = appellInfo.PerfixLang;
        return cellData;
    }

    public static createVipCellData(channel: number, vipType: number): ChatCellData {
        let cellData: ChatCellData = new ChatCellData();
        cellData.cellType = ChatCellType.VIP;
        cellData.faceLink = "asset.chatII.VipEffect";
        vipType == 2 ? cellData.faceLink = "asset.chatII.VipEffect2" : cellData.faceLink = "asset.chatII.VipEffect";
        return cellData;
    }

    public static createPlayerCellData(channel: number, userType: number, userId: number, serverName: string, vipGrade: number, consortiaId: number, name: string): ChatCellData {
        let cellData: ChatCellData = new ChatCellData();
    ...
    public static createConsortiaCellData(consortiaId: number, consortiaName: string): ChatCellData {
        let cellData: ChatCellData = new ChatCellData();
        cellData.cellType = ChatCellType.CONSORTIA;
        cellData.consortiaId = consortiaId;
        let str: string = "[" + consortiaName + "], " + LangManager.Instance.GetTranslation("consortia.view.myConsortia.recruit.ConsortiaRecruitFrame.command01");
        let roomData = {
    ...
    public static createRoomCellData(content: string): ChatCellData {
        ...
        if (rInfo.campaignId == 0) {
            str = LangManager.Instance.GetTranslation("QuickInviteWnd.PvpInviteTipPrefix02", rInfo.id) + LangManager.Instance.GetTranslation('welcomeTojoin')
        } else {
    ...
    public static parseInviteMsg(roomInfo: RoomInfo): string {
        ...
        if (roomInfo.campaignId == GlobalConfig.CampaignID.AncientRuins) {
            inviteMsg = StringHelper.format(LangManager.Instance.GetTranslation("QuickInviteWnd.AncientRuinsInviteTipPrefix", roomInfo.mapName, roomInfo.id));
        } else {
            let mapName: string = "";
            let templateInfo = roomInfo.mapTemplate ? roomInfo.mapTemplate : CampaignManager.Instance.mapModel.campaignTemplate;
            let lvstr = '';
            if (templateInfo) {
                lvstr = LangManager.Instance.GetTranslation("public.level3", templateInfo.MinLevel.toString());
            }
            mapName = LangManager.Instance.GetTranslation("public.format.concat2word", templateInfo.CampaignNameLang, lvstr);
            inviteMsg = LangManager.Instance.GetTranslation("QuickInviteWnd.PveInviteTipPrefix2", mapName, roomInfo.id);
        }
    ...
    public static createAppellLinkCellData(content: string): ChatCellData {
        ...
        let linkText = this.formatlinkData(linkData, LangManager.Instance.GetTranslation("appell.AppellFrame.AppellLink.LinkText"));
        cellData.text = linkText;
        return cellData;
    }

    public static createRoseBackCellData(content: string, senderName: string, receiverName: string): ChatCellData {
        let xml: any = XmlMgr.Instance.decode(content);
        let cellData: ChatCellData = new ChatCellData();
        cellData.cellType = ChatCellType.ROSE_BACK;
        cellData.text = "[url='event:'|cellType:" + ChatCellType.ROSE_BACK + "]" + "[color=#00F0FF]" + LangManager.Instance.GetTranslation("chat.utils.ChatFormat.presentBack") + "[/color]" + "[/url]";
    ...
    public static createReinforceCellData(content: string): ChatCellData {
        ...
        cellData.text = LangManager.Instance.GetTranslation("chat.utils.Chatformat.Reinforce");
        cellData.userId = xml.a.userId;
        cellData.data = {
            userId: xml.a.userId, mapId: xml.a.mapId, posX: xml.a.posX, posY: xml.a.posY
        }
        return cellData;
    }
    ...
    public static createSeekLinkCellData(content: string): ChatCellData {
        ...
        let linkText = this.formatlinkData(linkData, LangManager.Instance.GetTranslation("hunt.huntLink.gotoSeek"));
        cellData.text = linkText;
        return cellData;
    }
    ...
    public static getContentCellData(chatdata: ChatData): Array<ChatCellData> {
        ...
        msg = chatdata.msg;
        srr = msg.match(/\<([^>]*)>*/g);
        if (srr && srr.length > 0) {//存在特殊字符表情等
            list = list.concat(ChatFormat.createExtraChatCellByContent(srr, chatdata));
        } else if (chatdata.type == 2) {//公会招收
            cellData = ChatFormat.createConsortiaCellData(chatdata.consortiaId, chatdata.consortiaName);
            list.push(cellData);

            cellData = ChatFormat.createGeneralCellData(chatdata.msg, chatdata.channel);
            list.push(cellData);
        } else {
            //如果字符长度超出,分割成多段文本
            // let msgArray = chatdata.msg.split('');
    ...
    public static createPropCellData(jsonData: any): ChatCellData {
        ...
        let tempinfo: t_s_itemtemplateData = TempleteManager.Instance.getGoodsTemplatesByTempleteId(Number(jsonData.templateId));
        if (!tempinfo) return null;
        let info: GoodsInfo = new GoodsInfo();
        ...
        //物品名称颜色区分
        let goodsName: string = "[url='event:'|cellType:" + cellData.cellType + dataTxt + "]" + "[color=" + GoodsHelp.getGoodColorString(tempinfo.Profile) + "]" + '[' + info.templateInfo.TemplateNameLang + "]" + "[/color]" + "[/url]";
        cellData.text = goodsName;
        return cellData;
    }
}
