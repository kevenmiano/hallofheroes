...
export default class ShopDiscountGoodItem extends FUI_ShopDiscountGoodItem {
  ...
  public set info(value: DiscountShopGoodInfo) {
    ...
    this.txtLimit.text = LangManager.Instance.GetTranslation(
      "public.shop.limitbuy",
      value.buycount,
      value.limit,
    );
    ...
    if (this.discountModel.myDiscount <= 0) {
      ...
      let price =
        (value.price * this.discountModel.myDiscount) / 10 < 1
          ? 1
          : (value.price * this.discountModel.myDiscount) / 10;
  ...
  private btnBuyClick() {
    ...
    if (this.discountModel.myDiscount <= 0) {
      MessageTipManager.Instance.show(
        LangManager.Instance.GetTranslation("public.shop.takeTodayDiscount"),
      );
      return;
    }

    let price =
      (this._cellData.price * this.discountModel.myDiscount) / 10 < 1
        ? 1
        : (this._cellData.price * this.discountModel.myDiscount) / 10;
  ...
  private quantitySelectorCallBack(b: boolean, count: number, info: GoodsInfo) {
    if (b) {
      DiscountShopManager.Instance.operationDiscountShop(
        3,
        this._cellData.index,
        count,
      );
    }
  }
