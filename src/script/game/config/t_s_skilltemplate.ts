...
import { InheritRoleType, FaceType, BattleType } from "../constant/BattleDefine";
...
export class t_s_skilltemplateData extends t_s_baseConfigData {
	//
	public propSontypeArr = [9001, 9002, 9003, 9004, 9005, 9006, 9007, 9008, 9009, 9010]
	//TemplateId(编号)
	...
	public get actionTemplateInfo(): t_s_actionData {
		return ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_action, this.ActionId);
	}
	...
	public get preTempList(): any[] {
		...
		for (const key in arr) {
			...
			if (id > 0) {
				temp = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_skilltemplate, id);
				list.push(temp);
			}
		}
		return list;
	...
	public get curTemp(): t_s_skilltemplateData {
		return ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_skilltemplate, this.TemplateId);
	}

	public get nextTemp(): t_s_skilltemplateData {
		return ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_skilltemplate, this.NextTemplateId);
	}

	public get useWayString(): string {
		switch (this.UseWay) {
			case 1:
				return LangManager.Instance.GetTranslation("yishi.datas.templates.SkillTempInfo.UseWay01");
				break;
			case 2:
				return LangManager.Instance.GetTranslation("yishi.datas.templates.SkillTempInfo.UseWay02");
				break;
		}
		return "";
	}
	...
	public get skillTypeDesc(): string {
		if (this.curTemp.UseWay == 2) return "[" + LangManager.Instance.GetTranslation("yishi.datas.templates.SkillTempInfo.UseWay02") + "]";
	...
	private getTalentEffectCdTime(): number {
		...
		for (const key in talentEffectArr) {
			...
			var tempTalentEffect: t_s_talenteffecttemplateData = ConfigMgr.Instance.getTemplateByID(
	...
	private getTalentEffectIds() {
		...
		//符孔技能
		let skillWndData = (FrameCtrlManager.Instance.getCtrl(EmWindow.Skill) as SkillWndCtrl).data as SkillWndData;
		//天赋技能
		let existTalentList = [];
		 ...
		 */
		let list = GoodsManager.Instance.getHeroEquipListById(baseHero.id).getList() as GoodsInfo[];
		for (let i = 0, len = list.length; i < len; i++) {
			const info = list[i];
			let suitTemp: t_s_suitetemplateData = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_suitetemplate, info.templateInfo.SuiteId); //取得套装模板
			if (suitTemp) {
				let skillTemp: t_s_skilltemplateData = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_skilltemplate, suitTemp.Property1);
				if (skillTemp && !StringHelper.isNullOrEmpty(skillTemp.BufferIds)) {
					...
					for (let j: number = 0; j < bufferIds.length; j++) {
						...
						let bufferTemp: t_s_skillbuffertemplateData = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_skillbuffertemplate, bufferId);
		...
		if (tarPetData) {
			...
			for (let i = 0; i < bagArr0.length; i++) {
				let info = bagArr0[i];
				let suitTemp = TempleteManager.Instance.getPetEquipSuitData(info.suitId) as t_s_petequipsuitData;
				if (suitTemp) {
					...
					let skillTemp: t_s_skilltemplateData = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_skilltemplate, suitTemp.SuitSkill);
					if (skillTemp && !StringHelper.isNullOrEmpty(skillTemp.BufferIds)) {
						let bufferIds: any[] = skillTemp.BufferIds.split(",");
						for (let j: number = 0; j < bufferIds.length; j++) {
							let bufferId: number = bufferIds[j];
		...
		for (const key in existTalentList) {
			...
			let temp: t_s_skilltemplateData = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_skilltemplate, id + "");
			if (temp.SonType == 1001 || temp.SonType == 1002 || temp.SonType == 1003) {
			...
			for (let bufferId of buffers) {
				...
				let buffer: t_s_skillbuffertemplateData = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_skillbuffertemplate, bufferId);
				let talentEffects = buffer.TalentEffIds.split(",");
				for (let effectId of talentEffects) {
					if (+effectId == 0) {
						continue;
					}
					let effect: t_s_talenteffecttemplateData = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_talenteffecttemplate, effectId);
					if (effect.EffSkillType == -1 || effect.EffSkillType == this.SonType) {
	...
	private getBattleTalentEffect(effectIds: number[]) {
						...
						for (let i: number = 0; i < buffers.length; i++) {
							...
							if (tempInfo.TalentEffIds) {
								...
								for (let effectId of talentEffects) {
									...
									let effect: t_s_talenteffecttemplateData = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_talenteffecttemplate, effectId);
									if (effect && (effect.EffSkillType == -1 || effect.EffSkillType == this.SonType)) {
		...
		if (bmodel.battleType == BattleType.BATTLE_SECRET) {
			...
			if (scereInfo) {
				for (let index = 0; index < scereInfo.treasureInfoList.length; index++) {
					const element = scereInfo.treasureInfoList[index];
					if (element.template.Type == 1) {
						let skillTemp: t_s_skilltemplateData = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_skilltemplate, element.template.OwnSkill);
						if (!skillTemp) {
							continue
						}
						...
						for (let bufferId of buffers) {
							...
							let buffTemp: t_s_skillbuffertemplateData = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_skillbuffertemplate, bufferId);
							...
							for (let id of talentEffects) {
								...
								let effectTemp: t_s_talenteffecttemplateData = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_talenteffecttemplate, effectId);
								if (effectTemp.EffSkillType == -1) {
									if (battleEffectIds.indexOf(Number(effectId)) < 0) {
										battleEffectIds.push(effectId);
										Logger.info("[t_s_skilltemplate]角色秘境常驻buff中存在天赋技能", effectId)
									}
								}
							}
						}
	...
	private getDealDescript(): string {
		var descript: string;
		descript = StringHelper.replaceStr(this.DescriptionLang, "{Parameter1}", String(this.getTalentEffectParameter1()));
		descript = StringHelper.replaceStr(descript, "{Parameter2}", String(this.getTalentEffectParameter2()));
		descript = StringHelper.replaceStr(descript, "{Cost}", String(this.getTalentEffectSp()));
		return descript;
	}
