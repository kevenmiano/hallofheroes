...
export class DateFormatter {
  ...
  public static parse(dateStr: string, formatStr: string): Date {
    ...
    formatStr.replace(
      /([YMDhms])\1*/g,
      function (param1: any, param2: any, param3: any, param4: any): string {
  ...
  public static format(date: Date, formatStr: string): string {
    ...
    str = formatStr.replace(
      ...
      function (param1: any, param2: any, param3: any, param4: any): string {
        ...
        switch (_loc_2.charAt()) {
          case "Y": {
            _loc_3 = DateFormatter.getIntStrAtLength(
              date.getFullYear(),
              _loc_2.length,
            );
            break;
  ...
  public static unix2ASString(para: number): string {
    ...
    return (
      d.getFullYear() +
      "." +
      DateFormatter.getIntStrAtLength(d.getMonth() + 1, 2) +
      "." +
      DateFormatter.getIntStrAtLength(d.getDate(), 2) +
      "  " +
      DateFormatter.getIntStrAtLength(d.getHours(), 2) +
      ":" +
      DateFormatter.getIntStrAtLength(d.getMinutes(), 2) +
      ":" +
      DateFormatter.getIntStrAtLength(d.getSeconds(), 2)
    );
  }
   ...
   *
   */
  public static getConsortiaCountDate(
    secondCount: number,
    ...
    if (hour > 0) {
      timeStr +=
        ...
        LangManager.Instance.GetTranslation(
          sig ? "public.time.sigHour" : "public.time.hour",
        );
    }
    if (min > 0 && day <= 0) {
      if (min != 60) {
        timeStr +=
          min +
          LangManager.Instance.GetTranslation(
            sig ? "public.minute" : "yishi.utils.DateFormatter.timeStr.min",
          );
      }
    }
    return timeStr;
   ...
   * @returns
   */
  public static getMonthDayString(
    ms: number,
    middle: string = "",
    useCode: boolean = true,
  ): string {
    var timeStr: string = "";
    if (ms < 0) {
      return timeStr;
    ...
    let monthStr =
      date.getMonth() +
      1 +
      (useCode ? LangManager.Instance.GetTranslation("public.month") : "");
    let dayStr =
      date.getDate() +
      (useCode ? LangManager.Instance.GetTranslation("public.daily") : "");
   ...
   * @return -天-小时-分
   */
  public static getFullTimeString(
    sed: number,
    sig: boolean = false,
    value: string = "",
  ): string {
    ...
    if (hour > 0) {
      timeStr +=
        ...
        hour.toString() +
        LangManager.Instance.GetTranslation(
          value ? value : sig ? "public.time.sigHour" : "public.time.hour",
        );
    }
    if (min > 0) {
      timeStr +=
        ...
        LangManager.Instance.GetTranslation(
          value
            ...
            : sig
              ? "public.minute"
              : "yishi.utils.DateFormatter.timeStr.min",
        );
    }
    if (second > 0)
      timeStr +=
        second.toString() +
        LangManager.Instance.GetTranslation(
          value ? value : sig ? "public.time.second" : "public.time.second",
        );
    return timeStr;
  }
  ...
  public static getFormatTimeString(sed: number, split: string = ":"): string {
    ...
    if (hour > 0) {
      timeStr +=
        (day > 0 ? split : "") + ((hour > 10 ? "" : "0") + hour.toString());
    }
    if (min > 0) {
      timeStr +=
        (hour > 0 ? split : "") + ((min > 10 ? "" : "0") + min.toString());
    }
    if (second > 0)
      timeStr +=
        (min > 0 ? split : "") + ((second > 10 ? "" : "0") + second.toString());
    return timeStr;
  }
  ...
  public static getFullDateString(sed: number): string {
    ...
    var hour: number = Math.floor((sed - day * 24 * 3600) / 3600);
    var min: number = Math.floor((sed - day * 24 * 3600 - hour * 3600) / 60);
    var second: number = Math.floor(
      sed - day * 24 * 3600 - hour * 3600 - min * 60,
    );
    if (day > 0) {
      timeStr += day + LangManager.Instance.GetTranslation("public.day");
    }
    ...
    if (second > 0) {
      timeStr +=
        second + LangManager.Instance.GetTranslation("public.time.second");
    }
    return timeStr;
  }
  ...
  public static checkIsSameDay(date01: Date, date02: Date): boolean {
    if (
      date01.getFullYear() == date02.getFullYear() &&
      date01.getMonth() == date02.getMonth() &&
      date01.getDate() == date02.getDate()
    ) {
      return true;
    }
    return false;
