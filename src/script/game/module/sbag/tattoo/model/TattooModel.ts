...
export class TattooModel {
  ...
  public static PROPERTY_NAME_OLD: any[] = [
    "strength",
    "agility",
    "intelligence",
    "captain",
  ...
  public static PROPERTY_NAME: any[] = [
    "",
    "strength",
    "agility",
    "intelligence",
  ...
  private initData() {
    ...
    configInfo =
      TempleteManager.Instance.getConfigInfoByConfigName("Tattoo_consume");
    if (configInfo && configInfo.ConfigValue) {
      TattooModel.DragonCrystalId = Number(
        configInfo.ConfigValue.split(",")[0],
      );
    ...
    configInfo = TempleteManager.Instance.getConfigInfoByConfigName(
      "Tattoo_refresh_price",
    );
    if (configInfo && configInfo.ConfigValue) {
    ...
    configInfo =
      TempleteManager.Instance.getConfigInfoByConfigName("Identical_tattoo");
    if (configInfo && configInfo.ConfigValue) {
      this.identicalTattooNum = parseInt(configInfo.ConfigValue);
    }

    configInfo = TempleteManager.Instance.getConfigInfoByConfigName(
      "Tattoo_Class_Proterty_Value_Max",
    );
    if (configInfo && configInfo.ConfigValue) {
    ...
    configInfo = TempleteManager.Instance.getConfigInfoByConfigName(
      "Tattoo_Hight_MultiplicativeBasis",
    );
    if (configInfo && configInfo.ConfigValue) {
  ...
  public calculateAddProperty(): number[] {
    ...
    if (this.getImprintTotalCountByProperity(0, targetArr) > 0) {
      let addStrengthPrecent: number =
        (this.getImprintTotalCountByProperity(0, targetArr) * addProperty[0]) /
        100;
    ...
    if (this.getImprintTotalCountByProperity(1, targetArr) > 0) {
      let addAgilityPrecent: number =
        (this.getImprintTotalCountByProperity(1, targetArr) * addProperty[1]) /
        100;
    ...
    if (this.getImprintTotalCountByProperity(2, targetArr) > 0) {
      let addIntelligencePrecent: number =
        (this.getImprintTotalCountByProperity(2, targetArr) * addProperty[2]) /
        100;
    ...
    if (this.getImprintTotalCountByProperity(3, targetArr) > 0) {
      let addCaptainPrecent: number =
        (this.getImprintTotalCountByProperity(3, targetArr) * addProperty[4]) /
        100;
    ...
    if (this.getImprintTotalCountByProperity(5, targetArr) > 0) {
      let addPhysiquePrecent: number =
        (this.getImprintTotalCountByProperity(5, targetArr) * addProperty[3]) /
        100;
  ...
  public getTattooMaxAddValue(hole: TattooHole): number {
    ...
    let max: number = Math.ceil(
      (currMaxValue - hole.oldAddingValue) * this.multiplicativeBasis,
    );
    return max;
  }
  ...
  public canUpgrade(): boolean {
    ...
    return (
      hasNum >= this.tattooConsumeNum &&
      ArmyManager.Instance.thane.grades >= OpenGrades.TATTOO
    );
  }
  ...
  public hasNotReplaceProperty(): boolean {
    ...
    if (
      ...
      this.lastOpType != RoleCtrl.OP_REFRESH &&
      this.lastOpType != RoleCtrl.OP_ADVANCE_REFRESH
    ) {
      return true;
    }
    return false;
