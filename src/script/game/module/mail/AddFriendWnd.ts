...
export default class AddFriendWnd extends BaseWindow {
    ...
    public OnInitWind() {
        ...
        if (this.params) {
            ...
            this.selectItemIndex = this.params.itemIndex >= 0 ? this.params.itemIndex : -1;
        }
        this.tabSelectController = this.getController("tabSelect");
        this.tabSelectController.on(fairygui.Events.STATE_CHANGED, this, this.onChangeTab);
        this.list.itemRenderer = Laya.Handler.create(this, this.renderListItem, null, false);
        Utils.setDrawCallOptimize(this.list);
        NotificationManager.Instance.addEventListener(EmailEvent.ADD_FRIEND_CELL_CHECK, this.onSelectItem, this);
    ...
    onChangeTab() {
        ...
        switch (this.tabSelectController.selectedIndex) {
            case 0:
                ...
                break;
            case 1:
                this.initComboBox(ConsortiaManager.Instance.model.consortiaMemberList.getList());
                break;
        }
        if (this.tabSelectController.selectedIndex == this.selectTabIndex && this.selectItemIndex >= 0) {
    ...
    private initComboBox(list) {
        ...
        for (const key in list) {
            if (Object.prototype.hasOwnProperty.call(list, key)) {
                let info = list[key];
                if (info.userId == ArmyManager.Instance.thane.userId)
                    continue;
                listModel.push(info);
            }
        }
    ...
    OnHideWind() {
        super.OnHideWind();
        this.callbackFunc && this.callbackFunc(this.selectItem, this.tabSelectController.selectedIndex, this.list.selectedIndex);
    }
