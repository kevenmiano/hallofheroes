...
export class StoreBagCellClickMediator implements IMediator {
  ...
  private __mouseDoubleClick(c: Event) {
    ...
    let to_pos =
      this._cell.info.templateInfo.SonType == GoodsSonType.SONTYPE_INTENSIFY
        ? 1
        : 0;
    if (
      this.model.curTabIndex == ForgeData.TabIndex.XL &&
      this._cell.info.templateInfo.MasterType != GoodsType.EQUIP
    ) {
      let str: string = LangManager.Instance.GetTranslation(
        "cell.mediator.storebag.IntensifyBagCellDropMediator.command01",
      );
  ...
  private showResolveAlert(): boolean {
    ...
    if (outdate) {
      let content: string = LangManager.Instance.GetTranslation(
        "cell.mediator.storebag.StoreBagCellClickMediator.resolveStrengthenAlert",
      );
      ...
      SimpleAlertHelper.Instance.Show(
        ...
        null,
        null,
        this.startResolveAlertBack.bind(this),
      );
    }
    return outdate;
  }
  ...
  private get canDoubleClick(): boolean {
    ...
    if (this.model.curTabIndex == ForgeData.TabIndex.XQ) {
      if (
        this._cell.info.templateInfo.SonType == GoodsSonType.SONTYPE_MOUNT_PORP
      )
        return false;
    }
    if (
      this.model.curTabIndex == ForgeData.TabIndex.QH &&
      this.model.isIntensifing
    )
      return false;
    return true;
  }
  ...
  private get isMountJewel(): boolean {
    if (
      this.model.curTabIndex == ForgeData.TabIndex.XQ &&
      this._cell.info.templateInfo.SonType == GoodsSonType.SONTYPE_MOUNT
    ) {
      ...
      if (info) {
        ...
        if (emptyPos.length <= 0) {
          ...
          this.storeControler.sendItemMount(
            emptyPos[0],
            this._cell.info.pos,
            this._cell.info.bagType,
          );
