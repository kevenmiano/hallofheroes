...
export class TaskTemplate extends GameEventDispatcher {
	...
	public removeEvent() {
		this.thane.removeEventListener(PlayerEvent.THANE_LEVEL_UPDATE, this.__taskChangeHandler, this);
		this.consortiaInfo.removeEventListener(ConsortiaEvent.CONSORTIA_OFFER_CHANGE, this.__taskChangeHandler, this);
		this.playerInfo.removeEventListener(PlayerEvent.PLAYER_INFO_UPDATE, this.__taskChangeHandler, this);
		BuildingManager.Instance.removeEventListener(BuildingEvent.BUILDING_UPGRADE_RECEIVE, this.__taskChangeHandler, this);
		ArmyManager.Instance.removeEventListener(ServiceReceiveEvent.UPGRADE_PAWN_SUCCESS, this.__taskChangeHandler, this);
		FriendManager.getInstance().removeEventListener(FriendUpdateEvent.ADD_FRIEND, this.__taskChangeHandler, this);
		FriendManager.getInstance().removeEventListener(FriendUpdateEvent.REMOVE_FRIEND, this.__taskChangeHandler, this);
		this.goodsCountByTempId.removeEventListener(DictionaryEvent.ADD, this.__goodsCountChangeEvent, this);
		this.goodsCountByTempId.removeEventListener(DictionaryEvent.DELETE, this.__goodsCountChangeEvent, this);
		this.goodsCountByTempId.removeEventListener(DictionaryEvent.UPDATE, this.__goodsCountChangeEvent, this);
		ArmyManager.Instance.removeEventListener(ArmyEvent.SORT_PAWN, this.__taskChangeHandler, this);
		PlayerManager.Instance.removeEventListener(ServiceReceiveEvent.TRANSEFER_CASTLE_SUCCESS, this.__taskChangeHandler, this);
		this.playerModel.towerInfo1.removeEventListener(PlayerEvent.UPDATE_TOWER_INFO, this.__taskChangeHandler, this);
		this.playerModel.removeEventListener(PlayerEvent.SYSTIME_UPGRADE_DATE, this.__taskChangeHandler, this);
		this.consortiaModel.removeEventListener(ConsortiaEvent.UPDA_CONSORTIA_INFO, this.__taskChangeHandler, this);
	}
	...
	private addListenerTo(condition: t_s_questcondictionData) {
		switch (condition.CondictionType) {
			case 1://领主等级
				this.thane.addEventListener(PlayerEvent.THANE_LEVEL_UPDATE, this.__taskChangeHandler, this);
				break;
			case 2://建筑/科技等级
				BuildingManager.Instance.addEventListener(BuildingEvent.BUILDING_UPGRADE_RECEIVE, this.__taskChangeHandler, this);
				break;
			case 10://兵种升级 
				ArmyManager.Instance.addEventListener(ServiceReceiveEvent.UPGRADE_PAWN_SUCCESS, this.__taskChangeHandler, this);
				break;
			case 21://添加好友
				FriendManager.getInstance().addEventListener(FriendUpdateEvent.ADD_FRIEND, this.__taskChangeHandler, this);
				FriendManager.getInstance().addEventListener(FriendUpdateEvent.REMOVE_FRIEND, this.__taskChangeHandler, this);
				break;
			case 22://加入工会, 退出工会,检测是否还能完成
				this.playerInfo.addEventListener(PlayerEvent.PLAYER_INFO_UPDATE, this.__taskChangeHandler, this);
				break;
			case 23://新手进度
				
				break;
			case 27://收集士兵掉落物品
				this.goodsCountByTempId.addEventListener(DictionaryEvent.ADD, this.__goodsCountChangeEvent, this);
				this.goodsCountByTempId.addEventListener(DictionaryEvent.DELETE, this.__goodsCountChangeEvent, this);
				this.goodsCountByTempId.addEventListener(DictionaryEvent.UPDATE, this.__goodsCountChangeEvent, this);
				break;
			case 28://收集英雄掉落物品
				this.goodsCountByTempId.addEventListener(DictionaryEvent.ADD, this.__goodsCountChangeEvent, this);
				this.goodsCountByTempId.addEventListener(DictionaryEvent.DELETE, this.__goodsCountChangeEvent, this);
				...
				break;
			case 29://工会贡献
				this.consortiaInfo.addEventListener(ConsortiaEvent.CONSORTIA_OFFER_CHANGE, this.__taskChangeHandler, this);
				break;
			case 32://采集物品
				this.goodsCountByTempId.addEventListener(DictionaryEvent.ADD, this.__goodsCountChangeEvent, this);
				this.goodsCountByTempId.addEventListener(DictionaryEvent.DELETE, this.__goodsCountChangeEvent, this);
				...
				break;
			case 34://队形调整
				ArmyManager.Instance.addEventListener(ArmyEvent.SORT_PAWN, this.__taskChangeHandler, this);
				break;
			case 37://地图传送
				PlayerManager.Instance.addEventListener(ServiceReceiveEvent.TRANSEFER_CASTLE_SUCCESS, this.__taskChangeHandler, this);
			case 38://地下迷宫
				this.playerModel.towerInfo1.addEventListener(PlayerEvent.UPDATE_TOWER_INFO, this.__taskChangeHandler, this);
				break;
			case 39://二次登录
				this.playerModel.addEventListener(PlayerEvent.SYSTIME_UPGRADE_DATE, this.__taskChangeHandler, this);
				break;
			case 49://采集物品
				this.goodsCountByTempId.addEventListener(DictionaryEvent.ADD, this.__goodsCountChangeEvent, this);
				this.goodsCountByTempId.addEventListener(DictionaryEvent.DELETE, this.__goodsCountChangeEvent, this);
				...
				break;
			case 51://公会养成任务
				this.consortiaModel.addEventListener(ConsortiaEvent.UPDA_CONSORTIA_INFO, this.__taskChangeHandler, this);
				break;
			case 76://符文学习
				NotificationManager.Instance.addEventListener(RuneEvent.RUNE_UPGRADE, this.__taskChangeHandler, this);
				break;
			case 78://符文升级
				NotificationManager.Instance.addEventListener(RuneEvent.RUNE_UPGRADE, this.__taskChangeHandler, this);
				break;
	...
	public commit() {
		...
		this.changeDate = PlayerManager.Instance.currentPlayerModel.sysCurTimeBySecond;
		if (!this.taskInfo.isComplete && !this.hasCompleted) {
			...
			Laya.timer.callLater(TaskManage.Instance, TaskManage.Instance.dispatchEvent, [TaskEvent.TASK_DETAIL_CHANGE, this]);
		}
		...
		Laya.timer.callLater(this, this.dispatchEvent, [TaskEvent.TASK_DETAIL_CHANGE, this]);
	}
	...
	public get isAvailable(): boolean {
		...
		if (!this.isAchieved) {//从未完成->true
			return true;
		}
		if (!this.IsRepeat) {//不可重复完成->false
			return false;
		}
		return true;
	}
	...
	private checkChallengeSelf(condition: t_s_questcondictionData, index: number): string {
		if (this.taskInfo) {
	...
	private checkSencondLogin(condition: t_s_questcondictionData): string {
		var nowDate: number = new Date(this.playerModel.sysCurTimeBySecond * 1000).getDate();
	...
	private checkGoodsCount(condition: t_s_questcondictionData): string {
		var count: number = GoodsManager.Instance.getGoodsNumByTempId(parseInt(condition.Para1));
	...
	private checkConsortiaOffer(condition: t_s_questcondictionData): string {
		...
		if (this.playerInfo.consortiaTotalOffer >= condition.Para2)
			return LangManager.Instance.GetTranslation("buildings.offerreward.data.BaseOfferReward.progress");
		return " (" + this.playerInfo.consortiaTotalOffer + "/" + condition.Para2 + ")";
	}
	...
	private checkBuildingOrTecGrades(condition: t_s_questcondictionData): string {
		var building: BuildInfo = BuildingManager.Instance.getBuildingInfoBySonType(parseInt(condition.Para1));
		...
		return "(" + building.templeteInfo.BuildingGrade + "/" + condition.Para2 + ")";
	}
	private checkHeroGrades(condition: t_s_questcondictionData): string {
		var grade: number = condition.Para2;
		var current: number = 0;
		if (parseInt(condition.Para1) == 0 || this.thane.templateInfo.Job == parseInt(condition.Para1)) {
	...
	private checkWilanCount(condition: t_s_questcondictionData, index: number): string {
		if (parseInt(this.taskInfo["condition" + (index + 1)]) >= condition.Para2) {
			return LangManager.Instance.GetTranslation("buildings.offerreward.data.BaseOfferReward.progress");
		}
		else
			return " (" + this.taskInfo["condition" + (index + 1)] + "/" + condition.Para2 + ")"
	}
	 ...
	 * 
	 */
	private checkCount(condition: t_s_questcondictionData, index: number): string {
		if (this.taskInfo) {
			if (this.taskInfo["condition" + (index + 1)] >= condition.Para2) {
				return LangManager.Instance.GetTranslation("buildings.offerreward.data.BaseOfferReward.progress");
			}
	 ...
	 * 
	 */
	private checkRuneStudy(condition: t_s_questcondictionData, index: number): string {
		...
		if (this.taskInfo) {
			if (parseInt(condition.Para1) == 0) {
				...
				currentCount = this.thane.runeCate.getRuneInfoByRuneType(parseInt(condition.Para1)).grade;
			}
	 ...
	 * 
	 */
	private checkRuneGrade(condition: t_s_questcondictionData, index: number): string {
		var currentMaxGrade: number = 0;
		if (this.taskInfo) {
			if (parseInt(condition.Para1) == 0) {//任意符文达到要求的等级
				if(this.thane.runeCate.getMaxGradeRuneInfo()){
					currentMaxGrade = this.thane.runeCate.getMaxGradeRuneInfo().grade;
				}
			} else {//根据类型condition.Para1取得对应符文的等级；
				currentMaxGrade = this.thane.runeCate.getRuneInfoByRuneType(parseInt(condition.Para1)).grade;
			}
