...
export default class SinglePassModel extends FrameDataBase {
	...
	public static FLOOR_STR: Array<string> = ["", "Ⅰ", "Ⅱ", "Ⅲ", "Ⅳ", "Ⅴ", "Ⅵ", "Ⅶ", "Ⅷ", "Ⅸ", "Ⅹ"];
	...
	constructor() {
		...
		this._orderList = [];
		this._cardInfos = [];
		let cfgItem = TempleteManager.Instance.getConfigInfoByConfigName("SinglePass_Count");
		if (cfgItem) {
			this._maxCount = Number(cfgItem.ConfigValue);
		}
		this._bugleCount = 2;
		var cardInfo: SinglePassCardInfo;
		for (var i: number = 1; i <= SinglePassModel.MAX_FLOOR * SinglePassModel.TOLLGATE_PER_FLOOR; i++) {
	...
	public getCardInfoByFloor(floor: number): Array<SinglePassCardInfo> {
		...
		for (var i: number = len - SinglePassModel.TOLLGATE_PER_FLOOR; i < len; i++) {
	...
	public isMaxJudge(targetIndex: number): boolean {
		for (const key in this._cardInfos) {
			if (Object.prototype.hasOwnProperty.call(this._cardInfos, key)) {
				var cardInfo: SinglePassCardInfo = this._cardInfos[key];
				if (cardInfo.tollgate == targetIndex && cardInfo.judge == SinglePassModel.JUDGE_MAX) {
					return true;
				}
			}
		}
