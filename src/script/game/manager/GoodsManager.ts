...
export class GoodsManager extends GameEventDispatcher {
  ...
  private __bagUpdateHandler(pkg: PackageIn): void {
      ...
      if (this._goodsListByPos.hasOwnProperty(key) && !key.startsWith("__")) {
        ...
        if (info.bagType == BagType.Player) {
          ...
          if (this._preGoodsCount[info.templateId]) {
            this._preGoodsCount.add(info.templateId, {
              value: ~~this._preGoodsCount[info.templateId] + count,
            });
          } else {
            this._preGoodsCount.add(info.templateId, { value: count });
          }
        }
      }
    }

    let msg: ItemMovedListMsg = pkg.readBody(
      ItemMovedListMsg,
    ) as ItemMovedListMsg;
    ...
    if (
      ...
      FoisonHornManager.Instance.model.isOpen
    ) {
    ...
    for (let i: number = 0; i < msg.moved.length; i++) {
      ...
      if (flag) {
        ...
        good_info =
          this._goodsListByPos[
            changePos + "_" + changeObjectId + "_" + changeBagType
          ];
        if (!good_info) {
          good_info = new GoodsInfo();
          this._goodsListByPos.add(
            changePos + "_" + changeObjectId + "_" + changeBagType,
            good_info,
          );
          isAdd = true;
        }
        good_info = ItemHelper.readGoodsInfo(
          itemMsg.item as ItemInfoMsg,
          good_info,
        );
        ...
        Logger.info(
          "更新物品" +
            ...
            good_info.id +
            "----name:" +
            good_info.templateInfo.TemplateNameLang +
            "----count:" +
            good_info.count,
        );
        ...
        if (this._preGoodsCount[good_info.templateId]) {
          addCount =
            good_info.count - this._preGoodsCount[good_info.templateId];
        }
        ...
        Logger.info(
          "删除物品" + changeBagType + "_" + changeObjectId + "_" + changePos,
        );
        good_info = this._goodsListByPos[
          changePos + "_" + changeObjectId + "_" + changeBagType
        ] as GoodsInfo;
        if (good_info) {
          this._goodsListByPos.del(
            changePos + "_" + changeObjectId + "_" + changeBagType,
          );
          ...
          if (
            good_info.templateInfo.SonType == GoodsSonType.SONTYPE_TREASURE_MAP
          ) {
    ...
    //更新物品
    updateGoodes &&
      this.dispatchEvent(BagEvent.UPDATE_BAG, updateGoodes, addCounts);
    //删除物品事件
    ...
    for (const key in this._goodsListByPos) {
      if (this._goodsListByPos.hasOwnProperty(key) && !key.startsWith("__")) {
        ...
        if (this._newGoodsCount[info.templateId]) {
          this._newGoodsCount.add(info.templateId, {
            value: ~~this._newGoodsCount[info.templateId] + count,
          });
        } else {
          this._newGoodsCount.add(info.templateId, { value: count });
        }
        // }
      }
    }
    ...
    for (const key in this._goodsListByPos) {
      if (this._goodsListByPos.hasOwnProperty(key) && !key.startsWith("__")) {
        ...
        if (
          ...
          info.templateInfo &&
          info.templateInfo.SonType == sontype
        ) {
          count += info.count;
        }
      }
  ...
  public getBagCountByTempIdAndPos(
    bagType: number,
    tempId: number,
    pos: number,
  ): number {
  ...
  public getHeroEquipBagAndPlayerBagList(): SimpleDictionary {
    ...
    for (const key in this._goodsListByPos) {
      if (this._goodsListByPos.hasOwnProperty(key) && !key.startsWith("__")) {
        ...
        if (
          this.isGneralBagGoods(goods) ||
          this.isGneralStoreBagGoods(goods) ||
          goods.bagType == BagType.HeroEquipment ||
          goods.bagType == BagType.Hide
        ) {
  ...
  public findPetBagEmputyPos(): number {
    let str =
      TempleteManager.Instance.getConfigInfoByConfigName("pet_bag").ConfigValue;
  ...
  public isHeroGoods(heroId: number, goods: GoodsInfo): boolean {
    ...
    if (
      goods.bagType == BagType.HeroEquipment &&
      goods.objectId == heroId &&
      goods.templateInfo &&
      goods.templateInfo.MasterType == GoodsType.EQUIP
    ) {
      return true;
    } else {
      return false;
  ...
  public isHonerGoods(heroId: number, goods: GoodsInfo): boolean {
    ...
    if (
      goods.bagType == BagType.Honer &&
      goods.objectId == heroId &&
      goods.templateInfo &&
      goods.templateInfo.MasterType == GoodsType.HONER
    ) {
      return true;
    } else {
      return false;
   ...
   *
   */
  public getHeroGoodsListByTypeAndId(
    goodsType: number,
    id: number,
  ): SimpleDictionary {
    let dic: SimpleDictionary = new SimpleDictionary();
    for (const key in this._goodsListByPos) {
      if (this._goodsListByPos.hasOwnProperty(key) && !key.startsWith("__")) {
        ...
        if (
          goods.objectId == id &&
          goods.templateInfo &&
          goods.templateInfo.MasterType == goodsType
        ) {
          dic.add(goods.pos, goods);
        }
      }
  ...
  public getEmputyHonerPos(): number {
    for (let pos: number = 0; pos < 1; pos++) {
      let info: GoodsInfo =
        this._goodsListByPos[pos + "_" + this.thane.id + "_" + BagType.Honer];
      if (!info) {
        return pos;
      }
    }
  ...
  public getHeroGoodsListBySonTypeAndId(
    sonType: number,
    id: number,
  ): SimpleDictionary {
    let dic: SimpleDictionary = new SimpleDictionary();
  ...
  public getGoodsByObjectIdAndGoodID(
    objectId: number,
    goodsId: number,
  ): GoodsInfo {
    for (const key in this._goodsListByPos) {
  ...
  public getHideBagItemByPos(pos: number): GoodsInfo {
    let info: GoodsInfo = this._goodsListByPos[
      pos + "_0_" + BagType.Hide
    ] as GoodsInfo;
    return info;
  }
  ...
  public getGoodsNumByTempId(tempId: number): number {
    ...
    for (const key in this._goodsListByPos) {
      if (this._goodsListByPos.hasOwnProperty(key) && !key.startsWith("__")) {
        ...
        if (
          goods.objectId == 0 &&
          goods.bagType == BagType.Player &&
          goods.templateId == tempId
        ) {
          num += goods.count;
        }
      }
  ...
  public getGoodsNumByTempIdInHeroEquipBayAndPlayerBag(tempId: number): number {
    ...
    for (const key in this._goodsListByPos) {
      if (this._goodsListByPos.hasOwnProperty(key) && !key.startsWith("__")) {
        let goods: GoodsInfo = this._goodsListByPos[key];
        if (
          (this.isHeroGoods(this.thane.id, goods) ||
            this.isGneralBagGoods(goods)) &&
          goods.templateId == tempId
        ) {
          num += goods.count;
  ...
  public getGoodsNumBySonType(
    sonType: number,
    bagType: BagType = BagType.Player,
  ): number {
    let num: number = 0;
    for (const key in this._goodsListByPos) {
      if (this._goodsListByPos.hasOwnProperty(key) && !key.startsWith("__")) {
  ...
  public getGeneralBagPropList(): any[] {
    ...
    goodsList.keys.forEach((key) => {
      ...
      if (
        goods.templateInfo &&
        goods.templateInfo.MasterType == GoodsType.PROP
      ) {
        arr.push(goods);
      }
    });
    ...
    for (const key in dic) {
      if (dic.hasOwnProperty(key)) {
        ...
        if (
          ...
          info.strengthenGrade != info.templateInfo.StrengthenMax
        ) {
          return true;
        }
      }
    ...
    if (goods.templateInfo) {
      switch (type) {
        ...
        case BagSortType.Equip:
          ...
          if (
            goods.templateInfo.MasterType == GoodsType.EQUIP ||
            goods.templateInfo.SonType == GoodsSonType.SONTYPE_HONER
          ) {
            if (!this.isFashion(goods.templateInfo)) {
              flag = true;
            }
  ...
  public isInSellPos(sonType: number): boolean {
    ...
    for (const key in this._goodsListByPos) {
      if (this._goodsListByPos.hasOwnProperty(key) && !key.startsWith("__")) {
        let info: GoodsInfo = this._goodsListByPos[key];
        if (
          this._sellPosList.indexOf(info.pos) >= 0 &&
          info.templateInfo &&
          info.templateInfo.SonType == sonType
        ) {
          return true;
  ...
  public isEquip(temp: t_s_itemtemplateData): boolean {
    ...
    if (
      temp.SonType == GoodsSonType.SONTYPE_WEAPON ||
      temp.SonType == GoodsSonType.SONTYPE_ASSISTANT ||
      temp.SonType == GoodsSonType.SONTYPE_HEADDRESS ||
      temp.SonType == GoodsSonType.SONTYPE_CLOTHES ||
      temp.SonType == GoodsSonType.SONTYPE_MANTEAU ||
      temp.SonType == GoodsSonType.SONTYPE_NECKLACE ||
      temp.SonType == GoodsSonType.SONTYPE_RING ||
      temp.SonType == GoodsSonType.SONTYPE_TRINKET ||
  ...
  public checkEquipUpgrade(composeId: number): boolean {
    ...
    composeGoodsInfo.ownCount1 = composeGoodsInfo.template.Material1
      ? this.getGoodsNumByTempIdInHeroEquipBayAndPlayerBag(
          composeGoodsInfo.template.Material1,
        )
      : -1;
    composeGoodsInfo.ownCount2 = composeGoodsInfo.template.Material2
      ? this.getGoodsNumByTempIdInHeroEquipBayAndPlayerBag(
          composeGoodsInfo.template.Material2,
        )
      : -1;
    composeGoodsInfo.ownCount3 = composeGoodsInfo.template.Material3
      ? this.getGoodsNumByTempIdInHeroEquipBayAndPlayerBag(
          composeGoodsInfo.template.Material3,
        )
      : -1;
    composeGoodsInfo.ownCount4 = composeGoodsInfo.template.Material4
      ? this.getGoodsNumByTempIdInHeroEquipBayAndPlayerBag(
          composeGoodsInfo.template.Material4,
        )
      : -1;
