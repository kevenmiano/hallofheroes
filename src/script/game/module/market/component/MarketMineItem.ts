...
export default class MarketMineItem extends FUI_MarketMineItem {
    ...
    private initTranslate() {
        this.bpriceTxt.text = LangManager.Instance.GetTranslation("Market.mine.backPrice");
        this.cancelBtn.title = LangManager.Instance.GetTranslation("Market.mine.cancel");
        this.getBtn.title = LangManager.Instance.GetTranslation("Market.mine.getting");
        this.taxTxt.text = LangManager.Instance.GetTranslation("market.mine.taxTxt");
    ...
    public updateView() {
        ...
        if (this.typeCrol.selectedIndex == 0 && this.stateCrol.selectedIndex == 6) {            
            let getPoint = this._IMarketOrderMsg.tradingPoint - this._IMarketOrderMsg.tax;
            this.wpriceLab.text = getPoint + "";
        }


        //提取/取消/无操作
        this.extCrol.selectedIndex = (this._IMarketOrderMsg.status == 2) ? 1 : (this._IMarketOrderMsg.status == 4 || this._IMarketOrderMsg.status == 6) && !this._IMarketOrderMsg.extract ? 2 : 0;

        let tradingPoint = this._IMarketOrderMsg.tradingPoint;

        let backPoint = this._IMarketOrderMsg.point - tradingPoint;

        this.backCrol.selectedIndex = (this._IMarketOrderMsg.status == 6 && backPoint > 0) ? 1 : 0;
