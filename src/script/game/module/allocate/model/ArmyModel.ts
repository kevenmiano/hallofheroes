...
export default class ArmyModel extends FrameDataBase {
  ...
  public autoAllocate() {
    ...
    var count: number = 0;
    if (ap && ap.ownPawns != 0) {
      if (ap.ownPawns >= this.thane.attackProrerty.totalConatArmy) return;
  ...
  private getPawnNumber(pawn: ArmyPawn, newPawn: ArmyPawn): number {
    ...
    if (!newPawn) {
      ...
      number =
        this.thane.attackProrerty.totalConatArmy /
        pawn.templateInfo.NeedPopulation;
    } else {
      newPawnNumber = this.getNewNumberByID(newPawn.templateId);
      number =
        this.thane.attackProrerty.totalConatArmy /
        newPawn.templateInfo.NeedPopulation;
    }
    return number - oldNumber >= newPawnNumber
      ? newPawnNumber
      : number - oldNumber;
  }
      ...
      if (pawn && pawn.ownPawns != 0) {
        var value: number =
          (pawn.attack +
            ...
            pawn.live) *
            50000 +
          parseInt(pawn.templateInfo.NeedBuilding.toString());
        arr.push(value);
        dic[value] = pawn;
      }
    }
  ...
  public hasEquipEffect(property: string): number {
    var goodsList: SimpleDictionary =
      GoodsManager.Instance.getHeroEquipListById(this.thane.id);
  ...
  public hasAddedPoint(): boolean {
    if (this.thane.baseProperty.addAgilityPoint + this.addAgility > 0)
      return true;
    if (this.thane.baseProperty.addIntellectPoint + this.addAbility > 0)
      return true;
    if (this.thane.baseProperty.addPowerPoint + this.addStrength > 0)
      return true;
    if (this.thane.baseProperty.addPhysiquePoint + this.addPhysique > 0)
      return true;
    if (this.thane.baseProperty.addCaptainPoint + this.addCaptain > 0)
      return true;
    return false;
  }
