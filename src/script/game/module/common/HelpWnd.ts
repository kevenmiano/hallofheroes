...
export default class HelpWnd extends BaseWindow {
    ...
    public OnInitWind() {
        ...
        let title = this.params.title ? this.params.title : LangManager.Instance.GetTranslation("friends.view.FriendsHelpFrame.title");
        //@ts-ignore
    ...
    private addEvent() {
        this.contentlist.getChild('content').on(Laya.Event.LINK, this, this.onClickLink);
        // ToolTipsManager.Instance.register(this);
    }

    private offEvent() {
        this.contentlist.getChild('content').off(Laya.Event.LINK, this, this.onClickLink);
