...
export class EquipTipView extends FUI_EquipTipView {
    ...
    protected addPropertyArray: any[] = ["0", "20-100", "8-40", "6-30", "4-20", "2-10"];
    ...
    protected updateTransform() {
        if (this._info) {
            ...
            this.txt_gradeCount.text = LangManager.Instance.GetTranslation("yishi.view.tips.goods.ChatEquipTips.gradeCount", "[color=#01F0ED]&nbsp;&nbsp;" + scoreStr + "[/color]");
            if (!isfashion) {
                ...
                this.txt_time.text = LangManager.Instance.GetTranslation("yishi.view.tips.goods.EquipTip.time.text") + ":" + DateFormatter.getFullDateString(this._info.validDate * 60);
            } else {
                this.showFasionState();
            }

            this.txt_price.text = temp.SellGold == 0 ? "" : GoodsCheck.getSellGold(this._info).toString();
            let timeStr: string = "";
            if (this._info.id != 0) {
                ...
                if (this._info.bagType == BagType.HeroEquipment) {
                    equiped = GoodsManager.Instance.isHeroGoods(this.thane.id, this._info)
                } if (this._info.bagType == BagType.Honer) {
                    equiped = GoodsManager.Instance.isHonerGoods(this.thane.id, this._info)
                }

                this.equipType = equiped ? EquipTip.EQUIPED : 0;
                //操作按钮
                if (this.canOperate && GoodsCheck.checkGoodsCanEquip(this._info, this.thane, false) && GoodsCheck.isGradeFix(this.thane, this._info.templateInfo, false)) {
                    this.btn_use.visible = true;
                    if (this._equipType == EquipTip.EQUIPED) {
                        this.btn_use.text = LangManager.Instance.GetTranslation("armyII.viewII.skill.btnEquipOff");//卸下
                    }
                    else {
                        if (this.fashionModel.isFashion(this.info)) {
                            this.btn_use.text = LangManager.Instance.GetTranslation("tasktracetip.view.OpenBagTipView.btnTxt1");//
                        } else {
                            this.btn_use.text = LangManager.Instance.GetTranslation("armyII.viewII.skill.btnEquipOn");//装备
                        }
                    }
                }
                else {
            ...
            if (BagHelper.isOpenConsortiaStorageWnd()) {//是否打开公会宝箱
            ...
            if (this._info.templateInfo.ObtainId && this.showObtain) {
                if (BagHelper.isOpenPlayerInfo() || BagHelper.isOpenBag()) {
                    ...
                    this.equipTipsContent.getChild('btn_obtain').onClick(this, this.onObtain);
    ...
    private showFasionState() {
        ...
        let skillInfo: t_s_skilltemplateData = this.fashionModel.getFashionObjectSkillTemplate(temp);
        ...
        let identitySkill: t_s_skilltemplateData = TempleteManager.Instance.getSkillTemplateInfoById(this._info.appraisal_skill);
        let skillTip: string = (identitySkill == null ? this.addPropertyTxt(temp) : identitySkill.SkillTemplateName);
        this.txt_fashionIdentity.color = '#ffecc6';
        if (this._info.appraisal_skill > 0) {
            this.txt_time.text = LangManager.Instance.GetTranslation("fashion.FashionSwitchItem.Identity") + ':';
            ...
            if (identitySkill.Grades >= 5) {
                ...
                this.txt_fashionIdentity.text = skillTip + LangManager.Instance.GetTranslation("fashion.identity.max", maxskillTip);
            }
        } else {
            skillTip = (identitySkill == null ? this.addPropertyTxtMax(temp) : identitySkill.SkillTemplateName);
            //当未鉴定时文字为: 未鉴定（最大属性: 力量+100）
            this.txt_time.text = LangManager.Instance.GetTranslation("fashion.FashionSwitchItem.noIdentity") + ':';
            this.txt_time.color = '#666666';
            this.txt_fashionIdentity.text = LangManager.Instance.GetTranslation("fashion.identity.max", skillTip);
        }
        ...
        let pStr: string = "";
        if (temp.Property2 > 0) {
            pStr = StringHelper.format("[color={0}]{1}[/color]",
                GoodsSonType.getColorByProfile(6 - temp.Property2),
                //海外增加翻译
                LangManager.Instance.GetTranslation("public.fashionProfile",priority[temp.Property2]));
        }
        this.txt_fashionProfile.text = pStr;
        this.txt_gradeCount.text = LangManager.Instance.GetTranslation("fashion.identity.tip");
        this.dot.visible = true;
        this.grade_group.visible = this.txt_time.visible = this.txt_gradeCount.visible = this.txt_fashionProfile.visible = true;
    ...
    protected addPropertyTxt(info: t_s_itemtemplateData): string {
        switch (info.SonType) {
            case GoodsSonType.FASHION_WEAPON:
                return LangManager.Instance.GetTranslation("armyII.ThaneAttributeView.Tip01") + this.addPropertyArray[info.Property2];
            case GoodsSonType.FASHION_CLOTHES:
                return LangManager.Instance.GetTranslation("armyII.ThaneAttributeView.Tip02") + this.addPropertyArray[info.Property2];
            case GoodsSonType.FASHION_HEADDRESS:
                return LangManager.Instance.GetTranslation("armyII.ThaneAttributeView.Tip03") + this.addPropertyArray[info.Property2];
            case GoodsSonType.SONTYPE_WING:
                return LangManager.Instance.GetTranslation("armyII.ThaneAttributeView.Tip04") + this.addPropertyArray[info.Property2];
        }
        return "";
    }
    ...
    protected addPropertyTxtMax(info: t_s_itemtemplateData): string {
        ...
        switch (info.SonType) {
            case GoodsSonType.FASHION_WEAPON:
                return LangManager.Instance.GetTranslation("armyII.ThaneAttributeView.Tip01") + str;
            case GoodsSonType.FASHION_CLOTHES:
                return LangManager.Instance.GetTranslation("armyII.ThaneAttributeView.Tip02") + str;
            case GoodsSonType.FASHION_HEADDRESS:
                return LangManager.Instance.GetTranslation("armyII.ThaneAttributeView.Tip03") + str;
            case GoodsSonType.SONTYPE_WING:
                return LangManager.Instance.GetTranslation("armyII.ThaneAttributeView.Tip04") + str;
        }
        return "";
    }
    ...
    private onBtnUseClick1() {
        if (this.btn_use.title == LangManager.Instance.GetTranslation("tasktracetip.view.OpenBagTipView.btnTxt1")) {
            NotificationManager.Instance.dispatchEvent(FashionEvent.SWALLOW, this._info.templateInfo.SonType);
            NotificationManager.Instance.sendNotification(TipsEvent.EQUIP_TIPS_HIDE);
            return;
        }
        if (this.isFashion) {
        ...
        if (this._equipType == EquipTip.EQUIPED) {
            ...
            if (goodsInfo.existJewel()) {
                var content: string = LangManager.Instance.GetTranslation("OpenBagTipView.exchange.content");
                SimpleAlertHelper.Instance.Show(SimpleAlertHelper.SIMPLE_ALERT, null, null, content, null, null, (b: boolean) => {
                    this.moveBagToBag(this._info.bagType, this._info.objectId, this._info.pos, BagType.Player, 0, targetPos, 1, b)
                });
                Laya.timer.once(100, this, () => {
                    NotificationManager.Instance.sendNotification(TipsEvent.EQUIP_TIPS_HIDE);
                });
            } else {
                this.moveBagToBag(this._info.bagType, this._info.objectId, this._info.pos, BagType.Player, 0, targetPos, 1)
            }
        }
        else {
            if (this._info.templateInfo.SonType == GoodsSonType.SONTYPE_RUNNES) {
                let existRunes: GoodsInfo = GoodsManager.Instance.getBattleRunesByTempId(this._info.templateId);
                ...
                if (existRunes && existRunes.count != existRunes.templateInfo.MaxCount) {
                    ...
                    AudioManager.Instance.playSound(SoundIds.BAG_EQUIP_SOUND);
                    this.moveBagToBag(this._info.bagType, this._info.objectId, this._info.pos, BagType.Battle, this.thane.id, existRunes.pos, count);
                }
                else if (this._toPos != -1) {
                    AudioManager.Instance.playSound(SoundIds.BAG_EQUIP_SOUND);
                    this.moveBagToBag(this._info.bagType, this._info.objectId, this._info.pos, BagType.Battle, this.thane.id, this._toPos, this._info.count);
                }
            }
                ...
                AudioManager.Instance.playSound(SoundIds.BAG_EQUIP_SOUND);
                this.moveBagToBag(this._info.bagType, this._info.objectId, this._info.pos, BagType.Honer, this.thane.id, this._toPos, this._info.count);
            }
            else {
                let heroBag: GoodsInfo[] = GoodsManager.Instance.getHeroGoodsListByTypeAndId(this._info.templateInfo.MasterType, this.thane.id).getList();
                let pos_arr: number[] = GoodsSonType.getSonTypePos(this._info.templateInfo.SonType);
                ...
                if (pos_arr.length > 1) {
                    for (const key in heroBag) {
                        if (heroBag.hasOwnProperty(key)) {
                            ...
                            if ((i.templateInfo.SonType == this._info.templateInfo.SonType) && (i.pos == pos_arr[0] + t_index)) {
                                t_index++;
                            }
                        }
                    }
                    if (t_index >= pos_arr.length) {
                        ...
                        for (let i = 0, len = pos_arr.length; i < len; i++) {
                            const pos = pos_arr[i];
                            targetInfos.push(GoodsManager.Instance.getHeroEquipByPos(this.thane.id, pos));
                        }
                ...
                if (this._toPos != -1) {
                    AudioManager.Instance.playSound(SoundIds.BAG_EQUIP_SOUND);
                    if (this.btn_use.text == LangManager.Instance.GetTranslation("armyII.viewII.skill.btnEquipExchange")) {
                        let goodsInfo: GoodsInfo = this.getGoodsInfoByPos(this._toPos);
                        if (!goodsInfo.existJewel()) {
                            this.moveBagToBag(this._info.bagType, this._info.objectId, this._info.pos, BagType.HeroEquipment, this.thane.id, this._toPos, 1);
                        }
                        else {//有宝石
                            ...
                            var content: string = LangManager.Instance.GetTranslation("OpenBagTipView.exchange.content");
                            SimpleAlertHelper.Instance.Show(SimpleAlertHelper.SIMPLE_ALERT, null, prompt, content, confirm, cancel, this.moveConfirm.bind(this));
                            Laya.timer.once(100, this, () => {
                                NotificationManager.Instance.sendNotification(TipsEvent.EQUIP_TIPS_HIDE);
                            });
                        }
    ...
    private moveConfirm(b: boolean, flag: boolean) {
        if (b) {//确定替换
            this.moveBagToBag(this._info.bagType, this._info.objectId, this._info.pos, BagType.HeroEquipment, this.thane.id, this._toPos, 1, true);
        }
        else {
            this.moveBagToBag(this._info.bagType, this._info.objectId, this._info.pos, BagType.HeroEquipment, this.thane.id, this._toPos, 1, false);
        }
    }

    private getGoodsInfoByPos(pos: number): GoodsInfo {
        let goods: GoodsInfo = new GoodsInfo();
        let dic: SimpleDictionary = GoodsManager.Instance.getHeroGoodsListByTypeAndId(GoodsType.EQUIP, this.thane.id);
    ...
    private moveBagToBag(beginBagType: number, beginObjectId: number, beginPos: number, endBagType: number, endObjectId: number, endPos: number, count: number, isUnstail: boolean = false) {
        let roleCtrl: RoleCtrl = FrameCtrlManager.Instance.getCtrl(EmWindow.SRoleWnd) as RoleCtrl;
        ...
        PlayerManager.Instance.moveBagToBag(beginBagType, beginObjectId, beginPos, endBagType, endObjectId, endPos, count, isUnstail);
        Laya.timer.once(100, this, () => {
            NotificationManager.Instance.sendNotification(TipsEvent.EQUIP_TIPS_HIDE);
        });
    }
    ...
    private onClickObtain(item: fairygui.GButton) {
        ...
        if (obtainId > 10000 && obtainId < 20000) {//战役
            FrameCtrlManager.Instance.open(EmWindow.PveCampaignWnd, { CampaignId: campaignId });
        } else if (obtainId > 20000 && obtainId < 30000) {//多人副本
            FrameCtrlManager.Instance.open(EmWindow.PveMultiCampaignWnd, { CampaignId: campaignId, difficult: difficult });
        }
    ...
    private removeEvent() {
        if (this._info && this._info.templateInfo.ObtainId) {
            ...
            this.equipTipsContent.getChild('btn_obtain').offClick(this, this.onObtain);
        }
