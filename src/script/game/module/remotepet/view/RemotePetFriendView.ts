...
export class RemotePetFriendView extends FUI_RemotePetFriendView {
    ...
    private initView() {
        this.emptyLab.text = LangManager.Instance.GetTranslation("remotepet.friend.empty");
        this.titleLab.text = LangManager.Instance.GetTranslation("remotepet.friend.title");
    }

    private addEvent() {
        this.model.addEventListener(RemotePetEvent.FRIEND_LIST, this.__listHandler, this);
    }
    private removeEvent() {
        this.model.removeEventListener(RemotePetEvent.FRIEND_LIST, this.__listHandler, this);
    }
    ...
    private onClickItem(item: RemotePetFriendItemView) {
        ...
        var content = LangManager.Instance.GetTranslation("remotepet.RemotePetFriendFrame.content", info.friendName, info.petName, info.petFight);
        SimpleAlertHelper.Instance.popAlerFrame(prompt, content, confirm, cancel, { callback: this.alertBack.bind(this) });
