...
export class ConsortiaStorageWnd extends BaseWindow {
    ...
    private initEvent() {
        ...
        this._model.addEventListener(ConsortiaEvent.UPDA_CONSORTIA_INFO, this.__onConsortiaInfoUpdataHandler, this);
    ...
    private renderListItem(index: number, item: ConsortiaStorageCell) {
        ...
        this._itemDic.add(item.item.pos + "_" + item.item.objectId + "_" + item.item.bagType, item);
    ...
    private updateBagLimit() {
        ...
        this.bagLimit = preLevel == ConsortiaUpgradeType.MAX_LEVEL ? this._bagTotalNum : preLevel * ConsortiaModel.CASE_CELL_OPEN_BY_LEVEL;
    }
    ...
    private sortBag(isOverlay: boolean, sortType: number) {
        ...
        if (isChange || isOverlay) {
            GoodsManager.Instance.fixBagItem(send_pos_old, send_pos_new, BagType.Storage, isOverlay);
        }
    }
    ...
    private removeEvent() {
        ...
        this._model.removeEventListener(ConsortiaEvent.UPDA_CONSORTIA_INFO, this.__onConsortiaInfoUpdataHandler, this);
