...
export class PetExchangeShopWnd extends BaseWindow {
    ...
    private _moneyRes: string[] = ["asset.stop.Money101", "asset.stop.Money102", "asset.stop.Money103", "asset.stop.Money104", "asset.stop.Money105", "asset.stop.Money106"];
    ...
    private initData() {
        this._control = FrameCtrlManager.Instance.getCtrl(EmWindow.ShopWnd) as ShopControler;
    ...
    private initEvent() {
        ...
        this._shopModel.addEventListener(ShopEvent.GOODS_LIST_UPDATE, this.__refreshGoods, this);
        this._shopModel.addEventListener(ShopEvent.GOODS_INFO_UPDATE, this.__updateGoodsHandler, this);
        GoodsManager.Instance.addEventListener(BagEvent.UPDATE_BAG, this.__updateGoodsHandler, this);
        NotificationManager.Instance.addEventListener(NotificationEvent.SHOP_TIME_BUY_REFRESH,this.__refreshGoods,this);
    }
    ...
    private onListItemRender(index: number, item: ShopItem) {
        ...
        let payType:number = item.info.PayType - ShopGoodsInfo.PAY_BY_FRAGMENT_1001;
    ...
    private onListItemClick(item: ShopItem, evt: Laya.Event) {
        ...
        this._hasMoney = GoodsManager.Instance.getGoodsNumByTempId(this._currentSelectedItem.info.NeedItem);
        ...
        this.txt_itemName.color = FilterFrameText.Colors[eFilterFrameText.ItemQuality][goodsInfo.templateInfo.Profile - 1];
        let leftNum = 0;
        let maxCount = 999;
        if (item.info.WeeklyLimit > -1) {
            this.txt_limit_type.text = LangManager.Instance.GetTranslation("Shop.Promotion.weeklimit");
            this.txt_limit.text = (item.info.WeeklyLimit - item.info.weekCount) + "/" + item.info.WeeklyLimit;
            ...
            this.txt_limit.text = (item.info.OneDayCount != -1 && item.info.OneDayCount != undefined) ? (item.info.OneDayCount - item.info.OneCurrentCount) + "/" + item.info.OneDayCount : LangManager.Instance.GetTranslation("shop.view.GoodsItem.limited07");
        }
        ...
        if (item.info.WeeklyLimit > -1) {
            ...
            this.btn_buy.enabled = leftNum > 0 && !ShopManager.Instance.requestBuyState;
        }else{
            this.stepper.show(0, 1, 1, this.currentCanCount > 0 ? this.currentCanCount : 1, this.currentCanCount, 1, this._handler);
        ...
        this.costGoods = TempleteManager.Instance.getGoodsTemplatesByTempleteId(templateId)  
        // let costNum = GoodsManager.Instance.getGoodsNumByTempId(templateId);
        //消耗物品数字处显示修改为“当前道具数量/所需道具数量”
        this.txt_price.text = this._hasMoney +"/" + item.info.price * this.stepper.value;
    ...
    private updateMoney() {
        ...
        for (let index = 0; index < 4; index++) {
            ...
            this["loader_moneyType1_" + index].url = fgui.UIPackage.getItemURL("Space", element);
    ...
    private stepperChangeHandler(value: number) {
        this.txt_price.text = this._hasMoney +"/"+this._currentSelectedItem.info.price * this.stepper.value;
        if (Number(this.txt_price.text) > GoodsManager.Instance.getGoodsNumByTempId(this._currentSelectedItem.info.NeedItem)) {
    ...
    private get currentCanCount(): number {
        let pointCount: number = Math.floor(this._hasMoney / this._currentSelectedItem.info.price);
    ...
    private onBtnBuy() {
        ...
        if (this._hasMoney < info.price * this.stepper.value) {
            let itemName: string = (ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_itemtemplate, info.NeedItem) as t_s_itemtemplateData).TemplateNameLang;
            let str: string = LangManager.Instance.GetTranslation("notEnough", itemName);
        ...
        var content: string = LangManager.Instance.GetTranslation("shopcommwnd.buy.alert", totalCost,this.costGoods.TemplateNameLang ,this.stepper.value,this.item.info.templateInfo.TemplateNameLang);
    ...
    private removeEvent() {
        ...
        this._shopModel.removeEventListener(ShopEvent.GOODS_LIST_UPDATE, this.__refreshGoods, this);
        this._shopModel.removeEventListener(ShopEvent.GOODS_INFO_UPDATE, this.__updateGoodsHandler, this);
        GoodsManager.Instance.removeEventListener(BagEvent.UPDATE_BAG, this.__updateGoodsHandler, this);
        NotificationManager.Instance.removeEventListener(NotificationEvent.SHOP_TIME_BUY_REFRESH,this.__refreshGoods,this);
    }
