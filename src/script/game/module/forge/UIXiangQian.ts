...
export default class UIXiangQian extends BaseFguiCom {
    ...
    constructor(comp: fgui.GComponent) {
        ...
        this._txtEquipName = this.sourceEquipItem.getChild("rTxtDesc") as fgui.GRichTextField
        this._equipItem = this.sourceEquipItem.getChild("item") as StoreIntensifyCell;
        ...
        this.txtRecommend.text = LangManager.Instance.GetTranslation("store.view.mount.MountView.JobTipTxt" + (playerJob + 1));
    ...
    private __bagItemUpdateHandler(info: GoodsInfo) {
        let item: StoreIntensifyCell | ForgeTargetItem = this._itemDic[info.pos + "_" + info.objectId + "_" + info.bagType];
        if (item) {
            //@ts-ignore
            info.isInlay = true;
            item.info = info;
    ...
    private updateSparkCount() {
        this.txtSparkCount.text = "x" + GoodsManager.Instance.getGoodsNumByTempId(ItemID.BORT_PROP);
    }
    ...
    private updateView(value: GoodsInfo) {
        ...
        if (equipInfo) {
            ...
            this._txtEquipName.color = ForgeData.Colors[equipInfo.templateInfo.Profile - 1]
        } else {
        ...
        for (let i = 1; i <= ForgeData.XQJewelNum; i++) {
            ...
            if (item.bNotOpen) {
                ...
                // }
                //开启第四个宝石孔不需要史诗装备限制
                let desc = LangManager.Instance.GetTranslation("Forge.Inlay.ClickToOpen");// LangManager.Instance.GetTranslation(item.bSSEquipOpen ? "Forge.Inlay.OpenEpicEquip" : "Forge.Inlay.ClickToOpen")
                rTxtDesc.text = desc
            } else {
                if (item.info) {
                    rTxtDesc.text = item.item.info.templateInfo.DescriptionLang
                } else {
                    rTxtDesc.text = LangManager.Instance.GetTranslation("Forge.Inlay.CanInlay")
                }
            }
        }
    ...
    private getCanInlayItemIndex(): number {
        ...
        for (let i = 1; i <= ForgeData.XQJewelNum; i++) {
            let item: ForgeTargetItem = this["sourceJewelItem" + i] as ForgeTargetItem;
