...
export default class HitTestUtils {
    ...
    /**判断点击是否在对象范围内 */
    static isInArea(target: Laya.Sprite | Laya.Animation, point: Laya.Point): boolean {
        if (!target) return false;
        let targetGraphicBounds = target.getBounds();
        let stagePos:Laya.Point;
        if(target.parent) {
            //@ts-ignore
            stagePos = (target.parent).localToGlobal(new Laya.Point(target.x, target.y),true);
        } else {
            stagePos = new Laya.Point(target.x, target.y);
        }
        let rect = new Laya.Rectangle(stagePos.x, stagePos.y, targetGraphicBounds.width, targetGraphicBounds.height);
    ...
    static hitTest(target: Laya.Sprite | any, point: Laya.Point): boolean {
        ...
        try {
            ...
            let s: Uint8Array = texture.getPixels(Math.floor(point.x), Math.floor(point.y), 1, 1);
    ...
    static hitTestAlpha(target: Laya.Sprite | Laya.Texture, point: Laya.Point): number {
        if (!target) {
            return 0
        }
        try {
