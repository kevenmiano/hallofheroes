...
export default class FoisonHornManager {
    ...
    private initEvent() {
        ServerDataManager.listen(S2CProtocol.U_C_FOISON_HORN, this, this.__updateFoisonHornMsg);
    }
    ...
    private __updateFoisonHornMsg(pkg: PackageIn) {
        ...
        if (msg.activation) {
            ...
            for (let i: number = 0; i < goodsArr.length; i++) {
                ...
                if (arr.length == 3) {
                    ...
                    let hasCount:number = GoodsManager.Instance.getGoodsNumByTempId(info.templateId);
        ...
        NotificationManager.Instance.dispatchEvent(NotificationEvent.FOISONHORN_INFO_CHANGE)
    }
