...
export default class Tabbar {
  ...
  init(
    page_list: object,
    main_list: fgui.GButton[],
    sub_list_map: Map<number, fgui.GButton[]>,
    sel_callback: Function = null,
  ) {
  ...
  private interrupt(
    changeToTabIndex: number,
    elementIndex: number,
    bMainTab: boolean,
  ): boolean {
    let bBreak =
      this.interruptCallback && this.interruptCallback(changeToTabIndex);
    if (bBreak) {
      let list = bMainTab
        ? this._mainlist
        : this.getCurSubList(this._curMainIndex);
