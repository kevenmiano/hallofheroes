...
export class PlayerWalkMediator implements IMediator {
  ...
  public register(target: object) {
    ...
    this._target.info.addEventListener(
    ...
    NotificationManager.Instance.addEventListener(
      ...
      this.__playerArriveHandler,
      this,
    );
    NotificationManager.Instance.addEventListener(
      JoyStickEvent.JoystickMoving,
      this.__joystickMoving,
      this,
    );
    this._sendPosRate = 7;
    this._walkPath = [];
  ...
  public unregister(target: object) {
    ...
    if (this._target) {
      this._target.info.removeEventListener(
        AiEvents.UPDATE_PATHS,
        this.__updatePathHandler,
        this,
      );
  ...
  private sendPath() {
    ...
    if (this._walkPath.length > 0) {
      ...
      MapSocketOuterManager.sendCampaignArmyMove(
        this._target.data.id,
        this._walkPath,
        this._target.data.mapId,
        vType,
      );
  ...
  private __walkNextHandler(data: any) {
    ...
    if (
      !(
        ...
        WorldBossHelper.checkPetLand(this.mapModel.mapId)
      )
    ) {
      //世界BOSS,宠物岛以外
      if (
      ...
      this.mapModel.updateFog(
        this._target.x,
        this._target.y,
        FogGridType.OPEN_FOUR,
      );
    }
  }
  ...
  private __walkOverHandler(evt: ObjectsEvent) {
    ...
    if (
      ...
      // 鼠标点击在移动结束时候检测
      if (
        !this._doneWalkOverDetection &&
        this._lastWalkOverPos.distance(this._target.x, this._target.y) > 40
      ) {
      ...
      if (
        CampaignArmyViewHelper.selfArmyToEnd(this._target.aiInfo, this._target)
      ) {
        return;
      }
