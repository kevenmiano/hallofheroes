...
import { Enum_BagGridState, Enum_BagState } from "../../bag/model/Enum_BagState";
...
export class SBagCom extends FUI_SBagCom {
    ...
    private renderListItem(index: number, item: PlayerBagCell) {
        ...
        this._isConsortiaStorageShow = FrameCtrlManager.Instance.isOpen(EmWindow.ConsortiaStorageWnd);
        if (this._isBattleGuardShow) {
    ...
    protected sortFun(a: GoodsInfo, b: GoodsInfo): number {
        ...
        if (GoodsManager.Instance.isType(a, this._sortByType) && !GoodsManager.Instance.isType(b, this._sortByType)) {
            ...
            if (a.templateInfo.IsCanBatch == 1  && b.templateInfo.IsCanBatch == 0) {
                return -1;
            } else if (a.templateInfo.IsCanBatch == 0 && b.templateInfo.IsCanBatch == 1) {
                return 1;
            } else {
                if ((index_a == GoodsSonType.SONTYPE_OPENBOX
                    || index_a == GoodsSonType.SONTYPE_MULTI_BOX)
                    && index_b != GoodsSonType.SONTYPE_OPENBOX && index_b != GoodsSonType.SONTYPE_MULTI_BOX) {
                    return -1;
                } else if ((index_b == GoodsSonType.SONTYPE_OPENBOX
                    || index_b == GoodsSonType.SONTYPE_MULTI_BOX)
                    && index_a != GoodsSonType.SONTYPE_OPENBOX && index_a != GoodsSonType.SONTYPE_MULTI_BOX) {
                    return 1;
                } else {
                    if (index_a == GoodsSonType.SONTYPE_BUFFER && index_b != GoodsSonType.SONTYPE_BUFFER) {
                        return -1;
                    } else if (index_a != GoodsSonType.SONTYPE_BUFFER && index_b == GoodsSonType.SONTYPE_BUFFER) {
                        return 1;
                    } else {
                        if(BagHelper.checkCanUseGoods(a.templateInfo.SonType) && !BagHelper.checkCanUseGoods(b.templateInfo.SonType)){
                            return -1;
                        }else if(!BagHelper.checkCanUseGoods(a.templateInfo.SonType) && BagHelper.checkCanUseGoods(b.templateInfo.SonType)){
                            return 1;
                        }else{
                            if (index_a < index_b) {
                                return -1;
                                ...
                                if (a.templateId < b.templateId) {
                                    ...
                                    if (a.strengthenGrade < b.strengthenGrade) {
                                        ...
                                        if (a.templateInfo.NeedGrades < b.templateInfo.NeedGrades) {
                                            return -1;
                                        } else if (a.templateInfo.NeedGrades > b.templateInfo.NeedGrades) {
                                            return 1;
                                        } else {
                                            return 0;
                                        }
