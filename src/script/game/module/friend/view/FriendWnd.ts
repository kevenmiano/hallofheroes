...
export class FriendWnd extends BaseWindow {
  ...
  private getListByGroupId(id: number): ThaneInfo[] {
    ...
    switch (id) {
      ...
      case FriendGroupId.BLACKLIST:
        ...
        break;
      default:
        arr = FriendManager.getInstance().getListByGroupIdAndState(
          id,
          StateType.NO_LIMIT,
        );
    }
    arr = ArrayUtils.sortOn(
      arr,
      ["friendGrade", "friendGp", "grades", "nickName"],
      [
        ArrayConstant.NUMERIC | ArrayConstant.DESCENDING,
        ArrayConstant.NUMERIC | ArrayConstant.DESCENDING,
        ArrayConstant.NUMERIC | ArrayConstant.DESCENDING,
        ArrayConstant.CASEINSENSITIVE,
      ],
    );
  ...
  private __systimeUpdateHandler() {
    if (
      PlayerManager.Instance.currentPlayerModel.sysCurTimeBySecond -
        FriendManager.getInstance().model.lastReqRecentsTime >
      1800
    ) {
      let privateDataList: Array<ThaneInfo> =
        FriendManager.getInstance().privatePerson;
  ...
  private __updateInviteView() {
    let toolInviteFriendList: ToolInviteInfo[] =
      FriendManager.getInstance().toolInviteFriendList;
    if (toolInviteFriendList && toolInviteFriendList.length > 0) {
      ...
      this.btn_applyFriend.selfRedDotPos(
        this.btn_applyFriend.width - 20,
        6,
        true,
      );
    } else {
  ...
  private __updateRecommendView() {
    let recommendList: RecommendInfo[] =
      FriendManager.getInstance().recommendList;
    if (
      recommendList &&
