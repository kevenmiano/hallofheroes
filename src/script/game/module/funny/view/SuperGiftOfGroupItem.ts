...
export class SuperGiftOfGroupItem extends FUI_SuperGiftITitleItemCell {
	...
	/**返还集合动态间距数组 */
	private _diamondColumnGap: number[] = [0, 84, 33, 7, -8, -18, -25, -30, -35, -39, -42, -44, -46, -48];
	...
	protected onConstruct() {
		...
		this.progressBar = this.compDiamond.getChild("progressBar") as FUI_Prog_CDLine;
	...
	private initEvent() {
		...
		this.giftList.itemRenderer = Laya.Handler.create(this, this.onRenderGiftGoodItem, null, false);
		this.diamondList.itemRenderer = Laya.Handler.create(this, this.onRenderGiftDiamondItem, null, false);
	}
	...
	private onRenderGiftDiamondItem(index: number, item: FUI_SuperGiftDiaCom) {
		item.getChild("txt_goal").text = this._diamondList[index].sellCount.toString();
		item.getChild("txt_number").text = this._diamondList[index].backCount.toString();
		item.getController("finish").selectedIndex = this._sale >= this._diamondList[index].sellCount ? 1 : 0;
		if (index > 2 && this._updateBounds && this._diamondListMode === 2) {
			this.diamondList.width = this.diamondList.width + this.diamondList.columnGap + item.width;
	...
	private showAlter(): boolean {
		...
		if (result) {
			...
			SimpleAlertHelper.Instance.Show(SimpleAlertHelper.SIMPLE_ALERT, null, null, content, null, null, this.alterCallback.bind(this));
		}
	...
	private buyGift(): void {
		...
		if (this._purchase + this._purchased > this._quota) {
			MaskLockOper.Instance.doCall(false);
			MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("yishi.SuperGiftOfGroupManager.gift.quota.tip", this._quota));
			this.btn_buy.enabled = true;
			return;
		}
		...
		if (!SuperGiftOfGroupManager.Instance.open) {
			MaskLockOper.Instance.doCall(false);
			MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("yishi.SuperGiftOfGroupManager.open.exist"));
			this.btn_buy.enabled = true;
			return;
		}
	 ...
	 */
	public info(index: number, diamondTotal: number, value: SuperGiftOfGroupGiftMsg) {
		MaskLockOper.Instance.doCall(false);
		// this.btn_buy.enabled = false;
		...
		this._diamondList = this._info.levelMsg as SuperGiftOfGroupGiftDiamondsMsg[];
		this._diamondList = ArrayUtils.sortOn(this._diamondList, "level", ArrayConstant.NUMERIC);
		...
		if (
			...
			(this._originDiamondCount && this._originDiamondCount === this._diamondList.length)
		) {
		...
		if (this._diamondListMode === 1) {
			this.diamondList.columnGap = this._diamondColumnGap[this._diamondList.length - 1];
		}
		...
		// 礼包名称
		this.title.text = LangManager.Instance.GetTranslation("yishi.SuperGiftOfGroupManager.gift.title", value.sort); // value.giftName;
		// 礼包原价
		...
		// 每人限购数
		this.txt_limit.text = LangManager.Instance.GetTranslation("yishi.SuperGiftOfGroupManager.gift.quota", this._quota - this._purchased, this._quota);
		// 全服销售量
		this.txt_server_buy.text = LangManager.Instance.GetTranslation("yishi.SuperGiftOfGroupManager.gift.sale", this._sale);
		...
		for (var i = 0; i < this._diamondList.length; i++) {
			...
			if (this._sale >= sellCount) {
				...
				if (i > 0 && this._sale > this._diamondList[i - 1].sellCount) {
					progress += (this._sale - this._diamondList[i - 1].sellCount) * (split / (sellCount - this._diamondList[i - 1].sellCount));
				}
			}
		}
