...
export class ShopControler extends FrameCtrlBase {
   ...
   *
   * */
  public setCurrentGoodsList(
    list: any[],
    pageNum: number = 9,
    changeCurrentPage: boolean = true,
  ) {
    ...
    this.model.totalPage = Math.ceil(
      this.model.currentGoodsList.length / this.model.currentPageNum,
    );
  ...
  private getShopShowList(): any[] {
    let startIndex: number =
      (this.model.currentPage - 1) * this.model.currentPageNum;
  ...
  public mineralShopGoodsSort(a: ShopGoodsInfo, b: ShopGoodsInfo): number {
    let sort: number = 0;
    if (
      a.NeedMinGrade <= ArmyManager.Instance.thane.grades &&
      b.NeedMinGrade <= ArmyManager.Instance.thane.grades
    ) {
  ...
  public initConsortiaShopFrame() {
    ...
    goodsList2.sort((a, b) => {
      return a.NeedConsortiaLevels - b.NeedConsortiaLevels;
    });
  ...
  public initFarmShopFrame() {
    ...
    unLockedGoodsList.sort((a, b) => {
      return a.Sort - b.Sort;
    });
    lockedGoodsList.sort((a, b) => {
      return a.Sort - b.Sort;
    });
    lockedGoodsList.sort((a, b) => {
      return a.NeedGrades - b.NeedGrades;
    });
  ...
  private farmShopSortFunc(a: ShopGoodsInfo, b: ShopGoodsInfo): number {
    let at: t_s_itemtemplateData = ConfigMgr.Instance.getTemplateByID(
      ConfigType.t_s_itemtemplate,
      a.ItemId,
    );
    let bt: t_s_itemtemplateData = ConfigMgr.Instance.getTemplateByID(
      ConfigType.t_s_itemtemplate,
      b.ItemId,
    );
    if (!at || !bt) {
  ...
  public initPVPShopFrame() {
    ...
    this.model.totalPage = Math.ceil(
      goodsList.length / this.model.currentPageNum,
    );
    this.model.currentGoodsList = goodsList;
    this.model.showGoodsList = this.getShopShowList();
  }
  ...
  private pvpShopSortFunc(a: ShopGoodsInfo, b: ShopGoodsInfo): number {
    if (
      ArmyManager.Instance.thane.honer >= a.NeedGeste &&
      ArmyManager.Instance.thane.honer >= b.NeedGeste
    ) {
      if (a.Sort > 0 && b.Sort > 0) {
        ...
        if (
          at.MasterType == GoodsType.HONER &&
          bt.MasterType == GoodsType.HONER
        ) {
  ...
  private tempCompare(
    a: t_s_startemplateData,
    b: t_s_startemplateData,
  ): number {
    if (!a || !b) {
      return 0;
    }
