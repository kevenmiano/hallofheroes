...
export default class ChatAirBubbleManager {
    ...
    public static get Instance(): ChatAirBubbleManager {
        if (!ChatAirBubbleManager._Instance) ChatAirBubbleManager._Instance = new ChatAirBubbleManager();
    ...
    private __getAllBubbleRet(pkg: PackageIn) {
        ...
        NotificationManager.Instance.dispatchEvent(CharBubbleEvent.ALL_CHAT_BUBBLES);
    }
    ...
    private __changeBubbleRet(pkg: PackageIn) {
        ...
        if (info) {
            ...
            if (op == 2) {
                this._allBubbles.set(bubbleData.bubbleId, bubbleData);
                NotificationManager.Instance.dispatchEvent(CharBubbleEvent.CHAT_BUBBLES_OPT, op, info.bubbleId);
            } else if (op == 1) {
                if (this._allBubbles) {
                    for (let i of this._allBubbles.values()) {
                        if (i.bubbleId == bubbleData.bubbleId)
                            i.isUse = 1;
                        else
                            i.isUse = 2;
                    }
                }
                NotificationManager.Instance.dispatchEvent(CharBubbleEvent.CHAT_BUBBLES_OPT, op);
            }
        }
