...
export class SplitConfirmWnd extends BaseWindow {
  ...
  private onBtnSureClick() {
    PlayerManager.Instance.moveBagToBag(
      this._currGrid.item.bagType,
      this._currGrid.item.objectId,
      this._currGrid.item.pos,
      this._currGrid.item.bagType,
      this._currGrid.item.objectId,
      this._targetGridPos,
      this._currCount,
  ...
  private onBtnCountClick(step: number) {
    ...
    this._currCount = Math.min(
      Math.max(1, this._currCount ? this._currCount : 0),
      this._currGrid.info.count,
    );
    this.txt_num.text = this._currCount.toString();
  }
