...
export default class PassBuyWnd extends BaseWindow {
    ...
    private initLanguage(){
        this.frame.getChild('title').text = LangManager.Instance.GetTranslation('pass.text01');
    ...
    OnShowWind() {
        ...
        if(cfgVal){
            ...
            this.txt_max.text = LangManager.Instance.GetTranslation('pass.buy1',cfgVal);
        }
       
        cfgVal = TempleteManager.Instance.getConfigInfoByConfigName('passcheck_grade_diamond').ConfigValue
        if(cfgVal){
        ...
        if(this.curGrade >= this._maxGrade){
            ...
            this.txt0.text = LangManager.Instance.GetTranslation('buildings.BaseBuildFrame.maxGrade');
        }
    }
    ...
    updateList(){
        ...
        for (let i = this.curGrade+1; i <= this.nextGrade; i++) {
            let passData:t_s_passcheckData = TempleteManager.Instance.getPassCheckItemByGrade(i, this.ctrlData.passRewardInfo.passIndex);
            if(!passData){//配置表找不到，默认用0
    ...
    onBuy(){
        ...
        let content: string = LangManager.Instance.GetTranslation("pass.buy2",this.grade,this.totalPrice);
    ...
    private confirmback(b: boolean, check: boolean) {
        if (b) {
            if(this.totalPrice > PlayerManager.Instance.currentPlayerModel.playerInfo.point){
