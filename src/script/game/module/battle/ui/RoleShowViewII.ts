...
export class RoleShowViewII extends Laya.Sprite implements IEnterFrame {
    ...
    private createRightView() {
        ...
        if (this._rightIsBoss) {//如果右边是BOSS
            this._bossView = BossInfoView.createInstance() as BossInfoView;
            this._bossView.setParent(this.wnd.getContentPane());
            this._bossView.roleShowInfo2.visible = this._rightIsBoss;
            this._bossView.setInfo(this._boss);
            this._bossView.setTotalHp(this.battleModel.armyInfoRight.getBossesTotalHp());
            this._bossView.setCurrentHp(this.battleModel.armyInfoRight.getBossesLeftHp());
        }
    }
    ...
    public get bossBufferContainer(): BossBufferContainer {
        if (this._bossView)
            return this._bossView.bossBufferContainer;
        return null;
    }
    ...
    private isNeedShowRightView(): boolean {
        if (this.battleModel.battleType == BattleType.CASTLE_BATTLE ||
    ...
    private addEvent() {
        ...
        this.battleModel.addEventListener(BattleEvent.REDUCE_WORLD_BOSS_BLOOD, this.onReduceWorldBossBlood, this);
        ServerDataManager.listen(S2CProtocol.U_C_SYNC_BOSS_HP, this, this.__syncBossHpHandler);
        ServerDataManager.listen(S2CProtocol.U_C_ADD_BOSS_HP, this, this.__addBossHpHandler);
        ServerDataManager.listen(S2CProtocol.U_C_SYNC_ROOM_BOSS_HP, this, this.__roomBossHPSyncHandler);
    }
    ...
    private __roomBossHPSyncHandler(pkg: PackageIn) {
        ...
        if (bossInfo != null) {
            if (this.battleModel.battleType == BattleType.ROOM_BOSS_BATTLE) {
                ...
                // 初始化||有变化
                if (this.tailaPreHp == 0 || (this.tailaPreHp != 0 && Math.abs(this.tailaPreHp - syncHp) > 0)) {
                    let deltaHp = syncHp - bossInfo.bloodA
                    Logger.battle("多人本房间共享血量boss变化", syncHp, bossInfo.bloodA, deltaHp)
    ...
    private __syncBossHpHandler(pkg: PackageIn) {
        ...
        Logger.battle("[RoleShowViewII]世界BOSS血量服务器接收==", msg.curHp, msg.currentMillis);
    ...
    public enterFrame() {
        ...
        if (this.wnd.bottomBar && this.wnd.bottomBar.selfBloodView) {
            this.wnd.bottomBar.selfBloodView.updateHeroHp(precent, leftBlood, maxBoold);
        }
        if (this._rightIsBoss) {
    ...
    public dispose() {
        ...
        if (this.battleModel) {
            this.battleModel.removeEventListener(BattleEvent.REDUCE_WORLD_BOSS_BLOOD, this.onReduceWorldBossBlood, this);
        }
        ServerDataManager.cancel(S2CProtocol.U_C_SYNC_BOSS_HP, this, this.__syncBossHpHandler);
        ServerDataManager.cancel(S2CProtocol.U_C_ADD_BOSS_HP, this, this.__addBossHpHandler);
        ServerDataManager.cancel(S2CProtocol.U_C_SYNC_ROOM_BOSS_HP, this, this.__roomBossHPSyncHandler);
    }
}