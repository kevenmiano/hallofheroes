...
export default class FightingDescribleWnd extends BaseWindow {
  ...
  public OnInitWind() {
    ...
    if (frameData && frameData["tabIndex"]) {
      ...
      if (this._type == FightingType.F_EQUIP) {
        //装备
        this.scoreTxt.text = LangManager.Instance.GetTranslation(
          "FightingDescribleWnd.scoreTxt1",
        );
        this.frame.getChild("title").text = LangManager.Instance.GetTranslation(
          "FightingDescribleWnd.title1",
        );
      } else {
        //宝石
        this.scoreTxt.text = LangManager.Instance.GetTranslation(
          "FightingDescribleWnd.scoreTxt2",
        );
        this.frame.getChild("title").text = LangManager.Instance.GetTranslation(
          "FightingDescribleWnd.title2",
        );
      }
    }
    this.scoreNameTxt.text =
  ...
  private addEvent() {
    ...
    NotificationManager.Instance.addEventListener(
      NotificationEvent.REFRESH_FIGHT,
      this.refreshFightingView,
      this,
    );
  ...
  private refreshFightingView() {
    ...
    if (this._type == 1) {
      ...
      showIndex = FightingManager.Instance.getIndexByScore(
    ...
    this.scoreValueTxt.color = this.scoreNameTxt.color =
      this.getDescTxtColor(showIndex);
