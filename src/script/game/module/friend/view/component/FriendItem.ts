...
export class FriendItem extends FUI_FriendItem {
  ...
  set info(value: ThaneInfo) {
    ...
    this.txt_fight.text = LangManager.Instance.GetTranslation(
      "yishi.view.PlayerMenu.ap",
      this._info.fightingCapacity,
    );
    this.txt_grade.text = LangManager.Instance.GetTranslation(
      "public.level2",
      this._info.friendGrade,
    );
  ...
  private btnMoreClick() {
    ...
    UIManager.Instance.ShowWind(EmWindow.OuterCityArmyTips, [
      this._info,
      point,
    ]);
  }
