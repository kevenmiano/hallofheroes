...
export default class TreasureClaimMapWnd extends BaseWindow {
  ...
  public OnInitWind() {
    ...
    this._treasureNameTxt = new FilterFrameText(
      this.txt_name.width,
      this.txt_name.height,
      this.txt_name.font,
      this.txt_name.fontSize,
    );
    this._treasureNameTxt.pos(
      this.txt_name.x + this.txt_name.width / 2,
      this.txt_name.y,
    );
    this.addChild(this._treasureNameTxt);
    this.addEvent();
    this._quickItemCost = Number(
      TempleteManager.Instance.getConfigInfoByConfigName(
        "treasure_refresh_item",
      ).ConfigValue,
    );
    this._quickPointsCost =
      this._quickItemCost * TreasureMapModel.COST_ITEM_TO_POINTS;
  ...
  private __treasureInfoUpdateHandler(e: TreasureMapEvent): void {
    ...
    let itemtemplateData: t_s_itemtemplateData =
      TempleteManager.Instance.getGoodsTemplatesByTempleteId(mapID);
    if (itemtemplateData) {
      this._refreshItemCost = itemtemplateData.Property3;
      this._refreshPointsCost =
        this._refreshItemCost * TreasureMapModel.COST_ITEM_TO_POINTS;
      this._treasureNameTxt.text = itemtemplateData.TemplateNameLang;
      this._treasureNameTxt.setFrame(
        this.model.index + 1,
        eFilterFrameText.TreasureMapQuality,
      );
      this.txt_count.text =
        this.model.joinMax - this.model.joinCount + " / " + this.model.joinMax;
    }

    for (let i = 0; i < 4; i++) {
      let itemData: t_s_itemtemplateData =
        TempleteManager.Instance.getGoodsTemplatesByTempleteId(
          this.model.templateIds[i],
        );
  ...
  private setBtnEnabled(): void {
    if (
      this.model.index >= TreasureMapModel.QUALITY_MAX ||
      this.model.joinCount >= this.model.joinMax
    ) {
  ...
  private onRenderList(index: number, item: MapItem) {
    ...
    item.txt_name.color =
      FilterFrameText.Colors[eFilterFrameText.TreasureMapQuality][index];
  ...
  private onClaim(): void {
    ...
    if (needAlert && !FreedomTeamManager.Instance.teamIsFull) {
      let content = LangManager.Instance.GetTranslation(
        "map.internals.view.treasuremap.TreasureMapGetFrame.getBtnAlertTxt",
        this._refreshPointsCost,
      );
      UIManager.Instance.ShowWind(EmWindow.TodayNotAlert, {
        content: content,
        backFunction: this.claimBackFun.bind(this),
        state: 2,
      });
  ...
  private reqRefresh(quickly: boolean, useBind: boolean): void {
    if (this.model.joinCount < this.model.joinMax) {
      ...
      var hasItem: number = GoodsManager.Instance.getBagCountByTempId(
        BagType.Player,
        TreasureMapModel.COST_TEMPLATE_ID,
      );
  ...
  private onRefresh(): void {
    ...
    // }
    let content = LangManager.Instance.GetTranslation(
      "map.internals.view.treasuremap.TreasureMapGetFrame.refreshBtnAlertTxt",
      this._refreshPointsCost,
    );
    UIManager.Instance.ShowWind(EmWindow.TodayNotAlert, {
      content: content,
      backFunction: this.onRefreshCallback0.bind(this),
      state: 1,
    });
  }
  ...
  private onOnekey(): void {
    ...
    // }
    let content = LangManager.Instance.GetTranslation(
      "map.internals.view.treasuremap.TreasureMapGetFrame.quicklyRefreshBtnAlertTxt",
      this._quickPointsCost,
    );
    UIManager.Instance.ShowWind(EmWindow.TodayNotAlert, {
      content: content,
      backFunction: this.onRefreshCallback1.bind(this),
      state: 1,
    });
  }
