...
export default class LoadingUIMgr extends Laya.Sprite {
  ...
  initView() {
    ...
    if (this.loadComponent) {
      ...
      this.txt_progress =
        this.loadComponent.getChild("txt_progress").asTextField;
      this.txt_netSpeed =
        this.loadComponent.getChild("txt_netSpeed").asTextField;
      this.txt_netSpeed1 =
        this.loadComponent.getChild("txt_netSpeed1").asTextField;
      ...
      this.loadComponent.x =
        (Resolution.gameWidth - this.loadComponent.width) * 0.5;
      this.loadComponent.y =
        (Resolution.gameHeight - this.loadComponent.height) * 0.5;
  ...
  private get checkBtnCloseVisible() {
    return false;
    return (
      LoginManager.Instance.hasLogin &&
      this.hideCloseBtnWnds.indexOf(this.moduleName) == -1
    );
  }
  ...
  private update(value: number) {
    if (!this.isShowing || !this.loadComponent) return;
  ...
  private smoothProgress() {
    ...
    Laya.timer.once(mockTime * 1000, this, () => {
      ...
      if (this._smoothUpdateNum < 100) {
        ...
        this.curLoadCount++;
        if (this.curLoadCount < this.maxCount) this.smoothProgress();
      }
    });
  }
