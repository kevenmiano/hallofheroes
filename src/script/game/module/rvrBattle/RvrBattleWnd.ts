...
export default class RvrBattleWnd extends BaseWindow {
    ...
    private initData() {
        ...
        let total: number =  2;
        let curCount: number = PlayerManager.Instance.currentPlayerModel.playerInfo.warFieldCount;
        ...
        if (pvpWarFightDic[nearId]) {
            ...
            this.firstOpenTimeTxt.text = pvpWarFightDic[nearId].OpenTime + "-" + pvpWarFightDic[nearId].StopTime;
        }
        if (pvpWarFightDic[nearId + 1]) {
            this._dataList.push(pvpWarFightDic[nearId + 1]);
            this.secondOpenTimeTxt.text = pvpWarFightDic[nearId + 1].OpenTime + "-" + pvpWarFightDic[nearId + 1].StopTime;
        }
    ...
    private initEvent() {
        ...
        if (this.playerModel) this.playerModel.addEventListener(NotificationEvent.UPDATE_SYSTEM_TIME, this.updateSystemTime, this);
    }
    ...
    startHandler() {
        if (this._currentData) {
            let curCount: number = PlayerManager.Instance.currentPlayerModel.playerInfo.warFieldCount;
            if (curCount <= 0) {
                SingWarSocketSendManager.sendRequestPvpRemainNumber(this._currentData.CampaignId);
                return;
            }
    ...
    quickUseExpBtnClick() {
        ...
        if (pos == -1) {
            let content: string = LangManager.Instance.GetTranslation("bag.datas.doubleHornorNotEnoughTip");
            SimpleAlertHelper.Instance.Show(SimpleAlertHelper.SIMPLE_ALERT, null, null, content, null, null, (b) => {
                if (!b) return;
                let info: ShopGoodsInfo = TempleteManager.Instance.getShopTempInfoByItemId(ItemID.DOUBLE_HORNOR_PROP);
                if (!info) {
                    let str: string = LangManager.Instance.GetTranslation("bag.datas.notHaveItem");
            ...
            let content: string = LangManager.Instance.GetTranslation("bag.datas.doubleHornorUseTip");
            let num: number = GoodsManager.Instance.getGoodsNumByTempId(ItemID.DOUBLE_HORNOR_PROP)
            ...
            UIManager.Instance.ShowWind(EmWindow.UseGoodsAlert, {
                content: content, goodsId: ItemID.DOUBLE_HORNOR_PROP, goodsCount: goodsCount, callback: () => {
    ...
    private findNearlyCampaignId(): number {
        ...
        for (; startId < endId; startId++) { //前提 id小的开放早
            let temp: t_s_campaignData = dict[startId];
            if (temp) {
                let arr: Array<any> = temp.StopTime.split(":");
                if (now.getHours() < parseInt(arr[0]) ||
                    (now.getHours() == parseInt(arr[0]) && now.getMinutes() <= parseInt(arr[1]))) {
                    return startId;
                }
            }
        }
    ...
    private showExitTime(nearId: number) {
        ...
        if (temp) {
            ...
            let nowDate: Date = new Date(PlayerManager.Instance.currentPlayerModel.sysCurtime);
            ...
            Logger.xjy("[RvrBattleWnd]showExitTime nowHour:" + nowHour + ",nowMin:" + nowMin + ",nowSec:" + nowSec + ",stopHour:" + stopHour + ",stopMin:" + stopMin)
            if (deltaSec > 0) {
                this.cExitTime.selectedIndex = 1;
                this._exitTimer = new TimerTicker(1000, deltaSec,
                    () => {
