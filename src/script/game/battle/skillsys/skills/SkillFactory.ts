...
export class SkillFactory {
  ...
  private static createSkillBySkillId(skillId: number): BaseSkill {
    let skillTemplate: t_s_skilltemplateData =
   ...
   *
   */
  private static createSkillBySkillTemp(
    skillTemplate: t_s_skilltemplateData,
  ): BaseSkill {
    let skill: BaseSkill;
    ...
    Logger.battle(
      "[SkillFactory]createSkillBySkillTemp",
      actionId,
      actionTemplate,
    );
    if (!actionTemplate) {
    ...
    if (!skill) {
      switch (actionId) {
        ...
        case SkillType.FAILED:
          ...
          break;
        default:
          skill = SkillFactory.createSkillByPresentType(
            actionTemplate.presentType,
          );
          break;
      }
    }
   ...
   *
   */
  private static createSpecialSkill(
    skillTemplate: t_s_skilltemplateData,
  ): BaseSkill {
