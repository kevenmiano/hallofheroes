 ...
 * 外城pk, 追击玩家
 */
export default class OuterCityWarFightMediator
  implements IMediator, IEnterFrame
{
  ...
  public enterFrame() {
    ...
    if (this._targetInfo && armyView) {
      ...
      if (leng < 10000 && this._targetInfo.baseHero.isOnline) {
        //正常
        this.unLockWarFight();
        this.sendAttack();
   ...
   */
  private sendAttack() {
    OuterCitySocketOutManager.sendAttackPerson(
      this._targetInfo.baseHero.userId,
    );
  }
