...
/**幸运兑换 */
export default class LuckyExchangeView extends FUI_LuckyExchangeView implements FunnyContent {
    ...
    initView() {
        this.activityTimeDescTxt.text = LangManager.Instance.GetTranslation("funny.FunnyRightView.active.remainTime2");
        this.countDescTxt.text = LangManager.Instance.GetTranslation("MazeShopWnd.HasNumTxt");
        this.rareTxt.text = LangManager.Instance.GetTranslation("LuckyExchangeView.rareTxt");
        this.exchangeTxt.text = LangManager.Instance.GetTranslation("allmainexchange.str3");
        this.luckyTxt.text = LangManager.Instance.GetTranslation("LuckyExchangeView.luckyTxt");
        this.luckyDescTxt.text = LangManager.Instance.GetTranslation("LuckyExchangeView.luckyDescTxt");
        this._indexArr = [];
        if (this._infoData) {
            this._currentLuckValue = LuckyExchangeManager.Instance.getLuckyValueById(this._infoData.id);
            var goodInfo: GoodsInfo = new GoodsInfo();
            goodInfo.templateId = this._infoData.consumeItemId;
            this.baseItem.info = goodInfo;
            if (LuckyExchangeManager.Instance.getGoodsByRare(this._infoData).length > 0) {
                let list: Array<LuckExchangeItemTempMsg> = LuckyExchangeManager.Instance.getGoodsByRare(this._infoData);
    ...
    private initEvent() {
        this.rareInfoList.itemRenderer = Laya.Handler.create(this, this.renderRareInfoList, null, false);
        this.exchangeInfoList.itemRenderer = Laya.Handler.create(this, this.renderExchangeInfoList, null, false);
        this.exchangeBtn.onClick(this, this.exchangeBtnHandler);
        this.exchangeValueTxt.on(Laya.Event.INPUT, this, this.onNumTxtChange);
        GoodsManager.Instance.addEventListener(BagEvent.UPDATE_BAG, this.bagItemDeleteHandler, this);
        GoodsManager.Instance.addEventListener(BagEvent.DELETE_BAG, this.bagItemDeleteHandler, this);
        NotificationManager.Instance.addEventListener(NotificationEvent.LUCKY_INFO_CHANGE, this.refreshHandler, this);
    }
    ...
    private removeEvent() {
        ...
        GoodsManager.Instance.removeEventListener(BagEvent.UPDATE_BAG, this.bagItemDeleteHandler, this);
        GoodsManager.Instance.removeEventListener(BagEvent.DELETE_BAG, this.bagItemDeleteHandler, this);
        NotificationManager.Instance.removeEventListener(NotificationEvent.LUCKY_INFO_CHANGE, this.refreshHandler, this);
    }

    private refreshHandler() {
        this._currentLuckValue = LuckyExchangeManager.Instance.getLuckyValueById(this._infoData.id);
        this.luckyValueTxt.text = this._currentLuckValue + "/" + this._infoData.luckValue;
        this.progress.value = Math.floor(100 * this._currentLuckValue / this._infoData.luckValue);
    }
    ...
    private setRemainTime(data: LuckExchangeTempMsg) {
        var startTime: Date = DateFormatter.parse(data.startTime, "YYYY-MM-DD hh:mm:ss");
        var endTime: Date = DateFormatter.parse(data.endTime, "YYYY-MM-DD hh:mm:ss");
        if (startTime.getTime() > this.playerModel.nowDate) {
            this.activityTimeValueTxt.text = LangManager.Instance.GetTranslation("feedback.FeedBackItem.outDate");
        }
        this._remainTime = endTime.getTime() / 1000 - this.playerModel.sysCurTimeBySecond;
    ...
    private setActivityTimeValue() {
        if (this._remainTime >= 60) {
            this.activityTimeValueTxt.text = DateFormatter.getFullTimeString(this._remainTime);
        } else if (this._remainTime > 0) {
            this.activityTimeValueTxt.text = DateFormatter.getFullDateString(this._remainTime);
        } else {
            this.activityTimeValueTxt.text = LangManager.Instance.GetTranslation("feedback.FeedBackItem.outDate");
        }
    }
    ...
    private exchangeBtnHandler() {
        ...
        if (this._infoData) {
            if (parseInt(this.exchangeValueTxt.text) <= 0) {
                MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("activity.view.luckExchangeView.count"));
                return;
            }
            if (StringHelper.isNullOrEmpty(this.exchangeValueTxt.text)) {
                MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("activity.view.luckExchangeView.numTxt"));
                return;
            }
            LuckyExchangeManager.Instance.OperateLuckyExchange(this._infoData.id, parseInt(this.exchangeValueTxt.text));
        }
    }
    ...
    private bagItemDeleteHandler() {
        this._num = GoodsManager.Instance.getBagCountByTempId(BagType.Player, this._infoData.consumeItemId);
    ...
    private getInfoByIndex(index: number): Array<LuckExchangeItemTempMsg> {
        ...
        let arr: Array<LuckExchangeItemTempMsg> = LuckyExchangeManager.Instance.getGoodsByDropType(this._infoData, value);
        return arr;
    }
