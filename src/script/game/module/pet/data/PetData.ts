...
import t_s_upgradetemplate, { t_s_upgradetemplateData } from "../../../config/t_s_upgradetemplate";
...
export class PetData extends GameEventDispatcher {
	...
	/** 开英灵系统前的任务: 选择获取一个英灵 */
	public static FIRST_SELECT_PETLIST: number[] = [101101, 102101, 103101, 104101];
	/** */
	...
	/** 火系|水系|电系|风系|暗系|光系 */
	public static PET_TYPE_LIST: Array<number> = [101, 102, 103, 104, 105, 106, 107];
	...
	public set templateId(value: number) {
		...
		this.template = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_pettemplate, value.toString());
		if (this.template) {
	...
	public get hp(): number {
		let result: number = Math.floor((6 * this.stamina * this.coeStamina) / 1000);
		if (this.template) {
	...
	public static QualityDesc = [
		LangManager.Instance.GetTranslation("yishi.utils.GoodsHelp.getGoodQualityName01"),
		LangManager.Instance.GetTranslation("yishi.utils.GoodsHelp.getGoodQualityName02"),
		LangManager.Instance.GetTranslation("yishi.utils.GoodsHelp.getGoodQualityName03"),
		LangManager.Instance.GetTranslation("yishi.utils.GoodsHelp.getGoodQualityName04"),
		LangManager.Instance.GetTranslation("yishi.utils.GoodsHelp.getGoodQualityName05"),
		LangManager.Instance.GetTranslation("yishi.utils.GoodsHelp.getGoodQualityName06"),
	];
	...
	public static qualityStarIcons = ["", "Icon_Star_W", "Icon_Star_G", "Icon_Star_B", "Icon_Star_P", "Icon_Star_O"];
	...
	public static calculatefightPower(p: PetData): number {
		...
		power += p.magicAttack + p.magicDefense + p.physicalAttack + p.physicalDefense + p.hp / 5;
		return power;
	}
	...
	public get coeStrengthAutoAdd(): number {
		...
		let initTemp: t_s_pettemplateData = TempleteManager.Instance.getQualityGradeTemplate(this.template, this.initQuality);
		if (initTemp) {
	...
	public get magicDefense(): number {
		return Math.floor((this.armor * 0.9 * this.coeArmor + this.intellect * 0.9 * this.coeIntellect) / 1000);
	}

	/** 基础魔法防御 */
	public get magicDefenseBase(): number {
		return Math.floor((this.armorBase * 0.9 * this.coeArmor + this.intellect * 0.9 * this.coeIntellect) / 1000);
	}
	...
	public get physicalDefense(): number {
		return Math.floor((this.armor * 0.9 * this.coeArmor + this.strength * 0.9 * this.coeStrength) / 1000);
	}

	/** 基础物理防御 */
	public get physicalDefenseBase(): number {
		return Math.floor((this.armorBase * 0.9 * this.coeArmor + this.strength * 0.9 * this.coeStrength) / 1000);
	}
	...
	public static get upgradeTemplates(): any[] {
		if (!PetData._upgradeTemplates) {
			PetData._upgradeTemplates = TempleteManager.Instance.getTemplatesByType(this.UPGRAD_TYPE);
			PetData._upgradeTemplates = ArrayUtils.sortOn(PetData._upgradeTemplates, "Grades", ArrayConstant.NUMERIC);
		}
	...
	public static get qualityUpgradeTemplates(): Dictionary {
		if (!PetData._qualityUpgradeTemplates) {
			...
			let arr: any[] = TempleteManager.Instance.getTemplatesByType(this.QUALITY_UPGRADE_TYPE);
			arr = ArrayUtils.sortOn(arr, "Grades", ArrayConstant.NUMERIC);
			PetData._qualityUpgradeTemplates[PetData.REFINING_STAGE_1] = arr;

			arr = TempleteManager.Instance.getTemplatesByType(this.QUALITY_UPGRADE_TYPE2);
			arr = ArrayUtils.sortOn(arr, "Grades", ArrayConstant.NUMERIC);
	...
	private updateCoeLimit() {
		...
		this.coeStrengthLimit = PetData.calculateCoeLimit(this.template, this.temQuality, "PowerCoeLimit");
		this.coeIntellectLimit = PetData.calculateCoeLimit(this.template, this.temQuality, "IntelCoeLimit");
		this.coeStaminaLimit = PetData.calculateCoeLimit(this.template, this.temQuality, "PhysiCoeLimit");
		this.coeArmorLimit = PetData.calculateCoeLimit(this.template, this.temQuality, "ArmorCoeLimit");
	}
	 ...
	 *
	 */
	public static calculateCoeLimit(temp: t_s_pettemplateData, curQuality: number, type: string): number {
		...
		let currTemp: t_s_pettemplateData = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_pettemplate, tempId.toString());
		let nextTemp: t_s_pettemplateData = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_pettemplate, (tempId + 1).toString());
	...
	public static calculateCoeLimitMin(serNum: number, temp: t_s_pettemplateData, curQuality: number, type: string): number {
		let quality: number = Math.floor((curQuality - 1) / 5 + 1);
		let level: number = Math.floor(curQuality % 5 == 0 ? 5 : curQuality % 5);
		level -= 1;
		let tempId: number = temp.TemplateId + quality - temp.Quality;
		...
		return currNum + Math.floor((nextNum - currNum) / 5) * level + (serNum - qNum);
	}

	private findQualityUpgradeTemplateInfo(grade: number): t_s_upgradetemplateData {
		if (!this.template) return null;
		let list: any[] = PetData.qualityUpgradeTemplates[this.template.Property2] as Array<any>;
		if (!list) return null;
	...
	public isFullExp(): boolean {
		if (this.thaneInfo) {
			...
			if (this.grade > thaneGrade || (this.grade == thaneGrade && this.curExp >= this.upgradeTemplte.Data)) {
				return true;
			}
		}
		return false;
	...
	public set followSkills(value: string) {
		...
		this.followSkillTemplates = TempleteManager.Instance.getSkillTemplateInfoByIds(value);
	}
	...
	public set changeSkills(value: string) {
		...
		this.changeSkillTemplates = TempleteManager.Instance.getSkillTemplateInfoByIds(value)
		let newSkillTemp = null;
		if (this._changeSkills.length > 0 && value.length > 0) {
			if (value.length - this._changeSkills.length > 0) {
				// 新增技能 暂时只支持一个
				newSkillTemp = this.changeSkillTemplates[this.changeSkillTemplates.length - 1]
			}
		}
		Logger.info("PetData.changeSkills", value, newSkillTemp, this.changeSkillTemplates);
	...
	public get petSkillOutPutTypeLanguage(): string {
		let s: string = LangManager.Instance.GetTranslation("pet.petSkillOutputTypeList");
		let att: any[] = s.split("|");
	...
	/** 读取宠物信息 */
	public static createWithMsg(petMsg: PetInfoMsg, pet: PetData = null): PetData {
		if (!pet) {
			pet = new PetData();
		}
		...
		pet.potencyArr = [pet.atkpotential,pet.matpotential,pet.defpotential,pet.mdfpotential,pet.hppotential];
		...
		pet.potencyAtrArr = [pet.atkpotentialAtr,pet.matpotentialAtr,pet.defpotentialAtr,pet.mdfpotentialAtr,pet.hppotentialAtr];
		...
		pet.artifactAtrArr = [pet.atkbeneAtr,pet.matbeneAtr,pet.defbeneAtr,pet.mdfbeneAtr,pet.hpbeneAtr];
		if (petMsg.hasOwnProperty("isActiveSecond")) pet.isActiveSecond = petMsg.isActiveSecond;
		else pet.isActiveSecond = false;

		
		Logger.info("英灵信息", petMsg, pet, petMsg.skillIndex, petMsg.isActiveSecond, petMsg.changeSkills, petMsg.fastKey)
		return pet;
	}
	/**
	 *
	...
	public static createWidthString(str: string): PetData {
		...
		petData.remainPoint =
			(petData.grade - 1) * PetData.EACH_LEVEL_POINT - (petData.staminaAdd + petData.strengthAdd + petData.intellectAdd + petData.armorAdd);
	...
	public static getUseSkillBook(tempid: number, type: number = 0): GoodsInfo {
		let find: GoodsInfo;
		let goodsInfoList: any[] = GoodsManager.Instance.getBagGoodsByTemplateId(tempid);
		if (goodsInfoList.length <= 0) {
			return null;
		}

		goodsInfoList = ArrayUtils.sortOn(goodsInfoList, ["isBinds", "pos"], ArrayConstant.DESCENDING | ArrayConstant.NUMERIC);
		if (type == 0) {
	...
	public calcResolveCornea():number{
		return this.addPowerApti +this.addIntelApti + this.addPhysiApti +this.addArmorApti +2;
	}
	public calcResolveSoulStone():number{
		let result = this.totalQualityGp
		let temps = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_upgradetemplate, 'Types'+UpgradeType.UPGRADE_TYPE_SOUL_STONE) as t_s_upgradetemplateData[];
        if (temps){
