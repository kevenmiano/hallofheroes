...
export class ShootFrameExecuter {
  ...
  private execute() {
    ...
    if (
      this._frameData &&
      this._frameData.shootData.rippleRes &&
      this._frameData.shootData.rippleRes != ""
    ) {
    ...
    this._attackData.forEach((element) => {
      //BaseRoleInfo
      let target = BattleManager.Instance.battleModel.getRoleById(
        element.roleId,
      );
      if (!target) {
        Logger.warn(
          "[ShootFrameExecuter] 受击目标不存在",
          element.roleId,
          element.roleName,
        );
        return;
      ...
      if (attackRole) {
        Logger.battle(
          `[ShootFrameExecuter]${attackRole.roleName}(${attackRole.livingId})对${target.roleName}(${target.livingId})添加射箭特效: ${this._frameData.shootData.arrowRes}`,
        );
      }
      ...
      arrow.startPoint.x = movie.x =
        this._skill.getCurrentRole().point.x + this._startPoint.x;
      arrow.startPoint.y = movie.y =
        this._skill.getCurrentRole().point.y + this._startPoint.y;

      let distance: number = arrow.startPoint.distance(
        target.point.x,
        target.point.y,
      );
      ...
      if (this._frameData.shootData.flyType == ShootFrameData.FLY_TYPE_COMMON) {
        ...
        Laya.Tween.to(
          movie,
          {
            x: target.point.x,
            y: target.point.y - 70,
            update: new Laya.Handler(this, this.onArrowUpate, [arrow]),
          },
          flyTime,
          Laya.Ease.linearIn,
          Laya.Handler.create(this, this.onHit, [arrow, target]),
        );
      } else {
        //TODO  X方向匀速  Y先匀减速后匀加速  实现抛物线
        ...
        minPoint.y =
          minPoint.y < arrow.startPoint.y ? arrow.startPoint.y : minPoint.y;
        minPoint.y -= curve;
        Laya.Tween.to(
          movie,
          { y: minPoint.y },
  ...
  private onArrowUpate(arrow: ArrowSkillEffect) {
    if (
      ...
      if (arrow.rippleCount < currentCount) {
        ...
        arrow.lastPoint = new Laya.Point(
          arrow.getDisplayObject().x,
          arrow.getDisplayObject().y,
        );
      }
    }
  }
  private getCurrentRipple(arrow: ArrowSkillEffect): number {
    ...
    let count: number = Math.floor(
      distance / this._frameData.shootData.rippleGap,
    );
  ...
  private getRipplePos(arrow: ArrowSkillEffect, index: number): Laya.Point {
    ...
    let dis: number =
      this._frameData.shootData.rippleGap * index -
      arrow.travelDistance +
      lastDistance;
    let angle: number = Math.atan2(
      pt.y - arrow.lastPoint.y,
      pt.x - arrow.lastPoint.x,
    );
  ...
  private onHit(arrow: BattleEffect, target) {
    ...
    this._attackData.forEach((element: AttackData) => {
      if (target && element.roleId == target.livingId) {
        ...
        if (attackRole) {
          Logger.battle(
            `[ShootFrameExecuter]${attackRole.roleName}(${attackRole.livingId})的射箭击中${target.roleName}(${target.livingId}), damageValue=${element.damageValue}`,
          );
        }
        ...
        let roleView: BaseRoleView =
          BattleManager.Instance.battleMap.rolesDict[element.roleId];
        if (roleView) {
