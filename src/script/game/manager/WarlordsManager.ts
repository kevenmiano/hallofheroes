...
export default class WarlordsManager {
  ...
  private addEvent() {
    ServerDataManager.listen(
      S2CProtocol.U_C_LORDS_LOAD_MAINBORAD,
      this,
      this.__getMainInfoHandler,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_LORDS_ENTER_RESULT,
      this,
      this.__getMatchInfoHandler,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_LORDS_FINAL_ORDERLIST,
      this,
      this.__getCanBetListHandler,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_REQUEST_BET_LIST,
      this,
      this.__getAwardListHandler,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_SEND_FINALRWARD_STATE,
      this,
      this.__getRewardStateHandler,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_TAKEFINALREWARD_RESULT,
      this,
      this.__getRewardResultHandler,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_LORDS_CLOSE,
      this,
      this.__getWarlordsCloseHandler,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_LORDS_LOCAL_FIANL_ORDER,
      this,
      this.__getAccessFinalListHandler,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_LORDS_FINAL_TIPS,
      this,
      this.__getFinalNoticeHandler,
    );
  }

  private __getMainInfoHandler(pkg: PackageIn) {
    //收到武斗会主面板信息处理
    var msg: LordsOrderListMsg = pkg.readBody(
      LordsOrderListMsg,
    ) as LordsOrderListMsg;
    ...
    if (msg.lordsInfo) {
      ...
      for (var i: number = 0; i < len; i++) {
        ...
        this._tempInfo = this._model.getListData(
          WarlordsModel.TOP,
          jobType,
  ...
  private __getMatchInfoHandler(pkg: PackageIn) {
    //武斗会单场竞技结束信息返回
    var msg: LordsOrderListMsg = pkg.readBody(
      LordsOrderListMsg,
    ) as LordsOrderListMsg;
    ...
    switch (this._model.process) {
      ...
      case WarlordsModel.PROCESS_FINAL:
        ...
        if (msg.lordsInfo) {
          ...
          for (var i: number = 0; i < len; i++) {
            tempMsg = msg.lordsInfo[i];
            this._tempInfo = this._tempList.hasOwnProperty(i)
              ? this._tempList[i]
              : null;
            if (!this._tempInfo) {
              this._tempInfo = new WarlordsPlayerInfo();
  ...
  private __getCanBetListHandler(pkg: PackageIn) {
    //收到武斗会可下注列表
    var msg: LordsOrderListMsg = pkg.readBody(
      LordsOrderListMsg,
    ) as LordsOrderListMsg;
  ...
  private __getAwardListHandler(pkg: PackageIn) {
    //收到武斗会获奖列表
    var msg: LordsOrderListMsg = pkg.readBody(
      LordsOrderListMsg,
    ) as LordsOrderListMsg;
  ...
  private __getAccessFinalListHandler(pkg: PackageIn) {
    //收到进入决赛玩家列表
    var msg: LordsOrderListMsg = pkg.readBody(
      LordsOrderListMsg,
    ) as LordsOrderListMsg;
    ...
    if (msg.lordsInfo) {
      ...
      for (var i: number = 0; i < len; i++) {
        ...
        this._tempInfo = this.model.getWarlordsPlayerInfoByUserIdAndServerName(
          tempMsg.userId,
          tempMsg.serverName,
        );
        if (!this._tempInfo) {
      ...
      this._model.accessFinalList = ArrayUtils.sortOn(
        this._model.accessFinalList,
        ["sort", "job"],
        ArrayConstant.CASEINSENSITIVE | ArrayConstant.NUMERIC,
      );
  ...
  private __getFinalNoticeHandler(pkg: PackageIn) {
    //收到武斗会全服公告
  ...
  private __getRewardResultHandler(pkg: PackageIn) {
    ...
    var data = {
      ...
      gloryNum: msg.param1,
      mount: msg.param6,
      mountTime: msg.param3,
      appell: msg.param5,
      hasPrivilege: msg.param7,
    };
    ...
    if (data.hasPrivilege) {
      ...
      tipData.name = LangManager.Instance.GetTranslation(
        "tasktracetip.view.SecretTreeTipView.privilege",
      );
      tipData.content = LangManager.Instance.GetTranslation(
        "tasktracetip.view.SecretTreeTipView.callMonsterContent",
      );
      ...
      tipData.btnTxt = LangManager.Instance.GetTranslation(
        "tasktracetip.view.SecretTreeTipView.callNow",
      );
  ...
  private __loadRankListCompleteHandler(content: string) {
    ...
    this._model.beginChanges();
    this._model.rankListCreateDate = DateFormatter.parse(
      rankXML.info.createDate,
      "YYYY-MM-DD hh:mm:ss",
    );
    ...
    if (rankList instanceof Array) {
      for (let key in rankList) {
        if (Object.prototype.hasOwnProperty.call(rankList, key)) {
          ...
          this._tempInfo.thaneInfo.attackProrerty.totalPhyAttack = Number(
            item.attack,
          );
          this._tempInfo.thaneInfo.attackProrerty.totalPhyDefence = Number(
            item.defenc,
          );
          this._tempInfo.thaneInfo.attackProrerty.totalMagicAttack = Number(
            item.magicAttack,
          );
          this._tempInfo.thaneInfo.attackProrerty.totalMagicDefence = Number(
            item.magicDefenc,
          );
          this._tempInfo.thaneInfo.attackProrerty.totalForceHit = Number(
            item.hit,
          );
          this._tempInfo.thaneInfo.attackProrerty.totalParry = Number(
            item.pary,
          );
          this._tempInfo.thaneInfo.attackProrerty.totalLive = Number(item.live);
          this._tempInfo.thaneInfo.attackProrerty.totalConatArmy = Number(
            item.coant,
          );
          ...
          if (
            this.playerInfo.serviceName == this._tempInfo.serverName &&
            this.playerInfo.userId == this._tempInfo.userId
          ) {
  ...
  private sortByFightingCapacity(
    a: WarlordsPlayerInfo,
    b: WarlordsPlayerInfo,
  ): number {
    if (!a || !b) return 0;
  ...
  public enterWarlordsRoom() {
    ...
    if (
      this._model.process == WarlordsModel.PROCESS_FINAL &&
      !this._model.isEnterFinal
    ) {
      MessageTipManager.Instance.show(
        LangManager.Instance.GetTranslation(
          "yishi.manager.WarlordsManager.str01",
        ),
      );
      return;
    }
    if (
      this._model.process != WarlordsModel.PROCESS_PRELIM &&
      this._model.process != WarlordsModel.PROCESS_FINAL
    ) {
      MessageTipManager.Instance.show(
        LangManager.Instance.GetTranslation(
          "yishi.manager.WarlordsManager.str02",
        ),
      );
      return;
    }
    if (
      this._model.curRound >= this._model.totalRound &&
      this._model.process != WarlordsModel.PROCESS_FINAL
    ) {
      MessageTipManager.Instance.show(
        LangManager.Instance.GetTranslation(
          "yishi.manager.WarlordsManager.str03",
        ),
      );
      return;
    }
    ...
    if (inTeam) {
      MessageTipManager.Instance.show(
        LangManager.Instance.GetTranslation(
          "yishi.manager.WarlordsManager.str04",
        ),
      );
      return;
    }
  ...
  public reqEnterWarlordsRoom() {
    ...
    this._model.reqAgainTimer = setInterval(
      this.reqEnterAgain.bind(this),
      5000,
    );
  }

  private reqEnterAgain() {
    //请求进入武斗会, 但服务器可能出现未返回信息给客户端的情况, 导致房间内信息不更新和玩家未拉入战斗, 此处作几秒没收到返回再次请求的容错
    this._model.reqAgainCount++;
    if (
      SceneManager.Instance.currentType == SceneType.WARLORDS_ROOM &&
      this._model.reqAgainCount <= 5
    ) {
