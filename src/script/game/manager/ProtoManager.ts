...
export default class ProtoManager {
  ...
  public registerNetProto(c2sCode: number, s2cCodeList: number[]) {
    ...
    switch (c2sCode) {
      case C2SProtocol.C_CHARGE_ORDER:
        checkInfo.loadingTip = LangManager.Instance.GetTranslation(
          "shop.common.recharge",
        );
        break;
    }
  ...
  public preSetup() {
    this.registerNetProto(C2SProtocol.C_CHARGE_ORDER, [
      S2CProtocol.U_C_CHARGE_ORDER,
    ]); //充值
    this.registerNetProto(C2SProtocol.L_REGISTER_ROLE, [
      S2CProtocol.U_C_PLAYER_INFO,
    ]); //注册角色
    this.registerNetProto(C2SProtocol.U_C_SHOP_BUY, [S2CProtocol.U_C_SHOP_BUY]); //商城物品购买
    this.registerNetProto(C2SProtocol.C_FASHION_SHOP_BUY, [
      S2CProtocol.U_C_FASHION_SHOP_BUY,
    ]); //商城时装购买
    this.registerNetProto(C2SProtocol.C_PLAYER_SIGN_CMD, [
      S2CProtocol.U_C_PLAYER_SIGN,
    ]); //签到
    this.registerNetProto(C2SProtocol.C_PLAYER_SIGN_CMD, [
      S2CProtocol.U_C_CHANNEL_ALERT,
    ]); //补签活动结束
    this.registerNetProto(C2SProtocol.U_C_BAG_USEITEM, [
      S2CProtocol.U_C_BAG_MOVE_UPDATE,
      S2CProtocol.U_C_REWARD_FRESH,
    ]); //赠送鲜花  使用道具等|悬赏任务悬赏令刷新
    this.registerNetProto(C2SProtocol.U_C_HERO_RUNE_OP, [
      S2CProtocol.U_C_RUNE_OP,
    ]); //符文操作
    this.registerNetProto(C2SProtocol.C_STAR_BATCH_OP, [
      S2CProtocol.U_C_START_MOVE_UPDATE,
    ]); //星运背包出售
    this.registerNetProto(C2SProtocol.U_C_CAMPAIGN_ENTER, [
      S2CProtocol.U_C_CAMPAIGN_CREATE,
    ]); //战役/英雄之门开始
    this.registerNetProto(C2SProtocol.C_REWARD_FINISH, [
      S2CProtocol.U_C_REWARD_UPDATE,
    ]); //悬赏任务立即完成
    this.registerNetProto(C2SProtocol.C_REWARD_REMOVE, [
      S2CProtocol.U_C_REWARD_UPDATE,
    ]); //悬赏任务放弃
    this.registerNetProto(C2SProtocol.C_REWARD_ADD, [
      S2CProtocol.U_C_REWARD_UPDATE,
    ]); //悬赏任务接收
    this.registerNetProto(C2SProtocol.C_REWARD_FRESH, [
      S2CProtocol.U_C_REWARD_FRESH,
    ]); //悬赏任务
    this.registerNetProto(C2SProtocol.C_HERO_SKILLPOINT_RESET, [
      S2CProtocol.U_C_HERO_SKILLPOINT_RESET,
    ]); //技能洗点
    this.registerNetProto(C2SProtocol.U_C_CONSORTIA_LEVEL, [
    ...
    this.registerNetProto(C2SProtocol.C_MARKET_ORDER_ACTION, [
      S2CProtocol.U_C_MARKET_ORDERACTION_RESP,
    ]); //市场订单操作
  }
  ...
  checkS2CCode(code: number) {
    // this.unSynchronized();
    if (!this.isS2CProto(code)) return;
    // this.onSynchronized();
