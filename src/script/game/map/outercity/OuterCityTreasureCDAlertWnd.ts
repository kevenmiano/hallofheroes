...
export default class OuterCityTreasureCDAlertWnd extends BaseWindow {
  ...
  private initData() {
    ...
    this.contentTxt.text = LangManager.Instance.GetTranslation(
      "OuterCityTreasureCDAlertWnd.contentTxt",
      this._CDCostDiamondCount,
    );
    this.descTxt.text = LangManager.Instance.GetTranslation(
      "OuterCityTreasureCDAlertWnd.descTxt",
      this._leftCDNumber,
      this._CDMaxNumber,
    );
  }
    ...
    if (hasMoney < this._CDCostDiamondCount) {
      MessageTipManager.Instance.show(
        LangManager.Instance.GetTranslation(
          "OuterCityTreasureCDAlertWnd.confirm.tips",
        ),
      );
      return;
    }
