...
export default class SmallMapMineItem extends FUI_SmallMapMineItem {
    ...
    private initEvent() {
        this.countList.itemRenderer = Laya.Handler.create(this, this.renderCountList, null, false);
    }
    ...
    private refreshView() {
        if (this._info) {
            if (this._info.tempInfo) {
                this.levelTxt.text = LangManager.Instance.GetTranslation("fish.FishFrame.levelText",this._info.tempInfo.Grade);
                this.picLoader.url = PathManager.solveMapPngBySonType(this._info.tempInfo.SonType)
