...
export class CampaignMapView extends Laya.Sprite {
  ...
  protected registerList() {
    ...
    if (
      !WorldBossHelper.checkFogMap(this._mapModel.mapId) &&
      !WorldBossHelper.checkMaze(this._mapModel.mapId) &&
      !WorldBossHelper.checkIsNoviceMap(this._mapModel.mapId) &&
      !WorldBossHelper.checkMonopoly(this._mapModel.mapId)
    ) {
      //多人本
      arr.push(TeamListMediator);
      arr.push(NodeGuideMediator);
    } else if (
      this._mapModel.campaignTemplate.Types == 0 &&
      this._mapModel.campaignTemplate.isKingTower
    ) {
      //王者之塔
      ...
      this._mapModel.campaignTemplate.Types == 0 &&
      this._mapModel.campaignTemplate.SonTypes != 0
    ) {
      arr.push(TeamListMediator);
      arr.push(TrailMapMediator);
    } else if (this._mapModel.campaignTemplate.Types == 1) {
      //世界BOSS
    ...
    } else if (this._mapModel.campaignTemplate.Types == 2) {
      //采矿
    } else if (this._mapModel.campaignTemplate.Types == 3) {
      //修行
      //TODO 目前修行神殿改版不需要
    ...
    if (WorldBossHelper.checkConsortiaBoss(this._mapModel.mapId)) {
      //公会BOSS
    ...
    if (
      WorldBossHelper.checkMaze(this._mapModel.mapId) ||
      WorldBossHelper.checkMaze2(this._mapModel.mapId)
    ) {
      arr.push(NodeGuideMediator);
    }
  ...
  protected initView() {
    ...
    if (WorldBossHelper.checkSingleBgMap(mapId)) {
      ...
      this._bgLayer = new BgLayer(
        ...
        this._mapModel.floorData,
        true,
      );
      this.size(
        this._mapModel.mapTempInfo.Width,
        this._mapModel.mapTempInfo.Height,
      );
    } else {
      ...
      if (
        vw > MapInfo.MAX_WIDTH ||
        vh > MapInfo.MAX_HEIGHT ||
        totalPixel >= maxPixel
      ) {
    ...
    AvatarInfoUILayerHandler.setHandlerView(
      ...
      AvatarInfoUIName.CampaignMapView,
    );

    this._npcLayer = new NpcLayer(this._walkLayer);

    if (!WorldBossHelper.checkFogMap(mapId) && this.campaignTempInfo) {
      this._fogLayer = new FogView(
        this.campaignTempInfo.Width,
        this.campaignTempInfo.Height,
        this._mapModel.fogMaskData,
      );
  ...
  protected playMusic() {
    ...
    if (WorldBossHelper.checkSecretFb(mapId)) {
      let secretCfg = ConfigMgr.Instance.getTemplateByID(
        ConfigType.t_s_secret,
        mapId,
      ) as t_s_secretData;
      AudioManager.Instance.playMusic(
        PathManager.solveSoundPath(secretCfg.MusicPath),
        0,
      );
    } else {
  ...
  protected setStartPoint() {
    ...
    if (self) {
      center = PointUtils.scaleTransformII(
        new Laya.Point(self.curPosX, self.curPosY),
        20,
        20,
      );
    }
    ...
    if (!rect.contains(center.x, center.y)) {
      ...
      if (node) {
        ...
        let armyView: any =
          CampaignManager.Instance.controller.getArmyView(self);
        if (armyView) {
    ...
    this._mapModel.targetPoint =
      CampaignMapCenterCheckHelper.checkOutScene(center);
  }

  public moveCenterPoint(
    p: Laya.Point,
    dealy: number = 0.6,
    ...
    if (isTween) {
      TweenLite.to(this, dealy, {
        x: p.x,
        y: p.y,
        onComplete: this.moveEndCheck.bind(this),
      });
    } else {
  ...
  private __petBossSwitchHandler(msg: string, obj: object): void {
    if (
      this._mapModel == null ||
      BaseManager.petBossMapId == CampaignManager.Instance.petBossModel.mapId
    )
      return;
    if (WorldBossHelper.checkInPetBossFloor(this._mapModel.mapId)) {
      MediatorMananger.Instance.addRegisterMediator(
        WorldBossRiverMediator,
        this,
        this._mediatorKey,
      );
      MediatorMananger.Instance.addRegisterMediator(
  ...
  protected lookAtTarget(isAdd: boolean = false) {
    ...
    let armyView: any = CampaignManager.Instance.controller.getArmyView(
      this._mapModel.selfMemberData,
    );
    if (!armyView) {
      return;
    }
    if (isAdd) {
      this.moveCenterPoint(new Laya.Point(armyView.x, armyView.y), 0.1, false);
    }
  ...
  public setPosition($x: number, $y: number) {
    ...
    this.y = $y;
    if (
      Math.abs((this._prePoint.x - this.x) >> 0) > 100 ||
      Math.abs((this._prePoint.y - this.y) >> 0) > 100
    ) {
  ...
  private fixScreen() {
    ...
    if (Resolution.scaleFixWidth) {
      Resolution.scaleFixWidth_fixBgHeight(
        this._mapModel.mapTempInfo.Width,
        this._mapModel.mapTempInfo.Height,
        this,
      );
