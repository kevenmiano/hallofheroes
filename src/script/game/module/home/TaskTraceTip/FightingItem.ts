...
export default class FightingItem extends FUI_FightingItem {
    ...
    private lookInfoBtnHandler() {
        switch (this._info.type) {
            case FightingType.F_EQUIP:
                FrameCtrlManager.Instance.open(EmWindow.FightingDescribleWnd, { tabIndex: 1 });
                break;
            case FightingType.F_GEM:
                FrameCtrlManager.Instance.open(EmWindow.FightingDescribleWnd, { tabIndex: 2 });
                break;
    ...
    set info(value: FightIngModel) {
        ...
        if (this._info) {
            ...
            this.scoreValueTxt.color = this.scoreTxt.color = this.descTxt.color = this.getDescTxtColor(this._index);
    ...
    private getTypeIconUrl(type: number): string {
        switch (type) {
            ...
            case FightingType.F_GEM://宝石
                this.nameTxt.text = LangManager.Instance.GetTranslation("FightingItem.typeNameTxt1");
                return FUIHelper.getItemURL(EmPackName.Home, "Btn_Main_Gem2");
            case FightingType.F_TALENT:
                this.nameTxt.text = LangManager.Instance.GetTranslation("FightingItem.typeNameTxt2");
                return FUIHelper.getItemURL(EmPackName.Home, "Btn_Main_Talents2");
            case FightingType.F_START:
                this.nameTxt.text = LangManager.Instance.GetTranslation("tasktracetip.view.StarTipView.text");
                return FUIHelper.getItemURL(EmPackName.Home, "Btn_Main_Astro2");
            case FightingType.F_PET:
                this.nameTxt.text = LangManager.Instance.GetTranslation("HigherGradeOpenTipView.content23");
                return FUIHelper.getItemURL(EmPackName.Home, "Btn_Main_Sylph2");
            case FightingType.F_MOUNT:
                this.nameTxt.text = LangManager.Instance.GetTranslation("HigherGradeOpenTipView.content21");
