...
export default class PetBossModel {
  ...
  constructor() {
    this.configCountInfo = TempleteManager.Instance.getConfigInfoByConfigName(
      "pet_island_defend_reward_count",
    );
    this.ragePointInfo = TempleteManager.Instance.getConfigInfoByConfigName(
      "pet_island_defend_boss_rage",
    );
    this.openTimeInfo = TempleteManager.Instance.getConfigInfoByConfigName(
      "pet_island_defend_hour",
    );
    if (this.configCountInfo)
      this.maxCountArr = this.configCountInfo.ConfigValue.split(",");
    if (this.ragePointInfo)
      this.maxRagePoint = parseInt(this.ragePointInfo.ConfigValue);
    if (this.openTimeInfo)
      this.openTimeArr = this.openTimeInfo.ConfigValue.split(",");
  }
  ...
  public getNearlyOpenTimeSecond(): number {
    ...
    if (
      currentHour >= parseInt(this.openTimeArr[this.openTimeArr.length - 1])
    ) {
