...
export default class PetEquipSuccinctWnd extends BaseWindow {
	...
	private helpBtnClick() {
		...
		let content: string = LangManager.Instance.GetTranslation("PetFrame.help10");
	...
	private btn_succincClick() {
		...
		if (GoodsManager.Instance.getGoodsNumByTempId(this._templateId) < this._templateCount) {
		...
		PetCtrl.reqPetEquipSuccinc(this._petId, this._fontGoodsInfo.bagType, this._fontGoodsInfo.pos, 0);
	}

	private btn_changeClick() {
		this["btn_succinc"].enabled = false;
		this["btn_change"].enabled = false;
		PetCtrl.reqPetEquipSuccinc(this._petId, this._fontGoodsInfo.bagType, this._fontGoodsInfo.pos, 1);
	}
	...
	OnShowWind() {
		...
		this._petId = this._fontGoodsInfo.petData ? this._fontGoodsInfo.petData.petId : this._fontGoodsInfo.objectId;
		this.updateView();
		PetCtrl.reqPetEquipSuccinc(this._petId, this._fontGoodsInfo.bagType, this._fontGoodsInfo.pos, 2);
		ServerDataManager.listen(S2CProtocol.U_C_PET_SUCCINCT, this, this.onHandleServerData);
	}
	...
	private updateView(refreshFont: boolean = true) {
		...
		this["txt_petEuipName"].color = this._fontGoodsInfo.templateInfo.profileColor;
		this["txt_petEuipName"].text = this._fontGoodsInfo.templateInfo.TemplateNameLang;
		this["txt_petEuipStrengCount"].text = "+" + this._fontGoodsInfo.strengthenGrade;
	...
	private refreshTemplate() {
		...
		let config: any = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_config, "pet_equip_refresh");
		if (config) {
			let configs = config.ConfigValue.split(",");
			this._templateCount = Number(configs[0]) + Number(configs[1]) * Math.floor(this._fontGoodsInfo.strengthenGrade / 5);
		}
	...
	OnHideWind() {
		...
		ServerDataManager.cancel(S2CProtocol.U_C_PET_SUCCINCT, this, this.onHandleServerData);
		super.OnHideWind();
	}
}
