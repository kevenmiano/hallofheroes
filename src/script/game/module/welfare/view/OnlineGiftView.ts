...
export class OnlineGiftView extends FUI_OnlineGiftView {
  ...
  private onBtnReceiveClick(e: Laya.Event) {
    ...
    if (this._leftTimes <= 0) {
      let str: string = LangManager.Instance.GetTranslation(
        "lottery.LotteryFrame.noLeftCounts",
      );
  ...
  private onLineRewardBack(pkg: PackageIn) {
    ...
    if (msg.leftTime == -1) {
      let text = LangManager.Instance.GetTranslation(
        "lottery.LotteryFrame.dayAddMax",
      );
  ...
  private updateCountDown() {
    this.txt_countDown.text = LangManager.Instance.GetTranslation(
      "welfareWnd.OnlineGift.countDown",
    );
    if (this._countDown > 0) {
