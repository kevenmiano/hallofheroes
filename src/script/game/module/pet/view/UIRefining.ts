...
export default class UIRefining extends BaseFguiCom {
  ...
  private updateView() {
    ...
    this.txtCost.color =
      this._composeTemplate.NeedGold > ResourceManager.Instance.gold.count
        ? "#ff0000"
        : "#FFECC6";
    ...
    for (let i: number = 0; i < 4; i++) {
      ...
      if (
        this._composeTemplate.hasOwnProperty("Material" + index) &&
        this._composeTemplate["Material" + index] > 0
      ) {
        item.visible = true;
        ...
        let ownNum = GoodsManager.Instance.getGoodsNumByTempId(
    ...
    for (let i: number = 0; i < this.metaList.numChildren; i++) {
      ...
      if (
        ...
        if (
          ...
          this._composeTemplate["Count" + index]
        ) {
          result = false;
          break;
        }
  ...
  public getComposeTemplate(petData: PetData): t_s_composeData {
    ...
    for (let index = 0; index < list.length; index++) {
      ...
      if (
        info.Material1 == petData.template.PetType &&
   ...
   * @return
   */
  private getComposedPetData(
    target: PetData,
    composeTemplate: t_s_composeData,
  ): PetData {
    let p: PetData = target.clone();

    //换模板
    let list: t_s_pettemplateData[] =
      TempleteManager.Instance.getPetTemplatesByType(composeTemplate.Material1);
    for (let index = 0; index < list.length; index++) {
      const temp: t_s_pettemplateData = list[index];
      if (
        composeTemplate.NewMaterial == temp.Property2 &&
        temp.Quality == p.quality
      ) {
