...
export default class HTMLWnd extends BaseWindow {
    ...
    private addEvent() {
        this.content.getChild("contentText").on(Laya.Event.LINK, this, this.onTextEventLink);
    }

    private offEvent() {
        this.content.getChild("contentText").off(Laya.Event.LINK, this, this.onTextEventLink);
    }
    ...
    private onLoadHTML() {
        let loadURL = this.getPrivicyLINK + this.textURLKey + ".html" + "?v=" + new Date().getTime();
        ResMgr.Instance.loadRes(loadURL, (htmlText: string) => {
            if (!isOversea())
                this.parseCN(htmlText);
            else
                this.parseEN(htmlText);
