...
export class ConsortiaPermissionWnd extends BaseWindow {
  ...
  private refreshView() {
    ...
    for (
      let index = 0;
      index < ConsortiaDutyLevel.dutyOpenNameList.length;
      index++
    ) {
    ...
    for (const key in this._data.consortiaDutyList) {
      if (
        Object.prototype.hasOwnProperty.call(this._data.consortiaDutyList, key)
      ) {
        ...
        if (pos >= 0) {
          tmp[0][pos + 1] = dInfo.getRightsByIndex(ConsortiaDutyInfo.USESKILL)
            ? 1
            : 0;
          tmp[1][pos + 1] = dInfo.getRightsByIndex(
            ConsortiaDutyInfo.UPDATESKILL,
          )
            ? 1
            : 0;
          tmp[2][pos + 1] = dInfo.getRightsByIndex(ConsortiaDutyInfo.PASSINVITE)
            ? 1
            : 0;
          tmp[3][pos + 1] = dInfo.getRightsByIndex(ConsortiaDutyInfo.KICKMEMBER)
            ? 1
            : 0;
          tmp[4][pos + 1] = dInfo.getRightsByIndex(ConsortiaDutyInfo.CHANGEDUTY)
            ? 1
            : 0;
          tmp[5][pos + 1] = dInfo.getRightsByIndex(ConsortiaDutyInfo.SPEAK)
            ? 1
            : 0;
          tmp[6][pos + 1] = dInfo.getRightsByIndex(ConsortiaDutyInfo.SPEAK)
            ? 1
            : 0;
          tmp[7][pos + 1] = dInfo.getRightsByIndex(ConsortiaDutyInfo.TRANSFER)
            ? 1
            : 0;
          tmp[8][pos + 1] = dInfo.getRightsByIndex(ConsortiaDutyInfo.RENAME)
            ? 1
            : 0;
          tmp[9][pos + 1] = dInfo.getRightsByIndex(ConsortiaDutyInfo.MOIDIFYBBS)
            ? 1
            : 0;
          tmp[10][pos + 1] = dInfo.getRightsByIndex(ConsortiaDutyInfo.LEVEL)
            ? 1
            : 0;
          tmp[11][pos + 1] = dInfo.getRightsByIndex(ConsortiaDutyInfo.CALL_TREE)
            ? 1
            : 0;
          tmp[12][pos + 1] = dInfo.getRightsByIndex(ConsortiaDutyInfo.CALL_BOSS)
            ? 1
            : 0;
          tmp[13][pos + 1] = dInfo.getRightsByIndex(
            ConsortiaDutyInfo.STACKHEAD_SIGNIN,
          )
            ? 1
            : 0;
          tmp[14][pos + 1] = dInfo.getRightsByIndex(
            ConsortiaDutyInfo.STACKHEAD_EDITNOTICE,
          )
            ? 1
            : 0;
          tmp[15][pos + 1] = dInfo.getRightsByIndex(
