 ...
 * @LastEditTime: 2024-04-16 18:34:24
 * @Description: 单人秘境失败视图
 */
export class SecretFailedSingleCom extends FUI_SecretFailedSingleCom {
  ...
  set info(value: SecretInfo) {
    this._info = value;
    if (value) {
      this.txtFailedTitle.text = LangManager.Instance.GetTranslation(
        "Pve.secretScene.failedTip",
      );
      this.btnGiveUp.getChild("mainTitle").text =
        LangManager.Instance.GetTranslation("public.giveup");
      this.btnAgain.getChild("mainTitle").text =
        LangManager.Instance.GetTranslation("public.tryAgain");
      this.btnAgain.getChild("viceTitle").text =
        LangManager.Instance.GetTranslation(
          "public.consumeDiamond",
          PveSecretSceneData.ReviveConsumeCnt,
        );
    } else {
    }
  }
  ...
  btnAgainClick() {
    ...
    SimpleAlertHelper.Instance.ShowSimple(
      ...
      (b: boolean, flag: boolean) => {
        if (b) {
          SecretManager.Instance.sendReviver(
            this._info.secretId,
