...
export default class OutyardRewardWnd extends BaseWindow {
    ...
    private addEvent() {
        this.personList.itemRenderer = Laya.Handler.create(this, this.renderPersonListItem, null, false);
    ...
    private renderConsortiaListItem(index: number, item: OutyardConsortiaRewardItem) {
        if (!item || item.isDisposed) return;
        item.info = this._listData[index];
    }
