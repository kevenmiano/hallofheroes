...
export class ShowAvatar extends Laya.Sprite implements Disposeable {
  ...
  private initPath() {
    this._frontCloakPath = StringHelper.isNullOrEmpty(this._data.bodyAvata)
      ? this._frontCloakPath
      : this.getPathByStr(this._data.bodyAvata + "_cloak/front/");
    this._backHairPath = StringHelper.isNullOrEmpty(this._data.hairAvata)
      ? this._backHairPath
      : this.getPathByStr(this._data.hairAvata + "/back/");
    this._backCloakPath = StringHelper.isNullOrEmpty(this._data.bodyAvata)
      ? this._backCloakPath
      : this.getPathByStr(this._data.bodyAvata + "_cloak/back/");
    this._wingPath = StringHelper.isNullOrEmpty(this._data.wingAvata)
      ? this._wingPath
      : this.getWingPath(this._data.wingAvata + "/");
    if (
      !this._data.hideFashion &&
      this._data.hairFashionAvata != "" &&
      this._data.hairFashionAvata != null
    ) {
      this._frontHairPath = StringHelper.isNullOrEmpty(this._data.hairAvata)
        ? this._frontHairPath
        : this.getWingPath(this._data.hairAvata + "/front/");
    } else {
      this._frontHairPath = StringHelper.isNullOrEmpty(
        this._data.hairEquipAvata,
      )
        ? this._frontHairPath
        : this.getPathByStr(this._data.hairEquipAvata + "/front/");
    }
    if (
      !this._data.hideFashion &&
      this._data.bodyFashionAvata != "" &&
      this._data.bodyFashionAvata != null
    ) {
      this._bodyPath = StringHelper.isNullOrEmpty(this._data.bodyAvata)
        ? this._bodyPath
        : this.getWingPath(this._data.bodyAvata + "/");
    } else {
      this._bodyPath = StringHelper.isNullOrEmpty(this._data.bodyEquipAvata)
        ? this._bodyPath
        : this.getPathByStr(this._data.bodyEquipAvata + "/");
    }
    if (
      !this._data.hideFashion &&
      this._data.armsFashionAvata != "" &&
      this._data.armsFashionAvata != null
    ) {
      this._armysPath = StringHelper.isNullOrEmpty(this._data.armsAvata)
        ? this._armysPath
        : this.getWingPath(this._data.armsAvata + "/");
    } else {
      this._armysPath = StringHelper.isNullOrEmpty(this._data.armsEquipAvata)
        ? this._armysPath
        : this.getPathByStr(this._data.armsEquipAvata + "/");
    }
    if (this._data.petTemplate) {
      this._petPath = PathManager.solveHeroAvatarPath(
        this._data.petTemplate.PetAvatar + "_follow",
        2,
      );
    } else {
      this._petPath = "";
  ...
  public set data(value: ThaneInfo) {
    ...
    if (this._frontHair.fullUrl != this._frontHairPath) {
      this._frontHair.data = new GameLoadNeedData(
        this._frontHairPath,
        undefined,
        undefined,
        undefined,
        ShowAvatarPositon.FRONT_HAIR,
      );
    }
    if (this._body.fullUrl != this._bodyPath) {
      this._body.data = new GameLoadNeedData(
        this._bodyPath,
        undefined,
        undefined,
        undefined,
        ShowAvatarPositon.BODY,
      );
    }
    if (this._arms.fullUrl != this._armysPath) {
      this._arms.data = new GameLoadNeedData(
        this._armysPath,
        undefined,
        undefined,
        undefined,
        ShowAvatarPositon.ARMS,
      );
    }
    if (this._backHair.fullUrl != this._backHairPath) {
      this._backHair.data = new GameLoadNeedData(
        this._backHairPath,
        undefined,
        undefined,
        undefined,
        ShowAvatarPositon.BACK_HAIR,
      );
    }
    ...
    if (this._pet) {
      if (this._showPet) {
        this._pet.updateParts([
          new GameLoadNeedData(
            this._petPath,
            undefined,
            HeroLoadDataFactory.LEVEL,
            undefined,
            ShowAvatarPositon.PET,
          ),
        ]);
      } else {
  ...
  private getDefaultPathByString(str: string): string {
    return (
      ...
      this._defaultDic[this._data.templateInfo.Job] +
      str +
      this._data.templateInfo.Sexs +
      "/" +
      this._data.templateInfo.Sexs +
      ".json"
    );
