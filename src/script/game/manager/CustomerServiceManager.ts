...
export default class CustomerServiceManager extends GameEventDispatcher {
  ...
  constructor() {
    ...
    ServerDataManager.listen(
      S2CProtocol.U_C_QUESTION_REPLAY,
      this,
      this.onRecvReply,
    );
  }
        ...
        try {
          ...
          if (mailData.Result.Info instanceof Array) {
            for (let key in mailData.Result.Info) {
              if (
                Object.prototype.hasOwnProperty.call(mailData.Result.Info, key)
              ) {
  ...
  selectFile(fileData: string, file: any): void {
    ...
    this.model.fileName =
      _userInfo.site + "_" + _userInfo.userId + "_" + guidStr + fileType;
  ...
  public sendHttpRequest(): void {
    ...
    if (this.model.fileName) {
      ...
      fetch(PathManager.customerSubmitImagePath, {
        method: "post",
        body: formData,
      })
        .then(function (res) {
          ...
          //上传图片
          HttpUtils.httpRequest(
            PathManager.customerSubmitImagePath,
            "",
            this.model.fileName,
            "POST",
            "text",
  ...
  public onResultCompleted(result: string): void {
    switch (result) {
      case "1":
        ...
        MessageTipManager.Instance.show(
          LangManager.Instance.GetTranslation(
            "yishi.manager.CustomerServiceManger.content01",
          ),
        );
      ...
      case "0":
        MessageTipManager.Instance.show(
          LangManager.Instance.GetTranslation(
            "yishi.manager.CustomerServiceManger.content02",
          ),
        );
        break;
      case "-2":
        MessageTipManager.Instance.show(
          LangManager.Instance.GetTranslation(
            "yishi.manager.CustomerServiceManger.content03",
          ),
        );
        break;
      case "-3":
        MessageTipManager.Instance.show(
          LangManager.Instance.GetTranslation(
            "yishi.manager.CustomerServiceManger.content04",
          ),
        );
        break;
    }
  ...
  public sendEvaluate(): void {
    ...
    this._model.sendData.appraisal_content =
  ...
  private checkBlank(): boolean {
    ...
    if (
      String(this._model.sendData.question_content).length <
      CustomerServiceManager.MIN_CHARS
    ) {
      MessageTipManager.Instance.show(
        LangManager.Instance.GetTranslation(
