...
export class SkillInfo extends GameEventDispatcher {
    ...
    public set templateId(value: number) {
        if (this._templateId != value) {
            ...
            this.minGradeTemplateInfo = TempleteManager.Instance.getMinSkillTemplateInfoBySonType(this.templateInfo.SonType);
        }
    }
    ...
    public get templateInfo(): t_s_skilltemplateData {
        return ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_skilltemplate, this.templateId.toString());
    }
    ...
    public checkUpgradeCondition(info: t_s_skilltemplateData): any[] {
        ...
        if (info.NeedPlayerGrade > this.thane.grades) {
            arr.push(LangManager.Instance.GetTranslation("buildings.BaseBuildFrame.gradeValue", info.NeedPlayerGrade));
        }
        for (var temp of preTempList) {
            if (!this.thane.existSkill(temp)) {
                let str: string = LangManager.Instance.GetTranslation("public.level.name", temp.TemplateName, temp.Grades);
                arr.push(str);
            }
        }
        return arr;
    ...
    public checkTalentUpgradeCondition(info: t_s_skilltemplateData): any[] {
        ...
        if (info.NeedPlayerGrade > this.thane.talentData.talentGrade) {
            arr.push(LangManager.Instance.GetTranslation("talent.gradeCondition", info.NeedPlayerGrade));
        }
        for (var temp of preTempList) {
            if (!this.thane.existTalent(temp)) {
                var str: string = LangManager.Instance.GetTranslation("public.level.name", temp.TemplateNameLang, temp.Grades);
                arr.push(str);
            }
        }
        return arr;
