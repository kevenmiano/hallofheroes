...
export class BattleAvatarResUtils {
  ...
  static fixResOffset(data: GameLoadNeedData): [number, number] {
    ...
    // 1:战士  2:弓手  3:法师
    if (data.urlPath.indexOf("wing_magicball/2_0/") != -1) {
      // 女弓-魔法舞会翅膀
      fixX = -50;
    } else if (data.urlPath.indexOf("wing_firephoenix/2_0/") != -1) {
      // 女弓- 浴火之凤翅膀
      fixX = -50;
    } else if (data.urlPath.indexOf("fashion_weapon_haunt/2_0/") != -1) {
      // 女弓-鬼影重重武器
      fixX = -45;
    } else if (data.urlPath.indexOf("wing_haunt/2_0/") != -1) {
      // 女弓-鬼影重重翅膀
      fixX = -37;
    } else if (data.urlPath.indexOf("wing_tombraider/2_0/") != -1) {
      // 女弓-探险者翅膀
      if (data.sPart + data.level == HeroLoadDataFactory.PART_CLOAK2) {
        fixX = -35;
      }
    } else if (data.urlPath.indexOf("wing_flower_whisper/2_0/") != -1) {
      // 女弓-时光花语之翼
      if (data.sPart + data.level == HeroLoadDataFactory.PART_CLOAK2) {
        fixX = -35;
      }
