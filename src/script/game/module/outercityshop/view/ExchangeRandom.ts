...
export default class ExchangeRandom extends FUI_ExchangeRandom {
    ...
    private initData() {
        let cfgItem = TempleteManager.Instance.getConfigInfoByConfigName("ShopFresh_DayCount");
        if (cfgItem) {
    ...
    private updatelastFreshTime() {
        ...
        this.txt_refreshTime.text = LangManager.Instance.GetTranslation("auction.view.mysteryshop.MysteryShopFrame.txt_autoRefreshTime", DateFormatter.getSevenDateString(this._currentLeftFreshTime));
    ...
    private addEvent() {
        ...
        this.outerCityShopModel.addEventListener(OuterCityShopEvent.FRESH_VIEW, this.freshViewHandler, this);
    ...
    private offEvent() {
        ...
        this.outerCityShopModel.removeEventListener(OuterCityShopEvent.FRESH_VIEW, this.freshViewHandler, this);
    ...
    private onListShopClick(item: ExchangeRandomItem, evt: Laya.Event) {
        ...
        if (targetObj instanceof FUI_OuterCityShopItem) {
            ...
            if (item.info.isBuy) {
                MessageTipManager.Instance.show(LangManager.Instance.GetTranslation('ExchangeRandomItem.hasbuy'));
                return;
            }
            // 符文石
    ...
    /**刷新积分, 神秘石数量显示 */
    private refreshView() {
        this.txt_Mystery.text = GoodsManager.Instance.getGoodsNumByTempId(ShopGoodsInfo.MYSTERY_STONE).toString();//神秘石
        this.txt_Point.text = this.outerCityShopModel.consumePoints.toString();//积分
        var num: number = Math.pow(2, this.outerCityShopModel.fresh_count + 1) * this.outerCityShopModel.refreshNeedStrategy;
        if (num >= this.outerCityShopModel.refreshNeedStrategyMax) {
            num = this.outerCityShopModel.refreshNeedStrategyMax;
        }
        let cfgValue = 10;
        let cfgItem = TempleteManager.Instance.getConfigInfoByConfigName("ShopFresh_NeedDiamond");
        if (cfgItem) {
            cfgValue = Number(cfgItem.ConfigValue);
        }
        this.outerCityShopModel.refreshNeedPoint = cfgValue;
        if (this.outerCityShopModel.refreshNeedPoint > this.playerModel.playerInfo.point + this.playerModel.playerInfo.giftToken) {
    ...
    private quickFreshHandler() {
        ...
        if (this.checkGoods) {
            var content: string = LangManager.Instance.GetTranslation("outercityshop.QuickRefreshGoodsTip");
            ...
            SimpleAlertHelper.Instance.Show(SimpleAlertHelper.SIMPLE_ALERT, { checkRickText: checkStr, checkRickText2: checkStr2, checkDefault: true }, null, content, null, null, this.checkGoodsHandlerBack.bind(this));
            return;
        }
        this.checkFresh();
    }
    ...
    private alertTodayFrame() {
        var preDate: Date = new Date(SharedManager.Instance.outercityShopRefreshCheckDate);
        ...
        if (!check || preDate.getMonth <= preDate.getMonth && preDate.getDate < now.getDate)
            outdate = true;
        if (outdate) {
            var content: string = LangManager.Instance.GetTranslation("outercityshop.QuickRefreshPointTip", this.outerCityShopModel.refreshNeedPoint);
            UIManager.Instance.ShowWind(EmWindow.TodayNotAlert, { content: content, backFunction: this.todayNotAlertFresh.bind(this), closeFunction: null, point: this.outerCityShopModel.refreshNeedPoint });
        }
        else {
    ...
    private freshGoods(useBind: boolean = true) {
        OuterCityShopManager.instance.sendAction(3, this.outerCityShopModel.payType, useBind ? 2 : 1);
        this.setIsFreeFreshAble(false);
    }
    ...
    public setIsFreeFreshAble(isAble: boolean) {
        ...
        this.btn_refresh.title = LangManager.Instance.GetTranslation('OfferRewardWnd.nowRefreshTxt') + ` (${(this.cfgFreeCount - this.outerCityShopModel.fresh_count)}) `;
    }
