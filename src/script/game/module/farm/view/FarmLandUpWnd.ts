...
export default class FarmLandUpWnd extends BaseWindow {
  ...
  private initData() {
    ...
    if (this._farmInfo) {
      ...
      if (this._currentLevel >= FarmModel.MAX_LAND_LEVEL) {
        ...
        this.descTxt0.text = LangManager.Instance.GetTranslation(
          "yishi.view.tips.LandUpTip.buff",
          FarmModel.MAX_LAND_LEVEL * this._outputUpPercent,
        );
        ...
        this.descTxt0.text = LangManager.Instance.GetTranslation(
          "yishi.view.tips.LandUpTip.buff",
          this._currentLevel * this._outputUpPercent,
        );
        this.descTxt.text = LangManager.Instance.GetTranslation(
          "yishi.view.tips.LandUpTip.buff",
          (this._currentLevel + 1) * this._outputUpPercent,
        );
        this.costGoldTxt.text = (
          ((this._currentLevel *
            (this._currentLevel + 1) *
  ...
  private initEvent() {
    this._farmInfo.addEventListener(
      FarmEvent.LAND_GRADE_CHANGE,
      this.__landUp,
      this,
    );
  }
  ...
  private removeEvent() {
    this._farmInfo.removeEventListener(
      FarmEvent.LAND_GRADE_CHANGE,
      this.__landUp,
      this,
    );
    ...
    if (this._currentLevel >= FarmModel.MAX_LAND_LEVEL) {
      MessageTipManager.Instance.show(
        LangManager.Instance.GetTranslation(
          "farm.view.FarmBuildLayer.maxLandLevelTip",
        ),
      );
      return;
    }
