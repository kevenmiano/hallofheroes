...
import { ClollectActionEvent, NotificationEvent } from "../../constant/event/NotificationEvent";
...
export default class CollectionAction extends MapBaseAction {
    ...
    constructor($target, $nodeId: number) {
        ...
        this._nodeId = $nodeId;
        this._inSpace = false;
        NotificationManager.Instance.addEventListener(ClollectActionEvent.CANCEL_CLOLLECT, this.__cancellCollect, this)
        if (CampaignManager.Instance.mapModel) {
            this._mapId = CampaignManager.Instance.mapModel.mapId;
            let node: CampaignNode = CampaignManager.Instance.mapModel.getMapNodesById(this._nodeId);
            if (node) {
                CampaignManager.Instance.mapModel.onCollectionId = node.nodeId;
                if (node.sonType == 2352 || node.sonType == 2393 || node.sonType == 2394 || node.sonType == 2395)  //木材和公会秘境中的采集物件
                {
            ...
            this._inSpace = true;
            let spaceNode: SpaceNode = SpaceManager.Instance.model.getMapNodeById(this._nodeId);
            if (spaceNode) {
                SpaceManager.Instance.model.onCollectionId = spaceNode.nodeId;
                if (spaceNode.sonType == 9005)  //天空之城采集蔷薇
                {
                    this._sound = SoundIds.COLLECTION_TIMBER;//木材
                }
            }
    ...
    private removeMc() {
        if (this._simMc && !this._simMc.isDisposed) {
            LayerMgr.Instance.removeByLayer(this._simMc.displayObject, EmLayer.GAME_BOTTOM_LAYER);
            this._simMc = null;
        }
    }
    ...
    update() {
        ...
        if (this._count == 1) {
            let collectAssets: fairygui.GComponent = FUIHelper.createFUIInstance(EmPackName.Space, "CollectionAsset");
            if (!collectAssets) {
                return;
            }
            ...
            if (movie) {
                ...
                if (!this._inSpace) {
                    node = CampaignManager.Instance.mapModel.getMapNodesById(this._nodeId);
                }
                ...
                LayerMgr.Instance.addToLayer(this._simMc.displayObject, EmLayer.GAME_BOTTOM_LAYER);
                ...
                NotificationManager.Instance.addEventListener(NotificationEvent.LOCK_PVP_WARFIGHT, this.__fightHandler, this);
            } else {
                this.actionOver();
            }
        }
    ...
    private lockNode() {
        NotificationManager.Instance.sendNotification(NotificationEvent.LOCK_NODE, this._nodeId);
    }

    private unLockNode() {
        NotificationManager.Instance.sendNotification(NotificationEvent.UN_LOCK_NODE, this._nodeId);
    }
    ...
    private sendCollection() {
        ...
        if (!this._inSpace) {
            ...
            if (!node) {
                Logger.error("CollectionAction.sendCollection 找不到该节点", this._nodeId)
                return;
            }
            let mapId: number = mapModel.mapId;
            SocketSendManager.Instance.sendSessionOverToBattle(mapId, node.nodeId, CollectionAction.LEAVE);
        } else {
    ...
    private playerOverCall() {
        ...
        NotificationManager.Instance.removeEventListener(NotificationEvent.LOCK_PVP_WARFIGHT, this.__fightHandler, this);

        this.removeMc();
        if (!this._isCancel) {
            this.sendCollection();
            if ((WorldBossHelper.checkCrystal(this._mapId) && this.checkCollectAgain) ||
                (WorldBossHelper.checkMineral(this._mapId) && this.mineralPick < 4 && this.mineralCarNotFull) || this.getFlag()
            ) {
                GameBaseQueueManager.Instance.addAction(new CollectionAction(this._target, this._nodeId), true);
            }
            this.actionOver();
        }
        else {
    ...
    private getFlag(): boolean {
        ...
        let condition: t_s_rewardcondictionData = ringtask.conditionList[0] as t_s_rewardcondictionData;
        if (condition.CondictionType != 4) //不是采集
        {
            return false;
        }
        if (WorldBossHelper.checkMineral(this._mapId)) //在紫晶矿场中
        {
            return false;
        }
        ...
        if (WorldBossHelper.checkPetLand(this._mapId)) {
            let node: CampaignNode = CampaignManager.Instance.mapModel.getMapNodesById(this._nodeId);
            if (!node || node && node.info.types == PosType.COLLECTION && node.resource != 2) {
                return false;
            }
        }
        return true;
        ...
        if (node) {
            ...
            for (const key in acceptedList) {
                if (Object.prototype.hasOwnProperty.call(acceptedList, key)) {
                    ...
                    if (taskIdList.indexOf(task.TemplateId.toString()) > -1 && !task.isCompleted) {
                        return true;
                    }
                }
            }
            ...
            for (const key in baseRewardDic) {
                if (Object.prototype.hasOwnProperty.call(baseRewardDic, key)) {
                    ...
                    if (reward && reward.rewardTemp) {
                        if (taskIdList.indexOf(reward.rewardTemp.TemplateId.toString()) > -1 && !reward.isCompleted) {
                            return true;
                        }
                    }
                }
    ...
    dispose() {
        this._isCancel = true;
        this.removeMc();
        NotificationManager.Instance.removeEventListener(ClollectActionEvent.CANCEL_CLOLLECT, this.__cancellCollect, this)
        if (CampaignManager.Instance.mapModel) {
