...
export default class SevenGoalsModel extends GameEventDispatcher {
    ...
    public sevenNotNeedProgressTypeArr: Array<number> = [5, 11, 13, 14, 19, 45, 58, 82]//七日目标不需要显示的任务进度的类别数组
    ...
    public set rewardSite(value: number) {
        ...
        NotificationManager.Instance.dispatchEvent(NotificationEvent.SEVEN_GOALS_REWARDSITE_UPDATE);
    }
    ...
    public set giftSite(value: number) {
        ...
        NotificationManager.Instance.dispatchEvent(NotificationEvent.SEVEN_GOALS_GIFTSITE_UPDATE);
    }
    ...
    public getSevenTreasureArr() {
        ...
        this._sevenTreasureArr = ArrayUtils.sortOn(this._sevenTreasureArr, ["id"], [ArrayConstant.NUMERIC]);
    }
    ...
    public getSevenGiftBagArr() {
        ...
        this._sevenGiftBagArr = ArrayUtils.sortOn(this._sevenGiftBagArr, ["day"], [ArrayConstant.NUMERIC]);
    }
     ...
     * @returns 
     */
    public getTaskArray(dayValue: number, tabValue: number): Array<SevenTaskInfo> {
    ...
    public getDayArray(): Array<SevenDayInfo> {
        let valueStr: string = TempleteManager.Instance.getConfigInfoByConfigName("seventarget_Time").ConfigValue;
    ...
    public getTabArray(): Array<any> {
        ...
        for (let i = 1; i <= 3; i++) {
            let titleName: string = LangManager.Instance.GetTranslation("SevenGoalsTab.title" + i);
    ...
    private checkhasNotCompleteTask(): boolean {
        ...
        for (let i = 0; i < len; i++) {
            item = this._sevenTask[i];
            if (item && item.day == this.sevenCurrentDay && item.taskType == 1 && item.status == 2) {
                flag = true;
                break;
            }
        }
    ...
    public checkGoalsBox(day: number, integral: number): number {
        let status: number = 0;
        if (this.rewardSite >> day & 0x01) {//已经领取
            status = 3;
        }
        else {
            if (integral <= this.starNum)//可领取
            {
                status = 1;
            }
            else //不能领取
    ...
    private hasReciveTaskComplete(): boolean {
        ...
        for (let i = 0; i < len; i++) {
            item = this._sevenTask[i];
            if (item && item.day == this.sevenCurrentDay && item.taskType == 1 && item.status == 1) {
                flag = true;
                break;
            }
        }
    ...
    public getProgress(): number {
        ...
        for (let i: number = 0; i < this.sevenTreasureArr.length; i++) {
            ...
            if (status == 3) {
                currentValue++;
            } else if (info.integral <= this.starNum)//可领取
            {
                currentValue++;
            }
        }
