...
export class SpaceArmyView extends HeroAvatarView {
  ...
  private showFateSkillEffect() {
    ...
    args.forEach((element) => {
      let grade: number = parseInt(
        element.substr(element.length - 2, 2).toString(),
      );
      if (grade > maxGrade) {
        maxGrade = grade;
  ...
  protected addEvent() {
    ...
    if (this._data) {
      ...
      this._data.addEventListener(
        ...
        this.__updateSpaceArmyStateHandler,
        this,
      );
      if (this.isSelf) {
  ...
  private resetAlpha() {
    ...
    if (SpaceManager.Instance.model)
      b = SpaceManager.Instance.model.walkableValueIsPara(
        4,
        this.x / Tiles.WIDTH,
        this.y / Tiles.HEIGHT,
      );
  ...
  public stopWalkByOut() {
    this._info.pathInfo = [];
    this.nextWalk(
      new Laya.Point(this._data.curPosX + 5, this._data.curPosY + 5),
    );
  }
  ...
  public changeLandEffect() {
    if (SpaceManager.Instance.model.isOnObstacle) {
      ...
      //隐藏其他人的时候, 不隐藏自己的守护光圈
      if (
        (!SharedManager.Instance.hideOtherPlayer || this.isSelf) &&
        this._fateSkillEffect &&
        !this._fateSkillEffect.visible
      ) {
  ...
  private __updateInfoHandler(data: any) {
    if (data) {
      ...
      let spaceArmy: SpaceArmy =
        SpaceManager.Instance.model.getBaseArmyByUserId(t.userId);
      if (spaceArmy) {
      ...
      if (mountTemplate) {
        ...
        mountAvatar =
          mountTemplate.AvatarPath == ""
            ...
            : mountTemplate.AvatarPath;
        this._isFlying = SpaceManager.Instance.model.isFlying(
          this._data.mountTemplateId,
        );
  ...
  private showPet(petInfo: ArmyPetInfo) {
    ...
    if (this._petAvatarView) {
      ...
      this._petAvatarView.showAvatar(
        petName,
        petTemQuaity,
        petAvatar,
  ...
  protected __heroPropertyHandler(evt: PlayerEvent) {
    ...
    let spacePlayer: SpaceArmy =
      SpaceManager.Instance.model.getBaseArmyByUserId(
  ...
  public mouseClickHandler(evt: Laya.Event): boolean {
    ...
    if (this._avatar) {
      if ((<HeroAvatar>this._avatar).getCurrentPixels() > 0) {
        //判断是否玩家重叠
        (this.parent as SpaceWalkLayer).checkClickPlayerNum(
          evt.stageX,
          evt.stageY,
        );
        return true;
        ...
        //找下面一层;
        let arr: any[] = ObjectUtils.getObjectsUnderPoint(
          StageReferance.stage,
          StageReferance.stage.mouseX,
          StageReferance.stage.mouseY,
        );
        ...
        if (arr[0]) {
          for (let i: number = arr.length - 2; i >= 0; i--) {
            if (
              arr[i].parent instanceof HeroAvatar &&
              arr[i].parent.parent instanceof SpaceArmyView
            ) {
              ...
              if (armyView.getCurrentPixels() > 50) {
                ...
                PlayerManager.Instance.currentPlayerModel.selectTarget =
                  armyView.data;
                PlayerManager.Instance.currentPlayerModel.reinforce =
                  armyView.data;
                return true;
              }
            }
  ...
  private __chatHandler(chatData: ChatData) {
    let isOpen: boolean = ChatManager.Instance.getSwitchState(
      SpaceManager.Instance.model.mapTempInfo.MessageBoxType,
    );
  ...
  private get honerName(): string {
    return (
      this._data.baseHero.appellInfo && this._data.baseHero.appellInfo.TitleLang
    );
  }
