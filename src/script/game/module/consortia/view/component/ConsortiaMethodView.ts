...
export default class ConsortiaMethodView extends FUI_ConsortiaMethodView {
  ...
  private initView() {
    ...
    this.desxTxt1.text = LangManager.Instance.GetTranslation(
      "ConsortiaMethodView.desxTxt1",
    );
    this.desxTxt2.text = LangManager.Instance.GetTranslation(
      "ConsortiaMethodView.desxTxt2",
    );
  }

  private initEvent() {
    this.functionList.itemRenderer = Laya.Handler.create(
      this,
      this.renderFunctionListItem,
      null,
      false,
    );
    this.methodList.itemRenderer = Laya.Handler.create(
      this,
      this.renderMethodListItem,
      null,
      false,
    );
    ...
    this._consortiaModel.secretInfo.addEventListener(
  ...
  private removeEvent() {
    ...
    this._consortiaModel.secretInfo.removeEventListener(
      ConsortiaEvent.TREE_STATE_UPDATE,
      this.initMethodData,
      this,
    );
  ...
  private updatePrayRed() {
    let item: ConsortiaMethodItem = this.functionList.getChildAt(
      1,
    ) as ConsortiaMethodItem;
    ...
    if (this._consortiaModel.consortiaInfo.levels >= 6) {
      ...
      activityBossInfo.promptTxt = LangManager.Instance.GetTranslation(
        "consortia.view.myConsortia.activity.ConsortiaActivityListView.bossPromptTxt" +
          this._consortiaModel.bossInfo.state,
      );
      if (
        this._contorller &&
        this._contorller.getRightsByIndex(ConsortiaDutyInfo.CALL_BOSS)
      ) {
  ...
  private gotoBossCampaign() {
    ...
    if (
      this._consortiaModel.bossInfo.state != 1 &&
      this._consortiaModel.bossInfo.state != 2
    ) {
      str = LangManager.Instance.GetTranslation(
        "consortia.view.myConsortia.activity.ConsortiaActivityListView.bossPromptTxt" +
          this._consortiaModel.bossInfo.state,
  ...
  private consortiaTreasureInit() {
    ...
    consortiaTreasure.btnLabel = LangManager.Instance.GetTranslation(
      "consortia.view.myConsortia.activity.ConsortiaActivityListView.bossBtnLabel03",
    );
  ...
  private goToOuterCity() {
    ...
    if (!TaskManage.Instance.IsTaskFinish(TaskManage.SETARMY_TASK)) {
      str = LangManager.Instance.GetTranslation(
        "newbie.outerCity.outerOccupyAlert",
      );
      MessageTipManager.Instance.show(str);
      return;
    }
  ...
  private isConsortiaChairman(): boolean {
    ...
    if (consortiaControler) {
      ...
      if (
        consortiaModel &&
        consortiaModel.consortiaInfo.chairmanID == this.thane.userId
      ) {
        flag = true;
      }
    }
  ...
  private onTaskBtnClick() {
    if (this.consortiaInfo.storeLevel <= 0) {
      MessageTipManager.Instance.show(
        LangManager.Instance.GetTranslation("ConsortiaMethodView.functionTips"),
      );
      return;
    }
  ...
  private onShopBtnClick() {
    ...
    FrameCtrlManager.Instance.open(
      EmWindow.ShopWnd,
      { page: 1, returnToWin: EmWindow.Consortia, returnToWinFrameData: 1 },
      null,
      EmWindow.Consortia,
    );
  }
