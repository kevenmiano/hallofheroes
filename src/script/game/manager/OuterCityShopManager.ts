...
export class OuterCityShopManager {
  ...
  public static get instance(): OuterCityShopManager {
    if (!OuterCityShopManager._instance)
      OuterCityShopManager._instance = new OuterCityShopManager();
  ...
  private addEvent() {
    ServerDataManager.listen(
      S2CProtocol.U_C_MAPSHOPINFO,
      this,
      this.__mapShopInfoHandler,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_OUTER_CITY_SHOP_PASS,
      this,
      this.__updateShopInfoHandler,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_SHOPITEM_FRESH,
      this,
      this.__refreshGoodsListHandler,
    );
  }
  /**
   * 更新外城商店信息
  ...
  private __mapShopInfoHandler(pkg: PackageIn) {
    ...
    this.model.fresh_time = DateFormatter.parse(
      msg.freshTime,
      "YYYY-MM-DD hh:mm:ss",
    );
    ...
    for (let i: number = 0; i < msg.items.length; i++) {
      ...
      if (temp) {
        //数据错误，增加容错处理。
        let _itemTemInfo =
          TempleteManager.Instance.getouterCityShopRandomByMapShopId(
            temp.mapshopitemid,
          );
  ...
  private __refreshGoodsListHandler(pkg: PackageIn) {
    ...
    this.model.lastRefreshTime = DateFormatter.parse(
      msg.freshLastdate,
      "YYYY-MM-DD hh:mm:ss",
    );
