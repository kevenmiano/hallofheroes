...
export default class GemCom extends FUI_GemCom {
  ...
  private initEvent(): void {
    ...
    GemMazeManager.Instance.addEventListener(
      GemMazeEvent.GEMMAZE_UPDATE_INFO,
  ...
  private removeEvent(): void {
    ...
    GemMazeManager.Instance.removeEventListener(
      GemMazeEvent.GEMMAZE_UPDATE_INFO,
      this.onUpdateGemInfo,
      this,
    );
  ...
  private onCellClick(gemCell: GemCell): void {
    ...
    if (this._model.gemMazeInfo.timesLeft <= 0) {
      MessageTipManager.Instance.show(
        LangManager.Instance.GetTranslation("gemMaze.GemMazeView.TipsTxt"),
      );
      return;
    }
  ...
  private setCurrentCell(gemCell: GemCell): void {
    ...
    if (this._currentGemCell == gemCell) {
      //点击已选中的宝石, 则取消选中
    ...
    if (!this.isUpon(this._currentGemCell, gemCell)) {
      //如果2个宝石不相邻, 更换选中宝石
      this._currentGemCell.cellSelected = false;
      this._currentGemCell = gemCell;
      ...
      this.img_select.setXY(gemCell.x - 10, gemCell.y - 10);
      this.img_select.visible = true;
    } //交换宝石位置, 发送协议到服务器, 取消宝石选中
    else {
      let itemCount: number =
        GoodsManager.Instance.getMazeBagList().getList().length;
      if (itemCount >= 48) {
        let str: string = LangManager.Instance.GetTranslation(
          "gemMaze.gemMazeView.BagFullTips",
        );
  ...
  private updateAddAndNew(): void {
    GemMazeManager.Instance.dispatchEvent(
      GemMazeEvent.GEMMAZE_ADD_INTERGAL,
      this.gemInfoUpdateInfoVO.addIntergal,
    );

    for (
      let i = 0;
      i < this.gemInfoUpdateInfoVO.rewardInfo.rewardIdArr.length;
      i++
    ) {
      setTimeout(
        this.createIconAnime.bind(this),
        200 * i,
        this.gemInfoUpdateInfoVO.rewardInfo.rewardIdArr[i].rewardId,
      );
    }
    GemMazeManager.Instance.removeEventListener(
      GemMazeEvent.GEMMAZE_REMOVE_COMPLETE,
  ...
  private updateAddData(): void {
    ...
    for (let i = 0; i < this._model.gemMazeInfo.gemArr.length; i++) {
      ...
      if (info && info.isRemove && info.nextIndex == -1) {
        ...
        info.nextIndex = this.gemInfoUpdateInfoVO.gemAddArr[j].index;
        info.isAdd = true;
        let gemCell: GemCell = FUIHelper.createFUIInstance(
          "GemMaze",
          "GemCell",
        );
  ...
  private createIconAnime(rewardId: number): void {
    let item: BaseItem = FUIHelper.createFUIInstance(
      EmPackName.Base,
      "BaseItem",
    );
    ...
    Laya.Tween.to(
      item,
      { x: 1033 + 280 / 2, y: 312 + 344 / 2, scaleX: 0.3, scaleY: 0.3 },
      1000,
      Laya.Ease.quintIn,
      Laya.Handler.create(this, this._moveComplete, [item]),
    );
  }
  ...
  private _moveComplete(icon: BaseItem): void {
    TweenMax.to(icon, 0.5, {
      alpha: 0,
      onComplete: this.disapearComplete,
  ...
  private isUpon(gemCellA: GemCell, gemCellB: GemCell): boolean {
    ...
    if (gemCellA.gemInfo.index == gemCellB.gemInfo.index - 5) b = true;
    if (
      gemCellA.gemInfo.index % 5 == 0 &&
      gemCellB.gemInfo.index == gemCellA.gemInfo.index - 1
    )
      b = false;
    if (
      gemCellB.gemInfo.index % 5 == 0 &&
      gemCellB.gemInfo.index == gemCellA.gemInfo.index + 1
    )
      b = false;
    return b;
  ...
  private getXByIndex(_index: number): number {
    let _x: number =
      this.beginX + (this.Gem_Width + this.offsetX) * (_index % 5);
    return _x;
  }
  ...
  private getYByIndex(_index: number): number {
    let _y: number =
      this.beginY + (this.Gem_Height + this.offsetY) * Math.floor(_index / 5);
    return _y;
  }
  ...
  private getDownCount(_index: number): number {
    ...
    while (_index + 5 < 25) {
      _index += 5;
      let gemMazeGemInfoVO: GemMazeGemInfoVO =
        this._model.gemMazeInfo.getGemMazeGemInfoVOByIndex(_index);
