...
export default class NewbieBaseConditionMediator {
	...
	public static checkInScene(sceneId: number, mapId: number = 0): boolean {
		...
		if ((curType == SceneType.getSceneTypeById(sceneId)) && curScene && curScene.isEnterOver) {
	...
	public static checkCastleBuild(sonType: number): boolean {
		...
		if (!curScene.castleMap) { return false; }

		var bInfo: BuildInfo = BuildingManager.Instance.getBuildingInfoBySonType(sonType);
		if (bInfo) {
			var bView: CastleBuildingView = curScene.castleMap.buildingsDic.get(bInfo.buildingId);
	 ...
	 */
	public static checkCampaignNodeState(nodeId: number, stateType: number): boolean {
	 ...
	 */
	public static checkCampaignNodeView(nodeId: number, checkWidth: number = -1, checkHeight: number = -1): boolean {
		nodeId = Number(nodeId);
		...
		if (mapModel) {
			...
			return (nodeView && nodeView.stage && nodeView.visible && nodeView.width >= checkWidth && nodeView.height >= checkHeight);
		}
		return false;
	}
	...
	public static checkInCastleOrSpace(): boolean {
		return (NewbieBaseConditionMediator.checkInScene(1) || NewbieBaseConditionMediator.checkInScene(10) || NewbieBaseConditionMediator.checkInScene(2));
	}
	 ...
	 */
	public static checkCampaignNodeAvatarState(nodeId: number, state: number): boolean {
		// var mapModel: CampaignMapModel = CampaignManager.Instance.mapModel;
		// if (mapModel) {
	...
	public static checkNewbieNodeFinish(nodeStr: string, symbol: string = "0") {
		...
		for (let index = 0; index < arr.length; index++) {
			...
			isReachAllCons = NewbieModule.Instance.checkNodeFinish(Number(mainNodeId))
			if (symbol == "0") { // 有一个检测不通过 则不通过
	 ...
	 * @param param3  参数3
	 */
	public static checkConditionCommon(type: number, param1: string = null, param2: string = null, param3: string = null): boolean {
		type = Number(type)
		...
		switch (type) {
			case 1:  //多人本地图选择窗口是否存在
				Logger.xjy("[NewbieBaseConditionMediator]多人本地图选择窗口 c9 1  使用 c6 702900 代替")
				return false
			case 2:  //当前任务完成提示是否存在
				// TODO Newbie jeremy.xu 需要检测完成任务窗口是否存在 
				Logger.xjy("[NewbieBaseConditionMediator]TODO 需要检测完成任务窗口是否存在")
				return false
			...
			case 3:  //任务追踪栏是否存在(p1: taskTempId:int, 如有配置则再检查某任务追踪item是否存在)
				let spaceTaskInfoWnd = (NewbieUtils.getFrame(EmWindow.SpaceTaskInfoWnd) as SpaceTaskInfoWnd)
				var barIsExist: boolean = NewbieBaseConditionMediator.checkFrame(EmWindow.SpaceTaskInfoWnd)
				var taskTempId: number = Number(param1);
				if (taskTempId > 0) {
					return barIsExist && Boolean(spaceTaskInfoWnd) && Boolean(spaceTaskInfoWnd.getTraceItemByTaskTempId(taskTempId));
				} else {
					return barIsExist;
				}
			case 4:  //士兵招募窗口是否存在
			...
			case 7:  //检查某sontype物品数量(p1: sontype:int, p2: bagType:int, p3: checkNum:number)
				return (GoodsManager.Instance.getCountBySontypeAndBagType(Number(param1), Number(param2)) >= Number(param3));
			case 8:  //是否神秘人
				return (ArmyManager.Instance.thane.templateId == JobType.NEWCOMER);
			case 9:  //检查已携带士兵数量(p1: checkNum:number)
				return (ArmyManager.Instance.army.countAllArmyPawnNmber() >= Number(param1));
			case 10:  //能否觉醒
				var battleModel: BattleModel = BattleManager.Instance.battleModel;
				return (battleModel && battleModel.selfHero && battleModel.selfHero.canMorph());
			case 11:  //主工具条是否实例化完毕
			...
			case 12:  //是否处于任何场景
				...
				return (sceneType && sceneType != "" && sceneType != SceneType.EMPTY_SCENE);
			...
			case 15:  //是否已进入地下迷宫
				var playerModel: PlayerModel = PlayerManager.Instance.currentPlayerModel;
				return (playerModel && playerModel.towerInfo && playerModel.towerInfo.towerIndex > 0);
			case 16:  //能否使用自动战斗
				btn = (BattleManager.Instance.battleUIView ? BattleManager.Instance.battleUIView.autoFightBtn : null);
				return (btn && btn.visible && btn.enabled && btn.selected);
			case 17:  //自动战斗按钮是否可见
				btn = (BattleManager.Instance.battleUIView ? BattleManager.Instance.battleUIView.autoFightBtn : null);
				return (btn && btn.visible);
			case 18:  //战斗觉醒UI是否初始化完成
				Logger.xjy("[NewbieBaseConditionMediator]battleAwakenUIInit", BattleModel.battleAwakenUIInit)
				return BattleModel.battleAwakenUIInit;
			case 21:  //战斗窗口是否实例化完毕
				Logger.xjy("[NewbieBaseConditionMediator]战斗窗口是否实例化完毕", BattleWnd.ISINIT)
				return BattleWnd.ISINIT;
			case 22: //AllocateWnd中是否已经配兵
				let ap = ArmyManager.Instance.army.getPawnByIndex(0);
				return (ap && ap.templateInfo && ap.ownPawns > 0)
			case 23: //农场背包是否打开
				Logger.xjy("[NewbieBaseConditionMediator]showingBag", FarmManager.Instance.showingBag)
			...
			case 24: //农场土地是否可种植  TODO
				...
				if (wnd.isShowing) {
					...
					if (landLayer) {
						...
						return item && item.info && item.info.curOper == FarmOperateType.PLANT;
					}
					return false;
				}
				return true;
			case 25: //农场是否浇水过
				let ctrl1 = FrameCtrlManager.Instance.getCtrl(EmWindow.Farm) as FarmCtrl;
				...
				return false;
			case 26: //是否有种子
				return GoodsManager.Instance.getGoodsNumBySonType(GoodsSonType.SONTYPE_SEED) > 0;
			case 27: //农场土地是否初始化完成
