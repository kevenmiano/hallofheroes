...
export class NpcAttackHelper {
	...
	public static getAttackPointImp(info: any, attackPoint: Laya.Point, findCnt:number = 20): Laya.Point {
		...
		if (info instanceof CampaignNode) {
			...
			if (attack > 0) {
				while (!isWalk && count < findCnt) {
					...
					var b: boolean = CampaignManager.Instance.mapModel.getWalkable(gridX, gridY);
					var b2: boolean = CampaignManager.Instance.mapModel.getWalkable(gridX + offX, gridY + offY);
					var bNotOutRange = Math.ceil(Math.sqrt(offX * offX + offY * offY)) <= attack
	 ...
	 * 
	 */
	public static getAttackPoint(info: CampaignNode, star: Laya.Point, end: Laya.Point): Laya.Point {
		...
		for (var i: number = attack; i >= 0; i) {
			...
			var b: boolean = CampaignManager.Instance.mapModel.getWalkable(parseInt((tar.x / 20).toString()), parseInt((tar.y / 20).toString()));
			if (b) {
				break;
			}
			i -= Tiles.WIDTH;
