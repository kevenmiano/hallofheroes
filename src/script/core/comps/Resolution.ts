...
import { NativeEvent } from "../../game/constant/event/NotificationEvent";
...
export default class Resolution {
  ...
  static init() {
    ...
    let deltaAspectRatio =
      1 / this.frameAspectRatio - 1 / this.designAspectRatio;
    let topOffset = 0;
  ...
  private static createStageModel() {
    if (!this.stageMask) this.stageMask = new Laya.Sprite();
  ...
  private static resizeStageModel() {
    ...
    this.stageMask.graphics.drawRect(
      0,
      0,
      this.gameWidth,
      this.gameHeight,
  ...
  static addWidget(
    target: Laya.Sprite,
    alignType: UIAlignType = UIAlignType.LEFT,
  ) {
    if (!target) return;
    let widget = null;
  ...
  static get scaleFixWidth() {
    if (
      ...
      Laya.stage.designWidth / Laya.stage.designHeight
    ) {
      return true;
    }
    return false;
  ...
  // 宽适配模式适配场景地图的高
  static scaleFixWidth_fixBgHeight(
    width: number = Laya.stage.designWidth,
    height: number = Laya.stage.designHeight,
    sprite,
  ) {
