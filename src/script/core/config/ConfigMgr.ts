...
import t_s_consortialevel, { t_s_consortialevelData } from "../../game/config/t_s_consortialevel";
...
import t_s_petartifactproperty, { t_s_petartifactpropertyData } from "../../game/config/t_s_petartifactproperty";
...
export default class ConfigMgr extends GameEventDispatcher implements IManager {
    ...
    parse(_configXml: any, _siteXml: any) {
        ...
        pathInfo.RELOAD_ALERT = _configXml.RELOAD_ALERT ? Boolean(_configXml.RELOAD_ALERT) : false;
        pathInfo.LANGUAGE_DEF = _configXml.LANGUAGE_DEF ? _configXml.LANGUAGE_DEF : [];
        pathInfo.share_android = _configXml.share_android ? _configXml.share_android : [];
        pathInfo.share_ios = _configXml.share_ios ? _configXml.share_ios : [];
        pathInfo.share_web = _configXml.share_web ? _configXml.share_web : [];
        ComponentSetting.IOS_VERSION = Boolean(_configXml.IOS_INTERROGATION) && Utils.isApp();
        if (Utils.isWxMiniGame()) {
            ComponentSetting.IOS_VERSION = Boolean(_configXml.IOS_INTERROGATION) && Laya.Browser.onIOS;
        }
        if (Utils.isApp()) {
            //@ts-ignore
        ...
        ComponentSetting.VERSION_DOWNS = _configXml.VERSION_DOWNS ? _configXml.VERSION_DOWNS : [];
        ...
        if (pathInfo.ResourcePath != undefined && StringUtils.trim(pathInfo.ResourcePath) != "")
    ...
    loadZip(cfgType: ConfigType | string, callFunc?: Function) {
        ...
        //先以二进制方式加载zip包
        ResMgr.Instance.loadRes(url, async (res) => {
            let jsZip = new JSZip();
            //获取ZIP包内容传入JSZip中解析
            ...
            for (let file in fileData.files) {
                if (file.endsWith("json")) {
                    ...
                    if (clsObj) {
                        //解析配置表
                        clsData = ObjectTranslator.toInstance(jsonData[cls] || jsonData, clsObj);
    ...
    load(cfgType: ConfigType | string, callFunc?: Function) {
        ...
        let url = "";
        if (cfgType == ConfigType.t_s_gameconfig || cfgType == ConfigType.siteConfig) {
        ...
        ResMgr.Instance.loadRes(url, (res) => {
            ...
            if (clsObj) {
                //解析配置表
                if (res && cls) {
                    if (name == ConfigType.languageLogin || name == ConfigType.language) {
                        clsData = ObjectTranslator.toInstance(res, clsObj);
                    } else {
                        clsData = ObjectTranslator.toInstance(name ? res[name] : res, clsObj);
                    }
    ...
    getSync(cfgType: ConfigType | string) {
        if (this.configMap[cfgType])
            return this.configMap[cfgType];
        return null;
    }
    ...
    parseConfigToDic(cfgType: ConfigType | string, clsData: any, list: Object[]) {
        if (!this.configMapDic[cfgType])
            this.configMapDic[cfgType] = {}
        ...
        for (let k in dataList) {
            ...
            if ((clsData instanceof t_s_actiontemplate) || (clsData instanceof t_s_action)) {
                key = element["ActionId"]
            } else if (clsData instanceof t_s_campaign) {
                key = element["CampaignId"]
            } else if ((clsData instanceof t_s_upgradetemplate) || (clsData instanceof t_s_map)
                ...
                key = element["TemplateId"] + "_" + element["RepeatStep"] + "_" + element["RewardItemID"] + "_" + element["RewardItemCount"] + "_" + element["RepeatMinLevel"] + element["RepeatMaxLevel"];
                ...
                key = element["SKillId"];
            } else if (clsData instanceof t_s_mapmine || clsData instanceof t_s_mapphysicposition) {
                key = element["ID"];
            ...
            if (key != "" && key != undefined && !Number.isNaN(key)) {
                ...
                if (clsData instanceof t_s_action) {  //对应处理 TempleteManager.loadBaseTemplate
                    this.solve_t_s_action(cfgType, key, element, complete)
                } else if (clsData instanceof t_s_campaign) {
                    this.solve_t_s_campaign(cfgType, key, element, complete)
                } else if (clsData instanceof t_s_actiontemplate) {//把 t_s_**Data  包装一遍, 添加方法处理
    ...
    solve_t_s_action(cfgType: ConfigType | string, key: string, element: t_s_actionData, complete: boolean) {
        let map = this.configMapDic[cfgType]
        // 处理方式1 默认处理方式
        map[key] = element
        ...
        this.actionTemplate2[key] = temp[key].resolveImp(element) as ActionTemplateData
    ...
    solve_t_s_campaign(cfgType: ConfigType | string, key: string, element: t_s_campaignData, complete: boolean) {
        let map = this.configMapDic[cfgType]
        // 处理方式1 默认处理方式
        map[key] = element
    ...
    solve_t_s_outcityshop(cfgType: ConfigType | string, key: string, element: t_s_outcityshopData, complete: boolean) {
        let map = this.configMapDic[cfgType]
        // 处理方式1 默认处理方式
        map[key] = element
        // 处理方式2
    ...
    solve_t_s_shop(cfgType: ConfigType | string, key: string, element: t_s_shopData, complete: boolean) {
        let map = this.configMapDic[cfgType]
        // 处理方式1 默认处理方式
        map[key] = element;
    ...
    solve_t_s_consortialevel(cfgType: ConfigType | string, key: string, element: t_s_consortialevelData, complete: boolean) {
        let map = this.configMapDic[cfgType]
        // 处理方式1 默认处理方式
        map[key] = element
    ...
    solve_t_s_runegem(cfgType: ConfigType | string, key: string, element: t_s_runegemData, complete: boolean) {
        let map = this.configMapDic[cfgType]
        // 处理方式1 默认处理方式
        map[key] = element
        ...
        this.runegemDic[element.Types + "_" + element.Grades + '_' + element.RuneGemTypes] = element;
    }
