...
export default class UIAttrCommon extends BaseFguiCom {
    ...
    private addEvent() {
        ...
        GoodsManager.Instance.addEventListener(BagEvent.DELETE_BAG, this.onPutonEquip, this);
    }

    private onPutonEquip() {
        if (this.c1.selectedIndex == 2 || this.c1.selectedIndex == 1) {//潜能
            this.txtArtifactPhysicsAttackValue1.text = this._data.artifactAtrArr[0].toString();//物攻
            this.txtArtifactMagicAttackValue1.text = this._data.artifactAtrArr[1].toString();//魔攻
            this.txtArtifactPhysicsDefenseValue1.text = this._data.artifactAtrArr[2].toString();//物防
            this.txtArtifactMagicDefenseValue1.text = this._data.artifactAtrArr[3].toString();//魔防
    ...
    private initTitleTxt() {
        this.potencyTxt.text = LangManager.Instance.GetTranslation("UIAttrCommon.potencyTxt");
        for (let j: number = 1; j <= 5; j++) {
            this["titleTxt" + j].text = LangManager.Instance.GetTranslation("petWnd.potency.itemTitle" + j);
        }

        this.artifactTxt.text = LangManager.Instance.GetTranslation("UIArtifact.titleTxt");
        for (let j: number = 1; j <= 5; j++) {
            this["titleArtifactTxt" + j].text = LangManager.Instance.GetTranslation("petWnd.potency.itemTitle" + j);
        }
    ...
    refresh() {
        if (this._data.template) {
            ...
            this.btnPetType.icon = FUIHelper.getItemURL(EmPackName.Base, "Icon_PetType" + value.template.PetType);
            this.btnPetType.title = value.petTypeLanguage;
            this.btnStage.icon = FUIHelper.getItemURL(EmPackName.Base, "Icon_PetStage" + value.template.Property2);
            this.btnStage.title = value.petStageLanguage;

            this.txtPhysicsAttackValue.text = (value.physicalAttack + value.bagAttack).toString();
            this.txtPhysicsDefenseValue.text = (value.physicalDefense + value.bagDefence).toString();
            this.txtMagicAttackValue.text = (value.magicAttack + value.bagMagicattack).toString();
            this.txtMagicDefenseValue.text = (value.magicDefense + value.bagMagicdefence).toString();
        ...
        if (this.c1.selectedIndex == 1) {//潜能
            ...
            this.txtPhysicsDefenseValue1.text = this._data.potencyAtrArr[2].toString();//物防
            ...
            let str1:string = LangManager.Instance.GetTranslation("UIAttrCommon.content1");
            FUIHelper.setTipData(
            ...
            this.descTxt1.text = LangManager.Instance.GetTranslation("UIAttrCommon.descTxt1");
            this.updateArtifactInfo();
        } else if (this.c1.selectedIndex == 2) {//神器
            this.txtArtifactPhysicsAttackValue1.text = this._data.artifactAtrArr[0].toString();//物攻
            this.txtArtifactMagicAttackValue1.text = this._data.artifactAtrArr[1].toString();//魔攻
            this.txtArtifactPhysicsDefenseValue1.text = this._data.artifactAtrArr[2].toString();//物防
            this.txtArtifactMagicDefenseValue1.text = this._data.artifactAtrArr[3].toString();//魔防
            this.txtArtifactHpValue1.text = this._data.artifactAtrArr[4].toString();//生命
            let str2:string = LangManager.Instance.GetTranslation("UIAttrCommon.content2");
            FUIHelper.setTipData(
            ...
            this.updateArtifactInfo();
            this.descTxt2.text = LangManager.Instance.GetTranslation("UIAttrCommon.descTxt2");
        }
        this.refreshBtnView()
    }
    ...
    private btnSacrificeClick() {
        ...
        if (this._data.isEquiped()) {
            MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("petEquip.xianji"));
            return;
        }
        if (this._data.isPractice) {
            MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("pet.inPractice"));
            return;
        }

        if (this._data.isEnterWar) {
            MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("PetShowView.checkEnterWarForSacrifice"));
            return;
        }

        if (this.playerInfo.petChallengeFormationOfArray.indexOf(this._data.petId + "") >= 0) {
            MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("pet.inPetChanglle"));
            return;
        }

        if (this._data.isRemote) {
            MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("pet.inRemotePet"));
            return;
        }

        if (this._data.quality < 3) {//蓝色品质以下
            if (this._data.isDefenser) {
                let state: string = LangManager.Instance.GetTranslation("PetShowView.state1");
                let content: string = LangManager.Instance.GetTranslation("PetShowView.inGuard", state);
                content += "<br/>" + this.getSacrificeTip();

                SimpleAlertHelper.Instance.Show(null, null, null, content, null, null, this.__btnSacrificeClick.bind(this));
                return;
            }
            SimpleAlertHelper.Instance.Show(null, null, null, this.getSacrificeTip(), null, null, this.__btnSacrificeClick.bind(this));
        } else {
            let content: string = this.getStarHtmlName(this._data.name, this._data.quality) + "\n" + this.getSacrificeTip()
            SimpleAlertHelper.Instance.Show(null, null, null, content, null, null, this.__btnSacrificeClick.bind(this));
        }
    }
    ...
    private getStarHtmlName(name: string, q: number): string {
        let profileColors: any[] = ["#ffffff", "#999999", "#59cd41", "#32a2f8", "#c300ff", "#ff8000", "#ce0f0f", "#ce0f0f"];
        let color: string = profileColors[q];
        if (!color) color = profileColors[0];
        return LangManager.Instance.GetTranslation("PetShowView.tempTxt", color, name);
    }
    ...
    private getSacrificeTip(): string {
        ...
        if (this._data.quality <= 1) {
            tempStr = LangManager.Instance.GetTranslation("PetShowView.sacrificeBtnTip2", "1-2");
        }
        else if (this._data.quality == 2) {
            tempStr = LangManager.Instance.GetTranslation("PetShowView.sacrificeBtnTip2", "2-3");
        }
        else {
            count = Math.pow(this._data.quality - 1, 4);
            tempStr = LangManager.Instance.GetTranslation("PetShowView.sacrificeBtnTip", count);
        }
        return tempStr;
    }
    ...
    private btnJoinWarClick() {
        ...
        if (this._data.template && thaneGrade < this._data.template.NeedGrade) {
            msg = LangManager.Instance.GetTranslation("PetShowView.needPlayerGrade", this._data.template.NeedGrade);
            MessageTipManager.Instance.show(msg);
            return;
        }
    ...
    private btnCancelPracticeClick() {
        ...
        SimpleAlertHelper.Instance.Show(null, null, null, content, null, null, (result: boolean, flag: boolean) => {
            ...
            let petLands: PetLandInfo[] = FarmManager.Instance.model.myFarm.getPetLandList();
            for (let index = 0; index < petLands.length; index++) {
                ...
                if (petLandInfo && petLandInfo.petId == this._data.petId) {
                    FarmManager.Instance.sendFarmOper(this._data.userId, FarmOperateType.PET_PRACTICE_CANCEL, petLandInfo.pos, petLandInfo.petId, 0);
                    return;
                }
            }
        });
    ...
    private btnAttrHelpClick() {
        ...
        let content: string = LangManager.Instance.GetTranslation("pet.attribute.help");
