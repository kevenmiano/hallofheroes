 ...
 * @description 累计充值
 */
export default class CumulativeRechargeView
  extends FUI_CumulativeRechargeView
  implements FunnyContent
{
  ...
  initView() {
    this.nameTitle.text = LangManager.Instance.GetTranslation(
      "feedback.FeedBackFrame.title",
    );
    this.tips.text = LangManager.Instance.GetTranslation(
      "CumulativeRechargeView.tips",
    );
    this.refreshView();
  }
  ...
  private refreshView() {
    ...
    this.rechargeCount.text = LangManager.Instance.GetTranslation(
      "CumulativeRechargeView.chargeNumber",
      this.feedBackData.userPoint,
    );
    this._currentLeftOpenTime =
      this.feedBackData.endTime -
    ...
    if (this._currentLeftOpenTime > 0) {
      ...
      this.times.text = LangManager.Instance.GetTranslation(
        "funny.FunnyRightView.active.remainTime",
        DateFormatter.getFullDateString(this._currentLeftOpenTime),
      );
