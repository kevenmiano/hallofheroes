...
export class MonopolyManager extends GameEventDispatcher {
  ...
  private initEvent() {
    ServerDataManager.listen(
      S2CProtocol.U_C_ENTER_MONOPOLY_CAMPAIGN,
      this,
      this.__enterMonopolyCampaignResHandler,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_MONOPOLY_CAMPAIGN_OVER,
      this,
      this.__monopolyCampaignOverHandler,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_ROLL_DICE,
      this,
      this.__rollDiceResHandler,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_ROLL_DICE_CONFIRM,
      this,
      this.__rollDiceConfirmHandler,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_BUY_ROLL_DICE,
      this,
      this.__buyRollDiceHandler,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_SLOT_MACHINE,
      this,
      this.__slotMachineHandler,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_MONOFIGHT_GOBACK,
      this,
      this.onFightLoss,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_BUY_MAGIC_DICE,
      this,
      this.__buyMagicRollDiceHandler,
    );
  }
  ...
  private __monopolyCampaignOverHandler(pkg: PackageIn) {
    SocketSceneBufferManager.Instance.addPkgToBuffer(
      ...
      this.monopolyCampaignOverCallBack.bind(this),
    );
  }
  ...
  private onFightLoss(pkg: PackageIn) {
    ...
    SocketSceneBufferManager.Instance.addPkgToBuffer(
      pkg,
      rtnScene,
      this.__onRollDiceResHandler.bind(this),
    );
  }
      ...
      for (let i: number = 0; i < nodeCount; i++) {
        ...
        if (nodeMsg && campaignNode) {
          ...
          if (nodeInfo.position == this._model.position) {
            ...
            CampaignManager.Instance.mapModel.currentDieNodeId =
              campaignNode.info.id;
          }
        }
      }
    }
  ...
  private monopolyCampaignOverCallBack(pkg: PackageIn): void {
    let msg: CampaignNodeInfoMsg = pkg.readBody(
      CampaignNodeInfoMsg,
    ) as CampaignNodeInfoMsg;
  ...
  private __rollDiceConfirmHandler(pkg: PackageIn) {
    SocketSceneBufferManager.Instance.addPkgToBuffer(
      ...
      this.rollDiceConfirmCallBack.bind(this),
    );
  }
  ...
  public hideBoss() {
    var nodeInfo: MonopolyNodeInfo = this._model.nodeInfos[
      this._model.position - 1
    ] as MonopolyNodeInfo;
  ...
  public sendMonopolyBuy(buyType: number = 1, useBind: boolean = false): void {
    ...
    if (buyType == 1) {
      SocketManager.Instance.send(
        C2SProtocol.C_MONOPOLY_CAMPAIGN_BUY_MAGIC_DICE,
        msg,
      );
    } else {
