...
export default class AllocateItem
  extends FUI_AllocateItem
  implements DragObject
{
  ...
  onConstruct() {
    ...
    this.allocateCom = fgui.UIPackage.createObject(
      "Allocate",
      "AllocateCom",
      AllocateCom,
    ) as AllocateCom;
  ...
  private _clickHander() {
    if (this._pawn && this._pawn.ownPawns > 0) {
      ...
      if (UIManager.Instance.isShowing(EmWindow.SoliderInfoTipWnd)) {
        //如果有弹窗
        let soliderInfoTipWnd: SoliderInfoTipWnd = <SoliderInfoTipWnd>(
          UIManager.Instance.FindWind(EmWindow.SoliderInfoTipWnd)
        );
        if (
          soliderInfoTipWnd &&
          soliderInfoTipWnd.ap.templateId == this._pawn.templateId
        ) {
          //点击是同一个则关闭
          // UIManager.Instance.HideWind(EmWindow.SoliderInfoTipWnd);
        } else {
