...
export default class WarlordsRewardItem
  extends FUI_WarlordsRewardItem
  implements ITipedDisplay
{
  ...
  public set info(info: WarlordsRewardInfo) {
    ...
    if (info.type == WarlordsModel.REWARD_TYPE_TWO) {
      //物品
      this.picLoader.url = IconFactory.getGoodsIconByTID(
        info.goodsInfo.templateInfo.TemplateId,
      );
    } else {
