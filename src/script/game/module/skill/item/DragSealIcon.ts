 ...
 * 圣印拖动图标
 */
export default class DragSealIcon extends FUI_DragSealIcon implements DragObject {
    ...
    public set skillInfo(value: SkillInfo) {
        this._skillInfo  = value;
        let skillTemplateDic = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_skilltemplate, "SonType"+value.templateInfo.SonType);
        this._maxGrade = skillTemplateDic.length;
        this.level.text =  value.grade + '/'+this._maxGrade;
        if(this._skillInfo.templateInfo.Icons)
            this.iconloader.url = IconFactory.getTecIconByIcon(this._skillInfo.templateInfo.Icons);
    }
