...
export default class AirGardenGameSuDuWnd extends BaseWindow {
    ...
    private addEvent() {
        ...
        AirGardenSudokuManager.Instance.addEventListener(AirGardenSudokuManager.RECEIVE_MESSAGE, this.updateView, this);
        this.gbBtn.onClick(this, this.onCloseBtn);
    }

    private initTranslate() {
        this._targetScoreTxt.text = LangManager.Instance.GetTranslation("Carnival.AirGardenGameSuDu.targetScoreTxt");
        this._txtMyScore.text = LangManager.Instance.GetTranslation("Carnival.AirGardenGameSuDu.txtMyScore");
        this._txtTime.text = LangManager.Instance.GetTranslation("Carnival.AirGardenGameSuDu.txtTime");

        this._ruleTitle.text = LangManager.Instance.GetTranslation("Carnival.AirGardenGameSuDu.ruleTitle");
        this._toolTitle.text = LangManager.Instance.GetTranslation("Carnival.AirGardenGameSuDu.toolTitle");
        this._txtTimeAdd.text = LangManager.Instance.GetTranslation("Carnival.AirGardenGameSuDu.txtTimeAdd");

        this.useToolBtn.title = LangManager.Instance.GetTranslation("Carnival.AirGardenGameSuDu.useToolBtn");
        this.startBtn.title = LangManager.Instance.GetTranslation("Carnival.AirGardenGameSuDu.startBtn");
        this.ptxt.text = LangManager.Instance.GetTranslation("AirGardenGame.sudoku.title");
        this.ptxtShade.text = LangManager.Instance.GetTranslation("AirGardenGame.sudoku.title");
    }
    ...
    private initView() {
        ...
        this._txtRule.text = LangManager.Instance.GetTranslation("AirGardenGame.sudoku.rule");
    }
    ...
    private removeEvent() {
        ...
        AirGardenSudokuManager.Instance.removeEventListener(AirGardenSudokuManager.RECEIVE_MESSAGE, this.updateView, this);
    ...
    private updateView(msg: SudokuGameMsg) {
        ...
        if (msg.opType == 1 && this.startGame) {
            MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("AirGardenGame.sudoku.next"));
        }
        ...
        if (msg.opType == 1) {
            this.enableOption();
            let temp = TempleteManager.Instance.getCarnivalByType(CarnivalModel.TYPE_GAME_SHPK);
            if (temp.length)
                this.updateTargetPoints(temp[0].Target)
    ...
    private updateCd() {
        let curTime = PlayerManager.Instance.currentPlayerModel.sysCurTimeBySecond * 1000;
    ...
    private onSelectBoardCell(e) {
        ...
        if (target.$owner && target.$owner instanceof AirGardenGameSuDuItem) {
            ...
            if (!select) {
                MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("carnival.minigame.sudoku.select.tip"));
                return;
            }
            if (select.count <= 0) return;
            if (this.cellGroup.checkValue(item.row, item.col, select.value)) {
                MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("carnival.minigame.sudoku.tip"));
                return;
            }
            AirGardenSudokuManager.Instance.sendGameOption(2, item.col, item.row, select.value);
        }
    }
