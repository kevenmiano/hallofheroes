...
export default class DayGuideCatecory extends GameEventDispatcher {
	...
	public get finishList(): Array<LeedInfo> {
		...
		if (VIPManager.Instance.model.vipInfo.IsVipAndNoExpirt && VIPManager.Instance.model.vipInfo.VipGrade >= 3)//如果是vip用户,创建完成条目
		{
     ...
     * */
    public initDate() {
        var last: Date = PlayerManager.Instance.currentPlayerModel.playerInfo.signDate;//第一天签到日期
        // if (last.getDay() != 0) {
        //     if (last.getMonth() > 0) {
