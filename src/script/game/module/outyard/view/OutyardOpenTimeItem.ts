...
export default class OutyardOpenTimeItem extends FUI_OutyardOpenTimeItem {
    ...
    public set info(value: StackHeadOpenTimeMsg) {
        this._info = value;
        this.descTxt.text = LangManager.Instance.GetTranslation("outyard.OutyardOpenTimeItem.type" + this._index);
        if (this._info) {
            let zoneOffset = PlayerManager.Instance.currentPlayerModel.zoneId;
            var start: Date = Utils.formatTimeZone(Number(this._info.startTime), zoneOffset) ;
            var end: Date = Utils.formatTimeZone(Number(this._info.endTime), zoneOffset) 
            var startTime: string = this.getIntStrAtLength(start.getHours(), 2) + ":" + this.getIntStrAtLength(start.getMinutes(), 2);
            var endTime: string = this.getIntStrAtLength(end.getHours(), 2) + ":" + this.getIntStrAtLength(end.getMinutes(), 2);
