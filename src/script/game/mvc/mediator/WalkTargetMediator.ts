...
export class WalkTargetMediator implements IMediator {
  ...
  private __updateWalkTargetHandler(data: Laya.Point) {
    ...
    if (end) {
      ...
      this._walkTarget.playing = false;
      if (this._walkTarget.parent)
        this._walkTarget.parent.removeChild(this._walkTarget);
    }
  }
  private setWalkTargetDepth(end: Laya.Point) {
    ...
    if (WorldBossHelper.fogNoExits(this.mapModel.mapId)) {
      ...
      b = this.mapView.fogLayer
        ? this.mapView.fogLayer.checkFogEmpty(end.x, end.y)
        : false;
    }
    if (b) {
      this.mapView.addChild(this._walkTarget.displayObject);
    } else {
      this.mapView.mainBuidingLayer.centerLayer.addChild(
        this._walkTarget.displayObject,
      );
    }
  }
