...
export class BatchUseConfirmWnd extends BaseWindow {
  ...
  private updateCount() {
    ...
    if (this._info.isWearyItem) {
      ...
      let sugestCount = Math.floor(
        wearyTemp / this._info.templateInfo.Property2,
      );
      if (sugestCount < 1) {
        sugestCount = 1;
      }
      let ownCount = GoodsManager.Instance.getGoodsNumByTempId(
  ...
  private updateView() {
    ...
    this.txt_name.color =
      FilterFrameText.Colors[eFilterFrameText.ItemQuality][
        this._info.templateInfo.Profile - 1
      ];
    this.txt_hasNum.text =
      GoodsManager.Instance.getGoodsNumByTempId(this._info.templateId) + "";
  }
  ...
  protected onBtnSureClick() {
    ...
    if (data.templateInfo.SonType == GoodsSonType.SONTYPE_BLOOD) {
      ...
      this.hide();
    } else if (data.templateInfo.SonType == GoodsSonType.SONTYPE_MULTI_BOX) {
      //多选宝箱
      ...
      switch (data.templateInfo.Property2) {
        case -800: //荣耀水晶
          if (
            this.thane.gloryPoint <
            data.templateInfo.Property3 * this._currCount
          ) {
            //荣耀水晶不足
            str = LangManager.Instance.GetTranslation(
      ...
      if (itemBuffer) {
        ...
        if (
          data.templateInfo.Property1 == 5 &&
          data.templateInfo.Property2 > 0
        ) {
  ...
  private checkForUseBox(item: GoodsInfo, num: number) {
    ...
    let content: string = LangManager.Instance.GetTranslation(
      ...
      num * item.templateInfo.Property3,
      costName,
    );
  ...
  private checkForUsePetExpBook(item: GoodsInfo, count: number) {
    ...
    let content3: string = LangManager.Instance.GetTranslation(
  ...
  private checkForUsePetExpBookBack(result: boolean, flag: boolean, data: any) {
    ...
    if (!curPet) {
      msg = LangManager.Instance.GetTranslation(
        "checkForUsePetExpBook.content01",
      );
  ...
  private check(): boolean {
    if (
      ...
      let str: string = LangManager.Instance.GetTranslation(
        "bag.view.MulItemUseFrame.command03",
      );
      MessageTipManager.Instance.show(str);
      this.hide();
      return false;
