...
export default class OuterCityWarEnterWarSettingItem extends FUI_OuterCityWarEnterWarSettingItem {
    ...
    public refreshView() {
        if (this._info) {
            ...
            this.txtDefenceCastle.text = this.fightModel.getCastleName(this._info.enterWarCastleNodeId);
            ...
            switch (this._info.enterWar) {
                ...
                case BooleanType.TRUE:
                    if (this.fightModel.checkInCurCastleJoinBattleList(this._info.userId)) {
    ...
    private btnTickClick() {
        if (this.fightModel.isCastleFighting) {
            MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("outerCityWar.cannotSetEnterWar01"))
            FrameCtrlManager.Instance.exit(EmWindow.OuterCityWarEnterWarSettingWnd)
            return
        }
        if (this._info.enterWar == BooleanType.FALSE) {
            let isDefenceGuild = this.fightModel.checkDefenceGuild(this.fightModel.selfGuildId)
            let curEnterCnt = this.fightModel.getGuildPlayerCnt(this.fightModel.selfGuildId, false, BooleanType.TRUE)
            let limitEnterCnt = isDefenceGuild ? OuterCityWarModel.MaxDefenceCnt : OuterCityWarModel.MaxAttackCnt

            if (curEnterCnt >= limitEnterCnt) {
                this.btnTick.selected = false
                let totalGuildCnt = this.fightModel.getGuildPlayerCnt(this.fightModel.selfGuildId, false)
                let showCnt = Math.min(limitEnterCnt, totalGuildCnt)
                MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("outerCityWar.enterWarMax", showCnt))
            } else {
                if (this._info.getTotalPetCapaity() == 0) {
                    MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("outerCityWar.notHavePetTeam"))
                }
                OuterCityWarManager.Instance.sendJoinWar(this._info.userId, BooleanType.TRUE)
            }
        } else {
            OuterCityWarManager.Instance.sendJoinWar(this._info.userId, BooleanType.FALSE)
        }
