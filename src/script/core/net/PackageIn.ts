...
export class PackageIn extends ByteArray {
    ...
    public loadE(src: ByteArray, len: number, key: ByteArray) {
        ...
        for (i = 0; i < len; i++) {
            if (i > 0) {
                key._byteView_[i % 8] = (key._byteView_[i % 8] + origion._byteView_[i - 1]) ^ i;
                result._byteView_[i] = (origion._byteView_[i] - origion._byteView_[i - 1]) ^ key._byteView_[i % 8];
            }
            else {
    ...
    public readBody(message: { decode<T>(reader: (protobuf.Reader | Uint8Array)) },): any {
        ...
        let code: string = StringHelper.pad(this._code.toString(16), 4).toUpperCase();
    ...
    public readDateString(): string {
        return this.readShort() + "-" + this.readByte() + "-" + this.readByte() + " " + this.readByte() + ":" + this.readByte() + ":" + this.readByte();
    }
