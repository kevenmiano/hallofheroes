...
export class SAppellCom extends FUI_SAppellCom {
  ...
  private addEvent() {
    this.appellList.itemRenderer = Laya.Handler.create(
      this,
      this.renderListItem,
      null,
      false,
    ...
    this.appellmodel.addEventListener(
      AppellEvent.APPELL_DATA_UPDATA,
      this.__updateAppellDataHandler,
      this,
    );
    this.thane.addEventListener(
      PlayerEvent.THANE_INFO_UPDATE,
      this.__updateThaneInfoHandler,
      this,
    );
    this.thane.addEventListener(
  ...
  private removeEvent() {
    ...
    this.appellmodel.removeEventListener(
      AppellEvent.APPELL_DATA_UPDATA,
      this.__updateAppellDataHandler,
      this,
    );
  ...
  public onShowDetail(selected: boolean) {
    ...
    if (selected) {
      ...
      if (this._cellData.PerfixLang != "") {
        this.txt_name.fontSize = this._cellData.TextFontSize
          ? this._cellData.TextFontSize
          : 20;
        this.txt_name.color = AppellModel.getTextColorAB(
          this._cellData.TextColorIdx,
        );
      ...
      this._honerView = new AppellView(
        this._cellData.ImgWidth,
        this._cellData.ImgHeight,
        this._cellData.TemplateId,
      );
      ...
      list.removeChildren();
      let appellInfo = AppellManager.Instance.model.getAppellPowerInfo([
        this._cellData,
      ]) as AppellPowerInfo;
      if (appellInfo) {
        ...
        if (appellInfo.MagicDefence > 0) {
          item = list.addItem() as fgui.GComponent;
          item.getChild("txtValue").asLabel.text =
            "+" + appellInfo.MagicDefence;
  ...
  private __updateAppellDataHandler(e: AppellEvent) {
    if (this._lookAppellId == 0) {
      ...
      var appellInfo: t_s_appellData =
        TempleteManager.Instance.getAppellInfoTemplateByID(this._lookAppellId);
      if (appellInfo) {
  ...
  private refreshView(look: boolean = false) {
    ...
    if (this._lookAppellId > 0) {
      if (appellLength > 0) {
        ...
        this.onListItemClick(
          this.appellList.getChildAt(appellIndex) as SAppellCellItem,
          null,
        );
      }
    } else {
    ...
    if (this.thane.appellId != 0) {
      ...
      this._myhonerView = new AppellView(
        ArmyManager.Instance.thane.appellInfo.ImgWidth,
        ArmyManager.Instance.thane.appellInfo.ImgHeight,
        ArmyManager.Instance.thane.appellInfo.TemplateId,
      );
