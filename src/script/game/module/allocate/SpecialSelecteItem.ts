...
export default class SpecialSelectedItem extends fgui.GComponent {
  ...
  private refreshView() {
    this.playerCom.url = IconFactory.getSoldierIconByIcon(
      this._vData.templateInfo.Icon,
    );
    var tempInfo: t_s_specialtemplateData =
      TempleteManager.Instance.getPawnSpecialTemplateByID(
        parseInt(this._vData.specialAbility),
      );
    if (tempInfo) {
      this.NameTxt1.text = tempInfo.TemplateNameLang;
      this.NameTxt2.text = LangManager.Instance.GetTranslation(
        "public.level2",
        tempInfo.Grades,
      );
    }
  ...
  private onBtnSelectClick() {
    NotificationManager.Instance.dispatchEvent(
      NotificationEvent.UPDATE_SPECIALSELECTITEM,
      this.vData,
      this.Btn_select.selected,
    );
  }
