...
import { PlayerMountWnd } from "./PlayerMountWnd";

/**
 * @description 我的坐骑
 * @author zhihua.zhou
 */
export class MyMountWnd extends BaseWindow {
  ...
  private showMount(info: SimpleMountInfo): void {
    ...
    this.template = TempleteManager.Instance.getMountTemplateById(
      info.mountTempId,
    );
    if (this.template) {
      ...
      this._resUrl =
        ...
        "equip_show" +
        this.template.AvatarPath.toLocaleLowerCase() +
        "/2/2.json";
  ...
  public dispose() {
    ...
    let wnd: PlayerMountWnd = UIManager.Instance.FindWind(
