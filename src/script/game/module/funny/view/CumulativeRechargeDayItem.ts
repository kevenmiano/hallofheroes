...
export default class CumulativeRechargeDayItem extends FUI_CumulativeRechargeDayItem {
    ...
    private refreshView() {
        if (this._info) {
            this.iconBox.url = FUIHelper.getItemURL(EmWindow.Funny, "Icon_Box_Dev" + (this._index + 1));
            ...
            if (finishValue >= countValue) {//如果已经超出则不展示
                this.tipsState.selectedIndex = 0;
            } else {
                this.countText.text = LangManager.Instance.GetTranslation("CumulativeRechargeDayItem.countText", finishValue, countValue - finishValue);
            }
