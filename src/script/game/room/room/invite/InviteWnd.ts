...
export default class InviteWnd extends BaseWindow {
    ...
    OnShowWind() {
        ...
        this._tabbar.init({}, [this["tabMainFriend"], this["tabMainGuild"], this["tabMainHall"]], undefined, this.__selectTabCallback.bind(this));
        ...
        this.iTxtSearch.promptText = LangManager.Instance.GetTranslation("InviteWnd.iTxtSearch.text");
        this.iTxtSearch.on(Laya.Event.INPUT, this, this.__onTxtSearchChange);
        ConsortiaManager.Instance.getConsortiaUserInfos();
        NotificationManager.Instance.addEventListener(ChatEvent.BLACKLIST,this.onRecvBlackList,this);
    }
    ...
    OnHideWind() {
        ...
        NotificationManager.Instance.removeEventListener(ChatEvent.BLACKLIST,this.onRecvBlackList,this);
        super.OnHideWind();
    }
    ...
    private __interruptCallback(changeToTabIndex: number): boolean {
        switch (changeToTabIndex) {
            case InviteData.TabIndex.Guild:
                if (InviteData.playerInfo.consortiaID == 0) {
                    let str: string = LangManager.Instance.GetTranslation("room.view.invite.RoomInviteFrame.command01");
    ...
    private __selectTabCallback(index: number, lastTabIndex: number) {
        ...
        switch (index) {
            case InviteData.TabIndex.Friend:
                ...
                list = this.filterData(FriendManager.getInstance().friendList.getList());
                this.refreshList(list);
                break;
            case InviteData.TabIndex.Guild:
                // this.refreshList(ConsortiaManager.Instance.model.consortiaMemberList.getList());
                list = this.filterData(ConsortiaManager.Instance.model.consortiaMemberList.getList());
                this.refreshList(list);
                break
    ...
    private filterData(sourArr:Array<any>):Array<any>{
        let list:Array<any> = [];
        if (FreedomTeamManager.Instance.model == null) {//自己没有队伍
            return sourArr;
         }else{
             if(sourArr){
    ...
    private sendInivte(){
        ...
        if (roomInfo) {
            let str = LangManager.Instance.GetTranslation("room.view.invite.InviteItem.command01");
        ...
        if (this.type == RoomInviteType.SpaceInvite) {
            ...
            if (roomInfo && roomInfo.roomType == 1) {
                if (info.grades < OpenGrades.CHALLENGE) {
                    MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("room.view.invite.InviteItem.command01"));
                    return;
                }
            }
    ...
    private __renderListItem(index: number, item: InviteItem) {
        let cacheList = this.isSearching ? this._searchTempList : this.model.cacheList
