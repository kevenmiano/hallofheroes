...
export default class RuneCarveWnd extends BaseWindow {
    ...
    public OnInitWind() {
        ...
        this.frame.getChild('title').text = LangManager.Instance.GetTranslation('runeGem.str2');
        this.frame.getChild('helpBtn').asButton.icon = fgui.UIPackage.getItemURL(EmPackName.Base, 'Btn_Dia2_Warn');
        this.txt_desc0.text = LangManager.Instance.GetTranslation('runeGem.str10');
        this.txt_desc_h0.text = LangManager.Instance.GetTranslation('runeGem.str10');
        this.btn_select.title = LangManager.Instance.GetTranslation('runeGem.str19');
        this.btn_replace.title = LangManager.Instance.GetTranslation('tasktracetip.view.OpenBagTipView.btnTxt');
    ...
    OnShowWind(){
        ...
        //消耗道具ID, 消耗数量
        let str = TempleteManager.Instance.getConfigInfoByConfigName("runehole_use").ConfigValue;
        let arr = str.split(',');
        ...
        this.propTip.getChild('txt_tip0').text= goodsInfo.templateInfo.TemplateNameLang;
        this.propTip.getChild('txt_tip1').text= goodsInfo.templateInfo.DescriptionLang;
    ...
    OnBtnClose(){
        ...
        SimpleAlertHelper.Instance.Show(SimpleAlertHelper.SIMPLE_ALERT,null, prompt, content, confirm, cancel, this.confirmFunc1.bind(this));
    }
        ...
        for (let i = 0; i < holeIdArr.length; i++) {
            ...
            if(holeData){
                for (let j = 0; j < holeData.Length; j++) {
                    ...
                    if(holeCom){
                        holeCom.getChild('txt_name').asTextField.text = LangManager.Instance.GetTranslation('runeGem.holeType'+sid);
                        //判断是否连线
                        (holeCom.getController('c1') as fairygui.Controller).selectedIndex = (sid-1);
...
/**
    ...
    private addEvent(){
        ...
        NotificationManager.Instance.addEventListener(RuneEvent.CARVE_RUNE, this.__runeRefreshHandler, this);
    ...
    private removeEvent(){
        ...
        NotificationManager.Instance.removeEventListener(RuneEvent.CARVE_RUNE, this.__runeRefreshHandler, this);
    ...
    onCarve(){
        ...
        this.controler.reqRuneHoleOpt(this.runeInfo.runeId,RuneOperationCode.RUNE_HOLE_CARVE);
    ...
    private confirmFunc(result: boolean) {
        if(result){
            ...
            this.controler.reqRuneHoleOpt(this.runeInfo.runeId,RuneOperationCode.RUNE_HOLE_REPLACE);
            this.hide();
        }
    }
    ...
    onSelect(){
        ...
        SimpleAlertHelper.Instance.Show(SimpleAlertHelper.SIMPLE_ALERT,null, prompt, content, confirm, cancel, this.confirmFunc2.bind(this));
    }
    ...
    onReplace(){
        ...
        if(this.holeNum > 3){
            ...
            let cancel: string = LangManager.Instance.GetTranslation("public.cancel");
            let prompt: string = LangManager.Instance.GetTranslation("public.prompt");
            let content: string = LangManager.Instance.GetTranslation("runeGem.str40");
            SimpleAlertHelper.Instance.Show(SimpleAlertHelper.SIMPLE_ALERT,null, prompt, content, confirm, cancel, this.confirmFunc.bind(this));
        }else
        {
