...
export default class WishPoolView extends FUI_WishPoolView {
  ...
  private addEvent() {
    ...
    this.typeList.itemRenderer = Laya.Handler.create(
      this,
      this.renderTypeListItem,
      null,
      false,
    );
  ...
  private checkWish() {
    ...
    let wishpoolInfo: WishPoolInfo = this.wishPoolModel.allDic.get(
      this._currentSelectId,
    );
    if (wishpoolInfo) {
      if (
        this._currentSelectInfo.WeeklyLimit <= wishpoolInfo.WeekBuyCount &&
        this.status.selectedIndex == 0
      ) {
        //购买已达上限
        MessageTipManager.Instance.show(
          LangManager.Instance.GetTranslation("wishpool.checkWish.tips"),
        );
        return;
      }
      if (this._count < this._currentSelectInfo.CostItemNum) {
        ...
        if (this._currentSelectInfo.UnitPrice == 0) {
          ...
          this._needPointCount =
            (this._currentSelectInfo.CostItemNum - this._count) *
            this._currentSelectInfo.UnitPrice;
          let needAlert: boolean = true;
          let today: Date = new Date();
          let lastSaveDate: Date = new Date(
            SharedManager.Instance.wishPoolAlertDate,
          );
          if (lastSaveDate) {
            if (
          ...
          if (needAlert) {
            ...
            UIManager.Instance.ShowWind(EmWindow.TodayNotAlert, {
              content: content,
              backFunction: this.sendBuy.bind(this),
              state: 2,
            });
          } else {
            //不需要提示
  ...
  private compare(a: GoodsInfo, b: GoodsInfo) {
    ...
    if (sontypes.indexOf(a.templateInfo.SonType) >= 0) {
      ...
      if (a.templateInfo.SonType == 218) {
        aActive = MountsManager.Instance.avatarList.isLightTemplate(
          a.templateInfo.Property1,
        )
          ? 1
          : 0;
      }
    ...
    if (sontypes.indexOf(b.templateInfo.SonType) >= 0) {
      ...
      if (b.templateInfo.SonType == 218) {
        bActive = MountsManager.Instance.avatarList.isLightTemplate(
          b.templateInfo.Property1,
        )
          ? 1
          : 0;
      }
  ...
  private refreshView() {
    ...
    if (this._currentSelectInfo.Type == WishPoolModel.FASHION_CLOTHES) {
      ...
      this.descTxt1.text = LangManager.Instance.GetTranslation(
        "wishpoolView.descTxt1",
        this.getFashionGrade(minValue1),
      );
      ...
      this.descTxt2.text = LangManager.Instance.GetTranslation(
        "wishpoolView.descTxt2",
        this.getFashionGrade(minValue2),
      );
    ...
    this._count = GoodsManager.Instance.getBagCountByTempId(
      BagType.Player,
      this._currentSelectInfo.CostItemId,
    );
    ...
    this.wishDescTxt.text = LangManager.Instance.GetTranslation(
      "wishpoolView.descTxt11",
      leftCount,
    );
  }
      ...
      for (let i: number = 0; i < len; i++) {
        ...
        tipStr +=
          LangManager.Instance.GetTranslation(
            "wishpoolView.descTxt7",
            this.getFashionGrade(parseInt(strArr[0])),
            strArr[1],
          ) + "<br>";
      }
