...
export class RoomManager {
    ...
    public static get selectCampaign(): t_s_campaignData {
        if (!RoomManager.selectAreaInfo || !RoomManager.selectDiffculty) {
            Logger.warn("[RoomManager]selectAreaInfo=", RoomManager.selectAreaInfo, "selectDiffculty", RoomManager.selectDiffculty)
            return null
        }
        RoomManager._selectCampaign = RoomManager.selectAreaInfo.getMapByDifficult(RoomManager.selectDiffculty)
    ...
    public setup($roomInfo: RoomInfo) {
        ...
        ServerDataManager.listen(S2CProtocol.U_C_CAMPAIGN_ROOM_MEMBER, this, this.__roomMemberHandler);
        ServerDataManager.listen(S2CProtocol.U_C_CAMPAIGN_ARMYPOS_SEND, this, this.__requestTeamPosHandler);
        ServerDataManager.listen(S2CProtocol.U_C_CAMPAIGN_ROOM_EDIT, this, this.__updateRoomInfoHandler);

        ServerDataManager.listen(S2CProtocol.U_C_CAMPAIGN_ROOM_KILLPLAYER, this, this.__campaingkillOutHandler);
        ServerDataManager.listen(S2CProtocol.U_C_PLAYER_READY_STATE, this, this.__updateReaderStateHandler);
        ServerDataManager.listen(S2CProtocol.U_C_CAMPAIGN_ROOM_EXIT, this, this.__returnRoomHandler);
        ServerDataManager.listen(S2CProtocol.U_C_CAMPAIGN_ROOM_MASTER, this, this.__updateHouseOwnerHandler);
        ServerDataManager.listen(S2CProtocol.U_C_ROOM_STATE, this, this.__updateRoomStateHandler);
        ServerDataManager.listen(S2CProtocol.U_C_CAMPAGIN_ROOM_PLACESTATE, this, this.__roomPlaceStateChangeHandler);
        ServerDataManager.listen(S2CProtocol.U_C_HERO_BROAD_EQUIPMENT, this, this.__otherHeroInfoChange);
        NotificationManager.Instance.addEventListener(ChatEvent.UPDATE_CHAT_VIEW, this.__playerChatHandler, this);
        ServerDataManager.listen(S2CProtocol.U_C_VOTE_KILLPLAYE, this, this.__crossCampaingkillOutHandler);
        ServerDataManager.listen(S2CProtocol.U_C_MATCH_COMFIRE, this, this.__crossMatchComfirmHandler);
    }

    public dispose() {
        ServerDataManager.cancel(S2CProtocol.U_C_CAMPAIGN_ROOM_MEMBER, this, this.__roomMemberHandler);
        ServerDataManager.cancel(S2CProtocol.U_C_CAMPAIGN_ARMYPOS_SEND, this, this.__requestTeamPosHandler);
        ServerDataManager.cancel(S2CProtocol.U_C_CAMPAIGN_ROOM_EDIT, this, this.__updateRoomInfoHandler);

        ServerDataManager.cancel(S2CProtocol.U_C_CAMPAIGN_ROOM_KILLPLAYER, this, this.__campaingkillOutHandler);
        ServerDataManager.cancel(S2CProtocol.U_C_PLAYER_READY_STATE, this, this.__updateReaderStateHandler);
        ServerDataManager.cancel(S2CProtocol.U_C_CAMPAIGN_ROOM_EXIT, this, this.__returnRoomHandler);
        ServerDataManager.cancel(S2CProtocol.U_C_CAMPAIGN_ROOM_MASTER, this, this.__updateHouseOwnerHandler);
        ServerDataManager.cancel(S2CProtocol.U_C_ROOM_STATE, this, this.__updateRoomStateHandler);
        ServerDataManager.cancel(S2CProtocol.U_C_CAMPAGIN_ROOM_PLACESTATE, this, this.__roomPlaceStateChangeHandler);
        ServerDataManager.cancel(S2CProtocol.U_C_HERO_BROAD_EQUIPMENT, this, this.__otherHeroInfoChange);
        ServerDataManager.cancel(S2CProtocol.U_C_VOTE_KILLPLAYE, this, this.__crossCampaingkillOutHandler);
        ServerDataManager.cancel(S2CProtocol.U_C_MATCH_COMFIRE, this, this.__crossMatchComfirmHandler);
        ...
        if (BattleManager.preScene == SceneType.PVP_ROOM_SCENE || BattleManager.preScene == SceneType.PVE_ROOM_SCENE) {
    ...
    public get isSelfInRoom() {
        if (this._roomInfo) {
            let army = this._roomInfo.getPlayerByUserId(ArmyManager.Instance.thane.userId);
            return Boolean(army)
        }
        return false;
    }
    ...
    private __playerChatHandler(data: ChatData) {
        ...
        var aInfo: CampaignArmy = this._roomInfo.getPlayerByUserId(chatData.uid, chatData.serverName);
        if (aInfo)
    ...
    private readMember(player: RoomPlayerMsg) {
        ...
        var aInfo: CampaignArmy = this.roomInfo.getPlayerByUserId(player.playerId, player.serverName);
        if (!aInfo) {
        ...
        if (this._roomInfo && roomId == this._roomInfo.id) {
            ...
            if (userId == this.selfPlayerInfo.userId) {
                ...
                if (curScene == SceneType.PVE_ROOM_SCENE) {
                    ...
                    NotificationManager.Instance.sendNotification(NotificationEvent.VEHICLE_ROOM_KILL_OUT, userId);
                    this.dispose();
                    return;
                }
                ...
                var killPlayer: CampaignArmy = this._roomInfo.removePlayerByUserId(userId, "");
                if (killPlayer)
                    NotificationManager.Instance.sendNotification(NotificationEvent.ROOM_KILL_OUT, killPlayer.id);
            }
        }
        this._roomInfo.commit();
    }
    ...
    private __updateReaderStateHandler(pkg: PackageIn) {
        ...
        for (var i: number = 0; i < msg.readyState.length; i++) {
            ...
            var roomPlayer: CampaignArmy = this._roomInfo.getPlayerByUserId(userId, "");
            if (roomPlayer) {
    ...
    private __updateHouseOwnerHandler(pkg: PackageIn) {
        ...
        if (this._roomInfo) {
            ...
            this._roomInfo.commit();
            var roomPlayer: CampaignArmy = this._roomInfo.getPlayerByUserId(houseOwnerId, "");
            if (roomPlayer)
                roomPlayer.commit();
        }
    }
    ...
    private __crossCampaingkillOutHandler(pkg: PackageIn) {
        ...
        FrameCtrlManager.Instance.open(EmWindow.CrossPvPVoteWnd, { descStr: msg.password });
    }
    ...
    private __crossMatchComfirmHandler(pkg: PackageIn) {
        ...
        FrameCtrlManager.Instance.open(EmWindow.CrossPvPSuccessWnd, { leftBoss: msg.param1, totalBoss: msg.param2, campaignName: msg.param4, campaignUid: msg.param5, roomId: msg.param6 });
    }
