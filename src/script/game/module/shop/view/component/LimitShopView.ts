...
export default class LimitShopView extends FUI_LimitShopView {
  ...
  private initEvent() {
    ...
    NotificationManager.Instance.addEventListener(
      ...
      this.__refreshTimeBuy,
      this,
    );
    NotificationManager.Instance.addEventListener(
      ShopEvent.BUY_DISCOUNT_RET,
      this.buyDisCountRet,
      this,
    );
    NotificationManager.Instance.addEventListener(
  ...
  public updateView() {
    this.setBuybtnState();
    this._countGood = 0;
    let discountList: Array<any> = this._isVIPShop
      ? ShopManager.Instance.model.getTimeBuyList(1)
      : ShopManager.Instance.model.getTimeBuyList(0);
  ...
  private onTabChanged() {
    if (this._goodsList && this._goodsList.length > 0 && !this.isSelected) {
      ...
      this.onListShopClick(
        this.list_shop.getChildAt(childIndex) as LimitShopItem,
        null,
      );
    } else {
      if (
        this._currentSelectedItem &&
        this._currentSelectedItemData.type == 1
      ) {
        ShopManager.Instance.currBuyGoods = this._currentSelectedItemData;
        ShopManager.Instance.getGoodsBuyNum(
          (this._currentSelectedItemData as MainShopInfo).id,
        );
      } else {
  ...
  private updateRightView(iteminfo: MainShopInfo) {
    ...
    if (info.isGift) {
      ...
      this.txt_name.color =
        FilterFrameText.Colors[eFilterFrameText.ItemQuality][2];
      ...
      for (let i: number = 0; i < 9; i++) {
        ...
        shopGoods = TempleteManager.Instance.getGoodsTemplatesByTempleteId(
          info["templateId" + index],
        );
        if (!shopGoods) {
          continue;
        }
      ...
      this.txt_describe.text =
        LangManager.Instance.GetTranslation("propTips.detail");
      ...
      this.txt_name.text =
        ...
        "" +
        (goodsInfo.count > 1 ? "*" + goodsInfo.count.toString() : "");
      this.txt_name.color =
        FilterFrameText.Colors[eFilterFrameText.ItemQuality][
          goodsInfo.templateInfo.Profile - 1
        ];
      let desc = TempleteManager.Instance.getGoodsTempleteDesc(
  ...
  protected get isFashion(): boolean {
    ...
    if (
      !TempleteManager.Instance.getGoodsTemplatesByTempleteId(
        this._currentSelectedItemData.templateId,
      )
    ) {
      return false;
    }
  ...
  protected checkExist(tId: number): string {
    let goods: t_s_itemtemplateData =
      TempleteManager.Instance.getGoodsTemplatesByTempleteId(tId);
  ...
  private onBtnBuyClick() {
    if (this._currentSelectedItem) {
      ...
      if (this._discountTempleteId != 0 && this._discountTempleteData != null) {
        ...
        if (this._currentSelectedItemData.isGift) {
          content = LangManager.Instance.GetTranslation(
            "shop.view.limitShop.AlertContent",
            this._currentSelectedItem.info.names,
          );
        } else {
      ...
      SimpleAlertHelper.Instance.Show(
        ...
        confirm,
        cancel,
        this.confirmBuyCallback.bind(this),
      );
    } else {
  ...
  private get currentCanCount(): number {
    if (this._currentSelectedItemData.discount == 0) {
      //如果免费,则为当天能够买的数量
      if (
        this._currentSelectedItemData.count >=
      ...
      let pointCount: number = Math.floor(
        this.PayMoney /
          (this._currentSelectedItemData.price *
            this._currentSelectedItemData.discount),
      );
  ...
  public removeEvent() {
    ...
    this._btnDiscount &&
      this._btnDiscount.offClick(this, this.onBtnDiscount.bind(this));
