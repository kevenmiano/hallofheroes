...
export class TattooCom extends FUI_TattooCom {
  ...
  private onIconSmall(index: number) {
    ...
    this.txt_name_tips.text =
      LangManager.Instance.GetTranslation(
        "tattoo.TattooPopFrame.propertyName" + (index + 1),
      ) +
      " " +
  ...
  private holeClickHandle(index: number, e: Laya.Event) {
    ...
    if (clickHole.isLock) {
      ...
      this.txt_lock_tips
        .setVar("lv", openLv.toString())
        .setVar("index", id.toString())
        .flushVars();
    } else {
  ...
  private updateTips(): void {
    ...
    if (this.tattooModel.getImprintTotalCountByProperity(0, targetArr) > 0) {
      let addStrengthPrecent: number =
        (this.tattooModel.getImprintTotalCountByProperity(0, targetArr) *
          addStrength) /
        100;
      if (addStrength > 0) {
        addStrengthPrecentStr =
          "<font color='#59cd41'>+" + addStrengthPrecent + "</font>";
      } else {
        addStrengthPrecentStr =
          "<font color='#fa0600'>" + addStrengthPrecent + "</font>";
      }
    }
    let addAgilityPrecentStr: string = "";
    if (this.tattooModel.getImprintTotalCountByProperity(1, targetArr) > 0) {
      let addAgilityPrecent: number =
        (this.tattooModel.getImprintTotalCountByProperity(1, targetArr) *
          addAgility) /
        100;
      if (addAgility > 0) {
        addAgilityPrecentStr =
          "<font color='#59cd41'>+" + addAgilityPrecent + "</font>";
      } else {
        addAgilityPrecentStr =
          "<font color='#fa0600'>" + addAgilityPrecent + "</font>";
      }
    }
    let addIntelligencePrecentStr: string = "";
    if (this.tattooModel.getImprintTotalCountByProperity(2, targetArr) > 0) {
      let addIntelligencePrecent: number =
        (this.tattooModel.getImprintTotalCountByProperity(2, targetArr) *
          addIntelligence) /
        100;
      if (addIntelligence > 0) {
        addIntelligencePrecentStr =
          "<font color='#59cd41'>+" + addIntelligencePrecent + "</font>";
      } else {
        addIntelligencePrecentStr =
          "<font color='#fa0600'>" + addIntelligencePrecent + "</font>";
      }
    }
    let addCaptainPrecentStr: string = "";
    if (this.tattooModel.getImprintTotalCountByProperity(3, targetArr) > 0) {
      let addCaptainPrecent: number =
        (this.tattooModel.getImprintTotalCountByProperity(3, targetArr) *
          addCaptain) /
        100;
      if (addCaptain > 0) {
        addCaptainPrecentStr =
          "<font color='#59cd41'>+" + addCaptainPrecent + "</font>";
      } else {
        addCaptainPrecentStr =
          "<font color='#fa0600'>" + addCaptainPrecent + "</font>";
      }
    }
    let addPhysiquePrecentStr: string = "";
    if (this.tattooModel.getImprintTotalCountByProperity(5, targetArr) > 0) {
      let addPhysiquePrecent: number =
        (this.tattooModel.getImprintTotalCountByProperity(5, targetArr) *
          addPhysique) /
        100;
      if (addPhysique > 0) {
        addPhysiquePrecentStr =
          "<font color='#59cd41'>+" + addPhysiquePrecent + "</font>";
      } else {
        addPhysiquePrecentStr =
          "<font color='#fa0600'>" + addPhysiquePrecent + "</font>";
      }
    }
    let tipStr: string = "";
    if (addStrength > 0) {
      tipStr +=
        LangManager.Instance.GetTranslation(
          "tattoo.TattooPopFrame.propertyName1",
        ) +
        " +" +
        addStrength +
        "  " +
    ...
    if (addAgility > 0) {
      tipStr +=
        LangManager.Instance.GetTranslation(
          "tattoo.TattooPopFrame.propertyName2",
        ) +
        " +" +
        addAgility +
        "  " +
    ...
    if (addIntelligence > 0) {
      tipStr +=
        LangManager.Instance.GetTranslation(
          "tattoo.TattooPopFrame.propertyName3",
        ) +
        "  " +
        " +" +
        addIntelligence +
    ...
    if (addPhysique > 0) {
      tipStr +=
        LangManager.Instance.GetTranslation(
          "tattoo.TattooPopFrame.propertyName4",
        ) +
        " +" +
        addPhysique +
        "  " +
    ...
    if (addCaptain > 0) {
      tipStr +=
        LangManager.Instance.GetTranslation(
          "tattoo.TattooPopFrame.propertyName5",
        ) +
        " +" +
        addCaptain +
        "  " +
  ...
  private updateCore() {
    ...
    this.icon_core.url = fgui.UIPackage.getItemURL(
      EmWindow.SBag,
      "Icon_Tattoo_Core" + Math.ceil(this.tattooModel.coreStep / 2),
    );
    //{step=Ⅰ}阶龙纹核心
    this.txt_tips_core_name
      .setVar("step", StringUtils.getRomanNumber(this.tattooModel.coreStep))
      .flushVars();
    //当前龙纹阶级上限: [color=#ffecc6]{lv=1}阶[/color]
    // 属性上限: [color=#ffecc6]{value=1000}[/color]
    this.txt_tips_core_0
      .setVar("lv", StringUtils.getRomanNumber(this.tattooModel.coreStep))
      .setVar(
        "value",
        this.tattooModel
          .getProtertyValueMaxByStep(this.tattooModel.coreStep)
          .toString(),
      )
      .flushVars();
    ...
    if (nextLv > 0) {
      ...
      this.txt_tips_core_1
        .setVar("lv", StringUtils.getRomanNumber(this.tattooModel.coreStep + 1))
        .setVar(
          "value",
          this.tattooModel
            .getProtertyValueMaxByStep(this.tattooModel.coreStep + 1)
            .toString(),
        )
        .flushVars();
      ...
      this.txt_tips_core_2
        .setVar("color1", limitNum >= openedNum ? color1 : color2)
        .setVar("lv", StringUtils.getRomanNumber(this.tattooModel.coreStep))
    ...
    let grade: number = this.tattooModel.getGradeByStep(
      this.tattooModel.coreStep,
    );
    if (grade > 0) {
      this.txt_condition.text = LangManager.Instance.GetTranslation(
        "tattoo.TattooLevelUp.condition",
        grade,
        StringUtils.getRomanNumber(nextStep),
      );
    } else {
      this.txt_condition.text = LangManager.Instance.GetTranslation(
        "tattoo.TattooStepUp.limit",
      );
    }
  }
  ...
  private updateTattooInfo(type: number) {
    ...
    if (type == 4) {
      //龙纹核心升阶成功播放特效
      let move: fgui.GMovieClip = fgui.UIPackage.createObject(
        EmWindow.SBag,
        "sj",
      ).asMovieClip;
  ...
  private updateTotalNum() {
    ...
    for (let i = 0; i < 5; i++) {
      this["txt_num_" + i].text =
        propertyTypeNum[i] + "/" + this.tattooModel.identicalTattooNum;
    }
  }
  ...
  private onBtnHelpClick() {
    ...
    let content = LangManager.Instance.GetTranslation(
      "tattoo.TattooView.helpContent",
      this.tattooModel.identicalTattooNum,
    );
