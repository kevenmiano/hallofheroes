...
export default class HomeWnd extends BaseWindow {
  ...
  public OnInitWind() {
    ...
    this.goldenSheepView = new GoldenSheepView(
      this.goldenSheepBtn,
      this.goldenSheepShine,
      this.picIcon,
      this.leftTimeTxt,
      this.leftCom,
    );
    ...
    if (this.msgGroup) this.msgGroup.visible = !ComponentSetting.IOS_VERSION; //IOS提审版本不展示
    //添加公告
    ...
    FUIHelper.setTipData(this.powerBtn.view, EmWindow.WearyTips, goodsInfo);
  ...
  private async createBugleNotice() {
    ...
    LayerMgr.Instance.addToLayer(
      this.homeBugleNotice.displayObject,
      EmLayer.STAGE_TIP_LAYER,
    );
    this.homeBugleNotice.x =
      (this.contentPane.width - this.homeBugleNotice.width) / 2;
    this.homeBugleNotice.y = 160;

    this.systemNotice = await SystemNoticeWnd.createInstance();
    LayerMgr.Instance.addToLayer(
      this.systemNotice.displayObject,
      EmLayer.STAGE_TIP_LAYER,
    );

    this.systemNotice.x =
      (this.contentPane.width - this.systemNotice.width) / 2;
    ...
    switch (SceneManager.Instance.currentType) {
      ...
      case SceneType.CAMPAIGN_MAP_SCENE:
        ...
        if (WorldBossHelper.checkIsNoviceMap(mapID)) {
          ...
          //公会秘境
          this.showMapCtrl.selectedIndex =
            HomeWnd.MODE_CONSORTIA_SECRETINFO_SCENE;
        } else if (
          WorldBossHelper.checkTrailTower(mapID) ||
          WorldBossHelper.checkWorldBoss(mapID) ||
          WorldBossHelper.checkPvp(mapID) ||
          WorldBossHelper.checkGvg(mapID) ||
          WorldBossHelper.checkConsortiaDemon(mapID)
        ) {
          this.showMapCtrl.selectedIndex = HomeWnd.MODE_NONE_TOP_SCENE;
  ...
  initEvent() {
    ...
    this.playerModel.addEventListener(
      ...
      this.__spaceTargetChangeHandler,
      this,
    );
    NotificationManager.Instance.addEventListener(
  ...
  private removeEvent() {
    ...
    this.UnlockMapCameraBtn.onClick(
      this,
      this.onUnlockMapCameraClick.bind(this),
    );
  ...
  private updateHeadImg() {
    this.UnlockMapCameraBtn.view
      .getChild("com")
      .asCom.getChild("imgHead").icon = IconFactory.getPlayerIcon(
  ...
  private __thaneInfoChangeHandler(target) {
    ...
    if (
      simpleInfo.fightingCapacity > this._preFighting &&
      this._preFighting > 0 &&
      !this.__checkFrameExist()
    ) {
      Logger.info(
        "[HomeWnd]战斗力提升动画",
        this._preFighting,
        simpleInfo.fightingCapacity,
      );
      DelayActionsUtils.Instance.addAction(
        new HeroFightingUpdateAction(this._preFighting, simpleInfo),
      );
    }
  ...
  onLevelGiftTipClick() {
    ...
    FrameCtrlManager.Instance.open(EmWindow.Welfare, {
      str: LangManager.Instance.GetTranslation("welfareWnd.tabTitle.LevelGift"),
    });
  }
