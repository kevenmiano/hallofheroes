...
export default class AllocateCom extends FUI_AllocateCom implements DragObject {
  ...
  public refreshView() {
    ...
    if (this._pawn && this._pawn.ownPawns != 0) {
      ...
      if (this.soliderIcon) {
        this.soliderIcon.icon = IconFactory.getSoldierIconByIcon(
          this._pawn.templateInfo.Icon,
        );
      }
    } else {
      this.visible = false;
  ...
  public dispose() {
    if (this._pawn) {
      ...
      DoubleClickManager.Instance.disableDoubleClick(
        this.soliderIcon.displayObject,
      );
    }
    super.dispose();
  }
