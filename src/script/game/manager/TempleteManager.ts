...
import t_s_remotepet, { t_s_remotepettemplateData } from "../config/t_s_remotepet";
...
import t_s_vipprerogativetemplate, { t_s_vipprerogativetemplateData } from '../config/t_s_vipprerogativetemplate';
...
export class TempleteManager {
    ...
    public getTemplateByTypeAndLevel(level: number, type: number): t_s_upgradetemplateData {
        ...
        let findIdx = Utils.insertValueSearch(templateDic, 0, templateDic.length - 1, level, "Grades");
        if (findIdx >= 0)
            temp = templateDic[findIdx]
        return temp;
    }

    public getTemplateByTypeAndLevelAndSonType(level: number, type: number, sonType: number): t_s_upgradetemplateData {
        let templateDic = ConfigMgr.Instance.getDicSync(ConfigType.t_s_upgradetemplate);
        for (const dicKey in templateDic) {
            if (templateDic.hasOwnProperty(dicKey)) {
                let temp: t_s_upgradetemplateData = templateDic[dicKey];
                if (temp.Grades == level && temp.Types == type && temp.TemplateId == sonType) {
                    return temp;
                }
            }
        }
    ...
    getSkillBuffTemplateByID(templateId: number): t_s_skillbuffertemplateData {
        let temp = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_skillbuffertemplate, templateId);
        // for (const dicKey in templateDic) {
        //     if (templateDic.hasOwnProperty(dicKey)) {
    ...
    public getRuneTemplateByTid(tid: number): t_s_runetemplateData {
        let temp = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_runetemplate, tid);
        // for (const dicKey in templateDic) {
        //     if (templateDic.hasOwnProperty(dicKey)) {
    ...
    getBufferTemplateByType(buffType: number): Array<t_s_campaignbufferData> {
        // let tempDic = [];
        let templateDic = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_campaignbuffer, "Types" + buffType);
        // for (const dicKey in templateDic) {
        //     if (templateDic.hasOwnProperty(dicKey)) {
    ...
    public getStarTemplateById(id: number): t_s_startemplateData {
        let temp = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_startemplate, id);
        // for (const dicKey in templateDic) {
        //     if (templateDic.hasOwnProperty(dicKey)) {
    ...
    public getTemplateByTypeAndLevelAndID(level: number, type: number, templateId: number): t_s_upgradetemplateData {
        let temps = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_upgradetemplate, "Types" + type) as t_s_upgradetemplateData[];
        if (temps)
    ...
    /**通过商品ID获取商品配置 */
    public getRechargeTempleteByProductID(productId: string = ""): t_s_rechargeData {
        let temp = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_recharge, productId);
        // for (const dicKey in templateDic) {
        //     if (templateDic.hasOwnProperty(dicKey)) {
     ...
     * ProductType 为1
     */
    public getRechargeTempletes(productType: number = 1, param1?: number): t_s_rechargeData[] {
        ...
        for (const dicKey in templateDic) {
            if (templateDic.hasOwnProperty(dicKey)) {
                let temp: t_s_rechargeData = templateDic[dicKey];
                if (!param1) {
                    if (temp.ProductType == productType)
                        arr.push(temp);
                } else {
                    if (temp.ProductType == productType && param1.toString() == temp.Para1)
                        arr.push(temp);
                }
            }
        }
    ...
    public getTemplatesByType(type: number): t_s_upgradetemplateData[] {
        ...
        let arr = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_upgradetemplate, "Types" + type);
        !arr && (arr = [])
        return arr;
    }

    public getTemplatesByTypeAndId(type: number, id: number): t_s_upgradetemplateData[] {
        let arr: t_s_upgradetemplateData[] = [];
        let templateDic = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_upgradetemplate, "Types" + type);
        for (const dicKey in templateDic) {
            if (templateDic.hasOwnProperty(dicKey)) {
                let temp: t_s_upgradetemplateData = templateDic[dicKey];
     ...
     */
    public getVipPackageByType(grade: number, quality: number): t_s_vippackageData {
    ...
    public getPrivilegeTempletes(): t_s_vipprerogativetemplateData[] {
        let config = ConfigMgr.Instance.getConfigMap("t_s_vipprerogativetemplate") as t_s_vipprerogativetemplate;
        return config.mDataList.concat();
    }

    /**VIP特权 */
    public getPrivilegeTempletesByLevel(vipLevel: number): t_s_vipprerogativetemplateData[] {
        let arr: t_s_vipprerogativetemplateData[] = [];
        let templateDic = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_vipprerogativetemplate, "Grade" + vipLevel);
    ...
    /**VIP特权 */
    public getPrivilegeTempletesByType(type: number): t_s_vipprerogativetemplateData[] {
        //用的不多, 标记
        let arr: t_s_vipprerogativetemplateData[] = [];
        let templateDic = ConfigMgr.Instance.getDicSync(ConfigType.t_s_vipprerogativetemplate);
        let temp: t_s_vipprerogativetemplateData
        for (const dicKey in templateDic) {
            if (templateDic.hasOwnProperty(dicKey)) {
                temp = templateDic[dicKey];
                if (temp.type == type && temp.para1 > 0)
                    arr.push(temp);
            }
        }
        return arr;
     ...
     * @returns 
     */
    public getPrivilegeTempletesByTypeLevel(type: number, vipLevel: number): t_s_vipprerogativetemplateData {
        let item: t_s_vipprerogativetemplateData = null;
        let templateDic = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_vipprerogativetemplate, "Grade" + vipLevel) as t_s_vipprerogativetemplateData[];
        if (templateDic)
    ...
    public getMinRuneTemplateInfoByJob(job: number) {
        ...
        let runeTemplateDic = ConfigMgr.Instance.getDicSync(ConfigType.t_s_runetemplate);
        ...
        for (const dicKey in runeTemplateDic) {
            ...
            if (temp instanceof t_s_runetemplateData)
                if ((temp.Job.indexOf(0) >= 0 || temp.Job.indexOf(job) >= 0) && temp.RuneGrade == 1) {
    ...
    public getRuneTemplateInfoByRuneTypeAndLevel(runeType: number, level: number): t_s_runetemplateData {
        let runeTemplateDic = ConfigMgr.Instance.getDicSync(ConfigType.t_s_runetemplate);
        for (const dicKey in runeTemplateDic) {
            if (runeTemplateDic.hasOwnProperty(dicKey)) {
                let temp: t_s_runetemplateData = runeTemplateDic[dicKey];
    ...
    public getQuestionTempletes(): t_s_questionData[] {
        ...
        let tquestion = ConfigMgr.Instance.getConfigMap(ConfigType.t_s_question) as t_s_question;
    ...
    public getAllSkillTemplateInfoByJob(job: number): SimpleDictionary {
        ...
        let skillTemplateDic = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_skilltemplate, "MasterType" + job);
    ...
    public getAllTalentTemplateInfoByJob(job: number): SimpleDictionary {
        ...
        skillTemplateDic = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_skilltemplate, "MasterType" + 9);
        this.getSkillTemplateDic(skillTemplateDic, dic);
        skillTemplateDic = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_skilltemplate, "MasterType" + 100);
        this.getSkillTemplateDic(skillTemplateDic, dic);
        return dic;
    }
     ...
     */
     public getAllExtrajobSkillTemplateInfoByJob(job: number): SimpleDictionary {
        let jobArr= [];
        if(job==1||job==4){//战士屏蔽41
            jobArr = [42,43,44];
        }else  if(job==2||job==5){//射手屏蔽42
            jobArr = [41,43,44];
        }else{//法师屏蔽43
            jobArr = [41,42,44];
        }
        ...
        for (let i = 0; i < jobArr.length; i++) {
            ...
            let skillTemplateDic = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_skilltemplate, "MasterType" + jobtype);
            if (skillTemplateDic)
            for (const dicKey in skillTemplateDic) {
                if (skillTemplateDic.hasOwnProperty(dicKey)) {
                    temp = skillTemplateDic[dicKey];
     ...
     *
     */
    public getMinSkillTemplateInfoBySonType(sontype: number): t_s_skilltemplateData {
        let skillTemplateDic = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_skilltemplate, "SonType" + sontype);
        let temp: t_s_skilltemplateData = null;
        if (skillTemplateDic)
    ...
    */
    public getSkillTemplateInfoBySonTypeAndGrade(sontype: number, grade: number) {
        let skillTemplateDic: t_s_skilltemplateData[] = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_skilltemplate, "SonType" + sontype);
        ...
        let findIdx = Utils.insertValueSearch(skillTemplateDic, 0, skillTemplateDic.length - 1, grade, "Grades");
        if (findIdx >= 0) {
    ...
    public getActiveTemplates() {
        ...
        let activityTemplateDic = ConfigMgr.Instance.getDicSync(ConfigType.t_s_activetemplate);
    ...
    public getDropItemssByDropId(dropId: number): Array<t_s_dropitemData> {
        let templateDic = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_dropitem, "DropId" + dropId);
     ...
     * 获取掉落条件类型
     */
    public getDropConditionByType(condictionType: number): t_s_dropconditionData[] {
        let temp: t_s_dropconditionData[];
        let leedTemplateDic = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_dropcondition, "CondictionType" + condictionType);
    ...
    public getDropConditionByPara1(sonType: number): t_s_dropconditionData[] {
        //这要看看业务逻辑, 不好优化。
        let temp: t_s_dropconditionData[] = [];
        let leedTemplateDic = ConfigMgr.Instance.getDicSync(ConfigType.t_s_dropcondition);
        if (leedTemplateDic)
            for (const dicKey in leedTemplateDic) {
                if (leedTemplateDic.hasOwnProperty(dicKey)) {
                    let item: t_s_dropconditionData = leedTemplateDic[dicKey];
    ...
    public getLeedTemplateByID(id: number): t_s_leedtemplateData {
        let temp = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_leedtemplate, id);
        // for (const dicKey in leedTemplateDic) {
        //     if (leedTemplateDic.hasOwnProperty(dicKey)) {
     ...
     *
     */
    public getQualityGradeTemplate(target: t_s_pettemplateData, curQuality: number): t_s_pettemplateData {
        if (!target) {
            return null;
        }

        let list: t_s_pettemplateData[] = this.getPetTemplatesByType(target.PetType);
        for (let item of list) {
            if (item.TemplateNameLang == target.TemplateNameLang && item.Quality == curQuality) {
                return item;
            }
        }
        return null;
    ...
    public getPetEquipStrenData(level: number): t_s_petequipstrengthenData {
        ...
        let petTemplateDic = ConfigMgr.Instance.getDicSync(ConfigType.t_s_petequipstrengthen);
    ...
    public getPetEquipSuitData(suitId: number): t_s_petequipsuitData {
        ...
        let petTemplateDic = ConfigMgr.Instance.getDicSync(ConfigType.t_s_petequipsuit);
        for (const dicKey in petTemplateDic) {
            if (petTemplateDic.hasOwnProperty(dicKey)) {
    ...
    public getPetTemplateById(petId: number): t_s_pettemplateData {
        ...
        let petTempleteData = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_pettemplate, petId);
        // for (const dicKey in petTemplateDic) {
        //     if (petTemplateDic.hasOwnProperty(dicKey)) {
    ...
    public getAppellInfoTemplateByID(templeteId: number): t_s_appellData {
        ...
        TempleteData = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_appell, templeteId);
        // for (const dicKey in TemplateDic) {
        //     if (TemplateDic.hasOwnProperty(dicKey)) {
        //         let temp: t_s_appellData = TemplateDic[dicKey];
    ...
    public getMagicCardTemplateByID(templeteId: number) {
        ...
        TempleteData = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_powcardsuitetemplate, templeteId);
        // for (const dicKey in TemplateDic) {
        //     if (TemplateDic.hasOwnProperty(dicKey)) {
    ...
    public getPowerCardTemplateByID(templeteId: number) {
        ...
        TempleteData = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_powcardtemplate, templeteId);
        // for (const dicKey in TemplateDic) {
        //     if (TemplateDic.hasOwnProperty(dicKey)) {
    ...
    public getPetTemplatesByType(type: number): t_s_pettemplateData[] {
        let petTemplateDic = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_pettemplate, "PetType" + type);
        // for (const dicKey in petTemplateDic) {
        //     if (petTemplateDic.hasOwnProperty(dicKey)) {
        //         let temp: t_s_pettemplateData = petTemplateDic[dicKey];
    ...
    public getPetEquipAttri(AttributeId: number): t_s_attributeData {
        let tempDic = null;
        let itemTemplateDic = ConfigMgr.Instance.getDicSync(ConfigType.t_s_attribute)
    ...
    public getpetequipqualityData(Profile: number): t_s_petequipqualityData {
        let tempDic = null;
        let itemTemplateDic = ConfigMgr.Instance.getDicSync(ConfigType.t_s_petequipquality)
        for (const key in itemTemplateDic) {
            if (itemTemplateDic.hasOwnProperty(key)) {
    ...
    public getPawnTemplateById(Id: number): t_s_pawntemplateData {
        ...
        pawntemplateData = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_pawntemplate, Id);
    ...
    public getSkillTemplateInfoById(Id: number): t_s_skilltemplateData {
        ...
        skilltemplateData = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_skilltemplate, Id);
        // for (const dicKey in skillTemplateDic) {
        //     if (skillTemplateDic.hasOwnProperty(dicKey)) {
        //         let temp: t_s_skilltemplateData = skillTemplateDic[dicKey];
        //         if (temp.TemplateId == Id) {
    ...
    public getPawnSpecialTemplateByID(Id: number): t_s_specialtemplateData {
        ...
        specialtemplateData = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_specialtemplate, Id);
    ...
    public getBuildTemplateByID(Id: number): t_s_buildingtemplateData {
        ...
        buildtemplateData = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_buildingtemplate, Id);
        return buildtemplateData;
    }

    public getMinGradeBuildTemplate(sonType: number): t_s_buildingtemplateData {
        let buildtemplateData: t_s_buildingtemplateData = null;
        let buildtemplateDic = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_buildingtemplate, "SonType" + sonType);
    ...
    getConfigLanguages(): string[] {
        let openCfgs = TempleteManager.Instance.getConfigInfoByConfigName("Language");
        if (openCfgs) {
    ...
    getConfigInfoByConfigName(configName: string) {
        let temp = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_config, configName) as t_s_configData;
        if (!temp)
    ...
    public getCampaignTemplateByID(Id: number): t_s_campaignData {
        ...
        campaignTemplateData = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_campaign, Id);
    ...
    public getComposeTempleteByTempleteId(tid: number): t_s_composeData {
        let composeTemplateDic = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_compose, "Types" + ForgeData.COMPOSE_TYPE_GEM) as t_s_composeData[];
    ...
    public getCrystalTempleteByTempleteId(tid: number): t_s_composeData {
        let composeTemplateDic = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_compose, "Types" + ForgeData.COMPOSE_TYPE_CRYSTAL) as t_s_composeData[];
        if (composeTemplateDic)
            for (let info of composeTemplateDic) {
                if (info.NewMaterial == tid) {
                    return info;
    ...
    * @return 
    */
    public getEquipUpdateComposeTempleteByMainMaterialId(id: number): t_s_composeData {
        //要看业务逻辑, 这没有合适的优化
        let composeTemplateDic = ConfigMgr.Instance.getDicSync(ConfigType.t_s_compose)
        if (composeTemplateDic)
    ...
    public getComposeByType(types: number) {
        let composeTemplateDic = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_compose, "Types" + 7) as t_s_composeData[];
        return composeTemplateDic;
    }

    public getComposeById(Id: number) {
        return ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_compose, Id) as t_s_composeData;
    }

    private _upGradeComposeTemId: number = 0;//升级公式ID
    public getUpGradeComposeTemId(TemplateId: number): number {
        let composeTem = TempleteManager.Instance.getEquipUpdateComposeTempleteByMainMaterialId(TemplateId)
        if (composeTem) {
    ...
    public getGoodsTemplatesBySonType(sonType: number): Array<t_s_itemtemplateData> {
        // let tempDic = [];
        let itemTemplateDic = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_itemtemplate, "SonType" + sonType);
        // for (const key in itemTemplateDic) {
        //     if (itemTemplateDic.hasOwnProperty(key)) {
    ...
    public getGoodsTemplatesByTempleteId(templateId: number): t_s_itemtemplateData {
        // let tempDic = null;
        let info = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_itemtemplate, templateId)
        // for (const key in itemTemplateDic) {
        //     if (itemTemplateDic.hasOwnProperty(key)) {
        //         let info: t_s_itemtemplateData = itemTemplateDic[key];
    ...
    public getGoodsTempleteDesc(templateInfo: t_s_itemtemplateData): string {
        ...
        if (templateInfo.SonType == 206 && p3 > 0) {
            ...
            let dropArr: t_s_dropitemData[] = TempleteManager.Instance.getDropItemssByDropId(templateInfo.Property1);
            for (let i = 0; i < dropArr.length; i++) {
                const dropdata: t_s_dropitemData = dropArr[i];
                let cfg = TempleteManager.Instance.getGoodsTemplatesByTempleteId(dropdata.ItemId);
                if (dropdata.AppearType == 1) {
                    //必定开出: str0
                    if (str0 == "") {
                        str0 = langs.GetTranslation("propTips.boxTip1") + "<br>";
                    }
                    str0 += langs.GetTranslation("propTips.boxTip4", cfg.TemplateNameLang, dropdata.Data) + "<br>";
                    ...
                    if (showRandom && dropdata.Random) {
                        str1 += langs.GetTranslation("propTips.boxTip5", cfg.TemplateNameLang, dropdata.Data, dropdata.Random / 1000) + "<br>";
                    } else {
                        str1 += langs.GetTranslation("propTips.boxTip4", cfg.TemplateNameLang, dropdata.Data) + "<br>";
                    }
                    ...
                    if (showRandom && dropdata.Random) {
                        str2 += langs.GetTranslation("propTips.boxTip5", cfg.TemplateNameLang, dropdata.Data, dropdata.Random / 100000) + "<br>";
                    } else {
                        str2 += langs.GetTranslation("propTips.boxTip4", cfg.TemplateNameLang, dropdata.Data) + "<br>";
                    }
                }
            }
            ...
            if (templateInfo.DescriptionLang != "" && templateInfo.DescriptionLang != undefined) {
    ...
    public get pvpWarFightDic() {
        let campaignTemplateDataDic = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_campaign, "Types5");
        return campaignTemplateDataDic;
    }
    //没有调用
    public get worldBossTemplateDict(): Dictionary {
        let _pvpWarFightDic = new Dictionary();
        let campaignTemplateDataDic = ConfigMgr.Instance.getDicSync(ConfigType.t_s_campaign);
        for (const dicKey in campaignTemplateDataDic) {
            if (campaignTemplateDataDic.hasOwnProperty(dicKey)) {
                let temp: t_s_campaignData = campaignTemplateDataDic[dicKey];
                if (temp.Types >= 1) {
    ...
    public initTemplate() {
        let taskTemplateList = ConfigMgr.Instance.getDicSync(ConfigType.t_s_questtemplate);
        ...
        let offerRewardTemplateList = ConfigMgr.Instance.getDicSync(ConfigType.t_s_rewardtemplate);
    ...
    public getMountTemplateById(id: number): t_s_mounttemplateData {
        let temp = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_mounttemplate, id);
        // for (const dicKey in templateDic) {
        //     if (templateDic.hasOwnProperty(dicKey)) {
    ...
    public getMountsByType(type: number) {
        let temp = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_mounttemplate, "MountType" + type) as t_s_mounttemplateData[];
    ...
    public getRewardInfo(Time: number, Jobs: number, Gender: number): any[] {
        let rewardInfo: any[] = [];
        let temp = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_firstpay, Time + "_" + Jobs + "_" + Gender);
        // for (const dicKey in templateDic) {
        //     if (templateDic.hasOwnProperty(dicKey)) {
    ...
    public getMonthCardRechargeTemplete(Para1: string): t_s_rechargeData {
        ...
        for (const dicKey in templateDic) {
            if (templateDic.hasOwnProperty(dicKey)) {
                let temp: t_s_rechargeData = templateDic[dicKey];
                if (temp.ProductType == ProductType.MONTH_RECHARGE && temp.Para1 == Para1) {
                    rechargeData = temp;
                    break;
                }
            }
    ...
    public getGrowthFundInfoArr(): Array<GrowthFundItemInfo> {
        ...
        for (let index = 0; index < count; index++) {
            ...
            if (gradeDiamond != "")
                ...
                growthFundItemInfo.bindDiamondCount = 0;
    ...
    public getSevenTaskByTaskId(taskId: number): t_s_seventargetData {
        let tempDic = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_seventarget, taskId)
        // for (const key in itemTemplateDic) {
        //     if (itemTemplateDic.hasOwnProperty(key)) {
    ...
    public getConsortiaTempleteByTypeAndLevel(type: number, level: number): t_s_consortialevelData {
        let consortialevelDic = ConfigMgr.Instance.consortialevelDic;
        return consortialevelDic[type + "_" + level];
    }

    public getRuneGemCfgByTypeAndLevel(type: number, level: number, shapeId: number): t_s_runegemData {
    ...
    public getRecoverInfoTemplateByID(Id: number): t_s_recoverData {
        ...
        recoverData = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_recover, Id);
    ...
    public getConsortiaDropTemplateCate(site: number): t_s_dropviewData {
        ...
        dropData = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_dropview, site);
    ...
    getAppellTemplateByID(templateId: number): t_s_appellData {
        let templateDic = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_appell, templateId);
        // for (const dicKey in templateDic) {
        //     if (templateDic.hasOwnProperty(dicKey)) {
        //         let temp: t_s_appellData = templateDic[dicKey];
    ...
    public getGodArriveData(campId: number, gate: number): t_s_campaigndataData {
        ...
        let campaignTemplateDataDic = ConfigMgr.Instance.getDicSync(ConfigType.t_s_campaigndata);
        for (const dicKey in campaignTemplateDataDic) {
            if (campaignTemplateDataDic.hasOwnProperty(dicKey)) {
    ...
    public getAreaRewardByAreaId(areaId: number): SinglePassAreaRewardData {
        ...
        let dropArr: t_s_dropconditionData[] = TempleteManager.Instance.getDropConditionByType(DropCondictionType.AREA_REWAR);
     ...
     * 天穹之径BOSS奖励
     */
    public getSinglePassBossReward(type: number, floorId: number): SinglePassBossRewardData {
        var temp: SinglePassBossRewardData;
        let dropArr: t_s_dropconditionData[] = TempleteManager.Instance.getDropConditionByType(DropCondictionType.BOSS_REWARD);
        for (let index = 0; index < dropArr.length; index++) {
            const element: t_s_dropconditionData = dropArr[index];
            if (element.Para2 && element.Para2.length && element.Para2[0] == floorId && element.Para1 && element.Para1.length && element.Para1[0] == type) {
    ...
    private getRewardInfoByDropId(dropId: number): Array<GoodsInfo> {
        let temp = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_dropitem, "DropId" + dropId)
    ...
    public getSkillPropertyTemplateCate(Id: number): t_s_skillpropertytemplateData {
        let skilltemplateData: t_s_skillpropertytemplateData = null;
        skilltemplateData = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_skillpropertytemplate, Id);
    ...
    public getValueTemplateCate(Id: number): t_s_valueData {
        ...
        let valuetemplateData = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_value, Id);
    ...
    public getValueArrByType(type: number): Array<t_s_valueData> {
        ...
        let valueTemplateProDic = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_value, "Type" + type);
        // for (const dicKey in valueTemplateProDic) {
        //     if (valueTemplateProDic.hasOwnProperty(dicKey)) {
        //         let temp: t_s_valueData = valueTemplateProDic[dicKey];
        //         if (temp.Type == type) {
    ...
    public gameHeroTemplateCate(templateId: number): t_s_herotemplateData {
        let info = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_herotemplate, templateId)
    ...
    public getouterCityShopRandomByMapShopId(MapShopId: number): t_s_outcityshopData {
        let outerCityShopRandomTemplateDic = ConfigMgr.Instance.outerCityShopRandomTemplateDic;
    ...
    public getPassTask(taskType: number, taskId: number): t_s_passchecktaskData {
        let temp: t_s_passchecktaskData = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_passchecktask, taskId);
     ...
     * @returns 
     */
    public getPassCheckItemByGrade(grade: number, index: number): t_s_passcheckData {
        let dic = ConfigMgr.Instance.getDicSync(ConfigType.t_s_passcheck);
        for (const dicKey in dic) {
            if (dic.hasOwnProperty(dicKey)) {
                let temp: t_s_passcheckData = dic[dicKey];
    ...
    public getRemotePetTemplateById(index: number, type: number): RemotePetTurnTemplateInfo {
        let temp = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_remotepet, index + "_" + type);
        return temp;
    }
    ...
    public getAllRemotePetSkill(): t_s_skilltemplateData[] {
        let skillTemplateDic = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_skilltemplate, "MasterType200");
    ...
    public getRemotePetTurns(): t_s_remotepettemplateData[] {
        let dic = ConfigMgr.Instance.getConfigMap(ConfigType.t_s_remotepet) as t_s_remotepet;
        return dic.mDataList;
    }

    //通过技能Id 获得英灵远征关卡信息
    public getRemotePetTurnBySkill(skillId: number): t_s_remotepettemplateData {
        let temp = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_remotepet, "Skill" + skillId) as t_s_remotepettemplateData;
    ...
    public getHonorCfgs(): t_s_honorequipData[] {
        let tempDic = [];
        let itemTemplateDic = ConfigMgr.Instance.getDicSync(ConfigType.t_s_honorequip)
        for (const key in itemTemplateDic) {
            if (itemTemplateDic.hasOwnProperty(key)) {
     ...
     * @returns 
     */
    public geHonorCfgByType(upgradeType: number, level: number): t_s_honorequipData {
        let tempDic = null;
        let itemTemplateDic = ConfigMgr.Instance.getDicSync(ConfigType.t_s_honorequip)
        for (const key in itemTemplateDic) {
            if (itemTemplateDic.hasOwnProperty(key)) {
    ...
    public geHonorLevelByHonor(upgradeType: number, honor: number): t_s_honorequipData {
        let tempDic = null;
        let itemTemplateDic = ConfigMgr.Instance.getDicSync(ConfigType.t_s_honorequip)
        for (const key in itemTemplateDic) {
            if (itemTemplateDic.hasOwnProperty(key)) {
    ...
    public getOutyardRewardByType(type: number): OutyardRewardInfo[] {
        ...
        let dropArr: t_s_dropconditionData[] = TempleteManager.Instance.getDropConditionByType(DropCondictionType.OUTYARD_REWARD);
        for (let index = 0; index < dropArr.length; index++) {
            let element: t_s_dropconditionData = dropArr[index];
            if (element && element.Para1 && element.Para1.length && element.Para1[0] == type) {
                ...
                outyardRewardInfo.goodsInfoArr = this.getRewardInfoByDropId(element.DropId);
                outyardRewardArr.push(outyardRewardInfo);
            }

            if (type == 1) {
                outyardRewardArr = ArrayUtils.sortOn(outyardRewardArr, ["rank"], [ArrayConstant.NUMERIC]);
            }
            else {
                outyardRewardArr = ArrayUtils.sortOn(outyardRewardArr, ["cost"], [ArrayConstant.NUMERIC]);
            }
        }
    ...
    public getRuneActivationBySkillId(skillId: number) {
        let temp = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_runeactivation, skillId) as t_s_runeactivationData;
        return temp;
    }

    /**嘉年华-每日挑战 */
    getCarnivalDailyChallengeTempInfo(taskId: number): t_s_carnivaldailychallengeData {
        let temp = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_carnivaldailychallenge, taskId) as t_s_carnivaldailychallengeData;
        return temp;
    }
    ...
    getCarnivalPointExchangeTempInfo(Id: number): t_s_carnivalpointexchangeData {
        let temp = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_carnivalpointexchange, Id) as t_s_carnivalpointexchangeData;
        return temp;
    }

    /**嘉年华在线奖励 */
    getCarnivalByType(type: number): t_s_carnivalpointexchangeData[] {
        let temp = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_carnivalpointexchange, "Type" + type) as t_s_carnivalpointexchangeData[];
        if (!temp) temp = [];
        return temp;
    }

    /**嘉年华充值有礼 */
    getCarnivalLuckDrawTempList(mType: number, sType: number = 0): t_s_carnivalluckdrawData[] {
        let key = "SonType" + sType;
        let temp = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_carnivalluckdraw, key) as t_s_carnivalluckdrawData[];
        if (!temp) temp = [];
    ...
    /**市场可出售商品列表**/
    getItempricelimit() {
        let dic = ConfigMgr.Instance.getConfigMap(ConfigType.t_s_itempricelimit) as t_s_itempricelimit;
        return dic.mDataList;
    }

    public getArtifactTemplate(templateId: number): t_s_petartifactpropertyData {
        let temp = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_petartifactproperty, templateId) as t_s_petartifactpropertyData;
        return temp;
    }

    public getExtrajobCfg(jobType: number, jobLevel: number): t_s_extrajobData {
        let tempDic = null;
        let itemTemplateDic = ConfigMgr.Instance.getDicSync(ConfigType.t_s_extrajob)
        for (const key in itemTemplateDic) {
            if (itemTemplateDic.hasOwnProperty(key)) {
    ...
    public getExtrajobEquipCfg(equipType: number, equipLevel: number): t_s_extrajobequipData {
        let tempDic = null;
        let itemTemplateDic = ConfigMgr.Instance.getDicSync(ConfigType.t_s_extrajobequip)
        for (const key in itemTemplateDic) {
            if (itemTemplateDic.hasOwnProperty(key)) {
    ...
    public getExtrajobEquipStrenthenCfg(strenLevel: number): t_s_extrajobequipstrengthenData {
        let tempDic = null;
        let itemTemplateDic = ConfigMgr.Instance.getDicSync(ConfigType.t_s_extrajobequipstrengthen)
        for (const key in itemTemplateDic) {
            if (itemTemplateDic.hasOwnProperty(key)) {
    ...
    public getSkillIdByTypeAndGrade(sonType: number,profile:number,grade:number): t_s_skilltemplateData {
        let skilltemplateData: t_s_skilltemplateData = null;
        let skilltemplateDic= ConfigMgr.Instance.getDicSync(ConfigType.t_s_skilltemplate);
        for (const dicKey in skilltemplateDic) {
            if (skilltemplateDic.hasOwnProperty(dicKey)) {
                let temp: t_s_skilltemplateData = skilltemplateDic[dicKey];
                if (temp.MasterType == sonType && temp.SonType == profile && temp.Grades == grade) {
                    skilltemplateData = temp;
                    break;
                }
            }
