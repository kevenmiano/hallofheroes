...
export class SecretEventCom extends FUI_SecretEventCom {
  ...
  set info(value: number) {
    ...
    if (value) {
      ...
      if (this.cfg.BattlePath) {
        ...
        data.urlPath = PathManager.solveRolePath(
          this.cfg.BattlePath.toLocaleLowerCase(),
        );
      ...
      for (let index = 1; index <= PveSecretSceneData.EventOptCnt; index++) {
        let temp = ConfigMgr.Instance.getTemplateByID(
          ...
          this.cfg["Option" + index],
        ) as t_s_secretoptionData;
  ...
  private dealTypeTresure(optInfo: t_s_secretoptionData) {
    ...
    if (con2 && con3) {
      ...
      MessageTipManager.Instance.show(
        LangManager.Instance.GetTranslation("Pve.secretScene.tip.tresure"),
      );
    }
  }
  ...
  private sendOpt(optInfo: t_s_secretoptionData) {
    ...
    Logger.info(
      "[SecretEventCom]选项操作:" + str,
      this.model.secretInfo.secretId,
      optInfo.OptionId,
    );

    SecretManager.Instance.sendEventOpt(
      this.model.secretInfo.secretId,
      optInfo.OptionId,
      this.model.secretInfo.curLayer,
    );
  }
