...
export class ReinforceHandler implements IPackageHandler {
    ...
    public handle(packageIn: PackageIn) {
        ...
        for (i = 0; i < heroLen; i++) {
            ...
            if (heroRole.type == RoleType.T_NPC_BOSS) {
                Logger.battle("boss增援:" + heroRole.heroInfo.templateInfo.TemplateNameLang);
        ...
        if (bossArr.length > 0 && BattleManager.Instance.battleUIView) {
            if (BattleManager.Instance.battleUIView && BattleManager.Instance.battleUIView.getRoleShowViewII())
                BattleManager.Instance.battleUIView.getRoleShowViewII().bossReinforce(bossArr);
        }
        ...
        NotificationManager.Instance.dispatchEvent(BattleEvent.REINFORCE,herosReinforce);
    ...
    public static addRole(role) {
        ...
        if (role.face == FaceType.RIGHT_TEAM) {
            ...
            new MovePointAction(role.livingId, temp, 25, 0, false, false, true, false);
        }
        else {
            role.point = new Point(temp.x - 700, temp.y);
            new MovePointAction(role.livingId, temp, 25, 0, false, false, true, false);
