...
export default class UIZhuanHuan extends BaseFguiCom {
  ...
  constructor(comp: fgui.GComponent) {
    ...
    this.resolveList.itemRenderer = Laya.Handler.create(
      this,
      this.renderListItem,
      null,
      false,
    ...
    this._rTxtDesc = this.itemSource.getChild(
      "rTxtDesc",
  ...
  public sendSwitch(check: boolean = false) {
    ...
    if (this.itemTarget && this.itemCurrent) {
      ForgeSocketOutManager.sendSwitch(
        this.itemTarget.info.templateId,
        this.itemCurrent.info.templateId,
        this.itemCurrent.info.count,
      );
    }
  }
  ...
  private showSwitchAlert(): boolean {
    ...
    if (outdate) {
      ...
      SimpleAlertHelper.Instance.Show(
        ...
        null,
        null,
        this.switchAlertBack.bind(this),
      );
    }
    return outdate;
  }
  ...
  private __bagItemUpdateHandler(info: GoodsInfo) {
    Logger.log(
      "[UIZhuanHuan]__bagItemUpdateHandler",
  ...
  private __bagItemDeleteHandler(info: GoodsInfo) {
    Logger.log(
      "[UIZhuanHuan]__bagItemDeleteHandler",
      info.pos + "_" + info.objectId + "_" + info.bagType,
    );
    let item: StoreIntensifyCell =
      this._itemDic[info.pos + "_" + info.objectId + "_" + info.bagType];
  ...
  private updateView(value: GoodsInfo) {
    ...
    if (targetTransformId > 0) {
      this._switchTemp = ConfigMgr.Instance.getTemplateByID(
    ...
    if (VIPManager.Instance.model.vipInfo.IsVipAndNoExpirt) {
      //特权专享折扣
      let grade = VIPManager.Instance.model.vipInfo.VipGrade;
      let hasPrivilege = VIPManager.Instance.model.isOpenPrivilege(
        VipPrivilegeType.DISCOUNT,
        grade,
      );
      if (!hasPrivilege) {
        vipPrivilegeCount = 1;
      } else {
        let item = TempleteManager.Instance.getPrivilegeTempletesByTypeLevel(
          VipPrivilegeType.DISCOUNT,
          grade,
        );
        if (item) {
    ...
    this.txtCost.text = Math.floor(
      this._switchTemp.CostGold * value.count * vipPrivilegeCount,
    ).toString();
    ...
    this.txtAll.color = this.txtCost.color = CommonUtils.changeColor(
      this._switchTemp.CostGold * value.count * vipPrivilegeCount,
  ...
  private selectDefItem() {
    ...
    this.itemCurrent = this.resolveList.getChildAt(
      this.resolveList.selectedIndex,
    ) as ItemWithSelectEffect;
    this.setCurItemName();
  }
