...
export class SpaceSocketOutManager {
	...
	public static get Instance(): SpaceSocketOutManager {
		if (!SpaceSocketOutManager._instance) SpaceSocketOutManager._instance = new SpaceSocketOutManager();
	...
	public sendYearBoxCollect(nodeId: number) {
		if (ArmyManager.Instance.thane.grades < OpenGrades.SPACE_COLLECT) {
			MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("map.sapce.view.frame.DialogOptionItem.lifeskillTips"));
			return;
		}
