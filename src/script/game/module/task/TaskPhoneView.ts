...
export default class TaskPhoneView extends FUI_TaskPhoneView {
    ...
    public set taskTemp(value: TaskTemplate) {
        if (!value || value.conditionList[0].CondictionType != TaskConditionType.PHONE_CHECK)
            return;
        this._temp = value;
        if (this._temp) {
            if (this._temp.taskInfo.checkNumber == 0) {
                TaskSocketManager.sendCheckMobileNumber();
            } else if (this._temp.taskInfo.checkNumber == -1) {//发送区域
                this.setAreaView(false);
            } else if (this._temp.taskInfo.checkNumber == 1) {//验证区域
                this.setAreaView(true);
            }
            this._temp.taskInfo.addEventListener(Laya.Event.CHANGE, this.refresh, this);
            if (this._temp.taskInfo.phoneStr && this._temp.taskInfo.phoneStr != "") {
                this.phoneNumber.text = this._temp.taskInfo.phoneStr;
            }
        }
        this.refresh();
        this.phoneDescTxt.text = StringHelper.repHtmlTextToFguiText(this._temp.DetailLang);
    ...
    private refresh() {
        ...
        if (this._temp) {
            ...
            if (this.sendTimeUIBtn) {
                this.sendTimeUIBtn.title = "" + this._temp.taskInfo.sendTime + LangManager.Instance.GetTranslation("task.view.TaskMobilePhoneView.Text1");
            ...
            if (this.checkedTimeBtn) {
                this.checkedTimeBtn.title = "" + this._temp.taskInfo.checkTime + LangManager.Instance.GetTranslation("task.view.TaskMobilePhoneView.Text1");
    ...
    private setCheckedView(visible1: boolean) {
        if (this.checkedTimeBtn) this.checkedTimeBtn.visible = visible1 && this._isSended;
        this.checkUIBtn.enabled = !visible1;
    }

    private setSendedView(visible2: boolean) {
        if (this.sendTimeUIBtn) this.sendTimeUIBtn.visible = visible2 && !this._isSended;
        this.sendUIBtn.enabled = !visible2;
    }

    private initEvent() {
        ServerDataManager.listen(S2CProtocol.U_C_CHECK_SEND, this, this.__setFirstView);
    ...
    private removeEvent() {
        ServerDataManager.cancel(S2CProtocol.U_C_CHECK_SEND, this, this.__setFirstView);
        ...
        if (this._temp && this._temp.taskInfo) {
            this._temp.taskInfo.removeEventListener(Laya.Event.CHANGE, this.refresh, this);
        }
    }
    ...
    private __setFirstView(pkg: PackageIn) {
        ...
        if (this._temp.taskInfo.checkNumber == -1)//未发送号码
        {
            this._isSended = false;
        } else if (this._temp.taskInfo.checkNumber == 1) {//已发送
            this._isSended = true;
        }
