...
export default class SortWnd extends BaseWindow {
  ...
  private initView() {
    ...
    var titleText = LangManager.Instance.GetTranslation(
      "mainBar.SmallMapBar.rankingBtnTipData",
    );
  ...
  private updateRankTime() {
    ...
    let text = LangManager.Instance.GetTranslation(
      "sort.view.SortRightView.lastUpdateTime",
    );
    let time = this.smodel.createDate
      ? DateFormatter.format(this.smodel.createDate, "MM-DD hh:mm")
      : "";
  ...
  private __onSelectedChangeHandler() {
    ...
    this.para1
      .setVar(
        ...
        LangManager.Instance.GetTranslation(
          "PetChallengeInfoView.rankingLabel",
        ),
      )
      .flushVars();
    ...
    switch (this.smodel.currentSelected) {
      case SortModel.SELF_LEVEL:
        ...
        this.para2
          .setVar("text", LangManager.Instance.GetTranslation("public.grade"))
          .flushVars();
        str = LangManager.Instance.GetTranslation(
          "task.view.TaskRewardItem.iconTips01",
        );
      ...
      case SortModel.SELF_POW:
        this.para1
          .setVar(
            "value",
            this.playerInfo.playerOrdeInfo.getFightCapacityOrder(),
          )
          .flushVars();
        this.para2.visible = false;
        str = LangManager.Instance.GetTranslation("public.playerInfo.ap");
        this.para3.text =
          str + ": " + this.playerInfo.fightingCapacity.toString();
        break;
      case SortModel.CONSORTIA_LEVEL:
        this.para1
          .setVar(
            "text",
            LangManager.Instance.GetTranslation(
              "consortia.view.myConsortia.ConsortiaActivityPage.consortiaRank",
            ),
          )
          .flushVars();
        this.para2
        ...
        if (this.playerInfo.consortiaID == 0) {
          ...
          this.para1
            .setVar(
              "value",
              this.consortiaInfo.orderInfo
                ? this.consortiaInfo.orderInfo.gradeOrder.toString()
                : "0",
            )
            .flushVars();
          this.para2
            .setVar("value", this.consortiaInfo.levels.toString())
            .flushVars();
        ...
        if (this.playerInfo.consortiaID == 0) {
          ...
          this.para1
            .setVar(
              "value",
              this.consortiaInfo.orderInfo
                ? this.consortiaInfo.orderInfo.fightPowerOrder.toString()
                : "0",
            )
            .flushVars();
          this.para3.text =
            str + ": " + this.consortiaInfo.fightPower.toString();
          this.para2.visible = false;
        }
        break;
      case SortModel.SELF_HONOUR:
        this.para1
          .setVar("value", this.playerInfo.playerOrdeInfo.getHonourOrder())
          .flushVars();
        this.para2.visible = false;
        str = LangManager.Instance.GetTranslation(
          "sort.view.MemberTitleView.honour",
        );
      ...
      case SortModel.SELF_CHARMS:
        ...
        str = LangManager.Instance.GetTranslation(
          "sort.view.MemberTitleView.charms",
        );
      ...
      case SortModel.SELF_SOULSCORE:
        this.para1
          .setVar("value", this.playerInfo.playerOrdeInfo.getSoulScoreOrder())
          .flushVars();
        str = LangManager.Instance.GetTranslation(
          "sort.view.MemberTitleView.soulScore",
        );
        this.para3.text =
          str + ": " + MountsManager.Instance.mountInfo.soulScore.toString();
        this.para2.visible = false;
        break;
      case SortModel.PET_POWER:
        this.para2.visible = false;
        var sortData: SortData = this.smodel.isCross
          ? this.smodel.petRankingCross
          : this.smodel.petRanking;
  ...
  private layout(type: string) {
    ...
    for (let index = 0; index < 7; index++) {
      var txt: fgui.GTextField = this.rankTitle.getChild(
        "title" + index,
      ).asTextField;
      txt.text = "";
    }
  ...
  private getPlayerCrossOrder(): string {
    ...
    var order: string = LangManager.Instance.GetTranslation(
      "colosseum.view.ColosseumPlayerItem.newPeople",
    );
  ...
  private initTabList() {
    ...
    this.createTabData(
      SORT_TAB.LEVLE,
      LangManager.Instance.GetTranslation("Sort.Role.Grade"),
      "Tab_Dia2Y_Nor",
      "Tab_Dia2Y_Sel",
      this.rankTabData,
    );
    this.createTabData(
      SORT_TAB.POW,
      LangManager.Instance.GetTranslation("Sort.Role.Battle"),
      "Tab_Dia2Y_Nor",
      "Tab_Dia2Y_Sel",
      this.rankTabData,
    );
    this.createTabData(
      SORT_TAB.CONSORTIA_LEVEL,
      LangManager.Instance.GetTranslation("Sort.Role.LaborGrade"),
      "Tab_Dia2Y_Nor",
      "Tab_Dia2Y_Sel",
      this.rankTabData,
    );
    this.createTabData(
      SORT_TAB.CONSORTIA_POW,
      LangManager.Instance.GetTranslation("Sort.Role.LaborPow"),
      "Tab_Dia2Y_Nor",
      "Tab_Dia2Y_Sel",
      this.rankTabData,
    );
    this.createTabData(
      SORT_TAB.HONOUR,
      LangManager.Instance.GetTranslation("Sort.Role.Honor"),
      "Tab_Dia2Y_Nor",
      "Tab_Dia2Y_Sel",
      this.rankTabData,
    );
    this.createTabData(
      SORT_TAB.CHARMS,
      LangManager.Instance.GetTranslation("Sort.Role.Charm"),
      "Tab_Dia2Y_Nor",
      "Tab_Dia2Y_Sel",
      this.rankTabData,
    );
    this.createTabData(
      SORT_TAB.SOUL,
      LangManager.Instance.GetTranslation("Sort.Role.Soul"),
      "Tab_Dia2Y_Nor",
      "Tab_Dia2Y_Sel",
      this.rankTabData,
    );
    this.createTabData(
      SORT_TAB.PETPOWER,
      LangManager.Instance.GetTranslation("Sort.Role.PetPow"),
      "Tab_Dia2Y_Nor",
      "Tab_Dia2Y_Sel",
      this.rankTabData,
    );
    if (ComponentSetting.WARLORD) {
      this.createTabData(
        SORT_TAB.WARLORDS,
        LangManager.Instance.GetTranslation("Sort.Role.Warlords"),
        "Tab_Dia2Y2_Nor",
        "Tab_Dia2Y2_Sel",
        this.rankTabData,
      );
  ...
  private initAreaTabList() {
    ...
    this.createTabData(
      ZONE_TAB.LOCAL,
      LangManager.Instance.GetTranslation("SortWnd.currentServerTxt"),
      "Tab_Dia2_Nor",
      "Tab_Dia2_Sel",
      this.rankZoneTabData,
    );
    this.createTabData(
      ZONE_TAB.ALL,
      LangManager.Instance.GetTranslation("SortWnd.allServerTxt"),
      "Tab_Dia2_Nor",
      "Tab_Dia2_Sel",
      this.rankZoneTabData,
    );
  ...
  private initDateTabList() {
    ...
    this.createTabData(
      DATE_TAB.DAY,
      LangManager.Instance.GetTranslation("Sort.Role.DailyGain"),
      "Tab_Menu1_Nor",
      "Tab_Menu1__Sel",
      this.rankDateTabData,
    );
    this.createTabData(
      DATE_TAB.WEEK,
      LangManager.Instance.GetTranslation("Sort.Role.WeeklyGain"),
      "Tab_Menu1_Nor",
      "Tab_Menu1__Sel",
      this.rankDateTabData,
    );
    this.createTabData(
      DATE_TAB.ALL,
      LangManager.Instance.GetTranslation("Sort.Role.SumGain"),
      "Tab_Menu1_Nor",
      "Tab_Menu1__Sel",
      this.rankDateTabData,
    );
  ...
  private initWarsTabList() {
    ...
    this.createTabData(
      WARLORDS_TAB.WARRIOR,
      LangManager.Instance.GetTranslation("Sort.Role.Warrior"),
      "Tab_Dia2_Nor",
      "Tab_Dia2_Sel",
      this.rankWarlordsTabData,
    );
    this.createTabData(
      WARLORDS_TAB.WIZARD,
      LangManager.Instance.GetTranslation("Sort.Role.Wizard"),
      "Tab_Dia2_Nor",
      "Tab_Dia2_Sel",
      this.rankWarlordsTabData,
    );
    this.createTabData(
      WARLORDS_TAB.HUNTER,
      LangManager.Instance.GetTranslation("Sort.Role.Archer"),
      "Tab_Dia2_Nor",
      "Tab_Dia2_Sel",
      this.rankWarlordsTabData,
    );
  ...
  private createTabData(
    ...
    text: string,
    iconstr: string,
    selectedIconstr: string,
    targetArray: Array<any> = null,
  ): any {
  ...
  private addEvent() {
    ...
    this.rankTablist.on(
      Laya.Event.CLICK,
      this,
      this.__onChangeHandler.bind(this),
    );
    this.rankTablist.itemRenderer = Laya.Handler.create(
      this,
      this._rankTabRender,
      null,
      false,
    ...
    this.zoneTablist.on(
      Laya.Event.CLICK,
      this,
      this.__onzoneTabHandler.bind(this),
    );
    this.zoneTablist.itemRenderer = Laya.Handler.create(
      this,
      this._zoneTabRender,
      null,
      false,
    ...
    this.dateTablist.on(
      Laya.Event.CLICK,
      this,
      this.__onDateTabHandler.bind(this),
    );
    ...
    this.contentList.on(
      Laya.Event.CLICK,
      this,
      this.__onContentCellHandler.bind(this),
    );
    this.contentList.itemRenderer = Laya.Handler.create(
      this,
      this._contentCellabRender,
      null,
      false,
    );
    this.warLordsTablist.on(
      Laya.Event.CLICK,
      this,
      this.__onWarlordsTabHandler.bind(this),
    );
    this.warLordsTablist.itemRenderer = Laya.Handler.create(
      this,
      this.__onWarlordsTabRender,
      null,
      false,
    );
    this.warlordsContentList.on(
      Laya.Event.CLICK,
      this,
      this.__onWarlordsContentHandler.bind(this),
    );
    this.warlordsContentList.itemRenderer = Laya.Handler.create(
      this,
      this.__onWarlordsContentRender,
      null,
      false,
    );
    if (this.smodel)
      this.smodel.addEventListener(
        SortEvent.SELECTE_LIST_CHANG,
        this.__onSelectedChangeHandler,
        this,
      );
    if (this.smodel)
      this.smodel.addEventListener(
        SortEvent.WARLORD_RANK_DATA_UPDATE,
        this.refresh,
        this,
      );
  }
  ...
  private removeEvent() {
    ...
    if (this.smodel)
      this.smodel.removeEventListener(
        SortEvent.SELECTE_LIST_CHANG,
        this.__onSelectedChangeHandler,
        this,
      );
  ...
  private refreshwarlordsTitle() {
    ...
    var xx: number = 9;
    for (let index = 0; index < 7; index++) {
      var txt: fgui.GTextField = this.warlordsRankTitle.getChild(
        "title" + index,
      ).asTextField;
      txt.text = "";
    }
  ...
  private refreshAvatarView() {
    if (this._curSelectedInfo) {
      this.warloardsTitle.text =
        this._curSelectedInfo.army.baseHero.serviceName +
        this._curSelectedInfo.army.baseHero.nickName;
    } else {
      this.warloardsTitle.text = "";
    }
  }
  ...
  private refreshListView() {
    var totalList: Array<any> =
      this.smodel.warlordsDic[this.smodel.currentSelectedJob];
    let temp = [];
  ...
  private __onChangeHandler(evt) {
    AudioManager.Instance.playSound(SoundIds.CONFIRM_SOUND);
    let selectItem = this.rankTablist.getChildAt(
      this.rankTablist.selectedIndex,
    ).asButton;
