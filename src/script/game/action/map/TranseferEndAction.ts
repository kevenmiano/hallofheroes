...
export class TranseferEndAction extends MapBaseAction {
  ...
  public prepare() {
    let msg: PlayerMovieMsg = this._pkg.readBody(
      PlayerMovieMsg,
    ) as PlayerMovieMsg;
  ...
  public update() {
    ...
    if (this._node && this._count == 1) {
      CampaignManager.Instance.mapView.moveCenterPoint(
        new Laya.Point(this._node.x, this._node.y),
      );
  ...
  private armyViewPos() {
    if (this.selfArmyView) {
      ...
      this.selfArmyView.avatarView.angle = HeroAvatarViewHelper.twoPointAngle(
        vx,
        vy,
        this.selfArmyView.x,
        this.selfArmyView.y,
      );
      TweenMax.to(this.selfArmyView, 0.6, {
        x: vx,
        y: vy,
        alpha: 1,
        onComplete: this.transeferCallBack.bind(this),
      });
    } else {
  ...
  private get selfArmyView(): any {
    ...
    if (
      cModel &&
      cModel.selfMemberData &&
      CampaignManager.Instance.controller
    ) {
      try {
        var armyView: object = CampaignManager.Instance.controller.getArmyView(
          cModel.selfMemberData,
        );
        return armyView;
      } catch {
