...
export default class MineralView extends BaseFguiCom {
    ...
    private addEvent() {
        PlayerManager.Instance.addEventListener(PlayerEvent.PLAYER_INFO_UPDATE, this.__updateHandler, this);
        if (this.mineralModel) this.mineralModel.addEventListener(CampaignEvent.UPDATE_MINERAL_INFO, this.__updateHandler, this);
    ...
    private get shopControl(): ShopControler {
        return FrameCtrlManager.Instance.getCtrl(EmWindow.MineralShopWnd) as ShopControler;
    }

    private __updateHandler() {
        if (!this.mineralModel || !this.selfCarInfo) {
            Logger.log("-----------------___________----------------->>>找不到自身矿车信息");
            return;
        }
        //剩余采集次数
        if (this.selfCarInfo.is_own == 1) {//有矿车
            this.txt_leftCount.text = LangManager.Instance.GetTranslation("map.campaign.view.ui.mineral.countText", 5 - this.selfCarInfo.pick_count);
            if (this.carBtn) { this.carBtn.title = LangManager.Instance.GetTranslation("mineraView.handInCar") + LangManager.Instance.GetTranslation("public.parentheses1", this.getHandCount + "/" + this.mineralModel.maxCount); }
            this._currentStatus = MineralView.HAND_IN;
        } else if (this.selfCarInfo.is_own == 0) {
            this.txt_leftCount.text = LangManager.Instance.GetTranslation("map.campaign.view.ui.mineral.countText", 0);
            if (this.carBtn) { this.carBtn.title = LangManager.Instance.GetTranslation("mineraView.getCar") + LangManager.Instance.GetTranslation("public.parentheses1", this.getCarCount + "/" + this.mineralModel.maxCount); }
            this._currentStatus = MineralView.GET_CAR;
        }
        if (this.mineralModel.activeTime > 0) {//n被收益
            this.title.text = LangManager.Instance.GetTranslation("map.campaign.view.ui.mineral.multititleText", this.mineralModel.multiple);
            this.txt_doubleTime.text = LangManager.Instance.GetTranslation("map.campaign.view.ui.mineral.titleText2", this.mineralModel.multiple, "[color=#39a82d]" + Math.ceil(this.mineralModel.activeTime / 60) + "[/color]");
        } else {//普通收益
            this.title.text = LangManager.Instance.GetTranslation("map.campaign.view.ui.mineral.titleText1");
            this.txt_doubleTime.text = LangManager.Instance.GetTranslation("map.campaign.view.ui.mineral.doubleText");
        }
        //矿石
        this.txt_mineral.text = LangManager.Instance.GetTranslation("map.campaign.view.physics.MineralCarView.minerals", this.selfCarInfo.minerals + "/200");
        //紫晶
        this.txt_stone.text = LangManager.Instance.GetTranslation("map.campaign.view.physics.MineralCarView.mineralsCount", this.playerInfo.mineral);
    }

    private offEvent() {
        PlayerManager.Instance.removeEventListener(PlayerEvent.PLAYER_INFO_UPDATE, this.__updateHandler, this);
        if (this.mineralModel) this.mineralModel.removeEventListener(CampaignEvent.UPDATE_MINERAL_INFO, this.__updateHandler, this);
    ...
    private __onHelpBtnHandler() {
        let title = LangManager.Instance.GetTranslation("map.campaign.view.ui.mineral.HelpFrameTitle");
        let content = LangManager.Instance.GetTranslation("map.campaign.view.ui.mineral.HelpFrameText");
        UIManager.Instance.ShowWind(EmWindow.Help, { title: title, content: content });
    }

    private _carBtnHandler() {
        var nextNode: CampaignNode;
        if (this._currentStatus == MineralView.GET_CAR) {//领取矿车
            nextNode = CampaignManager.Instance.mapModel.getMineraCardNode();

        } else if (this._currentStatus == MineralView.HAND_IN) {//提交矿车
