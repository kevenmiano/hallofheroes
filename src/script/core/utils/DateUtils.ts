...
export default class DateUtils {
   ...
   * xx:xx:xx
   */
  public static FormatTimeString(str: string, serverTime: number): number {
    if (!str) {
      return 0;
    }
    let arr = str.split(":");
    ...
    let m = arr[1] || 0;
    let s = arr[2] || 0;
    let date = new Date(serverTime * 1000 + 1);
  ...
  public static formatMiniDateTime(mdt) {
    return (
      DateUtils.MINI_DATE_TIME_BASE +
      (mdt & 0x7fffffff) * DateUtils.MS_PER_SECOND
    );
  }
   ...
   * @returns string
   */
  public static getFormatBySecond(
    second,
    type: number = 1,
    showLength: number = 2,
  ) {
  ...
  private static FormatType5(ms, showLength = 2, typeStr = null) {
    ...
    if (showLength >= 4) {
      ...
      if (d > 0) {
        ms -= d * this.MS_PER_DAY;
        result +=
          (result.length > 0 ? this.formatTimeNum(d) : d) +
          (typeStr ? typeStr : "天");
      }
    }
    if (showLength >= 3) {
      ...
      if (h > 0) {
        ms -= h * this.MS_PER_HOUR;
        result +=
          (result.length > 0 ? this.formatTimeNum(h) : h) +
          (typeStr ? typeStr : "小时");
      }
    }
    if (showLength >= 2) {
      ...
      if (m > 0) {
        ms -= m * this.MS_PER_MINUTE;
        result +=
          (result.length > 0 ? this.formatTimeNum(m) : m) +
          (typeStr ? typeStr : "分");
      }
    }
  ...
  public static format_6(ms) {
    ...
    var second = date.getSeconds();
    return (
      this.formatTimeNum(hours) +
      ":" +
      this.formatTimeNum(minute) +
      ":" +
      this.formatTimeNum(second)
    );
  }
  /**
   * 格式7  X天/X小时/<1小时
  ...
  public static format_17(time: number): string {
    ...
    if (time >= DateUtils.MS_PER_DAY) {
      ...
      let h = Math.floor((time % DateUtils.MS_PER_DAY) / DateUtils.MS_PER_HOUR);
      let m = Math.floor(
        (time % DateUtils.MS_PER_HOUR) / DateUtils.MS_PER_MINUTE,
      );
      let s = Math.floor(
        (time % DateUtils.MS_PER_MINUTE) / DateUtils.MS_PER_SECOND,
      );
      str = d + "天" + h + "时" + m + "分" + s + "秒";
      return str;
    } else {
  ...
  public static compareOneDayTime(timeStr, h, m, s?): number {
    ...
    let T1 = parseInt(
      this.formatTimeNum(h) +
        this.formatTimeNum(m) +
        this.formatTimeNum(s || 0),
    );
    let T2 = parseInt(
      this.formatTimeNum(h1) +
        this.formatTimeNum(m1) +
        this.formatTimeNum(s1 || 0),
    );
    return T1 - T2;
  }
  ...
  public static MINI_DATE_TIME_BASE =
    Date.UTC(2010, 0) +
    new Date().getTimezoneOffset() * DateUtils.MS_PER_MINUTE;
