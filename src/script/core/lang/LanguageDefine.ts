...
export function hasLanguage(
  ...
  for (const key in GAME_LANG) {
    if (Object.prototype.hasOwnProperty.call(GAME_LANG, key)) {
      ...
      if (
        isMathLanguage(element.key, languageName) ||
        isMathLanguage(element.appkey, languageName)
      ) {
        if (GAME_LIMIT.indexOf(element.key) == -1) {
          //不在配置中
          return false;
        }
        return true;
...
export function getLangIndex(langKey: string): number {
  ...
  for (const key in GAME_LANG) {
    if (Object.prototype.hasOwnProperty.call(GAME_LANG, key)) {
      element = GAME_LANG[key];
      if (
        isMathLanguage(element.key, langKey) ||
        isMathLanguage(element.appkey, langKey)
      ) {
...
export function isCNLanguage(): boolean {
  ...
  let ret =
    isMathLanguage(langCfg.key, "zhcn") ||
    isMathLanguage(langCfg.key, "zhtw") ||
    isMathLanguage(langCfg.appkey, "Hans") ||
    isMathLanguage(langCfg.appkey, "Hant");
  return ret;
}
 ...
 * @returns boolean
 */
export function isMathLanguage(
  sourceLang: string,
  targetLang: string,
): boolean {
...
/**解析读取多语言 */
export function getMultiLangList(
  text: string,
