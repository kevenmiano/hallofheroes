...
export class TattooReinforceWnd extends BaseWindow {
  ...
  private __onBaptizeBtnClick(): void {
    ...
    let isAlert = SharedManager.Instance.checkIsExpired(
      SharedManager.Instance.tattooBaptizeCheckDate,
    );
    if (!isAlert) {
      this.alertBaptizeUseDiamond();
      return;
  ...
  private alertBaptizeUseDiamond(): void {
    if (this.tattooModel.notAlertBaptizeUseDiamond) {
      ...
      let content: string = LangManager.Instance.GetTranslation(
        "tattoo.TattooPopFrame.tattoo1",
        this.tattooModel.tattooBaptizePrice,
      );
      UIManager.Instance.ShowWind(EmWindow.TodayNotAlert, {
        content: content,
        backFunction: this.alertBaptizeUseDiamondBack.bind(this),
      });
    }
  }

  private alertBaptizeUseDiamondBack(
    ...
    if (check2) {
      if (
        ...
        this.tattooController.sendRefresh(
  ...
  private __onUpdateBtnClick(): void {
    ...
    let isAlert = SharedManager.Instance.checkIsExpired(
      SharedManager.Instance.tattooUpdateCheckDate,
    );
    if (!isAlert) {
      this.alertUpdateUseDiamond();
      return;
  ...
  private alertUpdateUseDiamond(): void {
    ...
    if (
      hole.oldStep >= this.tattooModel.coreStep &&
      this._oldAddValue >=
        this.tattooModel.getProtertyValueMaxByStep(hole.oldStep)
    ) {
      let content: string = LangManager.Instance.GetTranslation(
        "tattoo.TattooPopFrame.tattoo2",
      );
      UIManager.Instance.ShowWind(EmWindow.TodayNotAlert, {
        content: content,
        backFunction: this.alertUpdateUseDiamondBack.bind(this),
        state: 2,
      });
    } else {
      this.alertUpdateUseDiamondBack();
  ...
  private alertUpdateUseDiamondBack(check: boolean = false): void {
    ...
    this.tattooController.sendUpgrade(
      this.tattooController.tattooModel.currHoleIndex,
    );
  }
  ...
  private alertUpdateAdvanceUseDiamond(): void {
    ...
    let hole: TattooHole = this.holes[this._index];
    if (
      this._oldAddValue + 10 >
      this.tattooModel.getProtertyValueMaxByStep(hole.oldStep)
    ) {
      let content: string = LangManager.Instance.GetTranslation(
        "tattoo.TattooPopFrame.tattoo3",
      );
      UIManager.Instance.ShowWind(EmWindow.TodayNotAlert, {
        content: content,
        backFunction: this.alertUpdateAdvanceUseDiamondBack.bind(this),
        state: 2,
      });
    } else {
      this.alertUpdateAdvanceUseDiamondBack();
  ...
  private alertUpdateAdvanceUseDiamondBack(check: boolean = false): void {
    ...
    this.tattooController.sendSeniorUpgrade(
      this.tattooController.tattooModel.currHoleIndex,
    );
  }
  ...
  public refresh(): void {
    ...
    if (
      (this.tattooModel.lastOpType == RoleCtrl.OP_BAPTIZE ||
        this.tattooModel.lastOpType == RoleCtrl.OP_INFO) &&
      (hole.newAddProperty > 0 || hole.newReduceProperty > 0) &&
    ...
    if (
      hole.newAddProperty > 0 &&
      (this.tattooModel.lastOpType == RoleCtrl.OP_REFRESH ||
        this.tattooModel.lastOpType == RoleCtrl.OP_ADVANCE_REFRESH)
    ) {
      ...
      this.txt_update_property_value_new_0.text =
        (deltaAdd > 0 ? "+" : "") + (deltaAdd != 0 ? deltaAdd : "");
      this.txt_update_property_value_new_0.color =
        deltaAdd > 0 ? "#71F000" : "#FF2E2E";
      let deltaReduce: number = hole.newReduceValue - hole.lastReduceValue;
      this.txt_update_property_value_new_1.text =
        (deltaReduce > 0 ? "+" : "") + (deltaReduce != 0 ? deltaReduce : "");
      this.txt_update_property_value_new_1.color =
        deltaReduce > 0 ? "#71F000" : "#FF2E2E";
    }

    this.txt_material_update.text = this.tattooModel.tattooConsumeNum + "";
    this.txt_material_update_advance.text =
      this.tattooModel.tattooConsumeNum + "";
    // this.txt_tips_update.text = this.getTattooAddRange(hole);
    this.txt_tips_update_advance.text = LangManager.Instance.GetTranslation(
      "tattoo.TattooPopFrame.AddPropertyTips2",
    );
  ...
  private refreshPoint(): void {
    ...
    this.btn_baptize.enabled = this.tattooModel.canBaptize(
      this.holes[this.tattooModel.currHoleIndex],
    );
    let hasNum: number = GoodsManager.Instance.getBagCountByTempId(
      BagType.Player,
      TattooModel.DragonCrystalId,
    );
    this.txt_material_update.color =
      hasNum >= this.tattooModel.tattooConsumeNum ? "#FFECC6" : "#FF0000";
    let hasNum2: number = GoodsManager.Instance.getBagCountByTempId(
      BagType.Player,
      TattooModel.DragonCrystalId2,
    );
    this.txt_material_update_advance.color =
      hasNum2 >= this.tattooModel.tattooConsumeNum ? "#FFECC6" : "#FF0000";
  }
