...
export default class WarlordsCheckRewardWnd extends BaseWindow {
    ...
    private initEvent() {
        ...
        this.maxRewardList.itemRenderer = Laya.Handler.create(this, this.renderMaxListItem, null, false);
    }
    ...
    private initData() {
        ...
        for (var i: number = 0; i < maxCount; i++) {
            ...
            goodsRewardInfo.tipData = LangManager.Instance.GetTranslation("warlords.WarlordsCheckRewardFrame.privilege" + i);
            if (i == 0) {
                goodsRewardInfo.picUrl = IconFactory.getCommonIconPath("/shadoweffect.png");
            } else if (i == 1) {
                goodsRewardInfo.picUrl = IconFactory.getCommonIconPath("/callmonster.png");
            } else if (i == 2) {
                goodsRewardInfo.picUrl = IconFactory.getCommonIconPath("/lordbless.png");
            }
            if (i != 0) {
