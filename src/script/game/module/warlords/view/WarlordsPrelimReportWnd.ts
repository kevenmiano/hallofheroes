...
export default class WarlordsPrelimReportWnd extends BaseWindow {
  ...
  private initEvent() {
    this.warlordsModel.addEventListener(
      WarlordsEvent.ACCESS_FINALLIST_UPDATE,
      this.accessFinalListUpdateHandler,
      this,
    );
  ...
  private refresh() {
    if (
      this.warlordsModel.selfInfo.sort > 0 &&
      this.warlordsModel.selfInfo.sort <= 100
    ) {
      var group: string = WarlordsModel.checkIsTempleGroup(
        this.warlordsModel.selfInfo.fightingCapacityRank,
      )
      ...
      var time: string = this.warlordsModel.getMatchDate(3)
        ? DateFormatter.format(
            this.warlordsModel.getMatchDate(3),
            "MM-DD hh:mm",
          )
        : "";
      this.c1.selectedIndex = 1;
      this.MsgText2.text = LangManager.Instance.GetTranslation(
        "warlords.room.WarlordsPrelimReportView.MsgText1",
        this.warlordsModel.selfInfo.sort,
        this.warlordsModel.selfInfo.fightingCapacityRank,
        group,
        time,
      );
