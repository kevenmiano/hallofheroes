...
export class BuyFrame2 extends BaseWindow {
  ...
  private initView() {
    ...
    this.checkbox_useBind.title = LangManager.Instance.GetTranslation(
  ...
  protected __confirmHandler() {
    ...
    if (this._data) {
      if (this._itemInfo) {
        ...
        if (
          this._itemInfo.itemTemInfo.MinLevel >
          ArmyManager.Instance.thane.grades
        ) {
          //等级不满足
          MessageTipManager.Instance.show(
            LangManager.Instance.GetTranslation(
              "ExchangeRandomItem.txt_openDescible",
              this._itemInfo.itemTemInfo.MinLevel,
            ),
          );
          return;
  ...
  private buy() {
    ...
    if (this._itemInfo.itemTemInfo.Type == OuterCityShopModel.RANDOM_SHOP) {
      //随机兑换
      OuterCityShopManager.instance.sendAction(
        2,
        this._itemInfo.itemTemInfo.Type,
        this._itemInfo.index,
        payType,
      );
    } else if (
      this._itemInfo.itemTemInfo.Type == OuterCityShopModel.EXCHANGE_SHOP
    ) {
      //积分兑换
      ...
      OuterCityShopManager.instance.sendAction(
        2,
        this._itemInfo.itemTemInfo.Type,
        this._itemInfo.itemTemInfo.MapShopId,
        payType,
        buyCount,
      );
    }
  ...
  private get checkGoods(): boolean {
    ...
    for (var i: number = 0; i < this._needValueCount.length; i++) {
      ...
      if (this._needValueItemId[i] == -6000) {
        if (
          OuterCityShopManager.instance.model.consumePoints <
          this._needValueCount[i] * buyCount
        ) {
          MessageTipManager.Instance.show(
            LangManager.Instance.GetTranslation(
              "outercityshop.goodsItemTip",
              LangManager.Instance.GetTranslation("outercityshop.score"),
            ),
          );
          return true;
        }
  ...
  getMaxBuyCount(max: number) {
    for (var i: number = 0; i < this._needValueCount.length; i++) {
      if (
        this._needValueItemId[i] == "Score" ||
        this._needValueItemId[i] == -6000
      ) {
        let count = Math.floor(
          OuterCityShopManager.instance.model.consumePoints /
            this._needValueCount[i],
        );
        if (count < max) {
          max = count;
        }
      ...
      switch (this._needValueItemId[i]) {
        case -100: //黄金
        case -300: //战魂
          let count1 = Math.floor(
            ResourceManager.Instance.gold.count / this._needValueCount[i],
          );
          if (count1 < max) {
            max = count1;
          }
  ...
  protected refreshView() {
    ...
    if (
      (goodsTempInfo && goodsTempInfo.MasterType == GoodsType.EQUIP) ||
    ...
    if (this._itemInfo.itemTemInfo.Type == OuterCityShopModel.RANDOM_SHOP) {
      ...
      if (
        this._itemInfo.itemTemInfo.WeeklyLimit == "-1" ||
        this._itemInfo.itemTemInfo.WeeklyLimit == "0"
      ) {
        // 不限购
        this.txt_limitTitle.text = "";
        this.txt_limit.text = "";
        ...
        let buyCount = OuterCityShopManager.instance.model.getWeekLimitBuyCount(
          this._itemInfo.itemTemInfo.ItemId,
        );
        this.txt_limitTitle.text = LangManager.Instance.GetTranslation(
  ...
  private updateCostPrice(value: number = 1) {
    ...
    for (let i: number = 0; i < this._itemInfo.costInfos.length; i++) {
      ...
      if (costInfo[0] == "Score") {
        ...
        this._needValueItemId.push(
          TemplateIDConstant.TEMP_ID_MYSTERY_SHOP_SCORE,
        );
        this.sellType.selectedIndex = 1;
        price = Number(costInfo[1]);
        this.txt_price1.text = String(price * value);
        this.txt_price1.color = this.getColor(
          this.playerScore,
          Number(price * value),
        );
        ...
        //随机兑换
        var needGoodsTempInfo =
          TempleteManager.Instance.getGoodsTemplatesByTempleteId(costInfo[0]);
    ...
    if (flag) {
      ...
      if (this._needValueCount.length == 2) {
        //需要消耗2个
        this.sellType.selectedIndex = 0;
        this.txt_price2.text = (value * price).toString();
        this.txt_price2.color = this.getColorByType(
          this._needValueItemId[0],
          value * price,
        );
        this.txt_price1.text = (value * this._needValueCount[1]).toString();
        this.txt_price1.color = this.getColorByType(
          this._needValueItemId[1],
          this._needValueCount[1],
        );
        this.tipItem1.visible = true;
        this.tipItem1.visible = true;
  ...
  getColorByType(type: number, count: number) {
    ...
    if (type == -100) {
      ...
      //神秘石
      str = this.getColor(
        GoodsManager.Instance.getGoodsNumByTempId(ShopGoodsInfo.MYSTERY_STONE),
        count,
      );
    } else if (type == ShopGoodsInfo.MEDAL_TEMPID) {
      //勋章
      str = this.getColor(
        GoodsManager.Instance.getGoodsNumByTempId(ShopGoodsInfo.MEDAL_TEMPID),
        count,
      );
    } else {
      //积分
