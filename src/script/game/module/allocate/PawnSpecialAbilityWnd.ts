...
import { PawnEvent, ResourceEvent } from '../../constant/event/NotificationEvent';
...
export default class PawnSpecialAbilityWnd extends BaseWindow {
    ...
    public OnInitWind() {
        ...
        this.needCostGoldCount = Number(TempleteManager.Instance.getConfigInfoByConfigName("Special_Stretgy").ConfigValue);
    ...
    private addEvent() {
        ...
        ResourceManager.Instance.gold.addEventListener(ResourceEvent.RESOURCE_UPDATE, this.changeTextColor, this);
    }
    ...
    private removeEvent() {
        ...
        this._pawn && this._pawn.removeEventListener(PawnEvent.SPECIAL_ALIBITY, this.__changeChangeHandler, this);
        this._pawn && this._pawn.removeEventListener(PawnEvent.SPECIAL_BLESS, this.__blessChangeHandler, this);
        ...
        ResourceManager.Instance.gold.removeEventListener(ResourceEvent.RESOURCE_UPDATE, this.changeTextColor, this);
    }
    ...
    private __onReplaceHandler() {
        ...
        var content: string = LangManager.Instance.GetTranslation("armyII.viewII.pawnupgrade.PawnSpecialAbilityFrame.sureFrameDescribe");
        SimpleAlertHelper.Instance.Show(SimpleAlertHelper.SIMPLE_ALERT, null, prompt, content, confirm, cancel, this.activeAlertBack.bind(this));
    }
    ...
    private onComprehend(isquick?){
        let newTemp: t_s_specialtemplateData = TempleteManager.Instance.getPawnSpecialTemplateByID(parseInt(this._pawn.tempSpecial));
        let oldTemp: t_s_specialtemplateData = TempleteManager.Instance.getPawnSpecialTemplateByID(parseInt(this._pawn.specialAbility));
        if (newTemp && oldTemp && newTemp.Grades > oldTemp.Grades) {
            let str: string = LangManager.Instance.GetTranslation("buildings.casern.view.PawnSpecialAbilityFrame.BetterAlert");
    ...
    private _helpClick() {
        ...
        let content: string = LangManager.Instance.GetTranslation("buildings.casern.view.PawnSpecialAbilityFrame.helpContent");
    ...
    private refreshView() {
        if (this._pawn) {
            ...
            if (oldTemp) {
                ...
                this.PawnSpecialNameTxt1.text = LangManager.Instance.GetTranslation("public.level.name", oldTemp.TemplateNameLang, oldTemp.Grades);
            }
            if (newTemp) {
                this.specila2 = newTemp;
                this.playerCom2.setIcon(IconFactory.getCommonIconPath(newTemp.ResPath));
                this.PawnSpecialNameTxt2.text = LangManager.Instance.GetTranslation("public.level.name", newTemp.TemplateNameLang, newTemp.Grades);;
            }
            this._pawn && this._pawn.addEventListener(PawnEvent.SPECIAL_ALIBITY, this.__changeChangeHandler, this);
            this._pawn && this._pawn.addEventListener(PawnEvent.SPECIAL_BLESS, this.__blessChangeHandler, this);
            this.Btn_Replace.enabled = !StringHelper.isNullOrEmpty(this._pawn.tempSpecial);
            this.refreshBless();
    ...
    private _special1ClickHander() {
        if (UIManager.Instance.isShowing(EmWindow.SoliderSkillTipWnd))//如果有弹窗
        {
            ...
            let point = this.playerCom1.parent.localToGlobal(this.playerCom1.x, this.playerCom1.y);
            UIManager.Instance.ShowWind(EmWindow.SoliderSkillTipWnd, { posX: point.x, posY: point.y, pawnData: this.specila1 });
        }
    }
    ...
    private _special2ClickHander() {
        if (UIManager.Instance.isShowing(EmWindow.SoliderSkillTipWnd))//如果有弹窗
        {
            ...
            let point = this.playerCom2.parent.localToGlobal(this.playerCom2.x, this.playerCom2.y);
            UIManager.Instance.ShowWind(EmWindow.SoliderSkillTipWnd, { posX: point.x, posY: point.y, pawnData: this.specila2 });
        }
    }
    ...
    private refreshBless() {
        if (this._pawn) {
            let oldTemp: t_s_specialtemplateData = TempleteManager.Instance.getPawnSpecialTemplateByID(parseInt(this._pawn.specialAbility));
            var temp: t_s_upgradetemplateData = TempleteManager.Instance.getTemplateByTypeAndLevel(oldTemp.Grades, UpgradeType.UPGRADE_TYPE_PAWNSPECIAL_BLESS);
            this.PawnSpeciaProgress.value = Math.floor((this._pawn.blessNum / temp.Data) * 100);
