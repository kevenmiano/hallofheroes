...
export default class GeniusPanel extends FUI_Genius_Panel {
    ...
    onConStructor() {
        ...
        this.txt_cancel.text = LangManager.Instance.GetTranslation('armyII.viewII.information.InformationView.save');
        this.txt1.text = LangManager.Instance.GetTranslation('talent.sealTitle');
        this.circleCom = this.telent_com.getChild('clockwise') as FUI_ComTalentsCirle;
    ...
    showMask(index: number) {
        for (let i = 0; i < 4; i++) {
            const element: SealSkill = this.circleCom.getChild('item' + i) as SealSkill;
    ...
    private updateSonTypeItem() {
        ...
        for (let index = 0; index < array.length; index++) {
            ...
            let temp: t_s_skilltemplateData = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_skilltemplate, tempId.toString());
    ...
    getTalentPoint(branck: number): number {
        ...
        for (let i = 1; i < 8; i++) {
            var skillInfo: SkillInfo = this.thane.talentData.allTalentList[order + i] as SkillInfo;
    ...
    private updateTalentGrade() {
        ...
        this.txt_level.text = LangManager.Instance.GetTranslation("public.level3", ArmyManager.Instance.thane.talentData.talentGrade);
    }
    ...
    talenChangeHanler(info: SkillInfo) {
        if (info) {
            ...
            if (info.templateInfo && info.templateInfo.SonType == this.currentBranch) {//从底部技能栏升级时更新左边选中的状态
                let rightItem: SealSkill = this['item' + this.curIndex];
                rightItem.setData(info);
                rightItem.setSelected(true, 0);
            } else {//从底部技能栏升级时更新左边未选中的状态
                for (let i = 0; i < 4; i++) {
                    const element: SealSkill = this['item' + i];
                    if (element.Itemdata && element.Itemdata.templateInfo.SonType == sontype) {
    ...
    onDown() {
        ...
        Laya.Tween.to(this.item0, { rotation: itemRotation + 90 }, 1000, Laya.Ease.expoOut);
        Laya.Tween.to(this.item1, { rotation: itemRotation + 90 }, 1000, Laya.Ease.expoOut);
        Laya.Tween.to(this.item2, { rotation: itemRotation + 90 }, 1000, Laya.Ease.expoOut);
        Laya.Tween.to(this.item3, { rotation: itemRotation + 90 }, 1000, Laya.Ease.expoOut);
        Laya.Tween.to(this.circleCom, { rotation: rotation - 90 }, 1000, Laya.Ease.expoOut, Laya.Handler.create(this, this.onPlayEnd));
    }
    ...
    private addEvent() {
        ...
        NotificationManager.Instance.addEventListener(TalentEvent.SET_FAST_KEY, this.__setFastKeyHandler, this);
        ArmyManager.Instance.addEventListener(TalentEvent.TALENT_GRADUP_SUCC, this.talenChangeHanler, this);
        ArmyManager.Instance.addEventListener(TalentEvent.UPDATE_TREE, this.updateTree, this);
        this.sw1.onClick(this, this.onSwitchBtnTap, [0]);
        this.sw2.onClick(this, this.onSwitchBtnTap, [1]);
    }

    updateTree() {
        if (this.talent_index != this.thane.talentData.talent_index) {//切换模式才需要重置界面
            this.thane.talentData.resetUserTalentSkill();
            this.initView();
            this.talent_index = this.thane.talentData.talent_index;
        }
    ...
    private removeEvent() {
        ...
        NotificationManager.Instance.removeEventListener(TalentEvent.TALENT_RESET, this.onReset, this);
        NotificationManager.Instance.removeEventListener(TalentEvent.SET_FAST_KEY, this.__setFastKeyHandler, this);
        ArmyManager.Instance.removeEventListener(TalentEvent.TALENT_GRADUP_SUCC, this.talenChangeHanler, this);
        ArmyManager.Instance.removeEventListener(TalentEvent.UPDATE_TREE, this.updateTree, this);
        this.isAnimating = false;
    }
    ...
    private onSwitchBtnTap(talenIndex: number) {
        if (this.isSetting) {
            MessageTipManager.Instance.show(LangManager.Instance.GetTranslation('talent.switch'));
            return;
        }
        let talentData = ArmyManager.Instance.thane.talentData;
        this.updateSwitchBtn();
        ...
        if (talenIndex == 1 && !talentData.is_activeSecond) {
            let cfgValue = 0;
            let cfgItem = TempleteManager.Instance.getConfigInfoByConfigName("Second_talent");
            if (cfgItem) {
            ...
            let content: string = LangManager.Instance.GetTranslation("skill.genius.activeTelent", point);
