...
export default class OuterCityWarBuildingView extends CastleBuildingBaseView {
	...
	constructor(buildInfo: OuterCityWarBuildInfo) {
		...
		this._infoItem = FUIHelper.createFUIInstance(EmPackName.OuterCityWar, "OuterCityWarBuildBriefInfoItem");
		let pos = CastleConfigUtil.Instance.getBuildNamePos(this.sonType, EmCastlePos.OuterCityWar);
	...
	public mouseClickHandler(evt: Laya.Event): boolean {
		...
		switch (this.buildInfo.buildSortType) {
			...
			case EmOuterCityWarBuildSortType.Pet:
				FrameCtrlManager.Instance.open(EmWindow.OuterCityWarDefencerBuildWnd, {buildInfo:this.buildInfo})
				break;
			case EmOuterCityWarBuildSortType.AttackSite:
				// 进攻方阵营
				FrameCtrlManager.Instance.open(EmWindow.OuterCityWarAttackerBuildWnd, {buildInfo:this.buildInfo})
				break;
		}
		return true;
	}
