...
export class HeroLoadDataFactory {
    ...
    public static create(heroInfo: ThaneInfo, part: string): GameLoadNeedData {
        ...
        switch (sPart) {
            ...
            case "pet_body":
                if (heroInfo.petTemplate) {
                    path = PathManager.solveHeroAvatarPath(heroInfo.petTemplate.PetAvatar, 2);
                } else {
                    path = "";
                }
                break;
            case "body":
                if (heroInfo.templateId == 100) {//神秘人
                    path = PathManager.getDefaultJobBattleAvatar();
                } else if (heroInfo.bodyFashionAvata) {
                    path = PathManager.solveWingAvatarPath(heroInfo.bodyFashionAvata, heroInfo.templateInfo.Job, heroInfo.templateInfo.Sexs);
                } else if (heroInfo.bodyAvata) {
                    path = PathManager.solveHeroAvatarPath(heroInfo.bodyAvata, heroInfo.templateInfo.Sexs)
                } else {
                    path = PathManager.solveRoleDefaultPath(heroInfo.templateInfo.Job, "body", heroInfo.templateInfo.Sexs);
                }
                break;
            case "arms":
                if (heroInfo.armsFashionAvata) {
                    path = PathManager.solveWingAvatarPath(heroInfo.armsFashionAvata, heroInfo.templateInfo.Job, heroInfo.templateInfo.Sexs);
                } else if (heroInfo.armsAvata) {
                    path = PathManager.solveHeroAvatarPath(heroInfo.armsAvata, heroInfo.templateInfo.Sexs);
                } else {
                    path = PathManager.solveRoleDefaultPath(heroInfo.templateInfo.Job, "arms", heroInfo.templateInfo.Sexs);
                }
                break;
            case "hair":
                if (heroInfo.hairFashionAvata) {
                    path = PathManager.solveWingAvatarPath(heroInfo.hairFashionAvata, heroInfo.templateInfo.Job, heroInfo.templateInfo.Sexs);
                } else if (heroInfo.hairAvata) {
                    path = PathManager.solveHeroAvatarPath(heroInfo.hairAvata, heroInfo.templateInfo.Sexs);
                } else {
                    path = PathManager.solveRoleDefaultPath(heroInfo.templateInfo.Job, "hair", heroInfo.templateInfo.Sexs);
                }
                break;
            case "cloak":
                if (heroInfo.wingAvata) {
                    path = PathManager.solveWingAvatarPath(heroInfo.wingAvata, heroInfo.templateInfo.Job, heroInfo.templateInfo.Sexs);
                } else {
                    if (heroInfo.bodyAvata) {
                        path = PathManager.solveHeroAvatarPath(heroInfo.bodyAvata + "_cloak", heroInfo.templateInfo.Sexs);
                    } else {
                        path = PathManager.solveRoleDefaultPath(heroInfo.templateInfo.Job, "body", heroInfo.templateInfo.Sexs, "_cloak");
                    }
                }
                break;
        }
        ...
        gameLoadData = new GameLoadNeedData(path, undefined, levelFlag, level, srcPart);
        return gameLoadData;
    }
    ...
    public static createAll(thaneInfo: ThaneInfo) {
        ...
        info = HeroLoadDataFactory.create(thaneInfo, HeroLoadDataFactory.PART_CLOAK2);
        tempArr.push(info)
        info = HeroLoadDataFactory.create(thaneInfo, HeroLoadDataFactory.PART_ARMS_BACK);
        tempArr.push(info)
        info = HeroLoadDataFactory.create(thaneInfo, HeroLoadDataFactory.PART_HAIR4);
        tempArr.push(info)
        info = HeroLoadDataFactory.create(thaneInfo, HeroLoadDataFactory.PART_HAIR3);
        tempArr.push(info)
        info = HeroLoadDataFactory.create(thaneInfo, HeroLoadDataFactory.PART_BODY);
        tempArr.push(info)
        info = HeroLoadDataFactory.create(thaneInfo, HeroLoadDataFactory.PART_HAIR2);
        tempArr.push(info)
        info = HeroLoadDataFactory.create(thaneInfo, HeroLoadDataFactory.PART_ARMS);
        tempArr.push(info)
        info = HeroLoadDataFactory.create(thaneInfo, HeroLoadDataFactory.PART_HAIR1);
        tempArr.push(info)
        info = HeroLoadDataFactory.create(thaneInfo, HeroLoadDataFactory.PART_CLOAK1);
        tempArr.push(info)
        return tempArr
    }
}