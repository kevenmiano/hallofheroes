...
export default class MainToolBar extends BaseFguiCom {
    ...
    switch (buttonType) {
      ...
      case EmMainToolBarBtnType.STAR:
        if (
          ...
          GlobalConfig.NEWBIE_43000
        ) {
          SDKManager.Instance.getChannel().adjustEvent(
            RPT_EVENT.CLICK_ASTROLOGY,
          );
        }
        ...
        switch (SceneManager.Instance.currentType) {
          case SceneType.PVP_ROOM_SCENE:
            ...
            if (roomInfo && roomInfo.roomState == RoomState.STATE_COMPETEING) {
              MessageTipManager.Instance.show(
                LangManager.Instance.GetTranslation(
                  "ui.maintoolbar.exitPvpRoomCompeteing",
                ),
              );
              return;
            }
          ...
          case SceneType.WARLORDS_ROOM:
            let str: string = LangManager.Instance.GetTranslation(
              "ui.maintoolbar.MainToolBar.exitWarlordsRoom",
            );
            let prompt: string = LangManager.Instance.GetTranslation(
              "mainBar.MainToolBar.prompt",
            );
                ...
                if (b) {
                  if (
                    SceneManager.Instance.currentType ==
                      SceneType.BATTLE_SCENE &&
                    WorldBossHelper.checkPvp(mapModel.mapId)
                  ) {
                    //战场在战斗场景不强制退出
                    MessageTipManager.Instance.show(
                      LangManager.Instance.GetTranslation(
                        "mainToolBar.return.Tips",
                      ),
                    );
                    return;
                  }
                  ...
                  if (
                    this.playerModel.getAutoWalkFlag() == PlayerModel.AUTO_WALK
                  ) {
  ...
  refreshBtnPos() {
    ...
    let showRow1Btns = this.getBtnListByLocaltionType(
      EmMainToolBarBtnLocationType.Row1,
    );
    let showRow2Btns = this.getBtnListByLocaltionType(
      EmMainToolBarBtnLocationType.Row2,
    );
    let showCowBtns = this.getBtnListByLocaltionType(
      EmMainToolBarBtnLocationType.Cow,
    );
  ...
  createButtonByType(buttonType: EmMainToolBarBtnType): UIButton {
    ...
    let btnData = this.model.getBtnDataByButtonType(
      buttonType,
    ) as MainToolBarButtonData;
    let btnView = FUIHelper.createFUIInstance(
      EmPackName.Home,
      btnData.prefabName,
    ) as fgui.GButton;
   ...
   */
  public doMoveBtn(
    addTarget: UIButton = null,
    tweenShow: boolean = false,
    ...
    if (tweenList.length > 0) {
      ...
      let targetPos = this.model.getBtnPos(
        basePos,
        addBtnData.locationType,
        targetIdx,
      );
  ...
  public getBtnListByLocaltionType(
    locationType: EmMainToolBarBtnLocationType,
  ): UIButton[] {
    let temp = [];
    this._allUIBtns.forEach(
      (uiBtn: UIButton, btnType: EmMainToolBarBtnType) => {
