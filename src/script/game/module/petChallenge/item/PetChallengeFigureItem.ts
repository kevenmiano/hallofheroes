...
export class PetChallengeFigureItem extends FUI_PetChallengeFigureItem {
    ...
    private refresh() {
        if (this._info.isSelf) {
            ...
            this.txtName.text = LangManager.Instance.GetTranslation("PetChallengeListItemView.self");
        } else {
            this.txtName.color = "#00F0FF"
            this.txtName.text = LangManager.Instance.GetTranslation("PetChallengePetListInfoView.Title", this._info.userName);
        }
