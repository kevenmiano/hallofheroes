...
export default class WishPoolManager {
    ...
    private initEvent() {
        ServerDataManager.listen(S2CProtocol.U_C_WISH_POOL_RESP, this, this.__wishRespHandler);
        ServerDataManager.listen(S2CProtocol.U_C_WISH_POOL_ALL_INFO, this, this.__wishAllInfoHandler);
        ServerDataManager.listen(S2CProtocol.U_C_WISH_POOL_UPDATE, this, this.__wishUpdateHandler);
    }
    ...
    private __wishRespHandler(pkg: PackageIn) {
        ...
        if (msg) {
            if (!msg.wishResult) {
                MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("wishpool.result"));
                return;
            }
            this._goodsArr = [];
            ...
            this._goodsArr = ArrayUtils.sortOn(this._goodsArr,"templateId",ArrayConstant.DESCENDING);
            FrameCtrlManager.Instance.open(EmWindow.WishPoolResultWnd,{arr:this._goodsArr,type:2});
        }
    }
    ...
    private __wishUpdateHandler(pkg: PackageIn) {
        ...
        if (msg) {
            ...
            if(wishPoolInfo){
                ...
                NotificationManager.Instance.dispatchEvent(NotificationEvent.UPDATE_WISHDATA);
            }
        }
    }
