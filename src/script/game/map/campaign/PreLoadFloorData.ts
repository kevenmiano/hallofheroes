...
export class PreLoadFloorData {
   ...
   *
   */
  public getMapFloorData(
    data: Map<string, any>,
    mapId: number,
  ): Map<string, any> {
    let dic: Map<string, any> = new Map();
    if (
      WorldBossHelper.checkSliceBgMap(mapId) ||
      WorldBossHelper.checkSingleBgMap(mapId)
    ) {
    } else {
      let oldFloorUrl: string = "mapmaterial/floor/grassland/a00.jpg";
      let newFloorUrl: string =
        "mapmaterial/floor/greeland/greenland_floor/at01.jpg";
  ...
  public load(dic: Map<string, any>) {
    ...
    dic.forEach((element, key) => {
      MapElmsLibrary.Instance.getElementSource(
        key,
        this.loadImageResult.bind(this),
      ); //加载图片
    });
