...
export default class MessageLabel extends BaseWindow {
    ...
    OnShowWind() {
        ...
        if (this.animationData.type == FontType.MessageLabel || this.animationData.type == FontType.NONE) {
    ...
    doShowAnimation(): Promise<boolean> {
        return new Promise(resolve => {
            ...
            Laya.timer.once(1000, this, () => {//延迟1s再向上飘
                Laya.Tween.to(this, { y: targetY }, 500, undefined, Laya.Handler.create(this, (target) => {
                    UIManager.Instance.HideTips(EmWindow.MessageLabel + "_" + this.animationData.type, EmWindow.MessageLabel, this);
                    Laya.Tween.clearAll(target);
                    Laya.timer.clearAll(this);
                    resolve(true);
                }), 0, this.animationData.once, true);
            })
        });
