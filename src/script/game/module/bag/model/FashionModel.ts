...
export class FashionModel extends FrameDataBase {
  ...
  public gotoPreStep() {
    ...
    this.showDic.add(
      this.stepList[this.stepList.length - 1].sonType,
      this.stepList[this.stepList.length - 1].oldInfo,
    );
    this.showThane.hairFashionAvata = "";
    this.showThane.bodyFashionAvata = "";
    this.showThane.armsFashionAvata = "";
    ...
    for (let i: number = 0; i < this.showDic.getList().length; i++) {
      switch (this.getSonTypeByInfo(this.showDic.getList()[i])) {
        case GoodsSonType.FASHION_HEADDRESS: //帽子
          this.showThane.hairFashionAvata = this.getAvataByInfo(
            this.showDic.getList()[i],
          );
          break;
        case GoodsSonType.FASHION_CLOTHES: //衣服
          this.showThane.bodyFashionAvata = this.getAvataByInfo(
            this.showDic.getList()[i],
          );
          break;
        case GoodsSonType.FASHION_WEAPON: //武器
          this.showThane.armsFashionAvata = this.getAvataByInfo(
            this.showDic.getList()[i],
          );
          break;
        case GoodsSonType.SONTYPE_WING: //翅膀
          this.showThane.wingAvata = this.getAvataByInfo(
            this.showDic.getList()[i],
          );
          break;
      }
  ...
  public get fashionList(): GoodsInfo[] {
    let arr: GoodsInfo[] = [];
    if (this._openScene == FashionModel.FASHION_STORE_SCENE) {
      //时装合成分解界面
      if (this._selectedPanel == FashionModel.FASHION_SWITCH_PANEL) {
        arr = arr.concat(this.fashionEquipList).concat(this.fashionBagList);
      } else {
        arr = arr
          .concat(this.fashionBagList)
          .concat(this.fashionSoulBagList)
          .concat(this.fashionLuckyBagList);
      }
    } else if (this._openScene == FashionModel.SHOP_SCENE) {
      //时装商城界面
      arr = arr.concat(this.fashionEquipList).concat(this.fashionBagList);
      for (let i: number = 0; i < arr.length; i++) {
        for (let j: number = 0; j < this.showDic.getList().length; j++) {
          if (this.showDic.getList()[j] instanceof GoodsInfo) {
            if (
              arr[i] &&
              (arr[i] as GoodsInfo).id == this.showDic.getList()[j].id
            ) {
              arr.splice(i, 1);
              i = -1;
            }
  ...
  public hasFashionAtSonType(sonType: number): boolean {
    if (
      sonType != GoodsSonType.FASHION_WEAPON &&
      sonType != GoodsSonType.FASHION_CLOTHES &&
      sonType != GoodsSonType.FASHION_HEADDRESS &&
      sonType != GoodsSonType.SONTYPE_WING
    ) {
      return false;
    }
  ...
  public get fashionEquipList(): GoodsInfo[] {
    let arr: GoodsInfo[] = [];
    let arr1: GoodsInfo[] = GoodsManager.Instance.getHeroEquipListById(
      this.thane.id,
    ).getList();
  ...
  public get fashionLuckyBagList() {
    let arr: GoodsInfo[] = [];
    for (let info of GoodsManager.Instance.getGoodsByGoodsTId(
      FashionModel.FASHION_LUCKYCHARM,
    )) {
  ...
  public get fashionSoulBagList(): GoodsInfo[] {
    let arr: any[] = [];
    let arr1: GoodsInfo[] = GoodsManager.Instance.getGoodsByGoodsTId(
      FashionModel.FASHION_SOUL,
    );
    for (let i = 0; i < arr1.length; i++) {
      const info: GoodsInfo = arr1[i];
  ...
  public checkRedDot(checkAll: boolean = false): boolean {
    if (checkAll) {
      return (
        this.checkIdentiable(this._weaponTems, 0) &&
        this.checkIdentiable(this._clothesTems, 1) &&
        this.checkIdentiable(this._hatTems, 2) &&
        this.checkIdentiable(this._wingTems, 3)
      );
    }
  ...
  private checkIdentiable(array: t_s_itemtemplateData[], tag: number): boolean {
    ...
    array.forEach((element) => {
      if (this.hasIdentityedBook(element.TemplateId) == false) {
        //没鉴定过
  ...
  canIdentify(tempId: number): boolean {
    let result: boolean;
    if (this.hasIdentityedBook(tempId) == false) {
      //没鉴定过
  ...
  /**
     ...
     *
     */
  private fashionCompareFunction(
    a: t_s_itemtemplateData,
    b: t_s_itemtemplateData,
  ): number {
   ...
   *
   */
  public getFashionObjectSkillTemplate(
  ...
  public getIdentityedProperties(): any[] {
    ...
    for (let i = 0; i < this.identityedList.length; i++) {
      const info: FashionInfo = this.identityedList[i];
      let goods: t_s_itemtemplateData = this.getTemplateByID(
        info.fashionTempId,
      );
      if (!goods) {
        continue;
      }
      let skillPropTemp: t_s_skillpropertytemplateData =
        ConfigMgr.Instance.getTemplateByID(
  ...
  public getFashionLevelByInfo(info: GoodsInfo): number {
    ...
    if (
      !ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_skilltemplate, skillId)
    ) {
      skillId = Number(info.templateInfo.DefaultSkill.split(",")[0]);
    }
    let cfg = ConfigMgr.Instance.getTemplateByID(
      ConfigType.t_s_skilltemplate,
      skillId,
    );
    if (cfg) {
