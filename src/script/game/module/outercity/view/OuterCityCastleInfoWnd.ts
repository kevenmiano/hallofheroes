...
export class OuterCityCastleInfoWnd extends BaseWindow {
  ...
  private initView() {
    if (this._info.info.occupyPlayerId == 0) {
      this.frame.title = LangManager.Instance.GetTranslation(
        "map.internals.view.frame.CityInfoView.command08",
      );
    } else {
      this.frame.title = LangManager.Instance.GetTranslation(
        "map.internals.view.frame.CityInfoView.command09",
      );
    }

    //{name=玩家名字}[color=#00f1ff]{pos=（234,234）}[/color]
    this.txt_name.text =
      this._info.info.names +
      "  " +
      "[color=#00f1ff](" +
      this._info.convertX +
      "," +
    ...
    this.txt_consortia.text = this._info.info.occupyLeagueName
      ? this._info.info.occupyLeagueName
      : LangManager.Instance.GetTranslation(
          "map.internals.view.frame.CityInfoView.politeText2.none",
        );
    if (
      PlayerManager.Instance.currentPlayerModel.playerInfo.userId ==
      this._info.info.occupyPlayerId
    ) {
      this.btn_attack.title = LangManager.Instance.GetTranslation(
        "map.internals.view.frame.CityInfoView.command06",
      );
      return;
    }
    if (this._info.info.occupyPlayerId != 0) {
  ...
  protected OnBtnClose() {
    NotificationManager.Instance.dispatchEvent(
      ...
      new Point(this._info.x, this._info.y),
    );
    super.OnBtnClose();
  }
  ...
  private __playerNameClickHandler(): void {
    ...
    if (this._info.info.occupyPlayerId >= 1300000000) {
      let str: string = LangManager.Instance.GetTranslation(
        "map.internals.view.frame.CityInfoView.command05",
      );
