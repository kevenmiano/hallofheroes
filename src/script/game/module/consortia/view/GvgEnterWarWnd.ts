...
export class GvgEnterWarWnd extends BaseWindow {
    ...
    private initEvent() {
        ...
        ConsortiaManager.Instance.model.addEventListener(ConsortiaEvent.UPDATA_CONSORTIA_MEMBER, this.__consortiaMemberHandler, this);
    }
    ...
    public setListView(): void {
        ...
        this._memberList = [];
        let list = ConsortiaManager.Instance.model.getSortMemberList(this._sortField, this._isReverse);
    ...
    protected __helpHandler(): void {
        AudioManager.Instance.playSound(SoundIds.CONFIRM_SOUND);
        let title: string = LangManager.Instance.GetTranslation("friends.view.FriendsHelpFrame.title");
        let count = 25;
        let cfg = TempleteManager.Instance.getConfigInfoByConfigName("GVG_People");
        if (cfg) {
            count = Number(cfg.ConfigValue);
        }
        let content: string = LangManager.Instance.GetTranslation("consortia.GvgJionMemberHelpFrame.helpContent", count);
    ...
    private setTips(): void {
        ...
        this.txt_in.text = LangManager.Instance.GetTranslation("gvg.InwarTips") + LangManager.Instance.GetTranslation("gvg.NotinWarList");
        for (let i = 0, len = list.length; i < len; i++) {
            const info: ThaneInfo = list[i];
            if (info.userId == ArmyManager.Instance.thane.userId && (info.isTeamPlayer || info.userId == chairmanID)) {
                this.txt_in.color = "#FFECC6";
                this.txt_in.text = LangManager.Instance.GetTranslation("gvg.InwarTips") + LangManager.Instance.GetTranslation("gvg.InWarList");
                break;
            }
        }
    }
    ...
    private removeEvent() {
        ...
        ConsortiaManager.Instance.model.removeEventListener(ConsortiaEvent.UPDATA_CONSORTIA_MEMBER, this.__consortiaMemberHandler, this);
    }
