...
import t_s_qqgradeprivilege, {
  t_s_qqgradeprivilegeData,
} from "../../config/t_s_qqgradeprivilege";
...
export default class QQDawankaModel extends FrameDataBase {
  ...
  setInfo(info: UserQQDaWanKaMsg) {
    ...
    for (let i = 0; i < this.giftState.length; i++) {
      this.titleRedDotArrs[i] =
        this.giftState[i].isPrivilege || this.giftState[i].isWeekGift;
    }
  ...
  setRenderList() {
    ...
    for (let i = 0; i < this.QQGradePrivilegeDic.length; i++) {
      ...
      let vo2 = new QQDawankaItemVO(
        8,
        LangManager.Instance.GetTranslation("QQ.Hall.Dawanka.level8"),
      );
      vo2.rewards = this.getQQGradeData(i + 1).Privilegereward;
      let vo3 = new QQDawankaItemVO(
        9,
        LangManager.Instance.GetTranslation("QQ.Hall.Dawanka.level9"),
      );
