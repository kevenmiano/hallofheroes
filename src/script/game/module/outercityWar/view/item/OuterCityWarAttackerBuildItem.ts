...
export default class OuterCityWarAttackerBuildItem extends FUI_OuterCityWarAttackerBuildItem {
    ...
    public addEvent() {
        NotificationManager.Instance.addEventListener(OuterCityWarEvent.PLAYER_INFO, this.__updatePlayinfo, this)
    }

    public removeEvent() {
        NotificationManager.Instance.removeEventListener(OuterCityWarEvent.PLAYER_INFO, this.__updatePlayinfo, this)
    }
        ...
        if (this._info) {
            ...
            switch (this._info.state) {
                ...
                case EmOuterCityWarPlayerState.DEFANCE:
                    this.txtState.text = LangManager.Instance.GetTranslation("public.playerInfo.defanceState")
                    break;
                case EmOuterCityWarPlayerState.REPULSED:
                    this.setNormal(false)
                    this.txtState.text = LangManager.Instance.GetTranslation("public.playerInfo.repulsedState")
                    break;
            }
        } else {
