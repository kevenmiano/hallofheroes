...
export class MountTipView extends TaskTraceTipWnd {
	...
	initView() {
		super.initView();
		let htmlText = LangManager.Instance.GetTranslation("tasktracetip.view.MountTipView.content");
		this.setContentText(htmlText);
		this.setBtnTitle(LangManager.Instance.GetTranslation("tasktracetip.view.MountTipView.text"));
	}


	protected __btnHandler(e: Event) {
		super.__btnHandler(e);
		FrameCtrlManager.Instance.open(EmWindow.Forge, { tabIndex: GTabIndex.Forge_XQ })
