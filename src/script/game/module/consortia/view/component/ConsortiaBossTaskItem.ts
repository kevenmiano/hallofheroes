...
export default class ConsortiaBossTaskItem extends FUI_ConsortiaBossTaskItem {
    ...
    private refreshView() {
        ...
        this.maxProcess = 0;
        if (this._model.bossInfo.taskProcessArr && this._model.bossInfo.taskProcessArr.length)
            this._currentProcess = this._model.bossInfo.taskProcessArr[this._type];
        if (this._model.bossInfo.taskMaxProcessArr && this._model.bossInfo.taskMaxProcessArr.length)
            this.maxProcess = this._model.bossInfo.taskMaxProcessArr[this._type];
        if (this._currentProcess >= this.maxProcess) { //已完成
            this.taskTitleTxt.color = "#7ddf26";
            this.taskDescTxt.visible = false;
            this.taskTitleTxt.text = LangManager.Instance.GetTranslation("ConsortiaBoss.ConsortiaBossTaskItem.ItemTitleTxt" + this._type)
                + LangManager.Instance.GetTranslation("ConsortiaBoss.ConsortiaBossTaskItem.completeText");
        } else {
            this.taskTitleTxt.color = "#FFC68F";
            this.taskDescTxt.color = "#FFECC6";
            this.taskDescTxt.visible = true;
            this.taskTitleTxt.text = LangManager.Instance.GetTranslation("ConsortiaBoss.ConsortiaBossTaskItem.ItemTitleTxt" + this._type);
            this.taskDescTxt.text = LangManager.Instance.GetTranslation("ConsortiaBoss.ConsortiaBossTaskItem.ItemDescribeTxt" + this._type, this.currentProcess, this.maxProcess);
        }
        // if (this.vgroup) {
