...
export class NpcAlertMediator implements IEnterFrame, IMediator {
    ...
    public enterFrame() {
        ...
        this._armyPoint.x = selfArmy.x; this._armyPoint.y = selfArmy.y;
        var dis: number = this._npc.npcAiInfo.centerPoint.distance(this._armyPoint.x, this._armyPoint.y);
        if (dis > NpcEffectiveState.ATTACK_RADIUS - 20) {
            // Logger.yyz("[NpcAlertMediator]与玩家的距离太远", dis, this.npcNodeId)
            return;
        }
        var leng: number = this._npcPoint.distance(this._armyPoint.x, this._armyPoint.y)
        if (leng > NpcEffectiveState.BATTLE_RADIUS && leng < NpcEffectiveState.ALERT_RADIUS) {
            Logger.yyz("[NpcAlertMediator]玩家到达警戒区域 发送警戒消息", dis, this.npcNodeId)
            //todo不能让其的攻击路线长过400
    ...
    private __moveStateHandler(evt: AiEvents) {
        ...
        if (this._npc.npcAiInfo.moveState == AiStateType.NPC_CHASE_STATE) {
            ...
            if (this._mapModel.chckeNpcFriendState(<CampaignNode>this._npc.nodeInfo)) return;
