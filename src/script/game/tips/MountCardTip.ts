...
export class MountCardTip extends BaseTips {
    ...
    private updateView() {
        ...
        if (this._info) {
            ...
            //附加属性
            let mountTemplate: t_s_mounttemplateData = TempleteManager.Instance.getMountTemplateById(this._info.templateInfo.Property1);
            let strArray: any[] = LangManager.Instance.GetTranslation("mounts.WildsoulItem.tips01").split("|");
            let valueArray: any[] = [];
            if (mountTemplate) {
                valueArray = [
                    mountTemplate.Power,
            ...
            for (let i: number = 0; i < valueArray.length; i++) {
                if (valueArray[i] != 0 && i < strArray.length) {
                    tipStr += StringHelper.repHtmlTextToFguiText(StringHelper.format(strArray[i], valueArray[i])) + "<br>";
                }
            }
            this.txt_addition.text = tipStr; //"智力+10<br/>体质+30<br/>体质+30<br/>体质+30";
            this.txt_score.text = LangManager.Instance.GetTranslation("mountTip.soulscore", mountTemplate ? mountTemplate.SoulScore : 0);

            this.txt_desc.visible = mountTemplate ? mountTemplate.Property2 != -1 : false;
            ...
            if (this.txt_desc.visible) {
                this.txt_desc.text = LangManager.Instance.GetTranslation("mountTip.soulsAdd", Math.ceil(mountTemplate ? mountTemplate.Power / 5 : 0));
            }
