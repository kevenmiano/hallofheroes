...
import { CampaignEvent, CampaignMapEvent, FreedomTeamEvent, NotificationEvent, OuterCityEvent, SpaceEvent } from "../../../../constant/event/NotificationEvent";
...
export class CampaignWalkLayer extends Laya.Sprite implements IEnterFrame {
    ...
    protected addEvent() {
        ...
        this._model.addEventListener(OuterCityEvent.ADD_GARRISON, this.__addBaseArmyHandler, this);
        this._model.addEventListener(OuterCityEvent.REMOVE_ARMY, this.__removeBaseArmyHandler, this);
        this._model.addEventListener(CampaignEvent.MOVE_ARMY_TO_RANGE, this.__moveArmyToRangeHandler, this);
        this._model.addEventListener(CampaignEvent.STANDPOS, this.__standPosHandler, this);
        this._model.addEventListener(CampaignMapEvent.GO_TO_POS, this.__updateArmyPosHandler, this);
        this._model.addEventListener(CampaignMapEvent.MOVE_PATHS_ARMY, this.__armyPosBroadHandler, this);
        this._model.addEventListener(CampaignMapEvent.MOVE_TO_VISIBLE, this.__armyMoveToVisibleHandler, this);
        this._model.addEventListener(CampaignMapEvent.MOVE_TO_UNVISIBLE, this.__armyMoveToUnVisibleHandler, this);
        this._model.addEventListener(OuterCityEvent.UPDATE_SELF, this.__updateSelfHandler, this);
    }
    ...
    protected removeEvent() {
        ...
        this._model.removeEventListener(OuterCityEvent.ADD_GARRISON, this.__addBaseArmyHandler, this);
        this._model.removeEventListener(CampaignEvent.STANDPOS, this.__standPosHandler, this);
        this._model.removeEventListener(OuterCityEvent.REMOVE_ARMY, this.__removeBaseArmyHandler, this);
        this._model.removeEventListener(CampaignEvent.MOVE_ARMY_TO_RANGE, this.__moveArmyToRangeHandler, this);
        this._model.removeEventListener(CampaignMapEvent.GO_TO_POS, this.__updateArmyPosHandler, this);
        this._model.removeEventListener(CampaignMapEvent.MOVE_PATHS_ARMY, this.__armyPosBroadHandler, this);
        this._model.removeEventListener(CampaignMapEvent.MOVE_TO_VISIBLE, this.__armyMoveToVisibleHandler, this);
        this._model.removeEventListener(CampaignMapEvent.MOVE_TO_UNVISIBLE, this.__armyMoveToUnVisibleHandler, this);
        this._model.removeEventListener(OuterCityEvent.UPDATE_SELF, this.__updateSelfHandler, this);
    }

    private __standPosHandler(data: any) {
        Logger.info("[CampaignWalkLayer]同步军队站立位置", data)
        let sPos: com.road.yishi.proto.campaign.StandPosMsg = <com.road.yishi.proto.campaign.StandPosMsg>data;
        let serverName: string = sPos.serverName;
        let aView: CampaignArmyView = this._armys.get(serverName + "_" + sPos.id) as CampaignArmyView;
    ...
    private checkPlayerVisible(armyView: CampaignArmyView) {
        if (armyView.data) {
            let armyViewVisible = armyView.isSelf ? true : CampaignManager.Instance.getScenePlayerVisible(armyView.data.mapId, armyView.data.online);
            let armyViewNameVisible = CampaignManager.Instance.getScenePlayerNameVisible(armyView.data.mapId, armyView.data.online);
            armyView.visible = armyView.isSelf ? true : armyViewVisible;
            armyView.active = armyView.isSelf ? true : armyViewVisible;
            armyView.showInfo(armyViewNameVisible, armyView.isSelf || armyView.visible);
        }
    }

    protected initRegisterList() {
        let arr: any[] = [WalkDepthMediator, WalkTargetMediator];
        this._mediatorKey = MediatorMananger.Instance.registerMediatorList(arr, this, CampaignWalkLayer.NAME);
    }
    ...
    protected checkMapNeededInit(): boolean {
        ...
        return WorldBossHelper.checkMineral(this._model.mapId) ||
    ...
    protected addAmryView(armyInfo: CampaignArmy, state: number = 0): CampaignArmyView {
        if (this._armys.get(armyInfo.key)) {
        ...
        if (WorldBossHelper.checkMapId(armyInfo.mapId)) {
            cArmy.avatarView.scaleX = cArmy.avatarView.scaleY = .85;
        } else {
            if (armyInfo.userId == PlayerManager.Instance.currentPlayerModel.playerInfo.userId) {
                isSelfCampaignArmy = true;
                //新手刚进入地图播放从屏幕外走进的人物动画, 防止闪现
                if (!NewbieModule.Instance.checkNodeFinish(NewbieConfig.NEWBIE_10) && WorldBossHelper.checkIsNoviceMapLayer1(this._model.mapId)) {
                    cArmy.visible = false;
                    cArmy.active = false;
                    cArmy.x = 0;
                }
        ...
        if (nextNode) {
            ...
            let temp: Laya.Point = StringHelper.interpolate(nextPoint, curPoint, 100 / leng);
            // 初始朝向设置
            if (cArmy.data.angle == -1) {
                cArmy.avatarView.angle = HeroAvatarViewHelper.twoPointAngle(nextPoint.x, nextPoint.y, curPoint.x, curPoint.y);
            } else {
    ...
    protected canRenderArmyView(): boolean {
        if (this._hideOtherPlayers) {
            return false;
        } else if (this.isLimitPlayerNum && this.renderList.length >= this.maxRenderPlayerNum) {
            return false;
        }
        return true;
    }
    ...
    private __armyPosBroadHandler(data: any) {
        ...
        let armyView: CampaignArmyView = this._armys.get(serverName + "_" + msg.armyId) as CampaignArmyView;
        if (armyView) {
            ...
            if (msg.routes.length > 0) {
                ...
                if (leng > 250) {
                    ...
                    if (armyView.info.pathInfo && armyView.info.pathInfo.length >= nextIndex) {
                        leftPath = armyView.info.pathInfo.slice(nextIndex);
                    }
                }
                let paths: Array<Laya.Point> = [];
                for (let i: number = 0; i < msg.routes.length; i++) {
                    paths.push(new Laya.Point(parseInt((msg.routes[i].x / 20).toString()), parseInt((msg.routes[i].y / 20).toString())));
                }
            ...
            let armyInfo: CampaignArmy = this._model.getBaseArmyByArmyId(msg.armyId, serverName);
            if (armyInfo) {
    ...
    private __updateArmyPosHandler(data: com.road.yishi.proto.army.ArmyPosUpdatedMsg) {
        Logger.info("[CampaignWalkLayer]把军队移动到某个位置", data)
        let msg: com.road.yishi.proto.army.ArmyPosUpdatedMsg = data;
        let armyView: CampaignArmyView = this._armys.get(msg.serverName + "_" + msg.armyId) as CampaignArmyView;
        if (armyView) {
            ...
            if (leng > 150) {
                armyView.x = ser.x;
                armyView.y = ser.y;
                armyView.info.pathInfo = SearchPathHelper.astartFindPath(ser, new Laya.Point(msg.endPosX * 20, msg.endPosY * 20));
            }
        }
    }
    ...
    public checkClickPlayerNum(posx: number, posy: number, viewData?) {
        ...
        setTimeout(() => { this.isClickLimit = false; }, 1000);
        ...
        this['_children'].forEach(element => {
            if (element.avatarBaseViewType == eAvatarBaseViewType.CampaignArmy) {
                ...
                if (roleView.data.baseHero.userId != PlayerManager.Instance.currentPlayerModel.playerInfo.userId) {
                    ...
                    if (ishit) {
                        ...
                        if (WorldBossHelper.checkMineral(roleView.data.mapId)) {//
                            if (roleView.data.state != ArmyState.STATE_FIGHT && !roleView.isSelfConsortia && !roleView.isSelfTeam) {
                                armyArr.push(roleView);
                            }
                        } else if (WorldBossHelper.checkPvp(roleView.data.mapId)) {//战场                         
                            if (roleView.data.state != ArmyState.STATE_FIGHT && roleView.data.teamId != CampaignManager.Instance.mapModel.selfMemberData.teamId) {
                                armyArr.push(roleView);
                            }
                        } else {
                            armyArr.push(roleView);
            ...
            else if (element.avatarBaseViewType == eAvatarBaseViewType.CampaignNpc) {
                let npcView = element;
                let ishit = npcView.avatarView.hitTestPoint(posx, posy);
                if (ishit) {
                    if (WorldBossHelper.checkPvp(mapId) && npcView.getNpcTypes() != PosType.TRANSPORT_CAR) {
        ...
        if (this.clickPlayerArr.length > 1) {
            ...
            if (nodeView.avatarBaseViewType == eAvatarBaseViewType.CampaignArmy) {
                ...
                if (WorldBossHelper.checkPvp(data.mapId) || WorldBossHelper.checkMineral(data.mapId)) {
                    NotificationManager.Instance.dispatchEvent(NotificationEvent.LOCK_PVP_WARFIGHT, data);
                }
            } else if (nodeView.avatarBaseViewType == eAvatarBaseViewType.CampaignNpc) {
            ...
            if (WorldBossHelper.checkPvp(mapId)) {
                ...
                CampaignManager.Instance.controller.moveArmyByPos(this.mouseX, this.mouseY, true);
            }
        }
    }
    ...
    public onClickHandler(evt: Laya.Event): boolean {
        ...
        Logger.yyz("🔔点击地面进行移动！");
        return CampaignManager.Instance.controller.moveArmyByPos(this.mouseX, this.mouseY, true);
    }
    ...
    private __moveArmyToRangeHandler(data: any) {
        let armyView: CampaignArmyView = this._armys.get(data.key) as CampaignArmyView;
        if (!armyView) {//系统部队
            let serverName: string = "";
            if (data.baseHero) {
                serverName = data.baseHero.serverName;
            }
            let armyInfo: CampaignArmy = this._model.getSysArmyByArmyId(data.id, serverName);
            if (armyInfo) {
