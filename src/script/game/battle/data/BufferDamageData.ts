...
export class BufferDamageData {
  ...
  public set templateId(value: number) {
    ...
    this.templateInfo = ConfigMgr.Instance.getTemplateByID(
  ...
  public getBufferName(bufferTemp?: t_s_skillbuffertemplateData): string {
    ...
    if (bufferTemp) {
      let bufferField: string = StringHelper.getSubStrBetweenTwoChar(
        "{",
        "}",
      ...
      if (StringHelper.isNullOrEmpty(bufferField)) {
        ...
        let bufferValue: number = bufferTemp.hasOwnProperty(bufferField)
          ? Math.abs(bufferTemp[bufferField]) *
            this.layerCount *
            Math.max(1, this.level)
          : 0;
