...
import FaceSlapInfo, {
  faceSlapImgInfoMsg,
} from "../datas/faceslap/FaceSlapInfo";
...
export default class FaceSlapManager {
  ...
  private initEvent() {
    ServerDataManager.listen(
      S2CProtocol.U_C_FACEIMG_LIST,
      this,
      this.__faceSlapHandler,
    );
  }
  ...
  public getAtyData(): faceSlapImgInfoMsg {
    ...
    for (let index = 0; index < count; index++) {
      ...
      if (
        nowServerTime > startTime &&
        nowServerTime < endTime &&
        !state &&
        !notInDate
      ) {
        return dataItem;
  ...
  private isTodayDate(atyId: string): boolean {
    ...
    if (
      today.getFullYear() == slapDate.getFullYear() &&
  ...
  public get canShow(): boolean {
    if (!this._slapData || PlayerManager.Instance.isExistNewbieMask)
      return false;
