...
export default class CarnivalGamePageItem extends FUI_CarnivalGamePageItem {
    ...
    protected onConstruct(): void {
        ...
        !CarnivalManager.Instance.isRewardTime && (this._startBtn.title = LangManager.Instance.GetTranslation("carnival.active.timeover"));
        ...
        if (this._tempInfo) {
            ...
            switch (this._tempInfo.Type) {
                ...
                case CarnivalModel.GAME_TYPE_2:
                    this.txt_title.text = LangManager.Instance.GetTranslation("NewMemoryCardFrame.titleText.text");
                    break;
                case CarnivalModel.GAME_TYPE_3:
                    this.txt_title.text = LangManager.Instance.GetTranslation("AirGardenGame.sudoku.title");
                    break;
                default:
                    this.txt_title.text = "";
                    break;
            ...
            for (let key in list) {
                if (Object.prototype.hasOwnProperty.call(list, key)) {
                    ...
                    if (Number(arr[0]) == this._tempInfo.Type) {
                        ...
                        this.txt_reward.text = LangManager.Instance.GetTranslation("carnival.game.time", leftNum);
                        isFind = true;
                        break;
                    }
                }
            }
            if (!isFind) this.txt_reward.text = LangManager.Instance.GetTranslation("carnival.game.time", countsValue[selectType - 1]);
        } else {
            this.txt_reward.text = LangManager.Instance.GetTranslation("carnival.game.time", countsValue[selectType - 1]);
        }
    }
    ...
    private startHandler() {
        ...
        CarnivalManager.Instance.opRequest(CarnivalManager.OP_GAME, this._tempInfo.Type);
    }
