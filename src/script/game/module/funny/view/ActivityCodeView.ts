 ...
 * 激活码
 */
export default class ActivityCodeView
  extends FUI_ActivityCodeView
  implements FunnyContent
{
  ...
  private initView() {
    ...
    this.title.text = LangManager.Instance.GetTranslation(
      "activity.view.ActivityItem.input",
    );
  ...
  private initEvent() {
    ...
    this.TextBox.getChild("userName").asTextInput.on(
      Laya.Event.KEY_DOWN,
      this,
  ...
  private __keyDownHandler(event: KeyboardEvent) {
    if (Number(event.code) == Laya.Keyboard.ENTER) this.__onClick(null);
  ...
  private removeEvent() {
    ...
    this.TextBox.getChild("userName").asTextInput.off(
      Laya.Event.KEY_DOWN,
      this,
      this.__keyDownHandler,
    );
