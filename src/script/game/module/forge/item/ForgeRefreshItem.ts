...
export default class ForgeRefreshItem extends FUI_ForgeRefreshItem {
    ...
    private onBtnLock() {
        if (!this.ctrl.clcikLockFirst && this.selected == true) {
            ...
            let content: string;
            content = LangManager.Instance.GetTranslation("store.view.refresh.RefreshAttriItem.content.refreshLock");
            ...
            SimpleAlertHelper.Instance.Show(SimpleAlertHelper.USEBINDPOINT_ALERT, { checkRickText: checkStr, checkRickText2: checkStr2, checkDefault: true }, null, content, null, null, this._confirmLockFirst.bind(this));
            return;
        }
        if (this._skillTempId == -1) {
            this.selected = false;
    ...
    public set randomSkill(id: number) {
        ...
        if (!temp) return
        this.txtCur.text = temp.SkillTemplateName + " (" + LangManager.Instance.GetTranslation("store.view.refresh.RefreshAttriItem.Grades", temp.Grades) + ")";
    ...
    public set randomNewSkill(id: number) {
        let temp: t_s_skilltemplateData = TempleteManager.Instance.getSkillTemplateInfoById(id);
        if (!temp) return
        this.txtNext.text = temp.SkillTemplateName + " (" + LangManager.Instance.GetTranslation("store.view.refresh.RefreshAttriItem.Grades", temp.Grades) + ")";
    }
