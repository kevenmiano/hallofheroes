import LangManager from "../lang/LangManager.js";
/**
 * 我们目前所用的K、M、G、T等都是英文表示方法, 分别是Kilo（103）、Mega（106）、Giga（109）、Tera（1012）的简写,
 * 更大的还有Peta（1015）、Exa（1018）、Bronto（1021）等。
 */
let moneyNum: number[] = [
  1000000000000000000000, 1000000000000000000, 1000000000000000, 1000000000000,
...
export default class StringHelper {
  ...
  public static repHtmlTextToFguiText(s: string): string {
    ...
    for (let index = 0; index < matchTags.length; index++) {
      ...
      if (
        beginElement.indexOf("textformat") != -1 ||
        beginElement.indexOf("font") != -1
      ) {
      ...
      if (matchTagsReplace) {
        for (const matchkey in matchTagsReplace) {
          if (
            Object.prototype.hasOwnProperty.call(matchTagsReplace, matchkey)
          ) {
  ...
  public static removeHtmlTagByTextField(str: string): string {
    if (StringHelper.isNullOrEmpty(str)) return "";
  ...
  public static trimWords(char: string = ""): string {
    ...
    for (let index = 0; index < count; index++) {
      ...
      if (StringHelper.swords.indexOf(element) > -1) {
        //含有标点符号 返回
   ...
   * @return
   */
  public static getSubStrBetweenTwoChar(
    char1: string,
    char2: string,
    str: string,
  ): string {
   ...
   *
   */
  public static complementByChar(
    str,
    length,
    char: string = " ",
    ignoreHtml: boolean = true,
  ) {
    str = str + "";
    var byteLen = this.strByteLen(
      ignoreHtml ? str.replace(StringHelper.HTML, "") : str,
    );
  ...
  public static addColor(content: string, color: number) {
    ...
    var colorStr;
    if (typeof color == "string") colorStr = String(color);
  ...
  public static numberToChinese(num: number): string {
    ...
    if (num <= 10) {
      ...
      if (gw == 0) {
        chinese =
          StringHelper.numTenToChinese(Math.floor(sw)) +
          LangManager.Instance.GetTranslation("public.number.num10");
      } else {
        if (sw < 2) {
          chinese =
            LangManager.Instance.GetTranslation("public.number.num10") +
            StringHelper.numTenToChinese(gw);
        } else {
          chinese =
            StringHelper.numTenToChinese(Math.floor(sw)) +
            LangManager.Instance.GetTranslation("public.number.num10") +
