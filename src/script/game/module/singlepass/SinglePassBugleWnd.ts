...
export default class SinglePassBugleWnd extends BaseWindow {
  ...
  private initEvent() {
    ...
    this._singlePassModel.addEventListener(
      SinglePassEvent.UPDATE_BUGLE_INFO,
      this.updateBugleInfoHandler,
      this,
    );
    this._singlePassModel.addEventListener(
      SinglePassEvent.UPDATE_BUGLE_REWARDS,
      this.updateBugleRewardsHandler,
      this,
    );
  ...
  private onClickItem(item: SinglePassBugleItem) {
    ...
    var msg: string;
    var goodsCount: number = GoodsManager.Instance.getGoodsNumByTempId(
      SinglePassModel.BUGLE_TEMPLATE_ID,
    );
    if (goodsCount <= 0) {
    ...
    if (this._currentPointsCost <= 0) {
      ...
      var lastSaveDate: Date = new Date(
        SharedManager.Instance.singlePassBugleOpenCardUseBindDate,
      );
      if (SharedManager.Instance.checkIsExpired(lastSaveDate)) {
        ...
        this.openCard(
          this._currentIndex,
          SharedManager.Instance.singlePassBugleOpenCardUseBind,
        );
      }
    }
  }
  ...
  private showAlertFrame() {
    let content: string = LangManager.Instance.GetTranslation(
      "singlepass.bugle.SinglePassBugleView.OpenTips",
      this._currentPointsCost,
    );
    UIManager.Instance.ShowWind(EmWindow.TodayNotAlert, {
      content: content,
      backFunction: this.__todayNotAlertOpen.bind(this),
      state: 0,
      point: this._currentPointsCost,
    });
  }
  ...
  private updateBugleInfoHandler(evtData: SinglePassBugleInfo) {
    ...
    this.aginBtn.visible = false;
    this.numTxt.text = GoodsManager.Instance.getGoodsNumByTempId(
      SinglePassModel.BUGLE_TEMPLATE_ID,
    ).toString();
  }
  ...
  private helpBtnHandler() {
    ...
    let content: string = LangManager.Instance.GetTranslation(
      "singlepass.view.SinglePassBugleHelpFrame.helpContent",
    );
