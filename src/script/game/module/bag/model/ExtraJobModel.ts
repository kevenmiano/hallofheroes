...
export default class ExtraJobModel extends FrameDataBase {
    ...
    constructor() {
        ...
        let cfg = TempleteManager.Instance.getConfigInfoByConfigName("ExtraJob_EquipGemOpen");//  "ConfigValue": "2,3,4,5","Description": "【专精】魂器宝石孔开放所需阶数",
        if (cfg) {
        ...
        for (let i = 0; i < this._activeList.length; i++) {
            ...
            if (element) {
                ...
                if (isActived) {
                    ...
                    var nextInfo: t_s_extrajobData = TempleteManager.Instance.getExtrajobCfg(element.jobType, element.jobLevel + 1);
                    if (nextInfo) {
        ...
        for (let i = 0; i < this._equipList.length; i++) {
            ...
            if (element) {
                ...
                if (!isActived) {
                    //是否可升级
                    //是否可激活
                    let cfg = TempleteManager.Instance.getExtrajobEquipCfg(element.equipType, 1) as t_s_extrajobequipData;
                    if (cfg) {
                        //是否可以解锁
    ...
    private updateTotalLevel() {
        ...
        this.canActiveNum = 0;
        this.nextLevel = 0;
        let cfg = TempleteManager.Instance.getConfigInfoByConfigName("ExtraJob_CountActive");
        if (cfg) {
            let value: string = cfg.ConfigValue;
            if (value) {
                let arr = value.split(',');
 ...
 */
    ...
    public getAttr(info: ExtraJobEquipItemInfo) {
        ...
        let strenCfg: t_s_extrajobequipstrengthenData = TempleteManager.Instance.getExtrajobEquipStrenthenCfg(info.strengthenLevel);
        let cfg = TempleteManager.Instance.getExtrajobEquipCfg(info.equipType, info.equipLevel);
        if (cfg) {
            let propertyObject: { [key: string]: number } = {}
            if (cfg.Attack > 0) {
                let pVal: number = cfg.Attack;
                //强化等级增加的属性（绿色字体） 强化属性=强化加成百分比*魂器当前阶数的属性
                let addValue = strenCfg ? strenCfg.ExtraPropertyPercent / 100 * cfg.Attack : 0;
                if (strenCfg) {
                    pVal += Math.floor(addValue);
                }
                propertyObject[LangManager.Instance.GetTranslation("armyII.ThaneAttributeView.Tip13")] = pVal;
            }
            if (cfg.Defence > 0) {
                let pVal: number = cfg.Defence;
                //强化等级增加的属性（绿色字体） 强化属性=强化加成百分比*魂器当前阶数的属性
                let addValue = strenCfg ? strenCfg.ExtraPropertyPercent / 100 * cfg.Defence : 0;
                if (strenCfg) {
                    pVal += Math.floor(addValue);
                }
                propertyObject[LangManager.Instance.GetTranslation("armyII.ThaneAttributeView.Tip14")] = pVal;
            }
            if (cfg.MagicAttack > 0) {
                let pVal: number = cfg.MagicAttack;
                //强化等级增加的属性（绿色字体） 强化属性=强化加成百分比*魂器当前阶数的属性
                let addValue = strenCfg ? strenCfg.ExtraPropertyPercent / 100 * cfg.MagicAttack : 0;
                if (strenCfg) {
                    pVal += Math.floor(addValue);
                }
                propertyObject[LangManager.Instance.GetTranslation("armyII.ThaneAttributeView.Tip15")] = pVal;
            }
            if (cfg.MagicDefence > 0) {
                let pVal: number = cfg.MagicDefence;
                //强化等级增加的属性（绿色字体） 强化属性=强化加成百分比*魂器当前阶数的属性
                let addValue = strenCfg ? strenCfg.ExtraPropertyPercent / 100 * cfg.MagicDefence : 0;
                if (strenCfg) {
                    pVal += Math.floor(addValue);
                }
                propertyObject[LangManager.Instance.GetTranslation("armyII.ThaneAttributeView.Tip16")] = pVal;
            }
            if (cfg.Live > 0) {
                let pVal: number = cfg.Live;
                //强化等级增加的属性（绿色字体） 强化属性=强化加成百分比*魂器当前阶数的属性
                let addValue = strenCfg ? strenCfg.ExtraPropertyPercent / 100 * cfg.Live : 0;
                if (strenCfg) {
                    pVal += Math.floor(addValue);
                }
                propertyObject[LangManager.Instance.GetTranslation("armyII.ThaneAttributeView.Tip11")] = pVal;
            }
            this._allPropertys.push(propertyObject);
            for (let i = 1; i <= 4; i++) {//镶嵌宝石增加的属性
                const tempId = info['join' + i];
                if (tempId > 0) {
                    let temp: t_s_itemtemplateData = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_itemtemplate, tempId);
    ...
    public parseJewelAddProperty(templateInfo: t_s_itemtemplateData) {
        ...
        let addValue: number = Math.floor((templateInfo.totalAttribute * GoodsHelp.getJewelEffecyByGrade(this.thane.jewelGrades)) / 100);

        if (templateInfo.Power)
            propertyObject[LangManager.Instance.GetTranslation("armyII.ThaneAttributeView.Tip01")] = templateInfo.Power + addValue;
        if (templateInfo.Agility)
            propertyObject[LangManager.Instance.GetTranslation("armyII.ThaneAttributeView.Tip02")] = templateInfo.Agility + addValue;
        if (templateInfo.Intellect)
            propertyObject[LangManager.Instance.GetTranslation("armyII.ThaneAttributeView.Tip03")] = templateInfo.Intellect + addValue;
        if (templateInfo.Physique)
            propertyObject[LangManager.Instance.GetTranslation("armyII.ThaneAttributeView.Tip04")] = templateInfo.Physique + addValue;
        if (templateInfo.Captain)
            propertyObject[LangManager.Instance.GetTranslation("armyII.ThaneAttributeView.Tip05")] = templateInfo.Captain + addValue;
        if (templateInfo.Attack)
            propertyObject[LangManager.Instance.GetTranslation("armyII.ThaneAttributeView.Tip13")] = templateInfo.Attack + addValue;
        if (templateInfo.Defence)
            propertyObject[LangManager.Instance.GetTranslation("armyII.ThaneAttributeView.Tip14")] = templateInfo.Defence + addValue;
        if (templateInfo.MagicAttack)
            propertyObject[LangManager.Instance.GetTranslation("armyII.ThaneAttributeView.Tip15")] = templateInfo.MagicAttack + addValue;
        if (templateInfo.MagicDefence)
            propertyObject[LangManager.Instance.GetTranslation("armyII.ThaneAttributeView.Tip16")] = templateInfo.MagicDefence + addValue;
        if (templateInfo.ForceHit)
            propertyObject[LangManager.Instance.GetTranslation("armyII.ThaneAttributeView.Tip10")] = templateInfo.ForceHit + addValue;
        if (templateInfo.Live)
            propertyObject[LangManager.Instance.GetTranslation("armyII.ThaneAttributeView.Tip11")] = templateInfo.Live + addValue;
        if (templateInfo.Parry)
            propertyObject[LangManager.Instance.GetTranslation("armyII.ThaneAttributeView.Tip19")] = templateInfo.Parry + addValue;
