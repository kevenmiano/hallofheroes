...
export class BattleUtils {
  ...
  public static toStringAttackOrder(value: AttackOrderMsg): string {
    ...
    str +=
      "attackMillis : " +
      DateFormatter.parse(value.attackMillis, "YYYY-MM-DD hh:mm:ss").getTime() +
      "\t";
    ...
    for (
      ...
      for (
        ...
        point.x =
          centerPosX -
          (teamGas / 2) * rowOffset -
          index * BattleUtils.gasColumn * rowOffset -
          rowOffestArr[j] * rowOffset;
  ...
  public static rolePointByPos(pos: number, face: number): Laya.Point {
    ...
    if (battleModel && battleModel.selfSide == 1) {
      posArr =
        face == FaceType.LEFT_TEAM ? BattleUtils.posLeft : BattleUtils.posRight;
    } else {
      posArr =
        face == FaceType.RIGHT_TEAM
          ? BattleUtils.posRight
          : BattleUtils.posLeft; //如果是多人对战的防守方需要反过来;
    }
    if (face == 1) {
      switch (pos) {
        case 1:
  ...
  public static attackPointByPos(
    pos: number,
    face: number,
    off: number = 1,
    lineup: number = 0,
  ...
  public static ManagerDepth(dc: Laya.Sprite) {
    ...
    for (let key in dic) {
      ...
      dic.forEach((item: DepthSprite) => {
        num++;
        if (
          childMc.pointY < item.pointY &&
          dc.getChildIndex(childMc) > dc.getChildIndex(item)
        ) {
    ...
    function executeBleed() {
      ...
      for (let j: number = 0; j < singelAttackArr.length; j++) {
        ...
        if (role) {
          if (attackRole) {
            Logger.battle(
              `[BattleUtils]${attackRole.roleName}(${element.fId})对${role.roleName}(${element.roleId})产生伤害${element.damageValue}`,
            );
          }
          ...
          resistModel.currentResistSide =
            role.side == battleModel.selfSide ? 1 : 2;
   ...
   *
   */
  public static addSingleDannyTakeEffect(
    singelAttackArr: any[],
    dannyEffectData: object,
    $frame: number = 12,
    $checkDie: boolean = true,
    distance: number = 0,
    flyUpPower: number = 0,
  ...
  public static skillUpdateSp(skillData: SkillData, currentRole: any) {
    ...
    let addSpContainer;
    if (skillData.skillId == 3 || skillData.skillId == 4) {
      // 3和4 是胜利和失败
      return;
    }
    ...
    if (
      battleUIView &&
      currentRole instanceof HeroRoleInfo &&
      currentRole == BattleManager.Instance.battleModel.selfHero
    ) {
