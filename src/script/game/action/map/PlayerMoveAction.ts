...
export default class PlayerMoveAction extends MapBaseAction {
  ...
  prepare() {
    ...
    super.prepare();
    if (
      CampaignManager.Instance.mapModel.selfMemberData.id == this._msg.userId
    ) {
      MouseData.Instance.curState = MouseData.LOCK;
      if (this._msg.ismoveCamera) {
        CampaignManager.Instance.mapView.moveCenterPoint(
          new Laya.Point(
            this._msg.tarX * Tiles.WIDTH,
            this._msg.tarY * Tiles.HEIGHT,
          ),
        );
        CampaignManager.Instance.mapModel.updateFog(
          this._msg.tarX * Tiles.WIDTH,
          this._msg.tarY * Tiles.HEIGHT,
          FogGridType.OPEN_TWO,
        );
        MouseData.Instance.curState = MouseData.LOCK;
      }
      CampaignManager.Instance.controller.moveArmyByPos(
        this._msg.tarX * Tiles.WIDTH - 10,
        this._msg.tarY * Tiles.HEIGHT - 10,
      );
    } else {
      var prepareErrorTip: string = LangManager.Instance.GetTranslation(
        "yishi.actions.map.PlayerMoveAction.prepareErrorTip",
      );
      throw new Error(prepareErrorTip);
    }
    var aInfo: CampaignArmy =
      CampaignManager.Instance.mapModel.getBaseArmyByArmyId(this._msg.userId);
    var armyView: object =
      CampaignManager.Instance.controller.getArmyView(aInfo);
  ...
  update() {
    if (
      ...
      !this._playerView ||
      this._playerView.aiInfo.pathInfo.length == 0
    ) {
