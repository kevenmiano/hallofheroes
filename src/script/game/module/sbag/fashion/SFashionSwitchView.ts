...
export class SFashionSwitchView extends FUI_SFashionSwitchView {
    ...
    checkRedDot() {
        ...
        for (let i = 0; i < 4; i++) {
            this.list_tab.getChildAt(i).asButton.getChild('redDot').visible = this.fashionModel.redPointArr[i];
        }
    }
    ...
    private getCurrentFashionList(index: number): t_s_itemtemplateData[] {
        ...
        for (let index = 0; index < count; index++) {
            element = arr[index];
            isActive = element.Activation == 1 ? this.fashionModel.bookList.has(element.TemplateId) : true;
            if (isActive)
        ...
        tempData.sort((a, b) => {
            ...
            if (a.TemplateId in this.fashionModel.bookList) {
                if (this.fashionModel.hasIdentityedBook(a.TemplateId)) {//鉴定过
                    valA_2 = 50
                } else {
                    valA_2 = 10
                }
            ...
            if (b.TemplateId in this.fashionModel.bookList) {
                if (this.fashionModel.hasIdentityedBook(b.TemplateId)) {//鉴定过
                    valB_2 = 50
                } else {
                    valB_2 = 10
                }
    ...
    removeEvent() {
        ...
        FashionManager.Instance.removeEventListener(FashionEvent.FASHION_BOOK_RECEIVE, this.__receivedBookHandler, this);
    }
