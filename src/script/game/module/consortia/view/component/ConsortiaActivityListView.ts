...
export class ConsortiaActivityListView extends FUI_ConsortiaActivityListView {
  ...
  private initEvent() {
    ...
    ConsortiaManager.Instance.addEventListener(
      ConsortiaEvent.DEMON_SWITCH,
      this.__demonSwitchHandler,
      this,
    );
    this._model.addEventListener(
      ConsortiaEvent.UPDA_CONSORTIA_INFO,
      this.updateConsortiaInfo,
      this,
    );
    ...
    NotificationManager.Instance.addEventListener(
      ...
      this.treasureMineralUpdate,
      this,
    );
    NotificationManager.Instance.addEventListener(
      NotificationEvent.OUTYARD_STATE_INFO,
      this.updateOutyardInfo,
      this,
    );
    NotificationManager.Instance.addEventListener(
  ...
  private skillInit() {
    ...
    if (this._model.consortiaInfo.levels < 3) {
      activitySkillInfo.promptTxt = LangManager.Instance.GetTranslation(
        "consortia.view.myConsortia.activity.ConsortiaActivityListView.openSkillPanel",
      );
  ...
  private secretInit() {
    ...
    if (this._model.secretInfo.remainGainTime > 0) {
      activitySecretInfo.promptTxt = LangManager.Instance.GetTranslation(
        "consortia.view.myConsortia.activity.ConsortiaActivityListView.treeisOpen",
      );
    } else {
      activitySecretInfo.promptTxt = LangManager.Instance.GetTranslation(
        "consortia.view.myConsortia.activity.ConsortiaActivityListView.treeIsClose",
      );
    }
  ...
  private outYardInt() {
    ...
    activityGvgInfo.promptTxt = LangManager.Instance.GetTranslation(
      "consortia.view.myConsortia.activity.ConsortiaActivityListView.notInList",
    );
    let stateMsg: StackHeadStateMsg = OutyardManager.Instance.stateMsg;
    if (stateMsg) {
      if (OutyardManager.Instance.model.checkIsOpenDay()) {
        //如果是比赛日
        if (stateMsg.state == OutyardModel.OVER) {
          ...
          if (OutyardManager.Instance.model.isOpenTime()) {
            //在开放时间段内
            //争夺中
            activityGvgInfo.promptTxt = LangManager.Instance.GetTranslation(
              "consortia.view.myConsortia.activity.gvgTxt3",
            );
          } else if (stateMsg.state == OutyardModel.COLLECT) {
            activityGvgInfo.promptTxt = LangManager.Instance.GetTranslation(
              "consortia.view.myConsortia.activity.gvgTxt1",
            );
          } else if (stateMsg.state == OutyardModel.READY) {
            //备战中
            activityGvgInfo.promptTxt = LangManager.Instance.GetTranslation(
              "consortia.view.myConsortia.activity.gvgTxt2",
            );
          }
        }
      }
      if (this.isConsortiaChairman(ConsortiaDutyInfo.STACKHEAD_SIGNIN)) {
        //会长
        if (!stateMsg.myGuildSignup) {
          //未报名
          activityGvgInfo.btnLabel = LangManager.Instance.GetTranslation(
            "consortia.outYardInt.text",
          ); //报名
        } else {
          activityGvgInfo.btnLabel = LangManager.Instance.GetTranslation(
            "consortia.view.myConsortia.activity.ConsortiaActivityListView.goto",
          ); //前往
        }
      } else {
        //非会长
  ...
  private enterOutyard() {
    ...
    if (OutyardManager.Instance.model.checkIsOpenDay()) {
      ...
      //非比赛日
      let time = stateMsg.nextOpenTime;
      ...
      if (nextOpenTime == 0) {
        MessageTipManager.Instance.show(
          LangManager.Instance.GetTranslation(
            "outyard.TopToolsBar.noOpenToday",
          ),
        );
        return;
      }
      ...
      if (stateMsg.state == OutyardModel.NOT_OPEN) {
        //今天不开放
        if (stateMsg.isSpan) {
          MessageTipManager.Instance.show(
            LangManager.Instance.GetTranslation(
      ...
      if (stateMsg.state == OutyardModel.COLLECT) {
        ...
        MessageTipManager.Instance.show(
          LangManager.Instance.GetTranslation(
            "outyard.OutyardFrame.gatherToday",
          ),
        );
        return;
      }
  ...
  private gvgInit() {
    ...
    activityGvgInfo.btnLabel = LangManager.Instance.GetTranslation(
      "consortia.view.myConsortia.activity.ConsortiaActivityListView.managerList",
    );
    activityGvgInfo.promptTxt = LangManager.Instance.GetTranslation(
      "consortia.view.myConsortia.activity.ConsortiaActivityListView.notInList",
    );
    ...
    if (this.playerInfo.gvgIsOpen) {
      ...
      if (this.isInFightList()) {
        activityGvgInfo.promptTxt = LangManager.Instance.GetTranslation(
          "consortia.view.myConsortia.activity.ConsortiaActivityListView.warIsOpen",
        );
      } else {
        activityGvgInfo.promptTxt = LangManager.Instance.GetTranslation(
          "consortia.view.myConsortia.activity.ConsortiaActivityListView.notInList",
      ...
      //未开始 判断下次开始时间
      activityGvgInfo.promptTxt = LangManager.Instance.GetTranslation(
        "consortia.view.myConsortia.activity.ConsortiaActivityListView.warIsClose",
      );
    }
  ...
  private consortiaBossInit() {
    ...
    if (this._model.consortiaInfo.levels >= 6) {
      ...
      activityBossInfo.promptTxt = LangManager.Instance.GetTranslation(
        "consortia.view.myConsortia.activity.ConsortiaActivityListView.bossPromptTxt" +
          this._model.bossInfo.state,
      );
      if (this._contorller.getRightsByIndex(ConsortiaDutyInfo.CALL_BOSS)) {
        activityBossInfo.btnLabel = LangManager.Instance.GetTranslation(
          "consortia.view.myConsortia.activity.ConsortiaActivityListView.bossBtnLabel01",
        );
        activityBossInfo.operatingFun = this.operatBossHandler.bind(this);
      } else {
        activityBossInfo.btnLabel = LangManager.Instance.GetTranslation(
          "consortia.view.myConsortia.activity.ConsortiaActivityListView.bossBtnLabel02",
        );
  ...
  private consortiaTreasureInit() {
    ...
    consortiaTreasure.promptTxt = LangManager.Instance.GetTranslation(
      "consortia.view.myConsortia.activity.ConsortiaActivityListView.consortiaTreasure" +
        PlayerManager.Instance.currentPlayerModel.treasureState,
    );
    consortiaTreasure.btnLabel = LangManager.Instance.GetTranslation(
      "PetChallengeInfoView.lookReward",
    );
  ...
  private goToOuterCity() {
    ...
    if (SceneManager.Instance.currentType == SceneType.OUTER_CITY_SCENE) {
      str = LangManager.Instance.GetTranslation(
        "consortiaActivityListView.goToOuterCity.tips",
      );
      MessageTipManager.Instance.show(str);
      return;
    }
  ...
  private devilInit() {
    ...
    activityDevilInfo.btnLabel = LangManager.Instance.GetTranslation(
      "consortia.view.myConsortia.activity.ConsortiaActivityListView.openDevil",
    );
    if (this._model.demonInfo.state == 0) {
      //魔神祭祀已开启 0 可开启, 1 开启中, 2 已结束
      activityDevilInfo.promptTxt = LangManager.Instance.GetTranslation(
        "consortia.view.myConsortia.activity.ConsortiaActivityListView.devilisClose",
      );
      activityDevilInfo.isEnabled = true;
      this.checkStart(activityDevilInfo);
    } else if (this._model.demonInfo.state == 1) {
      //未开启
      activityDevilInfo.promptTxt = LangManager.Instance.GetTranslation(
        "consortia.view.myConsortia.activity.ConsortiaActivityListView.devilisOpen",
      );
      activityDevilInfo.isEnabled = false;
      this.checkStart(activityDevilInfo);
    } else if (this._model.demonInfo.state == 2) {
      //已结束
      activityDevilInfo.promptTxt = LangManager.Instance.GetTranslation(
        "consortia.view.myConsortia.activity.ConsortiaActivityListView.devilisEnd",
      );
      activityDevilInfo.isEnabled = false;
      this.checkStart(activityDevilInfo);
    } //异常
  ...
  private openDemon() {
    ...
    if (canJoin > 0) {
      let isChairman: boolean = this._contorller.getRightsByIndex(
        ConsortiaDutyInfo.UPDATEBUILDING,
      );
      if (isChairman && canJoin != 103) {
        this._contorller.openDevilFrame();
      } else {
        MessageTipManager.Instance.show(
          this._model.demonInfo.getLimitTip(canJoin),
        );
      }
    }
  }
  ...
  private removeEvent() {
    ...
    ConsortiaManager.Instance.removeEventListener(
      ConsortiaEvent.DEMON_SWITCH,
      this.__demonSwitchHandler,
      this,
    );
