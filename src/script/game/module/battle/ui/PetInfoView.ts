...
export default class PetInfoView extends FUI_PetInfoView {
      ...
      for (let i: number = 0; i < len; i++) {
        ...
        if (indexArr[i] > 0) {
          ...
          if (!this.hasFirstPetData) {
            ...
            this._hero = this.battleModel.getBaseRoleInfoByPetTemplateId(
              this._selectedPetData.templateId,
            );
  ...
  private updateStatus() {
    ...
    NotificationManager.Instance.dispatchEvent(
      ...
      this._selectedPetData,
    );
  }

  private onKeyDownHandler(event: Laya.Event) {
    if (Laya.Browser.now() - this._intervalTime < 100) {
      //切换操作会有0.1~0.2秒左右的隐藏CD（不能无限点击，防止控制标识乱飘）
      return;
    }
  ...
  private setSelectPetData(keyCode: number): PetData {
    ...
    if (shortcutCharUpper1 && keyCode == shortcutCharUpper1.charCodeAt(0)) {
      //按下"A"
      petData = (this.p1 as PetInfoItem).petData;
    } else if (
      shortcutCharUpper2 &&
      keyCode == shortcutCharUpper2.charCodeAt(0)
    ) {
      petData = (this.p2 as PetInfoItem).petData;
    } else if (
      shortcutCharUpper3 &&
      keyCode == shortcutCharUpper3.charCodeAt(0)
    ) {
  ...
  private getNewSelectedPetData() {
    ...
    for (let i: number = 0; i < 3; i++) {
      ...
      if (petInfoItem && petInfoItem.petData) {
        hero = this.battleModel.getBaseRoleInfoByPetTemplateId(
          petInfoItem.petData.templateId,
        );
  ...
  private _updateView() {
    ...
    for (let i: number = 0; i < len; i++) {
      ...
      if (
        item.petData &&
        item.petData.templateId == this._selectedPetData.templateId
      ) {
  ...
  setCurrentItemInfo() {
    this.currentPetIcon.icon = IconFactory.getPetHeadSmallIcon(
      this._selectedPetData.templateId,
    );
    if (this._hero) {
      this.txtHp.text = this._hero.bloodA + "/" + this._hero.totalBloodA;
