...
export class SocketDataProxyManager extends GameEventDispatcher {
  ...
  public readSocketDataByType(sceneType: string) {
    let scene: BaseSceneSocketDataProxy = this._sceneList[
      sceneType
    ] as BaseSceneSocketDataProxy;
    if (scene) {
  ...
  private chekckVehicle(
    ...
    if (
      ...
      pkg.code == S2CProtocol.U_C_PLAYER_SPACE_CREATE
    ) {
      let base: BaseSceneSocketDataProxy = this._sceneList[
        sceneType
      ] as BaseSceneSocketDataProxy;
      if (base) {
        base.dispose();
