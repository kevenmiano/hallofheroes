...
import { AvatarInfoUIEvent, NotificationEvent } from "../../../../constant/event/NotificationEvent";
 ...
 * 战场中人物名称的颜色只有两种
 */
export class PvpWarFightArmyView extends CampaignArmyView implements IBaseMouseEvent {
    ...
    public mouseClickHandler(evt: Laya.Event): boolean {
        ...
        if (this._avatar && (<HeroAvatar>this._avatar).getCurrentPixels() >= 0) {
            //判断是否玩家重叠
            (this.parent as CampaignWalkLayer).checkClickPlayerNum(evt.stageX, evt.stageY,this.data);
            return true;
        }
        return false;
    }
    ...
    public attackFun(){
        NotificationManager.Instance.dispatchEvent(NotificationEvent.LOCK_PVP_WARFIGHT, this._data);
    }
    ...
    private get inPvpMapAndNotSelfTeamAndNotInFight(): boolean {
        return (WorldBossHelper.checkPvp(this.mapModel.mapId) &&
