...
export class TranseferMediator implements IMediator
{
    ...
    public register(target:Object):void
    {
        ...
        PlayerManager.Instance.addEventListener(ServiceReceiveEvent.TRANSEFER_CASTLE_SUCCESS, this.__transeferHandler, this);
        NotificationManager.Instance.addEventListener(NotificationEvent.SWITCH_MAP_TRANSFER, this.__switchMapTranseferHandler, this);
    }

    public unregister(target:Object):void
    {
        this._target = null;
        PlayerManager.Instance.removeEventListener(ServiceReceiveEvent.TRANSEFER_CASTLE_SUCCESS, this.__transeferHandler, this);
        NotificationManager.Instance.removeEventListener(NotificationEvent.SWITCH_MAP_TRANSFER, this.__switchMapTranseferHandler, this);
    }
    ...
    {
        let cInfo:BaseCastle = PlayerManager.Instance.currentPlayerModel.mapNodeInfo;
        ...
        let file:string = MapUtils.getFileName(model.targetPoint.x, model.targetPoint.y, 1000, 1000);
        model.upPreNineSliceScaling(file);
        model.targetPoint.x = cInfo.x;
        model.targetPoint.y = cInfo.y;
        let file2:string = MapUtils.getFileName(model.targetPoint.x, model.targetPoint.y, 1000, 1000);
