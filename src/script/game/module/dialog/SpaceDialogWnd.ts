...
export class SpaceDialogWnd extends BaseWindow {
    ...
    private initView() {
        ...
        if (this._nodeInfo.nodeId == 17 && ArmyManager.Instance.thane.grades >= 35) {//环任务
    ...
    private initOptions() {
        ...
        if (this._nodeInfo.param3) {
            ...
            if (RingTaskManager.Instance.getRingTaskType() == RewardConditionType.TALKTASK) {
                ...
                {
                    let str: string = "11," + LangManager.Instance.GetTranslation("map.space.view.frame.SpaceDialogFrame.RingTask") + ",35";
                    options.push(str);
                }
            }
                ...
                if (p.optionType == DialogOptionItem.STORY_TASK) {
                    ...
                    if (TaskManage.Instance.cate.hasTask(task)) {
                        if (task.isCompleted) {
                            ...
                            //接取,进行中
                            p.optionTitle += LangManager.Instance.GetTranslation("YiShi.mainBar.view.TaskTraceItem.Doing");
                        }
                    } else {
                        //没接取
                    ...
                    RingTaskManager.Instance.addEventListener(RingTaskEvent.COMPLETETASK, this.ringTaskCompleteHandler, this);
                    ...
                    p.optionTitle = LangManager.Instance.GetTranslation("map.space.view.frame.SpaceDialogFrame.TreasureMapRewardTipsTxt", p.optionTitle);
                } else {
    ...
    private stringToObject(str: string): Object {
        let p: any = { optionType: 0, optionTitle: "", optionLevel: 0, optionParam: 0 };
        if (!str) {
            return p;
        }
    ...
    private storyTaskNextStep(param: any) {
        ...
        RingTaskManager.Instance.removeEventListener(RingTaskEvent.COMPLETETASK, this.ringTaskCompleteHandler, this);
        ...
        if (param.currentStep >= param.totalStep) {
            ...
            this.resetFrame(contents[0], contents[1], this.storyTaskNextStep.bind(this), param);
            param.currentStep++;
    ...
    private changeDialogContext(param: Object = null, b: boolean = true) {
        ...
        if (nodeId == this._nodeInfo.nodeId) {
            ...
            dialogItemInfo.title = LangManager.Instance.GetTranslation("map.space.view.frame.SpaceDialogFrame.RingTask.completeTask");
            this._list1Data.push(dialogItemInfo);
            this.list1.numItems = this._list1Data.length;
            this.list1.resizeToFit();
        } else {
            let condition: t_s_rewardcondictionData = RingTaskManager.Instance.getCurrCondition();
            ...
            dialogItemInfo.title = LangManager.Instance.GetTranslation("map.space.view.frame.SpaceDialogFrame.RingTask.goto");
    ...
    private getRingTaskTalkContent(): string {
        ...
        if (condition.CondictionType == RewardConditionType.COMMITITEM) {
            ...
            switch (ringTask.currTask.condition_2) {
                ...
                case ShopGoodsInfo.ATHLETICS_SHOP:
                    shopName = LangManager.Instance.GetTranslation("map.space.view.frame.SpaceDialogFrame.pvp");
                    break;
                case ShopGoodsInfo.MAZE_SHOP:
                    shopName = LangManager.Instance.GetTranslation("map.space.view.frame.SpaceDialogFrame.Maze");
                    break;
            }
            content = content.replace("{1}", shopName);
            let itemName: string = "";
            if (ringTask.currTask.condition_3 > 0) //物品ID
            {
        ...
        } else if (condition.CondictionType == RewardConditionType.KILLDUPLICATEMONSTER) {
            let str: string = LangManager.Instance.GetTranslation("map.space.view.frame.SpaceDialogFrame.killMonster");
            let _campaignTemp: t_s_campaignData = TempleteManager.Instance.getCampaignTemplateByID(ringTask.currTask.condition_2);
            str = str.replace("{0}", _campaignTemp.CampaignNameLang);
            str = str.replace("{2}", ringTask.currTask.condition_4.toString());
            content = str;
        } else {
            let temp: t_s_rewardcondictionData = RingTaskManager.Instance.getCurrCondition();
    ...
    private resetFrame(txt: string, opString: string, callFun: Function, param: Object) {
    ...
    private addEvent() {
        if (this.rewardLookBtn) this.rewardLookBtn.onClick(this, this.rewardLookBtnHandler);
    ...
    private removeEvent() {
        if (this.rewardLookBtn) this.rewardLookBtn.offClick(this, this.rewardLookBtnHandler);
    ...
    private addToList(optionItem: DialogOptionItem) {
        ...
        if (!ConfigManager.info.SYS_OPEN && this._nodeInfo.nodeId == SpaceNodeType.PET_CAMPAIGN) {
