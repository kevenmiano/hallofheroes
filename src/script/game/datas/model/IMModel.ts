...
export default class IMModel {
    ...
    private initStateList() {
        ...
        online.name = LangManager.Instance.GetTranslation("friends.im.view.IMStateComBox.online");
        leave.id = StateType.LEAVE;
        leave.name = LangManager.Instance.GetTranslation("friends.im.view.IMStateComBox.leave");
        leave.replyContent = replyList && replyList[leave.id] ? replyList[leave.id] : LangManager.Instance.GetTranslation("friends.im.view.IMStateComBox.leaveReply");
        busy.id = StateType.BUSY;
        busy.name = LangManager.Instance.GetTranslation("friends.im.view.IMStateComBox.busy");
        busy.replyContent = replyList && replyList[busy.id] ? replyList[busy.id] : LangManager.Instance.GetTranslation("friends.im.view.IMStateComBox.leaveReply");
        noDisturb.id = StateType.NO_DISTURB;
        noDisturb.name = LangManager.Instance.GetTranslation("friends.im.view.IMStateComBox.noDisturb");
        noDisturb.replyContent = replyList && replyList[noDisturb.id] ? replyList[noDisturb.id] : LangManager.Instance.GetTranslation("friends.im.view.IMStateComBox.leaveReply");
    ...
    public set curState(stateId: number) {
        ...
        NotificationManager.Instance.sendNotification(NotificationEvent.IMSTATE_CHANGE, this._stateList[this._curState]);
    }
