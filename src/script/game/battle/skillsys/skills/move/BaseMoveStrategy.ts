...
export class BaseMoveStrategy implements IMoveStrategy {
  ...
  protected getToRole(): any {
    if (
      this.getSkillData().data.length < 1 ||
      this.getSkillData().data[0].length < 1
    ) {
      return null;
    }
    let item: AttackData = this.getSkillData().data[0][0];
    //	BaseRoleInfo
    let toRole: any = BattleManager.Instance.battleModel.getRoleById(
      item.roleId,
    );
    return toRole;
  }
  ...
  protected getMoveForwardPoint(role: any): Laya.Point {
    ...
    // else
    //     pos = role.pos;
    if (moveFrameData.target == 0) pos = role.pos;
    else pos = moveFrameData.target;
  ...
  protected getMoveBackPoint(): Laya.Point {
    return BattleUtils.rolePointByPos(
      this._skill.getCurrentRole().pos,
      this._skill.getCurrentRole().face,
    );
  }
  ...
  protected getMoveForwardFrameData(): MoveFrameData {
    return this._skill
      .getActionTemplate()
      .getMoveForwardData(this._skill.getRoleSex()).moveForwardData;
  }
  ...
  protected getMoveBackFrameData(): MoveFrameData {
    return this._skill
      .getActionTemplate()
      .getMoveBackData(this._skill.getRoleSex()).moveBackData;
  }
}
