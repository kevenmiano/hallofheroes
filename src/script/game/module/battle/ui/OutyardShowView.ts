...
export default class OutyardShowView extends FUI_OutyardShowView {
  ...
  private __battleDefenceHandler() {
    let _msg: StackHeadBattleDefenceMsg =
      OutyardManager.Instance.battleDefenceMsg;
    if (!_msg) return;
    let frame: number = Number(
      ((_msg.defenceArmyTotal - _msg.defenceArmyAlive) /
        _msg.defenceArmyTotal) *
        100,
    );
    this.progress.value = frame;
    this.leftArmyTxt.text = LangManager.Instance.GetTranslation(
      "outyard.OutyardShowView.leftArmyTxt.text",
      _msg.defenceArmyAlive,
    );
    this.guildArmyTxt.text = LangManager.Instance.GetTranslation(
      "outyard.OutyardShowView.guildArmyTxt.text",
      100 - _msg.defenceProperty,
    );
  ...
  private __enterFrame() {
    ...
    BattleManager.Instance.battleUIView.bottomBar.selfBloodView.updateHeroHp(
      precent,
      leftBlood,
      maxBoold,
    );
