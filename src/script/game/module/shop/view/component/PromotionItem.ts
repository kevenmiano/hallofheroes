...
export default class PromotionItem extends FUI_PromotionItem {
  ...
  private onUpdateProductCount() {
    ...
    let rechargeCount = ShopManager.Instance.getTodayProductCount(
      this._cellData.ProductId,
    );
    ...
    if (this._cellData.ProductType == 6) {
      this.limitCount.text = LangManager.Instance.GetTranslation(
        "Shop.Promotion.weeklimitText",
        leftTxt,
        maxCount,
      );
    } else {
      this.limitCount.text = LangManager.Instance.GetTranslation(
        "Shop.Promotion.limitText",
        leftTxt,
        maxCount,
      );
    }
    ...
    if (rechargeCount >= maxCount) {
      ...
      if (Utils.isInteger(countMoney.toString())) {
        this.txt_money.text =
          ShopManager.Instance.getMoneyString() +
          parseInt(countMoney.toString()); //价格
      } else {
        this.txt_money.text =
          ShopManager.Instance.getMoneyString() + countMoney.toString(); //价格
      }
  ...
  public set info(value: t_s_rechargeData) {
    ...
    this.itemTitle.text = LangManager.Instance.GetTranslation(
      "Shop.Promotion.itemTitle" + Number(this.itemType.selectedIndex + 1),
    );
    this.onUpdateProductCount();
    this._listdata = [];
    let rewardDataStr = value.Para2;
    if (!rewardDataStr) return;
