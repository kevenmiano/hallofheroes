...
export default class EntryCom extends FUI_EntryCom {
  ...
  onSwitch() {
    ...
    let listData = [];
    if (!ComponentSetting.IOS_VERSION && isCustomerWebsite)
      //游戏官网
      listData.push({
        textKey: "PersonalCenterWnd.EntryCom.btnText1",
        iconName: "Btn_L2_Web",
        btnName: "btnWeb",
      });
    if (!Utils.isFrom4399())
      //游戏公告
      listData.push({
        textKey: "PersonalCenterWnd.EntryCom.btnText2",
        iconName: "Btn_L2_News",
        btnName: "btnNotice",
      });
    if (isCustomerService) {
      //游戏客服
      listData.push({
        textKey: "PersonalCenterWnd.EntryCom.btnText3",
        iconName: "Btn_L2_GM",
        btnName: "btnGM",
      });
    }
    ...
    if (!isOversea())
      //建议反馈
      listData.push({
        textKey: "PersonalCenterWnd.EntryCom.btnText4",
        iconName: "Btn_L2_Suggestion-1",
        btnName: "btnAdvice",
      });
    if (isApp)
      //账号管理
      listData.push({
        textKey: "PersonalCenterWnd.EntryCom.btnText5",
        iconName: "Btn_L2_IDManage",
        btnName: "btnPassword",
      });
    if (isCustomerAccounts) {
      ...
      if (isOversea()) {
        listData.push({
          textKey: "PersonalCenterWnd.EntryCom.btnText_fb",
          iconName: "Btn_L2_LikeFB",
          btnName: "btnOfficialAccount",
        });
      } else {
        listData.push({
          textKey: "PersonalCenterWnd.EntryCom.btnText6",
          iconName: "Btn_L2_OfficialAccounts",
          btnName: "btnOfficialAccount",
        });
      }
    }
    ...
    if (isDiscord)
      listData.push({
        textKey: "PersonalCenterWnd.EntryCom.btnText7",
        iconName: "Btn_L2_Discord",
        btnName: "btnDiscord",
      });
      ...
      case "btnOfficialAccount":
        if (isOversea()) {
          let facebookUrl: string =
            "https://www.facebook.com/Wartune-Lite-7ROAD-114695167888975/";
          if (!Utils.isApp()) {
