...
export class CampaignChapterInfo {
  ...
  public getMaxOpenArea(): CampaignAreaInfo {
    ...
    list.forEach((info: CampaignAreaInfo) => {
      ...
      if (
        ...
        if (!temp) temp = info;
        if (temp.areaId <= info.areaId) temp = info;
      }
    });
    return temp;
  }
  ...
  public isAllOpenArea(): boolean {
    ...
    this._areaDic.getList().forEach((info: CampaignAreaInfo) => {
      if (
        info.getMapByDifficult(CampaignMapDifficulty.General).state !=
        CampaignMapStatus.OVER_CAMPAIGN
      )
        result = false;
    });
    return result;
  ...
  public isAllLoaded(): boolean {
    let result: boolean = true;
    this._areaDic.getList().forEach((info: CampaignAreaInfo) => {
      if (!info.viewContentLoaded) result = false;
    });
    return result;
  }
  ...
  public getAreaByCampaignTem(temp: t_s_campaignData): CampaignAreaInfo {
    ...
    for (let index = 0; index < this._areaDic.keys.length; index++) {
      ...
      if (info.areaId == temp.AreaId) {
        if (
          info.getMapByDifficult(CampaignMapDifficulty.General).MinLevel <=
          this.thane.grades
        ) {
          return info;
        } else {
  ...
  public getMaxSelectAewa(): CampaignAreaInfo {
    ...
    list.forEach((info: CampaignAreaInfo) => {
      if (
        info.getMapByDifficult(CampaignMapDifficulty.General).state !=
          CampaignMapStatus.NO_ACCEPT_CAMPAIGN &&
        info.getMapByDifficult(CampaignMapDifficulty.General).MinLevel <=
          this.thane.grades
      ) {
        if (!temp) temp = info;
    ...
    switch (this.chapterId) {
      case 101:
        return LangManager.Instance.GetTranslation(
          "selectcampaign.data.CampaignChapterInfo.chapterName01",
        );
      case 102:
        return LangManager.Instance.GetTranslation(
          "selectcampaign.data.CampaignChapterInfo.chapterName02",
        );
      case 103:
        return LangManager.Instance.GetTranslation(
          "selectcampaign.data.CampaignChapterInfo.chapterName03",
        );
      case 104:
        return LangManager.Instance.GetTranslation(
          "selectcampaign.data.CampaignChapterInfo.chapterName04",
        );
      case 105:
        return LangManager.Instance.GetTranslation(
          "selectcampaign.data.CampaignChapterInfo.chapterName05",
        );
      case 106:
        return LangManager.Instance.GetTranslation(
          "selectcampaign.data.CampaignChapterInfo.chapterName06",
        );
      case 107:
        return LangManager.Instance.GetTranslation(
          "selectcampaign.data.CampaignChapterInfo.chapterName07",
        );
    }
    return "";
  }
