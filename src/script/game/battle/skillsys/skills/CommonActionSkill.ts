...
export class CommonActionSkill extends BaseSkill {
  ...
  private addMoveForwardAction() {
    let mf: MoveForwardContext = new MoveForwardContext(
      this,
      this.startMoveFun,
      this.endMoveFun,
    );
  ...
  private addMoveBackAction() {
    let mb: MoveBackContext = new MoveBackContext(
      this,
      this.startMoveFun,
      this.endMoveFun,
    );
  ...
  private createAttackAction(): SimpleScriptAction {
    let actionScript: SimpleScriptAction = new SimpleScriptAction(
      this._skillData.liftTime,
      this.skillComplete.bind(this),
    );
    ...
    actionScript.onUpdate = () => {
      ...
      for (let index = 0; index < this._actionTemplate.frames.length; index++) {
        ...
        if (frame.Frame == actionScript.frameCount) {
          if (
            frame.ActionType == SkillFrameType.ADD_DANNY ||
            frame.ActionType == SkillFrameType.SHOOT ||
            frame.ActionType == SkillFrameType.SHOOT2
          ) {
          ...
          if (
            ...
            Logger.battle(
              "[CommonActionSkill]createAttackAction.onUpdate",
              frame.effectData.count,
              frame,
            );
        ...
        if (
          !this._bufferAdded &&
          this._addBufferFrame == actionScript.frameCount
        ) {
          this.updataSp();
        }
      }
