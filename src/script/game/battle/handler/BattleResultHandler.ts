...
import { BattleEvent, BattleNotic, NotificationEvent, SceneEvent } from "../../constant/event/NotificationEvent";
...
export class BattleResultHandler {
    ...
    public handler(data: PackageIn) {
        ...
        if (SceneManager.Instance.currentType == SceneType.WARLORDS_ROOM) {//武斗会战斗中掉线特殊处理（玩家比赛中掉线再登陆进入武斗会房间, 会向服务器请求, 但服务器检测到跨服服务器已存在此玩家, 就不再加进玩家数据, 所以要再次请求, 并不执行场景切换）
            WarlordsManager.Instance.reqEnterWarlordsRoom();
            return;
        }
        if (this.battleModel) {
            if (this.battleModel.battleType == BattleType.CAMPAIGN_BATTLE || this.battleModel.battleCapity == 4) {//多人副本
                if (this.battlerModel) {//连击
        ...
        this._battleMsg = this._battlePkg.readBody(BattleReportMsg) as BattleReportMsg;
        this.readPkg();

        let str: string = LangManager.Instance.GetTranslation("battle.handler.BattleResultHandler.debug01");//退出战斗, battleId=
        if (this.battleModel) {
            Logger.battle(str + this.battleModel.battleId);
        }
        else {
            str = LangManager.Instance.GetTranslation("battle.handler.BattleResultHandler.debug02");//退出战斗,但找不到战斗相关数据
            Logger.battle(str);
        }
    ...
    private showNewVersionPane() {
        ...
        if (this.combat == 1 && isInBattle && SceneManager.Instance.currentType == SceneType.BATTLE_SCENE) {
            AudioManager.Instance.playSound(SoundIds.BATTLE_RESULT_WIN);
            NotificationManager.Instance.dispatchEvent(NotificationEvent.BATTLE_RESULT, true);
            try {
            ...
            NotificationManager.Instance.dispatchEvent(NotificationEvent.BATTLE_RESULT, false);
            try {
                if (this.needDetailInfo()) {
        ...
        if (this.battleModel && this.battleModel.battleType == BattleType.BATTLE_MATCHING && this.combat != 3) {// && BattleManager.preScene == SceneType.PVP_ROOM_SCENE) {
            let playerInfo = PlayerManager.Instance.currentPlayerModel.playerInfo;
            Logger.battle("[BattleResultHandler]pvp竞技场结算", playerInfo.mulSportChange)
            let win = playerInfo.mulSportChange >= 0 ? true : false;
            FrameCtrlManager.Instance.open(EmWindow.PvpRoomResultWnd, { win: win, change: Math.abs(playerInfo.mulSportChange) });
        } else {
    ...
    private needDetailInfo(): boolean {
        if (this.battleModel) {
            if (this.battleModel.battleType == BattleType.BATTLE_MATCHING ||
    ...
    private resultViewCloseFun() {
        let scene: string = (this._preScent != "" ? this._preScent : SwitchPageHelp.returnScene);
        ...
        if (!SharedManager.Instance.multiFailded) {
            if (this._battleMsg && this._battleMsg.battleResult == 2 && this._battleMsg.failCount > 4) {
     ...
     *
     */
    public static exitBattle(preScene: string = BattleManager.preScene, combat: number = 1, data: Object = null): string {
        //			CancelBattleEffectController.cancel();/、当前应策划要求取消 提示取消特效
        BloodHelper.setup();
        let sceneType: string = (preScene != "" ? preScene : SwitchPageHelp.returnScene);
        ...
        if (BattleManager.Instance.battleScene) {
            NotificationManager.Instance.addEventListener(SceneEvent.SCENE_SWITCH_CALL, BattleResultHandler.sceneSwitchCallHandler, this);
        }
        else {
        ...
        if (bmodel) {
            switch (bmodel.battleType) {
                ...
                case BattleType.WARLORDS_OVER:
                    sceneType = (WarlordsManager.Instance.model.process == WarlordsModel.PROCESS_FINAL ? SceneType.WARLORDS_ROOM : SwitchPageHelp.returnScene);
                    break;
        ...
        if (sceneType == SceneType.OUTER_CITY_SCENE) {
            ...
            if (MapData.movePos) {
                ...
                mapData.targetPoint = new Point(selfArmy.curPosX * 20, selfArmy.curPosY * 20);
            }
    ...
    private static sceneSwitchCallHandler(evt: Event) {
        ...
        BattleResultHandler.exitBattleComplete();
        NotificationManager.Instance.removeEventListener(SceneEvent.SCENE_SWITCH_CALL, BattleResultHandler.sceneSwitchCallHandler, this);
    }
    ...
    private readPkg() {
        ...
        let petBonus: number = this._battleMsg.petGpBonus > 0 ? this._battleMsg.petGpBonus : 0;
