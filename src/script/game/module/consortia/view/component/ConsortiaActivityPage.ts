...
export class ConsortiaActivityPage extends FUI_ConsortiaActivityPage {
    //@ts-ignore
    public member: ConsortiaMemberPage;
    //@ts-ignore
    ...
    private initView() {
        if (this._model.consortiaInfo.consortiaName == null
            || this._model.consortiaInfo.consortiaName == "") {
            return;
        }
        this.txtConsortiaName.text = this._model.consortiaInfo.consortiaName;
        this._model.dispatchEvent(ConsortiaEvent.REFRESH_CONSORTIA_NAME, this._model.consortiaInfo.consortiaName);
        this.txtConsortChairman.text = this._model.consortiaInfo.chairmanName;
        this.txtConsortiaRank.text = this._model.consortiaInfo.orderInfo ? this._model.consortiaInfo.orderInfo.gradeOrder.toString() : "";
        let maxCount = this._model.SortiaMaxMembers;
        this.txtConsortiaNum.text = this._model.consortiaInfo.currentCount.toString() + "/" + maxCount.toString();
        this.txtConsortiaWealth.text = this._model.consortiaInfo.offer.toString();
        this.txtConsortiaMaintain.text = this._model.consortiaInfo.levels * 500 + LangManager.Instance.GetTranslation("consortia.view.myConsortia.ConsortiaFrameTopView.maintenanceCostTxt.week");
        this.txtConsortiaContribution.text = this.playerInfo.consortiaOffer + "/" + this.playerInfo.consortiaTotalOffer;
        this.txtConsortiawuzi.text = this._model.consortiaInfo.consortiaMaterials.toString();//公会物资
        this.txtSelfJianse.text = this.playerInfo.consortiaJianse +"/" + this.playerInfo.consortiaTotalJianse;
        ...
        this.txtConsortiaLevel.text =LangManager.Instance.GetTranslation("public.level3",this._model.consortiaInfo.levels);
    ...
    private initEvent() {
        ...
        this._model.addEventListener(ConsortiaEvent.UPDA_CONSORTIA_INFO, this.__onConsortiaInfoUpdata, this);
        this._model.addEventListener(ConsortiaEvent.PRIZE_MEMBER_LIST_UPDATE, this.__prizeMemberListUpdateHandler, this);
        ...
        NotificationManager.Instance.addEventListener(NotificationEvent.CONSORTIA_BOSS_SWITCH,this.__consortiaOfferUpdata, this);
        this.playerInfo.addEventListener(PlayerEvent.CONSORTIA_OFFER_CHANGE, this.updateView, this);
        this.playerInfo.addEventListener(PlayerEvent.CONSORTIA_JIANSE_CHANGE, this.updateView, this);
    }

    private removeEvent() {
        this.upgradeBtn.offClick(this, this._upgradeBtnHandler);
        this.donateBtn.offClick(this, this._donateBtnHandler);
        this._model.removeEventListener(ConsortiaEvent.UPDA_CONSORTIA_INFO, this.__onConsortiaInfoUpdata, this);
        this._model.removeEventListener(ConsortiaEvent.PRIZE_MEMBER_LIST_UPDATE, this.__prizeMemberListUpdateHandler, this);
        this.impeachBtn.offClick(this, this.__impeachBtnClickHandler);
        this.changeBtn.offClick(this,this.changeBtnHander);
        NotificationManager.Instance.removeEventListener(NotificationEvent.CONSORTIA_BOSS_SWITCH,this.__consortiaOfferUpdata, this);
        this.playerInfo.removeEventListener(PlayerEvent.CONSORTIA_OFFER_CHANGE, this.updateView, this);
        this.playerInfo.removeEventListener(PlayerEvent.CONSORTIA_JIANSE_CHANGE, this.updateView, this);
    }
    ...
    private changeBtnHander(){
     ...
     private _extiConsortiaBtnHandler(evt: MouseEvent) {
        ...
        let content: string = LangManager.Instance.GetTranslation("consortia.view.myConsortia.ConsortiaFrame.content");
        let isFirst = PlayerManager.Instance.currentPlayerModel.playerInfo.addGuildCount <= 1;//加入公会次数
        let checkTxt: string = "";
        let hideCheck1: boolean = false;
        if (isFirst) {
            content += "<br>" + LangManager.Instance.GetTranslation("consortia.view.myConsortia.ConsortiaFrame.checkTxt.first");
            hideCheck1 = true;
        } else {
            checkTxt = LangManager.Instance.GetTranslation("consortia.view.myConsortia.ConsortiaFrame.checkTxt", point);
            hideCheck1 = false;
        }
        UIManager.Instance.ShowWind(EmWindow.TodayNotAlert, { content: content, checkTxt: checkTxt, hidecheck1: hideCheck1, state: 2, backFunction: this.exitCallback.bind(this), closeFunction: null });
    }

    private exitCallback(check: boolean) {
        if (this._model) {
            //退出公会
            if (this._model.consortiaInfo.chairmanID == this.playerInfo.userId
                && this._model.consortiaInfo.currentCount > 1) {
                let str: string = LangManager.Instance.GetTranslation("consortia.view.myConsortia.ConsortiaFrame.command02");
                MessageTipManager.Instance.show(str);
                return;
            }
            if (check) {
                let num: number = GoodsManager.Instance.getGoodsNumByTempId(208013);
                let point: number = PlayerManager.Instance.currentPlayerModel.playerInfo.point;
                let point2: string = TempleteManager.Instance.getConfigInfoByConfigName("Consortia_ClearCD_Point").ConfigValue;
                Logger.log(num < 0);
