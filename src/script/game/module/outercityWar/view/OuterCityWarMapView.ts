...
export class OuterCityWarMapView extends Laya.Sprite {
  ...
  private initEvent() {
    this._dragObj = new TweenDrag(
      this,
      FrameCtrlManager.Instance.getCtrl(EmWindow.OuterCityWarWnd).view,
    );
  ...
  public dragingCallBack() {
    if (CastleConfigUtil.MAP_RAW_WIDTH * this.scaleX < Resolution.gameWidth) {
      ...
      this.x <
      Resolution.gameWidth - CastleConfigUtil.MAP_RAW_WIDTH * this.scaleX
    ) {
      this.x =
        Resolution.gameWidth - CastleConfigUtil.MAP_RAW_WIDTH * this.scaleX;
    } else if (this.x > 0) {
