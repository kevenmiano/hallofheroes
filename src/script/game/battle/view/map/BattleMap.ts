...
export class BattleMap extends BattleCameraScene {
  ...
  public backGroundToColor(
    color: number = 0,
    easeInOutTime: number = 0.3,
    stopTime: number = 0.5,
    now: boolean = false,
    reversColor: boolean = false,
  ) {
    var tweenLayers: any[] = [];
  ...
  public backGroundToColorUseFrames(
    color: number,
    easeInFrame: number,
    easeOutFrame: number,
    endFrame: number,
    ...
    for (const key in this.childLayerSettingMap) {
      if (
        ...
        if (
          element != this._effectContainer &&
          element != this._rolesContainer &&
          element != this._profoundContainer
        ) {
          tweenLayers.push(element);
        }
      }
  ...
  public backGroundToColor2(brightness: number, time: number) {
    ...
    this._revertTimeoutId = setTimeout(
      this.revertBackGroundColor.bind(this),
      time,
    );
