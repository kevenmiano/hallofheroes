...
export default class ConsortiaBossTaskView extends BaseWindow {
    ...
    private initEvent() {
        ...
        NotificationManager.Instance.addEventListener(NotificationEvent.CONSORTIA_BOSS_SWITCH, this.consortiaSwitchHandler, this);
        NotificationManager.Instance.addEventListener(NotificationEvent.CONSORTIA_BOSS_STATE_UPDATE, this.consortiaBossStateUpdateHandler, this);
    ...
    private removeEvent() {
        ...
        NotificationManager.Instance.removeEventListener(NotificationEvent.CONSORTIA_BOSS_SWITCH, this.consortiaSwitchHandler, this);
        NotificationManager.Instance.removeEventListener(NotificationEvent.CONSORTIA_BOSS_STATE_UPDATE, this.consortiaBossStateUpdateHandler, this);
    ...
    private clickHandler(event: Laya.Event) {
        ...
        if (item instanceof ConsortiaBossTaskItem) {//点击任务项
            if (this.skillTipsCtr.selectedIndex == 1) {//tips现在已经是显示的
                if (this.selectedItem && this.selectedItem == item)//点击的是同一个则是关掉当前tips。
                {
                    this.selectedItem = null;
                    this.skillTipsCtr.selectedIndex = 0;
                } else if (this.selectedItem && this.selectedItem != item) {//点击的不是同一个则是替换当前tips的内容。
                    this.setRewardTxt(item); //设置tips内容
                }
            } else {//tips原来没有显示, 现在需要显示tips
                this.skillTipsCtr.selectedIndex = 1;
                this.setRewardTxt(item);//设置tips内容
            }
        } else if ((item instanceof fgui.GLoader) && (item.name == "icon1" || item.name == "icon2")) {//点击下面两个图标
            if (this.coinTipsCtr.selectedIndex == 1) {//道具tips现在已经是显示的
                if (this.selectedItem2 && this.selectedItem2 == item)//点击的是同一个则是关掉当前道具tips。
                {
                    this.selectedItem2 = null;
                    this.coinTipsCtr.selectedIndex = 0;
                } else if (this.selectedItem2 && this.selectedItem2 != item) {//点击的不是同一个则是替换当前tips的内容。
    ...
    private getGoodsBtnHandler(){
        let nextNode:CampaignNode = CampaignManager.Instance.mapModel.getConsortiaBossGoodsNode();
        if (nextNode) {
            ...
            CampaignManager.Instance.controller.moveArmyByPos(nextNode.x, nextNode.y, true, true);
        }
    }
    ...
    private setRewardTxt(item: ConsortiaBossTaskItem) {
        ...
        this.rewardTxt.text = LangManager.Instance.GetTranslation("ConsortiaBoss.ConsortiaBossTaskItem.rewardTips" + type);
    ...
    private getDescTxt(item: ConsortiaBossTaskItem, sonType: number): string {
        ...
        if (sonType == ConsortiaBossTaskView.SONTYPE_2) {
            if (item.currentProcess >= item.maxProcessValue) {
                skillData = TempleteManager.Instance.getSkillTemplateInfoBySonTypeAndGrade(sonType, 1);
            }
            else {
                skillData = TempleteManager.Instance.getSkillTemplateInfoBySonTypeAndGrade(sonType, 0);
            }
        }
        else {
            skillData = TempleteManager.Instance.getSkillTemplateInfoBySonTypeAndGrade(sonType, item.currentProcess);
        }
        if (skillData) {
            let skillBufferData: t_s_skillbuffertemplateData = TempleteManager.Instance.getSkillBuffTemplateByID(skillData.TemplateId);
            if (skillBufferData) {
    ...
    private setCoinTxt(item: fgui.GLoader) {
        ...
        if (item.name == "icon1") {
            this.coinDescTxt.text = LangManager.Instance.GetTranslation("ConsortiaBossTaskView.collectImage.tipdata");
        }
        else if (item.name == "icon2") {
            this.coinDescTxt.text = LangManager.Instance.GetTranslation("ConsortiaBossTaskView.propImage.tipdata");
        }
        else {
    ...
    private refreshView() {
        ...
        this._bossUserInfo = this._model.bossInfo.getBossUserInfoByUserId(this.playerInfo.userId);
    ...
    private consortiaSwitchHandler() {
        ...
        {
            this.fightTimeDescTxt.text = LangManager.Instance.GetTranslation("ui.tasktracebar.view.consortiaboss.ConsortiaBossSortView.startTxt");
        } else if (this._model.bossInfo.state == 2)//进行中
        {
            this.fightTimeDescTxt.text = LangManager.Instance.GetTranslation("ui.tasktracebar.view.petboss.PetBossSortView.timeTxt");
        }
    }
    ...
    private get leftTime(): string {
        var str: string = "";
        var endTime: number = DateFormatter.parse(this._model.bossInfo.endTime, "YYYY-MM-DD hh:mm:ss").getTime() / 1000;
        if (this._model.bossInfo.state == 1) {//准备中
        ...
        if (endTime > curTime) {
            ...
            if (this._model.bossInfo.state == 1) {
                str = LangManager.Instance.GetTranslation("ui.tasktracebar.view.petboss.PetBossSortView.starStr");
            } else if (this._model.bossInfo.state == 2) {
                str = LangManager.Instance.GetTranslation("ui.tasktracebar.view.petboss.PetBossSortView.endStr");
            }
        }
        return str;
    }
