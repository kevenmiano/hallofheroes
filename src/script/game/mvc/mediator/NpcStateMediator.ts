...
export class NpcStateMediator implements IMediator {
    ...
    if (this._npc.npcAiInfo.moveState == AiStateType.NPC_CHASE_STATE) {
      ...
      if (WorldBossHelper.checkPvp(mapId)) {
        if (this._npc.nodeInfo instanceof CampaignNode) {
          ...
          var target: CampaignArmy =
            CampaignManager.Instance.mapModel.getBaseArmyByArmyId(followId);
        ...
        if (this._npc.nodeInfo instanceof CampaignNode) {
          ...
          var target1: CampaignArmy =
            CampaignManager.Instance.mapModel.getBaseArmyByArmyId(followId1);
          armyView = CampaignManager.Instance.controller.getArmyView(target1);
          if (this.inMineralAndHasMinerals) armyView.changeSpeedTo(0.5);
          this._npc.npcAiInfo.speed = armyView.info.speed;
        } else {
          this._npc.npcAiInfo.speed = CampaignArmyView.HALF_SPEED;
        }
  ...
  private get inMineralAndHasMinerals(): boolean {
    ...
    var target: CampaignArmy =
      CampaignManager.Instance.mapModel.getBaseArmyByArmyId(followId);
    var armyView: object =
      CampaignManager.Instance.controller.getArmyView(target);
    if (
      this._npc instanceof MineralCarView &&
      target &&
      armyView instanceof MineralArmyView
    ) {
      if (
        (<MineralCarView>this._npc).cadInfo &&
        (<MineralCarView>this._npc).cadInfo.minerals > 0
      ) {
        return true;
      }
    }
