...
export class FarmPetListItem extends FUI_FarmPetListItem {
    ...
    public set info(value: PetData) {
        ...
        if (value) {
            ...
            this.getChild("title").asLabel.color = PetData.getQualityColor(this._info.quality - 1)
            this.icon = IconFactory.getPetHeadSmallIcon(value.templateId);
            this.txtLevel.text = LangManager.Instance.GetTranslation("public.level2", value.grade);
        } else {
            this.title = "";
            this.icon = "";
            this.txtLevel.text = "";
