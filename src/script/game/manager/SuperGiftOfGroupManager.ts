...
export class SuperGiftOfGroupManager {
	...
	public get giftList(): SuperGiftOfGroupGiftMsg[] {
		return ArrayUtils.sortOn(this._detailMessage.giftMsg, "sort", ArrayConstant.NUMERIC);
	}
	...
	private mock(): void {
		...
		let giftItem1: SuperGiftOfGroupGiftItemMsg = new SuperGiftOfGroupGiftItemMsg();
		giftItem1.itemId = 2050103;
		giftItem1.count = 58;
		gift1.itemMsg.push(giftItem1);

		let giftItem2: SuperGiftOfGroupGiftItemMsg = new SuperGiftOfGroupGiftItemMsg();
		giftItem2.itemId = 1070004;
		giftItem2.count = 28;
		gift1.itemMsg.push(giftItem2);

		let giftItem3: SuperGiftOfGroupGiftItemMsg = new SuperGiftOfGroupGiftItemMsg();
		giftItem3.itemId = 1060004;
		giftItem3.count = 5;
		gift1.itemMsg.push(giftItem3);

		let giftLevel2: SuperGiftOfGroupGiftLevelMsg = new SuperGiftOfGroupGiftLevelMsg();
		giftLevel2.level = 2;
		...
		let giftLevel1: SuperGiftOfGroupGiftLevelMsg = new SuperGiftOfGroupGiftLevelMsg();
		giftLevel1.level = 1;
		...
		let giftLevel3: SuperGiftOfGroupGiftLevelMsg = new SuperGiftOfGroupGiftLevelMsg();
		giftLevel3.level = 3;
	...
	public request() {
		if (this._mock) {
			this.mock();
			NotificationManager.Instance.sendNotification(NotificationEvent.SUPERGIFTOFGROUP_DETAIL_UPDATE);
		} else {
			let superGiftOfGroupMsg: SuperGiftOfGroupMsg = new SuperGiftOfGroupMsg();

			superGiftOfGroupMsg.op = 1;

			SocketManager.Instance.send(C2SProtocol.C_CCCACTIVE_CLIENTOP, superGiftOfGroupMsg, CCCActiveType.SUPERGIFTODGROUP);
		}
	}
	...
	private addEvent(): void {
		ServerDataManager.listen(S2CProtocol.U_C_SUPERGIFTOFGROUP_INFO, this, this.onReceiveDetail);
	}
	...
	private removeEvent(): void {
		ServerDataManager.cancel(S2CProtocol.U_C_SUPERGIFTOFGROUP_INFO, this, this.onReceiveDetail);
	}
	...
	private onReceiveDetail(pkg: PackageIn) {
		let superGiftOfGroupMsg: SuperGiftOfGroupMsg = pkg.readBody(SuperGiftOfGroupMsg) as SuperGiftOfGroupMsg;
	...
	private onReceiveState(pkg: PackageIn) {
		...
		if (!this._open) {
			if (FrameCtrlManager.Instance.isOpen(EmWindow.Funny)) FrameCtrlManager.Instance.exit(EmWindow.Funny);
		}

		NotificationManager.Instance.sendNotification(NotificationEvent.SUPERGIFTOFGROUP_STATE_UPDATE);
	}
}
