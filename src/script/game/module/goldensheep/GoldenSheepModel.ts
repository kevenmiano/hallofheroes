...
export default class GoldenSheepModel extends GameEventDispatcher {
    ...
    public getTopBtnShow(): boolean {
        ...
        if (this.openTime && DateFormatter.checkIsSameDay(this.openTime, this.playerModel.sysCurtime)
            && this.playerModel.sysCurtime.getTime() <= this.openTime.getTime()) return true;
        if (this.endTime && this.openTime
            && this.playerModel.sysCurtime.getTime() >= this.openTime.getTime()
            && this.playerModel.sysCurtime.getTime() < this.endTime.getTime()) return true;
        return false;
    }
