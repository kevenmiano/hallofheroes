...
export default class ChatFaceWnd extends BaseWindow {
    ...
    renderFaceListItem(index: number, item) {
        ...
        let data = {
            eindex: eindex,
            ename: LangManager.Instance.GetTranslation("chat.view.ExpressionList.Ename" + eindex)
        };
        item.itemdata = data;
    ...
    __onFaceItemSelect(targetItem) {
        ...
        NotificationManager.Instance.dispatchEvent(ChatEvent.CHAT_EMJOY_CLICK, targetData);
        this.OnBtnClose();
