...
export default class BindVertifyWnd extends BaseWindow {
  ...
  private _regExpPhone: RegExp =
    /^(13[0-9]|14[01456879]|15[0-35-9]|16[2567]|17[0-8]|18[0-9]|19[0-35-9])\d{8}$/;
  private _regExpMail: RegExp =
    /\w+([-+.]\w+)*@\w+([-.]\w+)*\.\w+(w+([.-]\w+))*/;
  ...
  OnInitWind(): void {
    ...
    this.checkbox.text = LangManager.Instance.GetTranslation(
      "BindVertify.checkBoxText",
    );
    this.setCenter();
  }
    ...
    if (this.frameData) {
      ...
      if (this.type == 1) {
        ...
        this.title.text = LangManager.Instance.GetTranslation(
          "BindVertify.bindPhoneTitle",
        );
        this.txtTip.text = LangManager.Instance.GetTranslation(
          "BindVertify.bindPhoneNumber",
        );
        ...
        this.title.text = LangManager.Instance.GetTranslation(
          "BindVertify.bindMailTitle",
        );
        this.txtTip.text = LangManager.Instance.GetTranslation(
          "BindVertify.bindMailAddr",
        );

        this.getController("type").selectedIndex = 1;
        this.checkbox.text = LangManager.Instance.GetTranslation(
          "BindVertify.checkBoxText1",
        );
      }
    }
  ...
  private reqBindCode() {
    if (this.type == 1) {
      ...
      if (!this._regExpMail.test(mailAddr)) {
        MessageTipManager.Instance.show(
          LangManager.Instance.GetTranslation(
            "MailCheckFrame.wrongEmailAddress",
          ),
        );
        return;
      }
  ...
  private sendBindCode() {
    ...
    if (this.type == 1) {
      Logger.info(
        "[BindVertifyWnd]提交手机验证码",
        code,
        C2SProtocol.C_USER_PIN_CHECK,
      );
      ...
      Logger.info(
        "[BindVertifyWnd]提交邮箱验证码",
        code,
        C2SProtocol.C_MAIL_CHECK,
      );
  ...
  private beginCool() {
    ...
    this.btnGetCode.title = LangManager.Instance.GetTranslation(
      "mailcheck.MailCheckFrame.refreshTxt",
      this.time,
    );
