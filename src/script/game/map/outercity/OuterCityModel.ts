...
export class OuterCityModel extends FloorMapInfo {
  ...
  public set selectPkArmy(value: BaseArmy) {
    ...
    let str: string;
    let selfConsortiaId: number =
      PlayerManager.Instance.currentPlayerModel.playerInfo.consortiaID;
    if (
      selfConsortiaId > 0 &&
      this.selectPkArmy.baseHero.consortiaID == selfConsortiaId
    ) {
      str = LangManager.Instance.GetTranslation(
        "map.outercity.mediator.mapview.OuterCityPkMediator.command01",
      );
    ...
    if (this.selectPkArmy.state == ArmyState.STATE_FIGHT) {
      str = LangManager.Instance.GetTranslation(
        "map.outercity.view.mapphysics.MapPhysicsCastle.command05",
      );
      MessageTipManager.Instance.show(str);
      return;
    }
  ...
  public addCastle(cInfo: BaseCastle): void {
    ...
    if (
      cInfo.tempInfo &&
      cInfo.tempInfo.ID == OuterCityModel.SELT_CALSTE_TEMPLATEID
    ) {
  ...
  public isFlying(mountTemplateId: number): boolean {
    ...
    let flyMountCfg =
      TempleteManager.Instance.getConfigInfoByConfigName("Fly_Mount");
    if (flyMountCfg) {
  ...
  private canAttackArmy(info: MapPhysics, isAttack: number): BaseArmy {
    ...
    if (army == null) {
      str = LangManager.Instance.GetTranslation(
        "map.outercity.OuterCityModel.command01",
      );
      MessageTipManager.Instance.show(str);
    } else if (
      army.state != ArmyState.STATE_WAITING &&
      !ArmyState.moveState(army.state)
    ) {
      str = LangManager.Instance.GetTranslation(
        "map.outercity.OuterCityModel.command02",
      );
      MessageTipManager.Instance.show(str);
    } else if (
      army.curPosX == info.posX &&
      army.curPosY == info.posY &&
      isAttack == 0
    ) {
      str = LangManager.Instance.GetTranslation(
        "map.outercity.OuterCityModel.command03",
      );
      MessageTipManager.Instance.show(str);
    } else if (
      army.mapId > GlobalConfig.Novice.NewMapID &&
      info.info.mapId == GlobalConfig.Novice.OutCityMapID
    ) {
      return army;
  ...
  public getNodeByNodeId(nodeId: number): t_s_mapmineData {
    let mapmineData: t_s_mapmineData = ConfigMgr.Instance.getTemplateByID(
  ...
  public hasOccpuySonNode(node: OutCityMineNode): Array<OutCityOneMineInfo> {
    ...
    hasOccpuyArr = ArrayUtils.sortOn(
      hasOccpuyArr,
      ["sort", "sonNodeId"],
  ...
  public checkHasCanOccpuy(wild: WildLand, node: OutCityMineNode): boolean {
    let flag: boolean = false;
    if (this.hasCanOccpuySonNode(node)) {
      //还存在未占领的金矿
      if (this.occupyCount(wild) < wild.tempInfo.Property1) {
        //自己没有达到上限
        flag = true;
      }
    }
  ...
  private parseNodeMineInfo(wildLandMsg: WildLandMsg): Array<OutCityMineNode> {
    ...
    if (wildLandMsg.allNodeInfo.length > 0) {
      ...
      for (let i: number = 0; i < len; i++) {
        let mineNode: OutCityMineNode = this.readNodeMineInfo(
          wildLandMsg.allNodeInfo[i] as OutCityNodeInfoMsg,
        ); //读取节点的金矿信息
  ...
  public readCastle(cInfo: WildLandMsg): BaseCastle {
    ...
    castleInfo = this.getCastleByIdXY(
      cInfo.id,
  ...
  public readWildLand(wildMsg: WildLandMsg): WildLand {
    ...
    wInfo = this.getWildLandByXY(
      wildMsg.posX * Tiles.WIDTH + "," + wildMsg.posY * Tiles.HEIGHT,
    );
    ...
    if (info.types == OutCityNodeTypeConstant.TYPE_MINE) {
      //金矿节点
    ...
    if (wildMsg.hasOwnProperty("simpleHeroInfo")) {
      ThaneInfoHelper.readHeroInfo(
        sysArmy.baseHero,
        wildMsg.simpleHeroInfo as SimpleHeroInfoMsg,
      );
    }
  ...
  public getSonNodeNameTxt(grade: number): string {
    return (
      this.currentSelectMine.tempInfo.NameLang +
      " " +
      LangManager.Instance.GetTranslation("public.level3", grade)
    );
  }
  ...
  public checkIsSameConsortiaById(consortiaId: number): boolean {
    ...
    if (
      (pModel.consortiaID > 0 && pModel.consortiaID == consortiaId) ||
      consortiaId == 0
    ) {
      flag = true;
    }
  ...
  public checkPlayerOccpuyNumberIsMax(): boolean {
    let flag: boolean = false;
    let maxNumber = this.getPlayerCanOccupyMineMaxCount(
  ...
  public findNeedUpdateNode(msg: OneMineInfoMsg): WildLand {
    ...
    if (targetWild) {
      ...
      if (secondeNodeInfo) {
        ...
        for (const key in secondeNodeInfo.nodeAllMineInfoDic) {
          ...
          if (info && info.sonNodeId == msg.sonNodeId) {
            //找到了孙子节点, 修改节点数据
    ...
    if (targetWild) {
      for (let m: number = 0; m < wild.selfOccpuyArr.length; m++) {
        ...
        if (item && item.sonNodeId == msg.sonNodeId) {
          //放弃该节点
  ...
  public checkOutScene(): boolean {
    ...
    if (
      outerCityArmyView &&
      (Math.abs(outerCityArmyView.x - centerX) >
        StageReferance.stageWidth / 2 ||
        Math.abs(outerCityArmyView.y - centerY) >
  ...
  public checkCanAttackVehicle(wildInfo: WildLand): boolean {
    ...
    if (!selfVehicle) {
      ...
      if (protectSameConsortiaFlag) {
        //护送有不同公会的人
        if (wildInfo.protectStatus == 1) {
          //护送队列在战斗中
          ...
          if (pushSameConsortiaFlag) {
            //推进有不同公会的人
            if (wildInfo.pushStatus != 1) {
              //推进队列在战斗中
              flag = true; //进战斗
            }
          }
    ...
    if (FreedomTeamManager.Instance.model) {
      for (const key in FreedomTeamManager.Instance.model.allMembers) {
        if (
          ...
          var member: BaseArmy =
            FreedomTeamManager.Instance.model.allMembers[key];
          if (
            member &&
            wildInfo.fightUserIdArray.indexOf(member.userId.toString()) != -1
          ) {
            flag = true;
          }
        }
