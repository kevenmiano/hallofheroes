...
export default class MapDataUtils {
    ...
    public static getTitleFileName(startX: number, startY: number, unitWidth: number = 1000, unitHeight: number = 1000): string {
        var nx: number = Math.floor(startX / unitWidth);
        var ny: number = Math.floor(startY / unitHeight);
        return nx + "_" + ny;
    }
     ...
     * 
     */
    public static updateSeparatorByArray(arr: Array<string>, oldStr: string = "_", newStr: string = ","): Array<string> {
    ...
    public static addStandPoint(path: Array<Laya.Point>): Array<Laya.Point> {
        ...
        while (off >= 0) {
            ...
            if (pre1 && pre2) {
                if ((pre2.x - pre1.x) > 0)//向右走
                {
                    if ((pre1.x - end.x) < 0)//超过
                    {
