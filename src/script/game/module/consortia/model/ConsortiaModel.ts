...
export class ConsortiaModel extends GameEventDispatcher {
    ...
    public checkSecretRedDot() {
        return this._secretSeened == false && this.secretInfo && this.secretInfo.remainGainTime > 0;
    }
    ...
    public static get renameCost(): number {
        ...
        let cfgTemp = TempleteManager.Instance.getConfigInfoByConfigName("Consortia_Rename");
        if (cfgTemp) {
    ...
    */
    public static get CREAT_NEEDED_GOLD(): number {
        ...
        let cfg = TempleteManager.Instance.getConfigInfoByConfigName("Consortia_Gold");
        if (cfg) {
     ...
     * @param reverse
     */
    public getSortMemberList(field: string = "fightingCapacity", reverse: boolean = true): ThaneInfo[] {
        let arr: ThaneInfo[] = [];
    ...
    public set alTarBlessLeftTime(value: number)//更改为当天的祈福次数而不是剩余次数
    {
    ...
    public get currentCasecellNum(): number {
        ...
        else if (this.consortiaInfo.storeLevel == ConsortiaUpgradeType.MAX_LEVEL) {
            num = ConsortiaModel.CASE_CELL_MAX_NUM;
        }
        else {
            num = this.consortiaInfo.storeLevel * ConsortiaModel.CASE_CELL_OPEN_BY_LEVEL;
        }
        return num;
    }
    ...
    private compareFun(item1: ConsortiaVotingUserInfo, item2: ConsortiaVotingUserInfo): number {
    ...
    public get hasVotePermission(): boolean {
        let player: PlayerInfo = PlayerManager.Instance.currentPlayerModel.playerInfo;
    ...
    initTaskInfo() {
        ...
        let cfg = TempleteManager.Instance.getConfigInfoByConfigName("ConsortiaTask_TotalScore")
        if (cfg && cfg.ConfigValue) {
            weekScoreList = cfg.ConfigValue.split(",")
            this.taskWeekScoreMax = Number(weekScoreList[this.TaskMaxRewardStageNum - 1])
            for (let index = 0; index < this.TaskMaxRewardStageNum; index++) {
                ...
                info.icon = FUIHelper.getItemURL(EmPackName.Base, "Icon_Box_Dev" + (index + 1))
        ...
        cfg = TempleteManager.Instance.getConfigInfoByConfigName("ConsortiaTask_RefreshNum")
        if (cfg && cfg.ConfigValue) {
            this.TaskMaxRefreshNum = Number(cfg.ConfigValue)
        }

        cfg = TempleteManager.Instance.getConfigInfoByConfigName("ConsortiaTask_StarPrice")
        if (cfg && cfg.ConfigValue) {
    ...
    getTaskTitleByType(type: ConsortiaTaskType): string {
        let titleArr = LangManager.Instance.GetTranslation("ConsortiaTaskWnd.taskTitleStr").split(",")
    ...
    getTaskContent(taskId: number, starNum: number): string {
        let taskCfg = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_consortiatask, taskId) as t_s_consortiataskData
        let itemCfg = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_itemtemplate, taskCfg.NeedItemId) as t_s_itemtemplateData
        if (!itemCfg) return ""
        ...
        let taskContent = LangManager.Instance.GetTranslation("ConsortiaTaskWnd.taskActStr"+ Number(taskCfg.Type) , needItemNum, needItemName);
        return taskContent
    }
    ...
    public checkPrayHasLeftCount(): boolean {
        let leftCommCount = this.totalCount - this.commAltarCount;
        let leftHighCount = this.totalCount - this.highAltarCount;
        return (leftCommCount > 0 && !this._commAltarSeened) || (leftHighCount > 0 && !this._hightAltarSeened);
    }
