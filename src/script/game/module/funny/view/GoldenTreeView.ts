...
import { BagEvent, BottleEvent } from "../../../constant/event/NotificationEvent";
...
export class GoldenTreeView extends FUI_GoldenTreeView implements FunnyContent {
    ...
    private initData() {
        ...
        configInfo = TempleteManager.Instance.getConfigInfoByConfigName("Bottle_Price");
    ...
    private initView() {
        ...
        let remainTime: number = this.bottleModel.endTime / 1000 - PlayerManager.Instance.currentPlayerModel.sysCurTimeBySecond;
        if (remainTime > 0) {
            this.setRemainTime();
        ...
        for (let i = 0, len = 5; i < len; i++) {
            (this["bottomIntergalBox" + (i + 1)] as BottleBottomIntergalBox).boxIndex = i;
        }
        this.updateLeftBox();
    ...
    private initEvent() {
        ...
        GoodsManager.Instance.addEventListener(BagEvent.UPDATE_BAG, this.__upDateHammerNum, this);
        GoodsManager.Instance.addEventListener(BagEvent.DELETE_BAG, this.__upDateHammerNum, this);
        //黄金神树层数变化以及礼包对应的人物信息发生改变
        BottleManager.Instance.addEventListener(BottleEvent.UPDATE_REWARD_USERINFO, this.updateLeftBox, this);
        BottleManager.Instance.addEventListener(BottleEvent.CT__UPDATE_BOX_STATUS, this.updateBottleBox, this);
    }
    ...
    public setRemainTime() {
        ...
        let remainTime: number = this.bottleModel.endTime / 1000 - this.playerModel.sysCurTimeBySecond;
        if (remainTime >= 60) {
        ...
        if (this.bottleModel.startTime > this.playerModel.nowDate) {
            this.txt_remainTime.text = LangManager.Instance.GetTranslation("public.unopen") + LangManager.Instance.GetTranslation("funny.FunnyRightView.active.timeText", DateFormatter.transDate(this._infoData.startTime/1000), DateFormatter.transDate(this._infoData.endTime/1000));
        }
    }
    ...
    private __upDateHammerNum(infos: GoodsInfo[]): void {
        ...
        for (let info of infos) {
            if (info) {
                if (info.bagType != BagType.Player || info.templateId != GoldenTreeView.HAMMER_ID) {
                    continue
                }
            }
        }
    ...
    private setHammersItem() {
        ...
        goods.count = GoodsManager.Instance.getGoodsNumByTempId(GoldenTreeView.HAMMER_ID);
        this.item.info = goods;
    }

    private onBtnHelpClick() {
        let title = LangManager.Instance.GetTranslation("public.help");
        let content = LangManager.Instance.GetTranslation('bottle.BottleFrame.helpFrame.helpContent');
    ...
    private updateLeftBox() {
        ...
        this.txt_currentCount.setVar("floor", this.bottleModel.nowLayer + "").flushVars();
        ...
        let _totalHeight: number = HEIGHT_ARRAY[0] + HEIGHT_ARRAY[1] + HEIGHT_ARRAY[2] + HEIGHT_ARRAY[3] + HEIGHT_ARRAY[4];
        if (score == 0) {
            _currentheight = 0;
        }
        else if (0 < score && score <= currentArray[0]) {
            _currentheight = MAXCURRENT * (score * HEIGHT_ARRAY[0] / currentArray[0] + 3) / _totalHeight;
        }
        else if (currentArray[0] < score && score <= currentArray[1]) {
            _currentheight = MAXCURRENT * ((score - currentArray[0]) * HEIGHT_ARRAY[1] / (currentArray[1] - currentArray[0]) + HEIGHT_ARRAY[0]) / _totalHeight;
        }
        else if (currentArray[1] < score && score <= currentArray[2]) {
            _currentheight = MAXCURRENT * ((score - currentArray[1]) * HEIGHT_ARRAY[2] / (currentArray[2] - currentArray[1]) + HEIGHT_ARRAY[0] + HEIGHT_ARRAY[1]) / _totalHeight;
        }
        else if (currentArray[2] < score && score <= currentArray[3]) {
            _currentheight = MAXCURRENT * ((score - currentArray[2]) * HEIGHT_ARRAY[3] / (currentArray[3] - currentArray[2]) + HEIGHT_ARRAY[0] + HEIGHT_ARRAY[1] + HEIGHT_ARRAY[2]) / _totalHeight;
        }
        else if (currentArray[3] < score && score < currentArray[4]) {
            _currentheight = MAXCURRENT * ((score - currentArray[3]) * HEIGHT_ARRAY[4] / (currentArray[4] - currentArray[3]) + HEIGHT_ARRAY[0] + HEIGHT_ARRAY[1] + HEIGHT_ARRAY[2] + HEIGHT_ARRAY[3]) / _totalHeight;
        }
    ...
    private updateBottleBox() {
        for (let i: number = 0; i < 5; i++) {
            (this["bottomIntergalBox" + (i + 1)] as BottleBottomIntergalBox).refreshStatus();
        }
        //当前开启次数: [color=#ffecc6]{floor=150}[/color]
        this.txt_currOpenNum.setVar("floor", this.bottleModel.openCount.toString()).flushVars();
    ...
    private openAlert(): boolean {
        ...
        if (outdate) {
            let content: string = LangManager.Instance.GetTranslation("bottle.view.BottleFrame.openTipData.openAlertContent", this._openCost, this._openNum);
            let checkTxt: string = LangManager.Instance.GetTranslation("yishi.view.base.ThewAlertFrame.text");
            UIManager.Instance.ShowWind(EmWindow.TodayNotAlert, { content: content, checkTxt: checkTxt, state: 2, backFunction: this.openAlertBack.bind(this), closeFunction: null });
        }
        return outdate;
    }
    ...
    private openCall(): void {
        if (this._checkBtn.selected) {
            let hammerNum: number = GoodsManager.Instance.getGoodsNumByTempId(GoldenTreeView.HAMMER_ID);
            if ((this.playerModel.playerInfo.point + hammerNum * this._openCost1) < this._openCost) {
        ...
        this.effect_tree.tranSacredGoldTree.play(Laya.Handler.create(this, this.effectPlayEnd), 1, 0, 0, 1.667);
        for (let i = 0, len = 5; i < len; i++) {
            this.effect_tree.effect_gold["effect_" + i].tranGolds.play(null, 3);
        }
        BottleManager.Instance.sendOpenInfo(0, this._openType, this._checkBtn.selected);
    }
    ...
    private removeEvent() {
        ...
        GoodsManager.Instance.removeEventListener(BagEvent.UPDATE_BAG, this.__upDateHammerNum, this);
        GoodsManager.Instance.removeEventListener(BagEvent.DELETE_BAG, this.__upDateHammerNum, this);
        //黄金神树层数变化以及礼包对应的人物信息发生改变
        BottleManager.Instance.removeEventListener(BottleEvent.UPDATE_REWARD_USERINFO, this.updateLeftBox, this);
        BottleManager.Instance.removeEventListener(BottleEvent.CT__UPDATE_BOX_STATUS, this.updateBottleBox, this);
    }
