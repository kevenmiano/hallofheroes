...
export class DiamondShopView extends FUI_DiamondShopView {
    ...
    private initEvent() {
        ...
        this.playerInfo.addEventListener(PlayerEvent.PLAYER_INFO_UPDATE, this.updateMoney, this);
    ...
    private onTabChanged() {
        ...
        this._listdata = this.shopModel.gemstoneGoodsList.concat(this.shopModel.petGoodsList, this.shopModel.homeItemList.concat(this.shopModel.propGoodsList), this.shopModel.cardReelList)
    ...
    private updateMoney(flag) {
        ...
        if (dataInfo) {
            this.txt_hasNum.text = GoodsManager.Instance.getGoodsNumByTempId(dataInfo.ItemId) + "";//更新已拥有数量
        }
    }
    ...
    private __updateGoodsHandler(infos: GoodsInfo[]) {
        ...
        for (let info of infos)
            this.txt_hasNum.text = GoodsManager.Instance.getGoodsNumByTempId(info.templateId) + "";
    }
    ...
    private onSelectItem(item: ShopItem) {
        ...
        this.txt_hasNum.text = GoodsManager.Instance.getGoodsNumByTempId(item.info.ItemId) + "";

        //周限购字段的优先级比日限购的优先级高, 当周限购字段填写数量大于0时, 优先判断每周限购次数显示, 当WeeklyLimit字段填写为-1时, 周限购不限量, 则先判断日限购字段显示
        if (item.info.WeeklyLimit > -1) {
            this.txt_limit_type.text = LangManager.Instance.GetTranslation("Shop.Promotion.weeklimit");
            let leftValue = item.info.WeeklyLimit - item.info.weekCount;
            this.txt_limit.text = (leftValue > 0 ? "[color=#FFECC6]" : "[color=#FF0000]") + leftValue + "[/color]/" + item.info.WeeklyLimit;
        } else {
            //当前可购买次数/最大购买次数
            this.txt_limit_type.text = LangManager.Instance.GetTranslation("Shop.Promotion.daylimit");
            if (item.info.OneDayCount != -1 && item.info.OneDayCount != undefined) {
                let leftValue = item.info.OneDayCount - item.info.OneCurrentCount;
                this.txt_limit.text = (leftValue > 0 ? "[color=#FFECC6]" : "[color=#FF0000]") + leftValue + "[/color]/" + item.info.OneDayCount;
            } else {
                this.txt_limit.text = LangManager.Instance.GetTranslation("shop.view.GoodsItem.limited07");
            }
        }
        this.txt_desc.getChild('content').text = goodsInfo.templateInfo.DescriptionLang;

        this._handler && this._handler.recover();
        this._handler = Laya.Handler.create(this, this.stepperChangeHandler, null, false);
        let leftNum = 0;
        let maxCount = 999;
        if (Number(item.info.OneDayCount) != -1 && item.info.OneDayCount != undefined) {//日限购
        ...
        } else if (Number(item.info.WeeklyLimit) != -1 && item.info.WeeklyLimit != undefined) {//周限购
            ...
            this.stepper.show(0, 1, 1, this.currentCanCount, leftNum, 1, this._handler);
    ...
    private checkUseBind(item) {
        this.usebind.selectedIndex = item.PayType == ShopGoodsInfo.PAY_BY_POINT ? 0 : 1;
    ...
    private onUseBindDiamond() {
        ...
        if (this.checkbox_useBind.selected) {//绑钻
            this.onsetMoneyType(ShopGoodsInfo.PAY_BY_GIFT);
    ...
    private onBtnDiscount() {
        if (this._currentSelectedItem && this._currentSelectedItemData) {
            ...
            FrameCtrlManager.Instance.open(EmWindow.DiscountWnd, { count: totalCost, selectId: this._discountTempleteId, selectData: this._discountTempleteData, posId: 0, callFunc: this.onDiscountHandler.bind(this), });
        }
    }
    ...
    private onBtnBuyClick() {
        if (this._currentSelectedItemData.WeeklyLimit != -1) {//周限购数量判断
            if (this._currentSelectedItemData.weekCount >= this._currentSelectedItemData.WeeklyLimit) {
                MessageTipManager.Instance.show(LangManager.Instance.GetTranslation('shop.utils.ShopingHelper.command06'));
                return;
            }
        }

        if (this._currentSelectedItemData.OneDayCount != -1) {//日限购数量判断
            if (this._currentSelectedItemData.OneCurrentCount >= this._currentSelectedItemData.OneDayCount) {
                MessageTipManager.Instance.show(LangManager.Instance.GetTranslation('shop.utils.ShopingHelper.command06'));
                return;
            }
        }
        ...
        if (this._discountTempleteId > 0) {
            leftCount = GoodsManager.Instance.getBagCountByTempId(BagType.Player, this._discountTempleteId);
        }
        let userDiscount: boolean = this._discountTempleteId != 0 && leftCount > 0;
        let temp: t_s_itemtemplateData = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_itemtemplate, this._currentSelectedItemData.ItemId)
        //如果绑定钻石不够,客户端提醒,不发给服务器
        //读取商城配置表价格数据
        let item: ShopGoodsInfo = TempleteManager.Instance.getShopTempInfoById(this._currentSelectedItemData.Id);
        if (useBind) {
    ...
    private showAlert(useBind: boolean, temp: ShopGoodsInfo, useDiscount: boolean = false) {
        let content: string = "";
        let countValue = 0;
        let discountValue = 0;
        if (useBind) {//绑定钻石购买
            countValue = this._currentSelectedItemData.price * this.stepper.value;
            content = LangManager.Instance.GetTranslation("ShopWnd.Alert.BindDiamondBuy", countValue);
        } else {//钻石购买
            if (useDiscount) {
                if (this._discountTempleteData)
                ...
                countValue = this._currentSelectedItemData.price * this.stepper.value;
                content = LangManager.Instance.GetTranslation("ShopWnd.Alert.DiamondBuy", countValue);
            }
        }
        UIManager.Instance.ShowWind(EmWindow.BuyGoodsAlert, { price: this._currentSelectedItemData.price, count: this.stepper.value, temp: temp, useBind: useBind, useDiscount: useDiscount, content: content, goodsId: temp.ItemId, goodsCount: this.stepper.value, callback: this.canBuyCallback.bind(this) });
    ...
    private canBuyCallback(b: boolean, data: any) {
        ...
        if (b) {
            ...
            if (useBind) {//绑钻
                costCount = PlayerManager.Instance.currentPlayerModel.playerInfo.point + PlayerManager.Instance.currentPlayerModel.playerInfo.giftToken;
            } else {//真钻
    ...
    private buy(temp: t_s_itemtemplateData, useBind: boolean, disCountTempId: number = 0) {
        if (this._currentSelectedItem && this._currentSelectedItemData && this.stepper) {
            if (temp.SonType == GoodsSonType.FASHION_CLOTHES || temp.SonType == GoodsSonType.FASHION_HEADDRESS ||
                temp.SonType == GoodsSonType.FASHION_WEAPON || temp.SonType == GoodsSonType.SONTYPE_WING) {
                ShopManager.Instance.sendShoping(this._currentSelectedItemData.Id, this.stepper.value, this.playerInfo.userId, false, C2SProtocol.C_FASHION_SHOP_BUY, useBind, disCountTempId);
            } else {
                ShopManager.Instance.sendShoping(this._currentSelectedItemData.Id, this.stepper.value, this.playerInfo.userId, false, C2SProtocol.U_C_SHOP_BUY, useBind, disCountTempId);
            }
            this.setBuybtnState();
        }
    }
    ...
    private get currentCanCount(): number {
        let pointCount: number = Math.floor(this.PayMoney / this._currentSelectedItemData.price);
    ...
    public get PayMoney(): number {
        if (this._currentSelectedItemData.PayType == ShopGoodsInfo.PAY_BY_POINT || this._currentSelectedItemData instanceof MainShopInfo) {
            return this.playerInfo.point;
        } else if (this._currentSelectedItemData.PayType == ShopGoodsInfo.PAY_BY_GIFT) {
            return this.playerInfo.giftToken + this.playerInfo.point;
        } else if (this._currentSelectedItemData.PayType == ShopGoodsInfo.PAY_BY_GOLD) {
            return ResourceManager.Instance.gold.count;
        } else if (this._currentSelectedItemData.PayType == ShopGoodsInfo.PAY_BY_OFFER) {
            return this.playerInfo.consortiaOffer;
        } else if (this._currentSelectedItemData.PayType == ShopGoodsInfo.PAY_BY_HONOR) {
            return GoodsManager.Instance.getGoodsNumByTempId(ShopGoodsInfo.MEDAL_TEMPID);
        } else if (this._currentSelectedItemData.PayType == ShopGoodsInfo.PAY_BY_MAZE) {
            return GoodsManager.Instance.getGoodsNumByTempId(MazeModel.SHOP_MAZE_COIN_TEMPID);
        } else if (this._currentSelectedItemData.PayType == ShopGoodsInfo.PAY_BY_GLORY) {
            return PlayerManager.Instance.currentPlayerModel.playerInfo.gloryPoint;
        } else if (this._currentSelectedItemData.PayType == ShopGoodsInfo.PAY_BY_BACKPLAYER) {
            return GoodsManager.Instance.getGoodsNumByTempId(ShopGoodsInfo.BACKPLAYER_TEMPID);
        } else {
            return this.playerInfo.point;
        }
    }
    ...
    private updateLimit() {
        ...
        if (this._currentSelectedItemData.WeeklyLimit > -1) {
            ...
            let weekLeftCount = (this._currentSelectedItemData.WeeklyLimit - this._currentSelectedItemData.weekCount)
            this.txt_limit.text = (weekLeftCount > 0 ? "[color=#FFECC6]" : "[color=#FF0000]") + weekLeftCount + "[/color]/" + this._currentSelectedItemData.WeeklyLimit;
            maxCount = Math.min(weekLeftCount, maxCount, this.currentCanCount);
        } else if (this._currentSelectedItemData.OneDayCount > -1) {
            let dayLeftCount = (this._currentSelectedItemData.OneDayCount - this._currentSelectedItemData.OneCurrentCount)
            this.txt_limit.text = (dayLeftCount > 0 ? "[color=#FFECC6]" : "[color=#FF0000]") + dayLeftCount + "[/color]/" + this._currentSelectedItemData.OneDayCount
    ...
    private onBuyResult() {
        ...
        this._discountTempleteData = null;
        this.c_discount.selectedIndex = 0;
        let usebind = this._currentSelectedItemData.PayType == ShopGoodsInfo.PAY_BY_POINT ? false : true;
    ...
    public removeEvent() {
        ...
        this.playerInfo.removeEventListener(PlayerEvent.PLAYER_INFO_UPDATE, this.updateMoney, this);
        GoodsManager.Instance.removeEventListener(BagEvent.UPDATE_BAG, this.__updateGoodsHandler, this);
