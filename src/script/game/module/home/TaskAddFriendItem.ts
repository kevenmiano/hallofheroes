...
import { FreedomTeamEvent, FriendUpdateEvent } from "../../constant/event/NotificationEvent";
...
export default class TaskAddFriendItem extends FUI_TaskAddFriendItem {
		...
		if (!this._vData) {
			if (WorldBossHelper.checkShowRoomTeam()) {
				...
				if (this.checkIsSingleCampign) {
					MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("selectcampaign.data.CanNotBegainCampaignInTeam1"));
					return;
				}
				FrameCtrlManager.Instance.open(EmWindow.Invite, { type: RoomInviteType.SpaceInvite });
			}
		}
	}
	...
	private updateView() {
		if (this._vData) {
			...
			if (this._vData.baseHero.frameId > 0) {
				let itemData: t_s_itemtemplateData = TempleteManager.Instance.getGoodsTemplatesByTempleteId(this._vData.baseHero.frameId);
				if (itemData) {
					this.playerIcon.headFrame = itemData.Avata;
					this.playerIcon.headEffect = (Number(itemData.Property1) == 1) ? itemData.Avata : "";
				}
			}else{
			...
			this.hpProgress.value = Math.floor((this._vData.hp / this._vData.maxHp) * 100);
			if (this.roomInfo && this.roomInfo.isCross) {
				...
				if (
					(FreedomTeamManager.Instance.hasTeam && FreedomTeamManager.Instance.model.captainId == this._vData.baseHero.userId) ||
					(this.roomInfo && this.roomInfo.houseOwnerId == this._vData.baseHero.userId && this.checkIsPve)
				) {
					this.teamImg.visible = true;
					this.userNameTxt.color = ColorConstant.BLUE_COLOR;
				} else {
			...
			if (this._showQuickInviteBtn) {
				...
				this.quick_invite.text = LangManager.Instance.GetTranslation("invite.friend.quick");
			}
		}
	}
	...
	private refreshIconFilter() {
		...
		if (FreedomTeamManager.Instance.memberIsOnline(this._vData.baseHero.userId) || this._vData.online) {
	...
	private get checkIsSingleCampign(): boolean {
		...
		if (currentScene == SceneType.CAMPAIGN_MAP_SCENE && mapModel && mapModel.isSingleCampaign()) {
			return true;
		}
		return false;
	}
