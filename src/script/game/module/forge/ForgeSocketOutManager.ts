...
export class ForgeSocketOutManager {
   ...
   * @param useBind
   */
  public static sendUpdateEquip(
    composeId: number,
    num: number,
    useBind: boolean = true,
    extendsMould: boolean = false,
  ) {
    let msg: StoreReqMsg = new StoreReqMsg();
    msg.composeId = composeId;
    msg.composeNum = num;
   ...
   * @param  bindType  绑定类型
   */
  public static sendRefreshforRandom(
    ary: any[],
    bindType: number,
    useBind: boolean = true,
  ) {
