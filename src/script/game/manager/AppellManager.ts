...
export default class AppellManager extends GameEventDispatcher {
  ...
  public setup() {
    ServerDataManager.listen(
      S2CProtocol.U_APPELL_DATA,
      this,
      this.__updateAppellDataHandler,
    );
    ServerDataManager.listen(
      S2CProtocol.U_APPELL_GET,
      this,
      this.__getAppellHandler,
    );
  }
  ...
  private __updateAppellDataHandler(pkg: PackageIn) {
    ...
    for (var i: number = 0; i < appells.length; i++) {
      ...
      if (
        this.thane.templateInfo &&
        item.Job != this.thane.templateInfo.Job &&
        item.Job != 0
      ) {
        continue;
      }
  ...
  private getAppellTip(pkg: PackageIn) {
    ...
    var appellInfo: t_s_appellData =
      TempleteManager.Instance.getAppellInfoTemplateByID(msg.appellId);
