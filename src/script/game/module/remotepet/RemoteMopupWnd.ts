...
export class RemoteMopupWnd extends BaseWindow {
  ...
  private initData() {
    ...
    let wearyCount = Math.floor(
      PlayerManager.Instance.currentPlayerModel.playerInfo.weary / this.weary,
    );

    this._turnCounts = Math.min(this._turnCounts, wearyCount);

    this.maxMopup = Math.min(
      this.maxMopup,
      this.model.turnInfo.currTurn + wearyCount - 1,
    );
    ...
    this.stepper.show(
      0,
      this.maxMopup,
      this.model.turnInfo.currTurn,
      this.maxMopup,
      this.maxMopup,
      1,
      Laya.Handler.create(this, this.textChangeHandler, null, false),
    );
  ...
  private startMopupHandler() {
    ...
    if (
      isInAAS &&
      PlayerManager.Instance.currentPlayerModel.indulgeTime / 3600 >= 3
    ) {
      MessageTipManager.Instance.show(
        LangManager.Instance.GetTranslation("remotepet.mopup.inAAS"),
      );
      return;
    } else if (mopupLayer == 0) {
      str = LangManager.Instance.GetTranslation(
        "mopup.view.RemoteMopupPrepareFrame.layerTipData1",
      );
      MessageTipManager.Instance.show(str);
      return;
    } else if (mopupLayer < model.turnInfo.currTurn) {
      str = LangManager.Instance.GetTranslation(
        "mopup.view.RemoteMopupPrepareFrame.layerTipData2",
      );
      MessageTipManager.Instance.show(str);
      return;
    } else if (mopupLayer > +this.maxMopup) {
      str = LangManager.Instance.GetTranslation(
        "mopup.view.RemoteMopupPrepareFrame.layerTipData3",
      );
      MessageTipManager.Instance.show(str);
      return;
    }
  ...
  private addMopupData(type: eMopupItemType = eMopupItemType.Mopuping) {
    let mopupData = {
      state: type,
      data: { count: this._layerCount, items: null },
    };
