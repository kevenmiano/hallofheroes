...
interface PathIPathSearcher {
  search(from: Point, end: Point, hittest: PathIHitTester): any[];
}
...
export class PathRoboSearcher implements PathIPathSearcher {
  ...
  public search(from: Point, end: Point, hittest: PathIHitTester): any[] {
    ...
    let leftPath: any[] = [];
    let rightPath: any[] = [];
    let left: boolean = this.searchWithWish(
      from,
      end,
      hittest,
      PathRoboSearcher.LEFT,
      leftPath,
    );
    let right: boolean = this.searchWithWish(
      from,
      end,
      hittest,
      PathRoboSearcher.RIGHT,
      rightPath,
    );
    if (left && right) {
   ...
   * 返回查找到的路径,null表示查找失败
   */
  private searchWithWish(
    from: Point,
    tto: Point,
    tester: PathIHitTester,
    wish: number,
    nodes: any[],
  ): boolean {
    ...
    //if(tester.isHit(from) && !endInBlock){
      ...
      //然后再从此行走点路径搜索
      let midSearch: boolean = this.searchWithWish(
        midTo,
        tto,
        tester,
        wish,
    ...
    let aheadSearch: boolean = this.doSearchWithWish(
      from,
      tto,
      tester,
      wish,
    ...
    if (nodes.length > 4) {
      ...
      let success: boolean = this.doSearchWithWish(
        tto,
        nodes[0],
        tester,
        0 - wish,
   ...
   * 如果返回null表示查找失败
   */
  private findFarestBlankPoint(
    from: Point,
    toPoint: Point,
    hitTester: PathIHitTester,
  ): Point {
   ...
   */
  private findReversseNearestBlankPoint(
    from: Point,
    toPoint: Point,
    hitTester: PathIHitTester,
  ): Point {
