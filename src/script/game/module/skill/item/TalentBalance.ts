...
export class TalentBalance extends FUI_TalentBalance {
  ...
  protected __washPoint(event: MouseEvent) {
    ...
    if (this.thane.talentData.curUsedTalentPoint == 0) {
      MessageTipManager.Instance.show(
        LangManager.Instance.GetTranslation("armyII.viewII.talent.content04"),
      );
      return;
    }
    let cfgValue = 100;
    let cfgItem =
      TempleteManager.Instance.getConfigInfoByConfigName("talent_reset_gold");
    if (cfgItem) {
    ...
    var str: string = LangManager.Instance.GetTranslation(
      "armyII.viewII.talent.content03",
      cfgValue,
    );
  ...
  private payCallBack(
    ...
    if (b) {
      if (flag) ArmySocketOutManager.sendWashSkillPoint(0, 1, true);
      else ArmySocketOutManager.sendWashSkillPoint(type, 1, flag);
  ...
  updateView() {
    ...
    let str =
      this.thane.talentData.talentPoint +
      "/" +
      ArmyManager.Instance.thane.talentData.talentGrade;
    this.talentPoint.text =
      LangManager.Instance.GetTranslation(
        "armyII.viewII.talent.TalentView.TalentPoint",
      ) + str;
    this.txt_gold.text = FormularySets.toStringSelf(
      ResourceManager.Instance.gold.count,
      GeniusPanel.STEP,
    );
    this.txt_exp.text = FormularySets.toStringSelf(
      this.thane.gp,
      GeniusPanel.STEP,
    );
    ArmyManager.Instance.thane.skillCate.dispatchEvent(
