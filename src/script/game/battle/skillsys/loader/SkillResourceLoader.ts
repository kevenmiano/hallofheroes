...
export class SkillResourceLoader {
    ...
    * 
    */
    private static loadBufferResource(bufferTempInfo: t_s_skillbuffertemplateData) {
        let arr: any[];
        if (bufferTempInfo) {//添加buffer的效果
    ...
    * 
    */
    private static loadTalentBufferResource(bufferTempInfo: t_s_skillbuffertemplateData) {
        if (bufferTempInfo && !StringHelper.isNullOrEmpty(bufferTempInfo.TalentEffIds) && bufferTempInfo.TalentEffIds.length > 0) {//添加buffer所带的天赋的效果
            let arr: any[];
            let talentIds: any[] = bufferTempInfo.TalentEffIds.split(',');
            talentIds.forEach(talentId => {
                let effect: t_s_talenteffecttemplateData = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_talenteffecttemplate, String(talentId))
                ...
                for (let index = 0; index < effectBuffer.length; index++) {
                    ...
                    let bufferTempInfo: t_s_skillbuffertemplateData = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_skillbuffertemplate, String(effectBufferId))
                    if (bufferTempInfo) {
                        arr = bufferTempInfo.getActionEffectArr();
                        if (arr[0]) {
                            this.addLoaderByLinkName(arr[0]);
    ...
    */
    private static addSkill(skillId: number, sex: number) {
        ...
        let skillTemp: t_s_skilltemplateData = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_skilltemplate, String(skillId))
        if (skillTemp) {
            let actionTemplate: ActionTemplateData = ConfigMgr.Instance.actionTemplate2[skillTemp.ActionId];
            if (actionTemplate) {
                ...
                if ( totalSoundNames.length > 0) {
                    for (let index = 0; index < totalSoundNames.length; index++) {
                        SkillResourceLoader.soundLoadList.push(ComponentSetting.getSkillSoundSourcePath(totalSoundNames[index]))
                    }
                }
            }
            ...
            if (!StringHelper.isNullOrEmpty(skillTemp.BufferIds)) {
                ...
                for (let j: number = 0; j < bufferIds.length; j++) {
                    ...
                    let bufferTempInfo: t_s_skillbuffertemplateData = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_skillbuffertemplate, String(bufferId))
    ...
    public static getLinkNamesBySkillId(skillId: number, sex: number): any[] {
        ...
        if (skillId != -1 && skillId != 0) {
            let skillTempInfo = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_skilltemplate, String(skillId))
            if (skillTempInfo) actionId = skillTempInfo.ActionId;
        }

        if (actionId != 0) {
            let actionTemp: ActionTemplateData = ConfigMgr.Instance.actionTemplate2[actionId]
            if (actionTemp) {
    ...
    */
    public static getPawnSkillIds(pawnId: number): any[] {
        let arr: any[] = [];
        let tempInfo: t_s_pawntemplateData = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_pawntemplate, pawnId.toString())
    ...
    */
    public static getHeroFullSkillIds(ids: any[]): any[] {
        ...
        let arr: any[] = ids.concat(new Array());
        for (let index = 0; index < ids.length; index++) {
            const id = ids[index];
            if (id <= 0) {
                continue;
            }
            tempInfo = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_skilltemplate, String(id))
            if (tempInfo && tempInfo.Parameter3 != 0 && arr.indexOf(tempInfo.Parameter3) == -1) {
