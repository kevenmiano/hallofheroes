 ...
 * 嘉年华---每日挑战
 */
export default class CarnivalDayTaskPage
  extends FUI_CarnivalDayTaskPage
  implements CarnivalBasePage
{
  ...
  private initData() {
    this.Img_Role_3.url = this.model.getThemeFolderImgPath(
      CARNIVAL_FOLDER.DAYTASK,
      "Img_Role_3",
    );
    //重置时间
  ...
  private refreshView() {
    ...
    list = ArrayUtils.sortOn(
      list,
      ["isReward", "data", "taskId"],
      [ArrayConstant.NUMERIC, ArrayConstant.DESCENDING, ArrayConstant.NUMERIC],
    );
    this._itemList = list;
