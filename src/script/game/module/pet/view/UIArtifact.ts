...
export default class UIArtifact extends BaseFguiCom {
  ...
  private initView() {
    for (let j: number = 1; j <= 5; j++) {
      this["itemQualification" + j].titleTxt.text =
        LangManager.Instance.GetTranslation("petWnd.UIArtifact.itemTitle" + j);
    }
    this.updateListData();
    this.initPetValue();
  }
  ...
  public set data(value: PetData) {
    this._petData = value;
    if (this._petData) {
      this._petData.addEventListener(
  ...
  private updatePetValue() {
    for (let j: number = 1; j <= 5; j++) {
      let maxWidth =
        (this.getMaxValue(j) * UIArtifact.MAX_WIDTH) / UIArtifact.MAX_VALUE; //算出来进度条的最大宽度;
      if (this.getMaxValue(j) > UIArtifact.MAX_VALUE) {
        ...
        (this["itemQualification" + j].prog2 as fgui.GProgressBar).max =
          UIArtifact.MAX_VALUE;
        (this["itemQualification" + j].prog3 as fgui.GProgressBar).max =
          this.getMaxValue(j) - UIArtifact.MAX_VALUE;
        if (this.getCurrentValue(j) > UIArtifact.MAX_VALUE) {
          ...
          (this["itemQualification" + j].prog3 as fgui.GProgressBar).value =
            this.getCurrentValue(j) - UIArtifact.MAX_VALUE;
        } else {
          (this["itemQualification" + j].prog2 as fgui.GProgressBar).value =
            this.getCurrentValue(j);
  ...
  private getCurrentValue(index: number): number {
    ...
    for (let i = 0; i < this.equipArr.length; i++) {
      ...
      //判断英灵是否有穿戴装备
      if (
        goodsInfo.objectId == this._petData.petId &&
  ...
  private getType(goodsInfo: GoodsInfo): number {
    let type: number = 0;
    if (GoodsCheck.hasIdentify(goodsInfo)) {
      //已经鉴定过
      type = 5;
    } else {
      //未鉴定
