...
export default class OuterCityWarDefenceSettingWnd extends BaseWindow {
    ...
    private initView() {
        this.txtFrameTitle.text = LangManager.Instance.GetTranslation("outerCityWar.defenceSetting")
        ...
        this.txtDefenceBuild.text = LangManager.Instance.GetTranslation("outerCityWar.defanceBuild")
    ...
    private refreshView() {
        ...
        this.txtPetCapaity.text = LangManager.Instance.GetTranslation("outerCityWar.petCapaity", PetData.getPetTypeLanguage(this.buildInfo.petType))
        ...
        let cur = this.fightModel.getGuildPlayerCnt(this.fightModel.selfGuildId, this.buildInfo.isPetBuild, BooleanType.TRUE, [EmOuterCityWarPlayerState.DEFANCE])
        let total = this.fightModel.getGuildPlayerCnt(this.fightModel.selfGuildId, false, BooleanType.TRUE)
    ...
    private onRenderListItem(index: number, item: OuterCityWarDefenceSettingItem) {
    ...
    private refreshPlayerInfoList() {
        this.playerInfoList = this.fightModel.getCurCastleJoinWarGuildPlayerList(this.fightModel.selfGuildId)
        ...
        if (this.buildInfo.isPetBuild) {
            ...
            if (this.buildInfo.buffType == 101) {
                this.playerInfoList.sort((pInfo1, pInfo2) => {
                    let capA = pInfo1.getPawnLevelByMasterType(this.buildInfo.buffCondition)
                    let capB = pInfo2.getPawnLevelByMasterType(this.buildInfo.buffCondition)
                    let b1 = capA > capB
                    let sumA1 = b1 ? 100 : 0
                    let sumB1 = b1 ? 0 : 100
                    // 没携带兵或上阵的不是对应的兵种
