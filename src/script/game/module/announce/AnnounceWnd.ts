...
export default class AnnounceWnd extends BaseWindow {
  ...
  public OnInitWind() {
    ...
    if (this.params && this.params.frameData) this.xml = this.params.frameData;
    this.refreshView();
  }
  ...
  addEvent() {
    this.menuTab.itemRenderer = Laya.Handler.create(
      this,
      this.renderListItem,
      null,
      false,
    ...
    this.content
      ...
      .on(Laya.Event.LINK, this, this.onMessageHandler);
  }

  offEvent() {
    if (this.menuTab && this.menuTab.itemRenderer) {
      if (this.menuTab.itemRenderer instanceof Laya.Handler) {
    ...
    if (this.menuTab) {
      this.menuTab.off(
        ...
        this,
        this.__onTabMenuSelect,
      );
    }
    this.content
      .getChild("contentText")
      .off(Laya.Event.LINK, this, this.onMessageHandler);
  }
  ...
  renderListItem(index: number, item: fgui.GButton) {
    ...
    item.selectedIcon = fgui.UIPackage.getItemURL(
      EmWindow.Announce,
      "Tab_Menu1_Title1",
    );
    if (titleData.isNoticeFlag == 1 && index != 0) {
      let redPointCom = FUIHelper.createFUIInstance(
        EmWindow.Announce,
        "RedPoint",
      );
    ...
    if (titleData && titleData.title) item.title = titleData.title;
    else item.title = index.toString();
  }
