...
export default class AutoWalkWnd extends BaseWindow {
    ...
    public OnInitWind() {
        ...
        this.offEvent();
        this.addEvent();
        let autoWalkTxt = LangManager.Instance.GetTranslation("AutoWalkView.autoBtn.title3");
    ...
    private addEvent() {
        ...
        if (this.playerModel) this.playerModel.addEventListener(CampaignEvent.AUTO_WALK_CHANGED, this.updateAutoWalkBtn, this);
    }

    private offEvent() {
        this.autoBtn.offClick(this, this.autoBtnHandler);
        if (this.playerModel) this.playerModel.removeEventListener(CampaignEvent.AUTO_WALK_CHANGED, this.updateAutoWalkBtn, this);
    }
    ...
    private updateAutoWalkBtn() {
        ...
        if (this.playerModel.getAutoWalkFlag() == PlayerModel.AUTO_WALK) {
            this.autoBtn.title = LangManager.Instance.GetTranslation("AutoWalkView.autoBtn.title4");
            ...
            this.autoBtn.title = LangManager.Instance.GetTranslation("AutoWalkView.autoBtn.title2");
            this.autoWalkAni.visible = false;
            this.autoWalkAni.stop();
            if (CampaignManager.Instance.controller) {
                let armyView: CampaignArmyView = CampaignManager.Instance.controller.getArmyView(this.campaignModel.selfMemberData);
