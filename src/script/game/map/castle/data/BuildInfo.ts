...
export class BuildInfo {
  ...
  public get sonType(): number {
    if (this.templeteInfo) return this.templeteInfo.SonType;
    return 0;
  }
  ...
  public get sort(): number {
    if (this.templeteInfo) {
      if (this.templeteInfo.SonType == BuildingType.OFFICEAFFAIRS) {
        //内政厅
        return 1;
      } else if (this.templeteInfo.SonType == BuildingType.CASERN) {
        //兵营
        return 3;
      } else if (this.templeteInfo.SonType == BuildingType.SEMINARY) {
        //神学院
        return 5;
      } else if (this.templeteInfo.SonType == BuildingType.CRYSTALFURNACE) {
        //精炼炉
        return 7;
      } else if (this.templeteInfo.SonType == BuildingType.WAREHOUSE) {
        //仓库
        return 9;
      } else if (this.templeteInfo.SonType == BuildingType.HOUSES) {
        //民居
        return 11;
      }
    }
  ...
  public get templeteInfo(): t_s_buildingtemplateData {
    ...
    let item: t_s_buildingtemplateData =
      TempleteManager.Instance.getBuildTemplateByID(this.templateId);
    ...
    if (!item) {
      ...
      if (
        !this.template ||
        (this.template && this.template.TemplateId != this.templateId)
      ) {
        this.template = item;
      }
    }
  ...
  public get nextTemplateInfo(): t_s_buildingtemplateData {
    ...
    let item: t_s_buildingtemplateData =
      TempleteManager.Instance.getBuildTemplateByID(
        this.templeteInfo.NextGradeTemplateId,
      );
    // let mDataList= data.mDataList;
    // for(let element of mDataList){
    ...
    if (!item) {
      ...
      if (
        !this._nextTemplate ||
        (this._nextTemplate && this._nextTemplate.TemplateId != item.TemplateId)
      ) {
