...
export default class GoldenSheepManager {
  ...
  private initEvent() {
    ServerDataManager.listen(
      S2CProtocol.U_C_GOLDEN_SHEEP,
      this,
      this.__goldenSheepHandler,
    );
  }
  ...
  private __goldenSheepHandler(pkg: PackageIn) {
    ...
    this.model.openTime = DateFormatter.parse(
      msg.openTime,
      "YYYY-MM-DD hh:mm:ss",
    );
    this.model.endTime = DateFormatter.parse(
      msg.stopTime,
      "YYYY-MM-DD hh:mm:ss",
    );
    ...
    if (msg.sheepData && msg.sheepData.length > 0) {
      ...
      for (let i: number = 0; i < len; i++) {
        let recordMsg: GoldenSheepPlayerMsg = msg.sheepData[
          i
        ] as GoldenSheepPlayerMsg;
