...
export class CampaignSocketOutManager {
  ...
  public static get Instance(): CampaignSocketOutManager {
    if (!CampaignSocketOutManager._instance)
      CampaignSocketOutManager._instance = new CampaignSocketOutManager();
  ...
  public sendReturnCampaignRoom(armyId: number, sendMsg: boolean = false) {
    Logger.info(
      "[CampaignSocketOutManager]sendReturnCampaignRoom 退出战役房间 armyId",
      armyId,
    );
  ...
  public sendTranseferState(
    //@ts-expect-error: External dependencies
    msg: com.road.yishi.proto.campaign.PlayerNodeStateMsg,
  ) {
