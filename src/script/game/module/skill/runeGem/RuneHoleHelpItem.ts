 ...
 */
 export class RuneHoleHelpItem extends FUI_RuneHoleHelpItem {
    ...
    set info(holeData: t_s_runeholeData) {
        this._info = holeData;
        this.initRuneHole();
        let shapeIdArr = [holeData.Hole1,holeData.Hole2,holeData.Hole3,holeData.Hole4,holeData.Hole5];
        if(holeData){
    ...
    private updateDesc(holeData:t_s_runeholeData){
        ...
        if(holeData && (holeData.Condition1 > 0 || holeData.Condition2 >0)){
            if(holeData.EffectType1 == 1){
                this.getGemEffDesc(holeData.Condition1,holeData.ConditionParam1,holeData.AttributeType1,holeData.ValueType1,holeData.ValueParam1);
            }else if(holeData.EffectType1 == 2)
            {
                this.getSkillEffDesc(holeData,holeData.Condition1,holeData.ConditionParam1);   
            }
            if(holeData.EffectType2 == 1){
                this.getGemEffDesc(holeData.Condition2,holeData.ConditionParam2,holeData.AttributeType2,holeData.ValueType2,holeData.ValueParam2);
            }else if(holeData.EffectType2 == 2)
            {
                this.getSkillEffDesc(holeData,holeData.Condition2,holeData.ConditionParam2); 
            }
    ...
    getGemEffDesc(Condition:number,ConditionParam:number,AttributeType:number,ValueType:number,ValueParam:string){
        let str:string= '';
        //判断激活条件-----------------------------------------------------------------------------------------
        if(Condition == 99){
     ...
     * @param holeCfg 
     */
    getSkillEffDesc(holeData:t_s_runeholeData,Condition:number,ConditionParam:number){
        let str:string='';
        //判断激活条件-----------------------------------------------------------------------------------------
        if(Condition == 99){
            str += LangManager.Instance.GetTranslation('runeGem.str25',ConditionParam);
