...
export class RingTask extends GameEventDispatcher {
  ...
  public getProgress(): any[] {
    ...
    for (var i: number = 0; i < len; i++) {
      var str: string;
      var conditionTemp: t_s_rewardcondictionData =
        this.ringTaskTemp.conditionList[i];
      switch (conditionTemp.CondictionType) {
        ...
        case RewardConditionType.KILLDUPLICATEMONSTER: //击杀副本怪物
          str = this.checkDuplicatemonster(
            this.currTask.condition_1,
            this.currTask.condition_4,
          );
          break;
  ...
  private __pawnsChangeHandler(e: ArmyPawn) {
    var pawn: ArmyPawn = e;
    var nowPawnsNum: number = ArmyManager.Instance.getTotalPawnsNumberBySonType(
  ...
  private checkPawnsCount(conditionTemp: t_s_rewardcondictionData): string {
    var pawnsNum: number = ArmyManager.Instance.getTotalPawnsNumberBySonType(
      conditionTemp.Para1,
    );
  ...
  private checkCount(
    ...
    if (this.currTask) {
      ...
      return (
        " (" +
        this.currTask["condition_" + (index + 1)] +
        "/" +
        conditionTemp.Para2 +
        ")"
      );
  ...
  private getflag(templeteId: number): boolean {
    var goodsInfo: GoodsInfo =
      GoodsManager.Instance.getBagGoodsByTemplateId(templeteId)[0];
    if (goodsInfo) {
  ...
  private __goodsChangeHandler2(arr: any[]) {
    for (let i: number = 0; i < this.conditionList.length; i++) {
      ...
      if (conTemp.Para1 == arr[2]) {
        if (
          this.goodsCountByTempId[conTemp.Para1] +
            this.getCount(conTemp.Para1) <
          conTemp.Para2
        )
          this.hasCompleted = false;
        this.commit();
