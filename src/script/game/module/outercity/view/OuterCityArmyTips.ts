...
import { ChatEvent, OuterCityEvent, RequestInfoEvent, SpaceEvent } from "../../../constant/event/NotificationEvent";
...
export class OuterCityArmyTips extends BaseWindow {
    ...
    private initData() {
        ...
        if (this._data) {
            if (this._data instanceof BaseArmy) {
                ...
                if (this._point.x + this.contentPane.width > Resolution.gameWidth) {//超出屏幕
            ...
            if (this._frameId > 0) {
                let itemData: t_s_itemtemplateData = TempleteManager.Instance.getGoodsTemplatesByTempleteId(this._frameId);
                if (itemData) {
    ...
    private initView() {
        ...
        this.txt_consortia.text = this._thane.consortiaName ? this._thane.consortiaName : LangManager.Instance.GetTranslation("yishi.view.PlayerMenu.consortiaName");
        ...
        if (DataCommonManager.playerInfo.userId == this._userId) {
            this.btn_deleteFriend.enabled = this.btn_addFriend.enabled = this.btn_black.enabled = this.btn_team.enabled = false;
            this.btn_private.enabled = this.btn_info.enabled = this.btn_inviteConsortia.enabled = false;
            this.btn_battle.visible = false;this.btn_petPk.visible = false;
            return;
        }
        let fInfo: FriendItemCellInfo = IMManager.Instance.getFriendInfo(this._thane.userId);  //因为可能从排行榜中点开菜单, 排行榜赋值进来的_thane.relation没值, 所以要取好友里的
        if (fInfo) {
        ...
        let showInvite: boolean = FreedomTeamManager.Instance.canInviteMember(this._userId);
        ...
        if (!StringHelper.isNullOrEmpty(this.playerInfo.consortiaName)) {
            showConsortia = (FrameCtrlManager.Instance.getCtrl(EmWindow.Consortia) as ConsortiaControler).getRightsByIndex(ConsortiaDutyInfo.INVITEMEMBER)
            showConsortia = showConsortia && StringHelper.isNullOrEmpty(this._thane.consortiaName);
        }
        ...
        if (SceneManager.Instance.currentType == SceneType.SPACE_SCENE) {//在天空之城
            this.btn_battle.enabled = true;
            this.btn_battle.title = LangManager.Instance.GetTranslation("yishi.view.PlayerMenu.pk");
        }
        else {
            this.btn_battle.enabled = true;
            this.btn_battle.title = LangManager.Instance.GetTranslation("outyard.OutyardActorView.attackBtn.text");
        }
    }

    private sameServiceName(): boolean {
        if (this._thane.serviceName) {
            return this._thane.serviceName == PlayerManager.Instance.currentPlayerModel.playerInfo.serviceName;
        }
        return true;
    }
    ...
    private canShowAttackBtn(): boolean {
        ...
        if (this._isRankData || this._isChatData || this.isSelf() || this._isFriendData) {
        }
        else {
            if (!this.isSelf() && !FreedomTeamManager.Instance.inMyTeam(this._userId)
            && (SceneManager.Instance.currentType == SceneType.OUTER_CITY_SCENE || SceneManager.Instance.currentType == SceneType.SPACE_SCENE)) {
                attack = true;
            }
        }
        return attack;
    ...
    private isSelf(): boolean {
        ...
        return this.playerInfo.userId == this._userId && this.playerInfo.serviceName == this._thane.serviceName;
    }
    ...
    private __privateHandler() {
        ...
        if (!FriendManager.getInstance().checkIsFriend(this._userId)) {
            MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("chat.notfriend"));
            return;
        }
    ...
    private __battleHandler() {
        ...
        if (SceneManager.Instance.currentType == SceneType.SPACE_SCENE) {
            ...
            let bArmy: BaseArmy = OuterCityManager.Instance.model.getWorldArmyById(this._data.id);
            if (!bArmy) {
                this.hide();
                return;
            }
            ...
            NotificationManager.Instance.dispatchEvent(OuterCityEvent.OUTERCITY_LOCK_WAR_FIGHT, bArmy);
            this.hide();
        }
    }
    ...
    private _petPkHandler(){
        ...
        if (SceneManager.Instance.currentType == SceneType.SPACE_SCENE) {
            ...
            if (ArmyState.checkCampaignAttack(armyInfo.state)) {
                SpaceManager.PKInvite = true;
                SpaceSocketOutManager.Instance.sendMapPetPkInviteTo(armyInfo.baseHero.userId);
            } else {
                //正在战斗不能攻击
                MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("playermenu.command01"));
            }
    ...
    private __onAddFriendHandler(): void {
        ...
        FriendManager.getInstance().sendAddFriendRequest(this._thane.nickName, RelationType.FRIEND);
        this.hide();
    }
    ...
    private __onBlacklistHandler(): void {
        FriendManager.getInstance().sendAddFriendRequest(this._thane.nickName, RelationType.BLACKLIST);
        this.hide();
    }
    ...
    private __onDelFriendHandler(): void {
        if (this._thane) {
            ...
            if (fInfo) {
                ...
                switch (fInfo.relation) {
                    case RelationType.RECENT_CONTACT:
                        str = LangManager.Instance.GetTranslation("yishi.view.PlayerMenu.str01");
                        break;
                    case RelationType.STRANGER:
                        str = LangManager.Instance.GetTranslation("yishi.view.PlayerMenu.delStrangerTip");
                        break;
                    case RelationType.BLACKLIST:
                        str = LangManager.Instance.GetTranslation("yishi.view.PlayerMenu.str02");
                        break;
                    default:
                        str = LangManager.Instance.GetTranslation("yishi.view.PlayerMenu.str03");
                }
                ...
                SimpleAlertHelper.Instance.Show(SimpleAlertHelper.SIMPLE_ALERT, [fInfo], prompt, str, confirm, cancel, this._delFriendResponse.bind(this));
            }
        }
        this.hide();
    }
        ...
        if (hInfo) {
            ...
            if (this._isRankData) {
                if (this.isSameZone()) {
                    ...
                    MessageTipManager.Instance.show(LangManager.Instance.GetTranslation('sort.SortGeneralListCell.notSameZone'));
                }
            }
            else {
                if (!this.isSameZone()) {
                ...
                Logger.xjy("[OuterCityArmyTips]__onInfoHandler", serverName, hInfo.nickName)
                if (serverName && serverName != PlayerManager.Instance.currentPlayerModel.playerInfo.serviceName) {
                    PlayerInfoManager.Instance.sendRequestSimpleInfoCross(hInfo.userId, serverName);
                } else {
                    PlayerInfoManager.Instance.sendRequestSimpleInfo(hInfo.userId);
                }
            }
    ...
    private isSameZone(): boolean {
        if (this._thane && this._thane.mainSite) {
            return PlayerManager.Instance.currentPlayerModel.userInfo.mainSite == this._thane.mainSite;
        }
        return true;
    }
    ...
    dispose(dispose?: boolean) {
        ...
        PlayerManager.Instance.currentPlayerModel.dispatchEvent(SpaceEvent.TARGET_CHANGE, null);
