...
import { ADName, ChannelID, DataCallback, NativeAdCallback, ResultCallback, ResultState, SDKConfig, isNull } from '../SDKConfig';
 ...
 */
    ...
    openCustomerService() {
        ...
        let param: object = {
            ...
            "roleId": ArmyManager.Instance.thane.userId ? ArmyManager.Instance.thane.userId.toString() : "",
            "roleName": "",
        ...
        if (PathManager.info.serviceURL != "")
            Laya.Browser.window.open(PathManager.info.serviceURL + "?param=" + paramString);
    }
    ...
    saveScreenshot(sp: Laya.Sprite, isShare: number = 1, code: number = 0, title: string = "title", desc: string = "desc") {
        ...
        //600x600 或者  600x314
        let htmlCanvas1: HTMLCanvas = sp.drawToCanvas(sp.width * sp.scaleX, sp.height * sp.scaleY, 0, 0);//把精灵绘制到canvas上面
        ...
        HttpUtils.httpRequest(PathManager.info.REQUEST_PATH, "fbshareservlet?" + param, fileStream, 'POST', 'text').then((data) => {
