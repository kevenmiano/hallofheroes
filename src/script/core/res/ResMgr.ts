import { EmPackName, EmWindow, UICFG, UI_PACKAGE } from '../../game/constant/UIDefine';
...
export default class ResMgr implements IManager {
    ...
    public loadRes(url: string, complete: Function = null, progress: Function = null, type?: string,
        ...
        }), type, priority, cache, group, ignoreCache, useWorkerLoader);
    }

    /**加载传类型资源 */
    public loadResItem(loadItem: Laya.loadItem, complete: Function = null, progress: Function = null, type?:string) {
        ...
        Laya.loader.load(temp, Laya.Handler.create(this, (res) => {
    ...
    private static s_BitmapFont: Map<string, Laya.BitmapFont> = new Map<string, Laya.BitmapFont>();
    ...
    public LoadBitmapFont(font: string[], callback: () => void, thisObject: any) {
        ...
        for (let i = 0; i < font.length; ++i) {
            ...
            ++loadNum;
            bitmapFont.loadFont(fontName + ".fnt", new Laya.Handler(this, (fontName: string, bitmapFont: Laya.BitmapFont) => {
    ...
    public loadFont(font: string): Promise<Laya.TTFLoader> {
        const loadFunc = new Promise<Laya.TTFLoader>(resolve => {
            Laya.loader.load(font, Laya.Handler.create(this, (res) => {
                resolve(res);
     ...
     * @param resArr 资源
     */
    public loadGroup(resArr: string[] | Laya.loadItem[], complete: Function = null, progress: Function = null, type?: string,
        priority?: number, cache?: boolean, group?: string | null, ignoreCache?: boolean, useWorkerLoader?: boolean, args?: any) {
        ...
        Laya.loader.load(resArr, Laya.Handler.create(this, (res) => {
            complete && complete(res);
        }), Laya.Handler.create(this, (prog) => {
            progress && progress(prog);
        }, null, false), type, priority, cache, group, ignoreCache, useWorkerLoader);
    }
     ...
     */
    public async loadGroupAsync(group: string[] | Laya.loadItem[], groupKey: string): Promise<boolean> {
     ...
     */
    public async loadAnimAsync(anim: Laya.Animation, animAddress: string): Promise<boolean> {
     ...
     * @param callFunc 
     */
    public loadFairyGui(packName: EmPackName | EmPackName[], callFunc: Function = undefined, progressFunc: Function = undefined, type: string = undefined, priority: number = 0, cache?: boolean) {
        let temp: any
        ...
        fgui.UIPackage.loadPackage(temp, Laya.Handler.create(this, (res) => {
            //加载完成
            callFunc && callFunc(res);
