...
import { PK_Event, SLGSocketEvent, SocketDataProxyEventEvent, SpaceEvent } from "../../constant/event/NotificationEvent";
...
export default class SpaceManager extends GameEventDispatcher {
	...
	public setup($mapId: number = SpaceManager.SpaceMapId, isProxy: boolean = false) {
		...
		if (isProxy) {
			...
			SocketDataProxyManager.Instance.readSocketDataByType(SceneType.SPACE_SCENE);
		} else {
	...
	private addSocketEvent(item: GameEventDispatcher) {
		item.addEventListener(S2CProtocol.U_C_PLAYER_SPACE_ENTER, this.__spaceEnterHandler, this);
		item.addEventListener(S2CProtocol.U_C_PLAYER_SPACE_LEAVE, this.__spaceLeaveHandler, this);
		item.addEventListener(S2CProtocol.U_C_PLAYER_SPACE_SYNC_POS, this.__spaceMoveHandler, this);
		item.addEventListener(S2CProtocol.U_C_HERO_BROAD_EQUIPMENT, this.__otherHeroInfoChange, this);
		item.addEventListener(S2CProtocol.U_C_PLAYER_SPACE_EVENT, this.__spaceEventHandler, this);
		item.addEventListener(SLGSocketEvent.U_PLAY_MOVIE, this.__playMovieHandler, this);
		item.addEventListener(S2CProtocol.U_C_SEND_STANDPOS, this.__standPosHandler, this);
	}

	private removeSocketEvent(item: GameEventDispatcher) {
		item.removeEventListener(S2CProtocol.U_C_PLAYER_SPACE_ENTER, this.__spaceEnterHandler, this);
		item.removeEventListener(S2CProtocol.U_C_PLAYER_SPACE_LEAVE, this.__spaceLeaveHandler, this);
		item.removeEventListener(S2CProtocol.U_C_PLAYER_SPACE_SYNC_POS, this.__spaceMoveHandler, this);
		item.removeEventListener(S2CProtocol.U_C_HERO_BROAD_EQUIPMENT, this.__otherHeroInfoChange, this);
		item.removeEventListener(S2CProtocol.U_C_PLAYER_SPACE_EVENT, this.__spaceEventHandler, this);
		item.removeEventListener(SLGSocketEvent.U_PLAY_MOVIE, this.__playMovieHandler, this);
		item.removeEventListener(S2CProtocol.U_C_SEND_STANDPOS, this.__standPosHandler, this);
	}
	...
	public visitSpaceNPC(npcId: number, isPopFrame: boolean = true, callback: Function = null) {
	...
	private addEvent() {
		ServerDataManager.listen(S2CProtocol.U_C_PLAYER_SPACE_SEND_EXSIT, this, this.__initPlayerListHandler);
		ServerDataManager.listen(S2CProtocol.U_C_PLAYER_REMOVE_IN_SPACE, this, this.__removePlayerListHandler);
		ServerDataManager.listen(S2CProtocol.U_C_HANGUP_PVP, this, this.__onPKRequestHandler);
	}

	private removeEvent() {
		ServerDataManager.cancel(S2CProtocol.U_C_PLAYER_SPACE_SEND_EXSIT, this, this.__initPlayerListHandler);
		ServerDataManager.cancel(S2CProtocol.U_C_PLAYER_REMOVE_IN_SPACE, this, this.__removePlayerListHandler);
	...
	private __spaceLeaveHandler(pkg: PackageIn) {
		...
		var aInfo: any = null;
		if (msg.playerId != this.selfPlayerInfo.userId && SpaceManager.Instance.model) {
			if (SpaceManager.Instance.model)
				aInfo = SpaceManager.Instance.model.removeBaseArmyByArmyId(msg.playerId);
		}
	}

	private __spaceMoveHandler(pkg: PackageIn) {
		SocketSceneBufferManager.Instance.addPkgToBuffer(pkg, SceneType.SPACE_SCENE, this.__spaceSyncPosHandler.bind(this));
	}
	...
	private __spaceSyncPosHandler(pkg: PackageIn) {
		...
		if (SpaceManager.Instance.model) SpaceManager.Instance.model.moveArmyTo(msg);
	}
	...
	private __spaceEventHandler(pkg: PackageIn) {
		...
		NotificationManager.Instance.dispatchEvent(SpaceEvent.SELECT_NODE, msg.param1);
	}
	...
	private __playMovieHandler(pkg: PackageIn) {
		...
		SpaceActionsFactory.createAction(msg.movieType, nodeView, pkg, 75, msg.targetId);
	...
	private __removePlayerListHandler(pkg: PackageIn) {
		...
		for (const key in msg.players) {
			if (msg.players.hasOwnProperty(key)) {
				...
				if (playerMsg.playerId != this.selfPlayerInfo.userId && SpaceManager.Instance.model) {
					SpaceManager.Instance.model.removeBaseArmyByArmyId(playerMsg.playerId);
				}
			}
		}
	}
