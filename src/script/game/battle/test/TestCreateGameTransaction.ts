...
export class TestCreateGameTransaction extends TransactionBase {
  ...
  private initBattleInfo(msg: any) {
    ...
    Logger.battle(
      "[TestCreateGameTransaction]initBattleInfo gameInfo",
      gameInfo,
    );
  ...
  private loadRoleView() {
    ...
    for (const roleInfo of gameInfo.roleList.values()) {
      ...
      if (roleInfo.load) roleInfo.load();
    }
  }
  /**
   * 根据职业加载施法动作特效
  ...
  private createPawnRoleInfo(gameInfo: BattleModel, msg: any): BattleModel {
    ...
    for (let i = 0; i < leng; i++) {
      ...
      Logger.battle(
        "[TestCreateGameTransaction]收到战斗士兵列表 roleName:" + role.roleName,
      );
  ...
  private createHeroRoleInfo(gameInfo: BattleModel, msg: any): BattleModel {
    ...
    for (let i = 0; i < leng; i++) {
      ...
      Logger.battle(
        "[TestCreateGameTransaction]收到战斗英雄列表 nickName=" +
          role.heroInfo.nickName,
      );
      gameInfo.addRole(role);
      if (role.petRoleInfo) {
  ...
  initRoles() {
    ...
    gameInfo.roleList.forEach((roleInfo, key) => {
      ...
      Logger.battle(
        "[TestCreateGameTransaction]initRoles roleInfo.pos:",
        roleInfo.pos,
        "roleInfo.face:",
        roleInfo.face,
        temp,
