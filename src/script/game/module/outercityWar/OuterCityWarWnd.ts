...
import { ChatEvent, OuterCityWarEvent } from "../../constant/event/NotificationEvent";
...
export default class OuterCityWarWnd extends BaseWindow {
    ...
    public resize() {
        ...
        this.map.x = -(this.map.getRealWidth() * OuterCityWarMapView.MAP_SHOW_ANCHOR_POINT.x - Resolution.gameWidth / 2);
        this.map.y = -(this.map.getRealHeight() * OuterCityWarMapView.MAP_SHOW_ANCHOR_POINT.y - Resolution.gameHeight / 2);
        this.map.dragingCallBack();
    }

    private addEvent() {
        OuterCityWarManager.Instance.sendLeaveOutcityWar(1);
        SocketSendManager.Instance.sendCurrentClientState(ClientStateType.CLIENT_CAMPAIGN);
        NotificationManager.Instance.addEventListener(OuterCityWarEvent.CASTLE_INFO, this.__castleInfo, this)
        NotificationManager.Instance.addEventListener(OuterCityWarEvent.ACTION_POINT, this.__actionPoint, this)
        NotificationManager.Instance.addEventListener(OuterCityWarEvent.DEFENCE_NPC_SCORE, this.__defanceNpcScore, this)
        NotificationManager.Instance.addEventListener(ChatEvent.UPDATE_CHAT_VIEW, this.__showChatBtn, this);
        NotificationManager.Instance.addEventListener(ChatEvent.UPDATE_CHAT_BUGLE_VIEW_VISIBLE, this.__showChatBtn, this);
        NotificationManager.Instance.addEventListener(ChatEvent.UPDATE_CHAT_VIEW_VISIBLE, this.__showChatBtn, this);
    }

    private removeEvent() {
        SocketSendManager.Instance.sendCurrentClientState(ClientStateType.CLIENT_WORLDMAP);
        NotificationManager.Instance.removeEventListener(OuterCityWarEvent.CASTLE_INFO, this.__castleInfo, this)
        NotificationManager.Instance.removeEventListener(OuterCityWarEvent.ACTION_POINT, this.__actionPoint, this)
        NotificationManager.Instance.removeEventListener(OuterCityWarEvent.DEFENCE_NPC_SCORE, this.__defanceNpcScore, this)
        NotificationManager.Instance.removeEventListener(ChatEvent.UPDATE_CHAT_VIEW, this.__showChatBtn, this);
        NotificationManager.Instance.removeEventListener(ChatEvent.UPDATE_CHAT_BUGLE_VIEW_VISIBLE, this.__showChatBtn, this);
        NotificationManager.Instance.removeEventListener(ChatEvent.UPDATE_CHAT_VIEW_VISIBLE, this.__showChatBtn, this);
    }
    ...
    private initView() {
        ...
        this.txtPowerDesc.text = LangManager.Instance.GetTranslation("outerCityWar.actionPoint")
        this.txtScoreDesc.text = LangManager.Instance.GetTranslation("outerCityWar.realTimeScore")
        this.btnGiveUpWar.title = LangManager.Instance.GetTranslation("public.giveup2")
        this.btnNotice.title = LangManager.Instance.GetTranslation("outerCityWar.presidentNotice")
        this.btnBattleList.title = LangManager.Instance.GetTranslation("public.battle.enterBattleList")
        this.comCampBlue.getChild("title").text = LangManager.Instance.GetTranslation("public.camp.defanceCamp")
        this.comCampRed.getChild("title").text = LangManager.Instance.GetTranslation("public.camp.attackCamp")
    }
    ...
    private __fightBtnState() {
        if (this.fightModel.getSelfAttackDuty()) {
            ...
            let isDefencer = this.fightModel.checkDefenceGuild(this.fightModel.selfGuildId)
            let isAttacker = this.fightModel.checkAttackGuild(this.fightModel.selfGuildId)
            this.btnGiveUpWar.visible = false
            this.btnDeclareWar.visible = false
            if (isDefencer) {
                this.btnGiveUpWar.visible = true
                let canNotGiveUp = this.fightModel.castleState == EmOuterCityWarCastlePeriodType.Fighting || this.fightModel.castleState == EmOuterCityWarCastlePeriodType.DeclaringWar
                this.btnGiveUpWar.enabled = !canNotGiveUp
            } else {
                this.btnDeclareWar.visible = true
                this.btnDeclareWar.enabled = !isAttacker
                this.btnDeclareWar.title = LangManager.Instance.GetTranslation(isAttacker ? "public.battle.declaredWar" : "public.battle.declareWar")
            }
        } else {
    ...
    private __enterWarGuild() {
        this.gOptInfo.visible = this.fightModel.checkGuildEnterWar(this.fightModel.selfGuildId);
    }
    ...
    private __guildScore() {
        ...
        for (let index = 0; index < OuterCityWarModel.AttackCampSite; index++) {
            ...
            let item = this.attackScoreList.getChildAt(index) as OuterCityWarScoreItem;
            if (item) {
                item.info = info;
            }
        }
    ...
    private __defanceNpcScore() {
        ...
        this.defanceScoreItem.txtScore.text = this.fightModel.castleDefenceNpcScore.toString();
    ...
    private __castleState() {
        ...
        if (flag) {
            ...
            this.stateTimer.addEventListener(TimerEvent.TIMER, this.__stateTimerHandler, this);
    ...
    private showLeftTimeTipSpecial() {
        ...
        switch (state) {
            ...
            case EmOuterCityWarCastlePeriodType.DeclareWar:
                break;
            case EmOuterCityWarCastlePeriodType.DeclaringWar:
                str = LangManager.Instance.GetTranslation("outerCityWar.castleState01", timeStr)
                break;
            case EmOuterCityWarCastlePeriodType.Fighting:
                str = LangManager.Instance.GetTranslation("outerCityWar.castleState02", timeStr)
                break;
            case EmOuterCityWarCastlePeriodType.Protected:
                str = LangManager.Instance.GetTranslation("outerCityWar.castleState03", timeStr)
                break;
        }
        this.gCastleState.visible = true;
    ...
    private __stateTimerHandler() {
        let time: number = this.stateTimer.repeatCount - this.stateTimer.currentCount;
        ...
        switch (state) {
            case EmOuterCityWarCastlePeriodType.DeclaringWar:
                str = LangManager.Instance.GetTranslation("outerCityWar.castleState01", dd)
                break;
            case EmOuterCityWarCastlePeriodType.Fighting:
                str = LangManager.Instance.GetTranslation("outerCityWar.castleState02", dd)
                break;
            case EmOuterCityWarCastlePeriodType.Protected:
                str = LangManager.Instance.GetTranslation("outerCityWar.castleState03", dd)
                break;
        }
    ...
    private btnAddPowerClick() {
        let content = LangManager.Instance.GetTranslation("outerCityWar.buyActionPoint", this.fightModel.ActionPointPrice, this.fightModel.ActionPointBuyGet)
    ...
    private btnGiveUpWarClick() {
        ...
        switch (state) {
            case EmOuterCityWarCastlePeriodType.DeclaringWar:
                MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("outerCityWar.cannotGiveup01"))
                break;
            case EmOuterCityWarCastlePeriodType.Fighting:
                MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("outerCityWar.cannotGiveup02"))
                break;
            default:
                let content: string = LangManager.Instance.GetTranslation("outerCityWar.giveupCastleAlert");
                SimpleAlertHelper.Instance.Show(null, null, null, content, null, null, (b) => {
                    if (b) {
    ...
    private btnDeclareWarClick() {
        ...
        switch (state) {
            case EmOuterCityWarCastlePeriodType.Peace:
                MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("outerCityWar.cannotDeclareWar03"))
                break;
            case EmOuterCityWarCastlePeriodType.DeclareWar:
            case EmOuterCityWarCastlePeriodType.DeclaringWar:
                let content: string = LangManager.Instance.GetTranslation("outerCityWar.consumeDeclareWar", this.fightModel.castleInfo.declareWarCost);
                SimpleAlertHelper.Instance.Show(null, null, null, content, null, null, (b) => {
                    if (b) {
                ...
                break;
            case EmOuterCityWarCastlePeriodType.Fighting:
                MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("outerCityWar.cannotDeclareWar01"))
                break;
            case EmOuterCityWarCastlePeriodType.Protected:
                MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("outerCityWar.cannotDeclareWar02"))
                break;
        }
    }
    ...
    private helpBtnClick() {
        ...
        UIManager.Instance.ShowWind(EmWindow.Help, { title: title, content: content1 + "<br>" + content2 + "<br>" + content3 });
    }
