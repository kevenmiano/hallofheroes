...
export class ShootActionSkill extends BaseSkill {
  ...
  private createCopyFrameData() {
    ...
    if (
      this._actionTemplate.shootTime > 0 &&
      this._actionTemplate.shootTime < this.getSkillData().data.length
    ) {
  ...
  private createShootAction(): SimpleScriptAction {
    ...
    let shootComplete = () => {
      ...
      if (
        this._shootCount >=
        this._actionTemplate.shootTime + this._copyFrameData.length
      ) {
        actionScript.finish();
      }
    };
