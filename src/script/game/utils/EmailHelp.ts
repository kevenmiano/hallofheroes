...
export default class EmailHelp {
    ...
    public static checkCanGetGoods(): number {
        ...
        if (!EmailHelp.BagNewCount) {
            var str: string = LangManager.Instance.GetTranslation("emailII.help.EmailHelp.command01");
    ...
    public static checkCanGetStart(): number {
        ...
        var starBagCount: number = PlayerManager.Instance.currentPlayerModel.playerInfo.starBagCount;
        var count: number = PlayerModel.ORIGINAL_OPEN_BAG_COUNT + starBagCount - playerStarNum;
        if (!count) {
            var str: string = LangManager.Instance.GetTranslation("emailII.help.EmailHelp.command02");
            MessageTipManager.Instance.show(str);
            return -1;
        }
        return count;
