...
export default class MarketRecordItem extends FUI_MarketRecordItem {
    ...
    private updateView() {
        ...
        let color = GoodsSonType.getColorByProfile(this._cachesGoods.templateInfo.Profile);
        ...
        if (this._data.type == 1) {
            ...
            //出售
            desc = LangManager.Instance.GetTranslation("Market.record.sell", time, color, goodsName, this._data.point, getPoint);

        } else if (bp > 0) {
            //求购退还差价
            desc = LangManager.Instance.GetTranslation("Market.record.buyBack", time, color, goodsName, this._data.tradingPoint, bp)

        } else {
            //求购
            desc = LangManager.Instance.GetTranslation("Market.record.buy", time, color, goodsName, this._data.tradingPoint);
        }
