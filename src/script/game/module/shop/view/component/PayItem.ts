...
export class PayItem extends FUI_PayItem {
  ...
  set index(value: number) {
    ...
    if (ChargeLotteryManager.instance.openChargeLottery) {
      ...
      let msg: LotteryInfoMsg = ChargeLotteryManager.instance.chargeMsg
        .lotteryInfo[value - 1] as LotteryInfoMsg;
      if (msg) {
        this.txt_times.text = "+" + msg.giveNum;
        // this.txt_times.y = msg.giveNum >= 100 ? 64 : 58;
        this.txt_leftTimes.text = LangManager.Instance.GetTranslation(
          "emailII.view.EmailItemView.leftTime",
          msg.maxNum - msg.curNum,
        );
  ...
  set info(value: t_s_rechargeData) {
    ...
    this.txt_diamond.text =
      value.Para2 +
      " " +
      LangManager.Instance.GetTranslation("mainBar.WorldBossBuyBuffBar.point"); //获得钻石
    if (Utils.isInteger(value.MoneyNum)) {
      this.txt_price.text =
        ShopManager.Instance.getMoneyString() + parseInt(value.MoneyNum); //价格
    } else {
      this.txt_price.text =
        ShopManager.Instance.getMoneyString() + value.MoneyNum; //价格
    }
