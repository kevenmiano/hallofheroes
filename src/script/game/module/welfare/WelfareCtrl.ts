...
export default class WelfareCtrl extends FrameCtrlBase {
    ...
    * index  第几天签到 1 为第一天
    */
    public sendSignReward(optType: number = 1, prizeSite: number = 0,payType:number = 0,index:number=0) {
    ...
    protected addDataListener() {
        ...
        ServerDataManager.listen(S2CProtocol.U_C_GRADE_PACKET_RECEIVE, this, this.levelBackInfo);
        ServerDataManager.listen(S2CProtocol.U_C_SEVEN_SIGN_RECEIVE, this, this.sevenLoginRewardInfo);
    }

    protected delDataListener() {
        super.delDataListener()
        ServerDataManager.cancel(S2CProtocol.U_C_GRADE_PACKET_RECEIVE, this, this.levelBackInfo);
        ServerDataManager.cancel(S2CProtocol.U_C_SEVEN_SIGN_RECEIVE, this, this.sevenLoginRewardInfo);
    }
    ...
    private levelBackInfo(pkg: PackageIn) {
        ...
        this.data.levelPackageArr = ArrayUtils.sortOn(this.data.levelPackageArr, ["packageState1", "id"], [ArrayConstant.DESCENDING | ArrayConstant.CASEINSENSITIVE]);
    }

    private updateFreePackageState(item: LevelGiftItemInfo, value: number): number {
        let state: number = 0;
        if (value >> item.id & 0x01) {//已经领取
            state = WelfareData.HAS_GET;
            this.data.currentGetPackageId = this.data.currentGetPackageId > item.id ? this.data.currentGetPackageId : item.id;
        }
        else {
            if (ArmyManager.Instance.thane.grades >= item.grade)//玩家等级大于礼包要求的等级
            {
                if (this.data.currentGetPackageId == item.id - 1)//可领取
                {
    ...
    private updateDiamondPackageState(item: LevelGiftItemInfo, value: number): number {
        let state: number = 0;
        if (value >> item.id & 0x01) {//已经购买
            state = 2;
        }
        else {
            state = 1; //可购买
    ...
    private getState1(item1: GradePacketMsg, item2: GradePacketRsp): number {
        let state: number = 0;
        if (item2.receiveSite >> item1.id & 0x01) {//已经领取
            state = WelfareData.HAS_GET;
            this.data.currentGetPackageId = this.data.currentGetPackageId > item1.id ? this.data.currentGetPackageId : item1.id;
        }
        else {
            if (ArmyManager.Instance.thane.grades >= item1.grade)//玩家等级大于礼包要求的等级
            {
                if (this.data.currentGetPackageId == item1.id - 1)//可领取
                {
                    state = WelfareData.CAN_GET;
                }
                else {
    ...
    private getState2(item1: GradePacketMsg, item2: GradePacketRsp): number {
        let state: number = 0;
        if (item2.buySite >> item1.id & 0x01) {//已经购买
            state = 2;
        }
        else {
            state = 1; //可购买
    ...
    */
    ...
    private sevenGoalsTaskRewardInfo(pkg: PackageIn) {
        ...
        if (msg) {
            ...
            this.data.sevenTask = ArrayUtils.sortOn(this.data.sevenTask, ["status"], [ArrayConstant.NUMERIC]);
            Logger.xjy("[WelfareCtrl] sevenGoalsTaskRewardInfo sevenTask=" + this.data.sevenTask);
        }
    ...
    private sevenLoginRewardInfo(pkg: PackageIn) {
        let msg: SevenSignReceiveRsp = pkg.readBody(SevenSignReceiveRsp) as SevenSignReceiveRsp;
        if (msg) {
            ...
            for (let i = 0; i < len; i++) {
                ...
                sevenLogingInfo.status = this.data.getSevenLoginRewardStatus(sevenLogingInfo.day, msg.rewardSite, this.data.sevenLoginTotalDays);
            }
        }
        this.data.sevenLoginRewardArr = ArrayUtils.sortOn(this.data.sevenLoginRewardArr, ["day"], [ArrayConstant.NUMERIC]);
        NotificationManager.Instance.dispatchEvent(NotificationEvent.SEVEN_LOGIN_REWARD_UPDATE);
    }
   ...
   private getCurrentDay(time: number): number {
    ...
    let startDate: Date = Utils.formatTimeZone(timeValue, PlayerManager.Instance.currentPlayerModel.zoneId);
    ...
    this.data.endTime = startDate.getTime() + totalAdd + 6 * 24 * 60 * 60 * 1000;
