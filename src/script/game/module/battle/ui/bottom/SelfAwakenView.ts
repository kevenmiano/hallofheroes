...
import { BattleEvent, BattleNotic, NotificationEvent, RoleEvent } from "../../../../constant/event/NotificationEvent";
...
export class SelfAwakenView extends GameEventDispatcher {
    ...
    public addEvent() {
        this._hero.addEventListener(RoleEvent.AWAKEN, this.__onSelfAwakenChangedHandler, this);
        this._hero.addEventListener(RoleEvent.IS_LIVING, this.__livingHandler, this);
        this._awakenlist.itemRenderer = Laya.Handler.create(this, this.__onRenderListItem, null, false);
        this._awakenlist.on(fairygui.Events.CLICK_ITEM, this, this.__onClickCellItem);
        NotificationManager.Instance.addEventListener(BattleEvent.BATTLE_COMPLETE, this.__battleEndHandler, this);
        NotificationManager.Instance.addEventListener(BattleNotic.SKILL_CD, this.__skillEnabeleHandler, this);
        NotificationManager.Instance.addEventListener(NotificationEvent.USE_SKILL_RESULT, this._skillResultHandler, this);
        NotificationManager.Instance.addEventListener(BattleNotic.RESTRICT_CHANGED, this.__restrictChangeHandler, this);
        ...
        NotificationManager.Instance.addEventListener(BattleEvent.BATTLE_DYNAMIC_UI_LOADED, this.onBattleDynimicUILoaded, this);
        Laya.stage.on(Laya.Event.KEY_DOWN, this, this.__onKeyDownHandler);
    }

    public removeEvent() {
        if (this._hero) {
            this._hero.removeEventListener(RoleEvent.AWAKEN, this.__onSelfAwakenChangedHandler, this);
            this._hero.removeEventListener(RoleEvent.IS_LIVING, this.__livingHandler, this);
        }
        this._awakenlist.off(fairygui.Events.CLICK_ITEM, this, this.__onClickCellItem);
        NotificationManager.Instance.removeEventListener(BattleEvent.BATTLE_COMPLETE, this.__battleEndHandler, this);
        NotificationManager.Instance.removeEventListener(BattleNotic.SKILL_CD, this.__skillEnabeleHandler, this);
        NotificationManager.Instance.removeEventListener(NotificationEvent.USE_SKILL_RESULT, this._skillResultHandler, this);
        NotificationManager.Instance.removeEventListener(BattleNotic.RESTRICT_CHANGED, this.__restrictChangeHandler, this);
        ...
        NotificationManager.Instance.removeEventListener(BattleEvent.BATTLE_DYNAMIC_UI_LOADED, this.onBattleDynimicUILoaded, this);
    ...
    private __onClickCellItem(item: PetAwakeingBtn) {
        ...
        if (this._hero.canMorph()) {
            skill = TempleteManager.Instance.getSkillTemplateInfoById(SkillData.PET_MORPH_SKILL);
            NotificationManager.Instance.sendNotification(BattleNotic.ACTION_SKILL, skill);
            this._lastClick = curent;
        } else if (this._hero.canCancelMorph()) {
            skill = TempleteManager.Instance.getSkillTemplateInfoById(SkillData.PET_UNMORPH_SKILL);
            NotificationManager.Instance.sendNotification(BattleNotic.ACTION_SKILL, skill);
            this._lastClick = curent;
        }
    ...
    private _skillResultHandler(e) {
        ...
        if (!success) {
            ...
            if (skillId == SkillData.PET_MORPH_SKILL && this._hero.useSkill700001) {
                this._hero.useSkill700001 = false;
            } else if (skillId == SkillData.PET_UNMORPH_SKILL && this._hero.useSkill700002) {
                this._hero.useSkill700002 = false;
            }
        }
        this.update();
    ...
    private updateAwakenValue() {
        ...
        let item: PetAwakeingBtn = this.awakenItem;
        item.getChild("txtCurAwaken").text = this._currentAwaken + " / " + BattleModel.AWAKEN_FULL_VALUE;
    ...
    public updateStateTip() {
        ...
        } else if (this._hero.useSkill700001 && this._hero.awaken == BattleModel.AWAKEN_FULL_VALUE) {
    ...
    public enable() {
        ...
        if (!this._hero.useSkill700001 && //没用过700001
            !this._hero.useSkill700002 &&
            (this._hero.canMorph() || this._hero.canCancelMorph())) { //能够变身 或 解除
            //设为button模式
            showButtonMode = true;
        }
