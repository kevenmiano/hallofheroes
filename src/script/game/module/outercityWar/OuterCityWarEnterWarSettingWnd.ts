...
export default class OuterCityWarBattleSettingWnd extends BaseWindow {
  ...
  private initView() {
    ...
    this.txtCapaity.text = LangManager.Instance.GetTranslation(
      "public.playerInfo.compositeCapaity",
    );
    this.txtState.text = LangManager.Instance.GetTranslation(
      "public.playerInfo.battleState",
    );
  }
  ...
  private onRenderListItem(
    index: number,
    item: OuterCityWarEnterWarSettingItem,
  ) {
  ...
  private refreshPlayerInfoList() {
    this.playerInfoList = this.fightModel.getGuildPlayerInfoList(
      this.fightModel.selfGuildId,
    );
    // 排序优先级：（1）是否在线（2）综合战力
    SortTools.MoreKeysSorter(
      this.playerInfoList,
      ["onlineSortWeight", "totalCapaity"],
      [ArraySort.LOWER, ArraySort.LOWER],
    );
  }
