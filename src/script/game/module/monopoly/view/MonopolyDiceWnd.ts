...
export default class MonopolyDiceWnd extends BaseWindow 
{
    ...
    initLanguage(){
        this.btn_dice0.title = LangManager.Instance.GetTranslation('monopoly.dice0');
        this.btn_dice1.title = LangManager.Instance.GetTranslation('monopoly.dice1');
    }
     ...
     */
    private setBtnState(btn: UIButton, redPointState: boolean, count: number = 0) {
        let btnView = btn;
        if (btnView) {
    ...
    rollDice(type:number){
        if(type == 0){
            ...
            this.mc.getChild('img_result').asLoader.url = fgui.UIPackage.getItemURL("Monopoly", 'Icon_Dice_'+point);
            this.mc.visible = true;
            this.diceMask.visible = true;
            this.mc.getTransition('tranDise').play(Laya.Handler.create(this,this.showDicePoint),1);
        }else{
    ...
    private addEvent():void
    {
        ...
        NotificationManager.Instance.addEventListener(MonopolyEvent.ROLL_DICE,this.rollDice,this);
    }
    ...
    private removeEvent():void
    {
        ...
        NotificationManager.Instance.removeEventListener(MonopolyEvent.ROLL_DICE,this.rollDice,this);
    }
    ...
    private onDice0():void 
    {
        ...
        {
            str = LangManager.Instance.GetTranslation("MonopolyBottomView.normalBtnClickTip");
            MessageTipManager.Instance.show(str);
            return;
        }
    ...
    private helpBtnClick() {
        ...
        let content: string = LangManager.Instance.GetTranslation('Monopoly.helpContent');
