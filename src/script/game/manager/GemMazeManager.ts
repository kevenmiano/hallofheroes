...
export class GemMazeManager extends GameEventDispatcher {
  ...
  private initEvent() {
    ServerDataManager.listen(
      S2CProtocol.U_C_GEMMAZE_INFO,
      this,
      this.__gemMazeInfoBuffHandler,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_GEMMAZE_UPDATE,
      this,
      this.__gemInfoUpdateHandler,
    );
    ServerDataManager.listen(
      S2CProtocol.U_GEMMAZE_OPRST,
      this,
      this.__invideResultHandler,
    );
    ServerDataManager.listen(
      S2CProtocol.U_GEMMAZE_BUYTIMES_RSP,
      this,
      this.__onBuyTimesHandler,
    );
    ServerDataManager.listen(
      S2CProtocol.U_GEMMAZE_GETBOX_RSP,
      this,
      this.__onGetBoxGoodsHandler,
    );
  }
  ...
  private __gemInfoUpdateHandler(pkg: PackageIn) {
    let msg: GemInfoUpdateMsg = pkg.readBody(
      GemInfoUpdateMsg,
    ) as GemInfoUpdateMsg;
  ...
  private __onBuyTimesHandler(pkg: PackageIn) {
    let msg: GemMazeBuyTimesRspMsg = pkg.readBody(
      GemMazeBuyTimesRspMsg,
    ) as GemMazeBuyTimesRspMsg;
  ...
  private __onGetBoxGoodsHandler(pkg: PackageIn) {
    let msg: GetRewardBoxRspMsg = pkg.readBody(
      GetRewardBoxRspMsg,
    ) as GetRewardBoxRspMsg;
