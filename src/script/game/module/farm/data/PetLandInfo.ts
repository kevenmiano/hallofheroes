...
export default class PetLandInfo extends GameEventDispatcher {
  ...
  public canFeed(): boolean {
    ...
    var beginDate: Date = DateFormatter.parse(
      this.beginTime,
      "YYYY-MM-DD hh:mm:ss",
    );
    var endDate: Date = DateFormatter.parse(
      this.endTime,
      "YYYY-MM-DD hh:mm:ss",
    );
    ...
    var curRegion: number = Math.floor(
      (curDate.getTime() - beginDate.getTime()) / regionSize,
    );
  ...
  public get growMinutes(): number {
    ...
    var curDate: Date = PlayerManager.Instance.currentPlayerModel.sysCurtime;
    var min: number = Math.floor(
      (curDate.getTime() - beginDate.getTime()) / (1000 * 60),
    );
    if (min < 0) min = 0;
    return min;
  }
  ...
  public get totalMatureMinutes(): number {
    ...
    var min: number = Math.ceil(
      (endDate.getTime() - beginDate.getTime()) / (1000 * 60),
    );
    return min;
  }
  ...
  public get remainMatureTime(): number {
    let endDate: Date = DateFormatter.parse(
      this.endTime,
      "YYYY-MM-DD hh:mm:ss",
    );
