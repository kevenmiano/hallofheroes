    ...
    public set attackOver(value: boolean) {
        ...
        if (this._attackOver) {
            if (this._curSide != this._currentResistSide) {
                NotificationManager.Instance.sendNotification(BattleNotic.SET_RESIST_TOTAL_DAMAGE, this._resistTotal, this._currentResistSide );
                NotificationManager.Instance.sendNotification(BattleNotic.SET_RESIST_VISIBLE, true, this._currentResistSide);
            } else {
                GameBaseQueueManager.Instance.addAction(new ResistDelayAction(this._attackOver, this._resistTotal, this._currentResistSide, 15));
            }
            this._curSide = this._currentResistSide;
            NotificationManager.Instance.sendNotification(BattleNotic.SET_RESIST_VISIBLE, true, this._currentResistSide);
        } else {
            NotificationManager.Instance.sendNotification(BattleNotic.SET_RESIST_VISIBLE, false, this._currentResistSide);
            this._resistTotal = 0;
        }
    }
}