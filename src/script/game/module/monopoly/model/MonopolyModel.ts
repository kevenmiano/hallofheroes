...
export default class MonopolyModel extends GameEventDispatcher {
    ...
    constructor() {
        ...
        let cfg = TempleteManager.Instance.getConfigInfoByConfigName("Monopoly_Day");
        if (cfg) {
        ...
        //云端历险魔力骰子购买信息
        cfg = TempleteManager.Instance.getConfigInfoByConfigName("Monopoly_Dice_BuyInfo");
        if (cfg) {
            let str: string = cfg.ConfigValue;
            if (str) {
    ...
    public getPath(): Array<any> {
        ...
        if (startPosition < endPosition) {
            ...
            for (i = this.nodeInfos.length - 1; i >= 0; i--) {
                ...
                if (nodeInfo.position <= startPosition && nodeInfo.position >= endPosition) {
                    path.push(new Laya.Point(nodeInfo.x, nodeInfo.y));
                }
            }
        }
