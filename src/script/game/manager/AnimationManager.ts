...
export class AnimationManager {
     ...
     * @param formatLen 
     */
    createAnimation(preUrl: string, aniName: string, startIdx: number = 1, sepSymbol: string = "_", formatLen = 0): boolean {
     ...
     * @param formatLen 
     */
    createAnimationAvater(preUrl: string, pkgName: string = "", actionName: string, frameY: number = 0, startIdx: number = 0, formatLen = 0): boolean {
        ...
        if (arr.length > 0) {
            ...
            Logger.log("[AnimationManager]createAnimationAvater success:", arr, cachName)
            Laya.Animation.createFrames(arr, cachName);
            return true
        }
        return false
     ...
     * @param itemHeight 
     */
    createAnimationWithTexture(url: string, cacheName: string, frameX: number = 8, frameY: number = 1, itemWidth: number = 0, itemHeight: number = 0): boolean {
        let curFrameX = 0
        let curFrameY = 0
        ...
        if (texture) {
            for (let index = 0; index < size; index++) {
                ...
                g.fillTexture(texture, 0, 0, itemWidth, itemHeight, undefined, new Laya.Point(-offsetX, -offsetY))
                graphics.push(g)

                Logger.log("[AnimationManager] offsetX=" + offsetX + ", offsetY=" + offsetY + ", itemWidth=" + itemWidth + ", itemHeight=" + itemHeight + ", curFrameX=" + curFrameX)
