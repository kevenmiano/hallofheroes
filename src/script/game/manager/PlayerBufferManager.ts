...
export class PlayerBufferManager extends GameEventDispatcher {
  ...
  private initEvent() {
    ServerDataManager.listen(
      S2CProtocol.U_C_BUFFER_LIST,
      this,
      this.__refreshCampaignBufferHandler,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_ITEM_BUFFER,
      this,
      this.__refreshItemBufferHandler,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_RATE_INFO,
      this,
      this.__douleExpHandler,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_PLAYER_ADDITION,
      this,
      this.__updatePlayerBasePropertyHandler,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_CASTLE_TIME,
      this,
      this.__updateDefenceTimeHandler,
    );
  }
  ...
  private __updateDefenceTimeHandler(pkg: PackageIn) {
    ...
    let buffer: PlayerBufferInfo = this
      ._itemBufferList[2001] as PlayerBufferInfo;
    if (buffer) {
  ...
  private __updatePlayerBasePropertyHandler(pkg: PackageIn) {
    ...
    if (msg.isExist) {
      ...
      this.basePropertyBuffer.bufferType =
        PlayerBufferType.BASE_PROPERTY_BUFFER;
    } else {
  ...
  private __douleExpHandler(pkg: PackageIn) {
    let msg: RateUpdateReqMsg = pkg.readBody(
      RateUpdateReqMsg,
    ) as RateUpdateReqMsg;
    ...
    if (msg.rate <= 0) {
      ...
      if (buffer) {
        ...
        if (
          type === PlayerBufferType.LORDS_BUFFER &&
      ...
      let endDay: number = DateFormatter.parse(
        msg.endDay,
        "YYYY-MM-DD hh:mm:ss",
      ).getTime();
      let beginDay: number = DateFormatter.parse(
        msg.beginDay,
        "YYYY-MM-DD hh:mm:ss",
      ).getTime();
      ...
      if (
        msg.beginTime == 0 &&
        msg.endTime == Number(23 * 3600 + 59 * 60 + 59)
      ) {
  ...
  private __refreshCampaignBufferHandler(pkg: PackageIn) {
    ...
    let msg: CampaignBufferMsg = pkg.readBody(
      CampaignBufferMsg,
    ) as CampaignBufferMsg;
  ...
  private __refreshItemBufferHandler(pkg: PackageIn) {
    let msg: ItemBufferRspMsg = pkg.readBody(
      ItemBufferRspMsg,
    ) as ItemBufferRspMsg;
