...
export class MainShopInfo {
    ...
    public get canTotalCount(): number {
        ...
        if (this._type != 2) {
            ...
            var currDay: number = Math.floor(this.playerModel.sysCurTimeBySecond / (3600 * 24));
            if (day == currDay) {
    ...
    public setPrice(price: number) {
        if (price > 0) {
            ...
            this._price = 0;
            var shopInfo: ShopGoodsInfo = TempleteManager.Instance.getShopMainInfoByItemId(this.shopId);
            if (shopInfo) {
                if (shopInfo.Point > 0) {
                    ...
                    if (Number(shopInfo.GiftToken * this.discount) < 1)
                        ...
                        this._price += Number(shopInfo.GiftToken * this.discount) * this.counts;
                    //						_payType = 2;
                }

            }

            shopInfo = TempleteManager.Instance.getShopMainTempInfoByItemId(this.templateId1);
            if (shopInfo) {
                if (shopInfo.Point > 0) {
                    if (Number(shopInfo.Point * this.discount) < 1)
                    ...
                    if (Number(shopInfo.GiftToken * this.discount) < 1)
                        ...
                        this._price += Number(shopInfo.GiftToken * this.discount) * this.count1;
                }
            }

            shopInfo = TempleteManager.Instance.getShopMainTempInfoByItemId(this.templateId2);
            if (shopInfo) {
                if (shopInfo.Point > 0) {
                    if (Number(shopInfo.Point * this.discount) < 1)
                    ...
                    if (Number(shopInfo.GiftToken * this.discount) < 1)
                        ...
                        this._price += Number(shopInfo.GiftToken * this.discount) * this.count2;
                }
            }

            shopInfo = TempleteManager.Instance.getShopMainTempInfoByItemId(this.templateId3);
            if (shopInfo) {
                if (shopInfo.Point > 0) {
                    if (Number(shopInfo.Point * this.discount) < 1)
                    ...
                    if (Number(shopInfo.GiftToken * this.discount) < 1)
                        ...
                        this._price += Number(shopInfo.GiftToken * this.discount) * this.count3;
                }
            }

            shopInfo = TempleteManager.Instance.getShopMainTempInfoByItemId(this.templateId4);
            if (shopInfo) {
                if (shopInfo.Point > 0) {
                    if (Number(shopInfo.Point * this.discount) < 1)
                    ...
                    if (Number(shopInfo.GiftToken * this.discount) < 1)
                        ...
                        this._price += Number(shopInfo.GiftToken * this.discount) * this.count4;
                }
            }

            shopInfo = TempleteManager.Instance.getShopMainTempInfoByItemId(this.templateId5);
            if (shopInfo) {
                if (shopInfo.Point > 0) {
                    if (Number(shopInfo.Point * this.discount) < 1)
                    ...
                    if (Number(shopInfo.GiftToken * this.discount) < 1)
                        ...
                        this._price += Number(shopInfo.GiftToken * this.discount) * this.count5;
                }
            }

            shopInfo = TempleteManager.Instance.getShopMainTempInfoByItemId(this.templateId6);
            if (shopInfo) {
                if (shopInfo.Point > 0) {
                    if (Number(shopInfo.Point * this.discount) < 1)
                    ...
                    if (Number(shopInfo.GiftToken * this.discount) < 1)
                        ...
                        this._price += Number(shopInfo.GiftToken * this.discount) * this.count6;
                }
            }

            shopInfo = TempleteManager.Instance.getShopMainTempInfoByItemId(this.templateId7);
            if (shopInfo) {
                if (shopInfo.Point > 0) {
                    if (Number(shopInfo.Point * this.discount) < 1)
                    ...
                    if (Number(shopInfo.GiftToken * this.discount) < 1)
                        ...
                        this._price += Number(shopInfo.GiftToken * this.discount) * this.count7;
                }
            }

            shopInfo = TempleteManager.Instance.getShopMainTempInfoByItemId(this.templateId8);
            if (shopInfo) {
                if (shopInfo.Point > 0) {
                    if (Number(shopInfo.Point * this.discount) < 1)
                    ...
                    if (Number(shopInfo.GiftToken * this.discount) < 1)
                        ...
                        this._price += Number(shopInfo.GiftToken * this.discount) * this.count8;
                }
            }
        }
    }
    ...
    public setContent() {
        this._content = "";
        let goodsTempInfo: t_s_itemtemplateData = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_itemtemplate, this.templateId);

        if (goodsTempInfo) {
            this._content += goodsTempInfo.TemplateNameLang;
            if (this.counts > 0) this._content += "x" + this.counts;
        }
        goodsTempInfo = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_itemtemplate, this.templateId2);
        if (goodsTempInfo) {
            this._content += ", " + goodsTempInfo.TemplateNameLang;
        ...
        goodsTempInfo = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_itemtemplate, this.templateId3);
        if (goodsTempInfo) {
            this._content += ", " + goodsTempInfo.TemplateNameLang;
            if (this.count3 > 0) this._content += "x" + this.count3;
        }

        goodsTempInfo = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_itemtemplate, this.templateId4);
        if (goodsTempInfo) {
            this._content += ", " + goodsTempInfo.TemplateNameLang;
            if (this.count4 > 0) this._content += "x" + this.count4;
        }

        goodsTempInfo = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_itemtemplate, this.templateId5);
        if (goodsTempInfo) {
            this._content += ", " + goodsTempInfo.TemplateNameLang;
            if (this.count5 > 0) this._content += "x" + this.count5;
        }

        goodsTempInfo = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_itemtemplate, this.templateId6);
        if (goodsTempInfo) {
            this._content += ", " + goodsTempInfo.TemplateNameLang;
            if (this.count6 > 0) this._content += "x" + this.count6;
        }

        goodsTempInfo = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_itemtemplate, this.templateId7);
        if (goodsTempInfo) {
            this._content += ", " + goodsTempInfo.TemplateNameLang;
            if (this.count7 > 0) this._content += "x" + this.count7;
        }

        goodsTempInfo = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_itemtemplate, this.templateId8);
        if (goodsTempInfo) {
            this._content += ", " + goodsTempInfo.TemplateNameLang;
