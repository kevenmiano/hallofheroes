...
export class ResistUIView extends Laya.Sprite {
  ...
  private initView() {
    ...
    this._staticText = DamageNumView.createText(
      LangManager.Instance.GetTranslation("DamageNum.Resist"),
      sstyle.size,
      sstyle.color,
      sstyle.bold,
      sstyle.italic,
  ...
  public show(visible: boolean = true) {
    ...
    Laya.Tween.from(
      this._damgeSprite,
      { alpha: 0, x: this._damageDestX - 100 * (this._side == 1 ? 1 : -1) },
      700,
  ...
  public hide() {
    Laya.Tween.to(
      this._damgeSprite,
      { alpha: 0, x: this._damageDestX - 96 * (this._side == 1 ? 1 : -1) },
      400,
      Laya.Ease.linearNone,
      Laya.Handler.create(this, this.onExitComplete),
    );
  }
