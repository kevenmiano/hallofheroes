...
export class BindMailView extends FUI_BindMailView {
  ...
  private initView() {
    ...
    if (
      state === 1 ||
      (state === 2 && WelfareManager.Instance.bindMailSelectPrivacy) ||
      state === 3 ||
      state === 4
    ) {
      let configTemp = TempleteManager.Instance.getConfigInfoByConfigName(
        "email_privacy_rewards",
      );
      if (!configTemp && configTemp.ConfigValue != "") return;
      let configValue = configTemp.ConfigValue;
      let configItems = configValue.split("|");
    ...
    if (state === 1 || state === 4)
      this.getController("privacy").selectedIndex = 0;
    if (
      (state === 2 && !WelfareManager.Instance.bindMailSelectPrivacy) ||
      state === 3
    )
      this.getController("privacy").selectedIndex = 1;
    ...
    switch (state) {
      ...
      case 3: // 已领取(不包含额外奖励)
        if (WelfareManager.Instance.bindMailSelectPrivacy) {
          ...
          this.btnBind.text = LangManager.Instance.GetTranslation(
            "friends.view.ToolInviteFriendItem.accept",
          );
        }
        break;
      case 4: // 已领取(包含额外奖励)
