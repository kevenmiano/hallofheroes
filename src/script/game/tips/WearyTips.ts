...
export default class WearyTips extends BaseTips {
    ...
    private addEvent() {
        GameManager.Instance.addEventListener(PlayerEvent.SYSTIME_UPGRADE_MINUTE, this.onSysTimeUpdateHandler, this);
    }

    private removeEvent() {
        GameManager.Instance.removeEventListener(PlayerEvent.SYSTIME_UPGRADE_MINUTE, this.onSysTimeUpdateHandler, this);
    ...
    private updateTimeText(): void {
        const timeStr: string = DateUtils.getFormatBySecond(this._nextRecoveryTimeSec, 3);
        this.timeTxt.text = LangManager.Instance.GetTranslation("WearyTips.RecoveryTime", timeStr, this._wearyRestore);
    }
