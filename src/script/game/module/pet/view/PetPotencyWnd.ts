...
export default class PetPotencyWnd extends BaseWindow {
    ...
    private __onTxtChangePower() {
        ...
        // 剩余可分配点
        let remCnt = this._data.remainPoint - (this._allocation[0] + this._allocation[1] + this._allocation[2] + this._allocation[3])
        if (remCnt <= 0) {
    ...
    private refresh() {
        ...
        this.tfIntelligence.text = (this._data.intellect + this._allocation[1]).toString();
        this.tfArmor.text = (this._data.armor + this._allocation[3]).toString();

        this.txtPowerAdd.text = this._allocation[0] > 0 ? this._allocation[0].toString() : ""
        this.txtStaminaAdd.text = this._allocation[2] > 0 ? this._allocation[2].toString() : ""
        this.txtIntelligenceAdd.text = this._allocation[1] > 0 ? this._allocation[1].toString() : ""
        this.txtArmorAdd.text = this._allocation[3] > 0 ? this._allocation[3].toString() : ""
        ...
        this.btnWash.enabled = (this._data.remainPoint != (this._data.grade - 1) * PetData.EACH_LEVEL_POINT);
    ...
    private btnConfirmClick() {
        ...
        if (flag) {
            ...
            this._allocation = [0, 0, 0, 0];
            this._remainPoint = 0;
            MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("GrowthInfoView.addPointSucc"));
        }
    }

    private btnWashClick() {
        if (!this._data) return;
        if (this._data.remainPoint == (this._data.grade - 1) * PetData.EACH_LEVEL_POINT) {
            MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("GrowthInfoView.noNeedWash"));
            return;
        }

        let content: string = LangManager.Instance.GetTranslation("pet.PetFrame.washPointTip", this.washPrice);
        ...
        SimpleAlertHelper.Instance.Show(SimpleAlertHelper.USEBINDPOINT_ALERT, { checkRickText: checkStr, checkRickText2: checkStr2, checkDefault: true }, null, content, null, null, this.__btnWashClick.bind(this));
    }
    ...
    private get washPrice(): number {
        if (this._cost == 0) {
            this._cost = 100;
            let temp: ConfigInfosTempInfo = TempleteManager.Instance.getConfigInfoByConfigName("pet_reset_att");
            if (temp) {
    ...
    private __btnWashClick(result: boolean, flag: boolean, id: number = 0, type: number = 0) {
        if (!this._data) return;
        if (!result) return;
    ...
    private checkUseBind() {
        let content: string = LangManager.Instance.GetTranslation("PetAdvancedView.useBindPointTip");
        let checkStr = LangManager.Instance.GetTranslation("mainBar.view.VipCoolDownFrame.useBind");
        let checkStr2 = LangManager.Instance.GetTranslation("mainBar.view.VipCoolDownFrame.promptTxt");
        SimpleAlertHelper.Instance.Show(SimpleAlertHelper.USEBINDPOINT_ALERT, { checkRickText: checkStr, checkRickText2: checkStr2, checkDefault: true }, null, content, null, null, this.__checkUseBind.bind(this));
    }
    ...
    private doWash(useBind: boolean) {
        ...
        if (player.allPoint < this.washPrice || (!useBind && player.point < this.washPrice)) {
