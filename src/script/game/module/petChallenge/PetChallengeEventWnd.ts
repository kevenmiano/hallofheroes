...
export default class PetChallengeEventWnd extends BaseWindow {
    ...
    public OnInitWind() {
        ...
        Logger.xjy("[PetChallengeEventWnd]OnShowWind", this.model.challengeEventList);
    ...
    private addEvent() {
        ...
        NotificationManager.Instance.addEventListener(PetChallengeEvent.CHALLENGE_EVENT_CHANGE, this.__refreshChallengeInfo, this);
    }

    private delEvent() {
        // this.itemList.itemRenderer.recover();
        Utils.clearGListHandle(this.itemList);
        NotificationManager.Instance.removeEventListener(PetChallengeEvent.CHALLENGE_EVENT_CHANGE, this.__refreshChallengeInfo, this);
    }
    ...
    private renderListItem(index: number, item: fgui.GComponent) {
        ...
        let tarNickName = LangManager.Instance.GetTranslation("public.bracket", data.tarNickName);
        ...
        if (data.isAttack) {
            win = data.result == 1;
            if (win) {
                lgkey = score > 0 ? "colosseum.challenge.attacked.txt1" : "colosseum.challenge.attacked.txt11";

            } else {
                lgkey = score < 0 ? "colosseum.challenge.attacked.txt2" : "colosseum.challenge.attacked.txt22";
            }

        } else {
            win = data.result == 2;
            if (win) {
                lgkey = score > 0 ? "colosseum.challenge.beAttacked.txt1" : "colosseum.challenge.beAttacked.txt11";
            } else {
                lgkey = score < 0 ? "colosseum.challenge.beAttacked.txt2" : "colosseum.challenge.beAttacked.txt22";

            }

        }

        str += LangManager.Instance.GetTranslation(lgkey, tarNickName, Math.abs(score));

        item.text = str;
    }

    public get model() {
        return FrameCtrlManager.Instance.getCtrl(EmWindow.PetChallenge).data as PetChallengeData
    }
