...
export class SimpleMapLayer extends MapBaseLayer {
  ...
  public set mapData(data: MapInfo) {
    ...
    this._model.addEventListener(
      OuterCityEvent.COMMIT_LOADING,
      this.__commitLoadHandler,
      this,
    );
  }
  ...
  public setAllPhysicsFilter() {
    ...
    this._is = true;
    let timeline1: TimelineMax = new TimelineMax({
      repeat: -1,
      yoyo: true,
      repeatDelay: 0.5,
    });
    let timeline2: TimelineMax = new TimelineMax({
      repeat: -1,
      yoyo: true,
      repeatDelay: 0.5,
    });
    let timeline3: TimelineMax = new TimelineMax({
      repeat: -1,
      yoyo: true,
      repeatDelay: 0.5,
    });
    let timeline4: TimelineMax = new TimelineMax({
      repeat: -1,
      yoyo: true,
      repeatDelay: 0.5,
    });
