...
export default class PetCampaignWnd extends BaseWindow {
  ...
  public OnInitWind() {
    ...
    let cfg =
      TempleteManager.Instance.getConfigInfoByConfigName("pet_battle_count");
    if (cfg) {
    ...
    if (PetCampaignManager.ISLOGIN) {
      ...
      this.leftRewardCount = PetCampaignManager.Instance.model.leftRewardCount =
        this.pet_battle_count -
        PetCampaignManager.Instance.model.userUiPlayListMsg.uiPlayRewardCount;
    } else {
      this.leftRewardCount = PetCampaignManager.Instance.model.leftRewardCount;
    }
  ...
  private addEvent() {
    ...
    this.tree.itemRenderer = Laya.Handler.create(
      this,
    ...
    PetCampaignManager.Instance.model.addEventListener(
      PetCampaignEvent.PET_CAMPAIGN_JUMP_LEVEL,
      this.onJumpLevel,
      this,
    );
    PetCampaignManager.Instance.addEventListener(
    ...
    PetCampaignManager.Instance.addEventListener(
      PetCampaignEvent.PET_CAMPAIGN_UPDATE,
  ...
  private offEvent() {
    ...
    PetCampaignManager.Instance.model.removeEventListener(
      PetCampaignEvent.PET_CAMPAIGN_JUMP_LEVEL,
      this.onJumpLevel,
      this,
    );
    PetCampaignManager.Instance.removeEventListener(
      PetCampaignEvent.PET_CAMPAIGN_TREE_SELECT,
      this.refreshLeft,
      this,
    );
  ...
  private initView() {
    ...
    if (this.frameData) {
      ...
      let index = this.petCampaignModel.playIdArrs.indexOf(
        this.frameData.playId,
      );
  ...
  private initTabList() {
    ...
    this.tabListData.forEach((value) => {
      let btn: fgui.GButton = <fgui.GButton>(
        this.TabList.addItemFromPool().asButton
      );
      btn.selected = false;
      btn.text = value;
  ...
  private onBuy() {
    ...
    let str = LangManager.Instance.GetTranslation(
      "PetCampaignLeftRewardTimes",
      this.leftRewardCount,
    );
    let content: string =
      str + "<br>" + LangManager.Instance.GetTranslation("petCampaign.useProp");
    this.propNum = GoodsManager.Instance.getGoodsNumByTempId(
      TemplateIDConstant.TEMP_ID_CRUSADE,
    );
    let goodsCount: string =
      LangManager.Instance.GetTranslation("MazeShopWnd.HasNumTxt") +
      this.propNum;
    UIManager.Instance.ShowWind(EmWindow.UseGoodsAlert, {
      content: content,
      goodsId: TemplateIDConstant.TEMP_ID_CRUSADE,
      goodsCount: goodsCount,
      hidecheck1: true,
      autoClose: this.propNum <= 1,
    ...
    if (b) {
      if (this.propNum <= 0) {
        MessageTipManager.Instance.show(
          LangManager.Instance.GetTranslation("petCampaign.useProp2"),
        );
      } else {
        let goods: GoodsInfo[] = GoodsManager.Instance.getBagGoodsByTemplateId(
          TemplateIDConstant.TEMP_ID_CRUSADE,
        );
        if (goods && goods[0]) {
  ...
  private initText() {
    this.txtTip.text = LangManager.Instance.GetTranslation(
      "selectcampaign.view.mayFall",
    );
    this.txtReward.text = LangManager.Instance.GetTranslation(
      "yishi.view.tips.SinglePassRewardTip.firstKill",
    );
  }
  ...
  private refreshView() {
    ...
    if (this.petCampaignModel.levelSort > 0) {
      let maxLevel =
        this.petCampaignModel.levelSort >= this.tree.numItems
          ? this.tree.numItems
  ...
  private refreshLeft() {
    this._selectedLevelData =
      PetCampaignManager.Instance.model.getCurLevelData();
    if (!this._selectedLevelData) return;
    this.txtRequireTip.text = LangManager.Instance.GetTranslation(
      "petCampaign.txtRequireTip",
      this._selectedLevelData.Grade,
    );
    //海外翻译新增
    this.txtLevelTitle.text = LangManager.Instance.GetTranslation(
      "petCampaign.Level",
      this._selectedLevelData.UiLevelSort,
    );
    this.DescriptionTip.text = LangManager.Instance.GetTranslation(
      "petCampaign.DescriptionTip",
      this._selectedLevelData.DescriptionLang,
    );
    ...
    if (this._selectedLevelData.IsContinuity == 0) {
      //不能扫荡
      this.gSweep.visible = false;
    } else {
      this.gSweep.visible = true;
    }
    if (this._selectedLevelData.ConsumeType != 0) {
      //不是消耗体力
    ...
    this.btnSweep.enabled =
      this.petCampaignModel.levelSort >= this._selectedLevelData.UiLevelSort
        ? true
        : false;
    ...
    if (this._selectedLevelData.Grade > ArmyManager.Instance.thane.grades) {
      this.txt_grade.text =
        ...
        LangManager.Instance.GetTranslation(
          "battle.logsys.BattleLogStarView.open",
        );
    } else {
      this.txt_grade.text = "";
    }
  ...
  private updateRewardStatus(msg: UserUiPlayInfoMsg) {
    ...
    if (msg && msg.itemAdd) {
      ...
      isItemAdd = true;
      this.leftRewardCount = PetCampaignManager.Instance.model.leftRewardCount =
        this.pet_battle_count - msg.uiPlayRewardCount;
    }
    let item: UserUiPlayInfoMsg;
    this._maxLevelSort = 0;
    let len: number =
      PetCampaignManager.Instance.model.userUiPlayListMsg.uiPlayInfoList.length;
    for (let i: number = 0; i < len; i++) {
      item = PetCampaignManager.Instance.model.userUiPlayListMsg.uiPlayInfoList[
        i
      ] as UserUiPlayInfoMsg;
      if (item && item.playId == this.petCampaignModel.curPlayID) {
        ...
        if (item.uiPlayRewardCount && !isItemAdd && msg) {
          this.leftRewardCount =
            PetCampaignManager.Instance.model.leftRewardCount =
              this.pet_battle_count - item.uiPlayRewardCount;
        }
        break;
      }
    }
    ...
    if (this._selectedLevelData.FristReward == 0) {
      //没有配置首通奖励
      ...
      if (this._selectedLevelData.UiLevelSort <= this._maxLevelSort) {
        ...
        if (
          item &&
          item.firstRewardSet.indexOf(this._selectedLevelData.UiLevelSort) != -1
        ) {
          //已经领取
    ...
    if (msg && msg.itemAdd) {
      ...
      let content: string =
        str +
        "<br>" +
        LangManager.Instance.GetTranslation("petCampaign.useProp");
      this.propNum = GoodsManager.Instance.getGoodsNumByTempId(
        TemplateIDConstant.TEMP_ID_CRUSADE,
      );
      let goodsCount: string =
  ...
  private updateRewardCount() {
    this.txt_count.text = LangManager.Instance.GetTranslation(
      "PetCampaignLeftRewardTimes",
      this.leftRewardCount,
    );
  }
  ...
  private onBtnChallenge() {
    ...
    if (this._selectedLevelData.Grade > ArmyManager.Instance.thane.grades) {
      MessageTipManager.Instance.show(
        LangManager.Instance.GetTranslation(
          "room.view.pve.instance.InstanceRightView.command04",
          this._selectedLevelData.Grade,
        ),
      );
      return;
    ...
    if (this._selectedLevelData.ConsumeType == 0) {
      ...
      let hasGoodsCount: number = GoodsManager.Instance.getGoodsNumByTempId(
        this._selectedLevelData.ConsumeType,
      );
      if (hasGoodsCount < this._selectedLevelData.ConsumeNum) {
        MessageTipManager.Instance.show(
          LangManager.Instance.GetTranslation("petCampaign.challengeTips2"),
        );
        return;
      }
    }
    ...
    if (!flag) {
      //没有携带英灵
      MessageTipManager.Instance.show(
        LangManager.Instance.GetTranslation("PetCampaignResultWnd.nextBtnTips"),
      );
      return;
    }
    PetCampaignManager.Instance.sendUIPlayChallenge(
      PetCampaignModel.ENTER_BATTLE,
      this.petCampaignModel.curPlayID,
      this.petCampaignModel.curLevelData.UiLevelId,
    );
  }
  ...
  private btnSweepHandler() {
    ...
    FrameCtrlManager.Instance.open(EmWindow.Mopup, {
      type: eMopupType.PetCampaignMopup,
      state: eMopupState.PetCampaignMopupPre,
      levelData: this._selectedLevelData,
    });
  }
  ...
  private lookRewardHandler() {
    FrameCtrlManager.Instance.open(EmWindow.PetGetRewardWnd, {
      selectedData: this._selectedLevelData,
      flag: this._isCanGet,
    });
  }
  ...
  private onTreeClick(item: PetCampaignLevelItem) {
    if (this.petCampaignModel.levelSort < item.index) {
      MessageTipManager.Instance.show(
        LangManager.Instance.GetTranslation("petCampaign.onTreeClick"),
      );
      return;
    }
  ...
  private getTxtSweepCostColor(): string {
    let str: string = ColorConstant.LIGHT_TEXT_COLOR;
    if (
      this._selectedLevelData.ContinuityGold >
  ...
  private checkInTeam(): boolean {
    ...
    if (inTeam) {
      MessageTipManager.Instance.show(
        LangManager.Instance.GetTranslation(
          "petCampaign.data.CanNotBegainPetCampaignInTeam",
        ),
      );
    }
    return inTeam;
