...
export class SkillItemListView {
  ...
  public initView(skills: any[]) {
    ...
    for (let i = 0; i < 10; i++) {
      ...
      if (i < SkillItemListView.TalentIndex) {
        //前6个为普通技能
    ...
    this._skillItemViews.forEach((item: BattleSkillItemII) => {
      if (item.data instanceof BattlePropItem) {
        ...
        if (
          ...
          skillId == (<t_s_skilltemplateData>item.data).Parameter3
        ) {
  ...
  public setEnableForce(value: boolean) {
    ...
    this._skillItemViews.forEach((element: BattleSkillItemII) => {
      if (
        element.data instanceof TrailPropInfo &&
        (<TrailPropInfo>element.data).skillTemp.IsUseOnDead == 1 &&
        (<TrailPropInfo>element.data).useCount > 0
      ) {
        //如果该技能为复活技能, 则自己死亡时可用
  ...
  public setEnable(value: boolean) {
    if (BattleManager.Instance.battleModel.isControledByBoss()) {
      //泰拉神庙副本里面boss控制了玩家, 持续时间内技能不可用
      this.setBeControled();
      return;
    }
    let isWait: boolean =
      BattleManager.Instance.battleModel.selfHero.isWaitingChangeBody;
  ...
  public refreshForSpChanged() {
    ...
    for (let index = 0; index < this._heroSkills.length; index++) {
      ...
      skillTInfo = ConfigMgr.Instance.getTemplateByID(
  ...
  public refreshForAwakenValueChanged() {
    ...
    for (let index = 0; index < PetData.CHANGE_SKILL_NUM; index++) {
      ...
      if (this._enable) {
        if (
          !self.isPetState ||
          this.battleModel.selfHero.awaken + skillTInfo.Cost < 0
        ) {
          item.enabled = false;
          item.setState(BattleSkillItemII.STATE_GRAY);
        } else {
