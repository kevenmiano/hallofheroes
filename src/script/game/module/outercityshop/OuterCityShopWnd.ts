...
export default class OuterCityShopWnd extends BaseWindow {
    ...
    private initView() {
        this.frame.getChild('title').text = LangManager.Instance.GetTranslation("auction.view.mysteryshop.MysteryShopFrame.titleText");
        this.txt_luckyTitle.text = LangManager.Instance.GetTranslation("auction.view.mysteryshop.MysteryShopFrame.txt_luckyTitle");
        this.updateBuyCount();
        // 去掉限购
    ...
    private updateBuyCount() {
        var config: ConfigInfosTempInfo = TempleteManager.Instance.getConfigInfoByConfigName("ShopBuy_MaxCount");
    ...
    helpBtnClick() {
        ...
        let content = LangManager.Instance.GetTranslation("auction.view.mysteryshop.helpContent");
        UIManager.Instance.ShowWind(EmWindow.Help, { title: title, content: content });
    }

    private addEvent() {
        this.outerCityShopModel.addEventListener(OuterCityShopEvent.FRESH_VIEW, this.onRefresh, this);
        this.outerCityShopModel.addEventListener(OuterCityShopEvent.UPDATE_LUCK_SHOW_INFO, this.updateLuckShowInfoHandler, this);
        this.listMsgs.itemRenderer = Laya.Handler.create(this, this.renderListItem, null, false);
    }

    private offEvent() {
        this.outerCityShopModel.removeEventListener(OuterCityShopEvent.FRESH_VIEW, this.onRefresh, this);
        this.outerCityShopModel.removeEventListener(OuterCityShopEvent.UPDATE_LUCK_SHOW_INFO, this.updateLuckShowInfoHandler, this);
    ...
    private updateLuckShowInfoHandler() {
        if (this.outerCityShopModel.shopLuckShowInfo) {
            this.listData = [];
            for (var i: number = 0; i < this.outerCityShopModel.shopLuckShowInfo.length; i++) {
