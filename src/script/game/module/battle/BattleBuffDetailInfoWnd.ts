...
import { InheritRoleType, BattleRoleBufferType, FaceType } from "../../constant/BattleDefine";
import { BattleEvent, NotificationEvent } from "../../constant/event/NotificationEvent";
...
export default class BattleBuffDetailInfoWnd extends BaseWindow {
    ...
    public get openLeft(): boolean { return this._openLeft }
    private _openRight: boolean = false;
    public get openRight(): boolean { return this._openRight }
    ...
    OnShowWind(): void {
        ...
        this.txtDescBuffL.text = LangManager.Instance.GetTranslation("Battle.roleBuffInfo.buffDesc")
        this.txtDescBuffR.text = LangManager.Instance.GetTranslation("Battle.roleBuffInfo.buffDesc")
        this.txtDescDebuffL.text = LangManager.Instance.GetTranslation("Battle.roleBuffInfo.deBuffDesc")
        this.txtDescDebuffR.text = LangManager.Instance.GetTranslation("Battle.roleBuffInfo.deBuffDesc")
    }
    ...
    private addEvent() {
        this.listRoleLeft.on(fgui.Events.CLICK_ITEM, this, this.__listRoleLeftClickItem);
        this.listRoleRight.on(fgui.Events.CLICK_ITEM, this, this.__listRoleRightClickItem);

        this.listRoleLeft.itemRenderer = Laya.Handler.create(this, this.__renderRoleLeftList, null, false);
        this.listRoleRight.itemRenderer = Laya.Handler.create(this, this.__renderRoleRightList, null, false);

        this.listDetailLeft.itemRenderer = Laya.Handler.create(this, this.__renderDetailLeftList, null, false);
        this.listDetailRight.itemRenderer = Laya.Handler.create(this, this.__renderDetailRightList, null, false);

        NotificationManager.Instance.addEventListener(BattleEvent.ROLE_BUFF_CHANGE, this.__refreshCurBuffList, this);
        NotificationManager.Instance.addEventListener(BattleEvent.REINFORCE, this.__reinforce, this);
        NotificationManager.Instance.addEventListener(BattleEvent.OPEN_BUFFINFO_PANEL, this.__openBuffPanel, this);
        if(CampaignManager.Instance.mapModel && WorldBossHelper.checkConsortiaBoss(CampaignManager.Instance.mapModel.mapId))
        {
            NotificationManager.Instance.addEventListener(NotificationEvent.CONSORTIA_BOSS_BUFFERIDS, this.__refreshCurBuffList, this);
        }
        this.imgLoad.onClick(this, this.modelClick)
    }

    private delEvent() {
        NotificationManager.Instance.removeEventListener(BattleEvent.ROLE_BUFF_CHANGE, this.__refreshCurBuffList, this);
        NotificationManager.Instance.removeEventListener(BattleEvent.REINFORCE, this.__reinforce, this);
        NotificationManager.Instance.removeEventListener(BattleEvent.OPEN_BUFFINFO_PANEL, this.__openBuffPanel, this);
        NotificationManager.Instance.removeEventListener(NotificationEvent.CONSORTIA_BOSS_BUFFERIDS, this.__refreshCurBuffList, this);
    }
    ...
    private __refreshCurBuffList(livingId: number) {
        if (this.openLeft && this.curSelLeftRoleInfo && livingId == this.curSelLeftRoleInfo.livingId) {
            this.refreshLeftDetailList()
            return
        }

        if (this.openRight && this.curSelRightRoleInfo && livingId == this.curSelRightRoleInfo.livingId) {
    ...
    private __renderDetailLeftList(index: number, item: fgui.GButton) {
        ...
        let itemData: BufferDamageData = this.curSelLeftRoleInfo.getBuffersWithFiller()[index]
        if (!itemData) {
            item.title = "";
            item.icon = "";
            return
    ...
    private __renderDetailRightList(index: number, item: fgui.GButton) {
        ...
        if(this.curSelRightRoleInfo.templateId == ConsortiaModel.CONSORTIA_BOSS_TEMPLATEID){
            itemData = this.curSelRightRoleInfo.getBuffersWithFiller(false,true)[index];
        }
        else{
    ...
    public refreshTurn(itemData: BufferDamageData, label: fgui.GObject) {
        if (!itemData) return;
        if (itemData.layerCount > 1 && itemData.templateInfo && itemData.templateInfo.PressCount > 0)
    ...
    private refreshView() {
        ...
        roleList.forEach((role: BaseRoleInfo) => {
            if (role.inheritType != InheritRoleType.Pet) {
                if (role.side == 1) {
                    role.face == FaceType.LEFT_TEAM ? this.arrLeftInfo.push(role) : this.arrRightInfo.push(role);
                }
                else {
                    role.face == FaceType.RIGHT_TEAM ? this.arrRightInfo.push(role) : this.arrLeftInfo.push(role);
                }
            }
        });
    ...
    private refreshLeftDetailList() {
        if (this.curSelLeftRoleInfo) {
            this.listDetailLeft.numItems = this.curSelLeftRoleInfo.getBuffersWithFiller().length
        }
    }

    private refreshRightDetailList() {
        if (this.curSelRightRoleInfo) {
            if(this.curSelRightRoleInfo.templateId == ConsortiaModel.CONSORTIA_BOSS_TEMPLATEID){//熔岩君主
                this.listDetailRight.numItems = this.curSelRightRoleInfo.getBuffersWithFiller(false,true).length
            }
            else{
                this.listDetailRight.numItems = this.curSelRightRoleInfo.getBuffersWithFiller(false,false).length
            }
        }
    }
}
