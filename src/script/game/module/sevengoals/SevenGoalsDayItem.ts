...
export default class SevenGoalsDayItem extends FUI_SevenGoalsDayItem {
    ...
    private refreshView() {
        if (this._info) {
            this.titleTxt.text = LangManager.Instance.GetTranslation("guidetask.GuideTaskFrame.dayBtnTxt0" + this._info.day);
            ...
            if (this._info.day > this.sevenGoalsModel.sevenCurrentDay) {
                ...
                this.redStatus.selectedIndex = this.sevenGoalsModel.checkDayRed(this._info.day) ? 1 : 0;
            }
        }
    }
