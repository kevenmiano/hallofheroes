...
function createBaseQuestInfo(): QuestInfo {
  return {
    templateId: 0,
    isComplete: false,
    condition1: 0,
...
export const QuestInfo: MessageFns<QuestInfo> = {
  encode(message: QuestInfo, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    ...
    if (message.addDate !== "") {
      writer.uint32(90).string(message.addDate);
    }
    return writer;
  },

  decode(input: BinaryReader | Uint8Array, length?: number): QuestInfo {
    const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
    let end = length === undefined ? reader.len : reader.pos + length;
    const message = createBaseQuestInfo();
  ...
  fromJSON(object: any): QuestInfo {
    return {
      ...
      addDate: isSet(object.addDate) ? globalThis.String(object.addDate) : "",
    };
  },

  toJSON(message: QuestInfo): unknown {
    const obj: any = {};
    if (message.templateId !== 0) {
    ...
    if (message.addDate !== "") {
      obj.addDate = message.addDate;
    }
    return obj;
  },

  create<I extends Exact<DeepPartial<QuestInfo>, I>>(base?: I): QuestInfo {
    return QuestInfo.fromPartial(base ?? ({} as any));
  },
  fromPartial<I extends Exact<DeepPartial<QuestInfo>, I>>(object: I): QuestInfo {
    const message = createBaseQuestInfo();
    ...
    message.addDate = object.addDate ?? "";
    return message;
  },
};
