...
export default class SkillWnd extends BaseWindow {
  ...
  public OnInitWind() {
    ...
    this["tabMenu_1"].getChild("redDot").icon = FUIHelper.getItemURL(
      "Base",
      "Lab_Arrow_Better",
    );
    this["tabMenu_4"].getChild("redDot").icon = FUIHelper.getItemURL(
      "Base",
      "Lab_Arrow_Better",
    );
  ...
  private checkTalentReddot() {
    ...
    this.setBtnState(
      this["tabMenu_4"],
      talentPoint > 0 || this.thane.talentData.checkUpgrade(),
    );
  }
  ...
  private checkSkillReddot() {
    ...
    if (ArmyManager.Instance.thane.grades > OpenGrades.MASTERY) {
      ...
      this.skillPanel.getChild("masteryTab").asCom.getChild("redDot").visible =
        extrajobReddot;
    }
   ...
   */
  enableOperate(boo) {
    this["tabMenu_1"].touchable =
      this["tabMenu_2"].touchable =
      this["tabMenu_4"].touchable =
        boo;
    ...
    switch (this.c1.selectedIndex) {
      case SkillWnd.page_JN: //技能
        title = LangManager.Instance.GetTranslation(
          "armyII.SkillFrame.HelpBtn.Tipdata",
        );
        if (this.skillPanel.isJobSkill) {
          content = LangManager.Instance.GetTranslation(
            "armyII.SkillHelpFrame.helpContent",
          );
        } else {
          title = LangManager.Instance.GetTranslation(
            "Skill.masterySkillHelpTitle",
          );
          content = LangManager.Instance.GetTranslation(
            "Skill.masterySkillHelp",
          );
        }
        break;
      ...
      case SkillWnd.page_TF: //天赋
        title = LangManager.Instance.GetTranslation(
          "armyII.SkillFrame.HelpBtn.TalentTipdata",
        );
        content = LangManager.Instance.GetTranslation(
          "armyII.TalentHelpFrame.helpContent",
        );
        break;
      case SkillWnd.page_FK:
        title = LangManager.Instance.GetTranslation(
          "armyII.SkillFrame.HelpBtn.RuneHoleTipdata",
        );
        content = LangManager.Instance.GetTranslation(
          "armyII.RuneHelpFrame.runeHoleContent",
        );
        break;
    }
  ...
  setSelectTab(tabIndex: number = 0) {
    ...
    switch (tabIndex) {
      ...
      case SkillWnd.page_FW: //符文
        title = LangManager.Instance.GetTranslation(
          "armyII.SkillFrame.runeTitle",
        );
        BagHelper.OPEN_RUNE_BAG_TYPE = -1;
        break;
      case SkillWnd.page_TF: //天赋
        title = LangManager.Instance.GetTranslation(
          "armyII.SkillFrame.talentTitle",
        );
        break;

      case SkillWnd.page_FK:
        title = LangManager.Instance.GetTranslation(
          "armyII.SkillFrame.runeHole",
        );
    ...
    if (this["tabMenu_3"].visible) {
      let state =
        PlayerManager.Instance.currentPlayerModel.playerInfo.runePowerPoint >=
          100 && ArmyManager.Instance.thane.grades >= OpenGrades.RUNE_OPEN;
      //是否已开启符文
