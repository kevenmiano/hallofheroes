...
export class LoginManager extends GameEventDispatcher {
    ...
    public setup() {
        ServerDataManager.listen(S2CProtocol.U_L_GET_PLAYER_LIST, this, this.s2c_get_player_list);
        ServerDataManager.listen(S2CProtocol.U_L_PREPARE_LOGIN, this, this.s2c_prepare_login);
        ServerDataManager.listen(S2CProtocol.U_L_REGISTER_ROLE_RSP, this, this.s2c_register_role);
        ServerDataManager.listen(S2CProtocol.U_G_LOGIN_OTHER, this, this.s2c_login_other);
        ServerDataManager.listen(S2CProtocol.U_L_CREATE_LOGIN_RSP, this, this.s2c_createLoginRsp);
        ServerDataManager.listen(S2CProtocol.U_G_LOGIN_GATEWAY, this, this.s2c_LoginRsp);
    }

    public removeEvent() {
        ServerDataManager.cancel(S2CProtocol.U_L_GET_PLAYER_LIST, this, this.s2c_get_player_list);
        ServerDataManager.cancel(S2CProtocol.U_L_PREPARE_LOGIN, this, this.s2c_prepare_login);
        ServerDataManager.cancel(S2CProtocol.U_L_REGISTER_ROLE_RSP, this, this.s2c_register_role);
        ServerDataManager.cancel(S2CProtocol.U_G_LOGIN_OTHER, this, this.s2c_login_other);
        ServerDataManager.cancel(S2CProtocol.U_L_CREATE_LOGIN_RSP, this, this.s2c_createLoginRsp);
        ServerDataManager.cancel(S2CProtocol.U_G_LOGIN_GATEWAY, this, this.s2c_LoginRsp);
    }
    ...
    private __timeUpdateHandler() {
        ...
        let leftTime: number = PlayerManager.Instance.currentPlayerModel.sysCurTimeBySecond % 3600;
        ...
        if (leftTime == 0) {
            if (PlayerManager.Instance.currentPlayerModel.sysCurtime.getDate() != PlayerManager.Instance.currentPlayerModel.sysCurtime.getDate()) {
    ...
    private sendSysTime() {
        ...
        let site = ModelMgr.Instance.getProperty(EmModel.USER_MODEL, UserModelAttribute.site);
     ...
     * 初始化登录流程
     */
    public c2s_createLoginReq(userName: string, pass: string, site: string, siteId: number, from: string, platId: number, appData: any = null) {
        ...
        if (appData) {
            ...
            if (appData.sdkVersion)
                msg.clientVersion = appData.sdkVersion;               // 11;//客户端版本
    ...
    private s2c_createLoginRsp(pkg: PackageIn) {
        ...
        switch (data.result) {
            case LoginResult.SUCCEED:
                let loginKey = ModelMgr.Instance.getProperty(EmModel.USER_MODEL, UserModelAttribute.loginKey);
                LoginManager.Instance.c2s_get_player_list(data.userName, loginKey, data.site);//请求玩家列表
                //开启检测心跳
            ...
            case LoginResult.OTHER_ERROR:
                //其他错误
                MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("login.otherError"));
                this.nativeReLogin();
                break;
            case LoginResult.CONTENT_ERROR:
                //地址栏参数个数不对
                MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("login.addressParaCountError"));
                this.nativeReLogin();
                break;
            case LoginResult.IP_ERROR:
                //无效IP
                MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("login.invalidIP"));
                this.nativeReLogin();
                break;
            case LoginResult.SITE_ERROR:
                //代理商错误
                MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("login.agentError"));
                this.nativeReLogin();
                break;
            case LoginResult.LOGIN_ERROR:
                //加密不正确
                MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("login.incorrectEncryption"));
                this.nativeReLogin();
                break;
            case LoginResult.WEBCASTLE_ERROR:
                //注册服务不正确
                MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("login.registrationServiceIncorrect"));
                this.nativeReLogin();
                break;
            case LoginResult.VERIFY_ERROR:
                MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("login.verityError"));
                this.nativeReLogin();
                break;
            case LoginResult.SITE_CLOSE:
                MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("site.close"));
                break;
            default:
                break;
        }
    ...
    private s2c_get_player_list(pkg: PackageIn) {
        ...
        if (data.loginState >= 0) {
            ModelMgr.Instance.setProperty(EmModel.USER_MODEL, UserModelAttribute.userInfoList, data.infoList);
            ModelMgr.Instance.setProperty(EmModel.USER_MODEL, UserModelAttribute.userName, data.userName);
            ModelMgr.Instance.setProperty(EmModel.USER_MODEL, UserModelAttribute.site, data.site);
            ModelMgr.Instance.setProperty(EmModel.USER_MODEL, UserModelAttribute.loginState, data.loginState);

            let loginKey = ModelMgr.Instance.getProperty(EmModel.USER_MODEL, UserModelAttribute.loginKey);
            let userinfo: SimpleUserInfo = ModelMgr.Instance.getProperty(EmModel.USER_MODEL, UserModelAttribute.userInfoList)[0] as SimpleUserInfo;

            let userInfo: UserInfo = PlayerManager.Instance.currentPlayerModel.userInfo;
            let playerInfo = PlayerManager.Instance.currentPlayerModel.playerInfo;
            playerInfo.site = ModelMgr.Instance.getProperty(EmModel.USER_MODEL, UserModelAttribute.site);
            userInfo.isActive = true;
            let newPassword = String(Math.floor(Math.random() * 10000000));
            userInfo.loginKey = newPassword;
            ModelMgr.Instance.setProperty(EmModel.USER_MODEL, UserModelAttribute.password, newPassword);
            let userId = 0;
            if (userinfo && userinfo.userId) {
                userId = userinfo.userId;
            }
            ModelMgr.Instance.setProperty(EmModel.USER_MODEL, UserModelAttribute.userId, userId);
            LoginManager.Instance.c2s_prepare_login(data.userName, loginKey, newPassword, data.site, userId);
        } else {
            MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("login.otherError"));
        }
    }
    ...
    private reLoginServer() {
        ...
        userInfo.loginKey = newPassword;
        ModelMgr.Instance.setProperty(EmModel.USER_MODEL, UserModelAttribute.password, newPassword);
        let userId = ModelMgr.Instance.getProperty(EmModel.USER_MODEL, UserModelAttribute.userId);
        LoginManager.Instance.c2s_prepare_login(userName, loginKey, newPassword, site, userId);
    }
     ...
     * @param userId
     */
    public c2s_prepare_login(userName: string, loginKey: string, password: string, site: string, userId: number) {
    ...
    private s2c_prepare_login(pkg: PackageIn) {
        ...
        if (data.result) {
            ModelMgr.Instance.setProperty(EmModel.USER_MODEL, UserModelAttribute.userName, data.userName);
            ModelMgr.Instance.setProperty(EmModel.USER_MODEL, UserModelAttribute.site, data.site);
            ModelMgr.Instance.setProperty(EmModel.USER_MODEL, UserModelAttribute.userId, data.userId);
            ModelMgr.Instance.setProperty(EmModel.USER_MODEL, UserModelAttribute.gateIp, data.gateIp);// 准备登录的网关IP
            ModelMgr.Instance.setProperty(EmModel.USER_MODEL, UserModelAttribute.port, data.port);// 准备登录的网关端口
            ...
            if (userInfo.todayFirstLogin) {//如果是首次登录, 清掉丰收号角的点击缓存数据
            ...
            if (data.message == PREPARE_LOGIN_RESULT.SUCCEED_NOVISUALIZE) {
                ...
                GameManager.Instance.onGameSocketConnect(data.gateIp, data.port, callFunc);//登录网关服
            }
        } else {
            ...
            if (data.message.indexOf("|") != -1) {
                ...
                if (waitSeconds == -2) {
                    //关闭登录Socket
                    LoginSocketManager.Instance.close();
                    MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("login.server.full"));
                } else {
    ...
    private async waitLoginServer() {
        ...
        if (this._waitLoginAlert == null || !this._waitLoginAlert.isShowing) {
            this._waitLoginAlert = await UIManager.Instance.ShowWind(EmWindow.HintWnd, LangManager.Instance.GetTranslation("login.server.loginwait", this.loginWaitTime));
        } else {
            this._waitLoginAlert.setHintText(LangManager.Instance.GetTranslation("login.server.loginwait", this.loginWaitTime));
        }
    }
    ...
    public c2s_register_role(userId: number, userName: string, nickName: string, sex: number, camp: number, icon: number, site: string, password: string) {
        ...
        this.registerTime = setTimeout(() => {
            ...
            let content: string = LangManager.Instance.GetTranslation("login.serverclosed.lost2");
            SimpleAlertHelper.Instance.Show(SimpleAlertHelper.SIMPLE_ALERT, null, prompt, content, confirm, "", () => {
                SDKManager.Instance.getChannel().reload();
            }, AlertBtnType.O, false, true, EmLayer.STAGE_TIP_LAYER);
    ...
    private s2c_register_role(pkg: PackageIn) {
        let data: registerRoleRsp = pkg.readBody(registerRoleRsp) as registerRoleRsp;
        if (data.value) {
            FrameCtrlManager.Instance.open(EmWindow.Waiting);
            Logger.yyz("创建角色成功: " + data.message);
            Laya.timer.once(3000, this, () => {
                if (PlayerManager.Instance.currentPlayerModel.playerInfo.nickName) {
                    SDKManager.Instance.getChannel().postGameEvent(GameEventCode.Code_1041);
                }
            });

            PackageOut.clientId = ModelMgr.Instance.getProperty(EmModel.USER_MODEL, UserModelAttribute.userId);
            ModelMgr.Instance.setProperty(EmModel.USER_MODEL, UserModelAttribute.gateIp, data.gateIp);// 准备登录的网关IP
            ModelMgr.Instance.setProperty(EmModel.USER_MODEL, UserModelAttribute.port, data.port);// 准备登录的网关端口
            //关闭登录Socket
            let callFunc = () => {
            ...
            if (data.gateIp == "" || data.gateIp == undefined) {
                MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("login.gatewayServerError"));
            } else {
                GameManager.Instance.onGameSocketConnect(data.gateIp, data.port, callFunc);//登录网关服
            }
        } else {
    ...
    public c2s_login_gateway() {
        ...
        msg.key = MD5Utils.md5(ModelMgr.Instance.getProperty(EmModel.USER_MODEL, UserModelAttribute.password));
        let key: Array<number> = [Math.ceil(Math.random() * 255),
        ...
        Math.ceil(Math.random() * 255)];
        msg.ekeys = key;
        msg.playerId = ModelMgr.Instance.getProperty(EmModel.USER_MODEL, UserModelAttribute.userId);
    ...
    private s2c_login_other(pkg: PackageIn) {
        ...
        let prompt: string = LangManager.Instance.GetTranslation("public.prompt");
        let confirm: string = LangManager.Instance.GetTranslation("public.logout");
        SimpleAlertHelper.Instance.Show(SimpleAlertHelper.SIMPLE_ALERT, null, prompt, data.notifyMsg, confirm, "", () => {
            SharedManager.Instance.setWindowItem("isLoginAfterRestart", "true");
            SDKManager.Instance.getChannel().logout();
        }, AlertBtnType.O, false, false, EmLayer.STAGE_TIP_LAYER);
    }
