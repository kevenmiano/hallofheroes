...
import { FarmEvent, WaterEvent } from "../../../../constant/event/NotificationEvent";
...
export class FarmTree extends FUI_FarmTree {
	...
	private refreshTreeExpBar() {
		...
		if (treeInfo && !treeInfo.notHasTree) {
			...
			let temp: t_s_upgradetemplateData = TempleteManager.Instance.getTemplateByTypeAndLevel(grade, UpgradeType.UPGRADE_TYPE_TREEPOWER);
			if (temp) {
				var waterCount: number = treeInfo.waterCount > temp.Data ? temp.Data : treeInfo.waterCount;
	...
	private refreshWaterState() {
		...
		// 今天不能充能，隐藏。其它情况置灰。
		if (!todayCanWater) { this.btnTipOpt.visible = false; return }
		if (waterLeftTime <= 0) return;

		if (!this.waterTimer) {
			this.waterTimer = new TimerTicker(1000, 0, this.onTickWater.bind(this), this.onCompleteWater.bind(this));
		}
	...
	private refreshPickState() {
		...
		//冷却 进入倒计时
		if (!this.pickTimer) {
			this.pickTimer = new TimerTicker(1000, 0, this.onTickPick.bind(this), this.onCompletePick.bind(this));
		}
	...
	public isShowWater() {
		...
		let upData: t_s_upgradetemplateData = TempleteManager.Instance.getTemplateByTypeAndLevel(this.farmModel.curSelectedFarmInfo.grade, UpgradeType.UPGRADE_TYPE_TREEPOWER);
	...
	private onTickPick() {
		this.txtCountDown.text = DateFormatter.getConsortiaCountDate(this.pickTimer.repeatCount - this.pickTimer.currentCount);
	}
	...
	private onTreeClickHandler(e?: Laya.Event) {
		...
		let curTreeInfo = this.model.currentSelectedTreeInfo;
		let isSelf = curTreeInfo.userId == this.thane.userId;
		if (curTreeInfo.fruitCount > 0
			&& isSelf
			&& curTreeInfo.left_pickTime > 0) {
			str = LangManager.Instance.GetTranslation("buildings.water.view.WaterViewContainer.command02");
		...
		if (this.pickFlag) {
			if (curTreeInfo.fruitCount < 1) {
				str = LangManager.Instance.GetTranslation("buildings.water.view.WaterViewContainer.command01");
				MessageTipManager.Instance.show(str);
				return;
			}
	...
	private onHarvestMsg(pkg: PackageIn) {
		...
		if (curSelectedFarm.userId != this.thane.userId) this.farmModel.myFarm.dayGpFromFriend += addExp;
	}
	...
	private onPickHavestHandler(pkg: PackageIn) {
		...
		let str = msg.harvestMsg.join(", ");
		MessageTipManager.Instance.show(str);
		this.farmModel.myFarm.gp += this.farmModel.expByOper(FarmOperateType.TREE_PICK);
	}
	...
	private showProgTransition(show: boolean, type: number = 0) {
		...
		if (type == 1) {
			this.txtProgTranstion.text = LangManager.Instance.GetTranslation("WaterViewContainer.getTxt");
		} else {
	...
	private initEvent() {
		...
		this.farmModel.addEventListener(FarmEvent.SELECTED_CHANGE, this.refreshTreeView, this);
		ServerDataManager.listen(S2CProtocol.U_C_HARVEST_MSG, this, this.onHarvestMsg);
		ServerDataManager.listen(S2CProtocol.U_C_TREE_PICK, this, this.onPickResultHandler);
		ServerDataManager.listen(S2CProtocol.U_C_PICK_HARVEST_MSG, this, this.onPickHavestHandler);
		WaterManager.Instance.addEventListener(WaterEvent.UPDATA_WATER_STATE, this.refreshTreeView, this);
		WaterManager.Instance.addEventListener(WaterEvent.TREE_UPDATE, this.refreshTreeView, this);
	}
	private removeEvent() {
		TweenLite.killTweensOf(this.progTranstion);
		this.imgTree.off(Laya.Event.CLICK, this, this.onTreeClickHandler);
		this.farmModel.removeEventListener(FarmEvent.SELECTED_CHANGE, this.refreshTreeView, this);
		ServerDataManager.cancel(S2CProtocol.U_C_HARVEST_MSG, this, this.onHarvestMsg);
		ServerDataManager.cancel(S2CProtocol.U_C_TREE_PICK, this, this.onPickResultHandler);
		ServerDataManager.cancel(S2CProtocol.U_C_PICK_HARVEST_MSG, this, this.onPickHavestHandler);
		WaterManager.Instance.removeEventListener(WaterEvent.UPDATA_WATER_STATE, this.refreshTreeView, this);
		WaterManager.Instance.removeEventListener(WaterEvent.TREE_UPDATE, this.refreshTreeView, this);
	}
