...
export default class CrossPvPSuccessWnd extends BaseWindow {
    ...
    private initView() {
        ...
        if (frameData) {
            ...
            this.BossNameDescTxt.text = LangManager.Instance.GetTranslation("CrossPvPSuccessWnd.BossNameDescTxt", this._leftCount, this._totalCount);
            this.campaignNameTxt.text = frameData.campaignName;
            this._campaignUid = frameData.campaignUid;
            this._roomId = frameData.roomId;
            this.leftTimeTxt.text = LangManager.Instance.GetTranslation("CrossPvPSuccessWnd.leftTimeTxt", this._timeCount);
    ...
    private updateLeftTimeTxt() {
        this._timeCount--;
        this.leftTimeTxt.text = LangManager.Instance.GetTranslation("CrossPvPSuccessWnd.leftTimeTxt", this._timeCount);
        if (this._timeCount <= 0) {//倒计时结束了玩家还没操作, 相当于拒绝邀请
    ...
    private confirmBtnHandler() {
        ...
        RoomSocketOuterManager.sendCrossAlertResult(1, this._campaignUid, this._roomId);
        this.hide();
    }
    ...
    private cancelBtnHandler() {
        Laya.timer.clear(this, this.updateLeftTimeTxt);
        RoomSocketOuterManager.sendCrossAlertResult(0, this._campaignUid, this._roomId);
        this.hide();
    }
