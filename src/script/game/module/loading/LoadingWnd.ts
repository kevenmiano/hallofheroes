...
export class LoadingWnd extends BaseWindow {
  ...
  private _resourcesList: Array<any> = [];
  ...
  public OnShowWind() {
    ...
    NotificationManager.Instance.addEventListener(
      NotificationEvent.CHECK_NETSPEED,
      this.onCheckNetSpeed,
      this,
    );
  ...
  private loadResources() {
    ...
    if (!ComponentSetting.configZip) {
      //非zip模式
      //加载游戏所需配置表文件
      for (const key in ConfigUrl) {
        if (Object.prototype.hasOwnProperty.call(ConfigUrl, key)) {
          if (key !== ConfigType.config) {
            //合并配置表此时不需要加载
            let item = {
              key: key,
              url: ComponentSetting.CONFIG_PREFIX + ConfigUrl[key].url,
              type: ResType.JSON,
            };
            this._resourcesList.push(item);
          }
      ...
      //如果configZip模式,则下载压缩包
      let item = {
        key: ConfigType.config,
        url: ComponentSetting.configZipUrl + ConfigUrl[ConfigType.config].url,
        type: ResType.ZIP_CONFIG,
      };
      this._resourcesList.push(item);
    }
  ...
  private loadNextResource() {
    ...
    switch (resItem.type) {
      case ResType.ZIP_CONFIG:
        ConfigMgr.Instance.loadZip(
          resItem.key,
          this.onResourcesLoadProgress.bind(this),
        );
        break;
      case ResType.JSON:
  ...
  private onLoadingProgress() {
    this.updateProObj(
      this._curLoadCount,
      this._needLoadCount,
      this.getStepTips(this._step),
    );
  }

  private onCheckNetSpeed(speed: number) {
    this.netSpeedText.text = LangManager.Instance.GetTranslation(
      "public.kbs",
      speed,
    );
  }
  ...
  private reqLoginInitData() {
    ...
    new LoginInitDataAccept().losyBackCall(
      this.onLoginInitDataAcceptComplete.bind(this),
    );
  }
  ...
  private onResourceLoadComplete() {
    if (this._curLoadCount < this._needLoadCount) return;
    this._step++;
    this.startLoader();
  }
  ...
  private updateProObj(
    ...
    self.progressText.text =
      tips + parseInt(((curCount * 100) / maxCount).toString()) + "%";
