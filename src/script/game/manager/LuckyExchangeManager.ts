...
export default class LuckyExchangeManager {
  ...
  private initEvent() {
    ServerDataManager.listen(
      S2CProtocol.U_C_LUCK_EXCHANGE_TEMP_MSG,
      this,
      this.__luckyTempHandler,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_LUCK_EXCHANGE_DATA_MSG,
      this,
      this.__luckyDataHandler,
    );
  }

  private __luckyTempHandler(pkg: PackageIn) {
    let msg: LuckExchangeTempListMsg = pkg.readBody(
      LuckExchangeTempListMsg,
    ) as LuckExchangeTempListMsg;
    this._tempList = msg;
    this.idMap = new Map();
    ...
    NotificationManager.Instance.dispatchEvent(
      NotificationEvent.LUCKY_TEMP_CHANGE,
    );
  }

  private __luckyDataHandler(pkg: PackageIn) {
    let msg: LuckExchangeInfoListMsg = pkg.readBody(
      LuckExchangeInfoListMsg,
    ) as LuckExchangeInfoListMsg;
    this._dataList = msg;
    NotificationManager.Instance.dispatchEvent(
  ...
  public get showData(): Array<LuckExchangeTempMsg> {
    ...
    var tempArr: Array<LuckExchangeTempMsg> =
      LuckyExchangeManager.Instance.tempList;
    for (var i: number = 0; i < tempArr.length; i++) {
      startTime = DateFormatter.parse(
        tempArr[i].startTime,
        "YYYY-MM-DD hh:mm:ss",
      );
      remainTime =
  ...
  public getGoodsByRare(
    temp: LuckExchangeTempMsg,
  ): Array<LuckExchangeItemTempMsg> {
