...
export default class ConsortiaEmailMemberItem extends FUI_ConsortiaEmailMemberItem {
    ...
    private refreshView() {
        if (this._info) {
            ...
            this.txtTitle.text = ConsortiaDutyLevel.getDutyByButyLevel(this._info.dutyId);
            this.headIcon.url = IconFactory.getPlayerIcon( this._info.headId, IconType.HEAD_ICON);
