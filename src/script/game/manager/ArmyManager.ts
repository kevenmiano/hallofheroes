...
import { ArmyEvent, NotificationEvent, RuneEvent, ServiceReceiveEvent, SkillEvent, SLGSocketEvent, TalentEvent } from "../constant/event/NotificationEvent";
...
export class ArmyManager extends GameEventDispatcher {
    ...
    public set isSorted(value: boolean) {
        ...
        if (value) {
            let taskTempId = 160
            if (!NewbieBaseConditionMediator.checkConditionCommon(3, taskTempId.toString())) {
    ...
    */
    ...
    public setup() {
        this.initPawnList();
        ServerDataManager.listen(S2CProtocol.U_C_SERIAL_ARMY, this, this.__getArmyListHandler);

    }

    private initEvent() {
        ServerDataManager.listen(S2CProtocol.U_C_ARMY_UPDATE_ARMYPAWN, this, this.__recievePawnListHandler);
        ServerDataManager.listen(S2CProtocol.U_C_ARMY_RECRUITED, this, this.__recievePawnListHandler);
        ServerDataManager.listen(S2CProtocol.U_C_ARMY_EDIT, this, this.__armypawnUpdateHander);
        ServerDataManager.listen(S2CProtocol.U_C_PAWN_INFO, this, this.__pawnLeveupHandler);
        ServerDataManager.listen(S2CProtocol.U_C_HERO_INFO, this, this.__uHeroInfoHandler);
        ServerDataManager.listen(S2CProtocol.U_C_HERO_HP_UPDATE, this, this.__heroUpdateHp);
        ServerDataManager.listen(S2CProtocol.U_C_HERO_SETFASTKEY, this, this.__fastKeySetHandler);
        ServerDataManager.listen(S2CProtocol.U_C_HERO_SKILL_UPGRADE, this, this.__skillUpgradeHandlker);
        ServerDataManager.listen(S2CProtocol.U_C_HERO_SKILLPOINT_RESET, this, this.__skillResetHandler);
        ServerDataManager.listen(S2CProtocol.U_C_TALENT_GRADE_UP, this, this.__talentGradeUpHandler);
        ServerDataManager.listen(S2CProtocol.U_C_TALENT_INFO, this, this.__changeTalentInfoHandler);
        ServerDataManager.listen(S2CProtocol.U_C_RUNE_OP, this, this.__runeOperateHandler);
        ServerDataManager.listen(S2CProtocol.U_C_RUNE_RESET, this, this.__runeResetHandler);
        ServerDataManager.listen(S2CProtocol.U_C_FATE_REQUEST, this, this.__fateGuardListHandler);
        ServerDataManager.listen(S2CProtocol.U_C_TATTOO_RSP, this, this.__tattooHandler);
        ServerDataManager.listen(S2CProtocol.U_C_FASHION_INFO, this, this.__fashionHandler);

        ServerDataManager.Instance.addEventListener(SLGSocketEvent.UPDTAE_ARMY, this.__updateArmyHandler, this);
    }
    ...
    private __recieveArmyHandler(pkg: PackageIn) {
        ...
        for (let j: number = 0; j < msg.armyPawn.length; j++) {
            ...
            SocketGameReader.readArmyPawnInfo(this.army, msg.armyPawn[j] as ArmyPawnInfoMsg);
        }
        //英雄
        if (msg.hasOwnProperty("simpleHeroInfo")) {
            ...
            if (msg.hasOwnProperty("fightingCapacity")) {
                ...
                PlayerManager.Instance.currentPlayerModel.playerInfo.fightingCapacity = msg.fightingCapacity;
            }
            ThaneInfoHelper.readHeroInfo(this.thane, msg.simpleHeroInfo as SimpleHeroInfoMsg, msg);
        }
        this.army.commit();
    }
     ...
     *
     */
    private __recievePawnListHandler(pkg: PackageIn) {//0x0064
        let msg: ArmyPawnUpdatedMsg = pkg.readBody(ArmyPawnUpdatedMsg) as ArmyPawnUpdatedMsg;
    ...
    private __armypawnUpdateHander(pkg: PackageIn) {
        ...
        for (let i: number = 0; i < msg.armyPawn.length; i++) {
            SocketGameReader.readArmyPawnInfo(this.army, msg.armyPawn[i] as ArmyPawnInfoMsg);
        }
        this.army.commit();
    }
    ...
    private __updateArmyHandler(armyMsg: ArmyMsg) {
        ...
        if (userId == this.thane.userId && (this.army.id == 0 || armyId == this.army.id)) {
            uArmy = this.army;
            if (!uArmy) {
                return;
            }
        ...
        if (uArmy.type == ArmyType.ARMY_INVITE || uArmy.type == ArmyType.ARMY_SYSTEM || uArmy.type == ArmyType.ARMY_TYPE_SERIAL) {
            NotificationManager.Instance.sendNotification(NotificationEvent.UPDTAE_ARMY, uArmy);
        }
    }
    ...
    private __uHeroInfoHandler(pkg: PackageIn) {
        let heroMsg: SimpleHeroInfoMsg = pkg.readBody(SimpleHeroInfoMsg) as SimpleHeroInfoMsg;
    ...
    private __skillUpgradeHandlker(pkg: PackageIn) {
        ...
        if (type == 0) {
            ...
            NotificationManager.Instance.sendNotification(SkillEvent.SKILL_UPGRADE, info);
            //yuyuanzhan 这个是干嘛的？
            ...
            NotificationManager.Instance.sendNotification(TalentEvent.TALENT_UPGRADE, info);
        }else if (type == 2) {//专精技能升级
            tempId = msg.templateId;
            let cfg:t_s_skilltemplateData =ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_skilltemplate, tempId.toString());
            if(cfg){
    ...
    private __skillResetHandler(pkg: PackageIn) {
        ...
        if (type == 0) {
            if (msg.resetResult) {
                ...
                NotificationManager.Instance.sendNotification(SkillEvent.SKILL_RESET, null);
            }
        }
        else if (type == 1) {
            if (msg.resetResult) {
                ...
                NotificationManager.Instance.sendNotification(TalentEvent.TALENT_RESET, null);
            }
        }
    }
    ...
    protected __talentGradeUpHandler(pkg: PackageIn) {
        let msg: TalentUpGradeMsg = pkg.readBody(TalentUpGradeMsg) as TalentUpGradeMsg;
        if (msg.upresult) {
            MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("this.army.talent.TalentGradeUpSucc"));
    ...
    protected __runeOperateHandler(pkg: PackageIn) {
        ...
        if (msg.count) {
            PlayerManager.Instance.currentPlayerModel.playerInfo.diamondIndex = msg.count;
        }
        if (msg.opResult) {
            ...
            if (msg.opType == RuneOperationCode.RUNE_TAKE) {
                ...
                if (msg.opType == RuneOperationCode.RUNE_STUDY) {
                    let str: string = LangManager.Instance.GetTranslation("armyII.viewII.rune.RuneStudyTipTxt");
                ...
                for (let i: number = 0; i < len; i++) {
                    ...
                    temp = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_runetemplate, runeMsg.runeId.toString());
                    info = this.thane.runeCate.getInfoByIndex(temp.RuneIndex);
                    if (msg.opType == RuneOperationCode.RUNE_HOLE_ACTIVE ||
                        msg.opType == RuneOperationCode.RUNE_HOLE_CARVE ||
                        msg.opType == RuneOperationCode.RUNE_HOLE_REPLACE) {//激活符文孔返回
                        if (info && runeMsg.runeHole) {
                            ...
                            if (msg.opType == RuneOperationCode.RUNE_HOLE_CARVE) {
                                MessageTipManager.Instance.show(LangManager.Instance.GetTranslation('runeGem.str21'))
                                NotificationManager.Instance.sendNotification(RuneEvent.CARVE_RUNE, changeInfo);
                            } else if (msg.opType == RuneOperationCode.RUNE_HOLE_REPLACE) {
                                NotificationManager.Instance.sendNotification(RuneEvent.RECV_REPLACE_RUNE_HOLE, changeInfo);
                            }
                        }
                    } else {
                        if (info && (info.grade < temp.RuneGrade || info.runeCurGp < runeMsg.runeGp)) {
                ...
                if (msg.opType == RuneOperationCode.RUNE_HOLE_CARVE) {
                    ...
                    NotificationManager.Instance.sendNotification(RuneEvent.RUNE_UPGRADE, changeInfo);
                }
            }
    ...
    protected __fateGuardListHandler(pkg: PackageIn) {
        ...
        for (let key in msg.fateInfo) {
            ...
            PlayerManager.Instance.currentPlayerModel.faterotaryCount = infoMsg.turnCount;
        }
    }
    ...
    protected __tattooHandler(pkg: PackageIn): void {
        let msg: TattooHoleRspMsg = pkg.readBody(TattooHoleRspMsg) as TattooHoleRspMsg;
        let info: FateRotarySkillInfo;
        let i: number = 0;
        ...
        if (ctrl.tattooModel.holes == null) {
            ...
            let hole: TattooHole;
            for (const holeMsg of msg.holeInfo)//TattooHoleMsg
            {
                hole = new TattooHole();
                hole.index = holeMsg.hole;//i++;
                if(msg.opType == RoleCtrl.OP_REFRESH || msg.opType == RoleCtrl.OP_ADVANCE_REFRESH)
                {
        ...
        NotificationManager.Instance.dispatchEvent(ArmyEvent.TATTOO_INFO, msg.opType);
    }
    ...
    protected __fashionHandler(pkg: PackageIn) {
        ...
        if (msg.hatAvata)//帽子
        {
            ...
            let skillInfo: t_s_skilltemplateData = this.fashionModel.getFashionObjectSkillTemplate(good2.templateInfo);
            good2.appraisal_skill = (skillInfo != null ? skillInfo.TemplateId : 0);
            fashionList.add(good2.pos, good2);
        }
        if (msg.clothAvata)//衣服
        {
            let good4: GoodsInfo = new GoodsInfo();
            good4.pos = 10;
            good4.bagType = 4;
            good4.templateId = msg.clothAvata;
            let skillInfo: t_s_skilltemplateData = this.fashionModel.getFashionObjectSkillTemplate(good4.templateInfo);
        ...
        if (msg.armAvata)//武器
        {
            ...
            let skillInfo: t_s_skilltemplateData = this.fashionModel.getFashionObjectSkillTemplate(good3.templateInfo);
        ...
        if (msg.wingAvata)//翅膀
        {
            ...
            let skillInfo: t_s_skilltemplateData = this.fashionModel.getFashionObjectSkillTemplate(good1.templateInfo);
            good1.appraisal_skill = (skillInfo != null ? skillInfo.TemplateId : 0);
            fashionList.add(good1.pos, good1);
        }
        if (msg.dragonHatAvata)//龙语者帽子
        {
        ...
        if (!this.fashionModel.isIdentify) {
            NotificationManager.Instance.dispatchEvent(NotificationEvent.FASHION_SHOW_CHANGE);
        } else {
    ...
    public checkCanUpgraeSeriaPawn(): boolean {
        ...
        return this.army.getPawnByIndex(0).templateInfo.Level < this.thane.grades - 3;
    }
    ...
    public checkCanUpgraeMaxLevelPawn(): boolean {
        ...
        return (max.templateInfo.Level < this.thane.grades - 3 && max.templateInfo.NextLevelTemplateId > 0);
    }
    ...
    public synchronizationAllArmyPawns(ap: ArmyPawn) {
        let armyPawn: ArmyPawn = this.getCastlePawn(ap.templateId, ap.templateInfo.SonType);
        if (armyPawn) {
