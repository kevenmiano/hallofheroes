...
export class AttackProrertyInfo {
  ...
  public hasEffect(property: string): number {
    if (
      property == "basePhyAttack" &&
      this.totalPhyAttack - this.basePhyAttack > 0
    ) {
      return 2;
    } else if (
      property == "basePhyDefence" &&
      this.totalPhyDefence - this.basePhyDefence > 0
    ) {
      return 2;
    } else if (
      property == "baseMagicAttack" &&
      this.totalMagicAttack - this.baseMagicAttack > 0
    ) {
      return 2;
    } else if (
      property == "baseMagicDefence" &&
      this.totalMagicDefence - this.baseMagicDefence > 0
    ) {
      return 2;
    } else if (
      property == "baseForceHit" &&
      this.totalForceHit - this.baseForceHit > 0
    ) {
      return 2;
    } else if (
      property == "baseConatArmy" &&
      this.totalConatArmy - this.baseConatArmy > 0
    ) {
      return 2;
      ...
      property == "baseParry" &&
      this.totalParry - this.baseParry > 0
    ) {
      return 2;
    } else if (
      property == "baseTenacity" &&
      this.totalTenacity - this.baseTenacity > 0
    ) {
      return 2;
    } else if (
      property == "baseIntensity" &&
      this.totalIntensity - this.baseIntensity > 0
    ) {
      return 2;
    }
    return 1;
  ...
  public get totalAttribute(): number {
    return (
      this.totalPhyAttack +
      this.totalPhyDefence +
      Number(this.totalMagicAttack * 1.05) +
      this.totalMagicDefence +
      this.totalForceHit
    );
  }

  public get totalIntensityEffect(): string {
    return ((this.totalIntensity * 100) / (this.totalIntensity + 8000)).toFixed(
      2,
    );
  }

  public get totalTenacityEffect(): string {
    return ((this.totalTenacity * 100) / (this.totalTenacity + 8000)).toFixed(
      2,
    );
  }
