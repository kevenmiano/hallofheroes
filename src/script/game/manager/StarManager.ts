...
export class StarManager extends GameEventDispatcher {
  ...
  private initEvent() {
    ServerDataManager.listen(
      S2CProtocol.U_C_START_MOVE_UPDATE,
      this,
      this.__starMoveUpdateHandler,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_STAR_PICK,
      this,
      this.__starPickHandler,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_STAR_RAND,
      this,
      this.__starRandHandler,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_STAR_COMPOSE,
      this,
      this.__starOneKeyComposeHandler,
    );
  }
    ...
    if (this.starModel.delWay == StarModel.QUICK_SELL && quickSellGold > 0) {
      ...
      MessageTipManager.Instance.show(
        LangManager.Instance.GetTranslation(
          "star.view.DisplayStarView.starSellTip",
          quickSellGold,
        ),
      );
    }
    ...
    if (!this._randomPosList) {
      ...
      for (let i: number = 0; i < len; i++) {
        if (this._randomPosList[i] != msg.site[i]) {
          ...
          this._starModel.openCrystal == i
            ? (this._starModel.openCrystal = -1)
            : (this._starModel.openCrystal = i);
          posChange = true;
        }
      }
  ...
  private __starOneKeyComposeHandler(pkg: PackageIn) {
    ...
    for (let index = 0; index < tempList.length; index++) {
      ...
      else this._tempStarList.del(info.pos);
    }
  }
   ...
   * @param combine
   */
  public sendBtnAutoStar(
    golden: number = 0,
    sellMaxQuality: number = 0,
    composeMaxQuality: number = 0,
  ) {
  ...
  public checkStarPlayerBagIsFull(): boolean {
    ...
    if (
      playerStarNum >=
      PlayerModel.ORIGINAL_OPEN_BAG_COUNT + this.playerInfo.starBagCount
    )
      return true;
    return false;
  }
  ...
  public getStarEquipBagIdleNum(): number {
    let num: number = 0;
    let hasNum: number = this.getStarListByBagType(StarBagType.THANE).getList()
      .length;
  ...
  public oneKeyCompose(type: number) {
    ...
    if (tmpList.length <= 1) {
      MessageTipManager.Instance.show(
        LangManager.Instance.GetTranslation(
          "armyII.viewII.horoscope.HoroscopeView.command01",
        ),
      );
      return;
    }
    ...
    for (let index = 0; index < tmpList.length; index++) {
      ...
      if (this.__oneKeyComposeCheck(info)) {
        ...
        if (
          info.grade <
          StarHelper.getStarMaxGradeByProfile(info.template.Profile)
        ) {
          if (info.template.Profile == 6) continue;
          if (!target) target = info;
          else
            target.template.Profile < info.template.Profile
              ? (target = info)
              : target;
        }
      }
    ...
    if (
      !target ||
      (totalExp = totalExp - StarHelper.getStarTotalExp(target)) <= 0
    ) {
      //总经验减目标星运经验后赋值给总经验, 再比较总经验是否<=0
      MessageTipManager.Instance.show(
        LangManager.Instance.GetTranslation(
          "armyII.viewII.horoscope.HoroscopeView.command01",
        ),
    ...
    let tmpStr =
      this.aKeyComposeBagType == StarBagType.TEMP
        ? "armyII.viewII.horoscope.HoroscopeView.command04"
        : "armyII.viewII.horoscope.HoroscopeView.command02";
    ...
    SimpleAlertHelper.Instance.Show(
      ...
      null,
      null,
      this.__oneKeyCompose.bind(this),
    );
  }
    ...
    if (boolean) {
      ...
      list.forEach((info: StarInfo) => {
        if (
          info != sInfo &&
          !info.composeLock &&
          info.template.Profile >= 5 &&
          info.template.Profile != 6
        ) {
          if (
            this.aKeyComposeBagType == StarBagType.PLAYER &&
            info.grade < 10
          ) {
            arr.push(info);
          } else {
      ...
      if (arr.length > 0) {
        ...
        let content = LangManager.Instance.GetTranslation(
          "cell.view.starbag.StarBagCell.confirmComposeTip",
          sname,
        );
        ...
        SimpleAlertHelper.Instance.Show(
          ...
          null,
          null,
          this.__oneKeyComposeBack.bind(this),
        );
        return;
      }
