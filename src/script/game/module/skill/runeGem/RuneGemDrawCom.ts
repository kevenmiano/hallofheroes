 ...
 */
 export default class RuneGemDrawCom extends FUI_RuneDrawCom{
    ...
    onConstruct() {
        ...
        let cfg = TempleteManager.Instance.getConfigInfoByConfigName("runehole_spot");
        ...
        this.btn_start.getChild('txtTitle2').asTextField.text = LangManager.Instance.GetTranslation('runeGem.str14',1);
        this.btn_ten.getChild('txtTitle2').asTextField.text = LangManager.Instance.GetTranslation('runeGem.str14',10);
        this.btn_start.getChild('txtTitle').asTextField.text = "x"+this.runehole_spot_use;
        this.btn_ten.getChild('txtTitle').asTextField.text = "x"+this.runehole_spot_use*10;
        (this.item.getChild('back') as fgui.GLoader).url = fgui.UIPackage.getItemURL("Base",'Img_Tips2_Bg');
    ...
    private addEvent(){
        ...
        PlayerManager.Instance.currentPlayerModel.playerInfo.addEventListener(PlayerEvent.RUNE_GEM_ENERGY, this.updateEnergy, this);
        PlayerManager.Instance.currentPlayerModel.playerInfo.addEventListener(PlayerEvent.RUNE_GEM_BAG_CAPICITY, this.updateBag, this);
    }
    ...
    private removeEvent(){
        ...
        ServerDataManager.cancel(S2CProtocol.U_C_LOTTERY_RUNE_INFO, this, this.onRecvLottery); 
        GoodsManager.Instance.removeEventListener(BagEvent.UPDATE_BAG, this.__bagItemUpdate, this);
        PlayerManager.Instance.currentPlayerModel.playerInfo.removeEventListener(PlayerEvent.RUNE_GEM_ENERGY, this.updateEnergy, this);
        PlayerManager.Instance.currentPlayerModel.playerInfo.removeEventListener(PlayerEvent.RUNE_GEM_BAG_CAPICITY, this.updateBag, this);
    }

    private initRewardTip(){
        this.rewardTip.getChild('txt0').text = LangManager.Instance.GetTranslation('runeGem.reward0')
        this.rewardTip.getChild('txt1').text = LangManager.Instance.GetTranslation('runeGem.reward1')
        this.rewardTip.getChild('txt2').text = LangManager.Instance.GetTranslation('runeGem.reward2')
    }
    ...
    private flyAni(){
        ...
        Laya.Tween.to(this.img_fragment,{x:0,y:0},600,Laya.Ease.sineOut,Laya.Handler.create(this,this._moveComplete));
    }
    ...
    private showTip(){
        let str = LangManager.Instance.GetTranslation('ruenGem.drawReward',this.rewardStr)
    ...
    private updateCountDown() {
        if (this._countDown > 0) {
            ...
            this.txt_time.text = LangManager.Instance.GetTranslation('ruenGem.leftTime',countDown,this.spot);
            this._countDown--;
        }
        else {
            this.hideTime();
    ...
    onBuy(){
        let data: ShopGoodsInfo = TempleteManager.Instance.getShopTempInfoByItemId(CommonConstant.RUNE_GEM_ENERGY);
        let obj = {
            info: data,
            count: 1,
