...
export class OuterCityWarModel extends GameEventDispatcher {
    ...
    /** 显示的建筑 */
    buildingSontypeList: number[] = [101, 301, 402, 501, 901, 1201, 1504, 1506, 1512, 1513, 1514, 1515, 1516, 1517, 1518, 1519];
    /** 进攻公会阵营   */
    ...
    get castleNodeName(): string {
        return this.castleInfo && this.castleInfo.tempInfo && this.castleInfo.tempInfo.NameLang;
    };
    get castleDefenceNpcName(): string {
        return this.castleInfo && this.castleInfo.defenceNpcTemp && this.castleInfo.defenceNpcTemp.TemplateNameLang;
    };
    ...
    initConfig() {
        for (let index = 0; index < this.buildingSontypeList.length; index++) {
            ...
            let tempInfo = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_buildingtemplate, tempId);
            if (tempInfo) {
        ...
        let priceCfg = TempleteManager.Instance.getConfigInfoByConfigName("CastleBattle_ActionPoint_BuyPrice");
        if (priceCfg) {
            this.ActionPointPrice = Number(priceCfg.ConfigValue);
        }
        let numCfg = TempleteManager.Instance.getConfigInfoByConfigName("CastleBattle_ActionPoint_BuyNum");
        if (numCfg) {
            this.ActionPointBuyGet = Number(numCfg.ConfigValue);
        }
        let Cfg = TempleteManager.Instance.getConfigInfoByConfigName("CastleBattle_BuildSiteCount");
    ...
    public getPlayerInfo(uid: number, utype = EmOuterCityWarHeroType.Hero): OuterCityWarPlayerInfo {
     ...
     * @returns 
     */
    public getGuildPlayerInfoList(guildId: number, isPet: boolean = false, enterWar?: BooleanType, stateList?: EmOuterCityWarPlayerState[]): OuterCityWarPlayerInfo[] {
        let temp = []
    ...
    public getGuildPlayerCnt(guildId: number, isPet: boolean = false, enterWar?: BooleanType, stateList?: EmOuterCityWarPlayerState[]): number {
        let temp = this.getGuildPlayerInfoList(guildId, isPet, enterWar, stateList)
        return temp.length;
    }

    /** 获取某个建筑的驻防点英雄信息 */
    public getBuildSitePlayerList(sonType: number, orderId: number): OuterCityWarPlayerInfo {
        for (const info of this.playerListMap.values()) {
    ...
    /** 获取公会的所有当前城堡参战玩家 */
    public getCurCastleJoinWarGuildPlayerList(guildID: number): OuterCityWarPlayerInfo[] {
        let temp = []
        for (const info of this.playerListMap.values()) {
            if (info.guildId == guildID) {
    ...
    public getAttackDuty(uid: number) {
        ...
        return dutyId == ConsortiaDutyLevel.CHAIRMAN || dutyId == ConsortiaDutyLevel.VICE_CHAIRMAN
    }
    ...
    /** 检查在参战列表的玩家  当前城堡 */
    public checkInCurCastleJoinBattleList(uid: number, heroType: EmOuterCityWarHeroType = EmOuterCityWarHeroType.Hero): boolean {
        ...
        if (playinfo.camp == CampType.Defence) {
            if (playinfo.enterWarCastleNodeId && this.castleNodeId != playinfo.enterWarCastleNodeId) {
                Logger.outcityWar("防守方玩家在别的城作为防守参战, 在本城不属于参战", playinfo, this.castleNodeId)
                return false;
            }
            return playinfo.enterWar == BooleanType.TRUE
        } else {
    ...
    /** 获取玩家在此建筑能否拥有buff */
    public getBuildBuffByUserId(userId: number, sonType: number): t_s_castlebattlebuildingskillData {
        ...
        for (let index = 0; index < playerInfo.buffInfoList.length; index++) {
            ...
            // if (flag) {
            let temp = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_castlebattlebuildingskill, buffInfo.buffId) as t_s_castlebattlebuildingskillData
            return temp
            // }
        }
    ...
    /** 获取建筑默认的buff */
    public getBuildBuffTemplateBySontype(sonType: number): t_s_castlebattlebuildingskillData {
        let temp
        let cfg = ConfigMgr.Instance.getDicSync(ConfigType.t_s_castlebattlebuildingskill)
    ...
    public getBuildBuffTemplateByPlayerCnt(playerCnt: number): t_s_castlebattlebuildingskillData {
        let targetCnt
        switch (playerCnt) {
            case 20:
            case 19:
    ...
    public getBuildSiteInfo(sonType: number, orderId: number): OuterCityWarBuildSiteInfo {
    ...
    public get isCastleFighting(): boolean {
        // 测试
        // return true
        return OuterCityWarManager.Instance.model.castleState == EmOuterCityWarCastlePeriodType.Fighting
    }
    ...
    public showReplaceTip() {
        ...
        let siteInfo = this.getBuildSiteInfo(this.attackSonType, this.attackOrderId)
        Logger.outcityWar("驻防点被攻下 提示放弃原先的占领新攻下的驻点", siteInfo && siteInfo.noOccupyState)
        if (siteInfo) {
            Laya.timer.once(OuterCityWarModel.ShowReplaceTipTime, this, this.onShowReplaceTipTimer)
            let content = LangManager.Instance.GetTranslation("outerCityWar.occupyReplace")
            SimpleAlertHelper.Instance.Show(null, null, null, content, null, null, (b) => {
                if (b) {
                    OuterCityWarManager.Instance.sendOccupyBuildSite(this.attackSonType, this.attackOrderId)
                } else {
                    OuterCityWarManager.Instance.sendUnLockBuildSite(this.attackSonType, this.attackOrderId)
                }
                this.resetReplaceTip();
