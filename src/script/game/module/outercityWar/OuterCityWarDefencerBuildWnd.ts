...
export default class OuterCityWarDefencerBuildWnd extends BaseWindow {
    ...
    private addEvent() {
        ...
        NotificationManager.Instance.addEventListener(OuterCityWarEvent.BUILD_INFO, this.__buildInfo, this)
        NotificationManager.Instance.addEventListener(OuterCityWarEvent.BUILD_PRE_COMPLETE_OCCUPY_STATE, this.__preCompleteOccupyState, this)
    }

    private removeEvent() {
        NotificationManager.Instance.removeEventListener(OuterCityWarEvent.ALL_BUILD_INFO, this.__allBuildInfo, this)
        NotificationManager.Instance.removeEventListener(OuterCityWarEvent.BUILD_INFO, this.__buildInfo, this)
        NotificationManager.Instance.removeEventListener(OuterCityWarEvent.BUILD_PRE_COMPLETE_OCCUPY_STATE, this.__preCompleteOccupyState, this)
    }

    private initView() {
        Logger.outcityWar("打开界面:" + this.buildInfo.buildName + this.buildInfo.sonType)
        ...
        this.txtCompleteOccupyScore.text = LangManager.Instance.GetTranslation("outerCityWar.completeOccupyScore")
        this.txtCompleteOccupyScoreVal.text = LangManager.Instance.GetTranslation("public.addMark", this.buildInfo.completeOccupyScore);
        // 列表的标题
        this.txtDefenceForce.text = LangManager.Instance.GetTranslation("public.playerInfo.defanceForce")
        this.txtGuild.text = LangManager.Instance.GetTranslation("public.playerInfo.guild")
        this.txtOccupyScore.text = LangManager.Instance.GetTranslation("outerCityWar.occupyScore")
        this.txtNickName.text = LangManager.Instance.GetTranslation("public.playerInfo.nickName")
        this.txtEnterBattlePet.text = LangManager.Instance.GetTranslation("public.playerInfo.enterBattlePet")
        this.txtPetTotalCapaity.text = LangManager.Instance.GetTranslation("public.playerInfo.petTotalCapaity")
        this.txtCapaity.text = LangManager.Instance.GetTranslation("public.playerInfo.capaity2")
        this.txtJob.text = LangManager.Instance.GetTranslation("public.playerInfo.job")
        this.txtPawn.text = LangManager.Instance.GetTranslation("public.playerInfo.pawn")
        ...
        for (let index = 0; index < OuterCityWarModel.BuildSiteCount; index++) {
            ...
            item.buidType = Number(this.buildInfo.isPetBuild ? EmOuterCityWarBuildSortType.Pet : EmOuterCityWarBuildSortType.Hero);
        }
    ...
    private initBuildBuffView() {
        ...
        this.buildSkillTemp = this.fightModel.getBuildBuffByUserId(this.fightModel.playerInfo.userId, this.buildInfo.sonType);
        if (!this.buildSkillTemp) {
            this.buildSkillTemp = this.fightModel.getBuildBuffTemplateBySontype(this.buildInfo.sonType)
        ...
        if (this.buildSkillTemp) {
            ...
            switch (this.buildInfo.buffType) {
                case 101:
                    let pawnTemplete = this.fightModel.getPawnTemplateByMastType(this.buildInfo.buffCondition);
                    buildBuffDesc = LangManager.Instance.GetTranslation("outerCityWar.buildBuff01", pawnTemplete && pawnTemplete.PawnNameLang);
                    break;
                case 102:
                    let jobName = JobType.getJobName(this.buildInfo.buffCondition)
                    buildBuffDesc = LangManager.Instance.GetTranslation("outerCityWar.buildBuff02", jobName);
                    break;
                case 201:
                    let petTypeName = PetData.getPetTypeLanguage(this.buildInfo.buffCondition)
                    buildBuffDesc = LangManager.Instance.GetTranslation("outerCityWar.buildBuff03", petTypeName);
                    break;
            }

            this.gSkill.visible = true;
            this.skillItem.getChild("txtTitle").text = LangManager.Instance.GetTranslation("public.level3", this.buildSkillTemp.ActiveSkillLevel);
            let skillTemp = this.buildSkillTemp.getActiveSkillTemplate();
            if (skillTemp) {
                this.skillItem.getChild("imgIcon").asLoader.url = IconFactory.getCommonIconPath(skillTemp.Icons)
                let tipData = new CommonTips2Data(skillTemp.SkillTemplateName, skillTemp.SkillDescription, buildBuffDesc)
                FUIHelper.setTipData(this.btnSkillItemTip.view, EmWindow.CommonTips2, tipData, new Laya.Point(-54, -54), TipsShowType.onClick);
            } else {
                Logger.outcityWar("配置技能模板不存在", this.buildSkillTemp.ActiveSkillTemplate)
            }
        } else {
    ...
    private __preCompleteOccupyState() {
        ...
        if (guildInfo) {
            if (guildInfo.camp == CampType.Attack) {
                ...
                color = this.buildInfo.preOneCompleteOccupyByAttacker ? ColorConstant.GREEN_COLOR : ColorConstant.RED_COLOR;
            } else if (guildInfo.camp == CampType.Defence) {
                str = this.buildInfo.strDefencerAttackBuildTip;
                color = this.buildInfo.preOneCompleteOccupyByDefencer ? ColorConstant.GREEN_COLOR : ColorConstant.RED_COLOR;
            } else {
                Logger.outcityWar("公会的阵营出错", guildInfo)
            }
        }
        if (str) {
            this.tfAttackTipCond.text = LangManager.Instance.GetTranslation("outerCityWar.attackBuildTipCond", color, str);
        } else {
    ...
    private refreshAllSiteView() {
        for (let orderId = 1; orderId <= OuterCityWarModel.BuildSiteCount; orderId++) {
