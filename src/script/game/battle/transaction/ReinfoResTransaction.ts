...
export class ReinfoResTransaction extends TransactionBase {
  ...
  public configure(param: object) {
    ...
    if (reinforceType == 1) {
      ...
      if (battleWnd) {
        battleWnd.reinforceViewHandler.setReinforceWaveInfo(
          msg.reinforceCount,
          1,
        );
      } else {
    ...
    for (p = 0; p < pawnIds.length; p++) {
      skillIds = skillIds.concat(
        SkillResourceLoader.getPawnSkillIds(pawnIds[p]),
      );
    }
    ...
    let heroFullSkillIds: any[] =
      SkillResourceLoader.getHeroFullSkillIds(heroIds);
