...
export class BattleLoadItemTask extends Sequence {
  ...
  protected onExecute() {
    ...
    if (asset && asset.meta && asset.meta.prefix) {
      BattleResPreloadManager.cachePublicSkillAni(
        asset.meta.prefix.toLocaleLowerCase(),
      );
      BattleResPreloadManager.cacheResistSheldAni(
        asset.meta.prefix.toLocaleLowerCase(),
      );
    }
    this.endAction(true);
...
export class BattleLoadFigureTask extends Sequence {
  ...
  protected onExecute() {
    ...
    ResMgr.Instance.loadResItem(this.item, (res) => {
      NotificationManager.Instance.dispatchEvent(
        RoleUnit.LOAD_BATTLERES_COMPLETE,
        { url: this.item.url, res: res },
      );
      this.endAction(true);
    });
  }
