...
export default class NewbieMainNode {
  ...
  public isFinish() {
    ...
    this._isFinish =
      NewbieModule.Instance.finishNodeStr.indexOf(this.mainNodeId.toString()) !=
      -1;
    if (this._isFinish) {
  ...
  public isSkip() {
    ...
    let isSkip = this.checkConditions(
      headNodeInfo.skipConditions,
      headNodeInfo.skipConditionParams,
      headNodeInfo.skipConditionInverted,
      headNodeInfo.skipConditionSymbol,
      false,
    );
    if (isSkip) {
  ...
  public isTrigger() {
    ...
    let headNodeInfo = this._data.subNodeInfoArr[0];
    let isTrigger = this.checkConditions(
      headNodeInfo.conditions,
      headNodeInfo.conditionParams,
      headNodeInfo.conditionInverted,
      headNodeInfo.conditionSymbol,
      false,
    );
    return isTrigger;
  }
  ...
  private __timerUpdateHandler(e) {
    ...
    //是否满足所有跳过条件
    let isReachAllSkipCons: boolean = this.checkConditions(
      this.curSubNodeInfo.skipConditions,
      this.curSubNodeInfo.skipConditionParams,
      this.curSubNodeInfo.skipConditionInverted,
      this.curSubNodeInfo.skipConditionSymbol,
      false,
    );
    ...
    //是否满足所有执行条件
    let isReachAllExecCons: boolean = this.checkConditions(
      this.curSubNodeInfo.conditions,
      this.curSubNodeInfo.conditionParams,
      this.curSubNodeInfo.conditionInverted,
      this.curSubNodeInfo.conditionSymbol,
      true,
    );
    if (isReachAllExecCons) {
      ...
      if (this.curSubNodeInfo.needWaitForComplete) {
        NewbieUtils.execFunc(
          this.curSubNodeInfo.actionFunc,
          this.curSubNodeInfo.actionParams,
          this.completeSubNode.bind(this),
          [this.curSubNodeInfo],
        );
      } else {
        NewbieUtils.execFunc(
    ...
    switch (conditionSymbol) {
      case 1:
        ...
        for (var j: number = 0; j < len; j++) {
          singleConValue = NewbieUtils.execFunc(
            conditions[j],
            conditionParams[j],
          );
          if (invertedLen > 0 && conditionInverted[j] == "1") {
            //如有配置条件取反, 则检查是否需要取反, 1取反
      ...
      default:
        ...
        for (var i: number = 0; i < len; i++) {
          ...
          if (invertedLen > 0 && conditionInverted[i] == "1") {
            //如有配置条件取反, 则检查是否需要取反, 1取反
            singleConValue = !singleConValue;
          }
          if (!singleConValue) {
  ...
  public checkSpaceTaskInfoWndInit() {
    // TODO 先这样做
    if (
      this.curSubNodeInfo.actionType == "a11" ||
      this.curSubNodeInfo.actionType == "a10"
    ) {
