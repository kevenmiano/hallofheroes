export default class ProgressHelp {
  static getCurrentValue(
    ...
    if (currentValue >= maxValue) {
      ...
      for (let i = 0; i < len; i++) {
        if (currentValue <= valueArray[0]) {
          //第一段区域
          returnValue =
            constValue * (maxValue / (valueArray[0] - 0)) * currentValue;
          break;
        } else if (currentValue >= valueArray[len - 2]) {
          //最后一段
          returnValue =
            (maxValue * (len - 1)) / len +
            constValue *
              ((maxValue / (maxValue - valueArray[len - 2])) *
                (currentValue - valueArray[len - 2]));
          break;
        } else if (
          valueArray[i] <= currentValue &&
          currentValue <= valueArray[i + 1]
        ) {
          //中间区域
          returnValue =
            maxValue * ((i + 1) / len) +
            constValue *
              ((maxValue / (valueArray[i + 1] - valueArray[i])) *
