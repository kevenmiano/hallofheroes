...
export class FarmFriendItem extends FUI_FarmFriendItem {
    ...
    public set info(data: ThaneInfo) {
        ...
        if (data) {
            ...
            this.farmStateInfo = FarmManager.Instance.model.getFarmStateInfo(data.userId)
            this.farmStateInfo.removeEventListener(FarmEvent.FRIEND_FARM_STATE_CHANGE, this.__friendFarmStateChangeHandler, this);
            this.farmStateInfo.addEventListener(FarmEvent.FRIEND_FARM_STATE_CHANGE, this.__friendFarmStateChangeHandler, this);
            this.imgOpt.icon = this.farmStateInfo ? this.farmStateInfo.curStateFrame : "";
        } else {
