...
export default class SkillWndCtrl extends FrameCtrlBase {
  ...
  public sendAddSkillPoint(
    ...
    if (info.nextTemplateInfo)
      ArmySocketOutManager.sendAddSkillpoint(
        info.nextTemplateInfo.TemplateId,
        type,
        true,
        subType,
      );
   ...
   * @param showTip
   */
  public sendSetFastKey(fastKey: string) {
    //设置快捷键
  ...
  public switchSkillIndex() {
    var index: number = 0;
    if (ArmyManager.Instance.thane.skillCate.skillIndex == 1) index = 0;
    else index = 1;
  ...
  public addPropertyTxt(value: GoodsInfo): string {
    ...
    let info: t_s_runegemData =
      TempleteManager.Instance.getRuneGemCfgByTypeAndLevel(
        value.templateInfo.SonType,
        value.strengthenGrade,
        value.templateInfo.Property1,
      );
    if (!info) return "";
    let propertys = "";
  ...
  public parseProperty(
    templateInfo:
      ...
      | t_s_runegemData
      | t_s_runeactivationData,
  ) {
    let propertyObject: { [key: string]: number } = {};
    if (!templateInfo) return propertyObject;
    if (
      templateInfo instanceof t_s_itemtemplateData ||
      templateInfo instanceof t_s_runegemData
    ) {
  ...
  public filterRuneAttribute(
    ...
    if (isResolve) {
      ...
      fileter =
        filterType == 1
          ? skillWndData.runeAttributeFilter
          : skillWndData.runeHoldAttributeFilter;
    }
  ...
  public isProfileFilter(filterType = 1, isResolve: boolean = false) {
    ...
    if (isResolve) {
      ...
      fileter =
        filterType == 1
          ? skillWndData.runeProfileFilter
          : skillWndData.runeHoldProfileFilter;
    }
