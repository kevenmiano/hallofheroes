  ...
  implements ITipedDisplay
{
  ...
  public update(index: number) {
    ...
    //剩余领奖次数
    let left_count =
      Number(this.model.boxChangeCount[index]) -
      Number(this.model.exchangeCount[index]);
    this.txt_count.text = LangManager.Instance.GetTranslation(
      "allmainexchange.str4",
      left_count,
      this.model.boxChangeCount[index],
    );
    if (left_count <= 0) {
  ...
  protected onClickExchange() {
    ...
    var allRemainCount: number =
      this.model.allChangeCount - this.model.changeItemCount;
    if (1 > allRemainCount && this.model.allChangeCount != 0) {
      //总兑换次数没了
      MessageTipManager.Instance.show(
        LangManager.Instance.GetTranslation("allmainexchange.str7"),
      );
      return;
    }
    if (
      Number(this.model.boxChangeCount[this.index]) -
        Number(this.model.exchangeCount[this.index]) <=
      0
    ) {
      //领奖次数完了
      MessageTipManager.Instance.show(
        LangManager.Instance.GetTranslation("allmainexchange.str9"),
      );
      return;
    }
    if (!this.checkCount()) {
      //所需物品不足
      MessageTipManager.Instance.show(
        LangManager.Instance.GetTranslation("allmainexchange.str21"),
      );
      return;
    }
    this.playMc();
  ...
  private exchangeComMap: fgui.GComponent[] = [];
  private playMc() {
    ...
    if (_exchangeCom) {
      ...
      _exchangeMc.setPlaySettings(
        ...
        Laya.Handler.create(this, () => {
          ...
          FUIPoolMgr.Instance.returnObject(
            _exchangeCom,
            FUI_ExchangeAnimation.URL,
          );
        }),
      );
    }
  ...
  protected onClickGetAward() {
    if (
      ...
      //领奖次数完了
      MessageTipManager.Instance.show(
        LangManager.Instance.GetTranslation("allmainexchange.str6"),
      );
      return;
    }
