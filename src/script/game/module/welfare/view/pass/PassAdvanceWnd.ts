...
export default class PassAdvanceWnd extends BaseWindow {
  ...
  private initLanguage() {
    this.frame.getChild("title").text =
      LangManager.Instance.GetTranslation("pass.text15");
  ...
  OnShowWind() {
    ...
    let rechargeData: t_s_rechargeData =
      TempleteManager.Instance.getPassCheckRechargeTemplete();
    if (rechargeData) {
      // this.txt_price1.text = '¥ '+ rechargeData.MoneyNum;
      if (Utils.isInteger(rechargeData.MoneyNum)) {
        this.txt_money.text =
          ShopManager.Instance.getMoneyString() +
          parseInt(rechargeData.MoneyNum); //价格
      } else {
        this.txt_money.text =
          ShopManager.Instance.getMoneyString() + rechargeData.MoneyNum; //价格
      }
      this.btn_buy.title = "";
    }
  }
