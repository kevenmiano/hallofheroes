...
export default class OutercityVehicleArmyView extends HeroAvatarView implements IBaseMouseEvent {
    tipData: any;
    ...
    private _timeStr: string = LangManager.Instance.GetTranslation("OuterCityVehicleInfoWnd.descTxt3");
    ...
    private initView() {
        ...
        if (!this._nameTxt) {
            this._nameTxt = new FilterFrameText(240, 24, undefined, 18, ColorConstant.YELLOW_COLOR);
    ...
    private playVehicleBody() {
        ...
        if (this.wildInfo.leftTime <= 0) {
            ...
            let angle = Math.atan2(this.wildInfo.targetPosY * 20 - parseInt((this.wildInfo.movePosY * 20).toString()), this.wildInfo.targetPosX * 20 - parseInt((this.wildInfo.movePosX * 20).toString()));
    ...
    private refreshView() {
        ...
        this._nameTxt.text = this._windInfo.tempInfo && this._windInfo.tempInfo.NameLang;
        if (this._windInfo && this._windInfo.info) {
            if (this._windInfo.info.occupyLeagueName != "") {//占领的公会信息有
                this._consortiaNameTxt.text = "<" + this._windInfo.info.occupyLeagueName + ">";
                if (OuterCityManager.Instance.model.checkIsSameConsortiaByName(this._windInfo.info.occupyLeagueName)) {//同工会的
        ...
        if (this.wildInfo.leftTime > 0 && this.wildInfo.targetPosX) {
            ...
            this._leftTimeTxt.text = this._timeStr + DateFormatter.getConsortiaCountDate(this._leftTime, false);
            Laya.timer.clearAll(this);
            this._oldPoint = new Laya.Point(this.wildInfo.movePosX * 20, this.wildInfo.movePosY * 20);
            this._count = 0;
            this._type = this.validateType(this._oldPoint,this.wildInfo.targetPosX * 20,this.wildInfo.targetPosY * 20);
            this._totalDistance = this._oldPoint.distance(this.wildInfo.targetPosX * 20, this.wildInfo.targetPosY * 20);
            this._totalStepX = Math.abs(this.wildInfo.movePosX * 20 - this.wildInfo.targetPosX * 20);
            this._totalStepY = Math.abs(this.wildInfo.movePosY * 20 - this.wildInfo.targetPosY * 20);
    ...
    private validateType(oldPoint:Laya.Point,targetX:number,targetY:number):number{
        let type:number = 0;
        if (oldPoint.x < targetX) {//目标点在物资车右边
            type = 1;
        } else if (oldPoint.x > targetX) {//目标点在物资车左边
            type = 2;
        }else if(oldPoint.x ==  targetX){
            if (oldPoint.y < targetY) {//目标点在物资车正下方
                type = 3;
            } else { //正上方
                type = 4;
            }
    ...
    playWalk() {
        this._count++;
        let stepX = this._count * this._windInfo.speed  * (20/EnterFrameManager.FPS)* this._totalStepX / this._totalDistance;
        let stepY = this._count * this._windInfo.speed  * (20/EnterFrameManager.FPS)* this._totalStepY / this._totalDistance;
        if(this._type == 1){//目标点在物资车右边
            if(this.x < this.wildInfo.targetPosX * 20){
                if (this.y < this.wildInfo.targetPosY * 20) {//右下方
                    stepY = stepY;
                } else {//右上方
                    stepY = -stepY;
                }
        ...
        }else if(this._type == 2){//目标点在物资车左边
            if(this.x > this.wildInfo.targetPosX * 20){
                stepX = -stepX;
                if (this.y < this.wildInfo.targetPosY * 20) {//左下方
                    stepY = stepY;
                } else { //左上方
                    stepY = -stepY;
                }
        ...
        }else if(this._type == 3){//目标点在物资车正下方
        ...
        }else if(this._type == 4){//目标点在物资车正上方
            if(this.y > this.wildInfo.targetPosY * 20){
                stepY = -stepY;
            }else{
                Laya.timer.clearAll(this);
    ...
    public set wildInfo(value: WildLand) {
        if (value) {
            ...
            if (!this._rolePart) {
                this._rolePart = new HeroMovieClipRef(HeroMovieClipRefType.OUTERCITY_VEHICLE);
                this._rolePart.mouseEnabled = true;
                this._rolePart.completeFunc = this.onPartLoadComplete.bind(this);
                let gameLoadData: GameLoadNeedData = new GameLoadNeedData(IconFactory.getOutercityVehicle(this._windInfo.tempInfo.SonType));
