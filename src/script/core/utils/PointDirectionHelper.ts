// @ts-nocheck
	...
	export class PointDirectionHelper
	{
		...
		public static getPointsBy(cur:Laya.Point,next:Laya.Point) :Array<Laya.Point>
		{
		...
		private static initPoint() 
		{
			...
			var p : Laya.Point;
			var arr :Array<Laya.Point> = [];
			p = new Laya.Point(-2,-2);arr.push(p);
			p = new Laya.Point(-2,-1);arr.push(p);
			p = new Laya.Point(-1,-2);arr.push(p);
			PointDirectionHelper._pointDic.set(PointDirectionHelper.L_T,arr);
			
			arr = [];
			p = new Laya.Point(0,-2);arr.push(p);
			...
			arr = [];
			p = new Laya.Point(-2,0);arr.push(p);
