...
export class LuckBlindBoxManager {
	 ...
	 */
	public requestProtocol(op: number, isOneKey: boolean = false, openNumber: number = 1): void {
		...
		SocketManager.Instance.send(C2SProtocol.C_CCCACTIVE_CLIENTOP, luckBlindBoxMessage, CCCActiveType.LUCKBLINDBOX);
	}
	...
	private initEvent(): void {
		ServerDataManager.listen(S2CProtocol.U_C_LUCKYBOX_INFO, this, this.onReceiveInfo);
		ServerDataManager.listen(S2CProtocol.U_C_LUCHYBOX_STATE, this, this.onReceiveState);
	}
	...
	private onReceiveInfo(pkg: PackageIn) {
		let luckBlindBoxMessage: LuckBlindBoxMessage = pkg.readBody(LuckBlindBoxMessage) as LuckBlindBoxMessage;
		...
		if (luckBlindBoxMessage.op == 1) {
			...
			this._luckBlindBoxMessage.leftNeedPoint = luckBlindBoxMessage.leftNeedPoint;
			...
			this._luckBlindBoxMessage.resultPos = luckBlindBoxMessage.resultPos.slice(0, luckBlindBoxMessage.resultPos.length);
		} else if (luckBlindBoxMessage.op == 3) {
			this._luckBlindBoxMessage.op = luckBlindBoxMessage.op;
			this._luckBlindBoxMessage.logs = luckBlindBoxMessage.logs.slice(0, luckBlindBoxMessage.logs.length);
		}

		NotificationManager.Instance.sendNotification(NotificationEvent.LUCKBLINDBOX_INFO_UPDATE);
	}
	 ...
	 * @returns
	 */
	private compareArrays(historyRewardArray: LuckBlindBoxRewardItemMessage[], currentRewardArray: LuckBlindBoxRewardItemMessage[]): boolean {
