...
export class PlayerInfo extends SimplePlayerInfo {
  ...
  protected static LAST_FREESKILL_LEARN_TIME: string =
    "LAST_FREESKILL_LEARN_TIME";
  public static get WEARY_MAX(): number {
    let cost: number = 200;
    let temp: ConfigInfosTempInfo =
      TempleteManager.Instance.getConfigInfoByConfigName("Player_weary");
    if (temp) {
  ...
  public static get WEARY_GET_MAX(): number {
    let cost: number = 600;
    let temp: ConfigInfosTempInfo =
      TempleteManager.Instance.getConfigInfoByConfigName("Weary_Limit");
    if (temp) {
      cost = parseInt(temp.ConfigValue);
    }
    return cost;
  ...
  public get isPassOpen(): boolean {
    let temp: ConfigInfosTempInfo =
      TempleteManager.Instance.getConfigInfoByConfigName("passcheck_open_time");
    if (temp) {
  ...
  public set noviceProcess(value: number) {
    ...
    if (
      this._noviceProcess == GlobalConfig.NEWBIE_11300 &&
      value == GlobalConfig.NEWBIE_11400
    ) {
      SDKManager.Instance.getChannel().adjustEvent(RPT_EVENT.FIRST_KILL);
    } else if (
      this._noviceProcess == GlobalConfig.NEWBIE_13500 &&
      value == GlobalConfig.NEWBIE_14000
    ) {
      SDKManager.Instance.getChannel().adjustEvent(RPT_EVENT.TASK_RESCUE_GIRL);
    } else if (
      this._noviceProcess == GlobalConfig.NEWBIE_16000 &&
      value == GlobalConfig.NEWBIE_16500
    ) {
      SDKManager.Instance.getChannel().adjustEvent(RPT_EVENT.CLICK_WEAPON);
    } else if (
      this._noviceProcess == GlobalConfig.NEWBIE_17500 &&
      value == GlobalConfig.NEWBIE_18000
    ) {
      SDKManager.Instance.getChannel().adjustEvent(RPT_EVENT.TASK_KILL_DEMONS);
    } else if (
      this._noviceProcess == GlobalConfig.NEWBIE_19000 &&
      value == GlobalConfig.NEWBIE_20000
    ) {
      SDKManager.Instance.getChannel().adjustEvent(RPT_EVENT.TASK_GET_TOWN);
    } else if (
      this._noviceProcess == GlobalConfig.NEWBIE_22000 &&
      value == GlobalConfig.NEWBIE_22500
    ) {
      SDKManager.Instance.getChannel().adjustEvent(
        RPT_EVENT.TASK_RECRUIT_GUNMEN,
      );
    } else if (
      this._noviceProcess == GlobalConfig.NEWBIE_22800 &&
      value == GlobalConfig.NEWBIE_23000
    ) {
      SDKManager.Instance.getChannel().adjustEvent(RPT_EVENT.TASK_GET_GOLDMINE);
    } else if (
      this._noviceProcess == GlobalConfig.NEWBIE_24000 &&
      value == GlobalConfig.NEWBIE_25500
    ) {
      SDKManager.Instance.getChannel().adjustEvent(RPT_EVENT.TASK_KILL_NEST);
    } else if (
      this._noviceProcess == GlobalConfig.NEWBIE_33000 &&
      value == GlobalConfig.NEWBIE_34000
    ) {
      SDKManager.Instance.getChannel().adjustEvent(RPT_EVENT.CLICK_JOIN_CLUB);
    } else if (
      this._noviceProcess == GlobalConfig.NEWBIE_35400 &&
      value == GlobalConfig.NEWBIE_35600
    ) {
  ...
  public getConsortiaSkills(): Array<t_s_consortialevelData> {
    ...
    if (this._consortiaPower > 0) {
      skillTemp = TempleteManager.Instance.getConsortiaTempleteByTypeAndLevel(
        ConsortiaUpgradeType.ATTACK,
        this._consortiaPower,
      );
      arr.push(skillTemp);
    }
    if (this._consortiaAgility > 0) {
      skillTemp = TempleteManager.Instance.getConsortiaTempleteByTypeAndLevel(
        ConsortiaUpgradeType.AGILITY,
        this._consortiaAgility,
      );
      arr.push(skillTemp);
    }
    if (this._consortiaIntellect > 0) {
      skillTemp = TempleteManager.Instance.getConsortiaTempleteByTypeAndLevel(
        ConsortiaUpgradeType.ABILITY,
        this._consortiaIntellect,
      );
      arr.push(skillTemp);
    }
    if (this._consortiaPhysique > 0) {
      skillTemp = TempleteManager.Instance.getConsortiaTempleteByTypeAndLevel(
        ConsortiaUpgradeType.PHYSIQUE,
        this._consortiaPhysique,
      );
      arr.push(skillTemp);
    }
    if (this._consortiaCaptain > 0) {
      skillTemp = TempleteManager.Instance.getConsortiaTempleteByTypeAndLevel(
        ConsortiaUpgradeType.CAPTAIN,
        this._consortiaCaptain,
      );
      arr.push(skillTemp);
    }
    if (this._consortiaGold > 0) {
      skillTemp = TempleteManager.Instance.getConsortiaTempleteByTypeAndLevel(
        ConsortiaUpgradeType.GOLD,
        this._consortiaGold,
      );
      arr.push(skillTemp);
    }
  ...
  public commit() {
    ...
    if (
      this._changeObj[PlayerInfo.VEHICLE_SCORE] ||
      this._changeObj[PlayerInfo.VEHICLE_LIST]
    ) {
  ...
  public get vehicleGrade(): number {
    ...
    let arr: t_s_upgradetemplateData[] =
      TempleteManager.Instance.getTemplatesByType(24);
