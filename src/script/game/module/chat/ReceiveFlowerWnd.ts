...
export default class ReceiveFlowerWnd extends BaseWindow {
    ...
    public OnInitWind() 
    {
        ...
        let goods:t_s_itemtemplateData = TempleteManager.Instance.getGoodsTemplatesByTempleteId(this.templateId);
        if(goods){
        ...
        let name: string = "[color=" + GoodsHelp.getGoodColorString(goods.Profile) + "][" + presentRose + "][/color]";
        this.txt_desc.text = '['+this.senderName+'] ' + LangManager.Instance.GetTranslation('simplegame.roses.RosePresentBackView.presentShowContent',name);
    }
    ...
    onTalk(){
        ...
        FriendManager.getInstance().addEventListener(FriendUpdateEvent.FRIEND_CHANGE, this.getSnsInfo,this);
        PlayerManager.Instance.sendRequestSimpleAndSnsInfo(this._userId, RequestInfoRientation.IM_INFO);
        this.onClose();
    }

    private getSnsInfo(snsInfo){
        FriendManager.getInstance().removeEventListener(FriendUpdateEvent.FRIEND_CHANGE, this.getSnsInfo,this);
