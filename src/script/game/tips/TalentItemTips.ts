...
export class TalentItemTips extends BaseTips {
    ...
    public OnInitWind() {
        ...
        this.txt_cost.text = LangManager.Instance.GetTranslation('talent.gradeCondition2',1);
    ...
    private showTalentGrade(){
        this.selectTalentItem.getChild('imgBg').visible = this.txt_cold.visible = false;
        this.contentPane.getController('c1').selectedIndex = 1;
        this.txt_name.text = LangManager.Instance.GetTranslation('FightSkillEdit.txt9');
        this.txt_name.color =this.skillName2.color = '#ffdc57';
        this.skillEffectRichText.text = LangManager.Instance.GetTranslation('talent.levelupDesc');
        this.txt_level.text = LangManager.Instance.GetTranslation("public.level3",this.thane.talentData.talentGrade);

        let gradeInfo: t_s_upgradetemplateData = TempleteManager.Instance.getTemplateByTypeAndLevel(this.thane.talentData.talentGrade + 1, 21);
        ...
        if (gradeInfo && !ismax) {
            ...
            this.nextSkillEffectRichText.text = LangManager.Instance.GetTranslation('talent.levelupDesc');
            this.item2.getChild('imgBg').visible = false;
            this.skillName2.text = LangManager.Instance.GetTranslation('FightSkillEdit.txt9');
            this.txt_lv1.text = LangManager.Instance.GetTranslation("public.level3",this.thane.talentData.talentGrade);
            this.txt_lv2.text = LangManager.Instance.GetTranslation("public.level3",(this.thane.talentData.talentGrade+1));

            this.upCostGoldNumber.text = FormularySets.toStringSelf(this._nextLevelNeedCoin, GeniusPanel.STEP);
            this.upCostExpNumber.text = FormularySets.toStringSelf(this._nextLevelExp, GeniusPanel.STEP);
            ...
            if (ResourceManager.Instance.gold.count < this._nextLevelNeedCoin || this.thane.gp < this._nextLevelExp)
            ...
            this.upCostGoldNumber.text = LangManager.Instance.GetTranslation("public.defaultnumber");
            this.upCostExpNumber.text = LangManager.Instance.GetTranslation("public.defaultnumber");
            this.upCostGoldNumber.color = '#FF2E2E';
            this.upCostExpNumber.color = '#FF2E2E';
            this.upgradeBtn.enabled = false;
            this.studyBox.visible = false;
    ...
    private showCurSkillInfo(info:SkillInfo){
        this.selectTalentItem.icon = IconFactory.getTecIconByIcon(info.templateInfo.Icons);
        this.txt_name.text = info.templateInfo.SkillTemplateName;
        this.txt_level.text = LangManager.Instance.GetTranslation("public.level3",info.templateInfo.Grades);
        this.skillEffectRichText.text = info.templateInfo.SkillDescription;
        if (info.templateInfo.PropCoolDown > 0) {
            this.txt_cold.text = LangManager.Instance.GetTranslation("yishi.view.tips.goods.SkillTips.cooldown01", info.templateInfo.PropCoolDown * 0.001);
        }
    }
    ...
    private showNextSkillInfo(next:t_s_skilltemplateData){
        ...
        this.txt_lv2.text = LangManager.Instance.GetTranslation("public.level3",next.Grades);
        if (next.PropCoolDown > 0) {
            this.nextSkillCold.text = LangManager.Instance.GetTranslation("yishi.view.tips.goods.SkillTips.cooldown01", next.PropCoolDown * 0.001);
        }
        if (Number(next.Cost) < 0) {
            this.nextSkillCost.text = LangManager.Instance.GetTranslation("yishi.view.tips.goods.SkillTips.cooldown02", Math.abs(Number(next.Cost)));
        }
        ...
        if (targetData instanceof SkillInfo) {
            ...
            if (info.grade == 0) {
                if (nextTemp.PropCoolDown > 0) {
                    this.txt_cold.text = LangManager.Instance.GetTranslation("yishi.view.tips.goods.SkillTips.cooldown01", nextTemp.PropCoolDown * 0.001);
                }
                this.txt_level.text = '';
                ...
                if (condition.length){
                    ...
                    this.studyBox.visible = false;
                    this.levelupBox.visible = true
                    this.upgradeBtn.title = LangManager.Instance.GetTranslation('armyII.viewII.skill.btnStudy');
                    this.studyBtn.enabled = true;
                } 
                this.txt1.text = LangManager.Instance.GetTranslation('ConsortiaSkillTowerWnd.n70')
                this.nextBox.visible = false;
            } else if (nextTemp) {
                this.txt_lv1.text = LangManager.Instance.GetTranslation("public.level3", info.templateInfo.Grades);
                this.showNextSkillInfo(nextTemp);          
                condition = info.checkTalentUpgradeCondition(nextTemp)
                condition.forEach(str => {
                    this.upgradeLimit.text += str + '<br>';
                });
                if (condition.length){//升级条件不足的是时候用studybox
                    //升级条件
                    this.txt1.text = LangManager.Instance.GetTranslation('yishi.view.tips.TechnologyTip.nextGrade.text')+':';
                ...
                if(this.upgradeBtn.title == LangManager.Instance.GetTranslation('armyII.viewII.skill.btnStudy')){
                    this.upgradeBtn.title = LangManager.Instance.GetTranslation('armyII.viewII.skill.btnUpgrade');  
            ...
            if(this.thane.talentData.talentPoint <= 0){
                this.txt_cost.text = LangManager.Instance.GetTranslation('talent.talentPoint');
                this.txt_cost.color = '#FF2E2E';
            }
          
            this.upgradeBtn.enabled = this.checkCondition(targetData) && condition.length == 0;
            ...
            this.txt_level.text = LangManager.Instance.GetTranslation("public.level3", temp.Grades);
            this.txt_name.text = temp.SkillTemplateName;
            // this.talentMethod.text = this.getSkillType(temp);
            if (temp.PropCoolDown > 0) {
                this.txt_cold.text = LangManager.Instance.GetTranslation("yishi.view.tips.goods.SkillTips.cooldown01", temp.PropCoolDown * 0.001);
            }
    ...
    public checkCondition(talentdata): boolean {
        ...
        if (_canAdd && talentdata.grade > 0) {//已经激活的
            _canAdd = true;
        }
        return _canAdd;
    }
    ...
    private checkCanAdd(talentdata): boolean {
        if (!talentdata.nextTemplateInfo)
            return false;
        if (this.thane.talentData.talentPoint <= 0)
            return false;
        if (talentdata.checkTalentUpgradeCondition(talentdata.nextTemplateInfo).length > 0)
            return false;
        return true
    }
     ...
     */
     private refreshData() {
        ...
        var gradeInfo: t_s_upgradetemplateData = TempleteManager.Instance.getTemplateByTypeAndLevel(this.thane.talentData.talentGrade + 1, 21);
        if (gradeInfo) {
            this._nextLevelNeedCoin = gradeInfo.TemplateId;
            this._nextLevelExp = gradeInfo.Data;
        ...
        if (gradeInfo)
            this.txt_lv2.text = (this.thane.talentData.talentGrade + 1).toString();//下一天赋等级
        else
            this.txt_lv1.text = LangManager.Instance.GetTranslation("public.defaultnumber");
    }
    ...
    private addEvent() {
        ...
        NotificationManager.Instance.addEventListener(TalentEvent.TALENT_UPGRADE, this.__talenChangeHanler, this);
    ...
    removeEvent() {
        ...
        NotificationManager.Instance.removeEventListener(TalentEvent.TALENT_UPGRADE, this.__talenChangeHanler, this);
    ...
    private levelupGrade(){
        ...
        if (ResourceManager.Instance.gold.count < this._nextLevelNeedCoin) {
            MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("armyII.viewII.talent.TalentGradeUpFailed01"));
            return;
        }

        if (this.thane.gp < this._nextLevelExp) {
            MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("armyII.viewII.talent.TalentGradeUpFailed02"));
            return;
        }

        ArmyManager.Instance.addEventListener(TalentEvent.TALENT_GRADUP_SUCC, this.__talentGradeUpSucc, this);
        ...
        if(this.params[0] == 1){
           this.levelupGrade();
            ...
            if(this.selectTalentData && this.selectTalentData.nextTemplateInfo){
                ArmySocketOutManager.sendAddSkillpoint(this.selectTalentData.nextTemplateInfo.TemplateId, 1);   
            }
        }
    }
     ...
     */
     protected __talentGradeUpSucc(event: TalentEvent) {
        ArmyManager.Instance.removeEventListener(TalentEvent.TALENT_GRADUP_SUCC, this.__talentGradeUpSucc, this);
        this.showTalentGrade();
    }
