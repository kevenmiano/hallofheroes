...
import { AiEvents, AvatarInfoUIEvent, NotificationEvent, PhysicsEvent } from "../../../../constant/event/NotificationEvent";
...
export class NpcAvatarView extends HeroAvatarView implements IBaseMouseEvent {
    ...
    public avatarBaseViewType: eAvatarBaseViewType = eAvatarBaseViewType.CampaignNpc;
    ...
    private setNameColor() {
        ...
        var selfArmy: CampaignArmy = CampaignManager.Instance.mapModel.selfMemberData;
        if (selfArmy) {
        ...
        } else if (WorldBossHelper.checkGvg(mapId) && this._nodeInfo.info.types == PosType.BOMBER_MAN) {
            // if (cInfo.param1 == 0) {
            //     nameColor = 1
            // }
    ...
    private initSpeed() {
        if (this._nodeInfo && this.info) {
            ...
            else if (WorldBossHelper.checkConsortiaDemon(this.mapModel.mapId))  //公会魔神祭坛
            {
    ...
    protected refreshAvatarView() {
        ...
        if (cInfo.info.types != PosType.ROBOT) {
            ...
            url = PathManager.getAvatarResourcePath(cInfo.sonType.toString(), -1, 1, AvatarPosition.BODY, -1, AvatarResourceType.NPC);
            ...
            if (WorldBossHelper.checkCrystal(this.mapModel.mapId) && cInfo.info.types == PosType.COPY_NPC) {
                //采集本 蒂娜
            ...
            url = PathManager.getAvatarResourcePath(bodyAvatar, obj.sex, 1, AvatarPosition.BODY, -1, AvatarResourceType.PLAYER_ARMY);
            args = this.createResourceLoadInfo(url,
                AvatarStaticData.getBaseNumByType(AvatarActionType.STAND, AvatarPosition.BODY),
                AvatarStaticData.getBaseNumByType(AvatarActionType.WALK, AvatarPosition.BODY),
                AvatarPosition.BODY);
            ...
            url = PathManager.getAvatarResourcePath(armyAvatar, obj.sex, 1, AvatarPosition.ARMY, -1, AvatarResourceType.PLAYER_ARMY);
            args = this.createResourceLoadInfo(url,
                AvatarStaticData.getBaseNumByType(AvatarActionType.STAND, AvatarPosition.ARMY),
                AvatarStaticData.getBaseNumByType(AvatarActionType.WALK, AvatarPosition.ARMY),
                AvatarPosition.ARMY);
            ResRefCountManager.loadRes(url, this.loaderCompleteHandler.bind(this), null, Laya.Loader.ATLAS, LoaderPriority.Priority_7, null, null, null, null, args);

            url = PathManager.getAvatarResourcePath(JobType.getDefaultHairUpByJob(job), obj.sex, 1, AvatarPosition.HAIR_UP, -1, AvatarResourceType.PLAYER_ARMY);
            args = this.createResourceLoadInfo(url,
                AvatarStaticData.getBaseNumByType(AvatarActionType.STAND, AvatarPosition.HAIR_UP),
                AvatarStaticData.getBaseNumByType(AvatarActionType.WALK, AvatarPosition.HAIR_UP),
                AvatarPosition.HAIR_UP);
            ResRefCountManager.loadRes(url, this.loaderCompleteHandler.bind(this), null, Laya.Loader.ATLAS, LoaderPriority.Priority_7, null, null, null, null, args);

            url = PathManager.getAvatarResourcePath(JobType.getDefaultHairDownByJob(job), obj.sex, 1, AvatarPosition.HAIR_DOWN, -1, AvatarResourceType.PLAYER_ARMY);
            args = this.createResourceLoadInfo(url,
                AvatarStaticData.getBaseNumByType(AvatarActionType.STAND, AvatarPosition.HAIR_DOWN),
                AvatarStaticData.getBaseNumByType(AvatarActionType.WALK, AvatarPosition.HAIR_DOWN),
                AvatarPosition.HAIR_DOWN);
            ResRefCountManager.loadRes(url, this.loaderCompleteHandler.bind(this), null, Laya.Loader.ATLAS, LoaderPriority.Priority_7, null, null, null, null, args);

            url = PathManager.getAvatarResourcePath(bodyAvatar + "_cloak", obj.sex, 1, AvatarPosition.CLOAK, -1, AvatarResourceType.PLAYER_ARMY);
            args = this.createResourceLoadInfo(url,
                AvatarStaticData.getBaseNumByType(AvatarActionType.STAND, AvatarPosition.CLOAK),
                AvatarStaticData.getBaseNumByType(AvatarActionType.WALK, AvatarPosition.CLOAK),
    ...
    private initMediator() {
        ...
        var selfArmay: CampaignArmy = CampaignManager.Instance.mapModel.selfMemberData;
        var b: boolean = false;
        ...
        else if (!b && this._nodeInfo.info.types != PosType.BOMBER_MAN && this._nodeInfo.info.types != PosType.COLLECTION) {//战场同team的npc不需要
        ...
        this._mediatorKey = MediatorMananger.Instance.registerMediatorList(arr, this, NpcAvatarView.NAME);
    }
    ...
    protected addEvent() {
        ...
        if (this._nodeInfo) {
            if (this._nodeInfo.info) {
                this._nodeInfo.info.addEventListener(PhysicsEvent.UP_STATE, this.__updateNodeStateHandler, this);
            }
            this._nodeInfo.addEventListener(PhysicsEvent.UP_SONTYPE, this.__updateAvatarHandler, this);
            this._nodeInfo.addEventListener(PhysicsEvent.FOLLOW_TARGET, this.__followTargetHandler, this);
            this._nodeInfo.addEventListener(PhysicsEvent.CHAT_DATA, this.__chatDataHandler, this);
            this._nodeInfo.addEventListener(PhysicsEvent.UPDATE_CUR_HP, this.__updateCurHpHandler, this);
            this._nodeInfo.addEventListener(PhysicsEvent.MOVE_POS, this.__movePosHandler, this);
        }
        if (this.info) {
            this.info.addEventListener(AiEvents.UPDATE_NEXT_POINT, this.__nextPointHandler, this);
        }
    ...
    protected removeEvent() {
        ...
        if (this._nodeInfo) {
            if (this._nodeInfo.info) {
                this._nodeInfo.info.removeEventListener(PhysicsEvent.UP_STATE, this.__updateNodeStateHandler, this);
            }
            this._nodeInfo.removeEventListener(PhysicsEvent.UP_SONTYPE, this.__updateAvatarHandler, this);
            this._nodeInfo.removeEventListener(PhysicsEvent.FOLLOW_TARGET, this.__followTargetHandler, this);
            this._nodeInfo.removeEventListener(PhysicsEvent.CHAT_DATA, this.__chatDataHandler, this);
            this._nodeInfo.removeEventListener(PhysicsEvent.UPDATE_CUR_HP, this.__updateCurHpHandler, this);
            this._nodeInfo.removeEventListener(PhysicsEvent.MOVE_POS, this.__movePosHandler, this);
        }
        if (this.info) {
            this.info.removeEventListener(AiEvents.UPDATE_NEXT_POINT, this.__nextPointHandler, this);
        }
    ...
    private __followTargetHandler(evt: PhysicsEvent) {
        if ((<CampaignNode>this._nodeInfo).followTarget) {
            MediatorMananger.Instance.addRegisterMediator(FollowTargetMediator, this, this._mediatorKey);
            MediatorMananger.Instance.removeRegisterMediator(NpcAttackAndChaseMediator, this, this._mediatorKey);
        }
        else {
            MediatorMananger.Instance.removeRegisterMediator(FollowTargetMediator, this, this._mediatorKey);
        }
    }
    ...
    public execute() {
        if (this.info && this.info.isLiving) {
            ...
            if (!WorldBossHelper.checkConsortiaDemon(mapId)) {
                ...
                if (this._info && this._info.pathInfo && this._info.pathInfo.length > this._info.walkIndex) {
                    this._isStand = false;
                    if (this.info.currentFrame >= this.info.totalFrame) {
                        ...
                        var offTime: number = PlayerManager.Instance.currentPlayerModel.sysCurTimeBySecond * 1000 - (<CampaignNode>this._nodeInfo).createTime;
                        var unitTime: number;
                        ...
                        if ((new Laya.Point(this.x, this.y)).distance(curPoint.x * 20, curPoint.y * 20) > 150) {
    ...
    private __updateNodeStateHandler(evt: PhysicsEvent) {
        ...
        switch (this._nodeInfo.info.state) {
            ...
            case NodeState.NONE:
                ...
                this.visible = false;
                Logger.info("[NpcAvatarView]不可见状态", this.objName, this._nodeInfo.info.state)
                break;
            case NodeState.FIGHTING:
                ...
                this.visible = true;
                this.setFireView();
                Logger.info("[NpcAvatarView]可见状态 交战中", this.objName, this._nodeInfo.info.state)
                break;
            default:
                this._info.isLiving = true;
                this.visible = true;
                Logger.info("[NpcAvatarView]可见状态", this.objName, this._nodeInfo.info.state)
        }
    ...
    private setFireView() {
        if (!this._attackView) {
            this._attackView = new SimpleAvatarView(110, 110, PathManager.fightStatePath, 10);
        }
    ...
    private set displayVisible(value: boolean) {
        AvatarInfoUILayerHandler.handle_CON_VISIBLE(this._uuid, AvatarInfoTag.ConsortiaName, value)
    ...
    private layouTextView(key: string) {
        ...
        NotificationManager.Instance.sendNotification(NotificationEvent.NPC_LOAD_COMPLETE, this);
    }
    ...
    public mouseClickHandler(evt: Laya.Event): boolean {
        ...
        CampaignManager.Instance.mapModel.selectNode = this._nodeInfo as CampaignNode;//设置选中节点, 用于在enterframe中的检测
        if (CampaignManager.Instance.mapModel.chckeNpcFriendState(this._nodeInfo as CampaignNode)) {
            return false;
        }
    ...
    public attackFunEx(): boolean {
        ...
        CampaignManager.Instance.mapModel.selectNode = this._nodeInfo as CampaignNode;
        this.attackFun();
    }
    ...
    public attackFun(): boolean {
        ...
        if (this._nodeInfo.info.types == PosType.BOMBER_MAN) {
            ...
            NotificationManager.Instance.dispatchEvent(NotificationEvent.LOCK_PVP_WARFIGHT, this._nodeInfo);
            return true;
        }
        ...
        if (this._nodeInfo.info.state == NodeState.FIGHTING) {
            ...
            if (this._nodeInfo.info.types == PosType.COLLECTION) {
                //pet
                NotificationManager.Instance.sendNotification(NotificationEvent.CHASE_NPC, this.nodeInfo);
                return true;
            }
        }
        ...
        var attackPoint = NpcAttackHelper.getAttackPoint(<CampaignNode>this.nodeInfo, new Laya.Point(aView.x, aView.y), new Laya.Point(this.x, this.y));
    ...
    public mouseMoveHandler(evt: Laya.Event): boolean {
        ...
        if (CampaignManager.Instance.mapModel.chckeNpcFriendState(<CampaignNode>this._nodeInfo)) {
            return canAttack;
        }
        if (this._avatar) {
            var teamId: number = (<CampaignNode>this._nodeInfo).param1;
            if (this._nodeInfo.info.types != PosType.COPY_NPC_HANDLER && teamId > 0 && this.mapModel.selfMemberData.teamId == teamId) {
                return canAttack;
            }
            canAttack = true;
            if (this._nodeInfo.info.types == PosType.COPY_NPC_HANDLER) {//资源回收官
                if (teamId > 0 && this.mapModel.selfMemberData.teamId == teamId) {
                    canAttack = false;
                }
                else {
