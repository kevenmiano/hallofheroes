...
export default class UIHelper {
    ...
    static closeWindows(emWindows: EmWindow[] = [], reverse: boolean = true, dispose: boolean = true) {
        if (emWindows.length == 0) {
            emWindows = [EmWindow.SpaceTaskInfoWnd, EmWindow.Home, EmWindow.LevelUp, EmWindow.LogWnd, EmWindow.AutoWalkWnd, EmWindow.Battle];
        }

        let emLayerArr: EmLayer[] = [EmLayer.STAGE_TOOLTIP_LAYER, EmLayer.GAME_UI_LAYER, EmLayer.GAME_DYNAMIC_LAYER, EmLayer.GAME_DYNAMIC_LAYER2]
