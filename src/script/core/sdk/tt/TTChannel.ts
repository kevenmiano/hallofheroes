...
export default class TTChannel extends BaseChannel {
  constructor(id: number) {
    ...
    if (tt.createRewardedVideoAd) {
      ...
      Logger.log(" TTChannel createRewardedVideoAd   is null ", tt);
    }
  ...
  checkForUpdate(callback: ResultCallback) {
    ...
    if (tt.getUpdateManager) {
      ...
      Logger.log("getUpdateManager", updateManager);
      updateManager.onCheckForUpdate((res) => {
        // 请求完新版本信息的回调
        Logger.log("onCheckForUpdate", res.hasUpdate);
        if (res.hasUpdate) {
          tt.showToast({
            title: "即将有更新请留意",
          });
