...
export default class OuterCityMapPlayerTips extends BaseWindow {
  ...
  private initData() {
    ...
    this.descTxt2.text = LangManager.Instance.GetTranslation(
      "cell.view.starbag.StarBagCell.level",
    );
    this.descTxt3.text = LangManager.Instance.GetTranslation(
      "sort.view.MemberTitleView.ap",
    );
    ...
    this.fightValueTxt.text =
      this._baseArmy.baseHero.fightingCapacity.toString();
  }
  ...
  private infoBtnHandler() {
    this._mapView.motionTo(
      new Laya.Point(
        this._baseArmy.armyView.x - StageReferance.stageWidth / 2,
        this._baseArmy.armyView.y - StageReferance.stageHeight / 2,
      ),
    );
  ...
  private gotoBtnHandler() {
    ...
    NotificationManager.Instance.dispatchEvent(
      ...
      new Laya.Point(this._baseArmy.armyView.x, this._baseArmy.armyView.y),
    );
    NotificationManager.Instance.dispatchEvent(
      NotificationEvent.CLOSE_OUTERCITY_MAP_WND,
    );
