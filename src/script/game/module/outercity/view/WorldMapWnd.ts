...
export class WorldMapWnd extends BaseWindow {
    ...
    private __mouseClickHandler(mapId: number, e: Laya.Event) {
        ...
        if (this.checkCurrentMapId(mapId)) {
            ...
            let content: string = LangManager.Instance.GetTranslation("worldmap.view.WorldMapFrame.content2");
            SimpleAlertHelper.Instance.Show(SimpleAlertHelper.SIMPLE_ALERT, null, prompt, content, confirm, cancel, this.__exitSocketHandler2.bind(this));
        }
        else {
            let confirm: string = LangManager.Instance.GetTranslation("public.confirm");
            let cancel: string = LangManager.Instance.GetTranslation("public.cancel");
            let prompt: string = LangManager.Instance.GetTranslation("public.prompt");
            let content: string = LangManager.Instance.GetTranslation("worldmap.view.WorldMapFrame.content");
            SimpleAlertHelper.Instance.Show(SimpleAlertHelper.SIMPLE_ALERT, null, prompt, content, confirm, cancel, this.__exitSocketHandler.bind(this));
        }
    }
    ...
    private __exitSocketHandler(boolean: boolean, flag: boolean): void {
        if (boolean) {
            ...
            if (this.__mapId == PlayerManager.Instance.currentPlayerModel.mapNodeInfo.info.mapId) {
                let str: string = LangManager.Instance.GetTranslation("worldmap.view.WorldMapFrame.command01");
                MessageTipManager.Instance.show(str);
            }
            else {
                let ctrl: WorldMapCtrl = FrameCtrlManager.Instance.getCtrl(EmWindow.WorldMapWnd) as WorldMapCtrl
    ...
    private checkArmyState(): boolean {
        ...
        if (this.currentScene == SceneType.OUTER_CITY_SCENE) {
            if (OuterCityManager.Instance.controler.selfArmy && OuterCityManager.Instance.controler.selfArmy.armyView) {
                let armyView: Object = OuterCityManager.Instance.controler.selfArmy.armyView;
                if (armyView.hasOwnProperty("isStand")) {
                    if (armyView["isStand"]) {
                        return true;
                    }
                    else {
                        MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("worldmap.view.WorldMapFrame.walking"));
                        return false;
                    }
    ...
    public checkScene(): boolean {
        ...
        if (this.currentScene == SceneType.CAMPAIGN_MAP_SCENE) {
            ...
            if (WorldBossHelper.checkGvg(mapId)) {
                str = LangManager.Instance.GetTranslation("worldboss.helper.WorldBossHelper.tip07");
            }
            else if (WorldBossHelper.checkConsortiaSecretLand(mapId)) {
                str = LangManager.Instance.GetTranslation("worldboss.helper.WorldBossHelper.tip08");
            }
            else if (WorldBossHelper.checkHoodRoom(mapId)) {
                str = LangManager.Instance.GetTranslation("worldmap.view.WorldMapFrame.command04");
            }
            else {
                str = LangManager.Instance.GetTranslation("worldmap.view.WorldMapFrame.command02");
            }
            MessageTipManager.Instance.show(str);
            return false;
        }
        else if (this.currentScene == SceneType.PVE_ROOM_SCENE || this.currentScene == SceneType.PVP_ROOM_SCENE || this.currentScene == SceneType.WARLORDS_ROOM) {
            str = LangManager.Instance.GetTranslation("worldmap.view.WorldMapFrame.command03");
            MessageTipManager.Instance.show(str);
            return false;
        }
        return true;
