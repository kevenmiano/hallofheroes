...
export default class PetEuipTrainWnd extends BaseWindow {
  ...
  private initLanguage() {
    this.frame.getChild("title").text =
      LangManager.Instance.GetTranslation("petEuip.title");
    this.btn_stren.title = LangManager.Instance.GetTranslation(
      "HigherGradeOpenTipView.content4",
    );
    this.btn_stren1.title =
      LangManager.Instance.GetTranslation("petEuip.stren");
    this.tabAttr.title = LangManager.Instance.GetTranslation("petEuip.stren");
    this.txt_1.text = LangManager.Instance.GetTranslation(
      "fashion.view.compose.composeRate",
    );
  ...
  OnShowWind() {
    ...
    if (this._goodsInfo.templateInfo.Property2) {
      this._dropitemDatas = TempleteManager.Instance.getDropItemssByDropId(
        this._goodsInfo.templateInfo.Property2,
      );
  ...
  updateView() {
    ...
    let cfg: t_s_petequipstrengthenData =
      TempleteManager.Instance.getPetEquipStrenData(
        this._goodsInfo.strengthenGrade + 1,
      );
    if (cfg && !this.isMax) {
      this.txt_percent.text = Math.round(cfg.Probability / 100) + "%";
      this.cashCondition =
        PlayerManager.Instance.currentPlayerModel.playerInfo
          .petEquipStrengNum >= cfg.StrengthenConsume;
      this.goldCondition =
        ResourceManager.Instance.gold.count >= cfg.StrengthenGold;
      ...
      this.btn_stren.enabled = this.btn_stren1.enabled =
        this.cashCondition && this.goldCondition;
    } else {
      ...
      this.txt_gold.text =
        ...
        "[/color]";
      this.txt_cash.text =
        PlayerManager.Instance.currentPlayerModel.playerInfo.petEquipStrengNum +
        "/" +
        "[color=#ff0000]" +
        0 +
        "[/color]";
  ...
  private updateAttr(masterAttr: string, sonAttr: string) {
    ...
    let level = this._goodsInfo.strengthenGrade;
    if (!this.qualityCfg) {
      this.qualityCfg = TempleteManager.Instance.getpetequipqualityData(
        this._goodsInfo.templateInfo.Profile,
      );
    }
    if (this.qualityCfg) {
        ...
        if (element.length > 0) {
          ...
          if (cfg) {
            ...
            if (this.isMax) {
              ...
              attrItem.getChild("txt_attr_val1").text =
                "+" + Utils.toFixedNum(Number(arr[1]) + Number(arr[2]), 3);
          ...
          if (element.length > 0) {
            ...
            if (cfg) {
              ...
              if (this.isMax) {
                ...
                if (
                  (this._goodsInfo.strengthenGrade + 1) %
                    PetData.PETEQUIP_SONATTR_RANDOM_UP ==
                    0 &&
                  isUnlockAll
                ) {
                  attrItem.getChild("txt_attr_val1").visible = false;
      ...
      for (let j = 0; j < lvArr.length; j++) {
        ...
        if (lv > 0) {
          ...
          if (!isUnlock) {
            ...
            if (item) {
              let txt =
                this["item" + attr_num].getChild("txt_lock").asTextField;
              txt.color = color;
    ...
    if (this.isMax) {
      ...
      this.btn_stren.enabled = this.btn_stren1.enabled = false;
      this.t1.text =
        LangManager.Instance.GetTranslation("faterotary.LevelMax") + "!";
    }
  }
  ...
  private btn_strenClick() {
    ...
    if (!this.cashCondition || !this.goldCondition) {
      MessageTipManager.Instance.show(
        LangManager.Instance.GetTranslation(
          "singlepass.bugle.SinglePassBugleView.NoLeftCount",
        ),
      );
      return;
    }
    ...
    this.strenAni.setPlaySettings(
      0,
      -1,
      1,
      -1,
      Laya.Handler.create(this, this.onAniEnd),
    );
    let petId = this._goodsInfo.petData
      ? this._goodsInfo.petData.petId
      : this._goodsInfo.objectId;
    PetCtrl.reqPetEquipStren(
      petId,
  ...
  private showTxtTip() {
    if (!this._recvMsg) {
      //动画播放完成后 没收到强化结果 loading提示
      this.mc_loading.visible = true;
      return;
    }
    let loader = new fairygui.GLoader();
    let iconUrl = this.strenResult
      ? this.img_ok.content
      : this.img_fail.content;
    ...
    Laya.Tween.to(
      ...
      500,
      Laya.Ease.backOut,
      Laya.Handler.create(this, this.hideTip, [loader]),
    );
  }
  ...
  hideTip(loader: fairygui.GLoader) {
    ...
    this.btn_stren.enabled = this.btn_stren1.enabled =
      this.cashCondition && this.goldCondition && !this.isMax;
  ...
  private helpBtnClick() {
    ...
    let content: string =
      LangManager.Instance.GetTranslation("PetFrame.help08");
