...
export class MainToolBarBtnActionHandler {
  ...
  play(anitime: number = 0.34) {
    ...
    let showRow1Btns = this.target.getBtnListByLocaltionType(
      EmMainToolBarBtnLocationType.Row1,
    );
    let showRow2Btns = this.target.getBtnListByLocaltionType(
      EmMainToolBarBtnLocationType.Row2,
    );
    let showCowBtns = this.target.getBtnListByLocaltionType(
    ...
    for (let index = 0; index < showRow1Btns.length; index++) {
      ...
      tempPos = basePos;
      if (bFold) {
        tempPos = this.target.model.getBtnPos(
          basePos,
          btnData.locationType,
          index,
        );
      }
      if (nodelay) {
