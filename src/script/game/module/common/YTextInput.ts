...
export class YTextInput extends FUI_YTextInput {
  ...
  public set focus(b: boolean) {
    if (this.isIOSNewVersion()) {
      if (b) this.txt_mobile.requestFocus();
      else (this.txt_mobile.displayObject as Laya.Input).focus = false;
    } else {
      if (b) this.txt_web.requestFocus();
      else (this.txt_web.displayObject as Laya.Input).focus = false;
    }
  }
    ...
    if (Utils.isIOS()) {
      ...
      Logger.yyz(
        "当前App版本号: " +
          versionName +
          "   数字号:" +
          Utils.getAppNumByName(versionName),
      );
      return (
        Utils.getAppNumByName(versionName) >= Utils.getAppNumByName("1.1.5")
      );
    } else {
      return false;
    }
