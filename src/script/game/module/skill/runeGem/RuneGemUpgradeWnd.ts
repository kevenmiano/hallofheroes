...
export default class RuneGemUpgradeWnd extends BaseWindow {
  ...
  onInitView() {
    this.frame.getChild("title").text =
      LangManager.Instance.GetTranslation("runeGem.str15");
    ...
    let arr = TempleteManager.Instance.getTemplatesByTypeAndId(
      200,
      this.currRuneData.templateInfo.SonType,
    );
  ...
  private __upgradeBtnClickHandler(evt) {
    ...
    this.control.reqRuneGemUpgrade(
      this.currRuneData.pos,
      this.currRuneData.id,
      value,
      this.currRuneData.bagType,
      RunesPanel.curRuneId,
    );
    // this.updateView();
  }
  ...
  setRunInfo(value: GoodsInfo) {
    ...
    if (next) {
      for (let p in next) {
        (i == 0 ? this.txt_attri_val1 : this.txt_attri_val12).text =
          next[p] + "";
        (i == 0 ? this.txt_add : this.txt_add2).text =
          `(+${i == 0 ? Number(this.txt_attri_val1.text) - Number(this.txt_attri_val0.text) : Number(this.txt_attri_val12.text) - Number(this.txt_attri_val02.text)})`;
        i++;
      }
    }
    ...
    let levlTmep: t_s_upgradetemplateData =
      TempleteManager.Instance.getTemplateByTypeAndLevelAndSonType(
        value.strengthenGrade == 0 ? 1 : value.strengthenGrade,
        200,
        value.templateInfo.SonType,
      );
    let nextlevlTmep: t_s_upgradetemplateData =
      TempleteManager.Instance.getTemplateByTypeAndLevelAndSonType(
        value.strengthenGrade + 1,
        200,
        value.templateInfo.SonType,
      );
    if (nextlevlTmep) {
      ...
      //最高级
      this.txt_level1.text = LangManager.Instance.GetTranslation(
  ...
  private updateRuneNum() {
    this.txt_own.text =
      PlayerManager.Instance.currentPlayerModel.playerInfo.runeNum + "";
  }

  private isMax: boolean = false;

  protected addPropertyTxt(
    info: t_s_runegemData,
    next: t_s_runegemData,
  ): string {
    if (!next) {
