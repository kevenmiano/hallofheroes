...
export default class VoiceMsgCell extends FUI_VoiceMsgCell {
    ...
    private onPlayVoice(){
        ...
        IMManager.Instance.updateVoiceStatus(this._cellData.uid,this._cellData.serverId);
        ...
        if(channel instanceof AndroidWebviewChannel){
            ...
            let chatData:ChatData = NativeChannel.msgHash.get(this._cellData.serverId);
    ...
    private __playerClickHandler(evt) {
        if (this._cellData) {
            ...
            NotificationManager.Instance.sendNotification(ChatEvent.PLAYER_NAME_CLICK, ret);
        }
    }
    ...
    set chatData(value) {
        if (value) {
            ...
            //系统 或 信息 栏
            if (value.uid != 0) {//发送者非系统
                let isSelf = value.uid == PlayerManager.Instance.currentPlayerModel.userInfo.userId;
             //紫钻
                if (isSelf || value.isFromMe) {//自己
                    this.chatCahnnelImg0.url = FUIHelper.getItemURL('Base', ChatChannel.getChatChannelIcon(value.channel));
                    ...
                    this.redpoint.x = this.msgbgself.x - this.msgbgself.width-this.redpoint.width/2;
                    this.redpoint.visible = false;
                } else {//其他人
                    this.chatCahnnelImg1.url = FUIHelper.getItemURL('Base', ChatChannel.getChatChannelIcon(value.channel));
                    this.userName1.color = ChatChannel.getChatChannelColor(value.channel);
                    this.userName1.text = value.senderName;
                    if(value.vipGrade <= 0 && value.channel == ChatChannel.BUBBLETYPE_TARGET){
                    ...
                    this.redpoint.x = this.img_voicebgL.x + this.img_voicebgL.width + this.redpoint.width/2-3;
                }
                if (value.userLevel) {
                ...
                this.headIcon.icon = IconFactory.getPlayerIcon(_headId, IconType.HEAD_ICON);
                ...
                if (this._cellData.appellId == 0) {
                    ...
                    let appellData = AppellManager.Instance.model.getAppellInfo(this._cellData.appellId);
                    if (appellData && appellData.PerfixLang != "" && appellData.PerfixLang != undefined) {
                        this.appellTxt.text = appellData.PerfixLang
                        this.appellTxt.color = AppellModel.getTextColorAB(appellData.TextColorIdx)
                ...
                }else
                {
                    ...
                    if(this._cellData.curTime){
                        this.txt_curTime.text = DateFormatter.transDateToMin(this._cellData.curTime);
                    }else
                    {
    ...
    private getAppellUrl(appellId: number): string {
        return "asset.chatII.Appell" + AppellManager.Instance.model.getPerfixStyle(appellId);
    }
