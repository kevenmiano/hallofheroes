...
export default class UIManager extends GameEventDispatcher implements IManager {
    ...
    constructor() {
        ...
        NotificationManager.Instance.addEventListener(GlobalEvent.POP_VIEW, this.onPopViewHandler, this);
    }
     ...
     */
    public ShowWind(type: EmWindow, param = null, immediately: boolean = true): Promise<any> {
        return new Promise<any>(resolve => {
            ...
            LoadingUIMgr.Instance.Show(type, uiInfo.packName, completeFunc, null, this.loadFailed(type, true), uiInfo.ShowLoading == false);
        });
    }
    ...
    private onPopViewHandler(target) {
        ...
        if (wind != null) {
            ...
            if (!this.isSingleInstanceWnd(type)) {
                ...
                // 资源释放
                Laya.timer.once(100, this, this.releaseModuleRes, [type, pkgList], false);
            }
        }
    }
    ...
    public get isShowSpaceTaskInfoWndScene() {
        ...
        if (SceneManager.Instance.currentType == SceneType.CAMPAIGN_MAP_SCENE) {
            ...
            return !((WorldBossHelper.checkMonopoly(model.mapId)
            ...
            for (let tempPackName of this.pkgWindowMaps.keys()) {
                ...
                for (let index = 0; index < emWindows.length; index++) {
                    ...
                    if (NewbieBaseConditionMediator.checkFrame(emWindow)) {
                        flag = true;
                        Logger.info("释放资源" + srcPackName + "失败, 已经打开引用该资源的模块" + emWindow)
                        break;
                    }
                    if (FrameCtrlManager.Instance.curOpeningModuleMap.get(emWindow)) {
                        flag = true;
                        Logger.info("释放资源" + srcPackName + "失败, 正在打开引用该资源的模块" + emWindow)
                        break;
                    }
                }
                if (flag) {
