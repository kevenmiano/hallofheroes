...
export default class RunesPanel extends FUI_Runes_Panel {
  ...
  private updateEditMode() {
    ...
    for (let i = 0; i < this.equipRunes.numChildren; i++) {
      const skillItem: FastRuneItem = this.equipRunes.getChildAt(
        i,
      ) as FastRuneItem;
  ...
  private initEvent() {
    this.equipRunes.itemRenderer = Laya.Handler.create(
      this,
      this.renderFastRunesListItem,
      null,
      false,
    );
    this.runesList.itemRenderer = Laya.Handler.create(
      this,
      this.renderRunesListItem,
      null,
      false,
    );
    ...
    this.runesList.on(
      ...
      this,
      this.__onRuleItemSelect,
    );
    this.equipRunes.on(
      fairygui.Events.CLICK_ITEM,
      this,
      this.__onFastRuleItemSelect,
    );
  ...
  private initRuneKeyList() {
    var keyList: Array<any> =
      ArmyManager.Instance.thane.runeCate.runeScript.split(",");
    var isLock: boolean = true;
    this._runeKeyList = [];
    for (var i: number = 0; i < 3; i++) {
      isLock = !(ArmyManager.Instance.thane.grades >= 35 + i * 5);
      if (!isLock && i < keyList.length) {
        var temp: t_s_runetemplateData =
          TempleteManager.Instance.getRuneTemplateByTid(keyList[i]);
        if (temp) {
          var info: RuneInfo =
            ArmyManager.Instance.thane.runeCate.getInfoByIndex(temp.RuneIndex);
  ...
  private onReplace(selfDragData?, dropTarget?) {
    let fastItem: FastRuneItem = this.checkRuneEquiped(
