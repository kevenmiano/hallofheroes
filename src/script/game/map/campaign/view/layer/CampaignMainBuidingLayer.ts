...
export class CampaignMainBuidingLayer extends Laya.Sprite {
  ...
  public static NAME: string =
    "map.campaign.view.layer.CampaignMainBuidingLayer";
  ...
  private init() {
    if (this.mapView && !this.mapView.stepRender) {
      if (
        this.inMonopoly &&
        MonopolyManager.Instance.model &&
        this._model.mapNodesData
      ) {
  ...
  private initMonopolyNodes(arr: any) {
    if (arr) {
      ...
      let nodeInfos: Array<MonopolyNodeInfo> =
        MonopolyManager.Instance.model.nodeInfos;
  ...
  public addNpc(node: CampaignNode) {
    ...
    if (node.info.types == PosType.FALL_CHEST) {
      ...
      let walkable: boolean = this._model.getWalkable(
        node.info.posX,
        node.info.posY,
      );
      if (!walkable) {
        let newPos: Laya.Point =
          CampaignArmyViewHelper.getNodeStationPointImp(node);
    ...
    if (
      node.info.types == PosType.COPY_LOGO ||
      node.info.types == PosType.STATIC
    ) {
  ...
  private createItemTips(
    $info: PhysicInfo,
    node: CampaignNode,
  ): MultipleDictionaryInfo {
    ...
    if ($info.types == PosType.COPY_NPC || $info.types == PosType.COPY_END) {
      let str: string = LangManager.Instance.GetTranslation(
        "yishi.view.PlayerMenu.playerLevel",
      );
