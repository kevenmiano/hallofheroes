...
export default class RuneHoleGem extends FUI_RuneHoleGem {
  ...
  updateRuneGem() {
    if (this.isunload) {
      ...
      let hole = this.getChild(
        "rune_hole_" + (this.curHoleIndex + 1),
      ) as FUI_RuneHole;
      ...
      for (let i = 0; i < this._bagData.length; i++) {
        ...
        if (gem) {
          ...
          if (goodsInfo.strengthenGrade == 0) {
            this.addPropertyTxt1(
              goodsInfo.templateInfo.Property1,
              goodsInfo,
              index + 1,
            );
          } else {
            let curTemp: t_s_runegemData =
              TempleteManager.Instance.getRuneGemCfgByTypeAndLevel(
                goodsInfo.templateInfo.SonType,
                goodsInfo.strengthenGrade,
                goodsInfo.templateInfo.Property1,
              );
            if (curTemp) {
    ...
    for (let i = 0; i < holeIdArr.length; i++) {
      ...
      if (holeData) {
        for (let j = 0; j < holeData.Length; j++) {
          ...
          this["attri_" + index]
            .getChild("holeType")
            .asCom.getControllerAt(0)
            .setSelectedIndex(shapeId - 1);
          this["attri_" + index].getChild("txt_name").text =
            LangManager.Instance.GetTranslation("runeGem.strType" + shapeId);
