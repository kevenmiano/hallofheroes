 ...
 * @description 累计充值天数
 */
export default class CumulativeRechargeDayView
  extends FUI_CumulativeRechargeDayView
  implements FunnyContent
{
  ...
  initView() {
    if (
      !this._infoData ||
      this._infoData.type != FunnyType.TYPE_CUMULATIVE_RECHARGE_DAY
    )
      return;
    this._remainTime =
    ...
    this.describeText.text = LangManager.Instance.GetTranslation(
      "CumulativeRechargeDayView.desContent",
    );
    this.setTimeText();
    let _itemList = [];
    ...
    for (
      ...
      if (!hasSelected) {
        if (
          ((this._infoData.bagList[j].status == 1 ||
            this._infoData.bagList[j].status == 2) &&
            this._infoData.bagList[j].param1 == "1") ||
  ...
  initEvent() {
    ...
    this.dayslist.itemRenderer = Laya.Handler.create(
      this,
      this.renderBoxList,
      null,
      false,
  ...
  removeEvent() {
    ...
    this.dayslist &&
      this.dayslist.off(fairygui.Events.CLICK_ITEM, this, this.__onItemSelect);
  ...
  private onShowItemReards(data: FunnyBagData) {
    ...
    if (data) {
      ...
      for (var i: number = 0; i < data.rewardList.length; i++) {
        if (data.rewardList[i].temType == 1) {
          ...
          ginfo.strengthenGrade =
            data.rewardList[i].strengthenGrade <= 0
              ? 1
              : data.rewardList[i].strengthenGrade;
      ...
      switch (
        data.status //领取状态(1: 可领取, 2: 已领取, 3: 不能领取)
      ) {
        case 1: //可领取
  ...
  private setTimeText() {
    ...
    this.activityTime.text =
      LangManager.Instance.GetTranslation("giftbag.GiftBagItem.activeTime") +
      DateFormatter.timeFormat3(startDate, ".") +
      "-" +
      DateFormatter.timeFormat3(endDate, ".");
    this.setRemainTime();
  }
  ...
  public setRemainTime() {
    ...
    if (this._infoData.startTime > this.playerModel.nowDate) {
      this.remainTImes.text =
        LangManager.Instance.GetTranslation("public.unopen") +
        LangManager.Instance.GetTranslation(
          "funny.FunnyRightView.active.timeText",
          DateFormatter.transDate(this._infoData.startTime),
          DateFormatter.transDate(this._infoData.endTime),
        );
    }
  }
