...
export class ArmyState {
  ...
  public static checkCampaignAttack(state: number): boolean {
    if (state == ArmyState.BATTLE_PENDING || state == ArmyState.STATE_FIGHT)
      return false;
    return true;
  }
  ...
  public static moveState(state: number): boolean {
    if (state == ArmyState.RECALLING || state == ArmyState.STATE_MOVING)
      return true;
    return false;
  }
  ...
  public static checkAttackRange(p1: Laya.Point, p2: Laya.Point): boolean {
    if (p1.distance(p2.x, p2.y) > GameUIConfig.ATTACK_RANGE) {
      var str: string = LangManager.Instance.GetTranslation(
        "map.internals.constant.ArmyState.command03",
      );
