...
export default class QuestionWnd extends BaseWindow {
    ...
    public OnInitWind() {
        super.OnInitWind();
        this.setCenter();
        this.frame.getChild('title').text = LangManager.Instance.GetTranslation("questionnaire.QuestionnaireFrame.titleText");
        this.addEvent();
        this.onInitListData();
    }

    private addEvent() {
        this.questList.itemProvider = Laya.Handler.create(this, this.getListItemResource, null, false);
        this.questList.itemRenderer = Laya.Handler.create(this, this.renderListItem, null, false);
        NotificationManager.Instance.addEventListener(QuestionEvent.SCROLL_TO_NEXT, this.scrollToNext, this);
        NotificationManager.Instance.addEventListener(QuestionEvent.FINISH_QUESTION, this.finishQuestion, this);
        NotificationManager.Instance.addEventListener(QuestionEvent.FOCUS, this.disableScroll, this);
        NotificationManager.Instance.addEventListener(QuestionEvent.BLUR, this.enableScroll, this);
    }
    ...
    private offEvent() {
        ...
        NotificationManager.Instance.removeEventListener(QuestionEvent.SCROLL_TO_NEXT, this.scrollToNext, this);
        NotificationManager.Instance.removeEventListener(QuestionEvent.FINISH_QUESTION, this.finishQuestion, this);
        NotificationManager.Instance.removeEventListener(QuestionEvent.FOCUS, this.disableScroll, this);
        NotificationManager.Instance.removeEventListener(QuestionEvent.BLUR, this.enableScroll, this);
    }
    ...
    private finishQuestion() {
        ...
        if (!ret) {
            ...
            let withoutAns = LangManager.Instance.GetTranslation("questionnaire.QuestionnaireFrame.withoutAns", index + 1);//答题未完成
            MessageTipManager.Instance.show(withoutAns);
            this.questList.scrollToView(index, true);
            return;
        }
        let finishExit = LangManager.Instance.GetTranslation("questionnaire.QuestionnaireFrame.finishExit");//答题完成退出
        ...
        SimpleAlertHelper.Instance.Show(SimpleAlertHelper.SIMPLE_ALERT, null, prompt, finishExit, confirm1, cancel1, this.alertFinishCB.bind(this), AlertBtnType.O);
    }

    private alertFinishCB(b: boolean, flag: boolean) {
        QuestionnaireManager.Instance.sendQuestionAnswer(QuestionWnd.END_ANSWER, QuestionnaireManager.Instance.model.questionId, this.answerList);
    ...
    protected OnBtnClose() {
        ...
        if (ret) {
            ...
            let confirm1: string = LangManager.Instance.GetTranslation("public.confirm");
            let cancel1: string = LangManager.Instance.GetTranslation("public.cancel");
            let msg = LangManager.Instance.GetTranslation("questionnaire.QuestionnaireFrame.withoutExit");//答题未完成退出
            SimpleAlertHelper.Instance.Show(SimpleAlertHelper.SIMPLE_ALERT, null, prompt, msg, confirm1, cancel1, (b: boolean, flag: boolean) => {
                if (b) {
                    super.OnBtnClose();
                }
            }, AlertBtnType.OC);
