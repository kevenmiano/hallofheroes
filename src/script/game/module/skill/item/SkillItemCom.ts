 ...
 * @description ***
 */
export default class SkillItemCom
  extends FUI_SkillItemCom
  implements DragObject, ITipedDisplay
{
  ...
  checkIsMasterySkill(skillInfo) {
    return (
      skillInfo.templateInfo.MasterType >= 41 &&
      skillInfo.templateInfo.MasterType <= 44
    );
  }
  ...
  private moveSkill(info: SkillInfo) {
    ...
    if (
      this.thane.skillCate.fastKey.indexOf(
        this._data.templateInfo.SonType.toString(),
      ) > 0
    )
      return;
    if (this.thane.fastKeyLength >= SkillPanel.SKILL_NUM) return;
    let point = this.parent.localToGlobal(this.x, this.y);
    NotificationManager.Instance.sendNotification(SkillEvent.FAST_KEY_MOVE, {
      data: this._data,
      point: point,
    });
  }
  ...
  setDragState(isdrag: boolean = true) {
    if (
      this._data &&
      this._data.templateInfo.UseWay != 2 &&
      this._data.grade != 0
    ) {
  ...
  private refreshView() {
    ...
    if (!this._isFastSkill) {
      Logger.log(
        "this._data.templateInfo.TemplateNameLang",
        this._data.templateInfo.TemplateNameLang,
      );
  ...
  public checkCondition() {
    ...
    if (this.checkCanAdd() && this._data.grade > 0) {
      //可加点
      UIFilter.normal(this._skillIcon.displayObject);
  ...
  private checkCanAdd(): boolean {
    ...
    if (this.isMasterySkill) {
      ...
      if (this._data.grade == 0) {
        let canStudy =
          jobLevel > 0 && jobLevel >= this._data.templateInfo.NeedPlayerGrade;
        return canStudy;
      } else {
        let canUpgrade =
          jobLevel >= this._data.nextTemplateInfo.NeedPlayerGrade;
        let isgoldEnough =
          ResourceManager.Instance.gold.count >=
          this._data.templateInfo.Parameter3;
        return canUpgrade && isgoldEnough;
      }
    }
    if (this.thane.skillCate.skillPoint <= 0) return false;
    if (
      this._data.checkUpgradeCondition(this._data.nextTemplateInfo).length > 0
    )
      return false;
    return true;
  }
  ...
  public set vdata(value: SkillInfo) {
    ...
    this.isMasterySkill =
      value.templateInfo.MasterType >= 41 &&
      value.templateInfo.MasterType <= 44;
    this.visible = true;
    this.refreshView();
  }
  ...
  private checkEquipState() {
    ...
    for (var i: number = 0; i < SkillPanel.SKILL_NUM; i++) {
      if (i < keyList.length) {
        if (keyList[i] != "-1") {
          var temp: SkillInfo = this.thane.getSkillBySontype(
            Number(keyList[i]),
          );
          if (!temp) {
