...
export default class FightingPetItem extends FUI_FightingPetItem {
  ...
  set info(value: object) {
    ...
    if (value["index"] == 3) {
      //英灵技能
      this.gotoBtn.title = LangManager.Instance.GetTranslation(
        "FightingPetItem.gotoBtn.titleTxt",
      );
    } else {
      this.gotoBtn.title = LangManager.Instance.GetTranslation(
  ...
  private setText() {
    var getIndex: number = FightingManager.Instance.getIndexByScore(
      this._info["integral"],
    );
    this.typeNameTxt.text = LangManager.Instance.GetTranslation(
      "FightingPetItem.typeNameTxt" + this._info["index"],
    );
    switch (this._info["index"]) {
      case 1: //品质
        this.descTxt.text = LangManager.Instance.GetTranslation(
          "fighting.FightingItem.desction10_" + this._info["integral"],
        );
        break;
      case 2: //资质
        this.descTxt.text = LangManager.Instance.GetTranslation(
          "fighting.FightingItem.desction11_" + getIndex,
        );
        break;
      case 3: //技能
        if (getIndex == 4) {
