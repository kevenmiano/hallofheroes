...
export class PveSecretFBItem extends FUI_PveSecretFBItem {
    ...
    set info(value: t_s_secretData) {
        this._info = value;
        if (value) {
            this.title = LangManager.Instance.GetTranslation("public.format.concat2word", value.TemplateNameLang, LangManager.Instance.GetTranslation("public.level3", value.NeedGrade));
