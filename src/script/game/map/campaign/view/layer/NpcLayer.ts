...
export class NpcLayer implements IEnterFrame {
  ...
  public addToStage(evt: Event) {
    ...
    this._model.addEventListener(
      CampaignEvent.MOVE_NPC,
      this.__npcMoveHandler,
      this,
    );
    this._model.addEventListener(
      CampaignEvent.NPC_CHASE,
    ...
    this._model.addEventListener(
      OuterCityEvent.REMOVE_NODE,
      this.__removeNpcHandler,
      this,
    );
  }
  ...
  private __npcMoveHandler(data: com.road.yishi.proto.campaign.NPCMoveMsg) {
    ...
    if (npc && npc.npcAiInfo) {
      ...
      npc.npcAiInfo.nextPoint = new Laya.Point(
        msg.curPosX * 20,
        msg.curPosY * 20,
      );
  ...
  public dispose() {
    MediatorMananger.Instance.unregisterMediatorList(
      this._mediatorKey,
      this._container,
    );
  ...
  public mouseMoveHandler(evt: Laya.Event): boolean {
    ...
    if (evt.target instanceof NpcAvatarView) {
      ...
      if (
        mc.nodeInfo &&
        mc.nodeInfo.info &&
        !PosType.checkNeutralState(mc.nodeInfo.info.types)
      ) {
