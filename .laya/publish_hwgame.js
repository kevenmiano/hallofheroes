...
let commandSuffix, adbPath, opensslPath, layarepublicPath;
let isDevices = false;
...
gulp.task("generateSign_HW", ["clearTempDir_HW"], function () {
  ...
  return new Promise((resolve, reject) => {
    ...
    cp.on("close", (code) => {
      console.log(`子进程退出码：${code}`);
      // 签名是否生成成功
      let privatePem = path.join(projDir, "private.pem"),
        certificatePem = path.join(projDir, "certificate.pem");
      let isSignExits =
        fs.existsSync(privatePem) && fs.existsSync(certificatePem);
      if (!isSignExits) {
...
gulp.task("copySignFile_HW", ["generateSign_HW"], function () {
  // debug签名拷贝，默认拷贝
  let privatePem = path.join(toolkitPath, "sign", "debug", "private.pem"),
  ...
  let signDest = `${projDir}/sign/debug`;
  ...
  if (config.hwSign.generateSign) {
    ...
    let xiaomiDest = `${projDir}/sign/release`,
      layaDest = `${workSpaceDir}/sign/release`;
    let stream = gulp.src([privatePem, certificatePem]);
    return stream.pipe(gulp.dest(xiaomiDest)).pipe(gulp.dest(layaDest));
  } else if (config.hwInfo.useReleaseSign && !config.hwSign.generateSign) {
    // 使用release签名，并且没有重新生成
    // 从项目中将签名拷贝到华为快游戏项目中
    let privatePem = path.join(workSpaceDir, "sign", "release", "private.pem"),
      certificatePem = path.join(
        workSpaceDir,
        "sign",
        "release",
        "certificate.pem"
...
gulp.task("modifyFile_HW", ["deleteSignFile_HW"], function () {
  ...
  let indexJsStr =
    versionCon && versionCon["index.js"] ? versionCon["index.js"] : "index.js";
  // 华为项目，修改game.js
...
gulp.task("modifyMinJs_HW", ["modifyFile_HW"], function () {
  ...
  if (!config.useMinJsLibs) {
    // 默认保留了平台文件，如果同时取消使用min类库，就会出现文件引用不正确的问题
    content = content.replace(
      /min\/laya(-[\w\d]+)?\.hwmini\.min\.js/gm,
      "laya.hwmini.js"
    );
  } else {
    content = content.replace(
      /(min\/)?laya(-[\w\d]+)?\.hwmini(\.min)?\.js/gm,
      "min/laya.hwmini.min.js"
    );
  }
...
gulp.task("version_HW", ["modifyMinJs_HW"], function () {
  ...
  if (config.version) {
    ...
    return gulp.src(srcList).pipe(revCollector()).pipe(gulp.dest(projDir));
  }
});
...
gulp.task("buildRPK_HW", ["version_HW"], function () {
  ...
  let signtoolPath = path.join(toolkitPath, "index.js"),
    releasePath = projDir,
...
gulp.task("updateAPK_HW", ["getDevices_HW"], function (cb) {
  ...
  new Promise((resolve, reject) => {
    ...
    .then(() => {
      return new Promise((resolve, reject) => {
        if (
          remoteAPKVer === localAPKVer ||
          donotUpdate(remoteAPKVer, localAPKVer)
        ) {
  ...
  return new Promise((resolve, reject) => {
    ...
    .then(() => {
      return new Promise((resolve, reject) => {
        ...
        let rpkFilePath = path.join(
          projDir,
          "dist",
          `${config.hwInfo.package}.rpk`
        );
        let cmd = `${adbPath}`;
    ...
    .then(() => {
      return new Promise((resolve, reject) => {
        ...
        let args = [
          ...
          "--es",
          "rpkpath",
          `file://data/local/tmp/${config.hwInfo.package}.rpk`,
          "--ei",
          "debugmode",
          "1",
          "--activity-clear-top",
          "com.huawei.fastapp.dev/com.huawei.fastapp.app.RpkRunnerActivity",
        ];
        let opts = {
          cwd: projDir,
          shell: true,
...
function donotUpdate(remoteAPKVer, localAPKVer) {
  ...
  let l1n = Number(localAPKVerN[1]), // local first number
