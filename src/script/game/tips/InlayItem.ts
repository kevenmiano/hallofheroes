...
export class InlayItem extends FUI_InlayItem
{
    ...
    {
        this.ld_icon.url = fgui.UIPackage.getItemURL("Base", this._resArr[value == -1 ? 9 : value == 0 ? 10 : this.getColorByType(value)]);
        this.txt.color = this._txtColors[value == -1 ? 1 : value == 0 ? 2 : 3];
        this.txt.text = value == -1 ? LangManager.Instance.GetTranslation("yishi.view.tips.goods.InlayItem.value01") :
            value == 0 ? LangManager.Instance.GetTranslation("yishi.view.tips.goods.InlayItem.value02") : this.getDiscription(value, objectId);
    }
    ...
    private getDiscription(value:number, objectId:number):string
    {
        ...
        {
            let addValue:number = Math.floor((temp.totalAttribute * GoodsHelp.getJewelEffecyByGrade(this.thane.jewelGrades)) / 100);
            if(addValue == 0)
            {
