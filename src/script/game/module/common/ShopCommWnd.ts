...
export default class ShopCommWnd extends BaseWindow {
  ...
  OnShowWind() {
    ...
    if (this.frameData) {
      ...
      if (this._shopType == ShopModel.STAR_SHOP) {
        ...
        this.frame.getChild("title").text = LangManager.Instance.GetTranslation(
          "consortia.view.myConsortia.building.ConsortiaShopFrame.titleText",
          lv,
        );
      } else if (this._shopType == ShopGoodsInfo.WARLORDS_SHOP) {
        this.frame.getChild("title").text = LangManager.Instance.GetTranslation(
          "view.subshop.WarlordsShopFrame.str01",
        );
  ...
  private __refreshGoods() {
    if (
      ...
      if (item) this.onClickItem(item as ShopItem);
    } else {
  ...
  updateBagPage() {
    this.PageNumTxt.text =
      this.ItemList.scrollPane.currentPageX +
      1 +
      "/" +
      Math.max(Math.ceil(this.ItemList.numItems / 8), 1);
  }
  ...
  private updateBuyLimitText() {
    ...
    if (this._selectedItemData.WeeklyLimit > -1) {
      ...
      this.limitCountTxt.text =
        leftCount.toString() + "/" + this._selectedItemData.WeeklyLimit;
    }
  }
  ...
  private checkExist(tId: number): string {
    ...
    var exist: WildSoulInfo = MountsManager.Instance.avatarList.getWildSoulInfo(
      goods.Property1,
    );
