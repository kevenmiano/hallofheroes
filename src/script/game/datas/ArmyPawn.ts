...
export class ArmyPawn extends GameEventDispatcher {
    ...
    public get live(): number {
        return this.attributeDic[ArmyPawn.LIVE].getTotalJoin(this.templateInfo.Live);
    }
    ...
    public get attack(): number {
        return (this.attributeDic[ArmyPawn.ATTACK] as PawnProperty).getTotalJoin(this.templateInfo.Attack);
    }
    ...
    public get defence(): number {
        return (this.attributeDic[ArmyPawn.DEFENCE] as PawnProperty).getTotalJoin(this.templateInfo.Defence);
    }
    ...
    public get magicAttack(): number {
        return (this.attributeDic[ArmyPawn.MAGIC_ATTACK] as PawnProperty).getTotalJoin(this.templateInfo.MagicAttack);
    }
    ...
    public get magicDefence(): number {
        return (this.attributeDic[ArmyPawn.MAGIC_DEFENCE] as PawnProperty).getTotalJoin(this.templateInfo.MagicDefence);
    }
    ...
    public get forceHit(): number {
        return (this.attributeDic[ArmyPawn.FORCEHIT] as PawnProperty).getTotalJoin(this.templateInfo.ForceHit);
    }
    ...
    public set templateId(value: number) {
        if (value != this._templateId) {
            ...
            this._templeteInfo = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_pawntemplate, this._templateId.toString());
            if (this.templateInfo)
                this._nexttempleteInfo = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_pawntemplate, this.templateInfo.NextLevelTemplateId.toString());
            this._changeObj[ArmyPawn.PROPERTY_CHANGE] = true;
        }
    }
    ...
    public get pawnEvaluate(): number {
        return Number(Math.pow(this.live * (this.attack + this.defence + this.magicDefence * 1.5) * (1 + this.magicAttack / 500), 0.45));
    }
