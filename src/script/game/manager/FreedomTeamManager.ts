...
import { FreedomTeamEvent, SpaceEvent } from "../constant/event/NotificationEvent";
...
export default class FreedomTeamManager {
    ...
    public setup() {
        ServerDataManager.listen(S2CProtocol.U_C_TEAM_CREATE, this, this.__teamCreateHandler);
        ServerDataManager.listen(S2CProtocol.U_C_TEAM_DISMISS, this, this.__teamDismissHandler);
        ServerDataManager.listen(S2CProtocol.U_C_TEAM_ADD_MEMBER, this, this.__teamAddMemberHandler);
        ServerDataManager.listen(S2CProtocol.U_C_TEAM_PLAYER_INFO, this, this.__teamPlayerInfoHandler);
        ServerDataManager.listen(S2CProtocol.U_C_TEAM_INVITE, this, this.__teamInviteHandler);
        ServerDataManager.listen(S2CProtocol.U_C_TEAM_LEAVE_FOR_NOW, this, this.__teamAFKHandler);
        ServerDataManager.listen(S2CProtocol.U_C_TEAM_CAPTAIN_CHANGE, this, this.__teamCaptainChangeHandler);
        ServerDataManager.listen(S2CProtocol.U_C_TEAM_MEMBER_LEFT, this, this.__teamMemberLeftHandler);
        ServerDataManager.listen(S2CProtocol.U_C_TEAM_KICK_MEMBER, this, this.__teamKickMemberHandler);
        ServerDataManager.listen(S2CProtocol.U_C_TEAM_FIGHT_POS_CHANGE, this, this.__teamFightPosChangeHandler);
    }
    ...
    private __teamCreateHandler(pkg: PackageIn) {
        ...
        NotificationManager.Instance.dispatchEvent(FreedomTeamEvent.TEAM_INFO_UPDATE, null);
        // 队伍相关操作去掉人物显示隐藏
    ...
    private __teamPlayerInfoHandler(pkg: PackageIn) {
        ...
        NotificationManager.Instance.dispatchEvent(FreedomTeamEvent.TEAM_INFO_SYNC, null);
    }
    ...
    private __teamInviteHandler(pkg: PackageIn) {
        ...
        if (this.checkScene()) {
            var content: string = LangManager.Instance.GetTranslation("managers.FreedomTeamManager.teamInviteTxt", msg.inviterName);
            PopFrameCheck.Instance.addAction(new FreedomTeamApplyAction(msg.inviterId, content));
        }
    }
    ...
    private checkScene(showTips: boolean = false): boolean {
        ...
        switch (currentType) {
            case SceneType.WARLORDS_ROOM:
                if (showTips) {
                    MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("managers.FreedomTeamManager.checkSceneTipsTxt01"));
                }
                return false;
                break;
        }
    ...
    private readMember(player:TeamPlayerInfoMsg) {
        ...
        var hInfo: ThaneInfo;
        if (player.playerId == PlayerManager.Instance.currentPlayerModel.playerInfo.userId) {
        ...
        if (player.hasOwnProperty('fightingCapacity')) {
            this._model.memberFightCapacity[player.playerId] = player.fightingCapacity;
        }
        if (player.hasOwnProperty('onlineState')) {
    ...
    public memeberGradesValid(min:number): boolean
		{
			if(this.hasTeam)
			{
                for (const member in this._model.allMembers) {
                    if (Object.prototype.hasOwnProperty.call(this._model.allMembers, member)) {
    ...
    {
        if(this.hasTeam && this._model.captainId == PlayerManager.Instance.currentPlayerModel.playerInfo.userId)
        {
            return true;
        }
        return false;
