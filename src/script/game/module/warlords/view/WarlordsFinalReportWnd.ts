...
export default class WarlordsFinalReportWnd extends BaseWindow {
    ...
    private initData() {
        if (this._info) {
            ...
            let displayRank: number = WarlordsModel.getDisplayRank(this._info["rank"]);
            this.myRank.setVar("rank", displayRank.toString()).flushVars();
            if (displayRank >= 1 && displayRank <= 16) {
                this.titleTxt.text = LangManager.Instance.GetTranslation("warlords.WarlordsBetSelectFrame.temple") + "-" + LangManager.Instance.GetTranslation("warlords.WarlordsFinalReportWnd.title");
            } else if (displayRank <= 100) {
                this.titleTxt.text = LangManager.Instance.GetTranslation("warlords.WarlordsBetSelectFrame.brave") + "-" + LangManager.Instance.GetTranslation("warlords.WarlordsFinalReportWnd.title");
            } else {
                this.titleTxt.text = LangManager.Instance.GetTranslation("warlords.WarlordsFinalReportWnd.title");
            }
            if (!StringHelper.isNullOrEmpty(mount)) {
                this.mountTxt.text = mount + "(" + LangManager.Instance.GetTranslation("consortia.view.myConsortia.ConsortiaMemberItem.dayTip", mountTime) + ")";
            }
            else {
                this.mountTxt.text = "";
            }
            if (!StringHelper.isNullOrEmpty(appell)) {
                this.appellTxt.text = LangManager.Instance.GetTranslation("yishi.view.frame.WarlordsFinalReportFrame.str01", appell);
            ...
            if (hasPrivilege && displayRank <= 1) {
                ...
                this.privilegeTxt.text = LangManager.Instance.GetTranslation("yishi.view.frame.WarlordsFinalReportFrame.privilegeContent");
            } else {
