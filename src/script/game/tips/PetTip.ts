...
import { PetChallengeEvent, PetChallengeState, PetTipType } from "../constant/PetDefine";
...
export class PetTip extends BaseTips {
    ...
    private get challengeState(): PetChallengeState {
        const arr = PlayerManager.Instance.currentPlayerModel.playerInfo.petChallengeFormationOfArray;
        if (this.tipData && arr) {
            let bInTeam = arr.indexOf(this.tipData.petId.toString()) > -1;
            this._challengeState = bInTeam ? PetChallengeState.Equiped : PetChallengeState.UnEquiped
        }
    ...
    protected OnClickModal() {
        super.OnClickModal();
        NotificationManager.Instance.dispatchEvent(PetChallengeEvent.RESET_SELECTED_STATE);
        this.hide();
    }
    ...
    private initView() {
        ...
        switch (this.tipType) {
            case PetTipType.PetChallenge:
                ...
                this.btnOpt.title = LangManager.Instance.GetTranslation(this.challengeState == PetChallengeState.UnEquiped ? "public.onFormation" : "public.unEquip");
                break;
            default:
        ...
        this.txt_level.text = LangManager.Instance.GetTranslation("public.level2", petData.grade);
        ...
        if (template) {
            ...
            this.imgPetType.icon = FUIHelper.getItemURL(EmPackName.Base, "Icon_PetType" + template.PetType);
        }
        for (let index = 0; index < 8; index++) {
            const skill = petData.changeSkillTemplates[index] as t_s_skilltemplateData;
    ...
    private initAttribute(info: PetData) {
        this.qualification0.txt_attributeName.text = LangManager.Instance.GetTranslation("pet.growthRate");
        this.qualification1.txt_attributeName.text = LangManager.Instance.GetTranslation("pet.strengthCoe");
        this.qualification2.txt_attributeName.text = LangManager.Instance.GetTranslation("pet.armorCoe");
        this.qualification3.txt_attributeName.text = LangManager.Instance.GetTranslation("pet.intellectCoe");
        this.qualification4.txt_attributeName.text = LangManager.Instance.GetTranslation("pet.staminaCoe");

        this.qualification0.txt_attributeValue.text = info.growthRate.toString();
        this.qualification1.txt_attributeValue.text = info.coeStrength + " / " + info.coeStrengthLimit;
        this.qualification2.txt_attributeValue.text = info.coeArmor + " / " + info.coeArmorLimit;
        this.qualification3.txt_attributeValue.text = info.coeIntellect + " / " + info.coeIntellectLimit;
        this.qualification4.txt_attributeValue.text = info.coeStamina + " / " + info.coeStaminaLimit;
    }
        ...
        switch (this.tipType) {
            case PetTipType.PetChallenge:
                ...
                if (this.challengeState == PetChallengeState.Equiped) {
                    ...
                    if (count == 1) {
                        MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("PetChallenge.unEquipPetTip"));
                    } else {
                        NotificationManager.Instance.dispatchEvent(PetChallengeEvent.READY_ON_FORMATION_STATE, { state: false, petId: this.tipData.petId });
                    }
                } else {
                    PetChallengeCtrl.bReadyOnFormation = true;
                    PetChallengeCtrl.curOptPetId = this.tipData.petId;
                    NotificationManager.Instance.dispatchEvent(PetChallengeEvent.READY_ON_FORMATION_STATE, { state: true, petId: this.tipData.petId });
                }
                break;
        }
        this.hide()
