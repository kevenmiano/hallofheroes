...
export class SinglePassWnd extends BaseWindow {
    ...
    private initData() {
        ...
        this.rewardItem = this.contentPane.getChild("rewardItem") as SinglePassRewardItem;
    ...
    private initEvent() {
        ...
        this._singlePassModel.addEventListener(SinglePassEvent.SINGLEPASS_INFO_UPDATE, this.singlePassInfoUpdateHandler, this);
        this._singlePassModel.addEventListener(SinglePassEvent.SINGLEPASS_UPDATE_REWARD, this.singlePassRewardUpdateHandler, this);
    ...
    private removeEvent() {
        ...
        this._singlePassModel.removeEventListener(SinglePassEvent.SINGLEPASS_INFO_UPDATE, this.singlePassInfoUpdateHandler, this);
        this.enterBtn.offClick(this, this.enterBtnHandler);
        this._singlePassModel.removeEventListener(SinglePassEvent.SINGLEPASS_UPDATE_REWARD, this.singlePassRewardUpdateHandler, this);
    }
    ...
    private singlePassInfoUpdateHandler() {
        ...
        if (this._singlePassModel.lastIndex > 0 && this._singlePassModel.lastIndex != this._singlePassModel.maxIndex) {
            var floor: number = parseInt(((this._singlePassModel.lastIndex - 1) / SinglePassModel.TOLLGATE_PER_FLOOR + 1).toString());
    ...
    private singlePassRewardUpdateHandler() {
        if (this._singlePassModel.areaRewardArray && this._singlePassModel.areaRewardArray.indexOf(this._singlePassModel.selectArea.toString()) != -1) {
    ...
    private changeSelectHander(index: number) {
        ...
        let delNum: number = 0;
        if (this._oldIndex > this._currentIndex)//点击左侧按钮
        {
        ...
        else if (this._oldIndex < this._currentIndex)//点击右侧按钮
        {
    ...
    private rankBtnHandler() {
        ...
        ResMgr.Instance.loadRes(PathManager.singlePassOrderPath, this.singPassOrderLoadCompleteHandler.bind(this), null, Laya.Loader.BUFFER)
    }
    ...
    private compareFunction(a: SinglePassOrderInfo, b: SinglePassOrderInfo): number {
        if (a.order > b.order) {
            return 1;
    ...
    private allocationBtnHandler() {
        var rewardCount: number = parseInt((this._singlePassModel.maxIndex / SinglePassModel.REWARD_LIMIT).toString());
        var nextRewardIndex: number = (rewardCount + 1) * SinglePassModel.REWARD_LIMIT;
        var floor: number = 0;
        var tollgate: number = 0;
        var content: string = "";
        floor = parseInt(((nextRewardIndex - 1) / SinglePassModel.TOLLGATE_PER_FLOOR + 1).toString());
        tollgate = nextRewardIndex % SinglePassModel.TOLLGATE_PER_FLOOR;
        if (tollgate == 0) {
            tollgate = SinglePassModel.TOLLGATE_PER_FLOOR;
        }
        if (this._singlePassModel.maxIndex < SinglePassModel.MAX_FLOOR * SinglePassModel.TOLLGATE_PER_FLOOR) {
            content = LangManager.Instance.GetTranslation("singlepass.SinglePassView.GetHornBtnTips02", SinglePassModel.FLOOR_STR[floor], tollgate, this._singlePassModel.bugleCount);
        } else {
            content = LangManager.Instance.GetTranslation("singlepass.SinglePassView.GetHornBtnTips03");
        }
        if (this._bugleLeftCount > 0) {
            this._singlePassModel.itemCount = this._singlePassModel.itemCount + this._bugleLeftCount;
    ...
    private helpBtnHandler() {
        ...
        let content: string = LangManager.Instance.GetTranslation("singlepass.view.SinglePassHelpFrame.helpContent");
    ...
    preLayerBtnHandler() {
        ...
        this._singlePassModel.selectIndex = SinglePassManager.Instance.getCardIndex();
        this.refreshView();
        this.scrollEffect();
    }
    ...
    nextLayerBtnHandler() {
        if (this._singlePassModel.selectArea == SinglePassModel.MAX_FLOOR) {
            MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("singlepass.order.SinglePassView.TipTxt"));
            return;
        }
    ...
    private refreshView() {
        ...
        if (this._singlePassModel.selectArea < this._singlePassModel.area) {
            ...
            this._selectAreaTollgateCount = this._singlePassModel.maxIndex - (this._singlePassModel.area - 1) * SinglePassModel.TOLLGATE_PER_FLOOR
        }
        this.frame.getChild("title").text = LangManager.Instance.GetTranslation("singlepass.order.SinglePassView.TitleTxt", SinglePassModel.FLOOR_STR[this._singlePassModel.selectArea]);
        ...
        let cardInfos: Array<SinglePassCardInfo> = this._singlePassModel.getCardInfoByFloor(this._singlePassModel.selectArea);
        let btnInfos: Array<SinglePassCardInfo> = this._singlePassModel.getCardInfoByFloor(this._singlePassModel.selectArea);
        ...
        if (this._singlePassModel.selectArea == this._singlePassModel.area) {
            if (this._singlePassModel.maxIndex == this._singlePassModel.area * SinglePassModel.TOLLGATE_PER_FLOOR) {
                this.nextLayerBtn.enabled = true;
        ...
        this._leftCount = this._singlePassModel.maxCount - this._singlePassModel.count;
        this.leftCountTxt.text = LangManager.Instance.GetTranslation("PetChallengeCDView.remainTimesLabel");
        if (this._leftCount <= 0) {
            this._leftCount = GoodsManager.Instance.getGoodsNumByTempId(SinglePassModel.PROP_TEMPLATE_ID);
            if (this._leftCount > 0) {
                this.leftCountTxt.text = LangManager.Instance.GetTranslation("singlepass.order.SinglePassView.LeftCountTxt");
            }
        }
        this.countTxt.text = this._leftCount.toString();
        var rewardCount: number = parseInt((this._singlePassModel.maxIndex / SinglePassModel.REWARD_LIMIT).toString());
        this._bugleLeftCount = rewardCount * this._singlePassModel.bugleCount - this._singlePassModel.itemCount;
        if (this._bugleLeftCount > 0) {
    ...
    private scrollEffectEnd() {
        this._currentIndex = (this.list.getFirstChildInView() + 1) % this.list.numItems;
        this.setSinglePassCardItemLight();
        this.cardTurnFunction();
        this.setBtnVisible();
    ...
    private setSinglePassCardItemLight() {
        ...
        this._currentSelectedItem = this.list.getChildAt(index) as SinglePassCardItem;
    ...
    private get maxFloor(): number {
        return this._singlePassModel.floors[this._singlePassModel.floors.length - 1];
    }
