...
interface ISceneSwitchAnimation {
  curScene(scene: BaseSceneView): void;
...
export class SceneManager extends GameEventDispatcher {
  ...
  public preSetup() {
    ...
    NotificationManager.Instance.addEventListener(
      SceneEvent.LOCK_SCENE,
      this.onSwitchLockScene,
      this,
    );
    NotificationManager.Instance.addEventListener(
      SceneEvent.HIDE_SCENE_OBJ,
      this.onHideSceneObj,
      this,
    );
  }
  ...
  public get lockScene(): boolean {
    let layer: any = LayerMgr.Instance.getLayerByType(EmLayer.GAME_TOP_LAYER);
    let layer2: any = LayerMgr.Instance.getLayerByType(
  ...
  public setScene(
    ...
    if (data && data.isDealyShowSmallLoading) data.isShowLoading = false;
    ...
    if (
      (data && data.isShowLoading) ||
      (data && !data.hasOwnProperty("isShowLoading")) ||
      !data
    )
      LoadingSceneWnd.Instance.Show();
  ...
  private createSwitchAnimationByType(type: SceneType): ISceneSwitchAnimation {
    ...
    if (type == SceneType.BATTLE_SCENE) {
      switchAnim = new EnterBattleSceneSwitchAnimation(
        this.sceneAninmationBack.bind(this),
      );
    } else {
      switchAnim = new SceneSwitchAninmation(
        this.sceneAninmationBack.bind(this),
      );
    }
  ...
  private forceSetScene(type: string) {
    this._tempType = type;
    this.lock = true;
    if (this._data && this._data.isShowLoading) LoadingSceneWnd.Instance.Show();
  ...
  private async emptyScene(scene: BaseSceneView) {
    ...
    if (this._currentScene && !this._currentScene.destroyed) {
      //避免偶现报错
  ...
  private setSceneImp(scene: BaseSceneView) {
    ...
    if (scene && scene != this._currentScene) {
      ...
      this._nextScene = scene;
      Logger.log(
        "SceneManager.setSceneImp _nextScene preLoadingStart: ",
        scene.SceneName,
      );
      if (!this._nextScene.prepared) this._nextScene.prepare();
      if (this._data && this._data.isDealyShowSmallLoading) {
        this._dealyShowSmallLoadingTime = setInterval(
          this.dealyShowSmallLoading.bind(this),
          2000,
          String(this.currentType),
  ...
  public preLoadingBack() {
    ...
    this._sceneSwitchAnimation = this.createSwitchAnimationByType(
      this._loadType,
    );
    Logger.log(
      "SceneManager.preLoadingBack",
      "nextScene:",
  ...
  public async sceneAninmationBack() {
    ...
    if (this._currentScene) {
      NotificationManager.Instance.dispatchEvent(
        NotificationEvent.BEFORE_LEAVEING_SCENE,
        true,
      );
      await this._currentScene.leaving();
    }
    ...
    await this._currentScene.enterOver();
    Laya.timer.once(this.getSceneEnabledTime, this, () => {
      this.enable = true;
    });
    ...
    Logger.log(
      "SceneManager.sceneAninmationBack场景动画完成回调:line391",
      this._sceneSwitchAnimation,
    );
    NotificationManager.Instance.dispatchEvent(
  ...
  private getSceneResources(type: string): Array<string> {
    ...
    switch (type) {
      ...
      case SceneType.VEHICLE:
        ...
        arr.push(
          ComponentSetting.getUISourcePath(EmWindow.VEHICLE_DAIMON_TRAIL),
        );
        arr.push(ComponentSetting.getUISourcePath(EmWindow.CAMPAIGN));
        break;
  ...
  protected createModel() {
    if (!this.modelMask) {
      ...
      this.modelMask.graphics.drawRect(
        0,
        0,
  ...
  removeSwitchAnimation() {
    Logger.log(
      "SceneManager.removeSwitchAnimation",
      this._sceneSwitchAnimation,
    );
    if (this._sceneSwitchAnimation) {
      this._sceneSwitchAnimation.stop();
