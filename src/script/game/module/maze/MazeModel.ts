...
export default class MazeModel extends FrameDataBase {
	...
	public initMazeShopFrame() {
		...
		let goodsList: Array<ShopGoodsInfo> = new Array();
		...
		for (const key in mazeShopTemplateDic) {
			if (Object.prototype.hasOwnProperty.call(mazeShopTemplateDic, key)) {
				...
				if (this.isJobFix(heroJob, cfgm.getTemplateByID(ConfigType.t_s_itemtemplate, gInfo.ItemId))) {
	...
	public getShopShowList(): Array<ShopGoodsInfo> {
		...
		if (endIndex > this.currentGoodsList.length) endIndex = this.currentGoodsList.length;
	...
	private mazeShopSortFunc(a: ShopGoodsInfo, b: ShopGoodsInfo): number {
		if (PlayerManager.Instance.currentPlayerModel.towerInfo2.maxIndex > 0) {
			this.curMaxIndex = PlayerManager.Instance.currentPlayerModel.towerInfo2.maxIndex;
			this.curA = a.MazeLayers2;
			this.curB = b.MazeLayers2;
		}
		else {
			this.curMaxIndex = PlayerManager.Instance.currentPlayerModel.towerInfo1.maxIndex;
			...
			if (a.Sort > 0 && b.Sort > 0) {
				...
				if (a.MazeLayers2 == b.MazeLayers2) {
					...
					if (a.MazeLayers == b.MazeLayers) {
						var at: t_s_itemtemplateData = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_itemtemplate, a.ItemId);
						var bt: t_s_itemtemplateData = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_itemtemplate, b.ItemId);
						if (!at || !bt) return 0;
