...
export default class TeamFormationItemDrag
  extends FUI_TeamFormationItemDrag
  implements DragObject
{
  ...
  public refresh() {
    if (this._hero) {
      ...
      this.icon = IconFactory.getHeadIcon(
        this._hero.headId,
        IconType.HEAD_ICON,
      );
  ...
  private onDragComplete(dstTarget, srcTarget) {
    if (dstTarget) {
      if (dstTarget instanceof TeamFormationItemDrag) {
        //目标对象存在
        //父容器为原始对象,则还原
        if (srcTarget != dstTarget) {
          ...
          if (mapModel) {
            ...
            if (dstThane) {
              dstArmy = mapModel.getBaseArmyByUserId(
                dstThane.userId,
                dstThane.serviceName,
              );
            }
            if (srcThane) {
              srcArmy = mapModel.getBaseArmyByUserId(
                srcThane.userId,
                srcThane.serviceName,
              );
            }
            if (srcArmy && !dstArmy) {
              if (srcArmy.state == ArmyState.STATE_FIGHT) {
                ...
                MessageTipManager.Instance.show(
                  LangManager.Instance.GetTranslation(
                    "TeamFormationWnd.cannotAdjustedDuringFight",
                  ),
                );
              }
            } else if (srcArmy && dstArmy) {
              if (
                srcArmy.state == ArmyState.STATE_FIGHT ||
                dstArmy.state == ArmyState.STATE_FIGHT
              ) {
                canSwap = false;
                MessageTipManager.Instance.show(
                  LangManager.Instance.GetTranslation(
