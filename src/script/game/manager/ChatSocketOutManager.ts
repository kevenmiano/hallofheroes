...
export default class ChatSocketOutManager {
  ...
  public static sendChat(chatData: ChatData) {
    ...
    let code: number;
    if (ChatSocketOutManager.initChatData(chatData, msg)) {
      //跨服战斗当前聊天
      protocolId = ChatManager.Instance.model.protocalId;
      code = C2SProtocol.CR_CHANNEL_CHAT;
      ChatManager.Instance.model.addSendProtocol(
        protocolId,
        ChatSocketOutManager.getTimer(),
      );
    } else {
      //普通聊天
      ...
      switch (sceneType) {
        ...
        case SceneType.BATTLE_SCENE:
          if (
            ...
            !CampaignManager.Instance.exit
          ) {
            if (
              CampaignManager.Instance.mapModel.isCross &&
              (ChatChannel.TEAM == chatData.channel ||
                ChatChannel.CURRENT == chatData.channel)
            ) {
              protocolId = ChatManager.Instance.model.protocalId;
  ...
  private static initChatData(
    ...
    switch (sceneType) {
      ...
      case SceneType.PVP_ROOM_SCENE:
        if (
          chatData.channel == ChatChannel.CURRENT ||
      ...
      case SceneType.VEHICLE:
        if (ChatSocketOutManager.sceneChat) {
          if (chatData.channel == ChatChannel.CURRENT) {
            msg.team = msg.team.concat(
              ChatSocketOutManager.sceneChat.currentChanelPlayerList,
            );
          } else if (chatData.channel == ChatChannel.TEAM) {
            msg.team = msg.team.concat(
              ChatSocketOutManager.sceneChat.teamChanelPlayerList,
            );
          }
        }
        break;
  ...
  private static initBattleTeamPlayer(msg: ChatChannelMsg) {
    ...
    if (
      battleModel &&
      battleModel.armyInfoLeft &&
      battleModel.armyInfoLeft.getHeros
    ) {
      for (const key in battleModel.armyInfoLeft.getHeros) {
        if (
          ...
          if (msg.team.indexOf(hero.userId) < 0) msg.team.push(hero.userId);
        }
      }
    }
  }
    ...
    if (roomInfo) {
      for (const key in roomInfo.playerList) {
        if (Object.prototype.hasOwnProperty.call(roomInfo.playerList, key)) {
          ...
          if (msg.team.indexOf(army.userId) < 0) msg.team.push(army.userId);
        }
      }
    }
  }
  ...
  private static initMapCurrentPlayer(msg: ChatChannelMsg) {
    ...
    if (mapModel) {
      ...
      allBaseArmy.forEach((element) => {
        if (msg.team.indexOf(element.userId) < 0) msg.team.push(element.userId);
      });
    }
  }
  /**
  ...
  private static initFreedomTeamPlayer(msg: ChatChannelMsg) {
    ...
    var selfMapId: number = FreedomTeamManager.Instance.model.getMemberByUserId(
      ArmyManager.Instance.thane.userId,
    ).mapId;
    if (
      WorldBossHelper.checkPvp(selfMapId) ||
      WorldBossHelper.checkGvg(selfMapId)
    ) {
      return;
    }
    for (const key in FreedomTeamManager.Instance.model.allMembers) {
      if (
        ...
        if (
          msg.team.indexOf(member.userId) < 0 &&
          !WorldBossHelper.checkPvp(member.mapId) &&
          !WorldBossHelper.checkGvg(member.mapId)
        ) {
