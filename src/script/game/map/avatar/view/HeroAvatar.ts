...
import { AvatarResourceType, AvatarStandTotalFrameX, AvatarTotalFrameX } from "../../../constant/AvatarDefine";
...
export class HeroAvatar extends HumanAvatar {
    ...
    constructor($key: string, $type: number, $sex: number, $delayLoad: boolean, job: number = 0, pixelSnapping: string = "auto", smoothing: boolean = true) {
    ...
    public loaderCompleteHandler(res: any, para: ResourceLoaderInfo) {
        ...
        // 站立
        let layer = this.getAvatarByPosition(para.position, para.positionAddSign, this._avatarStandList);
        if (!layer) return;
        ...
        // 行走
        layer = this.getAvatarByPosition(para.position, para.positionAddSign, this._avatarWalkList);
        if (!layer) return;
    ...
    protected drawWalk() {
        ...
        for (let index = 0; index < this._avatarWalkList.length; index++) {
            ...
            this._destPoint.x = rect.x - this.sizeMax[this.sizeType].x + cellInfoX + finalOffsetX;
            this._destPoint.y = rect.y - this.sizeMax[this.sizeType].y + cellInfoY + finalOffsetY;
            this._bitmap && this._bitmap.graphics.drawImage(layer.tempBitmapData, this._destPoint.x, this._destPoint.y);
    ...
    protected drawStand() {
        ...
        for (let index = 0; index < this._avatarStandList.length; index++) {
            ...
            if (cellInfo == null) {
                cellInfo = layouPara.getCellByKey(this.frameX, this.getFrameY(this.frameY));
            }
    ...
    public fixStandPosMountWalk() {
        for (let index = 0; index < this._avatarWalkList.length; index++) {
            ...
            if (layer.layouPara && layer.layouPara.embedData != paras) {
                Logger.info("[HeroAvatar]fixStandPosMountWalk 修正前:" + layer.layouPara.embedData + "修正后:" + paras);
    ...
    public set angle(num: number) {
        ...
        } else { //-90逆时针旋转至90,frameY=[0 1 2 3 4]
            if (num >= 270 && num < 280) {
                this.frameY = 4;
                this._mirror = 1;
            }
        ...
        if (this._propertyChangeCallback != null && (isMirrorChange || isFrameYChange)) {
    ...
    public getCurrentPixels(point?: Laya.Point): number {
        if (this._showTranslucenceView) {
            let pix = HitTestUtils.hitTestAlpha(this._translucenceView.displayObject, point);
            return pix;
        }
        if (!point) {
    ...
    protected nextFrameX() {
        ...
        switch (this._state) {
            case HeroAvatar.WALK:
                ...
                if (this._standFrameX >= this.standTotalFrameX * this._standWalkDValue) {
                    this._standFrameX = 0;
                }
                break;
            case HeroAvatar.STAND:
    ...
    public get npcBodySizeType(): number {
        ...
        for (let index = 0; index < this._avatarWalkList.length; index++) {
            const layer = this._avatarWalkList[index];
            if (layer.resourceInfo.isNpc && layer.resourceInfo.position == AvatarPosition.BODY) {
    ...
    public get translucenceBodySizeType(): number {
        if (this._translucenceBodySizeType > 0) return this._translucenceBodySizeType;
     ...
     * 
     */
    public removeAvatarByPositionAddSign(positionAddSign: string, position: string = null) {
        ...
        function spliceBypositionAddSign(arr: any[]) {
            ...
            for (let i: number = len - 1; i >= 0; i--) {
                ...
                if (baseLayer.resourceInfo.positionAddSign == positionAddSign) {
                    ...
                    if (toContinue && isMountPosition && baseLayer.resourceInfo.position == AvatarPosition.MOUNT_WING)//如果为坐骑时连同翅膀也删除
                    {
                        toContinue = false;
                    }
                    if (toContinue) {
    ...
    private getAvatarByPosition(position: string, positionAddSign: string, arr: any[]): AvatarBaseLayer {
        if (!arr) return null;
        for (let index = 0; index < arr.length; index++) {
            const layer = arr[index];
            if (layer.resourceInfo.position == position && layer.resourceInfo.positionAddSign == positionAddSign) {
                return layer;
            }
        }
    ...
    private getFrameY($frameY: number): number {
        let fY: number = 2;
        if (this.sizeType != 10 || HeroAvatar.SizeType_NEW10.indexOf(this.currentMountId) != -1) {
    ...
    private getMountOffsetY(type: number = HeroAvatar.STAND): number {
        ...
        for (let index = 0; index < arr.length; index++) {
            const layer = arr[index];
            if (layer.visible && layer.resourceInfo.position == AvatarPosition.MOUNT) {
    ...
    private updateDepth() {
        ...
        this._avatarWalkList = ArrayUtils.sortOn(this._avatarWalkList, "curBaseNum", ArrayConstant.NUMERIC | ArrayConstant.DESCENDING);
        ...
        this._avatarStandList = ArrayUtils.sortOn(this._avatarStandList, "curBaseNum", ArrayConstant.NUMERIC | ArrayConstant.DESCENDING);
