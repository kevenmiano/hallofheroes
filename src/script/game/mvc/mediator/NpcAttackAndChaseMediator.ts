...
export class NpcAttackAndChaseMediator implements IMediator, IEnterFrame
{
    ...
    public register(target:any)
    {
        ...
        this._npc.nodeInfo.info.addEventListener(PhysicsEvent.UP_STATE, this.__stateHandler,this);
        this._npc.npcAiInfo.addEventListener(AiEvents.MOVE_STATE, this.__aiMoveStateHandler,this);
    }
    ...
    public unregister(target:any)
    {
        ...
        this._npc.nodeInfo.info.removeEventListener(PhysicsEvent.UP_STATE, this.__stateHandler,this);
        this._npc.npcAiInfo.removeEventListener(AiEvents.MOVE_STATE, this.__aiMoveStateHandler,this);
    }
    ...
    private __stateHandler(evt:PhysicsEvent)
    {
        if(this._npc.nodeInfo.info.state != NodeState.EXIST)
        {
            if(this._attackArmy)
            {
                this._attackArmy.aiInfo.removeEventListener(AiEvents.UPDATE_PATHS, this.__updatePathHandler,this);
            }
            EnterFrameManager.Instance.unRegisteEnterFrame(this);
        }
    }
    ...
    {
        if(!this._npc || !this._npc.nodeInfo.info || this._npc.nodeInfo.info.state != NodeState.EXIST)
        {
            Logger.yyz("[NpcAttackAndChaseMediator]__aiMoveStateHandler npc节点或节点信息不存在")
            return;
        }
        Logger.yyz("[NpcAttackAndChaseMediator]__aiMoveStateHandler moveState", this._npc.npcAiInfo.moveState, this.npcNodeId)
        if (CampaignManager.Instance.mapModel && CampaignManager.Instance.mapModel.chckeNpcFriendState(<CampaignNode>this._npc.nodeInfo)) return;
        if(this._npc.npcAiInfo.moveState == AiStateType.NPC_CHASE_STATE)
        {
            this._attackArmy = this.getArmyView(this._npc.npcAiInfo.attackArmyId, this._npc.npcAiInfo.attackArmyServerName);
            if(!this._attackArmy)
            {   
                Logger.warn("[NpcAttackAndChaseMediator]__aiMoveStateHandler 找不到攻击对象", this.npcNodeId)
                return;
            }
            ...
            this._aiInfo.addEventListener(AiEvents.UPDATE_PATHS, this.__updatePathHandler, this);
            this._preDis = 10000;
            this.removeAlertIcon();
            this._alertIcon = FUIHelper.createFUIInstance(EmPackName.CampaignCommon, "CampaignAlertMC");
            this._alertIcon.y = this._npc.showNamePosY - this._alertIcon.height / 2 - 28;
        ...
        else
        {
            ...
            if(this._aiInfo)
            {
                this._aiInfo.removeEventListener(AiEvents.UPDATE_PATHS, this.__updatePathHandler,this);
            }
            EnterFrameManager.Instance.unRegisteEnterFrame(this);
        }
    }
    ...
    private searchPath()
    {
        let start:Laya.Point = CampaignManager.Instance.mapModel.getAroundWalkPoint(this._npc.x, this._npc.y);
        if(!start)
        {
            return;
        }
        let end:Laya.Point = CampaignManager.Instance.mapModel.getAroundWalkPoint(this._attackArmy.x, this._attackArmy.y);
        if(!end)
        {
            return;
        }
        ...
        // else
        // {
        let arr:any[] = SearchPathHelper.searchPath(new Laya.Point(this._npc.x, this._npc.y), new Laya.Point(end.x * 20, end.y * 20));
        if(!arr)
        {
            return;
        }
    ...
    public enterFrame()
    {
        let armyPos:Laya.Point = new Laya.Point(this._attackArmy.x, this._attackArmy.y);
        let dis:number = this._npc.npcAiInfo.centerPoint.distance(armyPos.x, armyPos.y);
        let attackRect:number = new Laya.Point(this._npc.x, this._npc.y).distance(armyPos.x, armyPos.y);
        let nodeId:number = (<CampaignNode>this._npc.nodeInfo).nodeId;
        let attackArmy:CampaignArmy = this.getArmyViewByArmyId(this._npc.npcAiInfo.attackArmyId, this._npc.npcAiInfo.attackArmyServerName);
        ...
        {
            Logger.yyz("[NpcAttackAndChaseMediator]部队开始进入战斗 攻击玩家", this.selfId);
            this._aiInfo.pathInfo = [];
            this._npc.aiInfo.pathInfo = [];
            this.startAttack();
            NotificationManager.Instance.dispatchEvent(NotificationEvent.SEND_CAMPAIGN_ARRIVE, null);
            CampaignManager.Instance.controller.sendCampaignArrive(this.selfId, (<CampaignNode>this._npc.nodeInfo).nodeId);
            EnterFrameManager.Instance.unRegisteEnterFrame(this);
            return;
        }
        ...
        this._count++;
        if((dis > NpcEffectiveState.ATTACK_RADIUS || attackRect > NpcEffectiveState.ALERT_RADIUS + 10) && isSelf)
        {
            Logger.yyz("[NpcAttackAndChaseMediator]玩家离开攻击范围 dis=", dis, "attackRect=", attackRect, "armyPos=", armyPos, "attackArmy=", attackArmy)
            this._npc.nodeInfo.chatData = null;
            MapSocketOuterManager.sendAlertState(nodeId, false);
            EnterFrameManager.Instance.unRegisteEnterFrame(this);
            return;
        }
        else if(attackRect > NpcEffectiveState.BATTLE_RADIUS && ((this._count % 5 == 0 && this._preDis < dis) || this._count % 20 == 0))
        {
            Logger.yyz("[NpcAttackAndChaseMediator]NPC追击玩家 dis=", dis, "attackRect=", attackRect, "armyPos=", armyPos, "attackArmy=", attackArmy)
            this.searchPath();
        }
        this._preDis = dis;
    }
    ...
    private getArmyViewByArmyId(id:number, serverName:string = ""):CampaignArmy
    {
        return CampaignManager.Instance.mapModel.getBaseArmyByArmyId(id, serverName);
    }
