...
export default class SortMemberItem extends FUI_SortRankItemCell {
    ...
    private refreshWarlords() {
        ...
        this.para0.text = LangManager.Instance.GetTranslation("sort.SortGeneralListCell.str01", this.cellData.orderId.toString());
        this.para1.text = this.cellData.army.baseHero.nickName + " " + this.cellData.army.baseHero.serviceName;
    }
    ...
    private refreshCrossView() {
        if (this.cellData) {
            ...
            this.clickRect0.onClick(this, this.onClickName, [this.smodel.currentSelected]);
            this.clickRect1.onClick(this, this.onClickRect1, [this.smodel.currentSelected]);

            switch (this.smodel.currentSelected) {
                case SortModel.SELF_LEVEL:
                    this.para1.text = this.cellData.nickName;
                    this.para2.text = JobType.getJobName(this.cellData.army.baseHero.templateInfo.Job);
    ...
    private onClickName(type: number): void {
        if (type == SortModel.PET_POWER) {//玩家点击英灵名称下方的名称则弹出英灵对比框
            let ctrl: SortController = FrameCtrlManager.Instance.getCtrl(EmWindow.Sort) as SortController;
        ...
        } else if (type != SortModel.CONSORTIA_LEVEL && type != SortModel.CONSORTIA_POW) {
            this.showUseMenu(Laya.stage.mouseX, Laya.stage.mouseY, this.cellData.nickName, this.cellData.userId, false);
        }
    }
    /**
     * 点击玩家名字弹出操作框
    ...
    private onClickRect1(type: number): void {
        ...
        if (type == SortModel.PET_POWER) {//玩家点击主人下方的名字依然弹出角色操作框
            ...
            if (PlayerManager.Instance.currentPlayerModel.playerInfo.consortiaID > 0) {
                ...
                showConsortia = showConsortia && (this.cellData as SortData).army.baseHero.consortiaID <= 0;
            }
    ...
    private isSameZone(): boolean {
        if (this.cellData) {
            return PlayerManager.Instance.currentPlayerModel.userInfo.mainSite == this.cellData.mainSite;
        }
        return false;
    }
        ...
        if (this.cellData) {
            ...
            switch (this.smodel.currentSelected) {
                ...
                case SortModel.SELF_HONOUR://荣耀
                    ...
                    this.para2.text = JobType.getJobName(this.cellData.army.baseHero.templateInfo.Job);
                    //称号
                    let cfg:t_s_honorequipData = TempleteManager.Instance.geHonorCfgByType(1,this.cellData.honorEquipStage);
                    if(cfg){
                ...
                case SortModel.PET_POWER://英灵战斗力
                    ...
                    this.para1.color = PetData.getQualityColor(parseInt(this.cellData.petData.quality) - 1);
                    break;
            }
        } else {
            this.para0.text = "";
