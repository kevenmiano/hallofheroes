 ...
 */
export class MapPhysicsBase
  extends Laya.Sprite
  implements IBaseMouseEvent, IManualTipTargetSize, ISelectMovie
{
  ...
  protected updateView() {
    ...
    if (
      state == NodeState.EXIST ||
      state == NodeState.STATE2 ||
      state == NodeState.STATE3
    ) {
      this.initView();
      this.setNomalView();
  ...
  private __onResComplete(res: any) {
    ...
    if (res && res.offset) {
      ...
      this.pivot(
        Math.round(this._movie.getBounds().width / 2),
        Math.round(this._movie.getBounds().height / 2),
      );
    }
    ...
    if (
      res &&
      res.offset &&
      res.offset.hasOwnProperty("posX") &&
      res.offset.hasOwnProperty("posY")
    ) {
    ...
    if (temp) {
      ...
      this.pivot(
        -Math.round(_x + temp.offsetX),
        -Math.round(_y + temp.offsetY),
      );
  ...
  public get sizeInfo() {
    if (this._movie) {
      let tmp = {
        x: Math.abs(this.pivotX),
        y: Math.abs(this.pivotY),
        width: this._movie.getBounds().width,
        height: this._movie.getBounds().height,
      };
  ...
  public gotoAndPlayCall(frame: any) {
    ...
    if (this._movie) {
      if (typeof frame == "number") {
        ...
        if (this._preUrl.indexOf("2383") != -1) {
          //公会战】玩家占领神秘塔后并进行防守, 敌方玩家与防守玩家发生战斗后并退出战斗场景后, 神秘塔资源消失了
  ...
  public set info(value: MapPhysics) {
    if (this._info) {
      this._info.off(
        OuterCityEvent.UPDATA_MAP_PHYSICS,
        this.__updateMapPhysicsHandler,
        this,
      );
    }
    this._info = value;
  ...
  public isMouseInHandleRangle(evt: any): boolean {
    ...
    let bNotOutRange =
      Math.ceil(Math.sqrt(offX * offX + offY * offY)) <= handlerRange;
    Logger.xjy(
      "[CampaignNpcPhysics]鼠标是否在触发范围",
      bNotOutRange,
