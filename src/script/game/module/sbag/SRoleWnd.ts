...
export class SRoleWnd extends BaseWindow {
  ...
  public OnInitWind() {
    super.OnInitWind();
    this.tab0.getChildAt(0).asButton.title =
      LangManager.Instance.GetTranslation("answer.view.rank.name");
    this.tab0.getChildAt(1).asButton.title =
      LangManager.Instance.GetTranslation("FarmWnd.bagTxt");
    this.tab0.getChildAt(2).asButton.title =
      LangManager.Instance.GetTranslation("fashion.identity.title");
    this.tab0.getChildAt(3).asButton.title =
      LangManager.Instance.GetTranslation("HigherGradeOpenTipView.content18");
    this.tab0.getChildAt(4).asButton.title =
      LangManager.Instance.GetTranslation("Bag.mastery");
    this.tab0.getChildAt(4).visible =
      ArmyManager.Instance.thane.grades >= OpenGrades.MASTERY;
    ...
    this.detailBtn.title = LangManager.Instance.GetTranslation(
      "fashion.identify.tip",
    );
    let cfg: ConfigInfosTempInfo =
      TempleteManager.Instance.getConfigInfoByConfigName("honoropenlevel");
    if (cfg) {
    ...
    this.btn_save.tooltips = LangManager.Instance.GetTranslation(
      "fashion.FashionSwitchView.saveBtn.tipData03",
    );
  ...
  private tagArr = [
    "armyII.viewII.pawnupgrade.property",
    "armyII.viewII.equip.JewelFrame.JewelNameTxt",
  ]; // 'battleGuard.NewGuildFrame.titleText'// ,'tattoo.frame.titleText'
  /**
  ...
  private updateNewFeature() {
    ...
    if (
      this.thane.grades >= OpenGrades.FORTUNE_GUARD &&
      ComponentSetting.FortuneGuard
    ) {
    ...
    this.tab1.itemRenderer = Laya.Handler.create(
      this,
      this.onRenderTab1,
      null,
      false,
  ...
  public OnShowWind() {
    ...
    if (
      this.frameData &&
      this.frameData.tabIndex &&
      this.frameData.tabIndex == 2
    ) {
    ...
    if (this.frameData && this.frameData.appellID) {
      ...
      this.onSelectTab0(null);
      if (
        this.tab1.getChildAt(2).asButton.title ==
        LangManager.Instance.GetTranslation("RvrBattleResultWnd.roleHonorTxt")
      ) {
      ...
      this.onSelectTab1(
        this.tab1.getChildAt(BAG_INDEX.TATTOO) as fairygui.GButton,
      );
  ...
  private initData() {
    ...
    this._equipList = [];
    this._fashionList = [];
    let str = fgui.UIPackage.getItemURL(
      "SBag",
      VIPManager.Instance.model.vipInfo.VipGrade.toString(),
    );
  ...
  onSelectTab0(item: fgui.GObject) {
    ...
    if (this._lastSelectIndex0 == 2 && index != 2) {
      //从外观切换到其他页签
  ...
  onSelectTab1(item: fairygui.GButton) {
    ...
    if (
      item &&
      item.title ==
        LangManager.Instance.GetTranslation("tattoo.frame.titleText")
    ) {
      this.onTattoo();
      ...
      item &&
      item.title ==
        LangManager.Instance.GetTranslation("HigherGradeOpenTipView.content25")
    ) {
      this.onFortune();
    }
  }
  ...
  private updateEquip() {
    ...
    for (let i = 0; i < this._equipList.length; i++) {
      ...
      this._equipViewList.add(
        equipGrid.item.pos + "_" + this.thane.id + "_" + equipGrid.item.bagType,
        equipGrid,
      );
    }
    ...
    let dic: SimpleDictionary =
      GoodsManager.Instance.getHeroEquipmentGoodListByTypeAndId(this.thane.id);
      ...
      if (dic.hasOwnProperty(key)) {
        ...
        if (
          PlayerManager.Instance.currentPlayerModel.sysCurTimeBySecond -
            info.beginDate.getTime() >=
          info.validDate * 60 * 1000
        ) {
  ...
  private updatePersonalInfo() {
    ...
    consortiaName = this.thane.consortiaName
      ? "<" + this.thane.consortiaName + ">"
      : "";
  ...
  private checkBattleGuardOpen(showTips: boolean = false): boolean {
    let arr: t_s_upgradetemplateData[] =
      TempleteManager.Instance.getTemplatesByType(28);
  ...
  private updatePlayerExp() {
    ...
    if (upGrade) {
      ...
      this.txt_progress.text = LangManager.Instance.GetTranslation(
        "mainBar.PlayerExperenceView.msg",
      );
    }
  }
  ...
  public getItemViewByPos(
    ...
    let item: PlayerEquipCell = this._equipViewList[
      pos + "_" + this.thane.id + "_" + bagType
    ] as PlayerEquipCell;
    if (!item) {
      item = this._fashionViewList[
        pos + "_" + this.thane.id + "_" + bagType
      ] as PlayerEquipCell;
    }
    if (!item) {
  ...
  private __bagItemUpdateHandler(infos: GoodsInfo[]) {
    ...
    for (let info of infos) {
      let item: PlayerEquipCell =
        this._equipViewList[
          info.pos + "_" + info.objectId + "_" + info.bagType
        ];
      if (!item) {
        item =
          this._fashionViewList[
            info.pos + "_" + info.objectId + "_" + info.bagType
          ];
      }
      if (!item) {
  ...
  protected addEvent() {
    ...
    this.checkbox_fashion.on(
      fgui.Events.STATE_CHANGED,
      this,
      this.__hideFashionHandler,
      [-1],
    );
    this.hide_0.on(fgui.Events.STATE_CHANGED, this, this.__hideFashionHandler, [
      0,
    ]);
    this.hide_1.on(fgui.Events.STATE_CHANGED, this, this.__hideFashionHandler, [
      1,
    ]);
    this.hide_2.on(fgui.Events.STATE_CHANGED, this, this.__hideFashionHandler, [
      2,
    ]);
    this.hide_3.on(fgui.Events.STATE_CHANGED, this, this.__hideFashionHandler, [
      3,
    ]);
    ...
    this.fashionModel.addEventListener(
      FashionEvent.CHANGE_SWITCH,
      this.__switchHandler,
      this,
    ); //时装合成与转换开关
    // FashionManager.Instance.addEventListener(FashionEvent.FASHION_BOOK_RECEIVE, this.__receivedBookHandler, this);
    NotificationManager.Instance.addEventListener(
      NotificationEvent.APPELL,
      this.__appellSwitchChange,
      this,
    );
    NotificationManager.Instance.addEventListener(
      FashionEvent.FASHION_BOOK_WEAR,
      this.__wearHandler,
      this,
    );
    NotificationManager.Instance.addEventListener(
      FashionEvent.SWITCH_FASION,
      this.onSwitchFash,
      this,
    );
    NotificationManager.Instance.addEventListener(
    ...
    NotificationManager.Instance.addEventListener(
      ...
      this.__doubleClickBagItemHandler,
      this,
    );
    this.fashion_com
      .getChild("fashcompose_com")
      .displayObject.on(
        FashionEvent.COMPOSE_SUCCEED,
        this,
        this.__onComposeCallback,
      );
    NotificationManager.Instance.addEventListener(
  ...
  private __fashionMsgHandler() {
    ...
    if (this._showThane) {
      ...
      weaponInfo = GoodsManager.Instance.getItemByPOB(
        0,
        equipObjectId,
        BagType.HeroEquipment,
      );
      clothesInfo = GoodsManager.Instance.getItemByPOB(
        2,
        equipObjectId,
        BagType.HeroEquipment,
      );
      if (clothesInfo) {
        this._showThane.bodyEquipAvata =
          ThaneEquipShowHelper.getAvatarByEquipInfo(clothesInfo);
      }
      if (weaponInfo) {
        this._showThane.armsEquipAvata =
          ThaneEquipShowHelper.getAvatarByEquipInfo(weaponInfo);
      }
      this._showThane.wingAvata =
        ThaneEquipShowHelper.getAvatarByEquipInfo(wingEquip);
      this._showThane.armsFashionAvata =
        ThaneEquipShowHelper.getAvatarByEquipInfo(armsFashion);
      this._showThane.bodyFashionAvata =
        ThaneEquipShowHelper.getAvatarByEquipInfo(clothFashion);
      this._showThane.hairFashionAvata =
        ThaneEquipShowHelper.getAvatarByEquipInfo(hairFashion);
  ...
  private setSaveBtnState() {
    ...
    for (let i = 0; i < this.fashionModel.fashionEquipList.length; i++) {
      ...
      if (
        this.fashionModel.saveList[info.templateInfo.SonType] &&
        this.fashionModel.saveList[info.templateInfo.SonType] != info.templateId
      ) {
        b = true;
      }
    }
    ...
    if (
      (this.fashionModel.saveList[GoodsSonType.FASHION_WEAPON] &&
        this.fashionModel.bookList[
          this.fashionModel.saveList[GoodsSonType.FASHION_WEAPON]
        ] == null) ||
      (this.fashionModel.saveList[GoodsSonType.FASHION_CLOTHES] &&
        this.fashionModel.bookList[
          this.fashionModel.saveList[GoodsSonType.FASHION_CLOTHES]
        ] == null) ||
      (this.fashionModel.saveList[GoodsSonType.FASHION_HEADDRESS] &&
        this.fashionModel.bookList[
          this.fashionModel.saveList[GoodsSonType.FASHION_HEADDRESS]
        ] == null) ||
      (this.fashionModel.saveList[GoodsSonType.SONTYPE_WING] &&
        this.fashionModel.bookList[
          this.fashionModel.saveList[GoodsSonType.SONTYPE_WING]
        ] == null)
    ) {
      b = false;
      this.btn_save.tooltips = LangManager.Instance.GetTranslation(
        "fashion.FashionSwitchView.saveBtn.tipData02",
      );
    }

    if (
      (this.fashionModel.saveList[GoodsSonType.FASHION_WEAPON] &&
        !this.fashionModel.hasFashionAtSonType(GoodsSonType.FASHION_WEAPON)) ||
      (this.fashionModel.saveList[GoodsSonType.FASHION_CLOTHES] &&
        !this.fashionModel.hasFashionAtSonType(GoodsSonType.FASHION_CLOTHES)) ||
      (this.fashionModel.saveList[GoodsSonType.FASHION_HEADDRESS] &&
        !this.fashionModel.hasFashionAtSonType(
          GoodsSonType.FASHION_HEADDRESS,
        )) ||
      (this.fashionModel.saveList[GoodsSonType.SONTYPE_WING] &&
        !this.fashionModel.hasFashionAtSonType(GoodsSonType.SONTYPE_WING))
    ) {
      b = false;
      this.btn_save.tooltips = LangManager.Instance.GetTranslation(
        "fashion.FashionSwitchView.saveBtn.tipData01",
      );
    }
    this._canSave = b;
  ...
  private onSwallow(sontype: number) {
    if (this.tab0.selectedIndex == 1) {
      //	当玩家通过“背包”对背包内的时装进行“装备”或者“替换”操作时, 左侧分类列表将从“背包”自动跳转至“时装”, 并默认打开时装图鉴；
      this.tab0.selectedIndex = 2;
      this.onSelectTab0(null);
  ...
  private __wearHandler(book: boolean, info: t_s_itemtemplateData) {
    ...
    switch (info.SonType) {
      case GoodsSonType.FASHION_CLOTHES:
        if (!this.checkbox_fashion.selected && !this.hide_1.selected) {
          //隐藏勾选则不显示
      ...
      case GoodsSonType.FASHION_HEADDRESS:
        if (!this.checkbox_fashion.selected && !this.hide_0.selected) {
          //隐藏勾选则不显示
      ...
      case GoodsSonType.FASHION_WEAPON:
        if (!this.checkbox_fashion.selected && !this.hide_2.selected) {
          //隐藏勾选则不显示
      ...
      case GoodsSonType.SONTYPE_WING:
        if (!this.checkbox_fashion.selected && !this.hide_3.selected) {
          //隐藏勾选则不显示
    ...
    if (!this.checkbox_fashion.selected) {
      //隐藏勾选则不显示
      this._figure.data = this._showThane;
    }
  ...
  protected __hideFashionHandler(type: number, event: Laya.Event) {
    ...
    if (type == -1) {
      fashionMsg.status = this.checkbox_fashion.selected
        ? "0,0,0,0"
        : "1,1,1,1";
    } else {
  ...
  private updateHideFashionBtnState() {
    ...
    this.checkbox_fashion.selected =
      this.hide_0.selected &&
      this.hide_1.selected &&
      this.hide_2.selected &&
      this.hide_3.selected;
  }
  ...
  private equipChange(): number {
    ...
    var newEquip: SimpleDictionary =
      GoodsManager.Instance.getHeroEquipListById(heroId);
    var oldEquip: SimpleDictionary = this.model.oldEquip;
    if (!oldEquip || newEquip.getList().length != oldEquip.getList().length)
      return heroId;
  ...
  private checkHonorRedDot(): boolean {
    ...
    if (idx > 0) {
      ...
      let showHonor =
        BagHelper.Instance.checkHonorUpLevel(this.thane.honorEquipLevel) ||
        BagHelper.Instance.checkHonorUpStage(this.thane.honorEquipStage);
  ...
  private checkTatooRedDot(): boolean {
    return (
      this.ctrl.tattooModel.canTattoo() && !this.ctrl.tattooModel.isClicked
    );
  }
  /**
   * 角色关联了多页签红点
  ...
  private updateDetailTips() {
    ...
    let propertyStr =
      "[color=#FFC68F]" +
      LangManager.Instance.GetTranslation("fashion.identify.attr") +
      "[/color]";
    let haved = false;
    let attrArr = [
      LangManager.Instance.GetTranslation("PetPotencyWnd.powerTxt"),
    ...
    if (!haved) {
      propertyStr =
        "[color=#FFC68F]" +
        LangManager.Instance.GetTranslation("RuneHold.notBonus") +
        "[/color]";
    }
