...
export class ConsortiaSkillHelper
{
    ...
    public static addOffer()
    {
        ...
        SimpleAlertHelper.Instance.Show(SimpleAlertHelper.SIMPLE_ALERT, null, prompt, content, confirm, cancel, this.__addOfferCallBack.bind(this));
    }
    ...
    public static addWealth()
    {
        ...
        let cancel:string = LangManager.Instance.GetTranslation("public.cancel");
        let prompt:string = LangManager.Instance.GetTranslation("public.prompt");
        let content:string = LangManager.Instance.GetTranslation("consortia.helper.ConsortiaUpgradeHelper.content");
        SimpleAlertHelper.Instance.Show(SimpleAlertHelper.SIMPLE_ALERT, null, prompt, content, confirm, cancel, this.__addOfferCallBack.bind(this));
    }
