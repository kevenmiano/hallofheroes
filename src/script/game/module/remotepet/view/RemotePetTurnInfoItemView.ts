...
export class RemotePetTurnInfoItemView extends FUI_RemotePetTurnInfoItemView {
  ...
  public freshView() {
    ...
    if (
      this._data.tempInfo.Index == this.model.turnInfo.currTurn &&
      this._data.tempInfo.Index % 5 == 0
    ) {
  ...
  private __clickHandler() {
    ...
    if (
      _data.tempInfo.Index == model.turnInfo.currTurn ||
      _data.tempInfo.Index <= model.turnInfo.maxTurn
    ) {
      // let t: int = getTimer();
