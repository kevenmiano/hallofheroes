...
export class SimpleAlertWnd extends BaseWindow {
    ...
    OnShowWind() {
        ...
        //设置默认文本
        this.checkRickText.text = LangManager.Instance.GetTranslation('BuyFrameI.useBindTxt'); //checkbox，UI里面不能使用默认值，否则底层的宽度计算会有问题 
    ...
    confirmBtnClick() {
        switch (this.params.type) {
            ...
            case SimpleAlertHelper.USEBINDPOINT_ALERT:
                if (this.params.data && this.params.data.point) {
                    ...
                    let totalPoint: number = this.checked ? selfPoint + bindPoint : selfPoint;
        ...
        let callFunc: Function = this.params && this.params.callback && this.params.callback;
...
export default class SimpleAlertHelper extends BaseWindow {
     ...
     */
    async Show(type: string, dataObj: any, title: string, text: string, confirmTxt?: string, cancelTxt?: string, callback?: Function, btnType: AlertBtnType = AlertBtnType.OC, autoClose: boolean = true, hideBtnClose: boolean = false, emLayer: EmLayer = EmLayer.STAGE_TOP_LAYER): Promise<SimpleAlertHelper> {
        if (!type) {
    ...
    /** 简化写法 */
    async ShowSimple(type: string, dataObj: any, text: string, callback?: Function, title?: string, confirmTxt?: string, cancelTxt?: string, btnType: AlertBtnType = AlertBtnType.OC, autoClose: boolean = true, hideBtnClose: boolean = false, emLayer: EmLayer = EmLayer.STAGE_TOP_LAYER): Promise<SimpleAlertHelper> {
        return this.Show(type, dataObj, title, text, confirmTxt, cancelTxt, callback, btnType, autoClose, hideBtnClose, emLayer)
    }
     ...
     * @param $width
     */
    popAlerFrame(title: string, msg: string, submitLabel: string = "", cancelLabel: string = "", data = null, alertType: string = SimpleAlertHelper.SIMPLE_ALERT) {
        switch (alertType) {
            case SimpleAlertHelper.SIMPLE_ALERT:
                this.Show(alertType, data, title, msg, submitLabel, cancelLabel, data && data.callback);
                break;
            case SimpleAlertHelper.FUNCTION_ALERT:
                this.Show(alertType, data, title, msg, submitLabel, cancelLabel, data && data.callback);
                break;
