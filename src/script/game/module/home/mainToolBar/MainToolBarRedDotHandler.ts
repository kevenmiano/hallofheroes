...
export class MainToolBarRedDotHandler {
  ...
  addEvent() {
    ...
    ArmyManager.Instance.thane.skillCate.addEventListener(
      PlayerEvent.THANE_SKILL_POINT,
      this.__skillPointUpdate,
      this,
    );
    NotificationManager.Instance.addEventListener(
      BagEvent.NEW_EQUIP,
      this.__betterEquipUpdate,
      this,
    );
  ...
  removeEvent() {
    ...
    ArmyManager.Instance.thane.skillCate.removeEventListener(
      PlayerEvent.THANE_SKILL_POINT,
      this.__skillPointUpdate,
      this,
    );
    ...
    MountsManager.Instance.avatarList.removeEventListener(
      MountsEvent.MOUNT_LIST_CHANGE,
      this.__mountRedPointUpdate,
      this,
    );
  ...
  private __bagItemUpdateHandler(goods: GoodsInfo[]) {
    ...
    this.setLevelState(
      btnBag,
      GoodsManager.Instance.checkHasBetterEquip(),
      true,
    );
  ...
  private __updateShopReddot() {
    ...
    let discountState =
      DiscountShopManager.Instance.model.open &&
      DiscountShopManager.Instance.model.myDiscount <= 0;
  ...
  private __receiveNewEmail() {
    ...
    if (count > 0) {
      ...
      (btn.view.getChild("countTxt") as fgui.GTextField).text =
        count.toString();
    } else {
  ...
  private __updateRuneReddot() {
    ...
    if (this._runeOpenLevel == 0) {
      this._runeOpenLevel = Number(
        TempleteManager.Instance.getConfigInfoByConfigName("OpenRuneGrade")
          .ConfigValue,
      );
    }
    //技能按钮
    let state =
      PlayerManager.Instance.currentPlayerModel.playerInfo.runePowerPoint >=
        100 &&
      ArmyManager.Instance.thane.grades >= this._runeOpenLevel &&
      ComponentSetting.RUNE_HOLE;
    state = state || this.thane.runeCate.hasActiveRune();
    this.setBtnState(btn, state);
    let view = btn.getView();
    let dot = view.getChild("redDot");
    let showArrow =
      this.thane.talentData.talentPoint > 0 ||
      this.thane.talentData.checkUpgrade() ||
      this.thane.skillCate.skillPoint > 0;
  ...
  private __updateFriendReddot() {
    ...
    if (
      FriendManager.getInstance().toolInviteFriendList.length > 0 ||
      (FriendManager.getInstance().recommendList.length > 0 &&
        FriendManager.SHOW_RED_DOT)
    ) {
      showRed = true;
    }
  ...
  public updateBagRedPoint() {
    ...
    let ctrl: RoleCtrl = FrameCtrlManager.Instance.getCtrl(
      EmWindow.SRoleWnd,
    ) as RoleCtrl; //背包按钮-角色-龙纹
    if (
      FashionManager.Instance.fashionModel.checkRedDot() ||
      ctrl.tattooModel.canTattoo() ||
      BagHelper.Instance.checkJewelRedDot() ||
    ...
    if (btnView) {
      ...
      if (dot.visible && levelState) {
        let showArrow =
          this.thane.talentData.talentPoint > 0 ||
          this.thane.skillCate.skillPoint > 0 ||
          this.thane.skillCate.checkExtrajobRedDot();
        dot.x = showArrow ? 23 : 73;
      }
