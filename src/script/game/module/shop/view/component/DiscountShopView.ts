...
export default class DiscountShopView extends FUI_DiscountShopView {
    ...
    private initView() {
        ...
        this.goodList.itemRenderer = Laya.Handler.create(this, this.renderGoodListItem, null, false);
    }

    private addEvent() {
        this.btnRandom.onClick(this, this.btnRandomClick);
        NotificationManager.Instance.addEventListener(ShopEvent.DISCOUNTSHOP_UPDATE, this.refreshView, this);
    }

    private removeEvent() {
        Laya.timer.clear(this, this.__timerRefreshGoodList)
        this.btnRandom.offClick(this, this.btnRandomClick);
        NotificationManager.Instance.removeEventListener(ShopEvent.DISCOUNTSHOP_UPDATE, this.refreshView, this);
    }
    ...
    private refreshScoreList() {
        for (let index = 0; index < 5; index++) {
            ...
            if (info) {
                this["imgScoreLine" + index].visible = this.discountModel.myScore >= info.score;
    ...
    private refreshView() {
        ...
        this.txt_time.text = LangManager.Instance.GetTranslation("public.zhi", beginTime + " ", " " + endTime);
        ...
        if (myDiscount > 0) {
            ...
            if (!isCNLanguage()) {
                ...
                if (this._isClickRandom) {
                    ...
                    let tMyDiscount = this.mcMyDiscount.getTransition("tRoll" + myDiscount);
                    if (tMyDiscount) {
                        this._refreshGoodImmediately = false
                        tMyDiscount.play(Laya.Handler.create(this, this.__myDiscountMCComplete), 1);
                    } else {
