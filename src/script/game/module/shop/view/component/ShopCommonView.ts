...
export class ShopCommonView extends FUI_ShopCommonView {
  ...
  private initView() {
    ...
    if (this._shopType == ShopModel.STAR_SHOP) {
      ...
      let str: string = WarlordsManager.Instance.model.getMatchDateString(
        5,
  ...
  private initEvent() {
    ...
    this.playerInfo.addEventListener(
      PlayerEvent.CONSORTIA_COIN_CHANGE,
      this.__onConsortiaCoinUpdata,
      this,
    );
    this.Btn_Buy.onClick(this, this._buyHander);
    ArmyManager.Instance.thane.addEventListener(
      PlayerEvent.GLORY_CHANGE,
      this.__updatePropsCount,
      this,
    );
  ...
  private updateLimit() {
    ...
    this.HasNumValueTxt.text =
      GoodsManager.Instance.getGoodsNumByTempId(this._selectedItemData.ItemId) +
      "";
  ...
  private onTab1Changed(cc: fgui.Controller) {
    ...
    let account: AccountCom = (
      (FrameCtrlManager.Instance.getCtrl(EmWindow.ShopWnd) as ShopControler)
        .view as ShopWnd
    ).account;
  ...
  private getPropCount(): number {
    ...
    if (this._shopType == ShopModel.STAR_SHOP) {
      ...
      count = GoodsManager.Instance.getGoodsNumByTempId(
  ...
  private _buyHander() {
    if (this._selectedItemData) {
      ...
      if (
        this._shopType == ShopModel.STAR_SHOP &&
        this._selectedItemData instanceof t_s_startemplateData
      ) {
        let str: string = "";
        if (this.playerInfo.starPoint < this._selectedItemData.StarPoint) {
          str = LangManager.Instance.GetTranslation(
            "star.view.StarShopItem.command01",
          );
        ...
        if (StarManager.Instance.checkStarPlayerBagIsFull()) {
          str = LangManager.Instance.GetTranslation(
            "star.view.StarShopItem.command02",
          );
          MessageTipManager.Instance.show(str);
          return;
        }
        if (this._selectedItemData.TemplateId == 4) {
          StarManager.Instance.sendExchangeStar(
            this._selectedItemData.TemplateId,
            this.stepper.value,
          );
          return;
        }
        ...
        let content: string = LangManager.Instance.GetTranslation(
          "star.view.StarShopItem.str",
          this._selectedItemData.StarPoint * this.stepper.value,
        );
        ...
        SimpleAlertHelper.Instance.Show(
          ...
          confirm,
          cancel,
          this.buyResponseHandler.bind(this),
        );
      } else if (this._shopType == ShopGoodsInfo.ADVCONSORTIA_SHOP) {
        ...
        if (this.checkIsRunes(this._selectedItemData) && goods.Property1 > 0) {
          //是符文
          let state = OuterCityShopManager.instance.model.getRuneState(goods);
          if (state == 1) {
            //已经学习
            MessageTipManager.Instance.show(
              LangManager.Instance.GetTranslation("shopcommand.buytip1"),
            );
            return;
          } else if (state == 2) {
            //已经拥有
            MessageTipManager.Instance.show(
              LangManager.Instance.GetTranslation("shopcommand.buytip2"),
            );
            return;
          } else {
        ...
        let own: number =
          PlayerManager.Instance.currentPlayerModel.playerInfo.consortiaOffer;
        ...
        let own: number =
          PlayerManager.Instance.currentPlayerModel.playerInfo.consortiaJianse;
        let need: number = this.stepper.value * this.getPrice();
        if (need > own) {
          MessageTipManager.Instance.show(
            LangManager.Instance.GetTranslation(
  ...
  private addOffer() {
    ...
    SimpleAlertHelper.Instance.Show(
      ...
      confirm,
      cancel,
      this.addOfferCallBack.bind(this),
    );
  }
  ...
  addOfferCallBack(b: boolean, flag: boolean) {
    if (b) {
      FrameCtrlManager.Instance.open(
        ...
        { returnToWin: EmWindow.ShopWnd, returnToWinFrameData: { page: 1 } },
        null,
        EmWindow.ShopWnd,
      );
    }
  ...
  public static getCurrencyNameByType(shopType: number): string {
    ...
    switch (shopType) {
      case ShopGoodsInfo.CONSORTIA_SHOP:
        str = LangManager.Instance.GetTranslation(
          "task.view.TaskRewardItem.iconTips06",
        ); //公会贡献
        break;
      case ShopGoodsInfo.FARM_SHOP:
        str = LangManager.Instance.GetTranslation(
          "mainBar.view.ResourceView.gold",
        ); //农场
        break;
      case ShopGoodsInfo.WARLORDS_SHOP:
        str = LangManager.Instance.GetTranslation(
          "warlords.WarlordsCheckRewardFrame.str07",
        ); //众神
        break;
      case ShopGoodsInfo.ADVCONSORTIA_SHOP:
        str = LangManager.Instance.GetTranslation(
          "task.view.shopCommWnd.buyTips",
        ); //高级公会商城
        break;
      case ShopGoodsInfo.CONSORTIA_HIGH_SHOP:
        str = LangManager.Instance.GetTranslation(
          "task.view.TaskRewardItem.iconTips07",
        ); //高级公会商城
        break;
    }
    return str;
  ...
  private renderListItem(index: number, item: ShopItem) {
    item.info = this._model.currentGoodsList[index]
  ...
  private _refreshRight() {
    if (this._selectedItemData) {
      ...
      this.CostMoneyTxt.text = (
        this.stepper.value * this.getPrice()
      ).toString();
  ...
  private updateBuyLimitText() {
    ...
    if (this._selectedItemData.type == ShopGoodsInfo.PAY_BY_GIFT) {
      ...
      let num: number =
        this._selectedItemData.limitCount - this._selectedItemData.count;
      this.limitCountTxt.text = "";
      if (num > 0) {
        this.limitTitleTxt.text = LangManager.Instance.GetTranslation(
      ...
      if (this._selectedItemData.canOneCount >= 0) {
        ...
        leftCount =
          this._selectedItemData.OneDayCount -
          this._selectedItemData.OneCurrentCount;
        this.limitCountTxt.text =
          (leftCount > 0 ? "[color=#FFECC6]" : "[color=#FF0000]") +
          leftCount +
          "[/color]" +
          "/" +
          this._selectedItemData.OneDayCount.toString();
    ...
    if (this._selectedItemData.WeeklyLimit > -1) {
      ...
      leftCount =
        this._selectedItemData.WeeklyLimit - this._selectedItemData.weekCount;
      this.limitCountTxt.text =
        (leftCount > 0 ? "[color=#FFECC6]" : "[color=#FF0000]") +
        leftCount +
        "[/color]" +
        "/" +
        this._selectedItemData.WeeklyLimit;
    }
  }
  ...
  private setBuyButtonStatus() {
    this.Btn_Buy.enabled = this.MaxCanBuyCount > 0;
    if (
      this._selectedItemData.ShopType == ShopGoodsInfo.CONSORTIA_SHOP ||
