...
export class FarmLandItem extends FUI_FarmLandItem {
	...
	private __onItemClick(e: Laya.Event) {
		if (!this.isOpen) {
			let str = (this.pos < FarmModel.MAX_OPEN_LAND ? LangManager.Instance.GetTranslation("farm.view.FarmLandLayer.needFarmGradeTip", (this.pos - 2) * 5) : LangManager.Instance.GetTranslation("public.unopen"));
			if (this.cNextOpen.selectedIndex == 1) {
				var curSelectedFarm: FarmInfo = this.model.curSelectedFarmInfo;
				if (this.checkIsSelf(curSelectedFarm.userId)) {//玩家在好友农场内点击好友的未解锁土地, 弹出提示五级开放, 更改为无需提示
				...
				if (this.model.curSelectedFarmInfo) {
					...
					if (this.checkIsSelf(uid)) {
						MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("farm.data.FarmLandItem.Lock"))
					}
				}			
			}
		}
		...
		switch (this._info.curOper)//操作类型
		{
			case FarmOperateType.PLANT://种植
				...
				if (!goodInfo) {
					let msg = LangManager.Instance.GetTranslation("farm.data.FarmSeedNotEnough")
				...
				//最多一种特殊作物
				if (this.farmManager.checkExistSameSpecialCrop(goodInfo.templateInfo, this.model.curSelectedFarmInfo)) {
					MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("farm.cell.FarmBagCellDropMediator.hasSpecialCropTip"));
					break;
				}

				this.farmManager.sendFarmOper(this._info.userId, FarmOperateType.PLANT, this._info.pos, goodInfo.pos, goodInfo.templateId);
				break;
			case FarmOperateType.STEAL://摘取
				...
				if (canBeStolen) {
					...
					this.farmManager.updateFarmStateByType(this._info.userId, FarmOperateType.STEAL, false);
					return;
				}
				if (this.model.myFarm.dayStealCount >= this.model.maxDayStealCount)//偷取次数是否达到上限
				{
					MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("farm.view.FarmLandItem.outStealCountTip"));
					return;
				}
				this.farmManager.sendFarmOper(this._info.userId, FarmOperateType.STEAL, this._info.pos);
				break;
			case FarmOperateType.WORM://除虫
				this.farmManager.sendFarmOper(this._info.userId, FarmOperateType.WORM, this._info.pos);
				break;
			case FarmOperateType.WEED://除草
				this.farmManager.sendFarmOper(this._info.userId, FarmOperateType.WEED, this._info.pos);
				break;
			case FarmOperateType.PICK://收获
				if (this.checkGoldLimit(this._info.cropTemp)) return;
				this.farmManager.sendFarmOper(this._info.userId, FarmOperateType.PICK, this._info.pos);
				break;
			case FarmOperateType.REVIVE://复活
				this.farmManager.sendFarmOper(this._info.userId, FarmOperateType.REVIVE, this._info.pos);
				break;
			case FarmOperateType.CLEAR://铲除
				var tip: string = LangManager.Instance.GetTranslation(this._info.isDie ? "farm.view.FarmOperMenu.DieClearTip" : "farm.view.FarmOperMenu.clearTip");
				SimpleAlertHelper.Instance.Show(SimpleAlertHelper.SIMPLE_ALERT, { oper: FarmOperateType.CLEAR, land: this._info }, null, tip, null, null, this.operResponse.bind(this));
				break;
	...
	private checkGoldLimit(cropTemp: t_s_itemtemplateData): boolean {
		if (cropTemp && cropTemp.Property2 == ItemID.GOLD_PROP && this.gold.count >= this.gold.limit)  //黄金爆仓后, 无法摘取黄金作物
		{
			let uid = 0
			if (this.model.curSelectedFarmInfo) {
			...
			let str = LangManager.Instance.GetTranslation("farm.view.FarmLandItem.goldLimitTip")
			if (!this.checkIsSelf(uid)) {
				MessageTipManager.Instance.show(str);
			} else {
				SimpleAlertHelper.Instance.Show(null, null, null, str, LangManager.Instance.GetTranslation("farm.view.FarmOperMenu.clear"), null, (b: boolean) => {
					if (b) {
						var tip: string;
						this._info.isDie ? tip = LangManager.Instance.GetTranslation("farm.view.FarmOperMenu.DieClearTip") : tip = LangManager.Instance.GetTranslation("farm.view.FarmOperMenu.clearTip");
						SimpleAlertHelper.Instance.Show(null, { oper: FarmOperateType.CLEAR, land: this._info }, null, tip, null, null, this.operResponse.bind(this));
					}
				})
			}
			return true;
	...
	private addLandListener(info: FarmLandInfo) {
		if (info) {
			info.addEventListener(FarmEvent.LANDINFO_UPDATE, this.__landInfoUpdateHandler, this);
	...
	private delLandListener(info: FarmLandInfo) {
		if (info) {
			info.removeEventListener(FarmEvent.LANDINFO_UPDATE, this.__landInfoUpdateHandler, this);
			info.removeEventListener(FarmEvent.OPER_BACK, this.__operBackHandler, this);
	...
	private __operBackHandler(e: FarmEvent) {
		var curSelectedFarm: FarmInfo = this.model.curSelectedFarmInfo;
		if (!this._info || !curSelectedFarm || curSelectedFarm.userId != this._info.userId) return;
	...
	private addExpAndChatTip(curFarm: FarmInfo) {
		...
		if (addExp <= 0) {
			switch (this.model.fullExpType) {
				case 1:
					this.txtGetMsg.text = LangManager.Instance.GetTranslation("farm.view.FarmLandItem.farmExpFull");
					break;
				case 2:
					this.txtGetMsg.text = LangManager.Instance.GetTranslation("farm.view.FarmLandItem.dayGpFull");
					break;
			}
		}
		else {
			this.txtGetMsg.text = LangManager.Instance.GetTranslation("farm.view.FarmLandItem.farmExp") + "+" + addExp;
		}
		switch (this._info.operBack) {
			...
			case FarmOperateType.ACCELERATE://加速
				MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("farm.view.FarmLandItem.accelerateSucceed"));
				break;
		}
		...
		if (!this.checkIsSelf(curFarm.userId)) {
			...
			var cropName: string = (this._info.cropTemp ? this._info.cropTemp.TemplateNameLang : "");
			this.farmManager.addChatTip(2, this._info.operBack, curFarm.userId, curFarm.nickName, cropName, addExp);
		}
		else {
	...
	private updateFriendFarmState(curFarm: FarmInfo) {
		...
		var fsInfo: FriendFarmStateInfo = this.model.getFarmStateInfo(curFarm.userId);
		for (let index = 0; index < curFarm.getLandList().length; index++) {
			const land: FarmLandInfo = curFarm.getLandList()[index];
			if (this.farmManager.checkHasOperNow(land, this._info.operBack)) hasOper = true;
		}
		fsInfo.beginChanges();
		switch (this._info.operBack) {
	...
	private showFailTip() {
		...
		switch (this._info.operBack) {
			...
			case FarmOperateType.WEED:
				...
				break;
			case FarmOperateType.REVIVE:
				str = LangManager.Instance.GetTranslation("farm.FarmManager.noReviveTip");
				break;
			case FarmOperateType.STEAL:
				if (!this._info.hasCrop)
					str = LangManager.Instance.GetTranslation("farm.data.FarmLandInfo.noCrop");
	...
	private __landInfoUpdateHandler() {
		...
		if (!this.checkIsSelf(this._info.userId) && this._curPhase == CropPhase.MATURE && this._info.canBeStolen) {
	...
	private refreshView() {
		if (this._info) {
			this.imgSelected.visible = this._info.hasCrop ? false : FarmManager.Instance.showingBag;
			let isSelf = this._info.userId == this.thane.userId;
			if(!isSelf){
				this.imgSelected.visible = false;
			}
			if (this._info.hasCrop && this._info.cropTemp) {
				var iconPath: string = IconFactory.getCropIcon(this._info.cropTemp, this._info.cropPhase);
				if (iconPath) {
	...
	private refreshViewByState(state: number) {
		...
		switch (state) {
			case CropState.GRASS:
				...
				this.btnLandOpt.icon = FUIHelper.getItemURL(EmWindow.Farm, "Icon_Farm_Weed")
				this.imgGrass2.visible = true;
				this.farmManager.updateFarmStateByType(this._info.userId, FarmOperateType.WEED, true);  //长草以后更新好友列表状态图标
				break;
			case CropState.WORM:
				this.showBtnOptEffect(true);
				this.btnLandOpt.icon = FUIHelper.getItemURL(EmWindow.Farm, "Icon_Farm_Bug")
				this.farmManager.updateFarmStateByType(this._info.userId, FarmOperateType.WORM, true);
				break;
		}
	}
	...
	private refreshViewByPhase(phase: number) {
		...
		switch (phase) {
			case CropPhase.NONE:
				...
				break;
			case CropPhase.GROW:
				if (this._curState != CropState.WORM && this._curState != CropState.GRASS) this.cleanEffect();
				break;
			case CropPhase.MATURE:
				this.cleanEffect();
				if (this._info.canBeStolen == "" || this.checkIsSelf(this._info.userId)) {
					this.showBtnOptEffect(true);
					this.btnLandOpt.icon = FUIHelper.getItemURL(EmWindow.Farm, "Icon_Farm_Collect")
					this.farmManager.updateFarmStateByType(this._info.userId, FarmOperateType.STEAL, true);
				}
			...
			case CropPhase.DIE:
				...
				if (this.checkIsSelf(this._info.userId)) {
					this.btnLandOpt.icon = FUIHelper.getItemURL(EmWindow.Farm, "Icon_Farm_Rootout")
				} else {
					this.btnLandOpt.icon = FUIHelper.getItemURL(EmWindow.Farm, "Icon_Farm_Revive")
				}
				this.farmManager.updateFarmStateByType(this._info.userId, FarmOperateType.REVIVE, true);
				break;
		}
	}
