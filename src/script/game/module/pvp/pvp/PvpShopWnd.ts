 ...
 * @LastEditTime: 2024-02-19 18:09:48
...
export default class PvpShopWnd extends BaseWindow {
  ...
  OnShowWind() {
    ...
    this.frame.getChild("title").text = LangManager.Instance.GetTranslation(
      "pvproomlist.view.AthleticsShopFrame.title",
    );
  ...
  private addEvent() {
    this.shopModel.addEventListener(
  ...
  private removeEvent() {
    this.shopModel.removeEventListener(
      ShopEvent.GOODS_LIST_UPDATE,
      this.__refreshGoods,
      this,
    );
    ...
    this.ItemList.off(fgui.Events.SCROLL_END, this, this.updateBagPage);
  ...
  helpBtnClick() {
    ...
    let value = Number(
      TempleteManager.Instance.getConfigInfoByConfigName("DayAddGeste")
        .ConfigValue,
    );
    let content = LangManager.Instance.GetTranslation(
      "bag.events.BagEvent.helpContent",
      value,
    );
  ...
  updateBagPage() {
    Logger.warn("updateBagPage ====", this.ItemList.scrollPane.isRightMost);
    this.PageNumTxt.text =
      this.ItemList.scrollPane.currentPageX +
      1 +
      "/" +
      this.shopModel.totalPage;
  }
  ...
  private renderListItem(index: number, item: ShopItem) {
    item.info = this.shopModel.currentGoodsList[index]
      ? this.shopModel.currentGoodsList[index]
      : null;
  }
  ...
  private __refreshPage() {
    this.PageNumTxt.text =
      this.shopModel.currentPage + "/" + this.shopModel.totalPage;
  }
  ...
  private _refreshRight() {
    if (this._selectedItemData) {
      ...
      this.updateLimit();
      let status: boolean =
        this._selectedItemData.NeedGeste > ArmyManager.Instance.thane.honer;
  ...
  private updateLimit() {
    ...
    if (GoodsCheck.isEquip(temp)) {
      ...
      if (weekLimitCount > 0) {
        ...
        this.limitCounTxt.text =
          weekLimitCount - weekHasBuyCount + "/" + weekLimitCount;
  ...
  private refreshItemTips(info: GoodsInfo, temp: t_s_itemtemplateData) {
    ...
    if (info.validDate > 0) {
      tempStr +=
        ...
        ":" +
        DateFormatter.getFullDateString(info.validDate * 60);
    }
    if (tempStr == "") {
      tempStr =
  ...
  OnHideWind() {
    super.OnHideWind();
    this.ItemList.removeChildrenToPool();
    this.removeEvent();
  }
