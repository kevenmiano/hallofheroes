 ...
 * 丰收号角
 */
export default class FoisonHornView
  extends FUI_FoisonHornView
  implements FunnyContent
{
  ...
  private refreshView() {
    this.timeValueTxt.text =
      this.foisonHornModel.openTime.substring(5).replace("-", ".") +
      " - " +
      this.foisonHornModel.stopTime.substring(5).replace("-", ".");
    ...
    FUIHelper.setTipData(this.picBtn, EmWindow.CommonTips, tipData);
    this.initView();
  }
  ...
  private getRewardStr(rewardStr: string): string {
    ...
    for (var i: number = 0; i < goodsArr.length; i++) {
      ...
      if (arr.length == 2) {
        goodTemp = TempleteManager.Instance.getGoodsTemplatesByTempleteId(
          parseInt(arr[0]),
        );
        if (goodTemp) {
          if (resultStr != "") {
