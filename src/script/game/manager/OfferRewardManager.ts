...
export default class OfferRewardManager extends GameEventDispatcher {
  ...
  public static get Instance(): OfferRewardManager {
    if (!OfferRewardManager._instance)
      OfferRewardManager._instance = new OfferRewardManager();
  ...
  private addEvent() {
    ServerDataManager.listen(
      S2CProtocol.U_C_REWARD_UPDATE,
      this,
      this.__updateRewardHandler,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_REWARD_FRESH,
      this,
      this.__refreshRewardListHandler,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_REWARD_COUNT,
      this,
      this.__updateRewardCountHandler,
    );
  }

  private __updateRewardCountHandler(pkg: PackageIn) {
    let msg: RewardCountUpdateRspMsg = pkg.readBody(
      RewardCountUpdateRspMsg,
    ) as RewardCountUpdateRspMsg;
    this.model.maxRewardCount =
      OfferRewardModel.BASE_REWARD_COUNT + msg.rewardCount;
      ...
      if (rewardInfo && rewardInfo.isSelect) {
        if (this.model.baseRewardDic[rewardInfo.templateId]) {
          ...
          if (rewardInfo.isComplete) {
            ...
            finishReward.changeDate =
              PlayerManager.Instance.currentPlayerModel.sysCurTimeBySecond;
            this.model.baseRewardDic[rewardInfo.templateId].rewardInfo =
              rewardInfo;
            this.dispatchEvent(
              RewardEvent.REWARD_TASK_UPDATE,
              this.model.baseRewardDic[rewardInfo.templateId],
            );
          }
        } else {
  ...
  private __refreshRewardListHandler(pkg: PackageIn) {
    let msg: RewardFreshRspMsg = pkg.readBody(
      RewardFreshRspMsg,
    ) as RewardFreshRspMsg;
    this.model.lastRefreshRewardListDate = DateFormatter.parse(
      msg.rewardLastdate,
      "YYYY-MM-DD hh:mm:ss",
    );
    ...
    if (rewardDetails) {
      ...
      for (let i: number = 0; i < msg.templateId.length; i++) {
        ...
        tempRewardTaskIndex.add(i + 1, { value: i });
      }
    } else {
      for (let i: number = 0; i < msg.templateId.length; i++) {
        ...
        tempRewardTaskIndex.add(i, { value: i });
      }
    }
    ...
    if (
      !UIManager.Instance.isShowing(EmWindow.OfferRewardWnd) &&
  ...
  public get completedRewardList(): any[] {
    ...
    for (let key in this.model.baseRewardDic) {
      if (Object.prototype.hasOwnProperty.call(this.model.baseRewardDic, key)) {
        ...
        if (reward.isCompleted) arr.push(reward);
      }
    }
    return arr;
  }
  ...
  public clear() {
    ...
    if (this.model.offerRewardTaskStateDic)
      this.model.offerRewardTaskStateDic.clear();
    if (this.model.offerRewardTaskProfileDic)
      this.model.offerRewardTaskProfileDic.clear();
    if (this.model.offerRewardTaskIndex)
      this.model.offerRewardTaskIndex.clear();
