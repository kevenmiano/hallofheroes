...
export default class PetCampaignResultWnd extends BaseWindow {
  ...
  private initView() {
    ...
    if (this._currentData) {
      ...
      this.levelTxt.text =
        str +
        " " +
        LangManager.Instance.GetTranslation("petCampaign.Level", this._level);
      ...
      if (this.hasNext()) {
        //还有下一关
        this._nextData =
          PetCampaignManager.Instance.model.getCurLevelList()[this._level];
        if (this._nextData) {
          ...
          if (this._nextData.ConsumeType == 0) {
            //消耗体力
            this.costIcon.url = FUIHelper.getItemURL(
              "Base",
              "Icon_Unit_Stamina_S",
            );
            let hasEnergy: number =
              PlayerManager.Instance.currentPlayerModel.playerInfo.weary;
            if (hasEnergy < this._nextData.ConsumeNum) {
              //体力不足
              this.costTxt.color = "#FF0000";
            }
          } else {
            this.costIcon.url = IconFactory.getGoodsIconByTID(
              this._nextData.ConsumeType,
            );
            let hasGoodsCount: number =
  ...
  private hasNext(): boolean {
    ...
    let userUiPlayInfoMsg: UserUiPlayInfoMsg =
      this.petCampaignModel.userUiPlayInfoMsg;
    let curList = PetCampaignManager.Instance.model.getCurLevelList();
    if (
      curList &&
      userUiPlayInfoMsg &&
      userUiPlayInfoMsg.levelSort == curList.length
    ) {
      //已经全部通关过了
      if (
        userUiPlayInfoMsg &&
        this._battleReportMsg.param1 >= userUiPlayInfoMsg.levelSort
      ) {
        flag = false;
      }
    } else {
  ...
  private exitBtnHander() {
    ...
    FrameCtrlManager.Instance.open(EmWindow.PetCampaignWnd, {
      playId: this._currentData.playId,
    });
  }
    ...
    if (this._nextData.ConsumeType == 0) {
      ...
      if (hasEnergy < this._nextData.ConsumeNum) {
        ...
        PetCampaignManager.Instance.sendUIPlayChallenge(
          ...
          this._nextData.UiPlayId,
          this._nextData.UiLevelId,
        );
      }
    } else {
      PetCampaignManager.Instance.sendUIPlayChallenge(
  ...
  private renderListItem(index: number, item: BaseItem) {
    ...
    if (this._goodsList[index].rewardType == 1) {
      //1 英灵战役首通奖励
      (item.getChild("title") as fairygui.GTextField).text =
        LangManager.Instance.GetTranslation(
