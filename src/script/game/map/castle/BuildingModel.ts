...
export default class BuildingModel extends Laya.EventDispatcher {
  ...
  public get queneList(): Array<object> {
    var teclist: Array<BuildingOrderInfo> =
      BuildingManager.Instance.model.tecOrderList;
    var colosseList: Array<BuildingOrderInfo> =
      BuildingManager.Instance.model.colosseumOrderList;
    var alchemylist: Array<BuildingOrderInfo> =
      BuildingManager.Instance.model.alchemyOrderList;
    var buildlist: Array<BuildingOrderInfo> =
    ...
    for (
      i = 0;
      i < colosseList.length && ArmyManager.Instance.thane.grades >= 16;
      i++
    ) {
      var item = { vData: null, type: 0 };
    ...
    if (i < 2) {
      //主界面内, 左侧功能栏中, 当前玩家为vip玩家后, 增加建筑队列该关闭
      var item = { vData: null, type: 0 };
      item.vData = null;
    ...
    for (let i: number = 0; i < datalist.length; i++) {
      ...
      if (itemData.vData && itemData.vData.type != 4) {
        itemData.vData.removeEventListener(
          Laya.Event.COMPLETE,
          this.__completeHandler,
          this,
        );
        itemData.vData.addEventListener(
          Laya.Event.COMPLETE,
          this.__completeHandler,
          this,
        );
  ...
  public setOrderList(value: Array<BuildingOrderInfo>) {
    ...
    for (let i: number = 0; i < len; i++) {
      ...
      if (order.orderType == 1) {
        if (order.remainTime > 0) this.buildOrderCount++;
        this._buildOrderList.push(order);
      } else if (order.orderType == 2) {
        if (order.remainTime > 0) this.tecOrderCount++;
        this._tecOrderList.push(order);
      } else if (order.orderType == 3) {
        if (order.remainTime > 0) this.alchemyOrderCount++;
  ...
  private checkHasSameId(
    arr: Array<BuildingOrderInfo>,
    orderId: number,
  ): boolean {
  ...
  public canBuildOrUpgradeByBuildingInfo(bInfo: BuildInfo): string {
    if (bInfo == null) {
      let str: string = LangManager.Instance.GetTranslation(
        "castle.datas.BuildingModel.str",
      );
    }
    let template: t_s_buildingtemplateData = bInfo.templeteInfo;
    if (bInfo.templateId == 0) {
      template = TempleteManager.Instance.getMinGradeBuildTemplate(
        template.SonType,
      );
    } else {
      template = TempleteManager.Instance.getBuildTemplateByID(
        template.NextGradeTemplateId,
      );
      if (template == null)
        return LangManager.Instance.GetTranslation(
          "SeminaryFrameRightView.command06",
        );
    }
  ...
  public canBuildOrUpgradeByBuildingInfoTec(bInfo: BuildInfo): string {
    ...
    if (template && template.PreTemplateId != 0) {
      let preTemplate: t_s_buildingtemplateData =
        TempleteManager.Instance.getBuildTemplateByID(template.PreTemplateId);
      existResult =
        LangManager.Instance.GetTranslation(
          "public.level.name",
          preTemplate.BuildingNameLang,
          preTemplate.BuildingGrade,
        ) + ", ";
      let buildingInfo: BuildInfo = this.getBuildingInfoBySonType(
        preTemplate.SonType,
      );
  ...
  public getPreBuilding(
    ...
    if (temp.PreTemplateId != 0) {
      let preTemplate: t_s_buildingtemplateData =
        TempleteManager.Instance.getBuildTemplateByID(temp.PreTemplateId);
      existResult +=
        LangManager.Instance.GetTranslation(
          "public.level.name",
          preTemplate.BuildingNameLang,
    ...
    if (list) {
      for (let i: number = 0; i < list.length; i++) {
        ...
        if (
          ...
          element.templeteInfo.MasterType == masterType
        ) {
          bInfo = element;
        }
      }
    ...
    if (list) {
      for (let i: number = 0; i < list.length; i++) {
        ...
        if (
          element &&
          element.templeteInfo &&
          element.templeteInfo.SonType == sonType
        ) {
          bInfo = element;
          break;
        }
  ...
  public getTitanPawnTimeEffect(temp: t_s_pawntemplateData): number {
    let arr: Array<t_s_buildingtemplateData> =
      this.getMaxEffectBuildingTemplate(temp);
    ...
    let titanInfo: BuildInfo = this.getBuildingInfoByMasterType(
      MasterTypes.MT_TITAN,
    );
    if (titanInfo) {
      let num: number =
        parseInt(
          (bTemp.Property3 - titanInfo.templeteInfo.Property3).toString(),
        ) / 100;
  ...
  public getMaxEffectBuildingTemplate(
    ...
    let buildTemp: t_s_buildingtemplateData =
      TempleteManager.Instance.getBuildTemplateByID(preTemplateId); //取得每一个建筑的薄
   ...
   * @return
   */
  public isExistBuildingBySontypeAndLevel(
    sonType: number,
    level: number,
  ): boolean {
    let flag: boolean = false;
    ...
    if (list) {
      for (let i: number = 0; i < list.length; i++) {
        ...
        if (
          element &&
          element.sonType == sonType &&
          element.templeteInfo &&
          element.templeteInfo.BuildingGrade >= level
        ) {
          flag = true;
        }
      }
  ...
  public isMaxLevel(buildingId: number): boolean {
    ...
    if (bInfo.templateId == 0) return false;
    if (
      TempleteManager.Instance.getBuildTemplateByID(bInfo.templateId)
        .NextGradeTemplateId == 0
    ) {
      return true;
    }
    return false;
  ...
  public getMinLevelBuindInfo(): BuildInfo {
    ...
    for (let i: number = 0; i < list.length; i++) {
      ...
      if (
        element.templeteInfo &&
        (element.templeteInfo.SonType == BuildingType.OFFICEAFFAIRS ||
  ...
  public checkHasTecCanUpdate(): boolean {
    ...
    if (tecArr.length > 0) {
      ...
      for (let i: number = 0; i < len; i++) {
        ...
        if (
          tecInfo &&
          BuildingManager.Instance.isUpgradeAvaliableNotTips(tecInfo)
        ) {
          flag = true;
          break;
        }
