...
export default class SiteZoneWnd extends BaseWindow {
    ...
    constructor() {
        ...
        let type1 = [
            RPT_EVENT.CLICK_US_CHOOSE,
            RPT_EVENT.CLICK_US_CONFIRM
        ]
        let type2 = [
            RPT_EVENT.CLICK_EU_CHOOSE,
            RPT_EVENT.CLICK_EU_CONFIRM
        ]
        let type3 = [
            RPT_EVENT.CLICK_OA_CHOOSE,
            RPT_EVENT.CLICK_OA_CONFIRM
        ]
    ...
    public OnInitWind(): void {
        ...
        this.frame.getChild('title').text = LangManager.Instance.GetTranslation("switch.siteZone.title");
        ...
        if (this.selectItemData) {
            ...
            if (lastData) {
                if (lastData.siteID != this.selectItemData.siteID) {
                    ...
                    let msg = LangManager.Instance.GetTranslation("switch.siteZone.confirmtips", this.selectItemData.siteName[langCfg.key]);
                    SimpleAlertHelper.Instance.Show(SimpleAlertHelper.SIMPLE_ALERT, this.selectItemData, prompt, msg, confirm1, cancel1, (ok: boolean, v2: boolean, data) => {
                        if (ok) {
                            saveZoneData(data);
    ...
    private adjustEvent(type: number, value: number) {
        ...
        if(this.trackEvent) {
            ...
            if (keyValue) {
                ...
                SDKManager.Instance.getChannel().trackEvent(0, keyValue, keyName, keyDes, "", "");
            }
        }
    }
    ...
    public OnShowWind(): void {
        super.OnShowWind();
        SDKManager.Instance.getChannel().trackEvent(0, TrackEventNode.Node_1004, "openSelectSiteWnd", "进入选大区界面", "", "");
