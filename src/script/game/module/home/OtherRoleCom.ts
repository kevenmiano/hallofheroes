...
export default class OtherRoleCom extends FUI_OtherRoleCom {
  ...
  public setRoleInfo(data: BaseArmy): void {
    ...
    if (data) {
      (<fgui.GLoader>this.playerIcon.getChild("n0")).url =
        IconFactory.getPlayerIcon(data.baseHero.headId, IconType.HEAD_ICON);
  ...
  private isSelf(): boolean {
    ...
    return (
      this.playerInfo.userId == this._data.userId &&
      this.playerInfo.serviceName == this._data.baseHero.serviceName
    );
  }
