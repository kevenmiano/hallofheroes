...
export default class MountRefiningItem extends FUI_MountRefiningItem {
    ...
    private refreshView() {
        ...
        if (this._isMax) {//达到最大
            ...
            if(this._index == 4){//显示兽魂值
                this.maxValueTxt.text = "+" + (this.starLevel * this._vData.StarSoulScore).toString();
            }
            else{ 
                this.maxValueTxt.text = "+" + (this.starLevel * this._vData.StarPower).toString();
            }
        }
        else {
        ...
        if(this._index == 4){
            this.valueTxt1.text = "+" + (this.starLevel * this._vData.StarSoulScore).toString();
            this.valueTxt2.text = "+" + ((this.starLevel + 1) * this._vData.StarSoulScore).toString();
            this.valueTxt3.text = "(" + (this._vData.StarSoulScore).toString() + ")";
        }
        else{
            this.valueTxt1.text = "+" + (this.starLevel * this._vData.StarPower).toString();
            this.valueTxt2.text = "+" + ((this.starLevel + 1) * this._vData.StarPower).toString();
    ...
    private getName(index: number): string {
        ...
        switch (index) {
            case 0:
                nameStr = LangManager.Instance.GetTranslation("armyII.ThaneAttributeView.Tip01");
                break;
            case 1:
                nameStr = LangManager.Instance.GetTranslation("armyII.ThaneAttributeView.Tip03");
                break;
            case 2:
                nameStr = LangManager.Instance.GetTranslation("armyII.ThaneAttributeView.Tip04");
                break;
            case 3:
                nameStr = LangManager.Instance.GetTranslation("armyII.ThaneAttributeView.Tip02");
                break;
            case 4:
                nameStr = LangManager.Instance.GetTranslation("sort.view.MemberTitleView.soulScore");
                break;
        }
        return nameStr;
    }
