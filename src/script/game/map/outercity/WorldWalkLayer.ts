...
export class WorldWalkLayer extends Sprite implements IEnterFrame {
  ...
  private __addToStageHandler(evt: Event): void {
    let arr: any[] = [WalkMouseEventMediator, OutercityNpcActivationMediator];
  ...
  private updateVehicleView() {
    for (let key in this._model.allVehicleNode) {
      ...
      if (wildLand) {
        vehicleArmyView = this._model.allVehicleViewDic.get(
          wildLand.templateId,
        );
        if (wildLand.status == 2 && vehicleArmyView) {
          ...
          if (vehicleArmyView) {
            ...
            vehicleArmyView.newX = parseInt(
              (wildLand.movePosX * 20).toString(),
            );
            vehicleArmyView.newY = parseInt(
              (wildLand.movePosY * 20).toString(),
            );
          }
        }
      }
  ...
  private layMapArmy(arr: any[]): void {
    for (let i = 0, len = arr.length; i < len; i++) {
      ...
      if (
        vArmy.x + vArmy.y == 0 ||
        (p1.distance(p2.x, p2.y) > 150 &&
          aInfo.id != ArmyManager.Instance.army.id)
      ) {
  ...
  public enterFrame(): void {
    ...
    for (const dicKey in this._model.allVehicleViewDic) {
      if (this._model.allVehicleViewDic.hasOwnProperty(dicKey)) {
        let bArmy: OutercityVehicleArmyView =
          this._model.allVehicleViewDic[dicKey];
  ...
  private checkPlayerVisible(armyView: OuterCityArmyView) {
    if (armyView) {
      if (armyView.isSelf || armyView.isTeammate) {
        //自己或者队伍可见
        ...
        let inDistance =
          this.selfArmy &&
          this.selfArmy.armyView &&
          Math.abs(armyView.x - this.selfArmy.armyView.x) <=
            this.CAN_SEE_LONG &&
          Math.abs(armyView.y - this.selfArmy.armyView.y) <= this.CAN_SEE_LONG;
        if (inDistance) {
          //距离符合
  ...
  private disposeAllArmyView(): void {
    ...
    dic = null;
    let allVehicleViewDic: Dictionary =
      OuterCityManager.Instance.model.allVehicleViewDic;
    for (const dicKey in allVehicleViewDic) {
      let outercityVehicleArmyView: OutercityVehicleArmyView =
        allVehicleViewDic[dicKey];
  ...
  public checkClickPlayerNum(posx: number, posy: number): boolean {
    ...
    setTimeout(() => {
      this.clickPlayerFlag = false;
    }, 1000);

    let armyArr = [];
    this.clickPlayerArr = [];

    let mapView = (SceneManager.Instance.currentScene as OuterCityScene)
      .mapView;
    ...
    if (mapView && mapView.npcLayer) {
      ...
      npcList.forEach((npcView) => {
        if (
          npcView.avatarView &&
          npcView.avatarBaseViewType == eAvatarBaseViewType.OuterCityNpc
        ) {
    ...
    if (this.clickPlayerArr.length > 1) {
      ...
      if (nodeView instanceof HeroAvatarView) {
        if (nodeView.avatarBaseViewType == eAvatarBaseViewType.OuterCityArmy) {
          ...
          nodeView.avatarBaseViewType == eAvatarBaseViewType.OuterCityNpc
        ) {
