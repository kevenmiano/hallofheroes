...
import { IMFrameEvent, IMEvent, ChatEvent } from "../../constant/event/NotificationEvent";
...
export default class HomePrivate extends FUI_HomePrivate {
    ...
    private initEvent(){
        ...
        IMManager.Instance.addEventListener(IMFrameEvent.REMOVE, this.updateHomePrivateVisible, this);
        IMManager.Instance.addEventListener(IMEvent.MSG_LIST_DEL, this.updateHomePrivateVisible, this);
        ...
        NotificationManager.Instance.addEventListener(ChatEvent.SHOW_NEW_MSG, this.getPrivatePerson, this);
    }

    private removeEvent(){
        this._playerIcon.offClick(this, this.playerHandler);
        IMManager.Instance.removeEventListener(IMFrameEvent.REMOVE, this.updateHomePrivateVisible, this);
        IMManager.Instance.removeEventListener(IMEvent.MSG_LIST_DEL, this.updateHomePrivateVisible, this);
        ...
        NotificationManager.Instance.removeEventListener(ChatEvent.SHOW_NEW_MSG, this.getPrivatePerson, this);
    }
    ...
    public getPrivatePerson() {
        ...
        if (this._privatePersonData && !this._privatePersonData.isRead && this._privatePersonData.senderName != ArmyManager.Instance.thane.nickName && SharedManager.Instance.privacyMsgCount > 0) {
    ...
    public set info(value:ChatData){
        ...
        if(this._info){
            if(this._info.headId == 0){
                this._playerIcon.url = IconFactory.getHeadIcon(this._info.job, IconType.HEAD_ICON) 
            }
            else{
                this._playerIcon.url = IconFactory.getHeadIcon(this._info.headId, IconType.HEAD_ICON) 
            }
        }
        else{
