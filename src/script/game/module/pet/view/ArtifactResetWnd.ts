...
export default class ArtifactResetWnd extends BaseWindow {
    ...
    OnShowWind() {
        ...
        this.artIcon1.isEquip.selectedIndex = GoodsCheck.hasEquipArtifact(this.artIcon1.info)?1:0;
        this.artIcon1.isIdentify.selectedIndex = GoodsCheck.hasIdentify(this.artIcon1.info ) ? 1 : 0;
        this.addEvent();
        this.updateView();
    }
    ...
    private addEvent() {
        ...
        NotificationManager.Instance.addEventListener(NotificationEvent.ARTIFACT_SELECT_GOODS_UPDATE, this.updateSelectView, this);
        NotificationManager.Instance.addEventListener(NotificationEvent.ARTIFACT_CANCEL_GOODS_UPDATE, this.updateCancelView, this);
        NotificationManager.Instance.addEventListener(NotificationEvent.ARTIFACT_SUCCESS_UPDATE,this.refreshView,this);
    }
    ...
    private removeEvent() {
        ...
        NotificationManager.Instance.removeEventListener(NotificationEvent.ARTIFACT_SELECT_GOODS_UPDATE, this.updateSelectView, this);
        NotificationManager.Instance.removeEventListener(NotificationEvent.ARTIFACT_CANCEL_GOODS_UPDATE, this.updateCancelView, this);
        NotificationManager.Instance.removeEventListener(NotificationEvent.ARTIFACT_SUCCESS_UPDATE,this.refreshView,this);
    }
    ...
    private onClickItem(selectedItem: ArtifactCell) {
        ...
        PlayerManager.Instance.currentPlayerModel.currentSelectArtifact = selectedItem.info;
    }
    ...
    private updateSelectView(goodsInfo: GoodsInfo) {
        if (this.artIcon1.info && this.artIcon2.info) {//1和2格子全部有神器了
            MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("ArtifactResetWnd.updateSelectTips"));
            return;
        } else if ((this.artIcon1.info && this.artIcon1.info.id == goodsInfo.id)
            || (this.artIcon2.info && this.artIcon2.info.id == goodsInfo.id)) {//已经装备上的
            MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("ArtifactResetWnd.updateSelectTips2"));
            return;
        }else if(!this.checkGrade(goodsInfo)){//等级不满足
            MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("ArtifactResetWnd.updateSelectTips3"));
            return;
        } else {
            //被选中的神器加上选中标识。从右侧找一个空格子填充进去
            if (this.artIcon1.info) {//1号格子有神器了
                this.artIcon2.type = 1;
                this.artIcon2.selectType = 1;
                this.artIcon2.info = goodsInfo;
                this.artIcon2.isEquip.selectedIndex = GoodsCheck.hasEquipArtifact(this.artIcon2.info)?1:0;
                this.artIcon2.isIdentify.selectedIndex = GoodsCheck.hasIdentify(this.artIcon2.info ) ? 1 : 0;
            } else {
                this.artIcon1.type = 1;
                this.artIcon1.selectType = 1;
    ...
    private updateCancelView(info: GoodsInfo) {
        if (this.artIcon1.info && this.artIcon1.info.id == info.id) {//取消的是1号格子
    ...
    private updateRightView() {
        if(this.artIcon1.info && this.artIcon2.info){//两个位置都有神器
            this.resetBtn.enabled = true;
            let level = 0;
            let addPrecent = 0;
            let templateData1: t_s_petartifactpropertyData = TempleteManager.Instance.getArtifactTemplate(this.artIcon1.info.templateId);
            let templateData2: t_s_petartifactpropertyData = TempleteManager.Instance.getArtifactTemplate(this.artIcon2.info.templateId);
            ...
            this.levelTxt1.text = LangManager.Instance.GetTranslation("public.level3",level) + ":";
            this.costNumTxt.text = this._costDiamond.toString();
            this.levelTxt2.text = LangManager.Instance.GetTranslation("public.level3",level-1) + ":";
    ...
    helpBtnClick() {
		let title = LangManager.Instance.GetTranslation("ArtifactResetWnd.HelpTitle");
		let content = LangManager.Instance.GetTranslation("ArtifactResetWnd.content");
    ...
    private isSelect(info: GoodsInfo): boolean {
        ...
        if (info) {
            if ((this.artIcon1 && this.artIcon1.info && info.id == this.artIcon1.info.id)
                || (this.artIcon2 && this.artIcon2.info && info.id == this.artIcon2.info.id)) {
                flag = true;
            }
        }
        return flag;
    ...
    private checkGrade(info: GoodsInfo): boolean {
        let flag: boolean = false;
        let templateId = this.artIcon1.info?this.artIcon1.info.templateId:this.artIcon2.info?this.artIcon2.info.templateId:0;
        let templateData1: t_s_petartifactpropertyData = TempleteManager.Instance.getArtifactTemplate(templateId);
        let templateData: t_s_petartifactpropertyData = TempleteManager.Instance.getArtifactTemplate(info.templateId);
        if(templateId >0){
            if (templateData1 && Math.abs(templateData1.Level - templateData.Level) <= 1) {
                flag = true;
            }
        }else{
            flag = true;
    ...
    private resetBtnHandler() {
        ...
        let content: string = LangManager.Instance.GetTranslation("ArtifactResetWnd.resetBtn.AlertContent",this._costDiamond);
        SimpleAlertHelper.Instance.Show(SimpleAlertHelper.SIMPLE_ALERT, null, prompt, content, confirm, cancel, (b: boolean) => {
            if (b) {
                ...
                if(this.artIcon1.info && this.artIcon2.info){
                    PetCtrl.sendArtifactReset(this.artIcon1.info.pos,this.artIcon1.info.objectId,this.artIcon2.info.pos,this.artIcon2.info.objectId);
                }
            }
    ...
    private refreshView(){
        ...
        this.artIcon1.isIdentify.selectedIndex = this.artIcon2.isIdentify.selectedIndex = 1;
        this.artIcon1.isEquip.selectedIndex = 0;
        this.artIcon2.isEquip.selectedIndex = 0;
        this.resetBtn.enabled = true;
        this.updateView();
