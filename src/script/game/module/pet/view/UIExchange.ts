...
export default class UIExchange extends BaseFguiCom {
  ...
  public set data(value: PetData) {
    if (value) {
      ...
      if (this._data && this._data.petId !== value.petId) {
        if (this._targetData)
          this._targetData.removeEventListener(
            PetEvent.PETINFO_CHANGE,
            this.onTargetPetChange,
            this,
          );
  ...
  private addEvent() {
    ...
    this.comp_targetDetail
      .getChild("comp_petItem")
      .onClick(this, this.onSelectTarget);
    this.comp_targetDetail
      .getChild("btn_selectTarget")
      .onClick(this, this.onSelectTarget);

    ServerDataManager.listen(
      S2CProtocol.U_C_PET_OP_RESULT,
      this,
    ...
    this.comp_sourceDetail.comb_starList.itemRenderer = Laya.Handler.create(
      this,
      this.onRenderSourceStar,
      null,
      false,
    );
    this.comp_targetDetail.comb_starList.itemRenderer = Laya.Handler.create(
      this,
      this.onRenderTargetStar,
      null,
      false,
    );
  }
  ...
  refreshSource() {
    ...
    this.comp_sourceDetail.label_development.text =
      LangManager.Instance.GetTranslation("pet.growthRate") + colon;
    this.comp_sourceDetail.label_powerFlair.text =
      LangManager.Instance.GetTranslation("pet.strengthCoe") + colon;
    this.comp_sourceDetail.label_armorFlair.text =
      LangManager.Instance.GetTranslation("pet.armorCoe") + colon;
    this.comp_sourceDetail.label_intelligenceFlair.text =
      LangManager.Instance.GetTranslation("pet.intellectCoe") + colon;
    this.comp_sourceDetail.label_brainFlair.text =
      LangManager.Instance.GetTranslation("pet.staminaCoe") + colon;

    this.comp_sourceDetail.txt_petName.text = this._data.name;
    this.comp_sourceDetail.txt_petName.color = PetData.getQualityColor(
      this._data.quality - 1,
    );
    (this.comp_sourceDetail.comp_petItem as PetItem).infoShowIcon(
      this._data,
      false,
    );
    (this.comp_sourceDetail.comp_petItem as PetItem).state =
      EmPetItemState.ItemUsing;

    this.comp_sourceDetail.txt_development.text =
      this._data.growthRate.toString();
    this.comp_sourceDetail.txt_powerFlair.text =
      this._data.coeStrength + " / " + this._data.coeStrengthLimit;
    this.comp_sourceDetail.txt_armorFlair.text =
      this._data.coeArmor + " / " + this._data.coeArmorLimit;
    this.comp_sourceDetail.txt_intelligenceFlair.text =
      this._data.coeIntellect + " / " + this._data.coeIntellectLimit;
    this.comp_sourceDetail.txt_brainFlair.text =
      this._data.coeStamina + " / " + this._data.coeStaminaLimit;

    this.comp_sourceDetail.btn_petType.icon = FUIHelper.getItemURL(
      EmPackName.Base,
      "Icon_PetType" + this._data.template.PetType,
    );
    this.comp_sourceDetail.btn_petType.title = this._data.petTypeLanguage;
    this.comp_sourceDetail.btn_stage.icon = FUIHelper.getItemURL(
      EmPackName.Base,
      "Icon_PetStage" + this._data.template.Property2,
    );
  ...
  refreshTarget() {
    ...
    this.comp_targetDetail.label_development.text =
      LangManager.Instance.GetTranslation("pet.growthRate") + colon;
    this.comp_targetDetail.label_powerFlair.text =
      LangManager.Instance.GetTranslation("pet.strengthCoe") + colon;
    this.comp_targetDetail.label_armorFlair.text =
      LangManager.Instance.GetTranslation("pet.armorCoe") + colon;
    this.comp_targetDetail.label_intelligenceFlair.text =
      LangManager.Instance.GetTranslation("pet.intellectCoe") + colon;
    this.comp_targetDetail.label_brainFlair.text =
      LangManager.Instance.GetTranslation("pet.staminaCoe") + colon;
    ...
    if (this._targetData) {
      ...
      this.comp_targetDetail.txt_petName.color = PetData.getQualityColor(
        this._targetData.quality - 1,
      );
      (this.comp_targetDetail.comp_petItem as PetItem).infoShowIcon(
        this._targetData,
        false,
      );
      (this.comp_targetDetail.comp_petItem as PetItem).state =
        EmPetItemState.ItemUsing;

      this.comp_targetDetail.txt_development.text =
        this._targetData.growthRate.toString();
      this.comp_targetDetail.txt_powerFlair.text =
        this._targetData.coeStrength +
        " / " +
        this._targetData.coeStrengthLimit;
      this.comp_targetDetail.txt_armorFlair.text =
        this._targetData.coeArmor + " / " + this._targetData.coeArmorLimit;
      this.comp_targetDetail.txt_intelligenceFlair.text =
        this._targetData.coeIntellect +
        " / " +
        this._targetData.coeIntellectLimit;
      this.comp_targetDetail.txt_brainFlair.text =
        this._targetData.coeStamina + " / " + this._targetData.coeStaminaLimit;

      this.comp_targetDetail.btn_petType.icon = FUIHelper.getItemURL(
        EmPackName.Base,
        "Icon_PetType" + this._targetData.template.PetType,
      );
      this.comp_targetDetail.btn_petType.title =
        this._targetData.petTypeLanguage;
      this.comp_targetDetail.btn_stage.icon = FUIHelper.getItemURL(
        EmPackName.Base,
        "Icon_PetStage" + this._targetData.template.Property2,
      );
      this.comp_targetDetail.btn_stage.title =
        this._targetData.petStageLanguage;
    }
  }
  ...
  private onRenderTargetStar(index: number, item: fairygui.GComponent) {
    let next: t_s_upgradetemplateData =
      this._targetData.qualityUpgradeTemplateInfo;
    if (next) {
      this.readerStar(
        index,
        item,
        this._targetData,
        this._targetData.temQuality % 5,
        1,
      );
      if (this._targetData.temQuality <= 20) {
        this.readerStar(
          ...
          item,
          this._targetData,
          (this._targetData.temQuality + 1) % 5,
          2,
        );
      }
    } else {
  ...
  onSelectTarget() {
    ...
    FrameCtrlManager.Instance.open(EmWindow.PetSelect, {
      sourcePet: this._data,
      targetPet: this._targetData,
    });
  }
  ...
  onSelectPet(item: PetData) {
    ...
    if (item) {
      ...
      this._targetData.addEventListener(
        PetEvent.PETINFO_CHANGE,
        this.onTargetPetChange,
        this,
      );
    ...
    if (this._data && this._targetData) {
      ...
      if (GoodsManager.Instance.getGoodsNumByTempId(this._templateId) <= 0) {
        ...
        var data: ShopGoodsInfo =
          TempleteManager.Instance.getShopTempInfoByItemId(this._templateId);
      ...
      MessageTipManager.Instance.show(
        LangManager.Instance.GetTranslation("pet.exchange.pet.lack"),
      );
      return;
    }
  }
  ...
  private onHelp(): void {
    ...
    let content: string =
      LangManager.Instance.GetTranslation("pet.exchange.help");
