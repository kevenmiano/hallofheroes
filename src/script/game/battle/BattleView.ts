...
import { ActionLabesType, BattleModelNotic, BattleType, InheritRoleType } from "../constant/BattleDefine";
import { BattleEvent, BattleNotic, ChatEvent } from "../constant/event/NotificationEvent";
...
export class BattleView extends Laya.Sprite {
    ...
    private addEvent() {
        NotificationManager.Instance.addEventListener(BattleModelNotic.ADD_ROLE, this.__modelAddRole, this);
        NotificationManager.Instance.addEventListener(BattleNotic.REFRESH_BACK_BITMAP, this.__refreshBackGroundBitmap, this);
        NotificationManager.Instance.addEventListener(BattleEvent.SHAKE_MAP_COMPLETE, this.onShakeMapComplete, this);
    ...
    private __playerChatHandler(chatData: ChatData) {
        if (!PlayerManager.Instance.currentPlayerModel.playerInfo.isOpenSettingType9 || chatData.uid == 0) {
            return;
        }
        ...
        if (chatData.channel == ChatChannel.BATTLE_CHAT) {
            ...
            if (!StringHelper.isNullOrEmpty(chatData.serverName)) {
                key = PlayerManager.Instance.currentPlayerModel.userInfo.mainSite + chatData.uid + "_" + chatData.serverName;
            } else {
                key = PlayerManager.Instance.currentPlayerModel.userInfo.mainSite + chatData.uid;
            }
        } else {
            ...
            if (!StringHelper.isNullOrEmpty(chatData.serverName)) {
                targetRole = this._model.getHeroRoleByUserId(userId, chatData.serverName);
                if (targetRole && (targetRole.side == this._model.selfHero.side)) {
                    showPop = true;
                }
                key = PlayerManager.Instance.currentPlayerModel.userInfo.mainSite + userId + "_" + chatData.serverName;
            } else {
                targetRole = this._model.getHeroRoleByUserId(userId);
                if (targetRole && (targetRole.side == this._model.selfHero.side)) {
                    showPop = true;
                }
                key = PlayerManager.Instance.currentPlayerModel.userInfo.mainSite + userId;
            }
        ...
        if (showPop) {
            ...
            if (!chatPopView) {
                chatPopView = FUIHelper.createFUIInstance(EmPackName.Base, "ChatBubbleTip");
                this.chatPopViewDic[key] = chatPopView;
            }
            chatPopView.updateContent(chatData.htmlText, chatData.encodemsg, null, 0, chatData.channel);
    ...
    private buildMap() {
        ...
        this._map.changeSingleMap(BattleManager.Instance.resourceModel.battleMapRes);
    ...
    private initRoles(roles: Dictionary) {
        for (const key in roles) {
            if (Object.prototype.hasOwnProperty.call(roles, key)) {
                ...
                let temp: Laya.Point = BattleUtils.rolePointByPos(roleInfo.pos, roleInfo.face);
    ...
    public addRole(roleInfo) {
        ...
        roleInfo.defaultAction = roleInfo.isLiving ? ActionLabesType.STAND : ActionLabesType.DIE;

        this.addRoleView(role);
        if (roleInfo.inheritType == InheritRoleType.Hero && (roleInfo).type != 3 && this.checkBattleType()) {
    ...
    protected __refreshBackGroundBitmap(event: Event) {
        ...
        if (BattleManager.Instance.mainViewContainer.view) {
            this._bitmap_background.texture = this.drawDisplayObject(BattleManager.Instance.mainViewContainer.view);
    ...
    public drawDisplayObject(obj: DisplayObject = null): Laya.Texture {
        ...
        if (obj instanceof Laya.Stage || (rect.width != 0 && rect.height != 0)) {
            let bimap_rect: Laya.Rectangle = new Laya.Rectangle(0, 0, Math.abs(rect.width), Math.abs(rect.height));
            ...
            let bitmapdata: Laya.Texture = new Laya.Texture(obj.texture, null, bimap_rect.width, bimap_rect.height);
    ...
    public dispose() {
        ...
        NotificationManager.Instance.removeEventListener(BattleModelNotic.ADD_ROLE, this.__modelAddRole, this);
        NotificationManager.Instance.removeEventListener(BattleNotic.REFRESH_BACK_BITMAP, this.__refreshBackGroundBitmap, this);
        NotificationManager.Instance.removeEventListener(BattleEvent.SHAKE_MAP_COMPLETE, this.onShakeMapComplete, this);
        ...
        ObjectUtils.disposeObject(this._redScreenEffect); this._redScreenEffect = null;
