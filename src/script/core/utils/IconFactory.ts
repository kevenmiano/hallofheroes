...
export class IconFactory {
  ...
  public static getStarIconPath(tempId: number | string): string {
    return (
      PathManager.resourcePath + "icon/star/" + tempId + "/" + tempId + ".json"
    );
  }
  ...
  public static getGoodsIconByTID(tid: number): string {
    let gTemplate: t_s_itemtemplateData = ConfigMgr.Instance.getTemplateByID(
  ...
  public static getBossIcon(mapId: number, med: string = ".png"): string {
    return (
      PathManager.resourcePath + "images/instanceimg/" + mapId + "/icon" + med
    );
  }
  ...
  /**头像 */
  public static getHeadIcon(
    headId: number,
    type: string,
    med: string = ".png",
  ): string {
    return (
      PathManager.resourcePath + "icon/player/" + headId + "/" + type + med
    );
  }

  public static getPlayerIcon(
    templateId: number,
    type: string,
    med: string = ".png",
  ): string {
    return (
      PathManager.resourcePath + "icon/player/" + templateId + "/" + type + med
    );
  }
  ...
  public static getHeadFrame(path: string): string {
    return (
      PathManager.resourcePath + "icon/icon_avatar_frame" + path + "/frame.png"
    );
  }
  ...
  public static getHeroicon(templateId: number): string {
    var hero: t_s_herotemplateData =
      TempleteManager.Instance.gameHeroTemplateCate(templateId);
    return hero ? PathManager.resourcePath + "icon" + hero.Icon : "";
  }

  public static getCropIcon(
    cropTemp: t_s_itemtemplateData,
    cropPhase: number,
  ): string {
    if (!cropTemp) return "";
  ...
  public static getGodArriveBossIcon(campId: number, gateId: number): string {
    ...
    if (data)
      temp = temp = ConfigMgr.Instance.getTemplateByID(
        ConfigType.t_s_herotemplate,
        bossId,
      );
    if (!temp) return null;
    return this.toLowerCase(
      PathManager.resourcePath + "images/level/" + temp.ResPath + ".png",
    );
  }
  ...
  public static getUiPlayIcon(tempId: number): string {
    if (tempId == 0) return null;
    return (
      PathManager.resourcePath + "icon/uiplay/Img_Dungeon_" + tempId + ".png"
    );
  }
