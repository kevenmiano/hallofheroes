...
export default class MailCtrl extends FrameCtrlBase {
  ...
  public deleteEmails(list: Array<number>) {
    ...
    if (!flag) {
      ...
      var content: string = LangManager.Instance.GetTranslation(
        "emailII.EmailControler.content",
      );
      ...
      SimpleAlertHelper.Instance.Show(
        ...
        null,
        cancel,
        this.__alertResponseHandler.bind(this),
      );
    }
  }

  private __alertResponseHandler(
    b: boolean,
    flag: boolean = false,
    id: number = 0,
    type: number = 0,
  ...
  public sendEmail(idList: Array<number>, title: string, content: string) {
    if (
      ResourceManager.Instance.gold.count < MailModel.WRITE_MAIL_NEEDED_GOLD
    ) {
