...
export class PetLandArmyView extends CampaignArmyView implements IBaseMouseEvent {
    constructor() {
        super();
    }
    ...
    private throughDispatch(e: Laya.Event): void {
        ...
        for (let i = len; i > 0; i--) {
            ...
            if (NodeState.displayState(node.info.state)) {
                ...
                if (nodeView && nodeView.mouseEnabled && nodeView != this && nodeView.stage) {
                    let bounds: Laya.Rectangle = nodeView.getBounds();
                    let pos = NewbieBaseActionMediator.localToGlobal(nodeView, new Laya.Point(0, 0))
                    let rect = new Laya.Rectangle(pos.x, pos.y, bounds.width, bounds.height)
