...
export class MoveForwardContext {
    ...
    constructor(skill: BaseSkill, startMoveFun: Function, endMoveFun: Function) {
        this._strategy = MoveForwardContext.createStrategy(skill, startMoveFun, endMoveFun);
    }

    private static createStrategy(skill: BaseSkill, startMoveFun: Function, endMoveFun: Function): IMoveStrategy {
        let strategy: IMoveStrategy;
        let type: number = -1;
        let backData: SkillFrameData = skill.getActionTemplate().getMoveForwardData(skill.getRoleSex())
        if (backData) {
