...
export default class OuterCityTreasureWnd extends BaseWindow {
    ...
    private initData() {
        ...
        let path: string = PathManager.solveMapPhysicsBySonType(this._wildLand.tempInfo.SonType);
        this.updateAvatar(path);
        let tempInfo: t_s_mapphysicpositionData = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_mapphysicposition, this._wildLand.templateId);
        if (tempInfo) {
            let add = tempInfo.Property1;
            this.resouceAddPrecentTxt.text = LangManager.Instance.GetTranslation("OuterCityMapTreasureTips.resouceAddTxt", add);
        }
    ...
    private initEvent() {
        ...
        if (this._outercityModel) this._outercityModel.addEventListener(OuterCityEvent.CURRENT_WILD_LAND, this.updateOccupyPlayerName, this);
        NotificationManager.Instance.addEventListener(OuterCityEvent.UPDATE_TREASURE_INFO, this.initView, this);
    ...
    private removeEvent() {
        ...
        if (this._outercityModel) this._outercityModel.removeEventListener(OuterCityEvent.CURRENT_WILD_LAND, this.updateOccupyPlayerName, this);
        NotificationManager.Instance.removeEventListener(OuterCityEvent.UPDATE_TREASURE_INFO, this.initView, this);
    ...
    private updateOccupyPlayerName() {
        if (this._wildLand.info.occupyLeagueName == "") {
            this.consortiaNamext.text = LangManager.Instance.GetTranslation("maze.MazeFrame.Order");
        }
        else {
            this.consortiaNamext.text = this._wildLand.info.occupyLeagueName;
        }
        if(this.playerModel.treasureState == OuterCityModel.TREASURE_STATE3){//争夺期间
            if(this._wildLand.info.occupyLeagueConsortiaId == 0){//没人占领, 攻击
                this.isShowAttack.selectedIndex = 1;
            }else if(this._wildLand.info.occupyLeagueConsortiaId > 0){//有人占领
                if(this._wildLand.info.occupyLeagueConsortiaId != this.thane.consortiaID){//不是自己公会的
                    this.isShowAttack.selectedIndex = 1;
                }else if(this._wildLand.info.occupyLeagueConsortiaId == this.thane.consortiaID){
    ...
    private initView() {
        ...
        if (this._countDown > 0) {
            this.timeTxt.text = "(" + DateFormatter.getConsortiaCountDate(this._countDown) + ")";
