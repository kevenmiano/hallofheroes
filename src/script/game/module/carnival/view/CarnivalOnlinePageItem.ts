...
export default class CarnivalOnlinePageItem extends FUI_CarnivalOnlinePageItem {
  ...
  protected onConstruct() {
    ...
    !CarnivalManager.Instance.isRewardTime &&
      (this._btn_unlock.title = LangManager.Instance.GetTranslation(
        "carnival.active.timeover",
      ));

    this.Img_Title07.url = this.model.getThemeFolderImgPath(
      CARNIVAL_FOLDER.ONLINE,
      "Img_Title07",
    );
  ...
  private onReceiveClick() {
    if (this.tempInfo)
      CarnivalManager.Instance.opRequest(
        CarnivalManager.OP_ONLINE_REWARD,
        this.tempInfo.Id,
      );
  }
