...
export default class FarmBagTipWnd extends BaseWindow {
  ...
  private updateView() {
    if (this._info) {
      var gTemp: t_s_itemtemplateData = ConfigMgr.Instance.getTemplateByID(
      ...
      if (gTemp) {
        ...
        this.goodsNameTxt.color = ComponentSetting.setFarmItemColor(
          gTemp.Profile,
        );
        ...
        else tempStr = gTemp.Property3.toString();
        this.getGoldTxt.text = tempStr;
        if (this._info.id != 0) {
          if (this._info.isBinds) {
            this.bindTxt.text = LangManager.Instance.GetTranslation(
              "yishi.view.tips.goods.EquipTipsContent.bind1",
            );
            this.bindTxt.color = "#FF0000";
          } else {
            this.bindTxt.text = LangManager.Instance.GetTranslation(
              "yishi.view.tips.goods.EquipTipsContent.bind2",
            );
            this.bindTxt.color = "#FFECC6";
          }
          this.priceTxt.text = LangManager.Instance.GetTranslation(
            "yishi.view.tips.goods.PropTips.price",
            gTemp.SellGold,
          );
        } else {
          this.priceTxt.text = "";
