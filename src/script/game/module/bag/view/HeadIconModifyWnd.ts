...
export class HeadIconModifyWnd extends BaseWindow {
  ...
  public OnInitWind() {
    ...
    this.frame.getChild("title").text = LangManager.Instance.GetTranslation(
      "HeadIconModifyWnd.titleTxt",
    );
    this.txt_head_pre.text = LangManager.Instance.GetTranslation(
      "HeadIconModifyWnd.previewTxt",
    );
  ...
  private addEventListener() {
    ...
    SNSManager.Instance.addEventListener(
      ...
      this.__snsInfoSaveCompleteHandler,
      this,
    );
  ...
  public OnShowWind() {
    super.OnShowWind();
    this.list_head.itemRenderer = Laya.Handler.create(
      this,
      this.renderHeadIconListItem,
      null,
      false,
    );
    this.list_frame.itemRenderer = Laya.Handler.create(
      this,
      this.renderHeadFrameListItem,
      null,
      false,
    );
    this.list_head.numItems = 26; //PathManager.getFaceNum();
    let headlist: Array<t_s_itemtemplateData> =
      TempleteManager.Instance.getGoodsTemplatesBySonType(
        GoodsSonType.SONTYPE_HEADFRAME,
      );
    let filterHeadList: Array<t_s_itemtemplateData> = [];
    let item: t_s_itemtemplateData;
    for (let i: number = 0; i < headlist.length; i++) {
      item = headlist[i];
      if (item && item.Activation == -1) {
        //未激活也展示
        filterHeadList.push(item);
      } else if (HeadIconModel.instance.hasActive(item)) {
        //激活的
        filterHeadList.push(item);
      }
    }
    filterHeadList = ArrayUtils.sortOn(
      filterHeadList,
      "TemplateId",
  ...
  private updateView() {
    ...
    if (!HeadIconModel.instance.hasActive(this._currentItemCfg)) {
      ...
      if (
        this._currentItemCfg &&
        this._currentItemCfg.TemplateId !=
          HeadIconModel.instance.currentEquipFrameId
      ) {
  ...
  private refreshView(value: number | t_s_itemtemplateData) {
    if (typeof value == "number") {
      ...
      this.icon_frame.headEffect =
        Number(tempData.Property1) == 1 ? tempData.Avata : "";
      ...
      if (!HeadIconModel.instance.hasActive(tempData)) {
        ...
        if (
          tempData &&
          tempData.TemplateId != HeadIconModel.instance.currentEquipFrameId
        ) {
          this.operationCtr.selectedIndex = 0;
        } else {
          this.operationCtr.selectedIndex = 1;
  ...
  private onClickFrameItem(item: HeadIcon, evt: Laya.Event) {
    ...
    if (HeadIconModel.instance.hasActive(item.sdata as t_s_itemtemplateData)) {
      //未激活的
      item.redStatus.selectedIndex = 0;
      let headFrameInfo: HeadFrameInfo =
        HeadIconModel.instance.allHeadFrameList.get(
          (item.sdata as t_s_itemtemplateData).TemplateId,
        );
      headFrameInfo.clickNum = 1;
      SharedManager.Instance.headIconClickDic[this.currentItemCfg.TemplateId] =
        1;
  ...
  private renderHeadFrameListItem(index: number, item: HeadIcon) {
    ...
    if (!HeadIconModel.instance.hasActive(item.sdata)) {
      //未激活的
      ...
      if (!SharedManager.Instance.headIconClickDic[item.sdata.TemplateId]) {
        //未点击的
        item.redStatus.selectedIndex = 1;
      }
    }
    item.select.selectedIndex =
      HeadIconModel.instance.currentEquipFrameId == item.sdata.TemplateId
        ? 1
        : 0;
  }
