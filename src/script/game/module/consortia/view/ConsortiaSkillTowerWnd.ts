...
export class ConsortiaSkillTowerWnd extends BaseWindow {
  ...
  private initEvent() {
    ...
    this._model.addEventListener(
      ConsortiaEvent.UPDA_CONSORTIA_SKILL_LIST,
      this.__onSkillListUpdata,
      this,
    );
    this._model.addEventListener(
  ...
  private onTabChanged(cc: fgui.Controller) {
    ...
    if (tabIndex == 0) {
      ...
      this.consortiaWealthTxt.text =
        this._model.consortiaInfo.consortiaMaterials + "";
    }
    ...
    if (this._skillInfos.length > 0) {
      ...
      this.onListItemClick(
        this.list.getChildAt(childIndex) as ConsortiaSkillTowerItem,
        null,
      );
    }
    this.updateBagPage();
  ...
  private initView() {
    ...
    let flag: boolean = this._contorller.getRightsByIndex(
      ConsortiaDutyInfo.UPDATESKILL,
    );
    if (flag) {
  ...
  private onResearchSkillBtnClick() {
    ...
    if (this.c1.selectedIndex == 0) {
      if (
        this._selectedSkillInfo.nextTemplateInfo.NeedOffer >
        this._model.consortiaInfo.offer
      ) {
        ConsortiaSkillHelper.addWealth();
        return;
      }
      this._contorller.consortiaUpgrade(
        this._selectedSkillInfo.nextTemplateInfo,
      );
    } else {
      if (
        this._selectedSkillInfo.nextTemplateInfo.NeedOffer >
        ...
        MessageTipManager.Instance.show(
          LangManager.Instance.GetTranslation(
            "ConsortiaSkillTowerWnd.researchSkillBtn.tips",
          ),
        );
        return;
      }
  ...
  private onListItemClick(item: ConsortiaSkillTowerItem, evt: Laya.Event) {
    ...
    this._selectedSkillInfo.addEventListener(
      ConsortiaEvent.ON_TEMPLETEID_UPDATA,
      this.__onSkillListUpdata,
      this,
    );
  ...
  private updateSelectedSkillInfo() {
    ...
    if (this.c1.selectedIndex == 0) {
      consortiaSkillLevel = this._model.consortiaInfo.getLevelByUpgradeType(
        this._selectedSkillInfo.type,
      ); //公会的技能等级
      playerSkillLevel =
      ...
      //高级公会技能
      consortiaSkillLevel = this._model.consortiaInfo.getHighLevelByUpgradeType(
        this._selectedSkillInfo.type,
      ); //公会的技能等级
      playerSkillLevel =
    ...
    let currPlayerTemp: t_s_consortialevelData =
      TempleteManager.Instance.getConsortiaTempleteByTypeAndLevel(
        this._selectedSkillInfo.type,
        playerSkillLevel,
      ); //玩家个人的技能等级相关的模板数据
    let nextPlayerTemp: t_s_consortialevelData =
      TempleteManager.Instance.getConsortiaTempleteByTypeAndLevel(
        this._selectedSkillInfo.type,
        playerSkillLevel + 1,
      ); //玩家个人的技能等级相关的模板数据
    ...
    this.txt_grade.text = LangManager.Instance.GetTranslation(
      "public.level3",
      `${playerSkillLevel}/${consortiaSkillLevel}`,
    );
    this.txt_currDesc.text = currPlayerTemp
      ? currPlayerTemp.DescriptionLang
      : LangManager.Instance.GetTranslation("public.notHave");
    this.txt_nextDesc.text = nextPlayerTemp
      ? nextPlayerTemp.DescriptionLang
      : LangManager.Instance.GetTranslation(
          "Consortia.SkillTower.ReachMaxLevel",
        );

    //学习条件
    if (!nextPlayerTemp) {
      this.txt_studyLimit0.text = LangManager.Instance.GetTranslation(
        "consortia.view.myConsortia.skill.ConsortiaSkillItem.tip.title2",
      );
    } else {
      this.txt_studyLimit0.text = LangManager.Instance.GetTranslation(
        "consortia.view.myConsortia.skill.ConsortiaSkillItem.tips",
      );
    }
    ...
    if (this.c1.selectedIndex == 0) {
      levelTemp = TempleteManager.Instance.getConsortiaTempleteById(
        this._selectedSkillInfo.nextTemplateInfo.PreTemplateId,
      );
      if (consortiaSkillLevel >= ConsortiaUpgradeType.MAX_LEVEL) {
        this.txt_studyLimit1.text = LangManager.Instance.GetTranslation(
          "consortia.view.myConsortia.skill.ConsortiaSkillItem.tip.title2",
        );
      } else {
        this.txt_studyLimit1.text = LangManager.Instance.GetTranslation(
          "consortia.view.myConsortia.skill.ConsortiaResearchSkillItem.limitTxtTip",
          levelTemp.Levels,
        );
      }
      if (
        skillTowerLevel < levelTemp.Levels ||
        consortiaSkillLevel >= ConsortiaUpgradeType.MAX_LEVEL
      ) {
      ...
      this.txt_price0.text = nextPlayerTemp
        ? nextPlayerTemp.Property2.toString()
        : "---";
    } else if (this.c1.selectedIndex == 1) {
      nextTemplateInfo =
        TempleteManager.Instance.getConsortiaTempleteByTypeAndLevel(
          this._selectedSkillInfo.type,
          consortiaSkillLevel + 1,
        );
      if (nextTemplateInfo) {
        levelTemp = TempleteManager.Instance.getConsortiaTempleteById(
          nextTemplateInfo.PreTemplateId,
        );
      }
      if (!nextTemplateInfo) {
        this.txt_studyLimit1.text = LangManager.Instance.GetTranslation(
      ...
      if (
        (levelTemp && skillTowerLevel < levelTemp.Levels) ||
        !nextTemplateInfo
      ) {
        this.researchSkillBtn.enabled = false;
        this.txt_studyLimit1.color = "#FE2E2C";
      ...
      this.txt_price0.text = nextPlayerTemp
        ? nextPlayerTemp.Property1.toString()
        : "---";
    }

    let nextConsortiaTemp: t_s_consortialevelData =
      TempleteManager.Instance.getConsortiaTempleteByTypeAndLevel(
        this._selectedSkillInfo.type,
        consortiaSkillLevel + 1,
      ); //公会的技能等级相关的模板数据
    this.txt_price1.text = nextConsortiaTemp
      ? nextConsortiaTemp.NeedOffer.toString()
      : "---";
  }
  ...
  private removeEvent() {
    ...
    this._selectedSkillInfo &&
      this._selectedSkillInfo.removeEventListener(
        ConsortiaEvent.ON_TEMPLETEID_UPDATA,
        this.__onSkillListUpdata,
        this,
      );
