...
export default class OutyardFigureWnd extends BaseWindow {
  ...
  private initView() {
    let config: Array<any> =
      ConfigInfoManager.Instance.getStackHeadActionPoint();
  ...
  private addEvent() {
    ...
    this.memberList.itemRenderer = Laya.Handler.create(
      this,
      this.renderMemberListItem,
      null,
      false,
    );
  ...
  private setChatBtnVisible() {
    if (
      (ChatManager.Instance.model.bigBugleList &&
        ChatManager.Instance.model.bigBugleList.length > 0) ||
      (ChatManager.Instance.model.allChats &&
        ChatManager.Instance.model.allChats.length > 0)
    ) {
  ...
  private __updateInfoHandler(attackList: Array<StackHeadAttackMsg>) {
    this.__refreshActorContain();
    let _attackArr: Array<OutyardAttackInfo> =
      OutyardManager.Instance.attackArr;
    ...
    for (i = 0; i < attackList.length; i++) {
      ...
      for (j = 0; j < _attackArr.length; j++) {
        ...
        if (attack.key == attackMsg.fromPos + "_" + attackMsg.toPos) {
          ...
          fromeItem =
            this["figure_" + attackMsg.fromPos + "_" + attackMsg.toPos];
          fromeItem.setPoint(toItem.x, toItem.y);
    ...
    if (selfMsg.actionPoint >= this._maxLimit) {
      MessageTipManager.Instance.show(
        LangManager.Instance.GetTranslation(
          "outyard.OutyardFrame.limitCannotBuy",
        ),
      );
      return;
    } else {
      FrameCtrlManager.Instance.open(EmWindow.OutyardBuyEnergyWnd, {
        _usePoint: this._usePoint,
        _maxLimit: this._maxLimit,
      });
    }
  }
  ...
  private __selfInfoHandler() {
    ...
    this.powerValueTxt.text =
      "" + (selfMsg.actionPoint < 0 ? 0 : selfMsg.actionPoint);
  }
  ...
  private __refreshActorContain() {
    ...
    this._memberListData = ArrayUtils.sortOn(
      guildArr,
      ["pos"],
      [ArrayConstant.NUMERIC],
    );
    this.memberList.numItems = this._memberListData.length;
    this.attackPrencentTxt.text = myGuild
      ? myGuild.attackBuffLevel * this._config1[1] + "%"
      : "";
    this.defencePrencentTxt.text = myGuild
      ? myGuild.defenceBuffLevel * this._config2[1] + "%"
      : "";
    this.noticeTxt.text = this.outYardModel
      ? this.outYardModel.getNextOpenStr()
      : "";
    let stateMsg: StackHeadStateMsg = OutyardManager.Instance.stateMsg;
    if (stateMsg.state == OutyardModel.FIGHTING) {
      this.fightStatusGroup.visible = true;
