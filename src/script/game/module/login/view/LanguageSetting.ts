...
export class LanguageSetting extends FUI_LanguageSetting {
    ...
    private onSwitchLanguage(data: LANGCFG) {
        ...
        var prompt: string = LangManager.Instance.getTranslationByLan("public.prompt", data.key);
        var confirm1: string = LangManager.Instance.getTranslationByLan("public.confirm", data.key);
        var cancel1: string = LangManager.Instance.getTranslationByLan("public.cancel", data.key);
        let msg = LangManager.Instance.getTranslationByLan("switchLanguage.tip", data.key);
        SimpleAlertHelper.Instance.Show(SimpleAlertHelper.SIMPLE_ALERT, null, prompt, msg, confirm1, cancel1, (ok: boolean) => {
            if (ok) {
                SDKManager.Instance.getChannel().trackEvent(0, TrackEventNode.CLICK_LANGUAGE_CONFIRM, "click_language_confirm", "点击切换语言二级弹窗“确认”按钮", "", "");
                SDKManager.Instance.getChannel().adjustEvent(RPT_EVENT.CLICK_LANGUAGE_CONFIRM);
                SharedManager.Instance.setWindowItem("GAME_LANGUAGE", data.index.toString());
