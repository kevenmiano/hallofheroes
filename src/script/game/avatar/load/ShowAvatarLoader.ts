...
export class ShowAvatarLoader extends Laya.Sprite implements Disposeable {
  ...
  public set data(data: any) {
    ...
    if (data instanceof GameLoadNeedData) {
      ...
      ResRefCountManager.loadRes(
        data.urlPath,
        this.__onResComplete.bind(this),
        null,
        Laya.Loader.ATLAS,
      );
    }
  ...
  protected __onResComplete(res: any) {
    ...
    if (this._content) {
      ...
      if (res && res.offset) {
        this.pivot(
          -Math.round(res.offset.footX),
          -Math.round(res.offset.footY),
        );
      } else {
        this.pivot(
          Math.round(this._content.getBounds().width / 2),
          Math.round(this._content.getBounds().height / 2),
        );
      }
    }
    if (this._loadComplete) {
      this._loadComplete(this._data);
    }
  }
