...
export default class RegisterSWnd extends BaseWindow {
  ...
  private initView() {
    ...
    this.roleItems = [this.roleItem_0, this.roleItem_1, this.roleItem_2];
    ...
    this.userName.promptText = LangManager.Instance.GetTranslation(
      "login.pleaseEnterNickname",
    );
  ...
  private initProfessionDesc(): void {
    ...
    if (obj) {
      obj.text = LangManager.Instance.GetTranslation(
        "RegisterSWnd.professionDesc0",
      );
    }
  ...
  private addEvent() {
    this.c1 &&
      this.c1.on(fgui.Events.STATE_CHANGED, this, this.onProfessionChange);
    ...
    for (let index = 0; index < 3; index++) {
      ...
      if (element) {
        let subController = element.getController("c3");
        subController &&
          subController.on(fgui.Events.STATE_CHANGED, this, this.onSexChange);
      }
    }
  ...
  private offEvent() {
    this.c1 &&
      this.c1.off(fgui.Events.STATE_CHANGED, this, this.onProfessionChange);
    ...
    for (let index = 0; index < 3; index++) {
      ...
      if (element) {
        let subController = element.getController("c3");
        subController &&
          subController.off(fgui.Events.STATE_CHANGED, this, this.onSexChange);
      }
    }
  ...
  private onEnterGame() {
    ...
    SDKManager.Instance.getChannel().trackEvent(
      0,
      TrackEventNode.Node_30004,
      "registerEnterGame",
      "点击创角界面的进入游戏按钮（包括不成功）",
      "创角界面",
      "准备创建角色了",
    );
    let userId: number = ModelMgr.Instance.getProperty(
      EmModel.USER_MODEL,
    ...
    if (nickName == "") {
      MessageTipManager.Instance.show(
        LangManager.Instance.GetTranslation("login.pleaseEnterNickname"),
      );
      return;
    }
    if (StringUtils.checkEspicalWorld(nickName)) {
      MessageTipManager.Instance.show(
        LangManager.Instance.GetTranslation(
          "login.nicknameContainsIllegalCharacters",
        ),
      );
      return;
    }
    if (StringHelper.getStringLength(nickName) > 12) {
      MessageTipManager.Instance.show(
        LangManager.Instance.GetTranslation("login.nicknameTooLong"),
      );
      return;
    }
    SDKManager.Instance.getChannel().adjustEvent(
      RPT_EVENT.CREATE_ROLE,
      userName,
    );
      ...
      if (data) {
        ...
        if (checkContent) {
          ...
          if (json.Result.value == "true") {
            ...
            let site: string = ModelMgr.Instance.getProperty(
              EmModel.USER_MODEL,
              UserModelAttribute.site,
            );
            let password: string = ModelMgr.Instance.getProperty(
              EmModel.USER_MODEL,
              UserModelAttribute.password,
            );
            LoginManager.Instance.c2s_register_role(
              userId,
              userName,
              nickName,
              sex,
  ...
  onBackToLogin() {
    SDKManager.Instance.getChannel().trackEvent(
      0,
      TrackEventNode.CLICK_REGISTER_BACK,
    ...
    FrameCtrlManager.Instance.open(
      EmWindow.Login,
      LoginManager.Instance.loginAccountParams,
    );
  }
  ...
  private onProfessionChange(mainController: fgui.Controller) {
    ...
    if (subController) {
      ...
      this.selectRole.selectedIndex =
        mainIndex * 2 + subController.selectedIndex;
    }
    // this.onBtnDice();
  }
