...
function createBaseFarmLogReq(): FarmLogReq {
  return { playerId: 0, operUserId: 0, operNickName: "", desc: "", operDate: "", operCount: 0, isAddFavGp: false };
}

export const FarmLogReq: MessageFns<FarmLogReq> = {
  encode(message: FarmLogReq, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
  ...
  decode(input: BinaryReader | Uint8Array, length?: number): FarmLogReq {
    const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
    let end = length === undefined ? reader.len : reader.pos + length;
    const message = createBaseFarmLogReq();
  ...
  fromJSON(object: any): FarmLogReq {
    return {
  ...
  toJSON(message: FarmLogReq): unknown {
    const obj: any = {};
    if (message.playerId !== 0) {
  ...
  create<I extends Exact<DeepPartial<FarmLogReq>, I>>(base?: I): FarmLogReq {
    return FarmLogReq.fromPartial(base ?? ({} as any));
  },
  fromPartial<I extends Exact<DeepPartial<FarmLogReq>, I>>(object: I): FarmLogReq {
    const message = createBaseFarmLogReq();
