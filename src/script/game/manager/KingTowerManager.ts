...
export class KingTowerManager {
  ...
  public static get Instance(): KingTowerManager {
    if (KingTowerManager._instance == null)
      KingTowerManager._instance = new KingTowerManager();
  ...
  private initEvent() {
    ServerDataManager.listen(
      S2CProtocol.U_C_KING_INFO,
      this,
      this.__updateKingTowerInfoHandler,
    );
  }

  private __updateKingTowerInfoHandler(pkg: PackageIn) {
    var msg: KingTowerInfoMsg = pkg.readBody(
      KingTowerInfoMsg,
    ) as KingTowerInfoMsg;

    if (msg.hasOwnProperty("enterCount")) this._info.kingCount = msg.enterCount;

    if (msg.hasOwnProperty("maxIndex")) this._info.maxIndex = msg.maxIndex;
    ...
    if (msg.hasOwnProperty("rewardCount")) {
      ...
      if (this._info.rewardCount > 0) {
        var diffType: string = this._info.difficultyStep(
          this._info.currentStep,
        );

        var str3: string = LangManager.Instance.GetTranslation(
          "yishi.manager.KingTowerManager.str",
          diffType,
          this._info.currentIndex,
          this._info.rewardCount,
        );
        DelayActionsUtils.Instance.addAction(
          new AlertTipAction(
            { message: str3 },
            this.KingTowerReport.bind(this),
