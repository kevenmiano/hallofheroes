...
export default class SeveranceSoliderWnd extends BaseWindow {
    ...
    public OnInitWind() {
        this.n1.getChild("title").text = LangManager.Instance.GetTranslation("armyII.viewII.allocate.DissmissPawnFrame.command01");
        this.addEvent();
        this.setCenter();
    }
    ...
    private __onOKHandler() {
        ...
        if (this._soliderNumber == 0) {
            MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("SeveranceSoliderWnd.moveconfig"));
            return;
        }
        else {
            ArmySocketOutManager.sendDismissPawn(this._pawn.templateId, this._soliderNumber);
            this.OnBtnClose();
        }
    }
