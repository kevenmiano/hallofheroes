...
export class SimplePlayerInfo extends GameEventDispatcher {
	...
	protected static IS_BIND_VERTIFY_PROMPTED: string = "IS_BIND_VERTIFY_PROMPTED";
	...
	public get multiCopyMaxCount(): number {
		let cfg = TempleteManager.Instance.getConfigInfoByConfigName("MultiCampaign_MaxCount");
		if (cfg) {
	...
	public get nickName(): string {
		return this._nickName == String(this._userId + "$") ? LangManager.Instance.GetTranslation("public.nickName") : this._nickName;
	}
	...
	public get firstHonerTemp(): t_s_upgradetemplateData {
		let tempList: t_s_upgradetemplateData[] = TempleteManager.Instance.getTemplatesByType(UpgradeType.UPGRADE_TYPE_HONER);
	...
	public get JJCMaxCount(): number {
		if (!this._JJCMaxCount) {
			if (TempleteManager.Instance.getConfigInfoByConfigName("MatchRoom_MaxCount"))
				this._JJCMaxCount = Number(TempleteManager.Instance.getConfigInfoByConfigName("MatchRoom_MaxCount").ConfigValue);
		}
	...
	public addPet($petData: PetData) {
		...
		this._petList = ArrayUtils.sortOn(this._petList, ["fightPower"], [ArrayConstant.NUMERIC | ArrayConstant.DESCENDING]);
		if (find) {
		...
		if (this.petChallengeIndexFormationOfArray.length > 0) {
			...
			for (let i: number = 0; i < len; i++) {
				if (this.petChallengeIndexFormationOfArray[i] > 0) {
					this.firstCtrPetTemplateId = this.petChallengeIndexFormationOfArray[i];
					break;
				}
			}
		} else {
