...
export class RemotePetAdjstPetItem extends FUI_AdjustPetItem {
  ...
  public updateView() {
    ...
    this._petIcon.url = IconFactory.getPetHeadSmallIcon(
      this._petData.templateId,
    );
  ...
  public set colorNum(v: number) {
    ...
    if (this.colorFlag.visible) {
      this.colorFlag.url = FUIHelper.getItemURL(
        "Base",
        "Icon_Num" + this._corlorNum,
      );
    }
  }
  ...
  private checkInRemote() {
    ...
    let remotePetFormationOfArray =
      this.model.petListInfo.remotePetFormationOfArray;
