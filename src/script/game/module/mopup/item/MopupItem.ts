...
export default class MopupItem extends FUI_MopupItem {
    ...
    public set info(data: any) {
        ...
        if (data.gp && data.gp > 0) {
            this.txtGetExp.text = LangManager.Instance.GetTranslation("yishi.view.tips.TaskTraceTip.gpTxt") + data.gp;
        } else {
            this.txtGetExp.text = ""
        }
    }
    ...
    private setTxtDescState(itemType: eMopupItemType = eMopupItemType.None) {
        switch (itemType) {
            case eMopupItemType.Mopuping:
                this.txtStateDesc.text = LangManager.Instance.GetTranslation("MopupWnd.MopupItem.Sweeping")
                break;
            case eMopupItemType.MopupEnd:
                this.txtStateDesc.text = LangManager.Instance.GetTranslation("MopupWnd.MopupItem.SweepEnd")
                break;
    ...
    public setTitle(numStr: string, type?: number) {
        this.title.text = LangManager.Instance.GetTranslation("mopup.MopupManager.RewardsTip");
        return;
        if (type == eMopupType.CampaignMopup) {
            this.title.text = LangManager.Instance.GetTranslation("mopup.view.CampaignMopupIngFrame.resultTitle", numStr);
        } else if (type == eMopupType.PetCampaignMopup) {
            this.title.text = LangManager.Instance.GetTranslation("mopup.view.CampaignMopupIngFrame.resultTitle", numStr);
        }
        else {
            this.title.text = LangManager.Instance.GetTranslation("mopup.view.MazeMopupIngFrame.resultTitle", numStr);
        }
    }
