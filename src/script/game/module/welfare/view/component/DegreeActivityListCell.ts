...
export default class DegreeActivityListCell extends FUI_DegreeActivityListCell {
    ...
    private __onClickHandler() {
        ...
        if (userLevel < this._cellData.templateInfo.Level) {
            MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("welfare.degress.goAheadText"));
            return;
        }
    ...
    set cellData(value: LeedInfo) {
        ...
        if (this._cellData.templateInfo) {
            this.title.text = value.templateInfo.TitleLang + " (" + this._cellData.currentCount + "/" + this._cellData.templateInfo.PassCount + ")";
        } else {
            this.title.text = "(" + this._cellData.currentCount + "/" + 0 + ")";
        }
        this.count.text = value.templateInfo ? value.templateInfo.Num.toString() : "0";
        let grade: number = value.templateInfo ? value.templateInfo.Level : 0;
        this.level.text = LangManager.Instance.GetTranslation("DegreeActivityListCell.leveTxt", grade);
        ...
        if (value.templateInfo && value.templateInfo.Time.indexOf(",")) {
            ...
            for (let index = 0; index < tempTimes.length; index++) {
                ...
                if (index < tempTimes.length - 1)
                    timeStr += tempValue + "<br>";
                else
                    timeStr += tempValue;
            }
        } else {
            timeStr = value.templateInfo ? value.templateInfo.Time.replace("|", "-") : "00:00";
        }
        if (timeStr == "00:00-23:59")//这种时间改下显示为全天
        {
            timeStr = LangManager.Instance.GetTranslation("DegreeActivityListCell.timeStr");
        }
