...
export default class SinglePassManager {
  ...
  public setup() {
    ...
    ServerDataManager.listen(
      S2CProtocol.U_C_RES_SINGLEPASS_INFO,
      this,
      this.__resSinglePassInfoHandler,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_BUGLE_REQ,
      this,
      this.__bugleReqHandler,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_BUGLE_REWARDS,
      this,
      this.__bugleRewardsHandler,
    );
  }
    ...
    if (msg.from == 0) {
      ...
      if (this.isFirstGetSinglePassInfo) {
        ...
        this._bugleLeftCount =
          Number(this._model.maxIndex / SinglePassModel.REWARD_LIMIT) *
            this._model.bugleCount -
          this._model.itemCount;
        if (this._bugleLeftCount > 0) {
          DelayActionsUtils.Instance.addAction(
            new AlertTipAction({ message: "" }, this.showHasBugleTips),
          );
        }
      }
    } else {
  ...
  private showHasBugleTips(result: string = "") {
    ...
    data.content = LangManager.Instance.GetTranslation(
      "taskTrace.SinglepassHasBugleYipsView.singlePassHas",
    );
  ...
  public sendRequestSinglePassAttack(targetIndex: number) {
    var content: string;
    var propCount: number = GoodsManager.Instance.getGoodsNumByTempId(
    ...
    if (this._model.count >= this._model.maxCount && propCount <= 0) {
      content = LangManager.Instance.GetTranslation(
        "managers.SinglePassManager.TipsTxt01",
      );
    ...
    if (targetIndex > this._model.maxIndex + 1) {
      content = LangManager.Instance.GetTranslation(
        "managers.SinglePassManager.TipsTxt02",
      );
      MessageTipManager.Instance.show(content);
      return;
    }
    if (this._model.isMaxJudge(targetIndex)) {
      content = LangManager.Instance.GetTranslation(
        "managers.SinglePassManager.TipsTxt03",
      );
      MessageTipManager.Instance.show(content);
      return;
    }
    var singlePassInfo: SinglePassCardInfo =
      this._model.getCardInfoByIndex(targetIndex);
    var data: t_s_campaigndataData = TempleteManager.Instance.getGodArriveData(
      SinglePassModel.CAMPAIGN_TEMPLATE_ID,
      singlePassInfo.tollgate,
    );
    var quardList: Array<string> = LangManager.Instance.GetTranslation(
      "singlepass.view.SinglePassCardItemView.PetTypeList",
    ).split("|");
    content = LangManager.Instance.GetTranslation(
      "singlepass.SinglePassView.SinglePassAttack",
    );
    if (singlePassInfo.judge > 0) {
      content = LangManager.Instance.GetTranslation(
        "singlepass.SinglePassView.SinglePassAttackAgain",
      );
    } else {
      content += LangManager.Instance.GetTranslation(
        "singlepass.SinglePassView.Suggest",
        quardList[parseInt(data.Param5) - 1],
      );
    }
  ...
  public needTurnNext(): boolean {
    var endIndex: number =
      this._model.selectArea * SinglePassModel.TOLLGATE_PER_FLOOR;
    if (
      this.getCardIndex() != 0 &&
      this._model.lastIndex == this._model.maxIndex &&
      this._model.maxIndex != endIndex &&
      this._model.selectArea == this._model.area
    ) {
      return true;
    }
    return false;
  ...
  public getCardIndex(): number {
    ...
    if (
      this._model.selectArea == this._model.area &&
      this._model.selectIndex >= startIndex &&
      this._model.selectIndex <= endIndex
    ) {
