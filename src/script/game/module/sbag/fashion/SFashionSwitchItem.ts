...
export class SFashionSwitchItem extends FUI_SFashionSwitchItem {
  ...
  private refreshView() {
    if (this._data) {
      ...
      let skillInfo: t_s_skilltemplateData =
        this.fashionModel.getFashionObjectSkillTemplate(this._data);
      ...
      if (this._data.Property2 > 0 && this._data.Property2 < 6) {
        this.profile.url = fgui.UIPackage.getItemURL(
          EmPackName.Base,
          this.profileRes[this._data.Property2 - 1],
        );
      }
      ...
      if (this.fashionModel.hasIdentityedBook(this._data.TemplateId)) {
        //鉴定过
        ...
        if (this._data.TemplateId in this.fashionModel.bookList) {
          ...
          if (this.fashionModel.preconditions) {
            ...
            this.btn_identity.getChild("redDot").visible =
              ResourceManager.Instance.gold.count >= count;
          }
        } else {
          //不可鉴定
  ...
  private __clickHandler(e: Laya.Event) {
    ...
    if (
      this._data.TemplateId in this.fashionModel.bookList &&
      this.fashionModel.bookList[this._data.TemplateId] == false
    ) {
      //新的  带New标志的
  ...
  private identityBtnClickHandler() {
    ...
    if (this.fashionModel.hasRefreshFull(this._data.TemplateId)) {
      ...
      msg = LangManager.Instance.GetTranslation(
        "fashion.FashionSwitchItem.identityedFull",
      );
    } else if (thaneInfo.grades < FashionModel.INDENTITY_OPEN_GRADE) {
      msg = LangManager.Instance.GetTranslation(
        "BattleGuardView.needGrade",
        FashionModel.INDENTITY_OPEN_GRADE,
      );
    }
    if (msg) {
    ...
    if (!this.fashionModel.hasIdentityedBook(this._data.TemplateId)) {
      ...
      let skillData: t_s_skilltemplateData =
        TempleteManager.Instance.getSkillIdByTypeAndGrade(
          this._data.SonType * 10,
          this._data.Property2,
          1,
        );
      let str;
      if (skillData) {
      ...
      content = LangManager.Instance.GetTranslation(
        "fashion.FashionSwitchItem.identityContent",
        count,
        str,
      );
      let confirm: string =
      ...
      SimpleAlertHelper.Instance.Show(
        ...
        confirm,
        cancel,
        this.indentityCallBack.bind(this),
      );
    } //鉴定过未洗练到最高级
    else {
      ...
      let str;
      if (skillInfo) {
        skillData = TempleteManager.Instance.getSkillIdByTypeAndGrade(
          this._data.SonType * 10,
          this._data.Property2,
          skillInfo.Grades + 1,
        );
        if (skillData) {
          if (skillInfo.Grades == 4) {
            ...
            str =
              skillData.TemplateNameLang + "~" + this.getMaxValue(skillData);
          }
        }
      }
      let title: string = LangManager.Instance.GetTranslation("public.prompt");
      ...
      let cancel: string = LangManager.Instance.GetTranslation("public.cancel");
      content = LangManager.Instance.GetTranslation(
        "fashion.FashionSwitchItem.refreshContent",
        count,
        str,
      );
      SimpleAlertHelper.Instance.Show(
  ...
  private indentityCallBack(result: boolean, flag: boolean, params: any) {
    ...
    if (!this.fashionModel.hasIdentityedBook(data.TemplateId)) {
      //未鉴定过
      ...
      let bagCount: number = GoodsManager.Instance.getGoodsNumByTempId(
