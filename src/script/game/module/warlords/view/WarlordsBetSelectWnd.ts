...
export default class WarlordsBetSelectWnd extends BaseWindow {
    ...
    private onClickItem() {
        ...
        for (let i: number = 0; i < this.playerList.numItems; i++) {
            let item: WarlordsBetSelectItem = this.playerList.getChildAt(i) as WarlordsBetSelectItem;
            if (selecteIndex == i) {
                item.flag = true;
    ...
    confirmBtnHandler() {
        ...
        {
            this.warlordsModel.addListData(WarlordsModel.CUR_BETTING, this.selectedRank, this._selecteItem.info);
        }
        this.OnBtnClose();
    }
    ...
    private refreshView() {
        ...
        if (betList) {
            ...
            for (let i = 0; i < len; i++) {
                ...
                if (warlordsPlayerInfo && (warlordsPlayerInfo.betRank == 0 || warlordsPlayerInfo.betRank != this.selectedRank)) {
