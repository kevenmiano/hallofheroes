...
export default class ActivationMovieStepRenderMediator
  implements IMediator, IEnterFrame
{
  ...
  public enterFrame() {
    ...
    if (this._count > CampaignMapView.STEP_RENDER_SPEED) {
      ...
      let rect: Laya.Rectangle = MapViewHelper.getCurrentMapRectFix(
        CampaignManager.Instance.mapView,
      );
      if (arr) {
      ...
      if (dic) {
        dic.forEach((aInfo) => {
          let outSight = this.outOfSight(
            aInfo.curPosX * Tiles.WIDTH,
            aInfo.curPosY * Tiles.HEIGHT,
            rect,
          );
          if (!outSight) {
