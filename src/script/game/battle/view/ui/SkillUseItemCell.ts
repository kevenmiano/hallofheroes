...
export default class SkillUseItemCell extends Laya.Sprite {
  ...
  private initView() {
    this._com = FUIHelper.createFUIInstance(
      EmPackName.Battle,
      "AssetBattleSkillGlowAsset",
    ) as fgui.GComponent;
    this._movie = this._com.getChild("movieIcon") as fgui.GMovieClip;
    this._icon = this._com.getChild("skillIcon") as fgui.GLoader;

    this._movie.setPlaySettings(
      1,
      10,
      1,
      7,
      Laya.Handler.create(this, this.onPlayComplete),
    );
  ...
  public set data(value) {
    ...
    if (value) {
      ...
      if (value != "") this._com.visible = true;
    } else {
      this.visible = false;
