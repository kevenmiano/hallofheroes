...
export default class QQDawankaManager extends GameEventDispatcher {
    ...
    public setup() {
        // this.addEvent();
        this.model.setQQGradeDic(ConfigMgr.Instance.getDicSync(ConfigType.t_s_qqgrade));
        this.model.setQQGradePrivilegeDic(ConfigMgr.Instance.getDicSync(ConfigType.t_s_qqgradeprivilege));
    }
    ...
    public addEvent() {
        ...
        ServerDataManager.listen(S2CProtocol.U_C_QQ_DAWANKA_INFO, this, this.__onDawankaInfo);

        ServerDataManager.listen(S2CProtocol.U_C_QQ_GIFT_RSP, this, this.__onDawankaGift);
    }
    ...
    __onDawankaInfo(pkg: PackageIn): void {
        var msg: UserQQDaWanKaMsg = pkg.readBody(UserQQDaWanKaMsg) as UserQQDaWanKaMsg;
