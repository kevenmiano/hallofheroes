...
export class StarHelper {
    ...
    public static getProfileNameByProfile(profile: number): string {
        ...
        switch (profile) {
            case 1:
                str = LangManager.Instance.GetTranslation("yishi.utils.GoodsHelp.getGoodQualityName01");
                break;
            case 2:
                str = LangManager.Instance.GetTranslation("yishi.utils.GoodsHelp.getGoodQualityName02");
                break;
            case 3:
                str = LangManager.Instance.GetTranslation("yishi.utils.GoodsHelp.getGoodQualityName03");
                break;
            case 4:
                str = LangManager.Instance.GetTranslation("yishi.utils.GoodsHelp.getGoodQualityName04");
                break;
            case 5:
                str = LangManager.Instance.GetTranslation("yishi.utils.GoodsHelp.getGoodQualityName05");
                break;
            case 6:
                str = LangManager.Instance.GetTranslation("yishi.utils.GoodsHelp.getGoodQualityName06");
                break;
            case 7:
                str = LangManager.Instance.GetTranslation("yishi.utils.GoodsHelp.getGoodQualityName07");
                break;
        }
        return str;
    }
    ...
    public static checkCanUpGrade(exp: number, info: StarInfo): number {
        ...
        var upgradeList: t_s_upgradetemplateData[] = StarHelper.getStarUpgradeListByProfile(info.template.Profile);
        ...
        for (let index = 0; index < upgradeList.length; index++) {
            ...
            if (temp.Grades == (info.grade + 1))
                exp = exp - (temp.Data - info.gp);
            if (temp.Grades > (info.grade + 1))
                exp = exp - temp.Data;
            if (exp >= 0)
                upGrade = temp.Grades;
    ...
    public static getStarTotalExp(info: StarInfo): number {
        ...
        var upgradeList: any[] = StarHelper.getStarUpgradeListByProfile(info.template.Profile);
        for (let index = 0; index < upgradeList.length; index++) {
    ...
    public static getStarAttributeAdd(info: StarInfo): string {
        var str: string = LangManager.Instance.GetTranslation("armyII.ThaneAttributeView.Tip01");
    ...
    public static getStarBufferName(info: StarInfo): string {
        var bufferTemp: t_s_skillbuffertemplateData = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_skillbuffertemplate, info.template.DefaultSkill[0]);
        if (bufferTemp) {
            var bufferField: string = StringHelper.getSubStrBetweenTwoChar("{", "}", bufferTemp.BufferNameLang);
            if (StringHelper.isNullOrEmpty(bufferField)) {
                return bufferTemp.BufferNameLang;
            }
            else {
                var bufferValue: number = bufferTemp.hasOwnProperty(bufferField) ? Math.abs(bufferTemp[bufferField]) * info.grade : 0;
    ...
    public static profileColors = ["#ffffff", "#999999", "#59cd41", "#32a2f8", "#a838f7", "#ff8000", "#ce0f0f", "#ce0f0f"];
    public static getStarHtmlName(temp: t_s_startemplateData): string {
        if (!temp) return "";
        if (!StarHelper.profileColors) {
            StarHelper.profileColors = ["#ffffff", "#999999", "#59cd41", "#32a2f8", "#a838f7", "#ff8000", "#ce0f0f", "#ce0f0f"];
        }
        var color: string = StarHelper.profileColors[temp.Profile];
        if (color) {
            return "\\[" + LangManager.Instance.GetTranslation("StarHelper.tempTxt", color, temp.TemplateNameLang) + "]";
        }
