...
export class NpcEnterFightMediator implements IMediator {
    ...
    public register(target: Object) {
        ...
        if (this._npc && this._npc.nodeInfo && this._npc.nodeInfo.info)
            this._npc.nodeInfo.info.addEventListener(PhysicsEvent.UP_STATE, this.__updateNodeStateHandler,this);
    }

    public unregister(target: Object) {
        if (this._npc && this._npc.nodeInfo && this._npc.nodeInfo.info)
            this._npc.nodeInfo.info.removeEventListener(PhysicsEvent.UP_STATE, this.__updateNodeStateHandler,this);
    }
    ...
    private __call() {
        if (this._attackView) {
            if (this._attackView.parent) this._attackView.parent.removeChild(this._attackView);
