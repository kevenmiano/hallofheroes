...
export default class CasernRecruitWnd extends BaseWindow {
  ...
  private Data() {
    ...
    if (this._pawn != null) {
      ...
      let popLimit: number =
        (this.population.limit - this.population.count) / needpop;
      ...
      if (this._maxNumber > this._initNumber) {
        ...
        this.progressSlider.value = Number(
          (100 * this._initNumber) / this._maxNumber,
        );
  ...
  private __onOKHandler() {
    ...
    if (this._soliderNumber == 0) {
      MessageTipManager.Instance.show(
        LangManager.Instance.GetTranslation("CasernRecruitWnd.moveconfig"),
      );
      return;
    } else {
      ...
      let totalCanStaffingPawn =
        ArmyManager.Instance.army.baseHero.attackProrerty.totalConatArmy; //可以带兵的总数量
      // 城堡中拥有的准备上阵的兵的数量
      ...
      if (emputyPos == 0) {
        ...
        if (this._soliderNumber + castleOwnPawnCount >= totalCanStaffingPawn) {
          ...
          this._soliderNumber + castleOwnPawnCount <
          totalCanStaffingPawn
        ) {
        ...
        if (
          curPawn.ownPawns > 0 &&
          curPawn.templateId == this._pawn.templateId
        ) {
          //加兵
          ...
          if (this._soliderNumber + curPawn.ownPawns >= totalCanStaffingPawn) {
            ...
            this._soliderNumber + curPawn.ownPawns <
            totalCanStaffingPawn
          ) {
  ...
  private getPopulationByGolds(pawn: ArmyPawn, gold: number): number {
    ...
    let arr: Array<t_s_buildingtemplateData> =
      BuildingManager.Instance.getMaxEffectBuildingTemplate(pawnTemp);
    if (!arr) return 0;
  ...
  private get recruitGold(): number {
    let arr: Array<t_s_buildingtemplateData> =
      BuildingManager.Instance.getMaxEffectBuildingTemplate(this._pawnTemp);
    if (!arr) return 0;
    arr = ArrayUtils.sortOn(arr, "Property4", ArrayConstant.NUMERIC);
    let buildTemp: t_s_buildingtemplateData = arr[0];
    let goldValue: number =
      this._model.playerEffect.getRecruitPawnResourceAddition(
        this._pawnTemp.GoldConsume * (1 - buildTemp.Property4 / 100),
      );
