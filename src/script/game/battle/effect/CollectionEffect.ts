...
export default class CollectionEffect {
  ...
  //职业类型
  constructor(
    backContainer: Laya.Sprite,
    frontContainer: Laya.Sprite,
    job: number = 1,
  ) {
  ...
  private show() {
    this._backEffectMovie = FUIHelper.createFUIInstance(
      EmWindow.BattleDynamic,
      "asset_battle_CollectGasAsset",
    ) as fgui.GComponent;
    if (this._backEffectMovie) {
      this._backEffect = this._backEffectMovie.getTransition("backEffect");
      this._backEffectMovie.displayObject.mouseEnabled =
        this._backEffectMovie.displayObject.mouseThrough = false;
  ...
  public updateToQteState() {
    ...
    this._frontEffectMovie = FUIHelper.createFUIInstance(
      "Battle",
      this.getFrontEffectLinkName(),
    );
    ...
    else this._frontContainer.addChild(this._frontEffectMovie.displayObject);

    this._frontEffectMovie.displayObject.mouseEnabled =
      this._frontEffectMovie.displayObject.mouseThrough = false;
    this._frontEffect.setValue("qte");
    //播放0.5秒-1.5秒部分的动效, 即0.5秒（包含）-1.5秒（包含）之间的所有帧。
    this._frontEffect.play(
      Laya.Handler.create(this, this.onPlayFrontEffect),
      1,
      0,
      0.5,
      1.5,
  ...
  private getFrontEffectLinkName(): string {
    ...
    else return "asset.battle.CollectGasFrontAsset";
  }
