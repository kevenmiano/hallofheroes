...
export class BloodHelper {
  ...
  private static processBufferData(skillData: SkillData, bloodDic: Dictionary) {
    ...
    for (let i: number = 0; i < skillData.buffers.length; i++) {
      ...
      for (let j: number = 0; j < buffer.damages.length; j++) {
        //遍历每一次掉血
        damageData = buffer.damages[j];
        roleInfo = BattleManager.Instance.battleModel.getRoleById(
          damageData.target,
        );

        if (roleInfo) {
          bloodInfo = BloodHelper.getTempRoleBlood(roleInfo);
          hurt = BloodHelper.updateBlood(
            bloodInfo,
            damageData,
            skillData.attackMillis,
          );
          ...
          BloodHelper.obtainWorldBossBlood(
            damageData.target,
  ...
  private static getTempRoleBlood(roleInfo: any): TempRoleBloodInfo {
    if (!BloodHelper.bloodDic[roleInfo.livingId]) {
      BloodHelper.bloodDic[roleInfo.livingId] =
        BloodHelper.createTempRoleBlood(roleInfo);
    }
  ...
  private static processSkillData(skillData: SkillData, bloodDic: Dictionary) {
    ...
    if (skillData.data) {
      for (let turn: number = 0; turn < skillData.data.length; turn++) {
        //每一回合
        turnData = skillData.data[turn];
        for (let i: number = 0; i < turnData.length; i++) {
          //每个对象
          attackData = turnData[i];
          if (attackData && attackData.roleInfo) {
            if (!BloodHelper.bloodDic[attackData.roleId]) {
              BloodHelper.bloodDic[attackData.roleId] =
                BloodHelper.createTempRoleBlood(attackData.roleInfo);
            }
            hurt = BloodHelper.updateBlood(
              BloodHelper.bloodDic[attackData.roleId],
              attackData,
    ...
    for (let i: number = 0; i < skillData.reliveList.length; i++) {
      ...
      if (reliveInfo.battleId != BattleManager.Instance.battleModel.battleId)
        continue;
      var roleInfo: any = BattleManager.Instance.battleModel.getRoleById(
        reliveInfo.livingId,
      );
  ...
  private static updateBlood(
    tempBloodInfo: TempRoleBloodInfo,
    data: object,
    time: number,
  ): number {
    let hasSync: boolean;
    ...
    if (data instanceof AttackData) {
      ...
      if (
        attackData.bloodType == BloodType.BLOOD_TYPE_SELF ||
        attackData.bloodType == BloodType.BLOOD_MAXHP
      ) {
        tempBloodInfo.maxBloodA -= attackData.hpMaxAdd;
        attackData.damageValue = BloodHelper.updateDamageValue(
          tempBloodInfo.bloodA,
          attackData.displayBlood,
          tempBloodInfo.maxBloodA,
          hasSync,
        );
        ...
        attackData.damageValue = BloodHelper.updateDamageValue(
          tempBloodInfo.bloodB,
          attackData.displayBlood,
          tempBloodInfo.maxBloodB,
          hasSync,
        );
      ...
      if (
        damageData.bloodType == BloodType.BLOOD_TYPE_SELF ||
        damageData.bloodType == BloodType.BLOOD_MAXHP
      ) {
        tempBloodInfo.maxBloodA -= damageData.hpMaxAdd;
        damageData.damageValue = BloodHelper.updateDamageValue(
          tempBloodInfo.bloodA,
          damageData.displayBlood,
          tempBloodInfo.maxBloodA,
          hasSync,
        );
  ...
  private static updateDamageValue(
    currentBlood: number,
    displayBlood: number,
    maxBlood: number,
  ...
  private static checkIsSyncForWorldBoss(
    target: number,
    time: number,
  ): boolean {
