...
export default class SecretModel {
  ...
  /** 未通关的秘境层数记录 */
  private _maxSecretRecordInfoMap: Map<SecretType, SecretRecordInfo> =
    new Map();
  lostTreasure: string;
  gainTreasure: string;

  constructor() {
    this._secretInfoMap.set(
      SecretType.Single,
      new SecretInfo(SecretType.Single),
    );
    ...
    this._maxSecretRecordInfoMap.set(
      SecretType.Single,
      new SecretRecordInfo(SecretType.Single),
    );
  }
