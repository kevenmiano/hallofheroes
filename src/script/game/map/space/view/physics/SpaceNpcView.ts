...
export class SpaceNpcView extends HeroAvatarView implements IBaseMouseEvent {
  ...
  protected createNickName() {
    AvatarInfoUILayerHandler.handle_CON_CREATE(
      ...
      { fontSize: 18, strokeWidth: 2, strokeColorIdx: 1, shadow: false },
    );
  }
  ...
  private refreshAvatarView() {
    ...
    Logger.xjy(
      "[SpaceNpcView]refreshAvatarView",
      cInfo.info.names,
      cInfo.nodeId,
    );
  ...
  public attackFun(): boolean {
    ...
    SpaceManager.Instance.visitSpaceNPC(
      (<SpaceNode>this.nodeInfo).nodeId,
      true,
    );
    return true;
  }
