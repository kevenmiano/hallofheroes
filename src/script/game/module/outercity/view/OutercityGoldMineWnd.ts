...
export default class OutercityGoldMineWnd extends BaseWindow {
    ...
    private initEvent() {
        this.mineList.itemRenderer = Laya.Handler.create(this, this.renderListItem, null, false);
        this.mineList.on(fgui.Events.CLICK_ITEM, this, this.onMineListItemClick);
        NotificationManager.Instance.addEventListener(OuterCityEvent.INIT_SECOND_NODE_DATA, this.updateSelectItemView, this);
        NotificationManager.Instance.addEventListener(OuterCityEvent.UPDATE_SECOND_NODE_DATA, this.updateView, this);
        NotificationManager.Instance.addEventListener(OuterCityEvent.OUTERCITY_SELECTE_CANCEL, this.resetView, this);
    ...
    private removeEvent() {
        ...
        NotificationManager.Instance.removeEventListener(OuterCityEvent.INIT_SECOND_NODE_DATA, this.updateSelectItemView, this);
        NotificationManager.Instance.removeEventListener(OuterCityEvent.UPDATE_SECOND_NODE_DATA, this.updateView, this);
        NotificationManager.Instance.removeEventListener(OuterCityEvent.OUTERCITY_SELECTE_CANCEL, this.resetView, this);
    ...
    private onMineListItemClick(selectedItem: MineItem, evt: Laya.Event) {
        let len: number = this.mineList.numItems;
        let item: MineItem;
        if (this._curSelectedItem) {//如果有选中的
            if (this._curSelectedItem.info.nodeId == selectedItem.info.nodeId) {//这次点击的是同一个, 则取消选中状态
        ...
        NotificationManager.Instance.dispatchEvent(NotificationEvent.OUTERCITY_SELECTE_MINEITEM, this._curSelectedItem);
    }
    ...
    public initViewBySelecteIndex(index: number) {
        ...
        if (index == 0) {
            ...
            if (this._curSelectedItem) {
                let info = this.outerCityModel.findSecondNodeById(this._nodeInfo, this._curSelectedItem.info.nodeId);
                this.goldMineInfoView.updateView(info);
            }
