...
export default class PetGuardTipWnd extends BaseWindow {
  ...
  public OnInitWind() {
    ...
    this.CampaignNameTxt.text =
      LangManager.Instance.GetTranslation("petIsland");
    this.txt01.text = LangManager.Instance.GetTranslation(
      "PetBossInfoView.str2",
    );
    this.txt02.text = LangManager.Instance.GetTranslation(
      "PetBossInfoView.str1",
    );
    this.txt03.text = LangManager.Instance.GetTranslation(
      "PetBossInfoView.str3",
    );
    this.bar = this.comPetGuardTip.getChild("bar") as fgui.GProgressBar;
    this.addEvent();
    this.refreshView();
    this.isOversea = this.getController("isOversea");
    if (this.isOversea) this.isOversea.selectedIndex = isOversea() ? 1 : 0;
  }
  ...
  private refreshView(): void {
    for (let i: number = 0; i < 3; i++) {
      ...
      (this["txt" + i] as fairygui.GTextField).text =
        LangManager.Instance.GetTranslation(
          "PetBossInfoView.monsterText" + i,
          currentCount,
          maxCount,
        );
    ...
    let currentFrame: number = Math.max(
      1,
      Math.min(
        100,
        Math.floor(
          (this.petBossModel.currentRagePoint * 100) /
            this.petBossModel.maxRagePoint,
        ),
      ),
    );
    this.txt_bar.text =
      this.petBossModel.currentRagePoint + "/" + this.petBossModel.maxRagePoint;
    this.bar.value =
      (this.petBossModel.currentRagePoint / this.petBossModel.maxRagePoint) *
      100;
  ...
  private btnHelpClick() {
    let title = LangManager.Instance.GetTranslation(
      "PetBossInfoView.helpBtn.tipdata",
    );
    let content = LangManager.Instance.GetTranslation(
      "PetBossInfoView.boss.helpFrame.helpContent",
    );
