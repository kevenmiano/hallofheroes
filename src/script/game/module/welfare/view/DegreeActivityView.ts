...
export default class DegreeActivityView extends FUI_DegreeActivityView {
  ...
  onConstruct() {
    ...
    this.dayResetTime.text = LangManager.Instance.GetTranslation(
      "DegreeActivityView.dayRestTxt",
    );
    this.weekResetTime.text = LangManager.Instance.GetTranslation(
      "DegreeActivityView.weekResetTxt",
    );
    this.initFetchItem();
    this.refreshView();
  }
  ...
  private initFetchItem() {
    ...
    for (const key in this.cate.allList) {
      if (Object.prototype.hasOwnProperty.call(this.cate.allList, key)) {
        ...
        if (taskTemp) this.activityTasks.push(taskItem);
      }
    }
    this.refreshList();
  }
   ...
   */
  private refreshList() {
    this.activityTasks = ArrayUtils.sortOn(
      this.activityTasks,
      ["isComplete", "sort"],
  ...
  private initEvent() {
    ...
    this.dayBoxlist.itemRenderer = Laya.Handler.create(
      this,
      this.renderDaydHandler,
      null,
      false,
    );
    this.weekBoxlist.itemRenderer = Laya.Handler.create(
      this,
      this.renderWeekdHandler,
      null,
      false,
    );
    this.listCell.setVirtual();
    this.listCell.itemRenderer = Laya.Handler.create(
      this,
      this.renderListdHandler,
      null,
      false,
    );
  }
  ...
  renderDaydHandler(index: number, item: DegreeActivityBox) {
    ...
    if (this.cate.hasPick(index + 1)) item.state = DegreeActivityBox.CLOSE;
    else if (this.cate.canPick(value))
      //判断活跃度值是否满足
      item.state = DegreeActivityBox.OPEN;
    else item.state = DegreeActivityBox.DEFAULT;
  }
  ...
  renderWeekdHandler(index: number, item: DegreeActivityBox) {
    ...
    if (this.cate.hasPick(index + 6)) item.state = DegreeActivityBox.CLOSE;
    else if (this.cate.canPickWeek(value)) item.state = DegreeActivityBox.OPEN;
    else item.state = DegreeActivityBox.DEFAULT;
  }
  ...
  private refreshView() {
    ...
    this.activityBar.value = ProgressHelp.getCurrentValue(
      this.cate.active,
      this.ctrlData.dayMaxDegreePoint,
