...
export class CrossMapSearchActionQueue implements IEnterFrame {
  ...
  public static get Instance(): CrossMapSearchActionQueue {
    if (!CrossMapSearchActionQueue._instance)
      CrossMapSearchActionQueue._instance = new CrossMapSearchActionQueue();
    return CrossMapSearchActionQueue._instance;
  }

  constructor() {
    this.map1 = [
      { from: 10000, to: 20001, by: 5 },
    ...
    this.map2 = [
      { from: 20004, to: 20003, by: 2000401 },
  ...
  enterFrame() {
    if (this._actionList.length > 0) {
      ...
      if (currentScene == SceneType.SPACE_SCENE) {
        if (toExcuteAction.mapId == SpaceManager.Instance.mapId) {
          //天空之城, 可自动寻路
          this.excute();
        }
        return;
      ...
      if (mapModel) {
        if (toExcuteAction.mapId == mapModel.mapId) {
          //在指定的地图, 可自动寻路
          this.excute();
        }
        return;
  ...
  private excute() {
    if (this._actionList.length > 0) {
      {
        if (this._actionList.length >= 2 && this._needSwitch) {
          //跨多个地图时容易出现黑屏问题(从英灵岛切换到天空城后, 玩家模型在传送阵上不移动, 此时切换到内城跳转任务到英灵岛黑屏)
