...
export class PetRenameWnd extends BaseWindow {
  ...
  __petOptRet(pkg: PackageIn) {
    ...
    if (opt == 14) {
      ...
      var msgTips: string = "";
      if (msg.result == 1) {
        msgTips = LangManager.Instance.GetTranslation(
          "pet.renameFrame.describt08",
        );
        MessageTipManager.Instance.show(msgTips);
      } else {
        msgTips = LangManager.Instance.GetTranslation(
          "noviceII.node.AddUserNameFrame.describt06",
        );
        MessageTipManager.Instance.show(msgTips);
        return;
      }
  ...
  private btnConfirmClick() {
    if (StringHelper.trim(this.tfRename.text) == "") {
      this.setStateText(
        0,
        LangManager.Instance.GetTranslation("pet.renameFrame.describt02"),
      );
      return;
    }
    if (this._nickName == this.tfRename.text) {
      this.setStateText(
        0,
        LangManager.Instance.GetTranslation("pet.renameFrame.describt09"),
      );
      return false;
    }
    ...
    if (
      FilterWordManager.containUnableChar(this.tfRename.text) ||
      FilterWordManager.filterWrod(this.tfRename.text) != this.tfRename.text
    ) {
      this.setStateText(
        0,
        LangManager.Instance.GetTranslation("pet.renameFrame.describt05"),
      );
      return;
    }
    this.btnConfirm.enabled = false;
    ...
    return HttpUtils.httpRequest(
      ...
      if (data) {
        try {
          ...
          self.setStateText(
            0,
            LangManager.Instance.GetTranslation(error.message),
          );
          return;
        }
        if (ContentTxt) {
          try {
            ...
            if (Result && Result.value == "true") {
              self.setStateText(
                1,
                LangManager.Instance.GetTranslation(Result.message),
              );
            ...
            self.setStateText(
              0,
              LangManager.Instance.GetTranslation(e.message),
            );
          }
        } else {
