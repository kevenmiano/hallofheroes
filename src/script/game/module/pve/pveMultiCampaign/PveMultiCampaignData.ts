...
export default class PveMultiCampaignData extends FrameDataBase {
    ...
    public static EvaluateRes = ["Lab_D_L", "Lab_C_L", "Lab_B_L", "Lab_A_L", "Lab_S_L"]
    public static YuanShuModeCnt = 3
    public static YuanShuModeCnt2 = 2
    public static get ChapterTitle() {
        return [LangManager.Instance.GetTranslation("selectcampaign.view.chapter01"),
    ...
    public static get KinTowerModeName() {
        return [LangManager.Instance.GetTranslation("kingtower.difficulty.simple"),
    ...
    public static get NormalModeName() {
        return [LangManager.Instance.GetTranslation("kingtower.difficulty.common"),
        LangManager.Instance.GetTranslation("kingtower.difficulty.nightmare")]
    }
    ...
    public static getDifficultyById(id: string): string {
        ...
        switch (id) {
            case '1':
                str = LangManager.Instance.GetTranslation("kingtower.difficulty.simple")
                break;
            case '2':
                str = LangManager.Instance.GetTranslation("kingtower.difficulty.common")
                break;
            case '3':
                str = LangManager.Instance.GetTranslation("kingtower.difficulty.hard")
                break;
            case '4':
                str = LangManager.Instance.GetTranslation("kingtower.difficulty.nightmare")
                break;
            case '5':
    ...
    public getTreeData(): any[] {
        ...
        */
        ...
        for (let i = 0; i < 4; i++) {
            ...
            if (chapterTitle == PveMultiCampaignData.ChapterTitle[2] && this.thane.grades < PveMultiCampaignData.LevelLimit) {
                mainTabEnable = false
            }
            firstData.push(new SelectCampaignItemData(eSelectCampaignItemType.FirstItem, chapterTitle, chapterInfo.chapterId, mainTabEnable))
            ...
            for (let i = 0; i < PveMultiCampaignData.AreaMaxLen; i++) {
                ...
                let campaignData = areaData.getMapByDifficult(CampaignMapDifficulty.General);
                ...
                let grayFilterFlag = campaignData.MinLevel > ArmyManager.Instance.thane.grades
                //王者之塔 屏蔽item
                // if (_tabThreeBtn.enabled && !ConfigManager.areaData.KING_TOWER && areaData.areaId == CommonConstant.KINGTOWER_AREAID)
                temp.push(new SelectCampaignItemData(eSelectCampaignItemType.SecondItem, { icon: areaData.getAreaSmallIcon(), text: areaData.getItemTitle() }, areaData.areaId, areaEnabled, grayFilterFlag))
            }
            secondData.push(new SelectCampaignItemData(eSelectCampaignItemType.SecondItem, temp))
        }
    ...
    public getDistrictRewardById(chapterId: number): any[] {
        ...
        let dropDatas = TempleteManager.Instance.getDropItemssByDropId(+("12" + chapterId));
        for (let dropItem of dropDatas) {
            arr.push(new DistrictRewardInfo(chapterId, dropItem.ItemId, dropItem.Data));
        }
        return arr;
    }
    ...
    public isNormalOpen(): boolean {
        if (this.normalMap && this.normalMap.state != CampaignMapStatus.NO_ACCEPT_CAMPAIGN) return true;
        return false;
    }

    public isHeroOpen(): boolean {
        if (this.heroMap && this.heroMap.state != CampaignMapStatus.NO_ACCEPT_CAMPAIGN) return true;
        return false;
    }

    public isHellOpen(): boolean {
        if (this.hellMap && this.hellMap.state != CampaignMapStatus.NO_ACCEPT_CAMPAIGN) return true;
        return false;
    }
    ...
    private checkExist(type: number): boolean {
        if (!this.selectedArea)
            return false;
        if (!this.selectedArea.getMapByDifficult(type))
            return false;
        return true;
    }
