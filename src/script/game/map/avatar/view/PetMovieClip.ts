...
export class PetMovieClip extends Laya.Sprite implements IEnterFrame {
	...
	public set url(value: string) {
		...
		let args = this.createResourceLoadInfo(this._url, [], [], AvatarPosition.PET);
		ResMgr.Instance.loadRes(this._url, this.loaderCompleteHandler.bind(this), null, Laya.Loader.ATLAS, undefined, null, null, null, null, args);
	}

	private createResourceLoadInfo(url: string, sornStand: any[], sornWalk: any[], position: string): ResourceLoaderInfo {
	...
	private drawStand() {
		...
		let pngName: string = AvatarActions.ACTION_STOP + "_" + this.frameX + "_" + fy + ".png";
		...
		if (texture) {
			...
			this._bitmap && this._bitmap.graphics.drawImage(texture, destPoint.x, destPoint.y, texture.width, texture.height);
		}
	}
	...
	private drawWalk() {
		...
		let fy: number = this.getFrameY(this.frameY);
		let pngName: string = AvatarActions.ACTION_WALK + "_" + this.frameX + "_" + fy + ".png";
		let texture = ResMgr.Instance.getRes(this._resourceInfo.preUrl + pngName)
