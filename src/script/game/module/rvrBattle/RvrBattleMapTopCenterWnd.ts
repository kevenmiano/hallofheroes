...
export default class RvrBattleMapTopCenterWnd extends BaseWindow {
  ...
  private initEvent() {
    ...
    if (this._mapModel)
      this._mapModel.addEventListener(
        CampaignMapEvent.OPENTIME_MV,
        this.openTimeHandler,
        this,
      );
  ...
  public initData() {
    this.leftOffTimeTitle.text =
      LangManager.Instance.GetTranslation("securitycode.view.TimeDownTxt") +
      ": ";
    this.leftTimeTitle.text = LangManager.Instance.GetTranslation(
      "RvrBattleMapTopCenterWnd.leftTimeTitle.text",
    );
  }

  private syncErrorTimeHandler() {
    var vFlaushTime: string = LangManager.Instance.GetTranslation(
      "map.campaign.view.ui.pvpmap.PvpMapTopCenterView.vFlaushTime",
    );
    MessageTipManager.Instance.show(
      vFlaushTime + ": " + CampaignManager.Instance.mapModel.syncErrorTime,
    );
    this.updateTime();
  }
  ...
  private updateOpenTimeHandler() {
    this.leftTimeTxt.text = DateFormatter.getSevenDateString(
  ...
  private updateOffTimeHandler() {
    if (this.leftOffTimeTxt && !this.leftOffTimeTxt.isDisposed)
      this.leftOffTimeTxt.text = DateFormatter.getSevenDateString(
