...
export default class DialogWordHandler {
  ...
  public start() {
    ...
    Laya.Tween.to(
      ...
      0.25,
      undefined,
      Laya.Handler.create(this, this.speakStart),
    );
  }
  ...
  private speakComplete() {
    if (!this._completed) {
      ...
      if (this._speakEnd != null) this._speakEnd();
  ...
  private parseDialogs(str: string) {
    ...
    while ((info = this._charsInfo.shift())) {
      this._bbaa =
        this._bbaa.slice(0, info.i) + info.c + this._bbaa.slice(info.i);
  ...
  public dispose() {
    ...
    this._completed = true;
    this._contents = [];
    if (this._textfield) Laya.Tween.clearAll(this._textfield);
  }
}
