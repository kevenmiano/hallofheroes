...
export class TaskTraceTipManager {
  ...
  private addEvent() {
    GoodsManager.Instance.addEventListener(
      BagEvent.ADD_GOODS,
      this.__addGoodsHandler,
      this,
    );
    GoodsManager.Instance.addEventListener(
      BagEvent.NEW_GOODS,
      this.__newGoodsHandler,
      this,
    );
    GoodsManager.Instance.addEventListener(
      BagEvent.UPDATE_BAG,
    ...
    this.playerInfo.addEventListener(
      ...
      this.__wearyChangeHandler,
      this,
    );
    VIPManager.Instance.model.addEventListener(
      VIPEvent.VIP_RECHARGE_TIP,
      this.__vipRechargeHandler,
      this,
    );
  }
  ...
  private __addGoodsHandler(goods: GoodsInfo) {
    ...
    if (
      goods &&
      goods.templateInfo.SonType == GoodsSonType.SONTYPE_OPENBOX &&
      goods.isNew
    ) {
      var data: TipMessageData = new TipMessageData();
  ...
  private __newGoodsHandler(evt) {
    ...
    for (var i: number = 0; i < list.length; i++) {
      ...
      if (
        this.thane.grades == 5 &&
        goods.templateInfo.SonType == GoodsSonType.SONTYPE_NOVICE_BOX
      ) {
        //5级新手宝箱
        this.checkNoviceBoxTip();
      }
      if (
        TaskTraceTipManager.showVipTipView &&
        GoodsCheck.isVipExperienceCard(goods)
      ) {
        //vip体验卡
        TaskTraceTipManager.showVipTipView = false;
        DelayActionsUtils.Instance.addAction(
          new AlertTipAction(
            { type: goods.templateId, data: goods },
            this.vipCardHandler.bind(this),
          ),
        );
        continue;
      }
      ...
      if (
        goods.templateInfo.SonType == GoodsSonType.SONTYPE_TREASURE_MAP &&
        TreasureMapManager.Instance.needShowTip
      ) {
        var data: TipMessageData = new TipMessageData();
        data.title = LangManager.Instance.GetTranslation("public.prompt");
        data.goods = goods;
        ...
        data.type = TipMessageData.OPENBOX_INTO_BAG;
        this.showProgressView(data);
      } else if (FashionManager.Instance.fashionModel.isFashion(goods)) {
        //时装
        var data: TipMessageData = new TipMessageData();
        data.title = LangManager.Instance.GetTranslation("public.prompt");
        data.goods = goods;
        ...
        data.content = LangManager.Instance.GetTranslation(
          "tasktracetip.view.OpenBagTipView.newFashion",
        );
        this.showView(data);
      }
    }
    if (arr.length > 0) {
      DelayActionsUtils.Instance.addAction(
        new AlertTipAction(
          arr[arr.length - 1],
          this.betterNewGoodsHandler.bind(this),
        ),
      );
    }
    arr = null;
  ...
  private __updateGoodsHandler(evts: GoodsInfo[], counts: number[]) {
    ...
    for (let evt of evts) {
      ...
      i++;
      if (
        evt.templateInfo.SonType == GoodsSonType.SONTYPE_OPENBOX &&
        count > 0 &&
        PlayerManager.Instance.currentPlayerModel.mailWndIsOpen
      ) {
        var data: TipMessageData = new TipMessageData();
        data.title = LangManager.Instance.GetTranslation("public.prompt");
        data.goods = evt;
  ...
  private __wearyChangeHandler(evt) {
    ...
    if (this.thane.grades < 20 && this.playerInfo.weary == 0) {
      var list: any[] = GoodsManager.Instance.getGoodsBySonType(
        GoodsSonType.SONTYPE_USEABLE,
      );
      for (const key in list) {
        if (Object.prototype.hasOwnProperty.call(list, key)) {
          ...
          if (info.templateInfo.Property1 == 5) {
            DelayActionsUtils.Instance.addAction(
              new AlertTipAction(info, this.useWearyHandler.bind(this)),
            );
          }
        }
      }
    ...
    for (const key in list) {
      if (Object.prototype.hasOwnProperty.call(list, key)) {
        ...
        if (
          ...
          !BoxManager.Instance.isRecviedByGrade(grade)
        ) {
          var goods: GoodsInfo =
            GoodsManager.Instance.getGeneralBagGoodsBySonType(
              GoodsSonType.SONTYPE_NOVICE_BOX,
            )[0];
          if (
            goods &&
            goods.templateInfo &&
            goods.templateInfo.NeedGrades == grade
          ) {
            DelayActionsUtils.Instance.addAction(
              new AlertTipAction(goods, this.noviceBoxAlertTip.bind(this)),
            );
          }
        }
      }
  ...
  public showView(data: TipMessageData) {
    ...
    if (view) view.Instance.Show(data);
  }
  ...
  public showProgressView(data: TipMessageData) {
    ...
    if (this.showTipList.length == 1) this.showView(data);
  }
  ...
  private checkShowTaskTraceTip(data: TipMessageData): boolean {
    ...
    switch (SceneManager.Instance.currentType) {
      ...
      case SceneType.WARLORDS_ROOM:
        if (
          data.type != TipMessageData.BUY_BLOOD &&
    ...
    switch (obj.type) {
      case GoodsSonType.SONTYPE_VIP_BOX:
        ...
        tipData.content = LangManager.Instance.GetTranslation(
          "tasktracetip.view.OpenBagTipView.vipcoin",
        );
        break;
      default:
  ...
  private kingContractCardHandler(good: GoodsInfo) {
    var tipData: TipMessageData = new TipMessageData();
    tipData.title = LangManager.Instance.GetTranslation(
      "KingContractTipView.title",
    );
    tipData.goods = good;
    tipData.content = LangManager.Instance.GetTranslation(
      "KingContractTipView.content",
    );
  ...
  private betterNewGoodsHandler(result: object) {
    ...
    data.goods = goods;
    data.type = TipMessageData.OPEN_BAG;
    if (FashionManager.Instance.fashionModel.isFashion(goods)) {
      //时装
      data.content = LangManager.Instance.GetTranslation(
        "tasktracetip.view.OpenBagTipView.newFashion",
      );
  ...
  private __disposeFromListHandler(evt) {
    ...
    if (index != -1) this._viewList.splice(index, 1);
  }
  ...
  public addBufferDisapearTip(buffer: PlayerBufferInfo) {
    DelayActionsUtils.Instance.addAction(
      new AlertTipAction(buffer, this.bufferDisapearTip.bind(this)),
    );
  }
