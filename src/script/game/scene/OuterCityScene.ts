...
export class OuterCityScene extends BaseSceneView {
  ...
  private initData(data: object): void {
    if (data instanceof MapInitData) {
      ...
      let map: number =
        mI.mapTempInfo.Id == 100
          ? this.mapNodeInfo.info.mapId
          : mI.mapTempInfo.Id;
        ...
        for (const tempKey in temp) {
          if (temp.hasOwnProperty(tempKey)) {
            ...
            if (selfVehicle) {
              if (
                c &&
                Math.abs(c.x - selfVehicle.x) < this.CAN_SEE_LONG &&
            ...
            if (this.selfArmy.armyView != null) {
              if (
                Math.abs(c.x - this.selfArmy.armyView.x) > this.CAN_SEE_LONG ||
                Math.abs(c.y - this.selfArmy.armyView.y) > this.CAN_SEE_LONG
              ) {
                if (c && c.info && c.info.types == PosType.OUTERCITY_MINE) {
                  //金矿全显示
                  c.nodeViewVisible = true;
                  continue;
                } else if (
                  c &&
                  c.info &&
                  c.info.types == PosType.OUTERCITY_BOSS_NPC
                ) {
                  //BOSS
                  if (
  ...
  private checkInTeamDistance(x: number, y: number): boolean {
    ...
    if (FreedomTeamManager.Instance.model) {
      //有队伍
      var members: Array<BaseArmy> =
        FreedomTeamManager.Instance.model.allMembers;
      for (let i: number = 0; i < members.length; i++) {
        let baseArmy: BaseArmy = members[i];
        if (
          baseArmy &&
          Math.abs(baseArmy.curPosX * Tiles.WIDTH - x) < this.CAN_SEE_LONG &&
      ...
      if (allCastles.hasOwnProperty(allCastlesKey)) {
        ...
        for (const tempKey in temp) {
          if (temp.hasOwnProperty(tempKey)) {
            ...
            if (
              (baseCastle &&
                baseCastle.tempInfo.ID ==
                  OuterCityModel.SELT_CALSTE_TEMPLATEID) ||
              baseCastle.defencerGuildName == this.playerInfo.consortiaName
            ) {
              if (
                Math.abs(baseCastle.x - x) < this.CAN_SEE_LONG &&
  ...
  private checkInVehicleDistance(x: number, y: number): boolean {
    ...
    for (const allVehicleDicKey in allVehicleDic) {
      if (allVehicleDic.hasOwnProperty(allVehicleDicKey)) {
        let outercityVehicleArmyView: OutercityVehicleArmyView =
          allVehicleDic.get(allVehicleDicKey);
        if (
          Math.abs(outercityVehicleArmyView.x - x) < this.CAN_SEE_LONG &&
          Math.abs(y - outercityVehicleArmyView.y) < this.CAN_SEE_LONG &&
          selfVehicle &&
          selfVehicle.wildInfo.templateId == parseInt(allVehicleDicKey)
        ) {
          flag = true;
        }
      }
      ...
      if (allCastles.hasOwnProperty(allCastlesKey)) {
        ...
        for (const tempKey in temp) {
          if (temp.hasOwnProperty(tempKey)) {
            let baseCastle: BaseCastle = temp[tempKey];
            if (
              Math.abs(baseCastle.x - c.x) < this.CAN_SEE_LONG &&
  ...
  private updateArmyVisible(): void {
    ...
    for (const allArmyDictKey in allArmyDict) {
      if (allArmyDict.hasOwnProperty(allArmyDictKey)) {
        ...
        //在队友的可见范围内
        if (
          this.checkInTeamDistance(outerCityArmyView.x, outerCityArmyView.y)
        ) {
          outerCityArmyView.visible = true;
          continue;
        }
        ...
        if (
          (this,
          this.checkInVehicleDistance(outerCityArmyView.x, outerCityArmyView.y))
        ) {
          outerCityArmyView.visible = true;
          continue;
        }
        if (this.selfArmy.armyView) {
          let inDistance =
            Math.abs(outerCityArmyView.x - this.selfArmy.armyView.x) <=
              this.CAN_SEE_LONG &&
            Math.abs(outerCityArmyView.y - this.selfArmy.armyView.y) <=
              this.CAN_SEE_LONG;
          if (inDistance) {
            outerCityArmyView.visible = notHideOther;
  ...
  private checkTeamMember(nodeX: number, nodeY: number): boolean {
    ...
    for (i = 0; i < arr.length; i++) {
      ...
      if (
        Math.abs(nodeX - baseAemy.armyView.x) > this.CAN_SEE_LONG ||
        Math.abs(nodeY - baseAemy.armyView.y) > this.CAN_SEE_LONG
      ) {
        b = false;
      } else {
        b = true;
  ...
  public enterOver(): Promise<void> {
    return new Promise(async (resolve) => {
      ...
      if (!HomeWnd.Instance.isShowing) {
        ...
        HomeWnd.Instance.getSmallMapBar().iconContainerY =
          center.y / UIConstant.SMALL_MAP_SCALE;
      }
      await UIManager.Instance.ShowWind(EmWindow.SpaceTaskInfoWnd);
      if (
        OuterCityManager.Instance.model &&
        PlayerManager.Instance.currentPlayerModel &&
        this.playerModel.sumCD >
        ...
        if (this._preSceneData.isOpenColosseum) {
          ...
          if (fightModel) {
            if (fightModel.castleNodeId) {
              FrameCtrlManager.Instance.open(EmWindow.OuterCityWarWnd, {
                needRefreshAll: true,
              });
            } else {
      ...
      if (
        this._parms &&
        this._parms instanceof MapInitData &&
        !this._parms["showMapName"]
      ) {
        resolve();
        return;
      }
      if (
        this._parms &&
        this._parms.hasOwnProperty("showMapName") &&
        !this._parms["showMapName"]
      ) {
        resolve();
        return;
      }
      UIManager.Instance.ShowWind(EmWindow.MapNameMovie, {
        backCall: this.sendComplete.bind(this),
        mapId: this._model.mapId,
        mapName: this._model.mapTempInfo.MapNameLang,
      });
        ...
        if (data instanceof BaseCastle) {
          ...
          switch (this._model.walkTargerData.info.types) {
            ...
            case PosType.OUTERCITY_COMMON_NPC:
              if (this._model.walkTargerData.hasOwnProperty("_tempInfo")) {
  ...
  private __startMoveHandler(data: any): void {
    ...
    if (data instanceof Laya.Point) {
    ...
    } else if (data instanceof MapPhysics) {
      //怪物
      ...
      if (temp.nodeView) {
        let mapPhysicsCastle: MapPhysicsCastle =
          temp.nodeView as MapPhysicsCastle;
        ...
        temp.info.types == PosType.OUTERCITY_BOSS_NPC ||
  ...
  private __updateScreenArmysHandler(pkg: PackageIn): void {
    ...
    if (
      flag &&
      !point &&
      !this._model.isFlying(ArmyManager.Instance.army.mountTemplateId)
    ) {
      //当前点不可行走，并且玩家不是飞行坐骑，对玩家坐标进行修正。
      ...
      for (let i: number = 0; i < len; i++) {
        ...
        if (outCityMineNode.nodeId == msg.nodeId) {
          ...
          for (let j: number = 0; j < nodeLen; j++) {
            ...
            if (oneMineInfoMsg.occupyPlayerId == this.playerInfo.userId) {
              //自己的
              outCityOneMineInfo.sort = 1;
            } else if (oneMineInfoMsg.guildId == this.playerInfo.consortiaID) {
              //同工会
   ...
   *   Socket请求数据
   ********************************/
  public sendReturnHomeArmy(): boolean {
    //传回城堡
    let str: string = "";
