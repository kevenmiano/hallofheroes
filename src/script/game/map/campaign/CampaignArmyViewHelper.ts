...
export class CampaignArmyViewHelper {
   ...
   *  触发节点返回true
   */
  public static selfArmyEvent(
    userId: number,
    aiInfo: BaseArmyAiInfo,
    target: object,
    p: Laya.Point,
  ...
  public static selfArmyToEnd(aiInfo: BaseArmyAiInfo, target: any): boolean {
    //自已部队停下来以后的事件检测
    var node: CampaignNode = CampaignArmyViewHelper.mapModel.getHandlerNode(
      target.x / 20,
      target.y / 20,
    );
    ...
    var p: Laya.Point = CampaignArmyViewHelper.getResetPoint(
      node,
      target,
      new Laya.Point(
        parseInt((target.x / 20).toString()),
        parseInt((target.y / 20).toString()),
      ),
    );
    ...
    Logger.xjy(
      "[CampaignArmyViewHelper]selfArmyToEnd 玩家位置",
      target.x,
      target.y,
      "触发节点位置",
      node.fixX,
  ...
  private static launchFight(
    ...
    if (CampaignArmyViewHelper.selfarmy) {
      var node: CampaignNode =
        CampaignArmyViewHelper.mapModel.getNotHandlerNodeByXY(
          nextPoint.x,
          nextPoint.y,
        ); ///找出事件点
    ...
    if (node.info.state == NodeState.FIGHTING) {
      if (
        WorldBossHelper.checkConsortiaSecretLand(
          CampaignArmyViewHelper.mapModel.mapId,
        )
      )
        return false;
      var msg: string;
      if (WorldBossHelper.checkPvp(CampaignArmyViewHelper.mapModel.mapId)) {
        CampaignManager.Instance.controller.sendCampaignArrive(
          CampaignArmyViewHelper.selfarmy.id,
          node.nodeId,
        );
        return true;
      }
      ...
      if (
        ...
        CampaignManager.Instance.mapModel.campaignTemplate.Capacity <= 1
      )
        return true;
  ...
  public static getNodeStationPoint(
    info: AIBaseInfo,
    node: CampaignNode,
  ): Laya.Point {
    //从当前点后退三个点
  ...
  public static getNodeStationPointImp(node: CampaignNode): Laya.Point {
    ...
    for (let point of arr) {
      if (
        CampaignManager.Instance.controller.getWalkable(
          node.posX + point.x,
          node.posY + point.y,
        )
      ) {
  ...
  private static goSelectPoint(target: any): CampaignNode {
    ...
    if (nInfo) {
      if (CampaignManager.Instance.mapModel.onCollectionId == nInfo.nodeId)
        return null;
  ...
  public static searchPath(
    ...
    //搜路线
    var startPoint: Laya.Point = PointUtils.scaleTransform(
      $start,
      unitWidth,
      unitHeight,
    );
    var endPoint: Laya.Point = PointUtils.scaleTransform(
      $end,
      unitWidth,
      unitHeight,
    );
  ...
  public static checkCollection(node: CampaignNode): boolean {
    if (node)
      if (CampaignArmyViewHelper.mapModel.onCollectionId == node.nodeId)
        return true;
    return false;
  }
  /**
  ...
  public static checkCollectionNode(nodeInfo: CampaignNode): boolean {
    ...
    if (nodeInfo.info) return nodeInfo.info.types == PosType.COLLECTION;
    else {
