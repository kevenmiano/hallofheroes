...
export default class CastleBuildingLoader {
    ...
    private isNotMoling() {
        ...
        for (let index = 0; index < this._pathArray.length; index++) {
            ...
            BuildingManager.Instance.model._resUrlNameMap.set(this._pathArray[index], true);
        }
    ...
    private _effectComplete() {
        for (let index = 0; index < this._pathArray.length; index++) {
            ...
            if (index == 0) {
                ...
                this._info.templeteInfo.view.graphics.loadImage(path, 0, 0, 0, 0, this.completePic.bind(this));
            } else {
                ...
                if (res) {
                    ...
                    AnimationManager.Instance.createAnimation(this._preUrl, "", 0, "", AnimationManager.MapPhysicsFormatLen);
                    let tl = new MovieClip(this._cacheName);
                    BuildingManager.Instance.model._cacheNameMap.set(this._cacheName, true);
                    if (!this._info.templeteInfo.effect && this._info.templeteInfo.SonType!= BuildingType.TREE) {
