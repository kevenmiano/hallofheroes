...
export default class OuterCityVehicleMediator
  implements IMediator, IEnterFrame
{
  ...
  public register(target: any) {
    this._selfInfo = OuterCityManager.Instance.model.getWorldArmyById(
      ArmyManager.Instance.army.id,
    );
    NotificationManager.Instance.addEventListener(
      OuterCityEvent.OUTERCITY_LOCK_VEHICLE_FIGHT,
      this.__lockWarFightHandler,
      this,
    );
    NotificationManager.Instance.addEventListener(
      OuterCityEvent.OUTERCITY_UNLOCK_VEHICLE_FIGHT,
      this.__unlockWarFightHandler,
      this,
    );
  }
  ...
  private __lockWarFightHandler(data: WildLand) {
    this._wildInfo = data;
    this._targetInfo = OuterCityManager.Instance.model.allVehicleViewDic.get(
      data.templateId,
    );
  ...
  public enterFrame() {
    ...
    let selfView: OuterCityArmyView = this._selfInfo
      .armyView as OuterCityArmyView;
    if (this._targetInfo && armyView) {
      ...
      if (str != "") {
        //追求目标有公会占领
        if (OuterCityManager.Instance.model.checkIsSameConsortiaByName(str)) {
          //同工会的
          if (leng < 10000) {
            this.unLockWarFight();
            FrameCtrlManager.Instance.open(
              EmWindow.OuterCityVehicleInfoWnd,
              this._targetInfo.wildInfo,
            );
          } else if (this._targetInfo.wildInfo.status == 2) {
            this.unLockWarFight();
          } else if (this._count % 70 == 0 || !this._hasWalk) {
            //跟随目标
        ...
        if (leng < 10000) {
          this.unLockWarFight();
          if (
            OuterCityManager.Instance.model.checkTeamInFight(this._wildInfo) ||
            OuterCityManager.Instance.model.checkCanAttackVehicle(
              this._wildInfo,
            )
          ) {
  ...
  private sendAttack() {
    OuterCityManager.Instance.vehicleAttck(
      this._targetInfo.wildInfo.templateId,
    );
   ...
   */
  private searchPath(
    selfArmy: OuterCityArmyView,
    targertArmy: OutercityVehicleArmyView,
  ) {
    let endPoint = new Laya.Point(targertArmy.x, targertArmy.y);
    let curr: Laya.Point = new Laya.Point(selfArmy.x, selfArmy.y);
    let paths;
    if (
      OuterCityManager.Instance.model.isFlying(
        ArmyManager.Instance.army.mountTemplateId,
      )
    ) {
      paths = [curr, endPoint];
    } else {
      paths =
        OuterCityManager.Instance.mapView.worldWalkLayer.pathScene.searchPath(
          curr,
          endPoint,
        );
    }
