...
export default class BuildingBase extends Laya.Sprite implements IStopEffect {
	...
	constructor() {
		...
		this.initName();
		this._imgRedPoint = FUIHelper.createFUIInstance(EmPackName.Base, "Img_RedDot");
		this.addEvent();
	}

	protected addEvent() {
		BuildingManager.Instance.addEventListener(BuildingEvent.BUILDING_NAME_SHOW_HIDE, this._changeBuildName, this);
		NotificationManager.Instance.on(NotificationEvent.STOP_EFFECT, this.__notificationHandler, this);
	}

	protected removeEvent() {
		BuildingManager.Instance.removeEventListener(BuildingEvent.BUILDING_NAME_SHOW_HIDE, this._changeBuildName, this);
		NotificationManager.Instance.off(NotificationEvent.STOP_EFFECT, this.__notificationHandler, this);
	}
	...
	public addChildByScale(scale: number) {
		let namePos = CastleConfigUtil.Instance.getBuildNamePos(this._buildingInfo.templeteInfo.SonType, EmCastlePos.Castle);
	...
	public removeName() {
		if (this._nameContain.parent) this._nameContain.parent.removeChild(this._nameContain);
	}
	...
	refreshView() {
		...
		if (!this._nameBack) {
			...
			if (this._buildingInfo.templeteInfo.SonType >= 1501) {
				...
				this._nameTxt.text = this._buildingInfo.templeteInfo.BuildingNameLang;
				this._levelTxt.text = this._buildingInfo.templeteInfo.BuildingGrade.toString();
	...
	public setRedPoint(b: boolean) {
		if (this._imgRedPoint) {
			...
			this._imgRedPoint.x = this._nameBack.x + this._nameBack.width - this._imgRedPoint.width / 2;
			this._imgRedPoint.y = this._nameBack.y - this._imgRedPoint.height * this._imgRedPoint.scaleY / 2;
	...
	public dispose() {
		...
		ObjectUtils.disposeObject(this._nameContain); this._nameContain = null;
