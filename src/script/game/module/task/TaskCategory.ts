...
export default class TaskCategory extends GameEventDispatcher {
  ...
  public addMobileTaskBySwitch() {
    ...
    for (let i: number = 0; i < this._acceptedTaskList.length; i++) {
      ...
      if (
        temp.conditionList[0].CondictionType == TaskConditionType.PHONE_CHECK
      ) {
        return;
      }
    }
  ...
  public addClientDownTaskBySwitch() {
    ...
    for (let i: number = 0; i < this._acceptedTaskList.length; i++) {
      let temp: TaskTemplate = this._acceptedTaskList[i];
      if (
        temp.conditionList[0].CondictionType == TaskConditionType.CLIENT_DOWN
      ) {
        return;
      }
    }
  ...
  private removeByType(taskTemp: TaskTemplate) {
    if (this._mainTaskList.indexOf(taskTemp) >= 0) {
      ...
      this._backplayerTaskList.splice(
        this._backplayerTaskList.indexOf(taskTemp),
        1,
      );
  ...
  public commit() {
    if (this._changeObj[TaskCategory.MANUAL_TASK_ADDED])
      this.dispatchEvent(
        TaskEvent.MANUAL_TASK_ADDED,
        this._changeObj[TaskCategory.MANUAL_TASK_ADDED],
      );
    if (this._changeObj[TaskCategory.MANUAL_TASK_REMOVE])
      this.dispatchEvent(
        TaskEvent.MANUAL_TASK_REMOVE,
        this._changeObj[TaskCategory.MANUAL_TASK_REMOVE],
      );
    if (this._changeObj[TaskCategory.ADD_TASK])
      this.dispatchEvent(
        TaskEvent.TASK_ADDED,
        this._changeObj[TaskCategory.ADD_TASK],
      );
    if (this._changeObj[TaskCategory.REMOVE_TASK])
      this.dispatchEvent(
        TaskEvent.TASK_REMOVE,
        this._changeObj[TaskCategory.REMOVE_TASK],
      );
  }

  public get traceList(): Array<TaskTemplate> {
    var vec: Array<TaskTemplate> = [];
    for (let i: number = 0; i < this._acceptedTaskList.length; i++) {
