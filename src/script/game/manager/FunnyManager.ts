...
export default class FunnyManager extends GameEventDispatcher {
  ...
  private initEvent() {
    ServerDataManager.listen(
      S2CProtocol.U_C_SUM_ACTIVE_TEMP,
      this,
      this.__funnyTemHandler,
    ); //精彩活动模版
    ServerDataManager.listen(
      S2CProtocol.U_C_SUMACTIVE_DATA,
      this,
      this.__funnyUserHandler,
    ); //精彩活动用户数据
    ServerDataManager.listen(
      S2CProtocol.U_C_NEW_PLAYER_GIFT,
      this,
      this.__backPlayerHandler,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_SUMACTIVE_ACTIVATEDATA_RESP,
      this,
      this.__funnyStateHandler,
    ); //多服参与活动的状态
  }
  ...
  protected __funnyStateHandler(pkg: PackageIn) {
    let msg: SumActiveActivateRespMsg = pkg.readBody(
      SumActiveActivateRespMsg,
    ) as SumActiveActivateRespMsg;
    ...
    switch (msg.ret) {
      case 0:
        str = LangManager.Instance.GetTranslation(
          "funny.datas.FunnyBagData.joinfailed1",
        );
        break;
      case 1:
        str = LangManager.Instance.GetTranslation(
          "funny.datas.FunnyBagData.joinsucc",
        );
        break;
      case -9:
        str = LangManager.Instance.GetTranslation(
          "funny.datas.FunnyBagData.joinfailed2",
        );
        break;
      case -10:
        str = LangManager.Instance.GetTranslation(
          "funny.datas.FunnyBagData.joinfailed3",
        );
        break;
      case -11:
        str = LangManager.Instance.GetTranslation(
          "funny.datas.FunnyBagData.joinfailed4",
        );
        break;
      case -12:
        str = LangManager.Instance.GetTranslation(
      ...
      default:
        str = LangManager.Instance.GetTranslation(
          "funny.datas.FunnyBagData.joinfailed7",
        );
    }
  ...
  private __funnyTemHandler(pkg: PackageIn) {
    ...
    for (var i: number = 0; i < actives.length; i++) {
      ...
      if (
        DateFormatter.parse(
          (actives[i] as SumActiveinfoMsg).stopTime,
          "YYYY-MM-DD hh:mm:ss",
        ).getTime() < this.playerModel.nowDate
      ) {
        continue; //过了展示时间则不添加
      }
      ...
      funnyData.startTime = DateFormatter.parse(
        activesData.beginDate,
        "YYYY-MM-DD hh:mm:ss",
      ).getTime();
      funnyData.endTime = DateFormatter.parse(
        activesData.endDate,
        "YYYY-MM-DD hh:mm:ss",
      ).getTime();
      funnyData.showStart = DateFormatter.parse(
        activesData.startTime,
        "YYYY-MM-DD hh:mm:ss",
      ).getTime();
      funnyData.showEnd = DateFormatter.parse(
        activesData.stopTime,
        "YYYY-MM-DD hh:mm:ss",
      ).getTime();
      ...
      for (var j: number = 0; j < packages.length; j++) {
        ...
        for (var k2: number = 0; k2 < conditions.length; k2++) {
          ...
          condition.value = (
            conditions[k2] as PackageConditionMsg
          ).conditionValue;
  ...
  private __funnyUserHandler(pkg: PackageIn) {
    ...
    for (var i: number = 0; i < msgLen; i++) {
      var funnyData: FunnyData = this.findFunnyDataById(
        (msg.datas[i] as ActiveData).activeId,
      );
      if (funnyData == null) {
        continue;
      }
      ...
      for (var k: number = 0; k < pagLen; k++) {
        var bagData: FunnyBagData = this.findFunnyBagDataById(
          funnyData,
          ((msg.datas[i] as ActiveData).packdata[k] as PackageData).packId,
        );
        if (bagData == null) {
          continue;
        }
        bagData.status = (
          (msg.datas[i] as ActiveData).packdata[k] as PackageData
        ).status;
        bagData.remainTime = (
          (msg.datas[i] as ActiveData).packdata[k] as PackageData
        ).countDown;
        ...
        if (bagData.status == 1) {
          if (
            funnyData.type != FunnyType.RECHARGE_TIME &&
            funnyData.type != FunnyType.TYPE_ONLINE
          ) {
            changeShine = true;
    ...
    if (
      this._needTipRegression &&
      this.playerInfo.isBackPlayer &&
      !this._hasGetGift &&
      this._regressionActiveId != ""
    ) {
      this._needTipRegression = false;
      DelayActionsUtils.Instance.addAction(
        new AlertTipAction({ message: "" }, this.__regressionDelayHandler),
      );
    }
    if (change) {
  ...
  public get showNumer(): number {
    ...
    for (i = 0; i < this._data.length; i++) {
      if (
        this._data[i].showEnd < this.playerModel.nowDate ||
        this._data[i].showStart > this.playerModel.nowDate
      )
        continue; //时间选择活动
      ...
      if (
        this._data[i].type == FunnyType.TYPE_LEAVE &&
        !this.playerInfo.isBackPlayer
      )
        continue; //老玩家回归活动
  ...
  public getLuckExchangeShowData(id: string = ""): LuckExchangeTempMsg {
    ...
    for (
      let i: number = 0;
      i < LuckyExchangeManager.Instance.tempList.length;
      i++
    ) {
  ...
  private sortMasterType(info1: any, info2: any): number {
    ...
    if (info1.temType == 1) {
      temp1 = TempleteManager.Instance.getGoodsTemplatesByTempleteId(
        info1.temId,
      );
    } else {
    ...
    if (info2.temType == 1) {
      temp2 = TempleteManager.Instance.getGoodsTemplatesByTempleteId(
        info2.temId,
      );
    } else {
  ...
  private __regressionDelayHandler(result: string) {
    ...
    TaskTraceTipManager.Instance.cleanByType(
      TipMessageData.REGRESSION_TIP_VIEW,
    );
  ...
  public get rechargeData(): FunnyData {
    for (var i: number = 0; i < this._data.length; i++) {
      if (
        this._data[i].type == FunnyType.RECHARGE_TIME &&
        this._data[i].startTime <=
          PlayerManager.Instance.currentPlayerModel.nowDate &&
        PlayerManager.Instance.currentPlayerModel.nowDate <=
          this._data[i].endTime
      ) {
        return this._data[i];
      }
    }
  ...
  public setActivityBtnState() {
    ...
    if (this._activityArr.length > 0) {
      for (var i: number = 0; i < this._activityArr.length; i++) {
        if (this._activityArr.indexOf(this.activityList[i]) < 0) {
          //出现新的活动兑奖
          if (this.canShine) this.dispatchEvent(FunnyEvent.ITEM_CANGET);
        }
      }
    }
    this._activityArr = this.activityList;
  ...
  public get selectedFunnyData(): FunnyData {
    for (var i: number = 0; i < this._data.length; i++) {
      if (this._data[i].id == this.selectedId) return this._data[i];
    }
    return null;
  }
