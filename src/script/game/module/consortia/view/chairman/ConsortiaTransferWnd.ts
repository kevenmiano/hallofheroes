...
export class ConsortiaTransferWnd extends BaseWindow {
  ...
  public OnInitWind() {
    ...
    this.frame.getChild("title").text = LangManager.Instance.GetTranslation(
      "ConsortiaTransferWnd.title",
    );
    this.txtDesc.text = LangManager.Instance.GetTranslation(
      "ConsortiaTransferWnd.txtDesc",
    );
    ...
    this.btnConfirm.title = LangManager.Instance.GetTranslation(
      "ConsortiaTransferWnd.btnConfirm",
    );
    this.initData();
    this.initEvent();
    this.initView();
  ...
  private __getUserInfoHandler(pkg: PackageIn) {
    ...
    if (
      msg.info &&
      msg.info.nickName.toLowerCase() == this.tfSearch.text.toLowerCase()
    ) {
      if (
        msg.info.userId ==
        ...
        str = LangManager.Instance.GetTranslation(
          "consortia.view.myConsortia.chairmanPath.ConsortiaTransferFrame.command04",
        );
      ...
      if (msg.info.grades < ConsortiaModel.CREAT_NEEDED_GRADES) {
        str = LangManager.Instance.GetTranslation(
          "consortia.view.myConsortia.chairmanPath.ConsortiaTransferFrame.command05",
        );
        MessageTipManager.Instance.show(str);
        return;
      }
  ...
  private btnConfirmClick() {
    ...
    if (StringHelper.isNullOrEmpty(this.tfSearch.text)) {
      var str: string = LangManager.Instance.GetTranslation(
        "consortia.view.myConsortia.chairmanPath.ConsortiaTransferFrame.command03",
      );
      MessageTipManager.Instance.show(str);
      return;
    }
