    ...
    set info(value: any) {
        ...
        if (this._info) {
            if (this._info instanceof ShopGoodsInfo) {
                ...
                this.txt_name.color = GoodsSonType.getColorByProfile(goodsInfo.templateInfo.Profile);
                ...
                this.runeGroup.visible = false;
                if (this._info.ShopType == ShopGoodsInfo.CONSORTIA_SHOP 
                    || this._info.ShopType == ShopGoodsInfo.ADVCONSORTIA_SHOP
                    || this._info.ShopType == ShopGoodsInfo.CONSORTIA_HIGH_SHOP) {//公会商城
                    this.enabled = true;
                    let goods: t_s_itemtemplateData = TempleteManager.Instance.getGoodsTemplatesByTempleteId(this._info.ItemId);
                    if (this.checkIsRunes(this._info) && goods.Property1 > 0) {//是符文
                        let state = OuterCityShopManager.instance.model.getRuneState(goods)
                        if (state == 1) {//已经学习
                            this.runeDescTxt.text = LangManager.Instance.GetTranslation("yishi.view.tips.goods.ComposeTip.vStudy");
                            this.runeGroup.visible = true;
                        } else if (state == 2) {//已经拥有
                            this.runeDescTxt.text = LangManager.Instance.GetTranslation("ExchangeRandomItem.isOwn");
                            this.runeGroup.visible = true;
                        } else {
                            this.runeGroup.visible = false;
                        }
                    ...
                    if (this._info.NeedConsortiaLevels > this.consortiaLevel) {
                        ...
                        this.txt_openDescible.text = LangManager.Instance.GetTranslation("consortia.view.myConsortia.building.ConsortiaShopGoodsItem.tips", this._info.NeedConsortiaLevels);
                    } else {
                        UIFilter.normal(this);
                        this.txt_openDescible.visible = false;
                        this.txt_openDescible.text = "";
                    }
                } else if (this._info.ShopType == ShopGoodsInfo.ATHLETICS_SHOP || this._info.ShopType == ShopGoodsInfo.WARLORDS_SHOP) {
                ...
                ResRefCountManager.loadRes(realPath, (res) => {
                    ...
                    this._movie.pos((110 - sourceSize.width) / 2, (120 - sourceSize.height) / 2)
