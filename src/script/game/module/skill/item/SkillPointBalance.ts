...
export class SkillPointBalance extends FUI_SkillBalance {
  ...
  _onRestSkill() {
    ...
    let cfgItem =
      TempleteManager.Instance.getConfigInfoByConfigName("skill_reset_gold");
    if (cfgItem) {
  ...
  updateView(isSetting: boolean = false) {
    ...
    let str =
      extra > 0
        ? +canuse + "/" + total + "(+" + extra + ")"
        : canuse + "/" + total;
    this.skillPoint.text =
      LangManager.Instance.GetTranslation("SkillPanel.skillPointTxt") + str;
    if (!isSetting) {
      this.skillResetBtn.enabled = !StringHelper.isNullOrEmpty(
        this.thane.skillCate.skillScript,
      );
    }
  }
