...
export default class MazeViewWnd extends BaseWindow {
    ...
    public OnInitWind() {
        ...
        if (WorldBossHelper.checkMaze(CampaignManager.Instance.mapModel.mapId)) {
            this.CampaignNameTxt.text = LangManager.Instance.GetTranslation("MazeFrameWnd.BtnGroupName0");
        }
        else {
            this.CampaignNameTxt.text = LangManager.Instance.GetTranslation("MazeFrameWnd.BtnGroupName1");
        }
        this.CurrentLayerDescTxt.text = LangManager.Instance.GetTranslation("map.campaign.view.ui.MazeView.current");
        this.TotalGpDescTxt.text = LangManager.Instance.GetTranslation("map.campaign.view.ui.MazeView.totalExp");
    ...
    private addEvent() {
        ...
        PlayerManager.Instance.addEventListener(PlayerEvent.UPDATE_TOWER_INFO, this.__onTowerChange, this);
    }

    private removeEvent() {
        this.Btn_Fall.offClick(this, this.BtnFallHander.bind(this));
        this.Btn_Shop.offClick(this, this.BtnShopHander.bind(this));
        PlayerManager.Instance.removeEventListener(PlayerEvent.UPDATE_TOWER_INFO, this.__onTowerChange, this);
    }


    private __onTowerChange() {
        this.CurrentLayerValueTxt.text = "" + PlayerManager.Instance.currentPlayerModel.towerInfo.towerIndex;
        this.TotalGpValueTxt.text = "" + PlayerManager.Instance.currentPlayerModel.towerInfo.totalGp;
    }

    private BtnFallHander() {
        let dataArray: Array<GoodsInfo> = new Array();
        let value: string = PlayerManager.Instance.currentPlayerModel.towerInfo.itemTempIds;
        ...
        FrameCtrlManager.Instance.open(EmWindow.DisplayItems, { itemInfos: dataArray, title: LangManager.Instance.GetTranslation("map.campaign.view.frame.MazeFallItemsFrame.title") });
    }
    ...
    private BtnShopHander() {
        ...
        if (WorldBossHelper.checkMaze(mapId)) {
            ...
            FrameCtrlManager.Instance.open(EmWindow.ShopWnd, { page:3, returnToWinFrameData: 1 });
        }
    }
