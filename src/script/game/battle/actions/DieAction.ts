...
export class DieAction extends GameBaseAction {
  ...
  public prepare() {
    ...
    if (
      this._currentRole == BattleManager.Instance.battleModel.selfHero &&
  ...
  public update() {
    ...
    if (this._timer == 0) {
      ...
      if (
        this._isPawn ||
        RoleActionSimplifyData.isSimplify(mc && mc.data && mc.data.urlPath)
      ) {
        //士兵或简化动作的怪, 播放死亡特效
        ...
        this._currentRole.actionMovieClip.addFrameScript(
