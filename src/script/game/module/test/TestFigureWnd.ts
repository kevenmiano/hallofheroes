...
export default class TestFigureWnd extends BaseWindow implements IEnterFrame {
  ...
  testAni() {
    ...
    Laya.timer.once(500, this, () => {
      TestCreateGameTransaction.Instance.initParent(this).startBattle(
        TestBattleData_5_fyundi.BattlePrepareMsg,
      );
    ...
    ResMgr.Instance.loadRes(
      "res/animation/skilleffect/buff_add_patup/buff.add.patup.json",
      (res) => {
        Logger.log("test", res);
        let roleAni = new Laya.Animation();
        Laya.stage.addChild(roleAni);
