...
import { ActionLabesType, BattleType, BloodType, FaceType, InheritRoleType, RoleType } from "../../../constant/BattleDefine";
...
export class BaseRoleView extends DepthSprite {
    ...
    protected initView() {
        ...
        if (this.checkCanshowBuffIcon()) {
            ...
            this._roleInfoUI.addChildWithTag(this._bufferContainer, RoleInfoUIEnum.BufferContainer);
        }

        this._heroNameTxt = new FilterFrameShadowText(200, 30, undefined, 18, undefined, "center", undefined, 0.5);
        this._roleInfoUI.addChildWithTag(this._heroNameTxt, RoleInfoUIEnum.RoleName);
        ...
        if (BattleModel.battleUILoaded) {
            ...
            NotificationManager.Instance.addEventListener(BattleEvent.BATTLE_UI_LOADED, this.onBattleUILoaded, this)
        }
    ...
    public setHeroNameTxtValue(nickName: string, $serverName: string = "") {
        ...
        if (serverName && this.isShowServiceNameBattleType(this.battleModel && this.battleModel.battleType)) {
    ...
    private onBattleMusicOnOff(event: BattleEvent) {
        if (this._info && this._info.actionLaya && this._info.actionLaya.Animation) {
            this._info.actionLaya.Animation.soundTransform = SharedManager.Instance.getSkillSoundTransform();
        }
    }

    protected onBattleUILoaded(event: BattleEvent) {
        NotificationManager.Instance.removeEventListener(BattleEvent.BATTLE_UI_LOADED, this.onBattleUILoaded, this)
        this.delayInitView();
    }
    ...
    protected refreshHeroText() {
        ...
        if (this._info.face == FaceType.RIGHT_TEAM) {
            ...
            let heroTempInfo: t_s_herotemplateData = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_herotemplate, this._info.templateId)
    ...
    protected async initBloodBar() {
        if (!this._stripView) {
            ...
            if (this._info.inheritType == InheritRoleType.Hero && this._info.type == RoleType.T_NPC_BOSS && (this._info.face == FaceType.RIGHT_TEAM || this._info.face == 0)) {
                this._stripComp = await fgui.UIPackage.createObject(EmPackName.Battle, "Prog_BossSmallCon") as fgui.GComponent;
                let realStrp = this._stripComp.getChild("strip").asCom
                let stripNum = BattleModel.getBossHpStripNum(this._info.heroInfo.templateId)
                let asset = []
                if (stripNum <= 1) {
                ...
                this._stripView = new StripUIView(asset, totalBloodA ? totalBloodA : -1, bloodA ? bloodA : -1);
            } else {
                this._stripComp = await fgui.UIPackage.createObject(EmPackName.Battle, this._info.face == FaceType.LEFT_TEAM ? "Prog_HpGreen" : "Prog_HpRed") as fgui.GComponent;
                let img = this._stripComp.getChild("level1").asImage
                this._stripView = new StripUIView([img], totalBloodA ? totalBloodA : -1, bloodA ? bloodA : -1);
            }

        }
        this._roleInfoUI.addChildWithTag(this._stripComp.displayObject, RoleInfoUIEnum.StripUIView);

        Logger.battle("[BaseRoleView]initBloodBar livingId", this._info.livingId, "totalBloodA", this._info.totalBloodA, "bloodA", this._info.bloodA, this._info)
    }
    ...
    private addFateSkillEffect() {
        if (this._info instanceof HeroRoleInfo) {
            ...
            let path: string = PathManager.getFateBattlePath(upgradeTemp.ActiveObject);
    ...
    private fateSkillComplete(info: LoaderInfo, content: Object) {
        ...
        if (this._fateSkillEffect) {
            ObjectUtils.disposeObject(this._fateSkillEffect); this._fateSkillEffect = null;
        }
    ...
    protected addEvent() {
        ...
        this._info.addEventListener(RoleEvent.BLOOD_CHANGE_S, this.__bloodChangedSecurity, this);
        this._info.addEventListener(RoleEvent.DIRECTION_CHANGE, this.__directionChange, this);
        this._info.addEventListener(RoleEvent.EFFECT_PLAY, this.__effectcPlay, this);
        this._info.addEventListener(RoleEvent.SHOW_BUFFER, this.__showBuffer, this);
        this._info.addEventListener(RoleEvent.RESET_BLOOD_SHIELD, this.resetStripShiled, this);
        this._info.addEventListener(RoleEvent.REFRESH_BUFFER, this.__reFreshBuffer, this);
    ...
    private setRoleAllContainerScaleX(value: number) {
        if (this._backEffectContainer && !this._backEffectContainer.destroyed) this._backEffectContainer.scaleX = value;
        if (this._effectContainer && !this._effectContainer.destroyed) this._effectContainer.scaleX = value;
        if (this._movieContainer && !this._movieContainer.destroyed) this._movieContainer.scaleX = value;
    }

    //buffer特效
    private __showBuffer(evtData) {
        let bufferDamageData: BufferDamageData = evtData;
        let bufferTempInfo = TempleteManager.Instance.getSkillBuffTemplateByID(bufferDamageData.templateId);
        if (!bufferTempInfo) {
            return
        }
        ...
        if (lastEffectRes) {
            ...
            if (!finded) {
                lastEffect = new BufferLastEffect(this._info, bufferDamageData, this._effectContainer, lastEffectRes);
                this._bufferLastEffects.push(lastEffect);
                lastEffect.show();
            }

        }
        Logger.battle(`[BaseRoleView]${this._info.roleName}(${this._info.livingId})添加特效buff:${bufferTempInfo.BufferNameLang}(${bufferTempInfo.Id}), buff资源:${effectName}, 持续性buff:${lastEffectRes}`);
    }
        ...
        for (let loseBuffer of bloodloseBuffer) {
            for (let d of loseBuffer.damages) {
                ...
                if (d.bloodType == 24 || d.bloodType == 6) {
                    bloodloseValue += d.damageValue * (loseBuffer.layerCount > 1 ? loseBuffer.layerCount : 1);
                }
            }
        }
        ...
        for (const key in heroList) {
            if (Object.prototype.hasOwnProperty.call(heroList, key)) {
                ...
                if (hero.type == RoleType.T_NPC_BOSS) {
                    if (!boss) boss = hero;
                    if (boss && boss.heroInfo && boss.heroInfo.grades < hero.heroInfo.grades)
                        boss = hero
                }
            }
        }
    ...
    public addBufferActionEffect(bufferId: number) {
        ...
        let bufferTempInfo: t_s_skillbuffertemplateData = TempleteManager.Instance.getSkillBuffTemplateByID(bufferId);
    ...
    protected removeEvent() {
        if (this._info) {
            ...
            this._info.removeEventListener(RoleEvent.POINT_CHANGE, this.__point, this);
            ...
            this._info.removeEventListener(RoleEvent.EFFECT_PLAY, this.__effectcPlay, this);
            this._info.removeEventListener(RoleEvent.SHOW_BUFFER, this.__showBuffer, this);
            this._info.removeEventListener(RoleEvent.FACE_CHANGE, this.__faceChange, this);
            this._info.removeEventListener(RoleEvent.BLOOD_CHANGE, this.__blood, this);
            this._info.removeEventListener(RoleEvent.BLOOD_CHANGE_S, this.__bloodChangedSecurity, this);
            this._info.removeEventListener(RoleEvent.DIRECTION_CHANGE, this.__directionChange, this);
            this._info.removeEventListener(RoleEvent.RESET_BLOOD_SHIELD, this.resetStripShiled, this);
            this._info.removeEventListener(RoleEvent.REFRESH_BUFFER, this.__reFreshBuffer, this);
        }
        NotificationManager.Instance.removeEventListener(BattleEvent.BATTLE_MUSIC_ON_OFF, this.onBattleMusicOnOff, this);
    }
    ...
    protected __die(evt: RoleEvent) {
        ...
        if (battleUI.getRoleShowViewII()) {
            let bossBufferContainer = battleUI.getRoleShowViewII().bossBufferContainer;
    ...
    private action(type: string, repeatType: string = null, nextType: string = ActionLabesType.STAND, waitFrame: number = 5, isStop: boolean = false) {
        this._info.actionMovie.actionAndPlay(type, repeatType, nextType, waitFrame, isStop);
        if (type == ActionLabesType.STAND) {
            this.info.direction = this._info.face == FaceType.RIGHT_TEAM ? "left" : "right"
        }
    }
    ...
    public setRoleInfoViewVisible(value: boolean) {
        ...
        if (!this._info.isLiving) {
            if ((this._info.inheritType == InheritRoleType.Hero) && ((this._info as HeroRoleInfo).type != 3)) {
    ...
    protected __blood(data: any) {
        ...
        if (bloodType == BloodType.BLOOD_TYPE_SELF) {
            mc = new DamageNumView(Math.abs(blood), bloodType, critical, blood > 0, selfCause, parry);
            let boss: HeroRoleInfo = this.battleModel.getWorldBoss()
            if (boss && this.info == boss) {
                BloodHelper.worldBossBloodCached += data.blood
            }
        } else if (bloodType == BloodType.BLOOD_TYPE_THIRD || bloodType == BloodType.BLOOD_TYPE_ARMY) {
            mc = new DamageNumView(blood, bloodType, critical, false, selfCause, parry);
    ...
    protected updateStrip() {
        if (this._stripView) {
            ...
            this._info.dispatchEvent(RoleEvent.HP, this._info.bloodA, this._info.totalBloodA);
        }
    }
    ...
    public addCollectionEffect(showEffect: boolean = true) {
        ...
        if (showEffect && BattleModel.battleDynamicLoaded) {
            this._collectionEffect = new CollectionEffect(this.backEffectContainer, this._effectContainer, this._info.heroInfo.templateInfo.Job);
        }
    ...
    public addSkillFlagByIconPath(iconPath: string) {
        ...
        ObjectUtils.disposeObject(this._skillItemUseCell); this._skillItemUseCell = null;
        ...
        let wnd = FrameCtrlManager.Instance.getCtrl(EmWindow.Battle).view as BattleWnd;
        wnd.addChildAt(this._skillItemUseCell, BattleModel.ZIndex_Bottom);

        // 在出生点位置附近
        let inBornPos = Math.abs(this.info.point.x - this.x) < 50 && Math.abs(this.info.point.y - this.y) < 50
    ...
    private addSkillItemUseCellToBattleWnd() {
        ...
        let globalPos = this._effectContainer.localToGlobal(new Laya.Point(0, 0), false)
        this._skillItemUseCell.setScaleX(fixDir);
        this._skillItemUseCell.x = globalPos.x + BaseRoleView.USE_SKILLITEM_POS.x * fixDir
    ...
    public addSkillAimFlag() {
        ...
        this._skillAimMc = FUIHelper.createFUIInstance(EmWindow.BattleDynamic, aimRes) as fgui.GMovieClip;
        if (this._skillAimMc) {
    ...
    */
    ...
    public get isSelf() {
        return this._info && this.battleModel && this._info == this.battleModel.selfHero;
    }

    public get isSelfConsortiaAndSide() {
        if (this._info && this._info.inheritType == InheritRoleType.Hero && this.battleModel) {
            let selfHero = this.battleModel.selfHero
            if (this._info.consortiaId && (this._info.consortiaId == selfHero.consortiaId)) {
                if (this._info.serverName == selfHero.serverName) {
                    return true
                } else {
                    Logger.battle("[BaseRoleView]同公会不同服务器", this._info.serverName, selfHero.serverName)
                    return false;
                }
            }
        }
    ...
    public dispose() {
        ...
        this._rolePart && ObjectUtils.disposeObject(this._rolePart); this._rolePart = null;
        this._fateSkillEffect && ObjectUtils.disposeObject(this._fateSkillEffect); this._fateSkillEffect = null;
        this._roleInfoUI && ObjectUtils.disposeObject(this._roleInfoUI); this._roleInfoUI = null;
        this._stripView && ObjectUtils.disposeObject(this._stripView); this._stripView = null;
        this._stripComp && ObjectUtils.disposeObject(this._stripComp); this._stripComp = null;
        this._bufferContainer && ObjectUtils.disposeObject(this._bufferContainer); this._bufferContainer = null;
