...
export class PropSkillBattleItem extends BattleSkillItemII {
    ...
    protected __bufferEnabeleHandler() {
        ...
        if (this._data instanceof BattlePropItem) {
            if (!BattleManager.Instance.battleModel.isAllPetPKBattle()) {
                this.bufferUnEnable = this.roleInfo.existUnenableSkillBuffer((<BattlePropItem>this._data).skillTempId);
                if (this.bufferUnEnable) {
                    ...
                    var hasReadySkill: boolean = BattleManager.Instance.battleModel.currentReadySkillId != -1;
                    var inTrialTowerBattle: boolean = BattleManager.Instance.battleModel.battleType == BattleType.TRIAL_TOWER_BATTLE;
    ...
    public set data(value: any) {
        this._data = value;
        if (value) {
            this._data.addEventListener(BattleEvent.PROP_USECOUNT_CHANGE, this.__countChangeHandler, this);
            var skillTemp: t_s_skilltemplateData = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_skilltemplate, this._data.skillTempId.toString())
    ...
    public startCD(delay: number = 0, cd: number = -1) {
        ...
        let coolDown = (this._data.getSkillTemplate() as t_s_skilltemplateData).PropCoolDown;
    ...
    public dispose() {
        if (this._data) {
            this._data.removeEventListener(BattleEvent.PROP_USECOUNT_CHANGE, this.__countChangeHandler, this);
        }
