...
export default class OuterCityWarNoticeWnd extends BaseWindow {
  ...
  private initView() {
    ...
    this.changeBtn.title = LangManager.Instance.GetTranslation(
      "public.modification",
    );
    this.frame.getChild("title").text = LangManager.Instance.GetTranslation(
      "consortia.presidentNotice",
    );
    let userId: number =
    ...
    if (
      temp &&
      (temp.dutyId == ConsortiaDutyLevel.CHAIRMAN ||
        temp.dutyId == ConsortiaDutyLevel.VICE_CHAIRMAN)
    ) {
      ...
      (this.tfInputSuffix.txt_web.displayObject as Laya.Input).mouseEnabled =
        false;
    }
  ...
  private __contentTxtChange() {
    ...
    this.descTxt.text = LangManager.Instance.GetTranslation(
      "OutyardNoticeWnd.descTxt",
      this._maxValue - this.tfInputSuffix.text.length,
    );
  }

  private __guildNotice() {
    this.tfInputSuffix.text =
      OuterCityWarManager.Instance.model.curCastleNoticeStr;
  }
  ...
  private changeBtnHandler() {
    ...
    if (noticeStr.length < 3) {
      MessageTipManager.Instance.show(
        LangManager.Instance.GetTranslation("OutyardNoticeWnd.changeBtnTips"),
      );
      return;
    }
