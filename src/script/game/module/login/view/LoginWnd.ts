...
import { NativeEvent, SelectServerEvent, WebViewEvent } from '../../../constant/event/NotificationEvent';
...
export default class LoginWnd extends BaseWindow {
    ...
    public OnInitWind() {
        ...
        if (Utils.isYYBApp() || Utils.isHUAWEIRY()) {//应用宝默认不勾选
                ...
                if (obj) {
                    ...
                    if (isLoginAfterRestart == "true") {
                        ...
                        if (obj) {
                            ...
                            ModelMgr.Instance.setProperty(EmModel.USER_MODEL, UserModelAttribute.channelId, channelId);
                            this.setPrivacyState()
                            if (!Number.isNaN(platform) && platform > 0 && this.userName && this.userName != '') {
    ...
    private checkPrivacy(): boolean {
        if (Utils.isHUAWEIRY() || Utils.isYYBApp() || Utils.isFromHY() || Utils.isFrom4399() || Utils.isQQHall() || Utils.isOppoApp() || Utils.isQQZone() || Utils.isQQMobile()) {
    ...
    onSwitchAccountHandler() {
        SimpleAlertHelper.Instance.Show(null, null, null, LangManager.Instance.GetTranslation("Login.SwitchAccountTip"), null, null, (b) => {
            if (b) {
                if (Utils.isAndroid()) {
                    this.clearUserData();
                    SDKManager.Instance.getChannel().logout(true);
    ...
    private setPrivacyState() {
        this.privacy.selectedIndex = (Utils.isYYBApp() || Utils.isOppoApp() || Utils.isFromHY() || Utils.isFrom4399() || Utils.isQQHall()) || Utils.isQQZone() || Utils.isQQMobile() || Utils.isHUAWEIRY() ? 1 : 0;
    }
    ...
    requestServerList(platformid: number | string, username?: string, useSite?: boolean) {
        ...
        SDKManager.Instance.getChannel().trackEvent(0, TrackEventNode.Node_1007, "请求区服列表", "请求区服列表", "", "");
            ...
            if (data) {
                try {
                    ...
                    if (content && content.length) {
                        ...
                        SDKManager.Instance.getChannel().trackEvent(0, TrackEventNode.Node_1008, "请求区服列表成功", "请求区服列表成功", "", "");
    ...
    onSelectServer() {
        if (this.serverListData && this.serverListData.ret >= 0) {
            SDKManager.Instance.getChannel().trackEvent(0, TrackEventNode.Node_1009, "点击选择区服列表", "点击选择区服列表", "", "");
    ...
    onChangeServer(serverData: ServerSite) {
        if (serverData) {
            SDKManager.Instance.getChannel().trackEvent(0, TrackEventNode.Node_1010, "选择确认区服", "选择确认区服", "", "");
    ...
    setServerInfo(serverData: ServerSite) {
        if (serverData) {
            ...
            //存储当前选择角色信息
            ModelMgr.Instance.setProperty(EmModel.USER_MODEL, UserModelAttribute.userId, serverData.playinfo);
    ...
    OnShowWind() {
        super.OnShowWind();
        SDKManager.Instance.getChannel().trackEvent(0, TrackEventNode.Node_1006, "进入登录界面", "进入登录界面", "", "");
    ...
    onEnterGame() {
        ...
        SDKManager.Instance.getChannel().trackEvent(0, TrackEventNode.Node_1011, "进入游戏", "点击登陆界面的进入游戏按钮（包括不成功）", "", "");
