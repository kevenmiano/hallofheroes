...
export default class SeminaryItem extends FUI_SeminaryItem {
    ...
    private onUpgrade() {
        if (BuildingManager.Instance.checkTecList(this._vdata)) {
            ...
            if (this._vdata.buildingId != 0) {//升级科技
                isMaxLevel = BuildingManager.Instance.model.isMaxLevel(this._vdata.buildingId);
                if (isMaxLevel) {
                ...
                BuildingManager.Instance.creatBuilding(this.getTemp(this._vdata).SonType);
            }
        }
    }
    ...
    private refreshView() {
        ...
        if (!this._nTemp) {
            ...
            let time: number = this.playerEffect.getBuildTimeAdditionValue(this._nTemp.UpgradeTime);
            let grade = VIPManager.Instance.model.vipInfo.VipGrade;
            let param1 = VIPManager.Instance.model.getPrivilegeParam1ByGrade(VipPrivilegeType.TECHNOLOGY_COOLDOWN, grade);
    ...
    private getNextTemp(temp: t_s_buildingtemplateData): t_s_buildingtemplateData {
