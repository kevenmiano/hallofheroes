...
export default class BufferLastEffect {
  ...
  public show() {
    ...
    var bufferTempInfo: t_s_skillbuffertemplateData =
      TempleteManager.Instance.getSkillBuffTemplateByID(
        this._bufferDamageData.templateId,
      );
    ...
    ResMgr.Instance.loadRes(fullUrl, (res) => {
      let cacheObj = BattleManager.Instance.resourceModel.getEffectMC(
        this._effectName,
      );
      if (cacheObj) {
        ...
        let basePt: Laya.Point = this._roleInfo.getSpecialPos(
  ...
  public dispose() {
    ...
    ResMgr.Instance.releaseRes(
      PathManager.solveSkillResPath(this._effectName, true, true),
    );
