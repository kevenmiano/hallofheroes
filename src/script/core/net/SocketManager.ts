 ...
 * 网关Socket
 */
export class SocketManager extends GameEventDispatcher implements IManager {
    ...
    public regEvent() {
        ...
        NotificationManager.Instance.on(SocketEvent.SERVER_DATA, self.onSocketDataRecord, self);
    }
    ...
    public onClose(evt) {
        ...
        SDKManager.Instance.getChannel().nativeLog("socketManager:onClose", "网关服务器已关闭");
    ...
    private onError(evt) {
        ...
        SDKManager.Instance.getChannel().nativeLog("socketManager:onError", "连接网关服务器失败");
    ...
    private onSuccess() {
        ...
        SDKManager.Instance.getChannel().nativeLog("socketManager:onSuccess", "连接网关服务器成功");
        //开启弱网检测
        this._socket.createWeakNet().model.on(WeakNetCheckModel.WeakNetClose, this.onClose, this);
    ...
    private onSocketData(pkg: PackageIn) {
        SDKManager.Instance.getChannel().nativeLog("socketManager:onSocketData:", "" + pkg.code);
    ...
    public close() {
        ...
        this._socket.createWeakNet().model.off(WeakNetCheckModel.WeakNetClose, this.onClose, this);
        if (socket) {
        ...
        this._socket = null;
        SDKManager.Instance.getChannel().nativeLog("socketManager:close", "关闭socket");
        NotificationManager.Instance.off(SocketEvent.SERVER_DATA, this.onSocketDataRecord, this);

    }

    public tryReconnect() {
        FrameCtrlManager.Instance.exit(EmWindow.Waiting);
        UIHelper.closeWindows([EmWindow.SpaceTaskInfoWnd, EmWindow.Home, EmWindow.LevelUp, EmWindow.LogWnd, EmWindow.AutoWalkWnd, EmWindow.RoomHall, EmWindow.PveSecretSceneWnd]);
        ...
        if (this.isMaxReconnected()) {
            SDKManager.Instance.getChannel().nativeLog("socketManager:tryReconnect", "已达到最大重连次数");
    ...
    public tick() {
        this.showReconnectHint(this.reconnectTimeCoolDown - this.reconnectTimeCounter, this.reconnectCounter);
        this.reconnectTimeCounter++;
        SDKManager.Instance.getChannel().nativeLog("socketManager:tryReconnect", "重新链接:+" + this.reconnectCounter);
    ...
    public resetReconnection() {
        SDKManager.Instance.getChannel().nativeLog("socketManager:resetReconnection", "重置链接");
    ...
    public async showReconnectHint(cd: number, recounter: number) {
        ...
        if (!this.reConnetAlert) {
            ...
            this.reConnetAlert = await UIManager.Instance.ShowWind(EmWindow.HintWnd, LangManager.Instance.GetTranslation("reconnection.tip", recounter + "/" + this.reconnectMacCount));
            //这里需要重新判断一次, 是否已经连接成功, 或者已经达到最大重连数了, 隐藏。
            if (this.isConnected() || this.isMaxReconnected()) this.hideReconnectHint();
        } else {
            this.reConnetAlert.visible = true;
            this.reConnetAlert.active = true;
            this.reConnetAlert.setHintText(LangManager.Instance.GetTranslation("reconnection.tip", recounter + "/" + this.reconnectMacCount));
        }
    }
