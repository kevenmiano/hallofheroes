...
export class Avatar extends Laya.Sprite implements Disposeable {
  ...
  public set showTranslucenceView(v: boolean) {
    ...
    if (!this._translucenceView) {
      this._translucenceView = FUIHelper.createFUIInstance(
        EmPackName.BaseCommon,
        "AvatarTranslucenceMC",
      ) as fgui.GMovieClip;
  ...
  public static get oldMountId(): any[] {
    var ret: any[] = [
      8138, 8098, 8175, 8144, 8132, 8169, 8187, 8168, 8183, 8167, 8188, 8114,
      8270, 8329, 8503, 8399, 8516, 8371,
    ];
    var con: ConfigInfosTempInfo =
      TempleteManager.Instance.getConfigInfoByConfigName("config_9mount_ids");
  ...
  protected static get SizeType_NEW10(): any[] {
    var ret: any[] = [8316];
    var con: ConfigInfosTempInfo =
      TempleteManager.Instance.getConfigInfoByConfigName("config_10mount_ids");
    if (con && con.ConfigValue) {
      ret = con.ConfigValue.split(",");
      for (var i: number = 0; i < ret.length; i++) {
        ret[i] = Number(ret[i]);
  ...
  public get sizeInfo() {
    ...
    if (obj) {
      ...
      //例: 外城野怪
      tmp = {
        x: this._drawRect.width / 2,
        y: this._drawRect.height,
