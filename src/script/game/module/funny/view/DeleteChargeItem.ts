...
export default class DeleteChargeItem extends FUI_DeleteChargeItem{
    ...
    private getRewardBtnHandler() {
        if (this._showData) {
            if(this._showData.endTime < PlayerManager.Instance.currentPlayerModel.nowDate){
    ...
    private refreshView() {
        if (this._info) {
            this.dayValueTxt.text = LangManager.Instance.GetTranslation("consortia.view.myConsortia.ConsortiaMemberItem.dayTip", this._info.conditionList[0].value);
            if(this._info.conditionList[0].value == 1 || this._info.conditionList[0].value == 2 ||this._info.conditionList[0].value==3){
                this["backPrecentTxt"+this._info.conditionList[0].value].text = this._info.conditionList[0].bak + " %";
