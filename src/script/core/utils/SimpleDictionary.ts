...
export class SimpleDictionary extends Dictionary {
  ...
  private setValue(key: any, data: object) {
    if (this[key]) {
      this.__dispatcher.dispatchEvent(DictionaryEvent.PREUPDATE, [
        data,
        this[key],
        key,
      ]); //参数为newData,oldData,key
      this.updateInArray(this[key], data);
      this[key] = data;
      this.__dispatcher.dispatchEvent(DictionaryEvent.UPDATE, [
        data,
        this[key],
        key,
      ]); //参数为newData,oldData,key
    } else {
      this.__dispatcher.dispatchEvent(DictionaryEvent.PREADD, [
        data,
        this[key],
        key,
      ]); //参数为newData,oldData,key
      ...
      this[key] = data;
      this.__dispatcher.dispatchEvent(DictionaryEvent.ADD, [
        data,
        this[key],
        key,
      ]); //参数为newData,oldData,key
  ...
  private delValue(key: any) {
    if (this[key]) {
      ...
      this.__dispatcher.dispatchEvent(DictionaryEvent.PREDELETE, [
        null,
        old,
        key,
      ]); //参数为newData,oldData,key
