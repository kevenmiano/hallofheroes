...
export class PetEquipPartCom extends FUI_PetEquipPartCom {
  ...
  private initView() {
    for (let j = 0; j < 6; j++) {
      ...
      (cell.getChild("part_icon") as fgui.GLoader).url =
        fgui.UIPackage.getItemURL(EmWindow.Pet, this.strArr[j]);
    }
    this.add_attri_tip.getChild("txt1").text =
      LangManager.Instance.GetTranslation("petEuip.tip01");
    this.btn_suit_attri.title =
      LangManager.Instance.GetTranslation("petEuip.tip02");
  }
  ...
  private clearAttr() {
    this.addAttriNum = 0;
    for (let i = 1; i <= 10; i++) {
      let attri_item = this.add_attri_tip.getChild(
        "attri_item" + i,
      ) as GoodAttributeItem;
      if (attri_item) {
  ...
  private updateAttr() {
    ...
    this.clearAttr();
    this.equipArr = GoodsManager.Instance.getGoodsByBagType(
    ...
    for (const attId in this.attriMap) {
      if (this.attriMap.hasOwnProperty(attId)) {
        ...
        let cfg: t_s_attributeData = TempleteManager.Instance.getPetEquipAttri(
          Number(attId),
        );
        if (cfg) {
          i++;
          ...
          if (attri_item) {
            attri_item.getChild("txt_attributeName").text =
  ...
  checkActiveSuit() {
    ...
    for (let i = 0; i < this.curPetSuitIdArr.length; i++) {
      ...
      let cfg: t_s_petequipsuitData =
        TempleteManager.Instance.getPetEquipSuitData(gInfo.suitId);
    ...
    for (const suitId in this.suitMap) {
      if (this.suitMap.hasOwnProperty(suitId)) {
        ...
        let cfg: t_s_petequipsuitData =
          TempleteManager.Instance.getPetEquipSuitData(Number(suitId));
        if (cfg) {
          if (val >= cfg.Amount) {
            //要考虑到多套相同的套装
  ...
  private showSuitTip(cfg: t_s_petequipsuitData) {
    if (!this.suit_tip.visible && cfg) {
      ...
      this.suit_tip.getChild("txt1").text = LangManager.Instance.GetTranslation(
        "petEuip.tip04",
        cfg.Amount,
      );
      this.suit_tip.getChild("descTxt").text = cfg.DescriptionLang;
      this.suit_tip.getChild("img_loader").asLoader.url =
        IconFactory.getCommonIconPath1(cfg.SuitIcon);
