...
export class PetChallengeAdjustWnd extends BaseWindow {
  ...
  private initPlayerPet() {
    this.playerPetDatas = ArrayUtils.sortOn(
  ...
  private onPetChallengeDragStart(evt: Laya.Event) {
    let btn: PetChallengeAdjustFormationItem = <
  ...
  private onPetDrop(
    otherView: PetChallengeAdjustFormationItem,
    evt: Laya.Event,
  ) {
  ...
  private onPetSkillDragStart(evt: Laya.Event) {
    let btn: PetChallengeAdjustSkillItem = <PetChallengeAdjustSkillItem>(
      fgui.GObject.cast(evt.currentTarget)
    );
    btn.stopDrag();
    if (!btn.info) return;
  ...
  private initSkillDataTemp() {
    if (this.curPet) {
      this.skillDataTempArr =
        ShopManager.Instance.model.petSkill[this.curPet.template.PetType];
      if (this.skillDataTempArr.length) {
        this.skillDataTempArr.sort((skillTempA, skillTempB) => {
          let a1 = skillTempA.UseWay == 2 ? 0 : 100;
          let a2 = this.curPet.checkSkillIsLearned(skillTempA.TemplateId)
            ? 10
            : 0;
          let sumA = a1 + a2;

          let b1 = skillTempB.UseWay == 2 ? 0 : 100;
          let b2 = this.curPet.checkSkillIsLearned(skillTempB.TemplateId)
            ? 10
            : 0;
          let sumB = b1 + b2;
          return sumB - sumA;
  ...
  private updateFastSkill() {
    ...
    for (let i = 0; i < this.petSkillViews.length; i++) {
      ...
      FUIHelper.setTipData(
        this.petSkillViews[i],
        EmWindow.PetSkillTips,
        petChallengeSkillList[i],
        null,
        null,
        this.curPet,
      );
  ...
  private formatPetSkill() {
    ...
    for (let index = 0; index < this.petSkillViews.length; index++) {
      ...
      if (item && item.info) {
        PetData.FastSkillKeyTemp +=
          this.curPet.changeSkillTemplates.indexOf(item.info) + ",";
      } else {
  ...
  public inChallengeTeam(petData: PetData) {
    ...
    return (
      this.control.data.formationString.indexOf(petData.petId.toString()) != -1
    );
  }
