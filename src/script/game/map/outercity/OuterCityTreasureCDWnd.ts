...
export default class OuterCityTreasureCDWnd extends BaseWindow {
  ...
  private initData() {
    this._leftMins =
      ...
      PlayerManager.Instance.currentPlayerModel.sysCurTimeBySecond; //CD剩余的秒数
    this._cdCostArray =
      ConfigInfoManager.Instance.getTreasureCdAccelerateSpend();
    this._maxSpeedNumber = this._cdCostArray.length;
    this._maxValue = ConfigInfoManager.Instance.getTreasureCd() * 60;
    if (this._leftMins > 0) {
      if (this.playerModel.isDeath) {
        //曾经达到过阈值
        this.timeTxt.color = "#FF0000"; //红色显示
        this.speedBtn.visible =
          this.playerModel.skipCount < this._maxSpeedNumber ? true : false;
      } else {
