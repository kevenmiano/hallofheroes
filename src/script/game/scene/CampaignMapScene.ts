...
import { CampaignMapEvent, NotificationEvent, PK_Event, SceneEvent, SLGSocketEvent } from "../constant/event/NotificationEvent";
...
export class CampaignMapScene extends BaseSceneView {
        ...
        return new Promise(async resolve => {
            ...
            if (!HomeWnd.Instance.isShowing) {
                if (!ismonopoly) {
                    ...
                    HomeWnd.Instance.getSmallMapBar().switchSmallMapState(SmallMapBar.CAMPAIGN_SMALL_MAP_STATE);
                }
            }
            if (!ismonopoly) {
            ...
            if (UIManager.Instance.isShowSpaceTaskInfoWndScene && !UIManager.Instance.isShowing(EmWindow.SpaceTaskInfoWnd)) {
            ...
            if (WorldBossHelper.checkSecretFb(this._mapModel.mapId)) {
                if (this._preSceneData && this._preSceneData.isOpenSecretSuccess) {
                    FrameCtrlManager.Instance.open(EmWindow.PveSecretSceneWnd, { battleReportMsg: this._preSceneData.data });
                } else {
                    FrameCtrlManager.Instance.open(EmWindow.PveSecretSceneWnd);
                }
            } else if (WorldBossHelper.checkPetLand(this._mapModel.mapId) || WorldBossHelper.checkMineral(this._mapModel.mapId)) {
                // //英灵岛, 紫晶矿场 农作物收获提示
            ...
            if (this._mapModel.campaignTemplate) {
                ...
                let currentTem: t_s_campaignData = ConfigMgr.Instance.campaignTemplateDic[campaignId];
                ...
                if (currentTem && currentTem.Capacity == 1 && currentTem.CampaignId != 1
                    && !FrameCtrlManager.Instance.isOpen(EmWindow.AutoWalkWnd))//单人本自动寻路
                {
    ...
    private gotoOutCampaign(): boolean {
        if (CampaignManager.Instance.exit) {
            this._gotoCastleTimeId = setInterval(this.gotoCastleScene.bind(this), 1000);//loading时被人踢了
            return true;
        }
        return false;
    }
    ...
    private searchPathSyc(arr: any[], para: Object) {
        let str: string = LangManager.Instance.GetTranslation("map.campaign.CampaignMapController.command01");
        MessageTipManager.Instance.show(str);
    }

    private saveBitmap() {
        if (!this._mapModel || !this._mapModel.mapTempInfo || !this._view || !this._view.staticRenderLayer) return;
     ...
     * *************************************************/
    private addEvent() {
        NotificationManager.Instance.addEventListener(S2CProtocol.U_C_CAMERA_MOVE.toString(), this.__campaignSceneMoveHandler, this);
        NotificationManager.Instance.addEventListener(SLGSocketEvent.U_PLAY_MOVIE, this.__playNodeMovieHandler, this);
        this._mapModel.addEventListener(CampaignMapEvent.ATTACK_BOSS_TEMA, this.__refreshTeamHandler, this);
        this._mapModel.addEventListener(CampaignMapEvent.ATTACK_BOSS_INVITE, this.__showInviteHandler, this);
        ServerDataManager.listen(S2CProtocol.U_C_PLAY_GETMOIVE, this, this.__playGetMovieHandler);
        ServerDataManager.listen(S2CProtocol.U_C_WARMOVIE, this, this.__warMovieHandler);
        ServerDataManager.listen(S2CProtocol.U_C_PLAYER_MOVE, this, this.__playerMoveHandler);
        ServerDataManager.listen(S2CProtocol.U_C_CAMPAIGN_CONFIRM, this, this.__confirmFrameHandler);
        ServerDataManager.listen(S2CProtocol.U_C_TAKEDROP_FAIL, this, this.__resetFallHandler);
        ServerDataManager.listen(S2CProtocol.U_C_NPC_CHAT, this, this.__npcChatHandler);
        ServerDataManager.listen(S2CProtocol.U_C_NPC_CHASE_ARMY, this, this.__npcChaseArmyHandler);
        ServerDataManager.listen(S2CProtocol.U_C_HANGUP_PVP, this, this.__onPKRequestHandler);
        ServerDataManager.listen(S2CProtocol.U_C_PLAYER_NODE_STATE, this, this.__playerTranseferStateHandler);
        //节点
        NotificationManager.Instance.addEventListener(NotificationEvent.UN_LOCK_NODE, this.__unlockNodeHandler, this);
        NotificationManager.Instance.addEventListener(NotificationEvent.SELF_HELP_RESET_POSITION, this.__resetPositionHandler, this);
    }

    private removeEvent() {
        NotificationManager.Instance.removeEventListener(S2CProtocol.U_C_CAMERA_MOVE.toString(), this.__campaignSceneMoveHandler, this);
        NotificationManager.Instance.removeEventListener(SLGSocketEvent.U_PLAY_MOVIE, this.__playNodeMovieHandler, this);
        if (this._mapModel) {
            this._mapModel.removeEventListener(CampaignMapEvent.ATTACK_BOSS_TEMA, this.__refreshTeamHandler, this);
            this._mapModel.removeEventListener(CampaignMapEvent.ATTACK_BOSS_INVITE, this.__showInviteHandler, this);
        }
        ServerDataManager.cancel(S2CProtocol.U_C_PLAY_GETMOIVE, this, this.__playGetMovieHandler);
        ServerDataManager.cancel(S2CProtocol.U_C_WARMOVIE, this, this.__warMovieHandler);
        ServerDataManager.cancel(S2CProtocol.U_C_PLAYER_MOVE, this, this.__playerMoveHandler);
        ServerDataManager.cancel(S2CProtocol.U_C_CAMPAIGN_CONFIRM, this, this.__confirmFrameHandler);
        ServerDataManager.cancel(S2CProtocol.U_C_TAKEDROP_FAIL, this, this.__resetFallHandler);
        ServerDataManager.cancel(S2CProtocol.U_C_NPC_CHAT, this, this.__npcChatHandler);
        ServerDataManager.cancel(S2CProtocol.U_C_NPC_CHASE_ARMY, this, this.__npcChaseArmyHandler);
        ServerDataManager.cancel(S2CProtocol.U_C_HANGUP_PVP, this, this.__onPKRequestHandler);
        ServerDataManager.cancel(S2CProtocol.U_C_PLAYER_NODE_STATE, this, this.__playerTranseferStateHandler);
        NotificationManager.Instance.removeEventListener(NotificationEvent.UN_LOCK_NODE, this.__unlockNodeHandler, this);
        NotificationManager.Instance.removeEventListener(NotificationEvent.SELF_HELP_RESET_POSITION, this.__resetPositionHandler, this);
    }
    ...
    private __playerTranseferStateHandler(pkg: PackageIn) {
        let msg: PlayerNodeStateMsg = pkg.readBody(PlayerNodeStateMsg) as PlayerNodeStateMsg;
    ...
    private __confirmFrameHandler(pkg: PackageIn) {
        ...
        if (type == 0) {
            UIManager.Instance.ShowWind(EmWindow.IconAlertHelperWnd, { data: [this.__callServerSccess, mapId, nodeId], content: [title, str, itemTempId, itemCount] });
        } else if (type == 2) {
            FrameCtrlManager.Instance.open(EmWindow.RvrBattleGetResourceWnd, { mapId: mapId, nodeId: nodeId });
        } else if (type == 3) {
            FrameCtrlManager.Instance.open(EmWindow.TrailDialog, { title: title, content: str, mapId: mapId, nodeId: nodeId, callBack: this.__callServerSccess.bind(this) })
        } else if (type == 4) {
            // 英灵岛 神秘人不触发对话
            if (nodeId == GlobalConfig.CampaignNodeID.Node_2000149) {
                if (TaskManage.Instance.cate.hasTaskAndNotCompleted(TaskManage.PET_SYSTEM_OPEN_TASK02)) {
            ...
            FrameCtrlManager.Instance.open(EmWindow.PetLandDialogWnd, { title: title, content: str, mapId: mapId, nodeId: nodeId, node: node });
        } else if (type == 5) {
            let node1: CampaignNode = this._mapModel.getMapNodeByNodeId(nodeId);
            if (node1.nodeId == CampaignMapModel.MINERA_GET_CAR_NODEID) {//领取矿车
                if (node1.nodeId == CampaignMapModel.MINERA_HAND_CAR_NODEID
                    && this.selfCarInfo.minerals < 200 && this.selfCarInfo.pick_count < 5
                    && this.selfCarInfo.is_own == 1 && SharedManager.Instance.checkIsExpired(SharedManager.Instance.handInMineralDate)) {
                    let content: string = LangManager.Instance.GetTranslation("map.campaign.view.frame.MineralDialogFrame.handInAlert");
                    UIManager.Instance.ShowWind(EmWindow.TodayNotAlert, { state: 2, content: content, backFunction: this.handInAlertBack.bind(this), dataArray: [mapId] });
                } else {
            ...
            UIManager.Instance.ShowWind(EmWindow.IconAlertHelperWnd, { data: [this.__mineralConfirmCall, mapId, nodeId], content: [title, str, itemTempId, itemCount] });
        } else if (type == 10) {//王者之塔
            FrameCtrlManager.Instance.open(EmWindow.TrailDialog, { title: title, content: str, mapId: mapId, nodeId: nodeId, callBack: this.__callServerSccess.bind(this) })
        } else if (type == 16) {//公会BOSS
    ...
    protected handInAlertBack(notAlert: boolean, data: Array<any>) {
        ...
        MapSocketOuterManager.sendFrameCallBack(data[0], CampaignMapModel.MINERA_GET_CAR_NODEID, true);
    }

    private __mineralConfirmCall(isSccess: boolean, mapId: number, nodeId: number) {
        if (isSccess) SocketSendManager.Instance.sendSessionOverToBattle(mapId, nodeId, CollectionAction.LEAVE);
    }
    ...
    private __playerMoveHandler(pkg: PackageIn) {
        SocketSceneBufferManager.Instance.addPkgToBuffer(pkg, BattleManager.preScene, this.createPlayerMoveAction.bind(this));
    }
    ...
    private __warMovieHandler(pkg: PackageIn) {
        SocketSceneBufferManager.Instance.addPkgToBuffer(pkg, SceneType.CAMPAIGN_MAP_SCENE, this.playerMovie.bind(this));
    }
    ...
    private __playGetMovieHandler(pkg: PackageIn) {
        SocketSceneBufferManager.Instance.addPkgToBuffer(pkg, SceneType.CAMPAIGN_MAP_SCENE, this.playGetMovie.bind(this));
    }

    private playGetMovie(pkg: PackageIn) {
        // Logger.xjy("[CampaignMapScene]playGetMovie", pkg, this._mapModel)
        if (this._mapModel && pkg) {
            let msg: FightOverMoiveMsg = pkg.readBody(FightOverMoiveMsg) as FightOverMoiveMsg;
            let armyInfo: CampaignArmy = this._mapModel.selfMemberData;
            if (armyInfo) GameBaseQueueManager.Instance.addAction(new BattleGetMoviesAction(armyInfo, msg));
        }
    }

    private exitSubmitBack(b: boolean) {
        if (b) {
            CampaignSocketOutManager.Instance.sendExitCampaignScene(this.currentArmyId);
        }
    }
    ...
    private __campaignSceneMoveHandler(pkg: PackageIn) {
        let msg: PlayerMoveCameraMsg = pkg.readBody(PlayerMoveCameraMsg) as PlayerMoveCameraMsg;
        GameBaseQueueManager.Instance.addAction(new SceneMoveAction(msg.targetX, msg.targetY), true);
        msg = null;
        pkg = null;
    }
    ...
    private __playNodeMovieHandler(pkg: PackageIn) {
        ...
        if (msg.movieTargetType == MovieTargetType.NODE) {
            ...
            let caInfo: CampaignArmy = this._mapModel.getBaseArmyByArmyId(msg.targetId);
            let armyView: Object = CampaignManager.Instance.controller.getArmyView(caInfo);
        ...
        CampaignActionsFactory.createAction(msg.movieType, node, pkg, delay, msg.targetId);
    }
    ...
    private initRectScan() {
        this._rectScanUtils.outWidth = this._mapModel.mapTempInfo.Width / Tiles.WIDTH;
        this._rectScanUtils.outHeight = this._mapModel.mapTempInfo.Height / Tiles.HEIGHT;
     ...
     *
     */
    public moveArmyByPos(endX: number, endY: number, isCheckRect: boolean = false, isAttack: boolean = false): boolean {//移动到某个点
        ...
        if (start.x < 0 || start.x > this._mapModel.mapTempInfo.Width || start.y < 0 || this._mapModel.mapTempInfo.Height < start.y) {
            return false;
        }
        if (isCheckRect) {
            this.initRectScan();
        ...
        if (end) {
            ...
            let startPos: Laya.Point = new Laya.Point(parseInt((start.x / 20).toString()), parseInt((start.y / 20).toString()));
            let endPos: Laya.Point = new Laya.Point(parseInt((end.x / 20).toString()), parseInt((end.y / 20).toString()));
            ...
            if (!arr || arr.length <= 0) {
                if (WorldBossHelper.checkPetLand(this._mapModel.mapId) ||
                    ...
                    if (CampaignArmyViewHelper.selfArmyEvent(army.userId, armyView.aiInfo, armyView, new Laya.Point(parseInt((start.x / 20).toString()), parseInt((start.y / 20).toString())), true)) {
                        return false;
                    }
                } else if (WorldBossHelper.single(this._mapModel.mapId)) {//单人副本里面没有路径的时候
                    let campaignNodeArr: Array<CampaignNode> = CampaignManager.Instance.mapModel.getNearTransportNode();//所有的传送点
                    ...
                    Logger.warn("[CampaignMapScene]点击物体, 无移动路径点, 没执行军队到达, 若需要在上面加条件", this._mapModel.mapId)
                }
            }

            if (distance > 2) {
                if (!this.checkWorldBossAndPlayerDie(army)) {
                    if (CampaignArmyViewHelper.selfArmyEvent(army.userId, armyView.aiInfo, armyView, new Laya.Point(parseInt((start.x / 20).toString()), parseInt((start.y / 20).toString())))) {
                        return false;
                    }
                }
            }
    ...
    private checkWorldBossAndPlayerDie(armyData: CampaignArmy): boolean {
        if (WorldBossHelper.checkWorldBoss(this._mapModel.mapId) &&
     ...
     *
     */
    public sendCampaignArrive(armyId: number, nodeId: number, protocolId: number = 0) {
        if (!this._bSendCampaignArriveCool) {
            ...
            Logger.base("🚩军队到达某个节点,向服务器发送消息 armyId-->" + armyId + " nodeId-->" + nodeId + " protocolId-->" + protocolId);
            MapSocketOuterManager.sendCampaignArrive(armyId, nodeId, true, protocolId);
        } else {
            Logger.base("🚩重复发送 军队到达某个节点armyId-->" + armyId + " nodeId-->" + nodeId)
        }
    }
