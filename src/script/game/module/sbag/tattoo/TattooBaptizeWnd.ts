...
export class TattooBaptizeWnd extends BaseWindow
{
    ...
    public refresh():void
    {
        ...
        if(hole.oldAddProperty >= 0)
        {
            ...
            this.txt_oldAddProperty.text = addPropertyStr + "  +" + hole.oldAddingValue;
            this.txt_oldReduceProperty.text = reducePropertyStr + "  " + hole.oldReduceValue;
        ...
        if(hole.newAddProperty >= 0)
        {
            ...
            this.txt_newAddProperty.text = addPropertyStr + "  +" + hole.newAddingValue;
            this.txt_newReduceProperty.text = reducePropertyStr + "  " + hole.newReduceValue;
        ...
        if(nextLv > 0)
        {
            ...
            //{step=2}阶龙纹核心
            // 龙纹属性≧{value=2000}
            this.txt_condition.setVar("step", StringUtils.getRomanNumber(this.tattooModel.coreStep + 1))
                .setVar("value", this.tattooModel.getProtertyValueMaxByStep(hole.oldStep).toString()).flushVars();
        }
        else
        {
    ...
    private showLockAlert():void
    {
        ...
        {
            let tip:string = LangManager.Instance.GetTranslation("TattooPopFrame.lockCostPrompt");//洗炼时锁定的属性会保留, 但每次洗炼会额外消耗钻石, 确定锁定？
            ...
            SimpleAlertHelper.Instance.Show(SimpleAlertHelper.USEBINDPOINT_ALERT, null, prompt, tip, confirm, cancel, this.showLockAlertBack.bind(this));
        }
        else
        {
    ...
    private showLockAlertBack(b:boolean, payType:boolean):void
    {
        if(b)
        {
            ...
            let clickedBtn:fgui.GButton = (this.clickIndex == 1 ? this.lock_old_0 : this.lock_old_1);
    ...
    private switchState():void
    {
        ...
        {
            (this.holes[this._index] as TattooHole).isLockAdd = !(this.holes[this._index] as TattooHole).isLockAdd;
            ...
            if(isLockAdd){
                ...
                if(arr){
                    let str = '[color=#5DAF2C]'+arr[0]+'[/color]' + '&nbsp;[color=#FFECC6]'+arr[1]+'[/color]'
                    this.txt_oldAddProperty.text = str;
                }
            }else{
                
                let addPropertyStr = LangManager.Instance.GetTranslation("tattoo.TattooPopFrame.propertyName" + hole.oldAddProperty);
                this.txt_oldAddProperty.text = addPropertyStr + "  +" + hole.oldAddingValue;
            }
        ...
        {
            (this.holes[this._index] as TattooHole).isLockReduce = !(this.holes[this._index] as TattooHole).isLockReduce;
            let isLockReduce:boolean = (this.holes[this._index] as TattooHole).isLockReduce;
            ...
            if(isLockReduce){
                ...
                let reducePropertyStr = LangManager.Instance.GetTranslation("tattoo.TattooPopFrame.propertyName" + hole.oldReduceProperty);
                this.txt_oldReduceProperty.text = reducePropertyStr + "  " + hole.oldReduceValue;
            }
    ...
    private refreshPoint():void
    {
        ...
        this.txt_material_1.text = this.lockNeedPoints + this.autoBuyNeedPoints + "";
        ...
        if(!autoBuy)
        {
            ...
            this.txt_material_0.color = hasNum >= this.tattooModel.tattooConsumeNum ? '#FFECC6' : '#FF0000';
    ...
    private __onTattooBtnClick():void
    {
        ...
        this.tattooType = 1;
        let hole:TattooHole = this.holes[this._index];
        if(this._oldAddValue >= this.tattooModel.getProtertyValueMaxByStep(hole.oldStep))
        {
            let content2000:string = LangManager.Instance.GetTranslation("tattoo.TattooPopFrame.tattoo");
            UIManager.Instance.ShowWind(EmWindow.TodayNotAlert, {content:content2000, backFunction:this.checkAvailableReplace.bind(this), closeFunction:null, state:2});
        }
        else
        {
            this.checkAvailableReplace();
    ...
    private checkAvailableReplace(check:boolean = false):void
    {
        ...
        {
            let content:string = LangManager.Instance.GetTranslation("tattoo.TattooPopFrame.existsAvailableReplace02");//新龙纹未替换, 是否继续洗炼？
            UIManager.Instance.ShowWind(EmWindow.TodayNotAlert, {content:content, backFunction:this.tattooBtnClickCallback.bind(this), closeFunction:null, state:2});
        }
        else
        {
            this.tattooBtnClickCallback();
    ...
    private hasEnoughPoints(type:number):boolean
    {
        ...
        {
            needGiftToken += (type == 1 ? this.autoBuyNeedPoints : this.autoBuyNeedPoints2);
        }
        else
        {
        ...
        if(this.playerInfo.point + this.playerInfo.giftToken < needPoint + needGiftToken)
        {
            return false;
        }
        return true;
    ...
    protected OnBtnClose()
    {
        ...
        if(hole.newAddProperty >= 0)
        {
            ...
            let cancel: string = LangManager.Instance.GetTranslation("public.cancel");
            let prompt: string = LangManager.Instance.GetTranslation("public.prompt");
            let content: string = LangManager.Instance.GetTranslation("tattoo.view.close.tips02");
            SimpleAlertHelper.Instance.Show(SimpleAlertHelper.SIMPLE_ALERT, null, prompt, content, confirm, cancel, (b:boolean)=>{
                if(b)
                {
                    ...
                    Laya.timer.callLater(this, ()=>{this.hide();});
                }
            });
        }
        else
