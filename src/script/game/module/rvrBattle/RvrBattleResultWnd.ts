...
export default class RvrBattleResultWnd extends BaseWindow {
  ...
  public OnHideWind() {
    ...
    if (
      ...
      PlayerManager.Instance.scoreRatingCondition ==
        StoreRatingsType.FIRST_RVR_MVP
    ) {
  ...
  private updateListHandler() {
    ...
    var data: Array<WarFightOrderInfo> =
      CampaignManager.Instance.pvpWarFightModel.warFightOrderList;
    if (!data || data.length === 0) return;
    data = ArrayUtils.sortOn(
      data,
      ["order"],
  ...
  private refreshList(arr: Array<WarFightOrderInfo>) {
    ...
    this.findMvp(arr);
    this._dataArray = ArrayUtils.sortOn(
      arr,
      ["score"],
      [ArrayConstant.NUMERIC | ArrayConstant.DESCENDING],
    );
  ...
  public get isVictoryed(): boolean {
    var pvpWarFightInfo: PvpWarFightInfo =
      CampaignManager.Instance.pvpWarFightModel.pvpWarFightInfo;
    var pvpWarReportInfo: WarReportInfo =
      CampaignManager.Instance.pvpWarFightModel.fightReportInfo;
    return (
      pvpWarFightInfo &&
      pvpWarReportInfo &&
      pvpWarFightInfo.teamId == pvpWarReportInfo.oneTeamId
    );
  }
