...
export class CampaignMapCenterCheckHelper {
  ...
  public static checkOutScene(p: Laya.Point, target?: Laya.Sprite): Laya.Point {
    if (p.x > CampaignMapCenterCheckHelper._startX)
      p.x = CampaignMapCenterCheckHelper._startX;
    if (p.y > CampaignMapCenterCheckHelper._startY)
      p.y = CampaignMapCenterCheckHelper._startY;
    ...
    var minX: number =
      StageReferance.stageWidth -
      (mMode ? mMode.mapTempInfo.Width * scaleX : 100000);
    var minY: number =
      StageReferance.stageHeight -
      (mMode ? mMode.mapTempInfo.Height * scaleY : 100000);
