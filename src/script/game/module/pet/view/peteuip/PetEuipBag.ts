...
export class PetEuipBag extends FUI_PetEuipBag {
  ...
  private initTabView() {
    for (let i = 0; i < this.list_tab.numChildren; i++) {
      ...
      (tabBtn.getChild("icon0") as fgui.GLoader).url =
        fgui.UIPackage.getItemURL(EmWindow.Pet, "Icon_Tab_" + (i + 1) + "_Nor");
      (tabBtn.getChild("icon1") as fgui.GLoader).url =
        fgui.UIPackage.getItemURL(EmWindow.Pet, "Icon_Tab_" + (i + 1) + "_Sel");
    }
  }
  ...
  private addEvent() {
    ...
    PlayerManager.Instance.currentPlayerModel.playerInfo.addEventListener(
      PlayerEvent.PET_BAG_CAPICITY,
      this.onBuyBagCellReceive,
      this,
    );
  ...
  private onBagRenderer(index: number, item: PetEquipCell) {
    ...
    if (state != Enum_BagGridState.Item) {
      ...
      (item.getChild("part_icon") as fgui.GLoader).url =
        fgui.UIPackage.getItemURL(
          EmWindow.Pet,
          this.urlArr[this.list_tab.selectedIndex],
        );
      return;
    }
    ...
    item.selectState = this.resolveCtrl.selectedIndex
  ...
  private onBuyClick() {
    if (this.curOpenCount >= this.maxBagCount) {
      MessageTipManager.Instance.show(
        LangManager.Instance.GetTranslation("petEquip.buymax"),
      );
      return;
    }
    let str =
    ...
    if (str) {
      ...
      SimpleAlertHelper.Instance.Show(
        ...
        null,
        null,
        this.confirmBuyBagCell.bind(this),
      );
    }
  }
  ...
  private onTabClick() {
    ...
    NotificationManager.Instance.dispatchEvent(
      PetEvent.PET_EUIP_BAG_PART,
      this.list_tab.selectedIndex,
    );
  }
  ...
  private filterBySonType() {
    ...
    for (let equipData of allBagDatas) {
      if (equipData.templateInfo.SonType == this.selectedSonType) {
        this.canEquip(equipData)
          ? canEquips.push(equipData)
          : unEquips.push(equipData);
      }
    }
    //强化等级排序
    canEquips.sort((a, b) => {
      return b.strengthenGrade - a.strengthenGrade;
    });
    unEquips.sort((a, b) => {
      return b.strengthenGrade - a.strengthenGrade;
    });
    //品质排序
    canEquips.sort((a, b) => {
      return b.templateInfo.Profile - a.templateInfo.Profile;
    });
    unEquips.sort((a, b) => {
      return b.templateInfo.Profile - a.templateInfo.Profile;
    });
  ...
  private relolveEquips() {
    ...
    for (let key in this.selectedGoods) {
      ...
      if (
        goodsInfo.strengthenGrade >= 10 ||
        goodsInfo.templateInfo.Profile > 3 ||
        goodsInfo.star >= 3
      ) {
        hasGoodEquip = true;
      }
  ...
  private checkConfirmRelosve(
    b: boolean,
    checked: boolean,
    goodsId: { goodsId: number; count: number }[],
  ) {
    if (!b) return;
    if (SharedManager.Instance.notAlertThisLogin) {
      ...
      UIManager.Instance.ShowWind(EmWindow.GetGoodsAlert, {
        goodsList: goodsId,
        callback: this.confirmRelosve.bind(this),
      });
    }
  }
