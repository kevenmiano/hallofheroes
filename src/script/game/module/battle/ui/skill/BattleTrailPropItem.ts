...
export class BattleTrailPropItem extends BattleSkillItemII {
  ...
  protected initView() {
    super.initView();
    this._txtGetSkillCount = this.view.getChild(
      "txtGetSkillCount",
    ) as fgui.GLabel;
  ...
  public set data(value: any) {
    ...
    if (this._data && this._data instanceof TrailPropInfo) {
      this._data.addEventListener(
        BattleEvent.TRAIL_USECOUNT_CHANGE,
        this.__countChangeHandler,
        this,
      );
      this._icon.icon = IconFactory.getTecIconByIcon(
        (<TrailPropInfo>this._data).skillTemp.Icons,
      );
