...
export default class AnswerItem extends FUI_AnswerItem {
    ...
    onInputAnswer(evt) {
        ...
        if (input.length > this._inputMax) {
            ...
            let text = LangManager.Instance.GetTranslation("questionnaire.QuestionnaireFrame.maxWordCount", this._inputMax);
    ...
    onItemStateChange(evt) {
        ...
        if (this._optData.Type == QuestionType.Multi) {
            if (count > this._optData.MaxChoose) {
                let text = LangManager.Instance.GetTranslation("questionnaire.QuestionnaireFrame.maxSelect", this._optData.MaxChoose);
                MessageTipManager.Instance.show(text);
                this.check.selected = !this.check.selected;
            } else if (count == this._optData.MaxChoose) {//选择完成,跳转下一题
                NotificationManager.Instance.dispatchEvent(QuestionEvent.SCROLL_TO_NEXT, this._optData.Id);
            }
        } else if (this._optData.Type == QuestionType.Single) {//选择完成,跳转下一题
            if (count == 1) {//选择完成,跳转下一题
                NotificationManager.Instance.dispatchEvent(QuestionEvent.SCROLL_TO_NEXT, this._optData.Id);
            } else if (count > 1) {
                let text = LangManager.Instance.GetTranslation("questionnaire.QuestionnaireFrame.maxSelect", this._optData.MaxChoose);
                MessageTipManager.Instance.show(text);
        ...
        this.input.editable = this.check.selected && this.inputCtrl.selectedIndex == 1;
