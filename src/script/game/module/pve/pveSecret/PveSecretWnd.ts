...
export default class PveSecretWnd extends BaseWindow {
  ...
  private addEvent() {
    NotificationManager.Instance.on(
      SecretEvent.SECRET_RECORD_INFO,
      this.refreshSecretRecord,
      this,
    );
  }
  ...
  private initView() {
    this.campaignList.on(
      fgui.Events.CLICK_ITEM,
      this,
      this.onCampaignClickItem,
    );
    this.campaignList.itemRenderer = Laya.Handler.create(
      this,
      this.onCampaignRenderItem,
      null,
      false,
    );
    this.tresureList.itemRenderer = Laya.Handler.create(
      this,
      this.onTresureListRenderItem,
      null,
      false,
    );
    this.dropList.itemRenderer = Laya.Handler.create(
      this,
      this.onDropRenderItem,
      null,
      false,
    );

    this.txtFrameTitle.text =
      LangManager.Instance.GetTranslation("Pve.secretTitle");
    this.txtTresure.text =
      LangManager.Instance.GetTranslation("Pve.secret.tresure");
    this.txtTresureDesc.text = LangManager.Instance.GetTranslation(
      "Pve.secret.tresureDesc",
    );
    this.txtDropTip.text = LangManager.Instance.GetTranslation(
  ...
  public defaultView() {
    ...
    this.onCampaignClickItem(
      this.campaignList.getChildAt(idx) as PveSecretFBItem,
    );
    ...
    for (let index = 0; index < this.campaignList.numChildren; index++) {
      ...
      if (item.info) {
        ...
        if (curSecretId && itemSecretId == curSecretId) {
          Logger.info(
            "[PveSecretWnd]当前进行秘境进度：",
            curSecretId,
            curRecordInfo.curLayer,
          );
  ...
  private refreshEnterTip() {
    ...
    if (this.secretInfo.secretId == this.model.secretCfg.SecretId) {
      ...
      if (checkGrade) {
        ...
        if (open) {
          this.txtEnterTip.text = LangManager.Instance.GetTranslation(
            "Pve.secret.enterCount",
            this.secretInfo.leftCount,
          );
        } else {
          ...
          this.txtEnterTip.text = LangManager.Instance.GetTranslation(
            "Pve.secret.enterPreTip",
            temp && temp.TemplateNameLang,
          );
          this.btnStart.enabled = false;
          this.txtEnterTip.color = ColorConstant.RED2_COLOR;
        }
      } else {
        this.txtEnterTip.text =
          LangManager.Instance.GetTranslation("public.undergrade");
        this.btnStart.enabled = false;
        this.txtEnterTip.color = ColorConstant.RED2_COLOR;
      }
    }
  ...
  private alertCount() {
    ...
    if (lackEnetrCnt) {
      let itemCnt = GoodsManager.Instance.getGoodsNumByTempId(
        ItemID.SECRET_PROP,
      );
      if (itemCnt > 0) {
        let temp = ConfigMgr.Instance.getTemplateByID(
        ...
        let content = LangManager.Instance.GetTranslation(
          "Pve.secret.enterCntNotEnoughTip2",
        ...
        MessageTipManager.Instance.show(
          LangManager.Instance.GetTranslation(
            "Pve.secret.enterCntNotEnoughTip",
          ),
        );
      }
      return true;
  ...
  private alertChange(enterType: SecretEnterType) {
    if (
      this.secretInfo.secretId &&
      this.secretInfo.secretId != this.model.secretCfg.SecretId
    ) {
      let str = LangManager.Instance.GetTranslation(
        "Pve.secret.changeSceretTip",
      );
      SimpleAlertHelper.Instance.ShowSimple(null, null, str, (b: boolean) => {
        if (b) {
          str = LangManager.Instance.GetTranslation(
            "Pve.secret.changeSceretTip2",
          );
  ...
  private sendStartSecret(enterType: SecretEnterType) {
    SecretManager.Instance.sendCreateRoom(
      1,
      this.model.secretCfg.SecretId,
      enterType,
      SecretStartType.Start,
    );
  }
