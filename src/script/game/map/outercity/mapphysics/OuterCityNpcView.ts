 ...
 * @ver 1.0
 */
export class OuterCityNpcView
  extends HeroAvatarView
  implements IBaseMouseEvent, IManualTipTargetSize
{
  ...
  public avatarBaseViewType: eAvatarBaseViewType =
    eAvatarBaseViewType.OuterCityNpc;
  ...
  public mouseClickHandler(evt: Laya.Event): boolean {
    ...
    if (
      (this.parent as OuterCityNpcLayer).checkClickPlayerNum(
        evt.stageX,
        evt.stageY,
      )
    )
  ...
  public set nodeInfo(value: MapPhysics) {
    ...
    this._nodeInfo.info.addEventListener(
      ...
      this.__upStateHandler,
      this,
    );
    this._nodeInfo.info.state = value.info.state;
    if (this._nodeInfo.info.types == PosType.OUTERCITY_BOSS_NPC) {
      //BOSS
      this.light = fgui.UIPackage.createObject(
        EmWindow.OuterCity,
        "ComBossLightMc3",
      ).asMovieClip;
    ...
    let url = PathManager.getAvatarResourcePath(
      body.toString(),
      -1,
      1,
      AvatarPosition.BODY,
  ...
  private showNameText(): void {
    this._nameText = this._nameText
      ? this._nameText
      : new FilterFrameShadowText(140, 18, undefined, 16);
    this._nameText.text = LangManager.Instance.GetTranslation(
      "public.level.name",
      this.wildLandInfo.tempInfo.NameLang,
      this.wildLandInfo.info.grade,
    );
  ...
  private __upStateHandler(evt: PhysicsEvent): void {
    ...
    Logger.xjy(
      "[OuterCityNpcView]__upStateHandler",
      state,
      this.wildLandInfo.tempInfo.NameLang,
      this.wildLandInfo.x / 20,
      this.wildLandInfo.y / 20,
    );
  ...
  public get tipData(): object {
    ...
    let apTip: string = LangManager.Instance.GetTranslation(
    ...
    info.addItem(
      this.wildLandInfo.tempInfo.NameLang +
        LangManager.Instance.GetTranslation(
          "public.level2",
          this.wildLandInfo.tempInfo.Grade,
        ),
      null,
    );
