...
export class VIPModel extends GameEventDispatcher {
    ...
    public set vipInfo(value: VipInfo) {
        ...
        if (this._vipInfo.VipGrade < value.VipGrade && this._vipInfo.VipGrade > 0) {
            this._vipInfo = value;
            title = LangManager.Instance.GetTranslation("tasktracetip.view.VipUpGradeTipView.TitleTxt");
        ...
        if (this._vipInfo.IsTakeGift && this._vipInfo.IsVipAndNoExpirt) {
            title = LangManager.Instance.GetTranslation("tasktracetip.view.VipGiftGetTipView.TitleTxt");
    ...
    public vipOpenTips() {
        let title: string = LangManager.Instance.GetTranslation("tasktracetip.view.VipRechargeTipView.TitleTxt");
    ...
    public get vipFriendPrivilege(): number {
        ...
        let arr: any[] = TempleteManager.Instance.getPrivilegeTempletesByType(VipPrivilegeType.FRIEND_COUNT);
        for (let i: number = 0; i < arr.length; i++) {
            if (this._vipInfo.VipGrade == arr[i].grade) {
                if (arr[i].para1 == 0)
                    return 0;
                else
                    return Number(arr[i].para1);
            }
        }
        return 0;
    }
    ...
    public isOpenPrivilege(type: number, vipLevel?: number): boolean {
        ...
        let temples = TempleteManager.Instance.getPrivilegeTempletesByLevel(vipLevel);
    ...
    public isVipMount(mountId: number): boolean {
        let temples = TempleteManager.Instance.getPrivilegeTempletesByType(VipPrivilegeType.MOUNT);
        for (let index = 0; index < temples.length; index++) {
            let element = temples[index];
    ...
    public getMinGradeHasPrivilege(type: number): number {
        let grade = 0;
        let temples: t_s_vipprerogativetemplateData[] = TempleteManager.Instance.getPrivilegeTempletesByType(type);
    ...
    public getPrivilegeParam1ByGrade(type: number, grade: number): number {
        let temp = TempleteManager.Instance.getPrivilegeTempletesByTypeLevel(type, grade);
        if (temp) {
