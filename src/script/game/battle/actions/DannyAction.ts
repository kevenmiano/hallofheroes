...
export class DannyAction extends GameBaseAction {
  ...
  constructor(
    ...
    $data: AttackData,
    $frame: number = 12,
    $checkDie: boolean = true,
    distance: number = 0,
    flyUpPower: number = 0,
    shakeCount: number = 4,
    flash: boolean = true,
    flashColor: number = 0x666666,
    ...
    if (
      this._currentRole.bloodA <= 0 &&
      this._data.bloodType != BloodType.REVIVE
    )
      return;
    if (!this._currentRole || !this._currentRole.actionManager) return;
    ...
    if (
      curAction &&
      curAction.inheritType == InheritIActionType.MovePointAction
    ) {
      this._isMoving = true;
      this._distance = 0;
    }
  ...
  private initHurt() {
    if (this._flash) {
      if (this._data && this._data.damageValue > 0) {
        ...
        if (
          RoleActionSimplifyData.isSimplify(
            roleView && roleView.rolePartUrlPath,
          )
        ) {
    ...
    Logger.battle(
      `[DannyAction]${this._currentRole.roleName}(${this._currentRole.livingId})受到伤害${this._data.damageValue}`,
    );
    ...
    this._currentRole.updateBlood(
      -this._data.damageValue,
    ...
    if (this._currentRole.realBloodA == 0 && this._data.damageValue != 0) {
      ...
      Logger.battle(
        "[DannyAction]此伤害杀死角色 需要播放死亡动作, 此处不能使用<=0 作为死亡的判断条件",
      );
      this._needPlayDieAction = true;
    }
    if (this._currentRole.side == 1) {
      //是己方
    ...
    if (
      this._data.damageValue != 0 &&
      (this._data.bloodType == BloodType.BLOOD_TYPE_SELF ||
        this._data.bloodType == BloodType.BLOOD_TYPE_ARMY)
    ) {
      //如果是己方英雄或者部队的伤害, 则计算连击数
      // BattleManager.Instance.batterModel.batterCount += 1;
    }

    if (this._data.bloodType == BloodType.REVIVE) {
      //复活
      let target: HeroRoleInfo = BattleManager.Instance.battleModel.getRoleById(
        this._data.roleId,
      ) as HeroRoleInfo;
  ...
  private revierBoss(role: HeroRoleInfo) {
    ...
    chatData.msg = LangManager.Instance.GetTranslation(
      "battle.actions.DannyAction.ChatDataMsg",
    );
    chatData.commit();
  ...
  private playDenfence() {
    if (
      this._data &&
      this._data.resistValue > 0 &&
      (this._currentRole.actionMovie.currentLabel == ActionLabesType.STAND ||
        this._currentRole.actionMovie.currentLabel == ActionLabesType.READY)
    ) {
      let per =
        this._data.resistValue /
        (this._data.resistValue + this._data.damageValue);
      if (per > 0.3) {
        let index;
        ...
        if (index && ResourceModel.Resist_Sheld_Names[index]) {
          let effect: BattleEffect = new DefenceEffect(
          ...
          mc.curCacheName =
            ResourceModel.Resist_Sheld_Prefix +
            ResourceModel.Resist_Sheld_Names[index] +
            "/";

          let isLeft: boolean =
            this._currentRole.face == FaceType.LEFT_TEAM ? true : false;
          if (isLeft) {
            mc.scaleX = 1;
            mc.x = 20;
          } else {
          ...
          let fullUrl = PathManager.solveResistSheldResPath(
            ResourceModel.Resist_Sheld_Names[index],
          );
  ...
  public prepare() {
    ...
    let roleInitPt: Laya.Point = BattleUtils.rolePointByPos(
      this._currentRole.pos,
      this._currentRole.face,
    );
  ...
  public update() {
    ...
    if (!this.finished && !this._isMoving) {
      this.toPoint = new Laya.Point(
        this._currentRole.point.x +
          0.5 * (this._endPoint.x - this._currentRole.point.x),
        this._currentRole.point.y,
      );
      this.toPointZ =
        this._currentRole.pointZ +
        this._currentPower * DannyAction.PERCENTAGE_GRAVITY_TO_PIXEL;
      if (this.toPointZ <= 0) {
        this.toPointZ = 0;
      } else {
  ...
  private removeBackDust() {
    if (this._backDust) {
      if (this._backDust.parent)
        this._backDust.parent.removeChild(this._backDust);
    }
    this._backDust = null;
  }
  ...
  protected actionOver() {
    ...
    let to_point: Laya.Point = BattleUtils.rolePointByPos(
      this._currentRole.pos,
      this._currentRole.face,
    );

    if (this._needPlayDieAction || this._currentRole.bloodA <= 0) {
      //角色执行伤害动作以后死亡
      this.finished = true;
      this.playDie();
    } else {
  ...
  private playDie() {
    Logger.battle(
      `[DannyAction]${this._currentRole.roleName}(${this._currentRole.livingId})受击后死亡`,
    );
    if (
   ...
   */
  private playDrop() {
    if (this._data.dropList && this._data.dropList.length > 0) {
      //死亡时掉落
      ...
      this._data.dropList.forEach((info: DropInfoMsg) => {
        if (info.winnerIds && info.winnerIds.length > 0) {
          info.winnerIds.forEach((id) => {
            if (self.livingId == id) {
              Logger.battle(
                "掉落数量:" + info.dropCount + "dropId " + info.dropId,
              );
  ...
  private isAttacking(): boolean {
    if (
      this._currentRole &&
      this._currentRole.actionMovie &&
      this._currentRole.actionMovie.movie
    ) {
      if (
        this._currentRole.actionMovie.movie.currentLabel &&
        this._currentRole.actionMovie.movie.currentLabel
          .toLowerCase()
          .indexOf("attack") > -1
      ) {
