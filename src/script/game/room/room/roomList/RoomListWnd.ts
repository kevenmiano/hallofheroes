...
export default class RoomListWnd extends BaseWindow {
    ...
    private _alertString: string = LangManager.Instance.GetTranslation("pveroomlist.view.PVERoomSearchFrame.alert");
    ...
    OnShowWind() {
        ...
        this.frame.getChild('title').text = LangManager.Instance.GetTranslation('pvp.PvPFrame.title');
        this.txt1.text = LangManager.Instance.GetTranslation('answer.view.rank.name');
        this.txt2.text = LangManager.Instance.GetTranslation('answer.view.rank.score');
        this.txt3.text = LangManager.Instance.GetTranslation('PveRoomListWnd.jobTxt');
        this.txtScoreTitle.text = LangManager.Instance.GetTranslation('RvrBattleMapRightWnd.myScoreTxt');
        this.txtEnterCountDesc.text = LangManager.Instance.GetTranslation('PveSelectCampaignWnd.enterCountTxt');

        this.btnSearchRoom.title = LangManager.Instance.GetTranslation('pveroomlist.view.PVERoomSearchFrame.title');
        this.btnCreateRoom.title = LangManager.Instance.GetTranslation('PveRoomListWnd.createRoomTxt');
        this.btnEnterRoom.title = LangManager.Instance.GetTranslation('PveRoomListWnd.enterRoomTxt');
    ...
    helpBtnClick() {
        ...
        content = LangManager.Instance.GetTranslation("room.roomList.roomHelpContent");
    ...
    private btnSearchClick() {
        if (this.iTxtSearch.text == this._alertString || this.iTxtSearch.text == "") {
        ...
        if (id > 0) {
            this.ctrl.sendSearchRoomInfo(this.ctrl.roomSceneType == RoomSceneType.PVE ? RoomType.NORMAL : RoomType.MATCH, id);
        }
        else if (id == 0) {
    ...
    private refreshRoomMembers() {
        if (this.selectRoom) {
            ...
            for (const key in this.selectRoom.roomPlayer) {
                if (Object.prototype.hasOwnProperty.call(this.selectRoom.roomPlayer, key)) {
                    ...
                    if (item) {
                        item.visible = true;
                        item.imgCaptain.visible = this.selectRoom.houseOwnerId == army.playerId;
    ...
    private btnSearchRoomClick() {
        FrameCtrlManager.Instance.open(EmWindow.FindRoom, {roomSceneType: RoomType.MATCH});
    }
