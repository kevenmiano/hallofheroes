...
export default class MountsWnd extends BaseWindow {
  ...
  private typeArray: Array<string> = [
    PropertyInfo.STRENGTH,
    PropertyInfo.INTELLECT,
    PropertyInfo.STAMINA,
    PropertyInfo.ARMOR,
  ];
  ...
  public OnInitWind() {
    ...
    this.frame.getChild("title").text =
      LangManager.Instance.GetTranslation("mounts.command07");
    ...
    this.item_poicon1.setInfo(
      ShopGoodsInfo.MOUNT_FOOD_TEMPID,
      true,
      FUIHelper.getItemURL("Base", "Icon_Unit_Mount_L"),
    );
  }
  ...
  private addEvent() {
    ...
    this.mountInfo.addEventListener(
      MountsEvent.MOUNT_INFO_CHANGE,
      this.__onMountInfoChange,
      this,
    );
    this.mountInfo.addEventListener(
      MountsEvent.PROP_LEVEL_UP,
      this.__showPropLevelupEffect,
      this,
    );
    this.mountInfo.addEventListener(
      MountsEvent.MOUNT_LEVEL_UP,
      this.__showLevelupEffect,
      this,
    );
  ...
  private removeEvent() {
    ...
    this.mountInfo.removeEventListener(
      MountsEvent.MOUNT_INFO_CHANGE,
      this.__onMountInfoChange,
      this,
    );
  ...
  private __updateNumberHandler() {
    ...
    this.cfgValue =
      this.getBagCount() < this.cfgValue
        ? this.getBagCount() < 1
          ? 1
          : this.getBagCount()
        : this.cfgValue;
  ...
  private trainBtnClick() {
    ...
    if (!this._selectedItem) {
      MessageTipManager.Instance.show(
        LangManager.Instance.GetTranslation("mounts.command09"),
      );
      return;
    }
    ...
    if (needAlert) {
      ...
      if (this._selectedItem.valueType == PropertyInfo.INTELLECT) {
        pName = LangManager.Instance.GetTranslation(
  ...
  private checkCountEnouth(value: number = 0): boolean {
    ...
    if (this.getBagCount() < value) {
      ...
      if (info) {
        ...
        MessageTipManager.Instance.show(
          LangManager.Instance.GetTranslation("mounts.lackProps"),
        );
      }
      enough = false;
    }
  ...
  private doTrain() {
    if (this._selectedItem.propertyInfo.isLimit) {
      MessageTipManager.Instance.show(
        LangManager.Instance.GetTranslation("mounts.gradeIsMax"),
      );
      return;
    }
    ...
    // }
    // if (flag) {
    let propValue: number = PropertyInfo.getPropertyValue(
      this._selectedItem.valueType,
    );
  ...
  private advanceTrainBtnClick() {
    ...
    var lastSaveDate: Date = new Date(
      SharedManager.Instance.advDomesticateAlertDate,
    );
    ...
    if (this._selectedItem.valueType == PropertyInfo.INTELLECT) {
      ...
      if (this.mountInfo.discount > 0 && this.mountInfo.discount < 10) {
        msg = LangManager.Instance.GetTranslation(
          ...
          Math.max(
            (this.cfgAdvTrainCost - 10 * this.getBagCount()) *
              this.mountInfo.discount *
              0.1,
            this._trainData.advanceTrainCount,
          ),
          pName,
        );
      } else {
  ...
  private doAdvanceTrain() {
    if (this._selectedItem) {
      ...
      // }
      // if (flag) {
      var propValue: number = PropertyInfo.getPropertyValue(
        this._selectedItem.valueType,
      );
  ...
  private mountBtnClick() {
    ...
    if (ArmyManager.Instance.thane.changeShapeId > 0) {
      MessageTipManager.Instance.show(
        LangManager.Instance.GetTranslation("mountsFrame.CannotMount"),
      );
      return;
    }
  ...
  private wildSoulBtnClick() {
    FrameCtrlManager.Instance.open(
      ...
      { returnToWin: EmWindow.MountsWnd },
      null,
      EmWindow.MountsWnd,
    );
  }
  ...
  private refreshView() {
    for (let i: number = 0; i < this.itemList.numChildren; i++) {
      let item: MountPropertyItem = this.itemList.getChildAt(
        i,
      ) as MountPropertyItem;
      if (item) {
        item.update();
      }
    }
    ...
    this.moveSpeedTxt.text =
      LangManager.Instance.GetTranslation("MountWnd.moveSpeedTxt") +
      " + " +
      this.mountInfo.speedAdd +
      "%";
    this.LevelTxt.text = LangManager.Instance.GetTranslation(
      "mounts.command01",
      this.mountInfo.grade,
    );
    this.growBarProgres.getChild("progress").text =
      this.mountInfo.currentGrowExp + " / " + this.mountInfo.growExpMax;
    this.growBarProgres.value =
      (this.mountInfo.currentGrowExp * 100) / this.mountInfo.growExpMax;
    this.mountNameTxt.text = this.mountInfo.template
      ? this.mountInfo.template.TemplateNameLang
      : "";
    this.mountInfo.template &&
      this.updateAvatar(this.mountInfo.template.AvatarPath);

    if (this.mountInfo.template && this.mountInfo.template.StarItem == 0) {
      this.starGroup.visible = false;
      this.mountNameTxt.y = 95;
    } else {
      var info: WildSoulInfo = this.wildSoulCollection.getWildSoulInfo(
        this.mountInfo.templateId,
      );
      if (info) {
  ...
  private getBagCount(): number {
    return GoodsManager.Instance.getGoodsNumByTempId(
      this._trainData.mountFoodTemplateId,
    );
  }
