...
import t_s_skillbuffertemplate, {
  t_s_skillbuffertemplateData,
} from "../../../../config/t_s_skillbuffertemplate";
...
export class BossBufferContainer extends Laya.Sprite {
  ...
  public initEvent(boss: HeroRoleInfo) {
    if (boss) {
      ...
      if (
        this._boss.heroInfo &&
        this._boss.heroInfo.templateId ==
  ...
  public removeEvent() {
    if (this._boss) {
      this._boss.removeEventListener(
        RoleEvent.REFRESH_BUFFER,
        this.__reFreshBuffer,
        this,
      );
  ...
  private _isLivingHandler() {
    if (this._boss && this._boss.isLiving == false)
      this._boss.cleanBuffers(false);
  ...
  private refreshConsortiaBossBuffer() {
    ...
    if (str) {
      ...
      bufferTempInfo = ConfigMgr.Instance.getTemplateByID(
  ...
  private showBuffer() {
    ...
    if (battleModel.battleType == BattleType.DOUBLE_BOSS_BATTLE) {
      let specialTempInfoData: t_s_skilltemplateData =
        ConfigMgr.Instance.getTemplateByID(
          ConfigType.t_s_skilltemplate,
          this._specialSkillId,
  ...
  private __clickItem(idx: number) {
    if (!this._commonTip) {
      this._commonTip = FUIHelper.createFUIInstance(
        EmPackName.Base,
        "CommonTips",
      );
    ...
    if (item) {
      ...
      let gPt: Laya.Point = item.displayObject.localToGlobal(
        new Laya.Point(0, 0),
        false,
        Laya.stage,
      );
      ...
      if (txt_content.textWidth >= txt_content.maxWidth) {
        //超出最大宽度, 更改适配方式
      ...
      this._commonTip.setXY(
        gPt.x - this._commonTip.width,
        gPt.y - this._commonTip.height,
      );
  ...
  private specialBufferView(specialTempInfoData: t_s_skilltemplateData) {
    ...
    item.txtLayerCount.text = "";
    item.tipData =
      specialTempInfoData.SkillTemplateName +
      "<br>" +
      specialTempInfoData.SkillDescription;
    return item;
  }
  ...
  private resetImmuneBuffer() {
    ...
    if (
      battleModel &&
      battleModel.battleType == BattleType.DOUBLE_BOSS_BATTLE
    ) {
      ...
      if (str2) {
        ...
        for (var j: number = 0; j < arr2.length; j++) {
          bufferTemp = ConfigMgr.Instance.getTemplateByID(
            ConfigType.t_s_skillbuffertemplate,
            arr2[j].toString(),
          ) as t_s_skillbuffertemplateData;
