...
import { BagEvent, NativeEvent, NotificationEvent } from "../../constant/event/NotificationEvent";
...
export default class PetChallengeWnd extends BaseWindow {
    ...
    private addEvent() {
        ...
        this.model.buildingOrder.addEventListener(Laya.Event.CHANGE, this.__refreshBuildOrder, this);
        this.playerInfo.addEventListener(PlayerEvent.PLAYER_PET_LIST_CHANGE, this.refreshPetItemList, this);
        NotificationManager.Instance.addEventListener(NativeEvent.AFTER_STATUS_BAR_CHANGE, this.onAfterStatusBarChange, this);
        NotificationManager.Instance.addEventListener(PetChallengeEvent.CHALLENGE_TIME_CHANGE, this.__refreshBuildOrder, this);
    ...
    private delEvent() {
        ...
        this.model.buildingOrder.removeEventListener(Laya.Event.CHANGE, this.__refreshBuildOrder, this);
        this.playerInfo.removeEventListener(PlayerEvent.PLAYER_PET_LIST_CHANGE, this.refreshPetItemList, this);
        NotificationManager.Instance.removeEventListener(NativeEvent.AFTER_STATUS_BAR_CHANGE, this.onAfterStatusBarChange, this);
        NotificationManager.Instance.removeEventListener(PetChallengeEvent.CHALLENGE_TIME_CHANGE, this.__refreshBuildOrder, this);
    ...
    private __refreshBuildOrder() {
        ...
        this.txtOrderTime.text = DateFormatter.getCountDate(this.model.buildingOrder.remainTime);
    ...
    private refreshPetItemList() {
        let petlist: PetData[] = [];
        for (let index = 0; index < this.playerInfo.petChallengeFormationOfArray.length; index++) {
        ...
        petlist = ArrayUtils.sortOn(petlist, ["fightPower"], [ArrayConstant.NUMERIC | ArrayConstant.DESCENDING])
    ...
    private refreshFigure() {
        ...
        (this["itemFigure" + (this._refreshFigureCnt + 1)] as PetChallengeFigureItem).info = challengeList[this._refreshFigureCnt];
    ...
    public refresSelfInfo() {
        this.txtMyRankValue.text = this.model.ranking <= 0 ? LangManager.Instance.GetTranslation("colosseum.view.ColosseumPlayerItem.newPeople") : this.model.ranking
        this.txtScoreValue.text = this.model.score
        this.txtTeamCapacityValue.text = this.model.totalFightPower
        this.itemHead.icon = IconFactory.getPlayerIcon(ArmyManager.Instance.thane.snsInfo.headId, IconType.HEAD_ICON);
    }

    private refresOutLanderAlert() {
        let num: number = GoodsManager.Instance.getGoodsNumByTempId(ItemID.OUTLANDER_ORDER);

        let content: string = LangManager.Instance.GetTranslation("PetChallenge.notEnoughOutLander01", this.model.buildingOrder.remainCount);
        let goodsCount: string = LangManager.Instance.GetTranslation("MazeShopWnd.HasNumTxt") + num;
        let params = {
            content: content, goodsId: ItemID.OUTLANDER_ORDER, goodsCount: goodsCount, autoClose: !this.notAutoCloseUseGoodAlert()
        };
    ...
    private btnOrderCoolClick() {
        ...
        if (VIPManager.Instance.model.vipCoolPrivilege) {
            ...
            if (cfgItemChallenge) {//挑战
                cfgItemChallengeValue = Number(cfgItemChallenge.ConfigValue);
            }
            let point = cfgItemChallengeValue * Math.ceil(order.remainTime / 60);
            UIManager.Instance.ShowWind(EmWindow.VipCoolDownFrameWnd, { type: QueueItem.QUEUE_TYPE_ALCHEMY, orderId: order.orderId, pointNum: point, backFun: this.__btnOrderCoolClick.bind(this) });
        }
    }

    private __btnOrderCoolClick(b: boolean, id: number = 0, type: number = 0, useBind: boolean = true) {
        if (b) {
    ...
    private useGoodeCallBack(b: boolean) {
        ...
        if (pos == -1) {
            let info: ShopGoodsInfo = TempleteManager.Instance.getShopTempInfoByItemId(ItemID.OUTLANDER_ORDER);
            if (info && (info.OneDayCount - info.OneCurrentCount) > 0) {
                ...
                MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("PetChallenge.notEnoughOutLander02"));
            }
        } else {
    ...
    private helpBtnClick() {
        let title: string = LangManager.Instance.GetTranslation("PetChallengeFrame.helpBtnTip");
        let grade: number = VIPManager.Instance.model.getMinGradeHasPrivilege(VipPrivilegeType.PET_ARENA_WAIT);
        let content: string = LangManager.Instance.GetTranslation("PetChallengeFrame.helpContent", grade);
