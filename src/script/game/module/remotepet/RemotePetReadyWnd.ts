...
export class RemotePetReadyWnd extends BaseWindow {
    ...
    private addEvent() {
        ...
        this.contentPane.displayObject.on(Laya.Event.CLICK, this, this.onStageClick);
    }

    private removeEvent() {
        this.model.removeEventListener(RemotePetEvent.COMMIT, this.__commitHandler, this);
    ...
    private onStageClick(evt: Laya.Event) {
        if (evt.target.parent && evt.target.parent["$owner"]) {
            ...
            if (sourceTarget instanceof RemotePetHeadItemView || sourceTarget instanceof RemotePetFriendItemView) {
                return;
            }
        }

        if (this._selectPetView.visible == true || this._selectFriendView.visible == true) {
    ...
    private onStartClick() {
        if (this.model.enterWarNum < 3) {
            MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("remotepet.views.RemotePetInfoView.tip"));
            return;
        }
