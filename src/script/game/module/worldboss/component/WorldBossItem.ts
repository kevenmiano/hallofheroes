...
export default class WorldBossItem extends FUI_WorldBossItem {
  ...
  private __enterHander() {
    ...
    if (this._info.MinLevel > ArmyManager.Instance.thane.grades) {
      str = LangManager.Instance.GetTranslation(
        "activity.view.ActivityItem.command02",
      );
  ...
  private alertWarning() {
    ...
    var content: string = LangManager.Instance.GetTranslation(
      "worldboss.BuyAvatarFrame.command04",
    );
    ...
    if (WorldBossManager.Instance.stuntman.hasBuyFor(this._info.CampaignId)) {
      ...
      if (this.isNeedAlert()) {
        let content: string = LangManager.Instance.GetTranslation(
          "map.campaign.worldBoss.buyAlert",
          this._payCount,
        );
        UIManager.Instance.ShowWind(EmWindow.TodayNotAlert, {
          content: content,
          backFunction: this.todayNotAlert.bind(this),
          closeFunction: this.cancelHandle.bind(this),
          state: 2,
        });
      } else {
  ...
  private isNeedAlert(): boolean {
    ...
    let lastSaveDate: Date = new Date(
      SharedManager.Instance.worldBossBuyStunmanFoCheckDate,
    );
