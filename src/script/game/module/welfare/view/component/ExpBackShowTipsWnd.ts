...
export default class ExpBackShowTipsWnd extends BaseWindow {
  ...
  public OnInitWind() {
    ...
    if (this.params) {
      ...
      if (
        this.params.resourceTxt1 &&
        !StringHelper.isNullOrEmpty(this.params.resourceTxt1)
      ) {
  ...
  confirmBtnClick() {
    if (this._type == 2) {
      ...
      if (this.check2Btn.selected) {
        ...
        MessageTipManager.Instance.show(
          LangManager.Instance.GetTranslation("ExpBackItem.cost.notEnough"),
        );
        return;
      }
    }
