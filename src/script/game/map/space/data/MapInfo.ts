...
export class MapInfo extends GameEventDispatcher {
    ...
    private _aroundPoints: any[] = [{ x: 0, y: 0 }, { x: -1, y: 0 }, { x: -1, y: 1 }, { x: -1, y: -1 }, { x: 0, y: 1 }, { x: 0, y: -1 }, { x: 1, y: 1 }, { x: 1, y: 0 }, { x: 1, y: -1 }];
    ...
    public getAroundWalkPoint($x: number, $y: number): Laya.Point {
        ...
        for (let i: number = 0; i < len; i++) {
            isWalk = this.getWalkable($x + this._aroundPoints[i].x, $y + this._aroundPoints[i].y);
            if (isWalk) {
