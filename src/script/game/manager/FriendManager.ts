...
import { ChatEvent, FindResultEvent, FriendGroupEvent, FriendUpdateEvent } from '../constant/event/NotificationEvent';
...
export class FriendManager extends GameEventDispatcher {
    ...
    private initRelationList(obj: Array<any>, getShared: boolean = false) {　//初始化好友列表
        let arr: any[] = obj;
    ...
    private initGroupList(obj: Array<any>) {　　//初始化自定义分组列表
        let defaultGroup = new FriendGroupInfo();
        let list: any[] = obj;
        defaultGroup.groupId = FriendGroupId.FRIEND;
        defaultGroup.groupName = LangManager.Instance.GetTranslation("public.friend");
    ...
    private initEvent() {
        ServerDataManager.listen(S2CProtocol.U_CH_ADD_RELATIONSHIP, this, this.__addFriend);
        ServerDataManager.listen(S2CProtocol.U_CH_REMOVE_RELATIONSHIP, this, this.__removeFriend);
        ServerDataManager.listen(S2CProtocol.U_CH_FRIEND_LOGIN, this, this.__friendLogin);
        ServerDataManager.listen(S2CProtocol.U_CH_FRIEND_LOGOUT, this, this.__friendLoginOut);
        ServerDataManager.listen(S2CProtocol.U_CH_FRIENDADD_CONFIRM, this, this.__addFriendConfirmHandler);
        ServerDataManager.listen(S2CProtocol.U_CH_RECOMMENDLIST, this, this._recommendFriend);
        ServerDataManager.listen(S2CProtocol.U_CH_FRIEND_UPDATE, this, this.__updateFriend);
        ServerDataManager.listen(S2CProtocol.U_CH_ADD_GROUP, this, this.__addGroupHandler);
        ServerDataManager.listen(S2CProtocol.U_CH_DEL_GROUP, this, this.__delGroupHandler);
        ServerDataManager.listen(S2CProtocol.U_CH_RENAME_GROUP, this, this.__renameGroupHandler);
        ServerDataManager.listen(S2CProtocol.U_CH_FRIEND_MOVE, this, this.__moveFriendHandler);
        ServerDataManager.listen(S2CProtocol.U_CH_FRIEND_ADDED, this, this.__friendAddHandler);
        ServerDataManager.listen(S2CProtocol.U_CH_CHAT_STATE, this, this.__recentContactStateUpdateHandler);
        ServerDataManager.listen(S2CProtocol.U_CH_FRIEND_SEARCH, this, this.__findFriendHandler);
        ServerDataManager.listen(S2CProtocol.U_CH_FRIEND_SEARCH_NICKNAME, this, this.__findFriendByNameHandler);
        ServerDataManager.listen(S2CProtocol.U_CH_FRIEND_UPDATE_PLAYINFO, this, this.__friendInfoUpdate);
        ServerDataManager.listen(S2CProtocol.U_CH_FRIEND_ADDED_LIST, this, this.__friendAddListHandler);
        ServerDataManager.listen(S2CProtocol.U_CH_FRIEND_REQMSG_ACT, this, this.__friendAddedMsgActHandler);
        ServerDataManager.listen(S2CProtocol.U_CH_INBLACK_LIST, this, this._recvInBlackList);
    ...
    private _recvInBlackList(pkg: PackageIn) {
        ...
        NotificationManager.Instance.dispatchEvent(ChatEvent.BLACKLIST, this._isInBlackList);
    }
    ...
    private __findFriendHandler(pkg: PackageIn) {
        ...
        let totalPage: number = Math.ceil(msg.totalRows / FriendModel.FIND_RESULTS_PER_PAGE);
        let list: any[] = [];
    ...
    private __findFriendByNameHandler(pkg: PackageIn) {
        let msg: FriendSearchNickNameRspMsg = pkg.readBody(FriendSearchNickNameRspMsg) as FriendSearchNickNameRspMsg;
        ...
        if (list.length > 0)
            ...
            MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("friends.findfriend.FindResultFrame.unfindTip"))
    }
    ...
    private __friendAddedMsgActHandler(pkg: PackageIn) {
        let msg: FriendAddedMsgAct = pkg.readBody(FriendAddedMsgAct) as FriendAddedMsgAct;
        let op = msg.op;
        if (op == 0) {
            let reqId = msg.reqId;
        ...
        NotificationManager.Instance.dispatchEvent(FriendUpdateEvent.INVITE_UPDATE, null);
    }
    ...
    private __friendAddListHandler(pkg: PackageIn) {
        let msg: FriendAddMsgList = pkg.readBody(FriendAddMsgList) as FriendAddMsgList;
        if (ArmyManager.Instance.thane.grades < 6 || PlayerManager.Instance.currentPlayerModel.playerInfo.refuseFriend) {
            return;
        }
        let list = msg.list;
    ...
    private __confirmAddFriend(b: boolean, nickName: string, id: number) {
        if (b) {
            FriendSocketOutManager.sendAddFriendRequest(nickName, RelationType.FRIEND);
        }
    }
    ...
    private __addFriendConfirmHandler(pkg: PackageIn) {
        let msg: FriendRelationMsg = pkg.readBody(FriendRelationMsg) as FriendRelationMsg;
        ...
        if (SceneManager.Instance.currentType == SceneType.BATTLE_SCENE) {
            this._bufferList.add(name, noticeMsg);
    ...
    private __moveFriendHandler(pkg: PackageIn) {
        let msg: MoveFriendReqMsg = pkg.readBody(MoveFriendReqMsg) as MoveFriendReqMsg;
    ...
    private __updateFriend(pkg: PackageIn) {
        ...
        if (this._friendList[id]) {
            ...
            if (msg.expAdded > 0)  //好感度增加聊天窗提示
            {
                let chatData: ChatData = Laya.Pool.getItemByClass("ChatData", ChatData);
                chatData.channel = ChatChannel.INFO;
                chatData.msg = LangManager.Instance.GetTranslation("yishi.manager.FriendManager.chatData.msg", info.nickName, msg.expAdded);
                chatData.commit();
    ...
    private __friendLogin(pkg: PackageIn) {
        ...
        if (fInfo) {
            ...
            if (relation == RelationType.FRIEND || relation == RelationType.STRANGER || relation == RelationType.BLACKLIST) {
                fInfo.beginChanges();
                fInfo.state = StateType.ONLINE;
                fInfo.commit();
                this.dispatchEvent(FriendUpdateEvent.FRIEND_CHANGE, fInfo);
            }
            if (relation == RelationType.FRIEND || relation == RelationType.STRANGER) {
                let chatData: ChatData = Laya.Pool.getItemByClass("ChatData", ChatData);
                let noticeMsg: string = "<a t='1' id='" + fInfo.userId + "' name='" + fInfo.nickName + "' consortiaId='" + fInfo.consortiaID + "'/>" +
                    LangManager.Instance.GetTranslation("yishi.manager.FriendManager.noticeMsg");
    ...
    private readFriendInfo(info: FriendInfoMsg) {
        ...
        if (playerInfo.relation == RelationType.BLACKLIST) {
            ...
            IMManager.Instance.cleanMsgListByIdAndType(playerInfo.userId, IMModel.MSG_DIC_UNREAD);
        }
    ...
    public sendAddAllFriend(idArr: number[], relation: number): boolean {
        if (ArmyManager.Instance.thane.grades < 6) {
            let str: string = LangManager.Instance.GetTranslation("yishi.manager.FriendManager.command01");
    ...
    public sendAddFriendRequest(name: string, relation: number, reqId: string = ""): boolean {
        if (ArmyManager.Instance.thane.grades < 6) {
            let str: string = LangManager.Instance.GetTranslation("yishi.manager.FriendManager.command01");
            MessageTipManager.Instance.show(str);
            return false;
        ...
        if (relation == RelationType.BLACKLIST) {
            ...
            let content: string = LangManager.Instance.GetTranslation("yishi.manager.FriendManager.addToBlacklistTip");
            SimpleAlertHelper.Instance.Show(SimpleAlertHelper.SIMPLE_ALERT, [name, relation, reqId], prompt, content, confirm, cancel, this.addRelationCall.bind(this));
            return false;
        }
    ...
    private checkFriendList(name: string, relation: number): boolean {
        let str: string = "";
        if (this.countFriend >= FriendManager.getInstance().maxFriendCount && relation != RelationType.BLACKLIST) {
            str = LangManager.Instance.GetTranslation("yishi.manager.FriendManager.command02");
            MessageTipManager.Instance.show(str);
            return false;
        }
        if (name == PlayerManager.Instance.currentPlayerModel.playerInfo.nickName) {
            if (relation != RelationType.BLACKLIST) {
                str = LangManager.Instance.GetTranslation("yishi.manager.FriendManager.command03");
                MessageTipManager.Instance.show(str);
            }
            else {
                str = LangManager.Instance.GetTranslation("yishi.manager.FriendManager.command04");
                MessageTipManager.Instance.show(str);
            }
            return false;
        }
        if (this.isExist(name, relation)) {
            str = LangManager.Instance.GetTranslation("yishi.manager.FriendManager.command05");
            MessageTipManager.Instance.show(str);
            return false;
        }
        if (this.FullBlackList(relation) >= 50) {
            ...
            let cancel: string = LangManager.Instance.GetTranslation("public.cancel");
            let prompt: string = LangManager.Instance.GetTranslation("public.prompt");
            let content: string = LangManager.Instance.GetTranslation("yishi.manager.FriendManager.content");
    ...
    public get countFriend(): number {
        let count: number = 0;
        if (this._friendList) {
            count = this._friendList.getList().length - this.FullBlackList(RelationType.BLACKLIST);
        }
        return count;
    }
    ...
    private _recommendFriend(pkg: PackageIn) { //收到更新推荐好友列表
        this.recommendList.length = 0;
        let msg: RecommendListMsg = pkg.readBody(RecommendListMsg) as RecommendListMsg;
        let friend: FriendPlayer;
        let info: RecommendInfo;
    ...
    public reqPrivatePerson(id: number) {
        if (!this.getRecentInfoById(id)) {
            PlayerManager.Instance.sendRequestSimpleAndSnsInfo(id, RequestInfoRientation.RECENT_CONTACT);
            return;
        }
    }
    ...
    public removePrivatePerson(id: number) {
        ...
        if (info) {
            this._recentPrivatePerson.splice(this._recentPrivatePerson.indexOf(info), 1);
            SharedManager.Instance.updateRecent(this.getRecentInfoIdList());
            this.dispatchEvent(FriendUpdateEvent.ADD_RECENT_CONTACT, null);
        }
        for (let i = 0; i < this._talkingPerson.length; i++) {
    ...
    public updatePrivatePerson(recent) {
        ...
        if (recent && info) {
            this._recentPrivatePerson[this._recentPrivatePerson.indexOf(info)] = recent;
    ...
    public getListForFarmFriend() {
        ...
        for (let info of friendList) {
            if (info.relation == RelationType.FRIEND && info.tarRelation == RelationType.FRIEND) {
                farmFriendList.push(info);
            }
        }
        farmFriendList = ArrayUtils.sortOn(farmFriendList, ["friendGrade", "friendGp"], ArrayConstant.DESCENDING | ArrayConstant.NUMERIC);
    ...
    public get friendListByArray(): any[] {
        ...
        arr = ArrayUtils.sortOn(arr, ["friendGrade", "friendGp"], ArrayConstant.DESCENDING | ArrayConstant.NUMERIC);
        return arr;
    }
