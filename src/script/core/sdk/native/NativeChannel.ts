...
import { GameEventCode, GameEventString } from "../../../game/constant/GameEventCode";
import { EmWindow } from "../../../game/constant/UIDefine";
import { NativeEvent, WebViewEvent } from "../../../game/constant/event/NotificationEvent";
...
export class NativeChannel extends BaseChannel {
    ...
    constructor(id: number) {
        ...
        if (this.isIOS) {
            ...
            this.bridge = Laya.Browser.window['PlatformClass'].createClass("demo.JSBridge");
        }
    ...
    public postGameEvent(code: number, exInfo: string = "") {
        ...
        if (this.isAndroid) {
            this.bridge.call("postGameEvent", code, roleInfo ? JSON.stringify(roleInfo) : "", exInfo ? exInfo : "");
        }
        else if (this.isIOS) {
            this.bridge.call("postGameEvent:::", code, roleInfo ? JSON.stringify(roleInfo) : "", exInfo ? exInfo : "");
        }
     ...
     */
    public trackEvent(type: number, node: number, eventName: string, eventDescribe: string, pageName: string, pageDescribe: string) {
        let userInfo: UserInfo = PlayerManager.Instance.currentPlayerModel.userInfo;
        let playerInfo = PlayerManager.Instance.currentPlayerModel.playerInfo;
        let roleInfo: object = {
        ...
        if (this.isAndroid) {
            this.bridge.call("trackEvent", type, node, JSON.stringify(roleInfo), eventName, eventDescribe, pageName, pageDescribe);
        }
        else if (this.isIOS) {
            this.bridge.call("trackEvent:::::::", type, node, JSON.stringify(roleInfo), eventName, eventDescribe, pageName, pageDescribe);
        }
        ...
        if (NativeChannel.lost == 1) {
            ...
            if (b) {
                ...
                if (channelId == 20360 || channelId == 20370) {
                    SDKManager.Instance.getChannel().postGameEvent(GameEventCode.Code_1080);
                }
    ...
    showNetworkAlert(tip: string = "") {
        ...
        if (this.isAndroid) {
            ...
            let content = tip != "" ? tip : LangManager.Instance.GetTranslation("login.serverclosed.lost2");
            let exit = LangManager.Instance.GetTranslation("public.exit");
            let calcel = LangManager.Instance.GetTranslation("public.cancel");
            SimpleAlertHelper.Instance.Show(SimpleAlertHelper.SIMPLE_ALERT, null, prmit, content, exit, calcel, () => {
        ...
        if (this.isAndroid) {
            if (Laya.Browser.window.conch) {
                ...
                if (nType == 0) {
                    ...
                    let msg: string = LangManager.Instance.GetTranslation("login.serverclosed.lost2");
                    SimpleAlertHelper.Instance.Show(SimpleAlertHelper.SIMPLE_ALERT, null, prompt, msg, confirm, cancel, (b: boolean, flag: boolean) => {
                        SDKManager.Instance.getChannel().relaunch();
                    }, AlertBtnType.O);
                }
                else {
        ...
        if (this.isAndroid) {
            if (!request) {
                ...
                this.bridge.callWithBack(function (result) {
                    ...
                    NotificationManager.Instance.sendNotification(NativeEvent.PERMISSION_UPDATE, result);
                    return result;
                }, "checkPermission", type, request);
            }
        }
        else if (this.isIOS) {
    ...
    saveScreenshot(sp: Laya.Sprite, isShare: number = 1, code: number = 0, title: string = "title", desc: string = "desc") {
        ...
        if (this.isAndroid) {
            this.bridge.callWithBack(function (result) {
                ...
                return result;
            }, "saveScreenshot", base64Str1, fileName, isShare, code, title, desc);
        }
        else if (this.isIOS) {
     ...
     * @param url 链接
     */
    shareURL(code: number = 0, title: string = "title", desc: string = "desc", url: string = "") {
        if (this.isAndroid) {
    ...
    pay(type: number, orderId: string, productId: string, channelProductId: string, productName: string, productDesc: string, money: string,
        ...
        let rExInfo: string = "";
        if (this.isAndroid) {
            this.bridge.call("pay", 0, orderId, productId, channelProductId, productName, productDesc, money,
            ...
            this.bridge.call("pay:::::::::::::::::::::", 1, orderId, productId, channelProductId, productName, productDesc, money,
                showCoin, gameExInfo, count, coinType, virtualCoinType, oExInfo,
                serverId, platformServerId, serverName, roleId, roleName, roleLevel, vipLevel, rExInfo);
        }
    }
    ...
    public static onPayResult(result: number) {
        if (result == 0) {//取消支付上报
            SDKManager.Instance.getChannel().postGameEvent(GameEventCode.Code_9999, JSON.stringify({ eventToken: GameEventString.cancel_purchase }));
        }
        else if (result == 1) {//支付成功上报
    ...
    startRecordAudio(recvId: string, chatType: number, extraText: string) {
        if (chatType == 2) {//私聊已经在此方法的调用出加过主区号了
        ...
        if (this.isAndroid) {
            ...
            this.bridge.call("startRecordAudioMessage:chatType:extraText:", recvId, chatType, extraText);
        }
    }
    ...
    stopAndSendAudio(attachMsg?: string) {
        if (this.isAndroid) {
            ...
            attachMsg = ArmyManager.Instance.thane.nickName + '|' + ArmyManager.Instance.thane.grades + '|' + headId;
     ...
     * */
    cancelRecordAudio() {
        MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("NativeChannel.cancelRecordAudio"));
    ...
    public static nativeCallJs_packageInfo(packageName: string, versionName: string, versionCode: number) {
        NativeChannel.packageName = packageName;
        NativeChannel.versionName = versionName;
        NativeChannel.versionCode = versionCode;
        Logger.yyz(`Android包体信息:  packageName: ${packageName}, versionName:${versionName}, versionCode:${versionCode}`);
    }
    ...
    public static nativeCallJs_errorCode(errorCode: number) {
        NotificationManager.Instance.sendNotification(NativeEvent.ERROR_CODE, errorCode);
    }
    ...
    public static nativeCallJs(info) {
        ...
        NotificationManager.Instance.sendNotification(WebViewEvent.RECEIVE_DATA, info);
    }

    //收到相册选择图片返回
    public static nativeCallPhoto(info) {
        NotificationManager.Instance.sendNotification(WebViewEvent.RECEIVE_PHOTO_CALL, info);
    }
    ...
    public static switchAccount(isLoginAfterRestart: boolean) {
        ...
        //保存切换账户是否需要重登状态
        SharedManager.Instance.setWindowItem("isLoginAfterRestart", isLoginAfterRestart.toString());
        if (isLoginAfterRestart && SceneManager.Instance.currentType != SceneType.LOGIN_SCENE) {//非登陆场景, 统一reload一次
    ...
    public static voiceLoginSuccess() {
        ...
        if (channel instanceof NativeChannel) {
            let consortiaID = PlayerManager.Instance.currentPlayerModel.playerInfo.consortiaID;
            if (consortiaID > 0) {
     ...
     */
    public static onSendAudioMessageCallBack(requestID: string, errorcode: number, strText: string,
        strAudioPath: string, audioTime: number, sendTime: number,
     ...
     */
    public static receiveVoiceMsg(chatType: number, receiveId: string, senderId: string, msgId: string, msgType: number,
        createTime: number, distance: number, savePath: string, isRead: number, mParam: string) {
        //解析为ChatData
        let chatData: ChatData = new ChatData();
        chatData.savePath = savePath;
        chatData.isFromMe = senderId == PlayerManager.Instance.currentPlayerModel.userInfo.mainSite + '_' + PlayerManager.Instance.currentPlayerModel.playerInfo.userId;
     ...
     * @param savePath
     */
    public static onDownLoadAudioMessageCallBack(msgId: string, savePath: string) {
     ...
     */
    public static getNoSafeAreaHeight(height1: number, height2: number, rotation: number) {
        NotificationManager.Instance.sendNotification(NativeEvent.STATUS_BAR_CHANGE, height1, height2, rotation);
    }
    ...
    public static getLostPkg(lost: number) {
        ...
        if (lost >= 1 && LoginManager.Instance.hasLogin) {//丢包率大于1提示玩家网络
        ...
        NotificationManager.Instance.dispatchEvent(NativeEvent.WIFI_STATE_UPDATE, lost);
    }
    ...
    private static async showLostHint() {
        ...
        if (this._weekNetHint == null || !this._weekNetHint.isShowing) {
            this._weekNetHint = await UIManager.Instance.ShowWind(EmWindow.HintWnd, LangManager.Instance.GetTranslation("login.serverclosed.lost1", this._weekNetWaitTime));
        }
        else {
            if (!SocketManager.Instance.reConnetAlert)//如果有断线重连的提示就不处理
            {
                this._weekNetHint.setHintText(LangManager.Instance.GetTranslation("login.serverclosed.lost1", this._weekNetWaitTime));
            } else {
                this._weekNetWaitTime--;
            }
        }
    ...
    createLoginReq(userName: string, pass: string, site: string, siteId: number, appData: any) {
    ...
    adjustEvent(eventType: string, value?: any) {
        if (isOversea()) {
            SDKManager.Instance.getChannel().postGameEvent(GameEventCode.Code_9999, JSON.stringify({ eventToken: eventType }))
        }
    }
        ...
        return new Promise<any>(resolve => {
            if (this.isAndroid) {
                this.bridge.callWithBack(function (result) {
                    ...
                    NotificationManager.Instance.dispatchEvent(NativeEvent.BIND_ACCOUNT_RET, result);
                    resolve(result);
                }, "bindAccount", type);
            }
            else if (this.isIOS) {
    ...
    checkBindState(): Promise<any> {
        return new Promise<any>(resolve => {
            this.bridge.callWithBack(function (result) {
                ...
                NotificationManager.Instance.dispatchEvent(NativeEvent.CHECK_BIND_ACCOUNT, info);
                resolve(info);
            }, "checkBindState");
        });
    }
