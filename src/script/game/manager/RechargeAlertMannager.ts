...
export default class RechargeAlertMannager extends GameEventDispatcher {
    ...
    setup() {
        NotificationManager.Instance.addEventListener(NotificationEvent.CHARGE_ORDER_RSP, this.startToRecharge, this);
        NotificationManager.Instance.addEventListener(NotificationEvent.SHOW_RECHARGE, this.showRecharge, this);
    }
    ...
    public show(closeFun: Function = null) {
        ...
        let content: string = LangManager.Instance.GetTranslation("yishi.manager.RechargeAlertMannager.content");
        ...
        SimpleAlertHelper.Instance.Show(SimpleAlertHelper.SIMPLE_ALERT, null, prompt, content, confirm, cancel, (b: boolean, flag: boolean, arr: any[]) => {
            if (b && !(Utils.isWxMiniGame() && ComponentSetting.IOS_VERSION)) {
                if (FrameCtrlManager.Instance.isOpen(EmWindow.ShopWnd)) {
                    NotificationManager.Instance.dispatchEvent(ShopEvent.PAGE_SELECTED, 7);
                } else {
    ...
    public navigateToRechargePage() {
        ...
        if (ComponentSetting.SITE && ComponentSetting.SITE.slice(0, 5) == "7road") {
            let site: string = PlayerManager.Instance.currentPlayerModel.userInfo.site;
            let user: string = PlayerManager.Instance.currentPlayerModel.userInfo.user;
    ...
    private showWaiting() {
        FrameCtrlManager.Instance.open(EmWindow.Waiting, { text: LangManager.Instance.GetTranslation("shop.common.recharge") });
    ...
    public recharge(productId: string = "", count: number = 1) {
        ...
        if (isOversea() && Utils.isApp() && Number(loginWay) == LoginWay.Type_GUEST) {
            SDKManager.Instance.getChannel().checkBindState().then((ret: Array<any>) => {
                ...
                if (ret && ret.length > 0) {
                    ...
                    var content: string = LangManager.Instance.GetTranslation("UpgradeAccountWnd.bindAccountRechargeSafeTips");
                    SimpleAlertHelper.Instance.Show(SimpleAlertHelper.SIMPLE_ALERT, null, prompt, content, confirm, cancel, this.onConfirmBindAccount.bind(this));
                    return;
                }
            })
            return;
        ...
        if (!this.checkRecharge()) {
            MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("public.notsurrpot.pay"))
            return;
        }
        if (productId == "") {
            return;
        } else if (DisplayLoader.isDebug) {//debug端跳转链接
            this.showWaiting();
    ...
    public startToRecharge(orderId: string, productId: string) {
        ...
        let selectedItemData: t_s_rechargeData = TempleteManager.Instance.getRechargeTempleteByProductID(productId);
        if (!selectedItemData) return;
        SDKManager.Instance.getChannel().postGameEvent(GameEventCode.Code_9999, JSON.stringify({ eventToken: GameEventString.view_purchase_ui }));
        if (selectedItemData.ProductType == ProductType.GROWTH_RECHARGE) {//成长基金
            channel2.pay(0/*selectedItemData.ProductType*/, orderId, productId, productId, selectedItemData.ProductName, selectedItemData.ProductDesc,
                selectedItemData.MoneyNum.toString(), Number(selectedItemData.Para1), "", 1, selectedItemData.MoneyType, "钻石", "");
        } else if (selectedItemData.ProductType == ProductType.PROMOTION || selectedItemData.ProductType == ProductType.PROMOTION_WEEK) {//特惠礼包
            channel2.pay(0/*selectedItemData.ProductType*/, orderId, productId, productId, selectedItemData.ProductName, selectedItemData.ProductDesc,
                selectedItemData.MoneyNum.toString(), Number(0), "", 1, selectedItemData.MoneyType, "钻石", "");
        } else {//其他
            channel2.pay(0/*selectedItemData.ProductType*/, orderId, productId, productId, selectedItemData.ProductName, selectedItemData.ProductDesc,
                selectedItemData.MoneyNum.toString(), Number(selectedItemData.Para2), "", 1, selectedItemData.MoneyType, "钻石", "");
        }
