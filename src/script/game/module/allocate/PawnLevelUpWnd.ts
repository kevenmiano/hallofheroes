...
export class PawnLevelUpWnd extends BaseWindow {
    ...
    private attributeArray: Array<string> = ["attack", "magicAttack", "defence", "magicDefence", "live"];
    ...
    public OnInitWind() {
        this.SkillTitleTxt.text = LangManager.Instance.GetTranslation("PawnLevelUpWnd.SkillTitleTxt");
        this.SpecialTitleTxt.text = LangManager.Instance.GetTranslation("PawnLevelUpWnd.SpecialTitleTxt");
        this.CostTxt.text = LangManager.Instance.GetTranslation("PawnLevelUpWnd.CostTxt");
    ...
    private addEvent() {
        ...
        this.PawnNameList.itemRenderer = Laya.Handler.create(this, this.renderItem, null, false);
        ...
        for (const key in this._tempArr) {
            if (Object.prototype.hasOwnProperty.call(this._tempArr, key)) {
                ...
                element.addEventListener(PawnEvent.PAWN_PROPERTY_CHAGER, this.__pawnChangeHandler, this);
                element.addEventListener(PawnEvent.SPECIAL_ALIBITY, this.__specialAblityChange, this);
            }
        }
    }
    ...
    private removeEvent() {
        ...
        this.PawnNameList.itemRenderer.recover();
        this.PawnNameList.itemRenderer = null;
        for (const key in this._tempArr) {
            if (Object.prototype.hasOwnProperty.call(this._tempArr, key)) {
                let element: ArmyPawn = this._tempArr[key];
                element.removeEventListener(PawnEvent.PAWN_PROPERTY_CHAGER, this.__pawnChangeHandler, this);
                element.removeEventListener(PawnEvent.SPECIAL_ALIBITY, this.__specialAblityChange, this);
            }
        }
    }
    ...
    private renderItem(index: number, item: fgui.GButton) {
        item.selected = false;
        item.text = this.getPawnNameByMasterType(this._tempArr[index].templateInfo.MasterType);
    ...
    private __onConvertBtnClick() {
        if (this.thane.grades < 40) {
            MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("buildings.casern.view.PawnLevelUpFrame.command08"));
            return;
        }
        let count: number = 0;
        for (let i: number = 0; i < this._tempArr.length; i++) {
            if (this._tempArr[i].templateId != this._currentSelected.templateId && this._tempArr[i].templateInfo.Level >= 10) {
                count += 1;
            }
        }
        if (count > 0) {
            //打开特性转换界面
            UIManager.Instance.ShowWind(EmWindow.SpecialSwitchWnd, this._currentSelected);
        }
        else {
            MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("armyII.viewII.pawnupgrade.PawnLevelUpFrame.noToConver"));
        }
    }
    ...
    private __onApperceiveClick() {
        ...
        //打开特性领悟界面
        UIManager.Instance.ShowWind(EmWindow.PawnSpecialAbilityWnd, this._currentSelected);
    }

    //升级
    private __upgradeHandler(e: MouseEvent) {
        if (ResourceManager.Instance.gold.count < this._needSoulValue) {
            var str: string = LangManager.Instance.GetTranslation("Allocate.GoldNotEnough");
            MessageTipManager.Instance.show(str);
            return;
        }
        var build: BuildInfo = BuildingManager.Instance.getBuildingInfoBySonType(BuildingType.CASERN);
        var nextTemp: t_s_pawntemplateData = this._currentSelected.nextTemplate;
        if (build && nextTemp && nextTemp.Level > build.templeteInfo.BuildingGrade) {
            str = LangManager.Instance.GetTranslation("buildings.casern.view.PawnLevelUpFrame.command06", nextTemp.Level);
            MessageTipManager.Instance.show(str);
            return;
        }
        else if (!nextTemp) {
    ...
    private __pawnChangeHandler(data: ArmyPawn) {
        if (data && data == this._currentSelected) {
            ...
            var str1: string = LangManager.Instance.GetTranslation("buildings.casern.view.PawnLevelUpFrame.command07", this._currentSelected.templateInfo.Level);
    ...
    private onClickItem(selectedItem: any) {
        ...
        this.PlayerCom.setIcon(IconFactory.getSoldierIconByIcon(this._currentSelected.templateInfo.Icon));
        this.PawnSpecialGradeTxt.text = this.getPawnNameByMasterType(this._currentSelected.templateInfo.MasterType);
        this.CurrentLevelTxt.text = LangManager.Instance.GetTranslation("public.level4_space2", this._currentSelected.templateInfo.Level);
        this.refreshRight();
    }

    private _skillClickHander(evt: Event) {
        evt.stopPropagation();
        if (UIManager.Instance.isShowing(EmWindow.SoliderSkillTipWnd))//如果有弹窗
        {
            UIManager.Instance.HideWind(EmWindow.SoliderSkillTipWnd);
        }
        else {
            let point = this.SkillCom.parent.localToGlobal(this.SkillCom.x, this.SkillCom.y);
            UIManager.Instance.ShowWind(EmWindow.SoliderSkillTipWnd, { posX: point.x, posY: point.y, pawnData: this.skillData });
        }
    }
    ...
    private _specialClickHander(evt: Event) {
        ...
        if (UIManager.Instance.isShowing(EmWindow.SoliderSkillTipWnd)) {
            ...
            let point = this.SpecialCom.parent.localToGlobal(this.SpecialCom.x, this.SpecialCom.y);
            UIManager.Instance.ShowWind(EmWindow.SoliderSkillTipWnd, { posX: point.x, posY: point.y, pawnData: this.specialAbilityData });
        }
    }
    ...
    private refreshRight(playMovie: boolean = false) {
        ...
        for (const key in this.attributeArray) {
            ...
            if (this._currentSelected[attribute] > 0) {
                ...
                (<fgui.GTextField>currentItem.getChild("AttributeNameTxt")).text = LangManager.Instance.GetTranslation(attributeNameKey);
                (<fgui.GTextField>currentItem.getChild("AttributeValueTxt")).text = this._currentSelected[attribute].toString();
                if (nextTemp) {
                    ...
                    addValueTxt.text = LangManager.Instance.GetTranslation("PawnLevelUpWnd.addValueTxt", nextPawn[attribute] - this._currentSelected[attribute]);
                }
            }
        }
        if (nextTemp) {
            this.NextLevelTxt.text = LangManager.Instance.GetTranslation("public.level4_space2", nextTemp.Level);
    ...
    private refreshSkill() {
        var maxTemplateId: number = ThaneInfoHelper.getPawnMaxTempIDBySontype(this._currentSelected.templateInfo.SonType);
        var maxTemp: t_s_pawntemplateData = TempleteManager.Instance.getPawnTemplateById(maxTemplateId);
        var list: any[] = maxTemp.HighSkill.split(",");
        for (var i: number = 0; i < 1; i++) {
            let temp: t_s_skilltemplateData;
            if (i < list.length) {
                temp = TempleteManager.Instance.getSkillTemplateInfoById(Math.ceil(list[i]));
                if (temp) {
    ...
    private refreshSpecialAbility() {
        let nextTemp: t_s_pawntemplateData = this._currentSelected.nextTemplate;
        let upgradeTemp: t_s_upgradetemplateData = nextTemp ? TempleteManager.Instance.getTemplateByTypeAndLevelAndID(nextTemp.Level, 1, nextTemp.TemplateId) : null;
        if (!upgradeTemp) {
            this.MaxLevelTxt.visible = true;
            this.MaxLevelTxt.text = LangManager.Instance.GetTranslation("PawnLevelUpWnd.MaxLevelTxt");
            ...
            this.SoulValueTxt.text = ResourceManager.Instance.gold.count + "/" + this._needSoulValue;
        ...
        if (this._currentSelected) {
            let specialTemp: t_s_specialtemplateData = TempleteManager.Instance.getPawnSpecialTemplateByID(parseInt(this._currentSelected.specialAbility));
            if (specialTemp) {
                this.SpecialAbilityNameTxt.color = "#FFECC6";
                this.SpecialCom.setIcon(IconFactory.getCommonIconPath(specialTemp.ResPath));
                this.SpecialCom.visible = true;
                this.SpecialAbilityNameTxt.text = LangManager.Instance.GetTranslation("public.level.name", specialTemp.TemplateNameLang, specialTemp.Grades);
                this.specialAbilityData = specialTemp;
            }
            if (this._currentSelected.templateInfo.Level < 10) {
                this.SpecialAbilityNameTxt.text = LangManager.Instance.GetTranslation("PawnLevelUpWnd.SpecialAbilityNameTxt.openTxt");
                ...
                if (this.thane.grades < 40) {
                    this.descTxt.text = LangManager.Instance.GetTranslation("PawnLevelUpWnd.SpecialAbilityNameTxt.openTxt2");
    ...
    private getPawnNameByMasterType(value: number): string {
        return LangManager.Instance.GetTranslation("PawnLevelUpWnd.soldier" + value);
    }
