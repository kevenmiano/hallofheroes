 ...
 * @ver 1.0
 */
export class RechargeLotteryView extends FUI_RechargeLotteryView implements FunnyContent {
	public declare item_0: RechargeLotteryGoodsItem;
	public declare item_1: RechargeLotteryGoodsItem;
	public declare item_2: RechargeLotteryGoodsItem;
	public declare item_3: RechargeLotteryGoodsItem;
	public declare item_4: RechargeLotteryGoodsItem;
	public declare item_5: RechargeLotteryGoodsItem;
	public declare item_6: RechargeLotteryGoodsItem;
	public declare item_7: RechargeLotteryGoodsItem;
	public declare item_8: RechargeLotteryGoodsItem;
	public declare item_9: RechargeLotteryGoodsItem;
	public declare item_10: RechargeLotteryGoodsItem;
	public declare item_11: RechargeLotteryGoodsItem;
	public declare item_12: RechargeLotteryGoodsItem;
	public declare item_13: RechargeLotteryGoodsItem;
	public declare item_14: RechargeLotteryGoodsItem;
	public declare item_15: RechargeLotteryGoodsItem;
	public declare item_16: RechargeLotteryGoodsItem;
	public declare item_17: RechargeLotteryGoodsItem;
	...
	public setRemainTime() {
		let checkEndTime: Date = DateFormatter.parse(this._chargeMsg.stopTime, "YYYY-MM-DD hh:mm:ss");
		let remainTime: number = checkEndTime.getTime() / 1000 - PlayerManager.Instance.currentPlayerModel.sysCurTimeBySecond; //结算剩余秒数
		if (remainTime >= 60) {
		...
		let checkStartTime: Date = DateFormatter.parse(this._chargeMsg.openTime, "YYYY-MM-DD hh:mm:ss");
		if (checkStartTime.getTime() > PlayerManager.Instance.currentPlayerModel.nowDate) {
	...
	private initEvent() {
		...
		NotificationManager.Instance.addEventListener(NotificationEvent.CHARGELOTTERY_RESULT_UPDATE, this.__chargeResultHandler, this);
	...
	private timeHandler() {
		let checkEndTime: Date = DateFormatter.parse(this._chargeMsg.stopTime, "YYYY-MM-DD hh:mm:ss");
		let remainTime = checkEndTime.getTime() / 1000 - PlayerManager.Instance.currentPlayerModel.sysCurTimeBySecond; //结算剩余秒数
		if (remainTime > 0) {
			this.setRemainTime();
	...
	private onTranComplete0() {
		...
		if (this._pauseTime > 0) {
			this.movie.play(Laya.Handler.create(this, this.onTranComplete1), 1, 0, this._pauseTime);
		} else {
			this.movie.play(Laya.Handler.create(this, this.onTranComplete1), 2, 0, this._pauseTime);
		}
	}

	private onTranComplete1() {
		this.movie.timeScale = 3;
		this.movie.play(Laya.Handler.create(this, this.onTranComplete2), 2, 0, this._pauseTime);
	}
	...
	private onTranComplete3() {
		this.movie.timeScale = 1;
		this.movie.setHook(`item${this._resultId - 1}`, Laya.Handler.create(this, this.updateResult));
		this.movie.play();
	}
	...
	private rechargeAlertHandler() {
		...
		let content: string = LangManager.Instance.GetTranslation("yishi.manager.rechargeAlertHandler.content");
		if (Utils.isWxMiniGame() && ComponentSetting.IOS_VERSION) {
			content = LangManager.Instance.GetTranslation("godarrive.GodArriveFrame.NoLeftCount");
		}
			...
			(b: boolean, flag: boolean, arr: any[]) => {
				if (b && !(Utils.isWxMiniGame() && ComponentSetting.IOS_VERSION)) {
					if (FrameCtrlManager.Instance.isOpen(EmWindow.ShopWnd)) {
						NotificationManager.Instance.dispatchEvent(ShopEvent.PAGE_SELECTED, 3);
					} else {
	...
	private removeEvent() {
		...
		NotificationManager.Instance.removeEventListener(NotificationEvent.CHARGELOTTERY_RESULT_UPDATE, this.__chargeResultHandler, this);
	}
