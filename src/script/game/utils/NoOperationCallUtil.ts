...
export default class NoOperationCallUtil
{
    ...
    public constructor(time:number, noOperationCall:Function, activeCall:Function)
    {
        this._time = time;
    ...
    private addEvent()
    {
        StageReferance.stage.on(Laya.Event.MOUSE_MOVE, this, this.__mouseMoveHandler);
        StageReferance.stage.on(Laya.Event.KEY_DOWN, this, this.__keyDownHandler);
    }

    private removeEvent()
    {
        StageReferance.stage.off(Laya.Event.MOUSE_MOVE, this, this.__mouseMoveHandler);
