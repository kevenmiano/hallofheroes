...
export default class AnswerManager extends GameEventDispatcher {
  ...
  public isOpen(): boolean {
    if (
      this.model.state > 0 &&
      ArmyManager.Instance.thane.grades >= AnswerManager.OPEN_GRADE
    ) {
      return true;
    }
    return false;
  ...
  private initEvent() {
    ServerDataManager.listen(
      S2CProtocol.U_C_ANSWER_PLAYER_INFO,
      this,
      this.__recievePlayerInfoHandler,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_ANSWER_RANK_LIST,
      this,
      this.__recieveRankListHandler,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_ANSWER_SINGLER_QUESTION,
      this,
      this.__recieveSinglerQuestionHandler,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_ANSWER_SYSTEM_STATE,
      this,
      this.__recieveSystemStateHandler,
    );

    this.model.addEventListener(
      AnswerEvent.SELECT_ANSWER,
      this.__onSelectAnswerHandler,
    );
    this.model.addEventListener(
      AnswerEvent.SELECT_ITEM,
      this.__onSelectItemHandler,
    );
  }
  /**
   * 选择答案</br>
