...
export class BindPhoneAlertAction extends MapBaseAction {
    ...
    public update() {
        ...
        if (this._count >= 24) {
            this._count = 0
            if ((SceneManager.Instance.currentType == SceneType.SPACE_SCENE) && !NewbieBaseActionMediator.isExistNewbieMask && !UIManager.Instance.isShowing(EmWindow.LevelUp)) {
                WelfareManager.Instance.firstOpenBindPhoneAlert();
                let content = LangManager.Instance.GetTranslation("BindVertify.alertGoBindPhone");
                SimpleAlertHelper.Instance.Show(SimpleAlertHelper.SIMPLE_ALERT, null, null, content, null, null, (b: boolean) => {
                    if (b) {
                        ...
                        FrameCtrlManager.Instance.open(EmWindow.Welfare, { str: LangManager.Instance.GetTranslation('welfareWnd.tabTitle.bindPhone') });
                    }
                });
                this.actionOver();
            }
