...
export class ConsortiaEmailWnd extends BaseWindow {
    ...
    private initEvent() {
        ...
        NotificationManager.Instance.addEventListener(NotificationEvent.UPDATA_CONSORTIAEMAIL_MEMBERLIST, this.updateHandler, this);
    ...
    private removeEvent() {
        ...
        NotificationManager.Instance.removeEventListener(NotificationEvent.UPDATA_CONSORTIAEMAIL_MEMBERLIST, this.updateHandler, this);
    ...
    private __onThemeChange(evt: Event) {
        let themeString: string = ChatHelper.parasMsgs(this.themeInput.asCom.getChild('userName').text);
    ...
    btnSendMailHandler() {
        ...
        if (ConfigManager.info.FILTER_TRIM) {
            ...
            this._subject = StringUtil.remove(FilterWordManager.filterWrod(this.themeTxt.text), " ");
            this._contentValue = StringUtil.remove(FilterWordManager.filterWrod(this.inputTips.text), " ");
        } else {
        ...
        if (!this._target) str = LangManager.Instance.GetTranslation("emailII.view.WriteEmailFrame.target");
        else if (!this._subject) str = LangManager.Instance.GetTranslation("emailII.view.WriteEmailFrame.subject");
        else if (!this._contentValue) str = LangManager.Instance.GetTranslation("emailII.view.WriteEmailFrame.content");
        else {
            var strArr: Array<string> = this._target.split(ConsortiaEmailWnd.SPLIT_STR);//输入框字符串按分隔符截取后的数组
            var membreInfoList: any[] = ConsortiaManager.Instance.model.consortiaMemberList.getList();//公会成员信息数组
            ...
            for (let i: number = 0; i < strArr.length; i++) {
                ...
                if (memberNickNameList.indexOf(nameStr) == -1) {
                    MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("consortia.view.myConsortia.chairmanPath.consortiaEmail.ConsortiaEmailGroupSendView.noCurrentMember"));
                    return;
                } else {
    ...
    private sendEmail(idList: Array<any>) {
        ...
        var content: string = LangManager.Instance.GetTranslation("consortia.view.myConsortia.chairmanPath.consortiaEmail.ConsortiaEmailGroupSendView.sendMail", this.costDiamond);
        SimpleAlertHelper.Instance.Show(SimpleAlertHelper.USEBINDPOINT_ALERT, {point:50, id: idList, checkDefault: true }, prompt, content, confirm, cancel, this.sendMailCall.bind(this));
    }

    private get costDiamond(): number {
        let cfgValue = 50;
        let cfgItem = TempleteManager.Instance.getConfigInfoByConfigName("Mail_NeedPoint");
        if (cfgItem) {
    ...
    private sendMailCall(b: boolean, flag: boolean, values:any) {
        if (b) {
            ...
            this._contorller.sendEmail(idList, this._subject, this._contentValue, flag);
        }
        this.OnBtnClose();
    }
        ...
        if (this._memberItemList.length > 0) {
            ...
            for (var i: number = 0; i < this._memberItemList.length; i++) {
                if (i != this._memberItemList.length - 1) {
                    this.receivetxt.text += (this._memberItemList[i].info.nickName + ConsortiaEmailWnd.SPLIT_STR);
                } else {
    ...
    private initView() {
        ...
        this.btnShlter.visible = PlayerManager.Instance.currentPlayerModel.checkChatForbidIsOpen()
    }
