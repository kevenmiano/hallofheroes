...
export class OuterCityShopModel extends GameEventDispatcher {
   ...
   *付费刷新物品需要支付钻石
   */
  private tempNeedPay: ConfigInfosTempInfo =
    TempleteManager.Instance.getConfigInfoByConfigName("MapShopPoint");
  public refreshNeedPoint: number = this.tempNeedPay
    ? parseInt(this.tempNeedPay.ConfigValue)
    : 10;
  /**
   *付费刷新物品需要支付战魂
   */
  private tempNeedStrategy: ConfigInfosTempInfo =
    TempleteManager.Instance.getConfigInfoByConfigName("MapShopStrategy");
  public refreshNeedStrategy: number = this.tempNeedStrategy
    ? parseInt(this.tempNeedStrategy.ConfigValue)
    : 20000;
  /**
   *付费刷新物品需要支付最大的战魂
   */
  private tempNeedStrategyMax: ConfigInfosTempInfo =
    TempleteManager.Instance.getConfigInfoByConfigName("MapShopStrategyMax");
  public refreshNeedStrategyMax: number = this.tempNeedStrategyMax
    ? parseInt(this.tempNeedStrategyMax.ConfigValue)
    : 1280000;
  ...
  public get lastFreshTime(): number {
    if (this.fresh_time) {
      return (
        this.fresh_time.getTime() / 1000 - this.playerModel.sysCurTimeBySecond
      );
    } else {
      return 0;
    }
  ...
  public resetData(
    curPage: number = 1,
    pageNum: number = OuterCityShopModel.MAX_GOODS_NUM,
  ) {
    this.currentPage = curPage;
    this.currentPageNum = pageNum;
    this.totalPage = Math.ceil(
      this.exchangeGoodsList.length / this.currentPageNum,
    );
  }
  ...
  public updateShowGoodsList() {
    ...
    if (endIndex > this.exchangeGoodsList.length)
      endIndex = this.exchangeGoodsList.length;
  ...
  public resetLookData(
    curPage: number = 1,
    pageNum: number = OuterCityShopModel.MAX_LOOK_GOODS_NUM,
  ) {
    this.currentLookPage = curPage;
    this.currentLookPageNum = pageNum;
    this.totalLookPage = Math.ceil(
      this.lookGoodsList.length / this.currentLookPageNum,
    );
  }

  public updateShowLookGoodsList() {
    var startIndex: number =
      (this.currentLookPage - 1) * this.currentLookPageNum;
    var endIndex: number = startIndex + this.currentLookPageNum;
    if (endIndex > this.lookGoodsList.length)
      endIndex = this.lookGoodsList.length;
  ...
  initExchangeGoodsData() {
    if (!this.exchangeGoodsList) {
      ...
      //优化标记 数据不是很大, 可以忽略
      var outerCityShopExchangeTemplateDic =
        ConfigMgr.Instance.outerCityShopExchangeTemplateDic;
      ...
      for (const key in outerCityShopExchangeTemplateDic) {
        if (
          ...
          let temp: t_s_outcityshopData = outerCityShopExchangeTemplateDic[
            key
          ] as t_s_outcityshopData;
          // 神秘商店兑换加上职业限定
          if (
            temp.NeedJob == 0 ||
            temp.NeedJob == ArmyManager.Instance.thane.templateInfo.Job
          ) {
      ...
      this.exchangeGoodsList = ArrayUtils.sortOn(
        this.exchangeGoodsList,
        ["Sort"],
  ...
  getUrlByType(type: number) {
    ...
    } else if (type == ShopGoodsInfo.MYSTERY_STONE) {
      //神秘石
      str = FUIHelper.getItemURL(EmPackName.Base, "Icon_Unit_MysteryStone");
    } else if (type == ShopGoodsInfo.MEDAL_TEMPID) {
      //勋章
  ...
  getRuneState(data: t_s_itemtemplateData) {
    var runeList: Array<any> =
      ArmyManager.Instance.thane.runeCate.allRuneList.getList();
    for (let key in runeList) {
      if (Object.prototype.hasOwnProperty.call(runeList, key)) {
        ...
        if (
          info &&
          info.templateInfo.RuneType == data.Property1 &&
          info.grade > 0
        ) {
          // 已学习
          return 1;
    ...
    for (const key in consortiaGoods) {
      if (Object.prototype.hasOwnProperty.call(consortiaGoods, key)) {
        ...
        if (
          gInfo &&
          gInfo.templateInfo &&
          gInfo.templateInfo.SonType == GoodsSonType.SONTYPE_PASSIVE_SKILL &&
          gInfo.templateInfo.Property1 == data.Property1
        ) {
          // 已拥有
          return 2;
        }
