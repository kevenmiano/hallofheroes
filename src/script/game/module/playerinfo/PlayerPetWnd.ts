...
export class PlayerPetWnd extends BaseWindow {
    ...
    public OnInitWind() {
        ...
        this.arttitleTxt.text = LangManager.Instance.GetTranslation("UIArtifact.titleTxt");
        this.titleTxt.text = LangManager.Instance.GetTranslation("UIAttrCommon.potencyTxt");
        for (let i: number = 1; i <= 5; i++) {
            this["arttxt" + i].text = LangManager.Instance.GetTranslation("petWnd.potency.itemTitle" + i);
            this["ptxt" + i].text = LangManager.Instance.GetTranslation("petWnd.potency.itemTitle" + i);
        }
        this.petCom.txt_property.text = this._data.petTypeLanguage;
        this.petCom.loader.icon = FUIHelper.getItemURL(EmPackName.Base, "Icon_PetType" + this._data.template.PetType);
        this.petCom.txtCapacity.text = this._data.fightPower.toString()
        this.petCom.txtName.text = this._data.name + " " + LangManager.Instance.GetTranslation("public.level2", this._data.grade);
    ...
    private addEvent(){
        ...
        NotificationManager.Instance.addEventListener(NotificationEvent.UPDATE_LOOKINFO_PETDATA, this.updateView,this);
        this.c1.on(fgui.Events.STATE_CHANGED, this, this.onTab1Changed);
    }

    private removeEvent(){
        Utils.clearGListHandle(this.goodsList);
        NotificationManager.Instance.removeEventListener(NotificationEvent.UPDATE_LOOKINFO_PETDATA, this.updateView,this);
    ...
    private onTab1Changed(cc: fgui.Controller) {
        ...
        NotificationManager.Instance.dispatchEvent(NotificationEvent.UPDATE_LOOKINFO_SELECTIDEX,cc.selectedIndex);
    }
    ...
    private initEquip() {
        for (let j = 0; j < 6; j++) {
            ...
            (cell.getChild('part_icon') as fgui.GLoader).url = fgui.UIPackage.getItemURL(EmPackName.PlayerInfo, this.strArr[j]);
        }
    }
    ...
    private refreshStatsAttrInfo() {
        let data = this._data
        if (this._data) {
            this.txtPhysicsAttackValue.text = (data.physicalAttack + data.bagAttack).toString();
            this.txtPhysicsDefenseValue.text = (data.physicalDefense + data.bagDefence).toString();
            this.txtMagicAttackValue.text = (data.magicAttack + data.bagMagicattack).toString();
            this.txtMagicDefenseValue.text = (data.magicDefense + data.bagMagicdefence).toString();
