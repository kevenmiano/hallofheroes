 ...
 * @ver 1.0
 */
export class OuterCityCloudView
  extends Sprite
  implements Disposeable, IEnterFrame
{
  ...
  private __dragSceneEndHandler(evt: OuterCityEvent): void {
    ...
    this.moveBitmap(
      this._cloudBitmapI,
      new Point(this._cloudBitmapI.x, this._cloudBitmapI.y),
    );
    this.moveBitmap(
      this._cloudBitmapII,
      new Point(this._cloudBitmapII.x, this._cloudBitmapII.y),
    );
  }
  ...
  private moveBitmap(bit: Sprite, point: Point): void {
    ...
    if (
      this.curMapPos.distance(this.curBitPos.x, this.curBitPos.y) >
      StageReferance.stageWidth + 200
    ) {
      if (point.x > -this.mapView.x) {
        point.x = -Math.random() * 1500 - this.mapView.x;
      } else {
        point.x =
          StageReferance.stageWidth - this.mapView.x + Math.random() * 1500;
      }
      point.y =
        100 + Math.random() * StageReferance.stageHeight - this.mapView.y;
  ...
  public enterFrame(): void {
    ...
    let vx: number =
      this._cloudPoint.x - this.curMapPos.x + StageReferance.stageWidth / 2;
    let vy: number =
      this._cloudPoint.y - this.curMapPos.y + StageReferance.stageHeight / 2;
    if (
      vx < -this._cloudBitmapI.width ||
      vx > w ||
      vy < -this._cloudBitmapI.height ||
      vy > h
    ) {
    ...
    vx =
      this._cloudPointII.x - this.curMapPos.x + StageReferance.stageWidth / 2;
    vy =
      this._cloudPointII.y - this.curMapPos.y + StageReferance.stageHeight / 2;
    if (
      vx < -this._cloudBitmapII.width ||
      vx > w ||
      vy < -this._cloudBitmapII.height ||
      vy > h
    ) {
