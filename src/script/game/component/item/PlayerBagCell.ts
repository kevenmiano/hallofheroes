...
import {
  Enum_BagGridState,
  Enum_BagState,
} from "../../module/bag/model/Enum_BagState";
...
export class PlayerBagCell extends FUI_PlayerBagCell {
  ...
  set info(value: GoodsInfo) {
    ...
    if (
      value &&
      value.templateInfo &&
      value.templateInfo.SonType == GoodsSonType.SONTYPE_TREASURE_MAP
    ) {
    ...
    if (
      value &&
      value.templateInfo &&
      value.templateInfo.SonType == GoodsSonType.SONTYPE_NOVICE_BOX
    ) {
      //是否新手礼包
    ...
    if (value) {
      ...
      this.img_unselected.visible = this.img_selected.visible =
