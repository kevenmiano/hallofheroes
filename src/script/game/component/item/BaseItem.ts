...
export class BaseItem extends FUI_BaseItem implements ITipedDisplay {
  ...
  public set newInfo(info: GoodsInfo) {
    ...
    if (info.templateInfo.Job[0]) {
      ...
      this.suit_icon.icon = FUIHelper.getItemURL(
        EmPackName.Base,
        "Icon_PetType" + info.templateInfo.Job[0],
      );
    }
  }
  ...
  public set info(info: GoodsInfo) {
    ...
    this.icon = IconFactory.getGoodsIconByTID(temp.TemplateId);
    if (
      temp.SonType == GoodsSonType.ARTIFACT &&
      temp.MasterType == GoodsType.EQUIP
    ) {
      let tempData: t_s_petartifactpropertyData =
        TempleteManager.Instance.getArtifactTemplate(temp.TemplateId);
      if (tempData) {
      ...
      this.isIdentify.selectedIndex = GoodsCheck.hasIdentify(this._info)
        ? 1
        : 0;
    ...
    if (sontypes.indexOf(info.templateInfo.SonType) >= 0) {
      ...
      if (info.templateInfo.SonType == 218) {
        //坐骑卡
        this.isActive.selectedIndex =
    ...
    if (GoodsManager.Instance.isFashion(temp)) {
      ...
      GoodsManager.Instance.isEquip(temp) ||
      ...
      if (info && info.templateInfo) {
        bagGoods = GoodsManager.Instance.checkCanEquip(
      ...
      if (bestGoodsInBag) {
        //背包中最好的&&比我装备的好
        this.img_better.visible =
          this.needShowBetterImg &&
          GoodsManager.Instance.isGneralBagGoods(info) &&
          GoodsCheck.checkGoodsBetterThanHero(info) &&
          info.id == bestGoodsInBag.id;
      }
    ...
    if (info.id == 0) {
      ...
      for (index = 1; index <= len; index++) {
        ...
        if (this.hasOwnProperty(key) && this[key] != null) {
          (this[key] as fgui.GLoader).url = fgui.UIPackage.getItemURL(
            "Base",
            CommonConstant.GEM_ITEMS_RES[10],
          );
      ...
      for (index = 1; index <= 4; index++) {
        ...
        if (this.hasOwnProperty(key) && this[key] != null) {
          ...
          (this[key] as fgui.GLoader).url = fgui.UIPackage.getItemURL(
            "Base",
            CommonConstant.GEM_ITEMS_RES[
              value == -1 ? 9 : value == 0 ? 10 : temp.Property1
            ],
          );
  ...
  public setTipStyle(temp: t_s_itemtemplateData) {
    if (
      temp.MasterType == GoodsType.EQUIP ||
      temp.MasterType == GoodsType.HONER
    ) {
      ...
      temp.MasterType == GoodsType.PROP ||
      temp.MasterType == GoodsType.PET_CARD ||
