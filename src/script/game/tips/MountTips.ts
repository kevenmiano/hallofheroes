...
export class MountTips extends BaseTips {
    ...
    private updateTransform() {
        ...
        var item: WildSoulInfo = MountsManager.Instance.avatarList.getWildSoulInfo(this._info.TemplateId);
        var strArray: Array<string> = LangManager.Instance.GetTranslation("mounts.WildsoulItem.tips01").split("|");
        var tipStr: string = "";
        var speed: number = 0;
        if (this._info.MountType == MountType.MAGIC)
            speed = this._info.Speed;
        var valueArray: Array<number> = [this._info.Power, this._info.Intellect, this._info.Physique, this._info.Agility, this._info.ExpandLevel, speed];
        ...
        this.willScoreTxt.text = LangManager.Instance.GetTranslation("mountTip.soulscore", this._info.SoulScore);
        if (this._info.MountType == MountType.NORMAL) {
            if (item) {
                ...
                this.date1.text = LangManager.Instance.GetTranslation("mounts.WildsoulItem.tips03", this._info.Property2);
                this.date1.visible = true;
            }
        } else {
            this.mountNameTxt.text = this._info.TemplateNameLang;
            ...
            if (this._info.NeedMountGrade > 0)
                needTips.push(LangManager.Instance.GetTranslation("mounts.WildsoulItem.tips.NeedGrade", this._info.NeedMountGrade));
            if (this._info.NeedItemId != 0) {
                var goodTemplate: t_s_itemtemplateData = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_itemtemplate, this._info.NeedItemId.toString());
                if (goodTemplate) {
            ...
            if (this._info.MountType == MountType.BEAST) {
                ...
                if (this._info && this._info.validity != 0) {
                    ...
                    if (this._info.validity < 0) {
                        this.specialTxt.text = LangManager.Instance.GetTranslation("mounts.WildsoulItem.forever");
                    } else {
                        this.specialTxt.text = LangManager.Instance.GetTranslation("mounts.WildsoulItem.tips02", this._info.validity);
                    }
                }
            }
            if (this.isFlying(this._info.TemplateId)) {
