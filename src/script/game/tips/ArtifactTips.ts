...
export default class ArtifactTips extends BaseTips {
      ...
      if (this._fromeType == ArtifactTips.OTHER_TYPE) {
        ...
        if (this._info.bagType == BagType.Player) {
          if (PlayerManager.Instance.currentPlayerModel.bagWndIsOpen) {
            ...
            if (GoodsCheck.hasIdentify(this._info)) {
              //已经鉴定
              this._fromeType = 2;
            } else {
              this._fromeType = 3;
        ...
        } else if (this._info.bagType == BagType.Storage) {
          //取出
          this._fromeType = 8;
        } else {
          this._fromeType = 9;
  ...
  private initView() {
    if (this._info && this._info.templateInfo) {
      ...
      this.isEquip.selectedIndex = GoodsCheck.hasEquipArtifact(this._info)
        ? 1
        : 0;
    ...
    SimpleAlertHelper.Instance.Show(
      ...
      (b: boolean) => {
        if (b) {
          ...
          if (
            this._templateData.ActiveGold > ResourceManager.Instance.gold.count
          ) {
            //黄金不够，提示
  ...
  //选择
  onSelectClick() {
    if (GoodsCheck.hasEquipArtifact(this._info)) {
      //已经装备了
      let confirm: string =
  ...
  private getPos(): number {
    ...
    for (let i = 0; i < equipArr.length; i++) {
      ...
      if (
        goodsInfo.pos == 6 &&
        goodsInfo.objectId == this.petCtrl.selectedPet.petId
      ) {
        hasSix = true;
      } else if (
        goodsInfo.pos == 7 &&
