...
export class FashionBagCellClickMediator implements IMediator {
  ...
  private get fashionModel(): FashionModel {
    return FrameCtrlManager.Instance.getCtrl(EmWindow.FashionWnd)
      .data as FashionModel;
  }
  ...
  protected __mouseClickByClicker(event: Laya.Event) {
    ...
    if (this.fashionModel.scene == FashionModel.SHOP_SCENE) {
      //商城时装界面
      if (
        this._cell.moveType == FashionModel.TRY_ON &&
        this.fashionModel.isFashion(this._cell.info)
      ) {
    ...
    } else if (this.fashionModel.scene == FashionModel.FASHION_STORE_SCENE) {
      //合成转换界面
      NotificationManager.Instance.sendNotification(BagNotic.DRAG_ITEM, [
        this._cell,
        this._cell.info.count,
      ]);
    }
  }
  ...
  protected __mouseDoubleClick(event: Laya.Event) {
    ...
    if (this.fashionModel.scene == FashionModel.SHOP_SCENE) {
      ...
      if (
        ...
        //合成
        if (this._cell.info.bagType == BagType.Player) {
          //放进合成区
          if (this.canMoveToHideBag(this._cell.info, true)) {
            if (
              this.fashionModel.isFashion(this._cell.info) &&
              this.getFashionLevelByInfo(this._cell.info) == 9
            ) {
              MessageTipManager.Instance.show(
                LangManager.Instance.GetTranslation(
                  "cell.mediator.fashionbag.FashionBagCellClickMediator.limitLevel",
                ),
              );
            }
            PlayerManager.Instance.moveBagToBag(
              this._cell.info.bagType,
              this._cell.info.objectId,
              this._cell.info.pos,
              BagType.Hide,
              0,
              this.fashionModel.to_pos,
              1,
            );
          }
        } else if (this._cell.info.bagType == BagType.Hide) {
          //从合成区取出
          PlayerManager.Instance.moveBagToBag(
            this._cell.info.bagType,
            this._cell.info.objectId,
      ...
      if (
        ...
        //转换
        if (
          this._cell.info.bagType == BagType.Player ||
          this._cell.info.bagType == BagType.HeroEquipment
        ) {
          //放进转换区
  ...
  private canMoveToHideBag(info: GoodsInfo, soul: boolean): boolean {
    ...
    if (info.templateId == FashionModel.FASHION_SOUL && soul) {
      //时装之魄
      if ((<GoodsInfo>hideInfo).templateId == FashionModel.FASHION_SOUL) {
        MessageTipManager.Instance.show(
          LangManager.Instance.GetTranslation(
            "fashion.view.compose.oneEquipLess",
          ),
        );
        return false;
      } else {
      ...
      (<GoodsInfo>hideInfo).templateInfo.SonType != info.templateInfo.SonType &&
      ...
      if (
        ...
        MessageTipManager.Instance.show(
          LangManager.Instance.GetTranslation(
            "fashion.FashionComposeView.sontypeCannotCompose",
          ),
        );
        return false;
      } else if (
        ...
        MessageTipManager.Instance.show(
          LangManager.Instance.GetTranslation(
            "fashion.FashionSwitchView.sontypeCannotSwitch",
          ),
        );
        return false;
      }
