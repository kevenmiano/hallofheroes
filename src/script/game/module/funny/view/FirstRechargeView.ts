 ...
 * 首充送礼
 */
export default class FirstRechargeView
  extends FUI_FirstRechargeView
  implements FunnyContent
{
  ...
  initView() {
    ...
    if (this._state == 2) {
      ...
      this.chargeBtn.title = LangManager.Instance.GetTranslation(
      ...
      this.chargeBtn.title = LangManager.Instance.GetTranslation(
        "yishi.loader.StartupResourceLoader.recharge",
      );
    }
  ...
  private chargeBtnHander() {
    if (this._state == 1) {
      FunnyManager.Instance.sendGetBag(
        2,
        FunnyManager.Instance.rechargeBagData.id,
      );
    } else {
  ...
  private sortMasterType(
    info1: FunnyRewardData,
    info2: FunnyRewardData,
  ): number {
    var temp1: any;
    var temp2: any;
