...
export class ConsortiaSkillTowerItem extends FUI_ConsortiaSkillTowerItem {
    ...
    private initData() {
        this._consortiaControler = FrameCtrlManager.Instance.getCtrl(EmWindow.Consortia) as ConsortiaControler
    }
    ...
    set info(value: ConsortiaTempleteInfo) {
        ...
        if (value) {
            ...
            if (_temp) {
                ...
                if (this._info.type >= ConsortiaModel.HIGH_SKILL_TYPE_MIN && this._info.type <= ConsortiaModel.HIGH_SKILL_TYPE_MAX) {
                    let studyLevel: number = PlayerManager.Instance.currentPlayerModel.getConsortiaHighSkillLevel(this._info.type);
                    let consortiaLevel: number = this.consortiaInfo.getHighLevelByUpgradeType(this._info.type);
                    this.txt_level.text = LangManager.Instance.GetTranslation("public.level3", `${studyLevel}/${consortiaLevel}`);

                } else {
                    let studyLevel: number = PlayerManager.Instance.currentPlayerModel.getConsortiaSkillLevel(this._info.type);
                    let consortiaLevel: number = this.consortiaInfo.getLevelByUpgradeType(this._info.type);
                    this.txt_level.text = LangManager.Instance.GetTranslation("public.level3", `${studyLevel}/${consortiaLevel}`);
                }
            }
        }
    ...
    private getTemp(): t_s_consortialevelData {
        if (this._info.type >= ConsortiaModel.HIGH_SKILL_TYPE_MIN && this._info.type <= ConsortiaModel.HIGH_SKILL_TYPE_MAX) {
            let studyLevel: number = PlayerManager.Instance.currentPlayerModel.getConsortiaHighSkillLevel(this._info.type);
            let consortiaLevel: number = this.consortiaInfo.getHighLevelByUpgradeType(this._info.type);
            let level: number = studyLevel < consortiaLevel ? studyLevel : consortiaLevel;
            if(level == 0){
                return TempleteManager.Instance.getConsortiaTempleteByTypeAndLevel(this._info.type, level+1);
            }
            else{
                return TempleteManager.Instance.getConsortiaTempleteByTypeAndLevel(this._info.type, level);
            }
        }else{
            let studyLevel: number = PlayerManager.Instance.currentPlayerModel.getConsortiaSkillLevel(this._info.type);
            let consortiaLevel: number = this.consortiaInfo.getLevelByUpgradeType(this._info.type);
