...
export class OpenBagTipView extends TaskTraceTipWnd {
	...
	initView() {
		...
		if(this._data && this._data.goods && FashionManager.Instance.fashionModel.isFashion(this._data.goods)){
			this.setBtnTitle(LangManager.Instance.GetTranslation("tasktracetip.view.OpenBagTipView.btnTxt1"))
		}
		else{
			this.setBtnTitle(LangManager.Instance.GetTranslation("tasktracetip.view.OpenBagTipView.btnTxt"));
		}
	...
	protected __btnHandler(evt) {
		...
		if(FashionManager.Instance.fashionModel.isFashion(this._data.goods)){
			FrameCtrlManager.Instance.open(EmWindow.SRoleWnd,{openFashionSwall:true});
			return;
		}
		this._goodsInfo = GoodsManager.Instance.getGoodsByGoodsIdFromGeneralBag(this._data.goods.id);
		...
		if (this._goodsInfo) {
			let heroBag: GoodsInfo[] = GoodsManager.Instance.getHeroGoodsListByTypeAndId(this._goodsInfo.templateInfo.MasterType, this.thane.id).getList();
			let pos_arr: number[] = GoodsSonType.getSonTypePos(this._goodsInfo.templateInfo.SonType);
			...
			if (pos_arr.length > 1) {
				for (const key in heroBag) {
					if (heroBag.hasOwnProperty(key)) {
						...
						if ((i.templateInfo.SonType == this._goodsInfo.templateInfo.SonType) && (i.pos == pos_arr[0] + t_index)) {
							t_index++;
						}
					}
				}
			...
			if (this._goodsInfo.templateInfo.MasterType == GoodsType.HONER) {
				...
				this.moveBagToBag(this._goodsInfo.bagType, this._goodsInfo.objectId, this._goodsInfo.pos, BagType.Honer, this.thane.id, this._toPos, 1);
			}
			else {
				if (this._toPos != -1) {
					...
					if (!goodsInfo.existJewel()) {
						this.moveBagToBag(this._goodsInfo.bagType, this._goodsInfo.objectId, this._goodsInfo.pos, BagType.HeroEquipment, this.thane.id, this._toPos, 1);
					}
					else {//有宝石
	...
	private moveConfirm(b: boolean, flag: boolean) {
		if (b) {//确定替换
			this.moveBagToBag(this._goodsInfo.bagType, this._goodsInfo.objectId, this._goodsInfo.pos, BagType.HeroEquipment, this.thane.id, this._toPos, 1, true);
		}
		else {
			this.moveBagToBag(this._goodsInfo.bagType, this._goodsInfo.objectId, this._goodsInfo.pos, BagType.HeroEquipment, this.thane.id, this._toPos, 1, false);
		}
	}
	...
	private moveBagToBag(beginBagType: number, beginObjectId: number, beginPos: number, endBagType: number, endObjectId: number, endPos: number, count: number, isUninstall: boolean = false) {
		PlayerManager.Instance.currentPlayerModel.openBagTipFlag = true;
		PlayerManager.Instance.moveBagToBag(beginBagType, beginObjectId, beginPos, endBagType, endObjectId, endPos, count, isUninstall);
	}
