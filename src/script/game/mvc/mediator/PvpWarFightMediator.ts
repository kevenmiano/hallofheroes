...
export default class PvpWarFightMediator implements IMediator, IEnterFrame {
  ...
  private __onClickHandler(evt: MouseEvent) {
    let armyView: object = CampaignManager.Instance.controller.getArmyView(
      this._targetInfo,
    );
  ...
  public enterFrame() {
    ...
    let selfView: any = CampaignManager.Instance.controller.getArmyView(
      this._selfInfo,
    );
    if (this._targetInfo && armyView) {
      if (
        this._targetInfo.online &&
      ...
      if (leng < 10000 && this._targetInfo.online) {
        //正常
        this.sendAttack();
        this.unLockWarFight();
  ...
  protected sendAttack() {
    ...
    if (WorldBossHelper.checkPvp(mapId)) {
      MapSocketOuterManager.sendAttackInPvpMap(
        this._targetInfo.userId,
        this._targetInfo.baseHero.serviceName,
      );
    }
    if (
  ...
  private searchPath(
    ...
    let start: Laya.Point =
      CampaignManager.Instance.mapModel.getAroundWalkPoint(startX, startY);
    if (!start) {
      return;
    }
    ...
    let arr: Array<any> = SearchPathHelper.searchPath(
