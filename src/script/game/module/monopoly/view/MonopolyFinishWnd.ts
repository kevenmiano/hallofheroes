...
export default class MonopolyFinishWnd extends BaseWindow {
  ...
  public OnInitWind() {
    ...
    this._itemInfos = [];
    this._rewardView1 = this.columns.getChild(
      "column1",
    ) as MonopolyFinishColumnView;
    this._rewardView2 = this.columns.getChild(
      "column2",
    ) as MonopolyFinishColumnView;
    this._rewardView3 = this.columns.getChild(
      "column3",
    ) as MonopolyFinishColumnView;
    this.initLanguage();
    this.addEvent();
  ...
  private initLanguage() {
    this.txt_title.text =
      LangManager.Instance.GetTranslation("monopoly.suceess");
    this.btn_start.title =
      LangManager.Instance.GetTranslation("monopoly.start");
    this.txt_first.text = LangManager.Instance.GetTranslation(
      "freeSlotmachineCount",
    );
  }
  ...
  private __infoChangeHandler(isBuyMagic): void {
    this.contentPane.getControllerAt(0).selectedIndex =
      this._model.slotMachineTimes > 0 ? 1 : 0;
    // this.updateBtnState();
    if (isBuyMagic) return;
    let tempMsg: TempMsg;
    let itemInfo1: MonopolyItemInfo = this._model
      .itemInfos[0] as MonopolyItemInfo;
    let itemInfo2: MonopolyItemInfo = this._model
      .itemInfos[1] as MonopolyItemInfo;
    let itemInfo3: MonopolyItemInfo = this._model
      .itemInfos[2] as MonopolyItemInfo;
    for (let i: number = 0; i < this._itemInfos.length; i++) {
      tempMsg = this._itemInfos[i] as TempMsg;
      if (
        this._rewardIndex1 == -1 &&
        itemInfo1 &&
        tempMsg.templateId == itemInfo1.templateId &&
        tempMsg.count == itemInfo1.count
      ) {
      ...
      if (
        ...
        itemInfo2 &&
        tempMsg.templateId == itemInfo2.templateId &&
        tempMsg.count == itemInfo2.count
      ) {
      ...
      if (
        ...
        itemInfo3 &&
        tempMsg.templateId == itemInfo3.templateId &&
        tempMsg.count == itemInfo3.count
      ) {
  ...
  private handleQuickCallBack(): void {
    ...
    if (this._rewardIndex1 != -1) {
      this._playMax++;
      this._rewardView1.start(
        this.rewardPlayCallBack.bind(this),
        this._rewardIndex1,
        10,
      );
      this._rewardIndex1 = -1;
  ...
  private showSlotRewardMsg(): void {
    ...
    this._model.itemInfos.forEach((itemInfo) => {
      goodsTempInfo = TempleteManager.Instance.getGoodsTemplatesByTempleteId(
        itemInfo.templateId,
      );
      if (goodsTempInfo) {
        ...
        content += LangManager.Instance.GetTranslation(
          "MonopolyManager.BattleGetTip02",
          goodsTempInfo.TemplateNameLang,
          itemInfo.count,
        );
      }
    });
    ...
    str = LangManager.Instance.GetTranslation(
      "MonopolyManager.BattleGetTip01",
      content,
    );
    ...
    if (this._model.isMail) {
      str = LangManager.Instance.GetTranslation(
        "vip.view.VipBoxFrame.command03",
      );
      MessageTipManager.Instance.show(str);
    }
  }
