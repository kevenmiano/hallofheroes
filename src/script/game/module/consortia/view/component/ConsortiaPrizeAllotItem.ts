...
export default class ConsortiaPrizeAllotItem extends FUI_ConsortiaPrizeAllotItem {
    ...
    private _tips2:string = LangManager.Instance.GetTranslation("consortia.view.myConsortia.building.ConsortiaPrizeAllotItem.tips2");
    private _tips1:string = LangManager.Instance.GetTranslation("consortia.view.myConsortia.building.ConsortiaPrizeAllotItem.tips1");
    ...
    public set info(value: ThaneInfo) {
        ...
        if (this._info) {
            ...
            this.LevelTxt.text = LangManager.Instance.GetTranslation("public.level3", this._info.grades.toString());
    ...
    private stepperChangeHandler() {
        if (this.numStep.value == 0 && this.numStep.value == this.info.receivedCount) {
            return;
        }
        var max: number = this.model.currentPrizeListCount + this.info.receivedCount;
        ...
        this.model.currentPrizeListCount = this.model.currentPrizeListCount + this._lastCount - this.count;
    }
