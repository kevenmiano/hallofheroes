...
export default class YMWebManager extends GameEventDispatcher {
    ...
    private get isNoneVoice(): boolean {
        return Utils.isQQHall() || Utils.isQQMobile() || Utils.isQQZone() || Utils.isWxMiniGame();
    }
    ...
    async setup() {
        ...
        this.loadymlib().then(() => {
            ...
            //@ts-ignore
            VoiceMessage.initRecorder().then(() => {
    ...
    private onLoginOK() {
        ...
        if (consortiaID > 0) {
            ...
            this.yim.joinRoom(roomID).then(function (e) {
    ...
    exitConsortiaRoom(roomid: string) {
        ...
        let roomId = YM_DEF.CONSORTIA_ROOM + roomid + this.getMainSite();
        this.yim.leaveRoom(roomId).then(function (e) {
    ...
    * */
    ...
    addVoiceItem(eventName: string, msgObj) {
        ...
        Logger.log("添加语音消息addVoiceItem: isFromMe" + msgObj.isFromMe, 'getDuration:' + msgObj.message.getDuration(), "msgObj.time:", msgObj.time);

        //message:receive:group:room123
        if (this.msgHash[msgObj.serverId]) {//过滤浏览器兼容问题导致的会收到多条同样的消息
            return;
        }
        // 把 msgObj 存起来
        this.msgHash[msgObj.serverId] = msgObj;
        Logger.log('msgObj.serverId', msgObj.serverId, 'this.msgHash', this.msgHash, 'sender: ', msgObj.senderId);
        //解析为ChatData
