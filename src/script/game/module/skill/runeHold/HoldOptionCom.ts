...
export class HoldOptionCom extends FUI_HoldOptionlCom {
    ...
    onConstruct() {
        super.onConstruct();
        this.propertys = [this.s1, this.s2, this.s3, this.s4, this.s5, this.s6, this.s7]; 
    }
    ...
    public updateView(info: RuneHoleInfo) {
        ...
        if (!info.opened) {
            ...
            this.openTips.setVar("level", RuneHoleInfo.RuneHoleOpenLevel[info.holeId - 1]).flushVars();
            return;
        }
    ...
    private updateSkill(info: RuneHoleInfo) {
        ...
        this.txt_desc.text = LangManager.Instance.GetTranslation('runeGem.property',percent);

        let tipTxt:string = '';
        //属性雕刻状态分为: 未解锁、无、具体百分比
        this.skillLocked.visible = !info.skill;
        if(!info.checkOpenAllRune()){
            tipTxt = LangManager.Instance.GetTranslation('runeGem.carveAtrri',LangManager.Instance.GetTranslation('runeGem.lock'));
            tipTxt +=  '<br>'+LangManager.Instance.GetTranslation('runeGem.carveSkill',LangManager.Instance.GetTranslation('runeGem.lock'));
        }else{
            if(percent == 0){
                tipTxt = LangManager.Instance.GetTranslation('runeGem.carveAtrri',LangManager.Instance.GetTranslation('public.notHave'));
            }else{
                tipTxt= LangManager.Instance.GetTranslation('runeGem.property',percent);
            }
            if(info.skill == 0){
                tipTxt +=  '<br>'+LangManager.Instance.GetTranslation('runeGem.carveSkill',LangManager.Instance.GetTranslation('public.notHave'));
            }
        }
    ...
    private setPropertyValue(s: FUI_HolePropertyItem, property: string, value: number, bonus: number) {
        if (!s) return;
        s.visible = true;
        bonus = bonus * value >> 0
