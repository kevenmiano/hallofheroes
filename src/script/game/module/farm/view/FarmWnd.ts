...
export default class FarmWnd extends BaseWindow {
  ...
  private addEvent() {
    ServerDataManager.listen(
      S2CProtocol.U_C_CANWATER_USERS,
      this,
      this.__getFriendFarmStateHandler,
    );
    ...
    FarmManager.Instance.model.addEventListener(
      FarmEvent.SELECTED_CHANGE,
      this.__selectedFarmChangeHandler,
      this,
    );
    FriendManager.getInstance().addEventListener(
      FriendUpdateEvent.FRIEND_UPDATE,
      this.__friendUpdateHandler,
      this,
    );
  ...
  private delEvent() {
    ...
    FarmManager.Instance.model.removeEventListener(
      FarmEvent.SELECTED_CHANGE,
      this.__selectedFarmChangeHandler,
      this,
    );
  ...
  private initView() {
    ...
    this.landUpBtn.title = LangManager.Instance.GetTranslation(
      "farm.view.FarmBuildLayer.landUp",
    );
    ...
    FarmManager.Instance.model.defaultSelectedUserInfo =
      ArmyManager.Instance.thane;
    this.__selectedFarmChangeHandler(
      FarmManager.Instance.model.curSelectedFarmInfo,
    );
  ...
  updateFarmLandLevel() {
    this.landUpBtn.view.getChild("comTitle").asCom.getChild("txtLevel").text =
      this._curSelectedFarm.landGrade.toString();
  }
  ...
  private __onLandUpClick(e: Laya.Event) {
    if (this._curSelectedFarm) {
      ...
      // 	return;
      // }
      FrameCtrlManager.Instance.open(EmWindow.FarmLandUpWnd, {
        farmInfo: this._curSelectedFarm,
      });
    }
  }
  ...
  private __selectedFarmChangeHandler(data: FarmInfo) {
    ...
    this.landUpBtn.view.getChild("comTitle").visible =
      this.landUpBtn.touchable = isSelf;
    if (isSelf) {
  ...
  public refreshExp() {
    if (
      !this.expProgressBar ||
      this.expProgressBar.isDisposed ||
      !this.isShowing
    )
      return;
    ...
    if (upGrade) {
      ...
      _offlinePrecent = Number(
        ((ArmyManager.Instance.thane.gp +
          ArmyManager.Instance.thane.offlineGp) *
          100) /
          upGrade.Data,
      );
      msg = LangManager.Instance.GetTranslation(
        "mainBar.PlayerExperenceView.msg2",
        ArmyManager.Instance.thane.gp + " / " + upGrade.Data,
        _percent,
        ArmyManager.Instance.thane.offlineGp,
      );
    } else {
    ...
    for (let i = 0; i < FarmModel.LAND_NUM; i++) {
      ...
      if (item.canHandleOneKey) {
        ...
        if (cropTemp.Property2 == ItemID.GOLD_PROP) {
          // 拾取/偷取  黄金作物
          if (
            (item.info.curOper == FarmOperateType.PICK ||
              item.info.curOper == FarmOperateType.STEAL) &&
  ...
  private helpBtnClick() {
    let content: string = LangManager.Instance.GetTranslation(
      "farm.view.FarmResourcesView.helpContent2",
    );
  ...
  private get hasVipPrevilige() {
    return VIPManager.Instance.model.isOpenPrivilege(
      VipPrivilegeType.FRAM,
