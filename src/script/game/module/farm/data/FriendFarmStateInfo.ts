...
export default class FriendFarmStateInfo extends GameEventDispatcher {
  ...
  public get curState(): number {
    if (this.canSteal) return FarmOperateType.STEAL;
    if (this.canGivePower) return FarmOperateType.GIVE_POWER;
    if (this.canFeed) return FarmOperateType.PET_FEED;
    if (this.canRevive) return FarmOperateType.REVIVE;
    if (this.canWorm) return FarmOperateType.WORM;
    if (this.canWeed) return FarmOperateType.WEED;
