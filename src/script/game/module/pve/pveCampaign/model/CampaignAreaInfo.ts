...
export class CampaignAreaInfo {
  ...
  public getAreaImg(bItemIcon: boolean = false): string {
    ...
    if (campaignData) {
      return PathManager.solveSelectCampaignPath(
        campaignData.DungeonId,
        bItemIcon ? campaignData.AreaId : undefined,
      );
    }
    return "";
  }
  ...
  public getAreaSmallIcon(): string {
    return PathManager.mapInstanceSmallIconPath(
      this.getMapByDifficult(CampaignMapDifficulty.General).CampaignId,
    );
  }
  ...
  public getItemTitle(): string {
    if (this.maxLevel != this.minLevel) {
      return LangManager.Instance.GetTranslation(
        "selectcampaign.view.itemTitle1",
        this.areaName,
        this.minLevel,
        this.maxLevel,
      );
      ...
      return LangManager.Instance.GetTranslation(
        "selectcampaign.view.itemTitle2",
        this.areaName,
        this.maxLevel,
      );
    }
  ...
  public getRecommendLevel(): string {
    let str =
      LangManager.Instance.GetTranslation(
        "selectcampaign.view.recommendedLevel",
      ) + this.minLevel;
    if (this.maxLevel != this.minLevel) {
      str += "-" + this.maxLevel;
    }
