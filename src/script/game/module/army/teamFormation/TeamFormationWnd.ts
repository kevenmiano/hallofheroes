...
export default class TeamFormationWnd extends BaseWindow {
  ...
  OnShowWind() {
    ...
    this.txtFrameTitle.text = LangManager.Instance.GetTranslation(
      "teammanager.pve.TeamSortFrame.title",
    );
    ...
    for (let i: number = 0; i < 9; i++) {
      ...
      if (i % 3 == 0) {
        ...
        item.getChild("title").text = LangManager.Instance.GetTranslation(
          "TeamFormationWnd.title1",
        );
      }
      if (i % 3 == 1) {
        // item.backTxt.setFrame(2);
        item.getChild("title").text = LangManager.Instance.GetTranslation(
          "TeamFormationWnd.title3",
        );
    ...
    NotificationManager.Instance.on(
      RoomEvent.EDIT_TEAM_FIGHT_POS,
      this.__editTeamFightPos,
      this,
    );
  }
  ...
  private __updateView(armyFightPosList: ArmyFightPosEditMsg[]) {
    for (let i = 0; i < this._itemList.keys.length; i++) {
      ...
      for (let index = 0; index < armyFightPosList.length; index++) {
        ...
        if (this.roomInfo.isCross) {
          armyInfo = this.roomInfo.getPlayerByUserId(
            msg.userId,
            msg.serverName,
          );
  ...
  private __teamInfoUpdateHandler() {
    if (FreedomTeamManager.Instance.hasTeam) {
      this.clean();
      var armyFightPosList: Array<any> =
        FreedomTeamManager.Instance.model.armyFightPos;
      for (let i: number = 0; i < armyFightPosList.length; i++) {
        var msg: MemberFightPos = armyFightPosList[i];
        var armyInfo: BaseArmy =
          FreedomTeamManager.Instance.model.getMemberByUserId(msg.memberId);
