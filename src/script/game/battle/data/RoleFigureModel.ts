    ...
    public static initRoleFigure(roleList: Map<number, any>) {
        roleList.forEach(roleInfo => {
            if (roleInfo.inheritType == InheritRoleType.Hero) {
                if ((roleInfo).type != 3) {//type是3的为boss, 其它是英雄  正常的情况下是null  即继续执行
                    ...
                    if (pet && pet.livingId == roleInfo.livingId) {
                        ...
                        if (roleInfo.templateId == 100) {//神秘人
                            let data: GameLoadNeedData = HeroLoadDataFactory.create((roleInfo as HeroRoleInfo).heroInfo, HeroLoadDataFactory.PART_BODY);
                            RoleFigureModel.loadList.push({ url: data.urlPath, type: Laya.Loader.ATLAS })
                        } else {
                            HeroLoadDataFactory.Part_List.forEach(part => {
                                let data: GameLoadNeedData = HeroLoadDataFactory.create((roleInfo as HeroRoleInfo).heroInfo, part);
                                RoleFigureModel.loadList.push({ url: data.urlPath, type: Laya.Loader.ATLAS })
                            });
                        }
                    }
                } else {//boss
                    let path = PathManager.solveRolePath((roleInfo as HeroRoleInfo).resPath)
                ...
                RoleFigureModel.loadList.push({ url: path, type: Laya.Loader.ATLAS })
            } else if (roleInfo.inheritType == InheritRoleType.Pet) { //TODO

            }
        });
        RoleFigureModel.loadList = ArrayUtils.unique(RoleFigureModel.loadList, "url")
