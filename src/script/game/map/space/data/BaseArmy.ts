...
export class BaseArmy extends GameEventDispatcher {
  ...
  public synchronizationAllPawn(ap: ArmyPawn) {
    for (let armyPawnsKey in this.armyPawns) {
      ...
      if (
        armyPawn.templateInfo &&
        armyPawn.templateInfo.SonType == ap.templateInfo.SonType
      ) {
  ...
  public get heroRace(): number {
    ...
    return (
      this.baseHero.attackProrerty.totalAttribute +
      this.baseHero.attackProrerty.totalLive / 5 +
      this.baseHero.attackProrerty.totalConatArmy
    );
  }
