...
export default class RingTaskRewardWnd extends BaseWindow {
    ...
    private rewardList = ["RewardPlayGP", "RewardGold", "RewardConsortiaOffer", "RewardPlayerOffer"];
        ...
        if (ringtaskInfos) {
            ...
            if (taskInfo && taskInfo.ringTaskTemp) {
                ...
                for (var i: number = 0; i < this.rewardList.length; i++) {
                    if (this.rightReward.hasOwnProperty(this.rewardList[i]) && parseInt(this.rightReward[this.rewardList[i]].toString()) > 0) {
                        let currentReward: RingTaskRewardInfo = new RingTaskRewardInfo();
                        currentReward.index = i + 1;
                        currentReward.count = parseInt((this.rightReward[this.rewardList[i]]).toString());
                        this.rewardDataList.push(currentReward);
                    }
                }
            }
            this.list1.numItems = this.rewardDataList.length;
            var goodList: any[] = RingTaskManager.Instance.getAllRingTaskGoodsReward();
            this.rewardGoodList = [];
