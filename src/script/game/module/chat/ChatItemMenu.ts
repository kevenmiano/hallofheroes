...
import { ChatEvent, FriendUpdateEvent, NotificationEvent, RequestInfoEvent } from '../../constant/event/NotificationEvent';
...
export default class ChatItemMenu extends BaseWindow {
    ...
    private btnTextKeys = ["ChatItemMenu_Chat", "ChatItemMenu_ADDFRI", "ChatItemMenu_COPY",
         "ChatItemMenu_MESSAGE","ChatItemMenu_INVITE","ChatItemMenu_CONSORTIA","ChatItemMenu_BLACK", 
          "ChatItemMenu_DELFRI","ChatItemMenu_ATTACK"];
    private btnTextStrs = {
        ChatItemMenu_Chat: LangManager.Instance.GetTranslation("chatII.view.ChatItemMenu.chatMessageText"),
        ChatItemMenu_ADDFRI: LangManager.Instance.GetTranslation("chatII.view.ChatItemMenu.addfriendText"),
        ChatItemMenu_COPY: LangManager.Instance.GetTranslation("chatII.view.ChatItemMenu.copyNametext"),
        ChatItemMenu_MESSAGE: LangManager.Instance.GetTranslation("chatII.view.ChatItemMenu.messageText"),
        ChatItemMenu_INVITE: LangManager.Instance.GetTranslation("yishi.view.PlayerMenu.invite"),
        ChatItemMenu_CONSORTIA: LangManager.Instance.GetTranslation("chatII.view.ChatItemMenu.consortiaBtnText"),
        ChatItemMenu_BLACK: LangManager.Instance.GetTranslation("chatII.view.ChatItemMenu.blacklistText"),
        ChatItemMenu_DELFRI: LangManager.Instance.GetTranslation("friends.im.IMFrame.delFriend"),//删除好友 添加好友这两个按钮同时只能显示一个, 并且位置是在一起的
        ChatItemMenu_ATTACK: LangManager.Instance.GetTranslation("ChatItemMenu.attack")
    };
     ...
     *
     */
    public static Show(name: string, id: number, showConsortia: boolean, $serverName: string = null, $showAttack: boolean = false, $isRobot: boolean = false, $showInvite: boolean = false, point?: Laya.Point, $thaneInfo?: ThaneInfo) {
        let obj = {
            name: name,
            id: id,
    ...
    public OnInitWind() {
        this.list.itemRenderer = Laya.Handler.create(this, this.renderFaceListItem, null, false);
        ...
        if (this._userId == PlayerManager.Instance.currentPlayerModel.playerInfo.userId) {
        ...
        if (SceneManager.Instance.currentType == SceneType.BATTLE_SCENE) {
            ...
            if (this.contentPane.y + this.contentPane.height > Resolution.gameHeight) {//超出屏幕
    ...
    private setBtnState() {
        if (DataCommonManager.playerInfo.userId == this._userId) {
            this.list.getChildAt(ChatItemMenu.ChatItemMenu_ADDFRI).enabled = this.list.getChildAt(ChatItemMenu.ChatItemMenu_BLACK).enabled = this.list.getChildAt(ChatItemMenu.ChatItemMenu_DELFRI).enabled = false;
            this.list.removeChildAt(ChatItemMenu.ChatItemMenu_BLACK);
            return;
        }
        var fInfo: FriendItemCellInfo = IMManager.Instance.getFriendInfo(this._userId);
        if (fInfo) {
            switch (fInfo.relation) {
                case RelationType.NONE://
                case RelationType.RECENT_CONTACT://
                    this.list.getChildAt(ChatItemMenu.ChatItemMenu_DELFRI).enabled = false;
                    break;
                case RelationType.FRIEND://删除好友
                    this.list.getChildAt(ChatItemMenu.ChatItemMenu_DELFRI).enabled = true;
                    this.list.getChildAt(ChatItemMenu.ChatItemMenu_ADDFRI).enabled = false;
                    break;
                case RelationType.STRANGER:
                    this.list.getChildAt(ChatItemMenu.ChatItemMenu_ADDFRI).enabled = false;//添加好友//陌生人列表中添加好友图标应置灰
        ...
        if (!this.sameServiceName()) {
            ...
            if (this.isSelf()) {
                ...
                this.list.getChildAt(ChatItemMenu.ChatItemMenu_ADDFRI).enabled = false;//添加好友
                this.list.getChildAt(ChatItemMenu.ChatItemMenu_BLACK).enabled = false;//黑名单
                this.list.getChildAt(ChatItemMenu.ChatItemMenu_CONSORTIA).enabled = false;//公会
        ...
        //PK  仅限天空之城
        if (SceneManager.Instance.currentType != SceneType.SPACE_SCENE || !this._showAttack) {//切磋
        ...
        if (wnd && wnd.isShowing) {
            if (wnd.chatTablist.selectedIndex == ChatItemMenu.ChatItemMenu_CONSORTIA) {
    ...
    private sameServiceName(): boolean {
        if (this._serverName) {
            return this._serverName == PlayerManager.Instance.currentPlayerModel.playerInfo.serviceName;
        }
        return true;
    }

    private sameUserId(): boolean {
        return this._userId == PlayerManager.Instance.currentPlayerModel.playerInfo.userId;
    }
    ...
    onRemoveEvent() {
        ...
        PlayerManager.Instance.removeEventListener(RequestInfoEvent.REQUEST_SIMPLEINFO, this.__recentContactHandler, this);
    }
    ...
    private __chatmessageHandler() {
        if (ArmyManager.Instance.thane.grades < 6) {
            var str: string = LangManager.Instance.GetTranslation("chat.view.ChatItemMenu.command01");
            MessageTipManager.Instance.show(str);
            this.hide();
            return;
        }
        if (this.isSelf()) {
            var str: string = LangManager.Instance.GetTranslation("chat.view.ChatInputView.content01");
            MessageTipManager.Instance.show(str);
            this.hide();
            return;
        }
        if (!FriendManager.getInstance().checkIsFriend(this._userId)) {
            MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("chat.notfriend02"));
            return;
        }
        PlayerManager.Instance.addEventListener(RequestInfoEvent.REQUEST_SIMPLEANDSNS_INFO, this.getSnsInfo, this);
        PlayerManager.Instance.sendRequestSimpleAndSnsInfo(this._userId, RequestInfoRientation.RECENT_CONTACT);
    }

    private getSnsInfo(orientation: number, pInfo: FriendItemCellInfo) {
        PlayerManager.Instance.removeEventListener(RequestInfoEvent.REQUEST_SIMPLEANDSNS_INFO, this.getSnsInfo, this);
        ...
        if (!FrameCtrlManager.Instance.isOpen(EmWindow.ChatWnd)) {
            FrameCtrlManager.Instance.open(EmWindow.ChatWnd, { thaneInfo: info, type: 1 });
        }
        else {
    ...
    private __addfriendHandler() {
        var str: string;
        if (ArmyManager.Instance.thane.grades < 6) {
            str = LangManager.Instance.GetTranslation("chat.view.ChatItemMenu.command01");
            MessageTipManager.Instance.show(str);
            this.hide();
            return;
        }
        if (this._isRobot) {
            str = LangManager.Instance.GetTranslation("chat.view.ChatItemMenu.RobotTips01");
            MessageTipManager.Instance.show(str);
            this.hide();
            return;
        }
        FriendManager.getInstance().sendAddFriendRequest(this._name, RelationType.FRIEND);
        this.hide();
    }
    ...
    private __deleteHandler() {
        ...
        if (this._thaneInfo) {
            if (this._thaneInfo.friendGrade >= 2) {
                var preDate: Date = new Date(SharedManager.Instance.deleteFriendCheckDate);
                ...
                if (!check || preDate.getMonth() <= preDate.getMonth() && preDate.getDay() < now.getDay())
                    outdate = true;
                if (outdate) {
                    var content: string = LangManager.Instance.GetTranslation("ChatItemMenu.deleteFriend.Alert", this._thaneInfo.friendGrade);
                    UIManager.Instance.ShowWind(EmWindow.TodayNotAlert, { content: content, backFunction: this.deleteFriendAlertBack.bind(this), closeFunction: null, state: 2 });
                }
                else {
    ...
    private __blacklistHandler() {
        ...
        if (this._isRobot) {
            str = LangManager.Instance.GetTranslation("chat.view.ChatItemMenu.RobotTips02");
            MessageTipManager.Instance.show(str);
            this.hide();
            return;
        }
        FriendManager.getInstance().sendAddFriendRequest(this._name, RelationType.BLACKLIST);
        this.hide();
    }
    ...
    private __messageHandler() {
        PlayerManager.Instance.addEventListener(RequestInfoEvent.REQUEST_SIMPLEINFO, this.__recentContactHandler, this);
        if (this._serverName && this._serverName != PlayerManager.Instance.currentPlayerModel.playerInfo.serviceName) {
            PlayerInfoManager.Instance.sendRequestSimpleInfoCross(this._userId, this._serverName);
        }
        else {
    ...
    private __pkBtnClickHandler() {
        if (SceneManager.Instance.currentType == SceneType.SPACE_SCENE) {
            var armyInfo: SpaceArmy = SpaceManager.Instance.model.getUserArmyByUserId(this._userId);
            if (!armyInfo) {
                this.hide();
                return;
            }
            if (ArmyState.checkCampaignAttack(armyInfo.state)) {
                ...
                SpaceSocketOutManager.Instance.sendMapPkInviteTo(armyInfo.baseHero.userId);
            } else {
                //正在战斗不能攻击
                MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("playermenu.command01"));
            }
        }
        this.hide();
    }
    ...
    private __recentContactHandler(data1: number, data2: ThaneInfo) {
        ...
        if (!(thane && thane.nickName)) {
            MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("ChatItemMenu.cannotSearchPlayerInfo"));
            return;
        }
        if (data1 == 10000) {
