...
export class KingContractManager
{
    ...
    private initEvent()
    {
        ServerDataManager.listen(S2CProtocol.U_C_KING_CONTRACTE, this, this.__updateKingContractHandler);
    }
    ...
    private __updateKingContractHandler(pkg:PackageIn)
    {
        ...
        for(let msgItem of infos)
        {
            ...
            if(info)
            {
                info.endData = DateFormatter.parse(msgItem.endData, "YYYY-MM-DD hh:mm:ss");
            }
        }
        NotificationManager.Instance.dispatchEvent(KingContractEvents.UPDATE_KINGCONTRACT, null);
    }
