...
export default class UIQiangHua extends BaseFguiCom {
  ...
  private onBtnConfirm() {
    ...
    if (ForgeData.getSuccessRate(info, this.equip2.info) <= 50) {
      //成功率<= 50%就弹确认框
      let isStrengthen: boolean = SharedManager.Instance.getProperty(
        SharedManager.IS_STRENGTHEN,
      );
      let strengthenCheckDate = SharedManager.Instance.getProperty(
        SharedManager.STRENGTHEN_CHECK_DATE,
      );
      ...
      if (
        !isStrengthen ||
        (strengthenDate &&
          strengthenDate.getMonth() <= now.getMonth() &&
          strengthenDate.getDate() < now.getDate())
      ) {
        outdate = true;
      }
      if (outdate) {
        let content: string = LangManager.Instance.GetTranslation(
          "store.Intensify.IntensifyView.isStrengthen",
        );
        ...
        SimpleAlertHelper.Instance.Show(
          ...
          null,
          null,
          this.strengthenAndSaveSO.bind(this),
        );
      } //没过期, 直接强化
      else {
        this.strengthen();
  ...
  private strengthen() {
    ...
    if (
      ...
      Number(StringHelper.trim(this.txtCost.text))
    ) {
  ...
  protected __bagItemUpdateHandler(info: GoodsInfo) {
    Logger.log(
      "[UIQiangHua]__bagItemUpdateHandler",
  ...
  private __bagItemDeleteHandler(info: GoodsInfo) {
    Logger.log(
      "[UIQiangHua]__bagItemDeleteHandler",
      info.pos + "_" + info.objectId + "_" + info.bagType,
    );
    let item: StoreIntensifyCell =
      this._itemDic[info.pos + "_" + info.objectId + "_" + info.bagType];
  ...
  protected __onIntensifyResult(pkg: PackageIn) {
    ...
    let strTip = LangManager.Instance.GetTranslation(
      msg.strengResult
        ? "store.Intensify.IntensifyView.successWord.text"
        : "store.Intensify.IntensifyView.failWord.text",
    );
    MessageTipManager.Instance.show(strTip);

    AudioManager.Instance.playSound(
      msg.strengResult
        ? SoundIds.INTENSIFY_SUCCEED_SOUND
        : SoundIds.INTENSIFY_FAIL_SOUND,
    );
  ...
  private updateView() {
    ...
    if (!info) {
      if (this.equip2.info) {
        PlayerManager.Instance.moveBagToBag(
          this.equip2.info.bagType,
          this.equip2.info.objectId,
          this.equip2.info.pos,
          BagType.Player,
          0,
          0,
          1,
    ...
    if (!bMaxGrade) {
      ...
      this.totalCost.text =
        ResourceManager.Instance.gold.count.toString() + " / ";
      this.txtSuccessRate.text =
        ForgeData.getSuccessRate(info, this.equip2.info) + "%";
      this.totalCost.color = this.txtCost.color = CommonUtils.changeColor(
        GoodsCheck.getIntensifuGold(info),
        ResourceManager.Instance.gold.count,
      );
    } else {
      /**
    ...
    this.txtName.text =
      ...
      this.ctrl.data.getStrengthenGrade(info);
    ...
    let strEquipLevel: string = LangManager.Instance.GetTranslation(
      "gemMaze.GemMazeSort.Level",
    );
    this.updateItem(
      this.item01,
      strEquipLevel,
      bMaxGrade ? "" : String(curGrade),
      bMaxGrade ? String(curGrade) : String(curGrade + 1),
    );
  ...
  private initAttribute(info: GoodsInfo, temp: t_s_itemtemplateData) {
    ...
    this.updateAttributeTxt(
      str,
      temp.Power,
      this.getCurr(temp.Power, info),
      this.getNex(temp.Power, info),
      bMaxGrade,
    );
    ...
    this.updateAttributeTxt(
      str,
      temp.Agility,
      this.getCurr(temp.Agility, info),
      this.getNex(temp.Agility, info),
      bMaxGrade,
    );
    ...
    this.updateAttributeTxt(
      str,
      temp.Intellect,
      this.getCurr(temp.Intellect, info),
      this.getNex(temp.Intellect, info),
      bMaxGrade,
    );
    ...
    this.updateAttributeTxt(
      str,
      temp.Physique,
      this.getCurr(temp.Physique, info),
      this.getNex(temp.Physique, info),
      bMaxGrade,
    );
    ...
    this.updateAttributeTxt(
      str,
      temp.Captain,
      this.getCurr(temp.Captain, info),
      this.getNex(temp.Captain, info),
      bMaxGrade,
    );
    ...
    this.updateAttributeTxt(
      str,
      temp.Attack,
      this.getCurr(temp.Attack, info),
      this.getNex(temp.Attack, info),
      bMaxGrade,
    );
    ...
    this.updateAttributeTxt(
      str,
      temp.MagicAttack,
      this.getCurr(temp.MagicAttack, info),
      this.getNex(temp.MagicAttack, info),
      bMaxGrade,
    );
    ...
    this.updateAttributeTxt(
      str,
      temp.Defence,
      this.getCurr(temp.Defence, info),
      this.getNex(temp.Defence, info),
      bMaxGrade,
    );
    ...
    this.updateAttributeTxt(
      str,
      temp.MagicDefence,
      this.getCurr(temp.MagicDefence, info),
      this.getNex(temp.MagicDefence, info),
      bMaxGrade,
    );
    ...
    this.updateAttributeTxt(
      str,
      temp.Parry,
      this.getCurr(temp.Parry, info),
      this.getNex(temp.Parry, info),
      bMaxGrade,
    );
    ...
    this.updateAttributeTxt(
      str,
      temp.Live,
      this.getCurr(temp.Live, info),
      this.getNex(temp.Live, info),
      bMaxGrade,
    );
    ...
    this.updateAttributeTxt(
      str,
      temp.ForceHit,
      this.getCurr(temp.ForceHit, info),
      this.getNex(temp.ForceHit, info),
      bMaxGrade,
    );
    ...
    this.updateAttributeTxt(
      str,
      temp.Conat,
      this.getCurr(temp.Conat, info),
      this.getNex(temp.Conat, info),
      bMaxGrade,
    );
  }
  ...
  private getCurr(preValue: number, gInfo: GoodsInfo): number {
    let curr: number =
      preValue * gInfo.strengthenGrade * 0.1 + gInfo.strengthenGrade * 5;
  ...
  private getNex(preValue: number, gInfo: GoodsInfo): number {
    ...
    let nex: number =
      preValue * info.strengthenGrade * 0.1 + info.strengthenGrade * 5;
    let curr: number =
      preValue * gInfo.strengthenGrade * 0.1 + gInfo.strengthenGrade * 5;
