...
import { CampaignMapEvent, PhysicsEvent, RoomEvent } from "../../../constant/event/NotificationEvent";
...
export class CampaignArmy extends BaseArmy {
    ...
    public setTranseferWait(value: boolean, nodeId: number, mapId: number) {
        ...
        if (nodeId != 0) {
            this.dispatchEvent(CampaignMapEvent.TRANSEFER_WAIT_STATE, { state: value, nodeId: nodeId, mapId: mapId });
        }
    }
    ...
    public get mapTempInfo(): t_s_mapData {
        return ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_map, String(this.mapId));
    }
    ...
    public syncArmyInfo() {
        ...
        if (id == this.userId) {
            ...
            if (this.baseHero) {
                ...
                else if (this.baseHero.serviceName == PlayerManager.Instance.currentPlayerModel.playerInfo.serviceName) {
                    if (army) {
                        army.state = this.state;
                    }
                }
