...
export default class AllocateCtrl extends FrameCtrlBase {
  ...
  public setOldData() {
    if (!this.model) return;
    this.model.oldEquip = GoodsManager.Instance.getHeroEquipListById(
      this.model.uArmy.baseHero.id,
    );
    var pos1Pawn: ArmyPawn = this.model.uArmy.getPawnByIndex(0);
  ...
  public sendRecruitPawn(
    type: number,
    recruitPawn: ArmyPawn,
    recruitNum: number,
  ...
  private __recievePawnListHandler() {
    ...
    if (this._recruitPawn) {
      Logger.info(
        "[AllocateCtrl]招募后自动上阵",
        this._type,
        this._recruitPawn.templateInfo.PawnNameLang,
        this._staffingCount,
        this._dstId,
        this._dstCount,
      );
  ...
  public staffingNewPawn(srcAp: ArmyPawn, srcCount: number) {
    Logger.info(
      "[AllocateCtrl]上阵新士兵:",
      srcAp.templateInfo.PawnNameLang,
      srcAp.templateId,
      srcCount,
    );
  ...
  private __showStopRecruitTip() {
    MessageTipManager.Instance.show(
      LangManager.Instance.GetTranslation("Allocate.GoldNotEnough2"),
    );
  }
  ...
  private __showStopRecruitTip2() {
    MessageTipManager.Instance.show(
      LangManager.Instance.GetTranslation("Allocate.PeopleNotEnough"),
    );
  }
