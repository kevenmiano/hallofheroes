...
export default class PveMultiCampaignWnd extends BaseWindow {
  ...
  public OnInitWind() {
    ...
    this._strEnterCount = LangManager.Instance.GetTranslation(
      "PveSelectCampaignWnd.enterCountTxt",
    );
    this._strTailaEnterCount = LangManager.Instance.GetTranslation(
      "PveSelectCampaignWnd.tailaWeekEnterCountTxt",
    );
  ...
  public defaultView() {
    ...
    if (this.frameData && this.frameData.CampaignId) {
      ...
      let openCampaignDataDic = this.model
        .openCampaignDataDic as t_s_campaignData[];
      for (const key in openCampaignDataDic) {
        if (Object.prototype.hasOwnProperty.call(openCampaignDataDic, key)) {
          ...
          if (this.model.thane.grades >= temp.MinLevel && !temp.isTaila) {
            if (
              !cfgData ||
              cfgData.MinLevel < temp.MinLevel ||
              (cfgData.MinLevel == temp.MinLevel &&
                cfgData.CampaignId < temp.CampaignId)
            ) {
              cfgData = temp;
            }
          }
    ...
    let defSelectedCp = this.model.selectedLand.getChapterById(
      fTreeNodeTar.data.id,
    );
    ...
    if (this.frameData) {
      ...
      let nameStr = PveMultiCampaignData.getDifficultyById(
        this.frameData.difficult,
      );
  ...
  private btnChangeFBClick() {
    ...
    let campaignId =
      this.model.selectedArea.getMapByDifficult(difficulty).CampaignId;
    NotificationManager.Instance.dispatchEvent(
      RoomHallEvent.CHANGE_CAMPAIGN,
      this.model.selectedChapter.chapterId,
      campaignId,
    );
  }
  ...
  private selectCampaignFilter() {
    ...
    if (
      this.model.thane.grades <
      this.model.selectedArea.getMapByDifficult(difficulty).MinLevel
    ) {
      str = LangManager.Instance.GetTranslation(
        "room.view.pve.instance.InstanceRightView.command03",
      );
      MessageTipManager.Instance.show(str);
      return true;
    }

    let camTemp: t_s_campaignData =
      this.model.selectedArea.getMapByDifficult(difficulty);
    if (camTemp.SonTypes != 0) {
      var isOver: boolean;
      if (camTemp.isKingTower) {
        //王者之塔次数判断
        isOver =
          KingTowerManager.Instance.kingTowerInfo.isKingTowerOverMaxCount;
        if (isOver) {
          str = LangManager.Instance.GetTranslation(
            "yishi.view.base.ThewAlertFrame.disclist06",
          );
          MessageTipManager.Instance.show(str);
          return true;
        }
  ...
  private refreshRight(
    ...
    let isKingTower = campaignAreaInfo.getMapByDifficult(
      CampaignMapDifficulty.General,
    ).isKingTower;
    let isYuanShu = campaignAreaInfo.getMapByDifficult(
      CampaignMapDifficulty.General,
    ).isYuanShu;
    ...
    if (isKingTower) {
      ...
      // 没英雄副本的不显示英雄按钮
      let hasHeroMap = campaignAreaInfo.getMapByDifficult(
        CampaignMapDifficulty.Hero,
      );
      if (!hasHeroMap) {
        modeCnt = 1;
        this.noModeBtn = true;
    ...
    if (!this.noModeBtn) {
      ...
      for (let index = 0; index < modeCnt; index++) {
        ...
        if (isKingTower) {
          ...
          if (index == 2) {
            notNeedFilterFlag =
              this.model.isHellOpen() &&
              this.model.heroMap.state == CampaignMapStatus.OVER_CAMPAIGN;
          }
        } else {
        ...
        notNeedFilterFlag =
          notNeedFilterFlag &&
          campaignAreaInfo.minLevel <= ArmyManager.Instance.thane.grades;
  ...
  private updateBtnStatus(noModeBtn: boolean) {
    let isKingTower = this.model.selectedArea.getMapByDifficult(
      CampaignMapDifficulty.General,
    ).isKingTower;
    let isYuanShu = this.model.selectedArea.getMapByDifficult(
      CampaignMapDifficulty.General,
    ).isYuanShu;
    let kingTowerMaxIndex = KingTowerManager.Instance.kingTowerInfo.maxIndex;
    ...
    if (this.cBtnState.selectedIndex == 0) {
      if (noModeBtn) {
        if (
          this.model.selectCampaign.MinLevel <=
      ...
      if (
        ...
        if (this.noModeBtn) {
          ...
          if (difficulty == 0) {
            ...
            difficulty == 1 &&
            (this.model.heroMap.state == CampaignMapStatus.NEW_CAMPAIGN ||
              this.model.heroMap.state == CampaignMapStatus.OPEN_CAMPAIGN)
          ) {
            this.pass.selectedIndex = 0;
          } else if (
            difficulty == 2 &&
            (this.model.hellMap.state == CampaignMapStatus.NEW_CAMPAIGN ||
              this.model.hellMap.state == CampaignMapStatus.OPEN_CAMPAIGN)
          ) {
            this.pass.selectedIndex = 0;
          }
        }
