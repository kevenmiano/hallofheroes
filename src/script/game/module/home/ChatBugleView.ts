...
export default class ChatBugleView extends FUI_HomeBigBugble {
    ...
    onClickName() {
        ...
        if(!chatData) return;
        if (chatData.uid == PlayerManager.Instance.currentPlayerModel.playerInfo.userId) {
            return;
        }
        ...
        this.showUseMenu(Laya.stage.mouseX, Laya.stage.mouseY, chatData.senderName, chatData.uid, showConsortia);
    }
    ...
    private showUseMenu(menuX: number, menuY: number, name: string, id: number, showConsortia: boolean, servername: string = null) {
        ...
        ChatItemMenu.Show(name, id, showConsortia, servername, false, false, true, point);
    }

    private get bigBugleBuffer(): Array<ChatData> {
        return ChatManager.Instance.model && ChatManager.Instance.model.bigBugleList;
    }
    ...
    autoHide() {
        let self = this;
        Laya.Tween.to(this, { alpha: 0 }, 150, undefined, Laya.Handler.create(this, () => {
            self.visible = false;
            self.alpha = 1;
            self.update();
        }));
    ...
    private update() {
        ...
        NotificationManager.Instance.dispatchEvent(ChatEvent.UPDATE_CHAT_BUGLE_VIEW_VISIBLE);
    }
