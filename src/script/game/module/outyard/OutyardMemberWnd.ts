...
export default class OutyardMemberWnd extends BaseWindow {
    ...
    private addEvent() {
        ...
        this.list2.itemRenderer = Laya.Handler.create(this, this.renderList2Item, null, false);
        this.typeCtr.on(fairygui.Events.STATE_CHANGED, this, this.onChangeController);
        this.tabCtr.on(fairygui.Events.STATE_CHANGED, this, this.onTabChangeController)
        NotificationManager.Instance.addEventListener(NotificationEvent.OUTYARD_USERLIST_INFO, this.onChangeController, this);
    }
    ...
    private removeEvent() {
        ...
        this.typeCtr.off(fairygui.Events.STATE_CHANGED, this, this.onChangeController);
        this.tabCtr.off(fairygui.Events.STATE_CHANGED, this, this.onTabChangeController)
        NotificationManager.Instance.removeEventListener(NotificationEvent.OUTYARD_USERLIST_INFO, this.onChangeController, this);
    }
    ...
    private initGroupBtn() {
        ...
        this._guildArr = ArrayUtils.sortOn(guildArr, ["isMySelf", "pos"], [ArrayConstant.DESCENDING, ArrayConstant.NUMERIC]);
        if (this._guildArr) {
            ...
            for (let i: number = 0; i < 4; i++) {
                ...
                if (i < len) {
                    ...
                    this["tabBtn" + i].view.getController("rankCtr").selectedIndex = outyardGuildInfo.order-1;
                } else {
    ...
    private onTabChangeController() {
        ...
        let attackValue = this._currentOutyardGuildInfo.attackBuffLevel * this._config1[1] + "%";
        let defenceValue = this._currentOutyardGuildInfo.defenceBuffLevel * this._config2[1] + "%";
        this.attackTxt.text = LangManager.Instance.GetTranslation("outyardMember.attack.text", attackValue);
        this.defenceTxt.text = LangManager.Instance.GetTranslation("outyardMember.defence.text", defenceValue);
        OutyardManager.Instance.OperateOutyard(OutyardManager.REQUEST_LIST, 0, this._currentOutyardGuildInfo.guildUid);
    }
    ...
    private isConsortiaMian(): boolean {
        ...
        if (consortiaControler && consortiaControler.getRightsByIndex(ConsortiaDutyInfo.STACKHEAD_SENIORGENERAL)) {
            flag = true;
        }
        return flag;
    }
