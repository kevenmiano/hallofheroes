...
export class ActionMovie extends Laya.Sprite {
     ...
     */
    public actionAndPlay(type: string, repeatType: string = null, nextType: string = null, waitFrame: number = 5) {
        ...
        switch (repeatType) {
            case ActionMovie.STOP:
                this._movie.addFrameScript(() => { this._movie.stop() })
                break;
            case ActionMovie.GOTO:
                this._movie.addFrameScript(() => { this.actionAndPlay(nextType); })
                break;
            case ActionMovie.REPEAT:
                this._movie.addFrameScript(() => { this.actionAndPlay(type, ActionMovie.REPEAT); })
                break;
            case ActionMovie.WAIT_AND_GOTO:
                this._movie.addFrameScript(() => {
                    if (RoleActionSimplifyData.isSimplify(this._movie.data.urlPath)) {
                        ...
                        Laya.timer.frameOnce(waitFrame, this, this.actionAndPlay, [nextType]);
                    }
                })
                break;
        }
    ...
    private checkWeakBreatheAction(type: string) {
        if (type == ActionLabesType.DANNY || type == ActionLabesType.DIE || type == ActionLabesType.FAILED) {
    ...
    private addWeakBreatheAction() {
        ...
        Laya.Tween.to(this._movie, { scaleX: 1.01, scaleY: 1.04 }, 800, Laya.Ease.linearInOut, Laya.Handler.create(this, () => {
            if (!this._movie || this._movie.destroyed) return;
            Laya.Tween.to(this._movie, { scaleX: 1, scaleY: 1 }, 800, Laya.Ease.linearInOut, Laya.Handler.create(this, () => {
                this.addWeakBreatheAction()
            }));
        }));
    }
