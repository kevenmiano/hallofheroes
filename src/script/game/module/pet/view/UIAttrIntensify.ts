...
export default class UIAttrIntensify extends BaseFguiCom {
	...
	private addEvent() {
		...
		GoodsManager.Instance.addEventListener(BagEvent.UPDATE_BAG, this.__onUpdateNumberHandler, this);
	}

	private removeEvent() {
		GoodsManager.Instance.removeEventListener(BagEvent.UPDATE_BAG, this.__onUpdateNumberHandler, this);
	}

	private initView() {
		let colon = LangManager.Instance.GetTranslation("public.colon2");
		this.itemQualification1.titleTxt.text = LangManager.Instance.GetTranslation("pet.strengthCoe") + colon;
		this.itemQualification2.titleTxt.text = LangManager.Instance.GetTranslation("pet.armorCoe") + colon;
		this.itemQualification3.titleTxt.text = LangManager.Instance.GetTranslation("pet.intellectCoe") + colon;
		this.itemQualification4.titleTxt.text = LangManager.Instance.GetTranslation("pet.staminaCoe") + colon;
		...
		this.tipItem1.setInfo(TemplateIDConstant.TEMP_ID_PET_COE_STONE, true, FUIHelper.getItemURL("Base", "Icon_Unit_Sepulcrum_L"));
	...
	private __trainHandler(target: fgui.GComponent, evt: any) {
		...
		if (this.data.isFullRes()) {
			MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("PetStrengthenView.isFullRes"));
			return;
		}
	...
	private __onUpdateNumberHandler(e: BagEvent = null) {
		...
		this.multi = this.growthStoneCount < this.multi ? (this.growthStoneCount < 1 ? 1 : this.growthStoneCount) : this.multi;
		this._needMax = this.getMaxCostValue();
		this.simBox.show(
			0,
			this.multi,
			1,
			this.growthStoneCount < this._needMax ? this.growthStoneCount : this._needMax,
			this.growthStoneCount < this._needMax ? this.growthStoneCount : this._needMax,
			10,
	...
	private updatePetValue() {
		for (let j: number = 1; j <= 4; j++) {
			let maxWidth = this.getMaxValue(j) * UIAttrIntensify.MAX_WIDTH / UIAttrIntensify.MAX_VALUE;//算出来进度条的最大宽度;
			if (this.getMaxValue(j) > UIAttrIntensify.MAX_VALUE) {
				...
				(this["itemQualification" + j].prog2 as fgui.GProgressBar).width = UIAttrIntensify.MAX_WIDTH;
				(this["itemQualification" + j].prog3 as fgui.GProgressBar).width = maxWidth - UIAttrIntensify.MAX_WIDTH;
				this["itemQualification" + j].txtValue.text = this.getCurrentValue(j) + "/" + this.getMaxValue(j);
				(this["itemQualification" + j].prog2 as fgui.GProgressBar).max = UIAttrIntensify.MAX_VALUE;
				(this["itemQualification" + j].prog3 as fgui.GProgressBar).max = this.getMaxValue(j) - UIAttrIntensify.MAX_VALUE;
				if (this.getCurrentValue(j) > UIAttrIntensify.MAX_VALUE) {
					(this["itemQualification" + j].prog2 as fgui.GProgressBar).value = UIAttrIntensify.MAX_VALUE;
					(this["itemQualification" + j].prog3 as fgui.GProgressBar).value = this.getCurrentValue(j) - UIAttrIntensify.MAX_VALUE;
				} else {
	...
	private get growthStoneCount(): number {
		let num: number = GoodsManager.Instance.getGoodsNumByTempId(GoodsCheck.PET_COE_STONE);
		return num;
	}
	...
	private checkGrowthStone(value: number): boolean {
		...
		if (userCount < value) {
			MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("PetStrengthenView.notEnougthGoods"));
			return false;
		}
		return true;
	}
