...
export default class PveSecretSceneWnd extends BaseWindow {
  ...
  private initView() {
    ...
    switch (scereType) {
      case SecretType.Single:
        this.failCom = FUIHelper.createFUIInstance(
          EmPackName.PveSecretScene,
    ...
    LayerMgr.Instance.addToLayer(
      this.sucessPassCom.displayObject,
      EmLayer.GAME_UI_LAYER,
    );

    let container = <Laya.Sprite>(
      (<any>LayerMgr.Instance.getLayerByType(EmLayer.GAME_TOP_LAYER))
    );
    ...
    UIManager.Instance.ShowWind(EmWindow.MapNameMovie, {
      mapId: this.secretId,
      mapName: this.model.cfg.TemplateNameLang,
    });
  }
  ...
  updateView() {
    ...
    this.rTxtLevel.text = LangManager.Instance.GetTranslation(
      "Pve.secretScene.level",
      layer + "/" + this.model.cfg.MaxLayer,
    );
    this.updateTresure();
