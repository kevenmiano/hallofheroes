...
export default class FightingManager extends GameEventDispatcher {
  ...
  private needGemArr: Array<number> = [
    1, 4, 16, 64, 256, 1024, 4096, 16384, 49152, 147456, 294912, 589824,
  ];
  ...
  private initDataList() {
    ...
    if (level >= OpenGrades.INTENSIFY) {
      let grade0 = FightingManager.Instance.getHeroEquipStrenGrade(
        GoodsSonType.SONTYPE_WEAPON,
      );
      let grade1 = FightingManager.Instance.getHeroEquipStrenGrade(
        GoodsSonType.SONTYPE_CLOTHES,
      );
  ...
  public strengEquipfun() {
    ...
    if (this.worstStrengEquip) {
      ...
      str = LangManager.Instance.GetTranslation(
        "fighting.FightingItem.equipnotStreng",
      ); // "当前没有英灵, 不能打开英灵";
      MessageTipManager.Instance.show(str);
    }
  }
  ...
  public getEquipScore(): number {
    ...
    if (level >= 1 && level < 10) {
      ...
      this.showEquipDesc = LangManager.Instance.GetTranslation(
        "fighting.FightingItem.desction3_" + qualityIndex,
      );
      ...
      if (strengScore < 100) {
        if (this.getAllHeroEquip().length > 0) {
          ...
          switch (scoreIndex) {
            ...
            case FightingManager.THIRD_GRADE:
              this.showEquipDesc = LangManager.Instance.GetTranslation(
                "fighting.FightingItem.desction1_1",
              ); //"装备强化可以大幅提高战力";
              break;
        ...
        if (this.getAllHeroEquip().length > 0) {
          ...
          this.showEquipDesc = LangManager.Instance.GetTranslation(
            "fighting.FightingItem.desction3_" + scoreIndex,
          );
        } else {
          this.showEquipDesc = LangManager.Instance.GetTranslation(
            "fighting.FightingItem.hasNotEquip",
        ...
        this.showEquipDesc = LangManager.Instance.GetTranslation(
          "fighting.FightingItem.EquipIsGood",
        );
      }
    } else if (level >= 19) {
      //开放了 装备品质  强化  洗练
      ...
      if (strengScore < 100) {
        ...
        if (this.getAllHeroEquip().length > 0) {
          ...
          this.showEquipDesc = LangManager.Instance.GetTranslation(
            "fighting.FightingItem.desction3_" + index,
          );
        } else {
          this.showEquipDesc = LangManager.Instance.GetTranslation(
            "fighting.FightingItem.hasNotEquip",
  ...
  public getEquipStrengScore(): number {
    ...
    for (var i: number = 0; i < len; i++) {
      ...
      var num: number =
        goods.strengthenGrade / goods.templateInfo.StrengthenMax;
    ...
    for (var i: number = 0; i < len; i++) {
      ...
      for (var j: number = 1; j < 6; j++) {
        ...
        if (skillId > 0) {
          ...
          var skillProtemp: t_s_skillpropertytemplateData =
            TempleteManager.Instance.getSkillPropertyTemplateCate(
              temp.TemplateId,
            );
          if (job == JobType.HUNTER || job == JobType.WARRIOR) {
            //战士和射手
            if (skillProtemp.Power > 0) {
              hasBesePro = true;
            }
          } else if (job == JobType.WIZARD) {
            //法师
            if (skillProtemp.Intellect > 0) {
              hasBesePro = true;
            }
          }
          if (skillProtemp.Physique) {
            //体质
            hasPhysiquePro = true;
          }

          if (skillProtemp.Agility) {
            //敏捷
            hasArmor = true;
          }
    ...
    if (suiteId <= 0) {
      ...
      if (lv2 % 10 == 0) {
        ...
        if (quality == 5) {
          //史诗套装
          var count: number =
            parseInt((lv1 / 10).toString()) - parseInt((lv2 / 10).toString());
          switch (count) {
            case 0:
              return 12;
              break;
  ...
  public getAllHeroEquip(): Array<GoodsInfo> {
    ...
    var equipDic: SimpleDictionary = GoodsManager.Instance.getHeroEquipListById(
      this.thaneInfo.id,
    );
    ...
    for (const key in equipDic) {
      if (Object.prototype.hasOwnProperty.call(equipDic, key)) {
        ...
        if (
          goods &&
          goods.templateInfo &&
          goods.templateInfo.SonType == sontype
        ) {
          //装备
  ...
  public getPetScore(): number {
    ...
    var score: number = 0;
    if (!this.getCurrPet()) {
      this.showPetDesc = LangManager.Instance.GetTranslation(
        "fighting.FightingItem.hasNotPet",
      ); //没有出战英灵";
    ...
    if (level < 55) {
      ...
      if (petQualityScore <= QualificationScore) {
        ...
        this.showPetDesc = LangManager.Instance.GetTranslation(
          "fighting.FightingItem.desction10_" + petQualityScore,
        );
        this.petFun = this.toPetadvanceBtn; //跳转到英灵提升品质
      } else {
        index = this.getIndexByScore(QualificationScore);
        this.showPetDesc = LangManager.Instance.GetTranslation(
          "fighting.FightingItem.desction11_" + index,
        );
      ...
      if (
        petQualityScore <= QualificationScore &&
        petQualityScore <= petSkillScore
      ) {
        switch (petQualityScore) {
          case 20:
            this.showPetDesc = LangManager.Instance.GetTranslation(
              "fighting.FightingItem.desction10_20",
            );
            break;
          case 40:
            this.showPetDesc = LangManager.Instance.GetTranslation(
              "fighting.FightingItem.desction10_40",
            );
            break;
          case 60:
            this.showPetDesc = LangManager.Instance.GetTranslation(
              "fighting.FightingItem.desction10_60",
            );
            break;
          case 80:
            this.showPetDesc = LangManager.Instance.GetTranslation(
              "fighting.FightingItem.desction10_80",
            );
            break;
          case 100:
            this.showPetDesc = LangManager.Instance.GetTranslation(
              "fighting.FightingItem.desction10_100",
            );
            break;
        }
        this.showPetDesc = LangManager.Instance.GetTranslation(
        ...
        QualificationScore <= petQualityScore &&
        QualificationScore <= petSkillScore
      ) {
        index = this.getIndexByScore(QualificationScore);
        this.showPetDesc = LangManager.Instance.GetTranslation(
          "fighting.FightingItem.desction11_" + index,
        ...
        petSkillScore <= petQualityScore &&
        petSkillScore <= QualificationScore
      ) {
        if (petSkillScore < 100 && petSkillScore >= 80) {
          this.showPetDesc = LangManager.Instance.GetTranslation(
            "fighting.FightingItem.desction12_2",
          ); //"建议学习更多英灵技能"
        } else if (petSkillScore < 80) {
          this.showPetDesc = LangManager.Instance.GetTranslation(
            "fighting.FightingItem.desction12_1",
          ); //"建议学习更多英灵技能"
        } else if (petSkillScore == 100) {
          this.showPetDesc = LangManager.Instance.GetTranslation(
            "fighting.FightingItem.desction12_3",
          );
        }
  ...
  public getQualificationScore(): number {
    ...
    var arr: Array<any> = TempleteManager.Instance.getValueArrByType(
      FightCondtionType.PET,
    );
    var paramA: number = 0;
    var paramB: number = 0;
    var paramC: number = 0;
    if (pettype == 104 || pettype == 101 || pettype == 105) {
      //物理宠物
      currValue = petdata.coeStrength + petdata.coeStamina + petdata.coeArmor;
    } else if (pettype == 102 || pettype == 106 || pettype == 103) {
      //智力宠物
    ...
    if (currValue < paramA) {
      ...
      return parseInt(
        (60 + ((currValue - paramA) / (paramB - paramA)) * 20).toString(),
      );
    } else if (currValue >= paramB && currValue < paramC) {
      return parseInt(
        (80 + ((currValue - paramB) / (paramC - paramB)) * 20).toString(),
      );
  ...
  public getGemAndMarkingScore(): number {
    ...
    if (level < 35) {
      ...
      this.showGemDesc = LangManager.Instance.GetTranslation(
        "fighting.FightingItem.desction4_" + index,
      );
      ...
      if (gemScore <= markingScore) {
        ...
        this.showGemDesc = LangManager.Instance.GetTranslation(
          "fighting.FightingItem.desction4_" + getIndex,
        );
        this.gemFun = this.gemgotoFun; // 跳转到宝石镶嵌
      } else {
        ...
        this.showGemDesc = LangManager.Instance.GetTranslation("");
        this.showGemDesc = LangManager.Instance.GetTranslation(
          "fighting.FightingItem.desction5_" + markingIndex,
        );
  ...
  public getGemScore(): number {
    ...
    for (var i: number = 0; i < len; i++) {
      ...
      for (var j: number = 1; j <= 4; j++) {
        ...
        if (gemId > 0) {
          var gemGoods: t_s_itemtemplateData =
            TempleteManager.Instance.getGoodsTemplatesByTempleteId(gemId);
  ...
  public getMarkingScore(): number {
    ...
    if (currLevel < paramA) {
      ...
      return parseInt(
        (60 + ((currLevel - paramA) / (paramB - paramA)) * 20).toString(),
      );
    } else if (currLevel >= paramB && currLevel < paramC) {
      return parseInt(
        (80 + ((currLevel - paramB) / (paramC - paramB)) * 20).toString(),
      );
  ...
  public getStartScore(): number {
    ...
    this.showStarDesc = LangManager.Instance.GetTranslation(
      "fighting.FightingItem.desction6_" + getIndex,
    );
  ...
  public getTalentAndRuneScore(): number {
    ...
    if (talentScore <= runeScore) {
      ...
      this.showTalentDesc = LangManager.Instance.GetTranslation(
        "fighting.FightingItem.desction7_" + index,
      );
    } else {
      var runeIndex: number = this.getIndexByScore(runeIndex);
      this.showTalentDesc = LangManager.Instance.GetTranslation(
        "fighting.FightingItem.desction8_" + runeIndex,
      );
    }
  ...
  public getMountScore(): number {
    ...
    var currValue: number =
      this.mountInfo.getProperty(PropertyInfo.STRENGTH).addition +
      this.mountInfo.getProperty(PropertyInfo.INTELLECT).addition +
      this.mountInfo.getProperty(PropertyInfo.STAMINA).addition +
      this.mountInfo.getProperty(PropertyInfo.ARMOR).addition;
    for (var i: number = 0; i < arr.length; i++) {
      var guideinfo: t_s_valueData = arr[i];
      if (guideinfo.MaxLv == level) {
        paramA = guideinfo.Parameter1;
    ...
    var index: number = this.getIndexByScore(score);
    this.showMountDesc = LangManager.Instance.GetTranslation(
      "fighting.FightingItem.desction9_" + index,
    );
  ...
  public checkStarLevel(): boolean {
    for (let i: number = 0; i < PlayerModel.PLAYER_STAR_BAG_COUNT; i++) {
      let info = StarManager.Instance.getStarInfoByBagTypeAndPos(
        StarBagType.PLAYER,
        i,
      );
  ...
  public checkGuildLevel(): boolean {
    if (this.consortiaModel.consortiaInfo) {
      ...
      let array: ConsortiaTempleteInfo[] =
        this.consortiaModel.baseSkillList.getList();
