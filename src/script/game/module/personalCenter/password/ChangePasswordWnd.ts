...
export default class ChangePasswordWnd extends BaseWindow {
    ...
    public OnInitWind(){
        ...
        this.setCenter();
        this.addEvent();
        this.txt_input0.restrict = this.txt_input1.restrict =this.txt_input2.restrict ='0-9';
        this.txt_input0.promptText = LangManager.Instance.GetTranslation("vicepassword.description9");
    ...
    private onChange(type:number){
        if(type == 0){
			...
			{
				this.txt_warn0.text =  LangManager.Instance.GetTranslation("vicepassword.description8");
			}
			else if(VicePasswordUtil.numCheck(this.txt_input0.text) == 2)
			{//超过6位数字
				this.txt_warn0.text =  LangManager.Instance.GetTranslation("vicepassword.description9");
        ...
        }else if(type == 1)
        {
			...
			{
				this.txt_warn1.text = LangManager.Instance.GetTranslation("vicepassword.description8");
				// MessageTipManager.instance().show(msg);
			}
			else if(VicePasswordUtil.numCheck(this.txt_input1.text) == 2)
			{//超过6位数字
				this.txt_warn1.text = LangManager.Instance.GetTranslation("vicepassword.description9");
				// MessageTipManager.instance().show(msg);
			}
        }else
        {
			...
			{
				this.txt_warn2.text = LangManager.Instance.GetTranslation("vicepassword.description8");
				// MessageTipManager.instance().show(msg);
			}
			else if(VicePasswordUtil.numCheck(this.txt_input2.text) == 2)
			{//超过6位数字
				this.txt_warn2.text = LangManager.Instance.GetTranslation("vicepassword.description9");
				// MessageTipManager.instance().show(msg);
			}
        }
    ...
    private onReset(){
        ...
        let content: string = LangManager.Instance.GetTranslation("vicepassword.description5");
        SimpleAlertHelper.Instance.Show(SimpleAlertHelper.USEBINDPOINT_ALERT, null, prompt, content, confirm, cancel, this.resetCallback.bind(this));
    }
    ...
    private onSubmit(){
        //判断是否为6位数字
        var msg:string;
        if(this.txt_input0.text == LangManager.Instance.GetTranslation("mainBar.vicePassword.setviewinputdes") || this.txt_input0.text == "")
        {
            msg = LangManager.Instance.GetTranslation("vicepassword.description15");
            MessageTipManager.Instance.show(msg);
            return;
        }
        if(this.txt_input1.text == LangManager.Instance.GetTranslation("mainBar.vicePassword.setviewinputdes")|| this.txt_input1.text == "")
        {
            msg = LangManager.Instance.GetTranslation("vicepassword.description14");
            MessageTipManager.Instance.show(msg);
            return;
        }
        if(this.txt_input2.text == LangManager.Instance.GetTranslation("mainBar.vicePassword.setviewinputdes")|| this.txt_input2.text == "")
        {
            msg = LangManager.Instance.GetTranslation("vicepassword.description13");
            MessageTipManager.Instance.show(msg);
            return;
        }
        if(VicePasswordUtil.strCheck(this.txt_input2.text) && VicePasswordUtil.strCheck(this.txt_input1.text))
        {
            ...
            else if(VicePasswordUtil.numCheckSame(this.txt_input2.text) || VicePasswordUtil.numCheckSame(this.txt_input1.text))
            {
                msg = LangManager.Instance.GetTranslation("vicepassword.description111");
                MessageTipManager.Instance.show(msg);
                return;
            }
            else if(VicePasswordUtil.numCheckContinuity(this.txt_input2.text) || VicePasswordUtil.numCheckContinuity(this.txt_input1.text))
            {
                msg = LangManager.Instance.GetTranslation("vicepassword.description112");
                MessageTipManager.Instance.show(msg);
                return;
            }
            else if(this.txt_input2.text == this.txt_input1.text)
            {
                VicePasswordUtil.vpOp(VicePasswordUtil.CHANGE_PW,this.txt_input1.text,this.txt_input2.text,this.txt_input0.text);
                return;
            }
        }
        else
