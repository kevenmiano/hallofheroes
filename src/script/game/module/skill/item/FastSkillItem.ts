...
import { NotificationEvent, SkillEvent } from "../../../constant/event/NotificationEvent";
...
* @description 技能操作栏
*/
export default class FastSkillItem extends fgui.GComponent implements DragObject,ITipedDisplay {
    extData: any='center';
    tipData: any;
    tipType: EmWindow;
    ...
    protected onConstruct() {
        ...
        this.skillitem = fgui.UIPackage.createObject('Skill', 'SkillItemCom', SkillItemCom) as SkillItemCom;
    ...
    public set isFastSkill(value: boolean) {
        if (this.skillitem)
            this.skillitem.isFastSkill = value;
    }

    public get isFastSkill(): boolean {
        if (this.skillitem)
            return this.skillitem.isFastSkill;
        return true;
    }
    ...
    setDragState(isdrag:boolean=true){
        if(isdrag){
            if (this.isUsed) {
                ...
                DragManager.Instance.registerDragObject(this.skillitem, this.onDragComplete.bind(this));
            }
        }else
        {
        ...
        if (dropTarget) {
            ...
            if (dropTarget instanceof SkillItemCom) {//目标对象存在
                ...
                if (sourTarget != dropTarget) {//两者交换
                    ...
                    NotificationManager.Instance.sendNotification(SkillEvent.SET_FAST_KEY, null);
                } else {
                    this.setSkillItemPos();
                    return;
                }
