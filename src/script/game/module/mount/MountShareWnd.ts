...
export default class MountShareWnd extends BaseWindow {
    ...
    public OnInitWind() {
        ...
        if (this.params) {
            ...
            this._info = this.wildSoulCollection.getWildSoulInfo(this._mountData.TemplateId);
            if (this._info) {
            ...
            this.levelTxt.text = LangManager.Instance.GetTranslation("public.level2", ArmyManager.Instance.thane.grades.toString());
    ...
    private addEvent() {
        ...
        this.sharelist.itemRenderer = Laya.Handler.create(this, this.renderShareListItem, null, false);
    ...
    private setShareDatalist() {
        ...
        let shareOpen = [
            ...
            { switch: ConfigManager.info.Instagram_SHARE, id: SHARE_CHANNEL.Instagram },
    ...
    public getIconUrl(): string {
        return IconFactory.getPlayerIcon(ArmyManager.Instance.thane.snsInfo.headId, IconType.HEAD_ICON);
    }
    ...
    private loaderCompleteHandler(res: any) {
        ...
        if (templateId == 1124) {//魔界战犀
            this._mountImgSprite.x -= 50;
            this._mountImgSprite.y -= 80;
        }
        else if (templateId == 7002 || templateId == 8135) {//魔兽影灵,荷鲁斯猎鹰
            this._mountImgSprite.y -= 80;
        } else if (templateId == 8054) {//摩羯座
            this._mountImgSprite.x += 30;
            this._mountImgSprite.y -= 40;
        } else if (templateId == 1128 || templateId == 8334) {//异域战虎,人马DJ
            this._mountImgSprite.x -= 40;
            this._mountImgSprite.y -= 60;
        } else if (templateId == 1111) {//幽冥雪猿
            this._mountImgSprite.y -= 30;
        } else if (templateId == 8093 || templateId == 8201) {//荷塘月色, 破坏炸弹
            this._mountImgSprite.x += 40;
            this._mountImgSprite.y -= 40;
        } else if (templateId == 3005) {//梦幻蓝
            this._mountImgSprite.x += 50;
            this._mountImgSprite.y -= 40;
        } else if (templateId == 1147) {//霜雪豹
            this._mountImgSprite.y -= 40;
        } else if (templateId == 8097 || templateId == 8233) {//麟龙, 图腾魔像
            this._mountImgSprite.y -= 80;
        } else if (templateId == 3002) {//幽冥魔狼
            this._mountImgSprite.y -= 20;
        } else if (templateId == 8072) {//呆萌飞鼠
            this._mountImgSprite.x += 30;
            this._mountImgSprite.y -= 40;
        } else if (templateId == 3080) {//圆舞火鸡
            this._mountImgSprite.x += 20;
            this._mountImgSprite.y -= 30;
        } else if (templateId == 8204 || templateId == 8026 || templateId == 8292) {//白羽,火箭飞艇, 梦神
            this._mountImgSprite.x += 20;
            this._mountImgSprite.y -= 30;
        } else if (templateId == 1109) {//阿狸
            this._mountImgSprite.x += 50;
        } else if (templateId == 3069) {//梦幻鹿
            this._mountImgSprite.y -= 120;
        } else if (templateId == 8288 || templateId == 3045) {//绿野麋鹿,熔岩火驹
            this._mountImgSprite.y -= 30;
        } else if (templateId == 8324) {//夜煞
            this._mountImgSprite.x += 20;
            this._mountImgSprite.y -= 60;
        } else if (templateId == 8284 || templateId == 8053) {//噬光兽, 囚牛
            this._mountImgSprite.x += 50;
            this._mountImgSprite.y -= 70;
        } else if (templateId == 8146 || templateId == 8073 || templateId == 8389 || templateId == 8214) {//朱厌 , 鲨鱼, 机械路霸, 铁元素
            this._mountImgSprite.y -= 40;
        } else if (templateId == 8216) {//比蒙, 
            this._mountImgSprite.x += 20;
            this._mountImgSprite.y -= 50;
        } else if (templateId == 1130) {//炽焰
            this._mountImgSprite.x -= 50;
            this._mountImgSprite.y -= 60;
        } else if (templateId == 8278 || templateId == 8198 || templateId == 8141) {//机械黄蜂, 瀑布元素, 黄金蛤蟆
            this._mountImgSprite.y -= 50;
        } else if (templateId == 8317 || templateId == 8235) {//幻影龙, 迷你飞船
            this._mountImgSprite.x -= 20;
            this._mountImgSprite.y -= 40;
        } else if (templateId == 8160) {//白金刚
            this._mountImgSprite.x -= 190;
            this._mountImgSprite.y -= 220;
        } else if (templateId == 8215) {//独轮车
            this._mountImgSprite.x += 30;
    ...
    private isExistCode(code: number): boolean {
        let config: number[] = []
        //区分Android iOS
        if (Utils.isAndroid()) {//android
            config = PathManager.info.share_android;
        } else if (Utils.isIOS()) {//iOS
            config = PathManager.info.share_ios;
        } else if (Utils.isWebWan()) {//web
