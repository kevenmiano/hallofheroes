...
export class ChargeLotteryManager {
    ...
    private initEvent(): void {
        ServerDataManager.listen(S2CProtocol.U_C_CHARGELOTTERYINFO, this, this.__chargelotteryHandler);
        ServerDataManager.listen(S2CProtocol.U_C_CHARGELOTTERY_STATE, this, this.__openStateHandler);
    }

    private __chargelotteryHandler(pkg: PackageIn): void {
        let msg: ChargePointLotteryMsg = pkg.readBody(ChargePointLotteryMsg) as ChargePointLotteryMsg;
        ...
        if (msg.op == 1) {
            ...
            this._chargeLotteryMsg.resultPos = msg.resultPos.slice(0, msg.resultPos.length);
        }
        else if (msg.op == 3) {
            this._chargeLotteryMsg.op = msg.op;
            this._chargeLotteryMsg.logs = msg.logs.slice(0, msg.logs.length);
        }
        NotificationManager.Instance.sendNotification(NotificationEvent.CHARGELOTTERY_RESULT_UPDATE);
    }
    ...
    public OperateChargeReq(op: number, isOneKey: boolean = false): void {
        ...
        SocketManager.Instance.send(C2SProtocol.C_CCCACTIVE_CLIENTOP, msg, CCCActiveType.CHARGELOTTERY);
    }
