...
export class RipplePreCreater{
    ...
    public static create(skillIds :any[], callback : Function):any[]
    {
        ...
        for(let i : number = 0; i < len; i++){
            id = skillIds[i];
            if(id != 0 && id != -1){
                skillTempInfo = ConfigMgr.Instance.getTemplateByID( ConfigType.t_s_skilltemplate, String(id));
                if(skillTempInfo){
                    actionData = ConfigMgr.Instance.actionTemplate2[skillTempInfo.ActionId]
                    if(actionData && actionData.presentType == ActionPresentType.SHOOT_ACTION){
    ...
    {
        let mc : MovieClip = BattleManager.Instance.resourceModel.getEffectMC(linkName) as MovieClip;
