...
export class HintBMFontCfg {
    ...
    constructor(url: string = '', fontdistance: number = 0, fontSize: number = 30) {
        this.url = url;
...
export class HintFontCfg {
    color: string = "#000000";
    fontSize: number = 24;
    constructor(font: string = Laya.Text.defaultFont, color: string = "#000000", fontSize: number = 24) {
        this.color = color
...
export class HintAniData {
    ...
    constructor(type: FontType = FontType.NONE, point: Laya.Point = null, once: boolean = false, fontCfg: HintFontCfg = null, bmfontCfg: HintBMFontCfg = null, delay: number = 0) {
        this.type = type
        this.point = point
        this.once = once
        this.delay = delay;
...
export default class HintUtils {
    ...
    static register() {
        if (HintUtils.isInit)
            return;
        HintUtils.BMFontCfgMap.set(FontType.BossBarNum, new HintBMFontCfg('ui://Font/' + FontType.BossBarNum, -15));
        HintUtils.BMFontCfgMap.set(FontType.LevelNum, new HintBMFontCfg('ui://Font/' + FontType.LevelNum, -5));
    ...
    */
    public static showTips(valueStr: string = "", hintType: HintType = HintType.HintLabel, viewData: string | HintViewData = '', aniData?: HintAniData, callback?: Function, delay: number = 300) {
        if (!HintUtils.isInit) {
            HintUtils.register();
        }
    ...
    private static async __delayShowTip() {
        ...
        let comp = this.create(msgHintType, msgValueStr, msgViewData, msgAniData, msgcallback);
        if (!comp) {
        ...
        if (msgAniData.showAction) {
            HintUtilsHelper.showAction(comp, msgHintType, msgViewData, msgAniData, msgcallback)
        }
    }

    private static create(msgHintType: HintType, content: string, viewData: string | HintViewData = '', aniData: HintAniData, callback?: Function, delay: number = 30) {
        ...
        switch (msgHintType) {
            default:
                HintUtilsHelper.refreshView(comp, msgHintType, content, viewData, aniData)
                break;
        }
