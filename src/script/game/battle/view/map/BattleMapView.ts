...
import { BattleEvent, BattleNotic, RoleEvent } from "../../../constant/event/NotificationEvent";
...
export class BattleMapView extends BattleMap {
    private static REVIVE_BATTLE_TYPE: Array<number> = [BattleType.DOUBLE_BOSS_BATTLE];
    ...
    protected addEvent() {
        ...
        NotificationManager.Instance.on(BattleEvent.SCENE_EFFECT_CLOSE, this.onSceneEffecCloseHandler, this);
        NotificationManager.Instance.on(BattleEvent.BATTLE_BGANI_LOADED, this.onBattleBgAniLoadHandler, this);
    }

    protected delEvent() {
        super.delEvent()
        Laya.stage.off(Laya.Event.VISIBILITY_CHANGE, this, this.onVisibilityChange);
        NotificationManager.Instance.off(BattleEvent.SCENE_EFFECT_CLOSE, this.onSceneEffecCloseHandler, this);
        NotificationManager.Instance.off(BattleEvent.BATTLE_BGANI_LOADED, this.onBattleBgAniLoadHandler, this);
    }
    ...
    private onBattleBgAniLoadHandler() {
        if (!this._mapBgAni) {
            ...
            this._mapBgAni = FUIHelper.createFUIInstance(EmWindow.BattleBgAni, PathManager.getBattleMapAniPath(mapId));
            if (this._mapBgAni) {
    ...
    private onRoleDie(info) {
        ...
        var isReviveBoss: boolean = BattleMapView.REVIVE_BATTLE_TYPE.indexOf(this.battleModel.battleType) >= 0;
        if ((info.inheritType == InheritRoleType.Hero) && ((info as HeroRoleInfo).type != 3)) {
            roleView.setBloodStripVisible(false, true);
            // Logger.battle("[BattleMapView]onRoleDie 英雄死亡")
        } else if ((info.inheritType == InheritRoleType.Hero) && ((info as HeroRoleInfo).type == 3) && isReviveBoss) {
    ...
    public changeMap(mapData: any, asset: string = "asset") {
        ...
        for (var i: number = 0; i < mapData.bg_back_arr.length; i++) {
            ...
            if (data.repeat) {
                var layer: RepeatBackGroundLayer = new RepeatBackGroundLayer(data[asset], data.transparent);
                ...
                if (SharedManager.Instance.allowSceneEffect) {
                    var t_layer: RepeatBackGroundLayer = new RepeatBackGroundLayer(data[asset]);
    ...
    public addGasEffect(effect: BattleEffect, repeat: number = 1) {
        ...
        NotificationManager.Instance.sendNotification(BattleNotic.REFRESH_BACK_BITMAP);
