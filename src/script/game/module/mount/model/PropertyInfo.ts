...
import { ArrayUtils, ArrayConstant } from '../../../../core/utils/ArrayUtils';
	...
	export class PropertyInfo extends GameEventDispatcher
	{
			...
			for (var i:number=0, len:number=arr.length; i < len; i++) {
				if (this._propGp >= arr[i].Data && 
					...
					if (len == 1) {
						...
						this._currentPropGp = Math.min(this._currentPropGp, this._currentPropMaxGp);
					}
					break;
				}
			}
		...
		public get grade():number {
			var g:number = (this._upgradeTemplate == null ? 1 : this._upgradeTemplate.Grades);
