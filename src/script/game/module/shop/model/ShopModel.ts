...
export class ShopModel extends GameEventDispatcher {
  ...
  private init() {
    ...
    this.hotGoodsList = ArrayUtils.sortOn(
      this.hotGoodsList,
      "Sort",
      ArrayConstant.NUMERIC,
    );
    this.propGoodsList = ArrayUtils.sortOn(
      this.propGoodsList,
      "Sort",
      ArrayConstant.NUMERIC,
    );
    this.gemstoneGoodsList = ArrayUtils.sortOn(
      this.gemstoneGoodsList,
      "Sort",
      ArrayConstant.NUMERIC,
    );
    this.backPlayerList = ArrayUtils.sortOn(
      this.backPlayerList,
      "Sort",
      ArrayConstant.NUMERIC,
    );
    this._recommendGoodsList = ArrayUtils.sortOn(
      this._recommendGoodsList,
      "Sort",
      ArrayConstant.NUMERIC,
    );
    this.giftGoodsList = ArrayUtils.sortOn(
      this.giftGoodsList,
      "Sort",
      ArrayConstant.NUMERIC,
    );
    this.cardReelList = ArrayUtils.sortOn(
      this.cardReelList,
      "Sort",
      ArrayConstant.NUMERIC,
    );
    this.petGoodsList = ArrayUtils.sortOn(
      this.petGoodsList,
      "Sort",
      ArrayConstant.NUMERIC,
    );
    ...
    this._farmShopTempArr = ArrayUtils.sortOn(
      this._farmShopTempArr,
      "NeedGrades",
      ArrayConstant.NUMERIC,
    );
  ...
  public set mainDiscountList(value: any[]) {
    ...
    for (let i: number = 0; i < len; i++) {
      ...
      shopInfo = TempleteManager.Instance.getShopMainInfoByItemId(
        discountInfo.shopId,
      );
      if (
        (!shopInfo || shopInfo.ItemId != discountInfo.templateId) &&
        discountInfo.shopId > 0
      ) {
        continue;
      }
      shopInfo = TempleteManager.Instance.getShopMainInfoByItemId(
        discountInfo.shopId1,
      );
      if (
        (!shopInfo || shopInfo.ItemId != discountInfo.templateId1) &&
        discountInfo.shopId1 > 0
      ) {
        continue;
      }
      shopInfo = TempleteManager.Instance.getShopMainInfoByItemId(
        discountInfo.shopId2,
      );
      if (
        (!shopInfo || shopInfo.ItemId != discountInfo.templateId2) &&
        discountInfo.shopId2 > 0
      ) {
        continue;
      }
      shopInfo = TempleteManager.Instance.getShopMainInfoByItemId(
        discountInfo.shopId3,
      );
      if (
        (!shopInfo || shopInfo.ItemId != discountInfo.templateId3) &&
        discountInfo.shopId3 > 0
      ) {
        continue;
      }
      shopInfo = TempleteManager.Instance.getShopMainInfoByItemId(
        discountInfo.shopId4,
      );
      if (
        (!shopInfo || shopInfo.ItemId != discountInfo.templateId4) &&
        discountInfo.shopId4 > 0
      ) {
        continue;
      }
      shopInfo = TempleteManager.Instance.getShopMainInfoByItemId(
        discountInfo.shopId5,
      );
      if (
        (!shopInfo || shopInfo.ItemId != discountInfo.templateId5) &&
        discountInfo.shopId5 > 0
      ) {
        continue;
      }
      shopInfo = TempleteManager.Instance.getShopMainInfoByItemId(
        discountInfo.shopId6,
      );
      if (
        (!shopInfo || shopInfo.ItemId != discountInfo.templateId6) &&
        discountInfo.shopId6 > 0
      ) {
        continue;
      }
      shopInfo = TempleteManager.Instance.getShopMainInfoByItemId(
        discountInfo.shopId7,
      );
      if (
        (!shopInfo || shopInfo.ItemId != discountInfo.templateId7) &&
        discountInfo.shopId7 > 0
      ) {
        continue;
      }
      shopInfo = TempleteManager.Instance.getShopMainInfoByItemId(
        discountInfo.shopId8,
      );
      if (
        (!shopInfo || shopInfo.ItemId != discountInfo.templateId8) &&
        discountInfo.shopId8 > 0
      ) {
        continue;
      }
      ...
      mainShopInfo.maxCurrentDate =
        DateFormatter.parse(
          discountInfo.maxCurrentDate,
          "YYYY-MM-DD hh:mm:ss",
        ).getTime() / 1000;
      mainShopInfo.url = discountInfo.url;
      mainShopInfo.names = discountInfo.names;
      mainShopInfo.beginDate =
        DateFormatter.parse(
          discountInfo.beginDate,
          "YYYY-MM-DD hh:mm:ss",
        ).getTime() / 1000;
      mainShopInfo.endDate =
        DateFormatter.parse(
      ...
      if (mainShopInfo.isGift) {
        ...
        let goodsTempInfo: t_s_itemtemplateData =
          ConfigMgr.Instance.getTemplateByID(
            ConfigType.t_s_itemtemplate,
            mainShopInfo.templateId,
          );
        if (goodsTempInfo)
          mainShopInfo.sortName = goodsTempInfo.TemplateNameLang;
    ...
    this._mainDiscountList = ArrayUtils.sortOn(
      this._mainDiscountList,
      "sortName",
      ArrayConstant.NUMERIC,
    );
    this._loopItemList = ArrayUtils.sortOn(
      this._loopItemList,
      "sortId",
      ArrayConstant.NUMERIC,
    );
    this._timeBuyList = ArrayUtils.sortOn(
      this._timeBuyList,
      "type",
  ...
  public set mainUrlList(value: any[]) {
    ...
    this._mainUrlList = ArrayUtils.sortOn(
      this._mainUrlList,
      "site",
      ArrayConstant.NUMERIC,
    );
  ...
  public set mainNewList(value: any[]) {
    ...
    this._mainNewList = ArrayUtils.sortOn(
      this._mainNewList,
      "sortId",
      ArrayConstant.NUMERIC,
    );
  ...
  public resetData(
    pageNum: number = ShopModel.PAGE_NUM,
    curTab: number = 1,
    curPage: number = 1,
  ) {
  ...
  public set currentTab(value: number) {
    ...
    if (this.currentGoodsList) {
      ...
      this.totalPage = Math.ceil(
        this.currentGoodsList.length / this.currentPageNum,
      );
  ...
  public getShopTempInfoById(id: number): ShopGoodsInfo {
    ...
    let consortiaHighShopGoodsList =
      ConfigMgr.Instance.consortiaHighShopTemplateDic;
  ...
  public get petSkill(): Dictionary {
    if (!this._petSkillBookDic) {
      ...
      const allSkill: any = ConfigMgr.Instance.getDicSync(
      ...
      for (const type of types) {
        ...
        for (const skill of Object.values(allSkill)) {
          ...
          if (skill["MasterType"] === type || skill["MasterType"] === 107) {
            skill["itemTemplate"] = ConfigMgr.Instance.getTemplateByID(
              ConfigType.t_s_itemtemplate,
              "Property1" + skill["TemplateId"],
            );
  ...
  public get petSkillBookDic(): Dictionary {
    if (!this._petSkillBookDic) {
      this._petSkillBookDic = new Dictionary();
      const types: number[] = [101, 102, 103, 104, 105, 106];
      const allBook: any = ConfigMgr.Instance.getDicSync(
        ConfigType.t_s_itemtemplate,
      );
      const allSkill: any = ConfigMgr.Instance.getDicSync(
        ConfigType.t_s_skilltemplate,
    ...
    if (!this._petSkillBookDic) {
      ...
      for (let i = 0; i < types.length; i++) {
        ...
        for (let i = 0; i < allbooks.length; i++) {
          ...
          let book: t_s_itemtemplateData = ConfigMgr.Instance.getTemplateByID(
            ConfigType.t_s_itemtemplate,
            shopBook.ItemId,
          );
          if (!book) {
