...
export class BasePropertyInfo {
  ...
  public hasEffect(property: string): number {
    if (property == "powerPoint" && this.totalPower - this.basePowerPoint > 0) {
      ...
      property == "agilityPoint" &&
      this.totalAgility - this.baseAgilityPoint > 0
    ) {
      return 2;
    } else if (
      property == "intellectPoint" &&
      this.totalIntellect - this.baseIntellectPoint > 0
    ) {
      return 2;
    } else if (
      property == "captainPoint" &&
      this.totalCaptain - this.baseCaptainPoint > 0
    ) {
      return 2;
    } else if (
      property == "physiquePoint" &&
      this.totalPhysique - this.basePhysiquePoint > 0
    ) {
      return 2;
    }
    return 1;
