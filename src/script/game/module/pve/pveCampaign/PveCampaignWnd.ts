...
export default class PveCampaignWnd extends BaseWindow {
    ...
    public OnInitWind() {
        ...
        this.btnModePT.title = LangManager.Instance.GetTranslation("selectcampaign.view.CampaignPointView.state02");
        this.btnModeYX.title = LangManager.Instance.GetTranslation("selectcampaign.view.CampaignPointView.state01");
        this.txtTip.title = LangManager.Instance.GetTranslation("selectcampaign.view.mayFall");
    ...
    private addEvent() {
        ...
        NotificationManager.Instance.on(PveCampaignData.CancelUseWearySupply, this.onCancelUseWearySupply, this)
    }

    private removeEvent() {
        this.tree && this.tree.off(fgui.Events.CLICK_ITEM, this, this.onClickTreeItem);
        NotificationManager.Instance.off(PveCampaignData.CancelUseWearySupply, this.onCancelUseWearySupply, this)
        // if (this.itemList) {
    ...
    public defaultView() {
        for (let index = 0; index < this._sTreeNodeList.length; index++) {
            ...
            if (this.model.taskCampaignTem) {
                defSelectedArea = this.model.recentChapter.getAreaByCampaignTem(this.model.taskCampaignTem);
                //选择副本完成, 任务追踪标志置空
                // this.model.taskCampaignTem = null;
            } else {
                if (this.frameData && this.frameData.CampaignId) {
                    let data: t_s_campaignData = ConfigMgr.Instance.campaignTemplateDic[this.frameData.CampaignId];
                    defSelectedArea = this.model.recentChapter.getAreaByCampaignTem(data);
                } else {
                    defSelectedArea = this.model.recentChapter.getMaxOpenArea() as CampaignAreaInfo
                }
            }
            if (!defSelectedArea) continue;
            let defSelectCampaign = defSelectedArea.getMapByDifficult(CampaignMapDifficulty.General)
    ...
    private btnSweepClick() {
        ...
        let template: t_s_configData = TempleteManager.Instance.getConfigInfoByConfigName(bSingleCp ? "SingleCampaign_Weary" : "MultiCampaign_Weary");

        let flag = weary < Number(template.ConfigValue);
        if (flag) {
            if (this.model.hasWearyMedicine()) {//背包拥有体力药水时,弹出体力补充弹窗
                FrameCtrlManager.Instance.open(EmWindow.WearySupplyWnd, { type: 1 });
            } else if (!this.model.hasWearyMedicine()) {//背包无体力药水时
                if (this.model.todayCanBuyWearyMedicine()) {//今日还能购买, 弹出高级体力药水快捷购买弹窗
                    ...
                    MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("selectcampaign.view.CampaignInfoView.noWeary"));
                }
            }
            return;
        }
        ...
        FrameCtrlManager.Instance.open(EmWindow.Mopup, { type: eMopupType.CampaignMopup, state: eMopupState.CampaignMopupPre })
    }
    ...
    private showThewAlert(): boolean {
        ...
        if (flag && outdate) {
            ...
            SimpleAlertHelper.Instance.Show(SimpleAlertHelper.USEBINDPOINT_ALERT, { checkRickText: checkStr, checkDefault: false }, null, content, null, null, this.thewAlertBack.bind(this));
        }
        return flag && outdate
    }
    ...
    private showPawnLevel(): boolean {
        ...
        if (CampaignRankManager.Instance.rankDic[this.model.selectCampaign.CampaignId]) return false;
        let armyPawn: ArmyPawn = ArmyManager.Instance.army.getPawnByIndex(0);

        let level: number = thane.grades < 20 ? this.model.selectCampaign.MinLevel - 2 : this.model.selectCampaign.MinLevel - 4;
        if (!armyPawn) armyPawn = ArmyManager.Instance.casernPawnList[PawnType.PAWN1];
        if (!armyPawn.templateInfo || armyPawn.templateInfo.Level >= level) return false;
        let content: string = LangManager.Instance.GetTranslation("selectcampaign.view.CampaignInfoView.content", level);
        SimpleAlertHelper.Instance.Show(SimpleAlertHelper.SIMPLE_ALERT, null, null, content, null, null, this.onEnter.bind(this));
        return true
    }
    ...
    private checkInTeam(): boolean {
        ...
        if (inTeam) {
            MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("selectcampaign.data.CanNotBegainCampaignInTeam"))
        }
        return inTeam;
    }
    ...
    public refreshTreeData(firstData: SelectCampaignItemData[], secondData: SelectCampaignItemData[], rewardTipData: SelectCampaignItemData[]) {
        if (!firstData) {
            return;
        }
        for (let index = 0; index < firstData.length; index++) {
            ...
            tipRewardNode._resURL = fgui.UIPackage.getItemURL(EmPackName.PveCampaign, "ItemRewardTip")
    ...
    private renderTreeNode(node: fgui.GTreeNode, obj: fgui.GComponent) {
        ...
        if (node.data.type == eSelectCampaignItemType.FirstItem) {
            ...
            btnBox.filters = [itemData.bComplete ? UIFilter.lightFilter : UIFilter.darkFilter];
            btnBox.onClick(null, (evt: Laya.Event) => {
                let itemInfos = this.model.getChapterRewardDataList()
                FrameCtrlManager.Instance.open(EmWindow.DisplayItems, { itemInfos: itemInfos, title: itemData.bComplete ? LangManager.Instance.GetTranslation("selectcampaign.view.Awarded") : LangManager.Instance.GetTranslation("selectcampaign.view.GetRewards") })
            })
        }
    ...
    private refreshRight(campaignAreaInfo: CampaignAreaInfo, difficulty: CampaignMapDifficulty = CampaignMapDifficulty.General) {
        ...
        this.btnModeYX.enabled = this.model.isHeroOpen() && this.model.normalMap.state == CampaignMapStatus.OVER_CAMPAIGN
        ...
        let config2: t_s_configData = TempleteManager.Instance.getConfigInfoByConfigName("Campaign_Sweep_Gold");
        this.txtSweepCost.text = config2.ConfigValue


        this.btnSweep.enabled = this.model.selectCampaign.state == CampaignMapStatus.OVER_CAMPAIGN;
        this.txtSweepTip.visible = this.model.selectCampaign.state != CampaignMapStatus.OVER_CAMPAIGN;
        this.btnChallenge.enabled = this.model.selectCampaign.state != CampaignMapStatus.NO_ACCEPT_CAMPAIGN;
    ...
    private getDescTxt(): string {
        ...
        if (this.model.selectCampaign.MinLevel > ArmyManager.Instance.thane.grades) {
            str = LangManager.Instance.GetTranslation("fund.fundRewardView.NewbieTip");
        }
        else if (this.model.selectCampaign.state == CampaignMapStatus.NO_ACCEPT_CAMPAIGN) {
            str = LangManager.Instance.GetTranslation("PveSelectCampaignWnd.descTxt.tip");
        }
        return str;
    }
