...
interface IBaseMouseEvent {
  mouseClickHandler(e: Laya.Event): void;
}
...
export default class NewbieBaseActionMediator {
  ...
  public static showMask(alpha: number = 0.6) {
    ...
    if (!NewbieBaseActionMediator._guideContainer) {
      ...
      LayerMgr.Instance.addToLayer(
        NewbieBaseActionMediator._guideContainer,
      ...
      NewbieBaseActionMediator._mask.graphics.drawRect(
        0,
        0,
        Resolution.gameWidth,
        Resolution.gameHeight,
        "#000000",
      );
      NewbieBaseActionMediator._guideContainer.addChild(
        NewbieBaseActionMediator._mask,
      );
      ...
      NewbieBaseActionMediator._guideContainer.addChild(
        NewbieBaseActionMediator._interactionArea,
      );
      //临时放这里, 放其它地方, 会造成循环引用
      Laya.stage.off(
        Laya.Event.RESIZE,
        NewbieBaseActionMediator,
    ...
    NewbieBaseActionMediator._guideContainer.hitArea = new Laya.Rectangle(
      0,
      0,
      1,
      1,
   ...
   */
  public static drawTargetOnMask(
    callback: Function,
    callArgs,
    $targetArgs: string,
    $needWaitOper: string,
    ...
    Logger.xjy(
      "[NewbieBaseActionMediator]drawTargetOnMask  在遮罩上绘制显示目标对象",
    );
    ...
    if (!targetObj) {
      Logger.xjy(
        "[NewbieBaseActionMediator]drawTargetOnMask  对象获取错误 $targetArgs=",
        $targetArgs,
        "$needWaitOper=",
        $needWaitOper,
      );
    ...
    if (!NewbieBaseActionMediator._drawContainer) {
      ...
      NewbieBaseActionMediator._drawContainer.name =
        "NewbieBaseActionMediator._drawContainer";
      ...
      NewbieBaseActionMediator._drawContainer.off(
        Laya.Event.CLICK,
        NewbieBaseActionMediator,
        NewbieBaseActionMediator.__onDrawTargetMouseEvent,
      );
      NewbieBaseActionMediator._drawContainer.off(
    ...
    let gPt = NewbieBaseActionMediator.localToGlobal(
      targetObj,
      new Laya.Point(0, 0),
    );
    gPt.x += $offsetX;
    ...
    NewbieBaseActionMediator._interactionArea.graphics.drawRect(
      gPt.x,
      gPt.y,
      targetSize.w,
      targetSize.h,
    ...
    if (needWaitOper) {
      if ($eventTarget != null && $eventTarget != "") {
        let eventTarget: DisplayObject =
          NewbieBaseActionMediator.getGuildTarget($eventTarget);
        NewbieBaseActionMediator._drawClickEventArgs = [
          eventTarget,
          callback,
          callArgs,
        ];
      ...
      if (needWaitDoubleClick) {
        NewbieBaseActionMediator._drawContainer.on(
          Laya.Event.DOUBLE_CLICK,
          NewbieBaseActionMediator,
          NewbieBaseActionMediator.__onDrawTargetMouseEvent,
        );
    ...
    switch (e.type) {
      case Laya.Event.CLICK:
        ...
        if (NewbieBaseActionMediator._drawClickEventArgs) {
          NewbieBaseActionMediator.dispatchAllChildEvent(
            NewbieBaseActionMediator._drawClickEventArgs[0],
            e,
          );
          NewbieUtils.execFunc(
            NewbieBaseActionMediator._drawClickEventArgs[1],
            NewbieBaseActionMediator._drawClickEventArgs[2],
          );
          NewbieBaseActionMediator._drawClickEventArgs = null;
  ...
  private static dispatchAllChildEvent(disObj: any, e: Laya.Event) {
    ...
    if (disObj instanceof DisplayObject) {
      ...
      for (let i: number = 0; i < childNum; i++) {
        NewbieBaseActionMediator.dispatchAllChildEvent(
          disContainer.getChildAt(i) as DisplayObject,
          e,
        );
      }
    }
  ...
  public static moveNPC(
    ...
    let npcView;
    if ($npcId <= 0) {
      npcView = CampaignManager.Instance.controller.getArmyView(
        mapModel.selfMemberData,
      );
    } else {
    ...
    let paths: Array<any> = CampaignManager.Instance.controller.findPath.find(
      new Laya.Point(beginX, beginY),
      new Laya.Point(endX, endY),
    );
    ...
    if (completeWhenWalkOver) {
      ...
      npcView.on(
        ...
        NewbieBaseActionMediator,
        NewbieBaseActionMediator.__armyWalkOverHandler,
      );
  ...
  private static __armyWalkOverHandler(target: any) {
    ...
    if (NewbieBaseActionMediator._moveEventArgs) {
      NewbieUtils.execFunc(
        NewbieBaseActionMediator._moveEventArgs[0],
        NewbieBaseActionMediator._moveEventArgs[1],
      );
  ...
  public static taskTraceItemShine(callback: Function, callArgs) {
    ...
    let taskTraceView = NewbieUtils.getFrame(
   ...
   */
  public static setTaskArrowGuild(
    callback: Function,
    callArgs,
    $state: number,
    value: number = 0,
    ...
    Logger.xjy(
      "[NewbieBaseActionMediator]setTaskArrowGuild 设置任务追踪箭头指引",
    );
    $state = Number($state);
    value = Number(value);
    ...
    NotificationManager.Instance.sendNotification(NewbieEvent.ARROW_STATE, {
      type: $state,
      tip: tip,
      callback: callback,
      callArgs: callArgs,
  ...
  public static showTaskWnd(callback: Function, callArgs, $state: number) {
    ...
    NotificationManager.Instance.sendNotification(
      ...
      { visible: Boolean($state), callback: callback, callArgs: callArgs },
    );
  }
   ...
   */
  public static moveFocusToFarmBuild(
    callback: Function,
    callArgs,
    buildType: number,
    isTween: string = "1",
   ...
   * @param tweenTime  移动时间
   */
  public static moveFocusToCastleBuild(
    callback: Function,
    callArgs,
    buildType: number,
    isTween: string = "1",
    ...
    if (curScene.castleMap) {
      curScene.castleMap.moveBuildToCenter(
        buildType,
        isTween == "1",
        tweenTime,
        callback,
   ...
   * @param buildType  建筑sontype
   */
  public static castleBuildShine(
    callback: Function,
    callArgs,
    buildType: number,
  ) {
  ...
  private static __colorEffectCompleteHandler(e: Event) {
    if (NewbieBaseActionMediator._colorEffect) {
      NewbieBaseActionMediator._colorEffect.removeEventListener(
        Laya.Event.COMPLETE,
        NewbieBaseActionMediator.__colorEffectCompleteHandler,
      );
      NewbieBaseActionMediator._colorEffect = null;
    }
    if (NewbieBaseActionMediator._colorEventArgs) {
      NewbieUtils.execFunc(
        NewbieBaseActionMediator._colorEventArgs[0],
        NewbieBaseActionMediator._colorEventArgs[1],
      );
      NewbieBaseActionMediator._colorEventArgs = null;
    }
  }
  ...
  public static createGuildFrame1(
    targetArgs: string,
    relativeX: number,
    relativeY: number,
    value: number = 0,
    isShineTarget: string = "1",
    arrowDirection: number = 0,
    $targetObj: DisplayObject = null,
  ) {
    ...
    if (!targetObj) {
      Logger.error(
        "[NewbieBaseActionMediator]createGuildFrame1 目标对象获取错误",
        targetArgs,
      );
      return;
    }
    ...
    NewbieResizeUtils.addTarget(
      frame,
      targetObj,
      new Laya.Point(relativeX, relativeY),
    );
    frame.showTween();
    ...
    frame.pos(
      pos.x + relativeX + anchorOffsetX,
      pos.y + relativeY + anchorOffsetY,
      true,
    );
    return frame;
  }
   ...
   * @param isShineTarget  目标对象是否闪烁
   */
  public static createGuildFrame2(
    targetArgs: string,
    relativeX: number,
    relativeY: number,
    value: number = 0,
    type: number = 0,
    isShineTarget: string = "0",
    appendFuncTargetArgs: string = "",
    ...
    let targetObj: DisplayObject =
      NewbieBaseActionMediator.getGuildTarget(targetArgs);
    if (!targetObj) {
      Logger.error(
        "[NewbieBaseActionMediator]createGuildFrame2 目标对象获取错误",
        targetArgs,
      );
      return;
    }
    ...
    let tempSkillName: string = NewbieBaseActionMediator.getJobSkillTempName(
      ArmyManager.Instance.thane.templateInfo.Job,
    );

    let content: string = NewbieBaseActionMediator.getContentByValue(value);
    if (type == 1) {
      content = LangManager.Instance.GetTranslation(
        "newbie.newbieBaseActionMediator.getSkill",
        tempSkillName,
      );
    }
    if (type == 2) {
      content = LangManager.Instance.GetTranslation(
        "newbie.newbieBaseActionMediator.clickUseSkill",
        tempSkillName,
      );
    }
    let pos = NewbieBaseActionMediator.localToGlobal(
    ...
    if (appendFuncTargetArgs) {
      let appendFuncTargetObj =
        NewbieBaseActionMediator.getAppendFuncTarget(appendFuncTargetArgs);
      if (appendFuncTargetObj) {
        ...
        Logger.error(
          "[NewbieBaseActionMediator]createGuildFrame2 目标对象2获取错误",
          appendFuncTargetArgs,
        );
      }
    }
   ...
   * @param value  对话框内容
   */
  public static createGuildFrame4(
    callback: Function,
    callArgs,
    targetArgs: string,
    relativeX: number,
    ...
    if (targetObj) {
      ...
      pos = NewbieBaseActionMediator.localToGlobal(
        targetObj,
        new Laya.Point(0, 0),
      );
   ...
   * @param content  箭头内容
   */
  public static createGuildArrow1(
    targetArgs: string,
    relativeX: number,
    relativeY: number,
    direction: number,
    value: number = 0,
    addToTarget: number = 0,
  ) {
    relativeX = Number(relativeX);
    relativeY = Number(relativeY);
    ...
    if (!targetObj) {
      Logger.error(
        "[NewbieBaseActionMediator]createGuildArrow1 目标对象获取错误",
        targetArgs,
      );
      return;
    }
    ...
    let arrow: NoviceArrowView = new NoviceArrowView(
      direction,
      content,
      true,
      null,
  ...
  public static localToGlobal(
    targetObj: DisplayObject | fgui.GComponent,
    srcPos: Laya.Point,
  ): Laya.Point {
    let gPt: Laya.Point;
    if (targetObj instanceof fgui.GObject) {
      gPt = (targetObj as fgui.GObject).displayObject.localToGlobal(
        new Laya.Point(0, 0),
        false,
        Laya.stage,
      );
  ...
  public static tweenTarget(
    ...
    let targetObj: DisplayObject =
      NewbieBaseActionMediator.getGuildTarget(targetArgs);
    let referTarget: DisplayObject =
      NewbieBaseActionMediator.getGuildTarget(referTargetArgs);
    ...
    if (referTarget && referTarget.parent) {
      let p = (referTarget.parent as DisplayObject).localToGlobal(
        new Laya.Point(referTarget.x, referTarget.y),
      );
    ...
    if (isFrom == "1") {
      ...
      Logger.xjy(
        "[NewbieBaseActionMediator]tweenTarget",
        targetArgs.x,
        targetArgs.y,
      );
  ...
  public static tweenCompleteCall(
    targetObj: object,
    ...
    if (disposeTarget) {
      ObjectUtils.disposeObject(targetObj);
      targetObj = null;
    }
    NewbieUtils.execFunc(callback, callArgs);
  }
   ...
   */
  public static setMainToolBarBtn(
    callback: Function,
    callArgs: Array<any>,
    $showArgs: string = null,
    $addShow: number = 0,
    $aniType: number = 1,
  ) {
    ...
    let showArgs: any[] =
      $showArgs && $showArgs != "" ? $showArgs.split("_") : null;
    if (!showArgs) return;
    ...
    if (addBtnData.locationType != EmMainToolBarBtnLocationType.Cow) {
      ...
      mainToolBar.doMoveBtn(btn, true, NewbieBaseActionMediator.playBtnEffect, [
        btn.view,
        callback,
        callArgs,
      ]);
   ...
   * 添加迷雾信息
   */
  public static moveOuterCityMap(
    posX: number,
    posY: number,
    isFog: number = 0,
  ) {
    ...
    OuterCityManager.Instance.mapView.novimotionTo(
      new Laya.Point(
        posX - StageReferance.stageWidth / 2,
        posY - StageReferance.stageHeight / 2,
      ),
    );
   ...
   * 自动寻路并攻击 unuse
   */
  public static autoMoveAndAttack(
    callback: Function,
    callArgs: Array<any>,
    posX: number,
    posY: number,
  ...
  public static sendOuterCityBattleAndQueue() {
    let str: string = LangManager.Instance.GetTranslation(
      "newbie.newbieBaseActionMediator.groupSoldiersJoin",
    );
   ...
   * @param callArgs  回调参数
   */
  public static createSelectRoleView(callback: Function, callArgs: Array<any>) {
    //此方法已无用
    NewbieUtils.execFunc(callback, callArgs);
  }
   ...
   * @returns
   */
  public static closeWindowsByTypes(
    args: string,
    reverse: string = "1",
    dispose: string = "1",
  ) {
  ...
  public static createGuildTarget($args: string) {
    ...
    switch (type) {
      case 1: //技能图标(param: 技能模板id)
        if (Number(args[0]) == 202) {
          tempSkillTemp = NewbieBaseActionMediator.getJobSkillTempId(
            ArmyManager.Instance.thane.templateInfo.Job,
          );
        }
        let skillTemp: t_s_skilltemplateData =
          TempleteManager.Instance.getSkillTemplateInfoById(tempSkillTemp);
        let skillItem = fgui.UIPackage.createObject(
          EmPackName.Battle,
          "BattleSkillItemCircle",
        ) as fgui.GComponent;
        let skillIcon = skillItem
          .getChild("item")
          .asCom.getChild("Img_Icon").asLoader;
    ...
    LayerMgr.Instance.addToLayer(
      NewbieBaseActionMediator._createTarget,
      EmLayer.NOVICE_LAYER,
    );
  }
  ...
  public static cleanCreateTarget() {
    ...
    if (gObj) {
      ObjectUtils.disposeObject(gObj);
      NewbieBaseActionMediator._createTarget = null;
    } else {
      ObjectUtils.disposeObject(NewbieBaseActionMediator._createTarget);
      NewbieBaseActionMediator._createTarget = null;
    }
  }
   ...
   * @param $referTargetArgs  参照对象参数, 默认为空
   */
  public static setTargetPosAndScale(
    $targetArgs: string,
    $x: string = "0",
    $y: string = "0",
    $scaleX: string = "1",
  ...
  public static sendSetSkillFastKey($value: string) {
    let arr: any[] = NewbieBaseActionMediator.getSkillTempsSontypeString(
      ArmyManager.Instance.thane.templateInfo.Job,
    ).split("_");
    let value: string = "";
    for (let key in arr) {
  ...
  public static showNewSkillInBattle(itemIdx: number, skillTempId: number) {
    ...
    Logger.xjy(
      "[NewbieBaseActionMediator]showNewSkillInBattle itemIdx:",
      itemIdx,
      "skillTempId:",
      skillTempId,
    );
    ...
    let item: BattleSkillItemII =
      BattleManager.Instance.battleUIView.bottomBar.skillList.skillItemViews[
        itemIdx
      ];
    if (skillTempId == -1) {
      item.data = null;
      ...
      if (skillTempId == 202) {
        temSkillTempId = NewbieBaseActionMediator.getJobSkillTempId(
          ArmyManager.Instance.thane.templateInfo.Job,
        );
      }
      let skillTemp = TempleteManager.Instance.getSkillTemplateInfoById(
        temSkillTempId,
  ...
  public static getJobSkillTempName(value: number): string {
    let skillTemp: t_s_skilltemplateData = ConfigMgr.Instance.getTemplateByID(
      ...
      NewbieBaseActionMediator.getJobSkillTempId(value),
    );
   ...
   */
  public static setTargetDisplayIndex(
    $targetArgs: string,
    $idx: number = -1,
    $showLayer: number = -1,
    $isCenter: string = "0",
    $isAutoCenter: string = "0",
    $maskType: number = 0,
  ) {
    $idx = Number($idx);
    $showLayer = Number($showLayer);
  ...
  public static moveNPCByTween(
    ...
    if (completeWhenWalkOver) {
      TweenMax.to(npcView, $moveTime, {
        x: $endX,
        y: $endY,
        onComplete: NewbieBaseActionMediator.moveByTweenCompleteCall,
        onCompleteParams: [npcView, callback, callArgs],
  ...
  private static moveByTweenCompleteCall(
    armyView,
    callback: Function,
    callArgs: Array<any>,
  ) {
   ...
   */
  public static setKeyboardState(
    callback: Function,
    callArgs: Array<any>,
    isEnabled: string = "0",
    setCanEnable: number = 0,
  ) {
    // switch (setCanEnable) {
    // 	case 1:
  ...
  public static clearCurrentGameAction() {
    ...
    if (curAction) {
      if (curAction instanceof MapBaseAction)
        (curAction as MapBaseAction).finished = true;
      curAction.dispose();
    }
  }
  ...
  public static getGuildTarget($args: string): any {
    ...
    switch (type) {
      ...
      case 3: //内城建筑(param: 建筑sontype)
        ...
        if (curScene.castleMap) {
          let bInfoII: BuildInfo =
            BuildingManager.Instance.getBuildingInfoBySonType(Number(args[0]));
          return curScene.castleMap.buildingsDic.get(
            bInfoII.buildingId,
      ...
      case 5: //战斗界面技能item(param1: item位置, param2: 配"icon"表示取图标)
        item =
          BattleManager.Instance.battleUIView.bottomBar.skillList
            .skillItemViews[Number(args[0])];
        if (args.length >= 2)
          return (item[args[1]] as fgui.GLoader).displayObject;
        else return item.value.displayObject;
      case 6: //装备格子(格子位置)
        return (
          NewbieUtils.getFrame(EmWindow.SRoleWnd) as SRoleWnd
        ).getItemViewByPos(Number(args[0]));
      case 7: //背包格子(格子位置)
        return (
          NewbieUtils.getFrame(EmWindow.SRoleWnd) as SRoleWnd
        ).getBaseBagItemByPos(Number(args[0])).item;
      case 8: //背包格子(SonType)
        return (
          NewbieUtils.getFrame(EmWindow.SRoleWnd) as SRoleWnd
        ).getBaseBagItemByType(Number(args[0])).item;
      case 9: //技能学习按钮
        attr = (NewbieUtils.getFrame(EmWindow.Skill) as SkillWnd).skillPanel[
          String(args[0])
        ];
      ...
      case 10: //技能加点按钮(param: 技能位置)
        return (
          NewbieUtils.getFrame(EmWindow.Skill) as SkillWnd
        ).skillPanel.getSkillItemByIndex(Number(args[0]));
      case 11: //右上角工具条按钮(param: 按钮类型)
        return HomeWnd.Instance.getSmallMapBar().getButtonByType(
          Number(args[0]),
        );
      case 12: //冷却队列item(param: 队列类型)
        return (
          NewbieUtils.getFrame(EmWindow.SpaceTaskInfoWnd) as SpaceTaskInfoWnd
        ).getQueueItemByType(Number(args[0]));
      case 13: //挑战对象item(param1: item位置, param1: item属性)
        item = (
          NewbieUtils.getFrame(EmWindow.Colosseum) as ColosseumWnd
        ).getItemByIndex(Number(args[0]));
        switch (args.length) {
          case 2:
      ...
      case 14: //社交界面
        switch (Number(args[0])) {
          case 1:
            return (NewbieUtils.getFrame(EmWindow.FriendWnd) as FriendWnd)
              .btn_addFriend.view;
          default:
      ...
      case 15: //推荐好友
        return (
          NewbieUtils.getFrame(EmWindow.AddFriendsWnd) as AddFriendsWnd
        ).getAddBtnByIndex(Number(args[0]));
      case 18: //悬赏窗口接受按钮(param: item位置)
        return (
          NewbieUtils.getFrame(EmWindow.OfferRewardWnd) as OfferRewardWnd
        ).getOfferTaskItemByIndex(Number(args[0])).Btn_accept;
      case 19: //神学院窗口科技升级按钮(param: item位置)
        return (
          NewbieUtils.getFrame(EmWindow.SeminaryWnd) as SeminaryWnd
        ).getItemByIndex(Number(args[0])).Btn_upgrade;
      ...
      case 26: //天空之城对话框item(param: item位置)
        spacsDialog = NewbieUtils.getFrame(
      ...
      case 29: //内城建筑effect_area(param: 建筑sontype)
        ...
        if (curScene.castleMap) {
          let bInfoII: BuildInfo =
            BuildingManager.Instance.getBuildingInfoBySonType(Number(args[0]));
          return (
            curScene.castleMap.buildingsDic.get(
              bInfoII.buildingId,
            ) as CastleBuildingView
          ).curView; //["effect_area"];
        }
      ...
      case 30: //副本中节点视图(param: 节点id)
        switch (args.length) {
          case 2:
            return CampaignManager.Instance.mapModel.getNodeByNodeId(
              Number(args[0]),
            )[args[1]];
          case 3:
            return CampaignManager.Instance.mapModel.getNodeByNodeId(
      ...
      case 31: //任务追踪item(param: 任务模板id)
        ...
        return (
          NewbieUtils.getFrame(EmWindow.SpaceTaskInfoWnd) as SpaceTaskInfoWnd
        ).getTraceItemByTaskTempId(Number(args[0]));
      case 311: //测试 任务栏点击
        Logger.xjy("[NewbieBaseActionMediator]测试 任务栏点击");
        return (
          NewbieUtils.getFrame(EmWindow.SpaceTaskInfoWnd) as SpaceTaskInfoWnd
        ).getTaskItemByIndex(Number(args[0]));
      case 32: //创建的指引对象
      ...
      case 34: //天空之城地图窗口item(param: npcId)
        ...
        let smalMapRightItem = (
          NewbieUtils.getFrame(EmWindow.SmallMapWnd) as SmallMapWnd
        ).getMapNodeItemByNodeId(Number(args[0]));
        switch (args.length) {
          case 2:
      ...
      case 40: //战斗中觉醒按钮
        return BattleManager.Instance.battleUIView.bottomBar.leftAwakenFigure
          .awakenItem;
      case 41: //战斗中自动战斗按钮
      ...
      case 71:
        ...
        Logger.xjy(
          "[NewbieBaseActionMediator]TODO magicCardSlotView.treasureBtn",
        );
        return null;
      ...
      case 82: //获取传送阵中世界地图11-20
        frame = NewbieUtils.getFrameByType(
      ...
      case 91: //装备界面tip的使用按钮
        btn = (
          NewbieUtils.getFrame(EmWindow.EquipContrastTips) as EquipContrastTips
        ).goodstip1.btn_use;
        return btn;
      case 100: //兵营CasernWnd招募按钮(param: 兵种sontype)
        return (
          NewbieUtils.getFrame(EmWindow.CasernWnd) as CasernWnd
        ).getRecruitPawnBtn(Number(args[0]));
      case 103: //部队中item(param: item位置)
        let allocateFrame = NewbieUtils.getFrame(
          EmWindow.AllocateWnd,
        ) as AllocateWnd;
        let allocateItem = allocateFrame.SoliderList.getChildAt(
          0,
        ) as AllocateItem;
        if (args.length > 0) {
          allocateItem = allocateFrame.SoliderList.getChildAt(
            Number(args[0]),
          ) as AllocateItem;
        }
        return allocateItem;
      ...
      case 105:
        ...
        if (!frame) return;
        return (frame as SkillWnd).skillPanel.getSkillItemByIndex(
          Number(args[0]),
        );
      ...
      case 120: //领取藏宝图界面的一键刷新按钮
        ...
        Logger.xjy(
          "[NewbieBaseActionMediator]TODO 领取藏宝图界面的一键刷新按钮",
        );
        return null;
