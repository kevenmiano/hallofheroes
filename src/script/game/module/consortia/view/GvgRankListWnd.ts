...
export class GvgRankListWnd extends BaseWindow {
    ...
    private initEvent() {
        ...
        this._controller.model.addEventListener(GvgEvent.UPDATE_GUILD_ORDER_GROUP, this.__updateGuildOrderGroupHandler, this);
    }
    ...
    private __updateGuildGroupHandler(): void {
        if (!this._controller.model.gvgScheduling) {
            ...
            MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("GvgRankListFrame.noDate"));  //"本轮公会战暂未排期"
        }
        else if (!this._controller.model.isFightMember) {
            MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("GvgRankListFrame.noQualifications")); //"您的公会本周没有获得参与公会战资格"
        }
        this.refreshView();
    }
    ...
    private refreshView(): void {
        let arr: GuildGroupInfo[] = this._controller.model.getGroupByIndex(this._curDay);
        if (!arr) {
            return;
        }
        ...
        for (let k: number = 0; k < count; k++) {
            //要对名次先进行调整
            let info: any = { team01: arr[k * 2], team02: arr[k * 2 + 1], index: k, day: this._curDay };
    ...
    private removeEvent() {
        ...
        this._controller.model.removeEventListener(GvgEvent.UPDATE_GUILD_ORDER_GROUP, this.__updateGuildOrderGroupHandler, this);
    }
