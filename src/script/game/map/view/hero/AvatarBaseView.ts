...
export class AvatarBaseView extends Laya.Sprite {
	...
	public get shadowAlpha(): number { return this._shadowAlpha; }
	...
	public get infoAlpha(): number { return this._infoAlpha; }
	public set infoAlpha(value: number) {
		this._infoAlpha = value;
		AvatarInfoUILayerHandler.handle_CON_ALPHA(this._uuid, AvatarInfoTag.All, value)
	}
	...
	protected createNickName() {
		...
		AvatarInfoUILayerHandler.handle_CON_CREATE(this._uuid, AvatarInfoTag.NickName, { strokeWidth: 0 })
	}

	protected createConsortiaName() {
		//Logger.info("[AvatarBaseView]createConsortiaName", this.objName, this._uuid)
		AvatarInfoUILayerHandler.handle_CON_CREATE(this._uuid, AvatarInfoTag.ConsortiaName, { strokeWidth: 0 })
	}
	...
	protected createQQDWKName() {
		...
		if (!QQDawankaManager.Instance.isDWK && !QQDawankaManager.Instance.isSuperDWK) return;
		if (this.avatarBaseViewType == eAvatarBaseViewType.SpaceArmy || this.avatarBaseViewType == eAvatarBaseViewType.CampaignArmy || this.avatarBaseViewType == eAvatarBaseViewType.OuterCityArmy) {
			AvatarInfoUILayerHandler.handle_CON_CREATE(this._uuid, AvatarInfoTag.QQ_DWK)
		}
	}

	protected get isDwk(): boolean {
		if (Utils.isQQHall() && this.isSelf) {
			return QQDawankaManager.Instance.isDWK || QQDawankaManager.Instance.isSuperDWK;
		} else {
			return false;
		}
	}
	...
	protected addEvent() {
		if (this._info) {
			this._info.addEventListener(AiEvents.UPDATE_PATHS, this.mNewMoveHandler, this);
		}
	}

	protected removeEvent() {
		if (this._info) this._info.removeEventListener(AiEvents.UPDATE_PATHS, this.mNewMoveHandler, this);
	}
	...
	public execute() {
		...
		if (this._info && this._info.pathInfo && this._info.pathInfo.length > this._info.walkIndex) {
			...
			if (this._info.currentFrame >= this._info.totalFrame) {
				var nextPoint: Laya.Point = this._info.pathInfo[this._info.walkIndexToNext] as Laya.Point;
		...
		if (this.petAvatarView.followType == 1) {
			...
			switch (sceneType) {
				case SceneType.OUTER_CITY_SCENE:
					OuterCityManager.Instance.mapView.worldWalkLayer.addChild(this._petAvatarView);
					break;
	...
	protected nextWalk(point: Laya.Point) {
		...
		var leng: number = this._prePoint.distance(this._nextPoint.x, this._nextPoint.y);
	...
	protected playMovie() {
		...
		var temp: Laya.Point = StringHelper.interpolate(this._nextPoint, this._prePoint, this.info.currentFrame / this.info.totalFrame);
		this.x = temp.x;
		this.y = temp.y;
	}
	...
	protected layoutTxtViewWithNamePosY() {
		this._showNamePosY = this.getPosYBySizeType(this._avatar.sizeType, this._avatar.flight);
	...
	protected setHitArea() {
		...
		if (rect != this._hitRect) {
			...
			this.hitArea.hit.drawRect(rect.x, rect.y, rect.width, rect.height, "#FF0000");
			this.setHitTestAlpha();

			if (GameConfig.debug || Laya.Utils.getQueryString("debug") == "true") {
				if (!this._debugSp) {
					this._debugSp = new Laya.Sprite();
					this._debugSp.alpha = .5;
					this.addChild(this._debugSp);
				}
				this._debugSp.graphics.clear();
				this._debugSp.graphics.drawRect(rect.x, rect.y, rect.width, rect.height, "#FF0000");
	...
	public set y(value: number) {
		...
		AvatarInfoUILayerHandler.handle_CON_POSY(this._uuid, value + this._showNamePosY)
	}
	...
	protected setName(name: string = "", nameColor?: number, grade?: number) {
		...
		AvatarInfoUILayerHandler.handle_NAME_FRAME(this._uuid, (nameColor == 0 ? 1 : nameColor), eFilterFrameText.AvatarName)
	}

	protected showChatPopView(chatData: ChatData) {
		AvatarInfoUILayerHandler.handle_CON_CREATE(this._uuid, AvatarInfoTag.ChatPopView, chatData)
		AvatarInfoUILayerHandler.handle_CON_POSX(this._uuid, this._px)
		AvatarInfoUILayerHandler.handle_CON_POSY(this._uuid, this._py + this._showNamePosY)
	}

	protected hideChatPopView(chatData: ChatData) {
		AvatarInfoUILayerHandler.handle_CON_DISPOSE(this._uuid, AvatarInfoTag.ChatPopView)
	}

	public showConsortia(visible: boolean = true): void {
		//Logger.info("[AvatarBaseView]showConsortia", this.objName, this.uuid, visible)
		AvatarInfoUILayerHandler.handle_CON_VISIBLE(this._uuid, AvatarInfoTag.ConsortiaName, visible)
	}

	public showVipIcon(visible: boolean = true) {
		AvatarInfoUILayerHandler.handle_CON_VISIBLE(this._uuid, AvatarInfoTag.Vip, visible)
	}

	public showQQDWKIcon(visible: boolean = true) {
		AvatarInfoUILayerHandler.handle_CON_VISIBLE(this._uuid, AvatarInfoTag.QQ_DWK, visible)
	}

	public showName(visible: boolean = true) {
		AvatarInfoUILayerHandler.handle_CON_VISIBLE(this._uuid, AvatarInfoTag.NickName, visible)
	}

	public showHoner(visible: boolean = true) {
		AvatarInfoUILayerHandler.handle_CON_VISIBLE(this._uuid, AvatarInfoTag.Appell, visible)
	}
	 ...
	 */
	public showInfo(visible: boolean = true, showFateSkillEffect: boolean = true) {
		AvatarInfoUILayerHandler.handle_CON_VISIBLE(this._uuid, AvatarInfoTag.Info, visible)
	...
	public disposePetInfo() {
		AvatarInfoUILayerHandler.handle_CON_DISPOSE(this._uuid, AvatarInfoTag.PetInfo)
	}
	...
	protected createResourceLoadInfo(url: string, sornStand: any[] = [], sornWalk: any[] = [], position: string, positionAddSign: string = ""): ResourceLoaderInfo {
	...
	public dispatchEvent(eventName: any, data?: any, data2?: any, data3?: any) {
		if (this._event)
			this._event.emit(eventName, data, data2, data3);
	}
	...
	public dispose() {
		...
		this.disposeShadows();
		// this.clearFireView();
		if (this._avatar) ObjectUtils.disposeObject(this._avatar); this._avatar = null;
		if (this._petAvatarView) ObjectUtils.disposeObject(this._petAvatarView); this._petAvatarView = null;
		if (this._fateSkillEffect) ObjectUtils.disposeObject(this._fateSkillEffect); this._fateSkillEffect = null;
		this.removeSelf();
	}
}