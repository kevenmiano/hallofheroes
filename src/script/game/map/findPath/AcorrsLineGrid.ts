...
export class AcorrsLineGrid {
  ...
  public getPsnode2(arr: Laya.Point[]): Laya.Point[] {
    ...
    for (let j: number = 0; j < arr.length; j++) primaryArray.push(arr[j]);
    ...
    for (i = 0; i < primaryArray.length; i++) {
      ...
      if (this._mapModel.hasOwnProperty("getNotHandlerNodeByXY")) {
        let node: CampaignNode = this._mapModel.getNotHandlerNodeByXY(
          test.x,
          test.y,
        );
        if (node) {
          if (node.resetPosX > 0 && node.resetPosY > 0) {
            ...
            if (returnArray.indexOf(lastTest) == -1) returnArray.push(lastTest);
          }
          if (returnArray.indexOf(test) == -1) returnArray.push(test);
        }
  ...
  private isCorrsed(s: Laya.Point, e: Laya.Point): boolean {
    ...
    if (this._isPanX || this._isPanY) {
      ...
      this._k =
        ((e.y - s.y) * this.nodeHeight) / ((e.x - s.x) * this.nodeWidth);
    ...
    for (i = minx; i <= maxx; i++) {
      ...
      if (px >= startPx && px <= endPx) {
        ...
        if (acnode == 1) return false;
        this._checkCornerPoint.x = px;
        this._checkCornerPoint.y = py;
        if (this.checkCorner(this._checkCornerPoint)) return false;
        this.node = new Laya.Point(
          parseInt((px / Tiles.WIDTH).toString()),
          parseInt((py / Tiles.HEIGHT).toString()),
        );
      }
    ...
    acnode =
      s.y < e.y
        ? this.getNodeValueByGridXY(s.x, s.y)
        : this.getNodeValueByGridXY(e.x, e.y);
    for (i = miny; i <= maxy; i++) {
  ...
  private getPx(yy: number): number {
    if (this._isPanY) return this._panYvalue;
    return (yy - this._b) / this._k;
  }

  private getPy(xx: number): number {
    if (this._isPanX) return this._panXvalue;
  ...
  private getNodeValueByXY(x: number, y: number): number {
    return this._findPath8.map()[parseInt((y / this.nodeHeight).toString())][
      parseInt((x / this.nodeHeight).toString())
    ].value;
  }
  private getNodeValueByGridXY(x: number, y: number): number {
    return this._findPath8.map()[parseInt(y.toString())][parseInt(x.toString())]
      .value;
  }
