...
export default class MineralUIMediator implements IMediator, IEnterFrame {
  ...
  private __onClickHandler(evt: Laya.Event) {
    ...
    if (nodeView1) {
      CampaignManager.Instance.mapModel.selectNode =
        nodeView1.nodeInfo as CampaignNode;
      nodeView1.attackFun();
    } else {
      var nodeView2: CampaignNpcPhysics = this._itemDic.get(
        btn.name,
      ) as CampaignNpcPhysics;
      CampaignManager.Instance.mapModel.selectNode =
        nodeView2.info as CampaignNode;
  ...
  public enterFrame() {
    ...
    var layer: CampaignMainBuidingLayer =
      CampaignManager.Instance.mapView.mainBuidingLayer;
    ...
    for (var displayObject of values) {
      if (displayObject instanceof CampaignNpcPhysics) {
        ...
        if (!btn) {
          btn = fgui.UIPackage.createObject(
            EmPackName.Home,
            "mineral_NpcBtn" + nodeInfo.sonType,
          ).asButton;
    ...
    for (const key in list) {
      if (Object.prototype.hasOwnProperty.call(list, key)) {
        ...
        if (
          !npcTarget ||
          !npcTarget.nodeInfo ||
          !npcTarget.nodeInfo.info ||
          npcTarget.nodeInfo.info.types != PosType.COPY_HANDLER
        ) {
          continue;
        }
        var nodeInfo: CampaignNode = npcTarget.nodeInfo as CampaignNode;

        var btn: fgui.GButton = this._btnDic.get(
          npcTarget.x + "," + npcTarget.y,
        );
        if (!btn) {
          btn = fgui.UIPackage.createObject(
            EmPackName.Home,
            "mineral_NpcBtn" + nodeInfo.nodeId,
          ).asButton;
