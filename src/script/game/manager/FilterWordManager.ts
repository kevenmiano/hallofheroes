...
export class FilterWordManager {
	...
	private static vCurrent: string = LangManager.Instance.GetTranslation("yishi.manager.this.Current");
	private static vConsortiaChannel: string = LangManager.Instance.GetTranslation("chatII.datas.getChatChannelName.consortiaChannel");
	private static vTeamChannel: string = LangManager.Instance.GetTranslation("chat.datas.getChatChannelName.TEAM");
	private static vPrivateChat: string = LangManager.Instance.GetTranslation("chat.view.PrivateChatFrame.title");
	private static vWorldChannel: string = LangManager.Instance.GetTranslation("chat.datas.getChatChannelName.SMALLBUGLE");
	private static vBIGBUGLE: string = LangManager.Instance.GetTranslation("chat.datas.getChatChannelName.BIGBUGLE");
	private static vOutAreaBigBugle: string = LangManager.Instance.GetTranslation("yishi.manager.this.OutAreaBigBugle");
	...
	public static setup(analyzer: FilterWordAnalyzer) {
		FilterWordManager.NICKADDWORDS = analyzer.nickNames ? analyzer.nickNames : [];
		FilterWordManager.WORDS = analyzer.words ? analyzer.words : [];
		FilterWordManager.unableChar = analyzer.unableChar ? analyzer.unableChar : "";
	...
	public static startup(): Promise<void> {
		return new Promise(resolve => {
			...
			let callBack = () => {
				...
				if (thirdAnalyzer)
					FilterWordManager.contact(thirdAnalyzer);
			}
	 ...
	 * @return 
	 */
	public static containSpecificChar(checkStr: string, specificStr: string): boolean {
	 ...
	 * 
	 */
	public static isGotForbiddenWords(str: string, level: string = "chat"): boolean {
	...
	private static formatThridForbiddenWords(str: string): string {
		...
		for (let i in FilterWordManager.THIRD_WORDS) {
			if (Object.prototype.hasOwnProperty.call(FilterWordManager.THIRD_WORDS, i)) {
				let thridWords = FilterWordManager.THIRD_WORDS[i];
			...
			if (idx > -1) {
				if (idx1 > -1 && idx2 <= str.length - 1) {
					if (str.slice(idx1, idx1 + 1) == "[" && str.slice(idx2, idx2 + 1) == "]") {
						...
						str = str.slice(0, idx) + this.getXXX(FilterWordManager.CHANNEL_WORDS[i].length) + str.slice(idx2);
					}
				}
			}
		}
	...
	public static filterWrod(s: string): string {
		...
		var re_str1: string = "";
		var re_str2: string = "";
		var ary: string[] = FilterWordManager.WORDS.concat(AgentFilterManager.Instance.agentFilter);
		if (re_str) {
		...
		if (re_str1) {
			re_str2 = this.formatForbiddenWords(re_str1, FilterWordManager.NICKADDWORDS);
		} else if (re_str) {
			re_str2 = this.formatForbiddenWords(re_str, FilterWordManager.NICKADDWORDS);
		} else {
	...
	private static getXXX(len: number): string {
		var startIdx: number = Math.round(Math.random() * (FilterWordManager.REPLACEWORD.length / 4));
		var str: string = FilterWordManager.REPLACEWORD.slice(startIdx, startIdx + len);
		return str;
	}
	//modified by Freeman
