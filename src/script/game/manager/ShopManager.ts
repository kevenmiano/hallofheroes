...
export class ShopManager {
    ...
    private initEvent() {
        ServerDataManager.listen(S2CProtocol.U_C_SHOP_MAIN, this, this.__shopHandler);
        ServerDataManager.listen(S2CProtocol.U_C_SHOP_DISCOUNT, this, this.__shopDiscountHandler);
        ServerDataManager.listen(S2CProtocol.U_C_SHOP_HASBUY, this, this.__getBuyDataInfosSuccess);
        ServerDataManager.listen(S2CProtocol.U_C_BUY_DISCOUNT, this, this.__shopBuyDiscount);
        ServerDataManager.listen(S2CProtocol.U_C_FLASHSALE_FRESH, this, this.__refreshTimeBuyHandler);
        ServerDataManager.listen(S2CProtocol.U_C_SHOP_BUY, this, this.__shoppingHandler);
        ServerDataManager.listen(S2CProtocol.U_C_CHARGE_ORDER, this, this.__chargeOrderRsp);
    }
    ...
    private __shopDiscountHandler(pkg: PackageIn) {
        this._requestBuyState = false;
        let msg: MainDiscountInfo = pkg.readBody(MainDiscountInfo) as MainDiscountInfo;
        ...
        for (let i: number = 0; i < len; i++) {
            ...
            if (shopInfo.id == msg.id) {
                ...
                shopInfo.maxCurrentDate = DateFormatter.parse(msg.maxCurrentDate, "YYYY-MM-DD hh:mm:ss").getTime() / 1000;
                shopInfo.beginDate = DateFormatter.parse(msg.beginDate, "YYYY-MM-DD hh:mm:ss").getTime() / 1000 -
                    PlayerManager.Instance.currentPlayerModel.sysCurTimeBySecond;
                shopInfo.endDate = DateFormatter.parse(msg.endDate, "YYYY-MM-DD hh:mm:ss").getTime() / 1000;
                return;
            }
        }
    ...
    private __shopHandler(pkg: PackageIn) {
        ...
        for (let i = 0; i < msg.discountInfo.length; i++) {
            ...
            discountInfo.vip > 0 ? vipTimeBuyHasChange = true : normalTimeBuyHasChange = true;
        }

        this.model.mainDiscountList = msg.discountInfo;
        this.model.mainNewList = msg.goodInfo;
        this.model.mainUrlList = msg.urlInfo;
        NotificationManager.Instance.dispatchEvent(NotificationEvent.SHOPHOMEPAGE_UPDATA);
        if (vipTimeBuyHasChange || normalTimeBuyHasChange) {//商城限时限购物品刷新时弹出提示
    ...
    private __getBuyDataInfosSuccess(pkg: PackageIn) {
        ...
        if (weekInfoLen == 0) {
            ...
            NotificationManager.Instance.dispatchEvent(NotificationEvent.SHOP_TIME_BUY_REFRESH);
        }else{     
             //周限购
            for (let i = 0; i < weekInfoLen; i++) {
                const element = this.weeklyDataInfo[i];
                let goods: ShopGoodsInfo = this.model.getShopTempInfoById(element.itemId);
                if (goods) {
     ...
     * */
    public sendPresent() {
        ServerDataManager.listen(S2CProtocol.U_C_CHECK_NICK, this, this.__checkNicknameBack);
        ShopManager.Instance.getUserId(this.model.giftInfo.nickname);
    }

    private __checkNicknameBack(pkg: PackageIn) {
        ServerDataManager.cancel(S2CProtocol.U_C_CHECK_NICK, this, this.__checkNicknameBack);

        let msg: CheckNickMsg = pkg.readBody(CheckNickMsg) as CheckNickMsg;
        if (!this.model.giftInfo || msg.nickName.toLowerCase() != this.model.giftInfo.nickname.toLowerCase()) {
            return;
        }

        if (msg.playerId != -1) {
            if (msg.playerId == PlayerManager.Instance.currentPlayerModel.playerInfo.userId) {
                let str: string = LangManager.Instance.GetTranslation("shop.ShopControler.command01");
                MessageTipManager.Instance.show(str);
                return;
            }
            this.sendShoping(this.model.giftInfo.goodId, this.model.giftInfo.count, msg.playerId, this.model.giftInfo.isDiscount);
        } else {
            let str: string = LangManager.Instance.GetTranslation("consortia.view.myConsortia.chairmanPath.ConsortiaTransferFrame.command06");
            MessageTipManager.Instance.show(str);
        }
     ...
     */
    public sendShoping(goodsId: number, count: number, receiveId: number, isDiscount: boolean = false, code: number = 0x1422, useBind: boolean = true, discountTempId: number = 0) {
        if (this.isCannotUsePoint) {
            return
        }

        this._requestBuyState = true;
        receiveId = receiveId == 0 ? PlayerManager.Instance.currentPlayerModel.playerInfo.userId : receiveId;
    ...
    private __refreshTimeBuyHandler(pkg: PackageIn) {
        this._requestBuyState = false;
        let msg: ShopMsg = pkg.readBody(ShopMsg) as ShopMsg;
        let beginTime: number = DateFormatter.parse(msg.refreshDate, "YYYY-MM-DD hh:mm:ss").getTime() / 1000;
        let str: string = TempleteManager.Instance.getConfigInfoByConfigName("FreshSeconds").ConfigValue;
        Logger.log(str);
    ...
    public disEvent() {
        NotificationManager.Instance.dispatchEvent(NotificationEvent.SHOP_TIME_BUY_REFRESH, null);
    }
     ...
     *
     */
    public sendShopingBloo(goodsId: number, count: number = 1, useBind: boolean = true) {
        ServerDataManager.listen(S2CProtocol.U_C_FAST_USE_BLOOD, this, this.__shoppingBlooHandler);

        let receiveId: number = PlayerManager.Instance.currentPlayerModel.playerInfo.userId;
        let msg: ShopMsg = new ShopMsg();
    ...
    protected __shoppingBlooHandler(pkg: PackageIn) {
        ServerDataManager.cancel(S2CProtocol.U_C_FAST_USE_BLOOD, this, this.__shoppingBlooHandler);
    ...
    private __chargeOrderRsp(pkg: PackageIn) {
        ...
        if (msg.result == 1) {
            NotificationManager.Instance.dispatchEvent(NotificationEvent.CHARGE_ORDER_RSP, msg.orderId, msg.productId);
        } else {
            MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("ShopManager.tip1"));
        }
    }
    ...
    private shopingResult(value: number, buyInfoID: number = 0) {
        ...
        switch (value) {
            case 1:
                ...
                break;
            case 2:
                str = LangManager.Instance.GetTranslation("shop.utils.ShopingHelper.command02");
                break;
            case 3:
                str = LangManager.Instance.GetTranslation("shop.utils.ShopingHelper.command03");
                break;
            case 4:
                ...
                break;
            case 5:
                str = LangManager.Instance.GetTranslation("shop.utils.ShopingHelper.command05");
                break;
            case 6:
                if (this.isHandsel == 0) {
                    if (this.getCountById(buyInfoID) == -1) {
                        str = LangManager.Instance.GetTranslation("shop.utils.ShopingHelper.command06");
                    } else {
                        if (this.getCountById(buyInfoID) > 0) {
                            str = LangManager.Instance.GetTranslation("shop.utils.ShopingHelper.command06");
                        } else {
                            str = LangManager.Instance.GetTranslation("shop.view.frame.BuyFrame.sellOver");
                        }
                    }
                } else if (this.isHandsel == 1) {
                    //						str = LangManager.Instance.GetTranslation("shop.utils.ShopingHelper.command13");
                    str = LangManager.Instance.GetTranslation("shop.utils.ShopingHelper.command14");
                }
                break;
            case 7:
                str = LangManager.Instance.GetTranslation("shop.utils.ShopingHelper.command07");
                break;
            case 8:
                str = LangManager.Instance.GetTranslation("shop.utils.ShopingHelper.command08");
                break;
            case 9:
                str = LangManager.Instance.GetTranslation("shop.utils.ShopingHelper.command09");
                break;
            case 10:
                str = LangManager.Instance.GetTranslation("shop.utils.ShopingHelper.command10");
                break;
            case 11:
                str = LangManager.Instance.GetTranslation("shop.utils.ShopingHelper.command11");
                break;
            case 12:
                str = LangManager.Instance.GetTranslation("shop.utils.ShopingHelper.command12");
                break;
            case 15:
                str = LangManager.Instance.GetTranslation("shop.utils.ShopingHelper.command15");
                break;
            case 17:
                str = LangManager.Instance.GetTranslation("shop.utils.ShopingHelper.command17");
                break;
            case 18:
                str = LangManager.Instance.GetTranslation("shop.utils.ShopingHelper.command18");
                break;
            case 19:
                str = LangManager.Instance.GetTranslation("shop.utils.ShopingHelper.command19");
                break;
            case 20:
                str = LangManager.Instance.GetTranslation("shop.utils.ShopingHelper.command20");
                break;
            case 21:
                str = LangManager.Instance.GetTranslation("shop.utils.ShopingHelper.command21");
                break;
        }
        //value 为1 表示成功
        // if (value == 1) {
    ...
    public showShopTimeBuyTip($isVIPShop: boolean = false) {
        ...
        if (FrameCtrlManager.Instance.isOpen(EmWindow.ShopWnd) && this.isTimeBuyOpen) {
            return;
        }
        let str: string = ($isVIPShop ? "tasktracetip.view.ShopTimeBuyTipView.vipContentTxt" : "tasktracetip.view.ShopTimeBuyTipView.contentTxt");
    ...
    private timerDesc(isVIPShop: boolean) {
        let descTime: number = this.timeBuyDesc - PlayerManager.Instance.currentPlayerModel.sysCurTimeBySecond;
        if (descTime <= 0) {
            clearInterval(this._timerId);
            this._timerId = 0;
