...
export default class UpgradeAccountWnd extends BaseWindow {
  ...
  private initData() {
    ...
    this.txt_title.text = LangManager.Instance.GetTranslation(
      "UpgradeAccountWnd.txt_title",
    );
    this.txt_content.text = LangManager.Instance.GetTranslation(
      "UpgradeAccountWnd.txt_content",
    );
    this.btnClaim.title = LangManager.Instance.GetTranslation(
      "UpgradeAccountWnd.btnClaim",
    );
    this.btnUpgrade.title = LangManager.Instance.GetTranslation(
      "UpgradeAccountWnd.btnUpgrade",
    );

    //
    this.dataList = [];
    let dataArray = [];
    let cfgReward = TempleteManager.Instance.getConfigInfoByConfigName(
      "upgradeaccount_rewards",
    );
    if (cfgReward) {
  ...
  onRespBindReward() {
    let state =
      PlayerManager.Instance.currentPlayerModel.playerInfo.userBindState;
    switch (state) {
      case 1: //
        MessageTipManager.Instance.show(
          LangManager.Instance.GetTranslation("UpgradeAccountWnd.RespResult01"),
        );
        break;
      case -1:
        MessageTipManager.Instance.show(
          LangManager.Instance.GetTranslation("UpgradeAccountWnd.RespResult02"),
        );
        break;
      case -2:
        MessageTipManager.Instance.show(
          LangManager.Instance.GetTranslation("UpgradeAccountWnd.RespResult03"),
        );
        break;
      default:
        break;
