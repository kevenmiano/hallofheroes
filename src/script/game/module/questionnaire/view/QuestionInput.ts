...
export default class QuestionInput extends FUI_QuestionInput {
    ...
    public set itemData(value: any) {
        ...
        this.content = LTextInput.create(this.displayObject, "", 20, '#FFECC6', this.inputlist.width, this.inputlist.height, this.inputlist.x, this.inputlist.y, 10);
    ...
    private __onContentChange(evt: Event) {
        ...
        if (input.length > this._itemData.MaxChoose) {
            ...
            let text = LangManager.Instance.GetTranslation("questionnaire.QuestionnaireFrame.maxWordCount", this._itemData.MaxChoose);
