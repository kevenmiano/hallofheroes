...
export class PetLandUIMediator implements IMediator, IEnterFrame {
    ...
    private addEvent(): void {
        if (TaskManage.Instance.cate) {
            TaskManage.Instance.cate.addEventListener(TaskEvent.TASK_ADDED, this.__refreshTaskHandler, this);
            TaskManage.Instance.cate.addEventListener(TaskEvent.TASK_REMOVE, this.__refreshTaskHandler, this);
        }
    ...
    private removeEvent(): void {
        if (TaskManage.Instance.cate) {
            TaskManage.Instance.cate.removeEventListener(TaskEvent.TASK_ADDED, this.__refreshTaskHandler, this);
            TaskManage.Instance.cate.removeEventListener(TaskEvent.TASK_REMOVE, this.__refreshTaskHandler, this);
        }
    ...
    private __refreshTaskHandler(): void {
        ...
        for (let i = 0, len = npcs.length; i < len; i++) {
            ...
            if (nodeInfo && nodeInfo.param4 == "1") {
                ...
                let btn: OneStatusButton = this._btnDic.get(npcTarget);
                if (btn) {
                    let flag: boolean = TaskManage.Instance.cate.hasTaskAndNotCompleted(TaskManage.PET_SYSTEM_OPEN_TASK02);
                    btn.visible = flag;
                }
            }
        }
        ...
        for (let i = 0, len = npcs.length; i < len; i++) {
            ...
            if (btn) {
                ...
                ResMgr.Instance.loadRes(path, (res) => {
                    if (res) {
                        let path: string = "res/game/common/map.space.NPCBtn" + nodeInfo.sonType + ".png";
                        if (nodeInfo.param4 == '3') {//英灵岛boss
    ...
    private __onClickHandler(evt: Laya.Event): void {
        ...
        if (nodeView) {
            CampaignManager.Instance.mapModel.selectNode = nodeView.nodeInfo as CampaignNode;
            nodeView.attackFun();
            evt.stopPropagation();
        }
    }
