    ...
    if (this._info) {
      ...
      if (gTemp) {
        ...
        if (parseInt((gTemp.Property1 / 60).toString()) <= 0)
          this._tempStr = LangManager.Instance.GetTranslation(
            "public.needMinutes",
            gTemp.Property1 % 60,
          );
      ...
      if (this.model.myFarm.grade < this._info.NeedGrades) {
        this.txt_openDescible.text = LangManager.Instance.GetTranslation(
          "farm.view.FarmShopItem.needGradeTip",
          this._info.NeedGrades,
        );
