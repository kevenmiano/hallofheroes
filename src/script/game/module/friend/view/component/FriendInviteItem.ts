...
export class FriendInviteItem extends FUI_FriendInviteItem {
    ...
    private updateView() {
        ...
        this.txt_lv.text = LangManager.Instance.GetTranslation("public.level3", this._info.grades);
    }

    private onBtnAgreeClick() {
        AudioManager.Instance.playSound(SoundIds.CONFIRM_SOUND);
        if (FriendManager.getInstance().countFriend >= FriendManager.getInstance().maxFriendCount) {
            let str: string = LangManager.Instance.GetTranslation("yishi.manager.FriendManager.command02");
            MessageTipManager.Instance.show(str);
            return;
        } else if (FriendManager.getInstance().countFriend < FriendManager.getInstance().maxFriendCount) {
            FriendManager.getInstance().sendAddFriendRequest(this._info.nickName, RelationType.FRIEND, this._info.reqId);
        }
    }
