...
export default class OutyardMemberFirstItem extends FUI_OutyardMemberFirstItem {
  ...
  public set info(value: OutyardUserInfo) {
    if (value) {
      ...
      var stateTxtNone: string = LangManager.Instance.GetTranslation(
        "outyard.OutyardMemberItem.DefenceHandler",
      );
      var stateTxtEnter: string = LangManager.Instance.GetTranslation(
        "outyard.OutyardMemberItem.DefeatedHandler",
      );
      this.statusTxt.text = this._info.defenceAlive
        ? stateTxtNone
        : stateTxtEnter;
