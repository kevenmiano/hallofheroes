...
export class GvgModel extends GameEventDispatcher {
    ...
    public set guildOrderGroup(value: GuildGroupInfo[]) {
        ...
        this.dispatchEvent(GvgEvent.UPDATE_GUILD_ORDER_GROUP, this._guildOrderGroup);
    }
    ...
    public getLeftGvgInfo(): GuildChallengeInfo {
        for (let i = 0, len = this._guildChallengeList.length; i < len; i++) {
            ...
            if (info.attackGuildId == ConsortiaManager.Instance.model.consortiaInfo.consortiaId) {
                return info;
            }
        }
        return null;
    ...
    public getGroupByIndex(index: number): GuildGroupInfo[] {
        if (index < GuildGroupIndex.group10 && this._guildGroup) {
            this._guildGroup = ArrayUtils.sortOn(this._guildGroup, "total" + index, ArrayConstant.NUMERIC | ArrayConstant.DESCENDING);
    ...
    public get isStartGroup(): number {
        let groups: number[] = [GuildGroupIndex.group1,
    ...
    public get isFightMember(): boolean {
        let consortiaId: number = PlayerManager.Instance.currentPlayerModel.playerInfo.consortiaID;
        if (consortiaId == 0) {
            return false;
        }
        for (let i = 0, len = this._guildGroup.length; i < len; i++) {
    ...
    public get isFightManager(): boolean {
        ...
        return (temp && (temp.dutyId == 1 || (temp.dutyId == 2 && temp.isTeamPlayer)));
    }
