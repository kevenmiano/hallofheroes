...
export default class CheckMailWnd extends BaseWindow {
  ...
  private getMailCheckPrize(): any {
    var configInfo: ConfigInfosTempInfo =
      TempleteManager.Instance.getConfigInfoByConfigName("mail_rewards");
  ...
  protected onSend(): void {
    ...
    this.txt_time.text = LangManager.Instance.GetTranslation(
      ...
      MailCheckModel.HAND_REFRESH_LIMIT,
    );
  ...
  protected onSubmit(): void {
    if (!this.checkBtn.selected) {
      let str = LangManager.Instance.GetTranslation(
        "mailCheck.MailCheckFrame.privacy",
      );
      MessageTipManager.Instance.show(str);
      return;
    }
    var checkCode: number = parseInt(this.txt_codeInput.text);
    MailCheckMgr.Instance.submitCheckCode(
      this.txt_addressInput.text,
      checkCode,
      null,
    );
  }
