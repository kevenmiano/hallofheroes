...
export default class TodayNotAlertWnd extends BaseWindow {
    ...
    public OnInitWind() {
        ...
        if (this.params) {
            //默认文本
            this.check1Btn.view.getChild("title").text = LangManager.Instance.GetTranslation("yishi.view.base.ThewAlertFrame.text");
            this.check2Btn.view.getChild("title").text = LangManager.Instance.GetTranslation("BuyFrameI.useBindTxt");
            if (this.params.state) {//0显示所有 , 1隐藏不在提示, 2隐藏绑定钻石
            ...
            if (this.params.checkTxt) {
                ...
                (this.check1Btn.view.getChild("group") as fgui.GGroup).ensureSizeCorrect();
            ...
            if (this.params.check2RickText) {
                ...
                (this.check2Btn.view.getChild("group") as fgui.GGroup).ensureSizeCorrect();
    ...
    confirmBtnClick() {
        if (this.backFunction != null) {
            ...
            if (!hideBind) {
                ...
                this.backFunction(this.check1Btn.selected, this.check2Btn.selected, this._paramArr);
            } else {
