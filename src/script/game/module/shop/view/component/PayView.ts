...
import { NotificationEvent, ShopEvent, VIPEvent } from '../../../../constant/event/NotificationEvent';
...
export class PayView extends FUI_PayView {
    ...
    private showUserVipExps() {
        ...
        let currTempletes: t_s_upgradetemplateData = TempleteManager.Instance.getTemplateByTypeAndLevel(userVipGrade, 4);//VIP升级配置
        let nextTempletes: t_s_upgradetemplateData = TempleteManager.Instance.getTemplateByTypeAndLevel(userVipGrade + 1, 4);//VIP升级配置
        let upExp = 0;
        let nextExp = 0;
        if (nextTempletes) {//
        ...
        if (upExp < nextExp) {
            ...
            if (nextTempletes) {//
                this.txt_vipexp.setVar("nextlevel", (userVipGrade + 1).toString()).flushVars();
            } else {
                this.txt_vipexp.setVar("nextlevel", (userVipGrade).toString()).flushVars();
            }
        } else {
    ...
    private initView() {
        this.showLottery.selectedIndex = ChargeLotteryManager.instance.openChargeLottery ? 1 : 0;
        ...
        //描述文本
        this.rTxtReward1.text = LangManager.Instance.GetTranslation('vip.vipPrivilege.helptip');//VIP特权描述
        //玩家VIP经验
    ...
    private initEvent() {
        ...
        VIPManager.Instance.addEventListener(VIPEvent.UPFRAMEVIEW_CHANGE, this.__onUpdateVipInfo, this);
        NotificationManager.Instance.addEventListener(NotificationEvent.CUSTOMER_SERVICE_SWITCH, this.onSwitch, this);
        NotificationManager.Instance.addEventListener(ShopEvent.PRODUCT_UPDATE_COUNT, this.onUpdateProduct, this);
        NotificationManager.Instance.addEventListener(NotificationEvent.CHARGELOTTERY_RESULT_UPDATE, this.onUpdateProduct, this);
    }
    ...
    private onBtnPrivilegeClick() {
        FrameCtrlManager.Instance.open(EmWindow.VipPrivilege, { returnToWin: EmWindow.ShopWnd, returnToWinFrameData: { page: 7 } }, null, EmWindow.ShopWnd);
    }
    ...
    private onLotteryBtnClick() {
        FrameCtrlManager.Instance.open(EmWindow.Funny, { activityType: FunnyType.RECHARGE_LOTTERY, returnToWin: EmWindow.ShopWnd, returnToWinFrameData: { page: 7 } }, null, EmWindow.ShopWnd);
    }
    ...
    private removeEvent() {
        ...
        VIPManager.Instance.removeEventListener(VIPEvent.UPFRAMEVIEW_CHANGE, this.__onUpdateVipInfo, this);
        NotificationManager.Instance.removeEventListener(NotificationEvent.CUSTOMER_SERVICE_SWITCH, this.onSwitch, this);
        NotificationManager.Instance.removeEventListener(ShopEvent.PRODUCT_UPDATE_COUNT, this.onUpdateProduct, this);
        NotificationManager.Instance.removeEventListener(NotificationEvent.CHARGELOTTERY_RESULT_UPDATE, this.onUpdateProduct, this);
    }
