...
export default class ConsortiaNewWnd extends BaseWindow{
    ...
    private resetConsortiaName(): boolean {
        var consortiaName: string = PlayerManager.Instance.currentPlayerModel.playerInfo.consortiaName;
        if (this._consortiaModel.consortiaInfo.chairmanID != PlayerManager.Instance.currentPlayerModel.playerInfo.userId) return false;
    ...
    private initEvent() {
        this._consortiaModel.addEventListener(ConsortiaEvent.RECRUITNUM_CHANGED, this.__recruitNumHandler, this);
        this._consortiaModel.addEventListener(ConsortiaEvent.UPDA_CONSORTIA_INFO, this.__onConsortiaInfoUpdata, this);
        NotificationManager.Instance.addEventListener(ConsortiaEvent.GET_ALTAR_GOODS, this.setRedStatus, this);
        NotificationManager.Instance.addEventListener(ConsortiaEvent.TASK_INFO, this.setRedStatus, this);
        ConsortiaManager.Instance.model.secretInfo.addEventListener(ConsortiaEvent.TREE_STATE_UPDATE, this.setRedStatus, this);
    }

    private removeEvent() {
        this._consortiaModel.removeEventListener(ConsortiaEvent.RECRUITNUM_CHANGED, this.__recruitNumHandler, this);
        this._consortiaModel.removeEventListener(ConsortiaEvent.UPDA_CONSORTIA_INFO, this.__onConsortiaInfoUpdata, this);
        NotificationManager.Instance.removeEventListener(ConsortiaEvent.GET_ALTAR_GOODS, this.setRedStatus, this);
        NotificationManager.Instance.removeEventListener(ConsortiaEvent.TASK_INFO, this.setRedStatus, this);
        ConsortiaManager.Instance.model.secretInfo.removeEventListener(ConsortiaEvent.TREE_STATE_UPDATE, this.setRedStatus, this);
    }
    ...
    private __recruitNumHandler() {
        UIButton.setRedDot(this.pActivity.member.recruitBtn, this._consortiaModel.recruitNum, 1);
    }
    ...
    private setCooldownTime() {
        if (this._consortiaModel && this._consortiaModel.consortiaInfo && this._consortiaModel.consortiaInfo.currentDate) {
            let time = ((this._consortiaModel.consortiaInfo.currentDate as Date).getTime() - (this._consortiaModel.consortiaInfo.codeBeginDate as Date).getTime()) / 1000;
