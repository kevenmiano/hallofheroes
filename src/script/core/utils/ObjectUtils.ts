...
export default class ObjectUtils {
     ...
     * 
     */
    public static disposeAllChildren(container: DisplayObjectContainer | fgui.GComponent | DisplayObject) {
    ...
    public static removeChildAllChildrenFromStage(container: DisplayObjectContainer) {
        if (!container) return;
        while (container.numChildren > 0) {
            container.removeChildAt(0);
        }
     ...
     */
    public static disposeObject(target: Object) {
        if (!target) return;
        ...
        try {
            let __proto__ = (value) => {
                ...
                if (value['__proto__']) {
                    ...
                    } else if ((parentType instanceof fgui.GComponent) || (parentType instanceof fgui.GObject)) {
    ...
    private static getCopyAbleType(): Array<string> {
        if (ObjectUtils._copyAbleTypes == null) {
            ObjectUtils._copyAbleTypes = new Array();
     ...
     * 
     */
    public static modifyVisibility(value: boolean, ...args: Array<DisplayObject>) {
     ...
     * 
     */
    public static copyPropertyByRectangle(source: Laya.Sprite, rt: Laya.Rectangle) {
        source.x = rt.x;
        source.y = rt.y;
    ...
    public static copyProperties(source: Object, target?: any): any {
        ...
        for (let key of Object.keys(source)) {
            if (null == source[key]) {
                ...
                let sub;
                if (source[key] instanceof Date) {//日期特殊处理
                    sub = source[key]
                } else {
                    sub = (typeof source[key] == 'object') ? ObjectUtils.copyProperties(source[key]) : source[key];
                }
                if (isArray) {
                    newObject.push(sub);
                } else {
     ...
     */
    public static copyPropertiesTo(target: Object, source: Object, useOrigDescript: boolean = false): any {
        if (ObjectUtils._descriptOjbXMLs == null) ObjectUtils._descriptOjbXMLs = new Dictionary();
        ...
        for (let key in source) {
            if (Object.prototype.hasOwnProperty.call(source, key)) {
                ...
                if (copyAbleTypes.indexOf(valueType) != -1) {
                    ...
                    if (valueType == "Date") {
                        target[propertyName] = ObjectUtils.copyDateType(source[propertyName])
                        continue;
                    }
