 ...
 * @description 内城
 */
export default class CastleScene extends BaseSceneView {
  ...
  private fixScreen() {
    ...
    this.castleMap.x = -(
      this.castleMap.getRealWidth() * CastleMapViewII.MAP_SHOW_ANCHOR_POINT.x -
      Resolution.gameWidth / 2
    );
    this.castleMap.y = -(
      this.castleMap.getRealHeight() * CastleMapViewII.MAP_SHOW_ANCHOR_POINT.y -
  ...
  public leaving(): Promise<void> {
    return new Promise(async (resolve) => {
      ...
      ObjectUtils.disposeObject(this.castleMap);
      this.castleMap = null;
      resolve();
    });
  }
