...
export class CastleMapViewII extends Laya.Sprite {
	...
	constructor() {
		...
		this.initWalklayer();
		this.size(CastleConfigUtil.MAP_SCENE_WIDTH, CastleConfigUtil.MAP_SCENE_HEIGHT);
		this.initEvent();
	}
	...
	private initEvent() {
		...
		BuildingManager.Instance.on(BuildingEvent.CREATE_NEW_BUILDING, this.__createNewBuildHandler, this);
		BuildingManager.Instance.on(BuildingEvent.BUILDING_UPGRADE_RECEIVE, this.__buildUpgradeHandler, this);
	}

	private removeEvent() {
		MouseMgr.Instance.remove(this);
		// this._dragObj.enable = false;
		BuildingManager.Instance.off(BuildingEvent.CREATE_NEW_BUILDING, this.__createNewBuildHandler, this);
		BuildingManager.Instance.off(BuildingEvent.BUILDING_UPGRADE_RECEIVE, this.__buildUpgradeHandler, this);
	}
	...
	private addName(value: number) {
		this._walkLayer.buildingsDic.forEach((v: CastleBuildingView, key: number) => {
	...
	private __buildUpgradeHandler(data: BuildInfo) {
		if (data instanceof BuildInfo) {
			...
			if (btemp.MasterType == MasterTypes.MT_INTERNALTECHNOLOGY
	...
	public dragingCallBack() {
		...
		else if (this.x < Resolution.gameWidth - CastleConfigUtil.MAP_SCENE_WIDTH * this.scaleX) {
			this.x = Resolution.gameWidth - CastleConfigUtil.MAP_SCENE_WIDTH * this.scaleX;
		}
		else if (this.x > 0) {
			this.x = 0;
		}

		if ((CastleConfigUtil.MAP_SCENE_HEIGHT * this.scaleY) < Resolution.gameHeight) {
			this.y = 0;
		}
		else if (this.y < Resolution.gameHeight - CastleConfigUtil.MAP_SCENE_HEIGHT * this.scaleY) {
			this.y = Resolution.gameHeight - CastleConfigUtil.MAP_SCENE_HEIGHT * this.scaleY;
		}
		else if (this.y > 0) {
			this.y = 0;
		}
	 ...
	 * @param callArgs 
	 */
	public moveBuildToCenter(sonType: number, isTween: boolean = true, tweenTime: number = 500, callback?: Function, callArgs?: Array<any>) {
		...
		let bInfoII: BuildInfo = BuildingManager.Instance.getBuildingInfoBySonType(Number(sonType));
		...
		if (build.curView) {
			Logger.xjy("[CastleConfigUtil]moveBuildToCenter", bInfoII.templeteInfo.BuildingNameLang, build.curView.width, build.curView.height, "pos[" + pos.x + "," + pos.y + "]", toPosX)
		}
	 ...
	 * @param callArgs 
	 */
	public moveToPos(x: number = -130, y: number = -180, isTween: boolean = false, tweenTime: number = 500, callback?: Function, callArgs?: Array<any>) {
		let posX = x
		let posY = y
		if ((CastleConfigUtil.MAP_SCENE_WIDTH * this.scaleX) < Resolution.gameWidth) {
			posX = 0;
		}
		else if (posX < Resolution.gameWidth - CastleConfigUtil.MAP_SCENE_WIDTH * this.scaleX) {
			posX = Resolution.gameWidth - CastleConfigUtil.MAP_SCENE_WIDTH * this.scaleX;
		}
		else if (posX > 0) {
			posX = 0;
		}
		...
		else if (posY < Resolution.gameHeight - CastleConfigUtil.MAP_RAW_HEIGHT * this.scaleY) {
			posY = Resolution.gameHeight - CastleConfigUtil.MAP_RAW_HEIGHT * this.scaleY;
		}
		else if (posY > 0) {
			posY = 0;
		}
		...
		if (isTween) {
			Laya.Tween.to(this, { x: posX, y: posY }, tweenTime, null, Laya.Handler.create(this, () => {
				NewbieUtils.execFunc(callback, callArgs);
			}
			))
		} else {
	...
	public dispose() {
		...
		ObjectUtils.disposeObject(this._backgroundLayer); this._backgroundLayer = null;
		ObjectUtils.disposeObject(this._animalLayer); this._animalLayer = null;
		ObjectUtils.disposeObject(this._walkLayer); this._walkLayer = null;
