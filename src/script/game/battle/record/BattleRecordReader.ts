...
export class BattleRecordReader extends Laya.EventDispatcher {
    ...
    public static curUrl: string = "http://10.10.4.164/web/record/20220707/3e46d6a5-bee1-4894-b982-410e546e0cab";
    ...
    public static paseData(arraybuffer: ArrayBuffer) {
        ...
        do {
            ...
            if (pkgLen > 0) {
                this._headerTemp.writeByte(this._readBuffer._byteAt_(this._readOffset + 4));
                this._headerTemp.writeByte(this._readBuffer._byteAt_(this._readOffset + 5));
                this._headerTemp.writeByte(this._readBuffer._byteAt_(this._readOffset + 6));
                this._headerTemp.writeByte(this._readBuffer._byteAt_(this._readOffset + 7));
    ...
    public static play() {
        ...
        if (NewbieBaseConditionMediator.checkInScene(1) || NewbieBaseConditionMediator.checkInScene(10)) {
            ...
            Laya.timer.loop(BattleRecordReader.LOOP_TIME, this, this.__enterFrameHandler);
        } else {
    ...
    private static __enterFrameHandler(evt: any) {
        ...
        for (let index = 0; index < this.recordList.length; index++) {
            ...
            if (this.time >= recordData.time && !recordData.handled) {
                ...
                let code: string = StringHelper.pad(recordData.pkg.code.toString(16), 4).toUpperCase();
                NotificationManager.Instance.dispatchEvent(SocketEvent.SERVER_DATA, recordData.pkg)
            }
        }
    }
