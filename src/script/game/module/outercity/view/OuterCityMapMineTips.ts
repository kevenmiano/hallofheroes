...
export default class OuterCityMapMineTips extends BaseWindow {
    ...
    private initView() {
        if (this._info) {
            if (this._info.tempInfo) {
                this.txt_name.text = this._info.tempInfo.NameLang + " " + LangManager.Instance.GetTranslation("public.level3",this._info.tempInfo.Grade);
                this.txt_count.text = "(" + this.outerCityModel.occupyCount(this._info) + "/" + this._info.tempInfo.Property1 +")";
                ...
                let owerTempInfo:t_s_mapphysicpositionData =  ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_mapphysicposition, propterty2.toString()) as t_s_mapphysicpositionData;
    ...
    private initEvent() {
        ...
        this.list.itemRenderer = Laya.Handler.create(this, this.renderCountList, null, false);
    }
