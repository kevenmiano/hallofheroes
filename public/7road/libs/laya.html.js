(function (exports, Laya) {
        ...
        static _multiLineLayout(element) {
            ...
            for (i = 0; i < n; i++) {
                ...
                if (oneLayout instanceof IHtml.HTMLBrElement) {
                    ...
                    htmlWord = oneLayout;
                    if (curLine.elements.length == 0 && htmlWord.char === ' ') {
                        continue
                    }
                    if (!htmlWord.isWord) {
                        // 修复laya文本数字结尾换行问题
    ...
    class HTMLElement {
        ...
        _getWords() {
            ...
            if (!txt || txt.length === 0)
                return null;
            var words = this._text.words;
            var splitWords = txt.split(HTMLElement.SplitRegex);
            if (words && words.length === splitWords.length)
                return words;
            words === null && (this._text.words = words = []);
            words.length = splitWords.length;
            var size;
            var style = this.style;
            var fontStr = style.font;
            for (var i = 0, n = splitWords.length; i < n; i++) {
                size = Laya.ILaya.Browser.measureText(splitWords[i], fontStr);
                words[i] = Laya.HTMLChar.create().setData(splitWords[i], size.width, size.height || style.fontSize, style);
            }
            return words;
        }
        _isChar() {
    ...
    //0400-052f 俄语
    // HTMLElement.defaultRegex = "(?=[\\u00BF-\\u03FF\\u0530-\\u1FFF\\u2C00-\\uD7FF]|\\b|\\s|\\d)(?![ÇĞIİÖŞÜçğıiöşüàèìòùÀÈÌÒÙáéíóúýÁÉÍÓÚÝâêîôûÂÊÎÔÛãñõÃÑÕäëïöüÿÄËÏÖÜŸåÅçÇæÆœŒðÐøØß。，！、》…）)}”】\\.\\,\\!\\?\\]\\:])(?![\\u060C-\\u06FE])";
    HTMLElement.defaultRegex = "(?=[\\u00BF-\\u1FFF\\u2C00-\\uD7FF]|\\b|\\s|\\d)(?![ÇĞIİÖŞÜçğıiöşüàèìòùÀÈÌÒÙáéíóúýÁÉÍÓÚÝâêîôûÂÊÎÔÛãñõÃÑÕäëïöüÿÄËÏÖÜŸåÅçÇæÆœŒðÐøØß。，！、》…）)}”】\\.\\,\\!\\?\\]\\:])(?![\\u060C-\\u06FE])(?![\\u0400-\\u052F])";
    HTMLElement.SplitRegex = new RegExp(HTMLElement.defaultRegex, "i");
    ...
    class HTMLImageElement extends HTMLElement {
        ...
        onloaded() {
            if (!this._style || !this._tex)
                return;
            var style = this._style;
            var w = style.widthed(this) ? -1 : this._tex.width;
            var h = style.heighted(this) ? -1 : this._tex.height;
            if (!style.widthed(this) && this._width != this._tex.width && this.width <= 0) {
                this.width = this._tex.width;
                // this.width = 30;
                this.parent && this.parent._layoutLater();
            }
            if (!style.heighted(this) && this._height != this._tex.height && this.height <= 0) {
