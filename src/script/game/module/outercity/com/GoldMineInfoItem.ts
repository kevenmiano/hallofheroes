...
export default class GoldMineInfoItem extends FUI_GoldMineInfoItem {
    ...
    private attackBtnHandler() {
        if (!StringHelper.isNullOrEmpty(this._wild.info.occupyLeagueName) && this._wild.info.occupyLeagueName != PlayerManager.Instance.currentPlayerModel.playerInfo.consortiaName) {
            MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("GoldMineInfoView.descTxt4"));
            return;
        }
        if (this.outerCityModel.checkPlayerOccpuyNumberIsMax()) {//玩家占领总数量达到上限
            if (SharedManager.Instance.outercityAttackTotalNeedTips) {//需要提示
                let content: string = LangManager.Instance.GetTranslation("GoldMineInfoItem.attackAllTips");
                UIManager.Instance.ShowWind(EmWindow.TodayNotAlert, { content: content, backFunction: this.attackAllConfirmHandler.bind(this), closeFunction: null, state: 2 });
            } else {
            ...
            if (this.outerCityModel.occupyCount(this._wild) >= this._wild.tempInfo.Property1) { //在这个矿点达到上限了不能占领
                if (SharedManager.Instance.outercityAttackCurrentNeedTips) {//需要提示
                    let content: string = LangManager.Instance.GetTranslation("GoldMineInfoItem.attackCurrentTips");
                    UIManager.Instance.ShowWind(EmWindow.TodayNotAlert, { content: content, backFunction: this.attackCurrentConfirmHandler.bind(this), closeFunction: null, state: 2 });
                } else {
                    this.attackOperation();
                }
            }
    ...
    private attackOperation() {
        ...
        OuterCitySocketOutManager.sendAttackMine(this.outerCityModel.mapId, posX, posY, this._info.nodeId, this._info.sonNodeId);
    }
    ...
    private refreshView() {
        if (this._info) {
            if (this._info.occupyPlayerId != 0) {
                ...
                this.userNameTxt.text = LangManager.Instance.GetTranslation("public.notHave");
            ...
            if (this._mapMineData) {
                this.goldNameTxt.text = LangManager.Instance.GetTranslation("public.level3", this._mapMineData.Grade);
            }
        }
    }
