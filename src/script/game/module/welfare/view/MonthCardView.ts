...
export default class MonthCardView extends FUI_MonthCardView {
  ...
  onInitView() {
    ...
    let monthCardData: t_s_rechargeData[] =
      TempleteManager.Instance.getRechargeTempletes(
        ProductType.MONTH_RECHARGE,
        1,
      ); //普通月卡
    let monthCardSuperData: t_s_rechargeData[] =
      TempleteManager.Instance.getRechargeTempletes(
        ProductType.MONTH_RECHARGE,
        2,
      ); //超级月卡
  ...
  private createDataObj(
    diamond: number = 0,
    bindDiamond: number = 0,
    lasttime: number = 0,
    cost: number = 0,
  ): object {
    let obj = {
