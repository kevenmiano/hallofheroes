...
export default class OutyardManager {
    ...
    constructor() {
        ...
        this.defenceDebuffArr = ConfigInfoManager.Instance.getStackHeadDefenceDebuff();
    }
    ...
    private initEvent() {
        ServerDataManager.listen(S2CProtocol.U_C_STACK_HEAD_STATE, this, this.__stateInfoHandler);
        ServerDataManager.listen(S2CProtocol.U_C_STACK_HEAD_FULL, this, this.__fullInfoHandler);
        ServerDataManager.listen(S2CProtocol.U_C_STACK_HEAD_UPDATE, this, this.__updateHandler);
        ServerDataManager.listen(S2CProtocol.U_C_STACK_HEAD_USER_LIST, this, this.__userlistHandler);
        ServerDataManager.listen(S2CProtocol.U_C_STACK_HEAD_BATTLE_DEFENCE, this, this.__battleDefenceHandler);//战斗中对手信息
        ServerDataManager.listen(S2CProtocol.U_C_STACK_HEAD_SELF, this, this.__selfInfoHandler);
        ServerDataManager.listen(S2CProtocol.U_C_STACK_HEAD_SERVERSTATE, this, this.__broadInfoHandler);
        ServerDataManager.listen(S2CProtocol.U_C_STACK_HEAD_RESP, this, this.__exitHandler);
        ServerDataManager.listen(S2CProtocol.U_C_STACK_HEAD_REPORT, this, this.__reportHandler);
    }

    //战报
    private __reportHandler(pkg: PackageIn) {
        let msg: StackHeadReportListMsg = pkg.readBody(StackHeadReportListMsg) as StackHeadReportListMsg;
        if (msg) {
            if (msg.type == 1) {//进攻列表
                ...
                for (let i: number = 0; i < msg.list.length; i++) {
                    outyardReportInfo = this.readReport(msg.list[i] as StackHeadReportMsg);
                    this._AttackReportArr.push(outyardReportInfo);
                }
                this._AttackReportArr = ArrayUtils.sortOn(this._AttackReportArr,"timestamp",ArrayConstant.DESCENDING);
            } else if (msg.type == 2) {//防守列表
                ...
                this._defenceReportArr = ArrayUtils.sortOn(this._defenceReportArr,"timestamp",ArrayConstant.DESCENDING);
            }
        ...
        if (this._AttackReportArr && this._AttackReportArr.length > this.MAX_COUNT 
            || this._defenceReportArr && this._defenceReportArr.length > this.MAX_COUNT) {
            NotificationManager.Instance.dispatchEvent(NotificationEvent.OUTYARD_REFRESH_REPORT);
        }
        else {
            NotificationManager.Instance.dispatchEvent(NotificationEvent.OUTYARD_INIT_REPORT);
        }
    }

    private readReport(stackHeadReportMsg: StackHeadReportMsg): OutyardReportInfo {
        ...
        outyardReportInfo.sourceUserNickName = stackHeadReportMsg.sourceUserNickName;
        ...
        outyardReportInfo.defenceDebuffLevel = stackHeadReportMsg.defenceDebuffLevel;
    ...
    private __exitHandler(pkg: PackageIn) {
        let msg: StackHeadRespMsg = pkg.readBody(StackHeadRespMsg) as StackHeadRespMsg;
        if (msg.op == OutyardManager.CLOSE_FRAME) {
            if (msg.result) {
                NotificationManager.Instance.dispatchEvent(NotificationEvent.OUTYARD_CLOSE_WND);
            }
        }
    }

    private __broadInfoHandler(pkg: PackageIn) {
        let msg: StackHeadStateMsg = pkg.readBody(StackHeadStateMsg) as StackHeadStateMsg;
        if (this._stateMsg) {
        ...
        NotificationManager.Instance.dispatchEvent(NotificationEvent.OUTYARD_STATE_INFO);
    }
    ...
    private __fullInfoHandler(pkg: PackageIn) {
        let msg: StackHeadFullMsg = pkg.readBody(StackHeadFullMsg) as StackHeadFullMsg;
        ...
        if (msg.op == OutyardManager.OPEN_FRAME) {
            ...
            for (i = 0; i < guildList.length; i++) {
                ...
                if(guildMsg.pos == msg.battleField.myGuildPos){//是我自己
                    guild.isMySelf = true;
                }
                else{
            ...
            NotificationManager.Instance.dispatchEvent(NotificationEvent.OUTYARD_OPEN_FRAME);
        }
        NotificationManager.Instance.dispatchEvent(NotificationEvent.OUTYARD_FULL_INFO);
    }

    //外域更新数据
    private __updateHandler(pkg: PackageIn) {
        let msg: StackHeadUpdateMsg = pkg.readBody(StackHeadUpdateMsg) as StackHeadUpdateMsg;
        let i: number;
        let j: number;
        let guild: OutyardGuildInfo;
        ...
        NotificationManager.Instance.dispatchEvent(NotificationEvent.OUTYARD_UPDATE_INFO, msg.attackList);
    }

    //外域公会成员列表
    private __userlistHandler(pkg: PackageIn) {
        let msg: StackHeadGuildUserListMsg = pkg.readBody(StackHeadGuildUserListMsg) as StackHeadGuildUserListMsg;
        this._seniorArr = [];
        ...
        let i: number = 0;
        let j: number = 0;
        if (msg.op == OutyardManager.REQUEST_LIST)//公会守备人员列表
        {
            ...
            this._nomalArr = ArrayUtils.sortOn(this._nomalArr, ["fightingCapacity", "job", "grades"], [ArrayConstant.DESCENDING, ArrayConstant.DESCENDING, ArrayConstant.DESCENDING]);
        ...
        else if (msg.op == OutyardManager.SET_SENIOR)//设置大将
        {
            for (i = 0; i < msg.seniorGeneralUser.length; i++) {
                seniorMsg = msg.seniorGeneralUser[i] as StackHeadSeniorGeneralMsg;
                ...
                if(this._memberList.get(info2.userUid)){
                    let outyardUserInfo:OutyardUserInfo = this._memberList.get(info2.userUid);
            ...
            MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("outyard.setMainSolider"));
        }
        NotificationManager.Instance.dispatchEvent(NotificationEvent.OUTYARD_USERLIST_INFO);
    }
    ...
    private __battleDefenceHandler(pkg: PackageIn) {
        let msg: StackHeadBattleDefenceMsg = pkg.readBody(StackHeadBattleDefenceMsg) as StackHeadBattleDefenceMsg;
        this._battleDefenceMsg = msg;
        NotificationManager.Instance.dispatchEvent(NotificationEvent.OUTYARD_BATTLE_DEFENCE);
    }

    //外域自身信息
    private __selfInfoHandler(pkg: PackageIn) {
        let msg: StackHeadSelfMsg = pkg.readBody(StackHeadSelfMsg) as StackHeadSelfMsg;
        this._selfMsg = msg;
        PlayerManager.Instance.currentPlayerModel.playerInfo.outyardCostEnergy = msg.weekPoint;
        NotificationManager.Instance.dispatchEvent(NotificationEvent.OUTYARD_SELF_INFO);
    }
    ...
    public OperateOutyard(op: number, pos: number = 0, notice: string = "", senior: StackHeadSeniorGeneralMsg = null, param1: number = 0) {
