...
export class EquipTipsContent extends FUI_EquipTipsContent {
    ...
    public set info(info: GoodsInfo) {
        ...
        this.clean();
        if (this.attributeInfo.height > this.attributeInfo.totalGroup.actualHeight) {
    ...
    protected initBase(info: GoodsInfo, temp: t_s_itemtemplateData) {
        ...
        if (this.isFashion(temp)) {
            ...
            if (this.getFashionLevelByInfo(info) == 9) {
                this.txt_fashionLevel.text = LangManager.Instance.GetTranslation("yishi.view.tips.goods.EquipTipsContent.fashionFullLevel");
            } else {
                this.txt_fashionLevel.text = this.getFashionLevelByInfo(info) + LangManager.Instance.GetTranslation("yishi.view.tips.goods.EquipTipsContent.fashionLevel");
            }
            //去除显示阶
        ...
        this.txt_grade.text = LangManager.Instance.GetTranslation("public.level4_space2", temp.NeedGrades);
    ...
    private initInlay(info: GoodsInfo) {
        ...
        if (isFashion) {
            ...
            if (info.templateInfo.TemplateId in booklist) {
                ...
                this.txt_bind.text = LangManager.Instance.GetTranslation('growthFundview.btn_active.title2')
