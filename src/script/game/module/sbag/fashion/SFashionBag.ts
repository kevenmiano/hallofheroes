...
export class SFashionBag extends FUI_SFashionBag {
  ...
  private initEvent() {
    ...
    this.model.addEventListener(
      FashionEvent.FASHION_STORE_CHANGE,
      this.__selectedChangeHandler,
      this,
    );
  ...
  private initItemList() {
    this.list_item.itemRenderer = Laya.Handler.create(
      this,
      this.renderListItem,
      null,
      false,
    ...
    for (let i: number = 0; i < list.length; i++) {
      ...
      if (
        ...
        !GoodsManager.Instance.isHeroGoods(
          this.userArmy.baseHero ? this.userArmy.baseHero.id : 0,
          gInfo,
        )
      ) {
      ...
      if (
        gInfo.templateInfo.Property5 != 1 &&
        gInfo.templateId != FashionModel.FASHION_SOUL &&
        this.model.selectedPanel == FashionModel.FASHION_SWITCH_PANEL
      ) {
        //不能转换的时装
        continue;
      }
  ...
  private renderListItem(index: number, item: FashionBagCell) {
    ...
    if (info) {
      if (
        this._deleteDic[info.pos + "_" + info.objectId + "_" + info.bagType]
      ) {
        ...
        item.info = info;
        this._itemDic.add(
          this.list_item.scrollPane.currentPageX +
            1 +
            "_" +
            info.pos +
            "_" +
  ...
  private sortFashionBag(item1: GoodsInfo, item2: GoodsInfo): number {
    ...
    if (
      item1.bagType == BagType.HeroEquipment &&
      item2.bagType != BagType.HeroEquipment
    ) {
      return -1;
    } else if (
      item1.bagType != BagType.HeroEquipment &&
