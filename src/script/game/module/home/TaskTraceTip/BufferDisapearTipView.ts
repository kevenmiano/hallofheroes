...
export class BufferDisapearTipView extends TaskTraceTipWnd {
  initView() {
    ...
    if (buffer) {
      var goodsInfo: t_s_itemtemplateData =
        TempleteManager.Instance.getGoodsTemplatesByTempleteId(
          buffer.goodsTemplateId,
        );
      var name: string =
        "[color=" +
        GoodsHelp.getGoodColorString(goodsInfo.Profile) +
        "]" +
        buffer.name +
        "[/color]";
      var content: string = LangManager.Instance.GetTranslation(
        "public.BufferDisapear",
      );
      _contentTxt = name + content;
    } else {
      _contentTxt = LangManager.Instance.GetTranslation(
        "taskTraceTips.ContentTxt",
      );
    }
