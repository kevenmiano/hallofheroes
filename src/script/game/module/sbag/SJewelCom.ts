...
export class SJewelCom extends FUI_SJewelCom {
    ...
    onShow() {
        this._max = GoodsManager.Instance.getCountBySontypeAndBagType(GoodsSonType.SONTYPE_SOUL_CRYSTAL, BagType.Player);
        this.txt_input.text = this._max.toString();
        if (!this.isInited) {//避免切换页签的时候重复添加、移除事件
            this.isInited = true;
            this.addEvent();
        }
    ...
    private addEvent() {
        ...
        this.thane.addEventListener(PlayerEvent.JEWELGP_UPDATE, this.__updateHandler, this);
        this.thane.addEventListener(PlayerEvent.JEWELGRADES_UPDATE, this.__updateHandler, this);
    ...
    private removeEvent() {
        ...
        this.thane.removeEventListener(PlayerEvent.JEWELGP_UPDATE, this.__updateHandler, this);
        this.thane.removeEventListener(PlayerEvent.JEWELGRADES_UPDATE, this.__updateHandler, this);
    ...
    private refreshView(): void {
        this.txt_lv0.text = LangManager.Instance.GetTranslation("armyII.viewII.equip.JewelFrame.JewelEffValueTxt", GoodsHelp.getJewelEffecyByGrade(this.thane.jewelGrades));
        this.txt_lv.text = LangManager.Instance.GetTranslation("public.level3", this.thane.jewelGrades);
        this.ownNum = GoodsManager.Instance.getCountBySontypeAndBagType(GoodsSonType.SONTYPE_SOUL_CRYSTAL, BagType.Player);
        var temp: t_s_upgradetemplateData = TempleteManager.Instance.getTemplateByTypeAndLevel(this.thane.jewelGrades + 1, UpgradeType.UPGRADE_TYPE_SOUL);
        if (temp) {
            this.bar.value = this.thane.jewelGp / temp.Data * 100;
            this.bar.getChild('title').asTextField.text = this.thane.jewelGp + '/' + temp.Data;
            this.btn_levelup.enabled = this.ownNum > 0
            this.txt_lv1.text = LangManager.Instance.GetTranslation("armyII.viewII.equip.JewelFrame.JewelEffValueTxt", GoodsHelp.getJewelEffecyByGrade(this.thane.jewelGrades + 1));
            ...
            temp = TempleteManager.Instance.getTemplateByTypeAndLevel(this.thane.jewelGrades, UpgradeType.UPGRADE_TYPE_SOUL);
            this.bar.value = 100;
            this.bar.getChild('title').asTextField.text = temp.Data + '/' + temp.Data;
            this.txt_lv1.text = LangManager.Instance.GetTranslation("buildings.casern.view.PawnLevelUpFrame.command03");
        ...
        NotificationManager.Instance.dispatchEvent(NotificationEvent.UPDATE_FISH_INFO)
    }
    ...
    private onLevelup(): void {
        ...
        Laya.Tween.to(this.txt_add, { y: 240 }, 500, Laya.Ease.sineOut, Laya.Handler.create(this, function () {
