...
export class FarmBagListView extends BaseFguiCom {
	...
	public onBag(){
		...
		if(this.seedBag.visible){
			...
			Laya.Tween.to(this.seedBag,{scaleX:0,alpha:0},30,null,Laya.Handler.create(this,function(){
				this.seedBag.visible = false;
			...
			Laya.Tween.to(this.seedBag,{scaleX:1,alpha:1},30,null,Laya.Handler.create(this,function(){
	...
	private __clickItem(item: FarmBagItem) {
		...
		NotificationManager.Instance.dispatchEvent(FarmEvent.LIGHT_AVAILABLE_LAND, true)
	}
	...
	public refreshView() {
		this.cleanGoods();
		this._seedList = GoodsManager.Instance.getGeneralBagGoodsBySonType(GoodsSonType.SONTYPE_SEED);
		//种子列表按照道具id从小到大排序一下
		this._seedList = ArrayUtils.sortOn(this._seedList, ["templateId"], ArrayConstant.NUMERIC|ArrayConstant.DESCENDING);
