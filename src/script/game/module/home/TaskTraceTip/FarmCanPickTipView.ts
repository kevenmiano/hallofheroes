...
export class FarmCanPickTipView extends TaskTraceTipWnd {
	...
	initView() {
		...
		this.setContentIcon(FUIHelper.getItemURL("Base", "asset.taskTraceTips.FarmIcon"));
	...
	protected __btnHandler(e: Event) {
		...
		if (SceneManager.Instance.currentType == SceneType.CAMPAIGN_MAP_SCENE) {
			var str = LangManager.Instance.GetTranslation("worldboss.helper.WorldBossHelper.tip06");
			MessageTipManager.Instance.show(str);
			return;
		}
