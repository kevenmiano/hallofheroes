 ...
 *
 */
export class MapCameraMediator
  extends Laya.Sprite
  implements IMediator, IEnterFrame
{
  ...
  private get bevelSpeed(): number {
    ...
    let armyView: object =
      CampaignManager.Instance.controller.getArmyView(data);
    if (data && armyView) {
  ...
  public register(target: any) {
    ...
    target.on(
      CampaignMapEvent.LOOK_AT_ROLE,
      this,
      this.__lookAtRoleHandler.bind(this),
    );
  }
  ...
  protected __onScreenRend(evt: Event) {
    if (this._updatePoint.x != 10000) {
      this._updatePoint = CampaignMapCenterCheckHelper.checkOutScene(
        this._updatePoint,
        this._target,
      );
      if (
  ...
  private __lookAtRoleHandler(data: any) {
    ...
    if (targetPosX + stageWidth > this.mapWidth * this._target.scaleX)
      targetPosX = this.mapWidth * this._target.scaleX - stageWidth;
    if (targetPosY + stageHeight > this.mapHeight * this._target.scaleY)
      targetPosY = this.mapHeight * this._target.scaleY - stageHeight;
    targetPosX = -targetPosX;
    targetPosY = -targetPosY;
    if (targetPosX > 0) targetPosX = 0;
    if (targetPosY > 0) targetPosY = 0;
    targetPosX =
      Math.abs(targetPosX - curPosX) < this._xLimen + this.bevelSpeed
        ? curPosX
        : targetPosX;
    targetPosY =
      Math.abs(targetPosY - curPosY) < this._yLimen + this.bevelSpeed
        ? curPosY
        : targetPosY;
    ...
    (<CampaignMapView>this._target).initPosition(
      this._updatePoint.x,
      this._updatePoint.y,
    );
  }
  ...
  public moveXY(para: boolean = false) {
    ...
    let targetPosX: number =
      this._role.x * this._target.scaleX - (stageWidth >> 1);
    let targetPosY: number =
      this._role.y * this._target.scaleY - (stageHeight >> 1) - this._yLimenOff;
    let curPosX: number = this._target.x;
    let curPosY: number = this._target.y;
    ...
    targetPosX =
      this.getAbs(targetPosX - curPosX) < this._xLimen + this.bevelSpeed
        ? curPosX
        : targetPosX;
    targetPosY =
      this.getAbs(targetPosY - curPosY) < this._yLimen + this.bevelSpeed
        ? curPosY
        : targetPosY;
    let offX: number = targetPosX - curPosX;
    let offY: number = targetPosY - curPosY;
    if (this.getAbs(offX) < this._xLimen && this.getAbs(offY) < this._yLimen)
      return;
