export class MapGrid {
  ...
  public static getConvertPoint(value: Laya.Point): Laya.Point {
    return new Laya.Point(
      parseInt((value.x / MapGrid.GRID_WIDTH).toString()),
      parseInt((value.y / MapGrid.GRID_HEIGHT).toString()),
    );
  }
  public static getPositionString(value: string): string {
    return (
      MapGrid.getConvertWidth(parseInt(value.split(",")[0].toString())) +
      "," +
      MapGrid.getConvertHeight(parseInt(value.split(",")[1].toString()))
    );
  }
}
