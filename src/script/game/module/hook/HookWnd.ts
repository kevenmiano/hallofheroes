...
export default class HookWnd extends BaseWindow {
    ...
    public OnInitWind() {
        ...
        //VIP特权体力领取
        let minLevel = VIPManager.Instance.model.getMinGradeHasPrivilege(VipPrivilegeType.HOOK_OUTDATE_STAMINA);
    ...
    helpBtnClick() {
        let title = '';
        let content = '';
        title = LangManager.Instance.GetTranslation("map.campaign.view.frame.HookHelperFrame.title");
        content = LangManager.Instance.GetTranslation("map.campaign.view.frame.HookHelperFrame.helpContent");
    ...
    private initEvent() {
        ...
        NotificationManager.Instance.addEventListener(HookEvent.UPDATE_HOOK, this.updateHookHandler, this);
    }

    private offEvent() {
        // this.list.itemRenderer.recover();
        Utils.clearGListHandle(this.list);
        NotificationManager.Instance.removeEventListener(HookEvent.UPDATE_HOOK, this.updateHookHandler, this);
    }
