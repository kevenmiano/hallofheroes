...
export default class SkillWndData extends FrameDataBase {
  ...
  public get runeHoles() {
    ...
    for (let i = 1; i <= 10; i++) {
      ...
      holeInfo.holeId = i;
      if (
        +RuneHoleInfo.RuneHoleOpenLevel[holeInfo.holeId - 1] >
   ...
   * @param val
   */
  public static getAttributeType(
    val: number,
    showStr1: boolean = false,
  ): string {
    let str: string = "";
    let str1: string = showStr1
      ? LangManager.Instance.GetTranslation("runeGem.title")
      : "";
    switch (val) {
      case 0: //没有效果
  ...
  private addEvent() {
    ServerDataManager.listen(
      S2CProtocol.U_C_RUNE_HOLE_INFO,
      this,
      this._runeHoldHandler,
    );
  }
