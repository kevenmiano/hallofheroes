...
export default class BaseFguiCom extends GameEventDispatcher {
    ...
    // TODO 提供只遍历一层的接口  
    public static autoGenerate(target: fgui.GComponent, toTarget: any, prefix: string = "") {
        ...
        for (var index = 0; index < nodeChildCount; index++) {
            ...
            if (elementName.toUpperCase().indexOf('BTN') != -1) {
                ...
                if (!toTarget[elementName]) {
                    ...
                    toTarget[elementName].callBack = (target: fgui.GButton, evt: Laya.Event) => {
                        let btnName = target.name;
    ...
    set visible(value: boolean) {
        ...
        this.comPart.displayObject && (this.comPart.displayObject.active = value);
    }

    dispose(destred = true) {
        if (this.comPart && !this.comPart.isDisposed) {
            destred && this.comPart.dispose();
        }
    }
}