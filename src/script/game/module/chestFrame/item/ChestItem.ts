...
export default class ChestItem extends FUI_ChestItem {
  ...
  protected onConstruct() {
    ...
    this.txtPlayerName = this.comUnFolder.getChild(
      "txtPlayerName",
    ) as fgui.GLabel;
  }
  ...
  public updateData(userId: number, nickName: string, info: GoodsInfo) {
    ...
    if (
      nickName == PlayerManager.Instance.currentPlayerModel.playerInfo.nickName
    ) {
      this.txtPlayerName.color =
        FilterFrameText.Colors[eFilterFrameText.AvatarName][3 - 1];
    } else {
      this.txtPlayerName.color =
        FilterFrameText.Colors[eFilterFrameText.AvatarName][1 - 1];
    }
