...
export default class PetCtrl extends FrameCtrlBase {
	...
	protected addEventListener() {
		...
		ServerDataManager.listen(S2CProtocol.U_C_UPACCOUNT_ARTIFACT_RECAST, this, this.__artifactRespHandler);
	}

	protected delEventListener() {
		super.delEventListener();
		ServerDataManager.cancel(S2CProtocol.U_C_UPACCOUNT_ARTIFACT_RECAST, this, this.__artifactRespHandler);
	}
	...
	private __artifactRespHandler(pkg: PackageIn) {
		...
		if (msg) {
			if (!msg.composeResult) {
				MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("artifact.result"));
				return;
			}
			let goodsArr = [];
			...
			NotificationManager.Instance.dispatchEvent(NotificationEvent.ARTIFACT_SUCCESS_UPDATE);
			FrameCtrlManager.Instance.open(EmWindow.WishPoolResultWnd, {arr:goodsArr,type:1});
		}
	}
	 ...
	 * @param skillId
	 */
	public static learnSkill(petId: number, goodsPos: number, skillId: number, type: number = 1, flag: boolean = true) {
		Logger.info("[PetCtrl]学习技能: petId=" + petId + ", goodsPos=" + goodsPos + ", skillId=" + skillId, type, flag ? 0 : 1);
		let msg: PlayerPetOpMsg = new PlayerPetOpMsg();
		msg.petId = petId;
		msg.opType = 5;
		msg.value = type;
	 ...
	 * @param skillId
	 */
	public static forgetSkill(petId: number, skillId: number, flag: boolean = true) {
	 ...
	 *
	 */
	public static trainResistance(petId: number, resType: number, goodsPos: number) {
		let msg: PlayerPetOpMsg = new PlayerPetOpMsg();
		msg.petId = petId;
		msg.opType = 8;
		msg.value = resType;
	 ...
	 */
	public static trainGrowthRate(petId: number, type: number = 1, useBind: boolean = true, useBindGoodsType: number = 0) {
		let msg: PlayerPetOpMsg = new PlayerPetOpMsg();
		msg.petId = petId;
		msg.opType = 4;
		msg.value = type;
	 ...
	 */
	public static reqPetEquipStren(petId: number, bagType: number, pos: number, isContinue: boolean) {
	...
	/**英灵洗炼 */
	public static reqPetEquipSuccinc(petId: number, bagType: number, pos: number, op: number) {
		let msg: PetEquipReqMsg = new PetEquipReqMsg();
		msg.petId = petId;
		msg.bagType = bagType;
		msg.pos = pos;
	...
	public static sendPetPotencyActive(petId: number, templateId: number, count: number) {
		let msg: PlayerPetOpMsg = new PlayerPetOpMsg();
		msg.opType = 17;
		msg.petId = petId;
	 ...
	 * @param pos 物品在玩家背包的位置
	 */
	public static sendArtifactReset(pos1: number, objectId1:number,pos2: number,objectId2:number) {
		let msg: StoreReqMsg = new StoreReqMsg();
		msg.holePos = pos1;
		msg.bagPos = pos2;
