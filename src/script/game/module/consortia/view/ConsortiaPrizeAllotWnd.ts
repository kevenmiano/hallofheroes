...
export default class ConsortiaPrizeAllotWnd extends BaseWindow {
  ...
  private initEvent() {
    ...
    this._model.addEventListener(
      ConsortiaEvent.PRIZE_LIST_COUNT_UPDATE,
      this.__prizeListCountUpdateHandler,
      this,
    );
  }
  ...
  private prizeAllotBtnHandler() {
    ...
    var content: string = LangManager.Instance.GetTranslation(
      "consortia.view.myConsortia.building.ConsortiaPrizeAllotFrame.allotConfirmContent",
    );
  ...
  private allotConfirm(b: boolean, flag: boolean) {
    ...
    ConsortiaSocketOutManager.consortiaPrizeAllotConfirm(
      this._model.currentPrizeTemplateId,
      infos,
    );
    this.OnBtnClose();
  }
  ...
  private onSerachHandler() {
    ...
    if (StringHelper.isNullOrEmpty(this.txt_search.text)) {
      var str: string = LangManager.Instance.GetTranslation(
        "consortia.view.myConsortia.building.ConsortiaPrizeAllotFrame.searchTip",
      );
  ...
  private refreshData() {
    ...
    this._prizeList = ArrayUtils.sortOn(
      this._prizeList,
      ["fightingCapacity", "nickName"],
      [
        ArrayConstant.NUMERIC | ArrayConstant.DESCENDING,
        ArrayConstant.CASEINSENSITIVE,
      ],
    );
    let index: number = this._prizeList.indexOf(
      this._model.prizeMemberList[this._model.consortiaInfo.chairmanID],
    );
    this._prizeList.splice(index, 1);
    this._prizeList.unshift(
      this._model.prizeMemberList[this._model.consortiaInfo.chairmanID],
    );
  }
  ...
  refreshView() {
    if (this.searchFlag) {
      ...
      for (let i = 0; i < this._prizeList.length; i++) {
        let info: ThaneInfo = this._prizeList[i];
        if (
          info.nickName.indexOf(StringHelper.trim(this.txt_search.text)) == -1
        ) {
          continue;
        }
        dataArray.push(info);
