...
export default class UIAttrAdvance extends BaseFguiCom {
	...
	private initView() {
		...
		this.itemQualification0.title.text = LangManager.Instance.GetTranslation("pet.growthRate") + colon;
		this.itemQualification1.title.text = LangManager.Instance.GetTranslation("pet.strengthCoe") + colon;
		this.itemQualification2.title.text = LangManager.Instance.GetTranslation("pet.armorCoe") + colon;
		this.itemQualification3.title.text = LangManager.Instance.GetTranslation("pet.intellectCoe") + colon;
		this.itemQualification4.title.text = LangManager.Instance.GetTranslation("pet.staminaCoe") + colon;
		this.tipItem1.setInfo(TemplateIDConstant.TEMP_ID_PET_GROWTH_STONE, true, FUIHelper.getItemURL("Base", "Icon_Unit_Sylph_L"));
	...
	private btnNormalClick() {
		...
		if (!PetModel.hasPet) {
			MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("pet.PetFrame.notHasPet"));
		}
		if (!this.data) return;
		if (!this.data.qualityUpgradeTemplateInfo) {
			MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("PetAdvancedView.tipCommand04"));
			return;
		}
	...
	private checkPointForNormalBack(b: boolean, check1: boolean, useBindPoint: boolean) {
		if (b) {
			if (check1) {
				...
				SharedManager.Instance.petNormalAdvanceCanUsePetStrong = ConfigManager.info.PET_STRONG;
	...
	private willUsedBindForNormal(useBind: boolean): number {
		let result: number = 0;
		let allGoods: any[] = GoodsManager.Instance.getBagGoodsByTemplateId(GoodsCheck.PET_GROWTH_STONE);
		...
		if (useBind && PlayerManager.Instance.currentPlayerModel.playerInfo.giftToken > 0) {
			result = 2;
		}
		return result;
	}
	...
	private checkUseBindForNoraml(useFlag: number) {
		...
		if (SharedManager.Instance.checkIsExpired(SharedManager.Instance.petadvanceWillbeBindCheckDate1)) {
			let content: string;
			if (useFlag == 1) {
				content = LangManager.Instance.GetTranslation("GrowthInfoView.useBindGoodsTip");
				content += "\n" + LangManager.Instance.GetTranslation("mainBar.view.VipCoolDownFrame.useGood");
			} else {
				// 2
				content = LangManager.Instance.GetTranslation("PetAdvancedView.useBindPointTip");
			}

			SimpleAlertHelper.Instance.Show(null, null, null, content, null, null, this.checkUseBindForNoramlBack.bind(this));
		} else {
			this.doNormal(this._tempObj.useBindPoint, SharedManager.Instance.petadvanceWillbeBindCheck1);
		}
	}
	...
	private checkUseBindForNoramlBack(result: boolean, flag: boolean) {
		...
		if (result) {
			...
			SharedManager.Instance.petadvanceWillbeBindCheck1 = this._tempObj.useGoodsType;
	...
	private doNormal(useBindPoint: boolean, useGoodsType: number = 0) {
		...
		PetCtrl.trainGrowthRate(this.data.petId, this._NumOfAdvanced, useBindPoint, useGoodsType);
	...
	private checkPointForSenior() {
		...
		if (SharedManager.Instance.checkIsExpired(SharedManager.Instance.petSeniorAdvancedAlertDate)) {
			let oneCost: number = this.getOnceAdvancedCost();
			let content: string = LangManager.Instance.GetTranslation("PetAdvancedView.tipCommand01", this.NumOfAdvanced, this.NumOfAdvanced);
			let state = 2;
			...
			this.checkWillBeBindForSenior(SharedManager.Instance.petSeniorAdvancedUseBind);
		}
	}

	private checkPointForSeniorBack(b: boolean, check1: boolean, useBindPoint: boolean) {
		if (b) {
			if (check1) {
				...
				SharedManager.Instance.petSeniorAdvanceCanUsePetStrong = ConfigManager.info.PET_STRONG;
	...
	private checkUseBindForSenior(useFlag: number) {
		...
		if (SharedManager.Instance.checkIsExpired(SharedManager.Instance.petadvanceWillbeBindCheckDate2)) {
			let content: string;
			if (useFlag == 1) {
				content = LangManager.Instance.GetTranslation("GrowthInfoView.useBindGoodsTip");
				content += "\n" + LangManager.Instance.GetTranslation("mainBar.view.VipCoolDownFrame.useGood");
			...
			SimpleAlertHelper.Instance.Show(null, null, null, content, null, null, this.checkUseBindForSeniorBack.bind(this));
		} else {
			this.doSenior(this._tempObj.useBindPoint, SharedManager.Instance.petadvanceWillbeBindCheck2);
		}
	}
	...
	private checkUseBindForSeniorBack(result: boolean, useBind: boolean) {
		...
		if (result) {
			...
			SharedManager.Instance.petadvanceWillbeBindCheck2 = this._tempObj.useGoodsType;
	...
	private get growthStoneCount(): number {
		let num: number = GoodsManager.Instance.getGoodsNumByTempId(GoodsCheck.PET_GROWTH_STONE);
		return num;
	}
	...
	private __updateNumberHandler() {
		...
		this._NumOfAdvanced = this.growthStoneCount < this._NumOfAdvanced ? (this.growthStoneCount < 1 ? 1 : this.growthStoneCount) : this._NumOfAdvanced;
		this.simBox.show(
			0,
			this._NumOfAdvanced,
			1,
	...
	private calcTotalExp(grade: number, curExp: number): number {
		let arr: any[] = PetData.qualityUpgradeTemplates[this.data.template.Property2];
	...
	private refresh() {
		let current: t_s_upgradetemplateData = this._data.currentQualityUpgradeTemplateInfo;
		...
		if (next && current) {
			...
			let nextCoeStrengthLimit: number = PetData.calculateCoeLimit(temp, nextQuality, "PowerCoeLimit");
			let nextCoeArmorLimit: number = PetData.calculateCoeLimit(temp, nextQuality, "ArmorCoeLimit");
			let nextCoeIntellectLimit: number = PetData.calculateCoeLimit(temp, nextQuality, "IntelCoeLimit");
			let nextCoeStaminaLimit: number = PetData.calculateCoeLimit(temp, nextQuality, "PhysiCoeLimit");

			let nextCoeStrength: number = PetData.calculateCoeLimitMin(this._data.coeStrength, temp, nextQuality, "CoeStrength");
			let nextCoeArmor: number = PetData.calculateCoeLimitMin(this._data.coeArmor, temp, nextQuality, "CoeArmor");
			let nextCoeIntellect: number = PetData.calculateCoeLimitMin(this._data.coeIntellect, temp, nextQuality, "CoeIntellect");
			let nextCoeStamina: number = PetData.calculateCoeLimitMin(this._data.coeStamina, temp, nextQuality, "CoeStamina");

			this.itemQualification0.txtCurValue.text = this._data.growthRate.toString();
			this.itemQualification1.txtCurValue.text = this._data.coeStrength + " / " + this._data.coeStrengthLimit;
			this.itemQualification2.txtCurValue.text = this._data.coeArmor + " / " + this._data.coeArmorLimit;
			this.itemQualification3.txtCurValue.text = this._data.coeIntellect + " / " + this._data.coeIntellectLimit;
			this.itemQualification4.txtCurValue.text = this._data.coeStamina + " / " + this._data.coeStaminaLimit;

			this.itemQualification0.txtNextValue.text = this._data.getGrowthRate(nextQuality).toString();
			this.itemQualification1.txtNextValue.text = nextCoeStrength + " / " + nextCoeStrengthLimit;
			this.itemQualification2.txtNextValue.text = nextCoeArmor + " / " + nextCoeArmorLimit;
			this.itemQualification3.txtNextValue.text = nextCoeIntellect + " / " + nextCoeIntellectLimit;
			this.itemQualification4.txtNextValue.text = nextCoeStamina + " / " + nextCoeStaminaLimit;
		} else {
			//满级已经没有模板了
			this.itemQualification0.txtCurValue.text = this._data.growthRate.toString();
			this.itemQualification1.txtCurValue.text = this._data.coeStrength + " / " + this._data.coeStrengthLimit;
		...
		if (next) {
			...
			this.progBless.value = Math.floor((this._data.qualityGp / next.Data) * 100);
		} else {
	...
	private getGoodsCount(type: number = 0): number {
		let arr: any[] = GoodsManager.Instance.getBagGoodsByTemplateId(GoodsCheck.PET_GROWTH_STONE);
		let count: number = 0;
	...
	private getOnceAdvancedCost(): number {
		let cost: number = 10;
		let temp: ConfigInfosTempInfo = TempleteManager.Instance.getConfigInfoByConfigName("pet_up_quality");
		if (temp) {
	...
	private checkGrowthStone(value: number): boolean {
		...
		if (userCount < value) {
			let info: ShopGoodsInfo = TempleteManager.Instance.getShopTempInfoByItemId(ShopGoodsInfo.PET_GROWTH_STONE_TEMPID, ShopGoodsInfo.PROP_GOODS);
			if (info) {
				FrameCtrlManager.Instance.open(EmWindow.BuyFrameI, { info: info, count: value });
			} else {
				MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("pet.lackProps"));
			}
			return false;
		}
		return true;
