...
export default class SoundEftManager extends BaseAudioManager {

    private soundChannelMap: TSMap<string, Laya.SoundChannel> = new TSMap();
    private static _instance: SoundEftManager;
    public static get Instance(): SoundEftManager {
        if (!SoundEftManager._instance) SoundEftManager._instance = new SoundEftManager();
        return SoundEftManager._instance;
    }
     ...
     * @param replaceSame 
     */
    public playMusic(state: string, loops: number = 0, replaceSame: boolean = false) {
