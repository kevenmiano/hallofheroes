...
let config, releaseDir;
let versionCon; // 版本管理version.json
let commandSuffix, layarepublicPath;
...
gulp.task("modifyFile_ByteDance", versiontask, function () {
  ...
  if (config.bytedanceInfo.subpack) {
    // 分包
...
gulp.task("version_ByteDance", ["modifyMinJs_ByteDance"], function () {
  ...
  if (config.version) {
    ...
    return gulp.src(srcList).pipe(revCollector()).pipe(gulp.dest(releaseDir));
  }
});
...
gulp.task("pluginEngin_ByteDance", ["version_ByteDance"], function (cb) {
  ...
  let isOldAsProj =
    fs.existsSync(`${workSpaceDir}/asconfig.json`) && !isHasCoreLib;
  let isNewTsProj =
    fs.existsSync(`${workSpaceDir}/src/tsconfig.json`) && !isHasCoreLib;
  ...
  if (
    !EngineVersion ||
    EngineVersion.includes("beta") ||
    !canUsePluginEngine(EngineVersion, minPluginVersion)
  ) {
    throw new Error(
      `该版本引擎无法使用引擎插件功能(engineVersion: ${EngineVersion})`
    );
  }
  ...
  new Promise(function (resolve, reject) {
    ...
    // 修改game.js
    let gameJsPath = path.join(releaseDir, "game.js");
    let platformJs = config.useMinJsLibs
    ...
    .then(function () {
      return new Promise(function (resolve, reject) {
        ...
        for (let i = 0, len = fullRemoteEngineList.length; i < len; i++) {
          ...
          fullRequireItem = config.useMinJsLibs
            ? `require("./libs/min/${item}")`
            : `require("./libs/${item}")`;
          if (indexJsCon.includes(fullRequireItem)) {
            let _item = item.replace(".min.js", ".js"),
              _minItem = item;
            localUseEngineList.push(_item);
            indexJsCon = indexJsCon
              .replace(fullRequireItem + ";", "")
              .replace(fullRequireItem + ",", "")
              .replace(fullRequireItem, "");
            // 如果引用了压缩的类库，将其重命名为未压缩的类库，并拷贝到libs中
            if (config.useMinJsLibs) {
              let oldMinlibPath = path.join(
                releaseDir,
                "libs",
                "min",
                _minItem
        ...
        if (isOldAsProj || isNewTsProj) {
          // 如果as||ts_new语言，开发者将laya.js也写入index.js中了，将其删掉
    ...
    .then(function () {
      return new Promise(async function (resolve, reject) {
        ...
        let layalibsPath = path.join(releaseDir, "laya-libs"),
        ...
        fs.writeFileSync(
          engineplugin,
          JSON.stringify(pluginCon, null, 4),
          "utf8"
        );
        ...
        for (let i = 0, len = localUseEngineList.length; i < len; i++) {
          ...
          let md5Str = await getFileMd5(
            path.join(releaseDir, "laya-libs", fileName)
          );
        ...
        fs.writeFileSync(
          enginesignature,
          JSON.stringify(signatureCon, null, 4),
          "utf8"
        );
        resolve();
      });
