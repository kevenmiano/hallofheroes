...
export function saveZoneData(value: SiteZoneData) {
    SharedManager.Instance.setWindowItem(SiteZoneKey.SITE_ZONE, JSON.stringify(value));
}
...
export function updateZoneData(zoneListData: SiteZoneData[]) {
    ...
    for (let index = 0; index < zoneListData.length; index++) {
        ...
        if ((zoneData.siteID == element.siteID && zoneData.area == zoneData.area) && !Object.is(zoneData, element)) {
...
export function isOversea(): boolean {
    let zoneData = getZoneData();
    if (zoneData && zoneData.area == ZONE_AREA.INLANG) {//地区区域为海外
        return false;
    }
    return true;
}