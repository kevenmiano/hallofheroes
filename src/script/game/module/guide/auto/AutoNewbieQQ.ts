...
export default class AutoNewbieQQ extends AutoNewbieBase {
    ...
    public processFunc(time: number) {
        ...
        this.showArrowTip(!isOpenDialog && !AutoNewbieMgr.Instance.executingAutoPath);
        this.showDialogTip(isOpenDialog && !AutoNewbieMgr.Instance.executingAutoDialog);
    }
    ...
    public processEndFunc() {
        ...
        if (isOpenDialog) {
            ...
            let targetNodeStr = SwitchPageHelp.formatSearchNode(GlobalConfig.CampaignID.NewbieLayer2, GlobalConfig.CampaignNodeID.Node_1000205);
    ...
    public showArrowTip(b: boolean) {
        if (UIManager.Instance.isShowing(EmWindow.SpaceTaskInfoWnd)) {
            let wnd = UIManager.Instance.FindWind(EmWindow.SpaceTaskInfoWnd) as SpaceTaskInfoWnd;
