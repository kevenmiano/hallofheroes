...
export default class AirGardenGameLlkManager extends GameEventDispatcher {
    ...
    public setup() {
        this._model = new LlkModel();
        ServerDataManager.listen(S2CProtocol.U_C_LINK_GAME_INFO, this, this.__llkInfoHandler);
    }
    ...
    private __llkInfoHandler(pkg: PackageIn) {
        ...
        if (msg.reward) {
            ...
            var str: string = LangManager.Instance.GetTranslation("AirGardenGame.hint3", msg.score, count);
            ...
            SimpleAlertHelper.Instance.Show(SimpleAlertHelper.SIMPLE_ALERT, null, prompt, str, confirm1, "", null, AlertBtnType.OC);
        }
        ...
        if (arr.length == LlkModel.ROW * LlkModel.COLUMN) {
            ...
            Logger.info(LangManager.Instance.GetTranslation("llk.view.LlkFrame.title") + msg.points);
        }
