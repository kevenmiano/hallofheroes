...
export class MessageTipManager {
  ...
  private constructor() {
    this.defaultPoint = new Laya.Point(
  ...
  public show(str: string, callBack?: Function, force: boolean = false) {
    if (
      !str ||
      (!force && SceneManager.Instance.currentType == SceneType.BATTLE_SCENE)
    ) {
      callBack && callBack();
      return;
    }
  ...
  private updateShowing() {
    ...
    for (let msgView of this.showingMsgView) {
      Laya.Tween.to(
        msgView,
        { y: this.defaultPoint.y - (length - pos) * this.pad },
        this.padDuration,
        Laya.Ease.linearIn,
      );
      pos++;
  ...
  private aninationMsg(msgView: MessageTipLab) {
    ...
    Laya.Tween.to(
      msgView,
      { alpha: 0.2 },
      this.hideAlphaDuration,
      Laya.Ease.linearNone,
      Laya.Handler.create(this, this.completeMsg, [msgView]),
      this.showAlphaDuration + this.showTime,
    );
  }
