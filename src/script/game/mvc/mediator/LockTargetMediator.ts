...
export class LockTargetMediator implements IMediator {
  ...
  public register(target: object) {
    ...
    this.createLoader();
    this._selectFlag.url = fgui.UIPackage.getItemURL(
      EmPackName.Base,
      this._selectRes[0],
    );
    this._playerModel.addEventListener(
      SpaceEvent.TARGET_CHANGE,
      this.__lockTargetHandler,
      this,
    );
  ...
  private __lockTargetHandler(data: any) {
    ...
    if (this._targetInfo) {
      ...
      if (
        ...
        PlayerManager.Instance.currentPlayerModel.reinforce == this._targetInfo
      ) {
  ...
  private lockTarget() {
    ...
    var armyView: object = this.getArmyView(this._targetInfo);
    if (
      armyView instanceof SpaceArmyView ||
      armyView instanceof CampaignArmyView
    ) {
