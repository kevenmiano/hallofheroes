...
export default class OuterCityVehicleItem extends FUI_OuterCityVehicleItem {
    ...
    private advanceBtnHandler() {
        if(this._info.pushStatus == 1){
            MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("OuterCityVehicleItem.advanceBtnHandler.pushStatus"))
            return;
        }
        ...
        if(PlayerManager.Instance.currentPlayerModel.playerInfo.consortiaID == 0){
            MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("OuterCityVehicleItem.advanceBtn.check"))
            return;
        }
        if(this._arr.length == OuterCityVehicleItem.MAX_COUNT){
            MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("OuterCityVehicleItem.advanceBtnHandler.tips"));
            return;
        }
        if(FreedomTeamManager.Instance.hasTeam){//如果存在队伍信息，则退出队伍
            FreedomTeamSocketOutManager.sendQuik();
        }
        OuterCityManager.Instance.vehicleOperation(OuterCityModel.VEHICLE_OPERATION_TYPE2, this._info.templateId);
    }
    ...
    private escortBtnHandler() {
        if(this._info.protectStatus == 1){
            MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("OuterCityVehicleItem.escortBtnHandler.protectStatus"))
            return;
        }
        let t: number = new Date().getTime();
		if (t - this._intervalTime < 300) {
		...
		this._intervalTime = t;
        if(PlayerManager.Instance.currentPlayerModel.playerInfo.consortiaID == 0){
            MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("OuterCityVehicleItem.escortBtn.check"))
            return;
        }
        if(this._arr.length == OuterCityVehicleItem.MAX_COUNT){
            MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("OuterCityVehicleItem.escortBtnHandler.tips"));
            return;
        }
        if(FreedomTeamManager.Instance.hasTeam){//如果存在队伍信息，则退出队伍
            FreedomTeamSocketOutManager.sendQuik();
        }
        OuterCityManager.Instance.vehicleOperation(OuterCityModel.VEHICLE_OPERATION_TYPE1, this._info.templateId);
    }
    ...
    private exitBtnHandler() {
		...
		this._intervalTime = t;
        if (this._index == 0) {
            OuterCityManager.Instance.vehicleOperation(OuterCityModel.VEHICLE_OPERATION_TYPE4, this._info.templateId);
        } else {
            OuterCityManager.Instance.vehicleOperation(OuterCityModel.VEHICLE_OPERATION_TYPE3, this._info.templateId);
        }
    }
    ...
    private refreshView() {
        if (this._info) {
            ...
            if (this.outercityModel.checkSelfInTeam(this._info, this._index)) {//玩家自己在队列中
                this.statusCtr.selectedIndex = 2;
            } else {
                this.statusCtr.selectedIndex = this._index;
            }
            if (this._index == 0) {//推进队列
                this._arr = this._info.pushPlayer;
                this.titleDescTxt.text = LangManager.Instance.GetTranslation("OuterCityVehicleItem.titleDescTxt1");
            } else {//护卫队列
                this._arr = this._info.protectPlayer;
                this.titleDescTxt.text = LangManager.Instance.GetTranslation("OuterCityVehicleItem.titleDescTxt2");
            }
            this.countTxt.text = this._arr.length + "/" + OuterCityVehicleItem.MAX_COUNT;
