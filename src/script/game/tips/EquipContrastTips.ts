...
export class EquipContrastTips extends BaseTips {
    ...
    private addEvent() {
        ...
        NotificationManager.Instance.addEventListener(TipsEvent.EQUIP_TIPS_OBTAIN, this.onObtain, this);
    ...
    private onObtain(){
        ...
        // this.goodstip3.ensureBoundsCorrect();
        // this.totalBox.ensureBoundsCorrect();  
        let point = this.goodstip1.parent.localToGlobal(this.goodstip1.x,this.goodstip1.y)
        //当前方向
    ...
    private updateTransform() {
        ...
        let equipeds: any[] = GoodsManager.Instance.getHeroGoodsListBySonTypeAndId(this._info.templateInfo.SonType, this.thane.id).getList();
        equipeds = ArrayUtils.sortOn(equipeds, "pos", ArrayConstant.NUMERIC);
        let equiped: GoodsInfo;
        if (equipeds.length >= 2) {
            this.goodstip1.btn_use.text = LangManager.Instance.GetTranslation("armyII.viewII.skill.btnEquipExchange");//替换
            ...
            this.goodstip2.info = equiped;
            this.goodstip2.visible = true;
            if (equiped.templateInfo.SonType == GoodsSonType.SONTYPE_RING || equiped.templateInfo.SonType == GoodsSonType.SONTYPE_TRINKET) {
                //戒指和饰品有两个, 只装备了1个的情况下 仍然显示装备按钮
                this.goodstip1.btn_use.text = LangManager.Instance.GetTranslation("armyII.viewII.skill.btnEquipOn");//装备
            }
            else {
                this.goodstip1.btn_use.text = LangManager.Instance.GetTranslation("armyII.viewII.skill.btnEquipExchange");//替换
            }
    ...
    private removeEvent() {
        ...
        NotificationManager.Instance.removeEventListener(TipsEvent.EQUIP_TIPS_OBTAIN, this.onObtain, this);
