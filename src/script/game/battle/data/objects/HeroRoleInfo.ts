...
export class HeroRoleInfo extends BaseRoleInfo {
  ...
  public updateSp(
    value: number,
    flyBool: boolean = true,
    increaseBool: boolean = true,
    showTipBool: boolean = true,
    buffCauseBool: boolean = false,
  ) {
    this._sp = value;
    ...
    this.dispatchEvent(RoleEvent.SP, {
      sp: value,
      fly: flyBool,
      increase: increaseBool,
      showTip: showTipBool,
      buffCause: buffCauseBool,
    });
  }
  ...
  private hasPawn(): boolean {
    let list: Array<any> = BattleManager.Instance.battleModel.getRoleBySide(
      this.side,
    );
  ...
  public canMorph(): boolean {
    return (
      this.isPetState == false && this.awaken == BattleModel.AWAKEN_FULL_VALUE
    );
  }
  ...
  public get icon() {
    ...
    if (this.isPetState && this.petRoleInfo) {
      ...
      if (this.playerIconId == 0) {
        //如果是非玩家,则使用英雄的icon
        iconStr = IconFactory.getHeroIconByPics(
          this.heroInfo.templateInfo.Icon,
        );
      } else {
        //使用领主的icon
        iconStr = IconFactory.getPlayerIcon(
          this.heroInfo.snsInfo.headId,
          IconType.HEAD_ICON,
        );
      }
    }
    ...
    if (this.type == 3) {
      ...
      for (let j: number = 0; j < immuneBuffer.length; j++) {
        ...
        if (name) {
          ...
          damageData.IconPath = IconFactory.getCommonIconPath(
            "/immune/" + type + ".png",
          );
