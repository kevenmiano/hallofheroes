...
export class SpaceArmyViewHelper {
	...
	public static robotVisibleTest(posX: number, posY: number): boolean {
		...
		if (!start) {
			...
			let armyView: any = CampaignManager.Instance.controller.getArmyView(selfInfo);
			if (!armyView) {
				return false;
			}
	...
	public static visitNode(nodeInfo: SpaceNode) {
		let armyView: any = SpaceManager.Instance.controller.getArmyView(SpaceArmyViewHelper.model.selfArmy);
		if (nodeInfo == SpaceArmyViewHelper.model.selectNode && SpaceArmyViewHelper.getNodeInRange(armyView)) {
			...
			if (SpaceArmyViewHelper.model.selectNode) {
				...
				if (SpaceArmyViewHelper.model.selectNode.param3) {
					let option: any[] = SpaceArmyViewHelper.model.selectNode.param3.split(",");
			...
			if (nodeInfo.info.types == SpaceNodeType.MOVEMENT) {
				attackPoint = SpaceNpcAttackHelper.getAttackPoint(nodeInfo, new Laya.Point(armyView.x, armyView.y), new Laya.Point(nodeInfo.x, nodeInfo.y));
			} else {
				attackPoint = new Laya.Point(nodeInfo.x, nodeInfo.y);
			}
			SpaceManager.Instance.controller.moveArmyByPos(attackPoint.x, attackPoint.y);
		}
	}
	...
	public static selfArmyToEnd(aiInfo: BaseArmyAiInfo, target: any): boolean {
		...
		if (!nodeInfo) {
			nodeInfo = SpaceArmyViewHelper.model.getHandlerNode(SpaceArmyViewHelper.viewPosXToInfoPosX(target.x), SpaceArmyViewHelper.viewPosYToInfoPosY(target.y));
	...
	private static nodeTrigger(node: SpaceNode) {
		...
		if (node.info.types == SpaceNodeType.MOVEMENT) {
			...
			let selfInfo: SpaceArmy = SpaceArmyViewHelper.model.selfArmy;
			let armyView: any = SpaceManager.Instance.controller.getArmyView(selfInfo);
			...
			NotificationManager.Instance.dispatchEvent(SpaceEvent.SELECT_NODE, node.info.id);
		}
		else {
			if (param3Arr[0] == "12") {
	...
	public static getOpenType(type: number): Object {
		let openType: Object = null;
		switch (type) {
			case 8:
	...
	public static openNodeFrame(type: number) {
		switch (type) {
			...
			case 3:
				...
				break;
			case 4:
				SimpleAlertHelper.Instance.Show(undefined, null, "", LangManager.Instance.GetTranslation("public.unopen"));
				break;
			case 5:
	...
	static getNodeInRange(target: any): SpaceNode {
		...
		if (nInfo && target) {
			if (nInfo.info.state == NodeState.EXIST) {
				let start: Laya.Point = new Laya.Point(SpaceArmyViewHelper.viewPosXToInfoPosX(target.x), SpaceArmyViewHelper.viewPosYToInfoPosY(target.y));
				let end: Laya.Point = new Laya.Point(SpaceArmyViewHelper.viewPosXToInfoPosX(nInfo.x), SpaceArmyViewHelper.viewPosYToInfoPosY(nInfo.y));
				let dis: number = start.distance(end.x, end.y);
