...
export class AppellView extends Laya.Sprite implements Disposeable {
  ...
  constructor(w: number, h: number, appellID: number) {
    ...
    this._text = new FilterFrameText(
      240,
      24,
      undefined,
      this._appellInfo ? this._appellInfo.TextFontSize : 20,
    );
    if (this._appellInfo) {
      this._text.text = this._appellInfo ? this._appellInfo.TitleLang : "";
      this._text.color = AppellModel.getTextColorAB(
        this._appellInfo.TextColorIdx,
      );
    }
    ...
    this._text.visible = this._appellInfo
      ? this._appellInfo.IsHide == 1
        ? false
        : true
      : true;
