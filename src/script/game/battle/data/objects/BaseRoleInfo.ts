...
export class BaseRoleInfo extends GameEventDispatcher {
  ...
  private realUpdateBlood(
    ...
    if (type == BloodType.BLOOD_TYPE_SELF || type == BloodType.BLOOD_MAXHP) {
    ...
    this.dispatchEvent(RoleEvent.BLOOD_CHANGE, {
      bloodType: type,
      blood: value,
      displayBlood: displayValue,
      havoc: havoc,
      selfCause: selfCauseVal,
      isDefault: defaultSkill,
      isParry: parry,
    });
  }
  ...
  public get point(): Laya.Point {
    if (!this._point) {
      ...
      this._point = isLeft
        ? BattleUtils.getPosition(FaceType.LEFT_TEAM, this.pos)
        : BattleUtils.getPosition(FaceType.RIGHT_TEAM, this.pos);
    }
    return this._point;
  }
  ...
  public set actionMovieClip(value: MovieClip) {
    ...
    if (this._actionMovie.movie.hasOwnProperty("pos_head")) {
      if (this._actionMovie.movie.pos_head) {
        pt = new Laya.Point(
          this._actionMovie.movie.pos_head.x,
          this._actionMovie.movie.pos_head.y,
        );
      } else {
        pt = new Laya.Point();
      }
    ...
    if (this._actionMovie.movie.hasOwnProperty("pos_body")) {
      if (this._actionMovie.movie.pos_body) {
        pt = new Laya.Point(
          this._actionMovie.movie.pos_body.x,
          this._actionMovie.movie.pos_body.y,
        );
      } else {
        pt = new Laya.Point();
      }
    ...
    if (this._actionMovie.movie.hasOwnProperty("pos_leg")) {
      if (this._actionMovie.movie.pos_leg) {
        pt = new Laya.Point(
          this._actionMovie.movie.pos_leg.x,
          this._actionMovie.movie.pos_leg.y,
        );
      } else {
        pt = new Laya.Point();
      }
    ...
    if (this.isLiving) {
      this.action.apply(
        this,
        this._cacheAction ? this._cacheAction : [this._defaultAction],
      );
    } else {
  ...
  public update() {
    ...
    this._concurrentActions.forEach((concurrentAct) => {
      if (concurrentAct.finished) {
        this._concurrentActions.splice(
          this._concurrentActions.indexOf(concurrentAct),
          1,
        );
      } else {
  ...
  public addBuffer(buffer: BufferDamageData) {
    ...
    if (tempInfo.AttackType == 19) {
      if (!this._restricted) {
        ...
        // if(BattleManager.Instance.battleModel.selfHero == this)
        if (
          BattleManager.Instance.battleModel.selfHero instanceof BaseRoleInfo
        ) {
          NotificationManager.Instance.sendNotification(
            BattleNotic.RESTRICT_CHANGED,
          );
   ...
   */
  public getBuffersWithFiller(
    buffLeft: boolean = true,
    isConsortiaBoss: boolean = false,
  ): Array<BufferDamageData> {
   ...
   *
   */
  private copyBufferData(
    curBuffer: BufferDamageData,
    targetBuffer: BufferDamageData,
  ): void {
  ...
  private isContainDannyAction(): boolean {
    ...
    if (
      this._actionManager.current.inheritType == InheritIActionType.DannyAction
    ) {
      return true;
      // }else if(this._actionManager.getActions().length > 0 && (this._actionManager.getActions()[0] instanceof DannyAction)){
    } else if (
      this._actionManager.getActions().length > 0 &&
      this._actionManager.getActions()[0].inheritType ==
        InheritIActionType.DannyAction
    ) {
      return true;
    }
    return false;
