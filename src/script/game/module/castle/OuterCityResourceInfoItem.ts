...
export default class OuterCityResourceInfoItem extends FUI_OuterCityResourceInfoItem {
    ...
    private lookBtnHandler() {
        if(SceneManager.Instance.currentType != SceneType.OUTER_CITY_SCENE){
            MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("map.outercity.OuterCityTreasureItem"));
            return;
        }
        ...
        var content: string = LangManager.Instance.GetTranslation("buildings.politics.view.FieldItem.content3", this.nameTxt.text, this.posTxt.text);
        SimpleAlertHelper.Instance.Show(SimpleAlertHelper.SIMPLE_ALERT, null, prompt, content, confirm, cancel, this.lookConfirm.bind(this));
    }

    private lookConfirm(b: boolean, flag: boolean) {
        if (b) {
            this._mapView.motionTo(new Laya.Point(Tiles.WIDTH*this._targetPoint.x - StageReferance.stageWidth / 2, this._targetPoint.y*Tiles.HEIGHT - StageReferance.stageHeight / 2));
            NotificationManager.Instance.dispatchEvent(OuterCityEvent.CLOSE_OUTERCITY_RESOURCE_WND);
    ...
    giveUpBtnHandler() {
        ...
        var cancel: string = LangManager.Instance.GetTranslation("public.cancel");
        var prompt: string = LangManager.Instance.GetTranslation("public.prompt");
        var content: string = LangManager.Instance.GetTranslation("buildings.politics.view.FieldItem.content",this.nameTxt.text, this.posTxt.text);
        SimpleAlertHelper.Instance.Show(SimpleAlertHelper.SIMPLE_ALERT, null, prompt, content, confirm, cancel, this.giveUpField.bind(this));
    }
    ...
    private giveUpField(b: boolean, flag: boolean) {
        if (b) {
            if (this._info) {
                ...
                OuterCitySocketOutManager.removeMine(position, this._info.nodeId, this._info.sonNodeId);
            } else {
                MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("buildings.politics.view.FieldItem.content02"));
            }
        }
    }
    ...
    private refresh() {
        this._targetPoint = new Laya.Point(parseInt(this._info.pos.split(",")[0]), parseInt(this._info.pos.split(",")[1]));
        this.posTxt.text = "[" + MapGrid.getPositionString(this._info.pos) + "]";
        this.nameTxt.text = this._info.fieldName;
        this.goldNumTxt.text = LangManager.Instance.GetTranslation("OuterCityResourceInfoItem.goldNumTxt", this._info.yield);
    }
