...
export default class LoginWndOS extends BaseWindow {
  ...
  public OnInitWind() {
    ...
    this.loginMethod &&
      this.loginMethod.onInit(new Func(this, this.handlerLoginMethod));
    ...
    if (this.frameData) {
      ...
      if (this.objData.isWan) {
        ...
        if (obj) {
          let restartFrom =
            SharedManager.Instance.getWindowItem("bindAccountRet");
          if (restartFrom == "true") {
  ...
  private SDKinitBack() {
    ...
    this.loginState.selectedIndex = 2;
    Logger.yyz(
      "SDKinitBack::this.loginState.selectedIndex:" +
        this.loginState.selectedIndex,
    );
  }
  ...
  private changeLoginMethod() {
    ...
    Logger.yyz(
      "changeLoginMethod::this.loginState.selectedIndex:" +
        this.loginState.selectedIndex,
    );
  ...
  addEvent() {
    ...
    NotificationManager.Instance.addEventListener(
      NotificationEvent.SWITCH_ACCOUNT,
      this.onSwitchAccountHandler,
      this,
    );
    NotificationManager.Instance.addEventListener(
  ...
  onGuestSwitchAccountHandler() {
    SDKManager.Instance.getChannel().trackEvent(
      0,
      TrackEventNode.LOGIN_ACCOUNTS_CHOOSE,
      "login_accounts_choose",
      "登录界面点击“登录其他账户”按钮",
      "",
      "",
    );
    SDKManager.Instance.getChannel().adjustEvent(
      RPT_EVENT.LOGIN_ACCOUNTS_CHOOSE,
    );
    ...
    if (errorCode != 111103) {
      ...
      if (LangManager.Instance.GetTranslation(codeStr) != "") {
        ...
        MessageTipManager.Instance.show(
          LangManager.Instance.GetTranslation(codeKey + defaultCode, errorCode),
        );
      }
    }
    this.onSwitchAccountHandler();
  ...
  private clearUserData() {
    ...
    this.loginState.selectedIndex = 0;
    Logger.yyz(
      "clearUserData::this.loginState.selectedIndex:" +
        this.loginState.selectedIndex,
    );
    let bindAccountRet = SharedManager.Instance.getWindowItem("bindAccountRet");
    let type = bindAccountRet == "true" ? 1 : 0;
  ...
  webLoginCallback(appData) {
    ...
    if (appData) {
      ...
      if (appData.userId == -1 && appData.token == -1) {
        //失败情况
        this.clearUserData();
        return;
      }
      ...
      if (data) {
        try {
          ...
          if (content && content.length) {
            ...
            Logger.yyz(
              "requestServerList::this.loginState.selectedIndex:" +
                this.loginState.selectedIndex,
            );
            let loginWay = SharedManager.Instance.getWindowItem("loginWay");
            if (Number(loginWay) == LoginWay.Type_GUEST) {
              this.loginWay.selectedIndex = this.serverListData.hasPlayer
                ? 1
                : 0;
            } else {
  ...
  onGuestEnterGame() {
    SDKManager.Instance.getChannel().trackEvent(
      0,
      TrackEventNode.LOGIN_TOURIST_CHOOSE,
      "login_tourist_choose",
      "登录界面点击“以游客继续”按钮",
      "",
      "",
    );
    SDKManager.Instance.getChannel().adjustEvent(
      RPT_EVENT.LOGIN_TOURIST_CHOOSE,
    );
    this.onEnterGame();
  }
  ...
  handlerLoginMethod(key?: number) {
    switch (
      ...
      case LoginWay.Type_Apple:
        SDKManager.Instance.getChannel().adjustEvent(
          RPT_EVENT.LOGIN_APPLE_ACCOUNT,
        );
        SDKManager.Instance.getChannel().trackEvent(
          0,
          TrackEventNode.LOGIN_APPLE_ACCOUNT,
          "LOGIN_APPLE_ACCOUNT",
          "登录界面点击苹果账号登录按钮",
          "",
          "",
      ...
      case LoginWay.Type_7ROAD: //7road
        SDKManager.Instance.getChannel().adjustEvent(
          RPT_EVENT.LOGIN_7ROAD_ACCOUNT,
        );
        SDKManager.Instance.getChannel().trackEvent(
          0,
          TrackEventNode.LOGIN_7ROAD_ACCOUNT,
          "login_7road_account",
          "登录界面点击七道账号登录按钮",
          "",
          "",
      ...
      case LoginWay.Type_GG: //google
        SDKManager.Instance.getChannel().adjustEvent(
          RPT_EVENT.LOGIN_GG_ACCOUNT,
        );
        SDKManager.Instance.getChannel().trackEvent(
          0,
          TrackEventNode.LOGIN_GG_ACCOUNT,
          "login_gg_account",
          "登录界面点击谷歌账号登录按钮",
          "",
          "",
      ...
      case LoginWay.Type_FB: //facebook
        SDKManager.Instance.getChannel().adjustEvent(
          RPT_EVENT.LOGIN_FB_ACCOUNT,
        );
        SDKManager.Instance.getChannel().trackEvent(
          0,
          TrackEventNode.LOGIN_FB_ACCOUNT,
          "login_fb_account",
          "登录界面点击Facebook账号登录按钮",
          "",
          "",
        );
        break;
      case LoginWay.Type_GUEST: //guest
        SDKManager.Instance.getChannel().adjustEvent(
          RPT_EVENT.LOGIN_TOURIST_ACCOUNT,
        );
        SDKManager.Instance.getChannel().trackEvent(
          0,
          TrackEventNode.LOGIN_TOURIST_ACCOUNT,
          "login_tourist_account",
          "登录界面点击游客账号登录按钮",
          "",
          "",
        );
