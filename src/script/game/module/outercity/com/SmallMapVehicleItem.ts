...
export default class SmallMapVehicleItem extends FUI_SmallMapVehicleItem{
    ...
    public set info(value: WildLand) {
        if (value) {
            ...
            if(value.info.occupyLeagueConsortiaId == 0){//中立的
                this.btn.icon = fgui.UIPackage.getItemURL("OuterCity", "Img_Material_vehicle1_S");
            }else if(value.info.occupyLeagueConsortiaId == PlayerManager.Instance.currentPlayerModel.playerInfo.consortiaID){//自己一方的
                this.btn.icon = fgui.UIPackage.getItemURL("OuterCity", "Img_Material_vehicle2_S");
            }else {//其他公会的
                this.btn.icon = fgui.UIPackage.getItemURL("OuterCity", "Img_Material_vehicle3_S");
            }
