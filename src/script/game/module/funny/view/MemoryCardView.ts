...
export class MemoryCardView extends FUI_MemoryCardView implements FunnyContent {
  ...
  private addEvent() {
    ...
    NotificationManager.Instance.addEventListener(
      NotificationEvent.MEMORYCARD_DATA_UPDATE,
      this.refreshView,
      this,
    );
  }
  ...
  private initView() {
    ...
    this.txt_btnBuyStep.text = LangManager.Instance.GetTranslation(
      "NewMemoryCardFrame.btnBuyStep.text",
    );
  ...
  public __clickItem(idx: number) {
    ...
    if (this.model.step <= 0) {
      MessageTipManager.Instance.show(
        LangManager.Instance.GetTranslation("NewMemoryCardItem.step.txt"),
      );
      return;
    }
    if (!this.model.lock && this.model.clickIndexArr.length < 2) {
      ...
      if (
        this.model.clickIndexArr.length == 1 &&
        this.model.clickIndexArr[0] == idx
      ) {
  ...
  public refreshView() {
    if (this._initFirstRefresh) {
      ...
      if (posArr.length == MemoryCardData.CARD_NUM) {
        ...
        SimpleAlertHelper.Instance.Show(
          ...
          (b) => {
            if (b) {
              MemoryCardManager.Instance.memoryCardSendOp(
                MemoryCardData.OP_REFRESH,
              );
            }
          },
          AlertBtnType.O,
    ...
    if (this.model.clickIndexArr.length >= 2) {
      for (let i: number = 0; i < this.model.clickIndexArr.length; i++) {
        let item = this.list.getChildAt(
          this.model.clickIndexArr[i],
        ) as MemoryCardItem;
        item.playAni();
      }
    }
  ...
  private btnRefreshClick() {
    ...
    let content: string = LangManager.Instance.GetTranslation(
      "NewMemoryCardFrame.refresh.content.txt",
    );
    SimpleAlertHelper.Instance.Show(
      null,
      null,
  ...
  private btnGetClick() {
    ...
    FrameCtrlManager.Instance.open(EmWindow.DisplayItems, {
      itemInfos: this.model.dropGoodsInfo,
  ...
  private btnBuyClick() {
    ...
    let tmp = TempleteManager.Instance.getConfigInfoByConfigName(
      "memorycard_limittimes",
    );
    ...
    if (this.model.step >= limit) {
      MessageTipManager.Instance.show(
        LangManager.Instance.GetTranslation("NewMemoryCardFrame.buy.limitText"),
      );
      return;
    }
    // let max: number = Number(TempleteManager.Instance.getConfigInfoByConfigName("MEMORYCARD_MAXTIMES").ConfigValue);
    tmp = TempleteManager.Instance.getConfigInfoByConfigName(
      "memorycard_maxtimes",
    );
    ...
    if (this.model.buyCount >= max) {
      MessageTipManager.Instance.show(
        LangManager.Instance.GetTranslation("NewMemoryCardFrame.buy.maxText"),
      );
      return;
    }

    tmp = TempleteManager.Instance.getConfigInfoByConfigName(
      "memorycard_costdiamond",
    );
    ...
    if (
      SharedManager.Instance.checkIsExpired(
        SharedManager.Instance.memoryCardBuyAlertDate,
      )
    ) {
      let itemTmp = ConfigMgr.Instance.getTemplateByID(
        ConfigType.t_s_itemtemplate,
        ItemID.MEMORY_CARD_KEY,
      ...
      UIManager.Instance.ShowWind(EmWindow.TodayNotAlert, {
        ...
        content: LangManager.Instance.GetTranslation(
          "NewMemoryCardFrame.TodayNotAlert.content.text",
          point,
          propName,
          leftCount,
        ),
  ...
  private buyCheckCallback() {
    ...
    let propNum: number = GoodsManager.Instance.getGoodsNumByTempId(
      ItemID.MEMORY_CARD_KEY,
    );
  ...
  private btnHelpClick() {
    ...
    let content: string = LangManager.Instance.GetTranslation(
      "NewMemoryCardFrame.helpTxt.text",
    );
