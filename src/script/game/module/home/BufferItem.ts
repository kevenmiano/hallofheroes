...
export default class BufferItem extends FUI_BufferItem {
  ...
  public set info(value: any) {
    ...
    if (
      ...
      this.icon1.url = FUIHelper.getItemURL(
        EmPackName.Base,
        "asset.resourceBar.ExtraBufferIcon_" + this._type,
      );
      let tipStr: string =
        "[color=#D32EDB][size=24]" +
        LangManager.Instance.GetTranslation(
          "yishi.view.resourceBar.ExtraBufferTitle" + this._type,
        ) +
        "[/size][/color]";
      if (this._type == 6) {
        //世界繁荣度
        ...
        tipStr += LangManager.Instance.GetTranslation(
          "yishi.view.resourceBar.ExtraBufferContent6",
          value[0],
        );
        ...
        tipStr = tipStr + "<br>";
        tipStr += LangManager.Instance.GetTranslation(
          "yishi.view.resourceBar.ExtraBufferContent11",
        );
      } else {
        tipStr = tipStr + "<br>";
        for (var i = 0; i < dataArray.length; i++) {
          ...
          if (item instanceof t_s_consortialevelData) {
            ...
            if ((item as BuildInfo).templeteInfo.MasterType == 13) {
              tecBuffer[0] = LangManager.Instance.GetTranslation(
                "map.internals.view.frame.FieldInfoView.goldTextText",
              );
            ...
            if (item == 0) {
              tipStr +=
                LangManager.Instance.GetTranslation(
                  "yishi.view.resourceBar.ExtraBufferSeminary",
                  this.playerInfo.seminaryEffect,
                ) + "\n";
            } else {
              tipStr +=
                "<br>" +
                LangManager.Instance.GetTranslation(
                  "yishi.view.resourceBar.ExtraBufferSeminaryTips",
                );
            }
          }
        }
      ...
      this.icon1.url = FUIHelper.getItemURL(
        EmPackName.Base,
        "asset.resourceBar.ExtraBufferIcon_" + this._info.type,
      );
      ...
      this.icon1.url = IconFactory.getCommonIconPath(
        this._info.bufferData.template.Icon,
      );
      this.tipData = this._info.bufferData;
  ...
  private __changeHandler() {
    ...
    if (this._info.leftTime >= 3600 * 24)
      timeStr =
        parseInt((this._info.leftTime / (3600 * 24)).toString()) +
        LangManager.Instance.GetTranslation("public.day");
    else if (this._info.leftTime >= 3600)
      timeStr =
        parseInt((this._info.leftTime / 3600).toString()) +
        LangManager.Instance.GetTranslation("public.time.sigHour");
    else if (this._info.leftTime >= 60)
      timeStr =
        parseInt((this._info.leftTime / 60).toString()) +
        LangManager.Instance.GetTranslation("public.minute");
    else
      timeStr =
        parseInt(this._info.leftTime.toString()) +
        LangManager.Instance.GetTranslation(
