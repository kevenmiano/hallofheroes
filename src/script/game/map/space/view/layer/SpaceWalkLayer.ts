...
import { CampaignMapEvent, OuterCityEvent, SpaceEvent } from "../../../../constant/event/NotificationEvent";
...
export class SpaceWalkLayer extends Laya.Sprite implements IEnterFrame {
    ...
    public get needRobotCount(): number {
        if (this._initPlayersFlag) {
            return SpaceWalkLayer.ROBOT_MAX - this._armyCount - this._userQueue.length - this._robotQueue.length;
        }
        return 0;
    }
    ...
    private initRegister() {
        var arr: any[] = [
            SpaceWalkTargetMediator,
            SpaceWalkDepthMediator
        ];
        this._mediatorKey = MediatorMananger.Instance.registerMediatorList(arr, this, SpaceWalkLayer.NAME);
    }

    private addEvent() {
        EnterFrameManager.Instance.registeEnterFrame(this);
        SpaceManager.Instance.model.addEventListener(OuterCityEvent.ADD_GARRISON, this.__addSpaceArmyHandler, this);
        SpaceManager.Instance.model.addEventListener(OuterCityEvent.REMOVE_ARMY, this.__removeSpaceArmyHandler, this);
        SpaceManager.Instance.model.addEventListener(CampaignMapEvent.MOVE_PATHS_ARMY, this.__armyPosBroadHandler, this);
        SpaceManager.Instance.model.addEventListener(CampaignMapEvent.MOVE_TO_VISIBLE, this.__armyMoveToVisibleHandler, this);
        SpaceManager.Instance.model.addEventListener(CampaignMapEvent.MOVE_TO_UNVISIBLE, this.__armyMoveToUnVisibleHandler, this);
        NotificationManager.Instance.addEventListener(SpaceEvent.HIDE_OTHERS, this.__hideOthersHandler, this);
    }

    private removeEvent() {
        EnterFrameManager.Instance.unRegisteEnterFrame(this);
        SpaceManager.Instance.model.removeEventListener(OuterCityEvent.ADD_GARRISON, this.__addSpaceArmyHandler, this);
        SpaceManager.Instance.model.removeEventListener(OuterCityEvent.REMOVE_ARMY, this.__removeSpaceArmyHandler, this);
        SpaceManager.Instance.model.removeEventListener(CampaignMapEvent.MOVE_PATHS_ARMY, this.__armyPosBroadHandler, this);
        SpaceManager.Instance.model.removeEventListener(CampaignMapEvent.MOVE_TO_VISIBLE, this.__armyMoveToVisibleHandler, this);
        SpaceManager.Instance.model.removeEventListener(CampaignMapEvent.MOVE_TO_UNVISIBLE, this.__armyMoveToUnVisibleHandler, this);
        NotificationManager.Instance.removeEventListener(SpaceEvent.HIDE_OTHERS, this.__hideOthersHandler, this);
    }
    ...
    private isPlaying(userId: number): boolean {
        ...
        if (userId != selfArmy.userId) {
            if (FreedomTeamManager.Instance.hasTeam && FreedomTeamManager.Instance.inMyTeam(userId)) {
                return true;
            }
    ...
    private removeArmy(armyInfo: SpaceArmy) {
        ...
        var armyView: SpaceArmyView = this._armys.get(armyInfo.key) as SpaceArmyView;
        if (armyView) {
    ...
    private __armyPosBroadHandler(msg: SpacePlayerMoveMsg) {
        ...
        for (const key in msg.players) {
            ...
            armyInfo = SpaceManager.Instance.model.getBaseArmyByUserId(playerMsg.playerId);
            if (!armyInfo) {
                continue;
            }
            ...
            for (var i: number = 0; i < pathLength; i++) {
                paths.push(new Laya.Point(Math.ceil(playerMsg.path[i].x), Math.ceil(playerMsg.path[i].y)));
            }

            var ser: Laya.Point = new Laya.Point(paths[0].x * Tiles.WIDTH, paths[0].y * Tiles.HEIGHT);
    ...
    public checkClickPlayerNum(posx: number, posy: number) {
        ...
        this['_children'].forEach(element => {
            if (element instanceof SpaceArmyView) {
                ...
                /** 自己 */
                let isSelf = spaceArmy.baseHero.userId == this.playerModel.playerInfo.userId
                /** 众神之战展示形象 */
                let warlordsId = spaceArmy.id - ShadowUILayer.INIT_ID
                let isWarlordsIdArmy = warlordsId == JobType.WIZARD || warlordsId == JobType.HUNTER || warlordsId == JobType.WARRIOR
        ...
        if (this.clickPlayerArr.length > 1) {
            ...
            if (this.clickPlayerArr[0] instanceof SpaceArmyView) {
                this.playerModel.selectTarget = (this.clickPlayerArr[0] as SpaceArmyView).data;
                this.playerModel.reinforce = (this.clickPlayerArr[0] as SpaceArmyView).data;
            }
        ...
        if (evt) {
            if (evt.target instanceof SpaceArmyView) {
                if ((<SpaceArmyView>evt.target).mouseClickHandler(evt)) {
                    SpaceManager.Instance.controller.moveArmyByPos(this.mouseX, this.mouseY, true)
                    return true;
                }
            }
        }
        return SpaceManager.Instance.controller.moveArmyByPos(this.mouseX, this.mouseY, true);
    }
    ...
    protected initPlayers() {
        var players: Map<number, SpaceArmy> = SpaceManager.Instance.model.allArmyDict;
    ...
    protected addArmys() {
        ...
        if (this._renderTime - this._lastTime >= SpaceWalkLayer.RENDER_TICK) {
            ...
            if (this._userQueue.length > 0) {
                ...
                armyInfo = SpaceTemplateManager.Instance.getRobotArmyInfoByUserId(userId);
                if (armyInfo) {
                    armyView = this.addArmy(armyInfo);
                }
            }
