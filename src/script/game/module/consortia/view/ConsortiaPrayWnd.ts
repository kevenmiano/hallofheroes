...
export default class ConsortiaPrayWnd extends BaseWindow {
    ...
    private initData() {
        ...
        this.descTxt1.text = LangManager.Instance.GetTranslation("securityCode.view.remainCountText");
        this.descTxt3.text = LangManager.Instance.GetTranslation("ConsortiaPrayWnd.descTxt3");
        this.descTxt4.text = LangManager.Instance.GetTranslation("ConsortiaPrayWnd.descTxt4");
        this.descTxt5.text = LangManager.Instance.GetTranslation("ConsortiaPrayWnd.descTxt5");
        this.descTxt6.text = LangManager.Instance.GetTranslation("ConsortiaPrayWnd.descTxt6");
        this._needCostArr = ConfigInfoManager.Instance.getConsortiaPrayCost();
        this._refreshCostArr = ConfigInfoManager.Instance.getConsortiaAltarRefresh();
    ...
    private initEvent() {
        ...
        NotificationManager.Instance.addEventListener(ConsortiaEvent.GET_ALTAR_GOODS, this.updateView, this);
    }
    ...
    private removeEvent() {
        ...
        NotificationManager.Instance.removeEventListener(ConsortiaEvent.GET_ALTAR_GOODS, this.updateView, this);
    }
    ...
    private setComRedStatus() {
        ...
        if (this.totalCount - this._model.commAltarCount > 0 && !this._model.commAltarSeened) {
            flag = true;
        }
        dot.visible = flag;
    }
    ...
    private setHighRedStatus() {
        ...
        let flag: boolean = false;
        if (!ConsortiaManager.Instance.model) return;
        if (this.totalCount - this._model.highAltarCount > 0 && !this._model.hightAltarSeened) {
            flag = true;
        }
        dot.visible = flag;
    }
    ...
    updateView() {
        ...
        if (tabIndex == 0) {
            ...
            for (var i: number = 0; i < 10; i++) {
                ...
                if (this._goodsItemList[i].info && this._goodsItemList[i].info.status == 1) {
            ...
            if (this._hasGetCommCount == 10) {
                ...
                this._commAllOpenNeedCount = this.getCommTotalCost(this._hasGetCommCount);
            }
            this._leftCommRefreshCount = this.maxRefreshCount - this._model.commFreshCount;
            if (this.playerInfo.consortiaOffer < this._commOneNeedCost) {//开启1次不足
            ...
            this.countTxt1.text = (this.totalCount - this._model.commAltarCount) + "/" + this.totalCount;
            this.countTxt2.text = this.playerInfo.consortiaOffer.toString();
            this.descTxt2.text = LangManager.Instance.GetTranslation("ConsortiaPrayWnd.descTxt2");
        } else if (tabIndex == 1) {
            ...
            if (this._hasGetHighCount == 10) {
                ...
                this._highAllOpenNeedCount = this.getCommTotalCost(this._hasGetHighCount);
            }

            this.countTxt1.text = (this.totalCount - this._model.highAltarCount) + "/" + this.totalCount;
            this.countTxt2.text = this.playerInfo.consortiaJianse.toString();
            this.descTxt2.text = LangManager.Instance.GetTranslation("ConsortiaPrayWnd.new.descTxt2");
            this._leftHighRefreshCount = this.maxRefreshCount - this._model.highFreshCount;

            if (this.playerInfo.consortiaJianse < this._highOneNeedCost) {//开启1次不足
                this.tipCountTxt2.color = ColorConstant.RED_COLOR;
            } else {
                this.tipCountTxt2.color = ColorConstant.LIGHT_TEXT_COLOR;
            }
        ...
        if (this.c1.selectedIndex == 0) {
            if (this._hasGetCommCount == 10) {
                ...
                if (this._leftCommRefreshCount <= 0) {
                    MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("ConsortiaPrayWnd.refreshBtnHandler.tips1"));
                    return;
                }
                ...
                var content: string = LangManager.Instance.GetTranslation("ConsortiaPrayWnd.refreshBtnHandler.AltarHelper.content1", this._refreshCostArr[1], this._leftCommRefreshCount, this.maxRefreshCount);
                SimpleAlertHelper.Instance.Show(SimpleAlertHelper.SIMPLE_ALERT, null, prompt, content, confirm, cancel, this.refreshConfirmBtnClick1.bind(this));
            }
        } else {
            if (this._hasGetHighCount == 10) {
                ...
                if (this._leftHighRefreshCount <= 0) {
                    MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("ConsortiaPrayWnd.refreshBtnHandler.tips2"));
                    return;
                }
                var confirm: string = LangManager.Instance.GetTranslation("public.confirm");
                var cancel: string = LangManager.Instance.GetTranslation("public.cancel");
                var prompt: string = LangManager.Instance.GetTranslation("public.prompt");
                var content: string = LangManager.Instance.GetTranslation("ConsortiaPrayWnd.refreshBtnHandler.AltarHelper.content2", this._refreshCostArr[1], this._leftHighRefreshCount, this.maxRefreshCount);
                SimpleAlertHelper.Instance.Show(SimpleAlertHelper.SIMPLE_ALERT, null, prompt, content, confirm, cancel, this.refreshConfirmBtnClick2.bind(this));
            }
        }
    }
    ...
    private refreshConfirmBtnClick1(b: boolean, flag: boolean) {
        if (b) {
            if (this.playerInfo.consortiaOffer < this._refreshCostArr[1]) {//贡献值不足提示
                MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("consortiaPrayWnd.refreshConfirm.tips1"))
                return;
            }
            this._contorller.refreshPrayCount(1);
        }
    ...
    private refreshConfirmBtnClick2(b: boolean, flag: boolean) {
        if (b) {
            if (this.playerInfo.consortiaJianse < this._refreshCostArr[1]) {//个人建设值不足提示
                MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("consortiaPrayWnd.refreshConfirm.tips2"))
                return;
            }
            this._contorller.refreshPrayCount(2);
        }
    ...
    private openOneBtnHandler() {
        if (this.c1.selectedIndex == 0) {
            if (this.totalCount - this._model.commAltarCount <= 0) {//次数不足
                MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("ConsortiaPrayWnd.openOneBtnHandler.tips1"));
                return;
            }
            if (this.playerInfo.consortiaOffer < this._needCostArr[this._hasGetCommCount]) {//贡献不足
                // MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("ConsortiaPrayWnd.openOneBtnHandler.tips2"));
                this.showConfirm();
                return;
            }
            if (this._hasGetCommCount == 10) {
                MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("ConsortiaPrayWnd.openOneBtnHandler.tips4"));
                return;
            }
            this._contorller.consortiaAltarBless(1, 1);
        } else if (this.c1.selectedIndex == 1) {
            if (this.totalCount - this._model.highAltarCount <= 0) {//次数不足
                MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("ConsortiaPrayWnd.openOneBtnHandler.tips1"));
                return;
            }
            if (this.playerInfo.consortiaJianse < this._needCostArr[this._hasGetHighCount]) {//建设不足
    ...
    private showConfirm() {
        SimpleAlertHelper.Instance.ShowSimple(SimpleAlertHelper.SIMPLE_ALERT, null, LangManager.Instance.GetTranslation("consortia.helper.ConsortiaAltarHelper.content02"), this.gotoConsortiaContribute.bind(this));
    }
    ...
    private get totalCount(): number {
        return TempleteManager.Instance.getConsortiaTempleteByTypeAndLevel(ConsortiaUpgradeType.CONSORTIA_ALTAR, this.consortiaInfo.altarLevel).Property2;
    }
