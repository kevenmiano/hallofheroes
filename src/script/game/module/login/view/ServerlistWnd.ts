...
export default class ServerlistWnd extends BaseWindow {
  ...
  addEvent() {
    this.serverList.itemRenderer = Laya.Handler.create(
      this,
      this.renderListItem,
      null,
      false,
    );
    this.serverItemlist.itemRenderer = Laya.Handler.create(
      this,
      this.renderInfoListItem,
      null,
      false,
    );
    this.serverList.on(
    ...
    this.serverItemlist.on(
      fairygui.Events.CLICK_ITEM,
      this,
      this.onServerInfoListSelect,
    );
  }
  ...
  onInitData() {
    ...
    if (this.params) this.serverListData = this.params;
  ...
  onServerListSelect() {
    ...
    if (this.serverListData.recordLoginServers.length && selectIndex == 0) {
      //如果存在登录历史记录
