...
export class ConsortiaApplyWnd extends BaseWindow {
  ...
  public OnInitWind() {
    ...
    this.txt_search.promptText = LangManager.Instance.GetTranslation(
      "ConsortiaApplyWnd.txtSearch",
    );
    //读取xml
    ...
    this.btnInviteRecord.title = LangManager.Instance.GetTranslation(
      "ConsortiaApplyWnd.btnInviteRecord.title",
    );
    this.btnApplayRecord.title = LangManager.Instance.GetTranslation(
      "ConsortiaApplyWnd.btnApplayRecord.title",
    );
    this.consortiaApplyRecord.getChild("title1").text =
      LangManager.Instance.GetTranslation("ConsortiaApplyCell.title1");
    this.consortiaApplyRecord.getChild("title2").text =
      LangManager.Instance.GetTranslation("ConsortiaApplyCell.title2");
    this.consortiaApplyRecord.getChild("title3").text =
      LangManager.Instance.GetTranslation(
        "ConsortiaApplyWnd.consortiaApplyRecord.title3",
      );
  ...
  private initEvent() {
    ...
    this.recordList.itemRenderer = Laya.Handler.create(
      this,
      this.renderRecordListItem,
      null,
      false,
    );
    this._data.addEventListener(
      ConsortiaEvent.UPDA_RECORD_LIST,
      this.onRecordListUpdata,
      this,
    );
    this._data.addEventListener(
      ConsortiaEvent.UPDA_CONSORTIA_LIST,
      this.onSearchListUpdata,
      this,
    );
  }
  ...
  private __onSearchHandler() {
    if (this.txt_search.text == "") {
      ...
      ConsortiaSocektSendManager.searchConsortia(
        "",
        this._contorller.randomCount,
        false,
      );
    ...
    if (this._inputStr != this.txt_search.text) {
      ...
      if (
        (this._contorller.randomCount - 1) *
          ConsortiaModel.CONSORTIA_SEARCH_PAGE_NUM >=
        this._contorller.model.setTotalRows
      ) {
        this._contorller.randomCount = 1;
      }
    }
  ...
  applyBtnHandler() {
    ...
    let item: ConsortiaApplyCell = this.consortiaList.getChildAt(
      selectedIndex,
    ) as ConsortiaApplyCell;
    if (!item) return;
