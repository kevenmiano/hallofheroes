...
import { BattleType, FaceType, RoleType, SideType } from "../../constant/BattleDefine";
...
export class CreateGameTransaction extends TransactionBase {
    ...
    private addHandler() {
        NotificationManager.Instance.addEventListener(BattleEvent.BATTLE_COMPLETE, this.onBattleComplete, this);
        ServerDataManager.listen(S2CProtocol.U_BATTLE_REPORT, this, this.onBattleInfoResultHandler);
    }

    private onBattleComplete(event: BattleEvent) {
        this._battlePkgs.shift();
        Logger.base("[CreateGameTransaction]战斗完成 是否进行下一场战斗", this._battlePkgs.length > 0);
    ...
    private onBattleInfoResultHandler(pkg: PackageIn) {
        ...
        if (this._battlePkgs.length > 0) {
            for (let index = this._battlePkgs.length - 1; index >= 0; index--) {
                ...
                const prepareMsg = preparePkg.readBody(BattlePrepareMsg) as BattlePrepareMsg;
                // Logger.warn("[CreateGameTransaction]队列中的battleId=", prepareMsg.battleId);
                if (reportMsg.battleId == prepareMsg.battleId) {
                    Logger.warn("[CreateGameTransaction]服务端战斗已经结束, 移除未执行的战斗", prepareMsg.battleId, "当前战斗", BattleManager.Instance.battleModel && BattleManager.Instance.battleModel.battleId);
    ...
    public handlePackage() {
        ...
        Logger.base("🔥新战斗 准备加载战斗资源", prepareMsg.battleId, Laya.Browser.now());
        if (this._battlePkgs.length > 1) {
            let desc: string = LangManager.Instance.GetTranslation("battle.transaction.CreateGameTransaction.desc");
            Logger.warn("[CreateGameTransaction]收到创建战斗协议,但当前正在战斗状态中!");
            return;
        }

        if (SceneManager.Instance.currentType == SceneType.PVP_ROOM_SCENE) {
            let msg: BattlePrepareMsg = this._pkg.readBody(BattlePrepareMsg) as BattlePrepareMsg;
    ...
    private initBattleInfo() {
        ...
        if (WorldBossHelper.checkSecretFb(gameInfo.mapId)) {
            ...
            let secreteventCfg = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_secretevent, secretInfo.eventId) as t_s_secreteventData
            gameInfo.mapResId = secreteventCfg && secreteventCfg.SceneId
            if (!gameInfo.mapResId) {
                Logger.error("战斗地图模板数据获取错误 秘境", msg.mapTempId, secretInfo.eventId)
            }
        } else {
            let mapTemp = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_map, msg.mapTempId)
        ...
        if (!gameInfo.selfHero) {//该场战斗里面不存在自己
            return;
        }
        ...
        if (msg.coolDown.length > 0) {//技能冷却队列
    ...
    private initBattleSkillRes() {
        ...
        SkillResourceLoader.loadList = ArrayUtils.unique(SkillResourceLoader.loadList, "url");
        SkillResourceLoader.soundLoadList = Array.from(new Set(SkillResourceLoader.soundLoadList));
    ...
    private initPawnSkill(skillIds: any[]) {
        ...
        if (model.soldierTemplates)//召唤怪的id列表
        {
            model.soldierTemplates.forEach(pId => {
                pawnTemp = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_pawntemplate, pId);
                pawnSkillIds = pawnSkillIds.concat(this.getAwakenSkillByAi(pawnTemp.AI));//召唤怪的ai技能
            });
        }

        model.roleList.forEach(role => {
            if (role instanceof PawnRoleInfo)//服务器发过来的士兵列表
            {
    ...
    private initHeroSkill(skillIds: any[]) {
        ...
        let model: BattleModel = BattleManager.Instance.battleModel;
        let morphSkills: any[] = [SkillData.PET_MORPH_SKILL, SkillData.PET_UNMORPH_SKILL];
        ...
        model.roleList.forEach(role => {
            if (role instanceof HeroRoleInfo) {
                ...
                hero.props.forEach(prop => {
                    if ((prop as BattlePropItem).skillTempId != 0 && skillIds.indexOf(prop.skillTempId) == -1) {
                ...
                let heroTempInfo: t_s_herotemplateData = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_herotemplate, hero.templateId)
                if (heroTempInfo) {
                    defaultSkill = heroTempInfo.DefaultSkill;
                }
                if (skillIds.indexOf(defaultSkill) == -1)//默认技能
                {
                ...
                for (let k: number = 0; k < hero.skillIds.length; k++)//英雄技能
                {
                ...
                for (let j: number = 0; j < hero.trialProps.length; j++)//试练技能
                {
                ...
                for (let k = 0; k < hero.petSkillIds.length; k++) {//变身后技能
        ...
        if (model.battleType == BattleType.PET_PK || model.battleType == BattleType.REMOTE_PET_BATLE) {
    ...
    private getAwakenSkillByAi(ai: number): any[] {
        let heroAisTemp: t_s_heroaiData = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_heroai, ai);
        let soldierIds: any[] = []
        let soldierArr: any[]
        let arr: any[] = []
        let soldierAi: number
        let pawnAisTemp: t_s_heroaiData;
        if (heroAisTemp) {//Soldiers:XXXX,Y,xxxx为士兵模板id, y为召唤出的士兵的站位
            if (heroAisTemp.Soldiers != null && heroAisTemp.Soldiers != "") {
                ...
                soldierIds.forEach(id => {
                    let pawnTemp: t_s_pawntemplateData = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_pawntemplate, id);
                    if (pawnTemp) {
                        heroAisTemp = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_heroai, pawnTemp.AI);
                        if (heroAisTemp) {
     ...
     *
     */
    private createPawnRoleInfo(gameInfo: BattleModel, msg: BattlePrepareMsg): BattleModel {
     ...
     *
     */
    private createHeroRoleInfo(gameInfo: BattleModel, msg: BattlePrepareMsg): BattleModel {
        ...
        for (let i: number = 0; i < leng; i++) {
            ...
            Logger.battle("收到战斗英雄列表:" + role.heroInfo.nickName, heroMsg, petMsg, msg.pets);
            gameInfo.addRole(role);
            if (role.petRoleInfo) {
        ...
        if (gameInfo.battleType == BattleType.REMOTE_PET_BATLE
            ...
            for (let item of gameInfo.roleList.values()) {
                if (item instanceof HeroRoleInfo) {
                    ...
                    if (!item.heroInfo.nickName) {
                        let temp = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_herotemplate, role.templateId) as t_s_herotemplateData;
     ...
     * @returns 
     */
    private refreshSelfSide(gameInfo: BattleModel, msg: BattlePrepareMsg): BattleModel {
     ...
     *
     */
    private createPetRoleInfo(gameInfo: BattleModel, msg: BattlePrepareMsg): BattleModel {
    ...
    private createLookBattleHeroRoleInfo(gameInfo: BattleModel, msg: BattlePrepareMsg): BattleModel {
        if (gameInfo.isAllPetPKBattle()) {
