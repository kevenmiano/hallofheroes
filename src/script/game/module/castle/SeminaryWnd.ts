...
export default class SeminaryWnd extends BaseWindow {
    ...
    public OnInitWind() {
        ...
        this.frame.getChild('title').text = LangManager.Instance.GetTranslation("HigherGradeOpenTipView.content10");
        this.n3Txt.text = LangManager.Instance.GetTranslation("HigherGradeOpenTipView.content10");
        this.n4Txt.text = LangManager.Instance.GetTranslation("CasernWnd.needResourceTxt");
        this.n5Txt.text = LangManager.Instance.GetTranslation("CasernWnd.needTimeTxt");
    ...
    private addEvent() {
        ...
        BuildingManager.Instance.addEventListener(BuildingEvent.BUILDING_UPGRADE_RECEIVE, this.__technologyUpdateHandler, this);
        BuildingManager.Instance.addEventListener(BuildingEvent.CREATE_NEW_BUILDING, this.__createNewTechItemHandler, this);
        if (this.order) {
    ...
    private removeEvent() {
        ...
        BuildingManager.Instance.removeEventListener(BuildingEvent.BUILDING_UPGRADE_RECEIVE, this.__technologyUpdateHandler, this);
        BuildingManager.Instance.removeEventListener(BuildingEvent.CREATE_NEW_BUILDING, this.__createNewTechItemHandler, this);
        if (this.order) {
    ...
    private onTabChanged() {
        if (this.tab.selectedIndex == 0) {
            ...
            if (ArmyManager.Instance.thane.grades < 50) {
                MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("SeminaryWnd.tabTips"));
                this.tab.selectedIndex = 0;
                this._itemList = this._itemList1;
                return;
            }
    ...
    private initQuestEvent() {
        ...
        for (let i: number = 0; i < buildOrderList.length; i++) {
            if (i == 0) {
                ...
                this._vData1.addEventListener(Laya.Event.COMPLETE, this.__completeHandler, this);
                ...
                this._vData2.addEventListener(Laya.Event.COMPLETE, this.__completeHandler, this);
    ...
    private removeQuestEvent() {
        ...
        for (let i: number = 0; i < buildOrderList.length; i++) {
            if (i == 0) {
                this._vData1 = buildOrderList[i];
                this._vData1.removeEventListener(Laya.Event.COMPLETE, this.__completeHandler, this);
                this._vData1.removeEventListener(Laya.Event.CHANGE, this.__changeHandler1, this);
            } else if (i == 1) {
                this._vData2 = buildOrderList[i];
                this._vData2.removeEventListener(Laya.Event.COMPLETE, this.__completeHandler, this);
    ...
    protected updateBuilderOrderTxt(vData: BuildingOrderInfo, tyep: number = 1) {
        ...
        if (tyep == 1) {
            if (vData && parseInt(vData.remainTime.toString()) <= 0) {//剩余时间小于0
    ...
    protected updateTxt(str1: string, str2: string) {
        if (this.order && parseInt(this.order.remainTime.toString()) <= 0) {
            ...
            if (this.order) {
                ...
                this.n15Txt.text = "";
                this.ReduceTimeValueTxt.color = "#00F0FF";
                Logger.error("还没有科技队列, 账号数据问题 LoginInitDataAccept __loadBuildOrderHandler 不存在orderType==2的情况");
            }
        }
    }
    ...
    private initData() {
        ...
        dic.forEach((element: BuildInfo) => {
            if (element && element.templeteInfo) {
                if (element.templeteInfo.MasterType == MasterTypes.MT_INTERNALTECHNOLOGY
    ...
    private isPrimary(buildInfo: BuildInfo): boolean {
        if (buildInfo && buildInfo.templeteInfo
    ...
    private initBuindInfo() {
        this.LvTxt.text = LangManager.Instance.GetTranslation("fish.FishFrame.levelText", this._data.templeteInfo.BuildingGrade);
        ...
        if (nextTemp) {
            this.CurrentValueTxt.text = this._data.getNextNeedResource(nextTemp)['GoldConsume'].toString();
            ...
            this.MaxValueTxt.text = LangManager.Instance.GetTranslation("buildings.BaseBuildFrame.maxGrade");
    ...
    private __reduceClickHandler() {
        ...
        if (this.order.remainTime > 0) {
            ...
            if (VIPManager.Instance.model.vipCoolPrivilege) {
                ...
                UIManager.Instance.ShowWind(EmWindow.VipCoolDownFrameWnd, { orderId: this.order.orderId, pointNum: costPoint, backFun: back.bind(BuildingManager.Instance) });
            }
        }
    }
    ...
    private __technologyUpdateHandler(data: BuildInfo) {
        ...
        if (bTemp && bTemp.MasterType == MasterTypes.MT_INTERNALTECHNOLOGY || bTemp.MasterType == MasterTypes.MT_WARTECHNOLOGY) {
            NotificationManager.Instance.dispatchEvent(TechnologyItemEvent.UPDATA, buildInfo);
        }
        this.initBuindInfo();
    ...
    private updateView() {
        var txt1: string = LangManager.Instance.GetTranslation("buildings.seminary.view.TechnologyOrderItemView.content");
        var txt2: string = LangManager.Instance.GetTranslation("buildings.seminary.view.TechnologyOrderItemView.content2");
