...
import {
  EmOuterCityWarBuildSiteOptType,
...
export default class OuterCityWarBuildSiteItem extends FUI_OuterCityWarBuildSiteItem {
  ...
  public onConstruct(): void {
    ...
    this.btnSettingDefance.text = LangManager.Instance.GetTranslation(
  ...
  public refreshView() {
    ...
    this.txtOccupyScore.text = LangManager.Instance.GetTranslation(
      "public.addMark",
      this._info.occupyScore,
    );
    this.txtOptState0.text = LangManager.Instance.GetTranslation(
      "outerCityWar.cannotJoinFight",
    );
    this.txtOptState3.text = LangManager.Instance.GetTranslation(
      "outerCityWar.cannotAttackSameGuild",
    );
    this.txtOptState6.text = LangManager.Instance.GetTranslation(
      "public.battle.battling",
    );
    ...
    if (!playerInfo) {
      ...
      if (playerInfo.heroType == EmOuterCityWarHeroType.Npc) {
        ...
        if (isPetStyle) {
          ...
          this.txtPetTotalCapaity.text = playerInfo
            .getTotalPetCapaity()
            .toString();
          for (
            let index = 1;
            index <= OuterCityWarModel.MaxEnterWarPetCnt;
            index++
          ) {
    ...
    let optType = Number(
      this._info.getOptType(this.fightModel.playerInfo.userId),
    );
    let idx = 0;
    switch (optType) {
  ...
  btnSettingDefanceClick() {
    FrameCtrlManager.Instance.open(EmWindow.OuterCityWarDefenceSettingWnd, {
      buildInfo: this._info.buildInfo,
      orderId: this._info.orderId,
    });
  }

  btnGiveUpClick() {
    let content: string = LangManager.Instance.GetTranslation(
      "outerCityWar.giveupSiteTip",
    );
    SimpleAlertHelper.Instance.Show(
      ...
      (b) => {
        if (b) {
          OuterCityWarManager.Instance.sendGiveUpBuildSite(
            this._info.buildInfo.sonType,
            this._info.orderId,
          );
        }
      },
  ...
  btnAttackClick() {
    ...
    OuterCityWarManager.Instance.sendAttackDefencerBuildSite(
      this._info.buildInfo.sonType,
      this._info.orderId,
    );
  }
  ...
  public checkCond() {
    ...
    if (
      guildInfo.camp == CampType.Attack &&
      !this.info.buildInfo.preOneCompleteOccupyByAttacker
    ) {
      MessageTipManager.Instance.show(
        LangManager.Instance.GetTranslation("outerCityWar.attackPreBuild"),
      );
      return false;
    }

    if (
      guildInfo.camp == CampType.Defence &&
      !this.info.buildInfo.preOneCompleteOccupyByDefencer
    ) {
      MessageTipManager.Instance.show(
        LangManager.Instance.GetTranslation("outerCityWar.attackPreBuild"),
      );
