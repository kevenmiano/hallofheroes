...
export default class RoomListItem extends FUI_RoomListItem {
    ...
    private refreshView() {
        this.resetItem();
        if (this._roomInfo) {
            this.txtNum.text = LangManager.Instance.GetTranslation("public.No", this._roomInfo.id);
            this.txtCapicity.text = this._roomInfo.curCount + " / " + this._roomInfo.capacity;
            this.imgBgNum.visible = true;
            this.imgLock.visible = this._roomInfo.isLock;
            this.imgBg.icon = fgui.UIPackage.getItemURL(EmWindow.RoomList, this.roomSceneType == RoomSceneType.PVE ? "Img_Room_Box1" : "Img_Room_Box2")

            if (this.roomSceneType == RoomSceneType.PVE) {
                if (this._roomInfo.mapTemplate) {
                    let lvstr = LangManager.Instance.GetTranslation("public.level4", "<br>", this.roomInfo.mapTemplate.MinLevel, this.roomInfo.mapTemplate.MaxLevel);
                    ...
                    this.txtTitle.text = this.roomInfo.mapTemplate.CampaignNameLang + lvstr;
                ...
                this.txtTitle.text = LangManager.Instance.GetTranslation("RoomListItem.Title01")
    ...
    private resetItem() {
        this.imgBg.icon = fgui.UIPackage.getItemURL(EmWindow.RoomList, "Img_Room_Box")
        this.txtNum.text = "";
        this.txtTitle.text = "";
