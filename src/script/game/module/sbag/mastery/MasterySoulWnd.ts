...
export default class MasterySoulWnd extends BaseWindow {
    ...
    public addEvent() {
        ...
        NotificationManager.Instance.addEventListener(ExtraJobEvent.CLICK_INLAY, this.onClickInay, this);
        NotificationManager.Instance.addEventListener(ExtraJobEvent.SELECT_HOLE, this.onSelectHole, this);
    }
    ...
    public removeEvent() {
        ...
        NotificationManager.Instance.removeEventListener(ExtraJobEvent.CLICK_INLAY, this.onClickInay, this);
        NotificationManager.Instance.removeEventListener(ExtraJobEvent.SELECT_HOLE, this.onSelectHole, this);
    }
    ...
    private onClickInay(info:GoodsInfo){
        if(info['isInlay']){
            PlayerManager.Instance.reqExtraJobEquip(5,this._info.equipType,info.bagType,info.pos,this.inlayCom.curSelectInlayHole.index) 
            return;
        }
        //若玩家未选中宝石孔：飘文字提示“请先选择镶嵌凹槽”
        if(!this.inlayCom.curSelectInlayHole){
            MessageTipManager.Instance.show(LangManager.Instance.GetTranslation('Mastery.chooseHole'));
            return;
        }
        //若当前选中宝石孔未开启：飘文字提示“魂器xx阶可开启凹槽”
        if(this.inlayCom.curSelectInlayHole.bNotOpen){
            MessageTipManager.Instance.show(LangManager.Instance.GetTranslation('Mastery.openHoleLevel2',this.inlayCom.curSelectInlayHole.needLevel));
            return;
        }
        ...
        PlayerManager.Instance.reqExtraJobEquip(4,this._info.equipType,info.bagType,info.pos,this.inlayCom.curSelectInlayHole.index) 
    }
    ...
    private updateStrenView(){
        this.txt_lv1.text = LangManager.Instance.GetTranslation('Mastery.levelNum',this._info.strengthenLevel);
        this.txt_lv2.text = LangManager.Instance.GetTranslation('Mastery.levelNum',this._info.strengthenLevel+1);
        let cfg = TempleteManager.Instance.getExtrajobEquipCfg(this._info.equipType,this._info.equipLevel);
        let curStrenCfg:t_s_extrajobequipstrengthenData = TempleteManager.Instance.getExtrajobEquipStrenthenCfg(this._info.strengthenLevel);
        let nextStrenCfg:t_s_extrajobequipstrengthenData = TempleteManager.Instance.getExtrajobEquipStrenthenCfg(this._info.strengthenLevel+1);
        this.updateCurAttr(cfg,curStrenCfg);
        if(!nextStrenCfg){//最大等级
            this.txt_open.text =  LangManager.Instance.GetTranslation("buildings.water.view.PlayerTreeExpView.msg01");
            ...
            if(nextStrenCfg && this._info.equipLevel < nextStrenCfg.NeedEquipLevel){//阶数不足不能强化
                this.txt_open.text =  LangManager.Instance.GetTranslation("Bag.mastery07",nextStrenCfg.NeedEquipLevel);
                this.txt_open.visible = true;
    ...
    private updateStageView(){
        this.txt_stage1.text = LangManager.Instance.GetTranslation('Mastery.stageNum',this._info.equipLevel);
        let cfg = TempleteManager.Instance.getExtrajobEquipCfg(this._info.equipType,this._info.equipLevel);
        let cfg1 = TempleteManager.Instance.getExtrajobEquipCfg(this._info.equipType,this._info.equipLevel+1);
        ...
        if(!cfg1){//最大等级
            ...
            this.magicDefence.getControllerAt(0).selectedIndex = 0;
            this.live.getControllerAt(0).selectedIndex = 0;
            this.txt_stage2.text = LangManager.Instance.GetTranslation('Mastery.stageNum',this._info.equipLevel+1);
            if(cfg1 && ExtraJobModel.instance.totalLevel < cfg1.NeedTotalJobLevel){//显示红色提示文本“秘典总等级达到xx级开放下一进阶等级”
                this.txt_open.text =  LangManager.Instance.GetTranslation("Bag.mastery06",cfg1.NeedTotalJobLevel);
                this.txt_open.visible = true;
                this.costBox.visible = false;
            }else{
                this.txt_open.visible = false;
    ...
    private updateStrenCost(nextInfo){
        if(nextInfo){
            ...
            this.txt_needProp.text = LangManager.Instance.GetTranslation('fish.FishFrame.countText',ownStr,nextInfo.CostItemCount);
            this.txt_gold.text = FormularySets.toStringSelf(nextInfo.CostGold, GeniusPanel.STEP);
    ...
    private updateCurAttr(attrCfg:t_s_extrajobequipData,strenCfg:t_s_extrajobequipstrengthenData) {
        if (attrCfg) {
            if(attrCfg.Attack >0){
                this.attack.getChild('rTxtName').text = LangManager.Instance.GetTranslation("armyII.ThaneAttributeView.Tip13");
                //强化等级增加的属性（绿色字体） 强化属性=强化加成百分比*魂器当前阶数的属性
                let addValue = strenCfg ? strenCfg.ExtraPropertyPercent/100* attrCfg.Attack : 0;
                addValue = Math.floor(addValue);
                this.attack.getChild('rTxtCur').text = (attrCfg.Attack+addValue).toString();     
            }
            this.attack.visible = attrCfg.Attack >0;

            if(attrCfg.Defence >0){
                this.defence.getChild('rTxtName').text = LangManager.Instance.GetTranslation("armyII.ThaneAttributeView.Tip14");
                let addValue = strenCfg ? strenCfg.ExtraPropertyPercent/100 * attrCfg.Defence : 0; 
                addValue = Math.floor(addValue);
                this.defence.getChild('rTxtCur').text =  (attrCfg.Defence+addValue).toString();     
            }
            
            if(attrCfg.MagicAttack >0){
                this.magicAttack.getChild('rTxtName').text = LangManager.Instance.GetTranslation("armyII.ThaneAttributeView.Tip15");
                let addValue = strenCfg ? strenCfg.ExtraPropertyPercent/100 * attrCfg.MagicAttack : 0; 
                addValue = Math.floor(addValue);
                this.magicAttack.getChild('rTxtCur').text = (attrCfg.MagicAttack+addValue).toString();        
            }
            this.magicAttack.visible = attrCfg.MagicAttack >0;

            if(attrCfg.MagicDefence >0){
                this.magicDefence.getChild('rTxtName').text = LangManager.Instance.GetTranslation("armyII.ThaneAttributeView.Tip16");
                let addValue = strenCfg ? strenCfg.ExtraPropertyPercent/100 * attrCfg.MagicDefence : 0; 
                addValue = Math.floor(addValue);
                this.magicDefence.getChild('rTxtCur').text = (attrCfg.MagicDefence+addValue).toString();        
            }
            this.magicDefence.visible = attrCfg.MagicDefence >0;

            if(attrCfg.Live >0){
                this.live.getChild('rTxtName').text = LangManager.Instance.GetTranslation("armyII.ThaneAttributeView.Tip11");
                let addValue = strenCfg ? strenCfg.ExtraPropertyPercent/100 * attrCfg.Live : 0; 
                addValue = Math.floor(addValue);
                this.live.getChild('rTxtCur').text = (attrCfg.Live+addValue).toString();        
     ...
     */
    private updateNextStrenAttr(attrCfg:t_s_extrajobequipData,curStrenCfg:t_s_extrajobequipstrengthenData,nextStrenCfg:t_s_extrajobequipstrengthenData) {
        let cfg = TempleteManager.Instance.getExtrajobEquipCfg(this._info.equipType,this._info.equipLevel);
        if (cfg) {
            let nextAdd;
            if(attrCfg.Attack >0){
                //强化等级增加的属性（绿色字体） 强化属性=强化加成百分比*魂器当前阶数的属性
                let addValue = curStrenCfg ? curStrenCfg.ExtraPropertyPercent/100 * attrCfg.Attack : 0;
                addValue = Math.floor(addValue);
                let addValue1 = nextStrenCfg ? nextStrenCfg.ExtraPropertyPercent/100 * attrCfg.Attack : 0;
                nextAdd = Math.floor(Math.max(addValue1-addValue));
                if(nextStrenCfg){
                    this.attack.getChild('rTxtNext').text = LangManager.Instance.GetTranslation('PropertyCurrentAndAdd', attrCfg.Attack+addValue,nextAdd);     
                }else{
            ...
            if(attrCfg.Defence >0){
                let addValue = curStrenCfg ? curStrenCfg.ExtraPropertyPercent/100 * attrCfg.Defence : 0; 
                addValue = Math.floor(addValue);
                let addValue1 = nextStrenCfg ? nextStrenCfg.ExtraPropertyPercent/100 * attrCfg.Defence : 0;
                nextAdd = Math.floor(Math.max(addValue1-addValue));
                if(nextStrenCfg){
                    this.defence.getChild('rTxtNext').text = LangManager.Instance.GetTranslation('PropertyCurrentAndAdd', attrCfg.Defence+addValue,nextAdd);     
                }else{
            ...
            if(attrCfg.MagicAttack >0){
                let addValue = curStrenCfg ? curStrenCfg.ExtraPropertyPercent/100 * attrCfg.MagicAttack : 0; 
                addValue = Math.floor(addValue);
                let addValue1 = nextStrenCfg ? nextStrenCfg.ExtraPropertyPercent/100 * attrCfg.MagicAttack : 0;
                nextAdd = Math.floor(Math.max(addValue1-addValue));
                if(nextStrenCfg){
                    this.magicAttack.getChild('rTxtNext').text = LangManager.Instance.GetTranslation('PropertyCurrentAndAdd', attrCfg.MagicAttack+addValue,nextAdd);     
                }else{
                    this.magicAttack.getChild('rTxtNext').text = attrCfg.MagicAttack.toString();
                }
            } 
            this.magicAttack.visible = attrCfg.MagicAttack >0;

            if(attrCfg.MagicDefence >0){
                let addValue = curStrenCfg ? curStrenCfg.ExtraPropertyPercent/100 * attrCfg.MagicDefence : 0; 
                addValue = Math.floor(addValue);
                let addValue1 = nextStrenCfg ? nextStrenCfg.ExtraPropertyPercent/100 * attrCfg.MagicDefence : 0;
                nextAdd = Math.floor(Math.max(addValue1-addValue));
                if(nextStrenCfg){
                    this.magicDefence.getChild('rTxtNext').text = LangManager.Instance.GetTranslation('PropertyCurrentAndAdd', attrCfg.MagicDefence+addValue,nextAdd);  
                }else{
                    this.magicDefence.getChild('rTxtNext').text = attrCfg.MagicDefence.toString();
                }    
            }
            this.magicDefence.visible = attrCfg.MagicDefence >0;

            if(attrCfg.Live >0){
                let addValue = curStrenCfg ? curStrenCfg.ExtraPropertyPercent/100 * attrCfg.Live : 0; 
                addValue = Math.floor(addValue);
                let addValue1 = nextStrenCfg ? nextStrenCfg.ExtraPropertyPercent/100 * attrCfg.Live : 0;
                nextAdd = Math.floor(Math.max(addValue1-addValue));
                if(nextStrenCfg){
                    this.live.getChild('rTxtNext').text = LangManager.Instance.GetTranslation('PropertyCurrentAndAdd', attrCfg.Live+addValue,nextAdd);     
                }else{
     ...
     * @param strenCfg 魂器当前阶数属性强化配置
     */
    private updateNextAttr(attrCfg:t_s_extrajobequipData,strenCfg:t_s_extrajobequipstrengthenData) {
        let cfg = TempleteManager.Instance.getExtrajobEquipCfg(this._info.equipType,this._info.equipLevel);
        if (cfg) {
            if(attrCfg.Attack >0){
                //强化等级增加的属性（绿色字体） 强化属性=强化加成百分比*魂器当前阶数的属性
                ...
                addValue = Math.floor(addValue);
                if(strenCfg){
                    this.attack.getChild('rTxtNext').text = LangManager.Instance.GetTranslation('PropertyCurrentAndAdd', attrCfg.Attack+addValue,addValue);     
                }else{
                    this.attack.getChild('rTxtNext').text =attrCfg.Attack.toString();
                }
            }
            ...
            if(attrCfg.Defence >0){
                ...
                addValue = Math.floor(addValue);
                if(strenCfg){
                    this.defence.getChild('rTxtNext').text = LangManager.Instance.GetTranslation('PropertyCurrentAndAdd', attrCfg.Defence+addValue,addValue);     
                }else{
                    this.defence.getChild('rTxtNext').text =attrCfg.Defence.toString();
                }
            }
            ...
            if(attrCfg.MagicAttack >0){
                ...
                addValue = Math.floor(addValue);
                if(strenCfg){
                    this.magicAttack.getChild('rTxtNext').text = LangManager.Instance.GetTranslation('PropertyCurrentAndAdd', attrCfg.MagicAttack+addValue,addValue);     
                }else{
                    this.magicAttack.getChild('rTxtNext').text = attrCfg.MagicAttack.toString();
                }
            } 
            ...
            if(attrCfg.MagicDefence >0){
                ...
                addValue = Math.floor(addValue);
                if(strenCfg){
                    this.magicDefence.getChild('rTxtNext').text = LangManager.Instance.GetTranslation('PropertyCurrentAndAdd', attrCfg.MagicDefence+addValue,addValue);  
                }else{
                    this.magicDefence.getChild('rTxtNext').text = attrCfg.MagicDefence.toString();
                }    
            }
            ...
            if(attrCfg.Live >0){
                ...
                addValue = Math.floor(addValue);
                if(strenCfg){
                    this.live.getChild('rTxtNext').text = LangManager.Instance.GetTranslation('PropertyCurrentAndAdd', attrCfg.Live+addValue,addValue);     
                }else{
                    this.live.getChild('rTxtNext').text = attrCfg.Live.toString();
                }    
            }
    ...
    private helpBtnClick() {
        ...
        let content: string = LangManager.Instance.GetTranslation("Bag.masterySoulHelp");
