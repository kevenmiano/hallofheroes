...
export default class PetPKSkillItemListView extends FUI_PetPKSkillItemListView {
    ...
    */
    ...
    public initView(skills: any[]) {
        ...
        for (let i = 0; i < 22; i++) {
            ...
            if (i < PetPKSkillItemListView.SkillIndex1) {//第一个英灵的6个普通技能
                let btnName = "itemSkill1_" + (i + 1);
                item = new BattleSkillItemII(this.getChild(btnName) as fgui.GComponent);
                item.index = i;
                item.numCount = i + 1;
            } else if (i >= PetPKSkillItemListView.SkillIndex1 && i < PetPKSkillItemListView.SkillIndex2) {//第二个英灵的6个普通技能
                let btnName = "itemSkill2_" + (i - 5);
                item = new BattleSkillItemII(this.getChild(btnName) as fgui.GComponent);
                item.index = i;
                item.numCount = i - 5;
            } else if (i >= PetPKSkillItemListView.SkillIndex2 && i < PetPKSkillItemListView.SkillIndex3) {//第三个英灵的6个普通技能
                let btnName = "itemSkill3_" + (i - 11);
                item = new BattleSkillItemII(this.getChild(btnName) as fgui.GComponent);
                item.index = i;
                item.numCount = i - 11;
            } else if (i == PetPKSkillItemListView.TalentIndex) {//天赋
                item = new BattleSkillItemII(this.getChild("itemTalent") as fgui.GComponent);
                ...
                if (index) {
                    ...
                    item = new PropSkillBattleItem(this.getChild(btnName) as fgui.GComponent);
                    item.index = i;
            ...
            if (!item) return
            item.subIndex = i;
            if (i == PetPKSkillItemListView.TalentIndex && this.battleModel.isLockTalent) {
     ...
     * 
     */
    public cdStart(skillId: number, waitTime: number = 0, cd: number = -1, cdType: number = 0) {
    ...
    private getItemViewBySkillId(skillId: number) {
        ...
        for (let item of this._skillItemViews) {
            ...
            if (item.data instanceof t_s_skilltemplateData && skillId == (item.data as t_s_skilltemplateData).Parameter3) {//使用qte以后, 收到的技能id为qte技能id, 原id为p3字段
                vec.push(item);
            }
        }
        return vec;
    ...
    private __skillClick(target: BattleSkillItemII, data: any) {
        if (!BattleManager.Instance.battleModel.currentHero.isLiving) {
            Logger.xjy("英灵 死亡 == ", BattleManager.Instance.battleModel.currentHero);
            return;
        }
        ...
        if (target.index == 18) {//天赋
            this.battleModel.skillUserInfoDic.set(target.data.TemplateId, { livingId: livingID, skillId: target.data.TemplateId, isProp: false });
        } else if (target.index == 19 || target.index == 20|| target.index == 21) {//符文
            this.battleModel.skillUserInfoDic.set(target.data.skillTempId, { livingId: livingID, skillId: target.data.skillTempId, isProp: true });
            this.battleModel.hasPropIngUsed = true;
        }
        NotificationManager.Instance.sendNotification(BattleNotic.PETPK_ACTION_SKILL, data);
    }
    ...
    public resetSelect(value: boolean) {
        for (let index = 0; index < this._specialSkills.length; index++) {
            ...
            if (element.data) {
                if (element.data instanceof t_s_skilltemplateData) {//天赋
                    element.selected(value);
                } else if (element.data instanceof BattlePropItem) {//符文
                    element.selected(value);
                }
            }
        }
        ...
        for (let index = 0; index < this._specialSkills.length; index++) {
            ...
            if (element.data) {
                if (element.data instanceof t_s_skilltemplateData) {//天赋
                    ...
                    if (!this.battleModel.skillUserInfoDic.get(skillId) && !this.battleModel.hasPropIngUsed) {
                        if (this._enable && element.data.hasOwnProperty("useCount") && element.data.useCount <= 0) {
                            continue;
                        }
                        element.parentEnable = this._enable;
    ...
    public checkIsLock() {
        ...
        for (let index = 0; index < this._skillItemViews.length; index++) {
            ...
            if (element.data) {
                ...
                if (this.existUnenableSkillBuffer(skillId) &&  this.checkIndex(element.index)) {
    ...
    private petExistUnenableSkillBuffer(id:number,data:PetData){
        ...
        for (const key in armyInfoHeros) {
            ...
            if (hRole && hRole.existUnenableSkillBuffer(id) && hRole.templateId == data.templateId) {
                flag = true;
            }
        }
        return flag;
    ...
    */
    public setEnable(value: boolean) {
        ...
        if (livingId > 0) {
            let hero1: HeroRoleInfo = this.battleModel.getBaseRoleInfoByPetTemplateId(this.battleModel.petTemplateId1);
            let hero2: HeroRoleInfo = this.battleModel.getBaseRoleInfoByPetTemplateId(this.battleModel.petTemplateId2);
            let hero3: HeroRoleInfo = this.battleModel.getBaseRoleInfoByPetTemplateId(this.battleModel.petTemplateId3);
            if (hero1 && hero1.petRoleInfo && livingId == hero1.petRoleInfo.livingId) {
                for (let index = 0; index < this._skillItemViews.length; index++) {
                    const element: BattleSkillItemII = this._skillItemViews[index];
            ...
            } else if (hero2 && hero2.petRoleInfo && livingId == hero2.petRoleInfo.livingId) {
                for (let index = 0; index < this._skillItemViews.length; index++) {
                    const element: BattleSkillItemII = this._skillItemViews[index];
            ...
            } else if (hero3 && hero3.petRoleInfo && livingId == hero3.petRoleInfo.livingId) {
                for (let index = 0; index < this._skillItemViews.length; index++) {
                    const element: BattleSkillItemII = this._skillItemViews[index];
    ...
    private checkIndex(index: number): boolean {
        ...
        if (this.petIndexCtr.selectedIndex == 0 && this.indexArr1.indexOf(index) != -1) {
            flag = true;
        } else if (this.petIndexCtr.selectedIndex == 1 && this.indexArr2.indexOf(index) != -1) {
            flag = true;
        } else if (this.petIndexCtr.selectedIndex == 2 && this.indexArr3.indexOf(index) != -1) {
            flag = true;
        }
        return flag;
    }
    ...
    public updateTalentSkill(info: t_s_skilltemplateData) {
        let item = this._skillItemViews[PetPKSkillItemListView.TalentIndex] as BattleSkillItemII;
        item.data = info;
    }
