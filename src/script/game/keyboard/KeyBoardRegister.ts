...
export default class KeyBoardRegister
{
    ...
    public setup()
    {
        KeyboardManager.Instance.addEventListener(Laya.Event.KEY_DOWN, this.__keyDownHandler, this);
        KeyboardManager.Instance.addEventListener(Laya.Event.KEY_UP, this.__keyUpHandler, this);
    }
    ...
    public registerMapKey(left:Function, right:Function, top:Function, down:Function)
    {
        this._leftFunction = left;
        this._rightFunction = right;
        this._topFunction = top;
        this._downFunction = down;
        KeyboardManager.Instance.registerKeyAction(KeyStroke.VK_LEFT, this.__leftKeyDownHandler.bind(this));
        KeyboardManager.Instance.registerKeyAction(KeyStroke.VK_RIGHT, this.__rightKeyDownHandler.bind(this));
        KeyboardManager.Instance.registerKeyAction(KeyStroke.VK_UP, this.__topKeyDownHandler.bind(this));
        KeyboardManager.Instance.registerKeyAction(KeyStroke.VK_DOWN, this.__downKeyDownHandler.bind(this));
    }

    public removeMapKey(left:Function, right:Function, top:Function, down:Function)
    {
        this._leftFunction = null;
        this._rightFunction = null;
        this._topFunction = null;
        this._downFunction = null;
        KeyboardManager.Instance.unregisterKeyAction(KeyStroke.VK_LEFT, this.__leftKeyDownHandler.bind(this));
        KeyboardManager.Instance.unregisterKeyAction(KeyStroke.VK_RIGHT, this.__rightKeyDownHandler.bind(this));
        KeyboardManager.Instance.unregisterKeyAction(KeyStroke.VK_UP, this.__topKeyDownHandler.bind(this));
        KeyboardManager.Instance.unregisterKeyAction(KeyStroke.VK_DOWN, this.__downKeyDownHandler.bind(this));
    }
    ...
    private showOuterCity()
    {
        ...
        var content:string = LangManager.Instance.GetTranslation("yishi.keyboard.KeyBoardRegister.content01");
        SimpleAlertHelper.Instance.popAlerFrame(prompt, content, confirm, cancel, {callback:this.__showOuterCityResponse.bind(this)});
    }
    ...
    private showInnerCity()
    {
        ...
        var cancel:string = LangManager.Instance.GetTranslation("public.cancel");
        var prompt:string = LangManager.Instance.GetTranslation("public.prompt");
        var content:string = LangManager.Instance.GetTranslation("yishi.keyboard.KeyBoardRegister.content02");
        SimpleAlertHelper.Instance.popAlerFrame(prompt, content, confirm, cancel, {callback:this.__showInnerCityResponse.bind(this)});
    }
    ...
    private __showOuterCityResponse(boolean:boolean, flag:boolean)
    {
        if(SceneManager.Instance.currentType != SceneType.CASTLE_SCENE)
        {
            ...
            if(str == "")
            {
                str = LangManager.Instance.GetTranslation("yishi.keyboard.KeyBoardRegister.command01");
            }
    ...
    private showMap()
    {
        ...
        if(CampaignManager.Instance.mapModel && WorldBossHelper.checkPetLand(CampaignManager.Instance.mapModel.mapId))
        {
            // FrameCtrlManager.Instance.open(EmWindow.SPACE, SpaceController.PETLAND_MAP);
        }
        else if(CampaignManager.Instance.mapModel && WorldBossHelper.checkMineral(CampaignManager.Instance.mapModel.mapId))
        {
    ...
    private showCampaign()
    {
        ...
        if(curScene != SceneType.CASTLE_SCENE && curScene != SceneType.SPACE_SCENE && curScene != SceneType.OUTER_CITY_SCENE)
        {
            return;
        }
        AudioManager.Instance.playSound(SoundIds.CAMPAIGN_OUTERCITY_STAR_SOUND, 0);
