...
export default class ScoreOrGesteAction extends MapBaseAction {
  ...
  update() {
    ...
    if (this._addScoreTime == 30) {
      cArmy = CampaignManager.Instance.mapModel.getBaseArmyByUserId(
        this._msg.userId,
        this._msg.serverName,
      );
      if (cArmy)
        cArmy.score = this._msg.isAdd ? this._msg.score : -this._msg.score;
      ...
      if (cArmy)
        cArmy.geste = this._msg.isAdd ? this._msg.geste : -this._msg.geste;
      this.actionOver();
    }
  }
}
