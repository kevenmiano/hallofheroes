...
export default class ConsumoCalcItem extends FUI_ConsumoCalcItem {
    ...
    public showRemainTime() {
        if (this._info && this._info.remainTime > 0) {
            PlayerManager.Instance.currentPlayerModel.addEventListener(PlayerEvent.SYSTIME_UPGRADE_SECOND, this.__timerHandler, this);
        }
    }
    ...
    public hideRemainTime() {
        PlayerManager.Instance.currentPlayerModel.removeEventListener(PlayerEvent.SYSTIME_UPGRADE_SECOND, this.__timerHandler, this);
    }
        ...
        if (this._info) {
            ...
            if (_data.hasExchageTimes && this._info.exchangeCount > 1) {
                if (this._info.getIsSpeacial()) {
                    this.txt_Count.text = this._info.finishValue.toString() + "/" + this._info.targetValue.toString();
                } else {
                    this.txt_Count.text = this._info.finishValue.toString() + "/" + this._info.exchangeCount.toString();
                }
            } else {
                this.txt_Count.text = "";
            }
