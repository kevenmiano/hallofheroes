...
export class TransmitHandler {
    ...
    public show(): void {
        ...
        this._containter = fgui.UIPackage.createObject(EmWindow.OuterCity, "asset.outercity.transmitBim").asImage;
    ...
    private __mouseUpHandler(event: Laya.Event): void {
        ...
        if (leng < 3 && this._containter) {
            let walkable: Point = this.mapView.worldWalkLayer.globalToLocal(this._upPos, true);
            if (!this.isWalkable(walkable.x, walkable.y)) {
                ...
                let content: string = LangManager.Instance.GetTranslation("map.campaign.view.ui.RiverView.txt.TransmitHandler.content",leftEnergy,totalEnergy);
                SimpleAlertHelper.Instance.Show(SimpleAlertHelper.SIMPLE_ALERT, null, prompt, content, confirm, cancel, this.__transferHandler.bind(this));
            }
            else {
                let str: string = LangManager.Instance.GetTranslation("map.outercity.mediator.mapview.TransmitHandler.command01");
    ...
    private __transferHandler(b: boolean, flag: boolean) {
        if (b) {
            ...
            if (100 <= current) {
                ...
                let content = LangManager.Instance.GetTranslation("map.outercity.mediator.mapview.TransmitHandler.command02", recharge2);
                let check1RickText: string = LangManager.Instance.GetTranslation("yishi.view.base.check2RickText.text");
                var num: number = GoodsManager.Instance.getGoodsNumByTempId(TemplateIDConstant.TEMP_ID_ENERG_STNE);
                let goodsCount: string = LangManager.Instance.GetTranslation("transmitHandler.goodsCount.text", num);
                UIManager.Instance.ShowWind(EmWindow.UseGoodsAlert, { content: content, goodsId: TemplateIDConstant.TEMP_ID_ENERG_STNE, goodsCount: goodsCount, check1RickText: check1RickText, callback: this.promptCallback.bind(this) });
                return;
            }
        }
    ...
    private promptCallback(flag: boolean, useBind: boolean) {
        if (flag) {
            ...
            if (num >= 1) {//道具数量足够, 直接传送
                ...
                let bindPoint: number = PlayerManager.Instance.currentPlayerModel.playerInfo.giftToken;
                let totalPoint: number = useBind ? selfPoint + bindPoint : selfPoint;
                if (totalPoint < 100) {
                    NotificationManager.Instance.dispatchEvent(NotificationEvent.SHOW_RECHARGE);
    ...
    private transferBack(result: boolean, flag: boolean, id: number = 0, type: number = 2): void {
        if (result) {
            if (!flag) {
                type = 1;
            }
    ...
    private __stageFrameHandler(): void {
        this._containter.x = StageReferance.stage.mouseX - this._containter.width / 2;
        this._containter.y = StageReferance.stage.mouseY - this._containter.height / 2;
    }
    ...
    public senPosation(pointX: number, pointY: number, type: number): void {
        ...
        this._timeid = setInterval(this.__transferPosationHandler.bind(this), 50000);
        ServerDataManager.listen(S2CProtocol.U_C_ARMY_POSATION, this, this.__transferPosationHandler);
        ServerDataManager.listen(S2CProtocol.U_C_ARMYPOS_FAIL, this, this.__transferPosationHandler);
    }
    ...
    private __transferPosationHandler(): void {
        ...
        ServerDataManager.cancel(S2CProtocol.U_C_ARMYPOS_FAIL, this, this.__transferPosationHandler);
    }
