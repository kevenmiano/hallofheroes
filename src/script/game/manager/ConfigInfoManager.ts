...
export default class ConfigInfoManager {
    ...
    public getKeyToArr2(key: string, splitChar: string = "|"): Array<any> {
        var configInfo: t_s_configData = TempleteManager.Instance.getConfigInfoByConfigName(key);
    ...
    public getKeyToInt(key: string): number {
        return parseInt(TempleteManager.Instance.getConfigInfoByConfigName(key).ConfigValue);
    }
    ...
    public getTimeArr(configArr: Array<string>): Array<number> {
        ...
        var tDate: Date = new Date(this.playerModel.sysCurtime.getFullYear(), this.playerModel.sysCurtime.getMonth(), this.playerModel.sysCurtime.getDay());
     ...
     *  根据开始时间和结束时间排序
     */
    private sortByStartTime(timeArr1: Array<number>, timeArr2: Array<number>): number {
