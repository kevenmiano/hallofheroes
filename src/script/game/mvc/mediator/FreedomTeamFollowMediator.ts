...
export class FreedomTeamFollowMediator implements IMediator {
  ...
  public register(target: any) {
    ...
    //先判断是否跟随别人
    if (
      FreedomTeamManager.Instance.model != null &&
      FreedomTeamManager.Instance.model.followId != 0
    ) {
      //证明现在正在跟随
      if (this._targetInfo == null) {
        //跨场景时, 重新拿值
        this._targetInfo = FreedomTeamManager.Instance.model.getMemberByUserId(
          FreedomTeamManager.Instance.model.followId,
        );
      }
    }
    //数据准备好了再监听
    NotificationManager.Instance.addEventListener(
      ...
      this.__lockFollowtTargetHandler,
      this,
    );
    NotificationManager.Instance.addEventListener(
  ...
  private __teamInfoSyncHandler(evt: FreedomTeamEvent) {
    ...
    if (
      FreedomTeamManager.Instance.model == null ||
      (FreedomTeamManager.Instance.model != null &&
        !FreedomTeamManager.Instance.model.followFlag)
    ) {
      return;
    }
    if (!FreedomTeamManager.Instance.hasTeam) {
      content = LangManager.Instance.GetTranslation(
        "map.internals.mediator.team.FreedomTeamFollowMediator.Tips03",
      );
      if (chatData) {
    ...
    if (this.inTheSameMap) {
      //和被跟随的人在同一个地图
      if (this._targetInfo == null)
        this._targetInfo = FreedomTeamManager.Instance.model.getMemberByUserId(
          this._targetInfo.userId,
        );
      var startPoint: Laya.Point = new Laya.Point(
        this.selfPoint.x * Tiles.WIDTH,
        this.selfPoint.y * Tiles.HEIGHT,
      );
      var endPoint: Laya.Point = new Laya.Point(
        this._targetInfo.curPosX * Tiles.WIDTH,
        this._targetInfo.curPosY * Tiles.HEIGHT,
      );
      ...
      if (distance > 4000000) {
        ...
        content = LangManager.Instance.GetTranslation(
          "map.internals.mediator.team.FreedomTeamFollowMediator.Tips05",
        );
        if (chatData) {
          chatData.channel = ChatChannel.INFO;
          chatData.msg = content;
      ...
      if (this._targetPoint == null) return;
      ...
      if (this._count % 4 == 0 || this.walkOver) {
        ...
        if (SceneManager.Instance.currentType == SceneType.CAMPAIGN_MAP_SCENE) {
          //天空之城就不用管, 战役地图场景就需要重新赋值
          var node: CampaignNode =
            CampaignManager.Instance.mapModel.getMapNodesByPoint(
              this._targetPoint,
            );
          if (node != null) CampaignManager.Instance.mapModel.selectNode = node;
        }
        this.searchPath(
          this.selfPoint.x,
          this.selfPoint.y,
  ...
  private __lockFollowtTargetHandler(data: any) {
    ...
    if (!this.checkScene()) {
      //只有天空之城、英灵岛、紫晶矿场, 这3个场景可以跟随
      content = LangManager.Instance.GetTranslation(
        "map.internals.mediator.team.FreedomTeamFollowMediator.Tips04",
      );
      MessageTipManager.Instance.show(content);
      if (FreedomTeamManager.Instance.model != null)
        FreedomTeamManager.Instance.model.followFlag = false;
    ...
    if (FreedomTeamManager.Instance.model != null)
      this._targetInfo =
        FreedomTeamManager.Instance.model.getMemberByUserId(userId);
    if (
      this._targetInfo &&
      FreedomTeamManager.Instance.memberIsOnline(userId)
    ) {
      //跟随者存在, 并且在线
      var startPoint: Laya.Point = new Laya.Point(
        this.selfPoint.x * Tiles.WIDTH,
      ...
      if (!this.inTheSameMap) {
        //不在同一个地图
        content = LangManager.Instance.GetTranslation(
          "map.internals.mediator.team.FreedomTeamFollowMediator.Tips06",
        );
        MessageTipManager.Instance.show(content);
        if (FreedomTeamManager.Instance.model != null)
          FreedomTeamManager.Instance.model.followFlag = false;
      ...
      this._targetPoint.y = this._targetInfo.curPosY;
      content = LangManager.Instance.GetTranslation(
        "map.internals.mediator.team.FreedomTeamFollowMediator.Tips01",
      );
      MessageTipManager.Instance.show(content);
      if (FreedomTeamManager.Instance.model != null)
  ...
  private get selfPoint(): Laya.Point {
    ...
    if (this._selfInfo instanceof SpaceArmy) {
      ...
      armyView = CampaignManager.Instance.controller.getArmyView(
        this._selfInfo,
      );
    }
    ...
    if (armyView instanceof SpaceArmyView) {
      return new Laya.Point(
        (<SpaceArmyView>armyView).x / Tiles.WIDTH,
        (<SpaceArmyView>armyView).y / Tiles.HEIGHT,
      );
    }
    ...
    // }
      ...
      return new Laya.Point(
        (<CampaignArmyView>armyView).x / Tiles.WIDTH,
        (<CampaignArmyView>armyView).y / Tiles.HEIGHT,
      );
    }
    return new Laya.Point();
  }
  ...
  private get selfMapId(): number {
    var self: BaseArmy = FreedomTeamManager.Instance.model.getMemberByUserId(
      this._selfInfo.userId,
    );
    return self.mapId;
  }
  ...
  private get walkOver(): boolean {
    ...
    if (armyView instanceof SpaceArmyView) {
      if (
        (<SpaceArmyView>armyView).aiInfo &&
        (<SpaceArmyView>armyView).aiInfo.pathInfo
      ) {
        if (
          (<SpaceArmyView>armyView).aiInfo.walkIndex ==
          (<SpaceArmyView>armyView).aiInfo.pathInfo.length
        ) {
          return true;
        }
      }
    ...
    // }
      ...
      if (
        (<CampaignArmyView>armyView).aiInfo &&
        (<CampaignArmyView>armyView).aiInfo.pathInfo
      ) {
        if (
          (<CampaignArmyView>armyView).aiInfo.walkIndex ==
          (<CampaignArmyView>armyView).aiInfo.pathInfo.length
        ) {
          return true;
        }
      }
  ...
  private getTransfer(): Laya.Point {
    ...
    if (this._targetInfo) {
      ...
      if (SceneManager.Instance.currentType == SceneType.SPACE_SCENE) {
        //天空之城
        var snode: SpaceNode =
          SpaceManager.Instance.model.getMapNodeById(nodeId);
        if (snode) {
        ...
        var cnode: CampaignNode =
          CampaignManager.Instance.mapModel.getMapNodeByNodeId(nodeId);
        if (cnode) {
  ...
  private searchPath(
    ...
    var end: Laya.Point;
    var path: any[];
    start = this.getAroundWalkPoint(
      parseInt(startX.toString()) * Tiles.WIDTH,
      parseInt(startY.toString()) * Tiles.HEIGHT,
    );
    if (!start) return;
    end = this.getAroundWalkPoint(
      parseInt(endX.toString()) * Tiles.WIDTH,
      parseInt(endY.toString()) * Tiles.HEIGHT,
    );
    if (!end) return;
  ...
  private getPath(startPoint: Laya.Point, endPoint: Laya.Point): any[] {
    if (SceneManager.Instance.currentType == SceneType.SPACE_SCENE) {
      ...
      return CampaignManager.Instance.controller.searchPath(
        startPoint,
        endPoint,
      );
    }
