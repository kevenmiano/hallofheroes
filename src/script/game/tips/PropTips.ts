...
export class PropTips extends BaseTips {
	...
	private initView() {
		...
		this.puarchargeTxt.text = LangManager.Instance.GetTranslation("market.tips.purcharge");
	}
	...
	private updateView() {
		...
		if (this._info.templateInfo.Property2 == 1 && this._info.templateInfo.SonType == 206) {
			let dropItems = TempleteManager.Instance.getDropItemssByDropId(this._info.templateId);
		...
		if (this._info.templateInfo.Property2 == 1 && this._info.templateInfo.SonType == GoodsSonType.SONTYPE_MULTI_BOX) {
			this.previewBtn.visible = true;
		...
		if (this._info.templateInfo.SonType == 206 && p3 > 0) {
			...
			let dropArr: t_s_dropitemData[] = TempleteManager.Instance.getDropItemssByDropId(this._info.templateInfo.Property1);
		...
		if (this._info.templateId == TemplateIDConstant.TEMP_ID_VIP_EXP) {
			...
			let cfgItem = TempleteManager.Instance.getConfigInfoByConfigName("grail_level_point");
			...
			let str = LangManager.Instance.GetTranslation("propTips.canUseCount", usedNum, max);
		...
		if (
			this._canOperate &&
			(BagHelper.checkCanUseGoods(this._info.templateInfo.SonType) || this._info.templateInfo.TemplateId == CommonConstant.PET_SOUL_STONE)
		) {
	...
	private onClickPreviewBtn() {
		if (this._previewType === "treasure") {
			...
			UIManager.Instance.ShowWind(EmWindow.PreviewBoxWnd, [this._info, 1, true]);
		...
		switch (obtainId) {
			...
			case 30004: //公会商城
				...
				if (PlayerManager.Instance.currentPlayerModel.playerInfo.consortiaID == 0) {
					...
					if (lv <= 0) {
						MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("shop.view.ShopFrame.command01"));
					} else {
						// FrameCtrlManager.Instance.open(EmWindow.ShopCommWnd, { shopType: ShopGoodsInfo.CONSORTIA_SHOP })
	...
	private usePetPearl() {
		...
		this.petTemplate = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_pettemplate, this._info.templateInfo.Property1);
		if (this.petTemplate) {
			...
			if (arr.length > 0) {
				...
				if (arr.length == 1) {
					...
					if (this.petTemplate.Quality <= petdata.quality && this.petTemplate.Property2 <= petdata.template.Property2) {
						let temp = TempleteManager.Instance.getGoodsTemplatesByTempleteId(TemplateIDConstant.TEMP_ID_PET_COE_STONE);
						if (temp) {
							quality = (this.petTemplate.Quality - 1) * 5 + 1;
							p1 = `[color=${PetData.getQualityColor(this.petTemplate.Quality - 1)}]${this._info.templateInfo.TemplateNameLang}[/color]`;
							p2 = `[color=${temp.profileColor}]${temp.TemplateNameLang + ' x' + this.getDecomposeResult(quality)}[/color]`;
							content = LangManager.Instance.GetTranslation("PetSave.txt8", p1, p2);
							SimpleAlertHelper.Instance.Show(SimpleAlertHelper.SIMPLE_ALERT, [this._info], prompt, content, confirm, cancel, this.confirmCallback.bind(this));
						}
					} else {
						//若英灵珠的阶级和品质任意一项大于当前拥有英灵，则弹出覆盖提示 使用后，英灵名会变化为为英灵名，是否确认？
						quality = Math.max(petdata.quality,this.petTemplate.Quality);//取高品质的英灵的
						p1 = `[color=${PetData.getQualityColor(petdata.quality - 1)}]${petdata.template.TemplateNameLang}[/color]`;
						//取高阶级的英灵
						if(this.petTemplate.Property2 < petdata.template.Property2){
							p2 = `[color=${PetData.getQualityColor(quality - 1)}]${petdata.template.TemplateNameLang}[/color]`;
						}else{
							p2 = `[color=${PetData.getQualityColor(quality - 1)}]${this.petTemplate.TemplateNameLang}[/color]`;
						}
						content = LangManager.Instance.GetTranslation("PetSave.txt9", p1, p2);
						SimpleAlertHelper.Instance.Show(SimpleAlertHelper.SIMPLE_ALERT, [this._info], prompt, content, confirm, cancel, this.confirmCallback.bind(this));
					}
				}
			} else {
	...
	private onBtnUseClick2() {
		if (this._info) {
			...
			if (this._info.templateInfo.SonType == GoodsSonType.SONTYPE_PET_CARD) {
				...
				this.hide();
				return;
			}else if(this._info.templateInfo.SonType == GoodsSonType.SONTYPE_HEADFRAME){
				let goodsData:t_s_itemtemplateData = TempleteManager.Instance.getGoodsTemplatesByTempleteId(this._info.templateId)
				if(HeadIconModel.instance.hasActive(goodsData)){
					MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("propTips.headIcon.hasActiveTips"));
					this.hide();
					return;
				}
				SocketSendManager.Instance.sendUseItem(this._info.pos);
			}
			else if (this._info.templateInfo.SonType == GoodsSonType.SONTYPE_SOUL_CRYSTAL) {
				//灵魂水晶
				if (ArmyManager.Instance.thane.grades < OpenGrades.VEHICEL) {
					MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("dayGuide.DayGuideManager.command02", OpenGrades.VEHICEL));
					this.hide();
					return;
				}
				//跳转到灵魂刻印
			...
			} else if (this._info.templateInfo.SonType == GoodsSonType.SONTYPE_MOUNT_FOOD) {
				//兽灵石
			...
			} else if (this._info.templateInfo.SonType == GoodsSonType.SONTYPE_PET_COST) {
				//英灵凝神珠
			...
			} else if (this._info.templateInfo.TemplateId == CommonConstant.PET_SOUL_STONE) {
				//圣魂石
				if (ArmyManager.Instance.thane.grades < OpenGrades.PET) {
					MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("dayGuide.DayGuideManager.command02", OpenGrades.PET));
					this.hide();
				...
				!GoodsCheck.isGradeFix(ArmyManager.Instance.thane, this._info.templateInfo, false)
			) {
			...
			} else if (this._info.templateInfo.SonType == GoodsSonType.SONTYPE_NOVICE_BOX) {
				//等级宝箱
				SocketSendManager.Instance.sendUseItem(this._info.pos);
				// FrameControllerManager.Instance.openControllerByInfo(UIModuleTypes.NOVICE_GRADE_BOX,null,this._info);
				return;
			} else if (this._info.templateInfo.SonType == GoodsSonType.SONTYPE_APPELL) {
				//称号卡
				SocketSendManager.Instance.sendUseItem(this._info.pos);
				this.hide();
				return;
			...
			} else if (this._info.templateInfo.SonType == GoodsSonType.SONTYPE_SEX_CHANGE_CARD) {
				//使用变性卡
				...
				let str: string = LangManager.Instance.GetTranslation("playerinfo.view.EquipInfoView.WillOpenAt", limitLevel);
				if (userGrade < limitLevel) {
					MessageTipManager.Instance.show(str);
					return;
				}
				UIManager.Instance.ShowWind(EmWindow.SinglePassBugleWnd);
			} else if (this._info.templateInfo.SonType == GoodsSonType.SONTYPE_MULTI_BOX) {
				//多选宝箱
				// UIManager.Instance.ShowWind(EmWindow.MultiBoxSelectWnd, [this._info, 1]);
				UIManager.Instance.ShowWind(EmWindow.PreviewBoxWnd, [this._info, 1, false]);
				this.hide();
				return;
			} else if (this.check()) {
	...
	private checkForUsePetLandTransfer(item: GoodsInfo) {
		...
		if (canUse) {
			...
			let str: string = LangManager.Instance.GetTranslation("checkForUsePetLandTransfer.content02");
			MessageTipManager.Instance.show(str);
		}
	}

	private getPetLandLevel(templateId: number): string {
		let level: string = LangManager.Instance.GetTranslation("checkForUsePetLandTransfer.level01");
		switch (templateId) {
			case 20001:
				level = LangManager.Instance.GetTranslation("checkForUsePetLandTransfer.level01");
				break;
			case 20002:
				level = LangManager.Instance.GetTranslation("checkForUsePetLandTransfer.level02");
				break;
			case 20003:
				level = LangManager.Instance.GetTranslation("checkForUsePetLandTransfer.level03");
				break;
			case 20004:
				level = LangManager.Instance.GetTranslation("checkForUsePetLandTransfer.level04");
				break;
		}
		return level;
	}
	...
	private check(): boolean {
		let str: string = "";
		if (
			this._info.templateInfo.SonType == GoodsSonType.SONTYPE_CONSORTIATIME_CARD &&
			(PlayerManager.Instance.currentPlayerModel.sysCurTimeBySecond - this.playerInfo.lastOutConsortia >= 24 * 3600 || !this.playerInfo.isAuto)
		) {
			//盟约之证
		...
		if (
			...
			this.playerInfo.composeList.indexOf(this._info.templateInfo.Property1) >= 0
		) {
