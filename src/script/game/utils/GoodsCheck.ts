...
export class GoodsCheck {
    ...
    public static checkGoodsByHero(data: GoodsInfo, hero: ThaneInfo, popMsg: boolean = true): boolean {
        if (!data) {
            return false;
        }
    ...
    public static checkGoodsCanEquip(data: GoodsInfo, hero: ThaneInfo, popMsg: boolean = false): boolean {
        if (!data) {
            return false;
        }
        ...
        if (data.templateInfo.MasterType != GoodsType.EQUIP && data.templateInfo.SonType != GoodsSonType.SONTYPE_RUNNES && data.templateInfo.MasterType != GoodsType.HONER) {
            return false;
        }
        if (!this.isJobFix(hero, data.templateInfo, popMsg)) {
            return false;
    ...
    public static isUpdateEquip(info: t_s_itemtemplateData): boolean {
        if (info.MasterType == GoodsType.EQUIP && info.SuiteId != 0 && info.NeedGrades >= 40)//为可进阶的装备
        {
            return true;
        }
        return false;
    ...
    public static isJobFix(hero: ThaneInfo, info: t_s_itemtemplateData, popMsg: boolean): boolean {
        ...
        if (popMsg) {
            MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("bag.helper.GoodsCheck.command01"));
        }
        return false;
    }

    public static isSexFix(hero: ThaneInfo, info: t_s_itemtemplateData, popMsg: boolean): boolean {
        if (info.Sexs == 2 || hero.templateInfo.Sexs == info.Sexs) {
            return true;
        }
        if (popMsg) {
            MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("bag.helper.GoodsCheck.command02"));
        }
        return false;
    }

    public static isGradeFix(thane: ThaneInfo, info: t_s_itemtemplateData, popMsg: boolean): boolean {
        if (info.NeedGrades == 0 || thane.grades >= info.NeedGrades) {
            return true;
        }
        if (popMsg) {
            MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("bag.helper.GoodsCheck.command03", info.NeedGrades));
        }
        return false;
    }

    public static checkGoodsCanEquipNotCheckGrade(data: GoodsInfo, hero: ThaneInfo, popMsg: boolean = false): boolean {
        if (!data) {
            return false;
        }
        if (data.templateInfo.MasterType != GoodsType.EQUIP && data.templateInfo.SonType != GoodsSonType.SONTYPE_RUNNES && data.templateInfo.MasterType != GoodsType.HONER) {
    ...
    public static checkGoodsBetterNotCheckGrade(goods: GoodsInfo): boolean {
        if (!this.checkGoodsCanEquipNotCheckGrade(goods, this.thane) || goods.templateInfo.SonType == GoodsSonType.SONTYPE_RUNNES) {
            return false;
        }
        let heroBag: any[] = GoodsManager.Instance.getHeroGoodsListBySonTypeAndId(goods.templateInfo.SonType, this.thane.id).getList();
    ...
    public static checkGoodsBetterThanHero(goods: GoodsInfo): boolean {
        if (!this.checkGoodsCanEquip(goods, this.thane) || goods.templateInfo.SonType == GoodsSonType.SONTYPE_RUNNES) {
            return false;
        }
    ...
    public static getIntensifuGold(info: GoodsInfo): number {
        ...
        // }
        return Math.floor(Math.pow(1.5, Math.floor(g / 10) + 1) * (p + 1) * Math.pow((s + 1), 2.2) * 5 + 500);
    }
    ...
    public static getSellGold(info: GoodsInfo): number {
        ...
        for (let i: number = 1; i <= s; i++) {
            count += Math.floor(Math.pow(1.5, Math.floor(g / 10) + 1) * (p + 1) * Math.pow(i, 2.2) * 5 + 500);
        }
        return Math.floor(info.templateInfo.SellGold + 0.7 * count);
    }

    public static isAddToChecker(beginData: GoodsInfo, endData: GoodsInfo): boolean {
        if (((beginData.isBinds == false && endData.isBinds == true) || (beginData.isBinds == true && endData.isBinds == false))       //绑定状态不同
            && beginData.templateInfo.SonType == endData.templateInfo.SonType                                                          //可叠加物品
            && beginData.templateId == endData.templateId                                                                             //模版ID一致
            && endData.count < endData.templateInfo.MaxCount)                                                                          //目标物品未满
        {
            return true;
        }
        return false;
    ...
    public static isKingCard(goods: GoodsInfo): boolean {
        return (goods.templateInfo.SonType == GoodsSonType.SONTYPE_USEABLE && goods.templateInfo.Property1 == 13)
    }
    ...
    public static hasIdentify(gInfo: GoodsInfo): boolean {
        ...
        if(gInfo.randomSkill1 || gInfo.randomSkill2 || gInfo.randomSkill3
            ||gInfo.randomSkill1 || gInfo.randomSkill2 || gInfo.randomSkill3){
            flag = true;
        }
        return flag;
    }
