  ...
  public static createAction(
    ...
    switch ($action) {
      case MovieType.DISAPPEARD:
        CampaignMovieQueueManager.Instance.addAction(
          new CommonAction($target, time),
        );
        break;
      case MovieType.TRANSPORT_START:
        SceneManager.Instance.enable = false;
        NotificationManager.Instance.dispatchEvent(
          JoyStickEvent.JoystickTriggerUp,
        );
        CampaignMovieQueueManager.Instance.addAction(
          new TranseferAction($target, $target.x, $target.y, $action),
        );
        break;
      case MovieType.TRANSPORT_END:
        CampaignMovieQueueManager.Instance.addAction(
          new TranseferEndAction(pkg, $action),
        );
        break;
      case MovieType.FLICKER:
        CampaignMovieQueueManager.Instance.addAction(
          new AlphaFlickerAction($target),
        );
        break;
      case MovieType.TRAP:
