...
export default class WearySupplyWnd extends BaseWindow {
  ...
  private __bagItemUpdate(good_infos: GoodsInfo[]) {
    for (let good_info of good_infos)
      if (
        good_info.templateId == ItemID.WEARY_MEDICINE0 ||
        good_info.templateId == ItemID.WEARY_MEDICINE1 ||
        good_info.templateId == ItemID.WEARY_MEDICINE2 ||
        good_info.templateId == ItemID.WEARY_MEDICINE3
      ) {
        this.refresh();
        break;
      }
