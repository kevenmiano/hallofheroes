...
export class DamageNumView extends Laya.Sprite {
  ...
  private createTextView() {
    ...
    if (this._type == BloodType.BLOOD_TYPE_ARMY) {
      tempStyle =
        this._n > 0
          ? DamageNumView.BattleNumberStyle.ShieldAdd
          : DamageNumView.BattleNumberStyle.ShieldSub;
      ...
      textList.push(
        DamageNumView.createText(
          LangManager.Instance.GetTranslation(
            this._n > 0 ? "DamageNum.ShieldAdd" : "DamageNum.ShieldSub",
          ),
          this._n > 0 ? 36 : 30,
          tempStyle.color,
          tempStyle.bold,
      ...
      if (this._isParry)
        textList.push(
          DamageNumView.createText(
            LangManager.Instance.GetTranslation("DamageNum.Parry"),
            30,
            parryStyle.color,
            tempStyle.bold,
            tempStyle.italic,
            parryStyle.stroke,
            parryStyle.strokeColor,
          ),
        );
      ...
      if (this._isCitical && !this._isParry)
        textList.push(
          DamageNumView.createText(
            LangManager.Instance.GetTranslation("DamageNum.Crit"),
            30,
            cirtStyle.color,
            tempStyle.bold,
            tempStyle.italic,
            cirtStyle.stroke,
            cirtStyle.strokeColor,
          ),
        );
      return textList;
    }
    ...
    if (this._isCitical) {
      ...
      if (this._isParry) {
        ...
        tempStyle = cirtStyle;
        let dlb = DamageNumView.createText(
          showTxt,
          tempStyle.size,
          tempStyle.color,
          tempStyle.bold,
        ...
        // dlb.width += 4;
        textList.push(dlb);
        let bjb = DamageNumView.createText(
          LangManager.Instance.GetTranslation("DamageNum.Crit"),
          76,
          tempStyle.color,
          tempStyle.bold,
