...
export default class OfferRewardWnd extends BaseWindow {
  ...
  public OnInitWind() {
    ...
    this.DailyTitleTxt.text = LangManager.Instance.GetTranslation(
      "buildings.offerreward.view.OfferCountView.dailyCount",
    );
    this.LeftRefreshTimeTxt.text = LangManager.Instance.GetTranslation(
      "OfferRewardWnd.LeftRefreshTimeTxt",
    );
    ...
    if (this.rewardModel.offerRewardTaskDic) {
      this.itemList.numItems =
        this.rewardModel.offerRewardTaskDic.getList().length;
    }
  }
  ...
  private addEvent() {
    ...
    this.rewardModel.addEventListener(
      Laya.Event.COMPLETE,
      this.__completeHandler,
      this,
    );
    this.rewardModel.addEventListener(Laya.Event.CHANGE, this.updateView, this);
    this.rewardManager.addEventListener(
      RewardEvent.OFFER_REWARD_LIST_REFRESH,
      this.__offerRewardListRefreshHandler,
      this,
    );
    this.rewardModel.addEventListener(
      RewardEvent.NEED_RESET_DATA,
      this.__resetDataHandler,
      this,
    );
    NotificationManager.Instance.addEventListener(
      KingContractEvents.UPDATE_KINGCONTRACT,
      this.__kingcontractHandler,
      this,
    );
  ...
  private removeEvent() {
    ...
    this.rewardModel.removeEventListener(
      Laya.Event.COMPLETE,
      this.__completeHandler,
      this,
    );
    ...
    this.rewardManager.removeEventListener(
      RewardEvent.OFFER_REWARD_LIST_REFRESH,
      this.__offerRewardListRefreshHandler,
      this,
    );
  ...
  private renderItemListHandler(index: number, item: OfferRewardItem) {
    ...
    if (index < this.rewardModel.offerRewardTaskDic.getList().length) {
      item.setData(
        this.rewardModel.offerRewardTaskDic[index],
        this.rewardModel.offerRewardTaskProfileDic[index],
        this.rewardModel.offerRewardTaskStateDic[index],
      );
    }
  }
  ...
  private refreshItemListView() {
    if (
      this.rewardModel.offerRewardTaskDic &&
      this.rewardModel.offerRewardTaskStateDic &&
      this.rewardModel.offerRewardTaskProfileDic
    ) {
      for (var i: number = 0; i < this._offerTaskList.length; i++) {
        this._offerTaskList[i].setData(
          this.rewardModel.offerRewardTaskDic[i],
          this.rewardModel.offerRewardTaskProfileDic[i],
          this.rewardModel.offerRewardTaskStateDic[i],
        );
      }
    }
  }
  ...
  private updateView() {
    this.TaskUpdateTimeTxt.text = DateFormatter.getCountDate(
      this.rewardModel.remainTime,
    );
  }
  ...
  private helpClick() {
    ...
    let content: string = LangManager.Instance.GetTranslation(
      "buildings.offerreward.OfferRewardHelpFrame.helpContent",
    );
  ...
  private setCoinBtnvisible() {
    ...
    if (
      itemtemplateData &&
      itemtemplateData.NeedGrades > ArmyManager.Instance.thane.grades
    ) {
  ...
  private refreshViewCoinCountTxt() {
    this._coinCount = GoodsManager.Instance.getBagCountByTempId(
      BagType.Player,
      this.coinTemplateId,
    );
  ...
  private coinRefreshHandler() {
    ...
    if (this.rewardModel.remainRewardCount <= 0) {
      MessageTipManager.Instance.show(
        LangManager.Instance.GetTranslation("offerReward.count"),
      );
      return;
    }
  ...
  private __onQuickRefreshClick() {
    ...
    if (OfferRewardManager.Instance.model.hasOrangeTaskNotComplete) {
      var content: string = LangManager.Instance.GetTranslation(
        "buildings.offerreward.view.OfferTimeView.content2",
      );
      var confirm: string =
      ...
      SimpleAlertHelper.Instance.Show(
        ...
        confirm,
        cancel,
        this.quickRefreshCallBack.bind(this),
      );
    } else {
      this.quickRefresh();
    }
  ...
  private quickRefresh() {
    ...
    this.refreshRewardListPayCall(
      true,
      SharedManager.Instance.offerRewardRefreshUseBind,
    );
  }
  ...
  private quickRefreshAlert(callBack: Function = null): boolean {
    var preDate: Date = new Date(
      SharedManager.Instance.offerRewardRefreshCheckDate,
    );
    ...
    if (outdate) {
      let cfgValue = 2;
      let cfgItem =
        TempleteManager.Instance.getConfigInfoByConfigName("FreshReward_Price");
      if (cfgItem) {
        cfgValue = Number(cfgItem.ConfigValue);
      }
      var content: string = LangManager.Instance.GetTranslation(
        "auction.view.mysteryshop.BuyTimeView.quickRefreshTip",
        cfgValue,
      );
      UIManager.Instance.ShowWind(EmWindow.TodayNotAlert, {
        content: content,
        backFunction: this.startRefreshAlertBack.bind(this),
        closeFunction: null,
        point: OfferRewardModel.REFRESH_REWARD_NEEDPAY,
      });
    }
    return outdate;
  }
  ...
  private __onFreeRefreshClick() {
    ...
    if (OfferRewardManager.Instance.model.hasOrangeTaskNotComplete) {
      ...
      SimpleAlertHelper.Instance.Show(
        ...
        confirm,
        cancel,
        this.confirmRefreshTaskListCallback.bind(this),
      );
    } else {
      OfferRewardManager.Instance.sendRefreshRewardList(
        true,
        SharedManager.Instance.offerRewardRefreshUseBind,
      );
    }
  ...
  private refreshView() {
    ...
    this.Btn_freeRefresh.enabled = this.Btn_quickRefresh.enabled =
      this.rewardModel.hasData;
    this.Btn_freeRefresh.visible = this.Btn_quickRefresh.visible = false;
    this.DailyCountTxt.text =
      this.rewardModel.remainRewardCount +
      " / " +
      this.rewardModel.maxRewardCount;
    if (this.rewardModel.freeRefreshCount >= 2) {
      this.Btn_quickRefresh.visible = true;
    } else {
      this.Btn_freeRefresh.visible = true;
      this.Btn_freeRefresh.title =
        LangManager.Instance.GetTranslation("offerrewardwnd.freeRefreshTxt") +
        LangManager.Instance.GetTranslation(
          "public.parentheses1",
          2 - this.rewardModel.freeRefreshCount,
        );
    }
  }
  ...
  public getOfferTaskItemByIndex(idx: number): OfferRewardItem {
    for (let index = 0; index < this.itemList.numItems; index++) {
      if (idx == index)
        return this.itemList.getChildAt(index) as OfferRewardItem;
    }
  }
