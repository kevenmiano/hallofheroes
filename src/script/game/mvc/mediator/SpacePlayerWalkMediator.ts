...
export class SpacePlayerWalkMediator implements IMediator, IEnterFrame {
	...
	public register(target: any) {
		...
		this._target.aiInfo.addEventListener(ObjectsEvent.WALK_OVER, this.__walkOverHandler.bind(this));
		this._target.aiInfo.addEventListener(ObjectsEvent.WALK_NEXT, this.__walkNextHandler.bind(this));
		this._target.aiInfo.addEventListener(AiEvents.UPDATE_PATHS, this.__updatePathHandler.bind(this));
		this._sendPosRate = 7;
		this._walkPath = [];
	...
	public unregister(target: any) {
		if (this._target && this._target.aiInfo) {
			this._target.aiInfo.removeEventListener(AiEvents.UPDATE_PATHS, this.__updatePathHandler.bind(this));
			this._target.aiInfo.removeEventListener(ObjectsEvent.WALK_OVER, this.__walkOverHandler.bind(this));
			this._target.aiInfo.removeEventListener(ObjectsEvent.WALK_NEXT, this.__walkNextHandler.bind(this));
		}
	...
	public enterFrame() {
		...
		var nodeInfo: SpaceNode = this._model.getMapNodeById(this._model.checkNodeId);
		...
		if (nodeInfo && selfView) {
			var star: Laya.Point = new Laya.Point(parseInt((selfView.x / 20).toString()), parseInt((selfView.y / 20).toString()));
			var end: Laya.Point = new Laya.Point(nodeInfo.posX, nodeInfo.posY);
			var leng: number = star.distance(end.x, end.y);
			if (Math.floor(leng) > nodeInfo.handlerRange * 2) {//为了解决"玩家在天空之城移动到特定位置与英灵竞技npc对话, 对话框打开后会自己关闭"
