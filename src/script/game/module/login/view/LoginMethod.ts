...
export class LoginMethod extends FUI_LoginMethod {
  ...
  onInit(func?: Func) {
    ...
    if (this.isIos) {
      this.listFullKey.set(0, [
        [LoginWay.Type_7ROAD, LoginWay.Type_FB],
        [LoginWay.Type_Apple, LoginWay.Type_GG, LoginWay.Type_GUEST],
      ]);
    } else {
      this.listFullKey.set(0, [
        [LoginWay.Type_7ROAD, LoginWay.Type_GG],
        [LoginWay.Type_FB, LoginWay.Type_GUEST],
      ]);
    }
    this.listFullKey.set(1, [[LoginWay.Type_7ROAD]]);
    this.offEvent();
    this.addEvent();
  }
  ...
  offEvent() {
    this.list1 &&
      this.list1.off(fairygui.Events.CLICK_ITEM, this, this._onSelectedItem);
    this.list2 &&
      this.list2.off(fairygui.Events.CLICK_ITEM, this, this._onSelectedItem);
  ...
  private renderListItem1(index: number, item: LoginMethodItem) {
    if (item && !item.isDisposed) {
      ...
      item.titleName = LangManager.Instance.GetTranslation(
        this.listTileString + item.key,
      );
    }
  }
