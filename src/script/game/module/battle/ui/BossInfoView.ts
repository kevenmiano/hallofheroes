...
export class BossInfoView extends FUI_BossInfoView {
  ...
  public get stripNum(): number {
    return this._stripNum;
  }
  ...
  onConstruct() {
    ...
    if (this.battleModel) {
      this.showTrail(
        this.battleModel.isTrail() ? this.battleModel.trialLayer : 0,
      );
    }
  ...
  private addEvent() {
    ...
    if (this.battleModel) {
      this.battleModel.addEventListener(
        BattleEvent.TRAIL_LAYER_CHANGE,
        this.__layerChangeHandler.bind(this),
        this,
      );
    }
  }
  ...
  protected initView() {
    ...
    let template: t_s_herotemplateData = ConfigMgr.Instance.getTemplateByID(
      ConfigType.t_s_herotemplate,
      this._info.heroInfo.templateId.toString(),
    );
    if (!template) {
      return;
    }
  ...
  public initBossHead(info?: HeroRoleInfo) {
    ...
    if (this._info.playerIconId == 0) {
      //如果是非玩家,则使用英雄的icon
      this._headUrl = IconFactory.getHeroIconByPics(
        this._info.heroInfo.templateInfo.Icon,
      );
    } else {
      //使用领主的icon
      this._headUrl = IconFactory.getHeroIconByPics(
  ...
  private initBossHp() {
    this._stripNum = BattleModel.getBossHpStripNum(
      this._info.heroInfo.templateId,
    );

    Logger.battle(
      "[BossInfoView]initBossHp",
      this._info.heroInfo.templateId,
      this._stripNum,
    );
