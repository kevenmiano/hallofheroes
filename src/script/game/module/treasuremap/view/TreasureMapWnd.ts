...
import { BagEvent, TreasureMapEvent } from "../../../constant/event/NotificationEvent";
...
export default class TreasureMapWnd extends BaseWindow {
    ...
    public OnInitWind() {
        ...
        this._digPointsCost = Number(TempleteManager.Instance.getConfigInfoByConfigName('treasure_imm_battle').ConfigValue);
        this.analyzeTreasureMap();
        this.img_pos.setXY(this._flagPosX-this.img_pos.width/2,this._flagPosY-this.img_pos.height/2);
    ...
    private addEvent():void{
        ...
        GoodsManager.Instance.addEventListener(BagEvent.DELETE_TREASURE_MAP,	this.__deleteTreasureMapHandler,this);
    ...
    private removeEvent():void{
        ...
        GoodsManager.Instance.removeEventListener(BagEvent.DELETE_TREASURE_MAP,	this.__deleteTreasureMapHandler,this);
    ...
    private onFight():void{
        if(this.checkScene()){
            ...
            let content: string = LangManager.Instance.GetTranslation("map.internals.view.treasuremap.TreasureMapUseFrame.digBtnAlertTxt3",this._digPointsCost,4);
            SimpleAlertHelper.Instance.Show(SimpleAlertHelper.USEBINDPOINT_ALERT, null, prompt, content, confirm, cancel, this.digBtnAlertCallBack.bind(this));
        }else{
			MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("map.internals.view.treasuremap.TreasureMapUseFrame.digBtnAlertTxt2"));
        }
    }
    ...
    private digBtnAlertCallBack(b:boolean,useBind:boolean):void
    {
        if(b){
            if(this.frameData)
            {
                ...
                {
                    TreasureMapSocketOutManager.sendDigTreasureMap(this.frameData.pos, useBind);
                }else
                {
    ...
    private onCheckMap():void{
        ...
        { 
            MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("map.internals.view.treasuremap.TreasureMapUseFrame.CheckMapTipTxt"));
            return;
        }
        ...
        else if(SceneManager.Instance.currentType == SceneType.CAMPAIGN_MAP_SCENE && WorldBossHelper.checkPetLand(CampaignManager.Instance.mapId))
        {
    ...
    {
        if(SceneManager.Instance.currentType == SceneType.CAMPAIGN_MAP_SCENE && WorldBossHelper.checkPetLand(CampaignManager.Instance.mapId))
        {
            return true;
        }
        return false;
    ...
    {
        if(SceneManager.Instance.currentType == SceneType.CAMPAIGN_MAP_SCENE && WorldBossHelper.checkMineral(CampaignManager.Instance.mapId))
        {
            return true;
        }
        return false;
    ...
    private analyzeTreasureMap():void
    {
        ...
        let mapTemplate:t_s_mapData = TempleteManager.Instance.getMapTemplateDataByID(goodsInfo.randomSkill1);
        ...
        }else if(tileWidth < goodsInfo.randomSkill2 && goodsInfo.randomSkill2 < tileWidth * 2)
        {
            posX = 2;
            flagPosX = goodsInfo.randomSkill2 - tileWidth;
        }else if(tileWidth * 2 < goodsInfo.randomSkill2 && goodsInfo.randomSkill2 < mapTemplate.Width)
        {
            posX = 3;
        ...
        }else if(tileHeight < goodsInfo.randomSkill3 && goodsInfo.randomSkill3 < tileHeight * 2)
        {
            posY = 2;
            flagPosY = goodsInfo.randomSkill3 - tileHeight;
        }
        else if(tileHeight * 2 < goodsInfo.randomSkill3 && goodsInfo.randomSkill3 < mapTemplate.Height)
        {
            posY = 3;
        ...
        this.loader.url = 'res/animation/images/treasure/'+goodsInfo.randomSkill1.toString() +'/0'+tileIndex+'.png';
    }
