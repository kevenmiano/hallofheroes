...
import { ExtraJobEvent, InteractiveEvent } from "../../../constant/event/NotificationEvent";
...
export class MasteryInlayItem extends FUI_MasteryInlayItem {
    ...
    private __clickHandler(c: Event) {
        this.selected = true;
        NotificationManager.Instance.dispatchEvent(ExtraJobEvent.SELECT_HOLE,this.index);
    }
    ...
    public set info(value: ExtraJobEquipItemInfo) {
        ...
        if (value) {
            ...
            } else if (id == 0) { //已打孔, 未装备 Icon_IconBox_Add
                this.item.info = null;
            } else if (id == -1) { //未打孔 Icon_IconBox70_Lock2   
                this.needLevel =  ExtraJobModel.instance.equipGemOpenArr[this.index-1];
                if(value.equipLevel < this.needLevel){//未解锁：显示红色文本提示“魂器xx阶开启”
                    this.bNotOpen = true;
                    this.rTxtDesc.color = "#FF2e2e"
                    this.rTxtDesc.text = LangManager.Instance.GetTranslation('Mastery.openHoleLevel',this.needLevel);           
                }else{
                    //未镶嵌：显示空格子
                    this.rTxtDesc.text ='';
                    this.bNotOpen = false;
