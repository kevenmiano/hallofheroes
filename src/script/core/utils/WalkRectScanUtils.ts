...
export class WalkRectScanUtils {
    ...
    for (let i: number = -count; i <= count; i++) {
      ...
      if (i == -count || i == count) {
        for (let j: number = -count; j <= count; j++) {
          ...
          if (
            this.outRect(
              parseInt((temp.x / Tiles.WIDTH).toString()),
              parseInt((temp.y / Tiles.HEIGHT).toString()),
            )
          )
            arr.push(new Laya.Point(temp.x, temp.y));
        }
      } else {
        temp.y = p.y;
  ...
  private getMinDistance(arr: Laya.Point[], start: Laya.Point): Laya.Point {
    ...
    arr.forEach((element) => {
      if (
        point.distance(start.x, start.y) > start.distance(element.x, element.y)
      ) {
        point = element;
      }
    });
