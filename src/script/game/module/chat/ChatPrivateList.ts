...
import { ChatEvent, FriendUpdateEvent } from "../../constant/event/NotificationEvent";
...
export default class ChatPrivateList extends FUI_PriviatePlayerList {
	...
	private initView() {
		...
		if (this.privateDataList.length > 0) {
			...
			ChatManager.Instance.model.privateData = (this.list.getChildAt(this.list.selectedIndex) as ChatPrivatePlayerCell).info;
		}
	}
	...
	private addEvent() {
		...
		this.list.on(fairygui.Events.CLICK_ITEM, this, this.__onPrivatePlayerSelect);
		NotificationManager.Instance.addEventListener(ChatEvent.ADD_PRIVATE_CHAT, this._selectTarget, this);
		NotificationManager.Instance.addEventListener(ChatEvent.CHAT_MESSAGE, this.__onChangeSelect, this);
		NotificationManager.Instance.addEventListener(ChatEvent.REMOVE_PRIVATE_CHAT, this.removePlayer, this);
		NotificationManager.Instance.addEventListener(ChatEvent.ADD_CHAT, this.onlineStatus, this);
		FriendManager.getInstance().addEventListener(FriendUpdateEvent.RECENT_CONTACT_UPDATE, this.updateView, this);
		FriendManager.getInstance().addEventListener(FriendUpdateEvent.ADD_RECENT_CONTACT, this.updateView, this);
		FriendManager.getInstance().addEventListener(FriendUpdateEvent.FRIEND_CHANGE, this.onFriendStatusChange, this);
	}
	...
	private offEvent() {
		...
		this.list.off(fairygui.Events.CLICK_ITEM, this, this.__onPrivatePlayerSelect);
		NotificationManager.Instance.removeEventListener(ChatEvent.ADD_PRIVATE_CHAT, this._selectTarget, this);
		NotificationManager.Instance.removeEventListener(ChatEvent.CHAT_MESSAGE, this.__onChangeSelect, this);
		NotificationManager.Instance.removeEventListener(ChatEvent.REMOVE_PRIVATE_CHAT, this.removePlayer, this);
		NotificationManager.Instance.removeEventListener(ChatEvent.ADD_CHAT, this.onlineStatus, this);
		FriendManager.getInstance().removeEventListener(FriendUpdateEvent.RECENT_CONTACT_UPDATE, this.updateView, this);
		FriendManager.getInstance().removeEventListener(FriendUpdateEvent.ADD_RECENT_CONTACT, this.updateView, this);
		FriendManager.getInstance().removeEventListener(FriendUpdateEvent.FRIEND_CHANGE, this.onFriendStatusChange, this);
	}
	...
	private onSelect(data) {
		...
		IMManager.Instance.cleanMsgListByIdAndType(data.userId, IMModel.MSG_DIC_UNREAD);
		if (this.privateDataList && !this.checkInList(data.userId)) this.privateDataList.push(data);
		this.requestOnlineStatus();
	}
	...
	private refreshData() {
		...
		if (this.privateDataList.length > 0) {
			...
			let cell = this.list.getChildAt(this.list.selectedIndex) as ChatPrivatePlayerCell;
			ChatManager.Instance.model.privateData = cell.info;
			this._currentSelected = cell.info;
			IMManager.Instance.cleanMsgListByIdAndType(cell.info.userId, IMModel.MSG_DIC_UNREAD);
		} else {
	...
	onAddPrivaiteChat() {
		...
		if (!UIManager.Instance.isShowing(EmWindow.AddFriendWnd)) {
			UIManager.Instance.ShowWind(EmWindow.AddFriendWnd, { callback: this.onConfirmAddFriend.bind(this), index: 0 });
		}
	}
	...
	private onConfirmAddFriend(selectTarget: any) {
		if (selectTarget) {
			...
			NotificationManager.Instance.sendNotification(ChatEvent.ADD_PRIVATE_CHAT, selectTarget);
		}
	}
