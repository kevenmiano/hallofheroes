...
export default class MazeRiverWnd extends BaseWindow {
  ...
  public OnInitWind() {
    this.descTxt.text = LangManager.Instance.GetTranslation(
      "map.campaign.view.ui.MazeRiverView.txt",
    );
  ...
  riverBtnClick() {
    if (this._cost == 0) {
      ...
      let cfgValue = 5;
      let cfgItem =
        TempleteManager.Instance.getConfigInfoByConfigName("Tower_Live");
      if (cfgItem) {
      ...
      var content: string = LangManager.Instance.GetTranslation(
        "map.campaign.view.ui.MazeRiverView.content01",
        cfgValue,
      );
      SimpleAlertHelper.Instance.Show(
        SimpleAlertHelper.USEBINDPOINT_ALERT,
        { point: this._cost, checkDefault: true },
        prompt,
        content,
        confirm,
        cancel,
        this.__riverCall.bind(this),
      );
    }
  }
