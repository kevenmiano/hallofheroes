 ...
 * @description ***
 */
export default class SysMsgCell
  extends FUI_SystemMsgCell
  implements ITipedDisplay
{
  ...
  set chatData(value) {
    if (value) {
      ...
      // }
      let _cellVector = [];
      _cellVector = _cellVector.concat(
        ChatFormat.getContentCellData(this._cellData),
      );
      ...
      this.message.text =
        "[color=" +
        ChatChannel.getChatChannelColor(value.channel) +
        "]" +
        elementText +
        "[/color]";
    }
    ...
    switch (
      ...
      case ChatCellType.STAR:
        ...
        let startemp: t_s_startemplateData =
          TempleteManager.Instance.getStarTemplateById(Number(jsonData.id));
