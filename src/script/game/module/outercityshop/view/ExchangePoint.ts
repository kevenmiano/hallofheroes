...
export default class ExchangePoint extends FUI_ExchangePoint {
  ...
  private addEvent() {
    ...
    this.outerCityShopModel.addEventListener(
      OuterCityShopEvent.EXCHANGE_GOODS_LIST_UPDATE,
  ...
  private refreshView() {
    this.txt_hasPoint.text = GoodsManager.Instance.getGoodsNumByTempId(
      ShopGoodsInfo.MYSTERY_STONE,
    ).toString(); //神秘石
    this.txt_hasBindPoint.text =
      this.outerCityShopModel.consumePoints.toString();
    this.outerCityShopModel.initExchangeGoodsData();
  }
  ...
  private refreshViewHandler() {
    let exchangeGoodsList: Array<any> =
      this.outerCityShopModel.exchangeGoodsList;
