...
export default class CampaignResultWnd extends BaseWindow {
  ...
  private initData() {
    ...
    //评价等级 (S A B C D)
    this.imgGrade.icon = fgui.UIPackage.getItemURL(
      EmWindow.CampaignResult,
      CampaignResultData.EvaluateRes[this._msg.totalAppraisal - 1],
    );
    this.imgGradeBg.icon = fgui.UIPackage.getItemURL(
      EmWindow.CampaignResult,
      CampaignResultData.EvaluateBgRes[this._msg.totalAppraisal - 1],
    );
    ...
    if (this.playerInfo.enterWarPet) {
      this.txtPetExp.text =
        "+" + String(this._msg.petGp > 0 ? this._msg.petGp : 0); //英灵经验
      this.imgPetHead.icon = IconFactory.getPetHeadSmallIcon(
    ...
    let gradeArr = [
      {
        name: LangManager.Instance.GetTranslation(
          "CampaignResult.KillBossTime",
        ),
        value: DateFormatter.getFullTimeString(this._msg.endBossTime),
        evaluate: this._msg.endBossAppraisal,
      },
      {
        name: LangManager.Instance.GetTranslation("CampaignResult.DieCnt"),
        value: this._msg.failedCount,
        evaluate: this._msg.failedAppraisal,
      },
    ];
    ...
    let expArr = [
      {
        name: LangManager.Instance.GetTranslation("CampaignResult.Team"),
        value: this._msg.teamJoin,
      },
      // { name: LangManager.Instance.GetTranslation("CampaignResult.Friend"), value: this._msg.friendJoin },
      {
        name: LangManager.Instance.GetTranslation("CampaignResult.Prop"),
        value: this._msg.propJoin,
      },
      {
        name: LangManager.Instance.GetTranslation("CampaignResult.Other"),
        value: this._msg.otherJoin,
      },
    ];
  ...
  private autoClickToNextHandler() {
    ...
    this.rTxtAutoCloseTip.text = LangManager.Instance.GetTranslation(
      "CampaignResult.AutoCloseTip",
      time,
    );
    this.timeCountDown++;
  }
