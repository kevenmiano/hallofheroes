...
export default class MountRefiningWnd extends BaseWindow {
  ...
  OnShowWind() {
    ...
    this.descTxt1.text = LangManager.Instance.GetTranslation(
      "MountRefiningWnd.descTxt1",
    );
    this.descTxt2.text = LangManager.Instance.GetTranslation(
      "MountRefiningWnd.descTxt2",
    );
    this.descTxt3.text = LangManager.Instance.GetTranslation(
      "MountRefiningWnd.descTxt3",
    );
    this.maxGradeTxt.text = LangManager.Instance.GetTranslation(
      "MountRefiningWnd.maxGradeTxt",
    );
    this._arr = TempleteManager.Instance.getTemplatesByType(
      UpgradeType.MOUNT_REFINING,
    );
  ...
  private addEvent() {
    ...
    this.mountInfo.addEventListener(
      MountsEvent.MOUNT_STAR_UP,
      this.mountStarInfoUpdate,
      this,
    );
  }
  ...
  private trainBtnHandler() {
    if (this._type == 3) {
      MessageTipManager.Instance.show(
        LangManager.Instance.GetTranslation("MountRefiningWnd.train.tips"),
      );
      return;
    }
    if (this._vData) {
  ...
  private helpBtnClick() {
    let title: string = LangManager.Instance.GetTranslation(
      "mountRefiningWnd.help.title",
    );
    let content: string = LangManager.Instance.GetTranslation(
      "mountRefiningWnd.help.helpContent",
    );
  ...
  private refreshView() {
    ...
    if (info) {
      ...
      this.img_star1.fillAmount = this.img_star2.fillAmount =
        info.starLevel / 5;
      this._currentStarLevel = info.starLevel;
      let goodsInfo: GoodsInfo = new GoodsInfo();
      this._totalCoinCount = GoodsManager.Instance.getBagCountByTempId(
        BagType.Player,
        this._costTemplateId,
      );
      this._totalMountCardCount = GoodsManager.Instance.getGoodsNumByTempId(
        this._vData.StarItem,
      );
      ...
      if (this._totalMountCardCount >= this._needCardCount) {
        //坐骑卡数量足够
        this.countTxt.text =
          this._totalMountCardCount.toString() + "/" + this._needCardCount;
        goodsInfo.templateId = this._vData.StarItem;
        this.baseItem.info = goodsInfo;
        this.countTxt.color = ColorConstant.LIGHT_TEXT_COLOR;
        this._type = 1;
        if (dot) dot.visible = false;
      } else if (this._totalCoinCount >= soulMonsterCount) {
        //道具数量足够
        this.countTxt.text =
          this._totalCoinCount.toString() + "/" + soulMonsterCount;
      ...
      if (info.starLevel >= MountsManager.Instance.maxStarGrade) {
        //达到最大等级
        ...
        this.progressTxt.text =
          info.blessing + "/" + this.getCurrentTotalBlessValue(info.starLevel);
      }
      this.progress.value =
        (info.blessing * 100) / this.getCurrentTotalBlessValue(info.starLevel);
    }
