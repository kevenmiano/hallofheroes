...
export default class SinglePassRewardItem extends FUI_SinglePassRewardItem implements ITipedDisplay {
    extData: any;
    tipData: any;
    tipType: EmWindow;
    ...
    public refreshRegister(value: boolean) {
        let singlePassAreaRewardData: SinglePassAreaRewardData = TempleteManager.Instance.getAreaRewardByAreaId(this.singlePassModel.selectArea);
    ...
    private rewardBtnHandler() {
        ToolTipsManager.Instance.unRegister(this);
        if (this.singlePassModel.areaRewardArray && this.singlePassModel.areaRewardArray.indexOf(this.singlePassModel.selectArea.toString()) != -1) {
            //已经领取点击给提示
            MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("mainBar.TopToolsBar.rechargeBtn.click02"));
            return;
        }
        else {
            if (this.singlePassModel.selectArea <= this.singlePassModel.area && this.singlePassModel.maxIndex >= this.singlePassModel.selectArea * SinglePassModel.TOLLGATE_PER_FLOOR) {
                SinglePassSocketOutManager.sendRequestGetAreaReward(this.singlePassModel.selectArea);
            }
            else {
                ToolTipsManager.Instance.register(this);
            }
