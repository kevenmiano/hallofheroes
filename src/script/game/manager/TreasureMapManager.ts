...
export default class TreasureMapManager {
  ...
  public setup() {
    ServerDataManager.listen(
      S2CProtocol.U_C_TREASUREMAP_INFO,
      this,
      this.__treasureMapInfoHandler,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_TREASUREMAP_REWARD,
      this,
      this.__treasureMapRewardHandler,
    );
  }

  private __treasureMapInfoHandler(pkg: PackageIn) {
    let msg: PlayerTreasureMsg = pkg.readBody(
      PlayerTreasureMsg,
    ) as PlayerTreasureMsg;
  ...
  private __treasureMapRewardHandler(pkg: PackageIn) {
    let content: string = LangManager.Instance.GetTranslation(
      "managers.TreasureMapManager.TipTxt1",
    );
  ...
  private __requestFrameCloseHandler(b: boolean, flag: boolean) {
    if (b) {
      TreasureMapSocketOutManager.sendReceiveTreasureMap(
        this._model.lastTreasureMap.pos,
        true,
      );
    } else {
      TreasureMapSocketOutManager.sendReceiveTreasureMap(
  ...
  public get needShowTip(): boolean {
    ...
    if (
      this._model.lastGetTime != 0 &&
      currentTime - this._model.lastGetTime <=
        1000 * TreasureMapModel.GET_TIP_DELTA
    ) {
      return false;
    }
  ...
  public useTreasureMap(info: GoodsInfo) {
    ...
    if (this._model.rewardCount >= this._model.rewardMax) {
      msg = LangManager.Instance.GetTranslation(
        "managers.TreasureMapManager.TipTxt6",
      );
    ...
    if (this.checkTreasureMap(info)) {
      if (!FreedomTeamManager.Instance.teamIsFull) {
        let content: string = LangManager.Instance.GetTranslation(
          "map.internals.view.treasuremap.TreasureMapUseFrame.digBtnAlertTxt1",
          4,
        );
        let confirm: string =
  ...
  private checkTreasureMap(info: GoodsInfo): boolean {
    ...
    if (!this.army || this.army.mapId != info.randomSkill1) {
      msg = LangManager.Instance.GetTranslation(
        "managers.TreasureMapManager.TipTxt2",
      );
      MessageTipManager.Instance.show(msg);
      return false;
    }
    ...
    let target: Laya.Point = new Laya.Point(
      info.randomSkill2,
      info.randomSkill3,
    );
    ...
    if (distance > TreasureMapModel.DISTANCE3 * Tiles.WIDTH) {
      msg = LangManager.Instance.GetTranslation(
        "managers.TreasureMapManager.TipTxt3",
        direction,
      );
      MessageTipManager.Instance.show(msg);
      return false;
    } else if (
      this.eqOrLittleFloat(
        distance,
        TreasureMapModel.DISTANCE3 * Tiles.WIDTH,
      ) &&
      distance > TreasureMapModel.DISTANCE2 * Tiles.WIDTH
    ) {
      msg = LangManager.Instance.GetTranslation(
        "managers.TreasureMapManager.TipTxt4",
        direction,
      );
      MessageTipManager.Instance.show(msg);
      return false;
      ...
      this.eqOrLittleFloat(
        distance,
        TreasureMapModel.DISTANCE2 * Tiles.WIDTH,
      ) &&
      distance > TreasureMapModel.DISTANCE1 * Tiles.WIDTH
    ) {
      msg = LangManager.Instance.GetTranslation(
        "managers.TreasureMapManager.TipTxt5",
        direction,
      );
      MessageTipManager.Instance.show(msg);
      return false;
  ...
  private getDirection(origin: Laya.Point, target: Laya.Point): string {
    let direction: string = LangManager.Instance.GetTranslation(
      "managers.TreasureMapManager.DirectionTxt1",
    );
    ...
    if (
      this.eqOrBigFloat(degree, -25.0) &&
      this.eqOrLittleFloat(degree, 25.0)
    ) {
      direction = LangManager.Instance.GetTranslation(
        "managers.TreasureMapManager.DirectionTxt1",
      );
    } else if (-65.0 < degree && degree < -25.0) {
      direction = LangManager.Instance.GetTranslation(
        "managers.TreasureMapManager.DirectionTxt2",
      );
    } else if (
      this.eqOrBigFloat(degree, -115.0) &&
      this.eqOrLittleFloat(degree, -65.0)
    ) {
      direction = LangManager.Instance.GetTranslation(
        "managers.TreasureMapManager.DirectionTxt3",
      );
    } else if (-155.0 < degree && degree < -115.0) {
      direction = LangManager.Instance.GetTranslation(
        "managers.TreasureMapManager.DirectionTxt4",
      );
    } else if (
      this.eqOrBigFloat(degree, 155.0) ||
      this.eqOrLittleFloat(degree, -155.0)
    ) {
      direction = LangManager.Instance.GetTranslation(
        "managers.TreasureMapManager.DirectionTxt5",
      );
    } else if (115.0 < degree && degree < 155.0) {
      direction = LangManager.Instance.GetTranslation(
        "managers.TreasureMapManager.DirectionTxt6",
      );
    } else if (
      this.eqOrBigFloat(degree, 65.0) &&
      this.eqOrLittleFloat(degree, 115.0)
    ) {
      direction = LangManager.Instance.GetTranslation(
        "managers.TreasureMapManager.DirectionTxt7",
      );
    } else if (25.0 < degree && degree < 65.0) {
      direction = LangManager.Instance.GetTranslation(
        "managers.TreasureMapManager.DirectionTxt8",
      );
    }
    return direction;
  }
  ...
  public checkReinforce(): boolean {
    ...
    if (PlayerManager.Instance.currentPlayerModel.reinforce) {
      ...
      let state: number =
        PlayerManager.Instance.currentPlayerModel.reinforce.state;
