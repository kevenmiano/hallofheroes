...
export default class EquipTipsAttribute extends FUI_EquipTipsAttribute {
    ...
    protected initBase(info: GoodsInfo, temp: t_s_itemtemplateData) {
        ...
        this.txt_intensify.text = info.templateInfo.StrengthenMax == 0 ? "" : LangManager.Instance.GetTranslation("yishi.view.tips.goods.EquipTipsContent.intensify", info.templateInfo.StrengthenMax);
    ...
    private setCastState(info: GoodsInfo, temp: t_s_itemtemplateData) {
        if (temp.StrengthenMax > 0 && temp.Profile >= 5 && temp.NeedGrades >= ForgeData.MOULE_NEED_TEMP_GRADE) {
            if (info.mouldRank > ForgeData.MOULD_MAX_RANK_SENIOR) {
                ...
                this.txt_intensify.text += '  ' + LangManager.Instance.GetTranslation("yishi.view.tips.goods.EquipTipsContent.castSelected.text3", info.mouldRankShow, info.mouldStar);
            } else {
                this.txt_intensify.text += '  ' + LangManager.Instance.GetTranslation("yishi.view.tips.goods.EquipTipsContent.castSelected.text2", info.mouldRankShow, info.mouldStar);
            }
        } else {
            //不可神铸
    ...
    private initAttribute(info: GoodsInfo, temp: t_s_itemtemplateData) {
        ...
        this.updateAttributeTxt(this.power, str, temp.Power, this.getAdd(temp.Power, info), this.getNewAdd(temp.Power, info), info.objectId);
        str = LangManager.Instance.GetTranslation("armyII.ThaneAttributeView.Tip02");
        this.updateAttributeTxt(this.agility, str, temp.Agility, this.getAdd(temp.Agility, info), this.getNewAdd(temp.Agility, info), info.objectId);
        str = LangManager.Instance.GetTranslation("armyII.ThaneAttributeView.Tip03");
        this.updateAttributeTxt(this.ability, str, temp.Intellect, this.getAdd(temp.Intellect, info), this.getNewAdd(temp.Intellect, info), info.objectId);
        str = LangManager.Instance.GetTranslation("armyII.ThaneAttributeView.Tip04");
        this.updateAttributeTxt(this.physique, str, temp.Physique, this.getAdd(temp.Physique, info), this.getNewAdd(temp.Physique, info), info.objectId);
        str = LangManager.Instance.GetTranslation("armyII.ThaneAttributeView.Tip05");
        this.updateAttributeTxt(this.captain, str, temp.Captain, this.getAdd(temp.Captain, info), this.getNewAdd(temp.Captain, info), info.objectId);
        str = LangManager.Instance.GetTranslation("armyII.ThaneAttributeView.Tip13");
        this.updateAttributeTxt(this.attack, str, temp.Attack, this.getAdd(temp.Attack, info), this.getNewAdd(temp.Attack, info), info.objectId);
        str = LangManager.Instance.GetTranslation("armyII.ThaneAttributeView.Tip15");
        this.updateAttributeTxt(this.magicAttack, str, temp.MagicAttack, this.getAdd(temp.MagicAttack, info), this.getNewAdd(temp.MagicAttack, info), info.objectId);
        str = LangManager.Instance.GetTranslation("armyII.ThaneAttributeView.Tip14");
        this.updateAttributeTxt(this.defence, str, temp.Defence, this.getAdd(temp.Defence, info), this.getNewAdd(temp.Defence, info), info.objectId);
        str = LangManager.Instance.GetTranslation("armyII.ThaneAttributeView.Tip16");
        this.updateAttributeTxt(this.magicDefence, str, temp.MagicDefence, this.getAdd(temp.MagicDefence, info), this.getNewAdd(temp.MagicDefence, info), info.objectId);
        str = LangManager.Instance.GetTranslation("armyII.ThaneAttributeView.Tip19");
        this.updateAttributeTxt(this.parry, str, temp.Parry, this.getAdd(temp.Parry, info), this.getNewAdd(temp.Parry, info), info.objectId);
        str = LangManager.Instance.GetTranslation("armyII.ThaneAttributeView.Tip11");
        this.updateAttributeTxt(this.live, str, temp.Live, this.getAdd(temp.Live, info), this.getNewAdd(temp.Live, info), info.objectId);
        str = LangManager.Instance.GetTranslation("armyII.ThaneAttributeView.Tip10");
        this.updateAttributeTxt(this.forceHit, str, temp.ForceHit, this.getAdd(temp.ForceHit, info), this.getNewAdd(temp.ForceHit, info), info.objectId);
        str = LangManager.Instance.GetTranslation("armyII.ThaneAttributeView.Tip17");
        this.updateAttributeTxt(this.conat, str, temp.Conat, this.getAdd(temp.Conat, info), this.getNewAdd(temp.Conat, info), info.objectId);
    }
    ...
    private initSkill(info: GoodsInfo, temp: t_s_itemtemplateData) {
        ...
        if (info.id == 0 && !this.txt_skill.text && info.templateInfo.RandomSkillCount != 0) {
            tempStr += "[" + LangManager.Instance.GetTranslation("yishi.view.tips.goods.EquipTipsContent.RandomSkillCount", info.templateInfo.RandomSkillCount) + "]<br>";
        }
        if (tempStr) {
    ...
    private initSuit(info: GoodsInfo, temp: t_s_itemtemplateData) {
        let skillString: string = "";
        let suitTemp: t_s_suitetemplateData = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_suitetemplate, temp.SuiteId);//取得套装模板
        if (suitTemp) {
            let heroId: number = info.objectId;
            let baseHero: ThaneInfo = ArmyManager.Instance.thane;
            let current: number;
            if (baseHero.id == heroId)//自己
            {
                let list: any[];
            ...
            this.txt_suitTitle.text = suitTemp.TemplateNameLang + "(" + (current > 0 ? current : 0) + "/" + num + ")";
            if (num > 1) {
            ...
            for (let k: number = 1; k < 9; k++) {
                //每套最大8件套装单件
                let suitGood: t_s_itemtemplateData = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_itemtemplate, suitTemp["Template" + k]);//取得单件模板（物品模板）
    ...
    private initSuitSkill(suitCount: number, skillIds: string, current: number) {
        //套装技能
        let text: fgui.GTextField;
        let skillList: t_s_skilltemplateData[] = TempleteManager.Instance.getSkillTemplateInfoByIds(skillIds);//取得技能模板
        for (let count: number = 0; count < skillList.length; count++) {
            ...
            if (skillTemp) {
                ...
                text.color = "#7d7d7d";
                if (suitCount <= 1 || count < current)//到达数量换颜色显示
                {
                    text.color = "#eedb05";
                }
                text.visible = true;
    ...
    private updateAttributeTxt(item: GoodAttributeItem, property: string, value: number, addValue: number, newAddValue?: number, objectId?: number) {
        if (value != 0) {
            item.visible = true;
    ...
    private getNewAdd(preValue: number, gInfo: GoodsInfo): number {
        let temp: t_s_itemtemplateData = TempleteManager.Instance.getGoodsTemplatesByTempleteId(gInfo.templateId);
        if (temp.StrengthenMax > 0 && temp.Profile >= 5 && temp.NeedGrades >= 50 && temp.MasterType != GoodsType.PET_EQUIP) {
            if (gInfo.mouldGrade > 0) {
                let curr: number = GoodsHelp.getMouldAddition(preValue, this.getPropertyMouldGrade(gInfo));
                return curr;
            }
        }
        return -1;
