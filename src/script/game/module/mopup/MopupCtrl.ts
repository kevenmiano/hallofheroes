...
export default class MopupCtrl extends FrameCtrlBase {
  constructor() {
    ...
    ServerDataManager.listen(
      S2CProtocol.U_C_CAMPAIGN_SWEEP,
      this,
      this.__refreshMopupInfoHandler.bind(this),
    );
  }
  ...
  private __refreshMopupInfoHandler(pkg: PackageIn) {
    ...
    if (msg.result.length > 0) {
      ...
      for (var i: number = 0; i < msg.result.length; i++) {
        if (msg.result[i]) this.data.resultInfoList.push(msg.result[i]);
        if (msg.result[i].time) this.data.mopupRemainTime = msg.result[i].time;
        if (msg.result[i].gp) this.data.mopupExt += msg.result[i].gp;
        ...
        if (this.data.mopupGoods && this.data.mopupGoods.length > 0) {
          for (var k: number = 0; k < msg.result[i].items.length; k++) {
            for (var j: number = 0; j < this.data.mopupGoods.length; j++) {
              if (
                msg.result[i].items[k].templateId ==
                this.data.mopupGoods[j].templateId
              ) {
                this.data.mopupGoods[j].count -= msg.result[i].items[k].count;
                if (this.data.mopupGoods[j].count <= 0) {
                  this.data.mopupGoods.splice(j, 1);
                  j--;
                }
              }
            }
    ...
    if (msg.status) {
      ...
      switch (msg.status) {
        ...
        case 2:
          //正在扫荡中
          mopupTipData = LangManager.Instance.GetTranslation(
    ...
    switch (msg.sweepType) {
      case eMopupType.CampaignMopup:
        FrameCtrlManager.Instance.open(EmWindow.Mopup, {
          ...
          state: eMopupState.CampaignMopuping,
        });
        break;
      case eMopupType.MazeMopup:
        FrameCtrlManager.Instance.open(EmWindow.Mopup, {
          type: eMopupType.MazeMopup,
          state: eMopupState.MazeMopuping,
        });
        break;
      case eMopupType.PetCampaignMopup:
        ...
        FrameCtrlManager.Instance.open(EmWindow.Mopup, {
          ...
          state: eMopupState.PetCampaignMopupping,
          UiLevelId: result.eIndex,
        });
        break;
    }
  }
  ...
  public sendMopupInfo(
    ...
    eIndex: number = 0,
    enterType: number = 1,
    openSilverBox: boolean = false,
    openSecretBox: boolean = false,
    payType: number = 2,
  ) {
