...
import { RewardEvent, RingTaskEvent, TaskEvent } from '../../constant/event/NotificationEvent';
...
export default class TaskWnd extends BaseWindow {
    ...
    private _rewardList = ["RewardPlayGP", "RewardGold", "RewardConsortiaOffer", "RewardPlayerOffer"];
    ...
    OnShowWind() {
        ...
        this.tree.treeNodeRender = Laya.Handler.create(this, this.renderTreeNode, null, false);
        this.conditionContentList.itemRenderer = Laya.Handler.create(this, this.renderConditionContentList, null, false);
        this.treeData = this.model.getTreeData();
        this.initTreeData(this.treeData[0]);
        this.ConditionTitleTxt.text = LangManager.Instance.GetTranslation("TaskWnd.ConditionTitleTxt");
        this.DescribleTitleTxt.text = LangManager.Instance.GetTranslation("TaskWnd.DescribleTitleTxt");
        this.RewardTitleTxt.text = LangManager.Instance.GetTranslation("TaskWnd.RewardTitleTxt");
    ...
    private addEvent() {
        ...
        OfferRewardManager.Instance.addEventListener(RewardEvent.REWARD_TASK_ADD, this.__addHandler, this);
        OfferRewardManager.Instance.addEventListener(RewardEvent.REWARD_TASK_FINISH, this.__addHandler, this);
        this.ringTaskManager.addEventListener(RingTaskEvent.REFRESHRING, this.__addHandler, this);
        OfferRewardManager.Instance.addEventListener(RewardEvent.REWARD_TASK_UPDATE, this.__updateRewardTaskHandler, this);
        TaskManage.Instance.addEventListener(TaskEvent.TASK_DETAIL_CHANGE, this.__updateRewardTaskHandler, this);
    }
    ...
    private removeEvent() {
        ...
        this.cate.removeEventListener(TaskEvent.TASK_ADDED, this.__addHandler, this);
        this.cate.removeEventListener(TaskEvent.TASK_REMOVE, this.__addHandler, this);
        OfferRewardManager.Instance.removeEventListener(RewardEvent.REWARD_TASK_ADD, this.__addHandler, this);
        OfferRewardManager.Instance.removeEventListener(RewardEvent.REWARD_TASK_FINISH, this.__addHandler, this);
        this.ringTaskManager.removeEventListener(RingTaskEvent.REFRESHRING, this.__addHandler, this);
        OfferRewardManager.Instance.removeEventListener(RewardEvent.REWARD_TASK_UPDATE, this.__updateRewardTaskHandler, this);
        TaskManage.Instance.removeEventListener(TaskEvent.TASK_DETAIL_CHANGE, this.__updateRewardTaskHandler, this);
    }
    ...
    renderTreeNode(node: fgui.GTreeNode, obj: fgui.GComponent) {
        ...
        if (node.data && node.data.type == eSelectTaskItemType.FirstItem) {
            ...
            if ((data instanceof BaseOfferReward) || (data instanceof RingTask)) {
                ...
                isComplete = data.taskInfo && data.isCompleted && data.taskInfo.isSelect;
            }
            if (isComplete) {
    ...
    private defaultView(type: number = -1) {
        ...
        if (type == -1) {
            ...
            list = ArrayUtils.sortOn(list, ["isCompleted", 'Sort'], [ArrayConstant.DESCENDING, ArrayConstant.NUMERIC]);
            var isVipAndNoExpirt: boolean = VIPManager.Instance.model.vipInfo.IsVipAndNoExpirt;
            var info: Object;
            for (let i: number = 0; i < list.length; i++) {
                info = list[0];
                if ((info instanceof TaskTemplate) && info['TemplateType'] == TaskType.VIP_TASK && !isVipAndNoExpirt) continue;
                if (info instanceof BaseOfferReward)
                    selectMenuType = 100;
                else
                    selectMenuType = (info as TaskTemplate).TemplateType;
                hasSet = true;
            }
        } else {
            selectMenuType = type;
        ...
        for (let index = 0; index < this._fTreeNodeList.length; index++) {
            let sTreeNode = this._fTreeNodeList[index];
            if (sTreeNode && sTreeNode.data && sTreeNode.data.taskType == selectMenuType) {
                for (let i: number = 0; i < sTreeNode.numChildren; i++) {
    ...
    private initTreeData(firstData: SelectTaskItemData[]) {
        ...
        for (let index = 0; index < firstData.length; index++) {
            ...
            let sData: Array<any> = TaskManage.Instance.cate.getListByType(selectTaskItemData.taskType);
            sData = ArrayUtils.sortOn(sData, ["isCompleted", 'Sort'], [ArrayConstant.DESCENDING, ArrayConstant.NUMERIC]);
            for (var j: number = 0; j < sData.length; j++) {
                if (sData[j].conditionList[0] instanceof t_s_questcondictionData == false)
                    continue;
            ...
            for (let i: number = 0; i < sData.length; i++) {
                ...
                sNode._resURL = fgui.UIPackage.getItemURL(EmWindow.TaskWnd, "TaskItemInfo")
                sNode.data = sData[i];
    ...
    private onClickTreeItem(itemObject: fgui.GObject) {
        ...
        if (itemData.type == eSelectTaskItemType.FirstItem) {
            ...
            if (model.selectedData instanceof TaskTemplate) model.selectedData["isNew"] = false;
    ...
    private refreshTargetView(data: any) {
        ...
        if (data instanceof TaskTemplate) {
            ...
            if (data.ringTaskTemp.Type != 3) {
                this.ConditionTitleTxt2.text = LangManager.Instance.GetTranslation("task.view.TaskTargetViewII.ringTaskNum", RingTaskManager.Instance.model.rewardNum);
            }
    ...
    private refreshCondition() {
        ...
        for (let i: number = 0; i < this._rightData.conditionList.length; i++) {
            ...
            if (this._rightData instanceof TaskTemplate) {
                ...
                if (this._isRingTask) {
                    if (this._ringTask.ringTaskTemp.Type == 3) {
                        ...
                        let str: string = num + " " + LangManager.Instance.GetTranslation("ui.tasktracebar.view.ringTaskNum");
                        ConditionContentTxt2 = str;
                    } else {
                        ConditionContentTxt2 = progress[i].toString();
                        let ringCondition: t_s_rewardcondictionData = RingTaskManager.Instance.getCurrCondition();
                        ...
                        switch (ringCondition.CondictionType) {
                            case RewardConditionType.COMMITITEM:
                                let goodstempleteInfo: t_s_itemtemplateData = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_itemtemplate, ringtask.currTask.condition_3);
                            ...
                            case RewardConditionType.KILLDUPLICATEMONSTER:
                                let campaignTemp: t_s_campaignData = TempleteManager.Instance.getCampaignTemplateByID(ringtask.currTask.condition_2);
            ...
            let conditionData: any = { contentTxt: ConditionContentTxt, contentTxt2: ConditionContentTxt2 };
    ...
    private refreshDescribeView(data: any) {
        ...
        if (data instanceof TaskTemplate) {
            ...
            var ringCondition: t_s_rewardcondictionData = RingTaskManager.Instance.getCurrCondition();
                    ...
                    case RewardConditionType.COMMITITEM:
                        ...
                        switch (ringtask.currTask.condition_2) {
                            case ShopGoodsInfo.CONSORTIA_SHOP:
                                shopName = LangManager.Instance.GetTranslation("public.consortia") + LangManager.Instance.GetTranslation("taskwnd.ringtask.titleTxt");
                                break;
                            case ShopGoodsInfo.ATHLETICS_SHOP:
                                shopName = LangManager.Instance.GetTranslation("map.space.view.frame.SpaceDialogFrame.pvp") + LangManager.Instance.GetTranslation("taskwnd.ringtask.titleTxt");
                                break;
                            case ShopGoodsInfo.MAZE_SHOP:
                                shopName = LangManager.Instance.GetTranslation("map.space.view.frame.SpaceDialogFrame.Maze") + LangManager.Instance.GetTranslation("taskwnd.ringtask.titleTxt");
                                break;
                        }
    ...
    private refreshRewardView(data: any) {
        ...
        if (data instanceof TaskTemplate) {
            ...
            if (this._ringTask.ringTaskTemp.Type == 3) {
                this.RewardTitleTxt.text = LangManager.Instance.GetTranslation("TaskWnd.RewardTitleTxt2");
    ...
    private refreshReward() {
        ...
        for (var i: number = 0; i < this._rewardList.length; i++) {
            if (this.rightReward.hasOwnProperty(this._rewardList[i]) && parseInt(this.rightReward[this._rewardList[i]].toString()) > 0) {
                let currentReward: TaskRewardItem = <TaskRewardItem>this.rewardList1.addItemFromPool();
                ...
                if (this.rightReward instanceof TaskTemplate) {
                    if (this.rightReward.TemplateType == TaskType.TASK_COMMON && i == 0)
                        currentReward.propertyValue = (parseInt(this.rightReward[this._rewardList[i]].toString()) + (this.thane.grades - this._rightData.NeedMinLevel) * 500).toString();
                    else
                        currentReward.propertyValue = parseInt(this.rightReward[this._rewardList[i]]).toString();
                } else if (this.rightReward instanceof OfferRewardTemplate) {
                    if (!this._isRing) {
                        if (this._baseReward.profile != 5)
                            currentReward.propertyValue = parseInt((this.rightReward[this._rewardList[i]] * this._baseReward.profile * this.thane.grades).toString()).toString();
                        else
                            currentReward.propertyValue = parseInt((this.rightReward[this._rewardList[i]] * (this._baseReward.profile + 1) * this.thane.grades).toString()).toString();
                    } else {
                        currentReward.propertyValue = parseInt((this.rightReward[this._rewardList[i]]).toString()).toString();
                    }
                }
            }
        }
    ...
    private refreshGoods() {
        ...
        for (var i: number = 0; i < len; i++) {
            ...
            var temp: t_s_itemtemplateData = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_itemtemplate, good.RewardItemID);
    ...
    private getTaskGoodList(info: any): any[] {
        ...
        if (info instanceof TaskTemplate) {
            for (let i: number = 0; i < list.length; i++) {
                ...
                if (good.TemplateId == info.TemplateId && good.fitJob(this.thane.templateInfo.Job)) {
                    arr.push(good);
                }
            }
    ...
    private refreshButton(data: any) {
        ...
        if (data.isCompleted) {
            ...
            if ((data as RingTask).ringTaskTemp.Type == 3) {
                ...
                this.Btn_giveup.visible = false;
                for (let i: number = 0; i < data.conditionList.length; i++) {
                    var ringConditionTemp: t_s_rewardcondictionData = data.conditionList[i];
                    if (ringConditionTemp.CondictionType == RewardConditionType.QTE || ringConditionTemp.CondictionType == RewardConditionType.HAMSTER_GAME) {
    ...
    private __getRewardHandler() {
        ...
        if (this._rightData instanceof BaseOfferReward) {
            if (this._rightData.isCompleted || VIPManager.Instance.model.isOpenPrivilege(VipPrivilegeType.OFFER_REWARD_LIMIT, VIPManager.Instance.model.vipInfo.VipGrade)) {
                ...
                OfferRewardManager.Instance.sendFinishRewardTask(this._rightData.rewardID);
            }
            return;
        } else if (this._rightData instanceof RingTask) {
    ...
    private __completeNowHandler() {
        ...
        if (this._rightData instanceof RingTask) {
            ...
            let hasGoods: any[] = GoodsManager.Instance.getBagGoodsByTemplateId(2131021);
            if (needAlert) {
                let point: number = 0;
                if (hasGoods.length <= 0) {
                    point = 10;
                }
                let content: string = LangManager.Instance.GetTranslation("task.TaskFrameII.usePointPrompt", point);
                UIManager.Instance.ShowWind(EmWindow.TodayNotAlert, { content: content, point: point, backFunction: this.quickSubmit.bind(this), closeFunction: null });
            } else {
                let hasMoney: number = PlayerManager.Instance.currentPlayerModel.playerInfo.point;
                if (useBind) hasMoney = PlayerManager.Instance.currentPlayerModel.playerInfo.allPoint;
            ...
            if (VIPManager.Instance.model.isOpenPrivilege(VipPrivilegeType.OFFER_REWARD_LIMIT, VIPManager.Instance.model.vipInfo.VipGrade)) {//有特权, 直接完成
                OfferRewardManager.Instance.sendFinishRewardTask(this._rightData.rewardID, true);
            } else {//没有特权
                if (!this._rightData) return;
                if (this._rightData.isCompleted) {
                    OfferRewardManager.Instance.sendFinishRewardTask(this._rightData.rewardID, true);
                    ...
                    // if (needAlert) {
                    let needGrade: number = VIPManager.Instance.model.getMinGradeHasPrivilege(VipPrivilegeType.OFFER_REWARD_LIMIT);
                    let content: string = LangManager.Instance.GetTranslation("taskRingItem.speed.AlertTxt", needGrade);
    ...
    private get todayNeedAlertForOpenKingContract(): boolean {
        ...
        var lastSaveDate: Date = new Date(SharedManager.Instance.openKingContractAlertDate);
        if (lastSaveDate) {
    ...
    private quickSubmit(b: boolean, flag: boolean) {
        ...
        let hasMoney: number = PlayerManager.Instance.currentPlayerModel.playerInfo.point;
        if (flag) hasMoney = PlayerManager.Instance.currentPlayerModel.playerInfo.allPoint;
        var hasGoods: GoodsInfo[] = GoodsManager.Instance.getBagGoodsByTemplateId(2131021);
        if (hasMoney < 10 && hasGoods.length <= 0) {
    ...
    private __enterCampainHandler() {
        ...
        var mapTemp: t_s_campaignData = ConfigMgr.Instance.worldBossDic[this._rightData.NeedFightId];
        ...
        if (this._rightData) {
            ...
            switch (curScene) {
                ...
                case SceneType.PVP_ROOM_SCENE:
                    var str: string = LangManager.Instance.GetTranslation("task.TaskFrameII.command02");
                    MessageTipManager.Instance.show(str);
                    return;
            ...
            var condition: t_s_rewardcondictionData = this._rightData.conditionList[0];
            if (condition)
                OfferRewardManager.Instance.sendRewardArrest(condition.CondictionType, condition.Para1);
        }
    }
    ...
    private __onRemoveRewardTaskClick() {
        ...
        let content: string = LangManager.Instance.GetTranslation("task.TaskFrameII.content");;
        SimpleAlertHelper.Instance.Show(undefined, null, prompt, content, confirm, cancel, this.removeRewardTaskCallBack.bind(this));
    }
