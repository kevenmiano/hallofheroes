...
export class RemotePetAdjustWnd extends BaseWindow {
  ...
  public OnInitWind() {
    ...
    for (let i = 0; i < this.petSkillViews.length; i++) {
      this.petSkillViews[i].on(
        fgui.Events.DRAG_START,
        this,
        this.onPetSkillDragStart,
      );
  ...
  private addEvent() {
    ...
    this.remotePetSkillList.itemRenderer = Laya.Handler.create(
      this,
      this.onRemotePetSkillRenderer,
      null,
      false,
    );
  ...
  private onPetDragStart(evt: Laya.Event) {
    let btn: RemotePetHeadItemView = <RemotePetHeadItemView>(
  ...
  private onPetSkillDragStart(evt: Laya.Event) {
    let btn: RemotePetSkillItemView = <RemotePetSkillItemView>(
      fgui.GObject.cast(evt.currentTarget)
    );
    //取消对原目标的拖动, 换成一个替代品
    btn.stopDrag();
  ...
  private onPetSkillDrop(otherView: any, evt: Laya.Event) {
    ...
    if (otherView instanceof PetSkillItem) {
      ...
      FUIHelper.setTipData(btn, EmWindow.PetSkillTips, otherView.info);
  ...
  private savePetSkill() {
    if (this.curPet.remotePetSkillsOfString != PetData.FastSkillKeyTemp) {
      ...
      RemotePetManager.sendPetSkills(
        this.curPet.petId,
        PetData.FastSkillKeyTemp,
      );
    }
