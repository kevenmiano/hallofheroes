...
export default class QuickInviteWnd extends BaseWindow {
  ...
  private initInviteContent() {
    ...
    if (this.roomSceneType == RoomSceneType.PVP) {
      ...
      if (this.roomInfo.campaignId == GlobalConfig.CampaignID.AncientRuins) {
        this._defaultContentPrefix = StringHelper.format(
          LangManager.Instance.GetTranslation(
        ...
        let templateInfo = this.roomInfo.mapTemplate
        ...
        if (templateInfo.isKingTower) {
          ...
          if (!templateInfo.isTaila) {
            mapName =
              templateInfo.CampaignNameLang + " " + lvstr + difficultyGrade;
          } else {
    ...
    let contentSuffix = this.thane.inviteContent
      ? this.thane.inviteContent
      : this._defaultContentSuffix;
    ...
    if (num == 0) {
      if (this.thane.smallBugleFreeCount <= 0) {
        ...
        let info: ShopGoodsInfo =
          TempleteManager.Instance.getShopTempInfoByItemId(tempId);
  ...
  private __onTxtChange() {
    ...
    let maxCount = TempleteManager.Instance.CfgMaxWordCount;
    this.txtWordLimit.text =
      txtTemp.length + this._defaultContentPrefix.length + "/" + maxCount;
  }
