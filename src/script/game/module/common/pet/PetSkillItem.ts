...
export class PetSkillItem extends FUI_PetSkillItem implements ITipedDisplay {
  ...
  public set info(value: t_s_skilltemplateData) {
    ...
    if (value) {
      ...
      this.cSuperSkill.setSelectedIndex(
        SkillPriorityType.isPetSuperSkill(value.Priority) ? 1 : 0,
      );
      this.registerTip();
    } else {
      this.tipData = null;
  ...
  public get isLearned(): boolean {
    return (
      this._info &&
      this.extData &&
      (this.extData as PetData).checkSkillIsLearned(this._info.TemplateId)
    );
  }
