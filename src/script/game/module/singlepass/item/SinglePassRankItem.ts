...
export default class SinglePassRankItem extends FUI_SinglePassRankItem {
  ...
  public set info(value: SinglePassOrderInfo) {
    ...
    if (this._info) {
      ...
      var tollgate: number = parseInt(
        (this._info.index % SinglePassModel.TOLLGATE_PER_FLOOR).toString(),
      );
      if (tollgate == 0) {
        tollgate = SinglePassModel.TOLLGATE_PER_FLOOR;
      }
      this.layerDescTxt.text = LangManager.Instance.GetTranslation(
        "singlepass.order.SinglePassOrderFrame.ItemTxt",
