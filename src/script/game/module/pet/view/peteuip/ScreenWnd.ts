...
export default class ScreenWnd extends BaseWindow {
  ...
  public OnInitWind() {
    ...
    for (let i = 1; i < 6; i++) {
      let btn: fairygui.GButton = this.contentPane.getChild(
        "qBtn" + i,
      ) as fairygui.GButton;
      (btn.getChild("title") as fairygui.GTextField).color =
        FilterFrameText.Colors[eFilterFrameText.PetQuality][i - 1];
      btn.getChild("title").text = LangManager.Instance.GetTranslation(
        "yishi.utils.GoodsHelp.getGoodQualityName0" + i,
      );
      ...
      btn1.getChild("title").text = LangManager.Instance.GetTranslation(
