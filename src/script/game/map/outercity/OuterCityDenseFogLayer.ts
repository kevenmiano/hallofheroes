...
export class OuterCityDenseFogLayerGrid {
    public get coordinateX(): number { return Math.floor(this.x / this.w); }
    public get coordinateY(): number { return Math.floor(this.y / this.h); }
    public x: number = 0;
    public y: number = 0;
    public w: number = 300;
    public h: number = 300;
    ...
    constructor(x: number, y: number, w: number, h: number, parent: OuterCityDenseFogLayer) {
        this.x = x;
        this.y = y;
        this.w = w;
        this.h = h;
...
export class OuterCityDenseFogLayer extends Sprite implements IEnterFrame {
    ...
    // 待绘制的所有格子
    private gridMap: Map<string, OuterCityDenseFogLayerGrid> = new Map<string, OuterCityDenseFogLayerGrid>()
    ...
    private addEvent(): void {
        ...
        this._model.addEventListener(OuterCityEvent.LAY_MAP_ARMY, this.__layMapArmyHandler, this);
        NotificationManager.Instance.addEventListener(OuterCityEvent.ADD_DENSEFOG_BY_BUILD, this.addBuildHole, this);
        NotificationManager.Instance.addEventListener(OuterCityEvent.ADD_DENSEFOG_BY_MINE, this.addMineHole, this);
        NotificationManager.Instance.addEventListener(SpaceEvent.HIDE_OTHERS, this.onTeamDismiss, this);
        NotificationManager.Instance.addEventListener(FreedomTeamEvent.TEAM_INFO_SYNC, this.__syncTeamInfoHandler, this);
        OuterCityManager.Instance.model.addEventListener(OuterCityEvent.REMOVE_ARMY, this.onRemoveArmy, this);
        NotificationManager.Instance.addEventListener(OuterCityEvent.SELF_CASTLE,this.addSelfBuildHole,this);
        NotificationManager.Instance.addEventListener(OuterCityEvent.CASTLE_INFO,this.updateCastleOccpuy,this);
    }

    private removeEvent(): void {
        EnterFrameManager.Instance.unRegisteEnterFrame(this);
        this._model.removeEventListener(OuterCityEvent.LAY_MAP_ARMY, this.__layMapArmyHandler, this);
        NotificationManager.Instance.removeEventListener(OuterCityEvent.ADD_DENSEFOG_BY_BUILD, this.addBuildHole, this);
        NotificationManager.Instance.removeEventListener(OuterCityEvent.ADD_DENSEFOG_BY_MINE, this.addMineHole, this);
        NotificationManager.Instance.removeEventListener(SpaceEvent.HIDE_OTHERS, this.onTeamDismiss, this);
        NotificationManager.Instance.removeEventListener(FreedomTeamEvent.TEAM_INFO_SYNC, this.__syncTeamInfoHandler, this);
        OuterCityManager.Instance.model.removeEventListener(OuterCityEvent.REMOVE_ARMY, this.onRemoveArmy, this);
        NotificationManager.Instance.removeEventListener(OuterCityEvent.SELF_CASTLE,this.addSelfBuildHole,this);
        NotificationManager.Instance.addEventListener(OuterCityEvent.CASTLE_INFO,this.updateCastleOccpuy,this);
    }
    ...
    enterFrame(): void {
        ...
        if (this.tickCount >= this._refreshCount) {
            ...
            if (this.selfCastlePos) {
                this.fovObjList.push(new Laya.Point(this.selfCastlePos.x, this.selfCastlePos.y))
            }
            ...
            for(const key in this._model.allVehicleViewDic){
                ...
                let point:Laya.Point = new Laya.Point(army.wildInfo.movePosX * 20,army.wildInfo.movePosY * 20);
            ...
            for (const key in this._model.allArmyDict) {
                if (this._model.allArmyDict.hasOwnProperty(key)) {
                    ...
                    if (army.userId == this.playerInfo.userId && army.armyView) {
                        ...
                        this.fovObjList.push(new Laya.Point(army.armyView.x, army.armyView.y))
                    } else {
    ...
    refreshGrid() {
        ...
        let centerX: number = Math.abs(StageReferance.stageWidth / 2 - this.target.x);
        let centerY: number = Math.abs(StageReferance.stageHeight / 2 - this.target.y);
        ...
        // 左上角像素坐标
        let topLeftX = centerCoordinateX * this.GRID_WIDTH - (Math.floor(this.RENDER_GRID_COW_CNT / 2) * this.GRID_WIDTH)
        let topLeftY = centerCoordinateY * this.GRID_HEIGHT - (Math.floor(this.RENDER_GRID_ROW_CNT / 2) * this.GRID_HEIGHT)
        // Logger.xjy("左上角坐标点: ", topLeftX, topLeftY)
        // 以屏幕中心为原点取RENDER_GRID_COW_CNT*RENDER_GRID_ROW_CNT个格子
        let keyArr = []
        if (!this.centerGrid || (this.centerGrid.coordinateX != centerCoordinateX || this.centerGrid.coordinateY != centerCoordinateY)) {
            for (let row = 0; row < this.RENDER_GRID_ROW_CNT; row++) {
                for (let cow = 0; cow < this.RENDER_GRID_COW_CNT; cow++) {
                    ...
                    if (!grid) {
                        grid = new OuterCityDenseFogLayerGrid(topLeftX + cow * this.GRID_WIDTH, topLeftY + row * this.GRID_HEIGHT, this.GRID_WIDTH, this.GRID_HEIGHT, this)
                        grid.initFogList()
                        this.gridMap.set(key, grid)
                    }
                    if (row == Math.floor(this.RENDER_GRID_ROW_CNT / 2) && cow == Math.floor(this.RENDER_GRID_COW_CNT / 2)) {
        ...
        this.gridMap.forEach((grid: OuterCityDenseFogLayerGrid) => {
            for (let index = 0; index < grid.fogList.length; index++) {
                ...
                for (let i = 0; i < this.fovObjList.length; i++) {
                    ...
                    if (pt.distance(node.x + grid.FOG_SIZE / 2, node.y + grid.FOG_SIZE / 2) < OuterCityDenseFogLayer.FOV) {
                        grid.showNode(node, false)
                        break;
                    }
                }
    ...
    public static get selfArmy() {
        ...
        for (const key in model.allArmyDict) {
            if (model.allArmyDict.hasOwnProperty(key)) {
                ...
                if (army.userId == PlayerManager.Instance.currentPlayerModel.playerInfo.userId && army.armyView) {
                    return army
                }
            }
        }
            ...
            if (allCastles.hasOwnProperty(allCastlesKey)) {
                ...
                for (const tempKey in temp) {
                    if (temp.hasOwnProperty(tempKey)) {
                        ...
                        if(castle && castle.defencerGuildName == this.playerInfo.consortiaName){
    ...
    public getKey(x: number, y: number): string {
        return Math.floor(x / this.GRID_WIDTH).toString() + "_" + Math.floor(y / this.GRID_HEIGHT).toString()
    }
