...
export default class SinglePassCardItem extends FUI_SinglePassCardItem {
  ...
  public set info(value: SinglePassCardInfo) {
    ...
    if (this._info && this._info.judge != SinglePassModel.TEST_VALUE) {
      ...
      this.playerIcon.url = IconFactory.getGodArriveBossIcon(
        SinglePassModel.CAMPAIGN_TEMPLATE_ID,
        this._info.tollgate,
      );
      this.gradeTxt.text = LangManager.Instance.GetTranslation(
        "SinglePassCardItem.gradeTxt",
        this._index,
      );
      var campaigndataData: t_s_campaigndataData =
        TempleteManager.Instance.getGodArriveData(
          SinglePassModel.CAMPAIGN_TEMPLATE_ID,
          this._info.tollgate,
        );
      ...
      let drops1: SinglePassBossRewardData =
        TempleteManager.Instance.getSinglePassBossReward(
          0,
          this._info.tollgate,
        );
      let drops2: SinglePassBossRewardData =
        TempleteManager.Instance.getSinglePassBossReward(
          2,
          this._info.tollgate,
        );
      ...
      if (this.showJudge) {
        ...
        str =
          LangManager.Instance.GetTranslation(
            "singlepass.view.SinglePassCardItemView.TipsTxt02",
          ) + "<br>";
        let item: GoodsInfo;
        if (drops1) {
        ...
        FUIHelper.setTipData(this.getReward, EmWindow.CommonTips, str);
      } else {
        this.gradeBg.visible = false;
        this.judge.visible = false;
        ...
        let str =
          LangManager.Instance.GetTranslation(
            "singlepass.view.SinglePassCardItemView.TipsTxt01",
          ) + "<br>";
        let item: GoodsInfo;
        if (drops1) {
