...
export class SpaceActivationMovieMediator implements IMediator, IEnterFrame {
  ...
  public enterFrame() {
    ...
    if (this._count > 25) {
      ...
      if (!arr) return;
      var rect: Laya.Rectangle = SpaceMapViewHelper.getCurrentMapRect(
        this._mapView,
      );
      for (let node of arr) {
        if (NodeState.displayState(node.info.state)) {
          var nodeView: object =
            SpaceManager.Instance.controller.getBuildView(node);
          if (!nodeView)
      ...
      dic.forEach((element) => {
        var armyView: object =
          SpaceManager.Instance.controller.getArmyView(element);
