...
export default class FirstPayManager {
  ...
  public setup() {
    ...
    ServerDataManager.listen(
      S2CProtocol.U_C_FIRST_CHARGE_AWARD,
      this,
      this._firstPayDataHandler,
    );

    NotificationManager.Instance.addEventListener(
      NotificationEvent.SWITCH_SCENE,
      this._onCheckOpenFirstPayHandler,
      this,
    );
    ArmyManager.Instance.thane.removeEventListener(
      PlayerEvent.THANE_LEVEL_UPDATE,
      this._onHeroLevelUpdateHandler,
      this,
    );
  }

  private _firstPayDataHandler(pkg: PackageIn) {
    let msg: FirstChargeAwardMsg = pkg.readBody(
      FirstChargeAwardMsg,
    ) as FirstChargeAwardMsg;
  ...
  private _onCheckOpenFirstPayHandler(e: NotificationEvent): void {
    ...
    //未充值/未领取完时
    if (
      !PlayerManager.Instance.currentPlayerModel.playerInfo.isFirstCharge ||
