...
export default class UIXiLian extends BaseFguiCom {
    ...
    private __onRefreshRandomProperty(pkg: PackageIn) {
        ...
        for (let i: number = 0; i < ForgeData.XLAttrNum; i++) {
            ...
            let temp: t_s_skilltemplateData = TempleteManager.Instance.getSkillTemplateInfoById(msg.randomSkill[i]);
            if (temp) {
        ...
        if (cnt > 0) {
            ...
            if (info) {
                ...
                this.comNextTotalEvalute.cPromote_.selectedIndex = newScore - preScore > 0 ? 1 : 2;
    ...
    private __onBagMoveUpdate(pkg: PackageIn) {
        ...
        this._ownLockCount = GoodsManager.Instance.getGoodsNumByTempId(ItemID.REFRESH_LOCK_PROP);
    ...
    private updateLockCount() {
        this.txtLockCount.text = "x" + GoodsManager.Instance.getGoodsNumByTempId(ItemID.REFRESH_LOCK_PROP);
    }
    ...
    public refreshMateStone() {
        ...
        let temp = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_itemtemplate, goodsInfo.templateId) as t_s_itemtemplateData
        let shopGoodsInfo = TempleteManager.Instance.getShopTempInfoByItemId(temp.Refresh);
        if (temp.Refresh > 0) {
            let goodsAry = GoodsManager.Instance.getBagGoodsByTemplateId(temp.Refresh);
            goodsAry.forEach((goods: GoodsInfo) => {
                if (goods.isBinds)
                    this._bindStoneMateCount += goods.count;
                else
                    this._nobindStoneMateCount += goods.count;
            });

            let needNum = ForgeData.mateMaterial(goodsInfo.templateId)
            let ownNum = this._useBindStone ? (this._bindStoneMateCount + this._nobindStoneMateCount) : this._nobindStoneMateCount;
            ...
            if (ownNum < needNum && shopGoodsInfo) {
                this._stoneOffsetDiamond = shopGoodsInfo.GiftToken * this._stoneOffsetCount;
            }

            let stoneTemp = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_itemtemplate, temp.Refresh) as t_s_itemtemplateData
            ...
            Logger.xjy("bindStoneCnt=" + this._bindStoneMateCount, "noBindStoneCnt=" + this._nobindStoneMateCount, "needNum=" + needNum)
            Logger.xjy("_stoneOffsetDiamond=" + this._stoneOffsetDiamond, "GiftToken=", shopGoodsInfo ? shopGoodsInfo.GiftToken : 0)
        } else {
            this.itemMate1.info = null
        }
    }
    ...
    private refreshMateNeedDiamond() {
        ...
        let tempLockCount = this._curLockCount <= this._ownLockCount ? 0 : (this._curLockCount - this._ownLockCount);
        if (this._useDiamondAutoBuy)
            this._needDiamondCount = this._stoneOffsetDiamond + tempLockCount * ForgeData.WASH_LOCK_PRICE;
        else
            this._needDiamondCount = tempLockCount * ForgeData.WASH_LOCK_PRICE;
        Logger.xjy("this._ownLockCount=" + this._ownLockCount, "this._curLockCount=" + this._curLockCount, "this._stoneOffsetDiamond=" + this._stoneOffsetDiamond, "this._needDiamondCount=" + this._needDiamondCount)
    }
    ...
    private getLockOffsetDiamond() {
        return this._ownLockCount >= this._curLockCount ? 0 : (this._curLockCount - this._ownLockCount)
    }
    ...
    private getRandomSkillCount(): number {
        ...
        for (let i: number = 1; i < 6; i++) {
            if (this.itemTarget.info["randomSkill" + i] > 0)
                randomSkillCount++;
        }
    ...
    private btnXiLianClick() {
        if (this.itemTarget.info.isLock) {
            let str = LangManager.Instance.GetTranslation("store.refresh.RefreshView.equipIsLock");
    ...
    private __btnXiLianClick(b: boolean, flag: boolean) {
        if (b) {
            ...
            if (this._curLockCount >= this.getRandomSkillCount()) {
                let str: string = LangManager.Instance.GetTranslation("store.view.refresh.RefreshView.command01");
                MessageTipManager.Instance.show(str);
                return;
            }
            ...
            if (this._stoneOffsetCount > 0) {
                let info: ShopGoodsInfo = TempleteManager.Instance.getShopTempInfoByItemId(this._stoneTempId, ShopGoodsInfo.PROP_GOODS);
                if (info) {
                    FrameCtrlManager.Instance.open(EmWindow.BuyFrameI, { info: info, count: this._stoneOffsetCount });
                } else {
            ...
            if (offsetLockCnt > 0 && this.showLockBuyAlert()) {
                let cfgValue = 10;
                let cfgItem = TempleteManager.Instance.getConfigInfoByConfigName("RefreshLock_Price");
                if (cfgItem) {
                    cfgValue = Number(cfgItem.ConfigValue);
                }
                let content = LangManager.Instance.GetTranslation("Forge.Refresh.RefreshLockLack", cfgValue);
                UIManager.Instance.ShowWind(EmWindow.TodayNotAlert, {
                    content: content, backFunction: (check1: boolean, check2: boolean) => {
            ...
            this.__btnXiLianClick2(true, SharedManager.Instance.storeLockBuy, SharedManager.Instance.storeLockBuyUseBind)
        }
    }
    ...
    private __btnXiLianClick2(b: boolean, check: boolean, useBindPoint: boolean) {
        ...
        Logger.xjy("[UIXiLian]sendRefreshforRandom bindingType=" + bindingType, "useBindDiamond=", useBindDiamond, lockStateArr)
        ForgeSocketOutManager.sendRefreshforRandom(lockStateArr, bindingType, useBindDiamond);
    }
    ...
    private resetAttr() {
        for (let i: number = 0; i < ForgeData.XLAttrNum; i++) {
            let item = this.attrList.getChildAt(i) as ForgeRefreshItem
            item.randomSkill = this.itemTarget.info ? this.itemTarget.info["randomSkill" + (i + 1)] : -1;
