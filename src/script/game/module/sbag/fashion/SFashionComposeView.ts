...
export class SFashionComposeView extends FUI_SFashionComposeView {
  ...
  private initEvent() {
    ...
    ServerDataManager.listen(
      S2CProtocol.U_C_FASHION_COMPOSE,
      this,
      this.__onComposeCallback,
    );
    this.model.addEventListener(
      FashionEvent.RESET_MOVIECLIP,
      this.__resetMovieclipHandler,
      this,
    );
  ...
  private __onComposeCallback(pkg: PackageIn) {
    ...
    let info: GoodsInfo = GoodsManager.Instance.getItemByPOB(
      msg.pos,
      msg.objectId,
      msg.bagType,
    );
  ...
  private __bagItemUpdateHandler(infos: GoodsInfo[]) {
    ...
    for (let info of infos) {
      let item: FashionBagCell =
        this._itemDic[info.pos + "_" + info.objectId + "_" + info.bagType];
      if (
        item /*&& this.model.selectedPanel == FashionModel.FASHION_COMPOSE_PANEL*/
      ) {
        item.alpha = 1;
        item.info = info;
  ...
  private getLevelUpByGp(gp: number) {
    ...
    while (
      (upgradeTemp = TempleteManager.Instance.getTemplateByTypeAndLevel(
        ++grades,
        UpgradeType.UPGRADE_FASHION,
      ))
    ) {
  ...
  private get calcGp() {
    ...
    if (GoodsCheck.isFashion(goods.templateInfo)) {
      ...
      if (goods.templateInfo.SonType == GoodsSonType.SONTYPE_WING) {
        //翅膀
      ...
      if (goods.templateId == 208021) {
        //幸运符
        baseGp = this.getFashionLuckyBaseGp();
      } else if (goods.templateId == 208020) {
        //时装之魄
  ...
  private getFashionLuckyBaseGp() {
    return +TempleteManager.Instance.getConfigInfoByConfigName("Fashion_Lucky")
      .ConfigValue;
  }

  private getFashionSoulBaseGp() {
    return +TempleteManager.Instance.getConfigInfoByConfigName("Fashion_Soul")
      .ConfigValue;
  }

  private getFashionWingBaseGp() {
    return +TempleteManager.Instance.getConfigInfoByConfigName("Fashion_Wing")
      .ConfigValue;
  }

  private getFashionOtherBaseGp() {
    return +TempleteManager.Instance.getConfigInfoByConfigName("Fashion_Other")
      .ConfigValue;
  }
