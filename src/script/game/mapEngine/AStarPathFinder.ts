...
export class AStarPathFinder {
  ...
  public find(startPoint: Laya.Point, endPoint: Laya.Point): Laya.Point[] {
    ...
    while (this.openList.length > 0) {
      ...
      let aroundNodes: any[] = this.getAroundsNode(
        currentNode.x,
        currentNode.y,
      );

      for (let node of aroundNodes) {
        //检测相邻的八个方向的节点
  ...
  private getAroundsNode(x: number, y: number): any[] {
    ...
    checkY = y;
    if (
      this.isWalkable(checkX, checkY) &&
      !this.getPointIsClose(checkX, checkY)
    ) {
  ...
  private getPointIsClose($x: number, $y: number): boolean {
    return this._allNodeDict[$x + "_" + $y] == null
      ? false
      : this._allNodeDict[$x + "_" + $y].isInClose;
  }
