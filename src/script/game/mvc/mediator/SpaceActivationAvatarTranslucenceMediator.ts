...
export default class SpaceActivationAvatarTranslucenceMediator implements IMediator, IEnterFrame {
    // 正常显示的人物
    ...
    public enterFrame() {
        this._frameCount++;
        if (this._frameCount > SpaceActivationAvatarTranslucenceMediator.CheckFrame) {
            this._frameCount = 0;
            ...
            if (dic && this.selfArmy) {
                ...
                for (let index = 0; index < tempArr.length; index++) {
                    ...
                    if (index < SpaceActivationAvatarTranslucenceMediator.NormalShowAvatarCnt) {
    ...
    private calDistance(armyInfo: SpaceArmy): number {
        ...
        let self: Laya.Point = new Laya.Point(this.selfArmy.curPosX, this.selfArmy.curPosY);
