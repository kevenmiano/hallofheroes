...
import { RoomEvent, RoomPlayerItemType, RoomSceneType } from "../../../constant/RoomDefine";
...
export default class RoomHallWnd extends BaseWindow {
	...
	OnShowWind() {
		...
		NotificationManager.Instance.addEventListener(NotificationEvent.UPDATE_CROSS_PVE_STATUS, this.__updateRoomInfoHanlder, this);
		NotificationManager.Instance.addEventListener(RoomEvent.HIDE_PVP_ROOM_LEFT_TIME, this.stopTime, this);
	}

	/**关闭界面 */
	OnHideWind() {
		super.OnHideWind();
		NotificationManager.Instance.removeEventListener(NotificationEvent.UPDATE_CROSS_PVE_STATUS, this.__updateRoomInfoHanlder, this);
		NotificationManager.Instance.removeEventListener(RoomEvent.HIDE_PVP_ROOM_LEFT_TIME, this.stopTime, this);
	}
	...
	private initView() {
		...
		this.cPvp.selectedIndex = this.ctrl.roomSceneType == RoomSceneType.PVP ? 1 : 0;
		this.initCross();
	...
	private initPlayerItemList() {
		...
		for (let i: number = 0; i < itemCount; i++) {
			...
			if (this.roomInfo) {
				...
				this._playerListPos.add(item, { x: item.x, y: item.y, width: item.width, height: item.height, target: item });
	...
	private initCross() {
		let timeArray: Array<string> = ConfigInfoManager.Instance.getMultiMatchTime();
		let timeStr: string;
		if (timeArray) {
		...
		this.corssActiveDescTxt.text = LangManager.Instance.GetTranslation("roomHallwnd.cross.corssActiveTimeTxt");
	...
	public startTime() {
		...
		this._timeCount = 0;
		this.corssLeftTmeTxt.text = LangManager.Instance.GetTranslation("roomHallwnd.cross.corssLeftTmeTxt", this._timeCount);
	...
	public checkCancelMatch() {
		...
		for (let index = 0; index < this._playerList.length; index++) {
			...
			if (player && player.info && player.info.baseHero && player.info.baseHero.mulSportScore >= 1900) {
				b = true;
				break;
			}
		}
	...
	private setMainTooBalStateReady() {
		HomeWnd.Instance.getMainToolBar().switchToolsBarState(this.mainToolBarReadyState);
	}
	...
	private get mainToolBarReadyState() {
		return this.ctrl.roomSceneType == RoomSceneType.PVE ? MainToolBar.PVE_ROOM_READY : MainToolBar.PVP_ROOM_READY;
	}
