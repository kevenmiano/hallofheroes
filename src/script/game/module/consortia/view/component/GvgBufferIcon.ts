...
export class GvgBufferIcon extends FUI_GvgBufferIcon implements ITipedDisplay {
  ...
  set info(value: GvgWarBufferInfo) {
    ...
    if (this._info) {
      ...
      this._info.addEventListener(
        GvgEvent.UPDATE_CURRENT_COUNT,
        this.__playCdEffectHandler,
        this,
      );
      ...
      this.tweenValue(0, this._info.lefTime)
        .onStart(this.startCd, this)
        .onComplete(this.cdCompleteHandler, this);
  ...
  protected __iconClick(): void {
    ...
    if (!isManager) {
      MessageTipManager.Instance.show(
        LangManager.Instance.GetTranslation("public.Gvgbuffericon.manager"),
      );
      return;
    }
    if (
      this._info.needPay > ConsortiaManager.Instance.model.consortiaInfo.offer
    ) {
    ...
    let content: string = LangManager.Instance.GetTranslation(
      "map.campaign.view.ui.gvg.GvgBufferIcon.useMessage",
      this._info.needPay,
      this._info.bufferNameLang,
    );
    SimpleAlertHelper.Instance.Show(
      ...
      confirm,
      cancel,
      this.__useBufferMessage.bind(this),
    );
  }
  ...
  private __playCdEffectHandler() {
    ...
    fgui.GTween.to(this._info.maxCdTimer, 0, this._info.maxCdTimer)
