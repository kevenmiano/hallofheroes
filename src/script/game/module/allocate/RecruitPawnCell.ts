...
export default class RecruitPawnCell extends FUI_RecruitPawnCell {
    ...
    public setArmyData(value: ArmyPawn) {
        if (this._data) {
            this._data.removeEventListener(PawnEvent.PAWN_PROPERTY_CHAGER, this.__pawnChangeHandler, this);
        }
        this._data = value;
        if (this._data) {
            this._data.addEventListener(PawnEvent.PAWN_PROPERTY_CHAGER, this.__pawnChangeHandler, this);
        }
        this.refreshView();
        this.updateIconData();
    }
    ...
    private addEvent() {
        ...
        ArmyManager.Instance.addEventListener(ServiceReceiveEvent.UPGRADE_PAWN_SUCCESS, this.__pawnLevelUpHandler, this);
        ArmyManager.Instance.addEventListener(ArmyEvent.ARMY_INFO_CHANGE, this.__pawnChangeHandler, this);
    }
    ...
    private removeEvent() {
        ...
        ArmyManager.Instance.removeEventListener(ServiceReceiveEvent.UPGRADE_PAWN_SUCCESS, this.__pawnLevelUpHandler, this);
        ArmyManager.Instance.removeEventListener(ArmyEvent.ARMY_INFO_CHANGE, this.__pawnChangeHandler, this);
    }
    ...
    private __recruitPawnBtn() {
        ...
        let maxEffectBuildingArray: any[] = BuildingManager.Instance.getMaxEffectBuildingTemplate(this.pawnTemp);
        maxEffectBuildingArray = ArrayUtils.sortOn(maxEffectBuildingArray, "Property4", ArrayConstant.NUMERIC);
        let buildTemp: t_s_buildingtemplateData = <t_s_buildingtemplateData>maxEffectBuildingArray[0];
        let race: number = buildTemp.Property4 / 100;
        let goldValue: number = this._model.playerEffect.getRecruitPawnResourceAddition(this.pawnTemp.GoldConsume * (1 - race));
        let str: string = "";
        if (number <= 0) {
            if (BuildingManager.Instance.getBuildingInfoBySonType(BuildingType.HOUSES).templeteInfo.NextGradeTemplateId == 0) {
                str = LangManager.Instance.GetTranslation("buildings.casern.view.RecruitPawnCell.command03");
                MessageTipManager.Instance.show(str);
            } else {
                str = LangManager.Instance.GetTranslation("buildings.casern.view.RecruitPawnCell.command04");
                MessageTipManager.Instance.show(str);
            }
    ...
    private _playerComClickHander() {
        if (UIManager.Instance.isShowing(EmWindow.SoliderInfoTipWnd)) {//如果有弹窗
            ...
            UIManager.Instance.ShowWind(EmWindow.SoliderInfoTipWnd, { posX: point.x, posY: point.y, pawnData: this._data, type: 3 });
        }
    }
    ...
    private refreshView() {
        ...
        let b: t_s_buildingtemplateData = TempleteManager.Instance.getBuildTemplateByID(parseInt(this.pawnTemp.NeedBuilding.toString()));
        let buildInfo: BuildInfo = b ? BuildingManager.Instance.getBuildingInfoBySonType(b.SonType) : null;
        let flag: boolean = buildInfo ? buildInfo.templeteInfo.BuildingGrade >= b.BuildingGrade : true;
        let ap = ArmyManager.Instance.army.getPawnByIndex(0);
        if (ap && ap.ownPawns > 0) {
            this.isEquip.selectedIndex = (ap.templateId == this.pawnTemp.TemplateId) ? 1 : 0;
        } else {
            this.isEquip.selectedIndex = 0;
        }
        if (this._data.canRecruit && flag) {
            this.unLock.selectedIndex = 0;
            this.titleNameTxt.text = this.pawnTemp.PawnNameLang + ' ' + LangManager.Instance.GetTranslation("buildings.casern.view.RecruitPawnCell.command06", this.pawnTemp.Level);
            this.number = this._data.ownPawns;
            this.needOpenGradeTxt.text = "";
            let onPawns = this._data.ownPawns + ArmyManager.Instance.getCasernOnPawn(this.pawnTemp.SonType)
            if (onPawns > 0)
                this.ownCount.text = LangManager.Instance.GetTranslation("buildings.casern.view.RecruitPawnCell.command07", onPawns);
            else
                this.ownCount.text = "";
            ...
            this.titleNameTxt.text = this.pawnTemp.PawnNameLang + ' ' + LangManager.Instance.GetTranslation("buildings.casern.view.RecruitPawnCell.command06", this.pawnTemp.Level);
            this.needOpenGradeTxt.text = LangManager.Instance.GetTranslation("buildings.casern.view.RecruitPawnCell.command05", this.getNeedBuilding(parseInt(this._data.templateInfo.NeedBuilding.toString())));
            this.ownCount.text = "";
    ...
    private getNeedBuilding(id: number): string {
        let b: t_s_buildingtemplateData = TempleteManager.Instance.getBuildTemplateByID(id);
        return LangManager.Instance.GetTranslation("public.level.name", b.BuildingNameLang, b.BuildingGrade);
    }
