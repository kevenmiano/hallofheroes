...
export default class ExchangeRandomItem extends FUI_OuterCityShopItem {
  ...
  protected onConstruct() {
    ...
    this.txt_best.text = LangManager.Instance.GetTranslation(
      "ExchangeRandomItem.txt_best",
    );
  ...
  refreshView() {
    this._goodsTempInfo =
      TempleteManager.Instance.getGoodsTemplatesByTempleteId(
        parseInt(this._itemInfo.itemTemInfo.ItemId),
      );
    ...
    if (this._itemInfo.itemTemInfo.IsDrop == 1) {
      //是精品物品
    ...
    if (this._itemInfo.isBuy || grades < this._itemInfo.itemTemInfo.MinLevel) {
      //已经购买
      ...
      // 没有购买过的符文石
      if (
        this._goodsTempInfo.SonType == GoodsSonType.SONTYPE_PASSIVE_SKILL &&
        this._goodsTempInfo.Property1 > 0
      ) {
        let state = OuterCityShopManager.instance.model.getRuneState(
          this._goodsTempInfo,
        );
    ...
    if (this._itemInfo.itemTemInfo.Type == OuterCityShopModel.RANDOM_SHOP) {
      ...
      if (
        ...
        this.txt_limit.text =
          LangManager.Instance.GetTranslation("Shop.Promotion.weeklimit") +
          `${limit - buyCount}/${limit}`;
      }
    }

    if (
      this._itemInfo.itemTemInfo.MinLevel > ArmyManager.Instance.thane.grades
    ) {
      //等级不满足
    ...
    for (let i: number = 0; i < this._itemInfo.costInfos.length; i++) {
      ...
      if (costInfo[0] == "Score") {
        ...
        this._btn1.icon = FUIHelper.getItemURL(
          EmPackName.Base,
          "Icon_Unit_Points",
        );
      } else {
