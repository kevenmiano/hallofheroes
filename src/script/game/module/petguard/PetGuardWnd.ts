...
export default class PetGuardWnd extends BaseWindow {
    ...
    public OnInitWind() {
        ...
        this.frame.getChild('title').text = LangManager.Instance.GetTranslation('TopToolBar.petGuard');
        this.btn_go.title = LangManager.Instance.GetTranslation('gameguide.GameGuideFrame.skipNow');
        this.txt1.text = LangManager.Instance.GetTranslation('petboss.PetbossItem.nextBossAlert');
        this.txt2.text = LangManager.Instance.GetTranslation('petboss.PetbossItem.describe');
        this.addEvent();
    ...
    private addEvent(){
        ...
        PlayerManager.Instance.addEventListener(PlayerEvent.PET_BOSS,this.updateSvrTime,this);
    ...
    updateSvrTime(){
        PlayerManager.Instance.removeEventListener(PlayerEvent.PET_BOSS,this.updateSvrTime,this);
        this.leftTimeNum = this.petBossModel.getNearlyOpenTimeSecond();
    }

    private showBossInfo(){
        this.c1.setSelectedIndex(1);
        var heroTemp:t_s_herotemplateData = TempleteManager.Instance.gameHeroTemplateCate(this.petBossModel.heroId);
        if(heroTemp)
        {
    ...
    private onGo():void
    {
        if(this.petBossModel.isOpen)
        {
            ...
            {
                let str = LangManager.Instance.GetTranslation("room.view.pve.RoomRightView.command03") + LangManager.Instance.GetTranslation("public.level4_space2",OpenGrades.PET);
                MessageTipManager.Instance.show(str);
            }else
            {
                SwitchPageHelp.walkToCrossMapTarget(this.petBossModel.mapId + "," +this.petBossModel.nodeId);
            }
        }else
        {
    ...
    removeEvent(){
        PlayerManager.Instance.removeEventListener(PlayerEvent.PET_BOSS,this.updateSvrTime,this);
        MsgMan.removeObserver(PetEvent.PET_BOSS_ITEM_CLICK,this,this.__clickHandler);
