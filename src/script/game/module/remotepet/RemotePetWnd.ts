...
export class RemotePetWnd extends BaseWindow {
    ...
    private addEvent() {
        this.chaptertTree.treeNodeRender = Laya.Handler.create(this, this.onTreeRenderer, null, false);
        this.chaptertTree.on(fgui.Events.CLICK_ITEM, this, this.onClickTreeItem);
        this.model.addEventListener(RemotePetEvent.COMMIT, this.onCommitHandler, this);
        PlayerManager.Instance.currentPlayerModel.playerInfo.addEventListener(PlayerEvent.WEARY_CHANGE, this.updatePower, this);
    }

    private removeEvent() {
        this.model.removeEventListener(RemotePetEvent.COMMIT, this.onCommitHandler, this);
        PlayerManager.Instance.currentPlayerModel.playerInfo.removeEventListener(PlayerEvent.WEARY_CHANGE, this.updatePower, this);
    }
    ...
    private initTree(curChater: number = 0) {
        ...
        for (let i = 0; i < turnList.length; i++) {
            ...
            if (hasChild) {
                topNode = treeNode;
                topNode.data = LangManager.Instance.GetTranslation("remotepet.chapter" + chapterIndex);
    ...
    private onTreeRenderer(node: fgui.GTreeNode, item: RemotePetChapterFold | RemotePetChapterItem) {
        item.info = node.data;
    ...
    private updatePower() {
        ...
        if (ArmyManager.Instance.army) {
            text = PlayerManager.Instance.currentPlayerModel.playerInfo.weary + "/200";
        }
