 ...
 export class SFortuneGuardCom extends FUI_SFortuneGuardCom{
    ...
    checkRedDot(){
        if(this.item0.itemData){
            this.modeList.getChildAt(0).asButton.getChild('redDot').visible = this.canLevelUp(this.item0.itemData);
            this.modeList.getChildAt(1).asButton.getChild('redDot').visible = this.canLevelUp(this.item1.itemData);
        }
    }
    ...
    private addEvent(){
        ...
        ServerDataManager.listen(S2CProtocol.U_C_FATE_REQUEST, this, this.__resuleHandler);
    }

    private removeEvent(){
        this.fateIcon.offClick(this, this.onShowTip);
        this.modeList.off(fgui.Events.CLICK_ITEM, this, this.__clickModeItem);
        ServerDataManager.cancel(S2CProtocol.U_C_FATE_REQUEST, this, this.__resuleHandler);
    }
    ...
    public refreshRunCircleView() {
        ...
        this.atkLab.text = this.txt_lv0.text =  LangManager.Instance.GetTranslation("public.level3", grade0);
        this.protectLab.text = this.txt_lv1.text = LangManager.Instance.GetTranslation("public.level3", +grade1);
    }
    ...
    private helpBtnClick() {
        ...
        let content = LangManager.Instance.GetTranslation("DragonSoulView.HelpContent");
