...
export default class ActivityManager {
  ...
  private initEvent() {
    ServerDataManager.listen(
      S2CProtocol.U_C_ACTIVE,
      this,
      this.__activityHandler,
    );
  }
  ...
  private __activityHandler(pkg: PackageIn) {
    ...
    for (var i: number = 0; i < len; i++) {
      ...
      info.endDate =
        DateFormatter.parse(arr[i].endDate, "YYYY-MM-DD hh:mm:ss").getTime() /
        1000;
      info.startDate =
        DateFormatter.parse(arr[i].startDate, "YYYY-MM-DD hh:mm:ss").getTime() /
        1000;
    ...
    this._activityList = ArrayUtils.sortOn(
      this._activityList,
      "sort",
  ...
  private __onLoadError(info, content) {
    throw new ErrorEvent(
      LangManager.Instance.GetTranslation("loader.LoadError.Actionmsg"),
    );
  }
  ...
  private returnResult(arr: Array<string>) {
    ...
    if (arr[0] == "-1") {
      switch (arr[arr.length - 1]) {
        case "0":
          str = LangManager.Instance.GetTranslation(
            "activity.ActivityManager.command01",
          );
          break;
        case "1":
          str = LangManager.Instance.GetTranslation(
        ...
        case "6":
          str = LangManager.Instance.GetTranslation(
            "activity.ActivityManager.command04",
          );
          break;
        case "7":
          str = LangManager.Instance.GetTranslation(
            "activity.ActivityManager.command05",
          );
          break;
        case "8":
          str = LangManager.Instance.GetTranslation(
            "activity.ActivityManager.command06",
          );
          break;
        case "10":
          str = LangManager.Instance.GetTranslation(
            "activity.ActivityManager.command07",
          );
          break;
        case "11":
          str = LangManager.Instance.GetTranslation(
            "activity.ActivityManager.command08",
          );
          break;
      }
    } else {
      switch (arr[arr.length - 1]) {
        ...
        case "2":
          str = LangManager.Instance.GetTranslation(
            "activity.ActivityManager.command10",
          );
          break;
        case "1":
        case "3":
        ...
        case "5":
          str = LangManager.Instance.GetTranslation(
            "activity.ActivityManager.command09",
          );
          break;
        case "6":
          str = LangManager.Instance.GetTranslation(
