...
export default class LlkModel {
  ...
  private oneCorner(
    ...
    var corner: LlkNodeData =
      this._list[node1.row * LlkModel.COLUMN + node2.col];
    if (corner.val == 0) {
  ...
  private connectCol(
    ...
    for (var i: number = min + 1; i < max; i++) {
      if (
        this._list[i * LlkModel.COLUMN + node1.col] &&
        this._list[i * LlkModel.COLUMN + node1.col].val > 0
      )
        return false;
  ...
  private connectRow(
    ...
    for (var i: number = min + 1; i < max; i++) {
      if (
        this._list[node1.row * LlkModel.COLUMN + i] &&
        this._list[node1.row * LlkModel.COLUMN + i].val > 0
      )
        return false;
