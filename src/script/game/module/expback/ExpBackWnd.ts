...
export default class ExpBackWnd extends BaseWindow {
  ...
  private refreshView() {
    ...
    this.countTxt.text =
      GoodsManager.Instance.getGoodsNumByTempId(ExpBackWnd.RECOVER_ID) + "";
  ...
  private extraGetBtnHandler() {
    ...
    let content: string = LangManager.Instance.GetTranslation(
      "ExpBackItem.advBtnHandler.tips",
      needPoint,
    );
    if (this.hasCount < this._needCount) {
      //材料不足
  ...
  alertCallback(noAlert: boolean, isCheck: boolean) {
    if (isCheck) {
      ...
      if (
        ...
        this._hasNeedCount * this._price
      ) {
        //钻石总量不够
        MessageTipManager.Instance.show(
          LangManager.Instance.GetTranslation(
            "ExpBackShowTipsWnd.diamond.notEnough",
          ),
        );
        return;
      }
