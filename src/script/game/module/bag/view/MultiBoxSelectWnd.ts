...
import { MultiBoxSelectEvent, QuestionEvent } from "../../../constant/event/NotificationEvent";
...
export default class MultiBoxSelectWnd extends BaseWindow {
    ...
    private addEvent() {
        ...
        this.list.itemRenderer = Laya.Handler.create(this, this.renderChannelListItem, null, false);
        NotificationManager.Instance.addEventListener(MultiBoxSelectEvent.FOCUS, this.disableScroll, this);
        NotificationManager.Instance.addEventListener(MultiBoxSelectEvent.BLUR, this.enableScroll, this);
    }

    private offEvent() {
        this.btn_sure.offClick(this, this.onComfirmHandler);
        this.btn_cancel.offClick(this, this.OnBtnClose);
        if (this.list.itemRenderer) { this.list.itemRenderer.recover(); this.list.itemRenderer = null; }
        NotificationManager.Instance.removeEventListener(MultiBoxSelectEvent.FOCUS, this.disableScroll, this);
        NotificationManager.Instance.removeEventListener(MultiBoxSelectEvent.BLUR, this.enableScroll, this);
    }
    ...
    private initView() {
        ...
        this._countHandler = Laya.Handler.create(this, this._countChangeHandler, null, false);
        ...
        this.selectedText.setVar('max', this._selectMaxCount.toString()).flushVars(); //可选择多少个
        let itemlist: t_s_dropitemData[] = TempleteManager.Instance.getDropItemssByDropId(dropId);
    ...
    private renderChannelListItem(index: number, item: MulitBoxSelectItem) {
        if (item && !item.isDisposed) {
            item.setItemData(index, this._itemListData[index], this._openCount, this._countHandler);
        }
    }

    /**增加或减少*/
    private _countChangeHandler(item: MulitBoxSelectItem, Id: number, count: number) {
    ...
    private updateItemsEnable() {
        ...
        for (let index = 0; index < itemsCount; index++) {
            let cellitem: MulitBoxSelectItem = this.list.getChildAt(index) as MulitBoxSelectItem;
    ...
    private updateSelectCount() {
        ...
        if (countBoxNumber > this._selectMaxCount) {
            this.selectedText.setVar('count', `[color=#FF2E2E]${countBoxNumber}[/color]`).flushVars();//已选择数量
        } else {
            this.selectedText.setVar('count', `[color=#FFECC6]${countBoxNumber}[/color]`).flushVars();//已选择数量
        }
        this.btn_sure.enabled = (countBoxNumber == this._selectMaxCount) && (selectboxCount == this._canSelectCount);
    }
