...
export class RemotePetSkillLevelUpItem extends FUI_RemotePetSkillLevelUpItem {
    ...
    private updateView() {
        ...
        this._icon.url = IconFactory.getCommonIconPath(this.skillInfo.templateInfo.Icons);
        this._skillName.text = this.skillInfo.templateInfo.SkillTemplateName;
        this._skillCD.text = this.skillInfo.templateInfo.CoolDown / 1000 + LangManager.Instance.GetTranslation("public.time.second");
        this._cost.text = LangManager.Instance.GetTranslation("remotepet.skill.point", this.skillInfo.templateInfo.Cost);
        ...
        let remotePetSkillStatus = RemotePetManager.Instance.model.remotePetSkillStatus
        // if (!this.isSub) {
        //     this.addEvent();
        // }
        //可加点
        ...
        if (this.skillInfo.grade == 0) {
            ...
            let turnInfo = TempleteManager.Instance.getRemotePetTurnBySkill(this.skillInfo.templateId);
            this._unlockTxt.text = LangManager.Instance.GetTranslation("remotepet.RemotePetLevelUp.unlock", turnInfo.IndexID);
        } else {
            ...
            let petTurns = TempleteManager.Instance.getRemotePetTurnsBySkill(this.skillInfo.templateId);
            ...
            this._getSkillCountTipTxt.setVar("level", nextAddSkillTurn + "").flushVars();
    ...
    private checkCanAdd(): boolean {
        ...
        if (GoodsManager.Instance.getGoodsNumByTempId(this.skillInfo.nextTemplateInfo.UpgradeNeedItem) < this.skillInfo.nextTemplateInfo.UpgradeNeedCount) return false;
        return true
    }
