...
export class ConsortiaControler extends FrameCtrlBase {
  ...
  protected addEventListener() {
    ...
    ServerDataManager.listen(
      S2CProtocol.U_C_CONSORTIA_INVITEDEL,
      this,
      this.__deleteApplyConsortiaHandler,
    );
    ServerDataManager.listen(
    ...
    ServerDataManager.listen(
      S2CProtocol.U_C_CONSORTIA_OFF,
      this,
      this.__consortiaContributeResult,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_CONSORTIA_PASS,
      this,
      this.__consortiaAcceptApplyHandler,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_CONSORTIA_FRESHALTAR,
      this,
      this.__getAltarBlessInfo,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_CONSORTIA_OFFER_LIST,
      this,
      this.__updataOfferList,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_EVENT_LIST,
      this,
      this.__refreshEventList,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_CONSORTIA_SEARCH,
      this,
      this.__refreshConsortiaList,
    );

    ServerDataManager.listen(
      S2CProtocol.U_C_CONSORTIA_ALTERINFO,
      this,
      this.__refreshAltarInfo,
    );

    ServerDataManager.listen(
      S2CProtocol.U_C_CONSORTIA_SITE,
      this,
      this.__setGoods,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_CONSORTIA_ADDFRESH,
      this,
      this.__addFreshItem,
    );
    //选举
    ServerDataManager.listen(
      S2CProtocol.U_C_CONSORTIA_VOTING_LIST,
      this,
      this.__setVotingUsers,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_CONSORTIA_VOTING,
      this,
      this.__updateVotingState,
    );
  }
  ...
  private outOfDate(): boolean {
    ...
    let intervalTime: number =
      nowDate.getTime() - this._model.consortiaInfo.warnDate.getTime();
  ...
  private __refreshConsortiaList(pkg: PackageIn) {
    let msg: ConsortiaSearchRspMsg = pkg.readBody(
      ConsortiaSearchRspMsg,
    ) as ConsortiaSearchRspMsg;
    let arr: ConsortiaInfo[] = [];
    for (let i: number = 0; i < msg.consortiaInfo.length; i++) {
      arr.push(
        this.readConsortiaInfo(msg.consortiaInfo[i] as ConsortiaInfoMsg),
      );
    }
  ...
  private __joinConsortiaHandler(pkg: PackageIn) {
    ...
    if (id) {
      ...
      NotificationManager.Instance.dispatchEvent(
        ConsortiaEvent.APPLY_CONSORTIA_SUCCESS,
      );
    }
  }
  ...
  private __operateConsortiaApplyResult(pkg: PackageIn) {
    ...
    for (const key in this._model.inviteList) {
      if (this._model.inviteList.hasOwnProperty(key)) {
        ...
        if (item.id == id) {
          this._model.inviteList.splice(
            this._model.inviteList.indexOf(item),
            1,
          );
          this._model.dispatchEvent(ConsortiaEvent.UPDA_RECORD_LIST, {
            bInvite: true,
          });
          break;
        }
  ...
  public creatConsortia(consortiaName: string) {
    ...
    if (
      ...
      let content: string = LangManager.Instance.GetTranslation(
        "Consortia.ConsortiaSocketOutManager.content2",
        point,
      );
  ...
  public sendGetConsortiaOffer() {
    ConsortiaSocketOutManager.getConsortiaOffer(
      this._model.contributeListCurrentPage,
      this._model.isHistory,
    );
  }

  private __updataOfferList(pkg: PackageIn) {
    let msg: ConsortiaContributeRspMsg = pkg.readBody(
      ConsortiaContributeRspMsg,
    ) as ConsortiaContributeRspMsg;
    let consortiaContributionList: ConsortiaUserInfo[] = [];
    this._model.contributeListTotalPage = Math.ceil(
      msg.totalRows / ConsortiaModel.CONTRIBUTE_RECORD_PAGE_NUM,
    );
  ...
  private copyProperty(
    uInfo1: ConsortiaUserInfo,
    uInfo2: ConsortiaContributeMsg,
  ) {
  ...
  public contributeSearchPageHandler(isLastPage: boolean) {
    if (isLastPage && this._model.contributeListCurrentPage > 1) {
      ...
      !isLastPage &&
      this._model.contributeListCurrentPage <
        this._model.contributeListTotalPage
    ) {
      this._model.contributeListCurrentPage += 1;
    } else {
      return;
  ...
  public consortiaUpgrade($type: object) {
    if ($type instanceof t_s_consortialevelData) {
      ConsortiaSocketOutManager.consortiaUpgrade(
        ($type as t_s_consortialevelData).Types,
      );
    } else {
  ...
  private __refreshEventList(pkg: PackageIn) {
    let msg: ConsortiaEventListRspMsg = pkg.readBody(
      ConsortiaEventListRspMsg,
    ) as ConsortiaEventListRspMsg;
    ...
    this._model.consortiaEventList = ArrayUtils.sortOn(
      arr,
      "createDate",
  ...
  private readEventInfo(info: ConsortiaEventInfoMsg): ConsortiaEventInfo {
    ...
    eventInfo.createDate = DateFormatter.parse(
      info.createDate,
      "YYYY-MM-DD hh:mm:ss",
    );
  ...
  public sendGetConsortiaApplyInfos() {
    ConsortiaSocketOutManager.getConsortiaApplyInfos(
      this._model.recruitCurrentPage,
    );
  }
    ...
    for (const key in $sInfoArr) {
      if ($sInfoArr.hasOwnProperty(key)) {
        ...
        if (Item.consortiaID && $flag) {
          let str: string = LangManager.Instance.GetTranslation(
            "consortia.ConsortiaControler.command08",
          );
          MessageTipManager.Instance.show(str);
        } else {
          ConsortiaSocketOutManager.consortiaAcceptOrRejectApply(
            Item.parameter1,
            $flag,
          );
  ...
  public getShowSkills() {
    for (let i: number = 1; i <= this.model.consortiaInfo.schoolLevel; i++) {
      let temp: t_s_consortialevelData =
      ...
      if (temp.NextTemplateId) {
        ...
        for (let j: number = 0; j < arr.length; j++) {
          let nextTemp: t_s_consortialevelData =
            TempleteManager.Instance.getConsortiaTempleteById(arr[j]);
          if (nextTemp) {
  ...
  public getShowHighSkills() {
    for (
      let i: number = ConsortiaModel.HIGH_SKILL_TYPE_MIN;
      i <= ConsortiaModel.HIGH_SKILL_TYPE_MAX;
      i++
    ) {
      let temp: t_s_consortialevelData =
        TempleteManager.Instance.getConsortiaTempleteByTypeAndLevel(i, 1);
      if (temp) {
  ...
  private sortBaseSkill(
    nextTemp: t_s_consortialevelData,
    ...
    if (!this._model.baseSkillList.hasOwnProperty("type_" + nextTemp.Types)) {
      ...
      tInof.isNext = isNext;
      tInof.level = this._model.consortiaInfo.getLevelByUpgradeType(
        nextTemp.Types,
      );
    }
  }
  ...
  private ifCanUpdata(temp: t_s_consortialevelData): boolean {
    ...
    let nextTemp: t_s_consortialevelData;
    if (
      temp.Types >= ConsortiaModel.HIGH_SKILL_TYPE_MIN &&
      temp.Types <= ConsortiaModel.HIGH_SKILL_TYPE_MAX
    ) {
      //高级技能
      level =
        PlayerManager.Instance.currentPlayerModel.getConsortiaHighSkillLevel(
      ...
      //     return false;
      // }
      nextTemp = TempleteManager.Instance.getConsortiaTempleteByTypeAndLevel(
        temp.Types,
        level + 1,
      );
      ...
      level = PlayerManager.Instance.currentPlayerModel.getConsortiaSkillLevel(
        temp.Types,
      );
      if (level >= ConsortiaUpgradeType.MAX_LEVEL) {
        let str: string = LangManager.Instance.GetTranslation(
          "consortia.ConsortiaControler.command09",
        );
        MessageTipManager.Instance.show(str);
        return false;
      }
  ...
  public sendConsortiaInvitePlayer($id: number) {
    ...
    let str: string = LangManager.Instance.GetTranslation(
      "consortia.ConsortiaControler.command10",
    );
    MessageTipManager.Instance.show(str);
  }
  ...
  private __setGoods(pkg: PackageIn) {
    let msg: ConsortiaInfoMsg = pkg.readBody(
      ConsortiaInfoMsg,
    ) as ConsortiaInfoMsg;
    NotificationManager.Instance.dispatchEvent(
      ConsortiaEvent.UPDE_ALTAR_SIZE,
      msg.site,
    );
  }
  ...
  private __setVotingUsers(pkg: PackageIn) {
    ...
    let msg: ConsortiaVotingUsersMsg = pkg.readBody(
      ConsortiaVotingUsersMsg,
    ) as ConsortiaVotingUsersMsg;

    let userInfos: ConsortiaVotingUserInfo[] = [];
    for (const key in msg.info) {
      if (msg.info.hasOwnProperty(key)) {
        let userMsg: ConsortiaVotingUserMsg = msg.info[
          key
        ] as ConsortiaVotingUserMsg;
        ...
        if (userInfo.userId == userID) {
          Logger.yyz(
            ...
            userInfo.votingId,
            "  公会投票序号: ",
            this._model.consortiaInfo.votingId,
          );
  ...
  private __updateVotingState(pkg: PackageIn) {
    Logger.yyz("接受投票后的信息");
    let msg: ConsortiaVotingRspMsg = pkg.readBody(
      ConsortiaVotingRspMsg,
    ) as ConsortiaVotingRspMsg;

    this._model.hasVoted = msg.result;
    let tip: string;
    if (msg.result) {
      tip = LangManager.Instance.GetTranslation(
        "consortia.view.myConsortia.chairmanElection.successTxt",
      );
    } else {
      tip = LangManager.Instance.GetTranslation(
        "consortia.view.myConsortia.chairmanElection.failTxt",
      );
    }
  ...
  public openDevilFrame() {
    ...
    // }
    // _devilFrame["show"]();
    let tip = LangManager.Instance.GetTranslation(
      "mvc.controler.KingContractControler.ConfigTipTxt",
    );
