...
export class FarmManager {
  ...
  private addEvent() {
    ServerDataManager.listen(
      S2CProtocol.U_C_FARM_INFO,
      this,
      this.__getFarmInfoHandler,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_FARMLAND_INFO,
      this,
      this.__farmlandUpdateHandler,
    );
  ...
  public static sendLandUp(landGrade: number) {
    ...
    let tempValue = 1;
    let cfg =
      TempleteManager.Instance.getConfigInfoByConfigName("Land_Up_Mulriple");
    if (cfg) {
      tempValue = Number(cfg.ConfigValue);
    }
    var tip: string = LangManager.Instance.GetTranslation(
      "farm.FarmManager.landUpPayTip",
      cost * tempValue,
      landGrade + 1,
    );
    SimpleAlertHelper.Instance.Show(
      SimpleAlertHelper.USEBINDPOINT_ALERT,
      { point: cost, checkDefault: true },
      null,
      tip,
      null,
      null,
      FarmManager.__sendLandUpBack.bind(this),
    );
  }
  /**
   * 土地升级确认或者取消支付的回调
   ...
   * @param type: 支付类型
   */
  private static __sendLandUpBack(
    b: boolean,
    flag: boolean,
    objectId: number = 0,
    type: number = 0,
    ...
    if (info && !info.isMine) {
      ...
      for (const key in landlist) {
        if (Object.prototype.hasOwnProperty.call(landlist, key)) {
          ...
          if (this.checkHasOperNow(land, FarmOperateType.STEAL))
            canSteal = true;
          if (this.checkHasOperNow(land, FarmOperateType.REVIVE))
            canRevive = true;
  ...
  private updateFriendFarmStateInfo(curSelectedFarm: FarmInfo) {
    ...
    for (const key in curSelectedFarm.getPetLandList()) {
      if (
        Object.prototype.hasOwnProperty.call(
          curSelectedFarm.getPetLandList(),
          key,
        )
      ) {
    ...
    var fsInfo: FriendFarmStateInfo =
      FarmManager.Instance.model.getFarmStateInfo(curSelectedFarm.userId);
    if (fsInfo) {
      fsInfo.beginChanges();
      fsInfo.canFeed = canFeed;
      fsInfo.commitChanges();
  ...
  private inSpecialCampaign(): boolean {
    if (SceneManager.Instance.currentType == SceneType.CAMPAIGN_MAP_SCENE) {
      if (CampaignManager.Instance.mapModel) {
        ...
        if (
          WorldBossHelper.checkHoodRoom(mapId) ||
    ...
    if (this.thane.grades >= OpenGrades.FARM) {
      if (
        ...
        if (mainBar) {
          let b =
            this.checkHasPick ||
            this.checkHasPickTree ||
            this.checkExistEmptyLand(this.model.myFarm) ||
            this.checkExistDie(this.model.myFarm);
  ...
  public checkExistSpecialCrop(fInfo: FarmInfo): boolean {
    ...
    for (const key in list) {
      if (Object.prototype.hasOwnProperty.call(list, key)) {
        const land = list[key];
        if (land.hasCrop && land.cropTemp && land.cropTemp.Property5 > 0)
          return true;
      }
    }
    return false;
   ...
   * 检查是否存在相同特殊作物
   */
  public checkExistSameSpecialCrop(
    gtemp: t_s_itemtemplateData,
    fInfo: FarmInfo,
  ): boolean {
    ...
    for (let key in list) {
      if (Object.prototype.hasOwnProperty.call(list, key)) {
        ...
        if (
          land.hasCrop &&
          land.cropTemp &&
          land.cropTemp.Property5 > 0 &&
          land.cropTemp.Property5 == gtemp.Property5
        )
          return true;
      }
    }
  ...
  public addChatTip(
    ...
    var str: string = "";
    var expAddStr: string =
      addExp > 0
        ? LangManager.Instance.GetTranslation(
            "farm.FarmManager.expAddTip",
            addExp,
          )
        : "";
    switch (type) {
        ...
        switch (oper) {
          case FarmOperateType.WORM:
            str = LangManager.Instance.GetTranslation(
              "farm.FarmManager.wormTip1",
              expAddStr,
            );
            break;
          case FarmOperateType.WEED:
            str = LangManager.Instance.GetTranslation(
              "farm.FarmManager.weedTip1",
              expAddStr,
            );
            break;
        }
      ...
      case 2:
        ...
        var fInfo: ThaneInfo =
          FriendManager.getInstance().getFriendById(othersId);
        if (fInfo) {
        ...
        switch (oper) {
          case FarmOperateType.WORM:
            str = LangManager.Instance.GetTranslation(
              "farm.FarmManager.wormTip2",
              expAddStr,
            );
            break;
          case FarmOperateType.WEED:
            str = LangManager.Instance.GetTranslation(
              "farm.FarmManager.weedTip2",
              expAddStr,
            );
            break;
          case FarmOperateType.REVIVE:
            str = LangManager.Instance.GetTranslation(
              "farm.FarmManager.reviveTip2",
              expAddStr,
            );
            break;
        }
  ...
  public loadFarmRes() {
    ResMgr.Instance.loadFairyGui(
      EmPackName.Farm,
      this.__enterFarmComplete.bind(this),
      this.__enterFarmProgress.bind(this),
    );
  }
