...
export class NpcArriveMediator implements IMediator, IEnterFrame {
    ...
    public enterFrame() {
        ...
        this._npcPoint.x = this._npc.x; this._npcPoint.y = this._npc.y;
        ...
        for (const key in nList) {
            if (nList.hasOwnProperty(key)) {
                ...
                if ((nInfo.info.types != PosType.COPY_NPC && nInfo.info.types != PosType.BOMBER_MAN) ||
                    nInfo.info.state != NodeState.EXIST)
                    continue;
                this._targetPoint.x = nInfo.nodeView.x; this._targetPoint.y = nInfo.nodeView.y;
                if (this._npcPoint.distance(this._targetPoint.x, this._targetPoint.y) < NpcEffectiveState.TOWER_RADIUS) {
                    (<NpcAvatarView>nInfo.nodeView).aiInfo.pathInfo = [];
                    if (isNaN(nInfo.createTime) ||
                        (PlayerManager.Instance.currentPlayerModel.sysCurTimeBySecond - nInfo.createTime / 1000 > 30)) {//怪物出生起码30秒后才能走到祭坛
                        ...
                        if (this._errorCount < 1) {
                            var time: number = PlayerManager.Instance.currentPlayerModel.sysCurTimeBySecond - nInfo.createTime / 1000;
                            this._errorCount++;
                        }
                    }
                }
    ...
    public register(target: Object) {
        ...
        if (this._npc && this._npc.info && this._npc.info.info.types == PosType.TOWER_DEFENCE) {
