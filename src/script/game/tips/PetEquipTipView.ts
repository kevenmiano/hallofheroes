...
export class PetEquipTipView extends FUI_PetEquipTipView {
  ...
  declare public comp_skillItem: PetSkillItem;
  ...
  set info(value: GoodsInfo) {
    ...
    this._petequipstrengthenDatas = ConfigMgr.Instance.getDicSync(
      ConfigType.t_s_petequipstrengthen,
    );

    if (this._info.templateInfo.Property2) {
      this._dropitemDatas = TempleteManager.Instance.getDropItemssByDropId(
        this._info.templateInfo.Property2,
      );
      if (this._dropitemDatas) {
        let dropItemIds = this._dropitemDatas.map((item) => item.ItemId);
        this._petequipattrDatas = ConfigMgr.Instance.getDicSync(
          ConfigType.t_s_petequipattr,
        );
        this._petequipattrDatas = dropItemIds.map(
          (key: string | number) => this._petequipattrDatas[key],
        );
      }
    }
    ...
    this.gradeCount = 0;
    if (
      this.petCtrl.selectedPet &&
      this._info.objectId == this.petCtrl.selectedPet.petId
    ) {
  ...
  getRefreshArr() {
    ...
    for (let index = 1; index <= this._info.strengthenGrade; index++) {
      let cfg: t_s_petequipstrengthenData =
        TempleteManager.Instance.getPetEquipStrenData(index);
      let curCfg: t_s_petequipstrengthenData =
        TempleteManager.Instance.getPetEquipStrenData(index);
  ...
  private initView() {
    ...
    this.txt_part_val.text = LangManager.Instance.GetTranslation(
      "petEuip.part" + this._info.templateInfo.SonType,
    );
    let cfg: t_s_petequipsuitData =
      TempleteManager.Instance.getPetEquipSuitData(this._info.suitId);
    ...
    if (hasPetSuitTemp) {
      ...
      for (let i = 0; i < bagArr0.length; i++) {
        ...
        if (
          goodsInfo.suitId == this._info.suitId &&
          petId == goodsInfo.objectId
        ) {
          equipNum++;
        }
      }
      ...
      if (cfg.Amount <= 1) {
        this.txt_suit_name.text = LangManager.Instance.GetTranslation(
          "PetEquipTipView.suitSkill",
        );
        let skillTemp = ConfigMgr.Instance.getTemplateByID(
        ...
        this.txt_suit_name.text =
          cfg.SuitNameLang + "(" + equipNum + "/" + cfg.Amount + ")";
      ...
      this.txt_suit_active_cond_val.color = _strengthenCond
        ? "#71f000"
        : "#aaaaaa";
      this.txt_suit_desc.color = _strengthenCond ? "#ffc68f" : "#aaaaaa";

      let skill: t_s_skilltemplateData =
        TempleteManager.Instance.getSkillTemplateInfoById(cfg.SuitSkill);
    ...
    let qualityCfg: t_s_petequipqualityData =
      TempleteManager.Instance.getpetequipqualityData(
        this._info.templateInfo.Profile,
      );
    if (qualityCfg) {
      this.txt_maxlv.text = LangManager.Instance.GetTranslation(
        "yishi.view.tips.goods.EquipTipsContent.intensify",
        qualityCfg.StrengMax,
      );
    }
  }
  ...
  private calAttr(attId: number, val: number) {
    ...
    switch (attId) {
      ...
      case 1007:
        if (petData) {
          result =
            (petData.magicAttack + petData.bagMagicattack) * (val / 10000);
        } else {
          result = val / 10000;
        }
        break;
      case 1008:
        if (petData) {
          result =
            (petData.physicalDefense + petData.bagDefence) * (val / 10000);
        } else {
          result = val / 10000;
        }
        break;
      case 1009:
        if (petData) {
          result =
            (petData.magicDefense + petData.bagMagicdefence) * (val / 10000);
        } else {
          result = val / 10000;
        }
        break;
  ...
  private getPetEquipBaseScore(petData: PetData): number {
    ...
    if (this.attrMap[3]) {
      ...
      if (petData) {
        Intellect =
          petData.intellectBase * (Number(this.attrMap[1003]) / 10000);
      } else {
    ...
    if (this.attrMap[6]) {
      ...
      if (petData) {
        Attack =
          (petData.physicalAttack + petData.bagAttack) *
          (Number(this.attrMap[1006]) / 10000);
      } else {
    ...
    if (this.attrMap[7]) {
      ...
      if (petData) {
        Defence =
          (petData.physicalDefense + petData.bagDefence) *
          (Number(this.attrMap[1007]) / 10000);
      } else {
    ...
    if (this.attrMap[8]) {
      ...
      if (petData) {
        MagicAttack =
          (petData.magicAttack + petData.bagMagicattack) *
          (Number(this.attrMap[1008]) / 10000);
      } else {
    ...
    if (this.attrMap[9]) {
      ...
      if (petData) {
        MagicDefence =
          (petData.magicDefense + petData.bagMagicdefence) *
          (Number(this.attrMap[1009]) / 10000);
      } else {
    ...
    if (this.attrMap[10]) {
      ...
      if (petData) {
        Live =
          (petData.hp + petData.bagLiving) *
          (Number(this.attrMap[1010]) / 10000);
      } else {
  ...
  private getAddition(preValue: number): number {
    ...
    return (
      Number(preValue * this._info.strengthenGrade * 0.1) +
      this._info.strengthenGrade * 5
    );
  }
  ...
  private updateAttr(masterAttr: string, sonAttr: string) {
      ...
      for (let i = 0; i < tempArr.length; i++) {
        ...
        if (element.length > 0) {
          ...
          if (cfg) {
            ...
            this.txt_master_attr.visible = this.txt_master_attr_val.visible =
              true;
          }
        }
      }
    ...
    let qualityCfg = TempleteManager.Instance.getpetequipqualityData(
      this._info.templateInfo.Profile,
    );
    if (!qualityCfg) {
    ...
    let maxLevel =
      curLevel >= Number(sonAttrStrengArr[sonAttrStrengArr.length - 1]);
    if (!maxLevel) {
      for (let j = 0; j < sonAttrStrengArr.length; j++) {
        const lv = Number(sonAttrStrengArr[j]);
        if (lv > 0 && curLevel < lv) {
          this.txt_sonattr_up_tip.visible = true;
          this.txt_sonattr_up_tip.text = LangManager.Instance.GetTranslation(
            "petEuip.stren.uptip",
            lv,
          );
          break;
    ...
    for (let i = 0; i <= sonAttrOpenArr.length; i++) {
      ...
      if (item) {
        ...
        if (curLevel < openLv) {
          ...
          item.getChild("txt_openCondition").text =
            LangManager.Instance.GetTranslation("petEuip.stren.lock2", openLv);
        } else {
          const element = tempArr[i];
          if (element && element.length > 0) {
            ...
            if (cfg) {
              ...
              item.getChild("txt_attributeValue").text =
                "+" + this.calAttr(attId, val);
              this.attrMap[attId] = val;
            }

            if (
              this._petequipattrDatas &&
              this._petequipattrDatas.some((obj) => obj.AttributeId === attId)
            ) {
              let petequipattrData: t_s_petequipattrData =
                this._petequipattrDatas.find(
                  (obj) => obj.AttributeId === attId,
                );
              let baseAttribute =
                petequipattrData.BaseValue.toString().split(",");
              let lv = 0;
              if (Number(arr[1]) === Number(baseAttribute[0])) {
                lv = 0;
              } else {
                lv =
                  (Number(arr[1]) - Number(baseAttribute[0])) /
                  Number(petequipattrData.StrengthenGrow);
              }
              lv = Math.floor(lv);
              if (lv > 0) {
                item.getChild("txt_attributeLevel").visible = true;
                item.getChild("txt_attributeLevel").text =
                  "(" +
                  LangManager.Instance.GetTranslation(
                    "mounts.command01",
                    Utils.toFixedNum(lv, 3),
                  ) +
                  ")";
              }
            }
            ...
            Logger.warn(
              "服务端数据与t_s_petequipquality配置不同步",
              tempArr,
              this._info,
            );
          }
  ...
  private btn_equipClick() {
    if (!this.petCtrl.selectedPet) {
      MessageTipManager.Instance.show(
        LangManager.Instance.GetTranslation("petEquip.nopet"),
      );
      this.hide();
      return;
    }
