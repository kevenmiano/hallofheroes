...
export class StripUIView implements IStrip {
    ...
    constructor(asset: fgui.GImage[], maxValue: number = -1, currentValue: number = -1, transitionGImg: fgui.GImage = null, colorOffset: Laya.Filter = UIFilter.normalFilter, twoWayTween: boolean = false) {
        this._asset = asset;
        ...
        this._bloodShieldBar = FUIHelper.createFUIInstance(EmPackName.Battle, "Img_Blood_Shield").asImage;
        ...
        this._bloodLoseBar = FUIHelper.createFUIInstance(EmPackName.Battle, "Img_Blood_Shield").asImage;
    ...
    public switchToIndex(index: number) {
        if (index < 0 || index > this._stripNum || index == this._currentIndex) return
     ...
     * @param tween 
     */
    public refreshProgress(value?: number, tween: boolean = false, delay: number = StripUIView.DelayTime, updateToZero: boolean = false) {
    ...
    public static calCurrentStrip(curValue: number, maxValue: number, stripNum: number): number {
        if (curValue <= 0) {
            return 0
        }
    ...
    private setCurStripValue(value: number) {
        if (this._curStripGImg)
            this._curStripGImg.fillAmount = value
    }
    ...
    private showCurStripGImg(value: boolean) {
        if (this._curStripGImg)
            this._curStripGImg.visible = value
    }
    ...
    public set bloodShield(v: number) {
        ...
        if ((this._bloodShieldValue + this._currentValue) > this._avgValue) {
            //血值百分比
            let bloodProg = this._currentValue / (this._avgValue + this._bloodShieldValue)
    ...
    public set bloodLoseValue(v: number) {
        if (v <= 0) { this.resetBloodLose(); return; }
