...
export class EnterBattleController {
  ...
  private switchToBattle() {
    ...
    if (BattleManager.Instance.started) {
      Logger.warn(
        "------------开始切换至战斗场景 exitFlag=" +
          Boolean(BattleManager.exitFlag) +
          ", loginToBattleFlag=" +
          Boolean(BattleManager.loginToBattleFlag),
      );
  ...
  private effectComplete() {
    ...
    if (model.useWay == 0) {
      SocketSendManager.Instance.sendLoadOver(
        model.battleId,
        PlayerManager.Instance.currentPlayerModel.playerInfo.userId,
      );
    } else {
      //增援战斗
      SocketSendManager.Instance.sendReinfoLoadOver(
        model.battleId,
        PlayerManager.Instance.currentPlayerModel.playerInfo.userId,
      );
    }
