...
export default class DiscountWnd extends BaseWindow {
  ...
  public OnInitWind() {
    ...
    if (selectId <= 0) {
      //默认选择一个
      this._selectedTempleteData = ShopManager.Instance.getDefaultSelectGoods(
  ...
  private addEvent() {
    ...
    NotificationManager.Instance.addEventListener(
      DISCOUNT_ITEM.DISCOUNT_ITEM_CHECK_STATE,
  ...
  private __itemClickHandler(item: DiscountItem, evt) {
    ...
    if (
      evtTarget &&
      !(target instanceof DiscountItem) &&
      target instanceof fgui.GButton
    ) {
  ...
  private updateCost() {
    ...
    this.txt_des.text = LangManager.Instance.GetTranslation(
  ...
  private renderListItem(
    index: number,
    item: DiscountItem | DiscountTitleItem,
  ) {
    ...
    if (item instanceof DiscountItem) {
      ...
      if (
        this._selectedTempleteId == this.listData[index].templateId &&
        this.listData[index].id == this._selectedTempId &&
        (this.listData[index] as GoodsInfo).validDate >= 0
      ) {
