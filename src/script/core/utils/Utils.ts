...
export default class Utils {
    ...
    public static NUM_CN: Array<Object> = [
        ...
        { en: "Four", zhcn: "四", "es": "Cuatro", "pt": "Quatro", "tr": "Dört", "de": "Vier" },
        { en: "Five", zhcn: "五", "es": "Cinco", "pt": "Cinco", "tr": "Beş", "de": "Fünf" },
        { en: "Six", zhcn: "六", "es": "Seis", "pt": "Seis", "tr": "Altı", "de": "Sechs" },
        { en: "Seven", zhcn: "七", "es": "Siete", "pt": "Sete", "tr": "Yedi", "de": "Sieben" },
        { en: "Eight", zhcn: "八", "es": "Ocho", "pt": "Oito", "tr": "Sekiz", "de": "Acht" },
        { en: "Nine", zhcn: "九", "es": "Nueve", "pt": "Nove", "tr": "Dokuz", "de": "Neun" }
    ];

    public static MONTHS: Array<Object> = [
        { en: "January", zhcn: "1月", "es": "Enero", "pt": "Janeiro", "tr": "Ocak", "de": "Januar" },
        { en: "February", zhcn: "2月", "es": "Febrero", "pt": "Fevereiro", "tr": "Şubat", "de": "Februar" },
        { en: "March", zhcn: "3月", "es": "Marzo", "pt": "Março", "tr": "Mart", "de": "März" },
        { en: "April", zhcn: "4月", "es": "Abril", "pt": "Abril", "tr": "Nisan", "de": "April" },
        { en: "May", zhcn: "5月", "es": "Mayo", "pt": "Maio", "tr": "Mayıs", "de": "Mai" },
        { en: "June", zhcn: "6月", "es": "Junio", "pt": "Junho", "tr": "Haziran", "de": "Juni" },
        { en: "July", zhcn: "7月", "es": "Julio", "pt": "Julho", "tr": "Temmuz", "de": "Juli" },
        { en: "August", zhcn: "8月", "es": "Agosto", "pt": "Agosto", "tr": "Ağustos", "de": "August" },
        { en: "September", zhcn: "9月", "es": "Septiembre", "pt": "Setembro", "tr": "Eylül", "de": "September" },
        { en: "October", zhcn: "10月", "es": "Octubre", "pt": "Outubro", "tr": "Ekim", "de": "Oktober" },
        { en: "November", zhcn: "11月", "es": "Noviembre", "pt": "Novembro", "tr": "Kasım", "de": "November" },
        { en: "December", zhcn: "12月", "es": "Diciembre", "pt": "Dezembro", "tr": "Aralık", "de": "Dezember" }
    ]
    ...
    public static UNITS: Array<Object> = [
        { en: "Thousand", zhcn: "千", es: "Mil", "pt": "Mil", "tr": "Bin", "de": "Eintausend" },
        { en: "Hundred", zhcn: "百", es: "Cien", "pt": "Cem", "tr": "Yüz", "de": "Einhundert" },
    ...
    public static overLength(num, isInt: boolean = false) {
        ...
        if (num < 100000) {
            ...
            if (num >= 1000000000) {
                ...
                str = Math.floor((num / 100000000)) + LangManager.Instance.GetTranslation("CommonUtils.text1")
                num = num % 100000000
                if (num > 10000) {
                    str += (Math.floor(num / 10000) + LangManager.Instance.GetTranslation("CommonUtils.text2"))
                }
            }
        } else {
            num = (num / 10000);
    ...
    */
    public static deepCopy(target) {
        if (Utils.checkType(target) === 'Object') { // 拷贝对象
            let o = {};
            for (let key in target) {
                o[key] = Utils.deepCopy(target[key])
            }
            return o;
        } else if (Utils.checkType(target) === 'Array') { // 拷贝数组
            var arr = []
            for (let i = 0, leng = target.length; i < leng; i++) {
                arr[i] = Utils.deepCopy(target[i])
            }
            return arr;
        } else if (Utils.checkType(target) === 'Function') { // 拷贝函数
            return new Function('return ' + target.toString()).call(this)
        } else if (Utils.checkType(target) === 'Date') { // 拷贝日期
            return new Date(target.valueOf())
        } else if (Utils.checkType(target) === 'RegExp') { // 拷贝正则
            return new RegExp(target)
        } else if (Utils.checkType(target) === 'Map') { // 拷贝Map 集合
            let m = new Map()
            target.forEach((v, k) => {
                m.set(k, Utils.deepCopy(v))
            })
            return m
        } else if (Utils.checkType(target) === 'Set') { // 拷贝Set 集合
            let s = new Set()
     ...
     * @param disTime 间隔时间
     */
    public static flashTarget(target: fgui.GComponent | Laya.Sprite, effect: Laya.Filter = UIFilter.redFilter, delay: number = 300) {
        if (target) {
    ...
    public static isFromHY(): boolean {
        let urlChannelID = Utils.GetUrlQueryString("channelId");
        if (urlChannelID == H5SDK_CHANNEL_ID.HY || urlChannelID == H5SDK_CHANNEL_ID.HY_S) {
            return true;
        }
        return false;
    }
    ...
    public static isFixAuto(): boolean {
        return Utils.isQQHall() || Utils.isLABA() || Utils.isQQZone() || Utils.isQQMobile();
    }
    ...
    */
    ...
    public static getAppNumByName(versionName: string): number {
        ...
        return parseInt(arr[0]) * 100000000 + parseInt(arr[1]) * 100000 + parseInt(arr[2]) * 100;
    }
    ...
    public static toCNUpper(num: number, lang: string = "zhcn"): string {
        ...
        var cnMoney: string = Utils.convertIntegerStr(dotLeft, lang) + Utils.convertDecimalStr(dotRight, lang);
        return cnMoney;
    }
     ...
     * @return 
     */
    private static convertThousand(num: string, level: number, lang: string = "zhcn"): ThousandNode {
        ...
        if (n1 == 0)
            ...
            node.desc += Utils.NUM_ARR[n1][lang] + Utils.UNITS_ARR[0][lang];
        ...
        if (n4 == 0)
            ...
            node.desc += Utils.NUM_ARR[n4][lang];

        if (node.desc.length > 0)
            node.desc += Utils.LEVELS[level];
        return node;
    }
    ...
    //插值查找 数据必须是有序
    public static insertValueSearch<T>(arr: T[], left: number, right: number, findVal: number, property: string): number {
        // console.log("insertValueSearch 被调用次数");
        // 注意: findVal < arr[0] || findVal > arr[arr.length - 1] 必须需要, 否则得到的mid值可能越界
        if (left > right || findVal < arr[0][property] || findVal > arr[arr.length - 1][property]) {
            return -1;
        }
        // 求出mid,因为是有序的, 可以预测值的方向和一个比例位置
        let mid = left + (right - left) * (findVal - arr[left][property]) / (arr[right][property] - arr[left][property]);
    ...
    public static colorHex2RGB(strHex: string): Laya.Color {
        ...
        if (reg.test(color)) {
            ...
            return new Laya.Color(colorChange[0], colorChange[1], colorChange[2], 255)
        } else {
    ...
    public static formatTimeZone(i: number, zoneId: string): Date {
        ...
        var utcTime = new Date(new Date(i).toLocaleString('zh-CN', { timeZone: zoneId }))
        return utcTime;
    }
