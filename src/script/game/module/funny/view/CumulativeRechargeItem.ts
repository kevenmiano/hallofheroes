...
export default class CumulativeRechargeItem extends FUI_CumulativeRechargeItem {
    ...
    protected onConstruct() {
        ...
        this._diamondBtn1.scaleParas.paraScale = this._diamondBtn2.scaleParas.paraScale = 1;
        let goodsInfo: GoodsInfo = new GoodsInfo();
        goodsInfo.templateId = TemplateIDConstant.TEMP_ID_DIAMOND;
        FUIHelper.setTipData(this._diamondBtn1.view, EmWindow.NewPropTips, goodsInfo);
        FUIHelper.setTipData(this._diamondBtn2.view, EmWindow.NewPropTips, goodsInfo);
    }
    ...
    private boxIconHandler(){
        FrameCtrlManager.Instance.open(EmWindow.CumulativeRechargeItemInfoWnd,this._info);
    }

    private confirmBtnHandler() {
        if (this._canGet && this._info) {
            var currentTime: number = PlayerManager.Instance.currentPlayerModel.sysCurTimeBySecond;
            ...
            if (currentTime >= startTime && currentTime <= endTime) {
                SocketSendManager.Instance.sendGetFeedBack(FeedBackManager.Instance.data.id, this._info.id, 2);
            } else {
    ...
    private refreshView() {
        if (this._info) {
            this.title1.text = LangManager.Instance.GetTranslation("CumulativeRechargeItem.title1", this._info.point);
            this.title2.text = LangManager.Instance.GetTranslation("CumulativeRechargeItem.title2", this._info.price);
            this.descTitle1.text = LangManager.Instance.GetTranslation("CumulativeRechargeItem.descTitle1");
            this.descTitle2.text = LangManager.Instance.GetTranslation("CumulativeRechargeItem.descTitle2");
            this.confirmBtn.title = LangManager.Instance.GetTranslation("map.campaign.view.fall.BattleFallGoodsView.recive");
            ...
            if (this._info.state)//已领取
            {
                this.confirmBtn.title = LangManager.Instance.GetTranslation("dayGuide.view.FetchItem.alreadyGet");
                this.confirmBtn.enabled = false;
                this._canGet = false;
            }
        }
