  ...
  implements DragObject, ITipedDisplay
{
  ...
  private swap(dstTarget, srcTarget) {
    ...
    if (dstTarget.info != null) {
      ...
      var tempName: string = StarHelper.getStarHtmlName(
        this._eatStar.info.template,
      );
      var tempExp: number = StarHelper.getStarTotalExp(this._beEatenStar.info);
      var tempLevel: number = StarHelper.checkCanUpGrade(
        tempExp,
        this._eatStar.info,
      );
      var tempStr: string = LangManager.Instance.GetTranslation(
        "cell.view.starbag.StarBagCell.tempStr",
        tempName,
        tempExp,
      );
      ...
      SimpleAlertHelper.Instance.Show(
        ...
        null,
        null,
        this.__composeResponse.bind(this),
      );
      this.setItemBack(srcTarget);
    } else {
      if (
        srcTarget.bagType == StarBagType.PLAYER &&
        dstTarget.bagType == StarBagType.THANE
      ) {
        if (srcTarget.info.template.Profile == 6) {
        ...
        for (let index = 0; index < len; index++) {
          ...
          if (info.template.MasterType == srcTarget.info.template.MasterType) {
            let str = LangManager.Instance.GetTranslation(
              "cell.view.starbag.StarBagCell.command02",
            );
            MessageTipManager.Instance.show(str);
            this.setItemBack(srcTarget);
            return;
  ...
  private __composeResponse(boolean: boolean, flag: boolean, target: StarItem) {
    if (boolean && this._info && target && target.info) {
      ...
      var maxGrade: number = StarHelper.getStarMaxGradeByProfile(
        this._eatStar.info.template.Profile,
      );
      if (tempTarget.info.grade >= maxGrade) {
        MessageTipManager.Instance.show(
          LangManager.Instance.GetTranslation(
            "cell.view.starbag.StarBagCell.command03",
          ),
        );
        return;
      }
      if (
        this._beEatenStar.info.template.Profile >= 5 &&
        this._beEatenStar.info.template.Profile != 6
      ) {
        let content: string = LangManager.Instance.GetTranslation(
          "cell.view.starbag.StarBagCell.confirmComposeTip",
          StarHelper.getStarHtmlName(this._beEatenStar.info.template),
        );
  ...
  private composeCall(b: boolean, flag: boolean, target: StarItem) {
    if (b && this._info && target) {
      ...
      StarManager.Instance.starMove(
        this._beEatenStar.info.pos,
        this._beEatenStar.info.bagType,
        this._eatStar.bagPos,
        this._eatStar.bagType,
        true,
      );
    }
  }
    ...
    if (data) {
      ...
      if (data.template) {
        ...
        ResRefCountManager.loadRes(realPath, (res) => {
          ...
          if (!res.meta) {
            Logger.warn(
              "[StarItem]info realPath assets meta is null",
              realPath,
            );
            return;
          }
          ...
          this._movie.pos(
            (this.width - sourceSize.width * StarItem.STAR_SCALE) / 2,
            (this.height - sourceSize.height * StarItem.STAR_SCALE) / 2,
          );
      ...
      let color =
        FilterFrameText.Colors[eFilterFrameText.StarQuality][
          data.template.Profile - 1
        ];
      this.txtName.color = color;

      if (
        this.bagType == StarBagType.PLAYER ||
        this.bagType == StarBagType.THANE
      ) {
