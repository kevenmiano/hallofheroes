...
export default class PetEquipSuccinctItem extends FUI_PetEquipSuccinctItem {
  ...
  set info(value: GoodsInfo) {
    ...
    if (this._info.suitId) {
      this._petEquipSuitData = TempleteManager.Instance.getPetEquipSuitData(
        this._info.suitId,
      );
    ...
    if (this._hasSuit && this._petEquipSuitData.SuitSkill) {
      this._skilltemplateData =
        TempleteManager.Instance.getSkillTemplateInfoById(
          this._petEquipSuitData.SuitSkill,
        );
      this._hasSkillIcon = Boolean(
        this._skilltemplateData && this._skilltemplateData.Icons,
      );
    }
  ...
  private updateView() {
    if (this._hasSuit) {
      ...
      let bagArray = GoodsManager.Instance.getGoodsByBagType(
        BagType.PET_EQUIP_BAG,
      );
      for (let i = 0; i < bagArray.length; i++) {
        if (
          bagArray[i].suitId == this._info.suitId &&
          bagArray[i].objectId == this._petId
        )
          equipNum++;
      }
      ...
      if (this._petEquipSuitData.Amount <= 1) {
        ...
        this.txt_suit_name.text =
          this._petEquipSuitData.SuitNameLang +
          "(" +
          equipNum +
          "/" +
          this._petEquipSuitData.Amount +
          ")";
      ...
      if (this._hasSkillIcon) {
        ...
        let _strengthenCond =
          this._info.strengthenGrade >= this._petEquipSuitData.StrengthenGrow;
    ...
    if (master) {
      ...
      for (let i = 0; i < masterAttribute.length; i++) {
        ...
        if (element.length > 0) {
          ...
          let attributeData: t_s_attributeData =
            TempleteManager.Instance.getPetEquipAttri(attributeId);
          if (attributeData) {
            ...
            attributeItem.getChild("txt_attr_key").text =
              attributeData.AttributeNameLang;
          ...
          if (element.length > 0) {
            ...
            if (
              ...
              if (Number(attribute[1]) === Number(baseAttribute[0])) {
                ...
                lv =
                  (Number(attribute[1]) - Number(baseAttribute[0])) /
              ...
              if (lv > 0) {
                ...
                attributeItem.getChild("txt_attr_val1").text =
                  LangManager.Instance.GetTranslation("mounts.command01", lv);
              }
            }
          }
        }
