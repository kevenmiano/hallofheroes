...
export class OutercityNpcActivationMediator implements IMediator
{
    ...
    public register(target:Object):void
    {
        ...
        this._target.on(OuterCityEvent.DRAG_SCENE_END, this, this.__dragScneHandler);
    }

    public unregister(target:Object):void
    {
        this._target = target as Sprite;
        this._target.off(OuterCityEvent.DRAG_SCENE_END, this, this.__dragScneHandler);
    }
    ...
    private outSceneMoviesImp(arr:any):void
    {
        ...
        for(const arrKey in arr)
        {
            if(arr.hasOwnProperty(arrKey))
            {
                ...
                let b:boolean = OutercityNpcActivationMediator.checkOutScene(temp, temp["sizeInfo"], this._target);
                temp['isPlaying'] = !b;
                if(b)
                {
                    cur++;
    ...
    public static checkOutScene(info:Sprite, curRect:any /** HeroAvatar.sizeMax */, $target:Sprite):boolean
    {
        let b:boolean = false;
        if(!info)
        {
        ...
        if(curRect)
        {
            ...
            else
            {
                ...
                if(dir < 0)
                {
                    if(disHeight > StageReferance.stageHeight / 2 + (curRect.height - Math.abs(curRect.y)))
                    {
                        b = true;
                    }
                }
                ...
                {// 检测物体在屏幕中心下方
                    if(disHeight > StageReferance.stageHeight / 2 + Math.abs(curRect.y))
                    {
                        b = true;
                    }
                }
