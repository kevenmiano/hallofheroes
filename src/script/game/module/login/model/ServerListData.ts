...
export default class ServerListData {
  ...
  parse(value: string) {
    ...
    } else if (this.recomandServers.length > 0) {
      //推荐区服
      this.defaultSelectServer =
        this.recomandServers[this.recomandServers.length - 1];
    } else {
      //所有区服
      let count = this.allSites.length;
      if (count > 0) this.defaultSelectServer = this.allSites[count - 1];
    }
    ...
    if (this.recordLoginServers.length) {
      ...
      playerServer.groupName = LangManager.Instance.GetTranslation(
        "ServerListData.groupName1",
      );
    ...
    if (this.recomandServers.length) {
      ...
      recomandServer.groupName = LangManager.Instance.GetTranslation(
        "ServerListData.groupName2",
      );
