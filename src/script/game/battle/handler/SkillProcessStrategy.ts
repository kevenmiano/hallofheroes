...
export class SkillProcessStrategy {
   ...
   *
   */
  public static processSynchronization(
    serverTime: number,
    skill: BaseSkill,
  ): number {
    ...
    if (differ < -20) {
      ...
      BattleManager.Instance.synchronization =
        clientTime + (serverTime - clientTime) * 0.2; //修正20%
    }
    return needSkip;
  }
   ...
   *
   */
  private static processWhenClientSlower(
    differ: number,
    skillN: BaseSkill,
  ): number {
    ...
    for (let i: number = 0; i < waitList.length; i++) {
      ...
      if (SkillProcessStrategy.checkIsCanSkip(skill.getSkillData().skillId)) {
        if (
          skill.getCurrentRole() == BattleManager.Instance.battleModel.selfHero
        ) {
