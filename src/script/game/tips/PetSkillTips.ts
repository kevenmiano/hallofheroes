...
export default class PetSkillTips extends BaseTips {
    ...
    private btnStudyClick() {
        ...
        /**道具不足时快捷购买 */
        if (GoodsManager.Instance.getGoodsNumByTempId(TemplateIDConstant.TEMP_ID_PET_SKILL_SCROLL) < skillTemp.Parameter3) {
            var data: ShopGoodsInfo = TempleteManager.Instance.getShopTempInfoByItemId(TemplateIDConstant.TEMP_ID_PET_SKILL_SCROLL);
        ...
        let content = LangManager.Instance.GetTranslation("PetLearnSkillView.usePointTip5", skillTemp.Parameter3, skillTemp.TemplateNameLang)
        SimpleAlertHelper.Instance.Show(SimpleAlertHelper.SIMPLE_ALERT, null, null, content, null, null,
            (b: boolean) => {
                if (b) {
                    PetCtrl.learnSkill(this.petData && this.petData.petId, undefined, skillTemp.TemplateId, PetData.SPECIAL_SKILL, false);
                }
            }
        );
        this.hide();
    ...
    private initView() {
        ...
        let learned = this.petData && this.petData.checkSkillIsLearned(skillTemp.TemplateId);
        ...
        if (skillTemp.CoolDown > 0) {
            this.txt_cd.text = LangManager.Instance.GetTranslation("yishi.view.tips.goods.SkillTips.cooldown01", Math.ceil(skillTemp.CoolDown * 0.001));
        }
        if (skillTemp.Cost > 0) {
        ...
        let effectDes: string = StringUtils.stringFormat2(skillTemp.SkillDescription, { key: "Parameter1", value: skillTemp.Parameter1 }, { key: "Parameter2", value: skillTemp.Parameter2 }, { key: "Parameter3", value: skillTemp.Parameter3 })
