...
export default class TopToolBar extends BaseFguiCom {
  ...
  constructor(container?: fgui.GComponent) {
    ...
    if (PlayerManager.Instance.currentPlayerModel.playerInfo.isSignOpen) {
      //七日登录开启
      if (this.ctrlData.sevenLoginRewardArr.length == 0) {
        this.control.requestSevenLoginInfo();
      }
    }
    if (PlayerManager.Instance.currentPlayerModel.playerInfo.isPassOpen) {
      //通行证开启
  ...
  private initResoueceTxt() {
    this.__refreshGold();
    this.voucherTxt.text = FormularySets.toStringSelf(
      PlayerManager.Instance.currentPlayerModel.playerInfo.point,
      HomeWnd.STEP,
    );
    this.giftTxt.text = FormularySets.toStringSelf(
      PlayerManager.Instance.currentPlayerModel.playerInfo.giftToken,
      HomeWnd.STEP,
    );
    this.voucherTxt.color =
      PlayerManager.Instance.currentPlayerModel.playerInfo.point < 0
  ...
  private initBtnPos() {
    ...
    this.btnDataList = ArrayUtils.sortOn(
      this.btnDataList,
      ["sort"],
  ...
  private __refreshGold() {
    ...
    this.goldTxt.text = FormularySets.toStringSelf(
  ...
  initBtnDataList() {
    ...
    if (this.isFirstChargeOpen) {
      this.createBtnData(
        MainBtnType.FIRSTPAY,
        FUIHelper.getItemURL(EmPackName.Home, "Btn_Eve_Recharge"),
        3,
        LangManager.Instance.GetTranslation("TopToolBar.btnText6"),
      );
    } else {
      this.removeBtnData(MainBtnType.FIRSTPAY);
    }
    if (
      this.thane.grades >= OpenGrades.FUNNY &&
      !ComponentSetting.IOS_VERSION
    ) {
      this.createBtnData(
        MainBtnType.FUNNY,
        FUIHelper.getItemURL(EmPackName.Home, "Btn_Eve_HotEvents"),
        1,
        LangManager.Instance.GetTranslation("TopToolBar.btnText1"),
      );
    }
    if (this.thane.grades >= OpenGrades.WELFARE) {
      this.createBtnData(
        MainBtnType.WELFARE,
        FUIHelper.getItemURL(EmPackName.Home, "Btn_Eve_Welfare"),
        2,
        LangManager.Instance.GetTranslation("TopToolBar.btnText2"),
      );
    }
    if (
      QuestionnaireManager.Instance.model.questionnaireVisible &&
      this.thane.grades >= OpenGrades.QUESTION_NAIRE
    ) {
      this.createBtnData(
        MainBtnType.QUESTIONNAIRE,
        FUIHelper.getItemURL(EmPackName.Home, "Btn_Eve_Survey"),
        4,
        LangManager.Instance.GetTranslation("TopToolBar.btnText7"),
      );
    }
    if (
      this.isMineralDouble &&
      this.thane.grades >= OpenGrades.MINERAL_DOUBLE &&
      this.mineralModel &&
      this.mineralModel.maxCount > 0 &&
      !WorldBossHelper.checkMineral(CampaignManager.Instance.mapId)
    ) {
      //紫金矿产双倍
      this.createBtnData(
        MainBtnType.MINTERAL,
        FUIHelper.getItemURL(EmPackName.Home, "Btn_Eve_Amethyst"),
        99,
        LangManager.Instance.GetTranslation("TopToolBar.btnText8"),
      );
    }
    if (
      this.thane.grades >= OpenGrades.MULTILORD &&
      ComponentSetting.MULTILORD
    ) {
      this.createBtnData(
        MainBtnType.MULTILORD,
        FUIHelper.getItemURL(EmPackName.Home, "Btn_Eve_Titan"),
        8,
        LangManager.Instance.GetTranslation(
          "ui.toptoolbar.TopToolsBar.tipData_multilords",
        ),
      );
    }
    if (
      this.playerInfo.canAcceptCrossScoreAward &&
      this.thane.grades >= OpenGrades.RVR
    ) {
      this.createBtnData(
        MainBtnType.CROSS_PVP_REWARD,
        FUIHelper.getItemURL(EmPackName.Home, "Btn_Eve_CrossServerRewards"),
        99,
        LangManager.Instance.GetTranslation(
          "mainBar.TopToolsBar.crossPvpAwardBtnTipData",
        ),
      );
    }
    if (this.thane.grades >= OpenGrades.MYSTERIOUS) {
      this.createBtnData(
        MainBtnType.MYSTERYSHOP,
        FUIHelper.getItemURL(EmPackName.Home, "Btn_Eve_MysteryShop"),
        99,
        LangManager.Instance.GetTranslation("HigherGradeOpenTipView.content17"),
      );
    }
    if (Utils.isQQHall()) {
      this.createBtnData(
        MainBtnType.QQHALL_DAWANKA,
        FUIHelper.getItemURL(EmPackName.Home, "Btn_Eve_QQ"),
        9,
        "",
      );
      if (ConfigManager.info.QQ_HALL_GIFT) {
        this.createBtnData(
          MainBtnType.QQHALL_GIFT,
          FUIHelper.getItemURL(EmPackName.Home, "Btn_Eve_qq1"),
          10,
          LangManager.Instance.GetTranslation("QQ.Hall.Gift.QQIcon"),
        );
      }
    }
    if (this.thane.grades >= OpenGrades.PET) {
      ...
      if (showPetboss) {
        this.createBtnData(
          MainBtnType.PETGUARD,
          FUIHelper.getItemURL(EmPackName.Home, "Btn_Eve_DefendSylphAtoll"),
          7,
          LangManager.Instance.GetTranslation("TopToolBar.petGuard"),
        );
      }
    }
    if (ExpBackModel.instance.openState == 1) {
      this.createBtnData(
        MainBtnType.EXP_BACK_FREE,
        FUIHelper.getItemURL(EmPackName.Home, "Btn_Eve_Gift"),
        9999,
        LangManager.Instance.GetTranslation("TopToolBar.btnText_expFree"),
        0,
        true,
      );
    ...
    if (ExpBackModel.instance.openState == 2) {
      this.createBtnData(
        MainBtnType.EXP_BACK_EXTRA,
        FUIHelper.getItemURL(EmPackName.Home, "Btn_Eve_Gift"),
        9999,
        LangManager.Instance.GetTranslation("TopToolBar.btnText_expExtra"),
        0,
        true,
      );
    } else {
    ...
    if (isOversea()) {
      if (this.thane.grades >= OpenGrades.ACTIVITY_TIME) {
        this.createBtnData(
          MainBtnType.ACTIVE_TIME,
          FUIHelper.getItemURL(EmPackName.Home, "Btn_Eve_Calendar"),
          9999,
          LangManager.Instance.GetTranslation("TopToolBar.activeTime"),
        );
      } else {
    ...
    if (mapModel && WorldBossHelper.checkPetLand(mapModel.mapId)) {
      this.createBtnData(
        MainBtnType.PET_EXCHANGE,
        FUIHelper.getItemURL(EmPackName.Home, "Btn_Main_Sylph"),
        9999,
        LangManager.Instance.GetTranslation("TopToolBar.btnText11"),
        0,
        false,
      );
    } else {
  ...
  private get isShowOutyard(): boolean {
    ...
    if (
      ...
      if (
        stateMsg.state != OutyardModel.NOT_OPEN &&
        this.outyardModel.checkIsOpenDay() &&
        PlayerManager.Instance.currentPlayerModel.sysCurtime.getHours() >= 1
      ) {
        flag = true;
  ...
  private checkOutyard(needRefresh: boolean = true) {
    ...
    if (flag) {
      this.createBtnData(
        MainBtnType.OUTYARD,
        FUIHelper.getItemURL(EmPackName.Home, "Btn_Eve_Outland"),
        0,
        LangManager.Instance.GetTranslation(
          "gvg.view.ready.GvgReadyFrame.title",
        ),
      );
    } else {
  ...
  private checkGoldenSheep() {
    ...
    if (ComponentSetting.GOLDEN_SHEEP && flag) {
      let titleStr: string = LangManager.Instance.GetTranslation(
        "mainBar.TopToolsBar.goldensheep",
      );
      this.createBtnData(
        MainBtnType.GOLDENSHEEL,
        FUIHelper.getItemURL(EmPackName.Home, "Btn_Eve_GoldenSheep"),
        99,
        titleStr,
      );
    } else {
  ...
  private checkSevenGoals() {
    let grade: number = parseInt(
      TempleteManager.Instance.getConfigInfoByConfigName("seventarget_Grade")
        .ConfigValue,
    );
    if (
      this.thane.grades >= grade &&
      DataCommonManager.playerInfo.isTargetOpen
    ) {
      //七日目标
      let titleStr: string = LangManager.Instance.GetTranslation(
        "mainBar.TopToolsBar.sevenGoals",
      );
      this.createBtnData(
        MainBtnType.SEVEN_GOALS,
        FUIHelper.getItemURL(EmPackName.Home, "Icon_7Target"),
        99,
        titleStr,
      );
  ...
  checkMiniGame(needRefresh: boolean = true) {
    ...
    if (ConfigManager.info.GEMMAZE) {
      if (ArmyManager.Instance.thane.grades >= OpenGrades.GEM_MAZE) {
        ...
        if (
          GemMazeManager.Instance.model.openDay.indexOf(curDay.toString()) != -1
        ) {
          gemMazeOpen = true;
        }
      }
    ...
    if (ConfigManager.info.MONOPOLY) {
      if (ArmyManager.Instance.thane.grades >= OpenGrades.GEM_MAZE) {
        ...
        if (
          MonopolyManager.Instance.model.openDay.indexOf(curDay.toString()) !=
          -1
        ) {
          monopolyOpen = true;
        }
    ...
    if (gemMazeOpen || monopolyOpen) {
      ...
      this.createBtnData(
        MainBtnType.MINIGAME,
        FUIHelper.getItemURL(EmPackName.Home, "Btn_Eve_GameBox"),
        1,
        LangManager.Instance.GetTranslation("TopToolBar.btnText9"),
      );
    } else {
  ...
  checkMicroApp(needRefresh: boolean = true) {
    ...
    let terminalCfg = TempleteManager.Instance.getConfigInfoByConfigName(
      "Microterminal_Platform",
    );
    if (terminalCfg) {
      let curChannelID = ModelMgr.Instance.getProperty(
        EmModel.USER_MODEL,
    ...
    if (
      isMicroPlat &&
      !PlayerManager.Instance.hasRecMicroAppReward &&
      ConfigManager.info.MICRO_APP
    ) {
      this.createBtnData(
        MainBtnType.MICROAPP,
        FUIHelper.getItemURL(EmPackName.Home, "Btn_Eve_Micro"),
        99,
        LangManager.Instance.GetTranslation(
          "mainBar.TopToolsBar.microterminal",
        ),
      );
    } else {
  ...
  checkPVP(needRefresh: boolean = true) {
    ...
    for (let index = 0; index < this._pvpOpenTimeArr.length; index++) {
      ...
      if (
        ...
        flag = true;
        this._pvpCanJoinLeftTime = Math.ceil(
          Math.abs(curTimeMin - element.startTimeMin),
        );
        break;
      }
    }
    ...
    if (flag) {
      this.createBtnData(
        MainBtnType.PVP,
        FUIHelper.getItemURL(EmPackName.Home, "Btn_Main_Arena"),
        0,
        LangManager.Instance.GetTranslation("TopToolBar.btnText13"),
      );
    } else {
  ...
  private checkWarlords() {
    ...
    var beginTime: number =
      warlordsModel.getMatchDate(1).getTime() / 1000 - 259200; //提前3天
    var curTime: number =
      PlayerManager.Instance.currentPlayerModel.sysCurTimeBySecond;
    var endTime: number =
      WarlordsManager.Instance.model.getMatchDate(5).getTime() / 1000;
    var isOpenTime: boolean =
      (curTime > beginTime && curTime < endTime) || this.checkWarlordsProcess();
    if (
      ComponentSetting.WARLORD &&
      this.thane.grades >= WarlordsModel.BET_OPEN_GRADE &&
      isOpenTime
    ) {
      let titleStr: string = LangManager.Instance.GetTranslation(
        "mainBar.TopToolsBar.warlordsBtnTipData",
      );
      this.createBtnData(
        MainBtnType.WARLORD,
        FUIHelper.getItemURL(EmPackName.Home, "Btn_Eve_ClassWars"),
        8,
        titleStr,
      );
    } else {
  ...
  private checkWarlordsReward() {
    if (
      ...
      !this._getWardRewarFlag
    ) {
      let titleStr: string = LangManager.Instance.GetTranslation(
        "mainBar.TopToolsBar.warlordsBtnTipData2",
      );
      this.createBtnData(
        MainBtnType.WARlORD_REWARD,
        FUIHelper.getItemURL(EmPackName.Home, "Btn_jingcai"),
        9,
        titleStr,
      );
    } else {
  ...
  private addEvent() {
    ...
    this.playerInfo.addEventListener(
      PlayerEvent.FIRSTCHARGE,
      this.__firstChargeHandler,
      this,
    );
    NotificationManager.Instance.addEventListener(
      NotificationEvent.FIRSTPAY_DATA_UPDATE,
      this.__firstChargeHandler,
      this,
    );
    this.playerInfo.addEventListener(
      PlayerEvent.WORLDBOSSSTATE,
      this.heroLevelUpdate,
      this,
    );
    NotificationManager.Instance.addEventListener(
    ...
    DayGuideManager.Instance.addEventListener(
      DayGuideEvent.DAILY_AWARD_STATE,
      this.__refreshWelfareRedPoint,
      this,
    );
    QuestionnaireManager.Instance.model.addEventListener(
      QuestionnaireEvent.QUESTIONNAIRE_CHANGE,
      this.updateQuestionVisible,
      this,
    );
    ...
    NotificationManager.Instance.addEventListener(
      NotificationEvent.LEVEL_GIFT_UPDATE,
      this.__refreshWelfareRedPoint,
      this,
    );
    DayGuideManager.Instance.cate.addEventListener(
      DayGuideEvent.LEED_PROGRESS_CHANGE,
      this.__refreshWelfareRedPoint,
      this,
    );
    DayGuideManager.Instance.cate.addEventListener(
      DayGuideEvent.ACTIVE_CHANGE,
      this.__refreshWelfareRedPoint,
      this,
    );
    DayGuideManager.Instance.cate.addEventListener(
      DayGuideEvent.WEEK_ACTIVE_CHANGE,
      this.__refreshWelfareRedPoint,
      this,
    );
    NotificationManager.Instance.addEventListener(
    ...
    NotificationManager.Instance.addEventListener(
      NotificationEvent.GROWTH_FUND_UPDATE,
      this.__refreshWelfareRedPoint,
      this,
    );
    NotificationManager.Instance.addEventListener(
      NotificationEvent.PASS_CHECK_AWARD_STATE,
      this.__refreshWelfareRedPoint,
      this,
    );
    NotificationManager.Instance.addEventListener(
      ExpBackEvent.UPDATE_INFO,
      this.__refreshWelfareRedPoint,
      this,
    );
    NotificationManager.Instance.addEventListener(
    ...
    ServerDataManager.listen(
      S2CProtocol.U_C_ONLINE_REWARD,
      this,
      this.onLineRewardBack,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_CROSS_SCORE_REWARD_RESULT,
      this,
      this.onCrossScoreRewardHandler,
    );
    NotificationManager.Instance.addEventListener(
    ...
    NotificationManager.Instance.addEventListener(
      SwitchEvent.SWITCH_GEMMAZE,
      this.checkMiniGame,
      this,
    ); //夺宝奇兵开关切换
    NotificationManager.Instance.addEventListener(
      PlayerEvent.MICRO_APP_EVENT,
      this.checkMicroApp,
      this,
    );
    NotificationManager.Instance.addEventListener(
      SwitchEvent.MICRO_APP,
      this.checkMicroApp,
      this,
    );
    GoldenSheepManager.Instance.model.addEventListener(
      GoldenSheepEvent.STATE_UPDATE,
      this.updateGoldenSheepVisible,
      this,
    );
    ...
    NotificationManager.Instance.addEventListener(
      SwitchEvent.SWITCH_MAP_SHOP,
      this.onCheckMysteryShop,
      this,
    );
    QQDawankaManager.Instance.model.addEventListener(
      QQ_HALL_EVENT.GARDE_CHANGE,
      this.__refreshQQHallRedPoint,
      this,
    );
    QQGiftManager.Instance.model.addEventListener(
    ...
    if (this.playerModel)
      this.playerModel.addEventListener(
        ...
        this.__updateSystemTime,
        this,
      );
    CarnivalManager.Instance.addEventListener(
      CarnivalManager.EVENT_UPDATE,
      this.onCarnivalUpdate,
      this,
    );
    NotificationManager.Instance.addEventListener(
      FunOpenEvent.UPDATE_FUNOPEN_STATE,
      this.checkNewFun,
      this,
    );
    ...
    if (this.warlordsModel)
      this.warlordsModel.addEventListener(
        WarlordsEvent.DATE_INIT_COMPLETE,
        this.refreshBtnViews,
        this,
      );
    NotificationManager.Instance.addEventListener(
      ExpBackEvent.UPDATE_EXPBACK_STATUS,
      this.refreshBtnViews,
      this,
    );
    NotificationManager.Instance.addEventListener(
      ConsortiaEvent.GOTO_GUILD_FIGHT,
      this.enterOutyardScene,
      this,
    );
    NotificationManager.Instance.addEventListener(
      PassCheckEvent.PASS_TASK_FRESH,
      this.__refreshWelfareRedPoint,
      this,
    );
  ...
  private onLineRewardBack(pkg: PackageIn) {
    let msg: OnlineRewardInfoRsp = pkg.readBody(
      OnlineRewardInfoRsp,
    ) as OnlineRewardInfoRsp;
  ...
  private onCrossScoreRewardHandler(pkg: PackageIn) {
    ...
    if (param4 == 1) {
      ...
      str = LangManager.Instance.GetTranslation(
        "crossscore.txt02",
        LangManager.Instance.GetTranslation("crossscore.name01"),
      );
    }
    if (param3 <= 0) {
      //未达到斗神宝箱的领取条件, 请再接再厉！
      ...
      str =
        LangManager.Instance.GetTranslation("crossscore.txt", param2) +
        str2 +
        str;
    }
  ...
  private onChallengeTakeHandler(pkg: PackageIn) {
    ...
    let str: string = LangManager.Instance.GetTranslation(
      "mainBar.TopToolsBar.str01",
      param1,
    );
    if (param2 > 0)
      str += LangManager.Instance.GetTranslation(
        "mainBar.TopToolsBar.str02",
        param2,
      );
    str += LangManager.Instance.GetTranslation(
      "mainBar.TopToolsBar.str04",
      param4,
    );
    SimpleAlertHelper.Instance.Show(
      null,
  ...
  private removeEvent() {
    ...
    QuestionnaireManager.Instance.model.removeEventListener(
      QuestionnaireEvent.QUESTIONNAIRE_CHANGE,
      this.updateQuestionVisible,
      this,
    );
    ...
    DayGuideManager.Instance.cate.removeEventListener(
      DayGuideEvent.LEED_PROGRESS_CHANGE,
      this.__refreshWelfareRedPoint,
      this,
    );
    ...
    DayGuideManager.Instance.removeEventListener(
      DayGuideEvent.DAILY_AWARD_STATE,
      this.__refreshWelfareRedPoint,
      this,
    );
    ...
    GoldenSheepManager.Instance.model.removeEventListener(
      GoldenSheepEvent.STATE_UPDATE,
      this.updateGoldenSheepVisible,
      this,
    );
    this.playerModel.playerInfo.removeEventListener(
      PlayerEvent.PLAYER_INFO_UPDATE,
      this.initResoueceTxt,
      this,
    );
    ...
    QQDawankaManager.Instance.model.removeEventListener(
      QQ_HALL_EVENT.GARDE_CHANGE,
      this.__refreshQQHallRedPoint,
      this,
    );
    QQGiftManager.Instance.model.removeEventListener(
      QQ_HALL_EVENT.GIFT_CHANGE,
      this.__refreshQQGiftRedPoint,
      this,
    );
    ...
    CarnivalManager.Instance.removeEventListener(
      CarnivalManager.EVENT_UPDATE,
      this.onCarnivalUpdate,
      this,
    );
  ...
  private checkNewFun() {
    if (this.thane.grades < OpenGrades.SHOW_FUNOPEN_TIP) {
      //10级出现
    ...
    if (newFunData && newFunData.length > 0) {
      this.btn_funOpen.title =
        LangManager.Instance.GetTranslation("FunOpen.opening"); //已开放
      ...
      this.btn_funOpen.icon = FunOpenManager.Instance.getFunIconUrl(
        curFun.Type,
      );
      ...
      if (nextArr.length) {
        ...
        this.btn_funOpen.title = LangManager.Instance.GetTranslation(
          "market.tips.openLevel",
          nextArr[0].Grade,
        ); //{0}级开放
  ...
  private onBtnListHClickItem(target) {
    ...
    switch (targetData.type) {
      ...
      case MainBtnType.FIRSTPAY: //首充送豪礼
        PlayerManager.Instance.currentPlayerModel.playerInfo.hasClickFirstCharge =
          true;
        this.__refreshFirstPayRedPoint();
      ...
      case MainBtnType.GOLDENSHEEL:
        let goldenSheepOpenTime: Date =
          GoldenSheepManager.Instance.model.openTime;
        let goldenSheepOpenTimeFormatterStr: string =
          LangManager.Instance.GetTranslation("public.dateFormat");
        let goldenSheepOpenTimeStr: string = DateFormatter.format(
          goldenSheepOpenTime,
          goldenSheepOpenTimeFormatterStr,
        );
        let tipStr: string;
        let index: number = GoldenSheepManager.Instance.model.index;
        if (index > 0) {
          let count: number = GoldenSheepManager.Instance.model.count;
          tipStr = LangManager.Instance.GetTranslation(
            "ui.toptoolbar.TopBtnItemView.goldenSheepOpenTips",
            count,
            index,
          );
        } else {
          tipStr = LangManager.Instance.GetTranslation(
            "ui.toptoolbar.TopBtnItemView.goldenSheepTips",
            goldenSheepOpenTimeStr,
          );
        }
      ...
      case MainBtnType.PVP:
        if (this._pvpCanJoin) {
          ...
          MessageTipManager.Instance.show(
            LangManager.Instance.GetTranslation(
              "mainBar.TopToolsBar.PvpTip",
              this._pvpCanJoinLeftTime,
            ),
          );
        }
        break;
  ...
  private enterOutyardScene() {
    ...
    let stateMsg: StackHeadStateMsg = OutyardManager.Instance.stateMsg;
    if (!stateMsg.myGuildSignup) {
      //未报名
      if (stateMsg.state == OutyardModel.COLLECT) {
        MessageTipManager.Instance.show(
          LangManager.Instance.GetTranslation(
            "outyard.TopToolsBar.NotCanSignup",
          ),
        );
        return;
      }
      if (stateMsg.canSignin) {
        //可报名
        if (this.isConsortiaChairman()) {
          //公会会长
          let config: number = ConfigInfoManager.Instance.getStackHeadJoinFee();
          let content: string = LangManager.Instance.GetTranslation(
            "outyard.OutyardBlessFrame.openTxt.text",
            config,
          );
          let prompt: string =
          ...
          SimpleAlertHelper.Instance.Show(
            ...
            confirm,
            cancel,
            this.__alertOutyardHandler.bind(this),
          );
        } else {
          //您所在的公会尚未报名
          MessageTipManager.Instance.show(
            LangManager.Instance.GetTranslation(
              "outyard.OutyardFrame.consortiaSignup",
            ),
          );
        }
      } //今日不可报名
        ...
        if (stateMsg.isSpan) {
          MessageTipManager.Instance.show(
            LangManager.Instance.GetTranslation(
              "outyard.TopToolsBar.nextOpenDateNew",
            ),
          );
        } else {
          MessageTipManager.Instance.show(
            LangManager.Instance.GetTranslation(
              "outyard.OutyardFrame.cannotReportToday",
            ),
          );
        }
      }
    ...
    if (nextOpenTime == 0) {
      MessageTipManager.Instance.show(
        LangManager.Instance.GetTranslation("outyard.TopToolsBar.noOpenToday"),
      );
      return;
    }
    ...
    //     return;
    // }
    if (!stateMsg.myGuildJoin) {
      //我自己今天不能玩
      MessageTipManager.Instance.show(
        LangManager.Instance.GetTranslation(
          "outyard.TopToolsBar.nextOpenDate",
          timeStr,
        ),
      );
      return;
  ...
  private checkCarnival(needRefresh: boolean = true) {
    if (CarnivalManager.Instance.isOpen) {
      this.createBtnData(
        MainBtnType.CARNIVAL,
        FUIHelper.getItemURL(EmPackName.Home, "Btn_Eve_Carnival"),
        11,
        LangManager.Instance.GetTranslation("TopToolBar.btnText12"),
      );
    } else {
  ...
  private isConsortiaChairman(): boolean {
    ...
    if (consortiaControler) {
      ...
      if (consortiaModel) {
        flag = consortiaModel.getRightsByIndex(
          ConsortiaDutyInfo.STACKHEAD_SIGNIN,
        );
      }
    }
    return flag;
  ...
  private checkWorldBoss() {
    if (this.thane.grades >= OpenGrades.WORLD_BOSS) {
      if (this.playerInfo.worldbossState) {
        ...
        this.createBtnData(
          MainBtnType.WORDLBOSS,
          FUIHelper.getItemURL(EmPackName.Home, "Btn_Eve_WorldBOSS"),
          6,
          LangManager.Instance.GetTranslation("TopToolBar.btnText3"),
        );
      } else {
  ...
  private checkRvrBattle(needRefresh: boolean = true) {
    ...
    if (DataCommonManager.playerInfo.isOpenPvpWar) {
      this.createBtnData(
        MainBtnType.RVRBATTLE,
        FUIHelper.getItemURL(EmPackName.Home, "Btn_Eve_Battle"),
        7,
        LangManager.Instance.GetTranslation("TopToolBar.btnText4"),
      );
    } else {
  ...
  private initGrowgrowthFundData() {
    this.ctrlData._growthFundInfoArr =
      TempleteManager.Instance.getGrowthFundInfoArr();
    let firstItem: GrowthFundItemInfo = new GrowthFundItemInfo();
    firstItem.packageState = 1;
    let rechargeData = TempleteManager.Instance.getRechargeTempletes(
      ProductType.GROWTH_RECHARGE,
    );
  ...
  initDegreeActivityData() {
    //获取活跃度配置宝箱
    let activityBoxs = TempleteManager.Instance.getDropConditionByType(
      DropCondictionType.DEGREE,
    );
  ...
  private getTipsStr(value: number): string {
    var arr: Array<t_s_dropitemData> =
      TempleteManager.Instance.getDropItemssByDropId(value);
    var str: string =
      LangManager.Instance.GetTranslation(
      ...
      for (var i: number = 0; i < arr.length; i++) {
        if (i == arr.length - 1) {
          if (arr[i].ItemId < 0) {
            ...
            str +=
              this.getGoodsName(arr[i].ItemId) +
              arr[i].Data +
              LangManager.Instance.GetTranslation(
                "shop.view.frame.BuyFrame.vipBuyInfo03",
              );
          }
        } else {
          if (arr[i].ItemId < 0) {
  ...
  private getGoodsName(itemId: number): string {
    let itemTmp =
      TempleteManager.Instance.getGoodsTemplatesByTempleteId(itemId);
    if (itemTmp) {
  ...
  __refreshFunnyHandler() {
    ...
    if (
      ...
      ChargeLotteryManager.instance.chargeMsg.leftCount > 0
    ) {
      awardState = true;
    }
  ...
  private _refreshSevenGoalsRedPoint() {
    ...
    for (let i = 1; i <= 7; i++) {
      let receiveActive =
        SevenGoalsManager.Instance.sevenGoalsModel.checkDayRed(i);
      if (receiveActive) {
  ...
  __refreshWelfareRedPoint() {
    ...
    if (
      !PlayerManager.Instance.currentPlayerModel.playerInfo.hasBuyGrowthFund &&
      !PlayerManager.Instance.currentPlayerModel.playerInfo
        .todayHasClickGrowthFund
    ) {
      //未购买成长基金并且当天没有点击过成长基金页签
      growthFundFlag = true;
    } else {
      //已经购买
    ...
    if (
      this.ctrlData.expBackItemDataArr.length > 0 &&
      !this.ctrlData.hasClickExpTab
    ) {
      expBackFlag = true;
    }
    ...
    let showRed = false;
    if (
      PlayerManager.Instance.currentPlayerModel.playerInfo.isPassOpen &&
      this.ctrlData.passRewardInfo.leftTime > 0
    ) {
      showRed = PassportView.isLogin && !this.ctrlData.passRewardInfo.isPay;
    }
    let passCheckFlag: boolean =
      this.ctrlData.canReceivePassCheckReward > 0 ||
      this.ctrlData.canReceivePassCheckTaskReward ||
      showRed;

    let welfareAwardState =
      signAwardState ||
      canReceiveLevelGift ||
      activityAwardState ||
      sevenLoginAwardState ||
      growthFundFlag ||
      expBackFlag ||
      this._onlineFlag ||
      passCheckFlag;
    if (WelfareManager.Instance.isReachOpenBindCon(1)) {
      welfareAwardState =
        welfareAwardState || WelfareManager.Instance.getBindPhoneRedFlag();
    }
    if (WelfareManager.Instance.isReachOpenBindCon(2)) {
      welfareAwardState =
        welfareAwardState || WelfareManager.Instance.getBindMailRedFlag();
    }
  ...
  __refreshQQHallRedPoint() {
    if (Utils.isQQHall()) {
      this.setTabBtnState(
        MainBtnType.QQHALL_DAWANKA,
        QQDawankaManager.Instance.model.isRedDot(),
      );
    }
  }
  ...
  __refreshQQGiftRedPoint() {
    if (Utils.isQQHall() && ConfigManager.info.QQ_HALL_GIFT) {
      this.setTabBtnState(
        MainBtnType.QQHALL_GIFT,
        QQGiftManager.Instance.model.isRedDot(),
      );
    }
  }
  ...
  private __refreshCarnivalRedPoint() {
    ...
    if (btn) {
      ...
      let carnivalLuckyTimes =
        CarnivalManager.Instance.model.hasLuckyDarwCount(); //幸运夺宝次数
      let carnivalFreeDiscount =
        CarnivalManager.Instance.model.hasFreeDiscount(); //特惠礼包免费礼包
      let carnivalOnlineAward =
        CarnivalManager.Instance.model.hasOnlineReward(); //在线奖励
      if (CarnivalManager.Instance.isRewardTime) {
        b = carnivalGameCount || carnivalFreeDiscount || carnivalOnlineAward;
      }
      let carnivalScoreAward =
        CarnivalManager.Instance.model.hasScoreRewardInfoAward(); //嘉年华 积分领奖
      let carnivalDaytaskAward =
        CarnivalManager.Instance.model.hasDayTaskReward(); //每日挑战奖励
      let carnivalRecharge =
        CarnivalManager.Instance.model.hasLeftlotteryCount(); //充值有礼
  ...
  private __refreshWarlordsBtnStatus() {
    ...
    let curTime: number =
      PlayerManager.Instance.currentPlayerModel.sysCurTimeBySecond;
    let endTime: number =
      WarlordsManager.Instance.model.getMatchDate(4).getTime() / 1000;
    var isOpenTime: boolean =
      (curTime > beginTime && curTime < endTime) || this.checkWarlordsProcess();
    if (
      ComponentSetting.WARLORD &&
  ...
  private __refreshRvrBattleBtnStatus() {
    ...
    if (btn) {
      if (this.thane.grades >= OpenGrades.RVR && btn) {
        if (
          DataCommonManager.playerInfo.isOpenPvpWar &&
          ArmyManager.Instance.thane.grades >= OpenGrades.RVR
        ) {
          UIButton.setEffect(btn.view, 1);
        } else {
          UIButton.setEffect(btn.view, 0);
  ...
  private __refreshPvpBtnstatus() {
    ...
    if (this.thane.grades >= OpenGrades.CHALLENGE) {
      ...
      for (let index = 0; index < this._pvpOpenTimeArr.length; index++) {
        const element = this._pvpOpenTimeArr[index];
        if (
          curTimeMin >= element.startTimeMin &&
          curTimeMin <= element.endTimeMin
        ) {
