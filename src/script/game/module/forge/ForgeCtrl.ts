...
import {BagEvent, ResourceEvent, StoreEvent} from "../../constant/event/NotificationEvent";
...
export default class ForgeCtrl extends FrameCtrlBase {
    ...
    protected addEventListener() {
        ...
        ServerDataManager.listen(S2CProtocol.U_C_ITEM_COMPOSE, this, this.__onComposeResult);
        ...
        ServerDataManager.listen(S2CProtocol.U_C_REFRESHFORRANDOM, this, this.__onRefreshRandomProperty);
        ServerDataManager.listen(S2CProtocol.U_C_ITEM_REFRESHPROPERTY, this, this.__onRefreshItemProperty);
        ServerDataManager.listen(S2CProtocol.U_C_BAG_MOVE_UPDATE, this, this.__onBagMoveUpdate);
        ServerDataManager.listen(S2CProtocol.U_C_MOULD_ITEM, this, this.__onRecvMouldItem);

        this.data.addEventListener(StoreEvent.REFRESH_LOCK_CHANGE, this.__refreshItemLock, this);
        GoodsManager.Instance.addEventListener(BagEvent.UPDATE_BAG, this.__bagItemUpdate, this);
        GoodsManager.Instance.addEventListener(BagEvent.DELETE_BAG, this.__bagItemDelete, this);
        ResourceManager.Instance.gold.addEventListener(ResourceEvent.RESOURCE_UPDATE, this.onResourceUpdate, this);
    }
    // override
    protected delEventListener() {
        super.delEventListener()
        ServerDataManager.cancel(S2CProtocol.U_C_ITEM_COMPOSE, this, this.__onComposeResult);
        ...
        ServerDataManager.cancel(S2CProtocol.U_C_REFRESHFORRANDOM, this, this.__onRefreshRandomProperty);
        ServerDataManager.cancel(S2CProtocol.U_C_ITEM_REFRESHPROPERTY, this, this.__onRefreshItemProperty);
        ServerDataManager.cancel(S2CProtocol.U_C_BAG_MOVE_UPDATE, this, this.__onBagMoveUpdate);
        ServerDataManager.cancel(S2CProtocol.U_C_MOULD_ITEM, this, this.__onRecvMouldItem);

        this.data.removeEventListener(StoreEvent.REFRESH_LOCK_CHANGE, this.__refreshItemLock, this);
        GoodsManager.Instance.removeEventListener(BagEvent.UPDATE_BAG, this.__bagItemUpdate, this);
        GoodsManager.Instance.removeEventListener(BagEvent.DELETE_BAG, this.__bagItemDelete, this);
        ResourceManager.Instance.gold.removeEventListener(ResourceEvent.RESOURCE_UPDATE, this.onResourceUpdate, this);
        this.moveGoodsBack();
    }
    ...
    private __bagItemUpdate(infos: GoodsInfo[]) {
        for (let info of infos)
            this.view.refresh(undefined, "BagItemUpdate", info)
    }
    ...
    public openHole(pos: number) {
        ...
        if (hasProp) {
            let content: string = LangManager.Instance.GetTranslation("store.StoreControler.content02");
            SimpleAlertHelper.Instance.Show(null, null, null, content, null, null, this.confirmback.bind(this));
        } else {
            let info: ShopGoodsInfo = TempleteManager.Instance.getShopTempInfoByItemId(ItemID.BORT_PROP);
    ...
    private inlayConfirm(b: boolean, check: boolean, joinPos: number, gInfoPos: number, gInfoType: number) {
        if (b)
            ForgeSocketOutManager.sendItemMount(joinPos, gInfoPos, gInfoType);
    }
    ...
    public sendCompose(compAll: boolean, useBind: boolean) {
        ...
        if(this.data.composeType == ForgeData.COMPOSE_TYPE_ADVANCE_EQUIP)
        {
            ...
            if(goodsInfo)
            {
                ...
                ForgeSocketOutManager.sendUpdateEquip(this.data.selectedCompose.templateId, 1, useBind, goodsInfo.mouldGrade > 0);
            }
        }
        else
        {
            ...
            PlayerManager.Instance.currentPlayerModel.forgeHeChengIsOpen = true;
            ForgeSocketOutManager.sendCompose(this.data.selectedCompose.template.Id, composeCount, this.data.composeAutoBuy, useBind);
        }
    }
    ...
    private __showComposeAlert(b: boolean, check: boolean = false, data: any) {
        if (b) {
            ...
            let composeCount = data.compAll ? this.data.maxComposeCount : this.data.currentComposeCount
            ForgeSocketOutManager.sendCompose(this.data.selectedCompose.template.Id, composeCount, this.data.composeAutoBuy, this.useBind);
        }
    }
}