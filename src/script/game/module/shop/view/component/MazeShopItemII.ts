...
export class MazeShopItemII extends FUI_MazeShopItemII {
    ...
    public set vdata(value: ShopGoodsInfo) {
        ...
        if (this._vData) {
            this.OpenDescibleTxt.text = "";
            if (this._vData.MazeLayers > PlayerManager.Instance.currentPlayerModel.towerInfo1.maxIndex) {
                this.OpenDescibleTxt.text = LangManager.Instance.GetTranslation("maze.shop.tips", this._vData.MazeLayers);
                UIFilter.gray(this);
                this.state.selectedIndex = 1;
            }
            if (this._vData.MazeLayers2 > PlayerManager.Instance.currentPlayerModel.towerInfo2.maxIndex) {
                this.OpenDescibleTxt.text = LangManager.Instance.GetTranslation("maze.shop.tips2", this._vData.MazeLayers2);
                UIFilter.gray(this);
                this.state.selectedIndex = 1;
            }
    ...
    private getName(): string {
        ...
        let goodsTempInfo: t_s_itemtemplateData = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_itemtemplate, this._vData.ItemId);
        if (!goodsTempInfo) {
            this._vData = null;
        ...
        this.GoodsNameText.color = FilterFrameText.Colors[eFilterFrameText.ItemQuality][goodsInfo.templateInfo.Profile - 1];
