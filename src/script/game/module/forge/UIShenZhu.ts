...
export default class UIShenZhu extends BaseFguiCom {
  ...
  constructor(comp: fgui.GComponent) {
    ...
    this.txt3.text =
      LangManager.Instance.GetTranslation("store.cast.CastView.BlessTxt") + ":";
    this.txt4.text =
      LangManager.Instance.GetTranslation("fashion.view.compose.composeRate") +
      ":";
    this.txt_max80.text = LangManager.Instance.GetTranslation(
      "faterotary.LevelMax",
    );
  ...
  private updateView() {
    ...
    this.img_textbg1.visible =
      this.img_textbg0.visible =
      this.imgArrow.visible =
        false;
    ...
    if (info) {
      ownNum1 = GoodsManager.Instance.getGoodsNumByTempId(
        info.MOULD_NEED_MATERIAL1,
      );
      ownNum2 = GoodsManager.Instance.getGoodsNumByTempId(
        info.MOULD_NEED_MATERIAL2,
      );
      ...
      this.img_textbg1.visible =
        ...
        this.imgArrow.visible =
          true;
      if (info.mouldGrade < info.MOULD_MAX_GRADE) {
        //未达满级
        mouldGrade = info.mouldGrade;
        this.c1.setSelectedIndex(0);
        if (
          info.templateInfo.SonType == GoodsSonType.SONTYPE_RELIC ||
          info.templateInfo.SonType == GoodsSonType.SONTYPE_HERALDRY
        ) {
        ...
        let color =
          ResourceManager.Instance.gold.count > cost ? "#FFECC6" : "#FF0000";
    ...
    if (info && !this.gMaxGrade.visible) {
      ...
      this.material1.item.titleColor =
        ownNum1 >= needMaterialCount1 ? "#ffffff" : "#ff2e2e";
    } else {
    ...
    if (info && !this.gMaxGrade.visible) {
      ...
      this.material2.item.titleColor =
        ownNum2 >= needMaterialCount2 ? "#ffffff" : "#ff2e2e";
    } else {
      this.material2.item.text = "--/--";
    }
    this.castBtn.enabled =
      ownNum1 >= needMaterialCount1 &&
      ownNum2 >= needMaterialCount2 &&
      ownNum1 > 0 &&
      ownNum2 > 0;
    ...
    if (goodsInfo && mouldGrade <= goodsInfo.MOULD_MAX_GRADE) {
      if (mouldGrade > 0) {
        let str = LangManager.Instance.GetTranslation(
          "yishi.view.tips.goods.MouldItem.Tips03",
          tempInfo.mouldRankShow,
          tempInfo.mouldStar,
        );
        if (isnext) {
          this.txt2.text = str;
        ...
        this.txt1.text = LangManager.Instance.GetTranslation(
          "yishi.view.tips.goods.MouldItem.Tips02",
        );
        // this.star1.visible = false;
        // _mouldMoon.visible = false;
        // _mouldDay.visible = false;
  ...
  private initAttribute(info: GoodsInfo, temp: t_s_itemtemplateData, isnext) {
    ...
    this.updateAttributeTxt(
      str,
      temp.Power,
      this.getAdd(temp.Power, info, isnext),
      bMaxGrade,
      this.getAdd(temp.Power, info, false),
    );
    ...
    this.updateAttributeTxt(
      str,
      temp.Agility,
      this.getAdd(temp.Agility, info, isnext),
      bMaxGrade,
      this.getAdd(temp.Agility, info, false),
    );
    ...
    this.updateAttributeTxt(
      str,
      temp.Intellect,
      this.getAdd(temp.Intellect, info, isnext),
      bMaxGrade,
      this.getAdd(temp.Intellect, info, false),
    );
    ...
    this.updateAttributeTxt(
      str,
      temp.Physique,
      this.getAdd(temp.Physique, info, isnext),
      bMaxGrade,
      this.getAdd(temp.Physique, info, false),
    );
    ...
    this.updateAttributeTxt(
      str,
      temp.Captain,
      this.getAdd(temp.Captain, info, isnext),
      bMaxGrade,
      this.getAdd(temp.Captain, info, false),
    );
    ...
    this.updateAttributeTxt(
      str,
      temp.Attack,
      this.getAdd(temp.Attack, info, isnext),
      bMaxGrade,
      this.getAdd(temp.Attack, info, false),
    );
    ...
    this.updateAttributeTxt(
      str,
      temp.MagicAttack,
      this.getAdd(temp.MagicAttack, info, isnext),
      bMaxGrade,
      this.getAdd(temp.MagicAttack, info, false),
    );
    ...
    this.updateAttributeTxt(
      str,
      temp.Defence,
      this.getAdd(temp.Defence, info, isnext),
      bMaxGrade,
      this.getAdd(temp.Defence, info, false),
    );
    ...
    this.updateAttributeTxt(
      str,
      temp.MagicDefence,
      this.getAdd(temp.MagicDefence, info, isnext),
      bMaxGrade,
      this.getAdd(temp.MagicDefence, info, false),
    );
    ...
    this.updateAttributeTxt(
      str,
      temp.Parry,
      this.getAdd(temp.Parry, info, isnext),
      bMaxGrade,
      this.getAdd(temp.Parry, info, false),
    );
    ...
    this.updateAttributeTxt(
      str,
      temp.Live,
      this.getAdd(temp.Live, info, isnext),
      bMaxGrade,
      this.getAdd(temp.Live, info, false),
    );
    ...
    this.updateAttributeTxt(
      str,
      temp.ForceHit,
      this.getAdd(temp.ForceHit, info, isnext),
      bMaxGrade,
      this.getAdd(temp.ForceHit, info, false),
    );
    ...
    this.updateAttributeTxt(
      str,
      temp.Conat,
      this.getAdd(temp.Conat, info, isnext),
      bMaxGrade,
      this.getAdd(temp.Conat, info, false),
    );
  }
  ...
  private getAdd(preValue: number, gInfo: GoodsInfo, isnext: boolean): number {
    let curr: number = GoodsHelp.getMouldAddition(
      preValue,
      this.getPropertyMouldGrade(gInfo, isnext),
    );
    return curr;
  }
  ...
  private onBtnConfirm() {
    ...
    if (
      ...
      parseInt(StringHelper.trim(this.txtCost.text))
    ) {
    ...
    if (
      ...
      str = LangManager.Instance.GetTranslation(
        "store.cast.CastView.MouldTips03",
      );
      MessageTipManager.Instance.show(str);
      return;
    }
