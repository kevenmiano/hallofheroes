...
export class TransportMediator implements IMediator {
  ...
  public register(target: any) {
    ...
    this._target.avatarView.on(
      Avatar.SIZETYPE_CHANGE,
      this,
      this.__onThaneInfoChange,
    ); //外形改变
    if (this.isSelf) {
    ...
    if (this.inMineral && this.mineralModel)
      this.mineralModel.addEventListener(
        CampaignEvent.UPDATE_MINERAL_CAR,
        this.__mineralCarHandler,
        this,
      );
    this.__isDieHandler();
  ...
  private checkMineralCar() {
    if (this.inMineral) {
      if (
        this.mineralModel.carInfos[this._data.baseHero.userId] &&
  ...
  private transportState(value: boolean) {
    if (value) {
      ...
      this._transport.url = FUIHelper.getItemURL(
        EmPackName.Base,
        "transporting",
      );
      ...
      this._transport.y = this._target.caluateTopPos();
      if (this.inPvpWar) {
        this._target.changeSpeedTo(0.5);
      }
      let info: ThaneInfo;
      ...
      if (this._transport && this._transport.displayObject.parent)
        this._transport.displayObject.parent.removeChild(
          this._transport.displayObject,
        );
  ...
  private readNodeInfo(): CampaignNode {
    ...
    if (this.inPvpWar) {
      infoNameTip = LangManager.Instance.GetTranslation(
        "map.campaign.mediator.pvp.TransportMediator.infoNameTip",
      );
      ...
      node.sonType = 5000;
      info.state =
        this.thane.userId == this._data.baseHero.userId
          ? NodeState.FRIENDLY
          : NodeState.EXIST;
