...
export class OuterCityFieldInfoWnd extends BaseWindow {
    ...
    private initView() {
        this.frame.title = this._wildLand.tempInfo.NameLang + " " + LangManager.Instance.GetTranslation("public.level3",this._wildLand.tempInfo.Grade);
        this.txt_name.text = this._wildLand.tempInfo.NameLang + "(" + this._wildLand.convertX + "," + this._wildLand.convertY + ")\n" + this.mapName;

        this.frame.title = LangManager.Instance.GetTranslation("public.level.name", this._wildLand.tempInfo.NameLang, this._wildLand.tempInfo.Grade)

        this.txt_name.text = this._wildLand.tempInfo.NameLang + "(" + this._wildLand.convertX + "," + this._wildLand.convertY + ")\n" + this.mapName;

        if (this._wildLand.info.occupyPlayerId == 0) {
            this.txt_occupation.text = LangManager.Instance.GetTranslation("map.internals.view.frame.FieldInfoView.occupationLordsFrameText2Text1");
            this.btn_operate.title = LangManager.Instance.GetTranslation("public.attack");
        } else {
            let occupationLordsFrameText2Text2: string = LangManager.Instance.GetTranslation("map.internals.view.frame.FieldInfoView.occupationLordsFrameText2Text2");
            this.txt_occupation.text = this._wildLand.info.occupyPlayerName || occupationLordsFrameText2Text2;
            if (this._wildLand.info.occupyPlayerId == this.thane.userId) {
                this.btn_operate.title = LangManager.Instance.GetTranslation("map.internals.view.frame.FieldInfoView.attackBtnText1");
            } else {
                this.btn_operate.title = LangManager.Instance.GetTranslation("public.attack");
            }
        }
        // this.txt_gold.text = "+" + this._wildLand.tempInfo.Property4 + "/" + LangManager.Instance.GetTranslation("public.time.hour");
        let curTime: number = PlayerManager.Instance.currentPlayerModel.sysCurTimeBySecond * 1000;
    ...
    private onBtnOperateClick() {
        if (this.thane.userId == this._wildLand.info.occupyPlayerId) {
            ...
            let prompt: string = LangManager.Instance.GetTranslation("map.FieldInfoFrame.prompt");
            let content: string = LangManager.Instance.GetTranslation("map.FieldInfoFrame.content");
            SimpleAlertHelper.Instance.Show(SimpleAlertHelper.SIMPLE_ALERT, null, prompt, content, confirm, cancel, this.__giveUpCall.bind(this));
        }
        else {
            OuterCityManager.Instance.controler.sendAttack(this._wildLand.posX, this._wildLand.posY);
            this.hide();
        }
    }
    ...
    private get mapName(): string {
        let map: t_s_mapData = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_map, this._wildLand.info.mapId);
