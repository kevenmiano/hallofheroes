...
export class OuterCitySocketOutManager {
     ...
     * 
     */
    public static sendAttackMine(mapId: number, posX: number, posY: number, nodeId: number = 0, sonNodeId: number = 0): void {
        var msg: MapPhysicAttackMsg = new MapPhysicAttackMsg();
        msg.mapId = mapId;
        msg.posX = posX;
        msg.posY = posY;
        ...
        Logger.xjy("[OuterCitySocketOutManager]sendAttack", mapId, posX, posY, nodeId, sonNodeId);
        SocketManager.Instance.send(C2SProtocol.U_C_ARMY_ATTACK, msg);
    }
    /**
     *传回城堡
     ...
     * @param  
     */
    public static removeMine(position: string, nodeId: number, sonNodeId: number) {
     ...
     * 请求金矿二级节点下面的子节点数据
     */
    public static requestSonNodeData(mapId: number, posX: number, posY: number, nodeId: number) {
        var msg: MapPhysicAttackMsg = new MapPhysicAttackMsg();
        msg.mapId = mapId;
        msg.posX = posX;
        msg.posY = posY
