...
export default class ConfigSoliderWnd extends BaseWindow {
  ...
  private Data() {
    let str: string =
      this._pawn.templateInfo.PawnNameLang + this._pawn.templateInfo.Level;
    ...
    if (emputyPos == 0) {
      ...
      if (this._soliderNumber == 0) {
        MessageTipManager.Instance.show(
          LangManager.Instance.GetTranslation("ConfigSoliderwnd.moveconfig"),
        );
        return;
      } else {
        tarNeedNum =
          tarTotalNum > this._soliderNumber ? this._soliderNumber : tarTotalNum;
        ArmyManager.Instance.army.addNewPawnByIndex(
          emputyPos,
          this._pawn,
          tarNeedNum,
        );
        ArmyManager.Instance.removePawnCountById(
      ...
      if (
        serPawn1.ownPawns > 0 &&
        serPawn1.templateId == this._pawn.templateId
      ) {
        if (serPawn1.ownPawns >= tarTotalNum) return;
        tarNeedNum = tarTotalNum - serPawn1.ownPawns;
        tarNeedNum =
          tarNeedNum > this._soliderNumber ? this._soliderNumber : tarNeedNum;
