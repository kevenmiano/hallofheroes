...
export default class SkillEditWnd extends BaseWindow {
  ...
  initLanguage() {
    for (let i = 2; i < 5; i++) {
      const element = LangManager.Instance.GetTranslation(
        "yishi.datas.consant.JobType.Name0" + i,
      );
    ...
    this.txt4.text = LangManager.Instance.GetTranslation(
      "FightSkillEdit.txt16",
    );
    this.txt5.text = LangManager.Instance.GetTranslation(
      "FightSkillEdit.txt17",
    );
    this.txt6.text = LangManager.Instance.GetTranslation(
      "FightSkillEdit.txt18",
    );
    this.txt7.text = LangManager.Instance.GetTranslation(
      "FightSkillEdit.txt19",
    );
    this.btn_reset.title = LangManager.Instance.GetTranslation(
      "armyII.viewII.information.InformationView.reset",
    );
    this.btn_save.title = LangManager.Instance.GetTranslation(
      "FightSkillEdit.txt20",
    );
    this.checkBtn.title = LangManager.Instance.GetTranslation(
      "FightSkillEdit.txt5",
    );
  }
  ...
  OnShowWind() {
    ...
    this.defaultSkill =
      TempleteManager.Instance.getMinSkillTemplateInfoBySonType(sonType);
  ...
  private addEvent() {
    for (let i = 1; i <= 4; i++) {
      ...
      let item1 = this.contentPane.getChild(
        "s_itemSkill_" + i,
      ) as EditSkillItem;
    ...
    this.list_tab.itemRenderer = Laya.Handler.create(
      this,
    ...
    SkillEditModel.instance.addEventListener(
      SkillEvent.SKILL_EDIT,
      this.onRecvMsg,
      this,
    );
  }
  ...
  private removeEvent(): void {
    ...
    SkillEditModel.instance.removeEventListener(
      SkillEvent.SKILL_EDIT,
      this.onRecvMsg,
      this,
    );
  ...
  refresh() {
    ...
    if (this._skillEditData.normalSkill != "") {
      ...
      for (let i = 1; i <= 4; i++) {
        ...
        if (str != "") {
          ...
          let item = this.contentPane.getChild(
            "itemSkill_" + i,
          ) as EditSkillItem;
          if (item) {
            if (arr[0] == "3") {
    ...
    if (
      this._skillEditData.normalSkill != "" ||
      this._skillEditData.specialSkill != ""
    ) {
      // this.hasEdited = true;
      this.updateButtonState();
    }
    ...
    if (this._curType == 0) {
      ...
      if (item) {
        ...
        if (resArr[0] == "2") {
          ...
          if (
            item.skillData &&
            item.skillData instanceof t_s_runetemplateData
          ) {
            //符文被替换成其他技能
            SkillEditModel.instance.curRuneNum--;
          }
      ...
      SkillEditModel.instance.setNormalPos(
        this._curJob,
        parseInt(index) - 1,
        skillId,
      );
      ...
      SkillEditModel.instance.setSpeciallPos(
        this._curJob,
        parseInt(index) - 1,
        skillId,
      );
  ...
  onAdd(btn: fairygui.GButton) {
    if (this.isEditing) {
      ...
      FrameCtrlManager.Instance.open(EmWindow.SkillEditSelectWnd, {
        job: this._curJob,
        type: this._curType,
      });
    }
  ...
  private onChange(): void {
    ...
    if (value > this._max) {
      ...
      MessageTipManager.Instance.show(
        LangManager.Instance.GetTranslation("FightSkillEdit.txt14"),
      );
    } else {
  ...
  public onTabClick(item: fgui.GObject, evt: Laya.Event) {
    if (item) {
      if (this.isEditing) {
        ...
        MessageTipManager.Instance.show(
          LangManager.Instance.GetTranslation("FightSkillEdit.txt21"),
        );
        return;
      }
    }
    ...
    this._skillEditData = SkillEditModel.instance.getSkillEditData(
      this._curJob,
    );
    this.refresh();
  }
    ...
    SimpleAlertHelper.Instance.Show(
      ...
      (b: boolean, flag: boolean, arr: any[]) => {
        if (b) {
          ...
          this.control.reqSkillEditInfo(
            2,
            this._curJob,
            this._percent,
            this._skillEditData.normalSkill,
            this._skillEditData.specialSkill,
            this._skillEditData.petId,
            this.checkBtn.selected,
          );
  ...
  onSave() {
    if (this.isEditing == false) {
      ...
      if (Number(this.txt_input.text) < this._min) {
        ...
        MessageTipManager.Instance.show(
          LangManager.Instance.GetTranslation("FightSkillEdit.txt23"),
        );
        return;
      }
      this.isEditing = false;
  ...
  onCheck() {
    ...
    if (this.checkEdit()) {
      ...
      PlayerManager.Instance.currentPlayerModel.playerInfo.isSkillEditOpen =
        !this.checkBtn.selected;
    } else {
      setTimeout(() => {
      ...
      MessageTipManager.Instance.show(
        LangManager.Instance.GetTranslation("FightSkillEdit.txt13"),
      );
    }
  }
