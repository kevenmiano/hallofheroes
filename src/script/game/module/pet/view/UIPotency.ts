...
export default class UIPotency extends BaseFguiCom {
	...
	private initView() {
		for (let i: number = 1; i <= 3; i++) {
			this["txtDesc" + i].text = LangManager.Instance.GetTranslation("petWnd.potency.txtDesc" + i);
		}
		for (let j: number = 1; j <= 5; j++) {
			this["itemQualification" + j].titleTxt.text = LangManager.Instance.GetTranslation("petWnd.potency.itemTitleTxt" + j);
		}
	...
	private __updateNumberHandler() {
		...
		this._initValue = GoodsManager.Instance.getGoodsNumByTempId(this._selectItem.info);
		if (this._initValue > 0) {
			let max = this.getMaxCost() > this._initValue ? this._initValue : this.getMaxCost();
	...
	private updatePetValue() {
		for (let j: number = 1; j <= 5; j++) {
			let maxWidth = this.getMaxValue(j) * UIPotency.MAX_WIDTH / UIPotency.MAX_VALUE;//算出来进度条的最大宽度;
			if (this.getMaxValue(j) > UIPotency.MAX_VALUE) {
				...
				(this["itemQualification" + j].prog2 as fgui.GProgressBar).width = UIPotency.MAX_WIDTH;
				(this["itemQualification" + j].prog3 as fgui.GProgressBar).width = maxWidth - UIPotency.MAX_WIDTH;
				this["itemQualification" + j].txtValue.text = this._petData.potencyArr[j - 1] + "/" + this.getMaxValue(j);
				(this["itemQualification" + j].prog2 as fgui.GProgressBar).max = UIPotency.MAX_VALUE;
				(this["itemQualification" + j].prog3 as fgui.GProgressBar).max = this.getMaxValue(j) - UIPotency.MAX_VALUE;
				if (this._petData.potencyArr[j - 1] > UIPotency.MAX_VALUE) {
					(this["itemQualification" + j].prog2 as fgui.GProgressBar).value = UIPotency.MAX_VALUE;
					(this["itemQualification" + j].prog3 as fgui.GProgressBar).value = this._petData.potencyArr[j - 1] - UIPotency.MAX_VALUE;
				} else {
					(this["itemQualification" + j].prog2 as fgui.GProgressBar).value = this._petData.potencyArr[j - 1];
					(this["itemQualification" + j].prog3 as fgui.GProgressBar).value = 0
				}
				if (this._petData.potencyArr[j - 1] - this._petData.potencyOldArr[j - 1] > 0) {
					this["itemQualification" + j].addTxt.text = "+" + (this._petData.potencyArr[j - 1] - this._petData.potencyOldArr[j - 1]).toString();
				...
				(this["itemQualification" + j].prog1 as fgui.GProgressBar).value = this._petData.potencyArr[j - 1];
				if (this._petData.potencyArr[j - 1] - this._petData.potencyOldArr[j - 1] > 0) {
					this["itemQualification" + j].addTxt.text = "+" + (this._petData.potencyArr[j - 1] - this._petData.potencyOldArr[j - 1]).toString();
					this._petData.potencyOldArr[j - 1] = this._petData.potencyArr[j - 1];
				} else {
	...
	private btnActiveHandler() {
		...
		if (this.hasReciveMax()) {
			MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("UIPotency.btnActiveTips"));
			return;
		}
		if (this._initValue == 0) {
		...
		PetCtrl.sendPetPotencyActive(this._petData.petId, this._selectItem.info, this.simBox.value);
	}
	...
	public set data(value: PetData) {
		if (this._petData) {
			if (this._petData.petId == value.petId) {//更新当前英灵的数据
				if (this._selectItem) {
					this.btnActive.enabled = true;
					this.__updateNumberHandler();
				} else {
	...
	private getMaxValue(type: number): number {
		...
		let tempId: number =  this._petData.template.TemplateId + quality - this._petData.template.Quality;
