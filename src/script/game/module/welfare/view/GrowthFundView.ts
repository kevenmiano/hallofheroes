...
export class GrowthFundView extends FUI_GrowthFundView {
  ...
  activeBtnHandler() {
    if (!this.playerInfo.hasBuyGrowthFund && this._grouthFunData) {
      //没有购买过成长基金
  ...
  refreshView() {
    if (!this.playerInfo.hasBuyGrowthFund) {
      //没有购买过成长基金
      if (Utils.isInteger(this.fund_money.toString())) {
        this.txt_money.text =
          ShopManager.Instance.getMoneyString() +
          parseInt(this.fund_money.toString()); //价格
      } else {
        this.txt_money.text =
          ShopManager.Instance.getMoneyString() + this.fund_money.toString(); //价格
      }
      ...
      this.btn_active.title = LangManager.Instance.GetTranslation(
    ...
    this.btn_active.titleColor = this.btn_active.enabled
      ? enableColor
      : disableColor;
    this.txt_num.text =
      this.model.hasGetBindCount + "/" + this.model.totalBindCount;
