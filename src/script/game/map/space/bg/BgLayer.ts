...
export class BgLayer extends Laya.Sprite implements Disposeable {
  ...
  constructor(
    ...
    if (this._mapTemplate.MapFileId == 30000) {
      //|| this._mapTemplate.MapFileId == 5201
      this._bgModel = new BgModel(
        this._mapTemplate.Width,
        this._mapTemplate.Height,
        this,
        this._mapTemplate.Id,
        this._mapTemplate.MapFileId,
        300,
        false,
      );
    } else {
  ...
  public dragingCallBack(e: Event = null) {
    ...
    if (this.height * this.scaleY < StageReferance.stageHeight) {
      ...
      this.y <
      StageReferance.stageHeight - this.height * this.scaleY
    ) {
  ...
  public removeUselessTiles() {
    ...
    for (let tx: number = 0; tx < this._bgModel.row; tx++) {
      for (let ty: number = 0; ty < this._bgModel.col; ty++) {
        ...
        if (this._bgModel.loadQueue.indexOf(tileInfo) == -1) {
          //不在加载队列中
  ...
  protected loadViewportTiles() {
    ...
    for (let tileInfo of arr) {
      ...
      let url: string =
        PathManager.getMapBackPaths(tileInfo.mapFileId) +
        "tile_" +
        tileInfo.key +
        ext;
  ...
  private renderTitle() {
    ...
    if (
      this._mapTemplate.Width > this.renderMax ||
      this._mapTemplate.Height > this.renderMax
    ) {
      this.handleLargeMap();
    } else {
      this.handleSmallMap();
      ...
      titles.some((title) => {
        if (title.isLoaded && !this.drawedTitles[title.key]) {
          ...
          if (viewTexture) {
            ...
            this.offlineSprite.graphics.drawTexture(
              viewTexture,
              titleView.x,
              titleView.y,
              viewTexture.width,
  ...
  private handleSmallMap() {
    let bufferTexture = this.offlineSprite.drawToTexture(
      this._mapTemplate.Width,
      this._mapTemplate.Height,
      0,
      0,
  ...
  protected updateBgView() {
    if (this._bgModel.updateViewportCenterTile()) {
      //如果中心贴片有变化
