...
export default class UIButton {
    ...
    constructor(button: fgui.GObject) {
        ...
        if (button instanceof fgui.GButton) {
            ...
            // @ts-ignore
            button._titleObject && (this.defaultStrokeColor = button._titleObject.strokeColor);
        }
    ...
    __mouseup(evt: Laya.Event) {
        ...
        if (evt.type == Laya.Event.MOUSE_UP && this.callBack) {
            ...
            this.callBackEx && this.callBackEx(this.view, evt, this.args)
        }
     ...
     * @param forceScale 
     */
    static setRedDot(target: fgui.GComponent, num: number, forceScale: number = 0.8) {
     ...
     * @returns 
     */
    static setRedDotPos(target: fgui.GComponent, x: number, y: number, force: boolean = false) {
        let redPointCom = target.getChild("redPoint")
    ...
    set enabled(enabled: boolean) {
        ...
        if (this._view instanceof fgui.GButton) {
            if (enabled) {
                ...
                // @ts-ignore
                this._view._titleObject && (this._view._titleObject.strokeColor = this.defaultStrokeColor);
            } else {
                // 按钮文字和描边直接设置为灰色, 但是能点击
                this._view.titleColor = '#aaaaaa';
                // @ts-ignore
                this._view._titleObject && (this._view._titleObject.strokeColor = '#666666');
            }
        }
