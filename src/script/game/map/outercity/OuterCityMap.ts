...
export class OuterCityMap extends Laya.Sprite implements Disposeable {
  ...
  private init(): void {
    ...
    if (this._model.mapTempInfo.Width < 5000) {
      this._staticRender = new MapRenderLayer(
        this,
        this._model.mapTempInfo.Width,
        this._model.mapTempInfo.Height,
      );
    ...
    AvatarInfoUILayerHandler.setHandlerView(
      ...
      AvatarInfoUIName.OuterCityMap,
    );

    this._borderLayer = new OuterCityBorderLayer();
    this.addChild(this._borderLayer);

    this._denseFogLayer = new OuterCityDenseFogLayer(
      this._model.mapTempInfo.Width,
      this._model.mapTempInfo.Height,
      this,
    );
  ...
  private __addedToStageHandler(evt: Event): void {
    ...
    let center: Point = MapViewHelper.targetSolveCenter(
  ...
  public motionTo(p: Point): void {
    ...
    this.isMove = true;
    let dis: number = MapViewHelper.checkDistance(
      this.x,
      this.y,
      center.x,
      center.y,
    );
    if (this._moveTimeId != null) {
      clearInterval(this._moveTimeId);
    }
    this._moveTimeId = null;
    this.alpha = 1;
    if (dis > 1000) {
      this.x = center.x;
      this.y = center.y;
      ...
      Laya.Tween.to(
        ...
        300,
        null,
        Laya.Handler.create(this, this.moveTweencompleted),
        0,
        true,
      );
    }
  ...
  private _moveTimeId: ReturnType<typeof setInterval> | null = null;
  ...
  public set x(value: number) {
    ...
    if (HomeWnd.Instance.isShowing) {
      HomeWnd.Instance.getSmallMapBar().iconContainerX =
        value / UIConstant.SMALL_MAP_SCALE;
    }
