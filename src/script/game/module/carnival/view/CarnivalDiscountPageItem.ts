...
import CarnivalModel, {
  CARNIVAL_FOLDER,
  CARNIVAL_THEME,
} from "../model/CarnivalModel";
...
export default class CarnivalDiscountPageItem extends FUI_CarnivalDiscountPageItem {
  ...
  public set info(value: t_s_carnivalpointexchangeData) {
    if (!value) return;
    this.Img_Gift_2.url = this.model.getThemeFolderImgPath(
      CARNIVAL_FOLDER.DISCOUNT,
      "Img_Gift_2",
    );
      ...
      if (this._tempInfo.Price > 0) {
        ...
        if (this._tempInfo.Price > points) {
          ...
          var content: string = LangManager.Instance.GetTranslation(
            "carnival.gift.buy",
            this._tempInfo.Price,
            Utils.toCNUpper(this._tempInfo.Sort, langCfg.key),
          );
  ...
  public refreshView() {
    ...
    if (this._tempInfo) {
      ...
      if (this._tempInfo.Target == 0) {
        ...
        this.txt_title.text = LangManager.Instance.GetTranslation(
          "carnival.gift.needPoint",
          this._tempInfo.Target,
          leftNum,
          maxNum,
        );
      }
        ...
        if (this._tempInfo.Target <= this.model.dayCharge) {
          ...
          if (this._tempInfo.Target == 0) {
            if (this._tempInfo.Price == 0) {
              ...
              this.btn_receive.text = LangManager.Instance.GetTranslation(
                "carnival.discountpageitem.freebuy",
              ); //
            } else {
    ...
    this.btn_receive.enabled =
      CarnivalManager.Instance.isRewardTime && this._btnReceive.enabled;
