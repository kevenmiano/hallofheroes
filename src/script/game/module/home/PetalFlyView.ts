...
export default class PetalFlyView extends Laya.Sprite
{
    ...
    */		
    constructor(petalNum:number, showTime:number, type:number = 1, popNum:number = 3, popProbability:number = 5){
        super();
    ...
    private onLoop():void
    {
        for (let i = 0; i < this._petalList.length; i++) {
            ...
            if (element.x > -50 && element.x < Laya.stage.width + 50 && element.y < Laya.stage.height + 50){
                element.drop();
            }
            else
            {
    ...
    public show(container:Laya.Sprite = null):void
    {
        if (container)
            ...
            LayerMgr.Instance.addToLayer(this,EmLayer.GAME_TOP_LAYER);
    }
    ...
    public dispose():void
    {
        ...
        TweenLite.to(this, this.FADEOUT_TIME/1000, {alpha:0,onComplete:this.disposeCall.bind(this)});
    }
    ...
    private disposeCall():void
    {
        ...
        if(this._petalList) this._petalList.length = 0; this._petalList = null;
