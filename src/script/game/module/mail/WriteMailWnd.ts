...
export default class WriteMailWnd extends BaseWindow {
    ...
    public OnInitWind() {
        super.OnInitWind();
        this.receiveList = [];
        this.frame.getChild('title').text = LangManager.Instance.GetTranslation("emailII.view.WriteEmailFrame.title");
        this.txtCost.text = LangManager.Instance.GetTranslation("emailII.view.WriteEmailFrame.cost", MailModel.WRITE_MAIL_NEEDED_GOLD);
        this.inputTips.visible = true;

        this.mailContent = LTextInput.create(this.contentPane.displayObject, "", 24, '#FFECC6', 706, 319, 320, 245, 30)
    ...
    private addEvent() {
        this.mailContent.register(this, this.__onContentChange, this.onMailContentFocusIn, this.onMailContentFocusOut)
        this.receiveInput.asCom.getChild('userName').on(Laya.Event.INPUT, this, this.__onUserChange);
        this.themeInput.asCom.getChild('userName').on(Laya.Event.INPUT, this, this.__onThemeChange);
        this.themeInput.asCom.getChild('userName').on(Laya.Event.BLUR, this, this.onThemeFocusOut);
    ...
    private offEvent() {
        ...
        this.receiveInput.asCom.getChild('userName').off(Laya.Event.INPUT, this, this.__onUserChange);
        this.themeInput.asCom.getChild('userName').off(Laya.Event.INPUT, this, this.__onThemeChange);
        this.themeInput.asCom.getChild('userName').off(Laya.Event.BLUR, this, this.onThemeFocusOut);
        this.btnAddFriend.onClick(this, this.onAddFriendHandler.bind(this));
        this.btnSendMail.onClick(this, this.onSendMailHandler.bind(this));
    ...
    private onAddFriendHandler() {
        ...
        UIManager.Instance.ShowWind(EmWindow.AddFriendWnd, { callback: this.onConfirmAddFriend.bind(this), itemIndex: itemIndex, index: tabIndex })
    }

    /**确定选择发送对象 */
    private onConfirmAddFriend(selectTarget: any, selectTabIndex: number = 0, itemIndex: number = 0) {
        if (selectTarget) {
            this.receiveList = [];
            this.receiveInput.asCom.getChild('userName').text = '';
    ...
    private __getUserInfoHandler(pkg: PackageIn) {
        ...
        if (msg && msg.info && msg.info.nickName.toLowerCase() == this._target.toLowerCase()) {
            var userIdList: Array<number> = [];
            userIdList.push(msg.info.userId);
            this.sendEmail(userIdList);
        } else {
            var str: string = LangManager.Instance.GetTranslation("emailII.view.WriteEmailFrame.content01", this.receiveInput.asCom.getChild('userName').text);
    ...
    private onSendMailHandler() {
        ...
        if (ConfigManager.info.FILTER_TRIM) {
            this._target = StringUtils.remove(this.receiveInput.asCom.getChild('userName').text, " ");
            this._subject = StringUtils.remove(FilterWordManager.filterWrod(this.themeInput.asCom.getChild('userName').text), " ");
            this._contentValue = StringUtils.remove(FilterWordManager.filterWrod(this.mailContent.text), " ");
        } else {
            this._target = this.receiveInput.asCom.getChild('userName').text;
            this._subject = FilterWordManager.filterWrod(this.themeInput.asCom.getChild('userName').text);
        ...
        if (!this._target)
            str = LangManager.Instance.GetTranslation("emailII.view.WriteEmailFrame.target");
        else if (!this._subject)
            str = LangManager.Instance.GetTranslation("emailII.view.WriteEmailFrame.subject");
        else if (!this._contentValue)
            str = LangManager.Instance.GetTranslation("emailII.view.WriteEmailFrame.content");
        else {
            if (this._currentSelected && this._currentSelected.nickName == this.receiveInput.asCom.getChild('userName').text) {
                var userIdList: Array<number> = [];
