...
export class OfferRewardModel extends GameEventDispatcher {
    ...
    public get remainRewardCount(): number {
        return (this.maxRewardCount - this.thane.rewardCount) > 0 ? (this.maxRewardCount - this.thane.rewardCount) : 0;
    }
    ...
    public set offerRewardTaskProfileDic(value: SimpleDictionary) {
        ...
        if (this._offerRewardTaskProfileDic) this._offerRewardTaskProfileDic.clear();
    ...
    public get lastRefreshRewardListTime(): number {
        if (this._lastRefreshRewardListDate) return this._lastRefreshRewardListDate.getTime();
        return 0;
    }
    ...
    public get refreshRemainTime(): number {
        if (this._lastRefreshRewardListDate) {
            var remainTime: number = OfferRewardModel.AUTO_REFRESH_INTERVAL - (this.playerModel.sysCurTimeBySecond - this._lastRefreshRewardListDate.getTime() / 1000);
            if (remainTime > 0)  //
    ...
    public get hasOrangeTaskNotComplete(): boolean {
        for (let index = 0; index < this.offerRewardTaskProfileDic.keys.length; index++) {
            let key = this.offerRewardTaskProfileDic.keys[index];
            let baseReward = this.offerRewardTaskProfileDic[key];
            if (baseReward > 4 && this.offerRewardTaskStateDic[key] == 1) { //橙色可接受
                return true;
            }
        }
        return false;
