...
export default class RoomHallRight extends BaseFguiCom {
  ...
  private refreshOptBtn() {
    ...
    this.btnCrossStart.enabled =
      this.roomInfo.allPlayerReader &&
      this.roomInfo.playerCount != RoomHallData.PlayerItemCnt;
    ...
    this.cCross.selectedIndex = 0;
    if (
      this.ctrl.openCrossPve &&
      mapTemplate &&
      (mapTemplate.DungeonId == 1 || mapTemplate.DungeonId == 2)
    ) {
      //开放了跨服多人本
  ...
  public get isSpecialFB() {
    ...
    if (mapTemplate) {
      return (
        mapTemplate.isTrailTower ||
        mapTemplate.isTaila ||
        mapTemplate.isKingTower
      );
    }
    return false;
  }
  ...
  private pveStartClick() {
    if (
      this.roomInfo.roomType == RoomType.NORMAL &&
      this.roomInfo.campaignId == 0
    ) {
      return;
    }
    ...
    if (!this.model.isCross && this.roomInfo.playerCount == 1) {
      let isTrailCampaign: boolean =
        this.roomInfo.mapTemplate.SonTypes == CommonConstant.TRAILTOWER_SONTYPE;
  ...
  private showTrialAlert(callback: Function): boolean {
    if (
      this.roomInfo.mapTemplate.isTrailTower &&
  ...
  private showThewAlert(callBack: Function = null): boolean {
    if (
      this.roomInfo.mapTemplate.SonTypes != 0 &&
      !this.roomInfo.mapTemplate.isTaila
    ) {
  ...
  private btnReadyClick() {
    ...
    if (
      this.roomInfo.houseOwnerId != this.model.thane.userId &&
    ...
    let land: CampaignLandInfo = CampaignTemplateManager.Instance.landDic[
      CampaignMapLand.None
    ] as CampaignLandInfo;
    ...
    if (mapTemplate.isKingTower) {
      ...
      if (area.getMapByDifficult(2))
        rank2 = dic[area.getMapByDifficult(2).CampaignId];
      if (
        this.roomInfo.houseOwnerId != this.model.thane.userId &&
        !rank &&
  ...
  private btnLockClick() {
    ...
    FrameCtrlManager.Instance.open(EmWindow.RoomPwd, {
      selCampaignID: this.roomInfo.campaignId,
    });
  }
