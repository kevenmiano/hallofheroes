...
export default class PassportView extends FUI_PassportView {
  ...
  protected onConstruct() {
    ...
    this.txt_tip.text = LangManager.Instance.GetTranslation(
      "Passport.tip1",
      this._maxGrade,
      this._maxGrade,
    );
    this.txt8.text = LangManager.Instance.GetTranslation("Passport.tip8");
    this._refresh_dayCfg = TempleteManager.Instance.getConfigInfoByConfigName(
      "passcheck_refresh_day",
    ).ConfigValue;
    this._refresh_weekCfg = TempleteManager.Instance.getConfigInfoByConfigName(
      "passcheck_refresh_week",
    ).ConfigValue;
    this.taskListData = [];
    this.refreshView();
    this.initLanguage();
  ...
  onRecvTaskFresh(isreward: false = false) {
    ...
    this.taskListData = ArrayUtils.sortOn(
      this.taskListData,
      ["status"],
    ...
    if (this.data.state == 1) {
      ...
      this.btn_refresh.title = LangManager.Instance.GetTranslation(
        "Passport.tip9",
        idx,
        arr.length,
      );
  ...
  initEvent() {
    ...
    this.taskList.itemRenderer = Laya.Handler.create(
      this,
      this.onRenderTaskList,
      null,
      false,
    );
    this.tab0.itemRenderer = Laya.Handler.create(
      this,
      this.onTab0,
      null,
      false,
  ...
  private refreshView() {
    ...
    for (let i = 1; i < 3; i++) {
      if (TempleteManager.Instance.hasPassTask(i)) {
        ...
        this.tab0Data.push(
          LangManager.Instance.GetTranslation("pass.tab" + (i - 1)),
        );
      }
    }
    this.tab0.numItems = this.tab0Data.length;
  ...
  onTab0(index: number, item: any) {
    if (item) {
      ...
      if (index == 0) {
        ...
        item.getChild("redDot").visible =
          this.ctrlData.canReceivePassCheckReward || showRed;
      } else {
        // item.getChild('redDot').visible = this.ctrlData.canReceivePassCheckTaskReward;
        item.getChild("redDot").visible =
          this.ctrlData.canReceivePassTaskByArea(index);
      }
    }
  }
  ...
  onRefresh() {
    if (this.data.state == 2) {
      MessageTipManager.Instance.show(
        LangManager.Instance.GetTranslation("Passport.tip7"),
      );
      return;
    }
    ...
    SimpleAlertHelper.Instance.Show(
      ...
      { checkRickText: checkStr, checkDefault: true },
      null,
      content,
      null,
      null,
  ...
  private confirmback(b: boolean, check: boolean) {
    if (b) {
      ...
      WelfareManager.Instance.reqPassTask(
        2,
        0,
        this.tab0.selectedIndex,
        0,
        buyType,
      );
    }
