...
export default class GoldMineInfoView extends FUI_GoldMineInfoView {
    ...
    private initEvent() {
        ...
        NotificationManager.Instance.addEventListener(NotificationEvent.OUTERCITY_SELECTE_MINEITEM, this.updateSelectedInfo, this);
    ...
    private removeEvent() {
        ...
        NotificationManager.Instance.removeEventListener(NotificationEvent.OUTERCITY_SELECTE_MINEITEM, this.updateSelectedInfo, this);
    ...
    private backBtnHandler() {
        NotificationManager.Instance.dispatchEvent(OuterCityEvent.OUTERCITY_SELECTE_CANCEL);
    }
    ...
    private updateSelectedInfo(mineItem: MineItem) {
        if (mineItem) {
            ...
            this.noResDescTxt.text = LangManager.Instance.GetTranslation("GoldMineInfoView.noResDescTxt", this.outerCityModel.currentSelectMine.tempInfo.NameLang);
            let mapMineData: t_s_mapmineData = this.outerCityModel.getNodeByNodeId(mineItem.info.nodeId);
            if (mapMineData) {
                this.goldNameTxt.text = this.outerCityModel.getSonNodeNameTxt(mapMineData.Grade);
                this.titleNameTxt.text = LangManager.Instance.GetTranslation("GoldMineInfoView.titleTxt")
                if (mineItem.info.nodeAllMineInfoDic.length == 0) {//这个子节点没有矿
    ...
    private callRefresh() {
        ...
        if (self.list.numItems >= this._listData.length) {
            ...
            //后续每次增加10条
            self.list.numItems += leftCount > this.pageAddCapicity ? this.pageAddCapicity : leftCount;
        }
        self.list.scrollPane.lockFooter(0);
    }

    private onTabChanged(e: Laya.Event) {
        if (this.tab.selectedIndex == 0) {//看空位置
        ...
        else if (this.tab.selectedIndex == 1) {//已经被占领的
            this._listData = this.outerCityModel.hasOccpuySonNode(this._info);
            this._len = this._listData.length;
            this.list.numItems = this._len;
            this.c1.selectedIndex = this._len > 0 ? 0 : 1;
        }
        if (!StringHelper.isNullOrEmpty(this._wild.info.occupyLeagueName)) {//有人占领
            if (this._wild.info.occupyLeagueName == PlayerManager.Instance.currentPlayerModel.playerInfo.consortiaName) {//自己公会的
                //玩家在各个矿点占领的矿总数有没有达到上限
                if (this.outerCityModel.checkPlayerOccpuyNumberIsMax()) {//玩家占领总数量达到上限
                    this.c2.selectedIndex = 1;
                    this.descTxt.text = LangManager.Instance.GetTranslation("GoldMineInfoView.descTxt1");
                }
                else {//总数量没有达到上限
                    if (this.outerCityModel.occupyCount(this._wild) >= this._wild.tempInfo.Property1) { //在这个矿点达到上限了不能占领
                        this.c2.selectedIndex = 1;
                        this.descTxt.text = LangManager.Instance.GetTranslation("GoldMineInfoView.descTxt2");
                    } else {
                ...
                this.descTxt.text = LangManager.Instance.GetTranslation("GoldMineInfoView.descTxt4");
            }
