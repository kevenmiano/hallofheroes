...
export class MapSocketInnerManager {
  ...
  public setup() {
    ServerDataManager.listen(
      S2CProtocol.U_C_MAPDATA_SYNC,
      this,
      this.s2c_mapdata_sync,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_PLAY_MOVIE,
      this,
      this.s2c_play_movie,
    );
    ServerDataManager.listen(
      S2CProtocol.U_B_PREPARE_OVER,
      this,
      this.s2c_prepare_over,
    );
    //>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>战斗>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>;
    ServerDataManager.listen(
      S2CProtocol.U_C_BATTLE_TALK,
      this,
      this.s2c_battle_talk,
    ); // 战斗剧情
    ServerDataManager.listen(S2CProtocol.U_B_PREPARE, this, this.s2c_prepare); //准备战斗
    ServerDataManager.listen(
      S2CProtocol.U_B_ATTACKING,
      this,
      this.s2c_battleHandle,
    );
    ServerDataManager.listen(
      S2CProtocol.U_B_START,
      this,
      this.s2c_battleHandle,
    );
    ServerDataManager.listen(
      S2CProtocol.U_B_GAME_OVER,
      this,
      this.s2c_battleHandle,
    );
    ServerDataManager.listen(
      S2CProtocol.U_B_USER_REINFORCE,
      this,
      this.s2c_battleHandle,
    );
    ServerDataManager.listen(
      S2CProtocol.U_B_USER_STOP,
      this,
      this.s2c_battleHandle,
    );
    ServerDataManager.listen(
      S2CProtocol.U_B_FIGHT_MODE,
      this,
      this.s2c_battleHandle,
    );
    ServerDataManager.listen(
      S2CProtocol.U_B_LOAD_RES,
      this,
      this.s2c_battleHandle,
    );
    ServerDataManager.listen(
      S2CProtocol.U_B_BATTLE_NOTICE,
      this,
      this.s2c_battleHandle,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_UPDATE_AWAKE,
      this,
      this.s2c_battleHandle,
    );
  }
  ...
  private s2c_mapdata_sync(pkg: PackageIn) {
    ...
    switch (msg.updatedType) {
      case MapSocketInnerManager.UPDATE_CASTLE:
        ...
        ServerDataManager.Instance.dispatchEvent(
  ...
  private s2c_play_movie(pkg: PackageIn) {
    ...
    let interval =
      Laya.Browser.now() -
      ChatManager.Instance.model.getProtocolTimeByID(pkg.extend1);
    if (interval >= 3) {
      Logger.info(
        "SLGSocketEvent.EVENT+AcceptProtocolType.U_C_PLAY_MOVIE协议超时 3 秒以上",
      );
    }
