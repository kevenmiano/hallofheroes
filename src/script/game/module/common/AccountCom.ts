...
export class AccountCom extends FUI_AccountCom {
    ...
    for (let info of infos) {
      if (
        info.templateInfo.TemplateId ==
          TemplateIDConstant.TEMP_ID_WISHPOOL_GOLD ||
        info.templateInfo.TemplateId == TemplateIDConstant.TEMP_ID_WISHPOOL
      ) {
        this.refreshWishPool();
      } else if (
        info.templateInfo.TemplateId == TattooModel.DragonCrystalId ||
        info.templateInfo.TemplateId == TattooModel.DragonCrystalId2
      ) {
        this.refreshDragonCrystal();
        ...
        info.templateInfo.TemplateId == TemplateIDConstant.TEMP_ID_MAZE
      ) {
        this.refreshTxt10();
      } else if (
        info.templateInfo.SonType == GoodsSonType.SONTYPE_SOUL_CRYSTAL
      ) {
        this.refreshGem();
      } else if (
        info.templateInfo.SonType == GoodsSonType.SONTYPE_DRAGON_SOUL
      ) {
        this.refreshDragon();
        ...
        info.templateInfo.TemplateId == TemplateIDConstant.GUILD_CONTRIBUTION
      ) {
        this.refreshTxt11();
      }
    }
   ...
   */
  refreshGem() {
    let own_num = GoodsManager.Instance.getCountBySontypeAndBagType(
  ...
  refreshDragon() {
    let own_num = GoodsManager.Instance.getCountBySontypeAndBagType(
      GoodsSonType.SONTYPE_DRAGON_SOUL,
      BagType.Player,
    );
  ...
  refreshWishPool() {
    let ownNum1 = GoodsManager.Instance.getGoodsNumByTempId(
      TemplateIDConstant.TEMP_ID_WISHPOOL,
    );
    let ownNum2 = GoodsManager.Instance.getGoodsNumByTempId(
      TemplateIDConstant.TEMP_ID_WISHPOOL_GOLD,
    );
  ...
  refreshDragonCrystal() {
    ...
    let own_num2 = GoodsManager.Instance.getGoodsNumByTempId(
   ...
   */
  refreshTxt10() {
    this.txt_money10.text = GoodsManager.Instance.getGoodsNumByTempId(
      TemplateIDConstant.TEMP_ID_MAZE,
    ).toString();
  }
  ...
  private onPetGrowthStoneBuy() {
    var data: ShopGoodsInfo = TempleteManager.Instance.getShopTempInfoByItemId(
