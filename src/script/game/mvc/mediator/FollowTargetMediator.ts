...
export class FollowTargetMediator implements IMediator {
  ...
  public register(target: object) {
    ...
    var serviceName: string = (<CampaignNode>this._curNpc.nodeInfo)
      .followTargetServiceName;
    var cArmy: CampaignArmy =
      CampaignManager.Instance.mapModel.getBaseArmyByArmyId(
        targetId,
        serviceName,
      );
    var armyView: CampaignArmy =
      CampaignManager.Instance.controller.getArmyView(cArmy);
    if (cArmy) this._followTarge = armyView;
    if (!this._followTarge || !this._followTarge.aiInfo) return;
    this._followTarge.aiInfo.addEventListener(
      AiEvents.UPDATE_PATHS,
      this.__followTargerHandler,
      this,
    );
    this._followTarge.aiInfo.addEventListener(
      AiEvents.NEXT_POINT,
    ...
    this._followTarge.addEventListener(
      ObjectsEvent.WALK_OVER,
      this.__targetWalkOverHandler,
      this,
    );
    this._curNpc.on(
      ObjectsEvent.WALK_OVER,
  ...
  public unregister(target: object) {
    if (this._followTarge) {
      if (this._followTarge.aiInfo) {
        this._followTarge.aiInfo.removeEventListener(
          AiEvents.UPDATE_PATHS,
          this.__followTargerHandler,
          this,
        );
      ...
      this._followTarge.removeEventListener(
        ObjectsEvent.WALK_OVER,
        this.__targetWalkOverHandler,
        this,
      );
  ...
  private getRecentIndex(arr: any[]): number {
    var cur: Laya.Point = new Laya.Point(
      parseInt((this._curNpc.x / 20).toString()),
      parseInt((this._curNpc.y / 20).toString()),
    );
  ...
  private __nextPointHandler(data: any) {
    ...
    if (index > this._minIndex && this._curPath) {
      ...
      var tPoint: Laya.Point = new Laya.Point(
        this._followTarge.x,
        this._followTarge.y,
      );
      ...
      if (arr.length > 0) {
        ...
        var temp: any[] = CampaignManager.Instance.controller.searchPath(
          new Laya.Point(
            parseInt((cur.x / Tiles.WIDTH).toString()),
            parseInt((cur.y / Tiles.HEIGHT).toString()),
          ),
          end,
        );
        if (!temp) temp = [];
  ...
  private __targetWalkOverHandler(evt: ObjectsEvent) {
    ...
    var tPoint: Laya.Point = new Laya.Point(
      parseInt((this._followTarge.x / 20).toString()),
      parseInt((this._followTarge.y / 20).toString()),
    );
    var path: any[] = this._curNpc.aiInfo.pathInfo.concat();
    var max: number = path.length - 1;
    for (var i: number = max; i > 0; i--) {
      if (i < this._curNpc.aiInfo.walkIndex) {
        path.splice(i, 1);
        continue;
      }
      ...
      if (leng > tPoint.distance(pre.x, pre.y)) {
        path.pop();
        continue;
      }
      if (leng < 2) {
  ...
  private setNpcAngle(stop: boolean = true) {
    ...
    var angle: number = HeroAvatarViewHelper.twoPointAngle(
      this._followTarge.x,
      this._followTarge.y,
      this._curNpc.x,
      this._curNpc.y,
