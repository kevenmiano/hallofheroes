...
export default class AirGardenGameMemoryCardWnd extends BaseWindow {
    ...
    private initView() {
        ...
        let temp: t_s_carnivalpointexchangeData[] = TempleteManager.Instance.getCarnivalByType(CarnivalModel.GAME_TYPE_2);
        if (temp && temp.length > 0) this.txt_score.text = LangManager.Instance.GetTranslation("samll.game.score", Number(temp[0].Target));

        this.txt_autoFlip.text = LangManager.Instance.GetTranslation("AirGardenGame.memorycard.str1", "0");
        this.txt_drawCards.text = LangManager.Instance.GetTranslation("AirGardenGame.memorycard.str2", "0");
        this.txt_myPoint.text = LangManager.Instance.GetTranslation("carnival.awardpoint.point", 0);
        this.txt_time.text = LangManager.Instance.GetTranslation("openBox.tip.countdown", 0);
    ...
    private initCards() {
        for (let index = 0; index < this.itemList.numChildren; index++) {
            const item = this.itemList.getChildAt(index) as AirGardenGameMemoryCardItem;
            let idx = index + 1;
    ...
    private initEvent() {
        ...
        this.m.addEventListener(AirGardenGameEvent.INIT_MEMORY_CARD, this.initData, this);
        this.m.addEventListener(AirGardenGameEvent.FLIP_MEMORY_CARD, this.flipCard, this);
        this.m.addEventListener(AirGardenGameEvent.AUTO_MEMORY_CARD, this.autoCard, this);
        this.m.addEventListener(AirGardenGameEvent.SHOW_MEMORY_CARD, this.showCard, this);
        this.m.addEventListener(AirGardenGameEvent.UPDATE_DATA_MEMORY_CARD, this.updateData, this);
    }
    ...
    private removeEvent() {
        ...
        this.m.removeEventListener(AirGardenGameEvent.INIT_MEMORY_CARD, this.initData, this);
        this.m.removeEventListener(AirGardenGameEvent.FLIP_MEMORY_CARD, this.flipCard, this);
        this.m.removeEventListener(AirGardenGameEvent.AUTO_MEMORY_CARD, this.autoCard, this);
        this.m.removeEventListener(AirGardenGameEvent.SHOW_MEMORY_CARD, this.showCard, this);
        this.m.removeEventListener(AirGardenGameEvent.UPDATE_DATA_MEMORY_CARD, this.updateData, this);
        this._timer.removeEventListener(TimerEvent.TIMER, this.__timerHandler, this);
    }
    ...
    public __clickItem(idx: number) {
        ...
        if (this.m.model.openCardIdxs.length < 2) {
            if (this.m.model.openCardIdxs.length == 1 && this.m.model.openCardIdxs[0] == idx) {
                return;
            }
            ...
            if (this.m.model.openCardIdxs.length >= 2) {
                this.lockList = true;
                Laya.timer.once(AirGardenGameMemoryCardItem.TURN_HALF_TIME * 2 + 1000, this, () => {
    ...
    private updateData(e: AirGardenGameEvent) {
        this.txt_autoFlip.text = LangManager.Instance.GetTranslation("AirGardenGame.memorycard.str1", "" + this.m.model.leftAutoChance);
        this.txt_drawCards.text = LangManager.Instance.GetTranslation("AirGardenGame.memorycard.str2", "" + this.m.model.leftCheckChance);
        this.txt_myPoint.text = LangManager.Instance.GetTranslation("carnival.awardpoint.point", this.m.model.score.toString());
    }

    private __timerHandler() {
        let remain: number = this._timer.repeatCount - this._timer.currentCount
        this.txt_time.text = LangManager.Instance.GetTranslation("openBox.tip.countdown", remain);
        if (remain <= 0) {
            this.hide();
        }
    }
    ...
    private onClickUse(type: number = 1) {
        ...
        if (type == 1) {
            if (this.m.model.leftAutoChance <= 0) {
                MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("AirGardenGame.memorycard.str5"));
                return;
            }
        } else if (type == 2) {
            if (this.m.model.leftCheckChance <= 0) {
                MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("AirGardenGame.memorycard.str6"));
                return;
            }
        }
        ...
        if (this.m.model.openCardIdxs.length == 1) {
            MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("AirGardenGame.memorycard.str3"));
            return;
        }
    ...
    private showCard(event: AirGardenGameEvent) {
        ...
        for (let i: number = 1; i <= this.ROW * this.COLOME; i++) {
            ...
            if (data) {
                let tempCard = this.cardItems.get(data.index) as AirGardenGameMemoryCardItem;
    ...
    private autoCard(event: AirGardenGameEvent) {
        ...
        let tempCard1: AirGardenGameMemoryCardItem = this.cardItems.get(this.m.model.cIndex1) as AirGardenGameMemoryCardItem;
        let tempCard2: AirGardenGameMemoryCardItem = this.cardItems.get(this.m.model.cIndex2) as AirGardenGameMemoryCardItem;
    ...
    private flipCard(e: AirGardenGameEvent) {
        ...
        let tempCard1: AirGardenGameMemoryCardItem = this.cardItems.get(this.m.model.openCardIdxs[0]);
        let tempCard2: AirGardenGameMemoryCardItem = this.cardItems.get(this.m.model.openCardIdxs[1]);
    ...
    private playAddScoreEffect(obj: AirGardenGameMemoryCardItem) {
        if (this.m.model.addScore2 > 0) {
            let str = LangManager.Instance.GetTranslation("AirGardenGame.memorycard.addScore", this.m.model.addScore + this.m.model.addScore2);
            str += "(+" + this.m.model.addScore2 + ")"
            MessageTipManager.Instance.show(str)
        } else {
            let str = LangManager.Instance.GetTranslation("AirGardenGame.memorycard.addScore", this.m.model.addScore);
            MessageTipManager.Instance.show(str)
        }
    ...
    protected OnBtnClose(): void {
        let str: string = LangManager.Instance.GetTranslation("AirGardenGame.exit.hint");
        SimpleAlertHelper.Instance.popAlerFrame(null, str, null, null, { callback: this.realDispose.bind(this) }, SimpleAlertHelper.SIMPLE_ALERT);
    }
