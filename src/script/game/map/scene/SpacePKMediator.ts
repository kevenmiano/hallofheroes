...
export default class SpacePKMediator implements IMediator {
  ...
  onRemoveArmy(target: SpaceArmy) {
    if (target) this.deletePreMsgByAttackId(target.userId);
  }
  ...
  private __onPKRequestHandler(msg: HangupAttackMsg) {
    ...
    if (
      ...
      this.timerId2 = Number(setTimeout(this.alertMsg.bind(this), 300, msg));
    } else if (
      msg.op == SpaceAttackOP.PET_ATTACK &&
      msg.attackId != this._selfInfo.userId
    ) {
      if (this.timerId2 != 0) {
        this._pkStack.push(msg);
      ...
      this.deletePreMsgByAttackId(msg.attackId);
      this.timerId2 = Number(
        setTimeout(this.petPKAlertMsg.bind(this), 300, msg),
      );
    }
  }
  ...
  private petPKAlertMsg(msg: HangupAttackMsg) {
    ...
    let submitLabel = LangManager.Instance.GetTranslation(
      "map.campaign.view.frame.PrepareBattleFrame1.accept",
    );
    let cancelLabel = LangManager.Instance.GetTranslation(
      "map.campaign.view.frame.PrepareBattleFrame1.reject",
    );
    let content = LangManager.Instance.GetTranslation(
      "map.hookMap.command02",
      msg.attackName,
    );
    let data = {
      ...
      msg: msg,
      check: true,
      checkRickText: LangManager.Instance.GetTranslation(
        "BeingInviteWnd.txtNotAcceptInvite",
      ),
    };
    SpacePKAlert.Instance.Show(
      data,
      title,
      content,
      submitLabel,
      cancelLabel,
      this.onPetPKAlertCall.bind(this),
      AlertBtnType.OC,
    );
    ...
    this.timerId = Number(
      setTimeout(
        ...
        SpaceManager.PKInviteAlertShowTime,
        false,
      ),
    );
  }
  ...
  public receivePetPKThisRequest(msg: HangupAttackMsg) {
    ...
    MapSocketOuterManager.receivePetPKRequest(
  ...
  public refusePetPKThisRequest(msg: HangupAttackMsg) {
    MapSocketOuterManager.refusePetPKRequest(
      this._selfInfo.userId,
      msg.attackId,
    );
    ...
    if (this._pkStack.length != 0) {
      this.timerId2 = Number(
        setTimeout(this.alertMsg.bind(this), 300, this._pkStack.pop()),
      );
    }
  }
