...
export default class WorldBossRiverWnd extends BaseWindow {
  ...
  private initData() {
    ...
    switch (this._type) {
      ...
      case WorldBossRiverWnd.PET_BOSS:
        ...
        tempInfo =
          TempleteManager.Instance.getConfigInfoByConfigName("Live_Pay");
        break;
      case WorldBossRiverWnd.DEMON:
        tempInfo =
          TempleteManager.Instance.getConfigInfoByConfigName("AltarLive_Offer");
        break;
    }
  ...
  private updateDiamond() {
    if (!this.diamondCtrl) {
      ...
      this.diamondCtrl.selectedIndex =
        this._type == WorldBossRiverWnd.WORLD_BOSS ? 1 : 0;
    }
    //钻石
  ...
  private __riverHandler() {
    ...
    switch (this._type) {
      case WorldBossRiverWnd.WORLD_BOSS:
        lastSaveDate =
          SharedManager.Instance.worldBossRiverTodayNeedAlertCheckDate;
        break;
      case WorldBossRiverWnd.DEMON:
    ...
    if (this._type == WorldBossRiverWnd.DEMON) {
      var contextStr: string = LangManager.Instance.GetTranslation(
        "map.campaign.view.ui.RiverView.content_river",
        this._riverPay +
          LangManager.Instance.GetTranslation("public.contribution"),
      );
      var confirm: string =
      ...
      if (needAlert) {
        let payStr: string =
          ...
          LangManager.Instance.GetTranslation(
            "mainBar.WorldBossBuyBuffBar.point",
          );
        ...
        UIManager.Instance.ShowWind(EmWindow.TodayNotAlert, {
          ...
          closeFunction: this.__closeHandler.bind(this),
          point: this._battlePoint,
        });
      } else {
        this.sendRiver(true);
      }
    ...
    if (b && this.checkPay) {
      ...
      if (this.selfMemberData.isDie == 1) {
        ...
        MessageTipManager.Instance.show(
          LangManager.Instance.GetTranslation("riverView.notDieState"),
        );
      }
    }
  }
  ...
  private battleHandler() {
    ...
    var lastSaveDate: Date = new Date(
      SharedManager.Instance.worldBossRiverTodayNeedAlertCheckDate_enterBattle,
    );
    if (lastSaveDate) {
    ...
    if (needAlert) {
      var content: string = LangManager.Instance.GetTranslation(
        "map.campaign.view.ui.RiverView.content_battle",
        this._battlePoint,
      );
      UIManager.Instance.ShowWind(EmWindow.TodayNotAlert, {
        content: content,
        backFunction: this.__todayNotAlert2.bind(this),
        closeFunction: this.__closeHandler2.bind(this),
        point: this._battlePoint,
      });
    } else {
