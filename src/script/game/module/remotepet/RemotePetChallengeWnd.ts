...
export class RemotePetChallengeWnd extends BaseWindow {
  ...
  private initView() {
    ...
    this.fgoodsList.itemRenderer = Laya.Handler.create(
      this,
      this.onGoodsItemRender1,
      null,
      false,
    );

    this.rgoodsList.setVirtual();
    this.rgoodsList.itemRenderer = Laya.Handler.create(
      this,
      this.onGoodsItemRender2,
      null,
      false,
    );
  ...
  private updateView() {
    ...
    this.lvNum.text = LangManager.Instance.GetTranslation(
      "RemotePetTurnTip.index",
      tempInfo.Index,
    );
    ...
    this.tips.text = LangManager.Instance.GetTranslation(
      "RemotePetTurnTip.mopupValue",
      tempInfo.Sweep,
    );
    let challenge =
      tempInfo.Index == this.model.turnInfo.currTurn ||
      tempInfo.Index <= this.model.turnInfo.maxTurn;
    ...
    this.imgPetType.icon = FUIHelper.getItemURL(
      EmPackName.Base,
      "Icon_PetType10" + tempInfo.Property,
    );
  }
