...
export class PawnCharacteristicsTipView extends TaskTraceTipWnd {
  ...
  initView() {
    super.initView();
    let contenttext = LangManager.Instance.GetTranslation(
      "tasktracetip.view.PawnCharacteristicsTipView.content",
    );
    this.setContentText(contenttext);
    this.setBtnTitle(
      LangManager.Instance.GetTranslation(
        "tasktracetip.view.PawnCharacteristicsTipView.text",
      ),
    );
  }
  ...
  protected __btnHandler(e: Event) {
    ...
    TaskTraceTipManager.Instance.cleanByType(
      TipMessageData.PAWN_CHARACTERISTICS_TIP_VIEW,
    );
