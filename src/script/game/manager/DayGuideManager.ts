...
export default class DayGuideManager extends GameEventDispatcher {
  ...
  private addEvent() {
    ServerDataManager.listen(
      S2CProtocol.U_C_PLAYER_SIGN,
      this,
      this.__signHandler,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_LEED_UPDATE,
      this,
      this.__onLeedUpdateHandler,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_LEED_RECEIVE,
      this,
      this.__onLeedReceiveHandler,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_DAYACTION,
      this,
      this.__dayActivityHandler,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_CLICKDATA,
      this,
      this.__dailyCount,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_MONTH_CARD,
      this,
      this.__monthCardHandler,
    );
    OfferRewardManager.Instance.addEventListener(
      RewardEvent.REWARD_TASK_FINISH,
      this.__finishRewardTaskHandler,
      this,
    );
  }
  /**
  ...
  private __dailyCount(evtData) {
    ...
    var msg: ClickDatasRspMsg = pkg.readBody(
      ClickDatasRspMsg,
    ) as ClickDatasRspMsg;
        ...
        if (Object.prototype.hasOwnProperty.call(datalist, key)) {
          ...
          if (info.TemplateId == idStrr) {
            if (idStrr == "4") {
              ...
              info.currentFinsh =
                fightCount > info.MaxCount ? info.MaxCount : fightCount;
            } else {
              info.currentFinsh =
                acceptInfo[i].keyData > info.MaxCount
                  ? info.MaxCount
                  : acceptInfo[i].keyData;
            }
          }
          if (info.TemplateId == "10" && idStrr == "2") {
            //多人竞技
            info.currentFinsh =
              acceptInfo[i].keyData > info.MaxCount
                ? info.MaxCount
    ...
    for (const key in this._dailyInfoList) {
      if (Object.prototype.hasOwnProperty.call(this._dailyInfoList, key)) {
        ...
        switch (info.TemplateId) {
          case "4": //战场
            info.currentFinsh = Math.max(
              info.MaxCount - Math.max(this.playerInfo.warFieldCount, 0),
              0,
            );
            break;
          case "6": //英雄之门
          ...
          case "8": //个人挑战
            info.currentFinsh =
              BuildingManager.Instance.model.getOrderById(10000).currentCount;
            break;
          case "11": //试炼之塔
          ...
          case "18": //王者之塔
            info.currentFinsh =
              KingTowerManager.Instance.kingTowerInfo.kingCount;
            break;
          default:
            break;
        }
  ...
  private __dayActivityHandler(evtData) {
    ...
    for (var i: number = 0; i < len; i++) {
      ...
      info.endDate = DateFormatter.parse(
        arr[i].endDate,
        "YYYY-MM-DD hh:mm:ss",
      ).getTime();
      info.startDate = DateFormatter.parse(
        arr[i].startDate,
        "YYYY-MM-DD hh:mm:ss",
      ).getTime();
  ...
  private __signHandler(evtData) {
    var pkg: PackageIn = evtData as PackageIn;
    var msg: PlayerSignRspMsg = pkg.readBody(
      PlayerSignRspMsg,
    ) as PlayerSignRspMsg;
  ...
  protected __onLeedUpdateHandler(pkg: PackageIn) {
    ...
    if (msg.state == 1) {
      //默认为0  为1 只更新totalNum,weekActive
      ...
      if (msg.hasOwnProperty("leed")) {
        for (var i: number = 0; i < msg.leed.length; i++) {
          this.addLeedData(
            DayGuideHelper.readLeedInfo(msg.leed[i] as LeedData),
          );
        }
      }
      this._cate.beginChange();
  ...
  protected __onLeedReceiveHandler(pkg: PackageIn) {
    var msg: LeedFinishedRspMsg = pkg.readBody(
      LeedFinishedRspMsg,
    ) as LeedFinishedRspMsg;
    this._cate.beginChange();
    this._cate.leedProcess = msg.leedProcess;
    this._cate.conmmitChange();
  }
  ...
  private addLeedData(info: LeedInfo) {
    if (this._cate.allList.hasOwnProperty(info.templateId)) this.update(info);
    else this._cate.allList[info.templateId] = info;
  }
    ...
    switch (type) {
      ...
      case DayGuideCatecory.PASS_MUL_CAMPAIGN:
        if (ArmyManager.Instance.thane.grades < OpenGrades.PVE_MUlTI_CAMPAIGN) {
          str = LangManager.Instance.GetTranslation(
            "dayGuide.DayGuideManager.command01",
          );
      ...
      case DayGuideCatecory.TRIAL:
        ...
        if (this.thane.grades < OpenGrades.TRIAL) {
          str = LangManager.Instance.GetTranslation(
            "dayGuide.view.DailyItem.UnderLevel",
          );
          MessageTipManager.Instance.show(str);
          return;
        }
      ...
      case DayGuideCatecory.CONSORTIA_SHOP:
        if (
          ...
          if (ConsortiaManager.Instance.model.consortiaInfo.shopLevel > 0) {
            ...
            MessageTipManager.Instance.show(
              LangManager.Instance.GetTranslation("consortia.shop.unopen"),
            );
          }
        }
        break;
        ...
        if (mapModel) {
          ...
          if (WorldBossHelper.checkPetLand(mapId)) {
            let tip = LangManager.Instance.GetTranslation(
              "dayGuide.DayGuideManager.command06",
            );
  ...
  private showFaildMes(str: string = "") {
    if (str == "")
      str = LangManager.Instance.GetTranslation(
        "dayGuide.DayGuideManager.command04",
      );
    MessageTipManager.Instance.show(str);
  }
}
