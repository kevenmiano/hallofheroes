...
export default class BoxManager extends GameEventDispatcher {
  ...
  private initEvent() {
    ServerDataManager.listen(
      S2CProtocol.U_C_PLAYER_TIME,
      this,
      this.__getHaveTimeBox,
    );
  }
  ...
  public isRecviedByGrade(grade: number): boolean {
    var progress: number =
      PlayerManager.Instance.currentPlayerModel.playerInfo.gradeProcess;
  ...
  public set remainTime(value: number) {
    ...
    if (this._remainTime == 0) this.dispatchEvent(TimeBoxEvent.TIME_COMPLETE);
  }
  ...
  public getCurrentTimePeriod(): number {
    ...
    for (var i: number = 0; i < this._timeBoxList.length; i++) {
      ...
      if (time == this.playerInfo.timeProgress) {
        if (this.playerInfo.timeGet) return Number(this._timeBoxList[i + 1]);
        else return time;
      }
    }
  ...
  public get needCountTime(): boolean {
    ...
    if (!this.playerInfo.timeGet)
      return (
        this.playerInfo.timeProgress <=
        Number(this._timeBoxList[this._timeBoxList.length - 1])
      );
    else
      return (
        this.playerInfo.timeProgress <
  ...
  public get nextTimePeriod(): number {
    ...
    for (var i: number = 0; i < len; i++) {
      if (this.playerInfo.timeProgress == Number(this._timeBoxList[i])) {
        if (i + 1 < len) return this._timeBoxList[i + 1];
        return -1;
      }
    }
    return -1;
