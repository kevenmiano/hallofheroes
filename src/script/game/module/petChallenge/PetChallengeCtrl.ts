...
export default class PetChallengeCtrl extends FrameCtrlBase {
  ...
  public constructor() {
    ...
    ServerDataManager.listen(
      ...
      this,
      this.__petchallengeTimeHandler,
    );
  }
  ...
  protected addEventListener() {
    ...
    ServerDataManager.listen(
      S2CProtocol.U_C_PET_CHALLENGE_LIST,
      this,
      this.__loadPetChallengeListHandler,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_TOP_CHALLENGE_INFO,
      this,
      this.__loadPetChallengeTopListHandler,
    );
    ServerDataManager.listen(
    ...
    ServerDataManager.listen(
      S2CProtocol.U_C_PET_CHALLENGE_LOG,
      this,
      this.__petEventLogHandler,
    ); // 英灵竞技战报列表)
  ...
  private sortChallengeTopList() {
    this.data.challengeTopList = ArrayUtils.sortOn(
      this.data.challengeTopList,
      ["ranking"],
      [ArrayConstant.NUMERIC],
    );
    let temp: PetChallengeObjectData = this.data.challengeTopList[
      this.data.challengeTopList.length - 1
    ] as PetChallengeObjectData;
  ...
  private sortChallengeList() {
    let temp: PetChallengeObjectData = this.data.challengeList[
      this.data.challengeList.length - 1
    ] as PetChallengeObjectData;
    if (temp && temp.ranking <= 0) {
  ...
  public sendChallengeCommand(userId: number) {
    ...
    if (this.data.buildingOrder.remainTime > 0) {
      ...
      tip = LangManager.Instance.GetTranslation(
        "PetChallengeController.nochallengepet",
      );
    }
    ...
    if (this.data.buildingOrder.remainCount <= 0) {
      ...
      if (
        SharedManager.Instance.checkIsExpired(
          SharedManager.Instance.buyPetChallengeCountDate2,
        )
      ) {
  ...
  public getCostGold(): number {
    ...
    let temp: ConfigInfosTempInfo =
      TempleteManager.Instance.getConfigInfoByConfigName("Jion_pet_challenge");
    if (temp) {
  ...
  public showBuyChallengeCountFrame1() {
    ...
    let content: string = LangManager.Instance.GetTranslation(
      "PetChallengeController.buyChallengeCount",
      this.getCostGold(),
    );
    SimpleAlertHelper.Instance.Show(
      ...
      null,
      null,
      this.buyChallengeCountBack1.bind(this),
    );
  }
  ...
  public showBuyChallengeCountFrame2() {
    ...
    let content: string = LangManager.Instance.GetTranslation(
      "PetChallengeController.buyChallengeCount2",
      this.getCostGold(),
    );
    SimpleAlertHelper.Instance.Show(
      SimpleAlertHelper.USEBINDPOINT_ALERT,
      ...
      null,
      null,
      this.buyChallengeCountBack2.bind(this),
    );
  }
  ...
  open(frameInfo: FrameCtrlInfo) {
    ...
    if (ArmyManager.Instance.thane.grades < OpenGrades.PET_PVP) {
      ...
      this.playerInfo.petList.length <= 0
    ) {
