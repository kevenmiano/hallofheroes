...
export class WaterManager extends GameEventDispatcher {
  ...
  private initEvent() {
    ServerDataManager.listen(
      S2CProtocol.U_C_TREE_FRIEND,
      this,
      this.__getTreeInfoHandler,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_WATERLOG_SEND,
      this,
      this.__logUpdateHandler,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_TREEINFO_UPDATE,
      this,
      this.__treeInfoUpdateHandler,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_SEND_WATER_RESULT,
      this,
      this.__waterStateHandler,
    );
    this.thane.on(
    ...
    if (msg.treeInfo.length > 0) {
      for (const key in msg.treeInfo) {
        ...
        if (StringHelper.isNullOrEmpty(info.nickName)) {
          var thaneInfo: ThaneInfo = FriendManager.getInstance().getFriendById(
            info.userId,
          );
          if (thaneInfo) info.nickName = thaneInfo.nickName;
          else info.nickName = "";
        }
   ...
   *
   */
  private __logUpdateHandler(pkg: PackageIn) {
    //0x0091
   ...
   *
   */
  private __treeInfoUpdateHandler(pkg: PackageIn) {
    //0x0093
    ...
    for (var i: number = 0; i < msg.tree.length; i++) {
      ...
      info.nextPickTime = DateFormatter.parse(
        treeInfo.nextPickTime,
        "YYYY-MM-DD hh:mm:ss",
      );
      info.lastWaterTime = DateFormatter.parse(
        treeInfo.lastWaterTime,
        "YYYY-MM-DD hh:mm:ss",
      );
      ...
      if (info.userId == this.thane.userId) {
        ...
        Logger.xjy(
          "[FarmManager]__treeInfoUpdateHandler",
          info.timeLeft,
          info.leftpickTime,
          info,
        );
  ...
  private __waterStateHandler(pkg: PackageIn) {
    ...
    if (userId == this.thane.userId) {
      Logger.xjy(
        "[WaterManager]__waterStateHandler 更新自己神树状态 canGivePower=false",
        waterId,
      );
      var fsInfo: FriendFarmStateInfo =
        this.farmModel.getFarmStateInfo(waterId);
      fsInfo.beginChanges();
