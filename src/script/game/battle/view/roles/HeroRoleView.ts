...
export class HeroRoleView extends BaseRoleView {
  ...
  public get rolePartUrlPath(): string {
    if ((this._info as HeroRoleInfo).type == RoleType.T_NPC_BOSS) {
      return (
        this._rolePart && this._rolePart.data && this._rolePart.data.urlPath
      );
    } else {
    ...
    if ((this._info as HeroRoleInfo).type != RoleType.T_NPC_BOSS) {
      ...
      if (pet && pet.livingId != this._info.livingId) {
        ...
        this._rolePart.data = HeroLoadDataFactory.create(
          (this._info as HeroRoleInfo).heroInfo,
          HeroLoadDataFactory.PART_PET_BODY,
        );
      }
      ...
      this._rolePart.data = new GameLoadNeedData(
        PathManager.solveRolePath(this._info.resPath),
      );
    }
  ...
  private onPetPartComplete(target: RoleUnit) {
    ...
    this._petMovie.name = target.content.data
      ? target.content.data.urlPath
      : "";
  ...
  private onPetPartComplete2(target: RoleUnit) {
    ...
    if (this._movieContainer.numChildren > 0)
    ...
    if (this._info.actionMovieClip) {
      ...
      Helpers.getPos(
        this._info.pos_head_hero,
        this._info.actionMovieClip,
        "pos_body",
      );
      Helpers.getPos(
        this._info.pos_body_hero,
        this._info.actionMovieClip,
        "pos_head",
      );
      Helpers.getPos(
        this._info.pos_leg_hero,
        this._info.actionMovieClip,
        "pos_leg",
      );
    }
  ...
  private onHeroLoadComplete() {
    ...
    //王者之塔boss加tint
    if (
      this._info.face == FaceType.RIGHT_TEAM &&
      BattleManager.Instance.battleModel.isKingTower()
    ) {
  ...
  protected delayInitView() {
    ...
    if (
      name == userID + "$" ||
      name == LangManager.Instance.GetTranslation("public.nickName")
    ) {
      if (this._info == BattleManager.Instance.battleModel.selfHero) {
    ...
    if (nickName == "" || nickName == undefined) {
      nickName = (this._info as HeroRoleInfo).heroInfo.templateInfo
        .TemplateNameLang;
    }
    this.setHeroNameTxtValue(nickName);

    if (BattleManager.Instance.battleModel.battleType == BattleType.PET_PK) {
      this.heroNameTxt.setFrame(
        (this._info as HeroRoleInfo).petRoleInfo.quality,
  ...
  private __onHeroAwakenChanged(e: RoleEvent) {
    ...
    let heroRole: HeroRoleInfo = this._info as HeroRoleInfo;
    let targetFrame: number = Math.floor(
      (heroRole.awaken * 100) / BattleModel.AWAKEN_FULL_VALUE,
    );
    if (this._awakenMc) {
  ...
  public changeBody() {
    ...
    Logger.battle(
      "变身完成isPetBody:",
      this.isPetBody,
      (this._info as HeroRoleInfo).isPetState,
    );
    ...
    if (this._info.actionMovieClip) {
      ...
      this.layoutEffect();
      Helpers.getPos(
        (this._info as HeroRoleInfo).pos_head_hero,
        this._info.actionMovieClip,
        "pos_body",
      );
      Helpers.getPos(
        (this._info as HeroRoleInfo).pos_body_hero,
        this._info.actionMovieClip,
        "pos_head",
      );
      Helpers.getPos(
        (this._info as HeroRoleInfo).pos_leg_hero,
        this._info.actionMovieClip,
        "pos_leg",
      );
    }
  ...
  public dispose() {
    ...
    this._petMovie = null;
    ObjectUtils.disposeObject(this._heroMovie);
    this._heroMovie = null;
    ObjectUtils.disposeObject(this._rolePart);
    this._rolePart = null;
    ObjectUtils.disposeObject(this._awakenShineMc);
    this._awakenShineMc = null;
    ObjectUtils.disposeObject(this._awakenMc);
    this._awakenMc = null;
