...
export class RolePropertyCom extends FUI_RolePropertyCom {
    ...
    onConstruct() {
        ...
        this.attList.itemRenderer = Laya.Handler.create(this, this.fashionBonusRender, null, false);
        this.initData();
    }
    ...
    private initData() { 
        ...
        this._pointNameList_1 = [ins.GetTranslation("pet.Reducing"),//减抗
    ...
    public OnInitWind(params) {
        ...
        this.txt_agility.text = this.txt_intellect.text = this.txt_physique.text=this.txt_power.text = str;
    ...
    public setFashionProperty(grades:number,msg:any){
        ...
        //当前等级数据
        let upgradeTemp = TempleteManager.Instance.getTemplateByTypeAndLevel(grades, UpgradeType.UPGRADE_FASHION); 
        if (upgradeTemp) {
            let skillString = upgradeTemp.TemplateNameLang;
            let skillList = TempleteManager.Instance.getSkillTemplateInfoByIds(skillString);
    ...
    private fashionBonusRender(index: number, item: FUI_FashionBounsAttributeItem) {
    ...
    private updateView() {
        ...
        this.txt_point_5.setVar("propertyName", `${this._pointNameList_0[5]}`).setVar("property", `${this._data.attackProrerty.totalIntensity}`).flushVars();
        this.txt_point_6.setVar("propertyName", `${this._pointNameList_0[6]}`).setVar("property", `${this._data.attackProrerty.totalTenacity}`).flushVars();
        ...
        this.txt_property_4.setVar("propertyName", `${this._propertyNameList[4]}`).setVar("property", `${this._data.attackProrerty.totalForceHit}`).flushVars();
        this.txt_property_5.setVar("propertyName", `${this._propertyNameList[5]}`).setVar("property", `${this._data.attackProrerty.totalParry}`).flushVars();
