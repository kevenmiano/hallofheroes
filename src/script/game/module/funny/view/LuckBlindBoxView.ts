 ...
 */
export class LuckBlindBoxView extends FUI_LuckBlindBoxView implements FunnyContent {
	//@ts-ignore
	...
	private initEvent() {
		...
		/**概率列表 */
		this.chanceBox.getChild("chanceList").asList.itemRenderer = Laya.Handler.create(this, this.onRenderChanceItem, null, false);
		...
		/**幸运盲盒信息更新 */
		NotificationManager.Instance.addEventListener(NotificationEvent.LUCKBLINDBOX_INFO_UPDATE, this.onInfoUpdate, this);
	}
	...
	private removeEvent() {
		...
		NotificationManager.Instance.removeEventListener(NotificationEvent.LUCKBLINDBOX_INFO_UPDATE, this.onInfoUpdate, this);
	}
	 ...
	 * @param item 概率项对象
	 */
	private onRenderChanceItem(index: number, item: LuckBlindBoxChanceBoxItem): void {
	...
	private onInfoUpdate(): void {
		this._luckBlindBoxMessage = LuckBlindBoxManager.Instance.luckBlindBoxMessage;
	...
	private onOpenNumberChangeHandler(value: number) {
		...
		this.diamondConsumeTxt.text = (this._diamondConsumeNumber * this._openNumber).toString();
	}
	...
	private onRuneButton(): void {
		...
		if (this._openTotal >= this._openMax) {
			MaskLockOper.Instance.doCall(false);
			MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("yishi.LuckBlindBoxManager.open.max"));
			return;
		}

		if (!LuckBlindBoxManager.Instance.luckBlindBoxOpen) {
			MaskLockOper.Instance.doCall(false);
			MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("yishi.LuckBlindBoxManager.open.exist"));
			return;
		}
	...
	private showOpenBlindBoxAlert(): boolean {
		...
		let originDate: Date = new Date(SharedManager.Instance.resolveStrengthenCheckDate);

		if (!checkFlag || (originDate.getMonth() <= currentDate.getMonth() && originDate.getDate() < currentDate.getDate())) result = true;
	...
	private openBlindBox(): void {
		...
		this.background.getChild("openBlindBox").asMovieClip.setPlaySettings(0, -1, 1, -1, Laya.Handler.create(this, this.onOpenBlindBoxCompleteHandler));
	}
	...
	private onOpenBlindBoxCompleteHandler(): void {
		LuckBlindBoxManager.Instance.requestProtocol(2, this._openNumber > 1, this._openNumber);
	}
	...
	private refreshView(): void {
		...
		this.runeBtn.getChild("consumeStatTxt").text = this._openTotal.toString() + "/" + this._openMax.toString();
		...
		openResidueMax = Math.floor(this._diamondTotal / this._diamondConsumeNumber);
		...
		this.simBox.show(0, 1, 1, openResidueMax, openResidueMax, 1, this._openNumberChangeHandler);
	}
	...
	private setResidueTime() {
		...
		if (this._residueTime >= 60) {
			...
			this.buttomBox.getChild("activityTimeTxt").text = DateFormatter.getFullTimeString(this._residueTime);
		} else if (this._residueTime > 0) {
			this.runeBtn.enabled = true;
			this.buttomBox.getChild("activityTimeTxt").text = DateFormatter.getFullDateString(this._residueTime);
		} else {
			this.runeBtn.enabled = false;
			this.buttomBox.getChild("activityTimeTxt").text = LangManager.Instance.GetTranslation("feedback.FeedBackItem.outDate");
		}
	}
	...
	private loopResidueTime(): void {
		let endTime: Date = DateFormatter.parse(this._luckBlindBoxMessage.stopTime, "YYYY-MM-DD hh:mm:ss");
