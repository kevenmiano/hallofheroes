...
export default class MailWnd extends BaseWindow {
  ...
  public OnShowWind() {
    ...
    if (
      EmailManager.Instance.mailModel.allList.keys.length >= 300 &&
      EmailManager.Instance.mailModel.totalLength >= 300
    ) {
      var prompt: string = LangManager.Instance.GetTranslation(
        "emailII.EmailControler.prompt",
      );
      var content: string = LangManager.Instance.GetTranslation(
        "emailII.EmailControler.content01",
      );
  ...
  private initView() {
    this.frame.getChild("title").text = LangManager.Instance.GetTranslation(
      "emailII.EmailFrame.title",
    );
    this.receiveBtn.view.asButton.text = LangManager.Instance.GetTranslation(
      "emailII.EmailFrame.attach",
    );
    this.deleteBtn.view.asButton.text = LangManager.Instance.GetTranslation(
      "emailII.EmailFrame.delete",
    );
    ...
    this.playInfoBtn.title = LangManager.Instance.GetTranslation(
      "emailII.EmailFrame.guild",
    );
  ...
  private addEvent() {
    this.oneKeyDeleteBtn &&
      this.oneKeyDeleteBtn.onClick(this, this.__oneKeyDeleteHandler.bind(this));
    this.oneKeyReceiveBtn &&
      this.oneKeyReceiveBtn.onClick(
        this,
        this.__oneKeyReceiveHandler.bind(this),
      );
    this.oneKeyReadBtn &&
      this.oneKeyReadBtn.onClick(this, this.__oneKeyReadHandler.bind(this));
    this.receiveBtn &&
      this.receiveBtn.onClick(this, this.__receiveHandler.bind(this));
    this.deleteBtn &&
      this.deleteBtn.onClick(this, this._deleteHandler.bind(this));
    this.c1.on(fairygui.Events.STATE_CHANGED, this, this.onChangeController);
    this.battleDeleteBtn &&
      this.battleDeleteBtn.onClick(
        this,
        this.battleDeleteBtnHandler.bind(this),
      );
    ...
    this.maillist.itemRenderer = Laya.Handler.create(
      this,
      this.renderMailListItem,
      null,
      false,
    );
    this.attachlist.itemRenderer = Laya.Handler.create(
      this,
      this.renderAttachListItem,
      null,
      false,
    );
    EmailManager.Instance.addEventListener(
      EmailEvent.DATA_EMAIL_DELETE,
      this.__deleteHandler,
      this,
    );
    if (this.mailModel) {
      this.mailModel.addEventListener(
        EmailEvent.DATA_EMAIL_UPDATE,
        this.__dateUpdateHandler,
        this,
      );
      this.mailModel.addEventListener(
        EmailEvent.UPDATE_EMAIL_STATUS,
        this.__updateStatus,
        this,
      );
      this.mailModel.addEventListener(
        EmailEvent.UPDATE_ALL_EMAIL_STATUS,
  ...
  private offEvent() {
    this.oneKeyDeleteBtn &&
      this.oneKeyDeleteBtn.offClick(
        this,
        this.__oneKeyDeleteHandler.bind(this),
      );
    this.oneKeyReceiveBtn &&
      this.oneKeyReceiveBtn.offClick(
        this,
        this.__oneKeyReceiveHandler.bind(this),
      );
    this.oneKeyReadBtn &&
      this.oneKeyReadBtn.offClick(this, this.__oneKeyReadHandler.bind(this));
    this.deleteBtn &&
      this.deleteBtn.offClick(this, this._deleteHandler.bind(this));
    this.maillist.off(
    ...
    EmailManager.Instance.removeEventListener(
      EmailEvent.DATA_EMAIL_DELETE,
      this.__deleteHandler,
      this,
    );
    if (this.mailModel) {
      this.mailModel.removeEventListener(
        EmailEvent.DATA_EMAIL_UPDATE,
        this.__dateUpdateHandler,
        this,
      );
  ...
  private refreshSelectedMail() {
    ...
    } else if (this.c1.selectedIndex == MailWnd.BATTLE_TYPE) {
      //战报
  ...
  private readEmail(mailData: EmailInfo) {
    if (mailData) {
      ...
      this.mailOverTime.text = DateFormatter.format(
        mailData.sendDate,
        "YYYY-MM-DD hh:mm",
      );
    ...
    if (this.c1.selectedIndex == MailWnd.SYSTEM_TYPE) {
      ...
      this.oneKeyReadBtn.enabled =
        this.mailModel.allBattleList.length > 0 &&
        this.mailModel.isBattleUnreadExist();
      if (mailData) {
      ...
      this.successTxt2.text = this.successTxt.text =
        LangManager.Instance.GetTranslation("mailwnd.battleReport.descTxt");
      let str: string;
      if (
        mailData &&
        mailData.report &&
        mailData.report.camp == BattleInfo.ATTACKED
      ) {
        str = LangManager.Instance.GetTranslation(
          "emailII.view.ReadBattleReportView.content01",
        );
      } else {
        str = LangManager.Instance.GetTranslation(
          "emailII.view.ReadBattleReportView.content02",
        );
      }
      this.playerIcon1.icon = IconFactory.getHeroicon(
        ArmyManager.Instance.thane.templateId,
      );
      ...
      if (mailData && mailData.report) {
        ...
        let posTxtStr: string =
          "(" +
          MapGrid.getPositionString(mailData.report.pos) +
          ")&nbsp;" +
          mailData.report.mapName;
        this.posTxt.text =
          "[url='event:'|type:" +
          posTxtStr +
          "]" +
        ...
        if (mailData.report.enemyHeroId != 0) {
          ...
          this.playerIcon2.icon = IconFactory.getHeroicon(
            mailData.report.enemyHeroId,
          );
  ...
  private onClickLink(evtData: string) {
    ...
    let isSystemMail =
      mailData.MailType == EmailType.SYStem_MAIL || mailData.SendId == 0;
    ...
    switch (clickType) {
      case "COPY":
        ...
        MessageTipManager.Instance.show(
          LangManager.Instance.GetTranslation("copy.success"),
        );
        break;
    }
  }
  ...
  private __updateStatus() {
    if (this._currentSelectedItem) {
      this.updateRedDot();
      this._currentSelectedItem.Itemdata = this.mailModel.getEmailById(
        this.mailModel.selectedMailId,
      );
    } else {
      this._updateAllEmailStatus();
    }
  ...
  private callRefresh() {
    ...
    var footer: fgui.GComponent = this.maillist.scrollPane.footer.asCom;
    if (!self.mailCtrl || self.mailCtrl.view.isDisposed) return;
    if (self.maillist.numItems >= this.listData.length) {
      ...
      //后续每次增加6条邮件
      self.maillist.numItems +=
        leftCount > this.mailModel.pageAddCapicity
          ? this.mailModel.pageAddCapicity
          : leftCount;
    }
  ...
  private __writeMailHandler() {
    ...
    if (ArmyManager.Instance.thane.grades < MailWnd.UNDERLEVEL) {
      ...
      var content: string = LangManager.Instance.GetTranslation(
        "emailII.EmailFrame.view.underSixLevel",
        MailWnd.UNDERLEVEL,
      );
      SimpleAlertHelper.Instance.Show(
        SimpleAlertHelper.SIMPLE_ALERT,
      ...
      if (!FrameCtrlManager.Instance.isOpen(EmWindow.WriteMailWnd)) {
        FrameCtrlManager.Instance.open(
          ...
          { returnToWin: EmWindow.MailWnd, users: [] },
          null,
          EmWindow.MailWnd,
        );
      }
  ...
  private __oneKeyDeleteHandler() {
    ...
    let preDate: Date = new Date(
      SharedManager.Instance.emailDeleteTipCheckDate,
    );
    ...
    if (needAlert) {
      if (this.c1.selectedIndex == MailWnd.SYSTEM_TYPE) {
        ...
        content = LangManager.Instance.GetTranslation(
          "emailII.EmailFrame.command03",
        );
      } else if (this.c1.selectedIndex == MailWnd.NORMAL_TYPE) {
        //普通邮件
        content = LangManager.Instance.GetTranslation(
          "emailII.EmailFrame.command06",
        );
      } else if (this.c1.selectedIndex == MailWnd.BATTLE_TYPE) {
        //战报
        content = LangManager.Instance.GetTranslation(
          "emailII.EmailFrame.command07",
        );
      }
      UIManager.Instance.ShowWind(EmWindow.TodayNotAlert, {
        content: content,
        backFunction: this.confirmDelete.bind(this),
        state: 2,
      });
    } else {
  ...
  private __oneKeyReceiveHandler() {
    let needAlert: boolean = true;
    let preDate: Date = new Date(
      SharedManager.Instance.emailReceiveTipCheckDate,
    );
    let now: Date = new Date();
    ...
    if (needAlert) {
      if (this.c1.selectedIndex == MailWnd.BATTLE_TYPE) {
        content = LangManager.Instance.GetTranslation(
          "emailII.EmailFrame.command05",
        );
      } else {
        content = LangManager.Instance.GetTranslation(
          "emailII.EmailFrame.command02",
        );
      }
      UIManager.Instance.ShowWind(EmWindow.TodayNotAlert, {
        content: content,
        backFunction: this.confirmReceive.bind(this),
        state: 2,
      });
    } else {
  ...
  confirmReceive(notalert: boolean, check: boolean) {
    ...
    if (this.checkIsAllRead()) {
      MessageTipManager.Instance.show(
        LangManager.Instance.GetTranslation("mailwnd.onkeyreceive.tips"),
      );
      return;
    }
  ...
  private __oneKeyReadHandler() {
    let needAlert: boolean = true;
    let preDate: Date = new Date(
      SharedManager.Instance.emailBattleReadTipCheckDate,
    );
    let now: Date = new Date();
    ...
    if (needAlert) {
      ...
      UIManager.Instance.ShowWind(EmWindow.TodayNotAlert, {
        content: content,
        backFunction: this.confirmBattleMailRead.bind(this),
        state: 2,
      });
    } else {
  ...
  private __replayHandler() {
    ...
    if (_data.MailType == EmailType.SYStem_MAIL || _data.SendId == 0) {
      var str: string = LangManager.Instance.GetTranslation(
        "emailII.view.ReadEmailView.content01",
      );
    ...
    if (!FrameCtrlManager.Instance.isOpen(EmWindow.WriteMailWnd)) {
      FrameCtrlManager.Instance.open(
        EmWindow.WriteMailWnd,
        { returnToWin: EmWindow.MailWnd, users: [_data.SendNickName] },
        null,
        EmWindow.MailWnd,
      );
    }
      ...
      if (this.checkHasAnnex(this.mailModel.currentReadEmail)) {
        MessageTipManager.Instance.show(
          LangManager.Instance.GetTranslation(
            "emailII.EmailControler.command01",
          ),
        );
        return;
      }
  ...
  private checkHasAnnex(EmailInfo: EmailInfo): boolean {
    ...
    if (
      (!EmailInfo.IsAnnex1 && EmailInfo.Annex1 != 0) ||
  ...
  private battleRecordBtnClick() {
    if (SpaceManager.PKInvite) {
      MessageTipManager.Instance.show(
        LangManager.Instance.GetTranslation("battle.record.playLater"),
      );
      return;
    }
    if (
    ...
    if (_data && _data.MailType == EmailType.BATTLE_REPORT) {
      ...
      if (url) {
        BattleRecordReader.loadData(
          PathManager.getBattleRecordPath(url),
          () => {
            BattleRecordReader.play();
          },
        );
      }
    }
  ...
  private onPosTxtHandler() {
    ...
    let posX: number = MapGrid.getConvertWidth(
      parseInt(_data.report.pos.split(",")[0]),
    );
    let posY: number = MapGrid.getConvertWidth(
      parseInt(_data.report.pos.split(",")[1]),
    );
    let posPoint: Laya.Point = new Laya.Point(posX * 50, posY * 50);
    let mInfo: MapInitData = new MapInitData();
    mInfo.mapTempInfo = TempleteManager.Instance.getMapTemplateById(
      _data.report.mapId,
    );
  ...
  private get mailModel(): MailModel {
    if (this.ctrl) return this.ctrl.data;
    return null;
  }
