...
export default class GameManager extends GameEventDispatcher {
    ...
    public parseConfig(configXml: any, siteXML: any) {
        ...
        if (!ConfigMgr.Instance.isInit) {
            ...
            if (Utils.isAndroid() || Utils.isIOS()) {
                ...
                PathManager.info.isLogoActive = PathManager.info.LOGO_CODE.indexOf(pkgName) == -1;
            } else {
    ...
    private initEvent() {
        PlayerManager.Instance.addEventListener(PlayerEvent.LOGIN_SUCCESS, this.__tryLoadStartupResource, this);//得到玩家信息
        NotificationManager.Instance.addEventListener(StartupEvent.CORE_SETUP_COMPLETE, this.__onCoreSetupLoadComplete, this);//UIMODEL和AMF
        NotificationManager.Instance.addEventListener(StartupEvent.CORE_LOAD_COMPLETE, this.__coreLoadCompleteHandler, this);
    }
    ...
    private onLoginSocketConnect() {
        Logger.base("连接登录socket : " + PathManager.SocktPath + ", Port:" + PathManager.SocketPort);
        LoginSocketManager.Instance.connect(PathManager.SocktPath, PathManager.SocketPort);
        LoginSocketManager.Instance.on(Laya.Event.COMPLETE, this.onLoginSocketConnectSuccess, this);
    }
    ...
    public onGameSocketConnect(ip?: string, port?: number, callFunc?: Function) {
        ...
        SocketManager.Instance.on(Laya.Event.COMPLETE, this.onSocketConnectSuccess, this);
    }
    ...
    private __tryLoadStartupResource(evt) {
        ...
        if (playerInfo.userId != 0) {
            GameManager.Instance.initUserInfo(PlayerManager.Instance.currentPlayerModel.userInfo);
    ...
    private __timeUpdateHandler() {
        ...
        if (leftTime == 0) {
            ...
            if (PlayerManager.Instance.currentPlayerModel.sysCurtime.getHours() == 5) {
            ...
            if (PlayerManager.Instance.currentPlayerModel.sysCurtime.getHours() == 21) {
                //21点关闭公会战按钮
            ...
            if (PlayerManager.Instance.currentPlayerModel.sysCurtime.getHours() == 0) {
    ...
    private requestLoginState() {//请求登陆状态
    ...
    private __switchSceneHandler(evt) {//此处执行登陆切换完场景需要执行的操作
    ...
    private checkScence(): boolean {
        ...
        switch (curScene) {
            ...
            case SceneType.CAMPAIGN_MAP_SCENE:
                ...
                let flag: boolean;
                if (model)
                    flag = (WorldBossHelper.checkWorldBoss(model.mapId) || WorldBossHelper.checkPvp(model.mapId) || WorldBossHelper.checkGvg(model.mapId));
                else
                    flag = false;
                if (flag) {
                    return false;
    ...
    private loginOverCall() {
        PlayerManager.Instance.removeEventListener(PlayerEvent.LOGIN_SUCCESS, this.__tryLoadStartupResource, this);
        NotificationManager.Instance.removeEventListener(StartupEvent.CORE_SETUP_COMPLETE, this.__onCoreSetupLoadComplete, this);
        NotificationManager.Instance.removeEventListener(StartupEvent.CORE_LOAD_COMPLETE, this.__coreLoadCompleteHandler, this);
        ...
        //请求用户绑定状态
        if (isOversea())
            PlayerManager.Instance.reqUserBindState();
        // ChatAirBubbleManager.Instance.reqAllAirBubbles();
        if (this._userInfo.isActive) {
            GameManager.openCache();
        } else {
            let vSQsevenRoad: string = LangManager.Instance.GetTranslation("yishi.manager.GameStoreManager.SQsevenRoad");
