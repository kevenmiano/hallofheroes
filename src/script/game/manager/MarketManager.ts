...
export default class MarketManager extends GameEventDispatcher {
  ...
  public setup() {
    ServerDataManager.listen(
      S2CProtocol.U_C_MARKET_ITEM_LIST,
      this,
      this.OnMarketInfoList,
    );

    ServerDataManager.listen(
      S2CProtocol.U_C_MARKET_MYORDER_LIST,
      this,
      this.OnMineOrderList,
    );

    ServerDataManager.listen(
      S2CProtocol.U_C_MARKET_SELLITEM_INFO,
      this,
      this.OnMarketSellList,
    );

    ServerDataManager.listen(
      S2CProtocol.U_C_MARKET_PURCHASEITEM_INFO,
      this,
      this.OnMarketPurcharseList,
    );
    ...
    ServerDataManager.listen(
      S2CProtocol.U_C_RESET_MARKET_USE_ORDERCOUNT,
      this,
      this.onResetUseOrderCount,
    );
  }
  ...
  public reqData() {
    ...
    if (!this.refreshMarketInfo()) {
      this.dispatchEvent(
        MarketManager.OnMarketInfoList,
        this.marketItemInfoList,
      );
    }
    ...
    if (!this.marketSuccessOrders) {
      ...
      this.dispatchEvent(
        MarketManager.OnMineOrderSucessList,
        this.marketSuccessOrders,
      );
    }
  }
  ...
  private OnMarketInfoList(pkg: PackageIn) {
    ...
    this.marketInfoUpdateTime =
      PlayerManager.Instance.currentPlayerModel.sysCurTimeBySecond;
    let msg: MarketItemListMsg = pkg.readBody(
      MarketItemListMsg,
    ) as MarketItemListMsg;
  ...
  private OnMineOrderList(pkg: PackageIn) {
    let msg: MarketOrderListMsg = pkg.readBody(
      MarketOrderListMsg,
    ) as MarketOrderListMsg;
    ...
    if (msg.op == 100) {
      ...
      for (let o of msg.orderList) {
        ...
        if (o.status == 6) {
          if (
            this.getOrderByOrderId(o.orderId, this.marketSuccessOrders) == null
          ) {
  ...
  private removeOrderByOrderId(
    orderId: string,
    marketOrders: IMarketOrderMsg[],
  ) {
    if (!marketOrders) return null;
  ...
  private OnMarketSellList(pkg: PackageIn) {
    let msg: MarketItemSellInfoMsg = pkg.readBody(
      MarketItemSellInfoMsg,
    ) as MarketItemSellInfoMsg;
  ...
  private OnMarketPurcharseList(pkg: PackageIn) {
    let msg: MarketItemPurchaseInfoMsg = pkg.readBody(
      MarketItemPurchaseInfoMsg,
    ) as MarketItemPurchaseInfoMsg;
    this.dispatchEvent(MarketManager.OnMarketPurcharseList, msg);
  }

  private OnMarketOrderOption(pkg: PackageIn) {
    let msg: MarketOrderRespMsg = pkg.readBody(
      MarketOrderRespMsg,
    ) as MarketOrderRespMsg;
    //出售
    if (msg.freeUseMarkeOrderCount) {
      this.freeUseMarkeOrderCount2 = msg.freeUseMarkeOrderCount;
    }
  ...
  private checkNotPurchargeState(goods: GoodsInfo) {
    //神器
    if (
      goods.templateInfo.MasterType == 1 &&
      goods.templateInfo.SonType == 120
    ) {
      //已经鉴定 不能出售
      return (
        goods.randomSkill1 ||
