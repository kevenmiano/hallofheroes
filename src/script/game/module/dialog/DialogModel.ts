...
export class DialogModel extends GameEventDispatcher {
    ...
    public get rightRoles(): Dictionary {
        ...
        this._infos.forEach(value => {
            if (value.direction == DialogConstInfo.RIGHT && StringUtils.isEmpty(value.event)) {
        ...
        for (let j: number = 0; j < itemXmlList.length; j++) {
            ...
            if (itemXmlList[j].roleId == 0) {
                ...
                if (PlayerManager.Instance.currentPlayerModel.playerInfo.nickName == PlayerManager.Instance.currentPlayerModel.userInfo.userId + "$") {
                    info.roleNameLang = LangManager.Instance.GetTranslation("public.nickName");
                }
                else {
                    info.roleNameLang = PlayerManager.Instance.currentPlayerModel.playerInfo.nickName;
                }
            }
            this.addInfo(info);
        }
