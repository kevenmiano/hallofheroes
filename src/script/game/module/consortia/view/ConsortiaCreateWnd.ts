...
export class ConsortiaCreateWnd extends BaseWindow {
    ...
    public OnInitWind() {
        super.OnInitWind();
        this.frame.getChild("title").text = LangManager.Instance.GetTranslation("ConsortiaCreateWnd.title");
        this.txtName.text = LangManager.Instance.GetTranslation("ConsortiaCreateWnd.txtNameTxt");
        this.txtCondition.text = LangManager.Instance.GetTranslation("ConsortiaCreateWnd.txtConditionTxt");
        this.descTxt.text = LangManager.Instance.GetTranslation("ConsortiaCreateWnd.descTxt");
        this.txtConditionValue.text = LangManager.Instance.GetTranslation("ConsortiaCreateWnd.txtConditionValue");
        this.btnConfirm.title = LangManager.Instance.GetTranslation("ConsortiaCreateWnd.btnConfirm.title");
    ...
    private initEvent() {
        ...
        this.tfSearch.promptText = LangManager.Instance.GetTranslation("ConsortiaContributeWnd.tfDiamond.text");
        this.tfSearch.on(Laya.Event.INPUT, this, this.onChanged);
        this.btnConfirm.onClick(this, this.btnConfirmHandler);
        this.thane.addEventListener(PlayerEvent.THANE_LEVEL_UPDATE, this.onPlayInfoUpdata, this);
        NotificationManager.Instance.addEventListener(LoginEvent.LOGIN_OTHER, this.onLoginOther, this);
    }

    private removeEvent() {
        this.tfSearch.off(Laya.Event.INPUT, this, this.onChanged);
        this.btnConfirm.offClick(this, this.btnConfirmHandler);
        this.thane.removeEventListener(PlayerEvent.THANE_LEVEL_UPDATE, this.onPlayInfoUpdata, this);
        NotificationManager.Instance.removeEventListener(LoginEvent.LOGIN_OTHER, this.onLoginOther, this);
    }
    ...
    private checkEnabled() {
        var goods: any[] = GoodsManager.Instance.getGoodsBySonType(GoodsSonType.SONTYPE_CONSORTIA_CARD);
        if (this.gold.count < ConsortiaModel.CREAT_NEEDED_GOLD && goods.length == 0) {
    ...
    private canCreat(): boolean {
        ...
        if (this.gold.count < ConsortiaModel.CREAT_NEEDED_GOLD) {
            str = LangManager.Instance.GetTranslation("consortia.view.club.CreatConsortiaFrame.command08", ConsortiaModel.CREAT_NEEDED_GOLD);
            MessageTipManager.Instance.show(str);
            this.txtCoin.color = "#FF0000";
            return false;
        }
        if (this.thane.grades < ConsortiaModel.CREAT_NEEDED_GRADES) {
            str = LangManager.Instance.GetTranslation("consortia.view.club.CreatConsortiaFrame.command09", ConsortiaModel.CREAT_NEEDED_GRADES);
            MessageTipManager.Instance.show(str);
            return false;
        }
        this.tfSearch.text = StringHelper.trim(this.tfSearch.text);
        if (StringHelper.isNullOrEmpty(this.tfSearch.text)) {
            str = LangManager.Instance.GetTranslation("consortia.view.club.CreatConsortiaFrame.command10");
            MessageTipManager.Instance.show(str);
            return false;
        }
        if (FilterWordManager.isGotForbiddenWords(this.tfSearch.text, "name") ||
            FilterWordManager.isGotForbiddenWords(this.tfSearch.text, "chat")) {
            str = LangManager.Instance.GetTranslation("consortia.view.club.CreatConsortiaFrame.command11");
            MessageTipManager.Instance.show(str);
            return false;
        }
        var length: number = this.tfSearch.text.replace(/[^\x00-\xff]/g, "xx").length;
        if (length > 10 || length < 4) {
            str = LangManager.Instance.GetTranslation("consortia.view.club.CreatConsortiaFrame.command12");
            MessageTipManager.Instance.show(str);
            return false;
        }
        return true;
