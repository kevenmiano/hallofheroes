...
export default class TrailMapShopCellItem extends fgui.GComponent {
    ...
    private btnBuyClick() {
        ...
        let content: string = LangManager.Instance.GetTranslation("campaign.TrailShop.BuyConfirmTxt", this._info.skillTemp.SkillTemplateName, this._info.cost);
        SimpleAlertHelper.Instance.Show(SimpleAlertHelper.USEBINDPOINT_ALERT, { checkRickText: checkStr, checkRickText2: checkStr2, checkDefault: true }, null, content, null, null, this.__btnBuyClick.bind(this));
    }

    private __btnBuyClick(b: boolean, flag: boolean) {
        if (b) {
            let cost = this._info.cost;
            let playerCount = PlayerManager.Instance.currentPlayerModel.playerInfo.allPoint;
            if (playerCount < cost) {
