...
export class UpgradePawnTipView extends TaskTraceTipWnd {
  ...
  initView() {
    ...
    let text = LangManager.Instance.GetTranslation(
      "tasktracetip.view.UpgradePawnTipView.content",
    );
    this.setContentText(text);
    this.setBtnTitle(
      LangManager.Instance.GetTranslation(
        "tasktracetip.view.UpgradePawnTipView.text",
      ),
    );
  }
  ...
  __btnHandler(e: Event) {
    ...
    if (!ap || !ArmyManager.Instance.casernPawnList[ap.templateInfo.SonType])
      ap = ArmyManager.Instance.casernPawnList[101] as ArmyPawn;
