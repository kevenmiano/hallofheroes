...
export default class AllocateFormationWnd extends BaseWindow {
    ...
    private initView() {
        for (let index = 1; index <= 6; index++) {
            this["figureItem" + index] = new ShowAvatarBattle(HeroMovieClipRefType.UI_PANEL)
    ...
    private __frontRowBtnClickHandler() {
        if (this._type == 1) {
            MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("FormationExWnd.frontRowBtnClickHandler.tips"));
            return;
        }
        ...
        if (this.ap && this.ap.templateInfo && this.ap.ownPawns > 0) {
            ...
            this._type = 1;
            ArmySocketOutManager.sendEditArmyPos(ArmyManager.Instance.army.baseHero.userId, ArmyManager.Instance.army.baseHero.fightPos, this.armyPawn.fightPos);
            ArmyManager.Instance.isSorted = true;
            this.refreshArmyView();
        } else {
            MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("armyII.ArmyControler.command02"));
            return;
        }
    ...
    private __backRowBtnClickHandler() {
        if (this._type == 0) {
            MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("FormationExWnd.backRowBtnClickHandler.tips"));
            return;
        }
        if (Laya.Browser.now() - this._intervalTime < 1000) {
            MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("activity.view.ActivityItem.command01"));
    ...
    private refreshRowBtnState() {
        if (this.ap.ownPawns != 0) {
            if (ArmyManager.Instance.army.baseHero.fightPos == this._frontRowIndex && this.armyPawn.fightPos == this._backRowIndex) {
                this.selectedPic.x = 478;
                this._type = 0;
            } else if (ArmyManager.Instance.army.baseHero.fightPos == this._backRowIndex && this.armyPawn.fightPos == this._frontRowIndex) {
                this.selectedPic.x = 724;
                this._type = 1;
            } else {
                this.selectedPic.x = 478;
    ...
    private refreshView() {
        ...
        this.pawnFigureData.urlPath = PathManager.solveRolePath(this.ap.templateInfo.Swf.toLocaleLowerCase())
