...
export class PetSkillItemListController {
    ...
    private addEvent() {
        NotificationManager.Instance.addEventListener(BattleNotic.REMOTEPET_SKILL, this.__skillUse, this);
        ...
        RemotePetManager.Instance.model.addEventListener(RemotePetEvent.COMMIT, this.onSkillCountHandler, this);


    }

    private removeEvent() {
        NotificationManager.Instance.removeEventListener(BattleNotic.REMOTEPET_SKILL, this.__skillUse, this);
        ...
        RemotePetManager.Instance.model.removeEventListener(RemotePetEvent.COMMIT, this.onSkillCountHandler, this);
    }
   ...
   */
    ...
    private get isWatchOnly(): boolean {
        if (BattleManager.Instance.battleModel.battleType == BattleType.PET_PK) {
            return true;
        } else if (BattleManager.Instance.battleModel.battleType == BattleType.MONOPOLY_BATTLE_HELP) {
            return true;
        }
        return false;
    }
    ...
    private __skillUse(data: any) {
        ...
        if (data instanceof t_s_skilltemplateData) {
            ...
            SocketSendManager.Instance.sendUsePetBattleSkill(battleId, currentskillId);
        ...
        if (result == 0)//服务器返回技能使用成功
        {
            ...
            if (heroView) {
                if (heroView.info && heroView.info.isLiving) {
                    skillInfo = TempleteManager.Instance.getSkillTemplateInfoById(skillId)
                    heroView.addCollectionEffect(false);//只在头上显示技能图标
                    heroView.addSkillFlag(skillInfo);
                    if (skillInfo.SonType == 20010)//技能CD清零
                    {
    ...
    private getSkillTempInfoById(skillId: number): t_s_skilltemplateData {
        ...
        for (let element of allSkill) {
            ...
            if (element.templateId == skillId || //技能id匹配
                (this._battleModel.isAutoFight && element.templateInfo.Parameter3 == skillId))		//技能QTE id 匹配
            {
