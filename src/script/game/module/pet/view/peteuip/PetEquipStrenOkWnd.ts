...
export default class PetEquipStrenOkWnd extends BaseWindow {
    ...
    OnShowWind() {
        super.OnShowWind();
        this.txt_lv1.text = LangManager.Instance.GetTranslation("public.level3",this.params.level);
        this.txt_lv2.text = LangManager.Instance.GetTranslation("public.level3",this._goodsInfo.strengthenGrade);
        if(this._goodsInfo){
        ...
        if(masterAttr){
            ...
            for (let i = 0; i < tempArr.length; i++) {
                ...
                if(element.length > 0){
                    ...
                    if(cfg){
                        this['item'+attr_num].getChild('txt_attr_key').text = cfg.AttributeNameLang;
                        this['item'+attr_num].getChild('txt_attr_val1').text = arr[1];
                        this['item'+attr_num].getChild('txt_attr_val').text = (Number(arr[1]) - Number(arr[2]))+'';
