...
import { ResourceEvent, TechnologyItemEvent } from '../../constant/event/NotificationEvent';
...
export default class SeminaryUpWnd extends BaseWindow {
    ...
    public OnInitWind() {
        super.OnInitWind();
        this.currentLevelTxt.text = LangManager.Instance.GetTranslation("yishi.view.tips.TechnologyTip.currentGrade");
        this.nextLevelTxt.text = LangManager.Instance.GetTranslation("yishi.view.tips.TechnologyTip.nextEffect.text");
        this.costTxt.text = LangManager.Instance.GetTranslation("CasernRecruitWnd.costResourceTxt");
    ...
    private addEvent() {
        ...
        NotificationManager.Instance.addEventListener(TechnologyItemEvent.UPDATA, this.__onDataUpdateHandler, this);
        ResourceManager.Instance.waterCrystal.addEventListener(ResourceEvent.RESOURCE_UPDATE, this.onCrystalUpdate, this);
    }

    private removeEvent() {
        this.btnSpeedup.offClick(this, this.onSpeedUp.bind(this));
        this.BtnUpgrade.offClick(this, this.__upgradeClickHandler.bind(this));
        NotificationManager.Instance.removeEventListener(TechnologyItemEvent.UPDATA, this.__onDataUpdateHandler, this);
        ResourceManager.Instance.waterCrystal.removeEventListener(ResourceEvent.RESOURCE_UPDATE, this.onCrystalUpdate, this);
    }
    ...
    private initData() {
        if (this._data) {
            ...
            if (this._temp.BuildingGrade) {
                this.LevelTxt.text = LangManager.Instance.GetTranslation("fish.FishFrame.levelText", this._temp.BuildingGrade);
            }
            else {
                this.LevelTxt.text = "";
            }
            ...
            if (!this._nTemp) {
                this.descTxt2.text = LangManager.Instance.GetTranslation("yishi.view.tips.TechnologyTip.topGrade");
            }
            else {
                this.descTxt2.text = this._nTemp.ActivityLang;
                this.refreshPreBuild(this._nTemp);
                this.refreshResource(this._nTemp);
                this.reduceTxt.text = LangManager.Instance.GetTranslation("yishi.view.tips.TechnologyTip.needTime.text");
                var time: number = this.playerEffect.getBuildTimeAdditionValue(this._nTemp.UpgradeTime);
    ...
    private refreshPreBuild(temp: t_s_buildingtemplateData) {
        ...
        var preList: Array<t_s_buildingtemplateData> = BuildingManager.Instance.getPreBuildingList(temp);
        if (preList.length == 0) {
            return;
        }
        var bTemp: t_s_buildingtemplateData = preList[0];
        var bInfo: BuildInfo = BuildingManager.Instance.getBuildingInfoBySonType(bTemp.SonType);
        if (!bInfo || bInfo.templeteInfo.BuildingGrade < bTemp.BuildingGrade) {
            this.conditionTxt.text = LangManager.Instance.GetTranslation("SeminaryUpWnd.condition", bTemp.BuildingNameLang, bTemp.BuildingGrade);
    ...
    private refreshResource(nextTemp: t_s_buildingtemplateData) {
        if (!nextTemp) {
            ...
            this.costValueTxt.text = ResourceManager.Instance.waterCrystal.count + "/0";
            return;
        }
        ...
        if (dic) {
            ...
            this.costValueTxt.text = ResourceManager.Instance.waterCrystal.count + "/" + needValue;
        }
    }

    private getNextGrade(temp: t_s_buildingtemplateData): t_s_buildingtemplateData {
        if (temp.NextGradeTemplateId != 0) {
            return TempleteManager.Instance.getBuildTemplateByID(temp.NextGradeTemplateId);
        }
        return null;
    }
    ...
    private onSpeedUp() {
        ...
        if (this.order.remainTime > 0) {
            ...
            if (VIPManager.Instance.model.vipCoolPrivilege) {
                ...
                UIManager.Instance.ShowWind(EmWindow.VipCoolDownFrameWnd, { orderId: this.order.orderId, pointNum: point, backFun: back.bind(BuildingManager.Instance) });
            }
        } else {
    ...
    private __upgradeClickHandler() {
        if (BuildingManager.Instance.checkTecList(this._data)) {
            ...
            if (this._data.buildingId != 0) {//升级科技
                isMaxLevel = BuildingManager.Instance.model.isMaxLevel(this._data.buildingId);
                if (isMaxLevel) {
                    var isMaxLevelTip: string = LangManager.Instance.GetTranslation("buildings.seminary.view.SeminaryFrameBottomView.isMaxLevelTip");
                ...
                BuildingManager.Instance.creatBuilding(this.getTemp(this._data).SonType);
            }
        }
    }
