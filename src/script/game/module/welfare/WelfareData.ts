...
export default class WelfareData extends FrameDataBase {
  ...
  protected show(): void {
    ...
    WelfareData.TAB_DATA[WelfareTab.DEGREE_ACTIVITY] =
      WelfareData.DegreeActivityView;
    ...
    let cfgItem =
      TempleteManager.Instance.getConfigInfoByConfigName("RECOVER_POINT");
    if (cfgItem) {
    ...
    if (this._levelPackageArr && this._levelPackageArr.length > 0) {
      ...
      if (arrItemId > 2) {
        ...
        for (let i = 0; i < this._levelPackageArr.length; i++) {
          if (
            this._levelPackageArr[i].id == arrPreItemId ||
            this._levelPackageArr[i].id == arrPreItemId - 1
          ) {
        ...
        for (let i = 0; i < this._levelPackageArr.length; i++) {
          if (
            this._levelPackageArr[i].id == 1 ||
            this._levelPackageArr[i].id == 2
          ) {
            arrPreItem = this._levelPackageArr[i];
            diamondArr.push(arrPreItem);
          }
  ...
  public get canSign(): boolean {
    let index = Math.floor(
      DateFormatter.compareDayIndex(this.playerInfo.signDate, this.today),
    );
    if (index <= 0) {
      index = 1;
    }
  ...
  public get canBuyLevelGift(): boolean {
    ...
    for (let i = 0; i < this.getLevelGiftDimaondArr().length; i++) {
      ...
      if (
        ...
        this.currentGetPackageId >= item.id
      ) {
        flag = true;
        break;
      }
  ...
  public get canReceivePassCheckReward(): number {
    let flag: number = 0;
    let array = TempleteManager.Instance.getPassCheckCfeByJob(
      this.passRewardInfo.passIndex,
    );
    if (array.length == 0) {
    ...
    if (array) {
      for (let i = 0; i < array.length; i++) {
        ...
        let isclaimed: boolean = this.passRewardInfo.isReceivedBase(
          element.Grade,
        );
        if (!isclaimed) {
          //未领取
          if (this.passGrade >= element.Grade) {
            //可领取
            flag = element.Grade;
            break;
          }
        ...
        if (!isclaimed) {
          //未领取
          if (this.passGrade >= element.Grade && this.passRewardInfo.isPay) {
            //可领取
            flag = element.Grade;
            break;
          }
    ...
    if (
      this.passRewardInfo.rewardGrade < this.passGrade &&
      this.passRewardInfo.rewardGrade > 0
    ) {
  ...
  public get needShowLevelGiftBtn(): boolean {
    ...
    for (let i = 0; i < this._levelPackageArr.length; i++) {
      let item: LevelGiftItemInfo = this._levelPackageArr[i];
      if (item && item.packageState1 != WelfareData.HAS_GET) {
        //有任何一个还没有领取, 则现在等级礼包项目
        flag = true;
        break;
      }
    }
  ...
  /**得到七日登录奖励领取状态 */
  public getSevenLoginRewardStatus(
    day: number,
    site: number,
    currentDay: number,
  ) {
