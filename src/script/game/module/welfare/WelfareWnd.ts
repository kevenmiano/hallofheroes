...
export default class WelfareWnd extends BaseWindow {
  ...
  private addActivityTimeBtn() {
    ...
    this.tabURL.push({
      url: ActivityTimeView.URL,
      sort: WelfareTab.ACTIVITY_TIME,
    });
    this.updateTabList();
  }
  ...
  private updateTabList() {
    this.tabData = ArrayUtils.sortOn(
      this.tabData,
      "sort",
  ...
  initDegreeActivityData() {
    ...
    this.ctrlData.dayDegreeBoxs = ArrayUtils.sortOn(
      this.ctrlData.dayDegreeBoxs,
      "point",
      ArrayConstant.NUMERIC,
    );
  ...
  onSelectTab(tabIndex: number) {
    if (
      !WelfareManager.Instance.bindPhoneTabFirstOpen &&
      this.tabURL[tabIndex].sort == WelfareTab.BIND_PHONE
    ) {
      WelfareManager.Instance.firstOpenBindPhoneTab();
    }
    if (
      !WelfareManager.Instance.bindMailTabFirstOpen &&
      this.tabURL[tabIndex].sort == WelfareTab.BIND_MAIL
    ) {
