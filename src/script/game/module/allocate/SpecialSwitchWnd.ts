...
import { NotificationEvent, PawnEvent } from '../../constant/event/NotificationEvent';
...
export default class SpecialSwitchWnd extends BaseWindow {
    ...
    public OnInitWind() {
        this.n5.getChild("title").text = LangManager.Instance.GetTranslation("buildings.casern.view.PawnSpecialAbilityFrame.title");
        this.n14.text = LangManager.Instance.GetTranslation("CasernRecruitWnd.costResourceTxt");
        let cfgItem = TempleteManager.Instance.getConfigInfoByConfigName("PawnChange_NeedPoint");
        if (cfgItem) {
    ...
    OnShowWind() {
        ...
        for (var i: number = 0; i < this.tempArr.length; i++) {
            if (this.tempArr[i].templateId != this._oldPawn.templateId && this.tempArr[i].templateInfo.Level >= 10) {
        ...
        this.setColorStyle();
        this.cfgNeedGold = Number(TempleteManager.Instance.getConfigInfoByConfigName("PawnChange_NeedStrategy").ConfigValue);
    ...
    private addEvent() {
        ...
        for (let key in this.tempArr) {
            if (Object.prototype.hasOwnProperty.call(this.tempArr, key)) {
                ...
                ap.addEventListener(PawnEvent.SPECIAL_ALIBITY, this.__converSucceeded, this);
            }
        }
        NotificationManager.Instance.addEventListener(NotificationEvent.UPDATE_CONFIRM_SPECIALSELECTITEM, this.__refreshHandler, this);
        NotificationManager.Instance.addEventListener(NotificationEvent.CLOSE_SPECIALSELECTWND, this.__resetPosHandler, this);
    }
    ...
    private removeEvent() {
        ...
        for (let key in this.tempArr) {
            if (Object.prototype.hasOwnProperty.call(this.tempArr, key)) {
                let ap: ArmyPawn = this.tempArr[key];
                ap.removeEventListener(PawnEvent.SPECIAL_ALIBITY, this.__converSucceeded, this);
            }
        }
        NotificationManager.Instance.removeEventListener(NotificationEvent.UPDATE_CONFIRM_SPECIALSELECTITEM, this.__refreshHandler, this);
        NotificationManager.Instance.removeEventListener(NotificationEvent.CLOSE_SPECIALSELECTWND, this.__resetPosHandler, this);
    }
    ...
    private refreshView() {
        var oldTemp: t_s_specialtemplateData = TempleteManager.Instance.getPawnSpecialTemplateByID(parseInt(this._oldPawn.specialAbility));
        var newTemp: t_s_specialtemplateData = TempleteManager.Instance.getPawnSpecialTemplateByID(parseInt(this._newPawn.specialAbility));
        ...
        this.NameTxt1.text = oldTemp.TemplateNameLang + " " + LangManager.Instance.GetTranslation("public.level2", oldTemp.Grades.toString());
        this.NameTxt2.text = newTemp.TemplateNameLang + " " + LangManager.Instance.GetTranslation("public.level2", newTemp.Grades.toString());
    ...
    private __onSelectHandler() {
        if (UIManager.Instance.isShowing(EmWindow.SpecialSelecteWnd)) {
            ...
            UIManager.Instance.ShowWind(EmWindow.SpecialSelecteWnd, { postion: point, pawn: this._newPawn });
        }
    }
    ...
    private __onSwitchHandler() {
        ...
        if (needAlert) {
            var content: String = LangManager.Instance.GetTranslation("armyII.viewII.pawnupgrade.PawnAbilityConvertFrame.pawnSpecial");
            var checkTxt: String = LangManager.Instance.GetTranslation("yishi.view.base.ThewAlertFrame.text");
            if (this.SoulSelect.selected)//战魂
            {
            ...
            } else if (this.DiamondSelect.selected)//钻石
            {
    ...
    private sendConverByPoint(useBind: boolean) {
        ...
        if (this._oldPawn.templateId == this._newPawn.templateId) {
            str = LangManager.Instance.GetTranslation("armyII.viewII.pawnupgrade.samePawn");
        ...
        ArmySocketOutManager.sendPawnChange(this._oldPawn.templateId, this._newPawn.templateId, 1, useBind);
    }
    ...
    private sendConverBySoul(useBind: boolean) {
        ...
        if (ResourceManager.Instance.gold.count < this.cfgNeedGold) {
            str = LangManager.Instance.GetTranslation("buildings.casern.view.PawnLevelUpFrame.command05");
            MessageTipManager.Instance.show(str);
            return;
        }
        ArmySocketOutManager.sendPawnChange(this._oldPawn.templateId, this._newPawn.templateId, 2, useBind);
    }
    ...
    private _helpClick() {
        ...
        let content: string = LangManager.Instance.GetTranslation("buildings.casern.view.PawnAbilityConvertFrame.helpContent");
