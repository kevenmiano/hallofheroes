...
import NewbieEvent, { NotificationEvent, RewardEvent, RingTaskEvent, TaskEvent } from '../../constant/event/NotificationEvent';
...
export default class TaskRingItem extends FUI_TaskRingItem {
    ...
    private _handIn: string = LangManager.Instance.GetTranslation("YiShi.mainBar.view.TaskTraceItem.HandInText");
    onConstruct() {
        super.onConstruct();
        this.bgActive = false;
        this.initEvent();
    ...
    private clean() {
        ...
        if (this._vdata && this._vdata instanceof TaskTemplate)
            this._vdata.removeEventListener(TaskEvent.TASK_DETAIL_CHANGE, this.__taskChangeHandler, this);
        else if (this._vdata && this._vdata instanceof BaseOfferReward)
            this._vdata.removeEventListener(RewardEvent.REWARD_TASK_UPDATE, this.__taskChangeHandler, this);
        else if (this._vdata && this._vdata instanceof RingTask)
            this._vdata.removeEventListener(RingTaskEvent.REFRESHRING, this.__taskChangeHandler, this);
        this._vdata = null;
    }
    ...
    public set vData(value: any) {
        ...
        if (this._vdata) {
            ...
            if (this._vdata instanceof TaskTemplate) {
                ...
                this._vdata.addEventListener(TaskEvent.TASK_DETAIL_CHANGE, this.__taskChangeHandler, this);
            }
            else if (this._vdata instanceof BaseOfferReward) {
                this._type = TaskRingItem.TypeBaseOfferReward;
                this._vdata.addEventListener(RewardEvent.REWARD_TASK_UPDATE, this.__taskChangeHandler, this);
            }
            else if (this._vdata instanceof RingTask) {
                this._type = TaskRingItem.TypeRing;
                this._vdata.addEventListener(RingTaskEvent.REFRESHRING, this.__taskChangeHandler, this)
            }
            this.refresh();
        }
    }
    ...
    private speedBtnHandler(e: Laya.Event) {
        if (this._vdata instanceof RingTask) {//跑环
    ...
    private get cfgRingTaskQuickCost(): number {
        let cfgValue = 10;
        let cfgItem = TempleteManager.Instance.getConfigInfoByConfigName("SkipRepeatReward_Price");
        if (cfgItem) {
    ...
    private ringTaskQuickHandler() {
        ...
        if (needAlert) {
            let content: string = LangManager.Instance.GetTranslation("task.TaskFrameII.usePointPrompt2");
            var num: number = GoodsManager.Instance.getGoodsNumByTempId(ShopGoodsInfo.IMPERIAL_PASS);
            let goodsCount: string = LangManager.Instance.GetTranslation("MazeShopWnd.HasNumTxt") + num;
            UIManager.Instance.ShowWind(EmWindow.UseGoodsAlert, { content: content, goodsId: ShopGoodsInfo.IMPERIAL_PASS, goodsCount: goodsCount, hidecheck1: false, callback: this.quickSubmit.bind(this) });
            return;
        }
        else {
    ...
    private quickSubmit(b: boolean, flag: boolean) {
        if (b) {
            ...
            var hasGoods: Array<GoodsInfo> = GoodsManager.Instance.getBagGoodsByTemplateId(ShopGoodsInfo.IMPERIAL_PASS);
            if (hasGoods.length > 0) {
                RingTaskManager.Instance.CompletedImmediately(b);
            } else {
                //数量不足时, 弹商城购买窗口
                let info: ShopGoodsInfo = TempleteManager.Instance.getShopTempInfoByItemId(ShopGoodsInfo.IMPERIAL_PASS, ShopGoodsInfo.PROP_GOODS);
                if (info) {
    ...
    private __clickHandler() {
        ...
        // NotificationManager.Instance.sendNotification(NewbieEvent.ARROW_STATE, { type: 0, tip: null });
        SwitchPageHelp.skipPageByTaskCondictionType(this._vdata, this.openTaskFrame, this);
    ...
    private checkTaskComplete(vData: any): boolean {
        if (vData instanceof BaseOfferReward) {
            ...
            if (vData.isCompleted) {
                if (PlayerManager.Instance.currentPlayerModel.playerInfo.noviceProcess == GlobalConfig.NEWBIE_32000) {
                    SDKManager.Instance.getChannel().adjustEvent(RPT_EVENT.TASK_TO_GRASSLAND);
                } else if (PlayerManager.Instance.currentPlayerModel.playerInfo.noviceProcess == GlobalConfig.NEWBIE_35600) {
                    SDKManager.Instance.getChannel().adjustEvent(RPT_EVENT.TASK_CHALLENGE);
                } else if (vData.TemplateId == 350) {
                    SDKManager.Instance.getChannel().adjustEvent(RPT_EVENT.TASK_GET_FOREST_UNDERWORLD);
                }
    ...
    private __taskChangeHandler() {
        ...
        NotificationManager.Instance.sendNotification(NotificationEvent.TASKRING_ITEM_UPDATE);
    }
    ...
    private setVState() {
        ...
        if (this._vdata instanceof TaskTemplate) {
            if (this._vdata.TemplateType == TaskType.TASK_ACT ||
            ...
            switch (this._vdata.TemplateType) {
                case TaskType.TASK_MAIN:
                    this.typeTxt.text = LangManager.Instance.GetTranslation("TaskRingItem.type.text1");
                    break;
                case TaskType.TASK_COMMON:
                    this.typeTxt.text = LangManager.Instance.GetTranslation("TaskRingItem.type.text2");
                    break;
                default:
                    this.typeTxt.text = "";
                    break;
            ...
            this.typeTxt.text = LangManager.Instance.GetTranslation("TaskRingItem.type.text3");
        } else if (this._vdata instanceof RingTask) {
            this.ringNumStr = "(" + (RingTaskManager.Instance.model.rewardNum - 1) + LangManager.Instance.GetTranslation("ui.tasktracebar.view.ringTaskNum") + ")";
            this.typeTxt.text = LangManager.Instance.GetTranslation("TaskRingItem.type.text4");

            if (RingTaskManager.Instance.getRingTask()) {
                this.cType.selectedIndex = 1;
                this.ringNumStr = "(" + RingTaskManager.Instance.model.rewardNum + LangManager.Instance.GetTranslation("ui.tasktracebar.view.ringTaskNum") + ")";
            } else {
                this.cType.selectedIndex = 0;
            }
        }
    ...
    private refreshCondition() {
        ...
        for (let i: number = 0; i < this._vdata.conditionList.length; i++) {
            ...
            if (String(progress[i]) == LangManager.Instance.GetTranslation("buildings.offerreward.data.BaseOfferReward.progress")) {
                continue;
            }
                ...
                if (RingTaskManager.Instance.getRingTask()) {
                    switch (ringCondition.CondictionType) {
                        case RewardConditionType.COMMITITEM:
                            let goodstempleteInfo: t_s_itemtemplateData = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_itemtemplate, this._vdata.currTask.condition_3);
                        ...
                        case RewardConditionType.KILLDUPLICATEMONSTER:
                            let mapTemplate: t_s_campaignData = TempleteManager.Instance.getCampaignTemplateByID(this._vdata.currTask.condition_2);
                            if (mapTemplate) {
                    ...
                    str = LangManager.Instance.GetTranslation("ui.tasktracebar.view.TaskTraceItem.taskcontent");
                    flag = true;
                }
            }
        ...
        if (this._flag) {
            ...
            this.typeTxt.color = this.ringTaskTxt.color = ColorConstant.LIGHT_TEXT_COLOR;
            ...
            if (this._vdata instanceof RingTask) {
                ...
                if (this._vdata.TemplateType == TaskType.TASK_MAIN) {//主线
                    this.typeTxt.color = this.ringTaskTxt.color = ColorConstant.GOLD_COLOR;
                }
    ...
    public startBlink(time: number = 1) {
        ...
        TweenMax.to(this.Img_Shine, 0.5, {
            alpha: 0, yoyo: true, repeat: time, onComplete: () => {
                TweenMax.killTweensOf(this.Img_Shine);
