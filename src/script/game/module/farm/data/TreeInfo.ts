...
export class TreeInfo extends GameEventDispatcher {
    ...
    public set leftpickTime(value: number) {
        ...
        if (this.left_pickTime <= 0) {
            ...
            if (this.userId == this.thane.userId && this.canPick && (currentScence == SceneType.CASTLE_SCENE || this.inHoodRoom())) {
    ...
    private inHoodRoom(): boolean {
        if (SceneManager.Instance.currentType == SceneType.CAMPAIGN_MAP_SCENE && CampaignManager.Instance.mapModel && WorldBossHelper.checkHoodRoom(CampaignManager.Instance.mapModel.mapId)) {
            return true;
        }
        return false;
    }
    ...
    public get remainWaerTime(): number {
        ...
        return (this.lastWaterTime.getTime() + 10 * 60 * 1000 - (this.serverTime.getTime() + Laya.Browser.now())) / 1000;
    }

    public get remainPickTime(): number {
        let curServerTime: number = this.serverTime.getTime() + Laya.Browser.now();
        let date2: Date = new Date(this.serverTime.getFullYear(), this.serverTime.getMonth(), this.serverTime.getDate() + 1, 0, 0, 0, 0);
