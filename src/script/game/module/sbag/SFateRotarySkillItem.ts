...
export class SFateRotarySkillItem extends FUI_SFotruneHead {
    ...
    protected onConstruct() {
        super.onConstruct();
        this.costTxt.text = LangManager.Instance.GetTranslation("armyII.viewII.equip.JewelFrame.JewelCostTxt");
    ...
    public refreshRunCircleView() {
        if (this._firstSkill) {
            ...
            if (temp.SkillDescription) {
                this.buffDesc.text = this.formateColor(temp.SkillDescription, "#FFECC6");
                this.buffDesc.x = 28;
                this.buffDesc.y = 162;
            }
            else {
                this.buffDesc.text = LangManager.Instance.GetTranslation("faterotary.LevelMin");
                this.buffDesc.x = 28 + 30;
                this.buffDesc.y = 162 + 30;
            }
            this.levelLab.text = LangManager.Instance.GetTranslation("public.level3", this._firstSkill.grades);

            if (nextTemp) {

                let nextSkillTemp = TempleteManager.Instance.getSkillTemplateInfoBySonTypeAndGrade(temp.SonType, temp.Grades + 1);
                ...
                this.nextBuffDesc.text = this.formateColor(nextSkillTemp.SkillDescription, "#71F000");
                ...
                this.nextBuffDesc.text = LangManager.Instance.GetTranslation("faterotary.LevelMax");
            ...
            if (grade >= 15) {
                this.newSkillLight.text = LangManager.Instance.GetTranslation("faterotary.FateRotaryFrame.GetFateStoneMax");
            } else {
                let lightLevel = (grade < 5 ? "5" : grade < 10 ? "10" : "15");
                this.newSkillLight.text = LangManager.Instance.GetTranslation("faterotary.FateRotaryFrame.GetFateStone1", lightLevel);
            }
        }
    }
    ...
    private initEvent() {
        ...
        this._firstSkill.addEventListener(FateGuardEvent.FATE_GUARD_GP, this.__skillChangeHandler, this);
    }

    private __textChangeHandler() {
        let max = GoodsManager.Instance.getCountBySontypeAndBagType(GoodsSonType.SONTYPE_FATE_STONE, BagType.Player);
    ...
    private removeEvent() {
        ...
        this._firstSkill && this._firstSkill.removeEventListener(FateGuardEvent.FATE_GUARD_GP, this.__skillChangeHandler, this);
    }
