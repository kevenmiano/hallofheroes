...
export class BufferHandler {
    ...
    public static bufferAffectAction(buffer: BufferDamageData) {
        ...
        switch (buffer.AttackType) {
            ...
            case 12://魔攻.	
                break;
            case 6://HP.
                BattleUtils.addSingleDanny([BufferHandler.convertDamageDataToAttackData(damageData, buffer.target)])
                break;
            case 7://SP.
            case 9://受到物伤(降低/提高)
            case 10://受到魔伤(降低/提高)
     ...
     * 
     */
    private static bufferAffectActionII(bufferDataII: BufferDamageData, executerId: number, index: number = 0) {
     ...
     * 
     */
    private static addDanny(bufferDataII: BufferDamageData, executerId: number, index: number = 0) {
        ...
        if (bufferDataII.damages && bufferDataII.damages.length > index) {
            ...
            if (!damage) return;
            attackData = BufferHandler.convertDamageDataToAttackData(damage, bufferDataII.target);

            let battleModel = BattleManager.Instance.battleModel
            let role = battleModel.getRoleById(attackData.roleId);
            if (role) {
                Logger.battle(`[BufferHandler]对${role.roleName}(${attackData.roleId})产生buff数值${attackData.damageValue}<${bufferDataII.buffName}>`)
            }
            ...
            if (attackData.bloodType == BloodType.BLOOD_TYPE_SP) {
                if (executerId != attackData.roleId) {
                    ...
                    if (roleInfo && roleInfo.updateSp) {
                        roleInfo.updateSp(roleInfo.sp + attackData.damageValue, false, false, true);
                    }
                }
            } else {
                BattleUtils.addSingleDanny([attackData], 12, true, 0, 0, 6, false, 0x666666, false)
            }
        }
    }
    /**
    ...
    private static removeBufferII(bufferDataII: BufferDamageData) {
        if (BattleManager.Instance.battleMap.rolesDict && BattleManager.Instance.battleMap.rolesDict[bufferDataII.target]) {
            let roleView: BaseRoleView = BattleManager.Instance.battleMap.rolesDict[bufferDataII.target] as BaseRoleView;
            if (roleView) {
     ...
     * 
     */
    private static convertDamageDataToAttackData(damageData: DamageData, roleId: number): AttackData {
    ...
    public static processBuffer(buffer: BufferDamageData, executerId: number, index: number = 0) {
        if (Logger.openBattleBuffCon(buffer)) {
            Logger.battle("XXX[BaseRoleInfo]processBuffer 处理buff:", buffer.fRoleName, buffer.buffName, buffer.processType, index, executerId, buffer.id);
        }
        ...
        if (buffer.processType == BufferProcessType.REMOVE || buffer.currentTurn == 0) {
            ...
            if (roleInfo && Logger.openBattleBuffCon(buffer)) {
                Logger.battle(`XXX[BufferHandler]移除${roleInfo.roleName}(${roleInfo.livingId})的buff<${buffer.buffName}>`, buffer);
            }
            return;
        }
