...
export class ToolTipsManager {
    ...
    public register(tipedDisplay: ITipedDisplay | any) {
        ...
        switch (tipedDisplay.showType) {
            ...
            case TipsShowType.onLongPress:
                let orSp: Sprite = tipedDisplay instanceof fgui.GObject ? tipedDisplay.displayObject : tipedDisplay;
            ...
            case TipsShowType.onLongPress2:
                let orSp2: Sprite = tipedDisplay instanceof fgui.GObject ? tipedDisplay.displayObject : tipedDisplay;
    ...
    private parsTarget(event: Laya.Event, sp: Laya.Sprite = null): Sprite | fgui.GObject {
    ...
    private ontipedDisplayDown(event: Laya.Event, sp: Laya.Sprite = null) {
        ...
        target.iSDown = true;
        target.mouseDownPoint = new Laya.Point(Laya.stage.mouseX, Laya.stage.mouseY);
    }
    ...
    private ontipedDisplayMove(event: Laya.Event, sp: Laya.Sprite = null) {
        ...
        let distance = target.mouseDownPoint.distance(Laya.stage.mouseX, Laya.stage.mouseY);
    ...
    public async showTip(event: Laya.Event, sp: Laya.Sprite = null) {
        ...
        if (target.tipData) {
            ...
            let tip: BaseWindow = await UIManager.Instance.ShowWind(target.tipType, [target.tipData, target.canOperate, target.extData], false);
    ...
    public setTipPos(target, tip: BaseWindow) {
        if (target.tipData) {
            ...
            if (target.startPoint) {
                let tipContainer: Sprite = LayerMgr.Instance.getLayer(UIManager.Instance.getUIInfo(target.tipType).Layer);
                let orSp: Sprite = target instanceof fgui.GObject ? target.displayObject : target;
                let startPos: Laya.Point = tipContainer.globalToLocal(orSp.localToGlobal(target.startPoint, true), true);
                let resultDirection: DirectionPos = this.getTipPriorityDirction(tip, target, target.tipDirctions);
                Logger.yyz("获取到的最佳Tips显示方向: ", resultDirection);

                let resultPos: Laya.Point = this.getTipPosByDirction(tip, target, resultDirection.direction);
                ...
                if (tip.getContentPane().width <= Resolution.gameWidth) {
                    tip.getContentPane().x = (Resolution.gameWidth - tip.getContentPane().width) / 2;
                }
                else {
                ...
                if (tip.getContentPane().height <= Resolution.gameHeight) {
                    tip.getContentPane().y = (Resolution.gameHeight - tip.getContentPane().height) / 2;
                }
                else {
            ...
            Logger.yyz("Tips大小: ", tip.getContentPane().width, tip.getContentPane().height);
        } else {
    ...
    private getTipPriorityDirction(tip: BaseWindow, target: ITipedDisplay, directions: string = "7,5,4,6,2,1,3,0"): DirectionPos {
        ...
        for (let i: number = 0; i < dirs.length; i++) {
            let ordinaryPos: Laya.Point = this.getTipPosByDirction(tip, target, +dirs[i]);
            //tips左上角点坐标
            let resultStartPos: Laya.Point = new Laya.Point(ordinaryPos.x + startPos.x, ordinaryPos.y + startPos.y);
            //tips右下角点坐标
            let contentWidth: number = 0;
            let contentHeight: number = 0;
            if (tip && tip.getContentPane()) { contentWidth = tip.getContentPane().width; contentHeight = tip.getContentPane().height; }
            let resultEndPos: Laya.Point = new Laya.Point(ordinaryPos.x + startPos.x + contentWidth, ordinaryPos.y + startPos.y + contentHeight);
            let directionPos: DirectionPos = this.creatDirectionPosII(resultStartPos, resultEndPos, parseInt(dirs[i]));
    ...
    private searchFixedDirectionPos(tempDirections: DirectionPos[]): DirectionPos {
        ...
        for (let i: number = 0; i < reverDirections.length; i++) {
            if (result == null) {
                ...
                let current: number = Math.abs(reverDirections[i].offsetX) + Math.abs(reverDirections[i].offsetY);
    ...
    private creatDirectionPosII(startPos: Laya.Point, endPos: Laya.Point, direction: number): DirectionPos {
        let directionPos: DirectionPos = new DirectionPos();
        directionPos.direction = direction;
     ...
     *
     */
    public getTipPosByDirction(tip: BaseWindow, target: ITipedDisplay, direction: number): Laya.Point {
    ...
    public unRegister(tipedDisplay: ITipedDisplay | any, hideTips: boolean = true) {
        if (!tipedDisplay || tipedDisplay.isDisposed || tipedDisplay.destroyed) {
            return;
        }
        this.removeEvent(tipedDisplay);
    ...
    public setMountActiveTxt(info: GoodsInfo, bindTxt: fgui.GTextField) {
        if (info && info.templateInfo && info.templateInfo.SonType == GoodsSonType.SONTYPE_MOUNT_CARD) {
            let mountTemplateId: number = info.templateInfo.Property1;
            let isActive: boolean = MountsManager.Instance.avatarList.isLightTemplate(mountTemplateId)
            if (isActive) {//激活的
                let wildSoulInfo: WildSoulInfo = MountsManager.Instance.avatarList.getWildSoulInfo(mountTemplateId)
                let maxStarGrade: number = MountsManager.Instance.maxStarGrade
                if (wildSoulInfo && wildSoulInfo.starLevel >= maxStarGrade) {//炼化到最大星级
                    bindTxt.text = LangManager.Instance.GetTranslation("yishi.view.tips.goods.EquipTipsContent.mountTips3");
                } else {
                    //时效坐骑不显示炼化
                    bindTxt.text = wildSoulInfo.template.StarItem ? LangManager.Instance.GetTranslation("yishi.view.tips.goods.EquipTipsContent.mountTips2") : "";
                }
            } else {
                bindTxt.text = LangManager.Instance.GetTranslation("yishi.view.tips.goods.EquipTipsContent.mountTips1");
            }
