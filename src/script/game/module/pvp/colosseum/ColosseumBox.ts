...
* @description 活跃度宝箱 
*/
export default class ColosseumBox extends FUI_ColosseumBox implements ITipedDisplay {
    ...
    public setInfo(value: t_s_singlearenarewardsData, lastWinCount: number, winCountReward: string) {
        this._boxdata = value;
        this.iconBox.url = FUIHelper.getItemURL(EmPackName.Pvp, "Icon_Box_Dev" + (value.Id - 2000));
    ...
    getState(lastWinCount: number, winCountReward: string) {
        if (this._boxdata.Property1 > lastWinCount) {
            ...
            return winCountReward.indexOf(this._boxdata.Id + '') == -1 ? ColosseumBox.OPEN : ColosseumBox.CLOSE;
        }
    }
    ...
    onBoxClick(evt: Laya.Event) {
        ...
        if (this._state == ColosseumBox.OPEN) {//领取宝箱奖励
    ...
    private getTipsStr(data: t_s_singlearenarewardsData): string {
        ...
        if (data.RewardItemID1) {
            str += this.getGoodsName(data.RewardItemID1) + ' x' + data.RewardItemCount1;
        }
        if (data.RewardItemID2) {
            str += '<br>' + this.getGoodsName(data.RewardItemID2) + ' x' + data.RewardItemCount2;
        }
        return str;
    }
