...
*/
export default class HoodRoomArmyView extends CampaignArmyView implements IBaseMouseEvent {
    ...
    public mouseOutHandler(evt: Laya.Event): boolean {
        if (!this.isSelf) {
            if (this._avatar && (this._avatar as HeroAvatar).getCurrentPixels() > 50) {
    ...
    public dispose() {
        if (this._avatar && this._filter) this._filter.setNormalFilter(this._avatar); this._filter = null;
        super.dispose();
    }

    public mouseClickHandler(evt: Laya.Event): boolean {
        if (this._avatar && (this._avatar as HeroAvatar).getCurrentPixels() < 50 || this.isSelf) {
            if (evt) {
                this.throughDispatch(evt);
            }
            return false;
