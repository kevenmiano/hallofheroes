...
export class ChatManager {
  ...
  public setup() {
    ...
    ServerDataManager.listen(
      S2CProtocol.U_C_CROSS_BIGBUGLE,
      this,
      this.__crossBugleHandler,
    );
    ServerDataManager.listen(
      S2CProtocol.U_CH_CHANNEL_CHA,
      this,
      this.__channelChatHandler,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_SMALL_BUGLE_FREE_COUNT,
      this,
      this.__samllBugleFreeCountHandler,
    );
    NotificationManager.Instance.addEventListener(
      ChatEvent.ADD_CHAT,
      this.__addChatHandler,
      this,
    );
  ...
  private __channelChatHandler(pkg: PackageIn) {
    if (!ChatManager.Instance.model) return;
    let chatExtend1: number = ChatManager.Instance.model.getProtocolTimeByID(
      pkg.extend1,
    );
    var interval: number = new Date().getTime() - chatExtend1 ? chatExtend1 : 0;
    if (interval >= 3) {
      Logger.warn(
        "SLGSocketEvent.EVENT+AcceptProtocolType.U_CH_CHANNEL_CHA协议超时 3 秒以上",
      );
    }
    ...
    if (msg.otherId == this.playerInfo.userId) {
      Logger.warn(
        "收到聊天协议    " + new Date().getTime() + this.analyzeExpression(msg),
      );
    }
    if (
    ...
    if (chatData.channel == ChatChannel.NOTICE) {
      if (chatData.bigBugleType >= 1 && chatData.bigBugleType <= 4) {
        //策划把系统公告显示从原来大喇叭里面分离出来
      ...
      chatData.senderName = LangManager.Instance.GetTranslation(
        "chat.datas.getChatChannelName.ALERT",
      );
      chatData.userType = UserType.NORMAL;
      this._model.addBigBugleData(chatData);
  ...
  public analyzeExpressionAfterNormal(msg: string): string {
    var str: string = msg.replace(
      EMOJI_REG_AFTER_NORMAL,
      "<a t='9' index='$2' name='$2'/>",
    );
    return this.filterSpecialChar(str);
  }
  ...
  private filterSpecialChar(str: string): string {
    //过滤特殊连字符为普通连字符, FTE解析此特殊连字符会引起flash player崩溃, 此字符实体名为&shy;
