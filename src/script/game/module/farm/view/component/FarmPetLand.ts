...
export class FarmPetLand extends FUI_FarmPetLand {
    ...
    if (this.isSelfFarm()) {
      if (!this._info) {
        if (FarmModel.getPetData().length > 0) {
          FrameCtrlManager.Instance.open(EmWindow.FarmPetSelect, {
            pos: this.pos,
          });
        } else {
          MessageTipManager.Instance.show(
            LangManager.Instance.GetTranslation("FarmPetSelectMeun.noPet01"),
          );
        }
  ...
  public update() {
    ...
    let sec =
      this._info.remainMatureTime > 43200 ? 43200 : this._info.remainMatureTime;
  ...
  private changeButtonMode() {
    ...
    let isOpen: boolean =
      PlayerManager.Instance.currentPlayerModel.petSystemIsOpened;
  ...
  private changeState() {
    if (this._info) {
      ...
      if (this.isSelfFarm()) {
        this.cState.selectedIndex = PlayerManager.Instance.currentPlayerModel
          .petSystemIsOpened
          ? 1
          : 0;
      } else {
  ...
  private isSelfFarm(): boolean {
    return (
      ...
      FarmManager.Instance.model.myFarm
    );
  }
