...
* @description *** 
*/
export default class ChatMsgCell extends FUI_ChatMsgCell implements ITipedDisplay {
    ...
    private onTapTranslateMsg() {
        ...
        if (this._cellData.translateMsg.trim() && this._cellData.translateLangKey == PlayerManager.Instance.currentPlayerModel.playerInfo.chatTranslateKey) return;
    ...
    public set chatData(value: ChatData) {
        ...
        if (value.serverName && value.serverName != "")//跨服判断
            isSameServer = (value.serverName == PlayerManager.Instance.currentPlayerModel.playerInfo.serviceName);

        let isSelf = (value.uid == PlayerManager.Instance.currentPlayerModel.userInfo.userId && isSameServer);
        ...
        if (this._cellData.frameId > 0) {
            let itemData: t_s_itemtemplateData = TempleteManager.Instance.getGoodsTemplatesByTempleteId(this._cellData.frameId);
            if (itemData) {
