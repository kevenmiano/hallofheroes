...
export class BattleSkillItemII {
    ...
    protected initView() {
        ...
        this._imgPetSkillBan = this._item.getChild("Img_PetSkillBan") as fgui.GImage;
        ...
        this._remoteSkillCount = this.view.getChild('remoteSkillCount') as fgui.GLabel;
        this._remoteSkillBg = this.view.getChild('remoteSkillBg') as fgui.GLabel;
    }

    private initEvent() {
        this.view.onClick(this, this.__iconClick)
        NotificationManager.Instance.addEventListener(BattleNotic.BUFFER_SKILL_ENABLE, this.__bufferEnabeleHandler, this);
        Laya.stage.on(Laya.Event.KEY_DOWN, this, this.onKeyDownHandler);
    }

    protected removeEvent() {
        this.view.offClick(this, this.__iconClick)
        NotificationManager.Instance.removeEventListener(BattleNotic.BUFFER_SKILL_ENABLE, this.__bufferEnabeleHandler, this);
    ...
    public onKeyDownHandler(event: Laya.Event) {
        ...
        if(this.battleModel.isAllPetPKBattle()){
            if (this._numCount == 18 ) {
                ...
                if (((event.keyCode == this._numCount + 48) || (event.keyCode == this._numCount + 96))) {
                    this.__iconClick(null);
                }
            }
        }else{
    ...
    protected __bufferEnabeleHandler() {
        ...
        if (this._data instanceof t_s_skilltemplateData) {
            if(!this.battleModel.isAllPetPKBattle()){
                this.bufferUnEnable = this.battleModel.selfHero.existUnenableSkillBuffer((<t_s_skilltemplateData>this._data).TemplateId);
                if (this.bufferUnEnable) {
                    ...
                    this._imgBan.visible = false
                    let isEnough: boolean = this.battleModel.selfHero.sp + this._data.Cost >= 0;
                    let hasReadySkill: boolean = this.battleModel.currentReadySkillId != -1;
                    let inTrialTowerBattle: boolean = this.battleModel.battleType == BattleType.TRIAL_TOWER_BATTLE;
    ...
    protected __iconClick(evt: any) {
        ...
        if (this.battleModel.selfHero.useSkill700001 || this.battleModel.selfHero.useSkill700002) {
            Logger.battle("技能不可用 正在变身/取消变身", this.battleModel.selfHero.useSkill700001, this.battleModel.selfHero.useSkill700002)
            return
        }
    ...
    public set data(value: any) {
        ...
        if (this._data) {
            ...
            if (this._imgBattleS) {
                this._imgBattleS.visible = SkillPriorityType.isSuperSkill(this._data.Priority);
            }
    ...
    public setState(state: number) {
        ...
        this.showShortKey(true)
        if (this.index == SkillItemListView.TalentIndex && this.battleModel.isLockTalent) {
    ...
    protected addCoolDownInfo(cd: number, coolType: number = 0, delay: number = 0) {
        if (this._data instanceof t_s_skilltemplateData && cd > 0) {
            ...
            Logger.battle("[BattleSkillItemII]addCoolDownInfo", cdMode.templateId, cdMode.cooldown, cdMode)
            Laya.timer.once(delay, this, () => {
                cdMode.start();
            })
        }
    ...
    protected startCDProcess(time: number) {
        ...
        Logger.battle("[BattleSkillItemII]初始进度", time, rate, this._data.CoolDown, this._data.AppearCD, this._cdAttr["prog"])
    ...
    public startAppearCD(delay: number = 0, cd: number = -1) {
        if (!this._data || !this._data.hasOwnProperty("AppearCD")) { return }
    ...
    public set enabled(value: boolean) {
        this._enabled = value
        this.setState(value ? BattleSkillItemII.STATE_NORMAL : BattleSkillItemII.STATE_DARK)
    }
    public get enabled() {
