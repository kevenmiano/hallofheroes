...
export default class SpacePKAlert extends BaseWindow {
  ...
  protected OnBtnClose() {
    this.params &&
      ...
      this.params.callback(false, this.checked, this.params.data);
    if (this.params.autoClose) this.hide();
  }
  ...
  confirmBtnClick() {
    this.params &&
      this.params.callback &&
      this.params.callback(true, this.checked, this.params.data);
    if (this.params.autoClose) this.hide();
  }
