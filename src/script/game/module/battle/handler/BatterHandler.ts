...
export class BatterHandler {
  ...
  private addEvent() {
    NotificationManager.Instance.addEventListener(
      ...
      this.__setBatterCount.bind(this),
      this,
    );
    NotificationManager.Instance.addEventListener(
      BattleNotic.SET_TOTAL_DAMAGE,
      this.__setTotalDamage.bind(this),
      this,
    );
  }
  ...
  public hide() {
    if (this._batterAsset)
      ...
      TweenLite.to(this._batterAsset, 0.4, {
        alpha: 0,
        x: this._batterDestX + BatterHandler.EXIT_DISTANCE,
        onComplete: this.onExitComplete.bind(this),
      });
  ...
  public dispose() {
    ...
    ObjectUtils.disposeViolentMc(this.batterNum);
    this.batterNum = null;
    ObjectUtils.disposeViolentMc(this.damageNum);
    this.damageNum = null;
    ObjectUtils.disposeViolentMc(this._batterAsset);
    this._batterAsset = null;
    // ObjectUtils.disposeViolentMc(this._damageAsset); this._damageAsset = null;
    if (this._shake) this._shake.stop();
    this._shake = null;
  }
