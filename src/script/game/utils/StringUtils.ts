...
export default class StringUtils {
    ...
    public static BASE64: string = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/';
     ...
     *       @tiptext
     */
    public static between(p_string: string, p_start: string, p_end: string): string {
        let str: string = '';
        if (p_string == null) {
            return str;
        }
     ...
     *       @tiptext
     */
    public static block(p_string: string, p_len: number, p_delim: string = "."): Array<string> {
    ...
    public static ljust(p_string: string, p_width: number, p_pad: string = ' '): string {
    ...
    public static rjust(p_string: string, p_width: number, p_pad: string = ' '): string {
        let pad: string = p_pad.substr(0, 1);
        if (p_string.length < p_width) {
    ...
    public static center(p_string: string, p_width: number, p_pad: string = ' '): string {
        let pad: string = p_pad.substr(0, 1);
    ...
    public static base64Encode(p_string: string): string {
        ...
        while (i < len) {
            ...
            if (i == len) {
                out += StringUtils.BASE64.charAt(c1 >> 2)
     ...
     *       @tiptext
     */
    public static countOf(p_string: string, p_char: string, p_caseSensitive: boolean = true): number {
        if (p_string == null) {
            return 0;
        }
     ...
     *       @tiptext
     */
    public static padRight(p_string: string, p_padChar: string, p_length: number): string {
        let s: string = p_string;
        while (s.length < p_length) {
            s += p_padChar;
        }
    ...
    public static properCase(p_string: string): string {
        ...
        let str: string = p_string.toLowerCase().replace(/\b([^.?;!]+)/, StringUtils.capitalize);
    ...
    public static relativePath(p_base: string, p_path: string, p_delim: string = '/'): string {
     ...
     *       @tiptext
     */
    public static remove(p_string: string, p_remove: string, p_caseSensitive: boolean = true): string {
        if (p_string == null) {
            return '';
        }
     ...
     *       @tiptext
     */
    public static truncate(p_string: string, p_len: number, p_suffix: string = "..."): string {
        if (p_string == null) {
            return '';
        }
        if (p_len == 0) {
    ...
    public static consortiaTxtStyle(str: string, symbol: string = ","): string {
        ...
        if (str.length >= 16) {
            ...
            rls = str.substring(0, len - 9) + symbol + str.substring(len - 9, len - 6) + symbol + str.substring(len - 6, len - 3) + symbol + str.substring(len - 3, len);
        } else if (str.length >= 7) {
            rls = str.substring(0, len - 6) + symbol + str.substring(len - 6, len - 3) + symbol + str.substring(len - 3, len);
    ...
    public static getRomanNumber(num: number): string {
        let romanNum: string[] = ["", "Ⅰ", "Ⅱ", "Ⅲ", "Ⅳ", "Ⅴ", "Ⅵ", "Ⅶ", "Ⅷ", "Ⅸ", "Ⅹ"];
        return romanNum[num];
    }
}