...
import HintUtils, { HintType, HintViewData, HintAniData, FontType } from "./HintUtils";
...
export default class HintUtilsHelper {
    ...
    static refreshView(comp: fgui.GComponent, hintType: HintType, content: string, viewData: string | HintViewData = '', aniData: HintAniData) {
        if (aniData.point) {
        ...
        if (aniData.type == FontType.NONE) {
            ...
            let bmFontCfg = aniData.bmFontCfg ? aniData.bmFontCfg : HintUtils.BMFontCfgMap.get(cfgType);
    ...
    static showAction(target: fgui.GComponent, hintType: HintType, viewData: string | HintViewData = '', aniData?: HintAniData, callback?: Function) {
