...
export class OuterCityMouseEventMediator implements IMediator {
  ...
  private __mouseMoveHandler(evt: Laya.Event): void {
    ...
    if (
      CursorManagerII.Instance.currentState == CursorManagerII.PVP_ATTACT_CURSOR
    ) {
      return;
    }
      ...
      if (mc) {
        if (mc["parent"] == this.mapView.mainBuidingLayer) {
          if (mc instanceof MapPhysicsCastle) {
            ...
            CursorManagerII.Instance.currentState != CursorManagerII.SELL_CIRSOR
          ) {
  ...
  private resetCursor(): void {
    ...
    if (
      cursorState != CursorManagerII.DRAG_CURSOR &&
      cursorState != CursorManagerII.SELL_CIRSOR
    ) {
      if (!TransmitHandler.Instance.mouseCursor) {
