...
export class SkillItemTips extends BaseTips {
    ...
    setSelectSkillInfo(data: SkillInfo) {
        ...
        this.selectSkillItem.icon = IconFactory.getTecIconByIcon(data.templateInfo.Icons);
        ...
        this.bePassiveSkillLabel.text = this.bePassiveSkillLabel2.text = this.getSkillType(info.templateInfo);
        let ismax: boolean;
        ...
        if (info.grade == 0) {//未激活
            ...
            if (nextTemp.CoolDown > 0) {
                this.skillCold.text = LangManager.Instance.GetTranslation("yishi.view.tips.goods.SkillTips.cooldown01", nextTemp.CoolDown * 0.001);
            }
            if (nextTemp.Cost < 0) {
                var num: number = Math.abs(nextTemp.Cost);
                this.skillCost.text = this.getCoststring(num);
            }
            let skillEffectText: string = StringUtils.stringFormat2(nextTemp.SkillDescription, { key: "Parameter1", value: Math.abs(nextTemp.Parameter1) }, { key: "Parameter2", value: Math.abs(nextTemp.Parameter2) }, { key: "Parameter3", value: Math.abs(nextTemp.Parameter3) }, { key: "Cost", value: Math.abs(nextTemp.Cost) })
            ...
            if (this.isMasterySkill) {
                if (this.checkCanStudy()) {
                    ...
                    let p1 = LangManager.Instance.GetTranslation('Mastery.jobtype' + jobtype)
                    let str = LangManager.Instance.GetTranslation('Mastery.sutdyCondition', p1, this.selectSkillData.templateInfo.NeedPlayerGrade);
            ...
            this.txt_lv1.text = LangManager.Instance.GetTranslation("public.level2", curTemp.Grades);
            this.txt_lv2.text = LangManager.Instance.GetTranslation("public.level2", curTemp.Grades + 1);
            this.nextBox.visible = true;
            this.skillName2.text = curTemp.TemplateNameLang;
            this.skillLevel.text = curTemp.Grades > 0 ? LangManager.Instance.GetTranslation("public.level2", curTemp.Grades) : '';
            if (curTemp.CoolDown > 0) {
                this.skillCold.text = LangManager.Instance.GetTranslation("yishi.view.tips.goods.SkillTips.cooldown01", curTemp.CoolDown * 0.001);
            }
            if (curTemp.Cost < 0) {
                num = Math.abs(curTemp.Cost);
                this.skillCost.text = this.getCoststring(num);
            }
            let skillEffect: string = StringUtils.stringFormat2(curTemp.SkillDescription, { key: "Parameter1", value: Math.abs(curTemp.Parameter1) }, { key: "Parameter2", value: Math.abs(curTemp.Parameter2) }, { key: "Parameter3", value: Math.abs(curTemp.Parameter3) }, { key: "Cost", value: Math.abs(curTemp.Cost) })
            this.skillEffectRichText.text = skillEffect;
            if (nextTemp.CoolDown > 0) {
                this.nextSkillCost.text = LangManager.Instance.GetTranslation("yishi.view.tips.goods.SkillTips.cooldown01", nextTemp.CoolDown * 0.001);
            }
            if (nextTemp.Cost < 0) {
                this.skillCost2.text = this.getCoststring(nextTemp.Cost);
            }
            let nextEffect: string = StringUtils.stringFormat2(nextTemp.SkillDescription, { key: "Parameter1", value: Math.abs(nextTemp.Parameter1) }, { key: "Parameter2", value: Math.abs(nextTemp.Parameter2) }, { key: "Parameter3", value: Math.abs(nextTemp.Parameter3) }, { key: "Cost", value: Math.abs(nextTemp.Cost) })
            ...
            if (this.isMasterySkill) {
                if (this.checkCanAddExtrajob(this.selectSkillData)) {
                    ...
                    let jobtype = this.selectSkillData.templateInfo.MasterType;
                    let p1 = LangManager.Instance.GetTranslation('Mastery.jobtype' + jobtype)
                    let str = LangManager.Instance.GetTranslation('Mastery.sutdyCondition', p1, this.selectSkillData.nextTemplateInfo.NeedPlayerGrade);
                    this.txt_condition.text = str;
                    this.txt_condition.visible = true;
                }
                this.list.visible = false;
            ...
            this.skillLevel.text = curTemp.Grades > 0 ? LangManager.Instance.GetTranslation("public.level2", curTemp.Grades) : '';
            if (curTemp.CoolDown > 0) {
                this.skillCold.text = LangManager.Instance.GetTranslation("yishi.view.tips.goods.SkillTips.cooldown01", Math.ceil(curTemp.CoolDown * 0.001));
            }
            if (curTemp.Cost < 0) {
            ...
            let skillEffect: string = StringUtils.stringFormat2(curTemp.SkillDescription, { key: "Parameter1", value: Math.abs(curTemp.Parameter1) }, { key: "Parameter2", value: Math.abs(curTemp.Parameter2) }, { key: "Parameter3", value: Math.abs(curTemp.Parameter3) }, { key: "Cost", value: Math.abs(curTemp.Cost) })
            this.skillEffectRichText.text = skillEffect;
            this.nextBox.visible = this.opreateGroup.visible = this.costGold.visible = this.studyBox.visible = false;
    ...
    public checkBtnCondition() {
        ...
        if (this.isMasterySkill) {
            ...
            this.txt_gold.text = selectSkillData.nextTemplateInfo.Parameter3.toString();
            isgoldEnough = ResourceManager.Instance.gold.count >= selectSkillData.templateInfo.Parameter3;
            if (!isgoldEnough) {
        ...
        if (_isOpen) {
            ...
            if (this.isMasterySkill) {
                ...
                else if (jobLevel >= this.selectSkillData.templateInfo.NeedPlayerGrade) {
    ...
    checkCanStudy(): boolean {
        let jobtype = this.selectSkillData.templateInfo.MasterType;
        let jobLevel = ExtraJobModel.instance.getExtrajobItemLevel(jobtype);
        if (jobLevel <= 0 || jobLevel < this.selectSkillData.templateInfo.NeedPlayerGrade) {//升级所需对应秘典等级读取字段NeedPlayerGrade
            return false;
        }
        return true;
    }
    ...
    private checkCanAdd(selectSkillData: SkillInfo): boolean {
        ...
        if (selectSkillData.checkUpgradeCondition(selectSkillData.nextTemplateInfo).length > 0) return false;
        return true
    }
    ...
    onRender(index: number, item: FUI_SkillItemCom) {
        if (item) {
            ...
            if (itemData instanceof t_s_skilltemplateData) {
                ...
                let skillInfo;
                if (this.isMasterySkill) {
                    skillInfo = this.thane.skillCate.getExtrajobSkillInfoBySkillTempId(itemData.TemplateId);
                } else {
                    skillInfo = this.thane.skillCate.getSkillInfoBySkillTempId(itemData.TemplateId);
                }
                if (skillInfo) {
    ...
    _onUpgradeSkill() {
        ...
        Logger.log('升级技能');
        if (this.isMasterySkill) {
            this.control.sendAddSkillPoint(this.selectSkillData, 2, this.selectSkillData.templateInfo.MasterType);
        } else {
