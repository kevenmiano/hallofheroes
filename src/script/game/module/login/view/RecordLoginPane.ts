...
export default class RecordLoginPane extends FUI_RecordList {
  ...
  addEvent() {
    ...
    this.allAccountList.itemRenderer = Laya.Handler.create(
      this,
      this.renderListItem,
      null,
      false,
    );
    this.allAccountList.on(
  ...
  onClickItem() {
    ...
    if (this.lastLoginItem && (this.lastLoginItem as AccountItem).serverInfo) {
      NotificationManager.Instance.dispatchEvent(
        ...
        (this.lastLoginItem as AccountItem).serverInfo,
      );
    }
  }
  ...
  onServerListSelect(selectedItem: AccountItem) {
    if (this.lastLoginItem) this.lastLoginItem.selected = false;
