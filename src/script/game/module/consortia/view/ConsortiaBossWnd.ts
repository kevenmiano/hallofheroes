...
export default class ConsortiaBossWnd extends BaseWindow {
  ...
  public OnInitWind() {
    ...
    this.infoTxt.text = LangManager.Instance.GetTranslation(
      "ConsortiaBossWnd.infoTxt",
    );
    this.callBossBtn.title = LangManager.Instance.GetTranslation(
      "ConsortiaBossWnd.callBossBtnTxt",
    );
    var index: number = 0;
    var tempArr: Array<t_s_upgradetemplateData> =
      TempleteManager.Instance.getTemplatesByType(42);
  ...
  private initView() {
    let bosstemp: t_s_consortiabossData =
      TempleteManager.Instance.getConsortiaBossRewardByLevel(1);
    if (!bosstemp) return;
