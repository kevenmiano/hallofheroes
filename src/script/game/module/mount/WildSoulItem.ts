...
export default class WildSoulItem extends FUI_WildSoulItem {
  ...
  private activeBtnHandler(event: Laya.Event) {
    ...
    if (this._vData && this.isLock) {
      if (this._vData.NeedMountGrade > this.curMountInfo.grade) {
        MessageTipManager.Instance.show(
          LangManager.Instance.GetTranslation(
            "mounts.UseGoodNeedGrade",
            this._vData.NeedMountGrade,
          ),
        );
        return;
      }
      ...
      let type: number = 0;
      let value: number = 0;
      let goodTemplate: t_s_itemtemplateData =
        ConfigMgr.Instance.getTemplateByID(
          ConfigType.t_s_itemtemplate,
          this._vData.NeedItemId.toString(),
        );
      if (goodTemplate) {
        //去背包中找解锁的物品
        let find: Array<GoodsInfo> =
          GoodsManager.Instance.getBagGoodsByTemplateId(
            goodTemplate.TemplateId,
          );
        if (find && find.length > 0) {
          //有则提示使用物品
          let validity: string;
          if (goodTemplate.Property2 > 0) {
            validity = LangManager.Instance.GetTranslation(
              "mounts.WildsoulItem.tips.Day",
              goodTemplate.Property2,
            );
          } else {
            validity = LangManager.Instance.GetTranslation(
              "mounts.WildsoulItem.tips.Long",
            );
          }
          content = LangManager.Instance.GetTranslation(
            "mounts.command10",
            goodTemplate.TemplateNameLang,
            validity,
          );
          type = 1;
          value = 1;
      ...
      if (type == 2) {
        SimpleAlertHelper.Instance.Show(
          ...
          {
            templateId: this._vData.TemplateId,
            paytype: type,
            count: value,
            point: this._vData.Property1,
          },
          prompt,
          content,
          confirm,
          cancel,
          this.__resultHandler.bind(this),
        );
      } else {
        SimpleAlertHelper.Instance.Show(
          SimpleAlertHelper.SIMPLE_ALERT,
          { templateId: this._vData.TemplateId, paytype: type, count: value },
          prompt,
          content,
          confirm,
          cancel,
  ...
  private changeBtnClick(event: Laya.Event) {
    ...
    if (ArmyManager.Instance.thane.changeShapeId > 0) {
      MessageTipManager.Instance.show(
        LangManager.Instance.GetTranslation(
          "mounts.WildSoulFrame.CannotChangeMount",
        ),
      );
      return;
    }
    if (this.vData) {
      ...
      if (this._isLock) {
        MessageTipManager.Instance.show(
          LangManager.Instance.GetTranslation("WildSoulItem.tips"),
        );
        return;
      }
      if (
        SpaceManager.Instance.model &&
        !SpaceManager.Instance.model.canLand(templateId)
      ) {
        MessageTipManager.Instance.show(
          LangManager.Instance.GetTranslation("map.space.CannotLand"),
        );
        return;
      }
      if (
        SwitchPageHelp.isInCampaign &&
        PlayerManager.Instance.currentPlayerModel.isOnObstacle
      ) {
  ...
  private lianHuaBtnHandler(event: Laya.Event) {
    ...
    if (!this.wildSoulCollection.isLightTemplate(this._vData.TemplateId)) {
      MessageTipManager.Instance.show(
        LangManager.Instance.GetTranslation("WildSoulItem.lianHua.tips"),
      );
      return;
    }
    FrameCtrlManager.Instance.open(EmWindow.MountRefiningWnd, {
      info: this._vData,
    });
  }
    ...
    if (this._vData) {
      ...
      if (this._vData.MountType == MountType.NORMAL) {
        this.baseItem.nameTxt.text =
          this._vData.TemplateNameLang +
          " " +
          LangManager.Instance.GetTranslation(
            "mounts.command01",
            this._vData.Property2,
          );
      ...
      let info: WildSoulInfo = this.wildSoulCollection.getWildSoulInfo(
      ...
      this.tipData = LangManager.Instance.GetTranslation(
        "mounts.WildsoulItem.tipsNoOpen",
      );
    }
  }
  ...
  private get isVIPMount(): boolean {
    if (this.vData) {
      let isVIPMount = VIPManager.Instance.model.isVipMount(
        this.vData.TemplateId,
      );
      return isVIPMount;
    }
    return false;
  ...
  private addWeakBreatheAction() {
    ...
    Laya.Tween.to(
      this._mountMovieClip,
      { scaleX: this._scaleNumber + 0.01, scaleY: this._scaleNumber + 0.04 },
      800,
      ...
      Laya.Handler.create(this, () => {
        ...
        Laya.Tween.to(
          this._mountMovieClip,
          { scaleX: this._scaleNumber, scaleY: this._scaleNumber },
          800,
          Laya.Ease.linearInOut,
          Laya.Handler.create(this, () => {
            this.addWeakBreatheAction();
  ...
  private lock() {
    ...
    if (this.wildSoulCollection.canActive(this._vData)) {
      ...
      //@ts-expect-error: activeBtn._titleObject not recognized
  ...
  private getLianHuaEnable(): boolean {
    ...
    let flag1: boolean =
      GoodsManager.Instance.getGoodsNumByTempId(this._vData.StarItem) > 0
        ? true
        : false;
