...
export class TailaFbManager {
    ...
    private initEvent() {
        ServerDataManager.listen(S2CProtocol.U_BATTLE_CONTROL_BUFF_STATE, this, this.__bossControlHandle);
    }

    private __bossControlHandle(pkg:PackageIn) {
        let msg: ControlBuffStateMsg = pkg.readBody(ControlBuffStateMsg) as ControlBuffStateMsg;
        BattleManager.Instance.battleModel.bossControlLivingId = (msg.controled ? msg.livingId : 0);
        if (BattleManager.Instance.battleModel.selfHero.livingId == msg.livingId) {
            NotificationManager.Instance.sendNotification(BattleNotic.SKILL_ENABLE, !msg.controled);
        }
        BattleManager.Instance.battleModel.getRoleById(msg.livingId).side = msg.side;
        this.runTo(msg.livingId, msg.side, msg.pos);
    }

    private  runTo(livingId: number, side: number, pos: number) {
        var role: BaseRoleInfo = BattleManager.Instance.battleModel.getRoleById(livingId);
        role.pos = pos;
        role.face = ((role.side == BattleManager.Instance.battleModel.selfSide) ? FaceType.LEFT_TEAM : FaceType.RIGHT_TEAM);
        var temp: Laya.Point = BattleUtils.rolePointByPos(role.pos, role.face);
        new MovePointAction(role.livingId, temp, 25, 0, false, false, true, false, function () {
            BattleManager.Instance.battleModel.getRoleById(livingId).side = side;
        }, null, true, null, 100, true);
    }
}