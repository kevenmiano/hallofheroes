...
export class CampaignMapCursorMediator implements IMediator {
	...
	private __mouseOutHandler(evt: Laya.Event) {
		...
		if (DragManager.Instance.isDraging) return;
		this._mouseDown = false;
		if (CursorManagerII.Instance.currentState != CursorManagerII.SELL_CIRSOR) CursorManagerII.Instance.resetCursor();
		this.startDragCall();
	}
	...
	private refreshCursor(evt: Laya.Event, target: Object) {
		if (CursorManagerII.Instance.currentState == CursorManagerII.DRAG_CURSOR) return;
		var pixel: number = 0;
		...
		if (evt.target instanceof CampaignNpcPhysics) {
			...
			if (CampaignArmyViewHelper.checkCollectionNode(nodeInfo)) {
				...
				if (mapModel && WorldBossHelper.checkPvp(mapModel.mapId)) {
					if(node.isMouseInHandleRangle(evt)){
						if (CursorManagerII.Instance.currentState != CursorManagerII.COLLECT_CURSOR) {
							CursorManagerII.Instance.showCursorByType(CursorManagerII.COLLECT_CURSOR);
						}
					}else{
			...
			if (CampaignArmyViewHelper.checkTreeNode(nodeInfo)) {
				switch (ConsortiaManager.Instance.model.secretInfo.treeState) {
					case ConsortiaSecretInfo.GIVE_POWER_STATE:
						if (CursorManagerII.Instance.currentState != CursorManagerII.WATER_CURSOR) {
							CursorManagerII.Instance.showCursorByType(CursorManagerII.WATER_CURSOR);
						}
						return;
				}
			}
			if (CampaignArmyViewHelper.checkAttackNode(nodeInfo)) {
				if (WorldBossHelper.checkGvg(CampaignManager.Instance.mapModel.mapId) &&
					CampaignManager.Instance.mapModel.selfMemberData.teamId != nodeInfo.param1) {
					if (CursorManagerII.Instance.currentState != CursorManagerII.ATTACK_CURSOR) {
						CursorManagerII.Instance.showCursorByType(CursorManagerII.ATTACK_CURSOR);
					}
					return;
			...
			if (npc.avatarView instanceof HeroAvatar) {
				...
				if (pixel > 50) {
					...
					if (CampaignArmyViewHelper.checkCollectionNode(nodeInfo)) {
						...
						if (CursorManagerII.Instance.currentState != CursorManagerII.SPACE_NPC_CURSOR) {
							CursorManagerII.Instance.showCursorByType(CursorManagerII.SPACE_NPC_CURSOR);
						}
						return;
					}
				}
			...
			if (armyView.avatarView instanceof HeroAvatar) {
				...
				if (pixel > 50) {
					if (CursorManagerII.Instance.currentState != CursorManagerII.SPACE_PLAYER_CURSOR) {
						CursorManagerII.Instance.showCursorByType(CursorManagerII.SPACE_PLAYER_CURSOR);
					}
					return;
				}
			}
			...
			if (armyView1.avatarView instanceof HeroAvatar) {
				...
				if (pixel > 50) {
					if (!CampaignArmyState.checkDied(armyView1.data.isDie) && armyView1.data.state != ArmyState.STATE_FIGHT) {
						if (this.isSelfConsortia(armyView1.data) || this.isSelfTeam(armyView1.data)) {
							CursorManagerII.Instance.showCursorByType(CursorManagerII.SPACE_PLAYER_CURSOR);
						} else {
							CursorManagerII.Instance.showCursorByType(CursorManagerII.ATTACK_CURSOR);
						}
	...
	private isSelfConsortia(army: CampaignArmy): boolean {
		if (army && army.baseHero) {
			...
			if (this.mapModel.isCross) {
				...
				return army.baseHero.consortiaName == this.playerInfo.consortiaName && army.baseHero.consortiaID == consortiaID;
			} else {
