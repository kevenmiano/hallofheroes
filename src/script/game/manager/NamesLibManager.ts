...
export class NamesLibManager {
    ...
    private static words: string[] = ["a", "b", "c", "d", "e", "f", "g", "h", "i", "j", "k", "l", "m", "n", "o", "p", "q", "r", "s", "t", "u", "v", "w", "x", "y", "z"];
    ...
    public static get Instance(): NamesLibManager {
        if (!this._Instance)
            this._Instance = new NamesLibManager();
        ...
        if (dataPath) {
            await ResMgr.Instance.loadRes(dataPath, (ret) => {
                if (ret) {
                    ...
                    if (arr.length >= 4) {//EN B
                        NamesLibManager.givenNameENB = arr[3].split("|");
                    }
                    if (arr.length >= 5) {//EN G
    ...
    public static newName(sexs: number): string {
        ...
        if (isCNLanguage()) {
            ...
            var givenIndexNumber: number = Utils.randomInt(0, this.numbers.length - 1);
            name += this.familyName[familyIndex] ? this.familyName[familyIndex] : "";
            let extra: string = this.numbers[givenIndexNumber] + "";
            if (sexs == 0) {
                name += this.givenNameG[givenIndexG] ? this.givenNameG[givenIndexG] + extra : "";
            } else {
                name += this.givenNameB[givenIndexB] ? this.givenNameB[givenIndexB] + extra : "";
            }
        } else {//规则: 名字中间插入0-9数字, 结尾加a-z字母~
            var givenIndexG: number = Utils.randomInt(0, this.givenNameENG.length - 1);
            var givenIndexB: number = Utils.randomInt(0, this.givenNameENB.length - 1);
            var givenIndexNumber: number = Utils.randomInt(0, this.numbers.length - 1);
            var givenIndexWord: number = Utils.randomInt(0, this.words.length - 1);
            let extra: string = this.numbers[givenIndexNumber] + this.words[givenIndexWord];
            if (sexs == 0) {
                name += this.givenNameENG[givenIndexG] ? this.givenNameENG[givenIndexG] + extra : "";
            } else {
                name += this.givenNameENB[givenIndexB] ? this.givenNameENB[givenIndexB] + extra : "";
            }
        }
        return name;
    }
