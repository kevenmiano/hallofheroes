...
export default class OfferRewardItem extends FUI_OfferRewardItem {
    ...
    private __onAcceptClick(event: Laya.Event) {
        if (this._vdata && this.checkCanAccept) {
            if (this.rewardPos == -1) {//接取悬赏公文任务
                if (this._vdata.NeedMaxLevel < ArmyManager.Instance.thane.grades) {//等级条件不满足
                    ...
                    var content: string = LangManager.Instance.GetTranslation("offerRewardItem.acceptClick.content1");
                    SimpleAlertHelper.Instance.Show(SimpleAlertHelper.SIMPLE_ALERT, null, prompt, content, confirm, cancel, this.onAcceptConfirm.bind(this));
                } else {
                ...
                if (this._vdata.NeedMaxLevel < ArmyManager.Instance.thane.grades) {//等级条件不满足
                    ...
                    var cancel: string = LangManager.Instance.GetTranslation("public.cancel");
                    var prompt: string = LangManager.Instance.GetTranslation("public.prompt");
                    var content: string = LangManager.Instance.GetTranslation("offerRewardItem.acceptClick.content2");
                    SimpleAlertHelper.Instance.Show(SimpleAlertHelper.SIMPLE_ALERT, null, prompt, content, confirm, cancel, this.onAcceptConfirm.bind(this));
                } else {
                    OfferRewardManager.Instance.sendAddRewardTask(this.rewardPos);
                }
    ...
    private __onGiveUpClick(event: Laya.Event) {
        if (this._vdata) {
            ...
            var cancel: string = LangManager.Instance.GetTranslation("public.cancel");
            var prompt: string = LangManager.Instance.GetTranslation("public.prompt");
            var content: string = LangManager.Instance.GetTranslation("task.TaskFrameII.content");
            SimpleAlertHelper.Instance.Show(SimpleAlertHelper.SIMPLE_ALERT, null, prompt, content, confirm, cancel, this.removeRewardTaskCallBack.bind(this));
        }
        event.stopPropagation();
    }
    ...
    private removeRewardTaskCallBack(b: boolean, flag: boolean) {
        if (b) {
            if (this._vdata && this._vdata.TemplateId) {
                let rewardData = OfferRewardManager.Instance.model.baseRewardDic[this._vdata.TemplateId];
                if (rewardData) {
    ...
    private get checkCanAccept(): boolean {
        ...
        if (this._vdata.isOutDate) {
            str = LangManager.Instance.GetTranslation("buildings.offerreward.view.OfferTaskItem.outDateTip");
            MessageTipManager.Instance.show(str);
            return false;
        }
        if (!this._vdata.isReachDate) {
            str = LangManager.Instance.GetTranslation("buildings.offerreward.view.OfferTaskItem.unreachDateTip");
            MessageTipManager.Instance.show(str);
            return false;
        }
        return true;
    ...
    private setColor(frame: number): string {
        let colorArray: Array<string> = ["#ffffff", "#59cd41", "#32a2f8", "#a838f7", "#ff8000"];
    ...
    public setData(temp: OfferRewardTemplate, profile: number, state: number) {
        ...
        if (this._vdata) {
            ...
            if (findedIndex >= 0) {
                this.TaskTitleTxt.text = this._vdata.TitleLang.substring(0, findedIndex);
            } else {
                this.TaskTitleTxt.text = this._vdata.TitleLang
            }
            this.TaskContentTxt.text = this._vdata.conditionList[0].CondictionTitleLang + " (0/" + this._vdata.conditionList[0].Para2 + ")";

            // 判断悬赏任务是否已经接取, 已经达到完成条件
            let tagColor = false
            let baseRewardTemp = OfferRewardManager.Instance.model.getAcceptTaskTemp(temp.TemplateId)
            if (baseRewardTemp && this._state == OfferRewardModel.REWARD_STATE_ACCEPTED) {
                let progress = baseRewardTemp.getProgress()
                if (progress.length > 0) {
                    this.TaskContentTxt.text = this._vdata.conditionList[0].CondictionTitleLang + " " + progress[0];
                }
            ...
            if (this._profile != 5) {
                this.TaskExpValueTxt.text = (this._vdata.RewardPlayGP * this._profile * this.thane.grades).toString();
                this.TaskGoldValueTxt.text = (this._vdata.RewardGold * this._profile * this.thane.grades).toString();
            }
            else {
                this.TaskGoldValueTxt.text = (this._vdata.RewardGold * (this._profile + 1) * this.thane.grades).toString();
                this.TaskExpValueTxt.text = (this._vdata.RewardPlayGP * (this._profile + 1) * this.thane.grades).toString();
            }
            ...
            switch (this._state) {
                ...
                case OfferRewardModel.REWARD_STATE_ACCEPTED:
                    ...
                    if (this.kingInfo.leftTime <= 0) {
                        ...
                        if (VIPManager.Instance.model.isOpenPrivilege(VipPrivilegeType.OFFER_REWARD_LIMIT, VIPManager.Instance.model.vipInfo.VipGrade)) {//有特权, 直接完成
                            this.Btn_quick.title = LangManager.Instance.GetTranslation("ui.tasktracebar.view.TaskTraceItem.CompletedImmediately");
                        }
                        else {
                            this.Btn_quick.title = LangManager.Instance.GetTranslation("gameguide.GameGuideFrame.skipNow");
                        }
                    } else {
                        this.Btn_accept.visible = true;
                ...
                case OfferRewardModel.REWARD_STATE_COMPLETED:
                    ...
                    this.Btn_quick.visible = false;
                    this.giveBtn.visible = false;
                    this.TaskContentTxt.text = this._vdata.conditionList[0].CondictionTitleLang + " (" + this._vdata.conditionList[0].Para2 + "/" + this._vdata.conditionList[0].Para2 + ")";
                    this.picIcon.url = FUIHelper.getItemURL("Space", "Icon_Task1_2");
                    break;
                case OfferRewardModel.REWARD_STATE_REACH_ALLCONDITION:
                    ...
                    this.Btn_quick.visible = false;
                    this.giveBtn.visible = false;
                    this.TaskContentTxt.text = this._vdata.conditionList[0].CondictionTitleLang + " " + LangManager.Instance.GetTranslation("buildings.offerreward.data.BaseOfferReward.progress");
                    this.picIcon.url = FUIHelper.getItemURL("Space", "Icon_Task1_2");
                    break;
            }
        }
