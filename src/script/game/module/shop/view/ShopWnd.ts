...
export class ShopWnd extends BaseWindow {
    ...
    private initView() {
        ...
        //更新充值按钮红点
        this.tab_discount.visible = this.discountModel.open && ConfigManager.info.DISCOUNT_SHOP_SWITCH;
        this.tab_limit.visible = this.shopModel.getTimeBuyList(0).length > 0;//限时商店
        this.Img_limitIcon.visible = this.tab_limit.visible;
        this.tab_maze.visible = ArmyManager.Instance.thane.grades >= OpenGrades.MAZE;
        this.tab_consortia.visible = PlayerManager.Instance.currentPlayerModel.playerInfo.consortiaID > 0 && this._consortiaShopLevel > 0;
        this.tab_pvp.visible = ArmyManager.Instance.thane.grades >= OpenGrades.CHALLENGE;
        ...
        this.tab_recharge.visible = !(Utils.isWxMiniGame() && ComponentSetting.IOS_VERSION);
        this.tab_promotion.visible = !(Utils.isWxMiniGame() && ComponentSetting.IOS_VERSION);
    ...
    private initEvent() {
        this.tab1 && this.tab1.on(fgui.Events.STATE_CHANGED, this, this.onTab1Changed);
        this.tab2 && this.tab2.on(fgui.Events.STATE_CHANGED, this, this.onTab2Changed);
        VIPManager.Instance.addEventListener(VIPEvent.VIP_PRIVILEGE_UPDATE, this.__flashPrivilege, this);
        NotificationManager.Instance.dispatchEvent(ShopEvent.GOODS_DISCOUNT_UPDATE, this.__flashLimitDiscount, this);
        NotificationManager.Instance.addEventListener(ShopEvent.PAGE_SELECTED, this.onPageSelect, this)
        NotificationManager.Instance.addEventListener(ShopEvent.DISCOUNTSHOP_OPENSTATE, this.onDiscountShopOpenState, this)
        NotificationManager.Instance.addEventListener(ShopEvent.DISCOUNTSHOP_UPDATE, this.__updateDiscountRedDot, this);
    }
    ...
    public OnShowWind() {
        ...
        if (frameData) {
            ...
            if ((Utils.isWxMiniGame() && ComponentSetting.IOS_VERSION) && pageValue == 3) {
                pageValue = 1;
            }
    ...
    private removeEvent() {
        this.tab1 && this.tab1.off(fgui.Events.STATE_CHANGED, this, this.onTab1Changed);
        this.tab2 && this.tab2.off(fgui.Events.STATE_CHANGED, this, this.onTab2Changed);
        VIPManager.Instance.removeEventListener(VIPEvent.VIP_PRIVILEGE_UPDATE, this.__flashPrivilege, this);
        NotificationManager.Instance.removeEventListener(ShopEvent.GOODS_DISCOUNT_UPDATE, this.__flashLimitDiscount, this);
        NotificationManager.Instance.removeEventListener(ShopEvent.PAGE_SELECTED, this.onPageSelect, this)
        NotificationManager.Instance.removeEventListener(ShopEvent.DISCOUNTSHOP_OPENSTATE, this.onDiscountShopOpenState, this)
        NotificationManager.Instance.removeEventListener(ShopEvent.DISCOUNTSHOP_UPDATE, this.__updateDiscountRedDot, this);
    }
    ...
    private setTabRechargeStyle() {
        if (this.tab_recharge) {
            let titleStr = LangManager.Instance.GetTranslation("ShopWnd.tabRechargeTxt");
            this.tab_recharge.title = this.getTitleNameStr(titleStr);
            this.tab_recharge.selectedTitle = this.getSelectTitleNameStr(titleStr);
            this.tab_recharge.icon = FUIHelper.getItemURL(EmPackName.Base, "Tab_Dia2Y2_Nor");
            this.tab_recharge.selectedIcon = FUIHelper.getItemURL(EmPackName.Base, "Tab_Dia2Y2_Sel");
        }
    }
