  ...
  private setPetName() {
    ...
    if (this._petNameTxt) {
      ...
      if (this._info.baseHero.petQuaity > 0) {
        this._petNameTxt.color =
          FilterFrameText.Colors[eFilterFrameText.PetQuality][
            this._info.baseHero.petQuaity
          ];
      }
      if (this._heroFigure.petAvatar) {
        this._petNameTxt.x =
          this._heroFigure.x +
          this._heroFigure.petAvatar.x -
          this._petNameTxt.width / 2;
        this._petNameTxt.y =
          this._heroFigure.y + this._heroFigure.petAvatar.y - 110;
      }
    }
  }
  ...
  private setHonerName() {
    ...
    if (this.isHoner) {
      this._honerView = new AppellView(
        this._info.baseHero.appellInfo.ImgWidth,
        this._info.baseHero.appellInfo.ImgHeight,
        this._info.baseHero.appellId,
      );
      this._honerView.x = this.txtPlayerName.x;
      let imgHeight: number = this._info.baseHero.appellInfo.ImgHeight;
      this._honerView.y = this._info.baseHero.consortiaName
        ? this.txtGuildName.y - imgHeight / 2
        : this.txtPlayerName.y - imgHeight / 2 - 20;
      this.showHoner();
    }
  }

  private showHoner() {
    if (this._heroFigureCon && this._heroFigureCon.displayObject)
