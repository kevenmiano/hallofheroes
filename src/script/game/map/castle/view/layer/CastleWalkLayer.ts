...
export default class CastleWalkLayer extends Laya.Sprite {
    ...
    private initClientBuild() {
        ...
        for (let i: number = 0; i < this._viewList.length; i++) {
            ...
            obj.zOrder = CastleConfigUtil.Instance.getBuildZorder(obj.buildingInfo.sonType)
        }
        this.updateHookInfo();
    }
    ...
    public updateBuildingView(bInfo: BuildInfo) {
        let bView: CastleBuildingView = this._buildingsDic.get(bInfo.buildingId) as CastleBuildingView;
    ...
    private initEvent() {
        ...
        NotificationManager.Instance.addEventListener(HookEvent.UPDATE_HOOK, this.updateHookInfo, this)
    }

    private removeEvent() {
        this.thane.removeEventListener(PlayerEvent.THANE_LEVEL_UPDATE, this.__levelUpdateHandler, this);
        NotificationManager.Instance.removeEventListener(HookEvent.UPDATE_HOOK, this.updateHookInfo, this)
    }
    ...
    public dispose() {
        this.removeEvent();
        BuildingManager.Instance.model.buildingListByID.getList().forEach((element: BuildInfo) => {
