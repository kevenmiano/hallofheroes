...
export class LimitShopItem extends FUI_LimitShopItem {
    ...
    public set info(value: MainShopInfo) {
        ...
        if (value) {
            ...
            if (this._info.isGift) {
                ...
                var goodsTempInfo: t_s_itemtemplateData = TempleteManager.Instance.getGoodsTemplatesByTempleteId(this._info.templateId);
                ...
                this.item_gift.url = "";
                if (goodsTempInfo)
                    this.txt_name.text = goodsTempInfo.TemplateNameLang;
            }
            //商品均为钻石购买
    ...
    public setTimeText(value: number) {
        if (this._info.type == 1) { //促销商品
            if (value > 60 * 60 * 24 * 30) {//超出1个月则设置为永久
                this.txt_timelimit.text = LangManager.Instance.GetTranslation("yishi.view.tips.goods.EquipTip.timeStr01");
            } else {
                this.txt_timelimit.text = DateFormatter.getStopDateString(value);
            }
        } else if (this._info.type == 2) {//限时抢购
    ...
    private setDiscount(value: number = 0) {
        ...
        if (value == 0) {
            this.txt_discount.text = LangManager.Instance.GetTranslation("public.free");
        } else {
