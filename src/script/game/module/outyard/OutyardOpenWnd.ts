...
export default class OutyardOpenWnd extends BaseWindow {
    ...
    private addEvent() {
        NotificationManager.Instance.addEventListener(NotificationEvent.OUTYARD_STATE_INFO, this.__refreshOpeningHandler, this);
        this.timeList.itemRenderer = Laya.Handler.create(this, this.renderTimeListItem, null, false);
        NotificationManager.Instance.addEventListener(NotificationEvent.OUTYARD_FULL_INFO, this.refreshState, this);
        NotificationManager.Instance.addEventListener(NotificationEvent.OUTYARD_UPDATE_INFO, this.refreshState, this);
    ...
    private removeEvent() {
        NotificationManager.Instance.removeEventListener(NotificationEvent.OUTYARD_STATE_INFO, this.__refreshOpeningHandler, this);
        this.timeList.itemRenderer.recover();
        Utils.clearGListHandle(this.timeList);
        NotificationManager.Instance.removeEventListener(NotificationEvent.OUTYARD_FULL_INFO, this.refreshState, this);
        NotificationManager.Instance.removeEventListener(NotificationEvent.OUTYARD_UPDATE_INFO, this.refreshState, this);
    ...
    private enterBtnHandler(){
        ...
        if(stateMsg.state == OutyardModel.OVER ||stateMsg.state == OutyardModel.CLEAR){
            MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("OutyardOpenWnd.enterBtnHandler.tips"));
            return;
        }
    ...
    private refreshState() {
        ...
        let stateMsg: StackHeadStateMsg = OutyardManager.Instance.stateMsg;
        if(stateMsg.state == OutyardModel.FIGHTING || stateMsg.state == OutyardModel.READY 
            ||stateMsg.state == OutyardModel.CLEAR || stateMsg.state == OutyardModel.OVER){
