...
export class LevelGiftItem2 extends FUI_LevelGiftItem2 {
    ...
    private refreshView() {
        ...
        this.boxItem.tipDirctions = "5,4,7,6";
         if (this._info.packageState2 == 1) {//可领取
            this.hasGet.selectedIndex = 0;
            if (ArmyManager.Instance.thane.grades >= this._info.grade) {
                this.btn_buy.enabled = true;
                this.btn_buy.title = LangManager.Instance.GetTranslation("LevelGiftItem2.buyTitle1");
            } else {
                this.btn_buy.title = LangManager.Instance.GetTranslation("guidetask.GuideTaskItem.taskFalgTxt.text02");
                this.btn_buy.enabled = false;
            }
        } else  if (this._info.packageState2 == 2){//已领取
            this.btn_buy.enabled = false;
            this.hasGet.selectedIndex = 1;
            this.btn_buy.title = LangManager.Instance.GetTranslation("LevelGiftItem2.buyTitle2");
        } 
    ...
    private buyHandler() {
        ...
        let name: string =  LangManager.Instance.GetTranslation("vip.view.VipGiftItem.vipTxt",this._info.grade);
        let cost: number = this._info.discount;
        let content: string = LangManager.Instance.GetTranslation("campaign.TrailShop.BuyConfirmTxt", name, cost);
    ...
    private getPackage(result: boolean, flag: boolean) {
        if (result) {
            if (this.playerInfo.point < this._info.discount) {//改为弹窗提示
