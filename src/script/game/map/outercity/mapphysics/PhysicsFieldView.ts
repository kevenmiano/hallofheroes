...
export class PhysicsFieldView extends MapPhysicsAttackingBase {
  ...
  protected initView(): void {
    if (!this._desctxt) {
      this._desctxt = new FilterFrameText(
        10,
        20,
        undefined,
        18,
    ...
    if (!this._owner) {
      this._owner = new FilterFrameText(
        140,
        20,
        undefined,
        18,
    ...
    if (pModel.userId == this.nodeInfo.info.occupyPlayerId) {
      ...
      this.nodeInfo.info.occupyLeagueName != ""
    ) {
      if (this.nodeInfo.info.occupyLeagueName == pModel.consortiaName) {
        ////同公会蓝色
    ...
    if (!this._name) {
      this._name = new FilterFrameText(
        140,
        20,
        undefined,
        18,
    ...
    if (this.nodeInfo.info.types == PosType.OUTERCITY_MINE) {
      //金矿节点
      let grade = this.nodeInfo.tempInfo.Grade;
      this._name.text =
        this.nodeInfo.tempInfo.NameLang +
        " " +
        LangManager.Instance.GetTranslation("public.level3", grade);
      if (this.nodeInfo.selfOccpuyArr.length > 0) {
        //玩家在当前节点有占领的矿点
        if (!this._countTxt) {
          this._countTxt = new FilterFrameText(
            140,
            20,
            undefined,
            18,
        ...
        this._countTxt.text = LangManager.Instance.GetTranslation(
          "PhysicsFieldView.countTxt",
          this.nodeInfo.selfOccpuyArr.length,
          max,
        );
      }
    } else {
  ...
  protected layouCallBack(): void {
    ...
    if (this.nodeInfo.info.types == PosType.OUTERCITY_TREASURE) {
      ...
      this._name.x = this._owner.x = parseInt(
        ((this._width - 140) / 2).toString(),
      );
      this._owner.y = this.moviePos.y - this._name.height - 7;
      this._name.y = this.moviePos.y;
    }
