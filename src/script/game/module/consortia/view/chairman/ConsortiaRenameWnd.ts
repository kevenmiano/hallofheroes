...
export class ConsortiaRenameWnd extends BaseWindow {
  ...
  public OnInitWind() {
    ...
    this.frame.getChild("title").text = LangManager.Instance.GetTranslation(
      "ConsortiaRenameWnd.title",
    );
    this.btnConfirm.title = LangManager.Instance.GetTranslation(
      "ConsortiaRenameWnd.btnConfirm.title",
    );
    this.tfRename.tooltips = LangManager.Instance.GetTranslation(
  ...
  private initView() {
    ...
    this.tfRename.promptText = LangManager.Instance.GetTranslation(
      "ConsortiaContributeWnd.tfDiamond.text",
    );
    this.btnConfirm.enabled = false;
    this.txtTip.text = LangManager.Instance.GetTranslation(
      "consortia.view.club.CreatConsortiaFrame.command02",
    );
    this.txtTipLimit.text = LangManager.Instance.GetTranslation(
      "consortia.view.club.CreatConsortiaFrame.command04",
    );
  ...
  private __renameHandler(pkg: PackageIn) {
    let msg: ConsortiaRenameRspMsg = pkg.readBody(
  ...
  private btnConfirmClick() {
    ...
    if (
      FilterWordManager.isGotForbiddenWords(vStr, "name") ||
      FilterWordManager.isGotForbiddenWords(vStr, "chat")
    ) {
