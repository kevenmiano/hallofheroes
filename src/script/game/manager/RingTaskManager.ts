...
import { RingTaskEvent, SLGSocketEvent } from '../constant/event/NotificationEvent';
...
export default class RingTaskManager extends GameEventDispatcher {
    ...
    private initEvent() {
        ServerDataManager.listen(S2CProtocol.U_C_REPEAT_REWARD_FINISH, this, this.__finishTaskHandler);
        ServerDataManager.listen(S2CProtocol.U_C_REPEAT_REWARD_UPDATE, this, this.__refreshTaskHandler);
        ServerDataManager.listen(S2CProtocol.U_C_REPEAT_REWARD_END, this, this.__finishAllTaskHandler);
        ServerDataManager.listen(S2CProtocol.U_C_REPEAT_REWARD_FNISHED_SUM_UPDATE, this, this.__refreshRingNumHandler);
    }
    ...
    private __refreshTaskHandler(pkg: PackageIn) {
        ...
        for (let i: number = 0; i < len; i++) {
            ...
            if (this.model.hasAccessList[rewardInfo.templateId]) //判断在已经接取的任务列表中是否存在rewardInfo
            {
                //存在
                ringTask = this.model.hasAccessList[rewardInfo.templateId];
                ringTask.isNew = false;
                if (rewardInfo.isComplete) //判断是否完成
                {
                    ...
                    ringTask.changeDate = PlayerManager.Instance.currentPlayerModel.sysCurTimeBySecond;
    ...
    private __refreshRingNumHandler(pkg: PackageIn) {
        ...
        if (this.model.rewardNum > 200) {
            ...
            if (!t) return;
            t.ringTaskTemp.TitleLang = LangManager.Instance.GetTranslation("managers.RingTaskManager.taskTitle");
    ...
    public CompletedImmediately(useBind: boolean = false) {
        ...
        SocketManager.Instance.send(C2SProtocol.C_REPEAT_REWARD_FINISH_IMMEDIATELY, msg);
    }
    ...
    public getRewardGoods(ringNum: number): any[] {
        ...
        for (let dicKey in goodsList) {
            if (goodsList.hasOwnProperty(dicKey)) {
                ...
                let ringArr: any[] = offerRewardGoodsTemp.RepeatStep.toString().split(",");
                if (this.getFlag(ringArr, ringNum)) {
                    if (grade >= offerRewardGoodsTemp.RepeatMinLevel && grade < offerRewardGoodsTemp.RepeatMaxLevel) {
    ...
    public getRewardGoodsTemp(ringTask: RingTask): any[] {
        let arr: any[] = [];
        if (ringTask) {
            let offerRewardGoods = TempleteManager.Instance.offerRewardGoodsTemplateList();
            let grade: number = ArmyManager.Instance.thane.grades;
            ...
            for (let dicKey in offerRewardGoods) {
                if (offerRewardGoods.hasOwnProperty(dicKey)) {
                    let offerRewardGoodsTemp: t_s_rewardgoodData = offerRewardGoods[dicKey];
                    let ringArr: any[] = offerRewardGoodsTemp.RepeatStep.toString().split(",");
    ...
    public getTalkTaskNPCNode(): number {
        ...
        let condition: t_s_rewardcondictionData = arr[0];
        if (condition.CondictionType == RewardConditionType.TALKTASK) //对话任务
        {
    ...
    public static get Instance(): RingTaskManager {
        if (!RingTaskManager._instance) RingTaskManager._instance = new RingTaskManager();
