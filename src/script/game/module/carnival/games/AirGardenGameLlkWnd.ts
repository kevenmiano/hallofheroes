...
export default class AirGardenGameLLKWnd extends BaseWindow {
    ...
    private initView() {
        ...
        var temp: t_s_carnivalpointexchangeData[] = TempleteManager.Instance.getCarnivalByType(CarnivalModel.GAME_TYPE_1);
        if (temp && temp.length) {
            this.txt_score.text = LangManager.Instance.GetTranslation("samll.game.score", temp[0].Target);
        }
        //x 请放入语言包里面
        ...
        this.txt_bomb.text = LangManager.Instance.GetTranslation("llk.view.LlkGoodsItem.GoodsName1", "" + 0);
        this.txt_wash_card.text = LangManager.Instance.GetTranslation("llk.view.LlkGoodsItem.GoodsName2", "" + 0);
    ...
    private __infoHandler(msg: string, body: Object) {
        ...
        if (StringHelper.isNullOrEmpty(this.llkModel.info.points)) {//空地图
            this.gameState.selectedIndex = 0;
        } else {
            this.gameState.selectedIndex = 1;
        }
        //"x" 请放入 语言包里面
        this.txt_myPoint.text = LangManager.Instance.GetTranslation("carnival.awardpoint.point", this.llkModel.info.score + "");
        this.txt_bomb.text = LangManager.Instance.GetTranslation("llk.view.LlkGoodsItem.GoodsName1", "" + this.llkModel.info.bombCount);
        this.txt_wash_card.text = LangManager.Instance.GetTranslation("llk.view.LlkGoodsItem.GoodsName2", "" + this.llkModel.info.resetCount);

    }

    private deleteNode(value: LlkDeleteData) {
        if (AirGardenGameLlkManager.Instance.model.checkByPos(value.node1, value.node2, true)) {
            let count: number = 0;
            let nodeIndex1: number = value.node1.row * LlkModel.COLUMN + value.node1.col;
            let nodeIndex2: number = value.node2.row * LlkModel.COLUMN + value.node2.col;
            ...
            for (var i: number = 0; i < count; i++) {
                ...
                if (nodeItem && !nodeItem.isDisposed)
                    nodeItem.play();
    ...
    private playCombox(value: number, point: Laya.Point) {
        HintTips.showHintLabel(HintTips.createLabLLk(LangManager.Instance.GetTranslation("airGardenGameLLKWnd.combot.tip", value), point));
    }

    /**播放获得积分 */
    private playScore(value: number, point: Laya.Point) {
        HintTips.showHintLabel(HintTips.createLabLLk(LangManager.Instance.GetTranslation("airGardenGameLLKWnd.score.tip", value), point));
    }
    ...
    private check() {
        ...
        var str: string = LangManager.Instance.GetTranslation("AirGardenGame.exit.hint");
        SimpleAlertHelper.Instance.popAlerFrame(prompt, str, confirm, cancel, { callback: this.realDispose.bind(this) });
        return;
    }
    ...
    private __useClickHandler(type: number): void {
        if (type == 1) {//炸弹
            if (this.llkModel.info.bombCount <= 0) {
                MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("AirGardenGame.hint1"));
            } else if (this.isNullMap) {
                MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("llk.view.LlkGoodsItem.gateNotStart"));
            } else {
                AirGardenGameLlkManager.Instance.llkOp(AirGardenGameLlkManager.USE_BOMB);
            }
        } else if (type == 2) {//洗牌
            if (this.llkModel.info.resetCount <= 0) {
                MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("AirGardenGame.hint2"));
            } else if (this.isNullMap) {
                MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("llk.view.LlkGoodsItem.gateNotStart"));
            } else {
        ...
        if (target.$owner && target.$owner instanceof AirGardenGameLLKItem) {
            ...
            if (targetItem) {
                ...
                if (this._selected) {
                    ...
                    if (this.llkModel.checkByPos(this._selected.NodeData, current.NodeData)) {
                        AirGardenGameLlkManager.Instance.llkDelete(this._selected.NodeData, current.NodeData);
                        this._selected = null;
                    } else {
