...
import { ILayer } from "../../../script/game/interfaces/Layer";
...
export default class SceneLayer extends Laya.Sprite implements ILayer {
  ...
  init(node?: any) {
    ...
    this.scrollRect = new Laya.Rectangle(
      0,
      0,
  ...
  pushView(view, zIndex?: number) {
    ...
    if (zIndex == undefined) view.zOrder = this.numChildren - 1;
    else view.zOrder = zIndex;
    if (view["showEffect"]) view.showEffect();
  }
  ...
  popView(view) {
    if (!view) {
      if (this.list.length > 0) {
        ...
        if (layerInfo["exit"]) layerInfo.exit();
      }
    } else {
  ...
  resize() {
    ...
    for (let index = 0; index < this.list.length; index++) {
      ...
      if (element["resize"]) element.resize();
    }
  }
