...
export default class AllocateWnd extends BaseWindow {
    ...
    public OnInitWind() {
        super.OnInitWind();
        this.frame.getChild('title').text = LangManager.Instance.GetTranslation("armyII.AllocateFrame.title");
        this.n5Txt.text = LangManager.Instance.GetTranslation("armyII.ThaneAttributeView.Tip12");
        this.BtnRecruit.title = LangManager.Instance.GetTranslation("armyII.viewII.allocate.AllocateView.recruit");
        this.addEvent();
        this.setCenter();
    ...
    private addEvent() {
        ...
        ArmyManager.Instance.army.addEventListener(ArmyEvent.ARMY_INFO_CHANGE, this.refreshView, this);
        ArmyManager.Instance.army.baseHero.addEventListener(PlayerEvent.THANE_INFO_UPDATE, this.refreshView, this);
        ArmyManager.Instance.addEventListener(ArmyEvent.ARMY_INFO_CHANGE, this.refreshView, this);
        this.SoliderList.itemRenderer = Laya.Handler.create(this, this.__renderListItem, null, false);
    }

    private removeEvent() {
        this.BtnFormation.offClick(this, this.__formationExHandler);
        this.BtnRecruit.offClick(this, this.__recruitHandler);
        ArmyManager.Instance.army.removeEventListener(ArmyEvent.ARMY_INFO_CHANGE, this.refreshView, this);
        ArmyManager.Instance.army.baseHero.removeEventListener(PlayerEvent.THANE_INFO_UPDATE, this.refreshView, this);
        ArmyManager.Instance.removeEventListener(ArmyEvent.ARMY_INFO_CHANGE, this.refreshView, this);
    ...
    private __formationExHandler() {
        if (!NewbieBaseConditionMediator.checkConditionCommon(22)) {
            MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("AllocateWnd.soldiersToBattle"));
            return
        }
    ...
    private refreshView() {
        ...
        if (this.ap && this.ap.templateInfo && this.ap.ownPawns > 0) {
            this.NameLevelTxt.text = this.ap.templateInfo.PawnNameLang + ' ' + LangManager.Instance.GetTranslation("buildings.casern.view.RecruitPawnCell.command06", this.ap.templateInfo.Level);
        } else {
            this.NameLevelTxt.text = "";
        }
        this.SoliderNumtxt.text = ArmyManager.Instance.army.countAllArmyPawnNmber() + "/" + ArmyManager.Instance.army.baseHero.attackProrerty.totalConatArmy
    ...
    private filterByCount(list: any[]): any[] {
        ...
        for (var i: number = 0; i < len; i++) {
            ...
            if (ap.ownPawns > 0 || (ap.templateId == serialPawn.templateId && serialPawn.ownPawns > 0)) {
                arr.push(list[i]);
            }
        }
        return arr;
