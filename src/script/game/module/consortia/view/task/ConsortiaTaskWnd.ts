...
export default class ConsortiaTaskWnd extends BaseWindow {
    ...
    private addEvent() {
        ...
        this._guildModel.on(ConsortiaEvent.TASK_WEEK_REWARD, this.updateWeekReward, this);
    }
    ...
    private removeEvent() {
        ...
        this._guildModel.off(ConsortiaEvent.TASK_WEEK_REWARD, this.updateWeekReward, this);
    }
    ...
    private initView() {
        ...
        this.guildRewardList.itemRenderer = Laya.Handler.create(this, this.onGuildRewardRenderItem, null, false)
        this.selfRewardList.itemRenderer = Laya.Handler.create(this, this.onSelfRewardRenderItem, null, false)
        this.recordList.itemRenderer = Laya.Handler.create(this, this.onRecordRenderItem, null, false)
        this.scoreBoxlist.itemRenderer = Laya.Handler.create(this, this.onScoreBoxRenderItem, null, false)
        this.scoreBoxlist.on(fgui.Events.CLICK_ITEM, this, this.onScoreBoxClickItem)

        this.txtFrameTitle.text = LangManager.Instance.GetTranslation("ConsortiaTaskWnd.title")
        this.txtTaskContent.text = LangManager.Instance.GetTranslation("ConsortiaTaskWnd.taskCon")
        this.txtDifficulty.text = LangManager.Instance.GetTranslation("ConsortiaTaskWnd.difficulty")
        this.txtGuildReward.text = LangManager.Instance.GetTranslation("ConsortiaTaskWnd.guildReward")
        this.txtSelfReward.text = LangManager.Instance.GetTranslation("ConsortiaTaskWnd.selfReward")
        this.txtFinishTodayTask.text = LangManager.Instance.GetTranslation("ConsortiaTaskWnd.finishTodayTask")
        this.txtTaskRecordTitle.text = LangManager.Instance.GetTranslation("ConsortiaTaskWnd.taskRecord")
        this.txtWeekScore.text = LangManager.Instance.GetTranslation("ConsortiaTaskWnd.weekScore")
        this.txtResetTip.text = LangManager.Instance.GetTranslation("ConsortiaTaskWnd.ResetTip")
        this.txtUpgradeStarMax.text = LangManager.Instance.GetTranslation("ConsortiaTaskWnd.upgradeStarTipMax2")

        this.btnFinish.title = LangManager.Instance.GetTranslation("public.finish")
        this.btnUpGradeStar.title = LangManager.Instance.GetTranslation("ConsortiaTaskWnd.upgrade")
    }
    ...
    private updateInfo() {
        ...
        // 任务进度
        this.txtTaskProg.text = LangManager.Instance.GetTranslation("ConsortiaTaskWnd.progress", taskInfo.progNum + "/" + taskInfo.finishNeedNum)

        // 升星价格
        this.txtUpgradeStarCost.text = this._guildModel.getTaskUpgradeStarPrice(taskInfo.starNum).toString()

        let maxStar = this._guildModel.taskInfo.starNum >= this._guildModel.TaskStarNum
    ...
    private updateRecord() {
        ...
        this.txtCurTodayTask.text = LangManager.Instance.GetTranslation("ConsortiaTaskWnd.finshNum", this._guildModel.taskFinishNum, this._guildModel.consortiaInfo.currentCount)
    }

    private updateWeekReward() {
        this.scoreBoxlist.numItems = this._guildModel.taskWeekScoreRewardList.length
        this.txtWeekScoreVal.text = this._guildModel.taskWeekScore + "/" + this._guildModel.taskWeekScoreMax
        this.progBar.value = this._guildModel.taskWeekScore / this._guildModel.taskWeekScoreMax * 100
    ...
    private btnRefreshClick() {
        let left = this._guildModel.TaskMaxRefreshNum - this._guildModel.taskRefreshNum
        if (left <= 0) {
            // 发送消息给服务器弹提示
            ConsortiaSocketOutManager.sendRefreshTask()
        } else {
            let str = LangManager.Instance.GetTranslation("ConsortiaTaskWnd.refreshTip", left)
    ...
    private btnUpGradeStarClick() {
        if (this._guildModel.taskInfo.starNum >= this._guildModel.TaskStarNum) {
            MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("ConsortiaTaskWnd.upgradeStarTipMax"))
            return
        }
        let price = this._guildModel.getTaskUpgradeStarPrice(this._guildModel.taskInfo.starNum)
        let str = LangManager.Instance.GetTranslation("ConsortiaTaskWnd.upgradeStarTip", price)
        SimpleAlertHelper.Instance.ShowSimple(SimpleAlertHelper.USEBINDPOINT_ALERT, { checkDefault: true }, str, (b: boolean, flag: boolean) => {
            if (b) {
                ConsortiaSocketOutManager.sendUpgradeTaskStar(flag ? PayType.BindDiamond : PayType.Diamond)
            }
        })
    }
