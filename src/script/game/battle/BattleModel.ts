...
export class BattleModel extends GameEventDispatcher {
  ...
  public getHeroRight(): HeroRoleInfo {
    for (const key in this.armyInfoRight.getHeros) {
      if (
        Object.prototype.hasOwnProperty.call(this.armyInfoRight.getHeros, key)
      ) {
  ...
  public getHeroLeft(): HeroRoleInfo {
    for (const key in this.armyInfoLeft.getHeros) {
      if (
        Object.prototype.hasOwnProperty.call(this.armyInfoLeft.getHeros, key)
      ) {
  ...
  public setAutoFight(value: number) {
    ...
    if (this.isOver) {
      ...
      SocketSendManager.Instance.sendAutoAttack(
        this.battleId,
        this._autoFightFlag,
      );
    }
  ...
  public static getBossHpStripNum(id: number): number {
    let num: number = 0;
    let heroTemp: t_s_herotemplateData = ConfigMgr.Instance.getTemplateByID(
  ...
  public addRole($role, needAdd: boolean = true) {
    ...
    if (
      this.isAllPetPKBattle() &&
      $role.livingId == 0 &&
      $role.side == this.selfSide
    ) {
      this.lookHero = $role;
    }
    if ($role.livingId != 0) {
    ...
    if ($role.side == this.selfSide) {
      if ($role.inheritType == InheritRoleType.Pawn) {
        ...
        if (
          ...
          $role.side == SideType.DEFANCE_TEAM
        ) {
          $role.userId =
            PlayerManager.Instance.currentPlayerModel.userInfo.userId;
          $role.serverName =
  ...
  public getRoleByPos(pos: number, side: number): any {
    for (const key in this._roleList) {
      if (Object.prototype.hasOwnProperty.call(this._roleList, key)) {
        ...
        if (item.pos == pos && item.side == side) return item;
      }
    }
    return null;
  }
  ...
  public isAllSkillExecuted(): boolean {
    ...
    for (let role of this._roleList.values()) {
      if (role.isLiving && role.actionManager && !role.actionManager.isOver) {
        if (
          role.actionMovie.currentLabel &&
          role.actionMovie.currentLabel != ActionLabesType.VICTORY
        ) {
          Logger.battle(
            "角色上还有动作未执行:" + role.actionMovie.currentLabel,
          );
          tmp = false;
          break;
        }
  ...
  public get isMulPve(): boolean {
    ...
    return (
      campaignTemplate &&
      campaignTemplate.Types == 0 &&
      campaignTemplate.Capacity == 3
    );
  }
  ...
  public getHeroRoleByUserId(
    ...
    if (StringHelper.isNullOrEmpty(serverName)) {
      ...
      for (let item of this._roleList.values()) {
        if (item.inheritType == InheritRoleType.Hero) {
          if (
            (item as HeroRoleInfo).userId == userId &&
            (item as HeroRoleInfo).serverName == serverName
          ) {
            tmp = item;
            break;
          }
  ...
  public isPvP(): boolean {
    ...
    for (let roleInfo of this._roleList.values()) {
      if (roleInfo instanceof HeroRoleInfo) {
        if (
          roleInfo.type != RoleType.T_NPC_BOSS &&
          roleInfo.type != RoleType.T_WILD_LAND
        ) {
          if (roleInfo.side != this.selfHero.side) {
            tmp = true;
            break;
  ...
  public get isLockTalent(): boolean {
    ...
    if (
      ArmyManager.Instance.thane.grades < OpenGrades.TALENT ||
      !ComponentSetting.GENIUS
    ) {
      bTalentLock = true;
    }
    return bTalentLock;
  ...
  public get isShowShortCut() {
    ...
    if (
      this.battleType == BattleType.GUILD_TOTEM_BATTLE &&
      this.isBossBattle()
    ) {
      b = false;
    }
  ...
  public isAllPetPKBattle(): boolean {
    if (
      this.battleType == BattleType.SPACEMAP_PET_PK ||
