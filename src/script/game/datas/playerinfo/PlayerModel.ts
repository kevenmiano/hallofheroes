...
import { CampaignEvent, NotificationEvent, SpaceEvent } from '../../constant/event/NotificationEvent';
...
export class PlayerModel extends GameEventDispatcher {
    ...
    public set selectTarget(value: BaseArmy) {
        ...
        if (this._selectTarget) {
            ...
            if (this._selectTarget && armyView && armyView.hasOwnProperty("unlockTargetAndShowFate")) {
        ...
        if (this._selectTarget && armyView && armyView.hasOwnProperty("lockTargetAndHideFate")) {
    ...
    public set reinforce(value: BaseArmy) {
        ...
        if (this._reinforceTarget && FreedomTeamManager.Instance.inMyTeam(this._reinforceTarget.userId) && this.targetIsFighting(this._reinforceTarget)) {
    ...
    public get worldBossIsAutoFight(): boolean {
        return this._wolrdBossAutoFightFlag == PlayerModel.WORLDBOSS_AUTO_FIGHT ? true : false;
    }
    ...
    public get worldBossIsAutoLive(): boolean {
        return this._wolrdBossAutoLiveFlag == PlayerModel.WORLDBOSS_AUTO_LIVE ? true : false;
    }
    ...
    public set vehicleTech(value: number) {
        ...
        NotificationManager.Instance.dispatchEvent(NotificationEvent.VEHICLE_TECH_CHANGE, this._vehicleTech);
    }
    ...
    public get openServerDay(): number {
		...
		let startDate: Date = Utils.formatTimeZone(this._openServerTime, zoneOffset); 
        let hours: number = startDate.getHours();
        let minutes: number = startDate.getMinutes();
        let seconds: number = startDate.getSeconds();
        let differenceTime: number = (23 - hours) * 1000 * 60 * 60 + (59 - minutes) * 1000 * 60 + (59 - seconds) * 1000;
        ...
        if (intervalMillisecond <= firstDayRefreshDifferenceTime) {
            ...
            day = Math.ceil((intervalMillisecond - firstDayRefreshDifferenceTime) / dayMin) + 1;
        }
    ...
    public set sysCurtime(value: Date) {
        ...
        Logger.yyz("当前服务器时间: ", DateFormatter.format(value, "YYYY-MM-DD hh:mm:ss"));
    }
    ...
    public setFightingCapacityRecord(flag: string) {
        if (flag == "start") {
            ...
            DelayActionsUtils.Instance.addAction(new HeroFightingUpdateAction(this.fightingCapacityRecord, ArmyManager.Instance.thane));
    ...
    public checkChatForbidIsOpen(): boolean {
        if (ConfigManager.info.CHAT_FORBID
            && this.nowDate
