...
export default class WorldBossModel extends FrameDataBase {
    ...
    public commit() {
        var b: boolean = false;
        var userId: number = PlayerManager.Instance.currentPlayerModel.playerInfo.userId;
        for (const key in this._woundInfoList) {
            if (Object.prototype.hasOwnProperty.call(this._woundInfoList, key)) {
                var temp: WoundInfo = this._woundInfoList[key];
        ...
        this._woundInfoList = ArrayUtils.sortOn(this._woundInfoList, ["wound"], [ArrayConstant.DESCENDING | ArrayConstant.NUMERIC]);
        this._allWoundInfoList = [];
        this._jobWoundInfoList = [];
        var job: number = ThaneInfoHelper.getJob(PlayerManager.Instance.currentPlayerModel.playerInfo.job);
        var i: number = 0;
        ...
        for (i = 0; i < this._woundInfoList.length; i++) {
            ...
            if (ThaneInfoHelper.getJob(this._woundInfoList[i].job) == job && indexJob < 10) {
