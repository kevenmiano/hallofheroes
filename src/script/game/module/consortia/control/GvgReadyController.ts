...
export class GvgReadyController extends FrameCtrlBase implements Disposeable {
  ...
  constructor() {
    ...
    this._model.guildOrder = this._consortiaModel.consortiaInfo.orderInfo
      ? this._consortiaModel.consortiaInfo.orderInfo.gradeOrder
      : 0;
  }
  ...
  protected addEventListener() {
    ...
    this._model.addEventListener(
      GvgEvent.GUILD_CHALLENGE_LIST,
      this.__guildChallengeHandler,
      this,
    );
    ...
    ServerDataManager.listen(
      S2CProtocol.U_C_REQUEST_GUILDLIST,
      this,
      this.__guildListHandler,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_SEND_NEEDOFFER,
      this,
      this.__needOfferHandler,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_REQUEST_GUILDCHALLENGE,
      this,
      this.__guildChallengeHanlder,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_GUILDWAR_TEAM,
      this,
      this.__guildWarTeamHandler,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_GUILDLAST_WEEK_ORDER,
      this,
      this.__getGuildWarOrderHandler,
    );
  }
  ...
  private __guildWarTeamHandler(pkg: PackageIn): void {
    let msg: GuildGroupListMsg = pkg.readBody(
      GuildGroupListMsg,
    ) as GuildGroupListMsg;
    let arr: any[] = [];
  ...
  private __getGuildWarOrderHandler(pkg: PackageIn): void {
    ...
    this._model.guildOrderDate = DateFormatter.parse(
      msg.orderTime,
      "YYYY-MM-DD hh:mm:ss",
    );
  ...
  private __guildChallengeHanlder(pkg: PackageIn): void {
    let msg: GuildChallengeOrderListMsg = pkg.readBody(
      GuildChallengeOrderListMsg,
    ) as GuildChallengeOrderListMsg;

    let challenges: any[] = [];
    for (let i = 0, len = msg.challenge.length; i < len; i++) {
      const chInfo: GuildChallengeInfoMsg = msg.challenge[
        i
      ] as GuildChallengeInfoMsg;
