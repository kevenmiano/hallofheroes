...
export default class GemMazeWnd extends BaseWindow {
  ...
  initLanguage() {
    this.txt1.text = LangManager.Instance.GetTranslation(
      "GemMazeWnd.currentPointTxt",
    );
    this.btn_rank.title = LangManager.Instance.GetTranslation(
      "mainBar.SmallMapBar.rankingBtnTipData",
    );
    this.btn_adjust.title =
      LangManager.Instance.GetTranslation("GemMazeWnd.tidyTxt");
    this.btn_add_count.getChild("txt1").asTextField.text =
      LangManager.Instance.GetTranslation(
        "map.outercity.view.frame.AttackAlertFrame.command02",
      );
  }
  ...
  private addEvent() {
    ...
    GemMazeManager.Instance.addEventListener(
      GemMazeEvent.GEMMAZE_GET_INIT_INFO,
      this.onGetInitInfo,
      this,
    );
    GemMazeManager.Instance.addEventListener(
      GemMazeEvent.GEMMAZE_UPDATE_BAG,
      this.updateBagView,
      this,
    );
    NotificationManager.Instance.addEventListener(
      NotificationEvent.ARMYFRAMEII_OPEN_FOR_GEMMAZE,
      this.onBagOpen,
      this,
    );
    NotificationManager.Instance.removeEventListener(
      NotificationEvent.ARMYFRAMEII_CLOSE_FOR_GEMMAZE,
      this.onBagClose,
      this,
    );
    ...
    GemMazeManager.Instance.addEventListener(
      GemMazeEvent.GEMMAZE_PLAYER_GRAIN,
      this.onPlayerGrain,
      this,
    );
  }
  ...
  updateView() {
    let gemInfo = GemMazeManager.Instance.model.gemMazeInfo;
    this.btn_add_count.getChild("txt_count").asTextField.text =
      gemInfo.timesLeft + "";
  ...
  private helpBtnClick() {
    ...
    let content = LangManager.Instance.GetTranslation(
      "gemMaze.GemMazeFrame.helpFrame.helpContent",
    );
  ...
  onAddCount() {
    ...
    if (GemMazeManager.Instance.model.gemMazeInfo.timesLeft >= model.maxStep) {
      MessageTipManager.Instance.show(
        LangManager.Instance.GetTranslation("gemMaze.gemMazeFrame.BuTimesTips"),
      );
      return;
    }
    ...
    if (
      ...
      model.buySteps * model.priceStep
    ) {
  ...
  private showBuyTimesTips() {
    let model = GemMazeManager.Instance.model;
    let str =
      model.gemMazeInfo.buyTimesLeft +
      "/" +
      TempleteManager.Instance.getConfigInfoByConfigName("Gem_Maze_BuyNum")
        .ConfigValue;
    let content: string = LangManager.Instance.GetTranslation(
      "gemMaze.GemMazeFrame.buyTimesTips",
      model.priceStep * model.buySteps,
      model.buySteps,
      str,
    );
