...
export class GameSocket extends Laya.EventDispatcher {
  ...
  private static KEY: Array<number> = [
    0xae, 0xbf, 0x56, 0x78, 0xab, 0xcd, 0xef, 0xf1,
  ];
  public static NEW_KEY: Array<number> = [
    0xae, 0xbf, 0x56, 0x78, 0xab, 0xcd, 0xef, 0xf1,
  ];
  ...
  public send(
    ...
    if (this._encrypted) {
      ...
      for (let i: number = 0; i < pkg.length; i++) {
        if (i > 0) {
          GameSocket.SEND_KEY._byteView_[i % 8] =
            (GameSocket.SEND_KEY._byteView_[i % 8] + pkg._byteView_[i - 1]) ^ i;
          pkg._byteView_[i] =
            (pkg._byteView_[i] ^ GameSocket.SEND_KEY._byteView_[i % 8]) +
            pkg._byteView_[i - 1];
        } else {
          pkg._byteView_[0] =
            pkg._byteView_[0] ^ GameSocket.SEND_KEY._byteView_[0];
        }
      }
    }
  ...
  public decrptBytes(src: ByteArray, len: number, key: ByteArray): ByteArray {
    ...
    for (i = 0; i < len; i++) {
      if (i > 0) {
        key._byteView_[i % 8] =
          (key._byteView_[i % 8] + src._byteView_[i - 1]) ^ i;
        result._byteView_[i] =
          (src._byteView_[i] - src._byteView_[i - 1]) ^ key._byteView_[i % 8];
      } else {
