...
export class PvpShopItem extends FUI_ShopItem {
  ...
  set info(value: ShopGoodsInfo) {
    ...
    if (this._info) {
      ...
      var honorTemp: t_s_upgradetemplateData = this.getHonorNameByValue(
        this._info.NeedGeste,
      );
  ...
  public getHonorNameByValue(honer: number): t_s_upgradetemplateData {
    var tempList: any[] = TempleteManager.Instance.getTemplatesByType(
    ...
    for (var i: number = 0; i < tempList.length; i++) {
      ...
      if (honer >= temp.Data) return temp;
    }
    return null;
  }
