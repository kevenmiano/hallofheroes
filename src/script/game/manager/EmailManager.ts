...
export default class EmailManager extends GameEventDispatcher {
  ...
  private addEvent() {
    ServerDataManager.listen(
      S2CProtocol.U_C_MAILITEM_MOVE_RESULT,
      this,
      this.__attachedHandler,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_MAIL_LIST,
      this,
      this.__attachedMailListUpdate,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_MAIL_INIT_LIST,
      this,
      this.__initMailList,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_MAIL_READ_RESP,
      this,
      this.__readUpdateList,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_MAIL_DEL_RESULT,
      this,
      this.__mailDelResult,
    );
  }
  ...
  private __initMailList(pkg: PackageIn) {
    ...
    for (let index = 0; index < count; index++) {
      ...
      if (mInfo && !this.mailModel.checkIsOutDate(mInfo)) {
        //检测是否过期
        dic.push(mInfo);
      }
    }
  ...
  private __mailDelResult(pkg: PackageIn) {
    ...
    if (msg.delList.length != 0) {
      ...
      var str: string = LangManager.Instance.GetTranslation(
        "emailII.EmailControler.command03",
      );
  ...
  private __attachedMailListUpdate(pkg: PackageIn) {
    ...
    for (let i: number = 0; i < msg.mailInfos.length; i++) {
      let mInfo: EmailInfo = EmailHelp.transferToMailInfo(
        msg.mailInfos[i] as MailInfoMsg,
      );
      if (mInfo && this.mailModel.allList[mInfo.Id]) {
        //如果存在就是更新老邮件的信息
  ...
  private __attachedHandler(pkg: PackageIn) {
        ...
        if (data && data.hasGoods) {
          if (data.MailType == EmailType.STAR_MAIL) {
            str = LangManager.Instance.GetTranslation(
              "yishi.manager.EmailManager.command02",
            );
            MessageTipManager.Instance.show(str);
          }
          if (data.MailType == EmailType.PET_MAIL) {
            ...
            str = LangManager.Instance.GetTranslation(
              "yishi.manager.EmailManager.command03",
            );
            MessageTipManager.Instance.show(str);
          }
          return;
    ...
    if (success && msg.pickupResult.length > 0) {
      str = LangManager.Instance.GetTranslation(
        "yishi.manager.EmailManager.command04",
      );
      MessageTipManager.Instance.show(str);
    }
  }
  ...
  public get mailModel(): MailModel {
    ...
    if (mailCtrl) return mailCtrl.data;
    return null;
  }
