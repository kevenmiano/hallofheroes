...
export default class WishPoolModel extends GameEventDispatcher {
  ...
  private init() {
    ...
    this.allWishPoolArr = ArrayUtils.sortOn(
      this.allWishPoolArr,
      "Id",
  ...
  initFashionGoods(wishingpoolData: t_s_wishingpoolData) {
    ...
    for (const dicKey in itemTemplateDic) {
      if (itemTemplateDic.hasOwnProperty(dicKey)) {
        ...
        if (temp && GoodsManager.Instance.isFashion(temp)) {
          //是时装
    ...
    fashionGoodsArr = ArrayUtils.sortOn(
      fashionGoodsArr,
      "sortNumber",
      ArrayConstant.NUMERIC,
    );
  ...
  private checkPassFashion(
    transformId: number,
    wishingpoolData: t_s_wishingpoolData,
  ): boolean {
  ...
  initMountGoods(wishingpoolData: t_s_wishingpoolData) {
    ...
    let mountTemplateDic = ConfigMgr.Instance.getDicSync(
      ...
      if (mountTemplateDic.hasOwnProperty(dicKey)) {
        ...
        if (temp) {
          if (
            ...
            if (
              itemtemplateData &&
              itemtemplateData.Property2 == -1 &&
              this.checkPassMount(temp.SoulScore, wishingpoolData)
            ) {
