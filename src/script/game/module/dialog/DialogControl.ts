...
export class DialogControl extends GameEventDispatcher implements Disposeable {
    ...
    private async show() {
        if (this._model && this._model.hasNextInfo() && SceneManager.Instance.currentType == this._model.scene) {
    ...
    private hide() {
        ...
        if (this._model && SceneManager.Instance.currentType != this._model.scene) {
            Logger.log(this._model, "dialog scene : " + this._model.scene + " current scene : " + SceneManager.Instance.currentType);
        }
        if (this._view) {
            this.dispose();
        }
        ...
        if (this._model && this._model.hasNextInfo() && SceneManager.Instance.currentType == this._model.scene) {
            ...
            if (StringUtils.isEmpty(msg.event)) {
                if (!this._view) {
                    ...
                    await UIManager.Instance.ShowWind(EmWindow.Dialog, dataObj).then((ret) => {
                        this._view = ret;
                    });
                }
    ...
    private dispatchDialogEvent(info: DialogMessageInfo) {
        let data: Object = { callBack: this.gotoAndNextMessage.bind(this), data: info };
