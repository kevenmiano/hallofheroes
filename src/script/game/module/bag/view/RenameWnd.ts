...
export class RenameWnd extends BaseWindow {
  ...
  private addEventListener() {
    ...
    ServerDataManager.listen(
      S2CProtocol.U_C_PLAYER_RENAME_RSP,
      this,
      this.__playerRenameRspHandler,
    );
  }
  ...
  private initView() {
    this.txt_prompt.text = LangManager.Instance.GetTranslation(
      "RenameWnd.promptTxt",
    ); // LangManager.Instance.GetTranslation("noviceII.node.AddUserNameFrame.describt01");
  ...
  private loadCheckNickname() {
    ...
    if (this._nickName == "") {
      str = LangManager.Instance.GetTranslation(
        "noviceII.node.AddUserNameFrame.describt02",
      );
    ...
    if (length < this.MIN_LENGTH) {
      str = LangManager.Instance.GetTranslation(
        "noviceII.node.AddUserNameFrame.describt03",
      );
      MessageTipManager.Instance.show(str);
      return;
    }
    if (length > this.MAX_LENGTH) {
      str = LangManager.Instance.GetTranslation(
        "noviceII.node.AddUserNameFrame.describt04",
      );
      MessageTipManager.Instance.show(str);
      return;
    }
    ...
    if (
      FilterWordManager.containUnableChar(this._nickName) ||
      FilterWordManager.filterWrod(this._nickName) != this._nickName
    ) {
      this.txt_prompt.text = LangManager.Instance.GetTranslation(
        "noviceII.node.AddUserNameFrame.describt05",
      );
          ...
          if (json.Result.value == "true") {
            switch (this._type) {
              case RoleModel.TYPE_COMPOSE:
                SocketSendManager.Instance.sendRename(
                  this._nickName,
                  RoleModel.TYPE_COMPOSE,
                );
                break;
              case RoleModel.TYPE_RENAME_CARD:
                if (this._data) {
                  ...
                  MessageTipManager.Instance.show(
                    LangManager.Instance.GetTranslation("RenameWnd.lackCard"),
                  );
                }
                break;
            }
  ...
  private __playerRenameRspHandler(pkg: PackageIn) {
    let msg: PlayerRenameRspMsg = pkg.readBody(
      PlayerRenameRspMsg,
    ) as PlayerRenameRspMsg;
    if (msg.result) {
      this.OnBtnClose();
    } else {
      this.btn_confirm.enabled = true;
