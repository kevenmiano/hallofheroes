...
export class StarTips extends BaseTips {
    ...
    private initView() {
        ...
        switch (this.type) {
            case StarBagType.TEMP:
                ...
                this.btnEquip.title = LangManager.Instance.GetTranslation("public.pickUp")
                break;
            ...
            case StarBagType.PLAYER:
                ...
                this.btnLock.title = LangManager.Instance.GetTranslation(this.locked ? "public.unLock" : "public.lock")
                this.btnEquip.title = LangManager.Instance.GetTranslation(this.type == StarBagType.THANE ? "public.unEquip" : "public.equip")
                break;
        ...
        this.txt_useLevel.text = LangManager.Instance.GetTranslation("public.level2", String(this.tipData.grade))
        let color = FilterFrameText.Colors[eFilterFrameText.StarQuality][this.tipData.template.Profile - 1];
    ...
    private initAttribute(info: StarInfo, template: t_s_startemplateData) {
        ...
        this.updateAttributeTxt(this.intellect, str , template.Intellect * info.grade);
        ...
        this.updateAttributeTxt(this.magicAttack, str , template.MagicAttack * info.grade);
        str = LangManager.Instance.GetTranslation("armyII.ThaneAttributeView.Tip16");
        this.updateAttributeTxt(this.magicDefence, str , template.MagicDefence * info.grade);
        ...
        if (this.tipData.template.Profile >= 2) {
            let maxGrade = StarHelper.getStarMaxGradeByProfile(this.tipData.template.Profile);
            if (this.tipData.grade >= maxGrade) {
                this.txt_gp.text = LangManager.Instance.GetTranslation("yishi.view.tips.goods.StarTip.gp01");
            }
            else {
                let nextGradeExp = StarHelper.getStarExp(this.tipData.grade + 1, this.tipData.template.Profile);
                if (this.tipData.template.Profile == 6)
                    this.txt_gp.text = LangManager.Instance.GetTranslation("yishi.view.tips.goods.StarTip.gp02", this.tipData.gp);
                else
                    this.txt_gp.text = LangManager.Instance.GetTranslation("yishi.view.tips.goods.StarTip.gp02", this.tipData.gp + "/" + nextGradeExp);
            }
        }
    }

    private updateAttributeTxt(item: GoodAttributeItem, proper: string, val: number) {
        item.visible = val > 0
        if (val > 0) {
    ...
    private btnNewComposeClick() {
        if (this.tipData.grade >= StarHelper.getStarMaxGradeByProfile(this.tipData.template.Profile)) {
            MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("starTip.ComposeMaxLevel", this.tipData.template.TemplateNameLang));
        } else {
            NotificationManager.Instance.dispatchEvent(StarEvent.STAR_NEW_COMPOSE, { pos: this.tipData.pos, compose: true });
        }
    ...
    private btnLockClick() {
        ...
        switch (this.type) {
            ...
            case StarBagType.PLAYER:
                //加锁或解锁
                this.starManager.sendStarComposeLock(this.tipData.bagType, this.tipData.pos);
                break;
        }
        this.hide()
    }
    ...
    private btnEquipClick() {
        ...
        switch (this.type) {
            case StarBagType.THANE:
                ...
                if (unEquipPos < 0) {
                    MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("cell.view.starbag.StarBagCell.command04"));
                } else {
                    StarManager.Instance.starMove(this.tipData.pos, StarBagType.THANE, unEquipPos, StarBagType.PLAYER);
                }
                break;
            case StarBagType.PLAYER:
                //装备  
                if (this.tipData.template.Profile == 6) {
                    MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("cell.view.starbag.StarBagCell.command01"));
                } else {
                    ...
                    //     //不存在相同的星运
                    // }
                    NotificationManager.Instance.dispatchEvent(StarEvent.STAR_EXCHANGE, { starInfo: this.tipData });
                }
                break;
        }
    ...
    private btnComposeClick() {
        if (this.tipData.grade >= StarHelper.getStarMaxGradeByProfile(this.tipData.template.Profile)) {
            ...
            NotificationManager.Instance.dispatchEvent(StarEvent.STAR_COMPOSE, { pos: this.tipData.pos, compose: true });
        }
    ...
    private btnExchangeClick() {
        NotificationManager.Instance.dispatchEvent(StarEvent.STAR_LEFT_EXCHANGE, { starInfo: this.tipData });
        this.OnClickModal();
    }
    ...
    private btnPickClick() {
        ...
        if (playerStarNum >= PlayerModel.ORIGINAL_OPEN_BAG_COUNT + DataCommonManager.playerInfo.starBagCount) {
            MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("star.view.StarIconView.command01"));
        } else {
    ...
    private sell(isPlayBag: boolean = false) {
        ...
        let content = LangManager.Instance.GetTranslation("StarBagWnd.SellStarTip", this.tipData.template.SellGold)
        SimpleAlertHelper.Instance.Show(null, null, null, content, null, null, (b: boolean) => {
            if (b) {
