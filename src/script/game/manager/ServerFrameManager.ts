...
export class ServerFrameManager {
  ...
  public static get Instance(): ServerFrameManager {
    if (!ServerFrameManager._instance)
      ServerFrameManager._instance = new ServerFrameManager();
  ...
  public createFrame($pkg: PackageIn, $callBack: Function) {
    let msg: CampaignNotifyMsg = $pkg.readBody(
      CampaignNotifyMsg,
    ) as CampaignNotifyMsg;
      ...
      case ServerFrameManager.DIALOG_FRAME: {
        throw new Error(
          LangManager.Instance.GetTranslation(
            "yishi.manager.ServerFrameManager.Error",
          ),
        );
