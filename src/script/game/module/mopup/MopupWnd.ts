...
export default class MopupWnd extends BaseWindow {
  ...
  OnShowWind() {
    ...
    if (
      this.frameData.type != eMopupType.CampaignMopup &&
      this.frameData.type != eMopupType.PetCampaignMopup
    ) {
      this.txtCountDesc.text = LangManager.Instance.GetTranslation(
        "MopupWnd.SweepLayer",
      );
    }
    if (this.frameData.type == eMopupType.PetCampaignMopup) {
      if (this.frameData.state == eMopupState.PetCampaignMopupping) {
        this._levelData = this.getUIPlayLevelListByUiLevelId(
          this.frameData.UiLevelId,
        );
      } else {
        this._levelData = this.frameData.levelData;
      }
      if (this._levelData.ConsumeType != 0) {
        //不是消耗体力
  ...
  // 添加提示Item
  private addTipItem(
    type: eMopupItemType = eMopupItemType.Mopuping,
  ): MopupItem {
  ...
  private __refreshMopupingList() {
    ...
    for (let index = 0; index < resultInfoList.length; index++) {
      ...
      item.info = itemData;
      item.itemType =
        itemData.type == 2 ? eMopupItemType.Mopuping : eMopupItemType.Mopuped;
      Logger.xjy(
        "[MopupWnd]__refreshMopupingList",
        this._accumCount,
        itemData.items,
      );
      item.setTitle(String(this._accumCount), this.frameData.type);
      ...
      if (
        (curCount > 0 && curCount == maxCount && itemData.type != 2) ||
  ...
  private __totalTimer() {
    ...
    this.txtLeftTime.text = DateFormatter.getConsortiaCountDate(
      this._remainTimeCnt,
    );
  }
  ...
  private __timerComplete() {
    ...
    if (this.frameData.state == eMopupState.CampaignMopuping) {
      this.ctrl.sendMopupInfo(
        eMopupType.CampaignMopup,
        eMopupOptType.GetResult,
        this._campaignTemp.CampaignId,
      );
      ...
      this.ctrl.sendMopupInfo(
        eMopupType.PetCampaignMopup,
        eMopupOptType.GetResult,
        this._levelData.UiPlayId,
      );
    }
  ...
  private dealCampaignData() {
    ...
    this._campaignTemp = ConfigMgr.Instance.getTemplateByID(
      ConfigType.t_s_campaign,
      this._campaignId,
    );
    if (this._campaignTemp)
      this.txtFrameTitle.text = this._campaignTemp.CampaignNameLang;
    this._costTime = Number(
      TempleteManager.Instance.getConfigInfoByConfigName("Campaign_Sweep_Time")
        .ConfigValue,
    );
    this._costGold = Number(
  ...
  private dealMazeData() {
    ...
    if (towerInfo.campaignId == 8101 || this.model.campaignId == 8101) {
      this.txtFrameTitle.text = LangManager.Instance.GetTranslation(
        "mopup.view.MazeMopupPrepareFrame.AbyssTitleText",
      );
    } else if (towerInfo.campaignId == 8001 || this.model.campaignId == 8001) {
      this.txtFrameTitle.text = LangManager.Instance.GetTranslation(
        "mopup.view.MazeMopupPrepareFrame.titleText",
      );
    }

    this._costTime = Number(
      TempleteManager.Instance.getConfigInfoByConfigName("Tower_Sweep_Time")
        .ConfigValue,
    );
    this._costGold = Number(
      TempleteManager.Instance.getConfigInfoByConfigName("Tower_Sweep_Gold")
        .ConfigValue,
    );
    let index = 0;
    if (this.model.towerInfo.maxIndex) index = this.model.towerInfo.maxIndex;
  ...
  private dealPetCampaignData() {
    let str: string;
    let baseDic: Map<number, t_s_uiplaybaseData> =
      ConfigMgr.Instance.getDicSync(ConfigType.t_s_uiplaybase);
    ...
    this.txtFrameTitle.text =
      str +
      "-" +
      LangManager.Instance.GetTranslation(
        "petCampaign.Level",
        this._levelData.UiLevelSort,
      );
    this._costTime = Number(
      TempleteManager.Instance.getConfigInfoByConfigName("Campaign_Sweep_Time")
        .ConfigValue,
    ...
    //根据黄金来计算最大的扫荡次数
    let goldMaxCount1: number = Math.floor(
      ResourceManager.Instance.gold.count / this._levelData.ContinuityGold,
    );
    ...
    if (this._levelData.ConsumeType == 0) {
      ...
      //额外消耗物品
      totalCount = GoodsManager.Instance.getGoodsNumByTempId(
        this._levelData.ConsumeType,
      );
  ...
  private __onTxtChange(evt: Event) {
    ...
    this.txtCostGold.text = this.hasMopupBuff
      ? "0"
      : String(count * this._costGold);
    this.txtCostTime.text = DateFormatter.getConsortiaCountDate(
      count * this._costTime,
    );
    ...
    switch (this.frameData.type) {
      ...
      case eMopupType.PetCampaignMopup:
        ...
        this.txt_rewardCount.text = LangManager.Instance.GetTranslation(
          "PetCampaignCostRewardTimes",
          count,
        );
  ...
  private dealPetCampaignClick() {
    ...
    if (count <= 0) {
      MessageTipManager.Instance.show(
        LangManager.Instance.GetTranslation("MopupWnd.SweepLayerTip"),
      );
    ...
    this.ctrl.sendMopupInfo(
      ...
      eMopupOptType.Mopup,
      this._levelData.UiPlayId,
      this.model.mopupCount,
      this._levelData.UiLevelId,
      1,
    );
    return false;
  }
  ...
  private dealCampaignClick(): boolean {
    ...
    if (this.model.weary < count * MopupData.WEARY) {
      let str = LangManager.Instance.GetTranslation(
        "mopup.MopupManager.mopupTipData03",
      );
    ...
    if (Number(this.txtCostGold) > ResourceManager.Instance.gold.count) {
      let str = LangManager.Instance.GetTranslation(
        "mopup.MopupManager.mopupTipData02",
      );
      MessageTipManager.Instance.show(str);
      return true;
    }
    ...
    this.ctrl.sendMopupInfo(
      ...
      0,
      1,
      this.btnTickSilverBox.selected,
      this.btnTickMysteryBox.selected,
    );
  ...
  private dealMazeClick(): boolean {
    ...
    if (count <= 0) {
      let str = LangManager.Instance.GetTranslation(
        "mopup.view.MazeMopupPrepareFrame.layerTipData1",
      );
      MessageTipManager.Instance.show(str);
      this.stepper.resetStepper(1);
      this.__onTxtChange(null);
      ...
      let str = LangManager.Instance.GetTranslation(
        "mopup.view.MazeMopupPrepareFrame.layerTipData2",
      );
      MessageTipManager.Instance.show(str);
      ...
      let str = LangManager.Instance.GetTranslation(
        "mopup.view.MazeMopupPrepareFrame.layerTipData3",
      );
      MessageTipManager.Instance.show(str);
      this.stepper.resetStepper(this._maxMopupCount);
      this.__onTxtChange(null);
    ...
    this.ctrl.sendMopupInfo(
      eMopupType.MazeMopup,
      eMopupOptType.Mopup,
      this.model.towerInfo.campaignId,
      0,
      this.model.mopupLayer,
      this.model.isDoubleProfit,
    );
  ...
  private setPage(mopuping: boolean = false) {
    ...
    if (this.frameData.type == eMopupType.CampaignMopup) {
      this.frameData.state = mopuping
        ? eMopupState.CampaignMopuping
        : eMopupState.CampaignMopupPre;
    } else if (this.frameData.type == eMopupType.PetCampaignMopup) {
      this.frameData.state = mopuping
        ? eMopupState.PetCampaignMopupping
        : eMopupState.PetCampaignMopupPre;
    } else {
      this.frameData.state = mopuping
        ? eMopupState.MazeMopuping
        : eMopupState.MazeMopupPre;
    }
  ...
  public speedImmediately() {
    if (this.model.mopupEnd) {
      let str: string = LangManager.Instance.GetTranslation(
        "mopup.view.MazeMopupIngFrame.mopupEnd",
      );
      MessageTipManager.Instance.show(str);
    } else {
      if (this.frameData.state == eMopupState.CampaignMopuping) {
  ...
  private btnSpeedClick() {
    ...
    if (
      VIPManager.Instance.model.isOpenPrivilege(VipPrivilegeType.SPEEDUP_SWEEP)
    ) {
    ...
    let cfgItem =
      TempleteManager.Instance.getConfigInfoByConfigName("FastSweep_Price");
    if (cfgItem) {
    ...
    let content = LangManager.Instance.GetTranslation(
      "mopup.view.MazeMopupIngFrame.AccelerateTip",
      acceleratePoint,
    );
    ...
    SimpleAlertHelper.Instance.Show(
      ...
      null,
      null,
      this.__btnSpeedClick.bind(this),
    );
  }
  ...
  private __btnSpeedClick(b: boolean, check: boolean) {
    ...
    if (this.model.mopupEnd) {
      ...
      if (
        this.frameData.state == eMopupState.CampaignMopuping ||
  ...
  private btnHelpClick() {
    let content = LangManager.Instance.GetTranslation(
      "mopup.view.MazeMopupPrepareFrame.mopupTipTxt",
    );
    UIManager.Instance.ShowWind(EmWindow.Help, { content: content });
  }

  private getUIPlayLevelListByUiLevelId(
    UiLevelId: number,
  ): t_s_uiplaylevelData {
  ...
  private get isMopuping(): boolean {
    return (
      this.frameData &&
      (this.frameData.state == eMopupState.CampaignMopuping ||
        this.frameData.state == eMopupState.MazeMopuping ||
        this.frameData.state == eMopupState.PetCampaignMopupping)
    );
  }
