...
export default class BaseWindow extends Laya.Sprite {
  ...
  public UICtrls: Map<string, fairygui.Controller> = new Map<
    string,
    fairygui.Controller
  >();
  ...
  public onCreate(type: EmWindow, param: any) {
    ...
    this.wndName = uiInfo.wndName
      ? uiInfo.wndName
      : ObjectTranslator.getClassName(this);
    this.uicfg = uiInfo;
    this.typeWin = type;
    this.params = param;
    ...
    if (this.contentPane == null) {
      ...
      if (!windObj) {
        Logger.error(
          "创建对象错误!!!   " + this.packName + " - " + this.wndName,
        );
        return false;
      }
      ...
      for (var index = 0; index < this.contentPane.numChildren; index++) {
        ...
        if (
          (elementName.indexOf("BACKGROUND") != -1 ||
            elementName.indexOf("BG") != -1) &&
          element.width >= 1000 &&
          element.height > 500
        ) {
      ...
      //窗口的标题
      // @ts-expect-error: frame is not recognized
      ...
      if (this.modal) {
        ...
        this.through = this.uicfg.mouseThrough
          ? this.uicfg.mouseThrough
          : false;
      }
  ...
  protected createModel() {
    if (!this.modelMask) this.modelMask = new Laya.Sprite();
  ...
  referenceNode(
    ...
    for (var index = 0; index < nodeChildCount; index++) {
      ...
      // 按钮节点全部引用至BaseWindow
      if (
        elementName.toUpperCase().indexOf("BTN") !== -1 &&
        element instanceof fgui.GButton
      ) {
  ...
  initButton() {
    this.recordBtnElement.forEach((element) => {
      ...
      if (
        element.name.toUpperCase() == "BTNCLOSE" ||
        element.name.toUpperCase() == "CLOSEBTN"
      ) {
  ...
  protected setCenter() {
    ...
    if (
      !this.resizeFullContent &&
      (Resolution.isWebVertical() || Utils.isQQHall())
    ) {
      let scale = BaseWindow.ScaleContentPane;
      this.contentPane.setScale(scale, scale);
      let scaleFix = (1 - scale) * 0.5;
      this.x =
        (Resolution.gameWidth - this.contentPane.sourceWidth) / 2 +
        this.contentPane.sourceWidth * scaleFix;
      this.y =
        (Resolution.gameHeight - this.contentPane.sourceHeight) / 2 +
        this.contentPane.sourceHeight * scaleFix;
    ...
    return new Promise((resolve) => {
      if (this.isNeedShowAnimation) {
        if (this.uicfg.EffectShow == EnUIShowType.NONE) {
          ...
          Laya.Tween.to(this.contentPane, { y: this.m_setPosY + 40 }, 250).to(
            this.contentPane,
            { y: this.contentPane },
            180,
            undefined,
  ...
  public KeepVerticalCenter(
    mainPart: Laya.Sprite,
    fromTop: number = 106,
    fromBottom: number = 106,
  ) {
  ...
  protected OnClickModal() {
    if (this.modelEnable) this.OnBtnClose();
  }
  ...
  setTitleKey(key: string) {
    if (this["frame"])
      this["frame"].getChild("title").text =
        LangManager.Instance.GetTranslation(key);
  }
