...
export default class VipCoolDownFrameWnd extends BaseWindow {
  ...
  public OnInitWind() {
    this.n3.text = LangManager.Instance.GetTranslation(
      "VipCoolDownFrameWnd.title",
    );
    this.n4.text = LangManager.Instance.GetTranslation(
      "VipCoolDownFrameWnd.n4",
    );
    this.n10.text = LangManager.Instance.GetTranslation(
      "VipCoolDownFrameWnd.n10",
    );
    this.n11.text = LangManager.Instance.GetTranslation(
      "VipCoolDownFrameWnd.n11",
    );
    this.addEvent();
    this.setCenter();
    ...
    this.group1.y = 55;
    let grade: number = 0;
    if (this._type == QueueItem.QUEUE_TYPE_ALCHEMY) {
      //挑战
      grade = VIPManager.Instance.model.getMinGradeHasPrivilege(
        VipPrivilegeType.PVP_PET_CHALLENGE_COOLDOWN,
      );
      if (grade > 0) {
        this.vipTxt.text = LangManager.Instance.GetTranslation(
          "VipCoolDownFrameWnd.vipTxt",
          grade,
        );
      }
    } else if (this._type == QueueItem.QUEUE_TYPE_COLOSSEUM) {
      grade = VIPManager.Instance.model.getMinGradeHasPrivilege(
        VipPrivilegeType.TAX_COOLDOWN,
      );
      if (grade > 0) {
        this.vipTxt.text = LangManager.Instance.GetTranslation(
          "VipCoolDownFrameWnd.vipTxt",
    ...
    if (
      ...
      this._point <= PlayerManager.Instance.currentPlayerModel.playerInfo.point
    ) {
