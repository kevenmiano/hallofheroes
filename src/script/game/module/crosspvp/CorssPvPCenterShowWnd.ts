...
import { NotificationEvent, CampaignEvent } from "../../constant/event/NotificationEvent";
...
export default class CorssPvPCenterShowWnd extends BaseWindow {
    ...
    private initView() {
        ServerDataManager.listen(S2CProtocol.U_C_CAMPAIGN_MATCHSTATE, this, this.__updateCrossBtnHanlder);
        this.initData();
    }
    ...
    private initShow(): boolean {
        ...
        if (this.roomInfo && this.roomInfo.isCross && this.roomInfo.playerCount < 4 
            && this.roomInfo.houseOwnerId == this.thane.userId && this.roomInfo.serverName == this.playerInfo.serviceName) {
            var mapTemp: t_s_mapData = TempleteManager.Instance.getMapTemplateById(CampaignManager.Instance.mapId);
            ...
            if (SceneManager.Instance.currentType == SceneType.CAMPAIGN_MAP_SCENE 
                && campaignTemp && campaignTemp.Capacity == 4 && (campaignTemp.DungeonId == 1 || campaignTemp.DungeonId == 2) && campaignTemp.Types == 0)//多人本
            {
                flag = true;
            }
        }
    ...
    public setCrossBtnState(state: number) {
        ...
        else if (state == 2) {//撮合中, 进行时间计算
            this.state.selectedIndex = 1;
            this._timeCount = 1;
            this.leftTimeTxt.text = LangManager.Instance.GetTranslation("roomHallwnd.cross.corssLeftTmeTxt", this._timeCount);
    ...
    private addEvent() {
        ...
        NotificationManager.Instance.addEventListener(NotificationEvent.CROSS_ADD_GOONBTN, this.initData, this);
        NotificationManager.Instance.addEventListener(CampaignEvent.CAMPAIGN_OVER, this.initData, this);
        this.roomInfo.addEventListener(RoomEvent.UPDATE_ROOM_BASE_DATA, this.initData, this);
    }

    private offEvent() {
        this.startBtn.offClick(this, this.startBtnHandler);
        this.cancelBtn.offClick(this, this.cancelBtnHandler);
        NotificationManager.Instance.removeEventListener(NotificationEvent.CROSS_ADD_GOONBTN, this.initData, this);
        NotificationManager.Instance.removeEventListener(CampaignEvent.CAMPAIGN_OVER, this.initData, this);
        this.roomInfo.removeEventListener(RoomEvent.UPDATE_ROOM_BASE_DATA, this.initData, this);
    }
     ...
     */
    startBtnHandler() {
        RoomSocketOutManager.sendCrossGoOn(1, this.playerInfo.userId, this.playerInfo.serviceName);
    }
     ...
     */
    cancelBtnHandler() {
        RoomSocketOutManager.sendCrossGoOn(0, this.playerInfo.userId, this.playerInfo.serviceName);
    }
    ...
    public OnHideWind() {
        ...
        ServerDataManager.cancel(S2CProtocol.U_C_CAMPAIGN_MATCHSTATE, this, this.__updateCrossBtnHanlder);
        super.OnHideWind();
