...
export default class HeroFightingUpdateAction extends MapBaseAction {
    ...
    update() {
        if (this.finished || SceneManager.Instance.currentType == SceneType.BATTLE_SCENE || PlayerManager.Instance.isExistNewbieMask) {
            this.dispose();
            return;
        }

        if (this._preFight > this._simpleInfo.fightingCapacity || this._count > 130 ||
        ...
        } else if (this._process < this._simpleInfo.fightingCapacity && this._view && this._view.parent) {
            if (this._view) this._view.setNumberView(this._process, this._delta);
        }

        if (this._process >= this._simpleInfo.fightingCapacity) {
            this._delta = 0;
            if (this._view) this._view.setNumberView(this._simpleInfo.fightingCapacity, this._delta);
        } else {
    ...
    private get speed(): number {
        if (this._speed == 0) {
            this._speed = Math.ceil((this._simpleInfo.fightingCapacity - this._preFight) / 50);
