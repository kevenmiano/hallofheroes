...
import { BottleEvent, NotificationEvent } from "../constant/event/NotificationEvent";
...
export class BottleManager extends GameEventDispatcher {
    ...
    private addEvent(): void {
        ServerDataManager.listen(S2CProtocol.U_C_BOTTLE_PASS, this, this.__updateBottleInfo);
        ServerDataManager.listen(S2CProtocol.U_C_BOTTLE_RESULT, this, this.__updateBottleResult);
        ServerDataManager.listen(S2CProtocol.U_C_BOTTLE_DROP_DATA, this, this.__updateBottleData);
        ServerDataManager.listen(S2CProtocol.U_C_BOTTLE_ACTIVE, this, this.__updateBottleUserInfomation);
    ...
    private __updateBottleInfo(pkg: PackageIn): void {
        ...
        if (msg.passType == 1) {
            if (this._model.bottleRewardsInfo) {
                this._model.bottleRewardsInfo = this._model.bottleRewardsInfo.concat(msg.infos);
            } else {
                this._model.bottleRewardsInfo = msg.infos;
            }
        } else if (msg.passType == 2) {//广播数据处理
            //
    ...
    private __updateBottleMsgs(msgs: string[]): string[] {
        ...
        for (let index = 0; index < count; index++) {
            ...
            if (items.length >= 3) {
                ...
                if (tempinfo)
                    props = props.replace("{0}", tempinfo.TemplateNameLang);
                count = items[2];
            }
            let langMsg = LangManager.Instance.GetTranslation("CastleServer.Bottle.Pass", users, props, count);
            temp.push(langMsg);
        }
        return temp;
    }
    ...
    private __updateBottleResult(pkg: PackageIn): void {
        ...
        this.dispatchEvent(BottleEvent.CT__UPDATE_BOX_STATUS, { "addCount": addCount, "alert": msg.alterBottle });
    }
    ...
    private __updateBottleData(pkg: PackageIn): void {
        let msg: BottleDropListMsg = pkg.readBody(BottleDropListMsg) as BottleDropListMsg;
        ...
        if (msg.score && msg.score.length > 0) {
            ...
            this._model.countRewardArr = ArrayUtils.sortOn(this._model.countRewardArr, "param", ArrayConstant.NUMERIC)
        }
        if (msg.storey && msg.storey.length > 0) {
            this._model.floorRewardArr = msg.storey as BottlePackage[];
            this._model.floorRewardArr = ArrayUtils.sortOn(this._model.floorRewardArr, "param", ArrayConstant.NUMERIC)
        }
        ...
        //删除开关表
        if (msg.open && PlayerManager.Instance.currentPlayerModel.sysCurTimeBySecond * 1000 < this._model.endTime) {
     ...
     */
    public sendOpenInfo(opType: number = 0, alterType: number = 0, useStick: boolean = true): void {
