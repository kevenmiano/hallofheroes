...
import { ActionLabesType, InheritIActionType } from "../../constant/BattleDefine";
...
export class MovePointAction extends GameBaseAction {
     ...
     */
    constructor($id: number, $tarPoint: Laya.Point, $speed: number, $leftTime: number = 0, $moveScene: boolean = false, $setScale: boolean = false,
        autoStand: boolean = true, $canReplace: boolean = false, endFun: Function = null, startFun: Function = null,
        playWalkAct: boolean = true, dustRes: string = null, dustGap: number = 100, moveForward: boolean = false) {
        super($moveScene);
    ...
    public update() {
        ...
        let leng: number = this._currentRole.point.distance(this._tarPoint.x, this._tarPoint.y);
        ...
        if (time < 1) {
            ...
            let x =  this._currentRole.point.x + (this._tarPoint.x - this._currentRole.point.x)*1/time
            let y =  this._currentRole.point.y + (this._tarPoint.y - this._currentRole.point.y)*1/time
    ...
    protected processBackDust() {
        ...
        if (this._dustRes && this._dustRes != "") {
            let len = this._dustPt.distance(this._currentRole.point.x, this._currentRole.point.y);
            if (len > this._dustGap) {
                if (this._currentRole.point.x - this._dustPt.x < 0) {
                    dir = -1;
                }
                angle = Math.atan2(this._currentRole.point.y - this._dustPt.y, this._currentRole.point.x - this._dustPt.x);
    ...
    public prepare() {
        ...
        if (!this._moveForward && this._currentRole.actionMovie.currentLabel == ActionLabesType.READY) {
            this.finish()
            return;
        }
        ...
        this._dustPt = new Laya.Point(this._currentRole.point.x, this._currentRole.point.y)
        let leng = this._tarPoint.distance(this._currentRole.point.x, this._currentRole.point.y);
        this._frame = Math.abs(leng / this._speed);

        this._currentRole.moving = true;
        if (this._currentRole == this.battleModel.selfHero) {
            NotificationManager.Instance.sendNotification(BattleEvent.SELF_MOVE_START);
        }
    }
