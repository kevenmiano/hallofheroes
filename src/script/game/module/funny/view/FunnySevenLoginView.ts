...
export default class FunnySevenLoginView extends FUI_FunnySevenLoginView implements FunnyContent {
    ...
    initView() {
        ...
        this.timeDescTxt.text = LangManager.Instance.GetTranslation("giftbag.GiftBagItem.activeTime");
        this.leftTimeDescTxt.text = LangManager.Instance.GetTranslation("funny.FunnyRightView.active.remainTime2");
        this.dayDescTxt.text = LangManager.Instance.GetTranslation("FunnySevenLoginView.dayTxt");
        ...
        this.dayValueTxt.text = LangManager.Instance.GetTranslation("consortia.view.myConsortia.ConsortiaMemberItem.dayTip", this.awardLists[0].finishValue);
        this.list1.numItems = _itemList.length;
    }

    private initEvent() {
        this.list1.itemRenderer = Laya.Handler.create(this, this.onRenderList, null, false);
    }
    ...
    setRemainTime() {
        ...
        if (this._remainTime >= 60) {
            this.leftTimeValueTxt.text = DateFormatter.getFullTimeString(this._remainTime);
        } else if (this._remainTime > 0) {
            this.leftTimeValueTxt.text = DateFormatter.getFullDateString(this._remainTime);
        } else {
            this.leftTimeValueTxt.text = LangManager.Instance.GetTranslation("feedback.FeedBackItem.outDate");
        }
    }
     ...
     * */
    private setTimeText() {
        this.timeValueTxt.text = LangManager.Instance.GetTranslation("funny.FunnyRightView.active.timeText", DateFormatter.getMonthDayString(this._infoData.startTime, ".", false), DateFormatter.getMonthDayString(this._infoData.endTime, ".", false));
        this.setRemainTime();
    }
