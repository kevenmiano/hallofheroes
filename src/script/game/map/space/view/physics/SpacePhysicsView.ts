...
export class SpacePhysicsView extends MapPhysicsAttackingBase {
  ...
  protected initView() {
    ...
    if ((this.info as SpaceNode).moveToMapId != 0) {
      let mapInfo: t_s_mapData = ConfigMgr.Instance.getTemplateByID(
        ConfigType.t_s_map,
        String((<SpaceNode>this.info).moveToMapId),
      );
  ...
  public get resourcesPath(): string {
    if (
      SpaceNodeType.isYearNode(this.info.info.types) &&
      (this.info as SpaceNode).stateId == SpaceNode.STATE0
    ) {
      return "";
    }
  ...
  private get isTransfer(): boolean {
    if (
      (<SpaceNode>this.info).param1 == SpaceNodeType.PARAM_TRANSFER ||
