...
export class ConsortiaSocektSendManager {
     ...
     */
    public static searchConsortia(consortiaName: string, pageIndex: number, isNewOpen: boolean) {
     ...
     * @param useBind   
     */
    public static consortiaRename($name: string, same: boolean = false, useBind: boolean = true) {
    ...
    public static applyJoinConsortiaSend($id: number) {
        ...
        if (!isFirst && isAuto && (num + 24 * 3600) > PlayerManager.Instance.currentPlayerModel.sysCurTimeBySecond) {
            ...
            let content: string = LangManager.Instance.GetTranslation("Consortia.ConsortiaSocketOutManager.content", point);
        ...
        for (const key in model.applyList) {
            if (model.applyList.hasOwnProperty(key)) {
                ...
                if (item.consortiaId == consortiaId) {
                    str = LangManager.Instance.GetTranslation("consortia.ConsortiaControler.command02");
                    MessageTipManager.Instance.show(str);
                    return true;
                }
            }
        }
        if (ConsortiaSocektSendManager.thane.grades < ConsortiaModel.CREAT_NEEDED_GRADES) {
            str = LangManager.Instance.GetTranslation("consortia.ConsortiaControler.command03", ConsortiaModel.CREAT_NEEDED_GRADES);
            MessageTipManager.Instance.show(str);
            return true;
        }
        if (model.applyList.length >= ConsortiaModel.APPLY_MAX) {
            str = LangManager.Instance.GetTranslation("consortia.ConsortiaControler.command04");
            MessageTipManager.Instance.show(str);
            return true;
        }
        return false;
