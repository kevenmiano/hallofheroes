...
export class BattleManager extends GameEventDispatcher {
  ...
  public static onFightCanceled() {
    ...
    if (BattleManager.Instance.battleModel) {
      logStr =
        "战斗被取消了,battleId=" + BattleManager.Instance.battleModel.battleId;
      Logger.battle(
        logStr,
        "BattleManager.Instance.battleModel.mapId",
        BattleManager.Instance.battleModel.mapId,
      );
    } else {
    ...
    if (
      ...
      WorldBossHelper.checkPvp(BattleManager.Instance.battleModel.mapId)
    ) {
      Logger.battle(
        "-----onFightCanceled 在战场中时间到了取消战斗时直接退出到天空之城:",
        logStr,
      );
  ...
  public isAllSkillExecuted(): boolean {
    if (
      this._skillSystem &&
      this._skillSystem.getSkillQueue().getActions().length > 0
    ) {
