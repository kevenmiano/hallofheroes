...
export default class BeingInviteWnd extends BaseWindow {
	...
	OnShowWind() {
		...
		if (this._quick) {
			...
			this["btnConfirm"].title = LangManager.Instance.GetTranslation("public.confirm1");
			this["btnCancel"].title = LangManager.Instance.GetTranslation("public.cancel1");
		} else {
			this._autoExitTimer = new TimerTicker(1000, InviteData.BeingInvite_AutoExitTime);
			this._autoExitTimer.addEventListener(TimerEvent.TIMER, this.__timerHandler, this);
			this._autoExitTimer.start();
		}
		this.txtNotAcceptInvite.text = LangManager.Instance.GetTranslation("BeingInviteWnd.txtNotAcceptInvite");

		ServerDataManager.listen(S2CProtocol.U_C_MULITY_CAMPAIGN_REQUEST, this, this.__mulityCampaignRequestHandler);
		PlayerManager.Instance.currentPlayerModel.playerInfo.on(PlayerEvent.MUTICOPY_COUNT, this.refreshCurrentCnt, this);
		PlayerManager.Instance.currentPlayerModel.playerInfo.on(PlayerEvent.TAILA_COUNT, this.refreshCurrentCnt, this);
	}
	...
	OnHideWind() {
		super.OnHideWind();
		if (!this._quick) {
			this._autoExitTimer.removeEventListener(TimerEvent.TIMER, this.__timerHandler, this);
			this._autoExitTimer.stop();
		}
		SimpleAlertHelper.Instance.HideByType(SimpleAlertHelper.SIMPLE_ALERT);

		ServerDataManager.cancel(S2CProtocol.U_C_MULITY_CAMPAIGN_REQUEST, this, this.__mulityCampaignRequestHandler);
		PlayerManager.Instance.currentPlayerModel.playerInfo.off(PlayerEvent.MUTICOPY_COUNT, this.refreshCurrentCnt, this);
		PlayerManager.Instance.currentPlayerModel.playerInfo.off(PlayerEvent.TAILA_COUNT, this.refreshCurrentCnt, this);
	}

	private refreshCurrentCnt() {
		this._campaign = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_campaign, this.tempId);
		if (this._campaign) {
			...
			this.rTxtContent.text = this._content + LangManager.Instance.GetTranslation(showTick ? "invite.campaign.income" : "invite.campaign.income.color", this._curCount, this._maxCount);
		}
	}

	private __timerHandler() {
		let time: number = InviteData.BeingInvite_AutoExitTime - this._autoExitTimer.currentCount;
		// Logger.xjy("__timerHandler", this._autoExitTimer.currentCount, time)
		if (time <= 0) {
			time = 0;
		}
		this.txtCountDown.text = time == 0 ? "" : DateFormatter.getConsortiaCountDate(time);
		if (time == 0) {
			this.hide();
		}
	}
	...
	public setInviteCancelIncome(selected: boolean) {
		let roomHallCtrl = FrameCtrlManager.Instance.getCtrl(EmWindow.RoomHall) as RoomHallCtrl
		roomHallCtrl.data.inviteCancelIncome = !selected;
	}

	public get isSpecialFB() {
		return this._campaign.isTrailTower || this._campaign.isTaila || this._campaign.isKingTower;
	}
	...
	private __mulityCampaignRequestHandler(pkg: PackageIn) {
		...
		if (this.roomType != msg.param3) {
			refresh = true;
			MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("Invite.room.change.type"));
		}
		...
		if ((this.position == 1) != isInCampaign) {
			refresh = true;
			MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("Invite.room.change.state"));
		}

		if (this._campaign.AreaId != templateInfo.AreaId) {
			refresh = true;
			MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("Invite.room.change.campaign"));
		}

		if (this._campaign.DifficutlyGrade != templateInfo.DifficutlyGrade) {
			refresh = true;
			MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("Invite.room.change.difficutly"));
		}
		...
		if (refresh) {
			...
			// 构建副本等级范围字符串
			lvstr =
				LangManager.Instance.GetTranslation("public.level3", templateInfo.MinLevel) +
				"-" +
				LangManager.Instance.GetTranslation("public.level3", templateInfo.MaxLevel);
			...
			let nickName: string = msg.param6 ? "[color=#00ff00]" + msg.param6 + "[/color]" : "";

			// 竞技邀请
			if (roomInfo.roomType == 1) {
				titleText = LangManager.Instance.GetTranslation("yishi.manager.BaseManager.frame");
				content = nickName ? LangManager.Instance.GetTranslation("yishi.manager.BaseManager.content02", nickName, roomInfo.id) :
					LangManager.Instance.GetTranslation("yishi.manager.BaseManager.content022", roomInfo.id);
			}
			...
			// 普通副本邀请
				...
				let str: string = StringHelper.isNullOrEmpty(msg.param4) ? nameMap : msg.param4;
				str += " " + lvstr;
		...
		if (
			!this.isAlert(
				...
				(b: boolean, check: boolean) => {
					...
					if (this.position == 1) {
						var content: string = LangManager.Instance.GetTranslation("chat.view.ChatView.EnterCampaignTips");
						SimpleAlertHelper.Instance.Show(SimpleAlertHelper.SIMPLE_ALERT, null, null, content, null, null, this.__btnConfirmClick.bind(this));
						return;
					}
				...
				LangManager.Instance.GetTranslation("room.view.pve.RoomRightView.notUserIncomeWillNotGetReward")
			)
		) {
			if (this.position == 1) {
				var content: string = LangManager.Instance.GetTranslation("chat.view.ChatView.EnterCampaignTips");
	...
	private showThewAlert(callBack: any) {
		if (this._campaign && this._campaign.SonTypes != 0) {
			if (this._campaign.isKingTower) {
				...
				if (this._curCount < 1) {
					return this.isAlert(
						this._curCount,
						callBack,
						LangManager.Instance.GetTranslation("yishi.view.base.ThewAlertFrame.disclistTRIAL_CHOSE")
					);
				}
				return false;
			...
			//如果是普通多人副本
			return this.isAlert(this.playerInfo.multiCopyCount, callBack, LangManager.Instance.GetTranslation("yishi.view.base.ThewAlertFrame.disclist01"));
		}

		return false;
	}

	private isAlert(currentCount: number, callBack: Function, content: string): boolean {
		...
		if (flag && outdate) {
			...
			SimpleAlertHelper.Instance.Show(SimpleAlertHelper.SIMPLE_ALERT, { checkRickText: checkTxt }, null, content, null, null, callBack);
		}
		return flag && outdate;
	}
	...
	private btnCancelClick() {
		if (this.btnNotAcceptInvite.selected) {
			InviteTipManager.Instance.set(EmInviteTipType.Room, this.playerId.toString(), true)
		}
		this.OnBtnClose();
	}
}
