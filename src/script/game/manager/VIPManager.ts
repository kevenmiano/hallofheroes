...
export class VIPManager extends GameEventDispatcher {
  ...
  private initEvent() {
    ServerDataManager.listen(
      S2CProtocol.U_C_VIP_INFO,
      this,
      this._vipInfoHandler,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_OPENVIPROULETTE,
      this,
      this._openVipRouletteHandler,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_VIPROULETTE_RESULT,
      this,
      this._openVipRouletteResultHandler,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_VIP_OPENTIPS,
      this,
      this._vipOpenTipsHandler,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_VIP_GIFT_RSP,
      this,
      this._vipOpenGiftHandler,
    );
  }
  ...
  public get vipGiftState(): boolean {
    ...
    for (let index = 0; index < giftStates.length; index++) {
      ...
      if (
        (userVip >= element.vip_grade && element.dayGiftState == 1) ||
        element.isFreeGift
      ) {
        effectState = true;
        break;
