...
export default class BuildingManager extends GameEventDispatcher {
	...
	initEvent() {
		ServerDataManager.listen(S2CProtocol.U_C_BUILDINGINFO_LIST, this, this.__updateBuildInfoHandler);
		ServerDataManager.listen(S2CProtocol.U_C_BUILDING_ORDER_SEND, this, this.__loadBuildOrderHandler);
		ServerDataManager.listen(S2CProtocol.U_C_BUILDING_ORDER_QUICK, this, this.__orderCoolHandler);
		ServerDataManager.listen(S2CProtocol.U_C_BUILDING_ORDERADD, this, this.__addOrderHandler);
		ServerDataManager.listen(S2CProtocol.U_C_BUILDING_ORDER_INFO, this, this.__userOrderHandler);
		ServerDataManager.listen(S2CProtocol.U_C_BUILDING_UPENERGY, this, this.__transerAddPowerHandler);
		ServerDataManager.listen(S2CProtocol.U_C_UNOCCPWILDLAND, this, this.__unoccupyHandler);
		ServerDataManager.listen(S2CProtocol.U_C_GOLD_IMPOSERESULT, this, this.__levyUpdateHandler);
		ServerDataManager.listen(S2CProtocol.U_C_SECURITY_INFO_UPDATE, this, this.__politicsDataHandler);
	}
	...
	private __politicsDataHandler(pkg: PackageIn) {
		...
		for (let i: number = 0; i < castleMsg.wildYield.length; i++) {
			...
			fInfo.createDate = DateFormatter.parse(wildYieldMsg.createTime, "YYYY-MM-DD hh:mm:ss");
	...
	private __levyUpdateHandler(pkg: PackageIn) {
		...
		let info: BuildInfo = this.model.getBuildingInfoBySonType(BuildingType.OFFICEAFFAIRS);
		if (info) {
	...
	private __updateBuildInfoHandler(pkg: PackageIn) {
		...
		if (result) {
			for (const key in msg.building) {
				...
				if (info.templeteInfo.MasterType == MasterTypes.MT_INTERNALTECHNOLOGY
					|| info.templeteInfo.MasterType == MasterTypes.MT_WARTECHNOLOGY) {
					NotificationManager.Instance.sendNotification(PlayerEvent.UPDATE_EXTRABUFFER);
				}
			}
		}
	}
	...
	private __orderCoolHandler(pkg: PackageIn) {
		...
		for (let i: number = 0; i < count; i++) {
			...
			if (!order) {
				let str: string = LangManager.Instance.GetTranslation("yishi.manager.BuildingManager.command01");
	...
	private __unoccupyHandler(pkg: PackageIn) {
		...
		if (result != "" && result != null) {
			let str: string = LangManager.Instance.GetTranslation("yishi.manager.BuildingManager.command02");
			MessageTipManager.Instance.show(str);
		}
	...
	public creatBuilding(sonType: number) {
		let templete: t_s_buildingtemplateData = TempleteManager.Instance.getMinGradeBuildTemplate(sonType);
		let msg: string = ResourceManager.Instance.canSubtractResources(templete.GoldConsume, templete.CrystalsConsume);
		if (msg != null) {
			let str: string = LangManager.Instance.GetTranslation("yishi.manager.BuildingManager.command03", msg);
			MessageTipManager.Instance.show(str);
			return;
		} else {
			...
			msg = BuildingManager.Instance.model.canBuildOrUpgradeByBuildingInfo(bInfo);
			if (StringHelper.isNullOrEmpty(msg) == false) {
				let str: string = LangManager.Instance.GetTranslation("yishi.manager.BuildingManager.command04", msg);
				MessageTipManager.Instance.show(str);
				return;
			}
	...
	public checkBuildList(info: BuildInfo): boolean {
		if (this._model.buildOrderCount >= this._model.buildOrderTotal) {
			...
			if (this._model.buildOrderTotal >= this._model.buildOrderLimit) {
				str = LangManager.Instance.GetTranslation("yishi.manager.BuildingManager.command05");
				MessageTipManager.Instance.show(str);
				// QueueBar.instance.playItemEffect(QueueItem.QUEUE_TYPE_BUILD);
			}
			else {
				str = LangManager.Instance.GetTranslation("yishi.manager.BuildingManager.command06");
				MessageTipManager.Instance.show(str);
				// QueueBar.instance.playItemEffect(QueueItem.QUEUE_TYPE_BUILD);
			}
			return false;
	...
	public checkTecList(info: BuildInfo): boolean {
		if (this._model.tecOrderCount >= this._model.tecOrderTotal) {
			if (this._model.tecOrderTotal >= this._model.tecOrderLimit) {
				MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("yishi.manager.BuildingManager.checkTecList01"));
				// QueueBar.instance.playItemEffect(QueueItem.QUEUE_TYPE_TEC);
			}
			else {
				MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("yishi.manager.BuildingManager.checkTecList02"));
				// QueueBar.instance.playItemEffect(QueueItem.QUEUE_TYPE_TEC);
			}
			return false;
		}
	 ...
	 * 
	 */
	public allCoolBack(result: boolean, id: number = 0, type: number = 0, useBind: boolean = true) {
		if (result) {
			if (id != 0) {
				let order: BuildingOrderInfo = this._model.getOrderById(id);
	...
	private checkEnoughResourceNotTips(bInfo: BuildInfo): boolean {
		let nextTemp: t_s_buildingtemplateData = TempleteManager.Instance.getBuildTemplateByID(parseInt((bInfo.templeteInfo.NextGradeTemplateId).toString()));
		let msg: string = ResourceManager.Instance.canSubtractResources(nextTemp.GoldConsume, nextTemp.CrystalsConsume);
		if (StringHelper.isNullOrEmpty(msg) == false) {
			return false;
		}
		return true;
	...
	private checkPreBuild(bInfo: BuildInfo): boolean {
		...
		if (StringHelper.isNullOrEmpty(msg) == false) {
			let str: string = LangManager.Instance.GetTranslation("yishi.manager.BuildingManager.command07", msg);
			MessageTipManager.Instance.show(str);
			return false;
		}
		return true;
	...
	private checkPlayerGrade(bInfo: BuildInfo): boolean {
		let templete: t_s_buildingtemplateData = bInfo.templeteInfo;
		let nextTemp: t_s_buildingtemplateData = TempleteManager.Instance.getBuildTemplateByID(templete.NextGradeTemplateId);
		if (nextTemp && nextTemp.PlayerGrades > this.thane.grades) {
			let str: string = LangManager.Instance.GetTranslation("yishi.manager.BuildingManager.command08", nextTemp.PlayerGrades);
			MessageTipManager.Instance.show(str);
			return false;
		}
		return true;
	 ...
	 * 
	 */
	public isExistBuildBySontypeAndLevel(sontype: number, level: number): boolean {
	 ...
	 * 
	 */
	public getPreBuildingList(temp: t_s_buildingtemplateData): Array<t_s_buildingtemplateData> {
		let list: Array<t_s_buildingtemplateData> = new Array();
		let preTemplateId: number = temp.PreTemplateId;
		if (preTemplateId != 0) {
			let preTemplate: t_s_buildingtemplateData = TempleteManager.Instance.getBuildTemplateByID(preTemplateId);
	...
	public checkMilitaryTecMaxGrade(): boolean {
		...
		this._model.buildingListByID.getList().forEach((info) => {
			if (info.templeteInfo.MasterType == MasterTypes.MT_TECHNICAL
				&& info.templeteInfo.BuildingGrade < this.thane.grades - 3
				&& info.templeteInfo.NextGradeTemplateId > 0) flag = true;
		});
		return flag;
	}
