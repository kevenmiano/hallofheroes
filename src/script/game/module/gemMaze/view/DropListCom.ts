...
export default class DropListCom extends FUI_DropList {
  ...
  updateView(gemLevel: number) {
    if (this._gemLevel != gemLevel) {
      ...
      let array: t_s_dropconditionData[] =
        TempleteManager.Instance.getDropConditionByType(101);
      for (let i = 0; i < array.length; i++) {
        const element = array[i];
        if (element.Para1[0] == gemLevel) {
          let goodsIdArr: Array<t_s_dropitemData> =
            TempleteManager.Instance.getDropItemssByDropId(element.DropId);
  ...
  private onRenderList(index: number, item: DropListItem) {
    if (item) {
      ...
      item.txt_name.color = GoodsSonType.getColorByProfile(
