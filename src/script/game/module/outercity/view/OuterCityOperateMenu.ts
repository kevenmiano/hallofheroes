...
export class OuterCityOperateMenu extends BaseWindow {
  ...
  public OnShowWind() {
    ...
    let pos: Point = smallMap.view.displayObject.localToGlobal(
      new Point(smallMap.opBtn.x, smallMap.opBtn.y),
      true,
    );
  ...
  private onListItemClick(item: fgui.GButton, evt: Laya.Event) {
    ...
    switch (item.data) {
      ...
      case 2:
        //回城
        let selfVehicleView1: OutercityVehicleArmyView =
  ...
  private localCastleHandler(): void {
    ...
    if (castleInfo.info.mapId != OuterCityManager.Instance.model.mapId) {
      let str: string = LangManager.Instance.GetTranslation(
        "map.outercity.mediator.mapview.OuterCitySmallMapMediator.command02",
      );
      MessageTipManager.Instance.show(str);
      return;
    }
    if (castleInfo) {
      this.mapView.motionTo(
        new Point(
          castleInfo.x - StageReferance.stageWidth / 2,
          castleInfo.y - StageReferance.stageHeight / 2,
        ),
      );
    }
  }
  ...
  private removeDenseFog(): void {
    ...
    UIManager.Instance.ShowWind(EmWindow.UseGoodsAlert, {
      content: str,
      goodsId: ItemID.HOLY_LIGHT,
      goodsCount: goodsCount,
      callback: this.removeDenseFogCallBack.bind(this),
    });
  }
  ...
  private localArmyHandler(): void {
    ...
    if (selfVehicle) {
      this.mapView.motionTo(
        new Point(
          selfVehicle.x - StageReferance.stageWidth / 2,
          selfVehicle.y - StageReferance.stageHeight / 2,
        ),
      );
      return;
    }
    let aInfo: BaseArmy = OuterCityManager.Instance.model.getWorldArmyById(
      army.id,
    );
    if (!aInfo) {
      return;
    ...
    if (armyView) {
      this.mapView.motionTo(
        new Point(
          armyView.x - StageReferance.stageWidth / 2,
          armyView.y - StageReferance.stageHeight / 2,
        ),
      );
      return;
    }
