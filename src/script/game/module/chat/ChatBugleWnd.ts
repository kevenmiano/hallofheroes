...
export default class ChatBugleWnd extends BaseWindow {
    ...
    private onInitData() {
        this.frame.getChild('title').text = LangManager.Instance.GetTranslation("ChatBugleWnd.titleTxt");
    ...
    private refreshBugleCount() {
        var num: number = GoodsManager.Instance.getGoodsNumByTempId(ShopGoodsInfo.BIG_BUGLE_TEMP_ID);
    ...
    private crossBugleHandler(str: string, itemList, starList, cardList) {
        if (!this._flag) {
            let num: number = GoodsManager.Instance.getGoodsNumByTempId(ShopGoodsInfo.CROSS_BUGLE_TEMP_ID);
            if (num == 0) {
                var command: string = LangManager.Instance.GetTranslation("chat.view.ChatInputView.command06");
                MessageTipManager.Instance.show(command);
                var data: ShopGoodsInfo = TempleteManager.Instance.getShopTempInfoByItemId(ShopGoodsInfo.BIG_BUGLE_TEMP_ID);
                let obj = {
                    info: data,
                    count: 1,
    ...
    private bigBugleHandler(str: string, itemList, starList, cardList) {
        ...
        SimpleAlertHelper.Instance.Show(null, null, null, content, null, null, function (b) {
            if (b) {
    ...
    private shopBugleBack(flag: boolean, data: Object) {
        if (flag) {
            ...
            if (itemId == ShopGoodsInfo.SMALL_BUGLE_TEMP_ID) {
                ChatManager.Instance.chatBySmallBugle(this._currentChatStr, data[1], data[2], data[3]);
            }
            else if (itemId == ShopGoodsInfo.BIG_BUGLE_TEMP_ID) {
                ChatManager.Instance.chatByBugle(this._currentChatStr, data[1], data[2], data[3]);
            } else if (itemId == ShopGoodsInfo.CROSS_BUGLE_TEMP_ID) {
                ChatManager.Instance.chatByCross(this._currentChatStr, data[1], data[2], data[3]);
            }
            this._flag = false;
        }
        this.refreshBugleCount();
    ...
    private onSendBugle() {
        let item: t_s_itemtemplateData = TempleteManager.Instance.getGoodsTemplatesByTempleteId(ShopGoodsInfo.BIG_BUGLE_TEMP_ID);
        if (item) {
            if (item.NeedGrades > ArmyManager.Instance.thane.grades) {
                MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("chatWnd.bigBugle.tips", item.NeedGrades), undefined, true);
                return;
            }
        }
        ...
        if (chatStr == "" || StringHelper.trim(chatStr) == '' || StringHelper.trim(chatStr) == undefined) {//空字符串不发送
            var command: string = LangManager.Instance.GetTranslation("yishi.view.ConfirmSellFrame.inputNull");
