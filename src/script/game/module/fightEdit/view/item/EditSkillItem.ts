...
export class EditSkillItem extends FUI_EditSkillItem {
    ...
    public setData(job:number,value: t_s_skilltemplateData|t_s_runetemplateData,showMask:boolean=true) {
        ...
        if (this._skillData) {
            ...
            if(value instanceof t_s_skilltemplateData){
                this.iconLoader.icon = IconFactory.getTecIconByIcon((this._skillData as t_s_skilltemplateData).Icons);
                if (this.Img_Battle_S) {
                    this.Img_Battle_S.visible = SkillPriorityType.isSuperSkill((this._skillData as t_s_skilltemplateData).Priority);
                }
                tipData = value;
            }else
            {
                this.Img_Battle_S.visible = false;
                tipData = TempleteManager.Instance.getSkillTemplateInfoById((this._skillData as t_s_runetemplateData).SkillTemplateId);
                this.iconLoader.icon = IconFactory.getTecIconByIcon((this._skillData as t_s_runetemplateData).Icon);
            }
            let isExist:boolean=false;
            let idx:number;
            if(this.type == 0){
                idx = SkillEditModel.instance.getNormalSortIndex(job,this._skillData.TemplateId);
                if(idx != -1){
                ...
                idx = SkillEditModel.instance.getSpecialSortIndex(job,this._skillData.TemplateId);
                if(idx != -1){
                    this.txt_posIdx.text = (idx+1).toString();
                    this.imgMask.visible = this.txt_posIdx.visible =true;
                    this.touchable = false;
