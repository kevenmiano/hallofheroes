...
export default class WarlordRoomWnd extends BaseWindow {
  ...
  private initData() {
    ...
    this.tabList.selectedIndex = this.warlordsModel.selfInfo.isTempleGroup
      ? 0
      : 1;
  }
  ...
  private helpBtnHandler() {
    ...
    let content: string = LangManager.Instance.GetTranslation(
      "warlords.room.WarlordsRoomView.helpContent1",
    );
  ...
  private refreshTopView() {
    if (this.warlordsModel.curRound >= this.warlordsModel.totalRound) {
      ...
      switch (this.warlordsModel.process) {
        case WarlordsModel.PROCESS_PRELIM:
          this.openDescTxt1.text = LangManager.Instance.GetTranslation(
            "WarlordRoomWnd.TopView.openDescTxt3",
          );
          break;
        case WarlordsModel.PROCESS_FINAL:
          this.openDescTxt1.text = LangManager.Instance.GetTranslation(
            "WarlordRoomWnd.TopView.openDescTxt4",
          );
          break;
      }
    } else {
      ...
      this._roundStr =
        (this.warlordsModel.curRound < this.warlordsModel.totalRound
          ? this.warlordsModel.curRound + 1
          : this.warlordsModel.curRound) +
        "/" +
        this.warlordsModel.totalRound;
      switch (this.warlordsModel.process) {
        case WarlordsModel.PROCESS_PRELIM:
          this._titleName = LangManager.Instance.GetTranslation(
            "WarlordRoomWnd.TopView.openDescTxt2",
          );
          break;
        case WarlordsModel.PROCESS_FINAL:
  ...
  private updateTimeHandler() {
    ...
    this.openDescTxt2.text = LangManager.Instance.GetTranslation(
      "WarlordRoomWnd.TopView.openDescTxt1",
      this._titleName,
      this._roundStr,
      this._timeCount,
    );
  ...
  private refreshRightView() {
    switch (this.warlordsModel.process) {
      case WarlordsModel.PROCESS_PRELIM:
        this.type.selectedIndex = 1;
        this.myScoreTxt
          .setVar("count", this.warlordsModel.selfInfo.prelimScore.toString())
          .flushVars();
        this.myCountTxt
          .setVar(
            "count",
            (
              this.warlordsModel.totalRound - this.warlordsModel.curRound
            ).toString(),
          )
          .flushVars();
        break;
  ...
  private onSelectedChangeHandler() {
    ...
    if (
      (tabIndex == 0 && this.warlordsModel.selfInfo.isTempleGroup) ||
      (tabIndex == 1 && !this.warlordsModel.selfInfo.isTempleGroup)
    ) {
      this.myRankTxt.visible = this.victoryNumTxt.visible = true;
      this.myRankTxt
        .setVar("rank", this.warlordsModel.selfInfo.displaySort.toString())
        .flushVars();
      this.victoryNumTxt
        .setVar("num", this.warlordsModel.selfInfo.winCount.toString())
        .flushVars();
    } else {
