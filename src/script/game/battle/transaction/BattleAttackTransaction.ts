...
export class BattleAttackTransaction extends BattleBaseTransaction {
  ...
  public configure(param: object) {
    ...
    let msg: AttackOrderMsg = this._pkg.readBody(
      AttackOrderMsg,
    ) as AttackOrderMsg;
    ...
    Logger.battle(
      "收到攻击指令:",
      battleModel.getRoleById(skillData.fId) &&
        battleModel.getRoleById(skillData.fId).roleName,
      skillData,
    );
    ...
    let skillStrategy: number = SkillProcessStrategy.processSynchronization(
      msg.frame,
