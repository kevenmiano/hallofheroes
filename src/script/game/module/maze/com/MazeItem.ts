...
export default class MazeItem extends FUI_MazeItem {
  ...
  protected onConstruct(): void {
    super.onConstruct();
    this.MyRecordTxt.text = LangManager.Instance.GetTranslation(
      "MazeFrameWnd.MyRecordTxt",
    );
    this.CurrentFloorTxt.text = LangManager.Instance.GetTranslation(
      "MazeFrameWnd.CurrentFloorTxt",
    );
    this.TotalExpTxt.text = LangManager.Instance.GetTranslation(
      "MazeFrameWnd.TotalExpTxt",
    );
    this.DoubleProfitTxt.text = LangManager.Instance.GetTranslation(
      "MazeFrameWnd.GetDoubleProfitTxt",
    );
    ...
    this.Btn_MopupContinue.onClick(
      this,
      this.__continueMopupHandler.bind(this),
    );
    this.DoubleProfit.on(
      fgui.Events.STATE_CHANGED,
      this,
      this.__doubleProfitHandler,
    );
  }
    ...
    if (this._vdata.maxIndex == 0 && this._vdata.towerIndex == 0) {
      ...
      MessageTipManager.Instance.show(
        LangManager.Instance.GetTranslation(
          "MazeItem.DoubleProfitHandler.tips",
        ),
      );
    }
  }
  ...
  private initView() {
    this.TitleTxt.color =
      this._type == EmMazeType.GroundMaze ? "#A4CBE2" : "#F3D783";
    this.TitleTxt.text = LangManager.Instance.GetTranslation(
      "MazeFrame.BtnGroupName" + this._type,
    );
    this.TodayFreeCountTxt.text = LangManager.Instance.GetTranslation(
      this._type == EmMazeType.AbyssMaze
        ? "MazeFrameWnd.TodayFreeCountTxt1"
        : "MazeFrameWnd.TodayFreeCountTxt",
    );
    if (this._type == EmMazeType.AbyssMaze) {
      ...
      if (!ComponentSetting.ABYSS_MAZE) {
        ...
        let hasMaze2Info =
          PlayerManager.Instance.currentPlayerModel.towerInfo2.campaignId != 0;
        if (!hasMaze2Info) {
          str = LangManager.Instance.GetTranslation(
            "maze.MazeFrame.Maze2openContent",
          );
        }
      }
      if (str) {
  ...
  public set vdata(value: TowerInfo) {
    ...
    this.MyRecordValue.text =
      this._vdata.maxIndex == 0 ? str : this._vdata.maxIndex.toString();
    // 剩余次数
    let freeCount = this._vdata.maxEnterCount - this._vdata.enterCount;
    this.TodayFreeCountValue.text =
      (freeCount > 0 ? freeCount : 0) + "/" + this._vdata.maxEnterCount;
    ...
    if (this._vdata.maxIndex < MazeModel.OPEN_SWEEP_LEVEL) {
      ...
      if (this._vdata.towerIndex > 0) {
        if (
          this._vdata.towerIndex == this._vdata.maxIndex &&
          this._vdata.pass != 2
        ) {
          this.cMazeOpt.setSelectedIndex(2); // 继续
  ...
  private __resetHandler() {
    ...
    SimpleAlertHelper.Instance.Show(
      ...
      null,
      null,
      this.__resetHandlerBack.bind(this),
    );
  }
  ...
  private __startHandler() {
    ...
    if (this._vdata.maxIndex == 0 && this._vdata.towerIndex == 0) {
      ...
      RoomSocketOuterManager.enterTowerInfo(
        this._vdata.campaignId,
        1,
        MopupManager.Instance.model.isDoubleProfit,
      );
    } else {
      if (this.checkNeedBuyMazeStone()) {
        let info: ShopGoodsInfo =
          TempleteManager.Instance.getShopTempInfoByItemId(
            MazeModel.MAZE_STONE,
          );
        if (!info) {
  ...
  private __startMopupHandler() {
    ...
    if (this.checkNeedBuyMazeStone()) {
      let info: ShopGoodsInfo =
        TempleteManager.Instance.getShopTempInfoByItemId(MazeModel.MAZE_STONE);
      if (!info) {
        this.showMazeStoneNotEnoughTip();
      } else {
        FrameCtrlManager.Instance.open(EmWindow.BuyFrameI, {
  ...
  private checkCanNotEnter(): boolean {
    if (this._vdata.enterCount >= this._vdata.maxEnterCount) {
      MessageTipManager.Instance.show(
        LangManager.Instance.GetTranslation("maze.MazeFrame.command01"),
      );
      return true;
    }
    return false;
  ...
  private setDoubleProfit() {
    if (this._type == EmMazeType.GroundMaze) {
      MopupManager.Instance.model.isDoubleProfit = this.DoubleProfit.selected
        ? 2
        : 1;
    }
  }
  ...
  private get needBuyCnt() {
    let ownCount = GoodsManager.Instance.getGoodsNumBySonType(
      GoodsSonType.SONTYPE_MAZE,
    );
    let needCount =
      this._type == EmMazeType.GroundMaze
        ? MazeModel.MAZE_STONE_USE
        : MazeModel.MAZE2_STONE_USE;
  ...
  private openMopup() {
    ...
    FrameCtrlManager.Instance.open(
      EmWindow.Mopup,
      { type: eMopupType.MazeMopup, state: eMopupState.MazeMopupPre },
      null,
  ...
  private showMazeStoneNotEnoughTip() {
    MessageTipManager.Instance.show(
      LangManager.Instance.GetTranslation("mazeframe.coin.countNum"),
    );
  }
}
