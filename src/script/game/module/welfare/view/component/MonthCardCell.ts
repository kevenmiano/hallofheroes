...
export default class MonthCardCell extends FUI_WelfareMonthCardCell {
  ...
  public set statedata(value: MonthCardInfo) {
    ...
    if (value.isPay)
      if (value.leftDays == 0) {
        this.time.text = LangManager.Instance.GetTranslation(
          "MonthCardCell.time1",
        );
      } else {
      ...
      this.time.text =
        "[color=#490100]" +
        LangManager.Instance.GetTranslation("faterotary.LevelMin") +
        "[/color]"; //剩余时间
    ...
    if (state) {
      ...
      if (Utils.isInteger(this._cost.toString())) {
        this.txt_money.text =
          ...
          parseInt(this._cost.toString()); //价格
      } else {
        this.txt_money.text =
          ShopManager.Instance.getMoneyString() + this._cost.toString(); //价格
      }
    }
  ...
  private onMonthCardHandler() {
    ...
    if (!this._statedata.isPay) {
      Logger.warn(
        "点击购买超值月卡!...  " + this.index + "  金额:" + this._cost,
      );
      let monthCfg = TempleteManager.Instance.getMonthCardRechargeTemplete(
        (this.index + 1).toString(),
      );
      if (!monthCfg) return;
      SDKManager.Instance.getChannel().adjustEvent(
        RPT_EVENT.MONTHLY_CARD_PURCHASE,
        { productId: monthCfg.ProductId },
      );
