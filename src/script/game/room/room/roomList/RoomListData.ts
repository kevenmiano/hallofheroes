...
export default class RoomListData extends FrameDataBase {
    ...
    private formatList(arr: t_s_campaignData[], type: number = RoomCampaignType.Multy) {
        let format = [];
        arr.forEach(item => {
            let obj = {};
            obj["CampaignId"] = item.CampaignId;
            obj["CampaignName"] = LangManager.Instance.GetTranslation("public.PVEroomlist", item.CampaignNameLang, item.MinLevel, item.MaxLevel);
            if (type != RoomCampaignType.Multy) {
                obj["CampaignName"] += " " + LangManager.Instance.GetTranslation("public.Activity");
            }
    ...
    private getComboListNames(): string[] {
        ...
        let temp: string[] = [LangManager.Instance.GetTranslation("RoomList.AllMap")]
