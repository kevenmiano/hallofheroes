...
export default class Pin extends FUI_Pin {
    ...
    private addEvent() {
        GameManager.Instance.addEventListener(PlayerEvent.SYSTIME_UPGRADE_MINUTE, this.onSysTimeUpdate, this);
        NotificationManager.Instance.addEventListener(NativeEvent.WIFI_STATE_UPDATE, this.onWifiStateUpdate, this);
    }

    private offEvent() {
        GameManager.Instance.removeEventListener(PlayerEvent.SYSTIME_UPGRADE_MINUTE, this.onSysTimeUpdate, this);
        NotificationManager.Instance.removeEventListener(NativeEvent.WIFI_STATE_UPDATE, this.onWifiStateUpdate, this);
    }
    ...
    onSysTimeUpdate() {
        ...
        Logger.yyz("当前服务器时间: ", DateFormatter.format(sysTime, "YYYY-MM-DD hh:mm:ss"));
        let hours: number = sysTime.getHours();
        let minutes: number = sysTime.getMinutes();
        let hourstr: string;
        let minutestr: string;
        hourstr = hours < 10 ? "0" + sysTime.getHours().toString() : sysTime.getHours().toString();
        minutestr = minutes < 10 ? "0" + sysTime.getMinutes().toString() : sysTime.getMinutes().toString();
    ...
    onWifiStateUpdate(value: number) {
        if (value >= 1) { //1
            this.setWifiState(3);
        } else if (value > 0.5 && value < 1) {//0.75,0.5,0.25
            this.setWifiState(2);
