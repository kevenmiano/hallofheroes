...
import { CampaignEvent, CampaignMapEvent, NotificationEvent, OuterCityEvent } from "../../constant/event/NotificationEvent";
...
export class CampaignMapModel extends FloorMapInfo {
	...
	public static sliceBgMapId: Array<number> = [10000, 10001, 10002, 20001, 20002, 20003, 20004, 30000,
		1011, 1021, 1031, 1032, 1051, 1061, 1071, 1072, 1073, 1081, 1082, 1083, 1091, 1111, 1131, 1132, 1141, 1142, 1151, 1161, 1171, 1172, 1181,
		1182, 1191, 1192, 1201, 1202, 1211, 1212, 1231, 1232, 1251, 1252, 1261, 1262, 1271, 1272, 1291, 1292, 1301,
		1302, 1311, 1312, 1313, 1331, 1332, 1341, 1342, 1351, 1352, 1371, 1391, 1392, 1401, 1402, 1411, 1412, 1431,
		1432, 1441, 1442, 1451, 1452, 1471, 1472, 1491, 1492, 1501, 1502, 1511, 1512, 1521, 1522, 1531, 1532, 1551,
		1552, 1553, 1554, 1555, 1561, 1562, 1563, 1564, 1565, 1571, 1572, 1581, 1582, 1591, 1592, 1601, 1602, 1611,
		1612, 1631, 1632, 1651, 1652, 1661, 1662, 1671, 1672, 1691, 1692, 1711, 1712, 1731, 1732, 1741, 1742, 1751,
		1752, 1771, 1772, 1791, 1792, 1811, 1812, 1821, 1822, 1831, 1832, 1851, 1852, 1861, 1862, 1871, 1872, 1873, 1891, 1892, 1893,
		//
		2011, 2012, 2031, 2032, 2033, 2051, 2052, 2053, 2054, 2071, 2072, 2073, 2074, 2075, 2081, 2082, 2083, 2084, 2085, 2091, 2092, 2111, 2112, 2114, 2121, 2122, 2124, 2131, 2132,
		2133, 2141, 2142, 2143, 2151, 2152, 2153, 2161, 2162, 2163, 2171, 2172, 2173, 2181, 2182, 2183, 2191, 2192, 2193, 2201, 2202, 2203, 2211, 2212, 2213, 2214, 2215, 2221, 2222,
		2223, 2224, 2225, 2231, 2232, 2233, 2241, 2242, 2243, 2251, 2252, 2253, 2261, 2262, 2263, 3001, 4000, 4001, 4002, 4003, 4004, 4005, 4006, 4007, 4008, 4009, 4010, 4011, 5001, 5002, 5003, 5201, 5301,
		6001, 7001, 7002, 7003, 7004, 7005, 7006, 7007, 7008, 7009, 7501, 7601,
		...
		8201, 8202, 8203, 8204, 2271, 2351, 2281, 2341, 2352, 2353
	];
	...
	public static getSingleBgMapPosList(dir: number = 1) {
		...
		let arr
		if (dir == 1) {
			arr = [new Laya.Point(baseX - gap, baseY), new Laya.Point(baseX - gap * 2, baseY), new Laya.Point(baseX - gap * 3, baseY)]
		} else {
			arr = [new Laya.Point(baseX + gap, baseY), new Laya.Point(baseX + gap * 2, baseY), new Laya.Point(baseX + gap * 3, baseY)]
		}
		return arr
	...
	public set currentFloorData(arr: any[]) {
		switch (this.mapId) {
			...
			case 20002:
			case 20003:
				if (this.mapTempInfo.Width > MapInfo.MAX_WIDTH || this.mapTempInfo.Height > MapInfo.MAX_HEIGHT) {
				...
				// 	tops = tops.concat(element);
				// });
				this.dispatchEvent(OuterCityEvent.CURRENT_CONFIG_CHANGE, { movies: movies, tops: tops });
				break;
			default:
				if (this.mapTempInfo.Width > MapInfo.MAX_WIDTH || this.mapTempInfo.Height > MapInfo.MAX_HEIGHT) {
					...
					for (const key in this.floorData) {
						...
						this.dispatchEvent(OuterCityEvent.CURRENT_CONFIG_CHANGE, { floor: floor, movies: movie, tops: top, id: key, start: start });
					}
				}
				break;
		}
	...
	public attackBossTeam(dic: Map<number, ThaneInfo>, time: number, boss: ThaneInfo) {
		this._attackBossTeamList = dic;
		this._leftFightTime = time;
		this._attackBoss = boss;
		this.dispatchEvent(CampaignMapEvent.ATTACK_BOSS_TEMA, { team: dic, time: time, boss: boss });
	}
	public inviteAttackBoos(inviteName: string, boss: ThaneInfo, time: number, pos: Laya.Point) {
		this.dispatchEvent(CampaignMapEvent.ATTACK_BOSS_INVITE, { inviteName: inviteName, time: time, boss: boss, pos: pos });
	}
	...
	public get campaignTemplate(): t_s_campaignData {
		...
		return TempleteManager.Instance.getCampaignTemplateByID(this.mapTempInfo.CampaignId);
	}
	...
	public aStarPathFinder($start: Laya.Point, $end: Laya.Point, off: number = 25): any[] {
		let startPoint: Laya.Point = new Laya.Point(parseInt(($start.x / 20).toString()), parseInt(($start.y / 20).toString()));
		let endPoint: Laya.Point = new Laya.Point(parseInt(($end.x / 20).toString()), parseInt(($end.y / 20).toString()));
		if (!this._pathFinder) {
	...
	public checkWalkable($pt: Laya.Point): boolean {
		let startPoint: Laya.Point = new Laya.Point(parseInt(($pt.x / 20).toString()), parseInt(($pt.y / 20).toString()));
		if (!this._pathFinder) {
			this._pathFinder = new AStarPathFinder(this.mapTielsData);
		}
	...
	public updateFog(posX: number, posY: number, type: number) {
		...
		let p: Laya.Point;
		p = new Laya.Point(parseInt((posX + CampaignMapModel.fogUnitWidth / 2).toString()), parseInt((posY + CampaignMapModel.fogUnitHeight / 2).toString()));
		...
		if (this.updateFogData(p.x, p.y, type))
			this.dispatchEvent(CampaignMapEvent.LOCAL_UPDATE_FOG, { x: p.x, y: p.y, vy: vy, type: type });
	}
	...
	private noteFog(vx: number, vy: number, byte: number) {
		let col: number = Math.ceil(this.mapTempInfo.Width / CampaignMapModel.fogUnitWidth);
		let index: number = vx * col + vy;
		this._noteFogArr.push({ index: index, value: byte });
		if (this._noteFogArr.length >= 3) {
			CampaignSocketOutManager.Instance.sendUpdateFogData(this._noteFogArr, this.mapId);
	...
	public syscFogNode(index: number, byte: number, col: number) {
		...
		if (!this._fogData || this._fogData[vx] == null || this._fogData[vx][vy] == null) return;
		...
		this.dispatchEvent(CampaignMapEvent.SYN_FOG_DATA, { x: vx, y: vy, vy: vvy, type: byte });
	}
	...
	public addBaseArmy(army: CampaignArmy, isSwitchMap: boolean = false) {
		...
		if (!this._userArmyList.has(key)) {
			...
			if (this.isCross) NotificationManager.Instance.dispatchEvent(NotificationEvent.CROSS_ADD_GOONBTN);
			return;
		}
		this._userArmyList.set(key, army);
		if (isSwitchMap) {
	...
	public reset() {
		...
		if (this._mapNodesData) this._mapNodesData.splice(0, this._mapNodesData.length);
	...
	public isSelfArmy(army: CampaignArmy): boolean {
		...
		if (this.isCross) {
			return Boolean(army.userId == userId && army.baseHero.serviceName == selfServerName);
		} else {
	...
	public removeBaseArmyByArmyId(armyId: number, serverName: string = ""): CampaignArmy {
		if (!this.isCross || serverName == null) serverName = "";
		let army = this._userArmyList.get(serverName + "_" + armyId) as CampaignArmy;
		if (army) {
			if (this.isSelfArmy(army)) {
				this._selfMemberData = null;
			}
	...
	public getBaseArmyByArmyId(armyId: number, serverName: string = ""): CampaignArmy {
		if (!this.isCross || serverName == null) serverName = "";
		return this._userArmyList.get(serverName + "_" + armyId);
	}

	public getBaseArmyByUserId(userId: number, serverName: string = ""): CampaignArmy {
		if (!this.isCross || serverName == null) serverName = "";
		...
		for (let element of values) {
			if (element.userId == userId) {
				if (serverName) {
					if (element.baseHero.serviceName == serverName)
						temp = element;
				} else {
					temp = element;
				}
	...
	public getUserArmyByUserId(userId: number, serverName: string = ""): CampaignArmy {
		if (!this.isCross || serverName == null) serverName = "";
		let values = this._userArmyList.values();
		for (let army of values) {
			if (army.userId == userId) {
				if (serverName) {
					if (army.baseHero.serviceName == serverName)
						return army;
				} else {
					return army;
				}
	...
	public get mapNodeStartPoint(): CampaignNode {
		...
		for (const key in this._mapNodesData) {
			...
			if (element.info.types == PosType.COPY_START) {
				if (WorldBossHelper.checkPvp(this.mapId) || WorldBossHelper.checkGvg(this.mapId)) {
	...
	private checkNode(node: CampaignNode): boolean {
		if (WorldBossHelper.checkWorldBoss(this.mapId)) {
			if (node && node.info && node.info.types == CampaignNodeType.WORLD_BOSS_NOT_PASS) {
	...
	public getNodeBySindId(signId: string): CampaignNode {
		for (const key in this._mapNodesData) {
			...
			if (element.info instanceof ChestInfo) {
				if ((<ChestInfo>element.info).signId == signId)
					return element;
			}
		}
		return null;
	...
	private checkNodePass(pos: string): boolean {
		for (const key in this._mapNodesData) {
			...
			if (pos == (element.posX + "," + element.posY)) {
				if (element.info.state == NodeState.FRIENDLY || element.info.state == NodeState.DESTROYED)
					return true;
				break;
			}
		}
	...
	public getNotHandlerNodeByXY($x: number, $y: number): CampaignNode {//取得指定点周围的非事件点
		...
		for (const key in this._mapNodesData) {
			if (Object.prototype.hasOwnProperty.call(this._mapNodesData, key)) {
				...
				switch (element.info.types) {
					...
					case PosType.COPY_NPC:
						if (element.attackTypes == 1) {
							...
							this._checkNodePoint.x = Math.floor(element.nodeView.x / Tiles.WIDTH);
							this._checkNodePoint.y = Math.floor(element.nodeView.y / Tiles.HEIGHT);
							if (element.info.state == NodeState.FIGHTING) break;
						}
						else {
							if (WorldBossHelper.checkConsortiaDemon(this.mapId) || WorldBossHelper.checkConsortiaBoss(this.mapId)) {
						...
						if (CampaignArmyState.checkDied(this.selfMemberData.isDie)) continue;
						break;
					case PosType.ATTACK_HANDLER://TODO
						if (WorldBossHelper.checkPvp(this.mapId) && this.selfMemberData.teamId != element.param1) continue;
						break;
					case PosType.DIE_CROSSING:
						if (selfTeamId == element.param1 && CampaignArmyState.checkDied(this.selfMemberData.isDie)) break;
						continue; break;
					case PosType.ENEMY_CROSSING:
						if (selfTeamId == element.param1) continue;
						break;
					case PosType.BOMBER_MAN:
						if (selfTeamId == element.param1 || element.info.state == NodeState.FIGHTING) continue;
						this._checkNodePoint.x = Math.floor(element.nodeView.x / Tiles.WIDTH);
						this._checkNodePoint.y = Math.floor(element.nodeView.y / Tiles.HEIGHT);
						break;
				...
				let leng: number = this._checkTempPoint.distance(this._checkNodePoint.x, this._checkNodePoint.y);
				if (leng <= attackDis) {
	...
	public getHandlerNode($x: number, $y: number): CampaignNode {//取得指定点附近的事件节点及掉落点
		$x = parseInt($x.toString());
		$y = parseInt($y.toString());
		this._checkTempPoint.x = $x;
		this._checkTempPoint.y = $y;
		...
		for (const key in this._mapNodesData) {
			if (Object.prototype.hasOwnProperty.call(this._mapNodesData, key)) {
				...
				switch (tempElement.info.types) {
					case PosType.COLLECTION:
						if (!WorldBossHelper.checkPvp(this.mapId) || WorldBossHelper.checkConsortiaBoss(this.mapId)) continue;
					...
					case PosType.FALL_CHEST:
					case PosType.HOOK_NODE:
						if (WorldBossHelper.checkPetLand(this.mapId) &&
						...
						dis = this._checkTempPoint.distance(this._checkNodePoint.x, this._checkNodePoint.y);
						// 战场拉矿防拉扯
						if (WorldBossHelper.checkPvp(this.mapId) && tempElement.sonType == BuildSonType.SONTYPE_2380) {
		...
		if (nodes.length > 0) {
			...
			nodes.forEach(node => {
				if (!minLenNode) {
					...
					let disMin = this._checkTempPoint.distance(minLenNode.posX, minLenNode.posY);
					if (disNew < disMin) {
						minLenNode = node
					}
				}
	...
	private checkCanNotPassNodeSpecial(node: CampaignNode): boolean {
		...
		if (mapModel && WorldBossHelper.checkIsNoviceMapLayer2(mapModel.mapId)) {
			let nodeTemp = mapModel.getMapNodeByNodeId(GlobalConfig.CampaignNodeID.Node_1000205)
			if (!nodeTemp) return false
			if (node.nodeId == GlobalConfig.CampaignNodeID.Node_1000208 && nodeTemp.info.state == NodeState.DESTROYED) {
				return true
			}
		}
		return false;
	...
	public chckeNpcFriendState(node: CampaignNode): boolean {
		if (node && node.info && node.info.types == PosType.COPY_NPC) {
			if (node.param1 > 0 && node.param1 == this.selfMemberData.teamId) return true;
		}
		return false;
	}
	...
	public getMapNodesByPoint(p: Laya.Point): CampaignNode {
		let temp: Laya.Point = new Laya.Point(p.x * Tiles.WIDTH, p.y * Tiles.HEIGHT);
		this._mapNodesData.forEach(element => {
			if (element.x == temp.x && element.y == temp.y)
				return element;
		})
		return null;
	}
	...
	private find(i: number, $x: number, $y: number): Laya.Point {
		...
		for (let a: number = -i; a < i; a++) {
			if (this.getPointValue($x + a, $y + i)) return new Laya.Point($x + a, $y + i);
			if (this.getPointValue($x + a, $y - i)) return new Laya.Point($x + a, $y - i);
			if (this.getPointValue($x + i, $y + a)) return new Laya.Point($x + i, $y + a);
			if (this.getPointValue($x - i, $y + a)) return new Laya.Point($x - i, $y + a);
		}
		return null;
	}
	...
	public getNeighbor($x: number, $y: number): Array<Laya.Point> {
		...
		if (this.getPointValue($x - Tiles.WIDTH, $y)) arr.push(new Laya.Point($x - Tiles.WIDTH, $y));
		if (this.getPointValue($x + Tiles.WIDTH, $y)) arr.push(new Laya.Point($x + Tiles.WIDTH, $y));
		if (this.getPointValue($x, $y - Tiles.HEIGHT)) arr.push(new Laya.Point($x, $y - Tiles.HEIGHT));
		if (this.getPointValue($x, $y + Tiles.HEIGHT)) arr.push(new Laya.Point($x, $y + Tiles.HEIGHT));
		return arr;
	}
	...
	public getPointValue($x: number, $y: number): boolean {
		$x = parseInt($x.toString());
		$y = parseInt($y.toString());
		return this.mapTielsData && (this.mapTielsData[$x + "_" + $y] ? true : false);

	}
	public moveArmyToRange(armyId: number, arr: any[]) {
		this.dispatchEvent(CampaignEvent.MOVE_ARMY_TO_RANGE, { id: armyId, range: arr });
	}
	...
	public movePathsArmy(msg: any) {
		...
		if (SceneManager.Instance.currentType != SceneType.CAMPAIGN_MAP_SCENE) {
			let uArmy: CampaignArmy = this.getBaseArmyByArmyId(msg.armyId, msg.serverName);
			if (uArmy) {
	...
	public gotoPosArmy(msg: any) {
		let aInfo: CampaignArmy = this.getBaseArmyByArmyId(msg.armyId, msg.serverName);
		if (aInfo && SceneManager.Instance.currentType != SceneType.CAMPAIGN_MAP_SCENE) {
	...
	public getSendNode(): CampaignNode {
		this.mapNodesData.forEach(element => {
			if (element.info.types == PosType.COPY_NODE_SEND ||
	...
	public getMineraHandCardNode(): CampaignNode {
		for (const key in this._mapNodesData) {
			let element = this._mapNodesData[key];
			if (element && element.nodeId == CampaignMapModel.MINERA_HAND_CAR_NODEID) {
				return element;
			}
		}
		return null;
	...
	public getConsortiaBossGoodsNode(): CampaignNode {
		for (const key in this._mapNodesData) {
			let element = this._mapNodesData[key];
			if (element && element.nodeId == CampaignMapModel.CONSORTIA_BOSS_GOODS_NODEID) {
				return element;
			}
		}
		return null;
	...
	public checkOutScene(): boolean {
		...
		let serverName = PlayerManager.Instance.currentPlayerModel.playerInfo.serviceName;
		let campaignArmy: CampaignArmy = CampaignManager.Instance.mapModel.getBaseArmyByUserId(userId, serverName);
		let armyView: any = CampaignManager.Instance.controller.getArmyView(campaignArmy);
