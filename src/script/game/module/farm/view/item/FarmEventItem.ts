...
export default class FarmEventItem extends FUI_FarmEventItem {
    ...
    public set info(value: WaterLogInfo) {
        ...
        this._info = value;
        if (this._info) {
            this.descTxt.text = LangManager.Instance.GetTranslation("FarmEventItem.descTxt",this._info.nickName,this._info.actionDes,DateFormatter.timeFormat2(this._info.time));
        }
        else {
            this.clean();
        }
