...
export default class DragManager extends GameEventDispatcher {
    ...
    public registerDragObject(dragCom, dragUpCallBack?: Function) {
        if (dragCom['getDragType'] != null) {
            ...
            this.dragMapCall.set(dragCom, { Func: dragUpCallBack, parent: dragCom.parent, displayParent: dragCom.displayObject.parent });
    ...
    private _onDragComMove(evt: Laya.Event) {
        ...
        let distance = this.mouseDownPoint.distance(Laya.stage.mouseX, Laya.stage.mouseY);
        ...
        LayerMgr.Instance.addToLayer(this.curTargetObj.displayObject, EmLayer.STAGE_DRAG_LAYER);
    ...
    private moveToStageXY(evt: Laya.Event) {
        if (this.curTargetObj) {
            this.curTargetObj.x = evt.stageX + (this.curTargetObj.pivotX == 0 ? -this.curTargetObj.width / 2 : 0);
            this.curTargetObj.y = evt.stageY + (this.curTargetObj.pivotY == 0 ? -this.curTargetObj.height / 2 : 0);
        }
    }
    ...
    private _onDragComMouseUp(evt: Laya.Event) {
        ...
        if (dropTarget) {
            //处理交换拖拽处理
            let isDropTarget = dropTarget['getDragEnable'] && dropTarget['getDragType'];
            if (!isDropTarget) {//非拖拽对象或容器
                evt.stopPropagation();
                if(this.curTargetObj){//uncaught error: Tween:target is null
                    Laya.Tween.to(this.curTargetObj, { x: this.globalStoreX, y: this.globalStoreY }, 150, Laya.Ease.cubicOut, Laya.Handler.create(this, () => {
