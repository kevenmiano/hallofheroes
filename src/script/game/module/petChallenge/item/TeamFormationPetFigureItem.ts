...
export class TeamFormationPetFigureItem extends FUI_TeamFormationPetFigureItem {
  ...
  private refresh() {
    ...
    this.txtName2.color =
      this.type == 2
        ? "#ffecc6"
        : PetData.getQualityColor(this._info.quality - 1);
    // this._info.template.PetAvatar = "/pet_athena"
    this._avatar.data = this._info.template;

    this.typeIcon.icon = FUIHelper.getItemURL(
      EmPackName.Base,
      "Icon_PetType" + this._info.template.PetType + "_s",
    );
  ...
  public light() {
    if (this.imgBg && !this.imgBg.isDisposed) {
      if (this.imgBg.displayObject && !this.imgBg.displayObject.destroyed) {
        this.imgBg.url = FUIHelper.getItemURL(
          EmPackName.BaseCommon,
          "Img_Formation_Platform_Sel",
        );
