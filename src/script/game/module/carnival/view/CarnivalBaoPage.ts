 ...
 * 嘉年华--幸运夺宝
 */
export default class CarnivalBaoPage
  extends FUI_CarnivalBaoPage
  implements CarnivalBasePage
{
  ...
  private initTranslate() {
    this.bao1StartBtn.title = LangManager.Instance.GetTranslation(
      "Carnival.CarnivalBaoPage.bao1StartBtn",
    );
    this.bao10StartBtn.title = LangManager.Instance.GetTranslation(
      "Carnival.CarnivalBaoPage.bao10StartBtn",
    );
  }
  ...
  private initView(): void {
    this.Img_LuckyDraw.url = this.model.getThemeFolderImgPath(
      CARNIVAL_FOLDER.BAO,
      "Img_LuckyDraw",
    );
    this.Img_Txt_LuckyDraw.url = this.model.getThemeFolderImgPath(
      CARNIVAL_FOLDER.BAO,
      "Img_Txt_LuckyDraw" + "_" + getDefaultLangageKey(),
    );
    this._leftTempList = TempleteManager.Instance.getCarnivalLuckDrawTempList(
      3,
      1,
    );
    this._leftCC = this.creatMoveCC(this._leftTempList, 1, 1);
    this._rightTempList = TempleteManager.Instance.getCarnivalLuckDrawTempList(
      3,
      2,
    );
    this._rightCC = this.creatMoveCC(this._rightTempList, 2, 2);

    this.txt_rules.text = LangManager.Instance.GetTranslation(
      "carnival.luck.baoTip",
    );

    let cfg =
      TempleteManager.Instance.getConfigInfoByConfigName("CarnivalLuckyDraw");
    if (cfg) {
    ...
    if (isPlay) {
      ...
      if (restr != "" && restr.length > 0) {
        ...
        if (idList.length == 2) {
          ...
          let findList: [number, t_s_carnivalluckdrawData] = this.findPrams(
            this._leftTempList,
            Number(idList[0]),
          );
          let leftIdx: number = Number(findList[0]);
          tInfo = findList[1] as t_s_carnivalluckdrawData;
          let gInfo: t_s_itemtemplateData =
            TempleteManager.Instance.getGoodsTemplatesByTempleteId(tInfo.Item);
          ...
          gInfo = TempleteManager.Instance.getGoodsTemplatesByTempleteId(
            tInfo.Item,
          );
          ...
          let rewardTips: string = LangManager.Instance.GetTranslation(
            "carnival.recharge.rewardMore",
            this._leftRewardStr,
            this._rightRewardStr,
          );
          ...
          while ((restr = restIds.pop())) {
            ...
            if (idList.length == 2) {
              ...
              rewardTips = LangManager.Instance.GetTranslation(
                "carnival.recharge.rewardMore",
                leftRewardStr,
                rightRewardStr,
              );
  ...
  private findPrams(
    arr: Array<t_s_carnivalluckdrawData>,
    tId: number,
  ): [number, t_s_carnivalluckdrawData] {
    let ret: [number, t_s_carnivalluckdrawData] = [-1, null];
    let len: number = arr ? arr.length : 0;
    let tInfo: t_s_carnivalluckdrawData;
    for (let i: number = 0; i < len; i++) {
      tInfo = arr[i];
      if (tInfo.Id == tId) {
        ret = [i, tInfo as t_s_carnivalluckdrawData];
        break;
      }
    }
    return ret;
  ...
  private creatMoveCC(
    tempList: Array<t_s_carnivalluckdrawData>,
    idx: number = 0,
    way: number = 0,
  ): MoveSomeController {
    ...
    let cc: MoveSomeController = new MoveSomeController(
      itemList1,
      itemList2,
      90,
      way,
  ...
  private showRewardInfos() {
    ...
    //播放飘获得动画
    GoodsManager.Instance.dispatchEvent(
      BagEvent.NEW_GOODS,
      this._tipGoodsPools,
    );
  }
  ...
  onDestroy() {
    ...
    this.offEvent();
    MaskLockOper.Instance.doCall(false);
    if (this._leftCC) this._leftCC.dispose();
    this._leftCC = null;
    if (this._rightCC) this._rightCC.dispose();
    this._rightCC = null;
