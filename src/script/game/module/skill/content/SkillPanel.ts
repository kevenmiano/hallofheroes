...
import NewbieEvent, { SkillEvent } from '../../../constant/event/NotificationEvent';
...
export default class SkillPanel extends FUI_Skill_Panel
{
    ...
    private initView() {
        ...
        this.masteryTab.visible = this.isopenMastery = this.thane.grades >= OpenGrades.MASTERY;
    ...
    private onTab(index){
        ...
        if (index == 1) {
            NotificationManager.Instance.dispatchEvent(NewbieEvent.MANUAL_TRIGGER, NewbieConfig.NEWBIE_374)
        }
        NotificationManager.Instance.dispatchEvent(SkillEvent.SWITCH_MASTERY,index);
    }
    ...
    private addEvent() {
        ...
        this.thane.skillCate.addEventListener(PlayerEvent.SKILL_INDEX, this.__switchSkillHandler, this);
        this.thane.skillCate.addEventListener(PlayerEvent.ACTIVE_DOUBLE_SKILL, this.__doubleSkillHandler, this);
        this.thane.skillCate.addEventListener(PlayerEvent.SKILL_FAST_KEY, this.__skillChangeHandler, this);
        NotificationManager.Instance.addEventListener(SkillEvent.FAST_KEY_MOVE, this.__fastKeyMoveHandler, this);
        NotificationManager.Instance.addEventListener(SkillEvent.SET_FAST_KEY, this.__setFastKeyHandler, this);
        NotificationManager.Instance.addEventListener(SkillEvent.PUTOFF, this._onEquipOffSkill, this);
        NotificationManager.Instance.addEventListener(SkillEvent.PUTON, this._onEquipOnSkill, this);
        NotificationManager.Instance.addEventListener(SkillEvent.REPLACE, this.onReplace, this);
        NotificationManager.Instance.addEventListener(SkillEvent.CLICK_MASTERY_SKILL, this.onSelectMasterySkill, this);
    }
    ...
    private offEvent() {
        ...
        this.thane.skillCate.removeEventListener(PlayerEvent.SKILL_INDEX, this.__switchSkillHandler, this);
        this.thane.skillCate.removeEventListener(PlayerEvent.ACTIVE_DOUBLE_SKILL, this.__doubleSkillHandler, this);
        this.thane.skillCate.removeEventListener(PlayerEvent.SKILL_FAST_KEY, this.__skillChangeHandler, this);
        NotificationManager.Instance.removeEventListener(SkillEvent.FAST_KEY_MOVE, this.__fastKeyMoveHandler, this);
        NotificationManager.Instance.removeEventListener(SkillEvent.SET_FAST_KEY, this.__setFastKeyHandler, this);
        NotificationManager.Instance.removeEventListener(SkillEvent.REPLACE, this.onReplace, this);
        this.fastskillList.off(fairygui.Events.CLICK_ITEM, this, this.onFastSkillSelect);
        this.skillList.off(fairygui.Events.CLICK_ITEM, this, this.onSkillSelect);
        NotificationManager.Instance.removeEventListener(SkillEvent.PUTOFF, this._onEquipOffSkill, this);
        NotificationManager.Instance.removeEventListener(SkillEvent.PUTON, this._onEquipOnSkill, this);
        NotificationManager.Instance.removeEventListener(SkillEvent.CLICK_MASTERY_SKILL, this.onSelectMasterySkill, this);
    }
    ...
    onSkillSelect(clickItem: SkillItemCom) {
        ...
        if (this.isSetting) {
            ...
            let _ispassiveSkill: boolean = this.selectSkillData.templateInfo.UseWay == 2;//被动不能装备,不能卸下
            if (_ispassiveSkill) {
            ...
            //单击已装备的技能图标可直接卸下技能
            let fastItem: FastSkillItem = this.checkSkillEquiped(this.selectSkillData); //检查是否已经装备
            if (fastItem) {
    ...
    */
    private refreshSkill() {
        ...
        this.updateBlink();
        NotificationManager.Instance.dispatchEvent(SkillEvent.UPDATE_SKILL_POINT, this.isSetting);
    ...
    private initFastKeyList() {
        ...
        for (var i: number = 0; i < SkillPanel.SKILL_NUM; i++) {
            var item: FastSkillItem = <FastSkillItem>this.fastskillList.addItemFromPool();
            if (i < keyList.length) {
        ...
        this.fastskillList.on(fairygui.Events.CLICK_ITEM, this, this.onFastSkillSelect);
    }
    ...
    private activeAlertBack(b: boolean, flag: boolean, obj: any) {
        if (b) {
            let hasMoney: number = PlayerManager.Instance.currentPlayerModel.playerInfo.allPoint;
    ...
    private __switchHandler(targetBtn) {
        if (this.isSetting) {//编辑技能状态时, 不可切换技能栏, 点击时弹出TIPS: 技能编辑模式中不可切换技能栏
        ...
        if (PlayerManager.Instance.currentPlayerModel.playerInfo.isSkillEditOpen) {
            ...
            SimpleAlertHelper.Instance.Show(SimpleAlertHelper.SIMPLE_ALERT, null, prompt, str, confirm, cancel, this.alertBack1.bind(this));
            return;
        }
    ...
    public resetSetting() {
        ...
        for (let i = 0; i < this.skillList.numChildren; i++) {
            const skillItem: SkillItemCom = this.skillList.getChildAt(i) as SkillItemCom;
            if (skillItem) {
    ...
    private onReplace(selfDragData?, dropTarget?) {
        ...
        if (equipedFastItem) {
            MessageTipManager.Instance.show(LangManager.Instance.GetTranslation('skill.equiped'));
            return;
        }
        let fastItem: FastSkillItem = this.checkSkillEquiped(dropTarget.getDragData()); //检查是否已经装备
        if (fastItem) {
            //下卸下
            this._onEquipOffSkill(fastItem);
            //再装备
    ...
    private __switchSkillHandler(e: PlayerEvent) {
        if (this.checkIsSetting()) {//正在编辑的时候收到协议不能处理
            return;
        }
        this.refreshSkill();
    ...
    public get fastKey(): string {
        ...
        for (let i: number = 0; i < len; i++) {
            ...
            if (item.vdata)
                str += (item.vdata.templateInfo.SonType + ",");
            else
                str += "-1,"
        }
        return str;
    ...
    public onSet() {
        if (this.btn_set.title == LangManager.Instance.GetTranslation('setting.SettingFrame.title')) {
            ...
            this.showFastSkillEquiping(true);
            NotificationManager.Instance.dispatchEvent(SkillEvent.RESET_SKILL_STATE, false)
        } else {   //保存 点击保存按钮可保存技能配置
            this.btn_set.title = LangManager.Instance.GetTranslation('setting.SettingFrame.title')
            this.isSetting = false;
            this.equiping = false;
            if(this.isJobSkill){
            ...
            this.showFastSkillEquiping(false);
            this.control.sendSetFastKey(this.fastKey);
            NotificationManager.Instance.dispatchEvent(SkillEvent.RESET_SKILL_STATE, true)
        }
        this.updateEditMode();
    }
    ...
    onSimDoubleSkill() {
        ...
        //锁定则打开激活双技能弹窗确定
        let cfgValue = 600;
        let cfgItem = TempleteManager.Instance.getConfigInfoByConfigName("Second_SkillPoint");
        if (cfgItem) {
        ...
        var cancel: string = LangManager.Instance.GetTranslation("public.cancel");
        var prompt: string = LangManager.Instance.GetTranslation("public.prompt");
        var content: string = LangManager.Instance.GetTranslation("armyII.viewII.skill.ActiveAlertContent", point);
        SimpleAlertHelper.Instance.Show(SimpleAlertHelper.USEBINDPOINT_ALERT, { point: point, checkDefault: true }, prompt, content, confirm, cancel, this.activeAlertBack.bind(this));
    }
