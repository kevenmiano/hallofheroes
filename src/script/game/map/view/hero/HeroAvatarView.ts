...
export class HeroAvatarView extends AvatarBaseView implements ISelectMovie {
  ...
  protected nextWalk(point: Laya.Point) {
    ...
    this._endPoint = new Laya.Point(
      this._nextPoint.x * 20 + 10,
      this._nextPoint.y * 20 + 10,
    );
    ...
    this._angle = Math.atan2(
      this._endPoint.y - curTilePosY,
      this._endPoint.x - curTilePosX,
    );
    ...
    if (
      !(
        toX >= this._nextPoint.x * 20 &&
        toX < (this._nextPoint.x + 1) * 20 &&
        toY >= this._nextPoint.y * 20 &&
        toY < (this._nextPoint.y + 1) * 20
      )
    ) {
