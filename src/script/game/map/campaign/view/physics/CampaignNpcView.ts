...
import { ConsortiaEvent, PhysicsEvent } from "../../../../constant/event/NotificationEvent";
...
export class CampaignNpcView extends MapPhysicsAttackingBase {
    public static NAME: string = "map.campaign.view.physics.physicsview.CampaignNpcView";
    private _mediatorKey: string;
    private _npcName: FilterFrameText = new FilterFrameText(140, 20, undefined, 14);
    ...
    public set info(value: MapPhysics) {
        super.info = value;
        this.initMediator();
        value.info.addEventListener(PhysicsEvent.UP_STATE, this.__updateNodeStateHandler, this);
        value.addEventListener(PhysicsEvent.VISIT_USER_ID, this.__updateVisitUserIdHandler, this);
        value.addEventListener(PhysicsEvent.UPDATE_CUR_HP, this.__updateCurHpHandler, this);
        ...
        if (CampaignArmyViewHelper.checkTreeNode(<CampaignNode>value))  //公会秘境神树
        {
            this._waterLayer = new ConsortiaTreeExpView();
            this._waterLayer.x = -20;
            this.addChild(this._waterLayer);
        }
        if (CampaignArmyViewHelper.checkDemonAltarNode(<CampaignNode>value))  //公会魔神祭坛主祭坛
        {
            if (!this._bloodMc) {
                this._bloodMc = fgui.UIPackage.createObject(EmPackName.Base, "NpcBloodBar").asProgress;
            }
            ...
            ConsortiaManager.Instance.model.demonInfo.addEventListener(ConsortiaEvent.DEMON_DURA_CHANGE, this.__demonAltarDuraUpdateHandler, this);
        }
    ...
    private gvgNameColor(): number {
        let frame: number = 1;
        if (WorldBossHelper.checkGvg(this.mapId)) {
            let selfTeam: number = CampaignManager.Instance.mapModel.selfMemberData.teamId;
    ...
    private initMediator() {
        ...
        this._mediatorKey = MediatorMananger.Instance.registerMediatorList(arr, this, CampaignNpcView.NAME);
    }
    ...
    private __updateCurHpHandler(evt: PhysicsEvent) {
        ...
        if (!this._bloodMc) {
            if (WorldBossHelper.checkGvg(this.mapId)) {
                ...
                this._bloodMc = fgui.UIPackage.createObject(EmPackName.Base, "BloodBar").asProgress;
    ...
    private checkInVisituserIds(idArr: Array<number>, serverNameArr: Array<string>): boolean {
        let flag: boolean = false;
        let userId: number = PlayerManager.Instance.currentPlayerModel.playerInfo.userId;
        let serverName: string = PlayerManager.Instance.currentPlayerModel.playerInfo.serviceName;
    ...
    private get visitUserId(): boolean {
        ...
        if (nodeInfo) {
            if (nodeInfo.visitUserIds && nodeInfo.visitServerNames && nodeInfo.visitServerNames.length > 0) {
                if (this.checkInVisituserIds(nodeInfo.visitUserIds, nodeInfo.visitServerNames)) {
                    return true;
                }
            }
            else {
    ...
    protected layouCallBack() {
        ...
        if (WorldBossHelper.checkGvg(this.mapId) || CampaignArmyViewHelper.checkDemonAltarNode(this.info as CampaignNode)) {
            if (this._bloodMc) {
    ...
    public dispose() {
        ...
        if (this.info) {
            this.info.info.removeEventListener(PhysicsEvent.UP_STATE, this.__updateNodeStateHandler, this);
            this.info.removeEventListener(PhysicsEvent.VISIT_USER_ID, this.__updateVisitUserIdHandler, this);
            this.info.removeEventListener(PhysicsEvent.UPDATE_CUR_HP, this.__updateCurHpHandler, this);
            this.info.removeEventListener(PhysicsEvent.MOVE_POS, this.__movePosHandler, this);
            this.info.removeEventListener(PhysicsEvent.PARA_1, this.__para1Handler, this);
        ...
        ConsortiaManager.Instance.model.secretInfo.removeEventListener(ConsortiaEvent.TREE_STATE_UPDATE, this.__consortiaTreeStateUpdateHandler, this);
        ConsortiaManager.Instance.model.demonInfo.removeEventListener(ConsortiaEvent.DEMON_DURA_CHANGE, this.__demonAltarDuraUpdateHandler, this);
        super.dispose();
    }
