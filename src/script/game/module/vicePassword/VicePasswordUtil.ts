...
export class VicePasswordUtil {
	 ...
	 * 
	 */
	public static vpOp(type: number, newNum: string = "0", newNum2: string = "0", oldNum: string = "0"): void {
	 ...
	 * 
	 */
	public static vpLockOp(type: number, itemPos: number = 0, petId: number = 0): void {
	...
	public static numCheck(str: string): number {
		...
		if (numP.test(str) == true) {
			...
			else if (moreThan6.test(str) == true) {//没有6个或多于6个
				return 2;
			}
			else
				return 3;
	...
	public static numCheckContinuity(str: string): boolean {
		//判断是否为6位连续
		var pattern: RegExp = /^(012345|123456|234567|345678|456789|987654|876543|765432|654321|543210)$/;
		if (pattern.test(str) == true) {
			return true;
		}
		return false;
