...
export class NodeGuideMediator implements IMediator, IEnterFrame {
  ...
  public register(target: object) {
    ...
    NotificationManager.Instance.addEventListener(
      ...
      this.__resetTimeHandler,
      this,
    );
    NotificationManager.Instance.addEventListener(
      NotificationEvent.CAMPAIGN_GUIDE,
      this.__campaignGuideHandler,
      this,
    );
    NotificationManager.Instance.addEventListener(
      NotificationEvent.WORK_NEXT_NODE,
      this.__walkNextNodeHandler,
      this,
    );
  }
  ...
  private createGuideArrow() {
    if (!this._arrow) {
      this._arrow = FUIHelper.createFUIInstance(
  ...
  private walkNextNodeHandler() {
    ...
    if (!nextNode) {
      ...
      if (
        mapModel &&
        WorldBossHelper.single(mapModel.mapId) &&
        PlayerManager.Instance.currentPlayerModel.getAutoWalkFlag() ==
          PlayerModel.AUTO_WALK
      ) {
        let campaignNodeArr: Array<CampaignNode> =
          CampaignManager.Instance.mapModel.getCrossMapTransportNode(); //所有的传送点
  ...
  private checkNodeGuide() {
    ...
    if (nextNode) {
      this._aInfo = CampaignManager.Instance.mapModel.selfMemberData;
      if (!this._aInfo) return;
      this._aView = CampaignManager.Instance.controller.getArmyView(
        this._aInfo,
      );
      if (!this._aView) return;

      var vNode: DisplayObject = CampaignManager.Instance.mapModel.getNodeById(
        nextNode.info.id,
      );
      if (vNode) {
        var leng: number = new Laya.Point(
          this._aView.x,
          this._aView.y,
        ).distance(vNode.x, vNode.y);
        var fog: boolean = true;
        if (CampaignManager.Instance.mapView.fogLayer) {
          fog = CampaignManager.Instance.mapView.fogLayer.checkFogEmpty(
            vNode.x,
            vNode.y,
          );
        }
        ...
        if (this._arrow) {
          if (!this._arrow.parent)
            this._aView.addChildAt(this._arrow.displayObject, 0);
