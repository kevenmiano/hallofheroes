...
export default class OuterCityBossInfoItem extends FUI_OuterCityBossInfoItem {
    ...
    private linkClickHandler(){
        ...
        NotificationManager.Instance.dispatchEvent(OuterCityEvent.CLOSE_OUTERCITY_OUTERCITYBOSS_WND);
    }
    
    private attackBtnHandler(){
        if(SceneManager.Instance.currentType != SceneType.OUTER_CITY_SCENE){
            MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("map.outercity.OuterCityBossInfoItem"));
            return;
        }
        NotificationManager.Instance.dispatchEvent(OuterCityEvent.CLOSE_OUTERCITY_OUTERCITYBOSS_WND);
        let posX: number = this._info.posX*Tiles.WIDTH/50;
    ...
    private refreshView() {
        ...
        this.levelTxt.text = LangManager.Instance.GetTranslation("public.level2",this._info.grade);
        let x: number = parseInt((this._info.posX * Tiles.WIDTH / 50).toString());
        let y: number = parseInt((this._info.posY * Tiles.HEIGHT / 50).toString());
        this.posLink.getChild("userNameTxt").text = "[" + x + "," + y + "]";
        (this.posLink.getChild("userNameTxt") as fgui.GTextField).color = ColorConstant.LIGHT_TEXT_COLOR;
        ...
        this.statusTxt.text = LangManager.Instance.GetTranslation("map.outercity.view.bossinfo.bossStatus" + this._info.bossStatus);
        if (this._info.bossStatus == 1) {//boss状态(0已消失, 1存在, 4战斗中)
            this.bossStatusCtr.selectedIndex = 1;
        } else if(this._info.bossStatus == 0){//已经击败
            this.bossStatusCtr.selectedIndex = 0;
            this.statusTxt.color = ColorConstant.RED_COLOR;
        } else if(this._info.bossStatus == 4){//战斗中
            this.bossStatusCtr.selectedIndex = 0;
