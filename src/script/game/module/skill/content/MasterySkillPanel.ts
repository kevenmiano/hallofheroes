...
import { ExtraJobEvent, SkillEvent } from "../../../constant/event/NotificationEvent";
...
export default class MasterySkillPanel extends FUI_MasterySkillPanel {
    ...
    protected onConstruct(): void {
        super.onConstruct();
        this.masteryList.itemRenderer = Laya.Handler.create(this, this.onItemRender, null, false);
    }
    ...
    private initData() {
        this.refreshData();
        NotificationManager.Instance.addEventListener(ExtraJobEvent.LEVEL_UP, this.refreshData, this);
        DragManager.Instance.addEventListener(SkillEvent.DRAGING,this.onDraging,this);  
    }
    ...
    public updateEditMode(isSetting){
        ...
        for (let i = 0; i < this.masteryList.numChildren; i++) {
            const masteryItem: MasterySkillItem = this.masteryList.getChildAt(i) as MasterySkillItem;
            if (masteryItem) {
                for (let j = 0; j < 6; j++) {
    ...
    public dispose(){
        DragManager.Instance.removeEventListener(SkillEvent.DRAGING,this.onDraging,this);
        NotificationManager.Instance.removeEventListener(ExtraJobEvent.LEVEL_UP, this.refreshData, this);
        super.dispose();
    }
