 ...
 * 嘉年华---在线奖励
 */
export default class CarnivalOnlinePage
  extends FUI_CarnivalOnlinePage
  implements CarnivalBasePage
{
  ...
  private init() {
    ...
    this.tempList = TempleteManager.Instance.getCarnivalByType(
      CarnivalModel.TYPE_ONLINE,
    );
  ...
  private renderPageItem(index: number, box: CarnivalOnlinePageItem) {
    if (box && !box.isDisposed) {
      ...
      let onLineTimes: number =
        (new Date().getTime() - this.model.openFrameTime) / 1000 / 60;
  ...
  private refreshView(op: number = -1): void {
    ...
    var onLineTimes: number =
      (sysTime.getTime() - this.model.openFrameTime) / 1000 / 60;
    onLineTimes += this.model.dayOnline;
    //在线
    this.carnival_online.type.selectedIndex = 3;
    this.carnival_online.txt_title.text = LangManager.Instance.GetTranslation(
      "carnival.onLineTitle",
    );
    this.carnival_online.txt_value.text = LangManager.Instance.GetTranslation(
      "carnival.onLineTime",
      parseInt(onLineTimes.toString()),
    );
    ...
    if (this.tempList) this.itemlist.numItems = this.tempList.length;
  }
