...
export class BattleLoadingView extends Laya.Sprite {
    ...
    private __enterFrame() {
        ...
        if (BattleModel.battleUILoaded) {
            ...
            Logger.battle("------------enter battle ui loaded complete-----------------");
            this._callback();
            return;
        }
        if (this.stage) {
            this.update(BattleManager.Instance.resourceModel.progress(), BattleManager.Instance.resourceModel.progressString());
        }
    }
