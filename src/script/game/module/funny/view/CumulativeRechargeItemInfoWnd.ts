...
export default class CumulativeRechargeItemInfoWnd extends BaseWindow {
    ...
    OnShowWind() {
        super.OnShowWind();
        this.frame.getChild('title').text = LangManager.Instance.GetTranslation("CumulativeRechargeItemInfoWnd.Title");
        this._info = this.params.frameData;
        if (this._info) {
            this._dataArray = [];
            this.titleTxt.text = LangManager.Instance.GetTranslation("CumulativeRechargeItemInfo.titleTxt", this._info.point);
            Utils.setDrawCallOptimize(this.itemList);
            this.itemList.itemRenderer = Laya.Handler.create(this, this.renderBoxList, null, false);
            for (let i = 0; i < 8; i++) {
