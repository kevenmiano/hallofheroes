...
export default class RuneGemBagCom extends FUI_RuneBagCom {
  ...
  updateView() {
    this.runeHoldProfileFilter = (
      this.control.data as SkillWndData
    ).runeProfileFilter;
    this.updateRuneBag();
  ...
  private addEvent() {
    ...
    ServerDataManager.listen(
      S2CProtocol.U_C_RESOLVE,
      this,
      this.__onResolveResult,
    );
    NotificationManager.Instance.addEventListener(
      RuneEvent.RESOLVE_RUNE_GEM,
      this.onResolveSelect,
      this,
    );
    NotificationManager.Instance.addEventListener(
    ...
    (this.control.data as SkillWndData).addEventListener(
  ...
  private removeEvent() {
    ...
    (this.control.data as SkillWndData).removeEventListener(
      SkillWndData.CHAMGE_RUNE_FILTER,
      this.updateRuneBag,
      this,
    );
  ...
  onRecvResolve(info: GoodsInfo) {
    ...
    //分解返回百分比
    let percent =
      +TempleteManager.Instance.getConfigInfoByConfigName("runehole_decompose")
        .ConfigValue;
    percent /= 100;
    ...
    while (strengthenGrade >= indexGrade) {
      levlTmep = TempleteManager.Instance.getTemplateByTypeAndLevelAndSonType(
        indexGrade,
        200,
        SonType,
      );
    ...
    UIManager.Instance.ShowWind(EmWindow.GetGoodsAlert, {
      goodsList: [{ goodsId: -1000, count: count >> 0 }],
      callback: () => {
        this.control.reqRuneGemResolve(info.pos.toString());
      },
    });
  ...
  onResolveSelect(pos: number, profile: number) {
    ...
    if (idx >= 0) {
      ...
      if (this.posArr.length == 0) {
        this.btn_sure.title =
          LangManager.Instance.GetTranslation("runeGem.str24");
      }
    } else {
  ...
  onBuy() {
    ...
    if (str) {
      ...
      SimpleAlertHelper.Instance.Show(
        ...
        { line: 1, checkRickText: checkStr, point: price },
        prompt,
        content,
        confirm,
        cancel,
