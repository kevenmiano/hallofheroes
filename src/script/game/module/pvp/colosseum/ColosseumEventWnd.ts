...
export default class ColosseumEventWnd extends BaseWindow {
    ...
    private renderListItem(index: number, item: fgui.GComponent) {
        ...
        //多语言 不能使用拼接
        if (data.isAttack) {
            str = LangManager.Instance.GetTranslation(data.forward > 0 ? "colosseum.challenge.attacked.txt1" : "colosseum.challenge.attacked.txt2", data.tarNickName, Math.abs(data.forward))
        } else {
            str = LangManager.Instance.GetTranslation(data.forward > 0 ? "colosseum.challenge.beAttacked.txt1" : "colosseum.challenge.beAttacked.txt2", data.tarNickName, Math.abs(data.forward))
        }
        item.text = str;
    }

    public get model() {
        return FrameCtrlManager.Instance.getCtrl(EmWindow.Colosseum).data as ColosseumData
    }
}
