...
export class MountsManager {
  ...
  public static get Instance(): MountsManager {
    if (MountsManager._instance == null)
      MountsManager._instance = new MountsManager();
  ...
  private getMaxStarGrade(): number {
    ...
    let arr: t_s_upgradetemplateData[] =
      TempleteManager.Instance.getTemplatesByType(UpgradeType.MOUNT_REFINING);
    if (arr.length > 0) {
  ...
  public setup() {
    ...
    ServerDataManager.listen(
      S2CProtocol.U_C_MOUNT_UPDATE,
      this,
      this.__onMountInfoUpdate,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_MOUNT_AVATARLIST,
      this,
      this.__onMountListUpdate,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_MOUNT_UPGRADESTAR,
      this,
      this.__onMountStarUpdate,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_SHARE_REWARD,
      this,
      this.__onMountShareRewardUpdate,
    );
    NotificationManager.Instance.addEventListener(
      NativeEvent.MOUNT_SHARE_RESULT,
      this.getMountShareResultHandler,
      this,
    );
  }
  ...
  private __onMountListUpdate(pkg: PackageIn) {
    ...
    for (let i: number = 0; i < msg.avatarInfo.length; i++) {
      ...
      wildSoulInfo.activeDate = DateFormatter.parse(
        item.activeDate,
        "YYYY-MM-DD hh:mm:ss",
      ); //激活
      wildSoulInfo.expairDate = DateFormatter.parse(
        item.expairDate,
        "YYYY-MM-DD hh:mm:ss",
      ); //过期
  ...
  private __onMountStarUpdate(pkg: PackageIn) {
    ...
    var wildSoulInfo: WildSoulInfo =
      MountsManager.Instance.avatarList.getWildSoulInfo(msg.mountTempId);
    ...
    if (wildSoulInfo) {
      ...
      if (msg.starLevel > wildSoulInfo.oldStarLevel) {
        //星级改变, 派发事件
        flag = true;
      }
