...
export class WaterTreeModel extends GameEventDispatcher {
  ...
  public set defaultSelectedUser(value: ThaneInfo) {
    if (this._currentSelectedUser == value) this.commitChanges();
    else this.currentSelectedUser = value;
  }

  public set currentSelectedUser(value: ThaneInfo) {
    if (this._currentSelectedUser == value) return;
    this._currentSelectedUser = value;
    this._currentSelectedTreeInfo = WaterManager.Instance.treeList[
      this._currentSelectedUser.userId
    ] as TreeInfo;
    ...
    if (!this._currentSelectedTreeInfo) {
      ...
      WaterManager.Instance.treeList[this._currentSelectedUser.userId] =
        this._currentSelectedTreeInfo;
    }
  ...
  private commitChanges() {
    this.dispatchEvent(
      WaterEvent.SELECTED_TREE_CHANGE,
      this._currentSelectedTreeInfo,
    );
  }
