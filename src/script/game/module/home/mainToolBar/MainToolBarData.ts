...
export class MainToolBarData {
  ...
  private popGoods() {
    ...
    if (
      this._newGoodsInfo &&
      this._newGoodsInfo.bagType != BagType.Bottle &&
      MainToolBar.FLASH_NEW_GOODS
    ) {
  ...
  private initBtnDataList() {
    ...
    data.locationType = EmMainToolBarBtnLocationType.Cow;
    data.name = LangManager.Instance.GetTranslation(
      "HomeWnd.btnTitle.pveCampaign",
    );
  ...
  initBtnDataListSubdivide() {
    ...
    this.btnDataListRow1 = ArrayUtils.sortOn(
      this.btnDataListRow1,
      ["sort"],
      [ArrayConstant.NUMERIC],
    );
    this.btnDataListRow2 = ArrayUtils.sortOn(
      this.btnDataListRow2,
      ["sort"],
      [ArrayConstant.NUMERIC],
    );
    this.btnDataListCow = ArrayUtils.sortOn(
      this.btnDataListCow,
      ["sort"],
      [ArrayConstant.NUMERIC],
    );
  ...
  /** 获取某按钮数据 */
  getBtnDataByButtonType(
    buttonType: EmMainToolBarBtnType,
  ): MainToolBarButtonData {
    for (let index = 0; index < this.btnDataList.length; index++) {
  ...
  getExitCampaignMessage(): string {
    var str = LangManager.Instance.GetTranslation(
      "mainBar.MainToolBar.campaignMsg",
    );
      ...
      if (this.selfMemberData && this.selfMemberData.isDie == 1) {
        if (this.towerInfo.freeEnterCount - this.towerInfo.enterCount > 0) {
          str = LangManager.Instance.GetTranslation(
            "map.campaign.view.ui.MazeRiverView.content05",
          );
        } else {
          str = LangManager.Instance.GetTranslation(
            "map.campaign.view.ui.MazeRiverView.content02",
          );
        }
        return str;
      }
    ...
    b =
      ...
      WorldBossHelper.checkConsortiaBoss(mapId);
    if (b) {
      str = null;
    }

    let mapTemp: t_s_mapData = TempleteManager.Instance.getMapTemplateById(
      CampaignManager.Instance.mapId,
    );
    let campaignTemp: t_s_campaignData;
    if (mapTemp)
      campaignTemp = ConfigMgr.Instance.campaignTemplateDic[mapTemp.CampaignId];
    if (
      this.roomInfo &&
      this.roomInfo.isCross &&
      this.roomInfo.playerCount > 1 &&
      campaignTemp &&
      ...
      //多人本
      str = LangManager.Instance.GetTranslation(
        "mainBar.MainToolBar.leaving crossCampaign",
      );
    }
    return str;
  }
