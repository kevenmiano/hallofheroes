...
import { CampaignMapEvent, FreedomTeamEvent, NotificationEvent, ObjectsEvent, TreasureMapEvent } from '../../constant/event/NotificationEvent';
...
export default class SmallMapWnd extends BaseWindow implements IEnterFrame {
    ...
    protected initData() {
        ...
        this._selfArmyView = SpaceManager.Instance.controller.getArmyView(this._selfArmy) as SpaceArmyView;
        ...
        this._walkTarget = FUIHelper.createFUIInstance('Home', "WalkTarget") as fgui.GComponent;
    ...
    protected addEvent() {
        ...
        this.mapChildImg.on(Laya.Event.MOUSE_DOWN, this, this.__mapMouseDownHandler);
        ...
        this._model.addEventListener(CampaignMapEvent.UPDATE_WALK_TARGET, this.__updateWalkTargetHandler, this);
        this._selfArmyView.aiInfo.addEventListener(ObjectsEvent.WALK_NEXT, this.__walkNextHandler, this);
        this._selfArmyView.aiInfo.addEventListener(ObjectsEvent.WALK_OVER, this.__walkOverHandler, this);
        NotificationManager.Instance.addEventListener(FreedomTeamEvent.TEAM_INFO_UPDATE, this.__updateTeamInfoHandler, this);
        NotificationManager.Instance.addEventListener(FreedomTeamEvent.TEAM_INFO_SYNC, this.__syncTeamInfoHandler, this);
        TreasureMapManager.Instance.model.addEventListener(TreasureMapEvent.TREASURE_INFO_UPDATE, this.__treasureInfoUpdateHandler, this);
    }
    ...
    protected updateBtnState() {
        this.scaleSmallBtn.enabled = this.mapScaleTimes > this.mapScaleMinTimes && !this.enableMap;
        this.scaleBigBtn.enabled = this.mapScaleTimes < this.mapScaleMaxTimes && !this.enableMap;
        this.resetBtn.enabled = this.mapScaleTimes != 1 && !this.enableMap;
    }

    protected removeEvent() {
        EnterFrameManager.Instance.unRegisteEnterFrame(this);
        this.mapChildImg.off(Laya.Event.MOUSE_DOWN, this, this.__mapMouseDownHandler);
        this.mapChildImg.off(Laya.Event.MOUSE_UP, this, this.__mapMouseUpHandler);
        this._model.removeEventListener(CampaignMapEvent.UPDATE_WALK_TARGET, this.__updateWalkTargetHandler, this);
        this._selfArmyView.aiInfo.removeEventListener(ObjectsEvent.WALK_NEXT, this.__walkNextHandler, this);
        this._selfArmyView.aiInfo.removeEventListener(ObjectsEvent.WALK_OVER, this.__walkOverHandler, this);
        NotificationManager.Instance.removeEventListener(FreedomTeamEvent.TEAM_INFO_UPDATE, this.__updateTeamInfoHandler, this);
        NotificationManager.Instance.removeEventListener(FreedomTeamEvent.TEAM_INFO_SYNC, this.__syncTeamInfoHandler, this);
        TreasureMapManager.Instance.model.removeEventListener(TreasureMapEvent.TREASURE_INFO_UPDATE, this.__treasureInfoUpdateHandler, this);
    }
    ...
    protected __mapMouseDownHandler(event: Laya.Event) {
        ...
        this.mapChildImg.on(Laya.Event.MOUSE_MOVE, this, this.__mapMouseMoveHandler);
    }
    ...
    protected __mapMouseUpHandler(event: Laya.Event) {
        ...
        let selfImgPos = this.getSelfImgPos();
        var targetX: number = (targetPos.x - selfImgPos.x) * 10 / this.mapScaleTimes;
        var targetY: number = (targetPos.y - selfImgPos.y) * 10 / this.mapScaleTimes;
    ...
    protected __mapMouseMoveHandler(evt: Laya.Event) {
        if (this.mapScaleTimes <= 1 || !this.isMouseDown || this.enableMap)
            return;
    ...
    protected __memberClickHandler(data: any) {
        ...
        NotificationManager.Instance.sendNotification(NotificationEvent.LOCK_TEAM_FOLLOW_TARGET, userId);
    }
    ...
    */
    ...
    protected syncSelfPath(end: Laya.Point = null) {
        ...
        let selfImgPos = this.getSelfImgPos();
        if (!endPoint) {
            if (!this._selfArmyView.aiInfo.pathInfo || this._selfArmyView.aiInfo.pathInfo.length == 0) {
                return;
            }
            if (this._selfArmyView.isStand && walkIndex >= this._selfArmyView.aiInfo.pathInfo.length) return;
            lastPoint = this._selfArmyView.aiInfo.pathInfo[this._selfArmyView.aiInfo.pathInfo.length - 1];
            for (var i: number = walkIndex - 1; i < this._selfArmyView.aiInfo.pathInfo.length; i++) {
            ...
            this._walkTarget.x = this.positionConvert((endPoint.x * this.mapScaleTimes)) + selfImgPos.x;
            this._walkTarget.y = this.positionConvert((endPoint.y * this.mapScaleTimes)) + selfImgPos.y;
        } else {
            this._path = this._selfArmyView.aiInfo.pathInfo.slice();

            this._walkTarget.x = this.positionConvert((endPoint.x * this.mapScaleTimes) * 0.05) + selfImgPos.x;
            this._walkTarget.y = this.positionConvert((endPoint.y * this.mapScaleTimes) * 0.05) + selfImgPos.y;
        }
    ...
    protected showPath() {
        ...
        for (i = 0; i < this._path.length; i++) {
            ...
            if (i > 0) {
                ...
                p1 = new Laya.Point(parseInt((this._selfArmyView.x / Tiles.WIDTH).toString()), parseInt((this._selfArmyView.y / Tiles.HEIGHT).toString()));
            }
    ...
    protected initNodePosition() {
        ...
        for (i; i < list.length; i++) {
            ...
            nameText = new FilterFrameText(100, 20, undefined, 18, "#fffd4f", "center", undefined, 0);;
            ...
            if (nodeInfo.info.types == 171) {
                ...
                transfer.x = this.positionConvert(nodeInfo.curPosX * this.mapScaleTimes) - transfer.width * 0.5;
                transfer.y = this.positionConvert(nodeInfo.curPosY * this.mapScaleTimes) - 40;
                nameText.x = transfer.x - (nameText.width - transfer.width) * .5;
                ...
                if (nodeInfo.nodeId == 18) {
                    ...
                    var condition: t_s_rewardcondictionData = ringtask.conditionList[0] as t_s_rewardcondictionData;
                ...
                npc.x = this.positionConvert(nodeInfo.curPosX * this.mapScaleTimes) - npc.width * 0.5;
                npc.y = this.positionConvert(nodeInfo.curPosY * this.mapScaleTimes) - npc.height * 0.5;
                nameText.x = npc.x - (nameText.width - npc.width) * .5;
            ...
            if (nodeInfo.nodeId == 4) {//英灵兑换节点
                nameText.x = nameText.x + 15;
            } else if (nodeInfo.nodeId == 19) {//藏宝图
                nameText.x = nameText.x - 25;
            } else if (nodeInfo.nodeId == 15) {//紫晶矿场
                nameText.x = nameText.x + 15;
            } else if (nodeInfo.nodeId == 5) {//英灵岛
    ...
    public dispose() {
        ...
        this._pathContainer && this._pathContainer.removeChildren(); this._pathContainer = null;
        this._nodeContainer1 && this._nodeContainer1.removeChildren(); this._nodeContainer1 = null;
        this._nodeContainer2 && this._nodeContainer2.removeChildren(); this._nodeContainer2 = null;
        this._teamContainer && this._teamContainer.removeChildren(); this._teamContainer = null;
        this._model = null;
        this._selfArmy = null;
