...
export default class MarketSellWnd extends BaseWindow {
    ...
    OnHideWind(): void {
        ...
        MarketManager.Instance.removeEventListener(MarketManager.OnMarketSellList, this.OnMarketSellList, this);
    ...
    private initTranslate() {
        ...
        this.sellTxt.text = LangManager.Instance.GetTranslation("Market.sell.sellPrice");
        this.buyTxt.text = LangManager.Instance.GetTranslation("Market.sell.buyPrice");
        ...
        this.pushBtn.title = LangManager.Instance.GetTranslation("Market.sell.pushBtn");
    }

    private addEvent() {
        this.pushBtn.onClick(this, this.onPushBtnTap);
        this.sellGoodsList.setVirtual();
        this.sellGoodsList.itemRenderer = Laya.Handler.create(this, this.onItemRender, null, false);
        this.sellGoodsList.on(fairygui.Events.CLICK_ITEM, this, this.onItemClick);
        MarketManager.Instance.addEventListener(MarketManager.OnMarketSellList, this.OnMarketSellList, this);
    ...
    private onItemRender(index: number, item: MarkGoodsItem) {
        ...
        item.selectedCrl.selectedIndex = this.curItem == item.MarketGoodsInfo ? 1 : 0;
    }
    ...
    private OnMarketSellList(msg: MarketItemSellInfoMsg) {
        if (this.curItem && this.curItem.goodsInfo.templateId == msg.templateId) {
            this.msg = msg;
            this.sellCountLab.text = LangManager.Instance.GetTranslation("Market.sell.buyMaxPrice", msg.purchaseMaxPrice == 0 ? "--" : msg.purchaseMaxPrice);
            this.buyCountLab.text = LangManager.Instance.GetTranslation("Market.sell.buyMinPrice", msg.sellMinPrice == 0 ? "--" : msg.sellMinPrice);
            this.goodsNameLab.text = this.curItem.goodsInfo.templateInfo.TemplateNameLang;
            this.goodsNameLab.color = GoodsSonType.getColorByProfile(this.curItem.goodsInfo.templateInfo.Profile);
            this.goodsItem.MarketGoodsInfo = this.curItem;
            let limit = this.curItem.limitInfo;

            if (!this.changeHandler) {
                this.changeHandler = Laya.Handler.create(this, this.onPriceChange, null, false);
            }
            ...
            this.sellStepper.show(0, defaultPoint, limit.MinPrice, limit.MaxPrice, limit.MaxPrice, priceStep, this.changeHandler);
            this.onPriceChange();
    ...
    private onPushBtnTap() {
        ...
        //最大求购订单
        let myMaxOrder = MarketManager.Instance.getSellOrderMaxByTemplateId(this.curItem.goodsInfo.templateId);
        //您正以更高价格求购相同物品
        if (myMaxOrder && this.sellStepper.value <= myMaxOrder.point) {
            MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("Market.buy.errorSame"));
            return;
        }
    ...
    private checkConfirm() {
        ...
        let content: string = LangManager.Instance.GetTranslation("Market.sell.pushTips1", this.payGold);
        UIManager.Instance.ShowWind(EmWindow.TodayNotAlert, { content: content, state: 2, backFunction: this.confirmSell.bind(this), closeFunction: null });
    }
    ...
    private sendBuy() {
        ...
        MarketManager.Instance.reqMarketOrderOpertion(1, "", 1, this.curItem.goodsInfo.templateId, count, price);
    }
    ...
    private resetView() {
        this.curItem = null;
        this.sellCountLab.text = LangManager.Instance.GetTranslation("Market.sell.buyMaxPrice", "--");
        this.buyCountLab.text = LangManager.Instance.GetTranslation("Market.sell.buyMinPrice", "--");
    ...
    public onCheckBtnClick() {
        ...
        if (haveNot) {
            MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("Market.sell.notInfoTip"));
            return;
        }
        MarketManager.Instance.reqMarketOpertion(5, this.curItem.goodsInfo.templateId);
    }
}