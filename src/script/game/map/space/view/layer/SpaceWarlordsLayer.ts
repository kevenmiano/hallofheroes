...
export default class SpaceWarlordsLayer implements IEnterFrame {
    ...
    private addEvent() {
        ...
        this.model && this.model.addEventListener(WarlordsEvent.TOP_AVATAR_CHANGE, this.__topAvatarChangeHandler, this);
    }

    private removeEvent() {
        EnterFrameManager.Instance.unRegisteEnterFrame(this);
        this.model && this.model.removeEventListener(WarlordsEvent.TOP_AVATAR_CHANGE, this.__topAvatarChangeHandler, this);
    }
    ...
    public refresh() {
        this._wizardWarlords = this.getUpdateArmyView(this._wizardWarlords, JobType.WIZARD, 80, 2289, 2155);
        this._warriorWarlords = this.getUpdateArmyView(this._warriorWarlords, JobType.WARRIOR, 80, 2406, 2211);
        this._shooterWarlords = this.getUpdateArmyView(this._shooterWarlords, JobType.HUNTER, 80, 2527, 2270);
        ...
        if (this._container && this._wizardWarlords && !this._wizardWarlords.parent) this._container.addChild(this._wizardWarlords);
        if (this._container && this._warriorWarlords && !this._warriorWarlords.parent) this._container.addChild(this._warriorWarlords);
        if (this._container && this._shooterWarlords && !this._shooterWarlords.parent) this._container.addChild(this._shooterWarlords);
    }

    private getUpdateArmyView($armyView: SpaceArmyView, $index: number, $angle: number, $x: number, $y: number): SpaceArmyView {
        var warlordsInfo: WarlordsPlayerInfo;
        var armyInfo: SpaceArmy;
        var aiInfo: AIBaseInfo;
        var armyView: SpaceArmyView = $armyView;
        warlordsInfo = this.model.getListData(WarlordsModel.TOP, $index) as WarlordsPlayerInfo;
    ...
    public enterFrame() {
        if (this._wizardWarlords && this._wizardWarlords.parent) this._wizardWarlords.execute();
        if (this._warriorWarlords && this._warriorWarlords.parent) this._warriorWarlords.execute();
        if (this._shooterWarlords && this._shooterWarlords.parent) this._shooterWarlords.execute();
    }
    ...
    public dispose() {
        this.removeEvent();
        ObjectUtils.disposeObject(this._wizardWarlords); this._wizardWarlords = null;
        ObjectUtils.disposeObject(this._warriorWarlords); this._warriorWarlords = null;
        ObjectUtils.disposeObject(this._shooterWarlords); this._shooterWarlords = null;
