...
export default class H5SDKChannel extends BaseChannel {
    ...
    initRoad_7(param?: any) {
        return new Promise<any>((resolve, reject) => {
            ...
            } else if (urlChannelID == H5SDK_CHANNEL_ID.HY || urlChannelID == H5SDK_CHANNEL_ID.HY_S) {
                libUrl = "libs/h5sdk/hy7RoadGame.js" + dateStr;
            } else if (urlChannelID == H5SDK_CHANNEL_ID.DY || urlChannelID == H5SDK_CHANNEL_ID.DY_S) {
            ...
            window.Road_7 = window.loadLib(libUrl, () => {
                ...
                //@ts-ignore
                window.Road_7.init(initData).then(async (res) => {
                    // 初始化成功回调
     ...
     * @param data 数据
     */
    pay(type: number, orderId: string, productId: string, channelProductId: string, productName: string, productDesc: string, money: string, showCoin: number, gameExInfo: any, count: number, coinType: string, virtualCoinType: string, oExInfo?: string): void {
        ...
        let productImg = "https://ts2-h5.7road.net/res/charge/Img_Top_up_" + productType + ".png";
        if (!gameExInfo) {
            gameExInfo = {
        ...
        //@ts-ignore
        window.Road_7.order(data).then((res) => {
            // 支付成功回调
