 ...
 * 嘉年华---欢乐游戏
 */
export default class CarnivalGamePage
  extends FUI_CarnivalGamePage
  implements CarnivalBasePage
{
  ...
  addEvent() {
    ...
    MouseMgr.Instance.register(
      this.displayObject,
      null,
      this.__onMouseDownHandler.bind(this),
      this.__onMouseUpHandler.bind(this),
      null,
    );
  }
  ...
  private initView() {
    ...
    for (var i: number = 1; i < 4; i++) {
      let tempInfo = TempleteManager.Instance.getCarnivalByType(
        i + CarnivalModel.TypeAdd,
      );
      let tempData = null;
      if (tempInfo.length) {
        tempData = tempInfo[0];
    ...
    this.carnival_reset.txt_title.text = LangManager.Instance.GetTranslation(
      "carnival.today.reset",
    );
  ...
  private moveGameItem(moveCount: number) {
    ...
    for (i = 0; i < Math.abs(moveCount); i++) {
      ...
      if (this._centerGameItemIndex < 0)
        this._centerGameItemIndex = this._centerGameItemIndex = itemsCount - 1;
      if (this._centerGameItemIndex > itemsCount - 1)
        this._centerGameItemIndex = 0;
    }
    ...
    TweenMax.to(this._gameItems[this._centerGameItemIndex], 0.3, {
      x: this.startX + 232,
      y: this.startY + 5,
      scaleX: 1,
      scaleY: 1,
      ...
      onCompleteParams: [
        this._gameItems[this._centerGameItemIndex],
        flag,
        itemsCount,
      ],
    });
    ...
    //左侧
    var leftItemIndex: number =
      this._centerGameItemIndex - 1 < 0
        ? this._gameItems.length - 1
        : this._centerGameItemIndex - 1;
    this._gameItems[leftItemIndex].Enabled = false;
    this._gameItems[leftItemIndex].index = -1;
    TweenMax.to(this._gameItems[leftItemIndex], 0.1, {
      x: this.startX + 5,
      y: this.startY + 30,
      scaleX: 0.9,
      scaleY: 0.9,
      alpha: 0.5,
      onComplete: this.onTweenComplete,
      onCompleteParams: [this._gameItems[leftItemIndex], flag, itemsCount],
    });

    //右侧
    var rightItemIndex: number =
      this._centerGameItemIndex + 1 > this._gameItems.length - 1
        ? 0
        : this._centerGameItemIndex + 1;
    this._gameItems[rightItemIndex].Enabled = false;
    this._gameItems[rightItemIndex].index = 1;
    TweenMax.to(this._gameItems[rightItemIndex], 0.1, {
      x: this.startX + 495,
      y: this.startY + 30,
      scaleX: 0.9,
      scaleY: 0.9,
      alpha: 0.5,
      onComplete: this.onTweenComplete,
      onCompleteParams: [this._gameItems[rightItemIndex], flag, itemsCount],
    });
  ...
  onTweenComplete(
    tweenItem: CarnivalGamePageItem,
    flag: number,
    count: number,
  ) {
