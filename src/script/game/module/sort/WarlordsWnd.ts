...
export default class WarlordsWnd extends BaseWindow {
  ...
  public OnInitWind() {
    ...
    let titleText = LangManager.Instance.GetTranslation(
      "warlords.WarlordsRankFrame.str01",
    );
  ...
  private addEvent() {
    this.tablist.on(
      Laya.Event.CLICK,
      this,
      this.__onSelectedChangeHandler.bind(this),
    );
  ...
  private initWarsTabList() {
    ...
    this.createTabData(
      ...
      LangManager.Instance.GetTranslation("yishi.datas.consant.JobType.Name02"),
      "Tab_Dia2_Nor",
      "Tab_Dia2_Sel",
      this.rankWarlordsTabData,
    );
    this.createTabData(
      WARLORDS_TAB.WIZARD,
      LangManager.Instance.GetTranslation("yishi.datas.consant.JobType.Name04"),
      "Tab_Dia2_Nor",
      "Tab_Dia2_Sel",
      this.rankWarlordsTabData,
    );
    this.createTabData(
      WARLORDS_TAB.HUNTER,
      LangManager.Instance.GetTranslation("yishi.datas.consant.JobType.Name03"),
      "Tab_Dia2_Nor",
      "Tab_Dia2_Sel",
      this.rankWarlordsTabData,
    );
    ...
    if (ThaneInfoHelper.getJob(ArmyManager.Instance.thane.job) - 1 == 0) {
      ...
      ThaneInfoHelper.getJob(ArmyManager.Instance.thane.job) - 1 ==
      1
    ) {
  ...
  private setTabData() {
    var totalList = this.smodel.getListData(
      WarlordsModel.LAST_RANK,
      this._curSelectedJob,
    );
    let temp = [];
    if (totalList) {
  ...
  private __playerInfoUpdateHandler(evt) {
    this.para2
      .setVar("value", this.playerInfo.fightingCapacity.toString())
      .flushVars();
  }
  ...
  public refresh() {
    ...
    this.time
      ...
      .flushVars();
    if (
      this.smodel.lastSelfRank >= 1 &&
      this.smodel.lastSelfRank <= WarlordsModel.DIVIDING_RANK
    ) {
      this.para1
        .setVar("value", this.smodel.lastSelfRank.toString())
        .flushVars();
    } else {
      this.para1
        .setVar(
    ...
    if (this.smodel.rankListCreateDate) {
      this.time
        .setVar(
          "time",
          DateFormatter.format(this.smodel.rankListCreateDate, "MM-DD hh:mm"),
        )
        .flushVars();
    } else {
    ...
    this.para2
      .setVar(
        "text",
        LangManager.Instance.GetTranslation(
          "warlords.WarlordsBetSelectFrame.ap",
        ) + ": ",
      )
      .flushVars();
    this.para2
  ...
  private __onSelectedChangeHandler() {
    ...
    if (
      this.smodel.isExistData(WarlordsModel.LAST_RANK, this._curSelectedJob)
    ) {
      this.refresh();
      this.setTabData();
    } else {
  ...
  private layout() {
    var titles: Array<string> = [
      ...
      LangManager.Instance.GetTranslation(
        "warlords.WarlordsBetSelectFrame.name",
      ),
      ...
      LangManager.Instance.GetTranslation(
        "warlords.room.WarlordsFinalRoomRightView.winTitle",
      ),
    ];
