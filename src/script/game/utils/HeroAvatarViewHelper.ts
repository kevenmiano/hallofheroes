...
import Tiles from "../map/space/constant/Tiles";
	...
	export class HeroAvatarViewHelper
	{
		...
		public static twoPointAngle(px:number,py:number,ex:number,ey:number) : number
		{//计算两个点之间的角度
			var vx : number = px - ex;
			var vy : number = py - ey;
		...
		public static directionMove(direction:string,speed:number,target:DisplayObject) 
		{//指定方向上的移动
			...
			switch(direction)
			{
				...
				case PointDirectionHelper.R_T:
					...
					break;
				default:
					var errorTip:string = LangManager.Instance.GetTranslation("map.internals.view.hero.helper.HeroAvatarViewHelper.errorTip") ;
					throw new Error(errorTip);
			}
		}
		public static checkDirectionBySetup(setup : number,next :Laya.Point,info:AIBaseInfo) : number
		{//按步长得出角度
			var off      : number = setup - info.walkIndex % setup;
			var index    : number = (info.pathInfo.length > info.walkIndex + off ? info.walkIndex+off : info.pathInfo.length-1);
			
			index = (index > setup ? index - setup : 0);
			index = info.walkIndex+1;
			var prePoint : Laya.Point = new Laya.Point(info.pathInfo[index].x*20,info.pathInfo[index].y*20);
			return HeroAvatarViewHelper.twoPointAngle(next.x,next.y,prePoint.x,prePoint.y);
		}
		public static intervalPoint(setup : number,next :Laya.Point,info:AIBaseInfo) : Laya.Point
		{//按步长得出角度
			var off      : number = setup - info.walkIndex % setup;
			var index    : number = (info.pathInfo.length > info.walkIndex + off ? info.walkIndex+off : info.pathInfo.length-1);
			return new Laya.Point(info.pathInfo[index].x*20,info.pathInfo[index].y*20);
		}
		
		public static getNextPoint(pre:Laya.Point,cur:Laya.Point,next:Laya.Point) : Laya.Point
		{//下一个达到的点
