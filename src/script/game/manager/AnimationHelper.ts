...
export class AnimationBaseData {
    constructor(effectName?: string, cacheName?: string, fullUrl?: string, prefix?: string) {
...
export class AnimationHelper {
     ...
     * @returns 
     */
    public static getSkillEffectAniData(effectName: string, transition: boolean = true, toLowerCase: boolean = true): AnimationBaseData {
        let aniData = new AnimationBaseData(effectName)  // TODO 对象池

        aniData.fullUrl = PathManager.solveSkillResPath(effectName, transition, toLowerCase)
        ...
        if (resJson) {
            ...
            Logger.warn("[AnimationHelper]getSkillEffectAniData资源未加载 部分数据获取失败")
        }
    ...
    public static getAniCache(aniData: AnimationBaseData): boolean {
        ...
        let success = false
        if (!cacheJson) {
            success = AnimationManager.Instance.createAnimation(prefix, "", 0, "", AnimationManager.BattleEffectFormatLen)
        } else {
            success = true
        }
        return success
     ...
     */
    public static clearAniCache(aniData: AnimationBaseData, releaseTexure: boolean = true) {
