...
export default class OuterCityWarDefenceSettingItem extends FUI_OuterCityWarDefenceSettingItem {
  ...
  public onConstruct() {
    ...
    this.txtDefenceState.text = LangManager.Instance.GetTranslation(
      "outerCityWar.defancing",
    );
  }
  ...
  public refreshView() {
    if (this._info) {
      ...
      this.txtDefenceBuild.text = this.buildInfo.isPetBuild
        ? this._info.defenseSitePetName
        : this._info.defenseSiteName;

      this.txtPetCapaity.text = this._info
        .getPetCapaityByType(this.buildInfo.petType)
        .toString();
      this.txtPetTotalCapaity.text = this._info.getTotalPetCapaity().toString();

      let state = this.buildInfo.isPetBuild
        ? this._info.statePet
        : this._info.state;
      switch (state) {
        ...
        case EmOuterCityWarPlayerState.DEFANCE:
          /** 相同建筑不可更换，不同建筑可更换驻防点 */
          let defenceSite = this.buildInfo.isPetBuild
            ? this._info.defenseSitePet
            : this._info.defenseSite;
  ...
  private btnConfirmClick() {
    ...
    if (this.info && this.buildInfo) {
      OuterCityWarManager.Instance.sendDefenceBuild(
        this.info.userId,
