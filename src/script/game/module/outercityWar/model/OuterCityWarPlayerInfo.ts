...
export class OuterCityWarPlayerInfo {
  ...
  set pawnTempId(v: number) {
    if (v < 0) {
      ...
      this._pawnTempId = v;
      this.pawnTempInfo = ConfigMgr.Instance.getTemplateByID(
  ...
  set defenseSite(v: number) {
    ...
    this.defenseSiteTemp = ConfigMgr.Instance.getTemplateByID(
  ...
  set defenseSitePet(v: number) {
    ...
    this.defenseSitePetTemp = ConfigMgr.Instance.getTemplateByID(
      ConfigType.t_s_buildingtemplate,
      tempId,
    );
  }
  ...
  public getPawnName(): string {
    if (this.pawnTempInfo) {
      return (
        this.pawnTempInfo.PawnNameLang +
        LangManager.Instance.GetTranslation(
          "public.level2",
          this.pawnTempInfo.Level,
        )
      );
    }
    return "";
