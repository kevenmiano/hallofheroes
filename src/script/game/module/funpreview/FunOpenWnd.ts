...
import t_s_levelupprompt, {
  t_s_leveluppromptData,
} from "../../config/t_s_levelupprompt";
import t_s_systemopentips, {
  t_s_systemopentipsData,
} from "../../config/t_s_systemopentips";
...
export default class FunOpenWnd extends BaseWindow {
  ...
  private parseData() {
    ...
    if (this.newFunData && this.newFunData.length > 0) {
      ...
      this.frame.getChild("title").asTextField.text =
        LangManager.Instance.GetTranslation("FunOpen.title");
      ...
      SharedManager.Instance.saveFunOpenType(
        curFun.Type,
        curFun.Order,
        curFun.Grade,
      );
      ...
      if (this.listdata.length) {
        this.frame.getChild("title").asTextField.text =
          LangManager.Instance.GetTranslation(
            "funpreview.openLevelTxt",
            this.listdata[0].Grade,
          );
      }
    ...
    switch (this.newFunData[0].Type) {
      ...
      case 5: //世界Boss
        if (!this.playerInfo.worldbossState) {
          MessageTipManager.Instance.show(
            LangManager.Instance.GetTranslation("FunOpen.wroldBoss"),
          );
        } else {
      ...
      case 9: //战场
        if (!this.worldFightState) {
          MessageTipManager.Instance.show(
            LangManager.Instance.GetTranslation("FunOpen.fight"),
          );
        } else {
