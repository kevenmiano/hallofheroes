...
import { FriendUpdateEvent, RequestInfoEvent } from "../../../../constant/event/NotificationEvent";
...
export default class ConsortiaPlayerMenu extends BaseWindow {
    ...
    private btnTextKeys = ["MESSAGE", "TEAMINVITE", "ADDFRI", "DEMOTION", "PROMOTION", "KICKOUT"];
    private btnTextStrs = {
        MESSAGE: LangManager.Instance.GetTranslation("yishi.view.PlayerMenu.info"), //查看信息
        TEAMINVITE: LangManager.Instance.GetTranslation("yishi.view.PlayerMenu.invite"),//邀请组队
        ADDFRI: LangManager.Instance.GetTranslation("yishi.view.PlayerMenu.addFriend"),//添加好友
        DELFRI: LangManager.Instance.GetTranslation("yishi.view.PlayerMenu.delFriend"),//删除好友
        PROMOTION: LangManager.Instance.GetTranslation("yishi.view.PlayerMenu.demotion"),
        DEMOTION: LangManager.Instance.GetTranslation("yishi.view.PlayerMenu.promotion"),
        KICKOUT: LangManager.Instance.GetTranslation("yishi.view.PlayerMenu.kickOutConsortia"),
    };

    public static Show(selfThane: ThaneInfo, itemThane: ThaneInfo, point: Laya.Point,contributeStr:string,jianSeStr:string) {
        let obj = {
            selfThane: selfThane,
            itemThane: itemThane,
            point: point,
    ...
    OnShowWind() {
        ...
        if(this.contentPane.y + this.contentPane.height > StageReferance.stageHeight){
        ...
        this.descTxt.text = LangManager.Instance.GetTranslation("ConsortiaPlayerMenu.descTxt");
        this.descTxt2.text = LangManager.Instance.GetTranslation("ConsortiaPlayerMenu.descTxt2");
    ...
    private addEvent() {
        this._itemThane.addEventListener(PlayerEvent.CONSORTIA_DUTY_CHANGE, this.__onDutyUpdata, this);
        FriendManager.getInstance().addEventListener(FriendUpdateEvent.FRIEND_UPDATE, this.__onFriendInfoUpdata, this);
    }

    private delEvent() {
        this._itemThane.removeEventListener(PlayerEvent.CONSORTIA_DUTY_CHANGE, this.__onDutyUpdata, this);
        FriendManager.getInstance().removeEventListener(FriendUpdateEvent.FRIEND_UPDATE, this.__onFriendInfoUpdata, this);
    }
    ...
    private __onFriendInfoUpdata() {
        var fInfo: FriendItemCellInfo = IMManager.Instance.getFriendInfo(this._itemUserID);
        if (fInfo) {
    ...
    private setBtnState() {
        ...
        var flag: boolean = this._contorller.getRightsByIndex(ConsortiaDutyInfo.CHANGEDUTY);
        if (flag && !this.closeRights) {
            if (this._selfThane.dutyId && (this._selfThane.dutyId < this._itemThane.dutyId && this._selfThane.dutyId != ConsortiaDutyLevel.ELITE)) {
                if (this._itemThane.dutyId > ConsortiaDutyLevel.VICE_CHAIRMAN && (this._selfThane.dutyId < this._itemThane.dutyId - 1 && this._selfThane.dutyId != ConsortiaDutyLevel.ELITE)) this.promotionBtn.enabled = true;
                if (this._itemThane.dutyId < ConsortiaDutyLevel.ELITE) this.demotionBtn.enabled = true;
            }
            else {
                this.promotionBtn.enabled = false;
                this.demotionBtn.enabled = false;
        ...
        if (flag && !this.closeRights) {
            if (this._selfThane.dutyId && (this._selfThane.dutyId < this._itemThane.dutyId && this._selfThane.dutyId != ConsortiaDutyLevel.NORMAL)) this.fireBtn.enabled = true;
    ...
    private __messageHandler() {
        ...
        if (this._serverName && this._serverName != PlayerManager.Instance.currentPlayerModel.playerInfo.serviceName) {
            PlayerInfoManager.Instance.sendRequestSimpleInfoCross(this._itemUserID, this._serverName);
        } else {
    ...
    private __friendHandler() {
        var str: string;
        if (ArmyManager.Instance.thane.grades < GlobalConfig.Communication.AddFriend) {
        ...
        if (fInfo) {
            switch (fInfo.relation) {
                ...
                case RelationType.BLACKLIST:
                    ...
                    FriendManager.getInstance().sendAddFriendRequest(this._itemName, RelationType.FRIEND);
                    ConsortiaPlayerMenu.Hide();
                    break;
            }
        } else {
    ...
    private __kickOutHandler() {
        var content = LangManager.Instance.GetTranslation("consortia.view.myConsortia.ConsortiaPlayerMenu.content");
        SimpleAlertHelper.Instance.Show(null, null, null, content, null, null, (b: boolean, check: boolean, data: any) => {
            if (b) {
                ConsortiaPlayerMenu.Hide();
