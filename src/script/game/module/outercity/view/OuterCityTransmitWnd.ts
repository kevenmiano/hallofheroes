...
export class OuterCityTransmitWnd extends BaseWindow {
  ...
  private initData() {
    ...
    this._maxPosX = Math.floor(
      Math.abs((stageWidth / 2 - this._mapView.width) / 50),
    );
    this._maxPosY = Math.floor(
      Math.abs((stageHeight / 2 - this._mapView.height) / 50),
    );
  }
  ...
  private initView() {
    ...
    let posX: number = Math.floor(
      (StageReferance.stageWidth / 2 - this._mapView.x) / 50,
    );
    let posY: number = Math.floor(
      (StageReferance.stageHeight / 2 - this._mapView.y) / 50,
    );
    posX = posX < 0 ? 0 : posX;
    posY = posY < 0 ? 0 : posY;
  ...
  private onXTextChange() {
    ...
    this.txt_xPos.text = Math.min(
      Math.max(this._minPosX, xPos),
      this._maxPosX,
    ).toString();
  }
  ...
  private onYTextChange() {
    ...
    this.txt_yPos.text = Math.min(
      Math.max(this._minPosY, yPos),
      this._maxPosY,
    ).toString();
  }
  ...
  private onBtnSureClick() {
    ...
    if (posX > 0 && posY > 0) {
      this._mapView.motionTo(
        new Point(
          posX * 50 - StageReferance.stageWidth / 2,
          posY * 50 - StageReferance.stageHeight / 2,
        ),
      );
      this.hide();
    }
