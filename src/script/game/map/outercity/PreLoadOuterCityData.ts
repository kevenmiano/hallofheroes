...
export class PreLoadOuterCityData extends PreLoadMapData {
    ...
    constructor(model: MapInfo, upProView: boolean = true, needSaveData:boolean = false) {
        super(model);
    ...
    protected loadData(): void {
        ...
        this._stepTimeId1 = setTimeout(this.loadSteps.bind(this), 10, 1);
        this._stepTimeId2 = setTimeout(this.loadSteps.bind(this), 100, 2);
        this._stepTimeId3 = setTimeout(this.loadSteps.bind(this), 180, 3);
    }
    ...
    private loadPreProgress(): void {
        ...
        this._loadPreCount++;
        if (this._upProView) {
            LoadingSceneWnd.Instance.update(this._loadPreCount / this._loadTotal * 100, this._loadPreCount + "/" + this._loadTotal, true);
        }
    ...
    private loadPro(isOver: boolean, cur: number, total: number): void {
        ...
        if (this._upProView) {
            LoadingSceneWnd.Instance.update(cur / this._loadTotal * 100, cur + "/" + this._loadTotal, true);
        }
        if (isOver) {
            this.backCall();
        }
