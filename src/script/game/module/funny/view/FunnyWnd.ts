...
import { FeedBackEvent, FunnyEvent, NotificationEvent } from "../../../constant/event/NotificationEvent";
...
export default class FunnyWnd extends BaseWindow {
	...
	private initShowData() {
		...
		if (AllManExchangeManager.Instance.model.isopen) {
			...
			excData.title = LangManager.Instance.GetTranslation("allmainexchange.str1");
			this._showData.push(excData);
		}
		if (BottleManager.Instance.openBottle) {
			...
			excData.title = LangManager.Instance.GetTranslation("bottle.view.BottleFrame.titleText");
			this._showData.push(excData);
		}
		if (ChargeLotteryManager.instance.openChargeLottery && ChargeLotteryManager.instance.chargeMsg) {
			///添加充值轮盘按钮
			let excData = new FunnyData();
			excData.type = FunnyType.RECHARGE_LOTTERY;
			excData.id = ChargeLotteryManager.instance.chargeMsg.activeId;
			excData.title = LangManager.Instance.GetTranslation("yishi.ChargeLotteryManager.title");
			this._showData.push(excData);
		...
		if (LuckBlindBoxManager.Instance.luckBlindBoxOpen) {
			...
			excData.title = LangManager.Instance.GetTranslation("yishi.LuckBlindBoxManager.title");
			this._showData.push(excData);
		...
		if(SuperGiftOfGroupManager.Instance.open){
			...
			excData.title = LangManager.Instance.GetTranslation("yishi.SuperGiftOfGroupManager.title");
			this._showData.push(excData);
		}
		if (MemoryCardManager.Instance.model.isOpen) {
			...
			excData.title = LangManager.Instance.GetTranslation("NewMemoryCardFrame.Activity.TabTitle");
			this._showData.push(excData);
		}
		for (var i: number = 0; i < this.funnyData.length; i++) {
			if (this.funnyData[i].showEnd < this.playerModel.nowDate || this.funnyData[i].showStart > this.playerModel.nowDate) continue; //时间选择活动
			if (this.funnyData[i].type == FunnyType.TYPE_ONLINE) continue; //不显示在线时长奖励
			if (this.funnyData[i].type == FunnyType.TYPE_LEAVE && !this.playerInfo.isBackPlayer) continue; //老玩家回归活动
			if (this.funnyData[i].type == FunnyType.TYPE_DELETE_CHARGE && this.funnyData[i].state == -3) continue;
			if (this.funnyData[i].type == FunnyType.TYPE_DELETE_FILELEVEL && this.funnyData[i].state == -3) continue;
		...
		if (FoisonHornManager.Instance.model.isOpen) {
			...
			excData.title = LangManager.Instance.GetTranslation("foisonHornview.title");
			this._showData.push(excData);
		}
		//添加激活码按钮
		if (ConfigManager.info.IOS_ACTIVITY_CODE) {
			...
			activityData.title = LangManager.Instance.GetTranslation("mainBar.SmallMapBar.activityRewardBtnTipData");
		...
		if (FeedBackManager.Instance.switchBtn1) {
			...
			activity.anyToGet = true;
			activity.title = LangManager.Instance.GetTranslation("feedback.FeedBackFrame.title");
	...
	private refreshView() {
		...
		for (var i: number = 0; i < count; i++) {
			...
			if (this._showData[i].showEnd < this.playerModel.nowDate || this._showData[i].showStart > this.playerModel.nowDate) continue; //时间选择活动
	...
	onEvent() {
		...
		FunnyManager.Instance.addEventListener(FunnyEvent.REFRESH_ITEM, this.__refreshHandler, this);
		FunnyManager.Instance.addEventListener(FunnyEvent.REFRESH_TEMP, this.__receiveTempHandler, this);
		NotificationManager.Instance.addEventListener(FunnyEvent.REMAIN_STATE, this.__refreshHandler, this);
		FeedBackManager.Instance.addEventListener(FeedBackEvent.FEEDBACK_RECEIVE_USERINFO, this.__refreshHandler, this);
		NotificationManager.Instance.addEventListener(NotificationEvent.MEMORYCARD_STATE_UPDATE, this.__refreshMemoryCardState, this);
		NotificationManager.Instance.addEventListener(NotificationEvent.FOISONHORN_RED_CHANGE, this.refreshFoisonRed, this);
		NotificationManager.Instance.addEventListener(NotificationEvent.FOISONHORN_INFO_CHANGE, this.refreshFoisonRed, this);
		NotificationManager.Instance.addEventListener(NotificationEvent.CHARGELOTTERY_RESULT_UPDATE, this.refreshLoggerHandler, this);
	}
	...
	offEvent() {
		...
		this.tabList.off(fairygui.Events.CLICK_ITEM, this, this.__onTabSelectHandler);
		this.activityCodeTab.offClick(this, this.onActivityCodeHandler);
		FunnyManager.Instance.removeEventListener(FunnyEvent.REFRESH_ITEM, this.__refreshHandler, this);
		FunnyManager.Instance.removeEventListener(FunnyEvent.REFRESH_TEMP, this.__receiveTempHandler, this);
		NotificationManager.Instance.removeEventListener(FunnyEvent.REMAIN_STATE, this.__refreshHandler, this);
		FeedBackManager.Instance.removeEventListener(FeedBackEvent.FEEDBACK_RECEIVE_USERINFO, this.__refreshHandler, this);
		NotificationManager.Instance.removeEventListener(NotificationEvent.MEMORYCARD_STATE_UPDATE, this.__refreshMemoryCardState, this);
		NotificationManager.Instance.removeEventListener(NotificationEvent.FOISONHORN_RED_CHANGE, this.refreshFoisonRed, this);
		NotificationManager.Instance.removeEventListener(NotificationEvent.FOISONHORN_INFO_CHANGE, this.refreshFoisonRed, this);
		NotificationManager.Instance.removeEventListener(NotificationEvent.CHARGELOTTERY_RESULT_UPDATE, this.refreshLoggerHandler, this);
	}
	...
	setBtnData(item: fgui.GButton, itemData: FunnyData) {
		...
		if (itemData.type == FunnyType.ACTIVITY_CODE_TYPE) {
			...
			item.selectedIcon = FUIHelper.getItemURL(EmPackName.Base, "Tab_Event2_Sel");
		}
		item.data = itemData;
	}
	...
	checkItemAward(data: FunnyData) {
		...
		//时间选择活动
		if (data.showEnd < this.playerModel.nowDate || data.showStart > this.playerModel.nowDate) {
			awardState = false;
		} else {
	...
	private refreshFoisonRed() {
		...
		this.setTabBtnState(LangManager.Instance.GetTranslation("foisonHornview.title"), flag);
	}
	...
	private refreshLoggerHandler() {
		...
		this.setTabBtnState(LangManager.Instance.GetTranslation("yishi.ChargeLotteryManager.title"), flag);
	}
				...
				if (data.type == FunnyConditionType.TYPE_DELETE_FILELEVEL) {
					...
					for (const key in data.bagList) {
						if (Object.prototype.hasOwnProperty.call(data.bagList, key)) {
							...
							if (element.conditionList[0].id == FunnyConditionType.TYPE_DELETE_FILELEVEL) {
	...
	__onTabSelectHandler() {
		...
		if (this._showData[tabIndex]) {
			...
			if (this._showData[tabIndex].type == FunnyType.LUCKY_EXCHANGE) {
				LuckyExchangeManager.Instance.idMap.set(this._showData[tabIndex].id, false);
				NotificationManager.Instance.dispatchEvent(NotificationEvent.LUCK_EXCHANGE_BTN_STATUS_CHANGE);
			...
			if (this._showData[tabIndex].type == FunnyType.FOISON_HORN) {
				...
				NotificationManager.Instance.dispatchEvent(NotificationEvent.FOISONHORN_RED_CHANGE);
			}
		}
	}
	...
	onShowContentType(type, atyId: string) {
		...
		if (!content) {
			...
			if (!content) { Logger.warn("不存在" + mapURL); return }
		}
	...
	private removeContent() {
		while (this.contentView.displayObject.numChildren > 1) {
			this.contentView.displayObject.removeChildAt(this.contentView.displayObject.numChildren - 1);
		}
	}
