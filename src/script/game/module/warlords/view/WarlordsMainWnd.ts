...
export default class WarlordsMainWnd extends BaseWindow {
  ...
  renderListItem(index: number, item: WarlordsPlayerItem) {
    if (!item) return;
    item.index = index + 1;
    let itemData = this.warlordsModel.getListData(
      WarlordsModel.TOP,
      this.getDataIndex(index + 1),
  ...
  private refresh() {
    if (this.warlordsModel.process == WarlordsModel.PROCESS_READY) {
      ...
      this.openStr2
        .setVar("num", this.warlordsModel.period.toString())
        .flushVars();
      ...
      this.openStr4
        .setVar("time", this.warlordsModel.getMatchDateString(3))
        .flushVars();
      ...
      let str: string = LangManager.Instance.GetTranslation(
        this.warlordsModel.isEnterFinal
          ? "warlords.WarlordsFrame.str06"
          : "warlords.WarlordsFrame.str07",
      );
      this.openStr5.text = str;
    } else if (this.warlordsModel.process == WarlordsModel.PROCESS_OVER) {
      this.time.selectedIndex = 4;
      this.openStr8
        .setVar("count", this.warlordsModel.period.toString())
        .flushVars();
    } else {
      this.time.selectedIndex = 5;
      this.openStr9.text = LangManager.Instance.GetTranslation(
        "warlords.WarlordsFrame.str09",
      );
    }
    this.PrelimScoreTxt.setVar(
      "count",
      this.warlordsModel.selfInfo.prelimScore.toString(),
    ).flushVars();
    this.FinalRankTxt.setVar(
      "count",
      this.warlordsModel.selfInfo.sort.toString(),
    ).flushVars();
    this.GloryTxt.setVar("count", this.thane.gloryPoint.toString()).flushVars();
  ...
  private helpBtnHandler() {
    ...
    let content: string = LangManager.Instance.GetTranslation(
      "warlords.WarlordsFrame.helpContent",
    );
