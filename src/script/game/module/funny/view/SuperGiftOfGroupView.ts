 ...
 */
export class SuperGiftOfGroupView
  extends FUI_SuperGiftOfGroup
  implements FunnyContent
{
  ...
  private onRenderGiftItem(index: number, item: SuperGiftOfGroupItem): void {
    item.info(
      index,
      this._diamondTotal,
      SuperGiftOfGroupManager.Instance.giftList[index],
    );
  }
  ...
  private setResidueTime() {
    ...
    if (this._residueTime >= 60) {
      this.activityTimeTxt.text = DateFormatter.getFullTimeString(
        this._residueTime,
      );
    } else if (this._residueTime > 0) {
      this.activityTimeTxt.text = DateFormatter.getFullDateString(
        this._residueTime,
      );
    } else {
      SuperGiftOfGroupManager.Instance.open = false;
      this.list.numItems = SuperGiftOfGroupManager.Instance.giftList.length;
      this.activityTimeTxt.text = LangManager.Instance.GetTranslation(
  ...
  private loopResidueTime(): void {
    let endTime: Date = DateFormatter.parse(
      SuperGiftOfGroupManager.Instance.stopTime,
      "YYYY-MM-DD hh:mm:ss",
    );
    this._residueTime =
