...
export class OuterCityDragMediator implements IMediator
{
    ...
    {
        let p:Point = MapViewHelper.mapMoveCheck(new Point(this._target.x, this._target.y));
        this._target.x = p.x;
        this._target.y = p.y;
        if(!this._dispatchStartEvent && p.distance(this._startPoint.x, this._startPoint.y) > 20)
        {
            this.dragSceneImp();
        }
        p = null;
    ...
    private dragStartCallBack():void
    {
        ...
        this._dragTimeId = 0;
        this._dragTimeId = setInterval(this.dragSceneImp.bind(this), 300);
    ...
    private dragSceneImp():void
    {
        ...
        this._dragTimeId = 0;
        this._target && this._target.event(OuterCityEvent.DRAG_SCENE_START, this._target);
    }
