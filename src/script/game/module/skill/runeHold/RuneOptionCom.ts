...
export class RuneOptionCom extends FUI_RuneOptionCom {
  ...
  private propertyView: {
    s: fairygui.GRichTextField;
    sa: fairygui.GRichTextField;
    a: fairygui.GImage;
    u: fairygui.GTextField;
  }[];
  onConstruct() {
    super.onConstruct();
    this.propertyView = [
      { s: this.s1, sa: this.sa1, a: this.a1, u: this.u1 },
      { s: this.s2, sa: this.sa2, a: this.a2, u: this.u2 },
    ];
    this.addEvent();
  ...
  private updateRuneGem(gem: GoodsInfo) {
    ...
    let nextlevlTmep: t_s_upgradetemplateData =
      TempleteManager.Instance.getTemplateByTypeAndLevelAndSonType(
        temp.strengthenGrade,
        200,
        temp.templateInfo.SonType,
      );
  ...
  private onLevelUpClick() {
    ...
    ctrl.reqRuneGemUpgrade(
      currRuneData.pos,
      currRuneData.id,
      cost,
      currRuneData.bagType,
      this._info.holeId,
    );
  }
