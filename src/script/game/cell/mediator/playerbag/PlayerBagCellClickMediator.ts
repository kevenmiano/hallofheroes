...
export class PlayerBagCellClickMediator implements IMediator {
  ...
  protected onDoubleClick(e: Laya.Event) {
    ...
    if (BagHelper.checkCanUseGoods(info.templateInfo.SonType)) {
      ...
      if (
        info.templateInfo.SonType != GoodsSonType.SONTYPE_NOVICE_BOX &&
        ...
        str = LangManager.Instance.GetTranslation(
          "cell.mediator.playerbag.PlayerBagCellClickMediator.command01",
        );
      ...
      if (
        info.templateInfo.SonType == GoodsSonType.SONTYPE_CONSORTIATIME_CARD &&
        (this.model.sysCurTimeBySecond - this.playerInfo.lastOutConsortia >=
          24 * 3600 ||
          !this.playerInfo.isAuto)
      ) {
        //盟约之证
        str = LangManager.Instance.GetTranslation(
          "cell.mediator.playerbag.PlayerBagCellClickMediator.command05",
        );
      ...
      if (info.templateInfo.SonType == GoodsSonType.SONTYPE_NOVICE_BOX) {
        ...
        info.templateInfo.SonType == GoodsSonType.SONTYPE_TREASURE_MAP
      ) {
        // TreasureMapManager.Instance.useTreasureMap(data);
      } else if (info.templateInfo.SonType == GoodsSonType.SONTYPE_BOX) {
        //消耗道具使用宝箱
        switch (info.templateInfo.Property2) {
          ...
          case -700: //经验
            if (
              ResourceManager.Instance.gold.count < info.templateInfo.Property3
            ) {
              //经验不足
              str = LangManager.Instance.GetTranslation(
                "cell.mediator.playerbag.PlayerBagCellClickMediator.command08",
        ...
        info.templateInfo.SonType == GoodsSonType.SONTYPE_RENAME_CARD
      ) {
        ...
        info.templateInfo.SonType == GoodsSonType.SONTYPE_SEX_CHANGE_CARD
      ) {
        //使用变性卡
        SocketSendManager.Instance.sendUseSexChangeCard(info.pos);
      } else if (
        info.templateInfo.SonType == GoodsSonType.SONTYPE_PASSIVE_SKILL
      ) {
        // FrameControllerManager.instance.armyController.startFrameByType(ArmyPanelEnum.SKILL_PANEL, 1);
      } else if (
        info.templateInfo.SonType == GoodsSonType.SONTYPE_PET_EXP_BOOK
      ) {
        // checkForUsePetExpBook(info);
      } else if (
        info.templateInfo.SonType == GoodsSonType.SONTYPE_PET_LAND_TRANSFER
      ) {
        // checkForUsePetLandTransfer(info);
      } else if (
        info.templateInfo.SonType == GoodsSonType.SONTYPE_SINGLE_PASS_BUGLE
      ) {
        ...
        let itemBuffer: PlayerBufferInfo =
          PlayerBufferManager.Instance.getItemBufferInfo(
            info.templateInfo.Property1,
          );
        if (itemBuffer) {
          if (info.templateInfo.Property3 < itemBuffer.grade) {
            str = LangManager.Instance.GetTranslation(
              "cell.mediator.playerbag.PlayerBagCellClickMediator.command02",
            );
            MessageTipManager.Instance.show(str);
            return;
          }
          ...
          if (
            info.templateInfo.Property1 == 5 &&
            info.templateInfo.Property2 > 0
          ) {
    ...
    if (
      !GoodsCheck.checkGoodsCanEquip(info, ArmyManager.Instance.thane, false)
    ) {
      return;
    }
    if (
    ...
    if (info.templateInfo.SonType == GoodsSonType.SONTYPE_RUNNES) {
      ...
      let existRunes: GoodsInfo = GoodsManager.Instance.getBattleRunesByTempId(
        info.templateId,
      );
      ...
      if (existRunes && existRunes.count != existRunes.templateInfo.MaxCount) {
        ...
        PlayerManager.Instance.moveBagToBag(
          this._cell.item.bagType,
          this._cell.item.objectId,
          this._cell.item.pos,
          BagType.Battle,
          this.thane.id,
          existRunes.pos,
      ...
      // FrameControllerManager.instance.armyController.frame['defaultContent'] = ArmyPanelEnum.EQUIP_PANEL;
      let heroBag: any[] = GoodsManager.Instance.getHeroGoodsListByTypeAndId(
        info.templateInfo.MasterType,
        this.thane.id,
      ).getList();
      let pos_arr: any[] = GoodsSonType.getSonTypePos(
        info.templateInfo.SonType,
      );
      ...
      if (pos_arr.length > 1) {
        for (const key in heroBag) {
          if (heroBag.hasOwnProperty(key)) {
            ...
            if (
              i.templateInfo.SonType == info.templateInfo.SonType &&
  ...
  public checkGoodsByHero(popMsg: boolean = true): boolean {
    return GoodsCheck.checkGoodsByHero(
      this._cell.info,
      ArmyManager.Instance.thane,
      popMsg,
    );
  ...
  private check(): boolean {
    let str: string = "";
    if (
      this._cell.info.templateInfo.Property1 == 8 &&
      this.playerInfo.consortiaID == 0
    ) {
      str = LangManager.Instance.GetTranslation(
        "cell.mediator.playerbag.PlayerBagCellClickMediator.command03",
      );
      MessageTipManager.Instance.show(str);
      return false;
    }
    if (
      this._cell.info.templateInfo.SonType == GoodsSonType.SONTYPE_COMPOSE &&
      this.playerInfo.composeList.indexOf(
        this._cell.info.templateInfo.Property1,
      ) >= 0
    ) {
      str = LangManager.Instance.GetTranslation(
        "cell.mediator.playerbag.PlayerBagCellClickMediator.command04",
      );
      MessageTipManager.Instance.show(str);
      return false;
    }
    if (
      this._cell.info.templateInfo.SonType == GoodsSonType.SONTYPE_REWARD_CARD
    ) {
      if (MopupManager.Instance.model.isMopup) {
        str = LangManager.Instance.GetTranslation(
          "mopup.MopupManager.mopupTipData01",
