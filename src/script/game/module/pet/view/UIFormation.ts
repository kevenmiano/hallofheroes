...
export default class UIFormation extends BaseFguiCom {
	...
	constructor(comp: fgui.GComponent) {
		super(comp);
		if(this.playerInfo)this.playerInfo.addEventListener(PlayerEvent.PLAYER_PET_LIST_CHANGE, this.__petformationChangeHandler, this);
		this.initView();
	}
	...
	private __petformationChangeHandler(){
		...
		let formation = [-1,-1, arr[0], arr[3],-1, arr[1], arr[4],-1, arr[2], arr[5]];
		let petId: number = 0;
		...
		this.petIndexViews = [this.battleNumItem1, this.battleNumItem2, this.battleNumItem3];
		...
		this.avataViews = [this.figureItem1, this.figureItem2,this.figureItem3,this.figureItem4,this.figureItem5,this.figureItem6];
		this.setFigureData();
	}
	...
	private initView() {
		for(let i:number = 1;i<=6;i++){
			...
			this["petFormationDescTxt"+ i].text = LangManager.Instance.GetTranslation(str);
		}
	...
	private setFigureData(){
		...
		let formation = [-1,-1, this.avataViews[0], this.avataViews[3],-1, this.avataViews[1], this.avataViews[4],-1, this.avataViews[2], this.avataViews[5]];
		for (let item of this.avataViews) {
			item.info = null;
		}
		for (let item of this.petFormationViews) {
			if (item.petData) {//存在数据
	...
	private onPetChallengeDragStart(evt: Laya.Event) {
		let btn: PetformationItem = <PetformationItem>fgui.GObject.cast(evt.currentTarget);
		//取消对原目标的拖动, 换成一个替代品
		btn.stopDrag();
	...
	private onPetDrop(otherView: PetformationItem, evt: Laya.Event) {
		if (!(otherView instanceof PetformationItem)|| otherView.isIndex.selectedIndex == 1) return;
		let btn: PetformationItem = <PetformationItem>fgui.GObject.cast(evt.currentTarget);
	...
	private onPetIndexDrop(otherView: PetformationItem, evt: Laya.Event) {
		if (!(otherView instanceof PetformationItem)||otherView.isIndex.selectedIndex == 0) return;
		let btn: PetformationItem = <PetformationItem>fgui.GObject.cast(evt.currentTarget);
		let temp: PetData = otherView.petData;
		otherView.petData = btn.petData;
		btn.petData = temp;
	...
	private formatPetFormation() {
		...
		NotificationManager.Instance.dispatchEvent(PetEvent.PET_SELECT_FORMATION_CHANGE);
		this.setFigureData();
	}

	public set data(value: PetData) {
		if (value) {
			if (PetModel.isClickPetList) {//点击英灵列表，如果这个英灵在阵型里面就移除，否则上阵
				...
				if (pos >= 0) {
					...
					for (let item of this.petFormationViews) {
						if (item.petData && item.petData.petId == this._selectedPetData.petId) {
							item.petData = null;
							break;
						}
					}
	...
	public dispose(destred?: boolean) {
		if(this.playerInfo)this.playerInfo.removeEventListener(PlayerEvent.PLAYER_PET_LIST_CHANGE, this.__petformationChangeHandler, this);
	}
}