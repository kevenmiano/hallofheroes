...
export default class SendFlowerWnd extends BaseWindow {
  ...
  initList() {
    ...
    this.listData = ArrayUtils.sortOn(
      this.listData,
      ["count"],
  ...
  private updateList() {
    ...
    for (let i = 0; i < array.length; i++) {
      ...
      let arr: GoodsInfo[] = GoodsManager.Instance.getBagGoodsByTemplateId(
        element.templateId,
      );
      if (arr && arr[0]) {
  ...
  private onChange(): void {
    ...
    this.txt_num.text = LangManager.Instance.GetTranslation(
      "yishi.view.RosePresentView.wordCount",
      len,
      maxCount,
    );
  }
  ...
  onSend() {
    if (this.c1.selectedIndex == 1) {
      ...
      // }
      if (
        FilterWordManager.isGotForbiddenWords(this.txt_desc.text, "name") ||
        FilterWordManager.isGotForbiddenWords(this.txt_desc.text, "chat")
      ) {
    ...
    this.sendRosePresent(
      this.curData.templateId,
      this._targetName,
      this.stepper.value,
      this.txt_desc.text,
    );
  ...
  showQuickBuy() {
    ...
    if (!data) {
      MessageTipManager.Instance.show(
        LangManager.Instance.GetTranslation("giftbag.GiftBagItem.noGoodsText"),
      );
      return;
    }
    let obj = {
  ...
  private sendRosePresent(
    ...
    let goodsArr: GoodsInfo[] =
      GoodsManager.Instance.getBagGoodsByTemplateId($roseTempId);
      ...
      if (!fInfo) {
        MessageTipManager.Instance.show(
          LangManager.Instance.GetTranslation(
            "friends.im.IMFrame.present.NotFriends",
          ),
        );
        return false;
      }
