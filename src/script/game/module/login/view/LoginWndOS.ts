...
import { NativeEvent, NotificationEvent, SelectServerEvent, WebViewEvent } from '../../../constant/event/NotificationEvent';
...
import { getZoneData, isOversea } from '../manager/SiteZoneCtrl';
...
import { LoginWayGuest } from './LoginWayGuest';
 ...
 * @description 登录
 */
export default class LoginWndOS extends BaseWindow {
    ...
    public loginWay: fgui.Controller;//App端登陆方式:0游客,1账号
    ...
    private loginGuest: LoginWayGuest;
    private btnAppEnterGame: UIButton;
    ...
    public OnInitWind() {
        ...
        this.loginWay = this.getController("loginWay");

        this.loginMethod && this.loginMethod.onInit(new Func(this, this.handlerLoginMethod));

        LoginLoad.onLoadSpine(this.loginBackground.displayObject, LoginLoad.LOGIN_SKELETON);
        if (this.version) {
            if (this.isApp) {
                this.device.selectedIndex = 1;
                //App v{appVersion=1.2.0.5}   Res v{resVersion=1.2.0.5}
                let appVersion: string = NativeChannel.versionName + "." + NativeChannel.versionCode;
                if (this.isIOS) {
                    appVersion = Laya.Browser.window.conchConfig && Laya.Browser.window.conchConfig.getAppVersion() + "." + Laya.Browser.window.conchConfig.getAppLocalVersion();
                }
                this.version.setVar("appVersion", appVersion).setVar("resVersion", Main.version).flushVars();
            } else {
        ...
        // this.loginState.selectedIndex = 2;
        this.cIsIOS.selectedIndex = this.isIOS ? 1 : 0;
        ...
        if (this.frameData) {
            ...
            if (this.objData.isWan) {//玩平台
                this.userData = this.frameData.user;
                if (this.userData) {//网页传入账户信息
                ...
                SharedManager.Instance.setProperty('userName', this.userData.user, "userDate", date);
                //获取对应的玩平台服务列表
            ...
            } else if (this.objData.isDebug || this.objData.isFCC) {//Debug端
                this.platformState.selectedIndex = 1;
                this.userData = this.frameData.user;
                let useSite = false;
                if (this.userData) {//网页传入账户信息
                ...
                ModelMgr.Instance.setProperty(EmModel.USER_MODEL, UserModelAttribute.channelId, channelId);
                this._appData = {
            ...
            } else if (this.objData.isWxMiniGame) {//小游戏
                this.platformState.selectedIndex = 1;
                this.userData = this.frameData.user;
                SDKManager.Instance.getChannel().initRoad_7().then(res => {
                    if (!res) return;
                    ...
                    ModelMgr.Instance.setProperty(EmModel.USER_MODEL, UserModelAttribute.channelId, platform);
                ...
                this.userData = this.frameData.user;
                this.platformState.selectedIndex = 1;
                SDKManager.Instance.getChannel().initRoad_7(this.userData).then(res => {
                    if (!res) return;
                    if (res) this._appData = res;
                    if (res.channelId) platform = Number(res.channelId);
                    this.userName = this.userData.user;
                    ModelMgr.Instance.setProperty(EmModel.USER_MODEL, UserModelAttribute.channelId, platform);
                    if (!this._appData.token || this._appData.token != this.frameData.user.token)
                ...
                if (obj) {
                    let restartFrom = SharedManager.Instance.getWindowItem("bindAccountRet");
                    if (restartFrom == "true") {
                        this.changeLoginMethod();
                    } else {
                        this._appData = obj;
                        let isLoginAfterRestart = SharedManager.Instance.getWindowItem("isLoginAfterRestart");
                        if (isLoginAfterRestart == "true") {
                            this.onSwitchAccountHandler();
                        } else {
                            if (obj) {
                                ...
                                ModelMgr.Instance.setProperty(EmModel.USER_MODEL, UserModelAttribute.channelId, channelId);
                                this.setPrivacyState()
                                if (!Number.isNaN(platform) && platform > 0 && this.userName && this.userName != '' && this.userName != "-1") {
                                    SharedManager.Instance.setProperty('userName', this.userName, "userDate", new Date());
                                    this.requestServerList(platform, this.userName);
                                }
                            } else {
                                this.showMaskLogin(false);
    ...
    private SDKinitBack() {
        ...
        this.loginState.selectedIndex = 2;
        Logger.yyz("SDKinitBack::this.loginState.selectedIndex:" + this.loginState.selectedIndex);
    }

    private changeLoginMethod() {
        this.loginState.selectedIndex = 0;
        Logger.yyz("changeLoginMethod::this.loginState.selectedIndex:" + this.loginState.selectedIndex);
        let bindAccountRet = SharedManager.Instance.getWindowItem("bindAccountRet");
        let type = bindAccountRet == "true" ? 1 : 0;
        this.loginMethod && this.loginMethod.setItems(type);
    }
    ...
    private get isApp(): boolean {
        return Utils.isApp();
    }

    /**检查隐私 */
    private checkPrivacy(): boolean {
        if (Utils.isYYBApp() || Utils.isFromHY() || Utils.isFrom4399() || Utils.isQQHall() || Utils.isOppoApp() || Utils.isQQZone() || Utils.isQQMobile()) {
    ...
    addEvent() {
        ...
        this.btnAppEnterGame.onClick(this, this.onEnterGame);
        ...
        this.serverInfoItem.changeServerBtn && this.serverInfoItem.changeServerBtn.onClick(this, this.onSelectServer);
        this.txtprivacy.on(Laya.Event.LINK, this, this.onPrivacyLink);
        this.loginGuest.onGuestEvent(this, this.onGuestEnterGame);
        this.loginGuest.onSwitchEvent(this, this.onGuestSwitchAccountHandler);
        NotificationManager.Instance.addEventListener(SelectServerEvent.CHANGE, this.onChangeServer, this);
        NotificationManager.Instance.addEventListener(WebViewEvent.RECEIVE_DATA, this.webLoginCallback, this);
        NotificationManager.Instance.addEventListener(NativeEvent.AFTER_STATUS_BAR_CHANGE, this.onAfterStatusBarChange, this);
        NotificationManager.Instance.addEventListener(NotificationEvent.SWITCH_ACCOUNT, this.onSwitchAccountHandler, this);
        NotificationManager.Instance.addEventListener(NativeEvent.SDK_INIT, this.SDKinitBack, this);
        NotificationManager.Instance.addEventListener(NativeEvent.ERROR_CODE, this.onLoginErrorBack, this);
    }

    offEvent() {
        this.btnEnterGame.offClick(this, this.onEnterGame);
        this.btnLoginGame.offClick(this, this.onLoginGame);
        this.btnAppEnterGame.offClick(this, this.onEnterGame);
        ...
        this.serverInfoItem.changeServerBtn && this.serverInfoItem.changeServerBtn.offClick(this, this.onSelectServer);
        this.txtprivacy.off(Laya.Event.LINK, this, this.onPrivacyLink);
        this.loginGuest.offGuestEvent(this, this.onGuestEnterGame);
        this.loginGuest.offSwitchEvent(this, this.onGuestSwitchAccountHandler);
        NotificationManager.Instance.removeEventListener(SelectServerEvent.CHANGE, this.onChangeServer, this);
        NotificationManager.Instance.removeEventListener(WebViewEvent.RECEIVE_DATA, this.webLoginCallback, this);
        NotificationManager.Instance.removeEventListener(NativeEvent.AFTER_STATUS_BAR_CHANGE, this.onAfterStatusBarChange, this);
        NotificationManager.Instance.removeEventListener(NotificationEvent.SWITCH_ACCOUNT, this.onSwitchAccountHandler, this);
        NotificationManager.Instance.removeEventListener(NativeEvent.SDK_INIT, this.SDKinitBack, this);
        NotificationManager.Instance.removeEventListener(NativeEvent.ERROR_CODE, this.onLoginErrorBack, this);
    }
    ...
    onGuestSwitchAccountHandler() {
        SDKManager.Instance.getChannel().trackEvent(0, TrackEventNode.LOGIN_ACCOUNTS_CHOOSE, "login_accounts_choose", "登录界面点击“登录其他账户”按钮", "", "");
        SDKManager.Instance.getChannel().adjustEvent(RPT_EVENT.LOGIN_ACCOUNTS_CHOOSE);
        this.loginMethod && this.loginMethod.setItems(0);
        this.onSwitchAccountHandler();
    }

    onLoginErrorBack(errorCode: number) {
        if (errorCode != 111103) {
            //切换登录方式
            let codeKey = "LoginSDK.ERROR.CODE_";
            let defaultCode = 0;
            let codeStr = codeKey + errorCode;
            //其他错误码提示
            if (LangManager.Instance.GetTranslation(codeStr) != "") {
                MessageTipManager.Instance.show(codeStr);
            } else {//defaultCodeTips
                MessageTipManager.Instance.show(LangManager.Instance.GetTranslation((codeKey + defaultCode),errorCode));
            }
        }
        this.onSwitchAccountHandler();
    }
    ...
    private clearUserData() {
        ...
        this.loginState.selectedIndex = 0;
        Logger.yyz("clearUserData::this.loginState.selectedIndex:" + this.loginState.selectedIndex);
        let bindAccountRet = SharedManager.Instance.getWindowItem("bindAccountRet");
        let type = bindAccountRet == "true" ? 1 : 0;
        this.loginMethod && this.loginMethod.setItems(type);
    }
    ...
    webLoginCallback(appData) {
        ...
        if (appData) {
            ...
            SharedManager.Instance.setWindowItem("loginWay", appData.type);
            if (appData.userId == -1 && appData.token == -1) {//失败情况
                this.clearUserData();
                return;
            }
        }
        ...
        if (this.isApp) {
            this.loginMethod && this.loginMethod.setItems();
        }
        ...
        if (!Number.isNaN(platform) && platform > 0 && this.userName && this.userName != '' && this.userName != '-1') {
            ...
            let date: Date = new Date();
            SharedManager.Instance.setProperty('userName', this.userName, "userDate", date);
            ModelMgr.Instance.setProperty(EmModel.USER_MODEL, UserModelAttribute.channelId, channelId);
            this.setPrivacyState();
            let shareState = SharedManager.Instance.privacyState;
            if (Utils.isYYBApp()) {//应用宝默认不勾选
    ...
    private setPrivacyState() {
        this.privacy.selectedIndex = (Utils.isYYBApp() || Utils.isOppoApp() || Utils.isFromHY() || Utils.isFrom4399() || Utils.isQQHall()) || Utils.isQQZone() || Utils.isQQMobile() ? 1 : 0;
    }
     ...
     * @param username 玩家昵称
     */
    requestServerList(platformid: number | string, username?: string, useSite?: boolean) {
        Logger.warn("requestServerList   ###############:", platformid, username);
        let zoneData = getZoneData();
        if (isOversea() && !useSite) {//海外版本采用平台ID
        ...
        if (useSite) {
            ...
            args['sign'] = MD5Utils.md5(String(platformid) + username + this.RSAKey).toUpperCase();
        ...
        SDKManager.Instance.getChannel().trackEvent(0, TrackEventNode.Node_1007, "RequestServerList", "请求区服列表", "", "");
        HttpUtils.httpRequest(sUrl, "clientsiteinfoservlet", params, 'POST', "arraybuffer", () => {
                ...
                try {
                    ...
                    if (content && content.length) {
                        ...
                        if (this.serverListData.ret < 0) {
                            ...
                            MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("login.failedReadServerList") + "(" + this.serverListData.ret + ")");
                            return;
                        }
                        SDKManager.Instance.getChannel().trackEvent(0, TrackEventNode.Node_1008, "RequestServerListSucc", "请求区服列表成功", "", "");
                        this.serverInfoItem.changeServerBtn.enabled = true;
                        let zoneData = getZoneData();
                        PlayerManager.Instance.currentPlayerModel.userInfo.isWhiteUser = this.serverListData.isWhiteUser;
                        PlayerManager.Instance.currentPlayerModel.userInfo.platformId = this.serverListData.platformId;
                        PlayerManager.Instance.currentPlayerModel.userInfo.area = zoneData.area;
                        PlayerManager.Instance.currentPlayerModel.userInfo.platformName = this.serverListData.platformName;
                        PlayerManager.Instance.currentPlayerModel.userInfo.user = this.userName;
                        ...
                        if (isSwitchSite == "true" && localSite) {
                            ...
                            siteInfo = this.serverListData.recentLoginServer ? this.serverListData.recentLoginServer : this.serverListData.defaultSelectServer;
                        }
                        this.setServerInfo(siteInfo);
                        this.showMaskLogin(false);
                        this.loginState.selectedIndex = 1;
                        Logger.yyz("requestServerList::this.loginState.selectedIndex:" + this.loginState.selectedIndex);
                        let loginWay = SharedManager.Instance.getWindowItem("loginWay");
                        if (Number(loginWay) == LoginWay.Type_GUEST) {
                            this.loginWay.selectedIndex = this.serverListData.hasPlayer ? 1 : 0;
                        } else {
                            this.loginWay.selectedIndex = 1;
                        }
                        if (this.objData.isWan) {//wan平台时间验证
                            this.btnEnterGame.enabled = !this.checkLoginOverTime;
                        }
                        this.gmBtn.visible = (Utils.isWanApp() || Utils.isWebWan() || this.objData.isDebug) && !ComponentSetting.IOS_VERSION;
    ...
    private reRequestServerList() {
        if (this.requestServerTimes <= this.requestServerMaxTimes) {
            ...
            this.requestServerList(this.requestPID, this.userName, this.userSite != "");
        } else {
            MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("login.failedReadServerList"));
    ...
    onSelectServer() {
        if (this.serverListData && this.serverListData.ret >= 0) {
            SDKManager.Instance.getChannel().trackEvent(0, TrackEventNode.Node_1009, "openSelectServer", "点击选择区服列表", "", "");
    ...
    onChangeServer(serverData: ServerSite) {
        if (serverData) {
            SDKManager.Instance.getChannel().trackEvent(0, TrackEventNode.Node_1010, "ChangeServer", "选择确认区服", "", "");
    ...
    setServerInfo(serverData: ServerSite) {
        if (serverData) {
            ...
            //贪玩渠道上报选择服务器的游戏事件
            let channelId: number = Number(ModelMgr.Instance.getProperty(EmModel.USER_MODEL, UserModelAttribute.channelId));
            if (channelId == 20370 || Utils.isH5SDK() || Utils.isWxMiniGame()) {
                let siteDate = {
                    gameZoneId: serverData.siteId,
                    gameZoneName: serverData.siteName
                }
                SDKManager.Instance.getChannel().postGameEvent(GameEventCode.Code_1023, JSON.stringify(siteDate));
            }
            ...
            ComponentSetting.VERTION_PATH = PathManager.info.TEMPLATE_PATH + "version.json";
            if (ComponentSetting.IS_BACK_FROM_REGISTER) {
                ComponentSetting.IS_BACK_FROM_REGISTER = false;
                this.onSelectServer();
            }
            let hasPlayer = (this.serverListData && this.serverListData.hasPlayer) ? true : false;
            if (PathManager.info.TEMPLATE_PATH != "" && this.userName != null && this.userName != "" && hasPlayer && !Utils.isFrom4399())
    ...
    private get checkLoginOverTime() {
        ...
        if (Math.abs(curServerTamp - wanTamp) > 120000) {//超出120S,不允许登录
            return true;
        }
        return false;
    }
    ...
    OnShowWind() {
        super.OnShowWind();
        SDKManager.Instance.getChannel().trackEvent(0, TrackEventNode.Node_1006, "openLoginWnd", "进入登录界面", "", "");
        ...
        if (ComponentSetting.VERSION_COMPARE)
    ...
    onGuestEnterGame() {
        SDKManager.Instance.getChannel().trackEvent(0, TrackEventNode.LOGIN_TOURIST_CHOOSE, "login_tourist_choose", "登录界面点击“以游客继续”按钮", "", "");
        SDKManager.Instance.getChannel().adjustEvent(RPT_EVENT.LOGIN_TOURIST_CHOOSE);
        this.onEnterGame();
    }
    ...
    onEnterGame() {
        ...
        SDKManager.Instance.getChannel().trackEvent(0, TrackEventNode.Node_1011, "enterGame", "进入游戏", "", "");
        if (!this.checkPrivacy()) {
            MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("login.privacy"));
            return;
        }
        if (!this.serverInfoItem.serverInfo) {
            MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("login.pleaseSelectServer"));
            return;
        }
        //非白名单用户  并且服务器未开启
        if (!this.serverListData.isWhiteUser) {
            if (!this.serverInfoItem.serverInfo.isOpen) {
                MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("login.serverNotOpen"));
                return;
            }
            if (this.serverInfoItem.serverInfo.isRepair) {
                MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("login.serverMaintenance"));
                return;
            }
        }
        FrameCtrlManager.Instance.exit(EmWindow.Announce);
        PlayerManager.Instance.currentPlayerModel.openServerTime = this.serverInfoItem.serverInfo.openTime;
        /***正式服和beta测试服判断***/
        let betaSites = PathManager.info.BETA_SITES;
        let versionType: VerType = betaSites.indexOf(this.selectServerData.mainSite) >= 0 ? SITE_MODE.BETA : SITE_MODE.RELEASE;
        let needAlert = PathManager.info.RELOAD_ALERT;
        if (!Utils.isWxMiniGame() && checkNeedReload && checkNeedReload(versionType)) {
            if (needAlert) {
                ...
                SimpleAlertHelper.Instance.Show(SimpleAlertHelper.SIMPLE_ALERT, null, prompt, msg, confirm1, cancel1, (ok: boolean) => {
                    if (ok) {
    ...
    private launchCallBack() {
        ...
        ModelMgr.Instance.setProperty(EmModel.USER_MODEL, UserModelAttribute.userName, userInfo.user);
        ModelMgr.Instance.setProperty(EmModel.USER_MODEL, UserModelAttribute.password, userInfo.password);
        ModelMgr.Instance.setProperty(EmModel.USER_MODEL, UserModelAttribute.tempPassword, userInfo.tempPassword);
        ModelMgr.Instance.setProperty(EmModel.USER_MODEL, UserModelAttribute.loginKey, userInfo.loginKey);
        ModelMgr.Instance.setProperty(EmModel.USER_MODEL, UserModelAttribute.site, this.serverInfoItem.serverInfo.siteName);
        ModelMgr.Instance.setProperty(EmModel.USER_MODEL, UserModelAttribute.key, key);
        ModelMgr.Instance.setProperty(EmModel.USER_MODEL, UserModelAttribute.site, this.selectSite);
        SDKManager.Instance.getChannel().createLoginReq(userInfo.user, userInfo.loginKey, this.serverInfoItem.serverInfo.siteName, this.serverInfoItem.serverInfo.siteId, this._appData)
    }
     ...
     * @param key
     */
    handlerLoginMethod(key?: number) {
        switch (key) {//19, 17, 13, 11
            case LoginWay.Type_Apple:
                SDKManager.Instance.getChannel().adjustEvent(RPT_EVENT.LOGIN_APPLE_ACCOUNT);
                SDKManager.Instance.getChannel().trackEvent(0, TrackEventNode.LOGIN_APPLE_ACCOUNT, "LOGIN_APPLE_ACCOUNT", "登录界面点击苹果账号登录按钮", "", "");
                break;
            case LoginWay.Type_7ROAD://7road
                SDKManager.Instance.getChannel().adjustEvent(RPT_EVENT.LOGIN_7ROAD_ACCOUNT);
                SDKManager.Instance.getChannel().trackEvent(0, TrackEventNode.LOGIN_7ROAD_ACCOUNT, "login_7road_account", "登录界面点击七道账号登录按钮", "", "");
                break;
            case LoginWay.Type_GG://google
                SDKManager.Instance.getChannel().adjustEvent(RPT_EVENT.LOGIN_GG_ACCOUNT);
                SDKManager.Instance.getChannel().trackEvent(0, TrackEventNode.LOGIN_GG_ACCOUNT, "login_gg_account", "登录界面点击谷歌账号登录按钮", "", "");
                break;
            case LoginWay.Type_FB://facebook
                SDKManager.Instance.getChannel().adjustEvent(RPT_EVENT.LOGIN_FB_ACCOUNT);
                SDKManager.Instance.getChannel().trackEvent(0, TrackEventNode.LOGIN_FB_ACCOUNT, "login_fb_account", "登录界面点击Facebook账号登录按钮", "", "");
                break;
            case LoginWay.Type_GUEST://guest
                SDKManager.Instance.getChannel().adjustEvent(RPT_EVENT.LOGIN_TOURIST_ACCOUNT);
                SDKManager.Instance.getChannel().trackEvent(0, TrackEventNode.LOGIN_TOURIST_ACCOUNT, "login_tourist_account", "登录界面点击游客账号登录按钮", "", "");
                break;
        }
        SDKManager.Instance.getChannel().showLogin(key);
    }
