...
export class MasteryCom extends FUI_MasteryCom {
  ...
  private addEvent() {
    ...
    this.secret_list.itemRenderer = Laya.Handler.create(
      this,
      this.renderListItem,
      null,
      false,
  ...
  private refreshView() {
    ...
    let str =
      ExtraJobModel.instance.leftNum +
      "/" +
      ExtraJobModel.instance.canActiveNum;
    this.txt_active.text = LangManager.Instance.GetTranslation(
      "Bag.mastery02",
      str,
    );
    if (ExtraJobModel.instance.nextLevel == 0) {
      this.txt_add.text =
        LangManager.Instance.GetTranslation("Mastery.activeMax");
    } else {
      this.txt_add.text = LangManager.Instance.GetTranslation(
        "Bag.mastery05",
        ExtraJobModel.instance.nextLevel,
      );
    }
  ...
  private renderListItem(index: number, item: FUI_SecretListItem) {
    ...
    item.data = info;
    if (info) {
      let name = LangManager.Instance.GetTranslation(
      ...
      item.getChild("iconLoader").asLoader.url = fgui.UIPackage.getItemURL(
        EmPackName.SBag,
        "Icon_Mastery_Unopened_L",
      );
  ...
  onSelectItem(item: FUI_SecretListItem) {
    if (item.data) {
      ...
      MessageTipManager.Instance.show(
        LangManager.Instance.GetTranslation("Mastery.unopen"),
      );
    }
  }
    ...
    if (count > 0) {
      for (const key in selfData) {
        if (Object.prototype.hasOwnProperty.call(selfData, key)) {
          ...
          selfstr +=
            `[color=#ffc68f][size=20]${key + "&nbsp;:&nbsp;"}[/size][/color][size=22]${element}[/size]` +
            "<br>";
        }
      }
    } else {
