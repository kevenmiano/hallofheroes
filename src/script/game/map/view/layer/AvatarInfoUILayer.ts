...
export class AvatarInfoUILayerHandler {
  ...
  static handle_NAME_FRAME(
    uuid: string,
    nameColor: number,
    type: eFilterFrameText,
  ) {
    if (!this._handlerView) {
      Logger.warn("[AvatarInfoUILayer]未设置handlerView！");
      return;
  ...
  static handle_CONSORTIA_STROKE(
    uuid: string,
    colorIdx: number,
    width?: number,
  ) {
...
export class AvatarInfoUILayer extends Laya.Sprite {
  ...
  public __conVisible(
    ...
    switch (type) {
      case AvatarInfoTag.Info:
        if (nickName) {
          nickName.visible =
            force || (visible && nickName.data && nickName.data.visible);
        }
        if (consortiaName) {
          consortiaName.visible =
            force ||
            (visible && consortiaName.data && consortiaName.data.visible);
        }
        if (vip) {
        ...
        if (appell) {
          appell.visible =
            force || (visible && appell.data && appell.data.visible);
        }
        if (qqDwk) {
          qqDwk.visible =
            force || (visible && qqDwk.data && qqDwk.data.visible);
        }
        break;
  ...
  public nickNameCreate(
    ...
    let nickName = new FilterFrameShadowText(
      240,
      20,
      undefined,
      fontSize,
  ...
  public consortiaNameCreate(
    ...
    let consortiaName = new FilterFrameShadowText(
      240,
      20,
      undefined,
      fontSize,
  ...
  public chatPopViewCreate(uuid: string, chatData?: ChatData) {
    ...
    let chatPopView = FUIHelper.createFUIInstance(
      EmPackName.Base,
      "ChatBubbleTip",
    ) as ChatPopView;
  ...
  public petStarCreate(uuid: string, name?: string) {
    ...
    let petStar = FUIHelper.createFUIInstance(
  ...
  public petTailMCCreate(uuid: string, name?: string) {
    ...
    let petTail = FUIHelper.createFUIInstance(
      EmPackName.BaseCommon,
      "PetTail",
    ) as fgui.GMovieClip;
    petTail.name = name;
