...
export class ConsortiaDemonInfo extends GameEventDispatcher {
  ...
  constructor() {
    ...
    for (var i: number = 1; i <= 2; i++) {
      ...
      if (i == 1) {
        configTemp = TempleteManager.Instance.getConfigInfoByConfigName(
          "ConsortiaAltarSceneNeedOffe_Live",
        );
      } else {
        configTemp = TempleteManager.Instance.getConfigInfoByConfigName(
          "ConsortiaAltarSceneNeedOffe_Fix",
        );
      }
      skillInfo.needPay = configTemp ? parseInt(configTemp.ConfigValue) : 1000;
      skillInfo.bufferNameLang = LangManager.Instance.GetTranslation(
        "yishi.manager.ConsortiaManager.demonSkillName0" + i,
      );
      skillInfo.DescriptionLang = LangManager.Instance.GetTranslation(
        "yishi.manager.ConsortiaManager.demonSkillDesc0" + i,
        skillInfo.needPay,
      );
  ...
  public get isCanOpen(): number {
    if (
      ConsortiaManager.Instance.model.consortiaInfo.levels <
      ConsortiaDemonInfo.NEED_CONSORTIA_LEVEL
    ) {
      return 2;
    }
    if (this.state != 0) {
  ...
  public get isCanJoin(): number {
    ...
    if (
      this.playerInfo.demonConsortiaId != 0 &&
      this.playerInfo.demonConsortiaId != this.playerInfo.consortiaID
    ) {
      return 101;
    }
    if (this.state == 2) {
  ...
  public get downTime(): number {
    ...
    var time: number =
      1260 -
      Math.floor(
        this.playerModel.sysCurTimeBySecond - this.openDate.getTime() / 1000,
      );
    ...
    switch (type) {
      case 1:
        return LangManager.Instance.GetTranslation(
          "buildings.casern.view.PawnLevelUpFrame.command04",
        );
      case 2:
        return LangManager.Instance.GetTranslation(
          "consortia.data.ConsortiaDemonInfo.consortiaLevelTip",
          ConsortiaDemonInfo.NEED_CONSORTIA_LEVEL,
        );
      case 10:
        return LangManager.Instance.GetTranslation(
          "consortia.data.ConsortiaDemonInfo.openedTip",
        );
      case 100:
        return LangManager.Instance.GetTranslation(
          "consortia.data.ConsortiaDemonInfo.unopenTip",
        );
      case 101:
        return LangManager.Instance.GetTranslation(
          "consortia.data.ConsortiaDemonInfo.joinedTip",
        );
      case 102:
        return LangManager.Instance.GetTranslation(
      ...
      case 104:
        if (this.state == 0) {
          ...
          return LangManager.Instance.GetTranslation(
            "consortia.data.ConsortiaDemonInfo.overTip",
          );
        }
      default:
        return "";
