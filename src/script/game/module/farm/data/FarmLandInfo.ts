...
export default class FarmLandInfo extends GameEventDispatcher {
    ...
    public set cropTempId(value: number) {
        ...
        this._cropTemp = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_itemtemplate, this._cropTempId)
    }
    ...
    public get cropPhaseString(): string {
        switch (this.cropPhase) {
            case CropPhase.DIE:
                return LangManager.Instance.GetTranslation("farm.data.FarmLandInfo.canRevive");
            case CropPhase.MATURE:
                return LangManager.Instance.GetTranslation("farm.data.FarmLandInfo.mature");
        }
    ...
    public get cropStateString(): string {
        switch (this.cropState) {
            case CropState.WORM:
                return LangManager.Instance.GetTranslation("farm.data.FarmLandInfo.canWorm");
            case CropState.GRASS:
                return LangManager.Instance.GetTranslation("farm.data.FarmLandInfo.canWeed");
        }
        return LangManager.Instance.GetTranslation("farm.data.FarmLandInfo.healthy");
    }
    ...
    public get curOper(): number {
        if (!this.hasCrop) {
            if (this.isSelfLand)
                return FarmOperateType.PLANT;
            else
                return FarmOperateType.NO_OPER;
        }
        if (this.isMature) {
            if (this.isSelfLand)
                return FarmOperateType.PICK;
            else
                return FarmOperateType.STEAL;
        }
        if (this.isDie) {
            if (this.isSelfLand)
                return FarmOperateType.CLEAR;
            else
    ...
    public get remainMatureTime(): number {
        let time: number = this.matureTime.getTime() / 1000 - this.sysCurTimeBySecond;
    ...
    public get totalMatureTime(): number {
        let time: number = this.matureTime.getTime() / 1000 - this.plantTime.getTime() / 1000;
        return time > 0 ? time : 0;
    }
    /**
     * 总共成熟时间 未加速前（秒）
    ...
    public get totalMatureTimeUnAccelerate(): number {
        let time: number = this.originMatureTime.getTime() / 1000 - this.plantTime.getTime() / 1000;
        return time > 0 ? time : 0;
    }
    ...
    public get growTime(): number {
        let time: number = this.sysCurTimeBySecond - this.plantTime.getTime() / 1000;
    ...
    public get isDie(): boolean {
        ...
        let dieTime: number = this.matureTime.getTime() / 1000 + this._cropTemp.Property1 * 60;
    ...
    public get isWormNow(): boolean {
        if (!this._cropTemp || this._cropTemp.Property4 == 0 || this.isMature || this.isDie) return false;
        ...
        let bTmp = (this.sysCurTimeBySecond > wormTimeP1 && this._isWormP1) || (this.sysCurTimeBySecond > wormTimeP2 && this._isWormP2);
    ...
    public get isGrassNow(): boolean {
        ...
        let bTmp = (this.sysCurTimeBySecond > grassTimeP1 && this._isGrassP1) || (this.sysCurTimeBySecond > grassTimeP2 && this._isGrassP2);
    ...
    public get canBeStolen(): string {
        for (let index = 0; index < this.stolenList.length; index++) {
            ...
            if (id == this.thane.userId) return LangManager.Instance.GetTranslation("farm.data.FarmLandInfo.hasStolen");
        }
        if (this.stolenCount >= FarmModel.MAX_STOLEN_COUNT) return LangManager.Instance.GetTranslation("farm.data.FarmLandInfo.notMuchLeft");
        if (this.outputCount < 10) return LangManager.Instance.GetTranslation("farm.data.FarmLandInfo.notMuchLeft");
        return "";
    }
    ...
    public get canHandleOneKey(): boolean {
        ...
        if ((curOper == FarmOperateType.STEAL && !this.canBeStolen) ||
