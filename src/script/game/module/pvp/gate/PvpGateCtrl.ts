...
export default class PvpGateCtrl extends FrameCtrlBase {
  ...
  private __onSearchRoom(pkg: PackageIn) {
    if (
      FrameCtrlManager.Instance.isOpen(EmWindow.RoomList) ||
      FrameCtrlManager.Instance.isOpen(EmWindow.PveMultiCampaignWnd)
    ) {
      Logger.xjy(
        "[PvpCtrl]房间界面点进入房间不需要执行以下逻辑, 在RoomListCtrl|PveSelectCampaign中处理",
      );
      return;
    }
