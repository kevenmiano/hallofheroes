...
export default class GemMazeBoxCom extends FUI_GemMazeBoxCom {
  ...
  private initView() {
    ...
    if (score <= this.SORT_INDEX_ARRAY[0]) {
      ...
      this.SORT_INDEX_ARRAY[0] < score &&
      score <= this.SORT_INDEX_ARRAY[1]
    ) {
      _y =
        40 +
        ((score - this.SORT_INDEX_ARRAY[0]) /
          (this.SORT_INDEX_ARRAY[1] - this.SORT_INDEX_ARRAY[0])) *
          110; //552 - 477 = 75
    } else if (
      this.SORT_INDEX_ARRAY[1] < score &&
      score <= this.SORT_INDEX_ARRAY[2]
    ) {
      _y =
        40 +
        110 +
        ((score - this.SORT_INDEX_ARRAY[1]) /
          (this.SORT_INDEX_ARRAY[2] - this.SORT_INDEX_ARRAY[1])) *
          110; //477 - 397 = 80
    } else if (
      this.SORT_INDEX_ARRAY[2] < score &&
      score <= this.SORT_INDEX_ARRAY[3]
    ) {
      _y =
        40 +
        110 +
        110 +
        ((score - this.SORT_INDEX_ARRAY[2]) /
          (this.SORT_INDEX_ARRAY[3] - this.SORT_INDEX_ARRAY[2])) *
          110; //397 - 317
    } else if (
      this.SORT_INDEX_ARRAY[3] < score &&
      score <= this.SORT_INDEX_ARRAY[4]
    ) {
      _y =
        40 +
        110 +
        ...
        110 +
        ((score - this.SORT_INDEX_ARRAY[3]) /
          (this.SORT_INDEX_ARRAY[4] - this.SORT_INDEX_ARRAY[3])) *
          110; //317 - 242
    }
  ...
  private initEvent(): void {
    GemMazeManager.Instance.addEventListener(
      GemMazeEvent.GEMMAZE_UPDATE_BOX_STATUS,
      this.updateBox,
      this,
    );
    GemMazeManager.Instance.addEventListener(
      GemMazeEvent.GEMMAZE_PLAYER_BOX_GOODS,
      this.onPlayer,
      this,
    );
  }
  ...
  public showAddPoint(point: number): void {
    ...
    let str = LangManager.Instance.GetTranslation(
      "gemMaze.GemMazeBoxView.showAddIntergal",
      point,
    );
    txt.text = str;
    txt.x = 220;
    ...
    TweenMax.to(txt, 1, {
      y: 500,
      onComplete: this.onShowAddIntergalComplete,
  ...
  private onShowAddIntergalComplete(txt: fgui.GTextField): void {
    TweenMax.to(txt, 1, {
      y: 300,
      alpha: 0,
      onComplete: this.onMoveAddIntergalComplete,
      onCompleteParams: [txt],
    });
  }
