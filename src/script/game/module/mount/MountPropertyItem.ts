...
export default class MountPropertyItem extends FUI_MountPropertyItem {
    ...
    private getTipsText(): string {
        ...
        var name: Array<string> = LangManager.Instance.GetTranslation("mounts.command02").split(",");
        var index: number = PropertyInfo.getPropertyValue(info.name);
        var val: number = info.grade * 5;
        let content: string;
        if ((info.addition - val) > 0) {
            content = LangManager.Instance.GetTranslation("mounts.PropertyItem.tips02", name[index], val + " +" + (info.addition - val));
        } else {
            content = LangManager.Instance.GetTranslation("mounts.PropertyItem.tips02", name[index], val);
        }
        content = `[color=#48d72a]${content}[/color]`;
        content += "<br/>";
        content += LangManager.Instance.GetTranslation("mounts.PropertyItem.tips03", name[index], info.grade);
        content += "<br/>";
        content += LangManager.Instance.GetTranslation("mounts.PropertyItem.tips04", info.gradeMax);
        return content;
    }
    ...
    public update() {
        ...
        this.tipData = this.getTipsText();
        this.progressValue.value = (this.propertyInfo.currentPropGp * 100 / this.propertyInfo.currentPropMaxGp);
        this.progressValue.getChild("progress").text = this.propertyInfo.currentPropGp + " / " + this.propertyInfo.currentPropMaxGp;
    }
