...
export default class UIHeCheng extends BaseFguiCom {
  ...
  private refreshTreeData() {
    ...
    let fDataList: ComposeGoodsInfo[] = bSuit
      ? this.model.composeSuitDic.getList()
      : this.model.composeDataList;
    ...
    for (let index = 0; index < fDataList.length; index++) {
      ...
      topNode._resURL = bSuit
        ? fgui.UIPackage.getItemURL(EmPackName.Base, "TabTree2")
        : fgui.UIPackage.getItemURL(EmWindow.Forge, "TabMenu");
        ...
        if (sDataList.length > 0) {
          for (let i: number = 0; i < sDataList.length; i++) {
            ...
            if (bSSSuit && this.model.defSEquipMetaId != 0) {
              if (
                sDataList[i].template.Material1 == this.model.defSEquipMetaId
              ) {
              ...
              if (
                sDataList[i].template.Material1 == this.model.defEquipMetaId
              ) {
                defSuitData = sDataList[i];
  ...
  private __renderTreeNode(node: fgui.GTreeNode, obj: fgui.GComponent) {
    ...
    switch (cellType) {
      ...
      case eForgeComposeTargetItemType.EPIC_CELL:
        ...
        obj.getChild("title").asLabel.color =
          ForgeData.Colors[cGoodsInfo.itemTemplate.Profile - 1];
        let txtDesc = obj.getChild("txtDesc");
        txtDesc.visible = cGoodsInfo.canMakeCount > 0;
        txtDesc.text = LangManager.Instance.GetTranslation(
          "Forge.Compose.CanComposeCnt",
          cGoodsInfo.canMakeCount,
        );
  ...
  private refreshRight(data: ComposeGoodsInfo) {
    ...
    if (this._data && this._data.template) {
      ...
      this.txtName.text =
        temp.TemplateNameLang + this.model.getStrengthenGrade(info);
  ...
  private refreshMaterials() {
    ...
    this.setBtnState();
    this.txtCost.text = (
      this._data.template.NeedGold * this.model.currentComposeCount
    ).toString();
    this.txtCostAll.text = (
      this._data.template.NeedGold * this.model.maxComposeCount
    ).toString();
    this.txtHas.text = ResourceManager.Instance.gold.count.toString();
    this.txtCostAll.color = CommonUtils.changeColor(
      this._data.template.NeedGold * this.model.maxComposeCount,
      ResourceManager.Instance.gold.count,
    );
    this.txtCost.color = CommonUtils.changeColor(
      this._data.template.NeedGold * this.model.currentComposeCount,
      ResourceManager.Instance.gold.count,
    );
  }
  ...
  private defaultView() {
    ...
    for (let index = 0; index < treeNodeList.length; index++) {
      ...
      if (
        treeNode.data &&
        this.model.selectedCompose &&
        treeNode.data.templateId == this.model.selectedCompose.templateId
      ) {
        ...
        treeNode.data &&
        this.model.defComposeGoodInfo &&
        treeNode.data.templateId == this.model.defComposeGoodInfo.templateId
      ) {
  ...
  private __compClick(compAll: boolean) {
    ...
    if (
      this.model.selectedCompose.template.NeedMinLevel >
    ...
    if (
      ResourceManager.Instance.gold.count <
      composeCnt * this.model.selectedCompose.template.NeedGold
    ) {
    ...
    if (
      this._data.template.Types == ForgeData.COMPOSE_TYPE_GEM ||
      this._data.template.Types == ForgeData.COMPOSE_TYPE_CRYSTAL
    ) {
      if (
        this._jewelGrade > ForgeData.JEWEL_LEVEL &&
        ArmyManager.Instance.thane.grades < ForgeData.JEWEL_GRADE_LIMIT
      ) {
        MessageTipManager.Instance.show(
          LangManager.Instance.GetTranslation(
            "Forge.NeedLevel",
            ForgeData.JEWEL_GRADE_LIMIT,
          ),
        );
        return;
      }
    ...
    let p2: number = GoodsManager.Instance.getGoodsNumByTempIdAndNotLock(
      this._data.template.Material2,
    );
    let c3: number = this._data.template.Count3 * composeCnt;
    let p3: number = GoodsManager.Instance.getGoodsNumByTempIdAndNotLock(
      this._data.template.Material3,
    );
    let c4: number = this._data.template.Count4 * composeCnt;
    let p4: number = GoodsManager.Instance.getGoodsNumByTempIdAndNotLock(
      this._data.template.Material4,
    );
    if (c1 > p1 || c2 > p2 || c3 > p3 || c4 > p4) {
      if (
        goods.SonType != GoodsSonType.SONTYPE_MOUNT &&
        goods.SonType != GoodsSonType.RESIST_GEM
      ) {
        let str = LangManager.Instance.GetTranslation(
          "store.view.compose.ComposeMatetialView.sendComposeTip04",
        );
        MessageTipManager.Instance.show(str);
        return;
      }
    ...
    if (this.itemTarget.info.templateInfo.MasterType == GoodsType.EQUIP) {
      let count = GoodsManager.Instance.getGoodsNumByTempId(
        this.itemTarget.info.templateInfo.TemplateId,
      ); //背包里有没有
      let count1 = GoodsManager.Instance.getBagCountByTempId(
      ...
      if (count > 0 || count1 > 0) {
        ...
        var prompt: string =
          LangManager.Instance.GetTranslation("compose.tipTitle");
  ...
  private needCountGem(value: number = 1): object {
    ...
    let bagCount: number =
      GoodsManager.Instance.getGoodsNumByTempIdAndNotLock(material);
    let tempCount: number = this._data.template.Count1 * count;

    for (let i: number = 0; i < 11; i++) {
      if (bagCount < tempCount) {
        let composeTemInfo: t_s_composeData =
          TempleteManager.Instance.getComposeTempleteByTempleteId(material);
        if (composeTemInfo) {
          ...
          let shopInfo: ShopGoodsInfo =
            TempleteManager.Instance.getShopTempInfoByItemId(peterTemId);
          let canComposeInfo: t_s_composeData =
            TempleteManager.Instance.getComposeTempleteByTempleteId(peterTemId);
          if (!canComposeInfo || !shopInfo) return null;
          let canCount: number = bagCount / canComposeInfo.Count1;
          count = count - canCount;
          let obj: object = {
            point: shopInfo.price * count,
            count: count,
            name: ConfigMgr.Instance.getTemplateByID(
              ConfigType.t_s_itemtemplate,
              peterTemId,
            ).TemplateName,
            needId: peterTemId,
  ...
  private needCountCrystal(value: number = 1): object {
    ...
    for (let i: number = 0; i < 11; i++) {
      if (bagCount < tempCount) {
        let composeTemInfo: t_s_composeData =
          TempleteManager.Instance.getCrystalTempleteByTempleteId(material);
        if (composeTemInfo) {
          material = composeTemInfo.Material1;
          count = tempCount - bagCount;
          peterTemId = composeTemInfo.NewMaterial;
          ...
          let shopInfo: ShopGoodsInfo =
            TempleteManager.Instance.getShopTempInfoByItemId(peterTemId);
          let canComposeInfo: t_s_composeData =
            TempleteManager.Instance.getCrystalTempleteByTempleteId(peterTemId);
          if (!canComposeInfo || !shopInfo) return null;
          let canCount: number = bagCount / canComposeInfo.Count1;
          count = count - canCount;
          let obj: object = {
  ...
  private checkMountCardCompose() {
    ...
    if (
      this.model.composeType == ForgeData.COMPOSE_TYPE_PROP &&
      selectedCompose &&
      selectedCompose.itemTemplate
    ) {
      if (
        selectedCompose.itemTemplate.SonType == GoodsSonType.SONTYPE_MOUNT_CARD
      ) {
        ...
        // 是否已经合成
        let bBagHas = GoodsManager.Instance.getGoodsByGoodsTId(
          selectedCompose.template.NewMaterial,
        );
        // 是否已经激活
        let bLight = MountsManager.Instance.avatarList.isLightTemplate(
          selectedCompose.itemTemplate.Property1,
        );
  ...
  private setBtnState() {
    if (
      !this.model.selectedCompose ||
      !this._data ||
      this.model.selectedCompose.canMakeCount == 0
    ) {
  ...
  private get isSuit() {
    return (
      this.model.composeType == ForgeData.COMPOSE_TYPE_EQUIP ||
      this.model.composeType == ForgeData.COMPOSE_TYPE_UPGRADE_EQUIP ||
      this.model.composeType == ForgeData.COMPOSE_TYPE_ADVANCE_EQUIP
    );
  }
  ...
  private initAttribute(info: GoodsInfo, temp: t_s_itemtemplateData): string {
    ...
    str =
      LangManager.Instance.GetTranslation("armyII.ThaneAttributeView.Tip17") +
      "&nbsp;";
    tempStr += this.updateAttributeTxt(
      str,
      temp.Conat,
  ...
  private initSkill(info: GoodsInfo, temp: t_s_itemtemplateData): string {
    ...
    for (let i: number = 1; i < 6; i++) {
      ...
      if (info["randomSkill" + i] != 0) hasSkill = true;
    }
    if (hasSkill == false) {
  ...
  private initSuitSkill(
    ...
    for (let count: number = 0; count < skillList.length; count++) {
      ...
      if (skillTemp) {
        ...
        else tempStr += skillTemp.SkillTemplateName;
      }
      tempStr += "[/color]<br>";
    }
    return tempStr;
