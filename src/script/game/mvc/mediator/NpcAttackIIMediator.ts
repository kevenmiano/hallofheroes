...
export class NpcAttackIIMediator implements IMediator, IEnterFrame
{
    ...
    private __aiMoveStateHandler()
    {
        ...
        if(this._npc.npcAiInfo.moveState == AiStateType.NPC_RANDOM_MOVE_STATE || this._npc.npcAiInfo.moveState == AiStateType.STAND)
        {
            this._handlerRange = ((<CampaignNode>this._npc.nodeInfo).handlerRange + 1) * Tiles.WIDTH;
            this._handlerRange = (this._handlerRange > Tiles.WIDTH ? this._handlerRange : NpcEffectiveState.BATTLE_RADIUS);
