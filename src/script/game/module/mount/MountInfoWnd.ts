...
export default class MountInfoWnd extends BaseWindow {
  ...
  private initView() {
    ...
    if (this._mountData) {
      ...
      var item: WildSoulInfo =
        MountsManager.Instance.avatarList.getWildSoulInfo(
          this._mountData.TemplateId,
        );
      var strArray: Array<string>;

      if (this._isLock || this._mountData.StarItem == 0) {
        strArray = LangManager.Instance.GetTranslation(
          "mounts.WildsoulItem.tips01",
        ).split("|");
      } else {
        strArray = LangManager.Instance.GetTranslation(
          "mounts.WildsoulItem.Newtips01",
        ).split("|");
      }
      var tipStr: string = "";
      var speed: number = 0;
      ...
      var valueArray: Array<number> = [
        this._mountData.Power,
        this._mountData.Intellect,
        this._mountData.Physique,
        this._mountData.Agility,
        this._mountData.ExpandLevel,
        speed,
      ];
      ...
      if (strArray.length >= valueArray.length) {
        for (var i: number = 0; i < valueArray.length; i++) {
          if (valueArray[i] != 0) {
            if (this._isLock) {
              tipStr +=
                StringHelper.format(strArray[i], valueArray[i]) + "<br/>";
            } else {
              tipStr +=
                StringHelper.newFormat(strArray[i], valueArray[i], addNumber) +
                "<br/>";
            }
          }
        }
      ...
      this.willScoreTxt.text = LangManager.Instance.GetTranslation(
        "mountTip.soulscore",
        this._mountData.SoulScore,
      );
      if (this._mountData.MountType == MountType.NORMAL) {
        //普通
        ...
        if (this._mountData.NeedMountGrade > 0)
          needTips.push(
            LangManager.Instance.GetTranslation(
              ...
              this._mountData.NeedMountGrade,
            ),
          );
        if (this._mountData.NeedItemId != 0) {
          var goodTemplate: t_s_itemtemplateData =
            ConfigMgr.Instance.getTemplateByID(
              ...
              this._mountData.NeedItemId.toString(),
            );
          if (goodTemplate) {
