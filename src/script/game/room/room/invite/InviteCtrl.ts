...
export default class InviteCtrl extends FrameCtrlBase {
  ...
  private __getHallListHandler(pkg: PackageIn) {
    ...
    for (let i: number = 0; i < msg.roomPlayer.length; i++) {
      ...
      if (roomMsg.playerId) player.userId = roomMsg.playerId;
      if (roomMsg.templateId) player.templateId = roomMsg.templateId;
      if (roomMsg.grades) player.grades = roomMsg.grades;
      if (roomMsg.nickName) player.nickName = roomMsg.nickName;
      if (roomMsg.sex) player.sexs = roomMsg.sex;
      if (roomMsg.consortiaId) player.consortiaID = roomMsg.consortiaId;
      if (roomMsg.consortiaName) player.consortiaName = roomMsg.consortiaName;
      if (roomMsg.fightingCapacity)
        player.fightingCapacity = roomMsg.fightingCapacity;
      if (roomMsg.playerState) player.state = roomMsg.playerState;
      if (roomMsg.changeShapeId) player.changeShapeId = roomMsg.changeShapeId;
      if (roomMsg.isVip) player.IsVipAndNoExpirt = roomMsg.isVip;
      if (roomMsg.vipType) player.vipType = roomMsg.vipType;
      if (roomMsg.headId) player.snsInfo.headId = roomMsg.headId;
      if (roomMsg.frameId) player.frameId = roomMsg.frameId;
