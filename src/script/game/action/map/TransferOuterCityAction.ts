...
export class TransferOuterCityAction extends MapBaseAction {
    ...
    public update() {
        if (this.__count == 1) {
            this._movie = fgui.UIPackage.createObject(EmPackName.Space, "CollectionAsset").asCom;
            ...
            if (this._movie) {
                ...
                LayerMgr.Instance.addToLayer(this._movie.displayObject, EmLayer.GAME_DYNAMIC_LAYER, -1);
                this.__stageResizeHandler();
            }
            let mInfo: MapInfo = new MapInfo();
            mInfo.mapId = PlayerManager.Instance.currentPlayerModel.mapNodeInfo.info.mapId;
    ...
    private __switchSceneHandler(sceneType: string) {
        ...
        NotificationManager.Instance.dispatchEvent(NotificationEvent.SWITCH_MAP_TRANSFER, null);
