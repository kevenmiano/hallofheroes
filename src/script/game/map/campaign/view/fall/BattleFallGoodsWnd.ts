...
export default class BattleFallGoodsWnd extends BaseWindow {
  ...
  private refreshView() {
    ...
    if (this.arrayData && this.arrayData.length > 0)
      for (let i: number = 0; i < this.arrayData.length; i++) {
        ...
        let battleFallGoodsItem: BattleFallGoodsItem = <BattleFallGoodsItem>(
  ...
  private pickUpGoods(goods: GoodsInfo): boolean {
    if (this.canPickup(goods)) {
      ...
      var tip: string = LangManager.Instance.GetTranslation(
        "map.campaign.view.fall.command01",
      );
  ...
  private canPickup(goods: GoodsInfo): boolean {
    ...
    var countInBag: number = GoodsManager.Instance.getGoodsNumByTempId(
      goods.templateId,
    );
  ...
  private sendGetSimpleGoods(id: number) {
    ...
    if (this.nodeInfo as CampaignNode) {
      ...
      var nodeView: PhysicsChestView = (this.nodeInfo as CampaignNode)
        .nodeView as PhysicsChestView;
