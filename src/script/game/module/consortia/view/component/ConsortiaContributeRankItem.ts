...
export class ConsortiaContributeRankItem extends FUI_ConsortiaContributeRankItem {
    ...
    public set info(value: ConsortiaUserInfo) {
        ...
        if (this._info) {
            ...
            this.txt3.text = String(this.isHistory ? this._info.totalOffer : this._info.todayOffer)
            this.imgRank.url = this.index <= 3 ? fgui.UIPackage.getItemURL(EmPackName.Base, RankData.RankRes[this.index - 1]) : ""
        }
        else {
            this.txt1.text = ""
            this.txt2.text = ""
    ...
    public setdata(index: number, value: ConsortiaUserInfo, isHistory: boolean = true) {
        this.index = index;
