...
import {
  EffectFrameData,
  ColorTransformFrameData,
} from "../mode/framedata/FrameDatas";
...
export class SkillFrameExecuter {
  ...
  public static execute(
    ...
    let executer: SkillFrameExecuter = new SkillFrameExecuter(
      skill,
      frameData,
      action,
    );
  ...
  private playAttackAction() {
    ...
    if (
      RoleActionSimplifyData.isSimplify(roleView && roleView.rolePartUrlPath)
    ) {
      let actionTemplate = this._skill.getActionTemplate();
      if (actionTemplate) {
        Logger.battle(
          "[SkillFrameExecuter]playAttackAction",
          actionTemplate.ActionId,
          actionTemplate.frames,
        );
        delayFrame =
          actionTemplate.getCompleteFrameIndex() - this._frameData.Frame;
      }
      this._currentRole.action(
        attackFrameLabel,
        ActionMovie.WAIT_AND_GOTO,
  ...
  private addEffect(isGas: boolean = false) {
    ...
    if (this._frameData.effectData.type == EffectFrameData.TYPE_ROLE) {
      if (isGas) {
        ...
        this._currentRole.view.addEffect(
          effect,
          1,
          this._frameData.effectData.arrange,
        );
      }

      Logger.battle(
        `[SkillFrameExecuter]${this._currentRole.roleName}(${this._currentRole.livingId})自身的特效: ${effectName}`,
      );
      ...
      let pt = BattleUtils.getPosition(
        isLeft ? FaceType.LEFT_TEAM : FaceType.RIGHT_TEAM,
        targetPos,
      );
      ...
      if (isGas) {
        ...
        if (this._currentRole && this._currentRole.map) {
          this._currentRole.map.addEffect(
            effect,
            1,
            this._frameData.effectData.arrange,
          );
      ...
      Logger.battle(
        `[SkillFrameExecuter]对地图添加${this._currentRole.roleName}(${this._currentRole.livingId})的特效: ${effectName}`,
      );
      ...
      Logger.battle(
        `[SkillFrameExecuter]对受击角色${str}添加${this._currentRole.roleName}(${this._currentRole.livingId})的特效: ${effectName}`,
      );
    }
  }
  ...
  private playBgColorEffect() {
    ...
    this._currentRole.map.backGroundToColor(
      frameData.color,
      frameData.tweenTime,
      frameData.stopTime,
      frameData.atOnceBool,
      frameData.isReduce,
    );
  }
  ...
  private playBgShockEffect() {
    ...
    this._currentRole.map.shakeScreen(
      this._frameData.bgShockData.time,
      this._frameData.bgShockData.range,
    );
  }
  ...
  private addDanny() {
    ...
    if (
      this._skill.getCurrentRole() ==
      BattleManager.Instance.battleModel.selfHero
    ) {
      selfCause = true;
    }
    ...
    if (effectData.effectRes && effectData.effectRes.length > 1) {
      Logger.battle(
        "[SkillFrameExecuter]受击者身上的特效",
        effectData.effectRes,
        singelAttackArr,
      );
      ...
      BattleUtils.addSingleDannyTakeEffect(
        singelAttackArr,
        effectObject,
        12,
        true,
        effectData.backDistance,
        0,
        4,
        true,
        0x666666,
  ...
  private zoomMap() {
    ...
    TweenLite.to(this._currentRole.map, this._frameData.zoomData.zoomTime, {
      //@ts-expect-error: External dependencies
      zoom: this._frameData.zoomData.zoom,
      overwrite: 0,
    ...
    function zoomInComplete() {
      if (this._frameData.zoomData.stopTime > 0) {
        setTimeout(
          this.backZoomStart.bind(this),
          this._frameData.zoomData.stopTime * 1000,
        );
      } else {
        this.backZoomStart();
      }
  ...
  private displacementForward() {
    ...
    new DisplacementAction(
      this._currentRole,
      this._frameData.displacementForward,
      this._skill,
    );
  }
  private displacementBack() {
    ...
    new DisplacementAction(
      this._currentRole,
      this._frameData.displacementBack,
      this._skill,
    );
  }
  private hideRole() {
    ...
    if (this._frameData.hideData.target == 0) {
      //0代表受击方
      let singelAttackArr: any[] = this._attackData[0];
      for (let j: number = 0; j < singelAttackArr.length; j++) {
        let command: AttackData = singelAttackArr[j] as AttackData;
        let role: any = BattleManager.Instance.battleModel.getRoleById(
          command.roleId,
        );
        if (role) {
