...
export default class CastleBuildingView extends BuildingBase {
  ...
  initClickEvent() {
    if (
      !this._curView ||
      this._buildingInfo.templeteInfo.SonType == BuildingType.TREE
    )
      return;
  ...
  refreshView() {
    ...
    if (view == null && !this._curView) {
      if (StringHelper.isNullOrEmpty(templete.PicPath) == false) {
        this.buildingLoader = new CastleBuildingLoader(
          this._buildingInfo,
          new Func(this, this.loadViewCallBack),
        );
      }
    }
    this.initMarketHalo();
  ...
  private loadViewCallBack(tempId: number) {
    ...
    } else this.setCurView(view);
    this.isLoaded = true;
    BuildingManager.Instance.dispatchEvent(
      BuildingEvent.BUILDING_LOAD_COMPLETE,
      this._buildingInfo,
    );
  ...
  private setCurView(view: Laya.Sprite) {
    ...
    if (view == null || this._curView) return;
    this._curView = view;
    ...
    if (this._effectView) {
      let index = 0;
      this._effectView.forEach((effectMC: MovieClip) => {
        let pos: Laya.Point = CastleConfigUtil.Instance.getBuildEffectPos(
          sonyType,
          index,
        );
    ...
    Logger.info(
      "加载完成",
      sonyType,
      this._buildingInfo.templeteInfo.BuildingNameLang,
    );
  ...
  private initMarketHalo() {
    if (this._buildingInfo.templeteInfo.SonType != BuildingType.MAKET_BUILD)
      return;
    if (!this._marketHalo) {
  ...
  public dispose() {
    ...
    ObjectUtils.disposeObject(this._curViewContainer);
    this._curViewContainer = null;
    this._curView = null;
