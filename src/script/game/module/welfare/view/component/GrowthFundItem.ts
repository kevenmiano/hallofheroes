...
export class GrowthFundItem extends FUI_GrowthFundItem {
  ...
  private refreshView() {
    ...
    if (this._vInfo.grade == 0) {
      ...
      this.txt_level.text = LangManager.Instance.GetTranslation(
        "growthFundview.item.txt_level2",
        this._vInfo.grade,
      );
      goods.count = this._vInfo.bindDiamondCount;
    ...
    this.c1.selectedIndex =
      this._vInfo.packageState - 1 >= 0 ? this._vInfo.packageState - 1 : 0;
    this.btn_receive.enabled = this.c1.selectedIndex == 1;
    let disableColor = "#aaaaaa";
    let enableColor = "#FFF6B9";
    this.btn_receive.titleColor = this.btn_receive.enabled
      ? enableColor
      : disableColor;
  }
