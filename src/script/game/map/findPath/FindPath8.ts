interface IFindPath {
  map(): any[][];
  path(): Laya.Point[];
  find(startPoint: Laya.Point, endPoint: Laya.Point): any[];
  dispose(): void;
}
...
export class FindPath8 implements IFindPath {
  ...
  public find(startPoint: Laya.Point, endPoint: Laya.Point): any[] {
    ...
    while (!found) {
      ...
      while (neighborIndex < neighborsLength) {
        ...
        if (
          currentNeighbor == this._endPoint &&
          Math.abs(
            (currentNeighbor.y - currentNode.y) /
              (currentNeighbor.x - currentNode.x),
          ) != 1
        ) {
  ...
  private count(param1: any, param2: any) {
    ...
    if (!param1.block) {
      ...
      if (
        Math.abs(param1.x - param2.x) == 1 &&
        Math.abs(param1.y - param2.y) == 1
      ) {
      ...
      if (param1.x > param2.x && param1.y < param2.y) {
        if (
          this._map[param1.y][param1.x - 1].value == 1 &&
          this._map[param1.y + 1][param1.x].value == 1
        ) {
          _loc_3 = _loc_3 + 10000;
        }
  ...
  private calculateCost(currentNode: any, previousNode: any) {
    ...
    if (
      Math.abs(currentNode.x - previousNode.x) == 1 &&
      Math.abs(currentNode.y - previousNode.y) == 1
    ) {
      cost += 4;
    }
    if (currentNode.open) {
