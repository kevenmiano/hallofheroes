...
export default class WorldBossSceneWnd extends BaseWindow {
    ...
    this._tabData = [
      {
        str: LangManager.Instance.GetTranslation(
          "worldboss.view.WoundInfoView.JobTabTxt",
        ),
      },
      {
        str: LangManager.Instance.GetTranslation(
          "worldboss.view.WoundInfoView.AllTabTxt",
        ),
      },
    ];
  ...
  OnShowWind() {
    ...
    this.allTabHandler();
    if (
      this.playerModel.getWolrdBossAutoFightFlag() ==
      PlayerModel.WORLDBOSS_AUTO_FIGHT
    ) {
      //自动寻路
      this.statusCtr.selectedIndex = 1;
      this.leftTimeTxt.text = "";
    } else if (
      this.playerModel.getWolrdBossAutoLiveFlag() ==
      PlayerModel.WORLDBOSS_AUTO_LIVE
    ) {
      //自动复活
  ...
  private initEvent() {
    ...
    if (this._mapModel)
      this._mapModel.addEventListener(
        CampaignMapEvent.SYNC_ERROR_TIME,
        this.syncErrorTimeHandler,
        this,
      );
    ...
    this.rankList.itemRenderer = Laya.Handler.create(
      this,
      this.renderRankListItem,
      null,
      false,
    );
    if (this.model)
      this.model.addEventListener(
        WorldBossEvent.UPDATE_WOUND_LIST,
        this.__updateWoundListHandler,
        this,
      );
  ...
  private updateBossState() {
    if (
      ...
      var nextNode: CampaignNode =
        CampaignManager.Instance.mapModel.getToAttackNode();
      if (!nextNode)
        nextNode = CampaignManager.Instance.mapModel.mapNodeEndPoint;
      if (nextNode) {
        var vNode: DisplayObject =
          CampaignManager.Instance.mapView.getNpcNodeById(nextNode.info.id);
        CampaignManager.Instance.mapModel.selectNode = nextNode;
        (<CampaignMapScene>CampaignManager.Instance.controller).moveArmyByPos(
          vNode.x,
          vNode.y,
          true,
          true,
      ...
      //自动复活
      WorldBossSocketOutManager.sendEnterBattle(
  ...
  private cancelFightBtnHander() {
    if (!this.playerModel) return;
    this.playerModel.setWorldBossAutoFight(
  ...
  private autoLiveBtnHander() {
    if (!this.checkHasOpen()) {
      MessageTipManager.Instance.show(
        LangManager.Instance.GetTranslation(
          "map.campaign.view.ui.demon.ConsortiaDemonSkillIcon.noOpen",
        ),
      );
      return;
    }
    var contextStr: string = LangManager.Instance.GetTranslation(
      "map.campaign.view.ui.RiverView.contentNew",
      this._autoLiveCost,
    );
    ...
    SimpleAlertHelper.Instance.Show(
      ...
      { point: this._autoLiveCost, checkDefault: true },
      prompt,
      contextStr,
      confirm,
      cancel,
      this.autoLiveAlertConfirm.bind(this),
    );
  }
  ...
  private autoLiveAlertConfirm(b: boolean, flag: boolean) {
    if (b) {
      if (!flag) {
        //没有勾选优先使用绑定钻石
        if (this.playerInfo.point < this._autoLiveCost) {
          //钻石不足
          MessageTipManager.Instance.show(
  ...
  private checkHasOpen(): boolean {
    ...
    for (const key in nList) {
      if (nList.hasOwnProperty(key)) {
        ...
        if (
          nInfo.info.types == PosType.ATTACK_HANDLER &&
          nInfo.info.state == NodeState.DESTROYED
        ) {
          flag = true;
        }
      }
  ...
  private helpHandler() {
    let title = LangManager.Instance.GetTranslation(
      "worldboss.view.WorldBossHelp.title1",
    );
    let content = LangManager.Instance.GetTranslation(
      "worldboss.view.BaseHelpFrame.helpText",
    );
  ...
  private updateView() {
    if (this.model.selfWoundInfo.wound < 0) {
      //未对BOSS造成伤害
      this.selfDamageTxt.text = LangManager.Instance.GetTranslation(
        "worldboss.view.WoundInfoItemView.nickName",
      );
    } else {
    ...
    var countValue: string = LangManager.Instance.GetTranslation(
      "WorldBoss.WoundInfo.countValue",
    );
    this.countValueTxt.text =
      this.model.totalNum < 300 ? countValue : this.model.totalNum + "";
    if (this.model.buffGrade > 0) {
      this.bufferDescTxt.text = LangManager.Instance.GetTranslation(
        "worldboss.view.WoundInfoView.buffGradeTip",
        this.model.buffGrade,
        this.model.buffGrade * 500,
      );
    } else {
      this.bufferDescTxt.text = LangManager.Instance.GetTranslation(
        "worldboss.view.WoundInfoView.buffTip",
      );
    }
    this.updateBossInfo();
  }
  ...
  private updateTimeView() {
    ...
    var cTemp: t_s_campaignData =
      ConfigMgr.Instance.worldBossDic[RoomManager.Instance.roomInfo.campaignId];
    ...
    this._offTime = parseInt(
      (
        (curDate.getTime() -
          curTime +
          CampaignManager.Instance.mapModel.syncErrorTime * 1000) /
        1000
      ).toString(),
    );
    if (this._offTime > 0) {
  ...
  private __updateSTimeHandler() {
    ...
    if (this.bufferLeftTimeTxt && !this.bufferLeftTimeTxt.isDisposed)
      this.bufferLeftTimeTxt.text = DateFormatter.getSevenDateString(
        this._offTime,
      );
    if (this._offTime <= 0) {
      Laya.timer.clear(this, this.__updateSTimeHandler);
  ...
  private updateBossInfo() {
    ...
    if (this._preHp != 0 && this._preHp - this._worldBossModel.curHp > 0)
      this._bossInfoView.updateWorldBossHp(
        this._worldBossModel.curHp - this._preHp,
      );
  ...
  private updateWorldBossInfo() {
    ...
    for (const key in arr) {
      if (Object.prototype.hasOwnProperty.call(arr, key)) {
        ...
        if (
          node.info.types == PosType.COPY_END ||
          node.info.types == PosType.COPY_NPC
        ) {
          endBoss = node;
          break;
        }
