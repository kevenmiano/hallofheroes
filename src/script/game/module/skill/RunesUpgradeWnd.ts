...
export default class RunesUpgradeWnd extends BaseWindow {
  ...
  onInitView() {
    this.frame.getChild("title").text = LangManager.Instance.GetTranslation(
      "armyII.viewII.rune.RuneUpgradeView.TitleTxt",
    );
    ...
    this.upCostLabel.text = LangManager.Instance.GetTranslation(
      "armyII.viewII.rune.RuneUpgradeView.UpgradeCostTxt",
    );

    this._runeCarryTwoLevel = Number(
      TempleteManager.Instance.getConfigInfoByConfigName("CarryTwoRune")
        .ConfigValue,
    );
  ...
  onShowHelpTIps() {
    let title = LangManager.Instance.GetTranslation(
      "armyII.SkillFrame.HelpBtn.RuneTipdata",
    );
    let content = LangManager.Instance.GetTranslation(
      "armyII.RuneHelpFrame.helpContent",
    );
  ...
  onCheckRuneHandler() {
    ...
    if (checkState) {
      this._chcekcIndex = 0;
      this.upCostGoldTick.selected = checkGoldState
        ? !checkGoldState
        : checkGoldState;
    } else {
      this._chcekcIndex = -1;
  ...
  onCheckGoldHandler() {
    ...
    if (checkGoldState) {
      this._chcekcIndex = 1;
      this.upCostRuneTick.selected = checkRuneState
        ? !checkRuneState
        : checkRuneState;
    } else {
      this._chcekcIndex = -1;
  ...
  private __upgradeBtnClickHandler(evt) {
    ...
    if (this._chcekcIndex == 0) {
      ...
      if (this.currRuneData.nextTemplateInfo && this.currRuneData.grade < 10) {
        if (
          ArmyManager.Instance.thane.grades <
            this.currRuneData.nextTemplateInfo.NeedGrade &&
          this.currRuneData.runeCurGp ==
            this.currRuneData.nextTemplateInfo.NeedGp
        ) {
      ...
      if (
        ...
        Number(this.CostGoldNumTxt.text)
      ) {
  ...
  private updateView() {
    ...
    let cfgValue = 2;
    let cfgItem =
      TempleteManager.Instance.getConfigInfoByConfigName("UpRune_Price");
    if (cfgItem) {
  ...
  private __runeRefreshHandler(evt) {
    ...
    for (let key in runeList) {
      if (Object.prototype.hasOwnProperty.call(runeList, key)) {
        ...
        if (
          info &&
          info.templateInfo.RuneType == this.currRuneData.templateInfo.RuneType
        ) {
  ...
  setRunInfo(value: RuneInfo) {
    ...
    this.runeNameTxt.text =
      value.templateInfo.TemplateNameLang +
      "  " +
      LangManager.Instance.GetTranslation(
        "public.level3",
        value.templateInfo.RuneGrade.toString(),
      ); //名称
    if (value.nextTemplateInfo) {
      this.upProgressBar.value =
        (value.runeCurGp / value.nextTemplateInfo.NeedGp) * 100;
      this.upProgressBar.getChild("progress").text =
        value.runeCurGp + "/" + value.nextTemplateInfo.NeedGp;
    } else {
      this.upProgressBar.value =
        (value.templateInfo.NeedGp / value.templateInfo.NeedGp) * 100;
      this.upProgressBar.getChild("progress").text =
        value.templateInfo.NeedGp + "/" + value.templateInfo.NeedGp;
    }
