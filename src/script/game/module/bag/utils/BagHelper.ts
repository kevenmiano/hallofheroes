...
import { RuneEvent, TipsEvent } from "../../../constant/event/NotificationEvent";
...
export class BagHelper extends GameEventDispatcher {
	...
	/**播放金币、钻石获得特效 */
	public static playGoldEffect(imgUrl: string, count: number = 1, startPoint: Laya.Point = new Laya.Point(667, 375)) {
		...
		item.Play(new Laya.Rectangle(pos.x, pos.y, Math.abs(startPoint.x - targetPos.x), Math.abs(startPoint.y - targetPos.y)), targetPos.x, targetPos.y);
	}
	...
	public playNewGoodsEffect(info: GoodsInfo, $callBack: Function = null) {
		...
		ResMgr.Instance.loadRes(path, (content: Laya.Texture) => {
			...
			let bagBtn = HomeWnd.Instance.getMainToolBar().getBtnByType(EmMainToolBarBtnType.BAG);
	...
	private _moveComplete(icon: Laya.Sprite) {
		...
		TweenMax.to(icon, 0.5, { alpha: 0, onComplete: (icon: Laya.Sprite)=>{
			if (!icon || icon.destroyed) return;
	...
	public static moveRoleBagToConsortiaBag(info: GoodsInfo, count: number = 1) {
		...
		if (!ConsortiaManager.Instance.model.caseBagCanUse) {
			str = LangManager.Instance.GetTranslation("cell.mediator.consortiabag.ConsortiaBagDoubleClickMediator.operateErrorTips");
		...
		if (GoodsManager.Instance.isSellPos(info.pos) && info.bagType == BagType.Player) {
			return;
		}
		let to_pos: number = -1;
		let num: number = (FrameCtrlManager.Instance.getCtrl(EmWindow.Consortia) as ConsortiaControler).model.currentCasecellNum;
		to_pos = this.check(num, info);
		if (to_pos == -1) {
			for (let j: number = 0; j < num; j++) {
				if (!GoodsManager.Instance.goodsListByPos.hasOwnProperty(j + "_" + 0 + "_" + BagType.Storage)) {
					to_pos = j;
					break;
				}
			}
		...
		if (to_pos != -1) {
			PlayerManager.Instance.moveBagToBag(info.bagType, info.objectId, info.pos, BagType.Storage, 0, to_pos, count);
			Laya.timer.once(100, this, () => {
				NotificationManager.Instance.sendNotification(TipsEvent.EQUIP_TIPS_HIDE);
			});
			return;
		}
		str = LangManager.Instance.GetTranslation("cell.mediator.consortiabag.ConsortiaBagDoubleClickMediator.command01");
		MessageTipManager.Instance.show(str);
	}

	public static check(num: number, data: GoodsInfo): number {
		let wnd: ConsortiaStorageWnd = FrameCtrlManager.Instance.getCtrl(EmWindow.ConsortiaStorageWnd).view;
	...
	public static canSuperposition(item: GoodsInfo, data: GoodsInfo): boolean {
		if (item.templateId == data.templateId && item.templateInfo.MaxCount >= item.count + data.count && item.isBinds == data.isBinds) {
			return true;
		}
		return false;
	}
	...
	public static moveConsortiaToRoleBag(info: GoodsInfo, count: number = 1) {
		...
		if (to_pos == -1) {
			let str: string = LangManager.Instance.GetTranslation("cell.mediator.consortiabag.ConsortiaCaseCellClickMediator.command05");
			MessageTipManager.Instance.show(str);
			return;
		}
		PlayerManager.Instance.moveBagToBag(info.bagType, info.objectId, info.pos, BagType.Player, 0, to_pos, count);
		Laya.timer.once(100, this, () => {
			NotificationManager.Instance.sendNotification(TipsEvent.EQUIP_TIPS_HIDE);
		});
	}
	...
	public static consortiaStorageOperate(info: GoodsInfo) {
		if (info) {
			...
			if (info.bagType == BagType.Storage) {
				//公会保管箱移到到玩家背包
				count = GoodsManager.Instance.getBagCountByTempIdAndPos(BagType.Storage, info.templateInfo.TemplateId, info.pos);
				// if (count > 1) {
	...
	public static operateRuneGem(info: GoodsInfo, type: number = 0) {
		if (info) {
			if (type == 0) {
				NotificationManager.Instance.sendNotification(RuneEvent.RUNE_GEM_RESOLVE, info);
			} else {
				NotificationManager.Instance.sendNotification(RuneEvent.RUNE_GEM_UPGRADE, info);
			}
		}
	}
	/**设置按钮文字 */
	...
	public static oneKeyMoveToConsortiaBag(array: Array<GoodsInfo>) {
		...
		if (needCount > leftCount) {
			//格子数不足
			str = LangManager.Instance.GetTranslation("bagwnd.consortia.storage.numberNotEnlogh");
			MessageTipManager.Instance.show(str);
			return;
		}
	...
	public static checkConsortiaStorageLeftNum(): number {
		let totalCount: number = (FrameCtrlManager.Instance.getCtrl(EmWindow.Consortia) as ConsortiaControler).model.currentCasecellNum;
	...
	public getUserCount(templeteId: number): number {
		...
		if (templeteId > 0) {
			count = GoodsManager.Instance.getBagCountByTempId(BagType.Player, templeteId); //只显示领主背包里的道具数量
		} else {
			switch (templeteId) {
				...
				case TemplateIDConstant.TEMP_ID_GIFT:
					count = PlayerManager.Instance.currentPlayerModel.playerInfo.giftToken;
					break;
				...
				case TemplateIDConstant.TEMP_ID_CONSORTIA:
					count = PlayerManager.Instance.currentPlayerModel.playerInfo.consortiaCoin;
					break;
				case TemplateIDConstant.TEMP_ID_PET_JINHUN:
					count = PlayerManager.Instance.currentPlayerModel.playerInfo.petEquipStrengNum;
					break;
				case TemplateIDConstant.GUILD_CONTRIBUTION:
					count = PlayerManager.Instance.currentPlayerModel.playerInfo.consortiaJianse;
					break;
				default:
					break;
			}
	...
	checkHonorUpLevel(honorEquipLevel) {
		//功勋升级，需要满足荣誉等级条件
		let curCfg: t_s_honorequipData = TempleteManager.Instance.geHonorCfgByType(0, honorEquipLevel);
		if (curCfg) {
		...
		let nextCfg: t_s_honorequipData = TempleteManager.Instance.geHonorCfgByType(0, honorEquipLevel + 1);
		if (nextCfg) {
			let own_num = GoodsManager.Instance.getGoodsNumByTempId(ShopGoodsInfo.MEDAL_TEMPID);
			return own_num >= curCfg.ConsumeMedal && ResourceManager.Instance.gold.count >= curCfg.ConsumeGold;
		}
		return false;
	}
	...
	checkHonorUpStage(honorEquipStage) {
		...
		let nextCfg: t_s_honorequipData = TempleteManager.Instance.geHonorCfgByType(1, nextstage + 1);
		if (!nextCfg) {
			return false;
		}
	...
	checkJewelRedDot() {
		...
		//是否最大等级
		let ownNum = GoodsManager.Instance.getCountBySontypeAndBagType(GoodsSonType.SONTYPE_SOUL_CRYSTAL, BagType.Player);
		var temp: t_s_upgradetemplateData = TempleteManager.Instance.getTemplateByTypeAndLevel(
		...
		temp = TempleteManager.Instance.getTemplateByTypeAndLevel(ArmyManager.Instance.thane.jewelGrades + 1, UpgradeType.UPGRADE_TYPE_SOUL);
		if (temp) {
	...
	checkFortuneRedDot() {
		let goodsCount = GoodsManager.Instance.getCountBySontypeAndBagType(GoodsSonType.SONTYPE_FATE_STONE, BagType.Player);
		...
		for (let i = 0; i < array.length; i++) {
			...
			let nextSkillTemp = TempleteManager.Instance.getSkillTemplateInfoBySonTypeAndGrade(temp.SonType, temp.Grades + 2);
			if (!nextSkillTemp) {
				resultArr[i] = false;
			} else {
				nextSkillTemp = TempleteManager.Instance.getSkillTemplateInfoBySonTypeAndGrade(temp.SonType, temp.Grades + 1);
				if (nextSkillTemp) {
