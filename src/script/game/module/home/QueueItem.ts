...
export default class QueueItem extends FUI_QueueItem {
	...
	public set vData(info: any) {
		...
		if (this._vData) {
			this._vData.removeEventListener(Laya.Event.COMPLETE, this.__completeHandler, this);
			this._vData.removeEventListener(Laya.Event.CHANGE, this.__changeHandler, this);
		}
		if (this._vData) {
			this._vData.addEventListener(Laya.Event.COMPLETE, this.__completeHandler, this);
			this._vData.addEventListener(Laya.Event.CHANGE, this.__changeHandler, this);
		}
		this.updateView();
	}
	...
	public __itemClickHandler() {
		...
		switch (this.type) {
			...
			case QueueItem.QUEUE_TYPE_COLOSSEUM:
				...
				else if (this.vData.remainCount > 0 || this.vData.remainBuyCount > 0) FrameCtrlManager.Instance.open(EmWindow.Colosseum);
				break;
			case QueueItem.QUEUE_TYPE_ALCHEMY:
				if (this.vData.remainTime > 0) this.coolItem();
				else {
					bInfo = BuildingManager.Instance.getBuildingInfoBySonType(BuildingType.OFFICEAFFAIRS);
					if (bInfo.property2 > bInfo.property1 || BuildingManager.Instance.innerLevy.length - bInfo.payImpose) this.openBuildFrame(bInfo);
				}
				break;
			...
			case QueueItem.QUEUE_TYPE_TEC: //科技
				if (this.vData.remainTime > 0) this.coolItem();
					...
					if (!flag) {
						MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("QueueItem.type.tec"));
						return;
					} else {
	...
	private coolItem() {
		...
		if (this._vData && this._vData.remainTime > 0) {
			...
			let cfgItemChallenge = TempleteManager.Instance.getConfigInfoByConfigName("QuickCoolChallenge_Price");
			...
			if (this.type == QueueItem.QUEUE_TYPE_COLOSSEUM || this.type == QueueItem.QUEUE_TYPE_PET)
	...
	private buyQueueItem() {
		let cfgValue = 100;
		let cfgItem = TempleteManager.Instance.getConfigInfoByConfigName("Order_NeedPay");
		if (cfgItem) {
			cfgValue = Number(cfgItem.ConfigValue);
		}
		var content: string = LangManager.Instance.GetTranslation("mainBar.QueueBar.content", cfgValue);
		...
		var prompt: string = LangManager.Instance.GetTranslation("mainBar.QueueBar.prompt");
	...
	private coolColosseBack(result: boolean, id: number = 0, type: number = 0, useBind: boolean = true) {
		if (result) {
	...
	private initEvent() {
		if (this.Btn_CompImmediately) this.Btn_CompImmediately.onClick(this, this.__clickHandler.bind(this));
	...
	private removeEvent() {
		...
		if (this.Btn_CompImmediately) this.Btn_CompImmediately.offClick(this, this.__clickHandler.bind(this));
	...
	private updateView() {
		...
		switch (this.type) {
			case QueueItem.QUEUE_TYPE_NULL:
				this.titleTxt1.text = "[color=#00aeef]" + LangManager.Instance.GetTranslation("mainBar.view.QueueItem.NULL") + "[/color]";
				// this.titleTxt1.color = "#00aeef";
				// this.Btn_CompImmediately.visible = false;
				this.cType.selectedIndex = 1;
				break;
			case QueueItem.QUEUE_TYPE_BUILD:
				txt1 = LangManager.Instance.GetTranslation("mainBar.view.QueueItem.BUILD");
				txt2 = LangManager.Instance.GetTranslation("mainBar.view.QueueItem.BUILD2");
				this.updateTxt(txt1, txt2);
				break;
			case QueueItem.QUEUE_TYPE_TEC:
				txt1 = LangManager.Instance.GetTranslation("buildings.seminary.view.TechnologyOrderItemView.content");
				txt2 = LangManager.Instance.GetTranslation("buildings.seminary.view.TechnologyOrderItemView.content2");
				this.updateTxt(txt1, txt2);
				break;
			case QueueItem.QUEUE_TYPE_COLOSSEUM:
				if (!this._vData) return;
				if (this._vData.remainCount > 0) {
					txt1 = LangManager.Instance.GetTranslation("mainBar.view.QueueItem.COLOSSEUM");
					str = txt1 + " " + this._vData.remainCount + " / " + this._vData.totalCount;
				} else {
					str = LangManager.Instance.GetTranslation("mainBar.view.QueueItem.COLOSSEUM1");
				}
				this.updateTxt(str, str);
				break;
			case QueueItem.QUEUE_TYPE_PET:
				if (!this._vData) return;
				str = LangManager.Instance.GetTranslation("QueueItem.petchallenge", this._vData.remainCount);
				this.updateTxt(str, str);
				break;
			case QueueItem.QUEUE_TYPE_ALCHEMY:
				...
				if (info.property2 - info.property1 == 0) {
					...
					if (maxLevy - info.payImpose == 0) {
						str = LangManager.Instance.GetTranslation("mainBar.view.QueueItem.ALCHEMY1");
					} else {
						txt1 = LangManager.Instance.GetTranslation("mainBar.view.QueueItem.ALCHEMY2");
						str = txt1 + " " + (maxLevy - info.payImpose) + " / " + maxLevy;
					}
				} else {
					txt1 = LangManager.Instance.GetTranslation("mainBar.view.QueueItem.ALCHEMY");
					str = txt1 + " " + (info.property2 - info.property1) + " / " + info.property2;
				}
				this.updateTxt(str, str);
				break;
			default:
	...
	protected updateTxt(str1: string, str2: string) {
		if (!this._vData) {
		...
		} else if (this._vData && parseInt(this._vData.remainTime.toString()) <= 0) {
			// this.Btn_CompImmediately.visible = false;
			this.cType.selectedIndex = 1;
			...
			let leftTimeTxt = "[color=#ffffff]" + DateFormatter.getCountDate(this._vData.remainTime) + "[/color]";
