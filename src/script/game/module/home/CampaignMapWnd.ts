...
import { CampaignMapEvent, FreedomTeamEvent, ObjectsEvent, TreasureMapEvent } from '../../constant/event/NotificationEvent';
...
export default class CampaignMapWnd extends SmallMapWnd {
    ...
    protected initData() {
        ...
        this._sortDataDic[30000] = [3000003, 3000004, 3000002, 3000005, 3000006, 3000008, 3000009];
        this._sortData = [];
    ...
    protected addEvent() {
        ...
        this._selfArmyView.on(ObjectsEvent.WALK_NEXT, this, this.__walkNextHandler.bind(this));
        this._selfArmyView.on(ObjectsEvent.WALK_OVER, this, this.__walkOverHandler.bind(this));
    ...
    protected removeEvent() {
        ...
        this._selfArmyView.off(ObjectsEvent.WALK_NEXT, this, this.__walkNextHandler.bind(this));
        this._selfArmyView.off(ObjectsEvent.WALK_OVER, this, this.__walkOverHandler.bind(this));
    ...
    protected initNodePosition() {
        ...
        for (i; i < list.length; i++) {
            ...
            if (WorldBossHelper.checkInPetBossFloor(this._model.mapId) && nodeInfo.info.types == PosType.PET_BOSS_MONSTER && nodeInfo.param4 != "3") {
                continue;
            }
            if (nodeInfo.info.state == NodeState.HIDE || nodeInfo.info.state == NodeState.DESTROYED) {
                continue;
            }
            ...
            if (nodeInfo.info.types == 171 || nodeInfo.sonType == 2336) {
                ...
                transfer.x = this.positionConvert(nodeInfo.curPosX) - transfer.width * 0.5;
                transfer.y = this.positionConvert(nodeInfo.curPosY) - 40;
                nameText.x = transfer.x - (nameText.width - transfer.width) * .5;
                if (nodeInfo.curPosY == 166) {//避免神秘人和传送阵文字重叠
    ...
    protected __mapMouseUpHandler(event: Laya.Event) {
        ...
        let selfImgPos = this.getSelfImgPos();
        var targetX: number = (targetPos.x - selfImgPos.x) * 8 / this.mapScaleTimes;
        var targetY: number = (targetPos.y - selfImgPos.y) * 8 / this.mapScaleTimes;
    ...
    protected __nodeClickHandler(node) {
        ...
        CampaignManager.Instance.controller.moveArmyByPos(node.curPosX * 20, node.curPosY * 20, true);
    }
}