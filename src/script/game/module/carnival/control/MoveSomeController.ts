 ...
 * $callBack 回调函数
 */
export default class MoveSomeController extends fgui.GComponent implements IEnterFrame {
     ...
     * @param $callBack
     */
    constructor($itemList: Array<any>, $itemList2: Array<any>, $itemSpace: number = 0, $way: number, $lessRun: number, $initSpeed: number, $initBackSpeed: number, $callBack: Function = null) {
        super();
    ...
    private initView() {
        ...
        for (i = 0; i < this._itemLen; i++) {
            ...
            if (item instanceof Laya.Sprite)
                this._firstSp.addChild(item);
            else if (item instanceof fgui.GComponent)
                this._firstSp.addChild(item.displayObject)

            if (item2 instanceof Laya.Sprite)
                this._secondSp.addChild(item2);
    ...
    public enterFrame() {
        ...
        if (this._runingPassTime >= this._lessRun && this._currIdx == this._targetIdx) {
            this.stop();
        } else {
            ...
            switch (this._way) {
                case this.UP:
                    ...
                    if (this._secondSp.y - this._speed <= 0) {
                        ...
                        this._firstSp.y = this._secondSp.y + this._itemSpace * this._itemLen;
                ...
                case this.LEFT:
                    ...
                    if (this._secondSp.x - this._speed <= 0) {
                        ...
                        this._firstSp.x = this._secondSp.x + this._itemSpace * this._itemLen;
                        this._tempSp = this._firstSp;
                        this._firstSp = this._secondSp;
                        this._secondSp = this._tempSp;
                    } else {
    ...
    public start(targetIdx: number) {
        ...
        Laya.Tween.to(this, { speed: this._initBackSpeed }, 0.1, Laya.Ease.quintInOut, null, delay);
    ...
    public dispose() {
        ...
        ObjectUtils.disposeObject(this._firstSp); this._firstSp = null;
        ObjectUtils.disposeObject(this._secondSp); this._secondSp = null;
