...
export default class PetCampaignLevelItem extends FUI_PetCampaignLevelItem {
    ...
    public set vdata(value: t_s_uiplaylevelData) {
        this._vdata = value;
        if (this._vdata) {
            this.txtLevel.text = LangManager.Instance.GetTranslation("petCampaign.Level", value.UiLevelSort);
            if (this.petCampaignModel) {
                if (this.petCampaignModel.levelSort == PetCampaignManager.Instance.model.getCurLevelList().length) {
                    if (this.petCampaignModel.levelSort == this._index + 1) {//设置默认选择项目
                        this.setSelect(true);
                    }
                    else {
                    ...
                    if (this.petCampaignModel.levelSort == this._index) {//设置默认选择项目
                        this.setSelect(true);
                    }
                    else {
                        this.setSelect(false);
        ...
        PetCampaignManager.Instance.model.addEventListener(PetCampaignEvent.PET_CAMPAIGN_TREE_SELECT, this.onSelect, this);
    }
