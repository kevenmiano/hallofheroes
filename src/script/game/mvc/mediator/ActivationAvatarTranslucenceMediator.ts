...
export default class ActivationAvatarTranslucenceMediator implements IMediator, IEnterFrame {
    // 正常显示的人物
        ...
        if (this._frameCount > ActivationAvatarTranslucenceMediator.CheckFrame) {
            ...
            if (dic && this.selfCampaignArmy) {
                ...
                for (let index = 0; index < tempArr.length; index++) {
                    ...
                    if (index < ActivationAvatarTranslucenceMediator.NormalShowAvatarCnt) {
    ...
    private calDistance(armyInfo: CampaignArmy): number {
        let self: Laya.Point = new Laya.Point(this.selfCampaignArmy.curPosX, this.selfCampaignArmy.curPosY);
