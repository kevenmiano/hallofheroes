...
export class RuneItemTips extends BaseTips {
  ...
  public OnInitWind() {
    ...
    this.txt_look.text =
      "[" +
      LangManager.Instance.GetTranslation("shop.GoodsItem.lookBtnTxt") +
      "]";
  ...
  private setRuneInfo(runeData: RuneInfo, isinit: boolean = false) {
    ...
    this.runeName.text = this.skillName2.text =
      this.runeSkillInfo.TemplateNameLang; //名称
    this.bePassiveSkillLabel.text =
      this.bePassiveSkillLabel2.text =
    ...
    if (runeData.grade == 0) {
      ...
      this.runeEffect.getChild("content").text =
        this.runeSkillInfo.DescriptionLang;
      ...
      if (
        !this.isMax &&
        runeData.nextTemplateInfo &&
        ArmyManager.Instance.thane.grades >= runeData.nextTemplateInfo.NeedGrade
      ) {
    ...
    if (this._selectRuneData.nextTemplateInfo) {
      ...
      this.img_barbg0.width = 0;
      let barValue =
        this._selectRuneData.runeCurGp /
  ...
  private __runeRefreshHandler(runeInfo: RuneInfo) {
    ...
    //是否是学习
    if (isStudy) {
      let runesPanel = (NewbieUtils.getFrame(EmWindow.Skill) as SkillWnd)
        .runesPanel;
      //检查是否有空槽
  ...
  private showProgress(count: number) {
    if (this._selectRuneData.nextTemplateInfo) {
      ...
      this.img_barbg0.width =
        (curExp / this._selectRuneData.nextTemplateInfo.NeedGp) * 346;
  ...
  private __upgradeBtnClickHandler(evt) {
    ...
    if (this._chcekcIndex == 0) {
      ...
      if (
        this._selectRuneData &&
        this._selectRuneData.nextTemplateInfo &&
        this._selectRuneData.grade < 10
      ) {
        if (
          ArmyManager.Instance.thane.grades <
            this._selectRuneData.nextTemplateInfo.NeedGrade &&
          this._selectRuneData.runeCurGp ==
            this._selectRuneData.nextTemplateInfo.NeedGp
        ) {
          str = LangManager.Instance.GetTranslation(
