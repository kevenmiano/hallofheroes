...
export default class ConsortiaElectionWnd extends BaseWindow {
  ...
  private initEvent() {
    ...
    this._consortiaModel.addEventListener(
      ConsortiaEvent.CONSORTIA_VOTINGLIST_CHANGE,
      this.refreshView,
      this,
    );
  ...
  private helpBtnHandler() {
    ...
    let content: string = LangManager.Instance.GetTranslation(
      "consortia.view.myConsortia.chairmanElection.help",
    );
  ...
  private onClickItem() {
    ...
    let consortiaElectionItem: ConsortiaElectionItem =
      this.playerList.getChildAt(selecteIndex) as ConsortiaElectionItem;
    ...
    for (let i: number = 0; i < this.playerList.numItems; i++) {
      let item: ConsortiaElectionItem = this.playerList.getChildAt(
        i,
      ) as ConsortiaElectionItem;
      if (selecteIndex == i) {
        item.flag = true;
      } else {
        item.flag = false;
  ...
  renderListItem(index: number, item: ConsortiaElectionItem) {
    item.enable =
      this._consortiaModel.hasVotePermission == false ||
      this._consortiaModel.hasVoted == true
        ? false
        : true;
  ...
  private refreshView() {
    let dataList: Array<ConsortiaVotingUserInfo> =
      this._consortiaModel.votingUsers;
    if (!dataList) return;
  ...
  private setDescTxt() {
    ...
    if (endDate.getFullYear() == startDate.getFullYear()) {
      this.descTxt.text = LangManager.Instance.GetTranslation(
        "consortia.view.myConsortia.chairmanElection.note",
        endDate.getMonth() + 1,
        endDate.getDate(),
        endDate.getHours(),
      );
      ...
      this.descTxt.text = LangManager.Instance.GetTranslation(
        "consortia.view.myConsortia.chairmanElection.note2",
        endDate.getFullYear(),
        endDate.getMonth() + 1,
        endDate.getDate(),
        endDate.getHours(),
