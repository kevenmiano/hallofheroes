...
export default class IMManager extends GameEventDispatcher {
  ...
  constructor() {
    ...
    if (!this._noOperationSetLeave) {
      this._noOperationSetLeave = new NoOperationCallUtil(
        300000,
        this.setLeaveIMState.bind(this),
        this.setOnlineIMState.bind(this),
      );
    }
    this.addEvent();
  ...
  private addEvent() {
    ServerDataManager.listen(
      S2CProtocol.U_CH_PRIVATE_CHAT_REPLY,
      this,
      this.__imSendMsgResponseHandler,
    );
    ServerDataManager.listen(
      S2CProtocol.U_CH_PRIVATE_CHAT,
      this,
      this.__receiveIMMsgHandler,
    );
    ServerDataManager.listen(
      S2CProtocol.U_CH_EXCHANGE_STATE,
      this,
      this.__imStateChangeHandler,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_TRANSLATE,
      this,
      this._receiveTranslate,
    );
    this.playerManager.addEventListener(
      RequestInfoEvent.REQUEST_SIMPLEANDSNS_INFO,
      this.__requestFriendInfoHandler,
      this,
    );
    this.playerManager.addEventListener(
      NotificationEvent.CHAT_TRANSLATE_SETTING,
      this.__receiveTranslateSetting,
      this,
    );
  }
  ...
  private __imSendMsgResponseHandler(pkg: PackageIn) {
    ...
    let info: FriendItemCellInfo = this.friendManager.checkIsExistFriendInfo(
      imInfo.toId,
    );
    if (!info) {
      this.playerManager.sendRequestSimpleAndSnsInfo(
        imInfo.toId,
  ...
  private updateUnreadMsg(imInfo: BaseIMInfo) {
    ...
    SharedManager.Instance.privacyMsgCount =
      this.model.unreadMsgDic.getList().length;
   ...
   * 请求最近联系人信息返回
   */
  private __requestFriendInfoHandler(
    orientation: number,
    ...
    if (this.friendManager.friendList[thane.userId]) {
      thane.friendGp = (
        this.friendManager.friendList[thane.userId] as FriendItemCellInfo
      ).friendGp;
      thane.friendGrade = (
        this.friendManager.friendList[thane.userId] as FriendItemCellInfo
      ).friendGrade;
  ...
  public get showIMFrameNum(): number {
    ...
    for (const key in this.model.openIMFrameDic) {
      if (
        Object.prototype.hasOwnProperty.call(this.model.openIMFrameDic, key)
      ) {
  ...
  public saveMsgCache(msg: BaseIMInfo) {
    let id: number =
      this.playerManager.currentPlayerModel.playerInfo.userId == msg.userId
        ? msg.toId
        : msg.userId;
  ...
  addVoice(chatData: ChatData) {
    ...
    let info: FriendItemCellInfo =
      this.friendManager.checkIsExistFriendInfo(key);
    if (!info) {
      this.playerManager.sendRequestSimpleAndSnsInfo(
        key,
        RequestInfoRientation.RECENT_CONTACT,
   ...
   * @param curPage  当前页码
   */
  public getCurPageIMHistoryList(
    userId: number,
    curPage: number,
  ): Array<BaseIMInfo> {
    let historyList: Array<BaseIMInfo> = this.getIMHistoryList(userId);
    if (historyList) {
      let totalPages: number = Math.ceil(
        historyList.length / IMModel.MSGS_PER_PAGE,
      );
      if (curPage > 0 && curPage <= totalPages) {
        let endIdx: number =
          curPage * IMModel.MSGS_PER_PAGE > historyList.length
            ? historyList.length
            : curPage * IMModel.MSGS_PER_PAGE;
  ...
  private _receiveTranslate(pkg: PackageIn) {
    ...
    if (translateRespMsg.code != 200) {
      Logger.error(
        "聊天翻译异常,Code:",
        translateRespMsg.code,
        ", Msg:",
        translateRespMsg.errorMsg,
      );
    }
