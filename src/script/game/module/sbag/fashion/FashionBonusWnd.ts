...
export class FashionBonusWnd extends BaseWindow {
    ...
    private initView() {
        ...
        //当前等级数据
        let upgradeTemp = TempleteManager.Instance.getTemplateByTypeAndLevel(fashionMsg.grades, UpgradeType.UPGRADE_FASHION);
        this.cur.setTitle(LangManager.Instance.GetTranslation("fashioncomposetip.current"));
        ...
        //下一等级
        let nextTemp = TempleteManager.Instance.getTemplateByTypeAndLevel(fashionMsg.grades + 1, UpgradeType.UPGRADE_FASHION);
        let nextAttListData: string[] = []
        if (nextTemp) {
            this.next.setTitle(LangManager.Instance.GetTranslation("fashioncomposetip.next"));
            ...
            //最高级
            this.next.setTitle(LangManager.Instance.GetTranslation("fashioncomposetip.maxGrade"));
        }
