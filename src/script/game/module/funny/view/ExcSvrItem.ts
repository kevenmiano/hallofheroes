 ...
 * 全民兑换小宝箱
 */
export default class ExcSvrItem
  extends FUI_ExcSvrItem
  ...
  private onClickGet(): void {
    if (this._state == ExcSvrItem.DEFAULT) {
      MessageTipManager.Instance.show(
        LangManager.Instance.GetTranslation("allmainexchange.str10"),
      );
      return;
    }
    if (this._state == ExcSvrItem.CLOSE) {
      MessageTipManager.Instance.show(
        LangManager.Instance.GetTranslation("allmainexchange.str8"),
      );
      return;
    }
