...
export default class OutyardFigureMemItem extends FUI_OutyardFigureMemItem {
  ...
  constructor() {
    ...
    this._attackCostPoint =
      ConfigInfoManager.Instance.getStackHeadAttackActionPoint();
  }

  protected onConstruct() {
    super.onConstruct();
    this.pawnLeftTxt.text = LangManager.Instance.GetTranslation(
      "outyard.OutyardActorView.leftArmyTxt.text",
    );
    this.scoreTxt.text = LangManager.Instance.GetTranslation(
      "outyard.OutyardActorView.scoreTxt.text",
    );
    this.addEvent();
  }
  ...
  private attackBtnHandler() {
    ...
    if (
      ...
      if (OutyardManager.Instance.stateMsg.state != 3) {
        //不是开战中
        MessageTipManager.Instance.show(
          LangManager.Instance.GetTranslation(
            "outyard.OutyardActorView.AttackClick.text",
          ),
        );
        return;
      }
      ...
      let content: string = LangManager.Instance.GetTranslation(
        "outyardFigureMemItem.attack.tips",
        this._attackCostPoint,
      );
        ...
        (b: boolean) => {
          if (b) {
            ...
            if (selfMsg.actionPoint < this._attackCostPoint) {
              MessageTipManager.Instance.show(
                LangManager.Instance.GetTranslation(
                  "outyardFigureMemItem.attack.pointTips",
                ),
              );
              return;
            }
            OutyardManager.Instance.OperateOutyard(
              OutyardManager.ATTACK,
              this._info.pos,
            );
          }
        },
      ...
      MessageTipManager.Instance.show(
        LangManager.Instance.GetTranslation("TopToolBar.outyardTips"),
      );
      return;
    }
  }
