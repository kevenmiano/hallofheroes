...
import { EmOuterCityWarBuildSortType, EmOuterCityWarBuildSiteState, EmOuterCityWarCastlePeriodType } from "../../../../constant/OuterCityWarDefine";
    ...
    public refreshView() {
        if (this._info) {
            ...
            if (this._info.isAttackSite) {
                let color = this._info.isRepulsed ? ColorConstant.RED_COLOR : ColorConstant.LIGHT_TEXT_COLOR;
                if (this.fightModel.castleState == EmOuterCityWarCastlePeriodType.Fighting && this._info.attackGuildId <= 0) {
