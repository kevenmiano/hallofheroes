...
export default class SkillEditSelectWnd extends BaseWindow {
    ...
    OnShowWind(){
        ...
        for (let index = 0; index < array.length; index++) {
            ...
            let temp: t_s_skilltemplateData = TempleteManager.Instance.getSkillTemplateInfoById(id);
            if(temp && temp.UseWay == 1 && (this.defualtId.indexOf(temp.SonType) == -1)){
        ...
        if(ArmyManager.Instance.thane.grades >= OpenGrades.TALENT){
            let skillInfo:SkillInfo = ArmyManager.Instance.thane.talentData.allTalentList[10];
            var temp: t_s_skilltemplateData = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_skilltemplate, skillInfo.templateId.toString());
            this.listData3.push(temp);
            let skillInfo1:SkillInfo = ArmyManager.Instance.thane.talentData.allTalentList[20];
            var temp1: t_s_skilltemplateData = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_skilltemplate, skillInfo1.templateId.toString());
            this.listData3.push(temp1);
            let skillInfo2:SkillInfo = ArmyManager.Instance.thane.talentData.allTalentList[30];
            var temp2: t_s_skilltemplateData = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_skilltemplate, skillInfo2.templateId.toString());
    ...
    initLanguage(){
        this.frame.getChild('title').text = LangManager.Instance.GetTranslation('PetChallengeSkillSelectWnd.titleTxt');
        this.txt1.text = LangManager.Instance.GetTranslation('FightSkillEdit.txt8');
        this.txt2.text = LangManager.Instance.GetTranslation('bag.datas.GoodsSonType.RUNNES');
    ...
    private addEvent(){
        ...
        this.list1.itemRenderer = Laya.Handler.create(this, this.onRenderListItem1, null, false);
        this.list2.itemRenderer = Laya.Handler.create(this, this.onRenderListItem2, null, false);
        this.list3.itemRenderer = Laya.Handler.create(this, this.onRenderListItem3, null, false);
    ...
    onClickList2(item:EditSkillItem){
        ...
        if(SkillEditModel.instance.curRuneNum >=2 && !this.isClickRune){
            MessageTipManager.Instance.show(LangManager.Instance.GetTranslation('FightSkillEdit.txt22'));
            item.selected = false;
            return
        }
        this.isRune = true;
    ...
    onSure(){
        if(this._curSkill == ''){
            MessageTipManager.Instance.show(LangManager.Instance.GetTranslation('chat.view.ChatView.NullMap'))
            return;
        }
        if(this.isRune){
            SkillEditModel.instance.curRuneNum++;
        }
        NotificationManager.Instance.dispatchEvent(SkillEvent.CHANGE_SKILL,this._curSkill);
        this.hide();
    }
