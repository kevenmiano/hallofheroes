...
export default class SpaceNpcAttackHelper {
  ...
  public static getAttackPoint(
    ...
    for (var i: number = attack; i >= 0; i) {
      ...
      var b: boolean = SpaceManager.Instance.model.getWalkable(
        Number(tar.x / 20),
        Number(tar.y / 20),
      );
      if (b) {
  ...
  private static getAttackPointImp(
    info: any,
    attackPoint: Laya.Point,
  ): Laya.Point {
    if (
      info instanceof SpaceNode &&
      (<SpaceNode>info).param1 != SpaceNodeType.PARAM_TRANSFER
    ) {
      ...
      if (attack > 0) {
        while (!isWalk && count < 20) {
          ...
          var b: boolean = SpaceManager.Instance.model.getWalkable(
            Number(attackPoint.x / 20) + offX,
            Number(attackPoint.y / 20) + offY,
          );
          if (b) {
