...
export default class PoliticsFrameWnd extends BaseWindow {
	...
	public OnInitWind() {
		...
		let configInfo: t_s_configData = TempleteManager.Instance.getConfigInfoByConfigName("Inner_Levy");
		...
		this.frame.getChild("title").text = LangManager.Instance.GetTranslation("PoliticsFrameWnd.title");
		this.politicsTitle.text = LangManager.Instance.GetTranslation("PoliticsFrameWnd.title");
		this.n5Txt.text = LangManager.Instance.GetTranslation("ResidenceFrameWnd.n5Txt");
		this.n6Txt.text = LangManager.Instance.GetTranslation("CasernWnd.needResourceTxt");
		this.n7Txt.text = LangManager.Instance.GetTranslation("CasernWnd.needTimeTxt");
		this.n13Txt.text = LangManager.Instance.GetTranslation("PoliticsFrameWnd.n13Txt");
		this.n14Txt.text = LangManager.Instance.GetTranslation("PoliticsFrameWnd.n14Txt");
		this.n15Txt.text = LangManager.Instance.GetTranslation("PoliticsFrameWnd.n15Txt");
		this.helpBtn.visible = false;
		let grade = VIPManager.Instance.model.vipInfo.VipGrade;
		if (VIPManager.Instance.model.isOpenPrivilege(VipPrivilegeType.COLD_QUEUE, grade)) {
		...
		let isOpenVip = VIPManager.Instance.model.isOpenPrivilege(VipPrivilegeType.BUILD_COOLDOWN, grade);
		...
		let cfgItemOrder = TempleteManager.Instance.getConfigInfoByConfigName("QuickenOrder_Price");
        if (cfgItemOrder) {
            //冷却
	...
	private addEvent() {
		...
		BuildingManager.Instance.addEventListener(BuildingEvent.BUILDING_UPGRADE_RECEIVE, this.__upgradeBuildingReceive, this);
		this.BtnLevy.onClick(this, this.__levyHandler.bind(this));
		this.BtnLevy_ex.onClick(this, this.__levyExHandler.bind(this));
		BuildingManager.Instance.addEventListener(BuildOrderEvent.COLL_ONE_ORDER, this.__orderChangeHandler, this);
		BuildingManager.Instance.addEventListener(BuildingEvent.U_C_GOLD_IMPOSERESULT, this.__changeHandler, this);
		if (this.order) {
			this.order.addEventListener(Laya.Event.COMPLETE, this.__completeHandler, this);
	...
	private removeEvent() {
		...
		BuildingManager.Instance.removeEventListener(BuildingEvent.BUILDING_UPGRADE_RECEIVE, this.__upgradeBuildingReceive, this);
		this.BtnLevy.offClick(this, this.__levyHandler.bind(this));
		this.BtnLevy_ex.offClick(this, this.__levyExHandler.bind(this));
		BuildingManager.Instance.removeEventListener(BuildOrderEvent.COLL_ONE_ORDER, this.__orderChangeHandler, this);
		BuildingManager.Instance.removeEventListener(BuildingEvent.U_C_GOLD_IMPOSERESULT, this.__changeHandler, this);
		if (this.order) {
			this.order.removeEventListener(Laya.Event.COMPLETE, this.__completeHandler, this);
	...
	private initQuestEvent() {
		let buildOrderList: Array<BuildingOrderInfo> = BuildingManager.Instance.model.buildOrderList;
		for (let i: number = 0; i < buildOrderList.length; i++) {
			if (i == 0) {
				this._vData1 = buildOrderList[i];
				this._vData1.addEventListener(Laya.Event.COMPLETE, this.buildOrderCompleteHandler, this);
				this._vData1.addEventListener(Laya.Event.CHANGE, this.__changeHandler1, this);
				this.__changeHandler1();
			} else if (i == 1) {
				this._vData2 = buildOrderList[i];
				this._vData2.addEventListener(Laya.Event.COMPLETE, this.buildOrderCompleteHandler, this);
				this._vData2.addEventListener(Laya.Event.CHANGE, this.__changeHandler2, this);
	...
	private removeQuestEvent() {
		...
		for (let i: number = 0; i < buildOrderList.length; i++) {
			if (i == 0) {
				this._vData1 = buildOrderList[i];
				this._vData1.removeEventListener(Laya.Event.COMPLETE, this.buildOrderCompleteHandler, this);
				this._vData1.removeEventListener(Laya.Event.CHANGE, this.__changeHandler1, this);
			} else if (i == 1) {
				this._vData2 = buildOrderList[i];
				this._vData2.removeEventListener(Laya.Event.COMPLETE, this.buildOrderCompleteHandler, this);
				this._vData2.removeEventListener(Laya.Event.CHANGE, this.__changeHandler2, this);
			}
		}
	}
	...
	protected updateBuilderOrderTxt(vData: BuildingOrderInfo, tyep: number = 1) {
		var txt1: string = LangManager.Instance.GetTranslation("mainBar.view.QueueItem.BUILD");
		var txt2: string = LangManager.Instance.GetTranslation("mainBar.view.QueueItem.BUILD2");
		if (tyep == 1) {
			if (vData && parseInt(vData.remainTime.toString()) <= 0) {
				...
				this.timeDescTxt1.text = txt2 + ":" + DateFormatter.getCountDate(vData.remainTime);
			...
			if (vData && parseInt(vData.remainTime.toString()) <= 0) {
				...
				this.timeDescTxt2.text = txt2 + ":" + DateFormatter.getCountDate(vData.remainTime);
	...
	private showAlertForLevyEx() {
		...
		let content: string = LangManager.Instance.GetTranslation("fashion.view.compose.notEnoughLuckyCharm3", this.needPoint, leftTimes);
	...
	private alertCallback(notAlert: boolean, useBind: boolean) {
		let hasMoney = PlayerManager.Instance.currentPlayerModel.playerInfo.allPoint;
	...
	private __levyExHandler() {
		...
		let leftTimes: number = maxLevy - info.payImpose;
		if (leftTimes <= 0) {
			MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("fashion.view.compose.notEnoughLuckyCharm4"));
			return;
		}
	...
	private refrehBottom() {
		var info: BuildInfo = BuildingManager.Instance.getBuildingInfoBySonType(BuildingType.OFFICEAFFAIRS);
		if (info && this.order) {
			if (info.property2 - info.property1 <= 0) {
				this.ex.selectedIndex = 1;
				this.LevyValueTxt_ex.text = this._inner_Levy[info.payImpose];
				this.n13Txt.text = LangManager.Instance.GetTranslation("PoliticsFrameWnd.n13Txt1");
				this.LevyValueTxt.text = this._inner_Levy.length - info.payImpose + " / " + this._inner_Levy.length;
				if (this._inner_Levy.length - info.payImpose < 1) this.tipItem_ex.visible = false;
				if (BuildingManager.Instance.levyExUseBind && !this.todayNeedAlertForLevyEx) {
				...
				this.n13Txt.text = LangManager.Instance.GetTranslation("PoliticsFrameWnd.n13Txt");
				this.LevyValueTxt.text = info.property2 - info.property1 + " / " + info.property2;
			}
			this.BtnLevy.enabled = info.property2 - info.property1 > 0 && this.order.remainTime <= 0 && this.thane.grades >= 14;
			...
			this.n15Txt.visible = this.ReduceTimeValueTxt.visible = this.Btn_Reduce.visible = false;
		}
		var gold: number = info.templeteInfo.Property1 * 6;
		var crystalBuildingInfo: BuildInfo = BuildingManager.Instance.getBuildingInfoBySonType(BuildingType.CRYSTALFURNACE);
		if (crystalBuildingInfo) gold += crystalBuildingInfo.templeteInfo.Property1 * 3;
		if (this.thane.grades >= 14) this.LevyGoldValueTxt.text = gold.toString();
		else this.LevyGoldValueTxt.text = "0";
	}

	private updateView() {
		var txt1: string = LangManager.Instance.GetTranslation("buildings.politics.view.update");
		var txt2: string = LangManager.Instance.GetTranslation("buildings.politics.view.update2");
	...
	protected updateTxt(str1: string, str2: string) {
		if (this.order && parseInt(this.order.remainTime.toString()) <= 0) {
			...
			if (this.order) {
				this.ReduceTimeValueTxt.text = DateFormatter.getCountDate(this.order.remainTime);
				this.Btn_Reduce.visible = true;
				...
				this.n15Txt.text = "";
				this.ReduceTimeValueTxt.color = "#00F0FF";
				Logger.error("还没有冷却队列, 账号数据问题 LoginInitDataAccept __loadBuildOrderHandler 不存在orderType==3的情况");
			}
		}
	}
	...
	protected __upgradeBuildingReceive(info: BuildInfo) {
		...
		if (btemp.MasterType == MasterTypes.MT_INTERNALTECHNOLOGY || btemp.MasterType == MasterTypes.MT_WARTECHNOLOGY) {
			return;
		}
	}
	...
	private refreshView() {
		...
		this.LvTxt.text = LangManager.Instance.GetTranslation("public.level2", this._data.templeteInfo.BuildingGrade);
		var currentTemp: t_s_buildingtemplateData = this._data.templeteInfo;
		this.CurrentValueTxt.text = currentTemp.ActivityLang;
		if (nextTemp) {
			this.ResourceValueTxt.text = this._data.getNextNeedResource(nextTemp)["GoldConsume"].toString();
			this.tipItem4.visible = true;
			var time: number = parseInt(this.playerEffect.getBuildTimeAdditionValue(nextTemp.UpgradeTime).toString());
			let grade = VIPManager.Instance.model.vipInfo.VipGrade;
			let param1 = VIPManager.Instance.model.getPrivilegeParam1ByGrade(VipPrivilegeType.BUILD_COOLDOWN, grade);
			...
			this.NextValueTxt.text = LangManager.Instance.GetTranslation("buildings.BaseBuildFrame.maxGrade");
