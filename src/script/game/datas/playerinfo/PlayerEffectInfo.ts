export class PlayerEffectInfo {
  ...
  public getBuildTimeAdditionValue(value: number): number {
    return (
      value * (1 + this._buildTimeAdditionRace / 100) + this._buildTimeAddition
    );
  }
  ...
  public getPawnUpgradeSoulAddition(value: number): number {
    return (
      value * (1 + this._pawnUpgradeSoulAdditionRace / 100) +
      this._pawnUpgradeSoulAddition
    );
  }
  ...
  public getRecruitPawnResourceAddition(value: number): number {
    return (
      value * (1 + this._recruitPawnResourceAdditionRace / 100) +
      this._recruitPawnResourceAddition
    );
  }
  ...
  public getTransferPowerLimitAddition(value: number): number {
    return (
      value * (1 + this._transferPowerLimitAdditionRace / 100) +
      this._transferPowerLimitAddition
    );
  }
  ...
  public getTransferCoolTimeAddition(value: number): number {
    return (
      value * (1 + this._transferCoolTimeAdditionRace / 100) +
      this._transferCoolTimeAddition
    );
  }
}
