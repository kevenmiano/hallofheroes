...
export class BattleResPreloadManager {
	...
	public static startLoadRes() {
		Logger.info("[BattleResPreloadManager]开始预加载副本资源", this._resLoadPaths)
	...
	public static startLoadPublicRes() {
		this._resPublicLoadPaths = this._resPublicLoadPaths.concat(ResourceModel.PublicSkill, ResourceModel.Resist_Sheld, SkillResourceLoader.skillNames);
	...
	public static cachePublicSkillAni(cacheName: string) {
		...
		if (exist) {
			...
			let success
			if (!cacheJson) {
				success = AnimationManager.Instance.createAnimation(cacheName, "", undefined, "", AnimationManager.BattleEffectFormatLen)
	...
	public static getPaths(pawnIds: any[], bossIds: any[]): any[] {
		...
		if (bossIds && bossIds.length > 0) {
			for (i = 0; i < bossIds.length; i++) {
				heroTempInfo = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_herotemplate, bossIds[i])
				if (heroTempInfo) {
					heroAisTemp = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_heroai, heroTempInfo.AI)
		...
		skillIds = skillIds.concat(SkillResourceLoader.getHeroFullSkillIds(heroDefSkillIds));

		let pawnTemp: t_s_pawntemplateData;
		if (pawnIds && pawnIds.length > 0) {
			for (i = 0; i < pawnIds.length; i++) {//将兵的技能放入技能数组.
				pawnSkillIds = pawnSkillIds.concat(SkillResourceLoader.getPawnSkillIds(pawnIds[i]));
				...
				if (pawnTemp) {
					let heroAisTemp: t_s_heroaiData = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_heroai, pawnTemp.AI)
					if (heroAisTemp) {
						skillIds = skillIds.concat(heroAisTemp.getSkills())
					}
		...
		let selfHeroInfo: t_s_herotemplateData = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_herotemplate, thaneInfo.templateId);
		let selfSkillIds: any[] = ArmyManager.Instance.thane.skillCate.skillScript.split(",")
		...
		rolePaths.push(HeroLoadDataFactory.create(thaneInfo, HeroLoadDataFactory.PART_ARMS_BACK).urlPath)
		rolePaths.push(HeroLoadDataFactory.create(thaneInfo, HeroLoadDataFactory.PART_HAIR4).urlPath)
		rolePaths.push(HeroLoadDataFactory.create(thaneInfo, HeroLoadDataFactory.PART_BODY).urlPath)
		rolePaths.push(HeroLoadDataFactory.create(thaneInfo, HeroLoadDataFactory.PART_CLOAK2).urlPath)
	...
	public static loadBossIconById(id: number) {
		...
		let bossTemp: t_s_herotemplateData = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_herotemplate, id);
		if (bossTemp) {
