...
export class SkillCategory extends GameEventDispatcher {
    ...
    public initAllSkill(job: number) {
        this._job = job;
        this.allSkillTemp = TempleteManager.Instance.getAllSkillTemplateInfoByJob(this._job);
        let dic: SimpleDictionary = TempleteManager.Instance.getMinSkillTemplateInfoByJob(this._job);
    ...
    public initExtraJobSkill() {
        ...
        this.allExtrajobSkillTemp = TempleteManager.Instance.getAllExtrajobSkillTemplateInfoByJob(this._job);
        let allExtrajobSkillMinTemp = TempleteManager.Instance.getExtrajobMinSkillTemplateInfoByJob(this._job);
        let skillInfo: SkillInfo = null;
        let tempId: number = 0;
        let minSkillTempList = allExtrajobSkillMinTemp.getList() as t_s_skilltemplateData[];
        for (let minSkillTemp of minSkillTempList) {
            ...
            if (skillInfo.templateId)
                this.allExtrajobSkillList.add(skillInfo.templateInfo.MasterType + '_'+skillInfo.templateInfo.Index, skillInfo);
        }
    }
    /**
    ...
    getExtrajobSkill(jobtype:number):SkillInfo[]{
        ...
        for (const key in this.allExtrajobSkillList) {
            ...
            if (skillInfo && skillInfo.templateInfo&& skillInfo.templateInfo.MasterType==jobtype) {
            ...
            if (skillInfo && skillInfo.templateInfo) {
                ...
                if(jobLevel > 0){
                    if(skillInfo.grade == 0){
                        ...
                        let canUpgrade = jobLevel >= skillInfo.nextTemplateInfo.NeedPlayerGrade;
                        let isgoldEnough = ResourceManager.Instance.gold.count >= skillInfo.templateInfo.Parameter3;
