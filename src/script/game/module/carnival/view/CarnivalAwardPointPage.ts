 ...
 * 嘉年华---积分领奖
 */
export default class CarnivalAwardPointPage
  extends FUI_CarnivalAwardPointPage
  implements CarnivalBasePage
{
  ...
  private refreshView() {
    ...
    if (nextInfo == null) {
      ...
      this.txt_describe.text = LangManager.Instance.GetTranslation(
        "carnival.awardpoint.describe",
        nextInfo.Target - currScore,
      );
    }
  ...
  private scrollListChange() {
    ...
    let i: number = 0;
    let tempData = [];
    let lastItem: CarnivalAwardPointPageItem = this.list.getChildAt(
      itemCount - 1,
    ) as CarnivalAwardPointPageItem;
  ...
  private setScore() {
    ...
    this.carnival_point.txt_title.text = LangManager.Instance.GetTranslation(
