...
export default class BattleSettingWnd extends BaseWindow {
  ...
  OnShowWind() {
    ...
    this.settingItem1.getChild("txt_name").asTextField.text =
      LangManager.Instance.GetTranslation("SettingWnd.settingData4");
    let btn_switch1: fairygui.GButton =
      this.settingItem1.getChild("btn_switch").asButton;
    btn_switch1.selected = SharedManager.Instance.allowAttactedEffect;
    btn_switch1.onClick(this, this.onSelectItem, [
      btn_switch1,
      SettingType.HIDE_FIGHTING_OBJECT,
    ]);

    this.settingItem2.getChild("txt_name").asTextField.text =
      LangManager.Instance.GetTranslation("GameSetCom.hightFPSTxt");
    let btn_switch2: fairygui.GButton =
      this.settingItem2.getChild("btn_switch").asButton;
    btn_switch2.selected = SharedManager.Instance.openHighFrame;
    btn_switch2.onClick(this, this.onSelectItem, [
      btn_switch2,
      SettingType.HIGH_FRAME,
    ]);
  }
  ...
  private onSelectItem(target: any, settingType: SettingType) {
    ...
    switch (settingType) {
      case SettingType.HIGH_FRAME:
        ...
        Laya.stage.frameRate = targetState
  ...
  private setBtnWithdrawState() {
    if (ArmyManager.Instance.thane.grades < OpenGrades.BATTLE_WITHDRAW)
      return (this.btnWithdraw.visible = false);
    ...
    this.btnWithdraw.enabled =
      !battleUIView.withdrawHandler.showWithdrawVote &&
      battleModel.isEnableWithdraw;
  }
