...
export default class MonopolyFinishColumnView extends FUI_MonopolyFinishColumnView{
    ...
    onConstruct() {
        super.onConstruct();
        this._itemState1 = new SinglePassCardStateInfo(0, 	0,  295, 0.8, SinglePassCardStateInfo.LEFT);
        this._itemState2 = new SinglePassCardStateInfo(0.3, 	0,  295, 0.8, SinglePassCardStateInfo.LEFT);
        this._itemState3 = new SinglePassCardStateInfo(1, 	0, 	 147, 1.0, SinglePassCardStateInfo.CENTER);
        this._itemState4 = new SinglePassCardStateInfo(0.3, 	0,   0, 0.7, SinglePassCardStateInfo.RIGHT);
        this._itemState5 = new SinglePassCardStateInfo(0, 	0,   0, 0.7, SinglePassCardStateInfo.RIGHT);
    ...
    private setItemState(itemView:MonopolyFinishRewardItem, itemState:SinglePassCardStateInfo):void
    {
    ...
    public start(callBack:Function = null, rewardIndex:number = 0, blurCount:number = 4):void
    {
        this._callBack = callBack;
        this._rewardIndex = rewardIndex;
        this._blurCount = blurCount;
        this._infoIndex = this._rewardIndex - (this._blurCount + this._delayCount) % this._infos.length;
        // this._blurIndex = 0;
        this._turnIndex = 0;
        // this._playing = true;
    ...
    private tweenCard(itemView:MonopolyFinishRewardItem, stateInfo:SinglePassCardStateInfo, delayTime:number):void
    {
        ...
        Laya.Tween.to(itemView,{y: stateInfo.y},delayTime,Laya.Ease.sineOut,Laya.Handler.create(this,this.tweenCardCompleteHandler,[itemView]))
    }
    ...
    private tweenCardCompleteHandler(itemView):void
    {
        ...
        if(this._turnCompleteCount >= this._turnCompleteMax)
        {
            ...
            }else if(this._turnIndex < this._blurCount + this._delayCount)
            {
                ...
                else if(this._turnIndex < this._blurCount + this._mediumCount + this._lowCount)
                {
                ...
                let _delayTime:number = this._delayTimes[this._turnIndex - this._blurCount]
                this.initInfo();
                this.initState();
