...
import { NotificationEvent, SpaceEvent } from "../constant/event/NotificationEvent";
...
export class SwitchPageHelp {
	...
	public static skipPageByTaskCondictionType(data: any, func: Function, target: any) {
		let str: string = LangManager.Instance.GetTranslation("yishi.SwitchPageHelp.taskNotFinish");
		if (data instanceof TaskTemplate) {
			...
			if (conTask[0]['Para4'] != "0" && conTask[0].CondictionType != 96) {//多人副本协助玩家杀怪任务96特殊处理
			...
			switch (conTask[0].CondictionType) {
				...
				case 32://收集任务
					if (SwitchPageHelp.checkCollectCampaign)
						SwitchPageHelp.walkNext(SwitchPageHelp.getCollectNodeIdByGoodId(Number(conTask[0].Para1)));
					else
						SwitchPageHelp.gotoCollectScene(data);
					break;
				case 34://阵型调整
			...
			switch (conOffer[0].CondictionType) {
				...
				case 4://采集
					if (SwitchPageHelp.checkCollectCampaign)
						SwitchPageHelp.walkNext(SwitchPageHelp.getCollectNodeIdByGoodId(conOffer[0].Para1));
					else
						SwitchPageHelp.gotoCollectScene(data);
					break;
				case 5://神树充能
			...
			switch (conRing[0].CondictionType) {
				...
				case RewardConditionType.COLLECTION:  //采集
					NotificationManager.Instance.dispatchEvent(NotificationEvent.DISPOSEVIEW);
				...
				case RewardConditionType.RESIDENTTASK:
					if (MopupManager.Instance.model.isMopup) {
						let str2: string = LangManager.Instance.GetTranslation("mopup.MopupManager.mopupTipData01");
	...
	private static challengeSelf(tid: number) {
		...
		switch (curScene) {
			...
			case SceneType.PVP_ROOM_SCENE:
				let str: string = LangManager.Instance.GetTranslation("task.TaskFrameII.command01");
				MessageTipManager.Instance.show(str);
				return;
	...
	private static goArrest(conditionList: t_s_rewardcondictionData[]) {
		...
		switch (curScene) {
			...
			case SceneType.PVE_ROOM_SCENE:
			case SceneType.PVP_ROOM_SCENE:
				let str: string = LangManager.Instance.GetTranslation("task.TaskFrameII.command02");
				MessageTipManager.Instance.show(str);
				return;
			case SceneType.SPACE_SCENE:
				BattleManager.preScene = SceneType.SPACE_SCENE;
		...
		if (conditionList) {
			OfferRewardManager.Instance.sendRewardArrest(conditionList[0].CondictionType, conditionList[0].Para1);
		}
	}
	// 		/**
	// 		 * 打开QTE
	...
	private static get checkCollectCampaign(): boolean {
		if (SwitchPageHelp.mapModel && WorldBossHelper.checkCrystal(SwitchPageHelp.mapModel.mapId))
			return true;
		return false;
	}
	/**
	...
	public static gotoCollectScene(info: any) {
		...
		let campaignTemp: t_s_campaignData = ConfigMgr.Instance.getTemplateByID(ConfigType.t_s_campaign, info.NeedFightId);
	...
	public static gotoSingleCampaign() {
		...
		if (ArmyManager.Instance.thane.grades < OpenGrades.PVE_CAMPAIGN) {
			str = LangManager.Instance.GetTranslation("dayGuide.DayGuideManager.command02", OpenGrades.PVE_CAMPAIGN);
			MessageTipManager.Instance.show(str);
			return
		}
		if (!SwitchPageHelp.checkScene()) return;
	...
	private static walkNext(nodeId: number = 0) {
		...
		if (nodeId == 0) {
			if (SceneManager.Instance.currentType == SceneType.OUTER_CITY_SCENE) {
				let str: string = LangManager.Instance.GetTranslation("emailII.view.ReadBattleReportView.command01");
				MessageTipManager.Instance.show(str);
				return;
			}
			NotificationManager.Instance.dispatchEvent(NotificationEvent.WORK_NEXT_NODE, null);
		} else {
			if (!SwitchPageHelp.mapModel) return;
			let targetNode: CampaignNode = SwitchPageHelp.mapModel.getMapNodeByNodeId(nodeId);
			...
			let targetView: DisplayObject = mapView.getNpcNodeById(targetNode.info.id);
			CampaignManager.Instance.controller.moveArmyByPos(targetView.x, targetView.y, false, true);
		}
	}
	 ...
	 *
	 */
	public static walkToCrossMapTarget(target: string, position: Laya.Point = null) {
		...
		for (let i: number = 0; i < mapArr.length; i++) {
			...
			path.push({ mapId: parseInt(mapPoint[0].toString()), nodeId: parseInt(mapPoint[1].toString()) });
		}
		...
		if (currentScene == SceneType.CASTLE_SCENE || currentScene == SceneType.FARM || currentScene == SceneType.OUTER_CITY_SCENE) {
			needSwitch = true;
			fromMapId = 10000;
		}
		else if (currentScene == SceneType.SPACE_SCENE) {//在天空之城
			fromMapId = 10000;
		}
		...
		if (fromMapId != targetNode.mapId) {
			let p: any[] = CrossMapSearchActionQueue.Instance.searchPath(fromMapId, targetNode.mapId);
			if (!p) {
				MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("task.TaskFrameII.command01"));
				return;
			}
			for (const key in p) {
		...
		for (let j: number = 0; j < path.length; j++) {
			let crossMapSearchAction = Laya.ClassUtils.getClass('CrossMapSearchAction');
	...
	public static gotoCampaignById(campaignId: number) {
		let str: string = "";
		if (MopupManager.Instance.model.isMopup)//扫荡中。返回
		{
			str = LangManager.Instance.GetTranslation("mopup.MopupManager.mopupTipData01");
			MessageTipManager.Instance.show(str);
			return;
		...
		if (currentTem == null)//无此副本信息
		{
			return;
		}
		if (currentTem.Capacity == 1 && SwitchPageHelp.mapModel && SwitchPageHelp.mapModel.campaignTemplate &&
			SwitchPageHelp.mapModel.campaignTemplate.CampaignId == campaignId)//处于当前单人本, 走到下一节点
		{
			SwitchPageHelp.walkNext();
			return;
		}
		if (!SwitchPageHelp.checkScene()) return;//处于其它副本地图中,返回
		if (currentTem.Capacity > 1)// 多人本
		{
			SwitchPageHelp.goSpaceAndFind(SpaceNodeType.ID_PVE_ROOMLIST);
		} else if (currentTem.Capacity == 1)//单人本
		{
			if (ArmyManager.Instance.thane.grades < currentTem.MinLevel) {
				MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("yishi.SwitchPageHelp.gotoCampaignById.levelLimit"));
				return;
			}
			if (WorldBossHelper.checkMaze2(currentTem.CampaignId)) {
				//切换到内城
				if (SceneManager.Instance.currentType == SceneType.SPACE_SCENE) {
					SceneManager.Instance.setScene(SceneType.CASTLE_SCENE, { isOpenPetMaze2: true });
				} else if (SceneManager.Instance.currentType == SceneType.CASTLE_SCENE) {
				...
				FrameCtrlManager.Instance.open(EmWindow.PveCampaignWnd, { "campaignData": currentTem });
			}
		}
	}
	...
	public static gotoUpgradePawnFrame(pawnMasterType: number) {
		...
		let army: ArmyPawn = ArmyManager.Instance.getCasernPawnByMastertype(pawnMasterType);
	...
	public static gotoConsortiaFrame(type: number = 1) {
		...
		if (ArmyManager.Instance.thane.grades < OpenGrades.CONSORTIA) {
			str = LangManager.Instance.GetTranslation("dayGuide.DayGuideManager.command02", OpenGrades.CONSORTIA);
			MessageTipManager.Instance.show(str);
			return;
		}
		if (type == 1) {
	...
	public static gotoStarFrame()//跳转到占星
	{
		...
		if (ArmyManager.Instance.thane.grades < OpenGrades.STAR) {
			str = LangManager.Instance.GetTranslation("dayGuide.DayGuideManager.command02", OpenGrades.STAR);
			MessageTipManager.Instance.show(str);
			return;
		}
	 ...
	 *
	 */
	public static gotoShopFrame(pageIndex: number = 0)//跳转到商城
	{
		// AudioManager.Instance.playSound(SoundIds.MAINTOOLBAR_CLICK_SOUND);
		if (ArmyManager.Instance.thane.grades < OpenGrades.SHOP) {
			let str: string = LangManager.Instance.GetTranslation("dayGuide.DayGuideManager.command02", OpenGrades.SHOP);
			MessageTipManager.Instance.show(str);
			return;
		}
	...
	public static gotoWorldBossFrame() {
		...
		if (ArmyManager.Instance.thane.grades < OpenGrades.WORLD_BOSS) {
			str = LangManager.Instance.GetTranslation("dayGuide.DayGuideManager.command02", OpenGrades.WORLD_BOSS);
			MessageTipManager.Instance.show(str);
			return;
		}
		if (!this.checkScene()) return;
		if (!this.playerInfo.worldbossState) {
			str = LangManager.Instance.GetTranslation("YiShi.SwitchPageHelp.WorldBoss");
			MessageTipManager.Instance.show(str);
			return;
		}
	...
	public static gotoWorldFightFrame() {
		...
		if (ArmyManager.Instance.thane.grades < OpenGrades.RVR) {
			str = LangManager.Instance.GetTranslation("dayGuide.DayGuideManager.command02", OpenGrades.RVR);
			MessageTipManager.Instance.show(str);
			return;
		}
		if (!this.checkScene()) return;
		if (!this.worldFightState) {
			str = LangManager.Instance.GetTranslation("YiShi.SwitchPageHelp.warFight");
			MessageTipManager.Instance.show(str);
			return;
		}
	...
	private static gotoDayGuideFrame()//跳转活跃度
	{
		...
		if (ArmyManager.Instance.thane.grades < OpenGrades.WELFARE) {
			str = LangManager.Instance.GetTranslation("mainBar.TopToolsBar.dayGuideBtn.underLevel");
			MessageTipManager.Instance.show(str);
		} else {
			FrameCtrlManager.Instance.open(EmWindow.Welfare, { str: LangManager.Instance.GetTranslation('welfareWnd.tabTitle.DegreeActivity') });
		}
	}
	/**
	 * 打开铁匠铺相应界面
	...
	public static gotoStoreFrame(tabIndex: number, materialId?: number) {
		...
		switch (tabIndex) {
			case GTabIndex.Forge_QH:
				if (ArmyManager.Instance.thane.grades < OpenGrades.INTENSIFY) {
					str = LangManager.Instance.GetTranslation("dayGuide.DayGuideManager.command02", OpenGrades.INTENSIFY);
					MessageTipManager.Instance.show(str);
					return;
				}
				break;
			...
			case GTabIndex.Forge_HC_SJ:
				if (ArmyManager.Instance.thane.grades < OpenGrades.INSERT) {
					str = LangManager.Instance.GetTranslation("dayGuide.DayGuideManager.command02", OpenGrades.INSERT);
					MessageTipManager.Instance.show(str);
					return;
				}
				break;
		...
		FrameCtrlManager.Instance.open(EmWindow.Forge, { tabIndex: tabIndex, materialId: materialId })
	}
	/**
	 * 打开悬赏
	 *
	...
	public static gotoOfferRewardFrame() {
		...
		if (ArmyManager.Instance.thane.grades < OpenGrades.OFFER_REWARD) {
			str = LangManager.Instance.GetTranslation("dayGuide.DayGuideManager.command02", OpenGrades.OFFER_REWARD);
			MessageTipManager.Instance.show(str);
			return;
		}
		if (SceneManager.Instance.currentType == SceneType.OUTER_CITY_SCENE) {
	...
	public static gotoMarkeFrame() {
		...
		if (ArmyManager.Instance.thane.grades < OpenGrades.MYSTERIOUS) {
			str = LangManager.Instance.GetTranslation("dayGuide.DayGuideManager.command02", OpenGrades.MYSTERIOUS);
			MessageTipManager.Instance.show(str);
			return;
		}
		if (SceneManager.Instance.currentType == SceneType.OUTER_CITY_SCENE) {
	...
	private static gotoOfficeFrame() {
		...
		let bInfo: BuildInfo = BuildingManager.Instance.getBuildingInfoBySonType(BuildingType.OFFICEAFFAIRS);
	...
	public static gotoCasernFrame() {
		...
		if (ArmyManager.Instance.thane.grades < OpenGrades.ARMY) {
			str = LangManager.Instance.GetTranslation("dayGuide.DayGuideManager.command02", OpenGrades.ARMY);
			MessageTipManager.Instance.show(str);
			return;
		}
		let bInfo: BuildInfo = BuildingManager.Instance.getBuildingInfoBySonType(BuildingType.CASERN);
		SwitchPageHelp.openBuildFrame(bInfo);
	}
	/**
	 * 打开神学院
	...
	public static gotoSeminaryFrame() {
		...
		if (ArmyManager.Instance.thane.grades < OpenGrades.SEMINARY) {
			str = LangManager.Instance.GetTranslation("dayGuide.DayGuideManager.command02", OpenGrades.SEMINARY);
			MessageTipManager.Instance.show(str);
			return;
		}
		if (!SwitchPageHelp.checkScene()) return;
	...
	public static gotoMazeFrame() {
		...
		if (ArmyManager.Instance.thane.grades < 23) {
			str = LangManager.Instance.GetTranslation("dayGuide.DayGuideManager.command02", 23);
			MessageTipManager.Instance.show(str);
			return;
		}
		if (!SwitchPageHelp.checkScene()) return;
	...
	public static gotoColosseumFrame() {
		...
		if (ArmyManager.Instance.thane.grades < OpenGrades.PVP_COLOSSEUM) {
			str = LangManager.Instance.GetTranslation("dayGuide.DayGuideManager.command02", OpenGrades.PVP_COLOSSEUM);
			MessageTipManager.Instance.show(str);
			return;
		}
		if (SceneManager.Instance.currentType == SceneType.OUTER_CITY_SCENE) {
	...
	public static gotoPvpFrame() {
		...
		if (ArmyManager.Instance.thane.grades < OpenGrades.CHALLENGE) {
			str = LangManager.Instance.GetTranslation("dayGuide.DayGuideManager.command02", OpenGrades.CHALLENGE);
			MessageTipManager.Instance.show(str);
			return;
		}
		if (SceneManager.Instance.currentType == SceneType.OUTER_CITY_SCENE) {
	...
	private static gotoBuildingFrame(sonType: number) {
		...
		// if (!SwitchPageHelp.checkScene()) return;
		let bInfo: BuildInfo;
		if ((sonType >= 1300 && sonType < 1500) || (sonType >= 2304 && sonType < 2446)) {
			bInfo = BuildingManager.Instance.getBuildingInfoBySonType(BuildingType.SEMINARY);
		} else {
	...
	public static gotoSoulMakeFrame() {
		...
		if (ArmyManager.Instance.thane.grades < OpenGrades.SOULMAKE) {
			str = LangManager.Instance.GetTranslation("dayGuide.DayGuideManager.command02", OpenGrades.SOULMAKE);
			MessageTipManager.Instance.show(str);
			return;
		}
		//TODO
	...
	public static gotoAlchemyFrame() {
		...
		if (ArmyManager.Instance.thane.grades < OpenGrades.ALCHEMY) {
			str = LangManager.Instance.GetTranslation("dayGuide.DayGuideManager.command02", OpenGrades.ALCHEMY);
			MessageTipManager.Instance.show(str);
			return;
		}
		//TODO
	...
	public static gotoFarmFrame() {
		...
		if (ArmyManager.Instance.thane.grades < OpenGrades.FARM) {
			str = LangManager.Instance.GetTranslation("dayGuide.DayGuideManager.command02", OpenGrades.FARM);
			MessageTipManager.Instance.show(str);
			return;
		}
		// if (!this.checkScene()) return;
	...
	public static gotoVehicleBuild() {
		if (SceneManager.Instance.currentType == SceneType.BATTLE_SCENE || SceneManager.Instance.currentType == SceneType.EMPTY_SCENE) {
			return;
		}
		let str: string;
		if (ArmyManager.Instance.thane.grades < 0) {
			str = LangManager.Instance.GetTranslation("dayGuide.DayGuideManager.command02", 0);
			MessageTipManager.Instance.show(str);
			return;
		}
		//TODO
	...
	public static gotoTreasureHunt() {
		...
		if (ArmyManager.Instance.thane.grades < 32) {
			str = LangManager.Instance.GetTranslation("dayGuide.DayGuideManager.command02", 32);
			MessageTipManager.Instance.show(str);
			return;
		}
		//TODO
	...
	public static gotoRunnesSkill() {
		...
		if (ArmyManager.Instance.thane.grades < OpenGrades.VEHICEL) {
			str = LangManager.Instance.GetTranslation("dayGuide.DayGuideManager.command02", OpenGrades.VEHICEL);
			MessageTipManager.Instance.show(str);
			return;
		}
		FrameCtrlManager.Instance.open(EmWindow.Skill, { tabIndex: GTabIndex.Skill_FW })
	}
	...
	public static gotoMarkingSkill() {
		...
		FrameCtrlManager.Instance.open(EmWindow.Skill, { tabIndex: GTabIndex.Skill_TF })
	}
	...
	public static gotoMounts() {
		...
		if (ArmyManager.Instance.thane.grades < OpenGrades.MOUNT) {
			str = LangManager.Instance.GetTranslation("dayGuide.DayGuideManager.command02", OpenGrades.MOUNT);
			MessageTipManager.Instance.show(str);
			return;
		}
	...
	public static returnToSpace(data: Object = null, isForce: boolean = false, showLoader: boolean = false) {
		if (this.returnScene == SceneType.SPACE_SCENE && SpaceManager.Instance.exit) {
			Logger.info("SwitchPageHelp.returnToSpace 返回天空之城失败 天空之城已销毁")
			return;
		}
	...
	public static goSpaceAndFind(target: number) {
		if (SceneManager.Instance.currentType == SceneType.SPACE_SCENE) {
			...
			let str: string = LangManager.Instance.GetTranslation("mainBar.SmallMapBar.spaceBtnNotOpen");
			MessageTipManager.Instance.show(str);
			return;
		}
		else {
	...
	public static isEnterSpaceNow(): boolean {
		...
		if (roomInfo) {
			let player = roomInfo.getPlayerByUserId(DataCommonManager.thane.userId, DataCommonManager.thane.serviceName)
	...
	public static openShop(type: number) {
		if (type == ShopGoodsInfo.SHOP) {
			...
			FrameCtrlManager.Instance.open(EmWindow.ShopWnd, { page: 3, returnToWinFrameData: 0 });
		}
			...
			FrameCtrlManager.Instance.open(EmWindow.ShopCommWnd, { shopType: ShopModel.STAR_SHOP })
		}
		else if (type == ShopGoodsInfo.FASHION_ALL_BUY) {
			MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("SwitchPageHelp.openTips1"));
		}
	...
	public static checkInBattle(showTip:boolean = true):boolean {
		...
		if (currentType && (currentType == SceneType.BATTLE_SCENE)) {
			if (showTip) {
				MessageTipManager.Instance.show(LangManager.Instance.GetTranslation("worldboss.helper.WorldBossHelper.tip12"));
			}
			return true;
		}
		return false
