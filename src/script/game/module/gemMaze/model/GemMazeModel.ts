...
export default class GemMazeModel extends GameEventDispatcher {
  ...
  constructor() {
    ...
    let cfgValue = "";
    let cfgItem =
      TempleteManager.Instance.getConfigInfoByConfigName("Gem_Maze_BuyInfo");
    if (cfgItem) {
  ...
  public setAllGemStatus(): void {
    for (let i = 0; i < this.gemMazeInfo.gemArr.length; i++) {
      this.gemMazeInfo.gemArr[i].isMove =
        this.gemMazeInfo.gemArr[i].isRemove =
        this.gemMazeInfo.gemArr[i].isAdd =
          false;
