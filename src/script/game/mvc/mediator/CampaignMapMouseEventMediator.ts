...
export class CampaignMapMouseEventMediator implements IMediator {
    ...
    private __mouseDownHandler(evt: Laya.Event) {
        ...
        if (WorldBossHelper.checkConsortiaDemon(mapId)) {
            if (evt.target instanceof CampaignNpcView) {
                ...
                if (target && target.info && target.info.info.types == PosType.TOWER_DEFENCE) {
                    return;
                }
            }
        }
    ...
    private __onClickHandler(evt: any) {
        ...
        PlayerManager.Instance.currentPlayerModel.setWorldBossAutoFight(PlayerModel.WORLDBOSS_CANCAL_AUTO_FIGHT);
        if (this.isFGUIButton(evt)) {
            return;
        }
    ...
    private isFGUIButton(evt) {
        if (evt && evt.target && fgui.GObject.cast(evt.target) instanceof fgui.GButton) {
            return true
        }
        return false
    }
