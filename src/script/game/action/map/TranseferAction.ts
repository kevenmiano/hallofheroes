...
export class TranseferAction extends MapBaseAction {
	...
	public update() {
		...
		if (this._count == 0) {
			...
			TweenMax.to(this.selfArmyView, .8, { alpha: 0, onUpdate: this.actionOver });
		}
		if (this._count == 30) {
			this.actionOver();
		}
	...
	protected actionOver() {
		...
		if (this.selfArmyView)
			TweenMax.killTweensOf(this.selfArmyView);
	}
	...
	private get selfArmyView(): any {
		...
		try {
			if (CampaignManager.Instance.mapModel.selfMemberData) {
				av = CampaignManager.Instance.controller.getArmyView(CampaignManager.Instance.mapModel.selfMemberData);
			}
		} catch (error) {
