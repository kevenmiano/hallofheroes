...
export class TaskManage extends GameEventDispatcher {
     ...
     */
    private loadTaskLog() {
        var player: PlayerInfo = PlayerManager.Instance.currentPlayerModel.playerInfo;
    ...
    private initEvent() {
        //任务更新
        ServerDataManager.listen(S2CProtocol.U_C_QUEST_UPDAT, this, this.__questUpdateHandler);
        //任务完成
        ServerDataManager.listen(S2CProtocol.U_C_QUEST_FINISH, this, this.__questFinishHandler);
    }

    private _isFirstLoad: boolean = true;
    /**获取个人任务信息*/
    private __questUpdateHandler(pkg: PackageIn) {
        var newTaskList: Array<TaskTemplate> = new Array();
        let msg: QuestUpdatedRspMsg = pkg.readBody(QuestUpdatedRspMsg) as QuestUpdatedRspMsg;
        ...
        for (var i: number = 0; i < length; i++) {
            ...
            if (!taskTemp.taskInfo) {//本地没有该任务数据
                taskTemp.taskInfo = new TaskInfo();
                this.readTaskInfo(taskTemp, questInfo);
                isNewAdd = true;
                if (!this.isAvailableTask(taskTemp)) {//登录是服务器推过来的日常任务, 
                    continue;
                }
            } else if (!this._cate.hasTask(taskTemp)) {//本地有任务数据, 但是该任务不在任务列表中, 也就是本次登录完成过的任务
                if (this.isAvailableTask(taskTemp))//该任务可以继续完成, 则加入任务列表
                {
                    this.readTaskInfo(taskTemp, questInfo);
                    isNewAdd = true;
                }
            } else {//本地有任务数据, 并且在任务列表中, 则更新任务
                this.readTaskInfo(taskTemp, questInfo);
            }
            if (isNewAdd) {
        ...
        if (isNewAdd)
            this.newTaskHandler(newTaskList);
    }

    private _newComplete: boolean;
    private __questFinishHandler(pkg: PackageIn) {
        var msg: QuestFinishedRspMsg = pkg.readBody(QuestFinishedRspMsg) as QuestFinishedRspMsg;
    ...
    private readTaskInfo(taskTemp: TaskTemplate, questInfo: QuestInfo) {
        Logger.info("[TaskManage]服务器更新任务进度", questInfo.templateId, taskTemp.TitleLang, questInfo, taskTemp);
        if (taskTemp) {
            ...
            taskTemp.taskInfo.completeDate = DateFormatter.parse(questInfo.completedDate, "YYYY-MM-DD hh:mm:ss");
            taskTemp.taskInfo.acceptDate = DateFormatter.parse(questInfo.addDate, "YYYY-MM-DD hh:mm:ss");
    ...
    private checkIsAchieved(info: TaskTemplate): boolean {
        var nowDate: number = PlayerManager.Instance.currentPlayerModel.sysCurTimeBySecond * 1000;
        var startDate: number = DateFormatter.parse(info.EndDate, "YYYY-MM-DD hh:mm:ss").getTime();
    ...
    private checkIsReceivedDate(info: TaskTemplate): boolean {
        var nowDate: number = PlayerManager.Instance.currentPlayerModel.sysCurTimeBySecond * 1000;
        var endDate: number = DateFormatter.parse(info.StartDate, "YYYY-MM-DD hh:mm:ss").getTime();
        if (nowDate < endDate) {
            return true;
        }
        return false;
    ...
    private chechNeedBuilding(info: TaskTemplate): boolean {
        ...
        var needTemp: t_s_buildingtemplateData = TempleteManager.Instance.getBuildTemplateByID(info.NeedBuildingTemp);
        if (needTemp) {
            var building: BuildInfo = BuildingManager.Instance.getBuildingInfoBySonType(needTemp.SonType);
            if (building && (needTemp.BuildingGrade > building.templeteInfo.BuildingGrade)) {
                return true;
            }
        }
        return false;
    ...
    private checkNeedGoods(info: TaskTemplate): boolean {
        ...
        var good: any[] = GoodsManager.Instance.getGoodsByGoodsTId(info.NeedItemTemp);
        if (good.length == 0) {
            return true;
        }
        return false;
    ...
    public isManualAvaliable(taskTemp: TaskTemplate): boolean {
        ...
        currentDate.setTime(PlayerManager.Instance.currentPlayerModel.sysCurTimeBySecond * 1000);
        if (taskTemp.taskInfo.repeatFinish < taskTemp.RepeatMax) return true;
        else if (!completeDate) return true;
        else if (DateFormatter.format(completeDate, "YYYY-MM-DD") != DateFormatter.format(currentDate, "YYYY-MM-DD")) return true;
        else if (parseInt((completeDate.getHours() / taskTemp.RepeatInterval).toString()) != parseInt((currentDate.getHours() / taskTemp.RepeatInterval).toString()) && taskTemp.taskInfo.repeatFinish <= taskTemp.RepeatMax) return true;
        return false;
    }
    /**
     * 是否需要新任务提示 
