...
export default class ChatHelper {
    ...
    public static checkCanSend(chatStr: string, channel: number, isvoice?: boolean): boolean {
        if (ArmyManager.Instance.thane.grades < 6 && channel != ChatChannel.WORLD) {//非世界频道
            str = LangManager.Instance.GetTranslation("chat.view.ChatInputView.command05");
            MessageTipManager.Instance.show(str);
            return false;
        }
        let worldmsgLevel: number = 20;//1.	世界频道发言需要修改发言等级要求为20级
        if (ArmyManager.Instance.thane.grades < 20 && channel == ChatChannel.WORLD) {
            str = LangManager.Instance.GetTranslation("chat.view.ChatInputView.command07", worldmsgLevel);
            MessageTipManager.Instance.show(str);
            return false;
        }
        var str: string = "";
        if (isvoice) {
            ...
            if (chatStr == "") {
                str = LangManager.Instance.GetTranslation("chat.view.ChatInputView.command01");
                MessageTipManager.Instance.show(str);
                return false;
            }
        }
        ...
        if (channel == ChatChannel.WORLD && ChatHelper.getTimer() - ChatHelper.lastSendTime3 < ChatHelper.SEND_CD_3) {
            str = LangManager.Instance.GetTranslation("chat.view.ChatInputView.command02", ChatHelper.SEND_CD_3 * 0.001);
            MessageTipManager.Instance.show(str);
            return false;
        }
        if (channel == ChatChannel.CONSORTIA && PlayerManager.Instance.currentPlayerModel.playerInfo.consortiaID == 0) {
            str = LangManager.Instance.GetTranslation("chat.view.ChatInputView.command03");
            MessageTipManager.Instance.show(str);
            return false;
        }
        if (channel == ChatChannel.TEAM) {
            if (!ChatHelper.checkTeamChatScene()) {
                str = LangManager.Instance.GetTranslation("chat.view.ChatInputView.command04");
                MessageTipManager.Instance.show(str);
                return false;
            }
        }
    ...
    private static checkTeamChatScene(): boolean {
        ...
        else if (CampaignManager.Instance.mapModel &&
            WorldBossHelper.checkIsNoTeamChatMap(CampaignManager.Instance.mapModel.mapId)) {
            canTeamChat = false;
        } else if (CampaignManager.Instance.mapModel && CampaignManager.Instance.mapModel.allBaseArmy.size <= 1) {
            canTeamChat = false;
        }
        return canTeamChat;
    }
    ...
    public static checkSceneType(): boolean {
        ...
        switch (scene) {
            case SceneType.CAMPAIGN_MAP_SCENE:
                ...
                if (model && model.campaignTemplate
