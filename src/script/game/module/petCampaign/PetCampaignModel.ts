...
export default class PetCampaignModel extends GameEventDispatcher {
  ...
  getUIPlayLevel(levelId: number) {
    let heroTemp: t_s_uiplaylevelData = ConfigMgr.Instance.getTemplateByID(
  ...
  setPlayId(id: number) {
    ...
    for (
      let i: number = 0;
      i < this.userUiPlayListMsg.uiPlayInfoList.length;
      i++
    ) {
  ...
  setTreeSelectIndex(index: number) {
    ...
    this.curLevelData = this.getUIPlayLevelListByType(this.curPlayID)[
      this.treeSelectIndex
    ];
  }
  ...
  getPlayLock(index: number): boolean {
    return (
      this.getUiPlayBaseByIndex(index).Grade > ArmyManager.Instance.thane.grades
    );
  }
