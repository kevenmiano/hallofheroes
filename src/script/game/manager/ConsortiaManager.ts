...
export class ConsortiaManager extends GameEventDispatcher {
  ...
  public setup() {
    ...
    if (this.playerInfo.consortiaID) {
      ...
      ServerDataManager.listen(
        S2CProtocol.U_C_CONSORTIA_INVITE,
        this,
        this.__consortiaInviteHandler,
      );
    }
  }
  ...
  private addEvent() {
    ServerDataManager.listen(
      S2CProtocol.U_CH_CONSORTIA_FRESHINFO,
      this,
      this.__refreshConsortiaInfo,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_CONSORTIA_INFO,
      this,
      this.__refreshConsortiaInfo,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_CONSORTIA_RENAME,
      this,
      this.__renameHandler,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_INVITEINFO_LIST,
      this,
      this.__refreshRecordList,
    );
    ServerDataManager.listen(
      S2CProtocol.U_CH_CONSORTIA_USERLOGIN,
      this,
      this.__userLogin,
    );
    ServerDataManager.listen(
      S2CProtocol.U_CH_CONSORTIA_USERLOGOUT,
      this,
      this.__userLogout,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_CONSORTIA_STATIC,
      this,
      this.__consortiaStageChange,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_CONSORTIAUSER_LIST,
      this,
      this.__updateConsortiaMember,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_DUTY_LIST,
      this,
      this.__refreshDutyInfos,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_CONSORTIA_DUTYUPDATE,
      this,
      this.__updataDutyInfo,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_CONSORTIA_INVITE_LIST,
      this,
      this.__refreshApplyList,
    );
    ...
    this.playerInfo.addEventListener(
      ...
      this.__consortiaChangeHandler,
      this,
    );

    ServerDataManager.listen(
      S2CProtocol.U_C_CONSORTIA_USERINVITE,
      this,
      this.__joinConsortiaHandler,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_CONSORTIA_USERPASS,
      this,
      this.__operateConsortiaApplyResult,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_CONSORTIA_PASS_USER,
      this,
      this.__operateConsortiaPassUser,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_CONSORTIA_LINK,
      this,
      this.__linkConsortia,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_FAM_INFO,
      this,
      this.__consortiaSecretUpdateHandler,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_FAM_TREE,
      this,
      this.__consortiaTreeUpdateHandler,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_SENDFAMLORDS_INFO,
      this,
      this.__consortiaSecretCallMonsterHandler,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_OPEN_CONSORTIA__ALTAR,
      this,
      this.__demonSwitchHandler,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_CONSORTIA_ALTAR_REPORT,
      this,
      this.__demonReportHandler,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_ALTAROVER_MSG,
      this,
      this.__demonEndHandler,
    );
    ServerDataManager.listen(
      S2CProtocol.U_CONSORTIA_ALTAR_USE_SKILL,
      this,
      this.__demonSkillUsedHandler,
    );

    ServerDataManager.listen(
      S2CProtocol.U_C_CONSORTIA_BOX_INFO,
      this,
      this.__prizeCheckHandler,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_CONSORTIA_TREASURE_BOX,
      this,
      this.__prizeAllotHandler,
    );

    this._model.addEventListener(
      ConsortiaEvent.IS_CONSORTIA_EXIST,
      this.__existHandler,
      this,
    );
    this._model.demonInfo.addEventListener(
      ConsortiaEvent.DEMON_DURA_CHANGE,
      this.__duraChangeHandler,
      this,
    );
    this._model.demonInfo.addEventListener(
      ConsortiaEvent.DEMON_WAVE_CHANGE,
      this.__waveChangeHandler,
      this,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_CONSORTIA_BOSS_SKILL,
      this,
      this.__reFreshBuffer,
    );
    ServerDataManager.listen(
      S2CProtocol.U_C_CONSORTIA_BOSS_START,
      this,
      this.__bossStateHandler,
    );

    ServerDataManager.listen(S2CProtocol.U_C_CONSORTIA_TASK, this, this.__task);
    ServerDataManager.listen(
      S2CProtocol.U_C_CONSORTIA_ALTERITEM,
      this,
      this.__refreshAltarGoods,
    );
  }

  private __refreshAltarGoods(pkg: PackageIn) {
    let msg: ConsortiaAltarInfoMsg = pkg.readBody(
      ConsortiaAltarInfoMsg,
    ) as ConsortiaAltarInfoMsg;
    if (msg) {
      if (msg.type == 1) {
        //普通
  ...
  private readConsortiaPrayInfo(msg: ConsortiaAltarInfoMsg) {
    ...
    if (msg.type == 1) {
      ...
      for (let i: number = 0; i < len; i++) {
        ...
        ConsortiaManager.Instance.model.commPrayGoodsList.set(
          consortiaPrayInfo.pos,
          consortiaPrayInfo,
        );
      }
    } else if (msg.type == 2) {
      ...
      for (let i: number = 0; i < len; i++) {
        ...
        goodsInfo.count = item.count;
        consortiaPrayInfo.goodsInfo = goodsInfo;
        ConsortiaManager.Instance.model.highPrayGoodsList.set(
          consortiaPrayInfo.pos,
          consortiaPrayInfo,
        );
      }
  ...
  private __bossStateHandler(pkg: PackageIn) {
    var msg: ConsortiaInfoMsg = pkg.readBody(
      ConsortiaInfoMsg,
    ) as ConsortiaInfoMsg;
    if (msg.bossInfo) {
      ...
      if (WorldBossHelper.checkConsortiaBoss(CampaignManager.Instance.mapId)) {
        CampaignManager.Instance.worldBossModel.totalHp =
          this._model.bossInfo.totalHp;
      }
    }
  ...
  private __prizeCheckHandler(pkg: PackageIn) {
    let msg: ConsortiaStorageInfoMsg = pkg.readBody(
      ConsortiaStorageInfoMsg,
    ) as ConsortiaStorageInfoMsg;
    ...
    for (let i: number = 0; i < infos.length; i++) {
      ...
      for (let j: number = 0; j < info.count; j++) {
        if (
          lastGoodsInfo &&
          lastGoodsInfo.templateInfo &&
          lastGoodsInfo.templateId == info.templateId &&
          lastGoodsInfo.templateInfo.MaxCount > lastGoodsInfo.count
        ) {
          lastGoodsInfo.count++;
          continue;
        }
  ...
  private __prizeAllotHandler(pkg: PackageIn) {
    let msg: ConsortiaTreasureBoxMsg = pkg.readBody(
      ConsortiaTreasureBoxMsg,
    ) as ConsortiaTreasureBoxMsg;
  ...
  private __consortiaChangeHandler() {
    if (this.playerInfo.consortiaID > 0) {
      ...
      let str: string = LangManager.Instance.GetTranslation(
        "yishi.manager.ConsortiaManager.exitConsortia",
      );
    ...
    if (flag) {
      for (const key in this._model.inviteList) {
        if (this._model.inviteList.hasOwnProperty(key)) {
          ...
          if (item.id == id) {
            let str: string = LangManager.Instance.GetTranslation(
              "yishi.manager.ConsortiaManager.command01",
              item.consortiaName,
            );
            MessageTipManager.Instance.show(str);
            // if (!this.controller.mainFrame)
  ...
  private __linkConsortia(pkg: PackageIn) {
    ...
    FrameCtrlManager.Instance.open(EmWindow.ConsortiaInfoWnd, {
      info: consortiaInfo,
    });
  }
  ...
  private __joinConsortiaHandler(pkg: PackageIn) {
    ...
    if (id) {
      ...
      let str: string = LangManager.Instance.GetTranslation(
        "yishi.manager.ConsortiaManager.command02",
      );
      MessageTipManager.Instance.show(str);
    }
  }
  ...
  private __refreshApplyList(pkg: PackageIn) {
    ...
    let totalPage: number = Math.ceil(
      msg.totalRows / ConsortiaModel.APPLY_CONSORTIA_PAGE_NUM,
    );
    ...
    for (let i: number = 0; i < msg.playerInfo.length; i++) {
      applyConsortiaList.push(
        this.readApplyInfo(msg.playerInfo[i] as SimplePlayerInfoMsg),
      );
    }

    this._model.recruitNum = msg.totalRows;
    this._model.applyConsortiaMap.set(
      this.model.recruitCurrentPage,
      applyConsortiaList,
    );
  ...
  __renameHandler(pkg: PackageIn) {
    var msg: ConsortiaRenameRspMsg = pkg.readBody(
      ConsortiaRenameRspMsg,
    ) as ConsortiaRenameRspMsg;
    if (msg.result) {
    } else {
    }
  }
  ...
  private __refreshConsortiaInfo(pkg: PackageIn) {
    let info: ConsortiaInfoMsg = pkg.readBody(
      ConsortiaInfoMsg,
    ) as ConsortiaInfoMsg;
    ...
    this._model.consortiaInfo.createDate = DateFormatter.parse(
      info.createDate,
      "YYYY-MM-DD hh:mm:ss",
    );
    this._model.consortiaInfo.speakTimes = info.speakTimes;
    this._model.demonInfo.state = info.altarState;
    this._model.demonInfo.openDate = DateFormatter.parse(
      info.altarDate,
      "YYYY-MM-DD hh:mm:ss",
    );
    ...
    if (id != this._model.consortiaInfo.chairmanID) {
      ...
      if (preId) {
        ...
        this._model.dispatchEvent(
          ConsortiaEvent.UPDA_CONSORTIA_CHAIRMAN,
          preId,
        );
      }
    }
    ...
    this._model.consortiaInfo.kickDate = DateFormatter.parse(
      info.kickDate,
      "YYYY-MM-DD hh:mm:ss",
    );
    ...
    this._model.consortiaInfo.deductDate = DateFormatter.parse(
      info.deductDate,
      "YYYY-MM-DD hh:mm:ss",
    );
    this._model.consortiaInfo.warnDate = DateFormatter.parse(
      info.warnDate,
      "YYYY-MM-DD hh:mm:ss",
    );
    this._model.consortiaInfo.codeType = info.codeType;
    this._model.consortiaInfo.codeBeginDate = DateFormatter.parse(
      info.codeBeginDate,
      "YYYY-MM-DD hh:mm:ss",
    );
    this._model.consortiaInfo.codeNeedDate = info.codeNeedDate;
    this._model.consortiaInfo.currentDate = DateFormatter.parse(
      info.currentDate,
      "YYYY-MM-DD hh:mm:ss",
    );
    this._model.consortiaInfo.isRobot = info.isRobot;
    this._model.consortiaInfo.groupPlacard = info.groupPlacard;
    this._model.consortiaInfo.consortiaSkillTypeDic =
      this.getConsortiaSkillTypeDic(info);

    //选举信息
    this._model.consortiaInfo.votingDate = DateFormatter.parse(
      info.votingDate,
      "YYYY-MM-DD hh:mm:ss",
    );
    ...
    if (info.orderInfo) {
      this._model.consortiaInfo.orderInfo = this.readOrderInfo(
        info.orderInfo as ConsortiaOrderInfoMsg,
      );
    }
  ...
  private refreshChairman(preId: number, currentId: number) {
    ...
    if (
      preId == this.playerInfo.userId ||
      currentId == this.playerInfo.userId
    ) {
  ...
  private readOrderInfo(info: ConsortiaOrderInfoMsg): ConsortiaOrderInfo {
    ...
    cInfo.lastDayDate = DateFormatter.parse(
      info.lastDayDate,
      "YYYY-MM-DD hh:mm:ss",
    );
    cInfo.lastWeekDate = DateFormatter.parse(
      info.lastWeekDate,
      "YYYY-MM-DD hh:mm:ss",
    );
    return cInfo;
  ...
  private __refreshRecordList(pkg: PackageIn) {
    let msg: ConsortiaUserInviteInfoListRspMsg = pkg.readBody(
      ConsortiaUserInviteInfoListRspMsg,
    ) as ConsortiaUserInviteInfoListRspMsg;
    ...
    for (let i: number = 0; i < msg.inviteInfo.length; i++) {
      ...
      this.readInviteInfo(
        invite,
        msg.inviteInfo[i] as ConsortiaUserInviteInfoMsg,
      );
      if (invite.fromType) {
    ...
    for (let i = 0, len = this._model.inviteList.length; i < len; i++) {
      ...
      if (item.id == this._inviteId) {
        let msg: string = LangManager.Instance.GetTranslation(
          "yishi.manager.ConsortiaManager.msg",
          item.inviteNickName,
          item.consortiaName,
        );
        PopFrameCheck.Instance.addAction(
          new ConsortiaApplyAction(this._inviteId, msg),
        );
      }
    }
    this._inviteId = 0;
  ...
  private readInviteInfo(
    invite: ConsortiaInviteInfo,
    info: ConsortiaUserInviteInfoMsg,
  ) {
    invite.id = info.id;
    invite.consortiaId = info.consortiaId;
    invite.consortiaName = info.consortiaName;
    invite.chairmanName = info.chairmanNickname;
    invite.createDate = DateFormatter.parse(
      info.createDate,
      "YYYY-MM-DD hh:mm:ss",
    );
    ...
    invite.inviteDate = DateFormatter.parse(
      info.inviteDate,
      "YYYY-MM-DD hh:mm:ss",
    );
  ...
  private __onContributeUpdata() {
    if (
      this._model.consortiaMemberList.hasOwnProperty(this.playerInfo.userId)
    ) {
      let player: ThaneInfo = this._model.consortiaMemberList[
  ...
  private __refreshDutyInfos(pkg: PackageIn) {
    ...
    for (let i: number = 0; i < msg.dutyInfo.length; i++) {
      let duteInfo: ConsortiaDutyInfo = this.readDutyInfo(
        msg.dutyInfo[i] as ConsortiaDutyInfoMsg,
      );
  ...
  public setRightsList() {
    ...
    if (item.dutyId) {
      let dInfo: ConsortiaDutyInfo = this._model.consortiaDutyList[
        item.dutyId
      ] as ConsortiaDutyInfo;
      if (!dInfo) {
        return;
      }
  ...
  private __updateConsortiaMember(pkg: PackageIn) {
    let msg: SimplePlayerListRspMsg = pkg.readBody(
      SimplePlayerListRspMsg,
    ) as SimplePlayerListRspMsg;
    ...
    if (uuid in this.membersCache) {
      let cacheMsg: SimplePlayerListRspMsg = this.membersCache[
        uuid
      ] as SimplePlayerListRspMsg;
    ...
    while (this.membersCache.length > 0) {
      let itemMsg: SimplePlayerListRspMsg = this
        .membersCache[0] as SimplePlayerListRspMsg;
  ...
  private updateConsortiaMember(msg: SimplePlayerListRspMsg) {
    ...
    for (let i: number = 0; i < msg.playerInfo.length; i++) {
      let itemMsg: SimplePlayerInfoMsg = msg.playerInfo[
        i
      ] as SimplePlayerInfoMsg;
      if (!itemMsg) {
        continue;
      }
  ...
  public addRobot() {
    if (
      this._model.consortiaMemberList.hasOwnProperty(
        this._model.consortiaInfo.chairmanID,
      )
    ) {
      return;
    }
  ...
  private updataMember(item1: ThaneInfo, item2: SimplePlayerInfoMsg) {
    ...
    if (item2 instanceof ThaneInfo) {
      ...
      item1.logOutTime = DateFormatter.parse(
        item2.logOutTime,
        "YYYY-MM-DD hh:mm:ss",
      );
    ...
    for (const key in this._model.consortiaMemberList) {
      if (
        Object.prototype.hasOwnProperty.call(
          this._model.consortiaMemberList,
          key,
        )
      ) {
  ...
  private __consortiaTreeUpdateHandler(pkg: PackageIn) {
    ...
    this._model.secretInfo.pickTime = DateFormatter.parse(
      msg.pickTime,
      "YYYY-MM-DD hh:mm:ss",
    );
    this._model.secretInfo.commitChanges();

    if (this._model.secretInfo.oper == ConsortiaSecretInfo.Call_STATE) {
      //召唤神树成功
      ...
      tipData.name = LangManager.Instance.GetTranslation(
        "tasktracetip.view.SecretTreeTipView.secret",
      );
      tipData.content = LangManager.Instance.GetTranslation(
        "tasktracetip.view.SecretTreeTipView.content",
      );
      tipData.icon = FUIHelper.getItemURL(
        "Base",
        "asset.taskTraceTips.TreeIcon",
      );
      tipData.btnTxt = LangManager.Instance.GetTranslation(
        "tasktracetip.view.SecretTreeTipView.text",
      );
      DelayActionsUtils.Instance.addAction(
        new AlertTipAction(tipData, this.showSecretTip),
      );
    }
  }
  ...
  private __consortiaSecretCallMonsterHandler(pkg: PackageIn) {
    ...
    this._model.secretInfo.callEndDate = DateFormatter.parse(
      msg.expairtDate,
      "YYYY-MM-DD hh:mm:ss",
    );
    ...
    if (isOpen) {
      ...
      tipData.title = LangManager.Instance.GetTranslation("public.prompt");
      tipData.name = LangManager.Instance.GetTranslation(
        "tasktracetip.view.SecretTreeTipView.warlordsMonster",
      );
      tipData.content = LangManager.Instance.GetTranslation(
        "tasktracetip.view.SecretTreeTipView.monsterAppearContent",
      );
      tipData.icon = FUIHelper.getItemURL(
        "Base",
        "asset.taskTraceTips.WarlordsIcon",
      );
      tipData.btnTxt = LangManager.Instance.GetTranslation(
        "tasktracetip.view.SecretTreeTipView.killNow",
      );
      DelayActionsUtils.Instance.addAction(
        new AlertTipAction(tipData, this.showSecretTip.bind(this)),
      );
    }
  }
  ...
  private __demonSwitchHandler(pkg: PackageIn) {
    let msg = pkg.readBody(
      ConsortiaAltarOpenRspMsg,
    ) as ConsortiaAltarOpenRspMsg;

    this.model.demonInfo.state = msg.result;
    switch (this.model.demonInfo.state) {
      case 1: //开启
        this.model.demonInfo.openDate = DateFormatter.parse(
          msg.altarDate,
          "YYYY-MM-DD hh:mm:ss",
        );
        let name: string =
        ...
        chatData.msg = LangManager.Instance.GetTranslation(
          "yishi.manager.ConsortiaManager.demonOpenTip",
          name,
        );
        chatData.commit();
        NotificationManager.Instance.sendNotification(
    ...
    if (msg.pop) {
      //祭坛开启右下角弹窗提示
      DelayActionsUtils.Instance.addAction(
        new AlertTipAction(null, this.showDemonOpenTip.bind(this)),
      );
    }
  ...
  private __demonReportHandler(pkg: PackageIn) {
    ...
    let participantList: SimpleDictionary =
      this.model.demonInfo.participantList;
    let wInfo: WoundInfo;
    ...
    for (let i = 0, len = msg.skill.length; i < len; i++) {
      const skillMsg: AltarCampaignSkillMsg = msg.skill[
        i
      ] as AltarCampaignSkillMsg;
  ...
  private demonEndCall(pkg: PackageIn) {
    ...
    MessageTipManager.Instance.show(
      LangManager.Instance.GetTranslation(
        "yishi.manager.BaseManager.consortiaDemonEndTip1",
        msg.param2,
      ),
    );
    let tip: string;
    if (msg.param1 <= 0) {
      tip = LangManager.Instance.GetTranslation(
        "yishi.manager.BaseManager.consortiaDemonEndTip4",
      );
    } else {
      tip =
        msg.param1 > 3
          ? LangManager.Instance.GetTranslation(
              "yishi.manager.BaseManager.consortiaDemonEndTip3",
              msg.param1,
            )
          : LangManager.Instance.GetTranslation(
              "yishi.manager.BaseManager.consortiaDemonEndTip2",
              msg.param1,
            );
    }
  ...
  private __demonSkillUsedHandler(pkg: PackageIn) {
    ...
    switch (msg.templateId) {
      case 1:
        this.addChatData(
          LangManager.Instance.GetTranslation(
            "yishi.manager.ConsortiaManager.demonSkillContent01",
            name,
          ),
        );
        MessageTipManager.Instance.show(
      ...
      case 2:
        this.addChatData(
          LangManager.Instance.GetTranslation(
            "yishi.manager.ConsortiaManager.demonSkillContent02",
            name,
          ),
        );
        MessageTipManager.Instance.show(
  ...
  private __task(pkg: PackageIn) {
    ...
    if (updateAll || updateTask) {
      ...
      if (!taskInfo.taskTemplete)
        return Logger.info("[ConsortiaManager]公会任务配置还未加载");
      ...
      taskInfo.taskContent = this._model.getTaskContent(
        taskMsg.taskId,
        taskMsg.starNum,
      );
      ...
      for (let i: number = 0; i < guildRewardArr.length; i++) {
        ...
        let rewardCfg = ConfigMgr.Instance.getTemplateByID(
          ConfigType.t_s_consortiataskreward,
          rewardId,
        ) as t_s_consortiataskrewardData;
    ...
    if (updateAll || updateRecord) {
      ...
      msg.recoredInfo = ArrayUtils.sortOn(
        msg.recoredInfo,
        ["time"],
  ...
  private __duraChangeHandler() {
    if (this._model.demonInfo.durability < 100) {
      this.addChatData(
        LangManager.Instance.GetTranslation(
          "map.campaign.view.ui.demon.ConsortiaDemonWoundView.altarDuraTip",
          this._model.demonInfo.durability +
            "/" +
            ConsortiaDemonInfo.MAX_ALTAR_DURA,
        ),
      );
    }
  }
  ...
  private __waveChangeHandler() {
    if (
      this._model.demonInfo.waveNum > 0 &&
      this._model.demonInfo.readyDownTime <= 0
    ) {
      this.addChatData(
        LangManager.Instance.GetTranslation(
          "map.campaign.view.ui.demon.ConsortiaDemonWoundView.altarWaveTip",
          this._model.demonInfo.waveNum,
        ),
      );
    }
  }
